...
We now spell out the construction used in
Lemma~\ref{lem:PVA2_proof}.  The key point is that the exchange takes place in the
...
\]
Moreover the Jacobi identity is tautological because $\{J_\lambda J\}$ is central.
\end{proposition}
...
Skew-symmetry only sketched & Use the same cylinder, now with the degree-$(-1)$ transposition sign \\
Leibniz target unstable & Use the mixed three-face Stokes projection and recover the standard PVA rule \\
Jacobi mixed two-face / three-face narratives & Use the full three-face singular Stokes argument \\
$\lambda$ vs.\ OPE variable conflated & Separate $\zeta$ and $\lambda$ and connect them by $\mathcal B$
\end{tabular}
\end{center}
