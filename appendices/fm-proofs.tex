...
\]
where $\varepsilon_S \geq 0$ is the radial parameter and the outward normal is $+\partial_{\varepsilon_S}$.
...
w_2 = \tfrac{1}{2}\varepsilon_{23}\, e^{i\theta_{23}}.
\]
...
\]
On $D_{\{2,3\}} \cong \FM_2(\C) \times \FM_2(\C)$, the factorization gives:
\begin{itemize}
...
\end{itemize}
In these coordinates, $z_1 - z_3 = \varepsilon_{13}\, e^{i\theta_{13}}$ and $w_1 = \varepsilon_{13}\, e^{i\theta_{13}}$. The propagator $1/(z_1 - z_3)$ produces a logarithmic singularity, and the residue at $\varepsilon_{13} = 0$ factorizes $D_{\{1,3\}} \cong \FM_2(\C) \times \FM_2(\C)$ into:
\begin{itemize}
...
\]
But the first contribution is $(+1) \cdot (+1) \cdot \int \gamma_{ST}$. So naively both have the same sign? This would give $2\int\gamma_{ST}$ rather than cancellation. The resolution is that the corner appears with coefficient $0$ in $\partial^2 = 0$: each codimension-2 face of a manifold with corners appears exactly twice in the iterated boundary, with opposite signs. The correct bookkeeping via Stokes on a manifold with corners gives:
...
\]
For each pair of disjoint $S, T$, the corner $D_S \cap D_T$ appears exactly twice: once as a boundary face of $D_S$ (the face where $T$ additionally collapses) and once as a boundary face of $D_T$ (the face where $S$ additionally collapses). The relation $\partial^2 = 0$ forces these two appearances to carry opposite incidence numbers, which is the content of~\eqref{eq:corner_cancellation}.
...
After integrating over the topological ($\R$) directions using the delta-function support of the propagator (Hypothesis~(H2)), the operation $m_k$ reduces to an integral over the positions of the $V = k-2$ internal vertices $z_{v_1}, \ldots, z_{v_{k-2}} \in \C$, with the external points $z_1, \ldots, z_k$ held fixed (as sources of the spectral parameters). The integration domain is a subset of $\C^{k-2}$, which has real dimension $2(k-2)$.
...
After careful analysis using the HT gauge-fixing (Hypothesis~(H1)), the effective integrand for $m_k$ on the reduced configuration space $\FM_k(\C)/(\C \rtimes \R_{>0})$ is a form of degree
\[
...
The above argument is for tree-level diagrams. At loop level, a diagram with $L$ loops has $E = V - 1 + L$ internal edges, providing $V - 1 + L$ holomorphic $1$-forms. The deficit becomes $(k-2) - (V-1+L) = (k-2) - (k-3+L) = 1 - L$. For $L \geq 1$, the deficit is $\leq 0$, so loop diagrams could in principle contribute. However, Hypothesis~(H1)(c) (one-loop finiteness) ensures that loop corrections in the cubic LG model vanish after renormalization, so $m_k = 0$ persists at all loop orders for $k \geq 4$.
\end{proof}
