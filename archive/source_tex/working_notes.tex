...
Drinfeld--Sokolov reduction.  The transferred coproduct, computed
by homological perturbation, sums over decorated trees.  Every
tree in the HPL expansion produces an output at ghost number
$\pm 1$.  The projection $p \otimes p$ onto
$\cW$-algebra cohomology kills both.  The obstruction is not
perturbative: it holds at every order, every arity, every
principal DS reduction in every simple Lie algebra.
...
$\Ainf$ tower & finite ($m_3 = 0$, class L)
...
Modularity is not an axiom on the closed algebra.  It is a
\textit{derived property} of the trace on the open sector.
...
The physical content: gravity does not produce particles.  The
gravitational coproduct is primitive because the graviton is
a composite (the stress tensor, built from the Sugawara
construction) and not a fundamental field.  The ghost-number
filtration, which is the algebraic encoding of the BV grading,
enforces this at every order.
...
Sugawara construction passes through the normal ordering, which
carries ghost number~$0$ on each input but produces an output
whose BRST homotopy $h$ necessarily shifts ghost number by
$\pm 1$.  Every tree in the HPL expansion of the transferred
coproduct has at least one internal vertex; every internal vertex
passes through~$h$.  The projection $p \otimes p$ onto
ghost-number-$0$ cohomology annihilates the result.  The
coproduct vanishes:
\[
\Delta^{\mathrm{grav}}(x) \;=\;
\tau_z(x) \otimes 1 \;+\; 1 \otimes x.
...
In gauge theory, the gluon IS the connection.  $J^a$ is
fundamental: ghost number~$0$, no passage through~$h$, no
obstruction.  The cross-term survives:
\[
\Delta_z(T_{\mathrm{aff}}) \;=\;
\tau_z(T_{\mathrm{aff}}) \otimes 1
\;+\; 1 \otimes T_{\mathrm{aff}}
...
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}lll@{}}
\textbf{Regime} & \textbf{Gauge} & \textbf{Gravity} \\
\hline
Primitive field & $J^a$ (connection) & $T$ (curvature) \\[3pt]
Origin & fundamental & Sugawara composite \\[3pt]
Coproduct & nontrivial & primitive \\[3pt]
Physical reading & particles split & only braids
\end{tabular}
\end{center}
...
all gravitational scattering.  The cubic pole: the bar kernel
$d\log(z-w)$ absorbs one power from the quartic OPE.  The
logarithmic derivative is always one degree milder than the
function itself.  This absorption is the structural content of
calling the bar construction a categorical logarithm.
...
Modularity is not an axiom on the closed algebra.  It is trace
plus clutching on the open sector.
...
\tau_z(x) \otimes 1 \;+\; 1 \otimes x
\]
for every $x \in \cW_k(\fg)$, with no corrections.
\end{theorem}

\begin{proof}
The HPL expansion of the transferred coproduct sums over
decorated planar trees.  Every tree with at least one internal
vertex carries at least one homotopy insertion $h$, shifting
ghost number away from $0$.  The projection
$p \otimes p$ onto ghost-number-$0$ cohomology annihilates
all such trees.

To see that no tree escapes: source trees (the affine coproduct
insertions) carry ghost number $+1$, and target trees (the
output legs) carry one tensor factor at ghost number $-1$.
Both are killed by $p \otimes p$.  The only surviving term is
the trivial tree (no internal vertices), which contributes the
primitive term.

The argument is independent of the specific Lie algebra, the
level, and the arity.  It depends only on the ghost number of
the BRST homotopy.
\end{proof}
...
Gravity does not produce particles.  It only braids them.

In three dimensions, there are no local gravitational degrees of
freedom and no propagating gravitons.  The bar-cobar machine
makes this algebraic: the coproduct channel is trivial, and
only the $r$-matrix channel survives.  The gravitational
$r$-matrix
\[
r^{\mathrm{Vir}}(z) \;=\; (c/2)/z^3 + 2T/z
\]
encodes all gravitational scattering.  BTZ black holes are
topological: they are monodromies of the $r$-matrix braiding,
not vertices of the coproduct.
...
entire $\Ainf$ structure is formal: $m_k = 0$ for $k \geq 3$.
...
this.  The BRST homotopy $h$ maps states at ghost number~$0$
to states at ghost number~$\pm 1$.  Every tree in the
homological perturbation lemma expansion of the transferred
coproduct passes through~$h$.  The projection $p \otimes p$
onto ghost-number-$0$ cohomology annihilates both signs.  The
transferred coproduct is
\[
\Delta^W(x) \;=\; \tau_z(x) \otimes 1 \;+\; 1 \otimes x.
\]
Primitive.  No cross-term.  No splitting.  At every arity, at
every order, for every principal DS reduction of every simple
Lie algebra.
...
Class & L & M & L & L & M \\[3pt]
$\Delta_L$ & $0$ & $\neq 0$ & $0$ & $0$ & $\neq 0$ \\[3pt]
Coproduct & nontrivial & primitive & nontrivial
  & nontrivial & primitive \\[3pt]
Why & fundamental & ghost kills & fundamental
  & DDCA, no DS & ghost kills \\[3pt]
$r_{\max}$ & $3$ & $\infty$ & $3$ & $3$ & $\infty$
\end{tabular}
\end{center}
...
\noindent
The pattern: DS reduction creates depth (L $\to$ M) and kills
the coproduct.  The Sugawara construction is the algebraic
mechanism.  The ghost sector is the memory of composition.
Gauge theories have fundamental fields and nontrivial
coproducts.  Gravitational theories have composite fields and
primitive coproducts.  The distinction is not parametric (it
is not the value of the central charge).  It is structural
(it is the ghost-number filtration of the BRST complex).
...
It knows that the gravitational coproduct is primitive: black
holes do not split.  The production vertex that would allow a
graviton to dissociate into constituents is killed by the
ghost-number projection at every arity, for every principal
...
The ghost kills the splitting.  The BRST homotopy carries
ghost number $\pm 1$, the projection annihilates it, and the
coproduct vanishes.  This is the algebraic reason that gravity
