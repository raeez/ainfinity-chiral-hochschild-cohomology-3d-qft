...
the shadow tower is infinite (complex conjugate roots); the
...
% =================================================================
\section{Shadow tower depths for $\cW_3$ and $\cW_4$: testing depth$(\cW_N) = N$}
...
\subsection*{$\cW_3$ shadow tower: explicit computation}
...
\subsection*{$\cW_4$ shadow tower: three variables}
...
the set of central charges $c$ for which the shadow tower
...
the recursive generation of the shadow tower on each primary
...
\item \emph{The shadow metric dimension grows.}
  ...
  sextic $\det Q_{\cW_3}$).
  The Galois structure of the multi-variable shadow tower encodes
...
\]
The first term generates the ``free'' shadow tower
...
acquires a cusp and $\alpha(c) \to \infty$.  The shadow tower
\emph{terminates}:
\[
...
$5c+22 = 49/2 = 7^2/2$.  Its shadow tower factorisation is:
\begin{alignat*}{2}
...
This is a \emph{$5$-adic singularity}: the shadow tower does not
converge in $\Q_5$.
...
shadow tower (as a function of~$c$) is bounded by
$|c + 22/5|_5 > 0$.

This means $\cM(2,5)$ is a \emph{$5$-adic boundary point}: the
arithmetic of the shadow tower at the prime $5$ is singular
...
of the shadow tower but visible $5$-adically.
\end{remark}
...
\item The shadow tower $S_r(c)$ for Virasoro minimal models is
...
\item The Lee--Yang model $\cM(2,5)$ is a $5$-adic singularity:
  the shadow tower diverges because $5c+22 = 0$.  No local
...
\item \textbf{(New)} New primes enter the numerator polynomial
  ...
  filtration on the shadow tower by arithmetic complexity.
...
\item The weak $p$-adic locality of $S_r(c)$ is consistent with
  the Riccati determinacy theorem: the entire shadow tower is
...
the content of AP19.  In this section we work primarily with
...
$:J^2: \sim T$ at $c = 1$.  In fact, the integer-weight
...
\item Virasoro at $c = 0$: Type~V, uncurved, with degenerate
  but nonzero higher-arity shadow tower.
...
\item The shadow tower is infinite ($r_{\max} = \infty$)
...
Part~(iv): this is the content of the shadow tower analysis
...
\item \textbf{The three regimes of uncurved algebras}:
  ...
  \item \emph{Degenerately uncurved} \textup{(}resonance regime\textup{)}:
    ...
    non-scalar shadow tower may be nontrivial.  Type~V or higher,
    $\dalg \geq 1$.
  \end{enumerate}
...
The shadow Postnikov tower of the Virasoro algebra
...
commutation relations.  Crucially, the Kac determinant
...
% =================================================================
\section{Modular periodicity of minimal model shadow towers}
...
\subsection{Verlinde ring shadow tower structure}
...
\item Shadow tower (fixed genus~$0$): converges for $c > c^* \approx
...
\item \textbf{Full Virasoro convergence.}  Scalar lane converges;
  shadow tower converges for $c > c^*$.  Full double series open.
...
\item \textbf{Borel summability of the shadow tower.}
  The scalar genus tower converges.  The shadow tower for
...
\item \textbf{The algebraic-analytic gap.}
  ...
  Moreover, Koszulness and unitarity are logically independent ---
...
shadow tower is governed by the Painlev\'e~III$_3$ equation:
...
The scalar shadow tower $\{S_r\}_{r \ge 2}$ of the Virasoro
