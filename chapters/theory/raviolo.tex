...
Let $V(\Phi) = \sum_{|\beta| \le N} c_\beta \prod_{i=1}^{r_\beta} (\partial_z^{a_i} \partial_t^{b_i} \Phi)(z_v, t_v)$ be an interaction vertex, where the sum is finite by item~(c).  The key observation is that on the product manifold $\C \times \R$, the jet bundle splits:
\[
