...
\item \emph{Vacuum axiom}: $Y(|0\rangle, z) = \mathrm{id}_V$ follows from the $A_\infty$ unit axiom $m_2(\mathbf{1},a) = a$.  The creation property $Y(a,z)|\mathbf{0}\rangle\big|_{z=0} = (-1)^{|a|}a$ follows from $m_2(a,\mathbf{1}) = (-1)^{|a|}a$ (the Koszul sign from the degree-$({-}1)$ operation $m_2$; this sign is absorbed by the standard state-field correspondence for dg vertex algebras).
