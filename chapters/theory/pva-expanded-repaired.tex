...
\section{Expanded proofs and model studies for the PVA descent}
...
chain-level lift of $T = [Q_{\mathrm{tot}}, G]$ to the all-loop quantum
object \cite{Costello2011Renorm,CG17,LurieHA}). The classical-to-quantum
bridge is the all-loop PVA--quantum HT lifting problem: the classical
$\lambda$-Jacobi limit must lift to an $E_3$-structure on
$Q$-cohomology. Every claim in this appendix concerns the classical
descent and is unconditional within the proved scope.

This appendix expands the argument of
...
\begin{remark}[D2--D6 lambda-bracket coefficients and the universal
...
chain-level shadow of the corresponding bar-complex residue at order
$n+1$ in the OPE pole expansion; via the universal-holography master
theorem (Theorem~\ref{thm:universal-holography-master}), these are the
classical limit of the BV-BRST bar differential $d_{\overline B}$ on
the boundary chiral algebra, and the Koszul conductor
$K = -c_{\mathrm{ghost}}(\mathrm{BRST})$ controls their normalisation:
the level shift entering the Sugawara stress tensor appears as the
universal correction to the D2 coefficient at the affine Kac--Moody
point. The $W(p)$ triplet (class M with logarithmic singularities) is
the single open case (Remark~\ref{rem:pva-wp-tempering-scope}):
tempering downgraded to conjectural.
\end{remark}

\subsection{The OPE variable $\zeta$ and the polynomial variable $\lambda$}

A source of confusion is the use of a single symbol for two distinct
formal parameters.
...
\begin{remark}[Two languages for the same singular data]
...
PVA axioms such as
$\{\partial a_\lambda b\}=-\lambda\{a_\lambda b\}$. The descent keeps both
...
The exchange of Lemma~\ref{lem:PVA2_proof} takes place in the
configuration space of two points in the $3$-dimensional bulk, not
inside the ordered line $\Conf_2^{<}(\R)$ by itself. The construction
is the following.
...
\begin{remark}[Regular and singular exchange signs]
The regular constant term $\mu(a,b)$ is degree $0$, so exchanging inputs
produces the usual graded-commutative sign
$(-1)^{\degree{a}\degree{b}}$.

The descended $\lambda$-bracket is written in the visible
vertex-algebra convention of Proposition~\ref{prop:PVA2_proof}:
\[
\{a_\lambda b\}
=-(-1)^{\degree{a}\degree{b}}\{b_{-\lambda-\partial}a\}.
\]
The bar desuspension has already been absorbed into the singular
operation used to define this bracket.  Shifted signs in the three-face
Jacobi calculation below belong to nested residue/corner orientation
bookkeeping; they do not replace the PVA2 skewsymmetry sign.
\end{remark}

\subsection{Local coordinates near the three pairwise divisors of $\FM_3(\C)$}

The Jacobi and Leibniz proofs use the same geometric input, merely projected
...
(\text{regular}).
\end{equation}
The crucial point is that this channel is \emph{present}; it is not a
nuisance term.  Its orientation sign is $\epsilon_{13}=-1$, and after
one moves the degree-$(-1)$ inner bracket past the external class $b$
one obtains the signed \(D_{\{1,3\}}\) contribution in the Jacobiator
normalization and the standard graded-Poisson sign in the Leibniz rule.

\subsection{How Jacobi and Leibniz are extracted}

\begin{proposition}[The singular--singular projection;
...
binary channels are singular.  In the Jacobiator normalization
\[
J_{\lambda,\mu}
=
\{a_\lambda\{b_\mu c\}\}
...
\]
the three oriented pairwise contributions become
\[
\{a_\lambda\{b_\mu c\}\},
\qquad
...
\begin{proof}
For $D_{\{1,2\}}$ the fused cluster carries the sum of the incoming
polynomial parameters, so the outer bracket is evaluated at
$\lambda+\mu$.  This raw residue is
\(\{\{a_\lambda b\}_{\lambda+\mu}c\}\), and it enters
\(J_{\lambda,\mu}\) with the minus sign fixed by the displayed
normalization.
...
shifted Koszul factor. The orientation sign $\epsilon_{13}=-1$ then
produces
\[
-\,(-1)^{(\degree{a}+1)(\degree{b}+1)}
\{b_\mu\{a_\lambda c\}\}.
...
\begin{proposition}[The mixed projection;
\ClaimStatusProvedHere; licensing $\gamma$ via the logarithmic
chain/pro ambient $\hypAmbientPro$, the oriented $\FM_3$ incidence
...
singular and the other is the regular constant term at collision
parameter $0$.  In the signed Leibniz-defect normalization
\[
...
b\cdot\{a_\lambda c\},
\]
the oriented mixed contributions are
\[
...
one now reads the outer or inner regular constant term as the unsuspended
regular product rather than as another bracket.  The pairwise faces
$D_{\{2,3\}}$, $D_{\{1,2\}}$, and $D_{\{1,3\}}$ give respectively
the three displayed summands of \(\mathcal L_\lambda\), with the two
right-hand Leibniz terms entering with the defect signs.  The
$D_{\{1,3\}}$ sign is the standard graded Poisson sign for moving the
degree-$(-1)$ operator $\{a_\lambda-\}$ across $b$, multiplied by the
defect sign.  Since the mixed sector contains exactly one singular
binary channel, the singular--singular Wick integral term is absent.
\end{proof}
...
\]
Hence the descent gives a commutative differential algebra with zero
$\lambda$-bracket.
\end{proposition}
...
\subsection{Geometric origin of each PVA axiom}

\begin{summary}[Geometric sources of the descended PVA structure]
\label{sum:pva-foundational-repair}
Each piece of the $(-1)$-shifted PVA structure on $H^\bullet(\A,Q)$
has a single geometric source:
\begin{itemize}
\item the product $\mu$ is the unsymmetrized regular constant term, and
 its graded commutativity is the exchange-cylinder homotopy in the
 regular sector (Proposition~\ref{prop:product-commutative});
\item skew-symmetry of the $\lambda$-bracket is the same cylinder in the
 singular sector, carrying the degree-$(-1)$ transposition sign
 (Proposition~\ref{prop:PVA2_proof});
\item the Leibniz rule is the mixed regular--singular three-face Stokes
 projection on $\FM_3(\C)$ (Proposition~\ref{prop:mixed-projection});
\item the Jacobi identity is the singular--singular three-face Stokes
 argument, with the non-consecutive divisor $D_{\{1,3\}}$ supplying the
 third term (Proposition~\ref{prop:ss-projection});
\item the collision variable $\zeta$ and the polynomial variable
 $\lambda$ are connected by the Borel transform $\mathcal B$
 (Definition~\ref{def:borel-transform-pva}).
\end{itemize}
The higher contraction homotopies $K_k$ for $k\ge3$ are constructed via
the affine retraction on $\R^{k-1}_{>0}$
(Lemma~\ref{lem:topological-contraction}), under the
compatible-nullhomotopy hypothesis of
