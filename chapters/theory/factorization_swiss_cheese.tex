...
$B$-cycles of $\Sigma_g$ is the curvature.

The \emph{existence} of the three models is independent of the
derived--coderived identification: each model is constructed directly
from the corresponding propagator choice.  Their \emph{equivalence} as
computational devices---i.e., the statement that all three compute the
same modular bar cohomology---follows from
Theorem~\ref{thm:three-models-gauge-orbit} and the derived--coderived
equivalence (Theorem~\ref{thm:derived-coderived-full}).  The latter
uses the Positselski framework~\cite{Positselski11}: conilpotency
of $\bar{B}^{\mathrm{ch}}(\cA)$ (by conformal weight filtration)
and completeness of the genus filtration
(Theorem~\ref{thm:modular-bar}(iii)) verify the hypotheses.
\end{remark}
