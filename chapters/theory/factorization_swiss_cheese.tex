...
the scalar bar lane this global excess is measured by the
Hodge-row anomaly
$\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$ \textup(per
\cref{thm:kappa-tuple-primitivity-orthogonal-shimura}: $\kappaChHodge$
is the unique row of $\kappaTuple{\cA}$ that classifies the
genus-$g$ curvature\textup).
...
the colimit structure. Thus the Verdier-intertwining assertion is a
well-defined global statement, not a stratum-wise assertion whose
colimit compatibility is unchecked.
\end{remark}
...
$\kappaChHodge(\cA)\cdot\omega_g^{\mathrm{Ar}}$; this scalar is not visible in the
formal-disc operad.
...
(Theorem~\ref{thm:homotopy-Koszul}) is a theorem about the
depth-zero associated graded of this local model, the coupled
statement being Conjecture~\ref{conj:coupled-sc-koszul}; its
promotion to an all-genus statement requires the
...
The modular characteristic $\kappaChHodge(\hat\fg_k)$ is computed from the
...
$\kappaChHodge(\hat\fg_0) = d/2$, the half-dimension of~$\fg$
(the pure vacuum anomaly). At $k = -h^\vee$: the Sugawara tensor
is undefined, $\kappaChHodge(\hat\fg_{-h^\vee}) = 0$ (the bar complex
...
\[
 ...
 \;=\;
 \kappaChHodge(\hat\fg_k) \cdot \omega_1^{\mathrm{Ar}}
 \;=\;
 \frac{(k+h^\vee)\,d}{2h^\vee} \cdot
 \frac{i}{2\operatorname{Im}\tau}\,
 dz \wedge d\bar z,
\]
confirming $\dfib^{\,2} = \kappaChHodge(\hat\fg_k)\cdot\omega_1^{\mathrm{Ar}}$ with the
Arakelov form $\omega_1^{\mathrm{Ar}} = \frac{i}{2\operatorname{Im}\tau}\,
...
The curvature $\kappaChHodge(\hat\fg_k)\cdot\omega_1^{\mathrm{Ar}}$ measures the
...
characteristic of $\mathrm{Vir}_c$ is $\kappaChHodge(\mathrm{Vir}_c) = c/2$
...
\begin{equation}\label{eq:virasoro-obstruction}
 ...
 \;=\;
 \frac{c}{2} \cdot \omega_1^{\mathrm{Ar}}
 \;=\;
 \frac{c}{2} \cdot
 \frac{i}{2\operatorname{Im}\tau}\, dz \wedge d\bar z,
\end{equation}
the curvature $\dfib^{\,2} = \kappaChHodge(\mathrm{Vir}_c)\cdot\omega_1^{\mathrm{Ar}}$.
...
\begin{tabular}{lcccc}
\textbf{Algebra} & \textbf{Class} & $d_{\mathrm{alg}}$ & $\kappaChHodge$ &
 \textbf{Curved at $g=1$?} \\
\hline
Heisenberg $\cH_k$ & G & $0$ & $k$ & Yes ($\kappaChHodge = k$) \\
...
\end{tabular}
\end{center}
All three are curved at genus~$1$ (for generic parameters), but their
genus-$0$ bar layers differ. Heisenberg is formal. Affine
Kac--Moody has the finite Lie--Jacobi cubic layer and no independent
wheel or contact tower. Virasoro has nonzero operations in
unbounded arity.
...
terminates at~$\kappaChHodge$. Affine Kac--Moody is
...
\[
\dfib^{\,2}=\kappaChHodge(\cA)\cdot\omega_g^{\mathrm{Ar}} .
\]
...
\item \textbf{Bar differential.} The differential
 ...
 $d_{\barB}^2 = 0$ and $\barB(F)$ is an ordinary dg coalgebra.
...
\item \textbf{Curvature at genus $g \geq 1$.} At genus~$1$, and on
 ...
 $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$ is
 the image of the annular trace under the clutching map:
 the cyclic trace on the open sector seeds the Calabi--Yau
 datum, and genus raising by clutching produces the fiber curvature
 $\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$. Its scalar moduli
 shadow lies in the Hodge lane: $\lambda_1=c_1(\det\mathbb E)$ for
 determinant curvature and $\lambda_g=c_g(\mathbb E)$ for the top
 genus-$g$ obstruction.
 ...
 = -2\pi i\, \omega_g^{\mathrm{Ar}}(z)$, where $\omega_g^{\mathrm{Ar}} = \frac{i}{2}
 ...
 fiber $\Sigma_g$ contracted with the OPE data of~$\cA$.
 ...
 \dfib^{\,2}
 \;=\;
 \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}},
 \]
 where the right side is the coderivation determined by the
scalar $\kappaChHodge(\cA)$ times the fiber Arakelov form
$\omega_g^{\mathrm{Ar}} \in H^{1,1}(\Sigma_g)$.
 ...
 $\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$ is the obstruction to
...
\item $h \cdot R + R \cdot h$: the cross-term. The residue of $h \cdot R$ at $z_1 = z_2$ extracts the OPE coefficient at the simple pole, leaving $R(z_2, z_3)$ evaluated at $z_2 = z_3$. Since $\bar\partial R = -\omega_{\mathrm{Ar}}$ (the Arakelov $(1,1)$-form), and the OPE coefficient is the structure constant weighted by $\kappaChHodge(\cA) = \mathrm{Tr}_\cA$, the double residue produces $\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$.
\end{enumerate}
The detailed computation at genus~$1$ for the Heisenberg algebra is in Section~\textup{\ref{sec:heisenberg-genus-1-complete}}; the mechanism at general genus replaces the Weierstrass $\zeta$-function by the prime-form propagator and $\omega_1^{\mathrm{Ar}}$ by~$\omega_g^{\mathrm{Ar}}$.
\end{proof}
...
\textup{(iii)} Curved geometric &
 $K^{(g)}_{\mathrm{Ar}}$ &
 $\dfib^{\,2} = \kappaChHodge \cdot \omega_g^{\mathrm{Ar}}$ &
...
curvature $\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$.
...
computational devices is narrower and precise: the two flat models
...
\item \textbf{Fixed-curve flat connection and family anomaly.}
 ...
 \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}},
 \end{equation}
 where $\omega_g^{\mathrm{Ar}} = \frac{i}{2}\sum_{\alpha,\beta}
 (\operatorname{Im}\tau)^{-1}_{\alpha\beta}\,
 \omega_\alpha \wedge \bar\omega_\beta$ is the Arakelov
 $(1,1)$-form on~$\Sigma_g$ and $\kappaChHodge(\cA)$ is the modular
 ...
 OPE (the tadpole), which computes~$\kappaChHodge(\cA)$.
...
$\kappaChHodge(\cA)$ \textup{(}Volume~I, Theorem~D at genus~$1$ and on the
...
renormalization framework \cite{Cos11}, and the difference is
substantive.
...
\begin{theorem}[BD--CG comparison and local formal-completion functor]
...
\item \textbf{Range of equivalence.} The local-shadow functor is an
 ...
 monodromy, and the scalar anomaly $\kappaChHodge(\cA)\cdot\omega_g^{\mathrm{Ar}}$.
\end{enumerate}
\end{theorem}
...
\end{enumerate}
Parts~(i) and~(ii) are standard foundational results proved in the
cited references. The new input here is~(iii): the mixed-sector
local-shadow comparison and its compatibility
...
on~$X$ and open points on~$\R$. Construct an explicit
comparison functor
\[
...
% remains an open chain-level coherence problem.
\begin{remark}[Pentagon of equivalences for $\SCchtop$]
...
Three further presentations define the open pentagon problem:
...
\]
is predicted to vanish by the coloured extension of
Calaque--Willwacher chiral formality (2015); no chain-level proof is
asserted here. The single-colour
projection of the pentagon onto the closed colour recovers
Theorem~\ref{thm:chiral-hochschild-trinity} for the three models
...
\begin{remark}[Pentagon substrate for the Universal Trace Identity]
...
obtained under the cross-volume trace-identity conjecture.
...
For the K3 bulk-boundary pair the open colour is the boundary algebra
on the elliptic curve, the closed colour is the nearby-cycle bulk over
the K3 nodal-smoothing host
...
K3. Under the closed-colour projection the Pentagon gives the Hochschild
...
$\SCchtop$ datum carrying the K3 Universal Trace Identity.
\end{example}
...
adjunction is a Quillen equivalence on the depth-zero associated
graded of the genus-$0$ formal-disc model; for the coupled model
the equivalence is conditional on
Conjecture~\ref{conj:coupled-sc-koszul}. The factorization framework isolates the all-genus extension:
the bar complex is then a \emph{curved} factorization coalgebra and
the relevant category is the coderived category.

\begin{theorem}[Factorization Swiss-cheese Koszul duality:
local recovery, categorical duality, and positive-genus mixed recovery]
...
\item \textbf{Positive-genus recovery.} At genus~$g \geq 1$,
 ...
 mixed curved factorization algebra requires a mixed
...
\item \textbf{Categorical placement.} At each genus~$g$, the
 ...
 \kappaChHodge \cdot \omega_g^{\mathrm{Ar}})$ belongs to the coderived category.
...
curved ($\dfib^{\,2} = \kappaChHodge \cdot \omega_g^{\mathrm{Ar}}$), and cobar
...
\item The anticommutativity is governed by the orientation
 lines, not by the bar-degree signs alone. On
 $\mathrm{gr}^g\,\Bmod$, both $D_0$ and $D_1$ have total
 degree $+1$ (each raises cohomological degree by~$1$ in the
 totalised complex), and the relative orientation of the
 separating and nonseparating boundary strata supplies the
 sign making them anticommute. Thus $D_0 D_1 + D_1 D_0 = 0$
...
$(\barB^{(g)}_{\mathrm{geom}}, \dfib^{\,2} = \kappaChHodge \cdot
\omega_g^{\mathrm{Ar}})$ lives in the coderived category. This is the
...
\item \textbf{Curved geometric} ($\dfib^{\,2} = \kappaChHodge \cdot
 \omega_g^{\mathrm{Ar}}$): the factorization coalgebra in \emph{single-valued
...
proved lane by $\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$.
\end{corollary}
...
has $\dfib^{\,2} = \kappaChHodge \cdot \omega_g^{\mathrm{Ar}}$ at genus~$1$, and on the
...
\begin{theorem}[Properad-level Theorem~A in the factorization ambient]\label{thm:properad-bar-cobar}
...
\begin{remark}[Range of the properad lift]
\label{rem:properad-range}
...
properads) are outside this theorem and are addressed in
...
local spectral exchange datum \((R(z),\Phi_\cA)\).  Assume in addition
a braid-descent datum on \(X^n_{\mathrm{ord}}\): a strictification of
the associator on triple overlaps, and the unitary exchange
normalization
\[
 R_{i,i+1}(z)\,R_{i+1,i}(-z)=\id
\]
on adjacent exchange collars.
...
well-defined by the supplied braid-descent datum: after the chosen
associator strictification, the braid relations
$(i\,i{+}1)(i{+}1\,i{+}2)(i\,i{+}1)
 = (i{+}1\,i{+}2)(i\,i{+}1)(i{+}1\,i{+}2)$
lift to $\Sigma_n$-relations by the strictified Yang--Baxter relation,
and the involutive relation
$(i\,i{+}1)^2 = 1$ lifts by the unitary normalization
...
\textit{Step 2 (braid coherence via the associator datum).} The symmetric-group
braid relation on three consecutive indices imposes the
identity
$(R_{12} R_{13} R_{23})(z_i, z_j, z_k)
 = (R_{23} R_{13} R_{12})(z_i, z_j, z_k)$
on the stalk $\cA^{\otimes 3}$ after the chosen associator
trivialization.  Before this strictification, Theorem~\ref{thm:YBE}
gives the associator-controlled relation with \(\Phi_\cA\); the
braid-descent datum is precisely the choice that turns it into a
local-system monodromy relation on codimension-$2$ strata.

\textit{Step 3 (involutivity).} $(i\,i{+}1)^2 = 1$ in $\Sigma_n$
forces $R(z_i - z_{i+1})\,R(z_{i+1} - z_i)\,\tau^2 = \mathrm{id}$,
equivalently $R(z)\,R_{\mathrm{op}}(-z) = \mathrm{id}$; this is not a
formal consequence of the local exchange family. It is the unitary
exchange normalization assumed in the statement.
...
The factorization Swiss-cheese framework unifies two frameworks
...
\begin{theorem}[Colour projections and factorization quantum groups]
...
$\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$ and verified it
...
\begin{definition}[Elliptic Arakelov Green kernel]
\label{def:elliptic-arakelov-green-kernel}
\index{Arakelov Green kernel!elliptic curve|textbf}
Let $\eta(\tau)$ denote the Dedekind eta function and set
\[
 \mu_\tau
 :=
 \frac{i}{2\,\operatorname{Im}\tau}\,dz\wedge d\bar z .
\]
The normalized Arakelov Green function on
$E_\tau=\C/(\Z+\tau\Z)$ is
\begin{equation}\label{eq:elliptic-arakelov-green}
 G_\tau(z)
 =
 -\log\left|
...
\end{equation}
It is normalized by
\begin{equation}\label{eq:elliptic-arakelov-ddbar}
...
\begin{equation}\label{eq:elliptic-arakelov-propagator-from-green}
 P_\tau(z)
 :=
 -\partial_zG_\tau(z)\,dz
 =
 ...
 \right)dz .
\end{equation}
Thus $P_\tau$ has residue $+1$ at $z=0$ and satisfies
$\bar\partial P_\tau=-2\pi i\,\mu_\tau$. The opposite convention
$+\partial_zG_\tau\,dz$ has residue $-1$ and reverses the edge
orientation signs in the bar differential; the formulas below use
\eqref{eq:elliptic-arakelov-propagator-from-green}.
\end{definition}
...
\begin{align}\label{eq:cross-contribution}
 ...
 \notag\\[3pt]
 &\;=\;
 \kappa \cdot \omega_1^{\mathrm{Ar}} \cdot [s^{-1}a],
\end{align}
where in the last step we used
$\omega_1^{\mathrm{Ar}} = \omega_{\mathrm{Ar}} = -\frac{\pi}{\operatorname{Im}\tau}\,
...
($\omega_1^{\mathrm{Ar}} = 2\pi i \cdot \omega_{\mathrm{Ar}}$ in the conventions
...
\end{equation}
where $\omega_1^{\mathrm{Ar}} = \frac{i}{2\operatorname{Im}\tau}\,
...
$\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$ with the Arakelov form
normalised so that $\int_{\Sigma_g}\omega_g^{\mathrm{Ar}} = 1$. For the
Heisenberg algebra $\cH_\kappa$, the modular characteristic is
$\kappaChHodge(\cH_\kappa) = \kappa$ (the level), and
$\omega_1^{\mathrm{Ar}} = \frac{i}{2\operatorname{Im}\tau}\,dz \wedge d\bar z$
satisfies $\int_{\Sigma_1}\omega_1^{\mathrm{Ar}} = 1$ (integrating over the
...
$\kappa \cdot \omega_1^{\mathrm{Ar}}$, and that this curvature arises solely
from the $\bar\partial$ of the non-holomorphic correction term in the
Arakelov propagator.
\end{remark}

\begin{theorem}[Scalar Arakelov genus tower and first tensor correction;
\ClaimStatusProvedHere]
\label{thm:scalar-arakelov-genus-tower}
\index{Arakelov genus tower!scalar uniform-weight theorem|textbf}
\index{cross-channel correction!first W3 term}
Let $\cA$ be a chirally Koszul boundary algebra whose modular curvature
is uniform-weight, so that the tensor curvature of
\cref{thm:theoremD-tensor-arakelov} reduces to the scalar trace
$\kappaChHodge(\cA)\omega_g^{\mathrm{Ar}}$. Then for every $g\ge1$ the scalar
genus coefficient is
\begin{equation}\label{eq:scalar-arakelov-genus-tower}
 F_g^{\mathrm{scal}}(\cA)
 =
 ...
 \frac{|B_{2g}|}{(2g)!}.
\end{equation}
In particular $\lambda_1^{\mathrm{FP}}=1/24$ and
$\lambda_2^{\mathrm{FP}}=7/5760$.

For a multi-weight algebra the scalar formula is only the diagonal
trace:
\begin{equation}\label{eq:multi-weight-arakelov-correction}
 F_g(\cA)
 =
 \kappaChHodge(\cA)\,\lambda_g^{\mathrm{FP}}
 +
...
\end{equation}
where $\delta F_g^{\mathrm{cross}}$ is the off-diagonal part of the
tensor-Arakelov curvature. For $\cW_3$ the first non-zero recorded
cross-channel term is
\begin{equation}\label{eq:w3-genus2-cross-channel-factorization}
...
\begin{proof}
The genus-$1$ case is the curvature computation above:
$\dfib^{\,2}=\kappaChHodge(\cA)\omega_1^{\mathrm{Ar}}$, with
$\lambda_1^{\mathrm{FP}}=\int_{\overline{\cM}_{1,1}}\lambda_1=1/24$.
For $g\ge2$, the scalar uniform-weight hypothesis says that every
stable-graph contraction lands in the one-dimensional diagonal
curvature line. The Faber--Pandharipande Hodge-integral identity gives
the displayed value of
$\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}\lambda_g$; multiplying by
the scalar curvature coefficient gives
\eqref{eq:scalar-arakelov-genus-tower}. If the weight bundle has more
than one channel, the curvature is the matrix
$K(\cA)\in\operatorname{Sym}^2(\cF^\vee)\otimes
\Omega^2(\overline{\cM}_{g,n})$ of
\cref{thm:theoremD-tensor-arakelov}. Its diagonal trace is the scalar
term and its off-diagonal entries are exactly
$\delta F_g^{\mathrm{cross}}(\cA)$. The $\cW_3$ value
\eqref{eq:w3-genus2-cross-channel-factorization} is the local
stable-graph computation recorded in
\cref{comp:platonic-W3} and checked by the genus-$2$ cross-channel
engine.
\end{proof}
...
$\Sigma_g$ with modular characteristic $\kappaChHodge(\cA)$.
...
\item \textbf{Corrected holomorphic model.}
 ...
 This model satisfies $\Dg{g}^{\,2} = 0$ (it is an ordinary
...
\item \textbf{Curved Arakelov model.}
 ...
 $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$ (it is a curved dg
 coalgebra).
\end{enumerate}
...
$\dfib^{\,2}=\kappaChHodge(\cA)\,\omega_g^{\mathrm{Ar}}\ne0$; strict intertwining holds only
...
When $\dfib^{\,2}=\kappaChHodge(\cA)\omega_g^{\mathrm{Ar}}\ne0$, the comparison
...
\[
 ...
 =
 \kappaChHodge(\cA)\omega_g^{\mathrm{Ar}} .
\]
...
uncurved locus $\kappaChHodge(\cA)\omega_g^{\mathrm{Ar}}=0$.
...
is an ordinary dg coalgebra ($\Dg{g}^{\,2} = 0$) and lives in the
derived category $\mathsf{D}(\barB\text{-}\mathsf{comod})$. On the
right (Arakelov model), $(\barB^{(g)}_{\mathrm{Ar}}, \dfib)$ is a
curved dg coalgebra ($\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$) and
...
The genus-$g$ bar complex with $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}
...
\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}})$ be the curved bar complex at genus~$g$
...
\item The curved bar complex $\barB^{(g)}(\cA)$ is a curved dg
 ...
 $\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$, and the structural relation is
 $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}} \cdot \id$.
 \end{itemize}
...
degree~$+1$ and satisfies $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$
...
curvature datum $\kappaChHodge(\cA) \cdot \omega_g^{\mathrm{Ar}}$. It is the algebra that
...
$\dfib^{\,2} = \kappa \cdot \omega_1^{\mathrm{Ar}}$ from the annular trace
...
curvature $\kappa \cdot \omega_1^{\mathrm{Ar}}$ deforms the commutative product:
the curved differential $d$ satisfies $d^2 = \kappa$
(writing $\kappa$ for $\kappa \cdot \omega_1^{\mathrm{Ar}}$ with the $(1,1)$-form
...
$\kappa \cdot \omega_1^{\mathrm{Ar}}$ (the failure of $d^2 = 0$, the obstruction
...
at non-critical level and HT/BV realisation datum $\Xi$, the realised
3d HT gauge theory $T_{\cA,\Xi}$ on $X \times \R$ has boundary
observables
$\Obs^\partial(T_{\cA,\Xi}) \simeq \cA$ as $\Eone$-chiral algebras and
bulk observables
$\Obs^{\mathrm{bulk}}(T_{\cA,\Xi}) \simeq
\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ in the declared comparison
ambient. The closed colour becomes an $E_3$-topological factorization
algebra after topologization. The last clause is a
post-topologization, single-colour statement: Dunn additivity enters
only after the closed holomorphic colour has become locally constant
on $Q_{\mathrm{tot}}$-cohomology, and it is not an identification of
the directed two-coloured operad $\SCchtop$ with $E_3$. The
class coverage is:
\begin{enumerate}[label=(\roman*)]
...
limit rung. The universal-holography master theorem covers the
non-logarithmic standard landscape; logarithmic
$\cW(p)$ tempering remains open pending the Adamovi\'c--Milas
...
The perturbative Swiss-cheese anomaly line at $K3 \times T^2$ has a
scalar Deligne characteristic in
$H^2(\overline{\mathcal{A}_2}, \mathcal{O}^*)$. After the Pfaffian
orientation, Hall--Borcherds comparison, and K3 elliptic-genus
normalisation are fixed, the primitive Borcherds trivialising section is
the BKM denominator $\Delta_5$ with
\[
...
\[
 \{\kappaCat,\kappaChHeis,
 \kappaBKM,\kappaFiber\}
 =\{0,3,5,24\},
\]
with $\kappaChHodge=0$ the remaining tuple row. The number
$\chi(\mathcal O_{K3})=2$ is a fibre witness; the equality
$5=2+3$ is only the $N=1$ K3 mnemonic, not a
$\kappaBKM$ additivity law.
\end{remark}
...
\begin{remark}[Compactness separates the two factorisation geometries]
...
\begin{remark}[Koszul-dual vertex-operator closure on K3]
...
\begin{remark}[Factorisation-Swiss-cheese on $\mathrm{Hilb}^n(\mathrm K3)$]
...
\begin{remark}[Spectral parameters via $\Omega$]
...
acts on the reduced compact Hall windows of \(D^b(K3 \times E)\) over
\(\Ran(E^{\mathrm{nod,sm}}_{24})\) (Davison 2017, arXiv:1512.04179;
Joyce--Song 2012 for the potential).  Passing from these windows to the
\(K3\times E\) finite radical-quotient Hall--Drinfeld doubles is the
Vol~III compact-window construction; recognising their completed limit
as the \(\Delta_5\) Borcherds current object requires the finite
pairing, PBW/no-extra, parity, centre, associator, and transition
defects to vanish. The scalar
convention is fixed before the character is read:
\[
...
formula. The
open-colour (topological) sector can carry the complementary
Katz--Klemm--Vafa BPS content only after the Hall--Borcherds
reconstruction data are supplied (KKV 1999, hep-th/9910181;
Oberdieck--Pandharipande 2016, arXiv:1607.05220): the scalar
enumerative coefficients alone do not produce
$\mathsf{SC}^{\mathrm{ch,top}}$ operations. The
...
BPS walls within the module category. The condition of
...
\S13) is the equivariant lift. Compare Vol~III
