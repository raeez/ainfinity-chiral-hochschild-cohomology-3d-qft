...
\omega_g$ is the measure of this excess --- it is global information
...
This chapter constructs the Swiss-cheese structure directly from
factorization data and derives the local operadic model as a
...
\end{equation}
on the disjointness locus, satisfying the obvious associativity and
commutativity constraints for iterated partitions
...
composition.  The key distinction: operadic composition composes
...
\begin{itemize}
\item $\mu_2(s^{-1}J \otimes s^{-1}J) = 0$ --- the Heisenberg OPE
...
%%----------------------------------------------------------------------
\subsection{Affine Kac--Moody $\hat\fg_k$}
...
is undefined, the modular characteristic diverges, and the
construction requires the Feigin--Frenkel centre (the critical
level is a different story, as cautioned in the Critical Pitfalls).
...
\smallskip\noindent
\textbf{Shadow depth.}
The \emph{algebraic shadow depth} of $\mathrm{Vir}_c$ is
$d_{\mathrm{alg}} = 1$: the first non-vanishing higher operation
is $m_3$, and all $m_k$ for $k \geq 4$ are determined by the
$A_\infty$ relations from $m_2$ and $m_3$.  The shadow depth
measures how far the chiral algebra is from being formal
(a formal algebra has $d_{\mathrm{alg}} = 0$: all $m_k = 0$
for $k \geq 3$).  The Virasoro algebra is the simplest
non-formal chiral algebra, with shadow depth exactly~$1$.
...
\begin{tabular}{lccc}
\textbf{Algebra} & $d_{\mathrm{alg}}$ & $\kappa$ &
  \textbf{Curved at $g=1$?} \\
\hline
Heisenberg & $0$ & $1$ & Yes ($\kappa = 1$) \\
$\hat\fg_k$ & $0$ & $(k+h^\vee)d/(2h^\vee)$ &
  Yes ($\kappa \neq 0$ for $k \neq -h^\vee$) \\
$\mathrm{Vir}_c$ & $1$ & $c/2$ & Yes ($c \neq 0$),
  plus $m_3 \neq 0$
\end{tabular}
\end{center}
...
at genus~$0$.  The Heisenberg and affine Kac--Moody algebras are
formal ($d_{\mathrm{alg}} = 0$): their bar complexes have
$d_{\barB}^2 = 0$ at genus~$0$.  The non-formality of the Virasoro
(its shadow depth $d_{\mathrm{alg}} = 1$) is the algebraic expression
of the fact that the Virasoro OPE contains poles of order $> 1$
beyond simple poles.  This is \emph{not} a defect but a feature:
the non-vanishing of $m_3$ IS the curved bar structure
$\dfib^{\,2} = \kappa\cdot\omega_1$ from Volume~I (Critical Pitfall:
formality fails at $d' = 1$ precisely because the logarithm acquires
monodromy).
\end{example}
...
this to the two-colored Swiss-cheese setting on the product
$\Sigma_g \times \R$.
...
\item \textbf{Curvature at genus $g \geq 1$.}  At genus~$g \geq 1$,
  ...
  $\omega_g \in H^0(\Mbar_g, \omega_{\Mbar_g})$ is the Hodge
  class and $\kappa(\cA) \in k$ is the scalar curvature of~$\cA$
...
The fiberwise differential $\dfib$ is a coderivation of the bar coalgebra, so $\dfib^{\,2}$ is determined by its cogenerator projection --- the projection to the arity-$1$ component $s^{-1}\cA$.  This cogenerator extracts a double residue:
\[
\mathrm{copr}(\dfib^{\,2})
...
the self-contraction (tadpole) survives --- this extracts
...
and differential-geometric (prefactorization algebras) --- produce
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item Beilinson--Drinfeld \cite{BD04} and Francis--Gaitsgory \cite{FG12} for the chiral/closed equivalence --- foundational results in factorization algebra theory.
\item Lurie \cite{HA}, Theorem~5.4.5.9, for the associative/open equivalence --- a standard result identifying locally constant factorization algebras on~$\R$ with $\Eone$-algebras.
...
The key verification: for disjoint $I_1, I_2$, the diagram
\[
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Factorization Koszul duality and derived-coderived equivalence}
...
\item \textbf{Derived-coderived equivalence.}  At each genus~$g$,
  there is an equivalence between the derived category of
  the genus-$g$ associated graded bar complex
  $(\mathrm{gr}^g\, \Bmod, D_0^2 = 0)$ and the coderived
  category of the curved geometric bar complex
  $(\barB^{(g)}_{\mathrm{geom}}, \dfib^{\,2} =
  \kappa \cdot \omega_g)$.  The passage between derived and
  coderived categories is mediated by the Positselski
  coderived equivalence for curved dg coalgebras, whose
  applicability follows from the homotopy-Koszulity of the
  local model.
\end{enumerate}
\end{theorem}
...
\omega_g)$ lives in the coderived category.  The Positselski
framework for curved dg coalgebras \cite{Pos11} establishes an
equivalence between the coderived category of a curved
coalgebra and the derived category of its associated graded,
provided the filtration is complete and exhaustive.  The
completeness of the genus filtration
(Theorem~\ref{thm:modular-bar}(iii)) and the
homotopy-Koszulity of the local model
(Theorem~\ref{thm:homotopy-Koszul}) together verify the
hypotheses of Positselski's theorem.
\end{proof}
...
equivalence between models~(i)/(ii) and model~(iii) is
Theorem~\ref{thm:factorization-SC-koszul}(iv).
\end{proof}
...
(Remark~\ref{rem:convolution-as-correspondence}) lifts naturally to
...
\end{tabular}
\end{center}
The bar complex presents the Swiss-cheese algebra in the same sense
that the Steinberg variety presents the Hecke algebra: both are
convolution algebras on self-intersection varieties of resolution
correspondences.
...
The Steinberg--Hecke parallel gains additional content in the
...
Thus: the bar complex presents the Swiss-cheese algebra, as the
...
The factorization Swiss-cheese framework of this chapter provides a
natural meeting point for two parallel programmes that have
developed largely independently: the factorization quantum groups
of Latyntsev \cite{Latyntsev23} and the dg-shifted Yangians of
Chapter~\ref{chap:dg-shifted-factorization}.  We now upgrade the
...
\item \textbf{Open-colour projection.}  The forgetful functor
  ...
  $\Eone$-algebra.  For $F = F_{\cA}$, this recovers the
...
monodromy of the KZ connection at level~$k$.
...
(proved in Chapter~\ref{chap:spectral-braiding}), the Koszul
dual $\cA^!$ of the affine vertex algebra $V_k(\fg)$ in the
$\Eone$-direction is the dg-shifted Yangian $Y_\hbar(\fg)$.
...
the monodromy of the KZ connection (computed by Latyntsev's
factorization homology) agrees with the quantum group $R$-matrix
(the Drinfeld--Kohno theorem), and our $R(z)$ recovers both
...
The key mechanism is that $\dfib^{\,2}$ does not arise from a
...
$\Phi_g \circ \Dg{g} = \dfib \circ \Phi_g$ then follows from the
fact that the structure maps (OPE residues) are unchanged by the
...
derived-coderived equivalence of
Theorem~\ref{thm:factorization-SC-koszul}(iv), made explicit
through the gauge transformation $\Phi_g$.
\end{proof}
...
category of such an object is trivial: every module is acyclic
because $d^2 \neq 0$ and hence every short exact sequence splits.
...
our setting, producing the curved Koszul duality at genus~$g$.
...
\item \textbf{Triviality of the ordinary derived category.}
If $\kappa_C \neq 0$, the ordinary derived category
$\mathsf{D}(C\text{-}\mathsf{comod}) = 0$.  Indeed, for any
comodule~$M$, the identity $d^2 = \kappa_C$ implies
$\id_M = \kappa_C^{-1} \cdot d^2$ (when $\kappa_C$ is a non-zero
scalar), so $\id_M$ is null-homotopic and $M \simeq 0$ in the
ordinary derived category.  The coderived category avoids this
collapse by using a weaker equivalence relation.
\end{enumerate}
\end{construction}
...
dual $\cA^!$ is an algebra (the cobar dual), and its
...
\begin{theorem}[Curved Koszul duality at genus~$g$]
...
\item The curved bar complex $\barB^{(g)}(\cA)$ is a curved dg
  ...
  \item The curvature element is
    $m_0 = \kappa \cdot \omega_g \in k \otimes
    H^{1,1}(\Sigma_g)$, satisfying $\dfib(m_0) = 0$.
  \end{itemize}
  The curved Maurer--Cartan equation
  \begin{equation}\label{eq:curved-MC}
    \dfib(\xi) + \xi \cdot \xi = -m_0
  \end{equation}
  governs deformations of the bar complex over $\Sigma_g$: a
  solution $\xi \in \barB^{(g),1}(\cA)$ is a curved
  $A_\infty$-morphism from $\cA$ to itself, twisted by the
  curvature.

\item \textbf{Cobar-bar adjunction in the coderived setting.}
  The cobar functor $\Omegach$ extends to curved dg coalgebras:
  applied to $\barB^{(g)}(\cA)$, it produces the curved dg
  algebra $\Omegach(\barB^{(g)}(\cA))$ with differential
  $d_\Omega$ satisfying
  $d_\Omega^2 = \kappa \cdot \omega_g$ (the curvature is
  inherited).  The bar-cobar adjunction descends to an
  equivalence between:
  \begin{equation}\label{eq:curved-koszul-equivalence}
    \Dco\bigl(\barB^{(g)}(\cA)\text{-}\mathsf{comod}\bigr)
    \;\simeq\;
    \Dctr\bigl(\cA^!\text{-}\mathsf{contra}\bigr),
  \end{equation}
  where $\cA^! = (\cA^!)_\kappa$ is the Koszul dual algebra
  twisted by the curvature $\kappa \cdot \omega_g$.  This is the
  \emph{curved Koszul duality} at genus~$g$.

\item The equivalence \eqref{eq:curved-koszul-equivalence} is
  compatible with the genus tower: the inclusion
  $\barB^{(g)} \hookrightarrow \barB^{(g+1)}$ via the
  non-separating clutching map
  \textup{(}Construction~\textup{\ref{constr:nonsep-clutching})}
  induces a restriction functor
  $\Dco(\barB^{(g+1)}\text{-}\mathsf{comod}) \to
  \Dco(\barB^{(g)}\text{-}\mathsf{comod})$
  that is intertwined by the Koszul equivalence with the
  corresponding restriction on the contraderived side.
\end{enumerate}
\end{theorem}

\begin{proof}
\textbf{Part (i)} is a verification of Positselski's axioms.  The
...
Construction~\ref{constr:bar-fact-coalgebra}(iv)).  The cocycle
condition $\dfib(m_0) = 0$ holds because $m_0 = \kappa \cdot \omega_g$
is a scalar times a closed form: $\dfib(\kappa \cdot \omega_g) =
\kappa \cdot \dfib(\omega_g) = 0$, since $\omega_g$ is the
class of the Arakelov form in cohomology and $\dfib$ acts on
cohomology classes by the commutator with the curvature,
$[\kappa \cdot \omega_g, \omega_g] = 0$ (scalars commute).
...
characteristic~$0$, there is an equivalence
\[
  \Dco(C\text{-}\mathsf{comod})
  \;\simeq\;
...
\smallskip\noindent
\emph{Ingredient~2: Identification of the dual.}
...
algebra is the completed cobar construction
$\widehat\Omegach(\barB^{(g)}(\cA))$, which is quasi-isomorphic
to the Koszul dual algebra $\cA^!$ (by the homotopy-Koszulity of
$\SCchtop$, Theorem~\ref{thm:homotopy-Koszul}, which ensures
that the cobar resolution is a cofibrant replacement).  The
curvature $\kappa \cdot \omega_g$ on the coalgebra side dualizes to
the curvature $\kappa \cdot \omega_g$ on the algebra side:
$(\cA^!)_\kappa$ is the Koszul dual algebra with the curved
differential $d_{A^!}^2 = \kappa \cdot \omega_g$.
...
\[
  ...
  \;\simeq\;
  \Dctr\bigl((\cA^!)_\kappa\text{-}\mathsf{contra}\bigr),
\]
where the first equivalence is Positselski's theorem and the
second is the Koszul identification of the dual.

\smallskip\noindent
\textbf{Part (iii).}  The compatibility with the genus tower
follows from the functoriality of Positselski's equivalence.
The non-separating clutching map
$\Delta_{\mathrm{cyc}}\colon \barB^{(g)} \to \barB^{(g+1)}$
is a morphism of curved dg coalgebras (it preserves the
coderivation and increases the curvature:
$\kappa \cdot \omega_g \mapsto \kappa \cdot \omega_{g+1}$).
Positselski's equivalence is natural with respect to morphisms
of curved dg coalgebras, hence the restriction functor on
comodules is intertwined with the restriction on contramodules.
\end{proof}
...
algebra on one generator --- the Koszul dual of the free Lie
coalgebra on one cogenerator).

At genus~$1$, the bar complex acquires curvature
$\dfib^{\,2} = \kappa \cdot \omega_1$
(Computation~\ref{comp:dfib-sq-heisenberg-genus1}).  The curved Koszul
dual $(\cH_\kappa^!)_\kappa$ is the algebra $k[x]$ with the curved
differential $d^2 = \kappa \cdot \omega_1$.  This curved algebra
...
\begin{equation}\label{eq:curved-koszul-heisenberg}
  \Dctr\bigl((\cH_\kappa^!)_\kappa\text{-}\mathsf{contra}\bigr)
  \;\simeq\;
  \mathsf{D}(\cW_\kappa\text{-}\mathsf{mod}),
\end{equation}
the derived category of modules over the Weyl algebra.  The
equivalence uses the fact that the contraderived category of a
