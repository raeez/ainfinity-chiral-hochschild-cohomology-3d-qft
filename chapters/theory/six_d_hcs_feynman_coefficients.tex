...
\begin{theorem}[One-loop scalar graph weight; \ClaimStatusProvedHere; licensing tags $\alpha+\beta+\gamma$]
...
\begin{theorem}[Two-loop scalar graph weight; \ClaimStatusProvedHere; licensing tags $\alpha+\beta+\gamma$]
...
\begin{theorem}[Third-obstruction scalar graph weight; \ClaimStatusProvedHere; licensing tags $\alpha+\beta+\gamma$]
\label{thm:6d-hcs-three-loop-coefficient}
At the third BV obstruction order two graph-complex classes contribute:
the primitive weight-five Kontsevich wheel and the decomposable square
of the weight-three sunrise class. Their
Feynman weights are
\begin{align}
...
\begin{proof}
The primitive summand is the Kontsevich five-spoke wheel \(W_{5}\).
The subscript in \(w_3^{\mathrm{wh}}\) records the BV obstruction
order, not the first Betti number of \(W_{5}\).
Iterated integration over the corresponding Fulton--MacPherson cell
uses the Arnold--Orlik--Solomon presentation of
\(H^\bullet_\DR(\Conf_{V(W_5)}(\C))\).
The result is the Brown reduction
(Brown~2012, \S5):
\[
\int_{\FM(W_5)}\bigwedge_{e\in E(W_5)} \eta_e
= \zeta(5)
\]
because \(W_{5}\) sits at depth one and motivic weight~$5$ in
$H^0(\GC_2)$ (Kontsevich~1999, \S3; Willwacher~
2015, Thm~1.1). The prefactor $(2\pi i)^{-5}$ comes from five
net propagator contractions.

For the decomposable graph class, the two components are copies of the
sunrise class of Theorem~\ref{thm:6d-hcs-two-loop-coefficient}.
...
by the primitive wheel and by its decomposable products. This
...
\begin{corollary}[Scalar BV finiteness at three loops with 24 M5-branes; \ClaimStatusProvedHere{} for scalar recursion, \ClaimStatusConditional{} for BKM lift; licensing tags $\alpha+\beta+\gamma+\epsilon$]
...
\begin{proposition}[CFG-to-6d-hCS coefficient comparison; \ClaimStatusConditional; licensing tags $\beta+\gamma$]
...
Kontsevich, and Drinfeld; \ClaimStatusProvedHere{} at scalar level,
\ClaimStatusConditional{} for BKM lift; licensing tags $\beta+\gamma$]
...
\approx 0.04166667, \\
w_2 &= \zeta(3)/(2\pi i)^3 \approx i\cdot 4.8460\times 10^{-3}, \\
w_3^{\mathrm{wh}} &= \zeta(5)/(2\pi i)^5 \approx -i\cdot 1.0589\times 10^{-4}, \\
...
(equivalently the $p^{n}$-coefficient of
$-\log(\Delta_5/(\eta(q)^{12}\eta(p)^{12}))$; $\phi_{-2,1}$ itself is holomorphic
...
referenced against the canonical \(\phi_{0,1}^{K3}\) table at
non-negative discriminant (Eichler--Zagier~1985, Ch.~9), supply the separate finite
Igusa scalar targets; they do not by themselves identify BV graph
classes.

\section{All-loop BV anomaly conjecture}
\label{sec:general-n-loop}

\begin{conjecture}[All-loop 6d hCS BV anomaly; \ClaimStatusConjectured; licensing tags $\beta+\gamma+\epsilon$]
...
$W(\hbar) = \sum_{n\geq 0} w_n \hbar^n$ is conjectured to compare with the
...
No all-loop theorem is claimed here. The proved surface is the
$n\leq 3$ scalar graph-complex calculation above, together with the
conditional finite-source/BKM lift through the named Hall/BV source,
hCS chain map, parity/root fixtures, denominator comparison, and graph
orientation data.
