...
$\Conf_k^<(\R)$. Holomorphic factorization on $\C$ and $E_1$ on $\R$
each carry their own local-to-global principle. The HT coupling is
asymmetric: a bulk OPE pole at $z_1 = z_2$ occurs at fixed
boundary ordering; a rearrangement of the ordering on $\R$ runs at
generic holomorphic positions.

The two-coloured Swiss-cheese operad $\SCchtop$ is the operad in
which the coupling lives. Its closed colour, the chiral operad
$\cP^{\mathrm{ch}}$ built from logarithmic forms on the
Fulton--MacPherson compactifications $\FM_k(\C)$, governs
holomorphic collisions. Its open colour $\Eone$, the
little-intervals operad, governs topological ordering. The
Swiss-cheese composition constraint -- no open-to-closed map --
realises bulk-to-boundary directionality. An $\Ainf$~chiral algebra
is an algebra over $C_\ast(W(\SCchtop))$; the universal example is
the boundary algebra $A_b = \End_\cC(b)$ of the open-sector
...
space $\Conf_n(\C)$ records positions but is noncompact, so its
top-degree forms do not integrate. The Fulton--MacPherson
compactification $\FM_n(\C)$ adds boundary strata encoding the
relative rates of approach, making the logarithmic-residue calculus
that extracts OPE coefficients well-defined.
...
The space $\Conf_n(\C)$ is a smooth noncompact manifold of real
dimension $2n$. Integration of chiral OPE residues forces a
compactification recording relative collision rates. The
Fulton--MacPherson compactification \cite{FM94} is the canonical
choice.
...
forms carrying this data are the logarithmic forms in the
...
Laurent series in spectral parameters recording derivatives of the
$\lambda$-bracket. Operadic composition is boundary restriction
...
\textbf{Completion:} The pole-order filtration completes the operad: forms with poles of order $\leq N$ at each divisor form a filtered subsystem.
\end{construction}
...
The chiral operad $\cP^{\mathrm{ch}}$ encodes the operadic
structure underlying vertex algebras together with the configuration-space
geometry. The completion and spectral parameters implement the formal
calculus of $\lambda$-brackets.
\end{remark}
...
\S\ref{sec:chiral_operad} governs holomorphic collisions alone.
Coupling it to an ordered topological direction requires specifying
the target dg-category in which operads act, and the
topological-to-algebraic functor converting spaces of configurations
into chain-level operations. Both are standard; fix them once for
the remainder of the chapter.

The ambient category is a presentable, stable, symmetric monoidal
dg-category $(\mathcal C,\otimes,\mathbf 1)$ (e.g.\ $\mathrm{Ch}_k$
for a field $k$ of characteristic zero), with $\otimes$ exact and
preserving colimits in each variable. Topological operads $O$ pass
to dg-operads $C_\ast(O)$ via singular chains (or de Rham forms when
convenient); the two are silently identified.

\begin{notation}
\label{not:fm-e1}
Let $\FM_k(\C)$ denote the Fulton--MacPherson compactification of $\Conf_k(\C)$; let $E_1(m)$ be the little-intervals operad. Set $C_\ast(\FM_k(\C))$ and $C_\ast(E_1(m))$ for their chains.
\end{notation}

\subsection{The Two-Coloured Operad $\mathsf{SC}^{\mathrm{ch,top}}$}
\label{subsec:sc-def}

$\SCchtop$ is a two-coloured \emph{symmetric} operad with a
directional colour-restriction. An HT
observable lives on a rectangle $D \times I \subset \C \times \R$,
holomorphic in the $\C$-factor and locally constant in the
$\R$-factor; the physical coupling runs one way, with collision data
flowing from the holomorphic bulk ($\mathsf{ch}$) into the
topological boundary ($\mathsf{top}$). Coloured-operad equivariance
permutes inputs together with their colour signature, so the
two-colour structure satisfies the standard symmetric coloured-operad
axioms; the directional restriction is the emptiness of every
component with closed output and an open input, and this emptiness
is stable under composition. Dunn additivity, an operadic
additivity statement for uncoloured little-disk operads,
does \emph{not} apply to $\SCchtop$; the tensor-product formulas
$E_p \otimes E_q \simeq E_{p+q}$ are statements about
single-coloured operads and are unavailable for the coupled
two-coloured operad.

The canonical model is the coupled half-plane model of
Definition~\ref{def:SC-operations}: mixed operations are the
Voronov compactifications $\FM_{k,m}(\mathbb H)$ of configurations
of $k$ interior and $m$ boundary points, with mixed-into-open
boundary insertion carrying the transverse position. The decoupled
componentwise model built from $\FM_k(\C)$ and $E_1(m)$ is the
depth-zero associated graded of the boundary stratification
(Remark~\ref{rem:product-forced-by-locality}); it is the model in
which the componentwise computations below are performed, and every
statement proved with it is a gr-level statement.

\begin{definition}[Colours and spaces of operations: coupled model
and its depth-zero associated graded; \ClaimStatusDefinitional]
\label{def:SC}
Let $\mathbf{Col}=\{\mathsf{ch},\mathsf{top}\}$. The
\emph{holomorphic--topological Swiss-cheese coloured operad}
$\mathsf{SC}^{\mathrm{ch,top}}$ is the coupled half-plane operad of
...
\mathsf{SC}^{\mathrm{ch,top}}\bigl((\underbrace{\mathsf{ch},\dots,\mathsf{ch}}_{k},\underbrace{\mathsf{top},\dots,\mathsf{top}}_{m});\mathsf{top}\bigr) &:= \FM_{k,m}(\mathbb H),\\
\mathsf{SC}^{\mathrm{ch,top}}(\ldots;\mathsf{ch}) &= \varnothing \ \text{ if any input is $\mathsf{top}$},
\end{align*}
with the full coloured composition of
Definition~\ref{def:SC-composition}: closed-into-closed FM
insertion, closed-into-open insertion at interior points, and
mixed-into-open insertion at boundary points. Its depth-zero
associated graded $\gr^0\SCchtop$ has mixed components
$\FM_k(\C)\times E_1(m)$ with componentwise composition (inserted
open-output operations contribute their $E_1$ factor; insertions
whose interior points would cross the boundary stratum land in
positive depth and vanish at $\gr^0$). Units are the degree--$1$
points in $\FM_1(\C)$ and $\FM_{0,1}(\mathbb H)$.
\end{definition}

\begin{proposition}[\ClaimStatusProvedHere]
\label{prop:SC-operad}
$\mathsf{SC}^{\mathrm{ch,top}}$ is a well-defined strict
two-coloured symmetric topological operad
(Proposition~\ref{prop:SC-well-defined}, Voronov's half-plane
Swiss-cheese model), and $\gr^0\SCchtop$ is a well-defined strict
two-coloured symmetric topological operad with componentwise
composition. In both, associativity, unitality, and coloured
symmetric-group actions hold strictly, and the directional colour
restriction is stable under composition.
\end{proposition}

\begin{proof}
For the coupled model this is
Proposition~\ref{prop:SC-well-defined}. For the depth-zero
associated graded the axioms follow from the component structure.
\emph{Associativity:} composition is componentwise FM insertion on the $\FM_k(\C)$ factor and interval composition on the $E_1(m)$ factor; since these factors are independent, associativity of the composite follows from associativity of each factor separately.
\emph{Unitality:} the unit in degree~$1$ is the pair $(\mathrm{pt},\mathrm{pt}) \in \FM_1(\C) \times E_1(1)$; both components are the identity element of their respective operads.
\emph{Coloured $\Sigma$-equivariance:} permutations act by relabelling inputs together with their colour signature; on the decoupled components the closed labels are permuted on the $\FM_k(\C)$ factor while the open inputs carry the ordered $E_1$ structure.
...
The decoupled components are \emph{not} closed under the coupled
mixed-into-open insertion: componentwise it would require a merge
...
boundary insertion point supplies that datum; at depth zero the
corresponding composites vanish, which is what the componentwise
composition of $\gr^0$ records.
\end{proof}
...
C_\ast(\FM_{k,m}(\mathbb H))\otimes A_{\mathsf{ch}}^{\otimes k}\otimes A_{\mathsf{top}}^{\otimes m}\to A_{\mathsf{top}}
\]
satisfying operadic identities; a \emph{strict $\gr^0$-algebra}
carries instead the mixed structure maps
$C_\ast(\FM_k(\C)\times E_1(m))\otimes A_{\mathsf{ch}}^{\otimes k}\otimes A_{\mathsf{top}}^{\otimes m}\to A_{\mathsf{top}}$
of the depth-zero associated graded, and every componentwise
construction below produces $\gr^0$-algebras. The \emph{homotopy} notion is defined via the Boardman--Vogt resolution $W(\mathsf{SC}^{\mathrm{ch,top}})$: a \emph{$W(\mathsf{SC}^{\mathrm{ch,top}})$–algebra} is an algebra over $C_\ast(W(\mathsf{SC}^{\mathrm{ch,top}}))$, encoding all higher coherences.
\end{definition}

\begin{remark}[Homotopy coherence]
The $W$–resolution equips trees indexing compositional patterns with edge-length parameters, turning strict equalities into explicit higher homotopies. This is the standard device encoding $A_\infty$/$E_1$ coherence in the operadic setting.
\end{remark}

\subsection{Closed-Color Comparison with the BD Chiral Operad}
\label{subsec:BD-compare}

Beilinson--Drinfeld construct the chiral operad $\mathsf P_{\mathrm{ch}}$ abstractly via $\ast$-operations on $D$-modules on powers of the curve~\cite[\S3.4]{BD04}. The closed color of $\SCchtop$ above is built concretely, from logarithmic forms on the FM compactification. The two agree up to quasi-isomorphism; the comparison proceeds through the Ran space.
...
\begin{proof}
The argument constructs a zigzag through an intermediate category of factorization algebras on the Ran space.
...
The zigzag of quasi-isomorphisms of operads is
\[
...
3d HT observable couples two colors: holomorphic along
$\C$, topological along $\R$, with OPE residues flowing from bulk
to boundary. A local-to-global principle seeing both colors works
directly with observables on rectangles
$D \times I \subset \C \times \R$. The output of this local analysis
is the formal collision shadow of the factorization algebra: a
$W(\SCchtop)$-algebra, not the full Ran-space or D-module
factorization datum.
...
Extracting the product-formal local operadic shadow requires a
descent principle on product covers. Weiss cosheaf descent is the
correct principle for factorization algebras; the argument below
...
\begin{proof}
The proof applies only to product covers. An arbitrary Weiss cover
by rectangles need not be cofinally refined by a product cover; that
case lies beyond the present statement.
...
(holomorphic descent). Composition gives the full Weiss descent
quasi-isomorphism.
\end{proof}
...
structure $\C \times \R$ as a product datum: the factorization of the
iterated hocolimit argument relies on an actual product cover, not
on an arbitrary rectangle Weiss cover. For arbitrary rectangle
covers, or for a general $3$-manifold~$M$ not of product form, the
Weiss descent comparison requires a separate cofinality or
factorization-envelope argument (e.g.\ as in~\cite{CG17}). The
product structure is the geometric reflection of the physical
decoupling of holomorphic and topological directions at the level of
the underlying manifold (Remark~\ref{rem:product-forced-by-locality}
...
\begin{proof}
The local operadic shadow and the essential-image restriction are
constructed in turn.
...
\end{itemize}
The formal local operation space is therefore
$\FM_k(\C)\times E_1(m)$, the mixed operation space of $\SCchtop$.
The no-open-to-closed rule is inherited from HT directionality:
topological boundary inputs do not produce a holomorphic closed
output.
...
higher homotopies for iterated inclusions. This is the standard
Boardman--Vogt construction~\cite{BV73}; applying chains yields
coherent $C_\ast(W(\SCchtop))$ operations.

\medskip\noindent
\textbf{Step 3: From a prefactorization algebra to its operadic shadow.}
Fix sufficiently small base rectangles around a point
...
to canonical quasi-isomorphism; equivalently, pass to the
corresponding germs. The closed color $A_{\mathsf{ch}}$ is the germ
seen by holomorphic collision operations; the open color
...
\]
The operadic identities are functoriality and associativity of
prefactorization structure maps, restricted to boundary strata of
$\FM_k(\C)\times E_1(m)$. Product Weiss descent makes the germ
invariant under replacement of a rectangle by a Weiss refinement.
...
rectangle by left Kan extension from the full subcategory of
...
$W(\SCchtop)$-operations. Factorization maps are induced by operadic
insertion. Cofibrancy of $W(\SCchtop)$ supplies the coherent
homotopies.

The construction is the factorization envelope on the rectangle
basis; it is left adjoint to taking the local operadic shadow. A
map from the envelope to a rectangle prefactorization algebra is the
...
Steps~3 and~4 produce the recovery only for prefactorization
algebras generated from the two stalk colors by rectangle
factorization and product Weiss descent. Global Ran-space data,
D-module monodromy, and higher-genus curvature belong to the
factorization layer; they are invisible to $W(\SCchtop)$. The
local/global distinction is the content of
...
$W(\mathsf{SC}^{\mathrm{ch,top}})$ is the genus-$0$ Swiss-cheese
object. Literal large-$N$ 't~Hooft counting requires two further
structures \textup{(}Vol~I, Cyclicity-Ribbon Theorem and Bare-Graph
Lemma\textup{)}:
...
\textup{(}Vol~I, Definition of ribbonized Swiss-cheese\textup{)},
whose graph amplitudes carry the exact weight
$N^{\chi(\Sigma_\Gamma)}$ \textup{(}Vol~I, Exact
$N^{\chi}$-Weighting Theorem\textup{)}. Bare stable-graph sums
without this enhancement give genus-organized expansions, not
canonical $N$-weighted 't~Hooft sums.
\end{remark}
...
(Theorem~\ref{thm:universal-holography-master}). Product Weiss
...
observables. Global factorization data lies outside its reach.
...
$\C \times \R$. This is the local content of the bulk
identification after an HT/BV realisation datum $\Xi$ is supplied:
...
The directionality $\SCchtop \neq E_3$ holds throughout: the
present recognition theorem produces $\SCchtop$ structure;
topologization is a separate theorem with explicit hypotheses
(conformal vector, non-critical level, \(Q\)-cohomological exactness
witness), proved for affine Kac--Moody at non-critical level on
\(Q_{\mathrm{tot}}\)-cohomology in
Theorem~\ref{thm:E3-topological-km}, conjectural for general
chiral algebras with conformal vector. Strict chain-level affine
topologization is a separate frontier; class~M chain-level
topologization is only asserted in the weight-completed category via
