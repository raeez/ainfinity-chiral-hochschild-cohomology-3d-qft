...
Dunn additivity \(E_{2} \simeq E_{1} \otimes E_{1}\) tensors the
first little-disks operad with itself. For the SC\(^{\mathrm{ch,top}}\)
genus tower of Chapter~\ref{ch:modular-swiss-cheese-operad} the
twisted analogue is
\[
...
\]
the factorisation twisted by a cochain \(\tau\) built from
\(R\)-matrix monodromy at each genus-raising clutching. Genus \(0\)
is uncurved Dunn. Genus \(1\) is the twisted tensor criterion
\autoref{prop:genus1-twisted-tensor-product}. Genus \(g \ge 2\)
reduces to an obstruction class \(\mathrm{Ob}_{g}\) in \(H^{2}\)
of a twisting-cochain complex; the criterion below isolates the
hypotheses making this \(H^{2}\) vanish.

Two distinct complexes carry \(\mathrm{Ob}_{g}\). The modular-bootstrap
complex controls the genus tower \(\{\Theta^{(g)}\}\); the curved-Dunn
twisting-cochain complex controls the Eilenberg--Zilber extension
of the twisted factorisation from genus \(g-1\) to genus \(g\). The
differentials differ. The modular-bootstrap vanishing is supplied
by a contracting homotopy; curved-Dunn vanishing follows after a
degree-\(2\) comparison identifies the two obstruction classes.
...
\(\kappaChHodge(A)\) and Arakelov form \(\omega_{g}\) on
...
but assemble them differently: the modular-bootstrap complex via the
tree-twisted Feynman transform of \(M\mathrm{Ass}\); the curved-Dunn
complex via the Eilenberg--Zilber machine applied to the tensor
factorisation. The differentials \(d_{\Theta^{(0)}}\) and \(d_{0}\)
are \emph{a priori} distinct on \emph{a priori} distinct graded
objects.

\begin{remark}[Two-complex distinction]
\label{rem:two-complex-fm67}
The modular-bootstrap complex records a stable-ribbon-graph
Maurer--Cartan problem; the curved-Dunn complex records a transverse
\(E_{1}\) twisting-cochain problem. No direct argument moves from
the Arakelov class on \(\overline{\cM}_{g,n}\) to the curved-Dunn
differential. Vanishing transports only after a low-degree
comparison map identifies the two obstruction classes.
\end{remark}
...
\begin{proof}
Let \(\xi\) be a \(d_{0}\)-closed degree-\(2\) cochain in
\(\mathrm{TCo}^{\bullet}_{g}(A)\), with class
\([\xi] \in H^{2}(\mathrm{TCo}^{\bullet}_{g}(A))\). By
hypothesis~(3), \(H^{2}(\Phi)\) is an isomorphism, so there is a
\(d_{\Theta^{(0)}}\)-closed degree-\(2\) cocycle
\(\tilde{\xi} \in \fg^{A}_{\mathrm{mod}}\) with
\(H^{2}(\Phi)[\tilde{\xi}] = [\xi]\). If
\(H^{2}\bigl(\fg^{A}_{\mathrm{mod}}, d_{\Theta^{(0)}}\bigr) = 0\),
then \([\tilde{\xi}] = 0\), hence
\([\xi] = H^{2}(\Phi)[\tilde{\xi}] = 0\); that is,
\(\xi = d_{0}\eta\) for a degree-\(1\) cochain \(\eta\).
Hypothesis~(1) supplies the explicit cocycle representatives
through which \(H^{2}(\Phi)\) is evaluated, and hypothesis~(2)
identifies the image of the bootstrap obstruction
\(\Theta^{(g)}\) with the curved-Dunn obstruction class
\(\mathrm{Ob}_{g}\), so the vanishing applies to the obstruction
class itself.
\end{proof}

\begin{remark}[Vol~I consumer]
\label{rem:bridge-vol1-consumer}
The Vol~I global-assembly statement uses the comparison as a
criterion: unconditional on loci where the low-degree chain map
\(\Phi\) has been constructed and checked; a named comparison
obligation elsewhere.
\end{remark}

\begin{remark}[Origin of the bridge]
\label{rem:bridge-provenance}
The bootstrap and curved-Dunn complexes share the same underlying
datum: \(R\)-matrix monodromy at sewing circles. The
modular-bootstrap complex organises it by stable ribbon graph
(Feynman transform); the curved-Dunn complex by twisting cochain
(Eilenberg--Zilber). The comparison map \(\Phi\), when
constructed, identifies their degree-\(2\) obstruction classes.
\end{remark}
...
modular characteristic \(\kappaChHodge(A)\). Assume there is a genus-one
...
the identity of the annular bootstrap insertion \(\Theta^{(1)}\)
with the curved-Dunn twisting cochain \(\tau_{1}\) as monodromy
classes. At \(g \ge 2\) the comparison matches stable-graph strata
with the higher Eilenberg--Zilber corrections.
\end{remark}
...
\autoref{prop:genus1-twisted-tensor-product} and compatible primitives
for every \(2\le h\le g\) are also supplied, the genus-\(\le g\)
truncation of the modular bar complex admits the twisted tensor
representative
\[
 \mathbf{B}_{\mathrm{mod}}\!\bigl(\cO^{A_{\infty}\text{-ch}}\bigr)
 \;\simeq\;
 ...
 \mathbf{B}\!\bigl(E_{1}^{\mathrm{tr}}\bigr).
\]
This representative is taken up to convolution gauge; an equivalence
of modular operads requires the higher coherence maps, the \(H^{1}\)
gauge datum, and the boundary formal-type input.
\end{theorem}
...
\begin{remark}[Beilinson-Mill\`es--Drummond-Cole comparison]
\label{rem:BMDC}
A model-categorical proof identifies both complexes as cofibrant
resolutions of the same curved modular operad and checks that the
resulting Quillen equivalence induces the same degree-\(2\)
obstruction class. The criterion above isolates the chain-level
input: the map \(\Phi\) and its degree-\(2\) cohomological effect.
\end{remark}

\section{Modular-bootstrap \texorpdfstring{$H^{2}$}{H2} criterion}
\label{sec:modular-bootstrap-h2-named-lemma}

The curved-Dunn transport theorem needs modular-bootstrap vanishing
as input. The vanishing is a contracting homotopy for the
...
The lemma is a vanishing criterion, not a proof that the contracting
homotopy exists in every landscape family. In concrete families the
homotopy is constructed from stable-ribbon-graph generators,
boundary contraction, and the genus-\(0\) chiral product. The
curved-Dunn vanishing theorem applies once that construction is in
hand.
\end{remark}

\begin{remark}[Chain-level vs \((\infty,1)\)-categorical readings]
\label{rem:lemma-two-lanes}
\autoref{lem:modular-bootstrap-H2-vanishing} sits in the
chain-level lane: \(\fg^{A}_{\mathrm{mod}}\) is a strict dg Lie
algebra with the genus-\(0\) twisted bracket as differential. The
\((\infty,1)\)-categorical translation is the vanishing of the
obstruction group controlling \(\pi_{1}\) of the Maurer--Cartan
space. The two readings are equivalent after the
Maurer--Cartan tangent-complex identification; neither supplies the
...
curved-Dunn vanishing to modular-bootstrap \(H^{2}\)-acyclicity
plus the degree-\(2\) comparison map \(\Phi\). A chain-level
primitive is stronger data. The genus-\(2\) criterion follows;
its higher-genus clutching extension is conditional on the same
comparison input and on the boundary KZB formal-type hypothesis of
...
\begin{remark}[Both stable graphs required]
\label{rem:witness-uses-both-graphs}
\autoref{def:handle-attachment-primitive-g2} supports the
handle-attachment primitive on both valence-one stable ribbon
graphs at \((g, n) = (2, 0)\). The \(\Gamma^{(2)}_{\flat}\)-only
choice fails \(\Sigma_{0}\)-equivariance: the bracket
\([\Theta^{(0)}, \Gamma^{(2)}_{\flat}]\) generates a self-bouquet
contribution at the moduli diagonal, absorbed by the
\(\Gamma^{(2)}_{\sharp}\) term and required for the cyclic
symmetry of the Feynman transform. The coefficients
\(\tfrac{1}{12}\) and \(\tfrac{1}{8}\) match the
inverse-automorphism weights
\(1/|\mathrm{Aut}(\Gamma^{(2)}_{\flat})| = 1/12\) and
\(1/|\mathrm{Aut}(\Gamma^{(2)}_{\sharp})| = 1/8\) of the
Getzler--Kapranov state-sum formula for \(M\mathrm{Ass}(2, 0)\)
\cite{GeK98}~equation~(5.13).
\end{remark}

\begin{remark}[Starting genus]
\label{rem:genuine-starting-genus-g2}
Curved-Dunn vanishing \(H^{2}(\text{curved-Dunn}) = 0\) starts at
\(g \ge 2\). At \(g = 1\):
\begin{itemize}
\item \([\Theta^{(1)}, \Theta^{(1)}] \equiv 0\) (single ribbon
 graph, no grafting target), giving \(D\Theta^{(1)} = 0\); the
 genus-one datum is the annular monodromy \(\tau_{1}\), not an
 all-genus vanishing theorem;
\item \(\tau_{1} = \mathrm{Mon}(R)\) is supplied by
 \autoref{prop:genus1-twisted-tensor-product}; no higher
 Eilenberg--Zilber correction enters.
\end{itemize}
At \(g \ge 2\) both become non-trivial: the bracket
\([\Theta^{(<g)}, \Theta^{(<g)}]\) has multiple grafting
contributions, and \(\tau_{g}\) carries a higher Eilenberg--Zilber
correction \(\Phi(P_{g})\) constructed inductively from
\(\tau_{<g}\). \autoref{thm:curved-dunn-H2-explicit-coboundary-g-geq-2}
makes the \(g = 2\) case explicit; higher-genus witnesses follow
by clutching the \(g = 2\) and \(g = 1\) primitives across nodal
divisors via
\autoref{cor:modular-operad-composition-all-levels}, conditional
on \autoref{conj:kzb-boundary-formal-type-extension}.
\end{remark}
...
non-zero \(\kappaChHodge(A)\), assume the modular-bootstrap
\(H^{2}\)-criterion of
\autoref{lem:modular-bootstrap-H2-vanishing}, the comparison
criterion of
...
Let \(A\) be a chirally Koszul algebra at non-zero \(\kappaChHodge(A)\)
...
level \(k\) with non-zero \(\kappaChHodge(A)\), and let \(g \ge 2\).
...
\(b_{0} |\psi\rangle = 0\) in string field theory
(Zwiebach~\cite{Zwiebach93}) and of the Lorenz gauge
\(d^{\ast} A = 0\) in Yang--Mills: a gauge-fixing condition
selects a unique representative of a homotopy class via a
Hodge-type projector, lifting cohomological identity to chain-level
equality. Conditional chain-level uniqueness
(\autoref{thm:harmonic-gauge-existence-uniqueness}(ii)) discharges
the Vol~I clause~(iv) requirement of strict equality.
\end{remark}

\begin{remark}[Comparison with the BMDC gauge]
\label{rem:BMDC-gauge-comparison}
The BMDC model structure on curved operads (\autoref{rem:BMDC})
fixes a cofibrant replacement up to Quillen-equivalence without
selecting a canonical representative; the harmonic gauge refines
the BMDC choice to a chain-level canonical form. The BMDC
cofibrant replacement is a Koszul resolution; in the pro-ambient
it admits a unique harmonic-gauge representative by
\autoref{thm:harmonic-gauge-existence-uniqueness}(ii). BMDC
supplies the \((\infty, 1)\)-categorical chain-level equivalence;
the harmonic gauge supplies the chain-level strictification within
that equivalence class.
\end{remark}

\begin{remark}[Chain-level vs \((\infty, 1)\)-categorical readings]
\label{rem:harmonic-gauge-two-lanes}
\autoref{thm:harmonic-gauge-existence-uniqueness} is chain-level:
\(\tau_{g}^{\mathrm{harm}}\) lives in the pro-chain ambient, and
its uniqueness is pro-chain uniqueness, not homotopy-coherent.
The \((\infty, 1)\)-categorical translation is contractibility of
the space of twisting cochains
\(\mathrm{MC}^{\mathrm{ch}}(\fg^{A}_{\mathrm{mod}}) \in
\mathrm{Spc}\) at each genus, proved by the same SDR machinery
via Dold--Kan. The chain-level refinement is required for Vol~I
\textup{thm:lagrangian-complementarity-global-upgrade}
clause~(iv); the \((\infty, 1)\) shadow is the Costello--Gwilliam
\cite{CG21} factorisation \(\infty\)-category setting.
\end{remark}
...
conditions on every two-coordinate corner hold. In the
...
exact rational computation is the displayed matrix calculation in
the two-dimensional $\fsl_2$ representation.
\end{proof}
...
\autoref{conj:kzb-boundary-formal-type-extension}. In the modular
Swiss-cheese chapter this same input is written in the explicit
\(q\)-coordinate form
\autoref{prop:irregular-kzb-q-formal-type-stokes-cocycle}. It
supplies boundary composition after formal type and mixed Stokes data;
it is not the curved-Dunn \(H^{2}\)-acyclicity mechanism of
\autoref{thm:curved-dunn-H2-vanishing-all-genera}.
\end{proof}
...
the conditional twisted tensor representative supplied by
\autoref{thm:curved-dunn-H2-vanishing-all-genera} removes the
algebraic degree-\(2\) twisting obstruction. The extension through
the boundary stratum is supplied by the formal-type input, whenever
its hypotheses hold.
...
\section{Determinant-line curvature and the conductor scalar}
\label{sec:chiral-deligne-pairing}

The Deligne pairing \(\langle L, M\rangle_{\mathrm{Del}}\)
is a line on the base of a family of curves
\(\pi : \mathcal{C}_{g} \to \overline{\mathcal{M}}_{g}\). Its first
Chern class is
\[
 c_{1}\bigl(\langle L,M\rangle_{\mathrm{Del}}\bigr)
 =
 \pi_{*}\bigl(c_{1}(L)c_{1}(M)\bigr).
\]
It is not the cup product
\(c_{1}(\det R\pi_{*}L)c_{1}(\det R\pi_{*}M)\) on the base. The
Vol~I conductor is therefore a coefficient of determinant-line
curvature, not a scalar integral of a Deligne-pairing product unless
a top-degree functional on \(\overline{\mathcal M}_{g}\) has also
been chosen.

\begin{definition}[Chiral Deligne line and determinant coefficient]
\label{def:chiral-deligne-pairing}
Let \(A, A'\) be chiral algebras on a smooth projective curve \(X\)
at non-critical level, and suppose that their chiral density data are
represented by line bundles \(\omega_A,\omega_{A'}\) on the universal
curve. Put
\[
 \mathscr L_{A,g}:=\det R\pi_{*}\omega_{A}.
\]
The \emph{chiral Deligne line} of the pair is
\(\langle \omega_A,\omega_{A'}\rangle_{\mathrm{Del}}\), with
first Chern class
\[
 c_{1}\bigl(\langle \omega_A,\omega_{A'}\rangle_{\mathrm{Del}}\bigr)
 =
 \pi_{*}\bigl(c_{1}(\omega_A)c_{1}(\omega_{A'})\bigr).
\]
The determinant-line coefficient of \(A\) is the scalar
\(\kappaChHodge(A)\) determined by the Hodge-class component
\[
 c_{1}(\mathscr L_{A,g})
 =
 \kappaChHodge(A)\lambda_1^{(g)} + B_{A,g},
\]
where \(\lambda_1^{(g)}:=c_1(\det R\pi_*\omega_\pi)\) is the Hodge
line class on \(\overline{\mathcal M}_g\); the symbol \(\lambda_g\)
denotes the top Hodge class \(c_g(\mathbb E)\) in the
Faber--Pandharipande integral. The coefficient
\(\kappaChHodge(A)\) is the scalar entering the Vol~I
derived-centre complementarity theorem.
\end{definition}

Three conventions are fixed. First, the Deligne line is symmetric
and biadditive in line bundles on the universal curve. Second,
\(\kappaChHodge(A)\) is the Hodge-class coefficient of the
determinant line, not the square of a Chern class. Third, a scalar
genus-\(g\) intersection number requires a top-degree functional on
\(H^{\bullet}(\overline{\mathcal M}_{g})\); no such functional is
part of the Deligne line itself.

\begin{proposition}[Determinant-line form of the conductor; \ClaimStatusProvedHere]
\label{prop:chiral-deligne-conductor-identity}
Let \(A\) be a chirally Koszul algebra with Koszul dual \(A^{!}\)
in the sense of Vol~I Theorem~A. Then the Vol~I conductor identity is
the determinant-line coefficient equality
\[
 \rho_K(A)
 =
 \kappaChHodge(A)+\kappaChHodge(A^{!}).
\]
Equivalently,
\[
 c_{1}(\mathscr L_{A,g})+c_{1}(\mathscr L_{A^{!},g})
 =
 \rho_K(A)\lambda_1^{(g)}+B_{A,g}+B_{A^{!},g}.
\]
The chiral Deligne line may pair the two density sheaves, but it does
not by itself produce the scalar \(\rho_K(A)\). Specialisations:
\begin{itemize}
\item Virasoro:
\[
\rho_K(\mathrm{Vir}_{c}) =
\kappaChHodge(\mathrm{Vir}_{c})
 + \kappaChHodge(\mathrm{Vir}_{26-c}) = c/2 + (26-c)/2 = 13.
\]
This is the derived-centre complementarity sum of archetype
\(\mathsf M\). The critical central charge \(c + (26-c) = 26\) is
the distinct scalar setting the Hodge-line shift.
\item Affine Kac--Moody \(\widehat{\mathfrak{g}}_{k}\):
 on the trace-form affine row
 \[
 \rho_K(\widehat{\mathfrak{g}}_{k}) =
 \kappaChHodge(\widehat{\mathfrak{g}}_{k})
	 + \kappaChHodge(\widehat{\mathfrak{g}}_{-k-2h^{\vee}})
 \;=\;0.
 \]
 The Sugawara Virasoro quotient is a separate image of the affine pair.
\item Bershadsky--Polyakov \(W_{k}(\mathfrak{sl}_{3}, f_{\min})\):
 the slot \(\rho_K(\mathrm{BP}_{k})\) is open.
 The central-charge conductor
 \(c_{\mathrm{BP}}(k)+c_{\mathrm{BP}}(-k-6)\equiv 50\)
 (Fateev--Lukyanov normalization) is a distinct scalar — a
...
\begin{proof}
By definition of the determinant-line coefficient,
\[
c_{1}(\mathscr L_{A,g})+c_{1}(\mathscr L_{A^{!},g})
=
\bigl(\kappaChHodge(A)+\kappaChHodge(A^{!})\bigr)\lambda_1^{(g)}
+B_{A,g}+B_{A^{!},g}.
\]
Vol~I Theorem~C identifies the coefficient of \(\lambda_1^{(g)}\) with
\(\rho_K(A)\). The displayed specialisations are the named Verdier
lanes needed here: affine has coefficient \(0\), Virasoro has
coefficient \(13\), and the Bershadsky--Polyakov coefficient is
open. No multiplication of the two base Chern classes is used.
\end{proof}

\begin{remark}[Lagrangian complementarity and Deligne geometry]
\label{rem:chiral-deligne-lagrangian}
Vol~I Theorem~C is a statement about the derived-centre coefficient
of the determinant-line curvature. A Deligne pairing supplies
additional Arakelov geometry on the universal curve. It becomes a
numerical pairing only after a top-degree functional on the base is
specified.
\end{remark}
...
determinantal line \(\mathscr L_{A,g}\), in the chosen Quillen
normalisation, with
\(\kappaChHodge(A)\omega_{\mathrm{WP}}\) plus boundary terms. The
Koszul flip changes the coefficient by the Vol~I conductor
\(\rho_K(A)\). This is the Bismut--Freed incarnation of the same
coefficient identity, not a Deligne self-intersection formula.
\end{remark}

\section*{Synopsis}

The architectural move is the bridge
...
complex: a low-degree comparison criterion. Once the chain map is
constructed, identifies obstruction classes, and is an isomorphism
on \(H^{2}\), modular-bootstrap acyclicity transports to
curved-Dunn acyclicity
(\autoref{thm:curved-dunn-H2-vanishing-all-genera}).

The Jimbo--Miwa irregular-singular framework gives a conditional
Stokes-sector composition theorem from a compatible boundary
formal type (\autoref{thm:irregular-singular-kzb-regularity}). The
remaining boundary input is
\autoref{conj:kzb-boundary-formal-type-extension}; under it,
\autoref{cor:modular-operad-composition-all-levels} delivers
modular operad composition for every \((g, n)\) with
\(2g - 2 + n > 0\) and every generic non-integral level, subject
to the algebraic comparison hypotheses.

At non-generic levels with resonances (minimal models, admissible
levels) the Stokes matrices may fail to be unipotent and require
the Hukuhara--Turrittin decomposition. These loci form a
Zariski-closed subset of the level parameter and carry their own
analysis.

\section{K3 base-point synthesis}
\label{sec:cdhg-supplement}

\begin{remark}[K3 base point at genus two]
\label{rem:cdhg-1}
The K3 curved-Dunn echo is the genus-$2$ paramodular instance of
the twisting-cochain problem for the bi-based Ran datum
$(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24}),\,\bar{\mathcal A}_2)$.
The surface-stage object carries $E_2$ factorisation data before
restriction; the native curve object after restriction is the
$E_1$-chiral algebra $\mathbf H_{\Delta_5,E}$. The BKM curvature
is $\omega_{\Delta_5}$, the Gritsenko--Nikulin $2$-cocycle of the
denominator identity. The genus-$2$ parameter base
$\bar{\mathcal A}_2$ carries the Pasol--Zagier 2013 cover for the
unnormalised Igusa square $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$:
the weight-$10$ Siegel modular-form convention, not the
Oberdieck--Pixton reduced-DT scalar
$\Phi_{10}^{\mathrm{OP}}=4096^{-1}\Delta_5^2$. The curved-Dunn
comparison resides on that modular-form sheet
\cite{GritsenkoNikulin1998,Borcherds1992,PasolZagier2013}; see
Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[K3 Dunn stop at genus two]
\label{rem:cdhg-2}
The Siegel-paramodular curved-Dunn construction at the K3 base
point stops at genus~$2$: the Borcherds--Gritsenko--Nikulin
denominator lives on the genus-$2$ paramodular base, matching
Vol.~I's modular Koszul arithmetic for K3 input. Higher-genus
curved-Dunn data require a different modular input: CY-$3$ or
CY-$\geq4$ constructions
\cite{PiatetskiShapiroShafarevich1971,BailyBorel1966}; see
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\section{Genus-$g$ obstruction tower via $\phi^{(n)}$}
\label{sec:cdhg-supplement-ii}

\begin{remark}[Genus-$g$ Maurer--Cartan obstruction]
...
(Remark~\ref{V1-rem:nc-phi3-4-5}, Vol.~I
Chapter~\ref{app:nilpotent-completion}) factor through the
curved-Dunn higher-genus $E_1$-chiral datum: the Maurer--Cartan
obstruction $\mathrm{obs}_g$ on $\overline{\mathcal M}_{g,n}(K3)$
admits
\[
\mathrm{obs}_g \;=\; \sum_{n=1}^{g+1}\,\phi^{(n)}\cdot c_{g,n},
\]
$c_{g,n}$ the Arakelov intersection of the Hodge line class
$\lambda_1^{(g)}$
with the $n$-th Brauer divisor on
$\overline{\mathcal M}_{g,n}$
\cite{Mumford1983}. Vol.~I Theorem~D specialises to
$\mathrm{obs}_g = \kappa_{\mathrm{fiber}}(K3)\cdot\lambda_1^{(g)}$,
$\kappa_{\mathrm{fiber}}(K3) = 24$ the Mukai-lattice fibre rank;
the Beilinson--Bloch height pairing implements
$\lambda_1^{(g)} = (1/12)\sum_{n}\phi^{(n)}\cdot c_{g,n}$, reducing the
sum to the universal formula. The comparison theorem suppresses
$\phi^{(n)}$ with $n > g+1$: the pentagon face of depth $n$
lies above the $(g+1)$-st twisting stratum; see Vol.~I
Remark~\ref{V1-rem:nc-Ainfty-open}.
\cite{Mumford1983,BeilinsonBloch2005,GritsenkoNikulin1998}.
\end{remark}

\begin{remark}[Tower bound $n \le 3g$, and the hypothesis $n \le g+1$]
\label{rem:cdhg-tower-bound}
The pentagon face of depth $n$ lives in $H^2(\mathcal{O}_{g,n})$
($\mathcal{O}_{g,n}$ the $n$-th stratum of the curved-Dunn operad
at genus~$g$); the twisting cochain of depth $n > g+1$ is
obstructed to $(g,n)$-transfer because the Mumford boundary of
$\overline{\mathcal M}_{g,n}$ has Chow dimension $3g - 3 + n - 1$,
and the $\phi^{(n)}$-coboundary is a $(2n-3)$-form, so the
pairing is non-trivial only when $2n - 3 \le 3g - 3 + n$, that
is $n \le 3g$. The sharper bound $n \le g+1$ used above is not a
consequence of this dimension count; it is the depth clause of the
comparison-theorem hypothesis package (suppression of
$\phi^{(n)}$ with $n > g+1$ by the $(g+1)$-st twisting stratum,
Vol.~I Remark~\ref{V1-rem:nc-Ainfty-open}), and it carries no
independent literature citation.
\cite{Mumford1983,BeilinsonBloch2005}.
\end{remark}

\section{Formal obs$_g$ expressions at $g = 9, 10$ and pro-object limit}
\label{sec:cdhg-supplement-iii}

\begin{remark}[Formal genus-$9$ and genus-$10$ expressions via
$\phi^{(8,9,10)}$: unverified comparison data]
\label{rem:cdhg-obs-9-10}
The expressions of this remark are formal expansions whose every
ingredient --- the comparison maps, the motivic MZV leg
decomposition, and the Borcherds legs --- is supplied by the
unverified comparison hypotheses; none has been computed in any
example, and no claim beyond bookkeeping consistency of the formal
expansion is made. Under those comparison hypotheses, the coherences
...
\]
At genus~$9$ the formal sum reaches $\phi^{(10)}$; the weight-$10$
motivic MZV graded piece has dimension
$d_{10}=7$ (where
$d_0=1$, $d_1=0$, $d_2=1$, and
$d_m=d_{m-2}+d_{m-3}$ for $m\geq3$), and the assignment of a
Borcherds leg
$(\Phi_{10}^{\mathrm{un}})^{5}/\eta^{120}$ is numerology on the
unverified comparison: the exponents match weights, and nothing
more is claimed. At genus~$10$ the formal sum reaches
$\phi^{(11)}$; the weight-$11$ motivic MZV graded piece has
$d_{11}=d_9+d_8=9$ generators, and the corresponding
Borcherds-leg assignment
$(\Phi_{10}^{\mathrm{un}})^{11/2}/\eta^{132}$ is numerology of the
same kind. The Arakelov
intersection numbers $c_{g,n}$ would be computed via the
Beilinson--Bloch height pairing of the Hodge line class
$\lambda_1^{(g)}$
against the $n$-th Brauer divisor on $\overline{\mathcal M}_{g,n}$
(Mumford~$1983$; Beilinson--Bloch~$2005$); Vol.~I Theorem~D
specialises the total sum to
$\mathrm{obs}_g = \kappa_{\mathrm{fiber}}(K3)\lambda_1^{(g)}
= 24\lambda_1^{(g)}$ only under the full comparison package, the
genus-\(9\) and genus-\(10\)
specialisation of Vol.~I Remark~\ref{V1-rem:nc-obs9-10}.
...
For each weight cutoff~$m$, choose a curved-Dunn complex
$\mathfrak c_{\leq m}$, a represented obstruction
$\mathrm{obs}_{\leq m}$, and transition maps $q_{m+1,m}$ satisfying
\[
 q_{m+1,m}(\mathrm{obs}_{\leq m+1})
 =\mathrm{obs}_{\leq m}.
\]
If the maps $q_{m+1,m}$ are degreewise surjective, the system is
Mittag--Leffler and
\[
 \mathrm{obs}_\infty=(\mathrm{obs}_{\leq m})_m
 \in\varprojlim_m\mathfrak c_{\leq m},
 \qquad
 \varprojlim{}^1_m\mathfrak c_{\leq m}=0.
\]
The factorisation-$\infty$-categorical statement is obtained from a
compatible diagram in the curved-Dunn factorisation category.  The
chain-level transition maps and the categorical comparison functor
are the two ambient hypotheses.
\end{remark}

\begin{remark}[Genus ten and eleven: represented obstruction data]
\label{rem:cdhg-obs-10-11}
Let \(\mathfrak c_g\) be the completed curved-Dunn deformation complex
at genus~\(g\).  Assume represented cochains
\(\psi_n\in\mathfrak c_g\) and comparison maps from the KZ-word and
Borcherds scalar sources through order \(g+1\).  Then
\[
 \mathrm{obs}_{10}
 =
 \sum_{n\leq11}\psi_n\,c_{10,n},
 \qquad
 \mathrm{obs}_{11}
 =
 \sum_{n\leq12}\psi_n\,c_{11,n},
\]
where the \(c_{g,n}\) are the coefficients of the chosen genus
comparison map.

Brown's Hoffman basis gives
\[
 d_{11}=9,\qquad d_{12}=12.
\]
At weight~$12$ the repeated period satisfies
\begin{equation}
\label{eq:cdhg-zeta3333-newton}
 \zeta(3,3,3,3)
 =
 \frac{\zeta(3)^4-6\zeta(3)^2\zeta(6)+3\zeta(6)^2
 +8\zeta(3)\zeta(9)-6\zeta(12)}{24}.
\end{equation}
Euler's even-zeta formula gives
$\zeta^{\mathfrak m}(12)\in
\mathbb Q\cdot(\zeta^{\mathfrak m}(2))^6$, so every term belongs to
the square of the positive-weight motivic ideal.  Its image in the
motivic indecomposable quotient is zero.  A
primitive weight-$12$ contribution to \(\mathrm{obs}_{11}\) is
therefore specified by a represented graph class \(\gamma_{12}\), its
curved-Dunn action, and a homotopy intertwining this action with the
genus comparison map.
\cite{Brown2011,Brown2012,Hoffman1997}.
\end{remark}

\begin{remark}[Motivic and curved-Dunn inverse systems]
\label{rem:cdhg-ML-12}
The weight-truncated motivic period spaces form a surjective inverse
system, with finite graded dimensions generated by
\((1-t^2-t^3)^{-1}\).  This system is Mittag--Leffler.

A curved-Dunn pro-obstruction consists of truncated complexes
\(\mathfrak c_{g,\leq m}\), transition maps
\[
 q_{m+1,m}\colon\mathfrak c_{g,\leq m+1}
 \longrightarrow\mathfrak c_{g,\leq m},
\]
compatible obstruction cocycles, and comparison maps from the
motivic truncations.  Degreewise surjectivity of \(q_{m+1,m}\)
implies \(\varprojlim^1\mathfrak c_{g,\leq m}=0\).
Thus the motivic system supplies the period coordinates, while the
curved-Dunn transition maps supply the pro-obstruction theorem.
\cite{Brown2011}.
\end{remark}

\begin{remark}[Weight-twelve graph action on the curved-Dunn complex]
\label{rem:cdhg-depth-4-grt1}
A primitive weight-$12$ action consists of
\[
 \gamma_{12}\in Z^0(\mathrm{GC}_2)_{12},
 \qquad
 \rho_g\colon\mathrm{GC}_2
 \longrightarrow\operatorname{Der}(\mathfrak c_g),
\]
a period map on \([\gamma_{12}]\), and degree-\((-1)\) homotopies
intertwining the KZ, curved-Dunn, and Borcherds images.  Under this
comparison datum, \(\rho_g(\gamma_{12})\) is the primitive
curved-Dunn correction.  Willwacher's isomorphism
\(H^0(\mathrm{GC}_2)\cong\mathfrak{grt}_1\) identifies its ambient
Grothendieck--Teichm\"uller class.  The scalar
\(\zeta(3,3,3,3)\) belongs to the product-period sector by
Equation~\eqref{eq:cdhg-zeta3333-newton}.
\cite{Willwacher2015,Brown2012,Drinfeld1990}.
\end{remark}

\begin{remark}[The two weight-twelve automorphic sections]
\label{rem:cdhg-fake-monster-non-interference}
The K3 section at order~$12$ is
\[
 \left(\frac{\Delta_5}{\eta^{12}}\right)^{12}
 =
 \frac{(\Phi_{10}^{\mathrm{un}})^6}{\eta^{144}},
\]
with raw modular weight \(-12\) on the K3 orthogonal domain of
signature \((2,1)\).  The Fake-Monster denominator \(\Phi_{12}\)
belongs to the orthogonal domain of
\(\mathrm{II}_{25,1}\), of signature \((25,1)\).  The curved-Dunn
comparison therefore records them in distinct typed summands, and
the K3 obstruction uses the first.
\cite{Borcherds1998,GritsenkoNikulin1998}.
\end{remark}

\begin{remark}[Exact repeated-zeta scalar]
\label{rem:cdhg-c12-explicit}
Newton's identity~\eqref{eq:cdhg-zeta3333-newton} gives
\[
 \zeta(3,3,3,3)
 =
...
\]
The first value is independently recovered by finite
elementary-symmetric sums in the sequence \(n^{-3}\).  The factor
\(12!\) defines a scalar simplex normalisation.  Its image in
\(\mathrm{obs}_{11}\) is determined by the word-to-cochain and genus
comparison maps of Remark~\ref{rem:cdhg-obs-10-11}; its primitive
projection is zero.
