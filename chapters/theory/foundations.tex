...
For Koszul algebras, the self-intersection groupoid $\Steinb \rightrightarrows \cL$ is \'etale (Proposition~\ref{prop:etale-groupoid-koszul}): both source and target maps are quasi-isomorphisms of derived stacks, because the higher Tor terms $\operatorname{Tor}^{\cO(\cM)}_i(B,B) = 0$ vanish for $i > 0$.  Koszul inversion is then the groupoid Morita theorem (Corollary~\ref{cor:koszul-morita}): an \'etale groupoid is Morita-equivalent to its base, so comodules and modules are equivalent, and the bar-cobar unit $\Omega(\barB(\cA)) \xrightarrow{\sim} \cA$ is a quasi-isomorphism.  Off the Koszul locus, the groupoid acquires genuine derived fibers---the higher $A_\infty$ operations---and the Morita equivalence is replaced by completion.
\end{remark}
...
\begin{remark}[The Koszul/non-Koszul dichotomy as the perturbative/non-perturbative divide]
\label{rem:koszul-perturbative-watershed}
\index{Koszul duality!perturbative/non-perturbative dichotomy|textbf}
\index{perturbative!Lagrangian intersection characterization}
\index{non-perturbative!excess intersection}
The Koszul/non-Koszul dichotomy is the mathematical form of the
perturbative/non-perturbative divide.  A Koszul algebra has a
\emph{clean} Lagrangian self-intersection: the derived intersection
...
A non-Koszul algebra has an \emph{excess}
