...
A three-dimensional holomorphic--topological (HT) field theory on
$\C_z \times \R_t$ has local observables that factorize
holomorphically in~$z$ and associatively in~$t$.  The governing
structure is the two-colored Swiss-cheese operad~$\SCchtop$ with
operation spaces $\FM_k(\C) \times E_1(m)$: a point in
$\FM_k(\C)$ records the collision pattern of~$k$ holomorphic
insertions, while a point in $E_1(m)$ records the linear ordering
of~$m$ topological insertions.  Its algebras are the
boundary theories of 3d~HT QFT; the primitive datum is the
...
a bordified curve --- the category itself, not any particular algebra
...
generator~$b$, just as a coordinate chart presents a manifold from
...
deformations of a vacuum object inside it.
...
The boundary algebra determines the open sector; the question is what
determines the \emph{bulk} --- the closed-sector observables that
restrict to the boundary but are not generated by it.  The answer is
the categorical derived center: the endomorphism algebra of the
identity functor of~$\cC$, which is the universal object that acts on
every module.
...
\item \textbf{Maurer--Cartan element.}
  ...
  at the chain level.  The shadow tower projections
...
arity-graded equations on the shadow invariants.
\end{proof}
...
\textup{(}$10{+}1{+}1$ conditions\textup{)} classifies when the algebraic presentation works.
\end{remark}
...
$\cA^!_{\mathrm{line}}$ is an $\SCchtop$-algebra
