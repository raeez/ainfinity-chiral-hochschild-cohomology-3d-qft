...
Volume~I constructed the bar complex $\barB^{\mathrm{ch}}(\cA)$
of a chiral algebra~$\cA$ on a curve~$X$.  As a graded vector
space it is the tensor coalgebra $T^c(s^{-1}\bar{\cA})$; as a dg
object it carries two structures:
...
This product structure is not a coincidence.  It is the operadic
fingerprint of a three-dimensional holomorphic--topological (HT)
quantum field theory on $\C_z \times \R_t$, where local
observables factorize holomorphically in~$z$ and associatively
in~$t$.  The mathematical framework is a two-colored Swiss-cheese
operad $\SCchtop$ whose operation spaces are exactly
$\FM_k(\C) \times E_1(m)$.  The central claim of this volume:
\emph{the bar complex of Volume~I, equipped with its coproduct, is
an algebra over~$\SCchtop$.}  The bar differential is the closed
(holomorphic) color; the bar coproduct is the open (topological)
color; the no-open-to-closed rule reflects the physical
directionality that bulk interactions can restrict to boundaries
but not conversely.

A deeper fact, proved in Volume~I
(Theorem~\ref*{thm:R-fact-canonical} of
Chapter~\ref*{ch:en-koszul-duality}), constrains the scope of this
claim: the $\R$-factorization is not an independent structure
layered on top of the chiral bar complex.  It is \emph{determined}
by the modular homotopy type of~$\cA$.  The argument is that the
...
The $\R$-factorization is a shadow of the modular homotopy type
projected onto the topological direction.  Given the holomorphic
data (factorization on $\operatorname{Ran}(X)$), the bar-cobar
adjunction produces a canonical tensor coalgebra model, and the
$\Eone$-coalgebra structure is automatic.  The bar construction
is the \emph{universal boundary condition}: the cofree resolution
of~$\cA$ viewed from the~$\R$-direction.  Volume~II develops the
consequences of this universality across three dimensions; the
Swiss-cheese structure it studies is not an ansatz but a theorem.
\end{principle}
...
locally constant factorization algebras on~$\mathbb R$ with
...
The key structural fact is that the operation spaces of
...
\medskip\noindent
\textbf{Conclusion.}
Assembling the three items via the product decomposition of the
$W$-resolved operation spaces yields the equivalence between~(i)
and~(ii).\qedhere
\end{proof}
...
operads~\cite{BM03,CM14}.  The key enabling fact is the
\emph{product decomposition} of the operation spaces of
$\mathsf{SC}^{\mathrm{ch,top}}$ together with the \emph{retract
property} of the closed color.  These two structural features reduce
the two-colored recognition to the product of two single-colored
recognitions plus a compatibility datum, and the compatibility datum
is precisely condition~(ii)(c).
\end{remark}
