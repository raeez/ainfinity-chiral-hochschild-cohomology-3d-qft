...
\item The shadow tower projections
  ...
  \textup{(}Volume~I, Appendix~\textup{\ref{app:nonlinear-modular-shadows}}\textup{)}.
\end{enumerate}
\end{theorem}
...
$\mathrm{C}^\bullet_{\mathrm{ch}}(A_b, A_b)$
(Volume~I, Theorem~\ref{thm:thqg-brace-dg-algebra}).
...
(Volume~I, Theorem~\ref{thm:mc2-bar-intrinsic}), which lives
...
to each arity (Volume~I, Appendix~\ref{app:nonlinear-modular-shadows}).
\end{proof}
...
For the affine lineage $\cA = \hat{\mathfrak{g}}_k$, the modular
characteristic is $\kappa = k$ (the level), and
$\omega_g = c_1(\lambda)$ where
$\lambda = \det R\pi_* \omega_{C/\overline{\mathcal M}_g}$ is the
Hodge bundle.  Thus $\mathcal{L}_k = (\det \lambda)^{\otimes k}$,
the $k$-th power of the determinant of the Hodge bundle---the
standard line bundle governing conformal blocks at level~$k$.
