...
The higher multiplications $m_k$ constructed above are defined by integrating
renormalized propagators over $\FM_k(\C) \times E_1(m)$
(\S\ref{subsec:mk-integrals}), but we have not yet verified that
these integrals satisfy the $A_\infty$ Stasheff identities.  That
verification requires a detailed analysis of the boundary structure
of $\FM_k(\C)$: the codimension-1 faces encode compositions
$m_i \circ m_j$, and the codimension-2 corners must cancel via
Arnold--Orlik--Solomon relations.  Section~\ref{sec:FM_calculus}
develops this Fulton--MacPherson calculus, proving the Stasheff
identities by Stokes' theorem on the compactified configuration space.
\end{remark}
