...
% BP central charge c^BP(k) = -((2k+3)(3k+1))/(k+3) at
...
The Bershadsky--Polyakov algebra $\mathrm{BP} = \cW^{(2)}(\fsl_3) = \cW^{k}(\fsl_3, f_{\min})$ is the minimal $\cW$-algebra of $\fsl_3$, obtained by quantum Hamiltonian reduction along the minimal nilpotent $f_{\min} \in \fsl_3$. Its generating fields are a stress tensor $T$ of conformal weight $2$, a pair of supercurrents $G^{\pm}$ of conformal weight $3/2$, and a $\fu(1)$-current $J$ of conformal weight $1$. The Kazhdan grading~$\Gamma$ of $f_{\min}$ in~$\fsl_3$ has denominator $d_f = 2$, so the short-root ghost currents attached to~$G^{\pm}$ live at half-integer Kazhdan weight and carry non-integer conformal weight $3/2$.

The fractional-weight ghost obstruction is resolved cohomologically in Vol~II
\cite[\S\ref*{sec:fractional-ghost-platonic}]{Vol2-fractional-ghost-platonic}
via the branched-cover
\[
 \pi \colon \widetilde X \to X, \qquad \pi(\tilde z) = \tilde z^{2}, \qquad
 \mathrm{Deck}(\pi) = \Z/2.
\]
Pulling the Kazhdan grading to~$\widetilde X$ integralises it; descending via $\Z/2$-invariants returns statements to~$X$. That route closes the cohomological $\Ethree^{\mathrm{top}}$ statement for BP on the original complex on~$X$ via the general good-graded theorem \cite[Theorem~\ref*{thm:E3-topological-DS-general-all-good-graded}]{Vol2-fractional-ghost-platonic}. The cohomological statement uses
\[
T_{\mathrm{DS}}(f)=[Q_{\mathrm{DS},f},G'_{\mathrm{DS},f,X}]
\]
on \(Q_{\mathrm{DS},f}\)-cohomology on~$X$; the strict chain-level
statement asks for the same identity on cochains in the DS
bulk--boundary reduction complex.

The strict chain-level lift descends from the double cover. On $\widetilde X$, the pulled-back Kazhdan grading is integer, the ghost system is integer-weight, and the chiral algebra at pulled-back level $\pi^{\ast} k$ belongs to class~$L$. For class~$L$ the strict chain-level antighost identity
\[
...
\]
holds unconditionally, by the closed-form $\eta_1 = \eta_1^{(\mathrm i)} + \eta_1^{(\mathrm{ii})}$ construction of \cite[Theorem~\ref*{V1-thm:sugawara-antighost-primitive-chain-level}]{Vol1-class-L-antighost}. The class-L Vol~I argument upstairs on the degree-$2$ cover, followed by descent of the corrected antighost $\widetilde G_1^{\widetilde X}$ to~$X$ via Galois invariants (preserving chain-level identities by exactness of $(\Z/2)$-invariants in characteristic $0$, Maschke), supplies a strict chain-level primitive $\widetilde G_{\mathrm{BP},1, X}$ for $T_{\mathrm{BP}}$ on the original BRST complex on~$X$.

The composition of class-L Wick absorption with Galois descent operates on the original complex, both for the existence of $\eta_1$ and for the descent of $\widetilde G_1$. The descended gauge correction $\eta_{\mathrm{BP}, 1, X}$ takes a closed-form expression in the four short-root ghost currents of BP's minimal-nilpotent ghost system, evaluable at specific levels. As a corollary, the class~M entry of Vol~I Theorem~\ref{V1-thm:topologization-tower} strengthens for BP: $\Ethree^{\mathrm{top}}$ holds on the original complex, not the weight-completed category.

\begin{remark}[Content beyond the cohomological reconstitution]
\label{rem:bp-new}
Compared with the cohomological $\Ethree^{\mathrm{top}}$ statement for BP of \cite[Theorem~\ref*{thm:E3-topological-DS-general-explicit-BP}]{Vol2-fractional-ghost-platonic} and the weight-completed class-M entry of Vol~I Theorem~\ref{V1-thm:topologization-tower}:
\begin{enumerate}
\item the descended gauge correction $\eta_{\mathrm{BP}, 1, X}$ admits closed form as the $\Z/2$-Galois-invariant trace of $\pi^{\ast}\eta_1$ on~$\widetilde X$, with concrete expression in the four short-root ghost currents $\bar c_{\pm \alpha_1}, \bar c_{\pm \alpha_2}$ and the Cartan ghost $\bar c_{h_1}$;
\item the strict chain-level identity $[Q_{\mathrm{tot}}, \widetilde G_{\mathrm{BP}, 1, X}] = T_{\mathrm{BP}}$ holds on the original BRST complex on~$X$ in $\cO_{\mathrm{bulk}}$, not on $Q$-cohomology, not in the weight-completed category, not on a quasi-isomorphic transfer model;
\item the Galois-descent preservation lemma for strict identities (Maschke $+$ Deck-invariance of $Q_{\mathrm{tot}}^{\widetilde X}$) shows that chain-level identities descend from covers;
\item the class~M entry of Vol~I Theorem~\ref{V1-thm:topologization-tower} strengthens for BP from weight-completed to original complex.
\end{enumerate}
\end{remark}

\begin{remark}[anchor: BP self-duality bridge]%
\label{rem:anchor-bp-self-duality}%
\index{Bershadsky--Polyakov!central-charge conductor}%
...
$K^{c}_{\mathrm{BP}} \equiv 50$ for the BP central charge
$c_{\mathrm{BP}}(k) = 25 - 6(k+3) - 24/(k+3)
= -(2k+3)(3k+1)/(k+3)$
(the Fateev--Lukyanov screening normalization
\eqref{eq:bp-central-charge-FL}, with $c_{\mathrm{BP}}(0) = -1$),
proved at Vol~I
\texttt{standalone/bp\_self\_duality.tex},
\textsf{thm:bp-koszul-conductor-polynomial}, supplies the
Koszul-pair packaging on the boundary side. Writing $t = k+3$, the
level-dependent part $-6t - 24/t$ is odd under the Feigin--Frenkel
involution $t \mapsto -t$ (i.e.\ $k \mapsto -k-6$), so
\[
 K^{c}_{\mathrm{BP}}(k)
...
conductor is the polynomial constant $50$, not a meromorphic
function. The ghost anomaly independently confirms criticality at
$50$: the BRST $bc$-systems for the four strong generators of
conformal weights $(2, 3/2, 3/2, 1)$ contribute
$c_{\mathrm{gh}}(h) = -2(6h^2-6h+1)$ each, hence
$-26 - 11 - 11 - 2 = -50$ in total, cancelling
$K^{c}_{\mathrm{BP}}$ exactly. The chain-level antighost identity
above is insensitive to the central-charge packaging (both sides of
\eqref{eq:bp-central-charge-FL} pull back to $\widetilde X$ via the
same Kazhdan-grading integralisation), so the universal-holography
reading on the BP fibre is conducted directly in the
Fateev--Lukyanov normalization; cross-volume readers should consult
Vol~I \texttt{standalone/bp\_self\_duality.tex} for the polynomial
conductor.
\end{remark}
...
\cite{Bershadsky91, Polyakov90, deBoerTjin93, KacRoanWakimoto03,
 Arakawa05BP}.
In the Fateev--Lukyanov screening normalization the central charge is
\begin{equation}\label{eq:bp-central-charge-FL}
 c^{\mathrm{FL}}_{\mathrm{BP}}(k)
 \;=\; -\,\frac{(2k+3)(3k+1)}{k+3}
 \;=\; -\,\frac{6k^2 + 11k + 3}{k+3}.
\end{equation}
\end{definition}
...
structure of Vol~I Theorem~\ref{V1-thm:sugawara-antighost-primitive-chain-level}
...
(Definition~\ref{V1-def:no-cfg} of
\cite[\S\ref*{V1-sec:setup-brst-affine-km}]{Vol1-class-L-antighost}).
...
\cite[equations \texttt{eq:T-sug}--\texttt{eq:R-self}]{Vol1-class-L-antighost}
...
Vol~I's
Proposition~\ref{V1-prop:QG1-remainder} at
\cite[\S\ref*{V1-sec:G1}]{Vol1-class-L-antighost}
...
\begin{proof}
\emph{Class-L Wick absorption upstairs.}
By
...
Theorem~\ref{V1-thm:sugawara-antighost-primitive-chain-level} of
\cite[\S\ref*{V1-sec:strict-chain-level}]{Vol1-class-L-antighost}
...
\end{align*}
follow from Propositions~\ref{V1-prop:eta-i-primitive} and
\ref{V1-prop:eta-ii-primitive} of
\cite[\S\ref*{V1-sec:eta1}]{Vol1-class-L-antighost} applied verbatim
...
\emph{Cartan improvement on cochains.}
The BP improvement
...
sums involve only closed OPE contractions),
\[
 [Q_{\mathrm{tot}}^{\widetilde X},\,
 \partial_{\tilde z}\pi^{\ast}\bar c_{h_j}(\tilde z)]
 \;=\; \partial_{\tilde z}\pi^{\ast}J_{h_j}(\tilde z),
\]
which, contracted with $(x_0)_i (\kappa^{-1})^{h_i h_j}$ and summed, gives
...
\emph{Neutral-element simplification for BP.}
The neutral
...
\eqref{eq:bp-improvement-upstairs}. Combined with the Cartan
improvement, this
...
\eqref{eq:bp-strict-chain-DS-upstairs} sits on~$\widetilde X$ in the
pulled-back BV complex. The corresponding statement on~$X$ on the
original BP complex follows by taking $\mathrm{Deck}(\pi) = \Z/2$-invariants;
strict (chain-level, not cohomological) identities descend.
...
is built from three types of terms, each verified for
$\Z/2$-invariance below.
...
descent, $D^{(1)}_n = 0$ downstairs for all $n \ge 3$. The
obstructions to coherence live upstairs, where the algebra is
class~L, and exact descent does not reintroduce them; BP on the
original complex carries strict chain-level
$\Ethree^{\mathrm{top}}$.
...
\begin{remark}[Strict chain-level versus weight-completed]
\label{rem:bp-chain-vs-weight-completed-sharp}
Theorem~\ref{thm:bp-chain-level-e3-topological} strengthens the
class~M entry of Vol~I Theorem~\ref{V1-thm:topologization-tower} for
BP. The Vol~I theorem establishes chain-level
$\Ethree^{\mathrm{top}}$-structure on the weight-completed category
for class M (Virasoro, $\cW_N$), via the MC5 harmonic mechanism and
the weight-completed BV-bar identity
prop:bv-bar-class-m-weight-completed. The present theorem reaches the
same conclusion for BP on the original BRST complex on~$X$, without
weight-completion. The mechanism routes through the double cover:
upstairs BP-with-pulled-back-Kazhdan is class~$L$ (finite shadow
depth $r_{\max} = 3$), for which the class-L antighost closure of
Vol~I gives strict chain-level identities unconditionally; Galois
descent preserves these identities at the chain level by Maschke
exactness. The class~M direct-sum argument alone does not reach the
strict chain-level conclusion on the original complex.
\end{remark}

\section{The BP Kappa-Conductor Identity as a Polynomial in~$k$}
\label{sec:bp-kappa-conductor}

\begin{proposition}[BP central-charge conductor; the $\kappa$-complementarity slot is open]
\label{prop:bp-kappa-conductor}
\index{Bershadsky--Polyakov algebra!Koszul conductor identity|textbf}
\ClaimStatusProvedElsewhere
The BP central charge in the Fateev--Lukyanov screening
normalization~\eqref{eq:bp-central-charge-FL},
\[
c_{\mathrm{BP}}(k)
\;=\; 25 - 6(k+3) - \frac{24}{k+3}
\;=\; -\,\frac{(2k+3)(3k+1)}{k+3},
\qquad c_{\mathrm{BP}}(0) = -1,
\]
has level-independent Feigin--Frenkel self-contraction
\begin{equation}\label{eq:bp-kappa-conductor}
 K^{c}_{\mathrm{BP}}(k)
 \;\coloneqq\; c_{\mathrm{BP}}(k) + c_{\mathrm{BP}}(-k-6)
 \;\equiv\; 50
 \qquad \text{in } \mathbb{Q}(k),
\end{equation}
where $\mathrm{BP}_{-k-6} = \cW^{-k-6}(\fsl_3, f_{\min})$ is the
operadic Koszul-dual BP-algebra at the Feigin--Frenkel-shifted
level (Vol~I
Theorem~\ref{V1-thm:bp-koszul-conductor-polynomial}). The ghost
anomaly gives the same constant with opposite sign: the BRST
$bc$-systems for the strong generators of conformal weights
$(2, 3/2, 3/2, 1)$ contribute $-26 - 11 - 11 - 2 = -50$, so
$K^{c}_{\mathrm{BP}} = -c_{\mathrm{ghost}}(\mathrm{BRST})$.

The derived-centre $\kappa$-complementarity sum
$\kappaChHodge(\mathrm{BP}_k) + \kappaChHodge(\mathrm{BP}_{-k-6})$
is a distinct invariant of the same Koszul pair — a chiral-Hodge
supertrace, not a central-charge self-contraction — and its value
is \emph{open}: no derivation of the BP $\kappa$-sum exists, and the
Vol~I landscape census records the BP slot of the complementarity
spectrum as open. The computed spectrum is
$\{0, 13, 250/3\}$ ($\mathsf{G}/\mathsf{L}/\mathsf{C}$ and the
$\mathsf{B}$-row Hodge sum give $0$; $\mathrm{Vir}_c$ gives $13$;
$\cW_3$ gives $250/3$), together with the open BP slot.
\end{proposition}

\begin{proof}
Write $t = k+3$, so $c_{\mathrm{BP}}(k) = 25 - 6t - 24/t$ and the
Feigin--Frenkel involution $k \mapsto -k-6$ is $t \mapsto -t$. The
level-dependent part $-6t - 24/t$ is odd in $t$, so
\[
 K^{c}_{\mathrm{BP}}(k)
 ...
 = 50,
\]
an identity of rational functions; the pole at $k=-3$ is removable
because the two summands carry equal-and-opposite singular parts.
The ghost cross-check uses $c_{\mathrm{gh}}(h) = -2(6h^2-6h+1)$ per
weight-$h$ $bc$-pair: $h = 2$ gives $-26$, $h = 3/2$ gives $-11$
(twice), $h = 1$ gives $-2$, total $-50$. The full symbolic
verification is recorded in Vol~I
\Cref{V1-thm:bp-koszul-conductor-polynomial}.

For the $\kappa$-complementarity slot: no bridge identity relating
$\kappaChHodge + \kappaChHodge^!$ to $K^{c}_{\mathrm{BP}}$ has been
derived for BP. The volume's generator-profile convention
$\sum_h 1/h$ over the strong-generator weights $(1, 3/2, 3/2, 2)$
evaluates to $17/6$, but no theorem identifies this (or any other
coefficient) as the ratio of the two invariants; the slot remains
open pending a first-principles computation of
$\kappaChHodge(\mathrm{BP}_k)$.
\end{proof}

\begin{remark}[Role of $K^{c}_{\mathrm{BP}} = 50$ in the present chapter]
\label{rem:bp-role-of-K-50}
The central-charge conductor $K^{c}_{\mathrm{BP}} = 50$ does not enter the
...
to the Koszul conductor. The conductor is relevant, however,
...
theorem and the Koszul conductor identity are complementary pieces
...
\ClaimStatusProvedHere
The class-M entry of Vol~I Theorem~\ref{V1-thm:topologization-tower} for BP
strengthens from the weight-completed category to the original
complex. Vol~I establishes chain-level $\Ethree^{\mathrm{top}}$ for BP
in the weight-completed category via the MC5 harmonic mechanism
(prop:bv-bar-class-m-weight-completed); Theorem~\ref{thm:bp-chain-level-e3-topological}
establishes chain-level $\Ethree^{\mathrm{top}}$ for BP on the
original complex via the branched-cover, class-L absorption, and
Galois descent route.
\end{corollary}

\begin{proof}
The class-M entry of Vol~I Theorem~\ref{V1-thm:topologization-tower} gives
...
\section{Three disjoint derivations}
\label{sec:bp-independent-verification-decorators}

The strict chain-level identity admits three pairwise-disjoint
verification routes attached to
Theorem~\ref{thm:bp-chain-level-e3-topological}. Each operates on
input independent of the others.

\begin{remark}[Kac--Roan--Wakimoto 2003 BRST cohomology]
\label{rem:bp-decorator-D1-KRW}
\index{BP verification!BRST cohomology (Kac-Roan-Wakimoto)}
The BRST cohomology computation for W-algebras by Kac, Roan, and
Wakimoto~\cite{KacRoanWakimoto03} establishes, at the cohomological
level, the concentration of the DS reduction of an affine VA in a
single BRST degree at non-critical level, with explicit formulas for
the concentration degree and the induced stress tensor
$T_{\mathrm{DS}}(f)$. For BP, the KRW computation gives
$T_{\mathrm{BP}}$ as an element of $H^{0}(Q_{\mathrm{tot}})$ together
with its self-OPE (Virasoro algebra with central charge
$c^{\mathrm{FL}}_{\mathrm{BP}}$ in the present normalization). The route is algebraic-cohomological:
$Q_{\mathrm{tot}}$-cohomology via BRST techniques, without 3d HT bulk
or branched covers. Target: existence of $T_{\mathrm{BP}}$ with
central charge $c^{\mathrm{FL}}_{\mathrm{BP}}(k) = -(2k+3)(3k+1)/(k+3)$.
\end{remark}

\begin{remark}[de Boer--Tjin 1993 screened free-field]
\label{rem:bp-decorator-D2-dBT}
\index{BP verification!screened free-field (de Boer-Tjin)}
The de Boer--Tjin screened free-field
construction~\cite{deBoerTjin93} realises BP as a sub-VA of three
free bosons plus one free $\beta\gamma$ pair, generated as the
...
of Theorem~\ref{thm:bp-galois-descent} follows by direct Wick
contraction in the free-field realisation, with $Q_{\mathrm{tot}}$ the
sum of the two screening-charge residues. The route is
analytic-free-field: OPE computations in the free-field ambient,
without 3d HT bulk or abstract BRST cohomology. Target: symbolic
verification of the Wick identity for the descended antighost at
representative levels.
\end{remark}

\begin{remark}[Symbolic $\fsl_3$ Wick at $k = -3/2$]
\label{rem:bp-decorator-D3-SymPy}
\index{BP verification!symbolic structure-constant Wick}
Symbolic verification of the structural identities entering
Theorem~\ref{thm:bp-sugawara-antighost-strict-chain-level-on-branched-cover}
at $\fsl_3$ level $k = -3/2$ (a representative non-critical level for
BP) records: (i) total antisymmetry of $f_{abc}$ for $\fsl_3$ in the
Chevalley basis normalised to long-root length $\sqrt 2$; (ii) closure
of all $8 \cdot 8 \cdot 8 = 512$ Jacobi identities (reduced modulo
symmetry of the three-index antisymmetriser to a smaller irreducible
set of non-trivial Jacobi closures, each checked by direct
structure-constant substitution); (iii) the value of the Sugawara
prefactor $1/(2(k+h^{\vee})) = 1/(2(-3/2 + 3)) = 1/3$; (iv) the value
of the BP central charge
$c^{\mathrm{FL}}_{\mathrm{BP}}(-3/2) = -(0)(-7/2)/(3/2) = 0$ at the boundary level,
and $c^{\mathrm{FL}}_{\mathrm{BP}}(0) = -(3)(1)/3 = -1$ at generic level $k = 0$.
The route is purely symbolic-algebraic: structure constants and
trace-form normalisation, without 3d HT bulk, BRST cohomology, or
free-field realisation. Target: numerical values of structural
constants confirmed at representative levels.
\end{remark}

\begin{remark}[Pairwise disjointness of the three routes]
\label{rem:bp-disjointness}
The three verification routes share no input. The BRST cohomology
route operates on $Q_{\mathrm{tot}}$-cohomology via Koszul/BRST
techniques; the screened free-field route operates on free-field OPEs
via screening-charge residues; the symbolic route operates on abstract
structure constants of $\fsl_3$. The pairwise intersection of
derivation input is empty.
\end{remark}

\begin{remark}[Symbolic finite-level check]
\label{rem:bp-compute-module}
The finite-level symbolic check uses the class-L antighost closure,
the branched-cover Galois descent, Kac--Roan--Wakimoto BRST
cohomology, the de Boer--Tjin screened free-field realisation, and
the $\fsl_3$ Wick calculation at $k = -3/2$. The three verification
routes operate on disjoint derivation paths.
\end{remark}

\section{Bershadsky--Polyakov chain-level topologisation}
...
non-critical level $k \ne -3$, on the original BRST complex on~$X$.
The proof composes three components:
...
The Koszul conductor identity
$K^{c}_{\mathrm{BP}}(k)
=c_{\mathrm{BP}}(k)+c_{\mathrm{BP}}(-k-6)\equiv 50$, with the
derived-centre $\kappa$-complementarity slot
$\kappaChHodge(\mathrm{BP}_k) + \kappaChHodge(\mathrm{BP}_{-k-6})$
open (Proposition~\ref{prop:bp-kappa-conductor}), complements this
structural result with the Koszul-dual pair datum; together they
assemble the full BP holographic package. The corollary
(Corollary~\ref{cor:bp-E3-original-complex-not-just-weight-completed})
strengthens Vol~I Theorem~\ref{V1-thm:topologization-tower} class-M entry for
...
Three prior structures supply the inputs: class-L antighost closure
for affine $V_k(\fg)$ at non-critical level upstairs, branched-cover
integralisation with Galois descent for good $(1/d_f)$-graded
nilpotents, and the general $E_\infty$-topologisation theorem. The
class-M-adjacent BP strengthening is its non-principal
minimal-nilpotent instance.

The independent verifications
(\S\ref{sec:bp-independent-verification-decorators}) draw on three
disjoint routes: the Kac--Roan--Wakimoto 2003 BRST cohomology, the
de Boer--Tjin 1993 screened free-field realisation, and the symbolic
$\fsl_3$ Wick at $k = -3/2$, pairwise disjoint per
Remark~\ref{rem:bp-disjointness}.
