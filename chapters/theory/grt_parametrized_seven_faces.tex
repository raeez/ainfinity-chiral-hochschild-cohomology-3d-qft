...
Seven literatures name a single element of the $E_1$ ordered
convolution algebra,
\[
 r(z) \;=\; \Res^{\coll}_{0,2}\bigl(\Theta_{\cA}\bigr)
 \;\in\; \cA^{!}\otimes\cA^{!}[\![z^{-1}]\!],
\]
in seven normalisations, seven Casimir bases, seven connection
conventions. Vol~I read the seven as vertices of a star and
proved a seven-way equivalence through the bar hub. The hub is
the origin of a torsor for the Drinfeld
Grothendieck--Teichm\"uller group $\GRT(K)$ acting by associator
transport. Each face is an associator-coordinate chart over a
coefficient field $K$; the seven rational finite-gauge
representatives admit two canonical companions, Brown's motivic
face $F_8$ and Willwacher's operadic face $F_9$, comparable after
scalar extension to a common coefficient field. The face space is
a scheme over $\GRT$, and ``seven'' is the rational finite-gauge
slice.

The torsor presentation closes four confusions. The
landscape-census chained equality
$k\cdot\Omega_{\mathrm{tr}}/z = \Omega/((k+h^{\vee})z)$ is a
Casimir rescaling in the finite inner gauge subgroup
$\GRT^{\mathrm{fin}}\subset\GRT$, a tensor identity between
normalisations, not a rational-function identity. The
F1$\leftrightarrow$F4 identification is the injection of $r(z)$
into Drinfeld associator data; the MZV tail
$\Phi_{\KZ}-\Phi_{\rat}$ is invisible to $r(z)$. The F7 entry
splits: F7 (Gaudin simple-pole) and F7$'$ (class-M top $A_\infty$
$m_3$) coincide on class L and separate on class M. The
Heisenberg-versus-symplectic-fermion asymmetry is a
$\mathbb{Z}/2$-factor on collision residues, absorbed into the
super-variant
$\Gsuper_K = \GRT(K)\rtimes(\mathbb{Z}/2)^{|\mathrm{odd}|}$.

Section~\ref{sec:grt-setup} defines $\Face_K(\cA)$ as the scheme
of chain-level presentations of $r(z)$ compatible with the
bar-intrinsic dGLA and establishes the canonical $\GRT$-torsor
structure. Section~\ref{sec:grt-nine} enumerates the nine faces
F1--F9. Section~\ref{sec:grt-torsor-thm} states the master
theorem: $\Face_K(\cA)$ is a $\GRT(K)$-torsor after a coefficient
field and formality base point are fixed. Section~\ref{sec:f1-f4}
proves the F1$\leftrightarrow$F4 injection and the MZV
invisibility. Section~\ref{sec:f7-disambig} separates F7 from
F7$'$ between class L and class M. Section~\ref{sec:grtfin}
reads the Casimir rescaling as a $\GRT^{\mathrm{fin}}$ gauge.
Section~\ref{sec:super} installs the super-variant.
Sections~\ref{sec:f8-motivic} and~\ref{sec:f9-willwacher}
install $F_8$ (Brown motivic) and $F_9$ (Willwacher operadic) as
higher cochain faces. Section~\ref{sec:f8-f9-dual} identifies
$F_8$ and $F_9$ as geometric and operadic incarnations of one
orbit after scalar extension.
...
Torsor statements sit over a characteristic-zero coefficient
field $K$ after a formality base point. $\Face_K(\cA)$ is the
chain-level face space after scalar extension to $K$; the acting
group is $\GRT(K)$. The historical faces $F_1,\ldots,F_7$ are
rational finite-gauge presentations where the transport is
binary or connection-level. Brown's face $F_8$ lives over the
motivic period algebra, or over its real period realisation;
Willwacher's face $F_9$ is defined over $\mathbb Q$ at the
graph-complex Lie-algebra level. The
$F_8\leftrightarrow F_9$ comparison is a comparison after scalar
extension to a common coefficient field; $F_8$ is not a rational
point of $\GRT(\mathbb Q)$.

The symbol $r(z)$ denotes the binary degree-two collision
residue. MZV and graph-complex data enter higher cochain
operations, starting with the arity-three twisting component
$\tau_{(3)}$ and the $A_\infty$ operation $m_3$; the cohomological
binary residue is invariant.
\end{convention}
...
Fix a chirally Koszul chiral algebra $\cA$ in the standard
landscape and a characteristic-zero coefficient field $K$
(Convention~\ref{conv:grt-coefficient-field}).
$\Theta_{\cA}^{E_1}\in\mathfrak{g}^{E_1}_{\cA}$ is the universal
Maurer--Cartan element in the ordered convolution algebra (Vol~I,
\S\text{shadow-tower}); the degree-$2$ collision residue is
\[
...
\end{enumerate}
The equivalence classes of such presentations after scalar
extension to $K$, modulo strict $\MC$-gauge equivalence in
$\mathfrak{m}$, form the \emph{face space} $\Face_K(\cA)$.
\end{definition}

\begin{remark}[Base point: $F_1$ as identity]
\label{rem:f1-base-point}
The bar hub $F_1$ — the twisting morphism
...
group; throughout the chapter $F_1$ is the identity coset.
\end{remark}

\begin{proposition}[GRT-action on $\Face_K(\cA)$]
\label{prop:grt-action-face}
After fixing $K$, a formality base point, and a chain model, the
group $\GRT(K)$ acts freely and transitively on the
associator-coordinate component of $\Face_K(\cA)$. The abstract
free-transitive action is Tamarkin--Drinfeld--Willwacher formality;
the assertion that a named face is a $K$-point is per-face, with
the coefficient-field scope of
Convention~\ref{conv:grt-coefficient-field}.
\end{proposition}

\begin{proof}
Tamarkin's theorem (Tamarkin 1998; Gerstenhaber--Voronov;
Willwacher 2015, Thm.~1.1) constructs a free transitive action of
$\GRT(K)$ on quasi-isomorphism classes of formality transfers
$E_2\xrightarrow{\sim} B(\Ass^{!})$. This action descends to the
bar-intrinsic dGLA of any associative chiral algebra through the
canonical functor $\Ass\to\ChirAss$. For $\Phi\in\GRT(K)$ and a
presentation $(\mathfrak{m},r,\nabla,\Phi_0)$ the action returns
the presentation with associator $\Phi\cdot\Phi_0$, Casimir basis
pulled back by the finite inner gauge of $\Phi$, and connection
convention $\nabla^\Phi = \Phi\cdot\nabla$. Freeness — $\GRT(K)$
acts without fixed points on formality transfers — is Drinfeld's
1991 Leningrad theorem. Transitivity is the pentagon axiom with
Tamarkin--Kontsevich formality on the associator-coordinate
component. Passage from an abstract associator coordinate to a
named face is per-$K$: F8 and F9 require the scalar extensions of
Convention~\ref{conv:grt-coefficient-field}.
\end{proof}
...
\begin{remark}[Torsor versus star]
\label{rem:torsor-vs-star}
Vol~I's seven-face programme drew the seven presentations as a
star with the bar hub at the centre, routing every pairwise
identification through $F_1$. The star is the accessibility
diagram; the groupoid of gauge transports between non-hub
vertices is the torsor. The edge $F_i\to F_j$ is an element
$\Phi_{ji}\in\GRT(K)$ after scalar extension to a common
coefficient field; the star is the subgraph of edges through
$F_1 = \id$.
\end{remark}

\begin{remark}[Star diagram and torsor action]
The seven-way equivalence of presentations is the
$\GRT(K)$-orbit relation. The bar hub is a base point; every
non-hub edge is an associator transport in the torsor.
\end{remark}
...
Table~\ref{tab:nine-faces} lists the nine face coordinates. Seven
are historical rational finite-gauge coordinates; F8 and F9 enter
as higher cochain coordinates after the scalar extensions of
Convention~\ref{conv:grt-coefficient-field}.
...
\begin{remark}[The cardinality nine]
The seven-face enumeration selects the coordinates writable
without motivic or graph-complex machinery. Brown's motivic face
$F_8$ and Willwacher's operadic face $F_9$ enter as higher cochain
coordinates at the generators of $\fgrt = \mathrm{Lie}(\GRT)$.
Truncation to seven preserves the binary residue $r(z)$ and hides
the associator-dependent tower.
\end{remark}
...
$\GRT$-torsor over a fixed coefficient field]
...
Corollary~\ref{cor:face-torsor}. The rational finite-gauge claim
on the binary historical faces names the explicit transport from
$F_1$ after the Casimir and connection convention are fixed:

\smallskip\noindent
\emph{$\Phi_{12}$ (Bar $\to$ DNP $R$-twist).} The DNP $R$-twist
acts on the Drinfeld spectral coproduct,
\[
\Delta_z^R = R(z)\Delta_z R(z)^{-1},\qquad
R(z) = 1 + \hbar r(z) + O(\hbar^2).
\]
The bar deconcatenation coproduct retains the
topological-factorisation structure. At order $\hbar$ the twist is
the identity on the associator: $\Phi_{12}\in\GRT^{\mathrm{fin}}$.

\smallskip\noindent
\emph{$\Phi_{13}$ (Bar $\to$ KZ PVA).} The KZ convention writes
$r(z) = \Omega/((k+h^\vee)z)$ in the Killing--Casimir basis; the
transport is the Casimir rescaling with simple-pole shift
$k\mapsto k+h^\vee$, both in $\GRT^{\mathrm{fin}}$
(Section~\ref{sec:grtfin}).

\smallskip\noindent
\emph{$\Phi_{14}$ (Bar $\to$ GZ26).} The Gaiotto--Zeng commuting
differentials are the Laplace transform of the Fay kernel in the
$r$-matrix presentation; the Laplace transform is the identity on
the associator. Commutativity is flatness of $\nabla^\Phi$ on the
sphere (Section~\ref{sec:f1-f4}).

\smallskip\noindent
\emph{$\Phi_{15}$ (Bar $\to$ Yangian).} The classical limit
$R_{\mathrm{Yang}}(u)-1 = \frac{\hbar P}{u}+O(\hbar^2)$ is a
rational function in $u$ with no leading MZV data, so
$\Phi_{15}\in\GRT^{\mathrm{fin}}$.

\smallskip\noindent
\emph{$\Phi_{16}$ (Bar $\to$ Gaudin).} The Gaudin residue
$H_i^{\Gaud} = \sum_{j\neq i}\Omega_{ij}/(z_i-z_j)$ is the
simple-pole reduction of $F_1$'s binary-degree component;
$\Phi_{16}$ is the finite inner gauge of the simple-pole
truncation.

\smallskip\noindent
\emph{$\Phi_{17}$ (Bar $\to$ STS; $F_7'$ top $m_3$ separately).}
On class G/L the Sklyanin tensor sits in
$\mathrm{Sym}^2(\fg)^{\fg}\otimes \mathbb{Q}((z))$; its associator
is a finite rational twist, $\Phi_{17}\in\GRT^{\mathrm{fin}}$. On
class M the additional entry is $F_7'$ — the top $A_\infty$
operation — whose transport carries MZV-coefficient corrections
at weight $\geq 3$ in the higher cochain tower
(Section~\ref{sec:f7-disambig}).

\smallskip
Uniqueness of $\Phi_{ij}$ follows from freeness of the
$\GRT$-action (Proposition~\ref{prop:grt-action-face}) after
scalar extension to a common coefficient field. The motivic and
operadic associators $\Phi_{18},\Phi_{19}$ enter in
Sections~\ref{sec:f8-motivic} and~\ref{sec:f9-willwacher} under
Convention~\ref{conv:grt-coefficient-field}.
\end{proof}

\begin{remark}[Star diagram and torsor transports]
Vol~I's seven-way chain
$F_1\Leftrightarrow F_2\Leftrightarrow\cdots\Leftrightarrow F_7$
is, on the binary historical slice, the statement that the finite
transports $\Phi_{1i}$ belong to $\GRT^{\mathrm{fin}}$. The
transports are associator-valued; the torsor statement separates
the higher cochain faces from the binary residue.
\end{remark}
...
the bridge between them exists after that convention
change.
\end{proof}

\begin{remark}[Casimir rescaling as tensor identity]
The tensor identity and the level rescaling are distinct gauge
transformations. Proposition~\ref{prop:grtfin-casimir} separates them:
Casimir rescaling is the tensor identity, while level rescaling is the
Sugawara/KZ connection convention. The two presentations are comparable
only after both moves are named; they are not the same Casimir in the
same level convention.
\end{remark}
...
The construction upgrades the Vol~I seven-face programme to a
...
Theorem~\ref{thm:seven-faces-as-GRT-torsor}, under the
coefficient-field and concrete-face hypotheses of
Convention~\ref{conv:grt-coefficient-field}. The four compatibility
statements are:
...
These three anchors are independent verification sources for
Theorems~\ref{thm:seven-faces-as-GRT-torsor},
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Associator action and slab-bimodule}
\label{sec:grt-qualifiers}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The $\GRT$-torsor master Theorem~\ref{thm:seven-faces-as-GRT-torsor}
has three ambient hypotheses: fixed coefficient field, fixed
formality base point, and diagonal slab-bimodule restriction. These
hypotheses determine the precise content of the torsor statement.
...
not included in this torsor construction. See
...
recovers the diagonal $\GRT(K)$-torsor on
$\Face_K(\cA, \cA) = \Face_K(\cA)$ after scalar extension.
\end{remark}

\begin{remark}[Compareerences to the climax]
...
\item[(c)] \emph{Volume~III $\Phi$ functor.} The Volume~III
 ...
 $Y_{\hbar}(\mathfrak{so}(4,20))$, with the
 $Y_{\hbar}(\mathfrak{so}(4|20))$ Hodge-parity refinement under its
 parity hypothesis), $\bC^{3}$
