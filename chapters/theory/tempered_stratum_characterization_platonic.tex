...
\section*{Tempered-Stratum Theorem}

Class~$\mathsf{M}$ at fixed non-zero $\kappaChHodge(\cA)$, non-critical
level, and $g \ge 2$ partitions into \emph{tempered} and
\emph{non-tempered} strata by the hypothesis
\[
 (\mathrm{H}_S) \qquad
 \limsup_{r \to \infty}
 ...
 0,
\]
with the chain-level $E_3$-topological structure on the original
complex established under $(\mathrm{H}_S)$ in
Theorem~\ref{thm:chain-level-e3-original-complex-conditional}. For
every generic Virasoro algebra the factorially normalised root test
gives zero, while the unweighted Laurent series has explicit
convergence radius $\rho_*(c)=|c|/6$.

The principal theorems read as follows.
\emph{(Universal Virasoro tempering.)} For every central charge
$c \in \bbR$ with $c \notin \{0\} \cup \cD^{\mathrm{sev}}_{\mathrm{Kac}}$,
the shadow tower of $\mathrm{Vir}_c$ satisfies
...
generic Virasoro algebra; the Banach $E_3$-model holds on this
locus, with raw direct-sum descent governed by the
...
converges at every $\rho < \infty$ under temperedness; the bare
geometric convergence radius of the Laurent series is
$\rho_*(c) = |c|/6$.

\section{The ratio test for the Virasoro shadow tower}
\label{sec:ratio-test}

A direct d'Alembert ratio test on the shadow sequence
$\{S_r(\mathrm{Vir}_c)\}_{r \ge 2}$ together with
\textsf{thm:shadow-tower-asymptotic-closed-form} of Vol~I, which supplies the
closed form of the leading Laurent coefficient
$A_r = \lim_{c \to \infty} c^{r-2} S_r(\mathrm{Vir}_c) =
8 (-6)^{r-4} / r$, settles the limsup.
...
\ref{V1-thm:shadow-tower-tier-4-closed-form}. Each subleading tier
...
\begin{remark}[Finite-$r$ transition window]
\label{rem:virasoro-finite-r-window}
The transitional-window bound
(Proposition~\ref{prop:virasoro-finite-r-small-c-bound}) completes
the finite-$r$ small-$|c|$ analysis. The tempering conclusion
...
$\lvert S_r(\mathrm{Vir}_c) \rvert \ge K(c) r^{-1/c}$ leaves the
Stirling denominator $(r!)^{1/r}\sim r/e$ uncancelled:
\[
 (K r^{-1/c} / r!)^{1/r}
 \;=\;
 K^{1/r} \cdot r^{-1/(c r)}
 ...
 0.
\]
A factorial lower bound $\lvert S_r \rvert \gtrsim r! / e^r$ is what
$\limsup (S_r / r!)^{1/r} = 1/e$ requires. The shadow tower of generic
Virasoro satisfies the Laurent upper bound
$\lvert S_r \rvert \le C(c) \cdot r \cdot 6^r / c^r$
(Proposition~\ref{prop:virasoro-s-r-upper-bound}), so
\[
 \lvert S_r \rvert / r!
 \;\le\;
 ...
 \cdot r^{-r} \to 0
\]
super-polynomially in $r$, at every $c > 0$.
\end{remark}
...
generic Virasoro algebra. The Banach $E_3$-model holds for every such
$\mathrm{Vir}_c$; descent to the raw direct-sum curved-Dunn complex
is the finite-propagation condition of
Theorem~\ref{thm:chain-level-e3-original-complex-conditional}.
\end{theorem}
...
This is Definition~\ref{def:analytically-tempered-class-m}; the
analytic hypothesis $(\mathrm{H}_S)$ in
Theorem~\ref{thm:chain-level-e3-original-complex-conditional} is
discharged. Raw direct-sum descent is the finite-propagation
assertion of that theorem.
\end{proof}

\begin{remark}[Virasoro factorial obstruction]
\label{rem:retraction-1-over-e}
Theorem~\ref{thm:tempered-stratum-contains-virasoro} gives
\[
 ...
 0,
\]
via the Stirling factor $(r!)^{-1/r}\sim e/r\to0$. The unnormalised
ordinary radius is
$\rho_*(c)=|c|/6$ by
Proposition~\ref{prop:virasoro-rho-star-closed-form}; the factorially
normalised obstruction vanishes. The Banach model carries the
analytic tempering input; raw direct-sum descent is the
...
$\lvert\lvert\lvert \cdot \rvert\rvert\rvert_{\mathrm{KS}, \rho}$ of
Definition~\ref{def:kac-shapovalov-norm} has finiteness governed by
two distinct radii: the \emph{exponential-generating} radius
$R^{\mathrm{exp}}(c) = \infty$ already established, and the
\emph{ordinary-generating} radius $\rho_*(c)$ of the unweighted series
$\sum_r S_r \rho^r$.
...
\begin{remark}[Geometric content of $\rho_*(c) = |c|/6$]
\label{rem:rho-star-geometric-meaning}
The radius $\rho_*(c) = |c|/6$ has three readings.

\textbf{(i) Large-$c$ semiclassical.} $\rho_*(c) \to \infty$ as
$c \to \infty$: the shadow tower is asymptotically small, the Banach
norm is finite at every radius, and the original complex agrees with
the weight-completed complex at the Banach level.

\textbf{(ii) Small-$c$ strong coupling.} $\rho_*(c) \to 0$ as
$c \to 0^+$: the shadow tower grows, the Banach norm is finite on an
$o(c)$ radius, and the original complex couples most strongly to its
weight completion.

\textbf{(iii) Virasoro scaling constant.} The factor $6$ is the
absolute value of the leading-Laurent recursion ratio
$-6(r-1)/r \to -6$ as $r \to \infty$, the $c=\infty$ reduction of the
Virasoro collision amplitude
$2 \cdot 3 = (\text{two copies of } S_3) \cdot (\text{coupling at
binary strata})$ in the master-equation recurrence. For principal
$\cW_N$ the analogous factor is the $N$-dependent Riccati recursion
ratio $-\beta_N$ (Section~\ref{sec:wn-ratio-test}).

$\rho_*(c)$ is the classical-action radius of the Virasoro
perturbative expansion of the modular Laplacian at $c = \infty$, with
leading classical action $|c| \cdot \log(6 / |c|)$; the critical radius
$|c| / 6$ is where the formal series ceases to converge absolutely.
This is the Virasoro analogue of the Yang--Mills large-$N$ 't Hooft
limit.
\end{remark}
...
The proof of Proposition~\ref{prop:virasoro-ratio-test} extends to
principal $\cW_N$ at $N = 3$ unconditionally, with Riccati ratio
$-\beta_3$ in place of $-6$. The all-rank principal $\cW_N$
generalisation is the finite-envelope criterion below; the harmonic
beta law gives the conditional radius refinement.
...
\item $S_2(\cW_{3, c}) = \kappaChHodge(\cW_{3, c}) = c \cdot (H_3 - 1) = 5c/6$
...
\begin{proof}
\emph{(1)} $\kappaChHodge(\cW_{N, c}) = c (H_N - 1)$ is the universal kappa
formula for principal $\cW_N$, with $H_N = \sum_{j=1}^N 1/j$. At $N = 3$: $H_3 = 1 + 1/2 + 1/3 = 11/6$, so
$H_3 - 1 = 5/6$ and $\kappaChHodge(\cW_{3, c}) = 5c/6$.
...
independently of the $\kappa$-ratio $\kappaChHodge(\cW_3)/\kappaChHodge(\mathrm{Vir})
...
\begin{remark}[Harmonic candidate for $\beta_N$ at $N \geq 4$]
\label{rem:beta-N-first-principles-derivation-path}
The two-channel identity $\beta_3=6+4$ is the first non-Virasoro
instance of the harmonic spin-lane candidate
\[
 \beta_N
 \;=\;
...
\end{itemize}
Conjecture~\ref{conj:beta-N-harmonic-closed-form} delivers the
same value from the kappa-ratio scaling law
$A_r(\cW_N)=(\kappaChHodge(\cW_N)/\kappaChHodge(\Vir))^{r-3}A_r(\Vir)$ and
$\kappaChHodge(\cW_N)=c(H_N-1)$. Triangular and quadratic
extrapolations agree with the harmonic candidate at $N=2,3$ and
separate at $N=4$.
\end{remark}
...
\[
 ...
 0,
\]
discharging the analytic tempering hypothesis for $\cW_3$ at every
generic $c$. Descent to the raw direct-sum curved-Dunn complex is the
finite-propagation condition of
...
\begin{proof}
Transcribe the proofs of
Theorem~\ref{thm:tempered-stratum-contains-virasoro} and
Proposition~\ref{prop:virasoro-rho-star-closed-form}, with the
Virasoro factor $6$ replaced by the $\cW_3$ factor $\beta_3 = 10$
from Proposition~\ref{prop:w3-shadow-leading-asymptotic}(4). The
Stirling factor $(r!)^{-1/r} \sim e/r \to 0$ is $\beta$-independent,
whence
$\limsup (|S_r| / r!)^{1/r} \le \beta_3 / c \cdot e/r \cdot (\text{bounded
tier factor}) \to 0$ as $r \to \infty$ at every fixed $c \neq 0$.
\end{proof}

\begin{theorem}[Finite-\(\beta_N\) criterion for principal $\cW_N$,
$N \ge 4$]
\label{conj:wn-tempered-general}
For every integer $N \ge 2$ and every $c \in \bbR$ with
$c \notin \{0\} \cup \cD^{\mathrm{sev}}_{\mathrm{Kac}}(\cW_N)$, assume
the principal $\cW_N$ shadow tower has a finite Riccati envelope
\[
 ...
 \qquad (r\ge 4)
\]
for some finite $\beta_N>0$. Then $\cW_{N, c}$ satisfies
$(\mathrm{H}_S)$.
\end{theorem}
...
finite-envelope criterion, Theorem~\ref{thm:wn-tempered-all-N}, proves
$(\mathrm{H}_S)$ from the displayed hypothesis by the
$\beta$-independent Stirling estimate. It does not prove the all-rank
Riccati envelope.
\end{proof}

\begin{remark}[Conditional harmonic radius]
\label{rem:wn-conditional-harmonic-radius}
Assume Conjecture~\ref{conj:beta-N-harmonic-closed-form}. Then
...
\begin{remark}[$\cW_4$ discriminator at generic $c$]
\label{rem:w4-numerical-probe}
The first discriminator after $\beta_2=6$ and $\beta_3=10$ is
\[
 \beta_4=12(H_4-1)=13.
\]
The linear value $N+1$ gives $5$, the triangular extrapolation $15$,
the quadratic extrapolation $16$. The harmonic candidate is
rational-valued: $\beta_5=77/5$ separates from every integer-valued
extrapolation under the kappa-ratio scaling law.
\end{remark}
...
Theorem~\ref{thm:tempered-stratum-contains-w3} combine to give the
principal tempered locus.

\begin{corollary}[Principal tempered locus]
\label{cor:original-complex-dichotomy-healed}
The principal non-logarithmic tempered locus is as follows.

\emph{Virasoro generic-$c$ stratum.} The non-tempered stratum of
Virasoro is empty: every generic $\mathrm{Vir}_c$ with
$c \notin \{0\} \cup \cD^{\mathrm{sev}}_{\mathrm{Kac}}$ is tempered.

\emph{Principal $\cW_3$ generic-$c$ stratum.} The non-tempered stratum
of principal $\cW_3$ is empty: every generic
$\cW_{3, c}$ with $c \notin \{0\} \cup
\cD^{\mathrm{sev}}_{\mathrm{Kac}}(\cW_3)$ is tempered.

\emph{Principal $\cW_N$, $N \ge 4$.}
Theorem~\ref{conj:wn-tempered-general} establishes tempering on the
finite-envelope locus. The harmonic closed form for the ordinary
convergence radius is conditional on the beta-law surface.

\emph{Logarithmic $\cW(p)$.} The Riccati-recursion analogue of
Proposition~\ref{prop:w3-shadow-leading-asymptotic} is not in hand for
$\cW(p)$: the Adamovic--Milas strong-generator presentation is not
quadratic-Koszul. The logarithmic loci carry unbounded Massey
amplitudes (Gurarie 1993; Flohr 1996), so finite Zhu data do not place
$\cW(p)$ in the tempered stratum. Its status is the separate
Conjecture~\ref{conj:tempered-stratum-contains-wp}, whose inputs are
the regular $TW/WW$ bound and the logarithmic boundary estimate.

\emph{Remaining open loci.} The candidates for non-tempering on the
non-degenerate class~M locus are: (i) $\cW(p)$ logarithmic and its
$\bZ/2$-orbifolds; (ii) Monster-type VOAs (twisted sectors of
lattice orbifolds); (iii) coset realisations with irrational central
charge.
\end{corollary}

\begin{proof}
Virasoro, $\cW_3$, and principal $\cW_N$ statements combine
Theorem~\ref{thm:tempered-stratum-contains-virasoro},
Theorem~\ref{thm:tempered-stratum-contains-w3}, and
Theorem~\ref{conj:wn-tempered-general} on the finite-envelope locus.
The logarithmic $\cW(p)$ statement requires the Adamovic--Milas
generator count, the regular $TW/WW$ amplitude bound, and the
logarithmic boundary estimate; Monster-type statements require the
Frenkel--Lepowsky--Meurman twisted-sector analysis.
\end{proof}

\section{Independent derivations}
\label{sec:tempered-stratum-iv-summary}

The theorems admit the following independent derivations.
...
\item \emph{Verified against}: direct numerical evaluation of
 ...
 at $r = 10$ and five values $c \in \{1/2, 1, 7, 13, 100\}$; the
 recurrence independently iterates the chain-A master equation from
...
\item \emph{Derived from}: $\cW_3$ Fateev-Lukyanov cubic coupling
 ...
 10$.
\item \emph{Verified against}: the first Laurent coefficients satisfy
 $A_4^{\cW_3}=10/3$ and $A_5^{\cW_3}=-80/3$, hence
 $A_5^{\cW_3}/A_4^{\cW_3}=-8=-10\cdot 4/5$.
...
\section{Placement in the Topologisation Ladder}
\label{sec:tempered-stratum-location}

The non-emptiness question for the tempered stratum of class~M at
non-trivial central charge:

\begin{itemize}[nosep]
\item \textbf{Tempered principal locus}: every generic
 $\mathrm{Vir}_c$ and every generic principal $\cW_{3,c}$ is
 tempered; principal $\cW_{N,c}$ for $N\ge4$ is tempered on the
 finite-envelope locus. The Banach $E_3$-model carries the analytic
 input on that locus; raw direct-sum descent is the finite-propagation
 assertion of
 Theorem~\ref{thm:chain-level-e3-original-complex-conditional}.
\item \textbf{Ordinary convergence radius}:
 $\rho_*(c)=|c|/6$ for Virasoro and
 $\rho_*^{\cW_N}(c)=|c|/[12(H_N-1)]$ for principal $\cW_N$ under the
 harmonic beta-law hypothesis.
\item \textbf{Open non-principal loci}: tempering of logarithmic
 $\cW(p)$ and tempering of Monster-type twisted sectors.
\item \textbf{Cross-links}: Vol~I supplies the Laurent closed forms;
Vol~II supplies the KS-$\rho$ Banach framework, the curved-Dunn
complex, the chain-level $E_3$ theorem under $(\mathrm{H}_S)$, and
the iterated Sugawara ladder $\mathrm{E}_{k+2}$-top theorem.
\end{itemize}

Within class~M at non-degenerate central charge, every algebra
admitting a finite leading-Laurent closed form of order
$\le C \cdot \beta^r$ for computable $\beta$ (Virasoro, $\cW_3$, and
principal $\cW_N$ ranks with the finite envelope) is tempered, with the
Banach topologisation input thus supplied. Raw direct-sum
topologisation is governed by finite propagation. The
finite-envelope problem for principal $\cW_N$ at $N\ge4$ and the
non-principal class~M cases (logarithmic, Monster orbifolds) remain
open.
...
\item \textbf{($\cW_N$, $N \ge 4$.)}
 ...
 Theorem~\ref{conj:wn-tempered-general} above on the finite-envelope
 locus. Conditional closed-form ratio
 $\beta_N = 12(H_N - 1)$ (Vol~II
 Chapter~\ref{ch:beta-N-closed-form-all},
 Conjecture~\ref{conj:beta-N-harmonic-closed-form}). Realises
 \textsf{cor:rung-w-N} for all $N \ge 2$ where the finite envelope is
 supplied. The infinite rung \textsf{cor:rung-w-infty}
 (the $\Einfty$-topological endpoint $\cW_\infty[\mu]\to\Einfty^{\mathrm{top}}$)
 is conditional on the four primary-source hypotheses
 $\hypProchazka, \hypCKL, \hypPRSh, \hypYamada$
 (\cite{ProchazkaRapcak18,CKL-parafermion,PopeRomansShen-winfty-wedge,
 Bakas-Winfty,Yamada}; licensing $\delta$) and is governed by
 Theorem~\textup{\ref{thm:e-infinity-specialisation-Winfty}} of
 \textsf{e\_infinity\_topologization.tex}. The finite-$N$ tempered
 stratum is established independently of that endpoint.
\item \textbf{(Logarithmic $\cW(p)$ triplet.)} The remaining theorem is
 Conjecture~\ref{conj:tempered-stratum-contains-wp} of Vol~II
 Chapter~\ref{ch:logarithmic-wp-tempered-analysis},
 and its proof obligation is the regular Adamovi\'c--Milas $TW/WW$
 character-amplitude bound together with the separate logarithmic
 $\phi_{0,1}$ boundary estimate. This does not obstruct the rest of the
 climax.
\item \textbf{(Irrational cosets.)} Closed by Vol~II
 Chapter~\ref{ch:irrational-cosets-tempered}
 (Theorem~\ref{thm:irrational-coset-tempered}), through the refined
 VSKR + BGG tempering criterion.
\item \textbf{(Schellekens 71 + Monster $V^\natural$.)} Governed by
 Chapter~\ref{ch:schellekens-71-alpha-classification-platonic}
 (Theorem~\ref{thm:schellekens-71-all-alpha-zero}): Type~A has
 vacuous finite-orbifold obstruction, Type~B is the Leech
 $\mathbb Z/2$ row after the normalized local BV sign and
 finite-orbifold descent are supplied, and Type~C requires the
 chosen cyclic action to carry a trivialized
 $[\alpha_{\mathrm{orb}}]\in H^3(B\mathbb Z/n;U(1))$ plus
 finite-orbifold BV descent.
\end{itemize}
The non-degenerate locus on which the Banach topologisation input is
available is therefore: Virasoro, $\cW_3$, principal $\cW_N$ ranks
where a finite envelope is supplied (including BP non-principal via
Vol~II Chapter~\ref{chap:bp-chain-level-strict-platonic}),
pure Niemeier rows and the Schellekens orbifold rows satisfying their
local DW/BV descent hypotheses, together with irrational cosets in the
parafermion + affine-Heisenberg + Virasoro-in-affine families.
The single residual stratum is logarithmic $\cW(p)$; the raw
direct-sum chain-level statement on each tempered locus is the
finite-propagation descent problem.
\end{remark}
