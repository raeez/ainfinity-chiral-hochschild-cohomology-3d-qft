...
The PVA structure on $H^\bullet(\A, Q)$ encodes the $\lambda$-bracket
(singular OPE) and commutative product (regular OPE) as separate
operations linked by the Leibniz rule.  This is the ``abelian shadow''
of a richer algebraic structure: the full chain-level $A_\infty$
operations $\{m_k\}$ organize into a brace algebra, whose
cohomological descent produces the Gerstenhaber bracket on chiral
Hochschild cohomology $\mathrm{ChirHoch}^\ast(\A)$.  The PVA bracket
is recovered as the degree-$(-1)$ component of the Gerstenhaber
bracket, and the Leibniz rule is a shadow of the brace compatibility.
Section~\ref{sec:chiral_hochschild} develops this Hochschild-theoretic
perspective via dimensional descent from the Poisson sigma model.
\end{remark}
