...
Passing to cohomology with respect to the BV-BRST differential $Q = m_1$, the quantum $A_\infty$ data collapses to a classical Poisson vertex algebra.

\begin{maintheorem}[Cohomological descent to PVA; \ClaimStatusProvedHere]
...
\item The cohomology $H^\bullet(\A,Q)$ inherits a commutative product
  \textup(from the regular part of~$m_2$\textup) and a
  $(-1)$-shifted $\lambda$-bracket
  \textup(from the singular part of~$m_2$\textup);
\item These operations satisfy the axioms of a Poisson vertex algebra
  \textup(sesquilinearity, skew-symmetry, Jacobi identity, vacuum,
  Leibniz rule\textup);
\item All higher operations $m_{k \geq 3}$ vanish in cohomology: on
  $Q$-closed inputs, $m_k$ is $Q$-exact for $k \geq 3$, with
  homotopies constructed from the contractibility of
  $\Conf_k^{<}(\R)$.
\end{enumerate}
\end{maintheorem}
...
\label{rmk:shifted-PVA-signs}
The above defines an \emph{unshifted} PVA.  The $(-1)$-shifted PVA proved in Theorem~\ref{thm:cohomology-PVA-main} satisfies the shifted variants: skew-symmetry acquires a sign $(-1)^{(|a|+1)(|b|+1)}$, and similarly for Jacobi and Leibniz.
\end{remark}
...
$\{a_\lambda b\} = \sum_{n \ge 0} a_{(n)}b\,\lambda^{n}$,
recording the same OPE coefficients $a_{(n)}b$ as a formal power series in~$\lambda$.
...
\begin{lemma}[Jacobi Identity Verification; \ClaimStatusProvedHere]
...
It remains to show that $m_k = 0$ in cohomology for $k \geq 3$.
...
The proof uses $d' = 1$ (one topological direction). For $d' \geq 2$, the Gaiotto--Kulp--Wu formality theorem \cite{GKW25} implies higher operations $m_k$ vanish at the chain level, since quantum corrections stabilize after one loop. For $d' = 1$, quantum corrections persist (non-formality), so the chain-level structure is genuinely $A_\infty$, and only on cohomology do we recover a PVA.
\end{remark}
...
\begin{theorem}[Cohomology is a PVA; \ClaimStatusProvedHere]
...
\begin{proposition}[Skew-Symmetry in Cohomology; \ClaimStatusProvedHere]
...
\begin{proposition}[Vacuum Axiom in Cohomology; \ClaimStatusProvedHere]
...
The $m_k$ with $k \geq 3$ vanish in cohomology.  The mechanism is topological: $\Conf_k^{<}(\R)$ is contractible.
...
Proposition \ref{prop:m3_vanish} requires exactly one topological direction ($d'=1$). For $d' \geq 2$, the Gaiotto--Kulp--Wu formality theorem \cite{GKW25} implies higher operations vanish already at the chain level (after one-loop renormalization). For $d'=1$, they persist at the chain level but vanish in cohomology, yielding PVAs rather than vertex algebras.
\end{remark}

\begin{remark}[Physical interpretation of contractibility]
Physically, contractibility of $\Conf_k^{<}(\mathbb{R})$ says that \emph{time-ordered} configurations of $k$ operators on the topological line are homotopically trivial for $k \geq 3$. The holomorphic direction $\mathbb{C}$ carries OPE data (radial ordering, singular coefficients), while the topological direction $\mathbb{R}$ carries only the bar-complex ordering. The persistence of $m_2$ is the \emph{product}, and the vanishing of $m_{k \geq 3}$ is the statement that no higher coherence data survives passage to the Poisson--vertex algebra. This is the precise sense in which $d'=1$ holomorphic-topological theories produce PVAs rather than $A_\infty$-algebras: the bar direction is one-dimensional and therefore contractible above degree zero.
\end{remark}

\begin{remark}[Why $d'=1$ produces PVAs and $d'=2$ produces $E_2$ algebras]
\label{rem:contractibility-and-d-prime}
The topological simplex
$\Conf_k^{<}(\R) \cong \Delta^{k-2}_{\circ}$ is contractible
because $\R$ is $1$-dimensional.  This is the reason that $d'=1$
theories produce PVAs on cohomology, while $d'=2$ theories (e.g.,
...
cohomologically trivial.  The contractibility of the topological
factor provides a homotopy that trivializes all $m_k$ for
$k \geq 3$ on cohomology---but \emph{not} at the chain level,
...
\label{rem:transition-to-hochschild}
...
Beilinson--Drinfeld (Volume~I, Definition~\ref{def:coisson}): a
...
\label{rem:formality-genus1-curvature}
At genus $g \geq 1$, the contractibility argument of
Proposition~\ref{prop:m3_vanish} breaks: the configuration space
$\mathrm{Conf}_k(\Sigma_g)$ has nontrivial higher homology from
the fundamental group.  In Volume~I's language, this
non-contractibility manifests as the curvature
$d_{\mathrm{fib}}^2 = \kappa(\cA) \cdot \omega_g$.  The genus-$1$
...
\label{subsec:hca-descent}
\index{homotopy chiral algebra!PVA descent}

The vanishing of $m_3$ on cohomology
(Proposition~\ref{prop:m3_vanish}) produces the PVA structure, but
at the chain level the $A_\infty$ operations $\{m_k\}_{k \geq 3}$
are generically nonzero.  Malikov--Schechtman~\cite{MS24} provide
a complementary perspective: the \v{C}ech complex of the sheaf of
boundary chiral algebras carries the structure of a
...
(Volume~I, Theorem~\ref{thm:cech-hca}).  The PVA is the strict
shadow of $\Phi_{\cA_\partial}$ obtained by passing to cohomology.

\begin{proposition}[HCA structure on boundary \v{C}ech complex;
\ClaimStatusConjectured]
\label{prop:hca-boundary-cech}
Let $\cA_\partial$ be the sheaf of boundary chiral algebras on a
curve~$X$ obtained by interval compactification of a 3d
holomorphic-topological theory (standing hypothesis
\textup{(H1)--(H4)}).  For an affine open cover $\mathcal{U}$ of~$X$,
...
\item $F_3$ is the Jacobiator homotopy when
  ...
  by Volume~I, Proposition~\textup{\ref{prop:two-element-strict}},
...
\label{rem:pva-strict-truncation-hca}
...
$F_3$ (Jacobiator homotopy) & $0$ (contractibility of $\Conf_3(\C)$) \\
$\{F_n\}_{n \geq 4}$ & $0$ (PVA is strict)
\end{tabular}
\end{center}
