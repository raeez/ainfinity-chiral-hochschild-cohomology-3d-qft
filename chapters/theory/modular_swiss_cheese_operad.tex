...
Chapter~\ref{ch:factorization-swiss-cheese}. The structural point is
local/global separation: the topological operad
...
curvature $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g$. The global
...
$\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g$ is a \emph{global} phenomenon.
...
\item the curvature $\kappaChHodge(\cA) \cdot \omega_g$.
\end{itemize}
...
$E_1^{p,q} \to E_1^{p+1,q-2}$ and is induced by the genus-raising
operator $D_1 = \kappa \cdot \Deltacyc$ on bar cohomology.
...
More generally, on the genus-$p$ copy, $d_1(a^n)$ is computed by
contracting all pairs in $s^{-1}a^{\otimes n}$, yielding
\begin{equation}\label{eq:heis-d1-formula}
 d_1^{p,n}(a^n) = \binom{n}{2}\, \kappa \cdot a^{n-2}
 \;\in\; E_1^{p+1, n-2},
\end{equation}
...
$d_1 \colon E_1^{p,n} \to E_1^{p+1,n-2}$ is an isomorphism for every
$p\geq0$ and all $n \ge 2$, and $d_1 = 0$ on rows $n=0,1$.
The $E_2$ page is:
\begin{equation}\label{eq:heis-E2-generic}
 E_2^{p,q} = 0 \quad \text{for } p \ge 1 \text{ or } q \ge 2.
\end{equation}
The only surviving entries are
...
\item \emph{All columns $p \ge 1$:} fix the conserved total
 degree $N=q+2p$. The $d_1$-string in total degree $N$ is
 \[
 E_1^{0,N}\longrightarrow E_1^{1,N-2}\longrightarrow
 E_1^{2,N-4}\longrightarrow\cdots\longrightarrow
 E_1^{\lfloor N/2\rfloor,\,N-2\lfloor N/2\rfloor}.
 \]
 Every displayed arrow before the terminal row $0$ or~$1$ is a
 nonzero map between one-dimensional spaces and hence an isomorphism.
 Therefore every positive-genus entry is either killed by the next
 contraction or is the image of the previous one, and
 $E_2^{p,q}=0$ for all $p\geq1$.
\end{itemize}

At $\kappa \neq 0$, the curvature $\kappa \cdot \omega_1$ gives the
uniform genus-raising contraction. In this Heisenberg spectral sequence
only $E_2^{0,0}$ and $E_2^{0,1}$ survive; no higher differential can
hit them for bidegree reasons, and all other $E_2$ entries are zero.
...
At $\kappa = 0$, the OPE pairing vanishes. With the genus-completed
convention of Definition~\ref{def:genus-completion}, the modular bar
complex decomposes as the completed product
\[
 \Bmod(\Heis_0)=\prod_{g\geq 0} B^{(g)}(\Heis_0),
\]
with no inter-genus differential, and each finite-genus summand has
cohomology $\Bbbk[a]$.

The dichotomy is the spectral-sequence shadow of curvature:
$\kappaChHodge(\Heis_\kappa) = \kappa$, and $\kappa \cdot \omega_g$
is the class detected by the genus-raising contraction. When
$\kappa = 0$ the completed genus tower decouples; when
$\kappa \neq 0$ the contraction couples adjacent genus levels and
removes the high-degree Heisenberg classes computed above.
\end{computation}
...
\end{equation}
In the adjoint representation on the ordered basis $\{e, f, h\}$,
$\operatorname{ad}(e)(f) = h$, $\operatorname{ad}(e)(h) = -2e$,
$\operatorname{ad}(f)(e) = -h$, $\operatorname{ad}(f)(h) = 2f$, so
\[
\operatorname{ad}(e)
= \begin{psmallmatrix} 0 & 0 & -2 \\ 0 & 0 & 0 \\ 0 & 1 & 0
\end{psmallmatrix},
\quad
\operatorname{ad}(f)
= \begin{psmallmatrix} 0 & 0 & 0 \\ 0 & 0 & 2 \\ -1 & 0 & 0
\end{psmallmatrix},
\quad
\operatorname{ad}(e)\operatorname{ad}(f)
= \begin{psmallmatrix} 2 & 0 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 2
\end{psmallmatrix},
\]
whence $\operatorname{Tr}(\operatorname{ad}(e)\operatorname{ad}(f)) = 4$.
This value normalises the form to $(e,f)_k = k$ and $(h,h)_k = 2k$,
consistent with the level-$k$ OPE~\eqref{eq:sl2-opes}. Therefore
\[
 D_1(s^{-1}e \otimes s^{-1}f)
 = (-1)^{1}\,\frac{k \cdot 4}{4}\,
 \xi_{\mathrm{nsep}}(\mathbf{1})
...
The bicomplex identity is verified on
$s^{-1}e \otimes s^{-1}f \in B_2(\slhat_2)$.
...
$D_0^2 = 0$ on the degree-$3$ bar element
$s^{-1}e \otimes s^{-1}f \otimes s^{-1}h \in B_3(\slhat_2)$
follows from a cancelling pair. The differential $D_0$ acts
...
$k \ge 4$ are chosen recursively from Stasheff boundary sources after
fixing the contraction data), the resulting operations are generically
nonzero. This is class~$\mathbf{M}$ in the Volume~I
classification.
\end{example}

\begin{verification}[Generating depth: the arity-four source for \(m_4\)]
\label{ver:virasoro-m4}
\index{Virasoro algebra!Stasheff relations at degree 4}
The degree-four Stasheff relation determines the source \(d m_4\), not
a cohomological value of \(m_4\). The relation reads:
...
\begin{multline}\label{eq:stasheff-4-virasoro}
 ...
 + m_2(T, m_3(T,T,T)) = 0,
\end{multline}
up to Koszul signs (suppressed: $T$ has even degree
$\degree{T} = 2$, so all signs are $+1$).

All terms in~\eqref{eq:stasheff-4-virasoro} involve only
$m_2$ and $m_3$. They form the arity-four boundary source
\(\mathrm{Obs}_4=m_2\circ m_3+m_3\circ m_2\). Since \(m_1=0\)
on cohomology, the displayed relation says only that
\(\mathrm{Obs}_4=0\) there. On the chain model the full relation is
\[
  m_1m_4 + m_2\circ m_3 + m_3\circ m_2
  + m_4\circ m_1^{\otimes 4}=0,
\]
and a fixed contraction \(h\) chooses the primitive
\[
  m_4(T,T,T,T)=-h\,\mathrm{Obs}_4(T,T,T,T).
\]
Thus arity four supplies the source for \(d m_4\), not a canonical
cohomology class of \(m_4\). The boundary strata of $\FM_4(\C)$
contributing to \(D_0^2\) still decompose entirely into compositions
of \(m_2\) and \(m_3\); the extra datum is the SDR gauge choosing
the primitive.

The same statement applies inductively at all degrees $k \ge 5$:
the Stasheff relation at degree~$k$ determines the source for
\(d m_k\) in terms of $\{m_2, m_3, \ldots, m_{k-1}\}$, and the
chosen contraction selects \(m_k\). Since \(m_3\) is the only
independent source generator beyond \(m_2\), the generating depth is
$d_{\mathrm{gen}}(\Vir_c) = 3$; the algebraic depth is
$d_{\mathrm{alg}}(\Vir_c) = \infty$ because the chosen higher chain
operations are generically nonzero.
...
The depth-zero associated graded is obtained by forgetting the
...
mixed face, denoted $d_{\mathrm{mix}}$. It is
...
mixed open--closed space with a product. The geometric input follows
the Fulton--MacPherson compactification for manifolds with
...
(Chapter~\ref{ch:factorization-swiss-cheese}). The associated
$\SCmod$-coalgebra $\mathrm{Loc}(\cF)$ is constructed as follows.
...
$\kappaChHodge(\cA)\cdot\omega_g$ are outside the formal-collision
skeleton.
\end{proposition}
...
$\kappaChHodge(\cA) \cdot \omega_g \in H^2(\overline{\cM}_g)$.
...
formality and mixed-face convergence]
\label{thm:modular-hkoszul-SC}
\index{Swiss-cheese operad!modular homotopy-Koszulity|textbf}
The depth-zero associated graded of the genus-$0$
formal-collision specialization of $\SCmod$ is
homotopy-Koszul by Theorem~\ref{thm:homotopy-Koszul}; the coupled
genus-$0$ statement is Conjecture~\ref{conj:coupled-sc-koszul}. The
all-genus modular statement for $\SCmod$ holds under three
hypotheses (together with that conjecture wherever the coupled
mixed sector enters):
\begin{enumerate}[label=\textup{(\roman*)}]
...
Theorem~\ref{thm:homotopy-Koszul} proves that the depth-zero
associated graded $\gr\,\SCchtop$ is
homotopy-Koszul, assembling three inputs: Koszulity of the
homology Swiss-cheese operads~\cite{Hoe09,HL12},
...
The function $S$ is symmetric. By definition,
...
\[
 ...
 + \frac{1}{(z-x)(y-w)},
\]
a nonzero symmetric rational function: over the common
denominator $(x-y)(y-z)(z-x)(x-w)(y-w)(z-w)$ its numerator is the
symmetric polynomial
$\sum_{\mathrm{sym}} x^2 y^2 - \sum x^2 yz + 6\,xyzw - \cdots$,
which does not vanish. The four-point remainder $S$ is therefore
not zero even at genus~$0$; it is the genus-$0$ shadow of the
Bergman-kernel remainder, symmetric in all four arguments. What
vanishes at genus~$0$ is the \emph{cyclic three-point} sum
$\Sigma_{\mathrm{sym}}$ of~\eqref{eq:fay-three-variable-S}, namely
$\sum_{\mathrm{cyc}} f(z_1,z_2)f(z_2,z_3)$; this is the partial
fraction identity
\[
 \frac{1}{(z_1-z_2)(z_2-z_3)}
 + \frac{1}{(z_2-z_3)(z_3-z_1)}
 + \frac{1}{(z_3-z_1)(z_1-z_2)} = 0,
\]
which holds because $\mathbb{P}^1$ has no holomorphic $1$-forms.
The contraction of $S$ with a totally antisymmetric tensor
(Step~(a.iv)) annihilates it at every genus.

\smallskip\noindent\emph{Genus-$1$ verification.}
At genus~$1$, $E(x,y) = \vartheta_1(x-y;\tau)/\vartheta_1'(0;\tau)$
and $f(x,y) = \zeta(x-y;\tau)$. The object entering $D_0^2$ is the
cyclic three-point remainder
$\Sigma_{\mathrm{sym}}$ of~\eqref{eq:fay-three-variable-S}; with
$u = z_1 - z_2$, $v = z_2 - z_3$, so $z_3 - z_1 = -u-v$, it reads
\[
 \Sigma_{\mathrm{sym}}
 = \zeta(u)\,\zeta(v)
 + \zeta(v)\,\zeta(-u-v)
...
$\Sigma_{\mathrm{sym}} = \tfrac{1}{2}[\wp(u) + \wp(v) + \wp(u+v)]$
as in~\eqref{eq:fay-genus1-product}. Since $\wp$ is even
($\wp(-z) = \wp(z)$), this is manifestly symmetric in
$(z_1, z_2, z_3)$ (invariant under $u \leftrightarrow v$ and
$u \leftrightarrow -(u+v)$): a nonzero symmetric function,
annihilated only by contraction with an antisymmetric tensor.
...
Since $f(z_3, z_3)$ has a pole, the \emph{product identity}
applies directly: multiply~\eqref{eq:fay-quadratic}
...
\frac{1}{2}\frac{\wp'(u) - \wp'(v)}{\wp(u) - \wp(v)}$, so
$\zeta(-u-v) = -\zeta(u) - \zeta(v) -
...
The dg operad $C_\ast(\FM_k(\Sigma_g))$ is formal
(quasi-isomorphic to its homology) by extension of the Kontsevich
...
\emph{Range.} The extension from genus~$0$ to genus~$g$
...
\]
given by prime-form configuration space integrals exists once the
positive-genus analytic comparison supplies an operadic $\Linf$
morphism from the KZB/prime-form propagator, compatible with
stable-graph clutching, mixed faces, and the period correction data.
At genus~$0$ this recovers Kontsevich's
...
along the diagonal~$\Delta$. At genus $g \ge 0$, this is replaced
by the holomorphic propagator
...
unit leading coefficient in any local coordinate $z$,
...
For every admissible graph $\Gamma$ of type
...
The integrand
$\bigwedge_{e \in E(\Gamma)} \omhol_{ij}$ near $\partial_S$ is
...
Formally: induction on $k$, the number of
...
The boundary contributions to $d \cU_g$ vanish, so
...
The genus-$0$ Arnold relation on
the screen suffices in place of the genus-$g$ Fay trisecant
identity~\eqref{eq:genus-g-arnold}: the
...
The quasi-isomorphism property of $\cU_g$ remains:
its linear part $\cU_g^{(1)}$ induces an isomorphism
on homology. The argument is direct.
...
structure. For each~$g$, the explicit complex structure (any
smooth projective curve) provides a~$\cU_g$ defined and
satisfying all the required properties.
\end{proof}

\begin{remark}[Relation to genus~$0$]
\label{rem:genus-g-formality-range}
...
The bar complex of the mixed-colour operad
...
of~$\epsilon$ to the $p$-th filtration level. Then
...
\begin{remark}[Flat modular homotopy-Koszulity and curved comparison]
\label{rem:modular-hkoszul-range}
\index{homotopy-Koszulity!modular!range}
Theorem~\ref{thm:modular-hkoszul-SC} has the following
consequences and required extra inputs:
\begin{itemize}
\item \emph{Flat model:} bar-cobar equivalence for the flat
...
care at three points, recorded here with detailed verifications in
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Consequences and hypotheses}
...
\begin{corollary}[Flat bicomplex and bar-cobar comparison under hypotheses]
...
curvature $\kappaChHodge(\cA) \cdot \omega_g$ is the $d_1$-obstruction
class
...
has $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g$, so the bar complex
...
\index{three models!operadic range}
...
\section{Irregular-singular KZB,
 curved-Dunn bridge, and cross-genus Maurer--Cartan}
\label{sec:irregular-kzb-curved-dunn-mc}
...
\item The modular-bootstrap $H^2$ complex and the curved-Dunn
 ...
 theories. Vanishing transports only through a low-degree
 comparison criterion.
\item The genus-1 twisted-tensor-product proposition
 (Proposition~\ref{prop:genus1-twisted-tensor-product}) is a
 twisted tensor criterion, not an exactness statement for the Hodge
 class.
\item The cross-genus Maurer--Cartan equation uses modular-bootstrap
 acyclicity as an algebraic input; KZB controls boundary composition
 only after compatible formal types and mixed Stokes data are supplied.
\end{itemize}

The required analytic input is the irregular-singular KZB framework of
Jimbo--Miwa--Ueno~\cite{JMU81}. It supplies Stokes-sector composition
at generic non-integral level once a compatible boundary formal type
and the mixed Stokes clutching data have been constructed. It does not
prove curved-Dunn \(H^2\)-vanishing.

% =====================================
\subsection{Irregular-singular KZB composition}
\label{subsec:irregular-kzb}

The composition theorem requires fixing the genus-$g$
...
theorem below is a normal-form statement under explicit hypotheses:
once the KZB
...
\begin{theorem}[Irregular-singular KZB composition under boundary
formal-type hypotheses]
...
\item after finite ramification of each sewing parameter, every
nodal divisor has a compatible boundary formal type of finite pole
order;
\item the associated Stokes sectors and Stokes matrices satisfy the
mixed clutching cocycle compatibility at two-edge corners.
\end{enumerate}
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{(Irregular-singular extension.)}
 ...
 each boundary divisor $\delta_\Gamma$ has the assumed formal type
 after ramification. A simple logarithmic nodal pole is regular
 singular; irregularity begins only from higher pole order in the
 ramified coordinate.
...
\item \emph{Formal normal form.} A diagonal irregular part
 $Q_\Gamma(t_e) = \sum_{m=1}^{d_e}
 \mathrm{diag}(\lambda_{i,m})t_e^{-m}$ after finite ramification
 $q_e=t_e^{N_e}$, with pole order $d_e$ in the ramified coordinate.
\item \emph{Exponential torus.} $\mathbb{T}_\Gamma =
 (\mathbb{C}^\times)^r \subset \mathrm{GL}(V^{\otimes \#\mathrm{ins}})$
 attached to the non-zero exponential differences of the formal
 type.
...
\item \emph{Stokes matrices.} $S_\ell^\pm \in
 U_\ell^\pm \subset \mathrm{GL}$ are analytic Stokes data lying in
 the Stokes groups determined by the formal type. The formal type
 determines the Stokes directions and groups, not the matrices
 themselves.
\end{itemize}
...
datum; the Stokes matrices are additional analytic monodromy data,
not consequences of the formal type alone.
...
descends to the functoriality identity once the third hypothesis
supplies the two-edge sector-crossing coherence. Concretely:
\[
 \Pi_{\gamma_1 \cdot (\gamma_2 \cdot \gamma_3)} \;=\;
 \Pi_{\gamma_1} \circ \Pi_{\gamma_2} \circ \Pi_{\gamma_3} \;=\;
 \Pi_{(\gamma_1 \cdot \gamma_2) \cdot \gamma_3},
\]
with the assumed Stokes cocycle supplying the sector-crossing
coherence. Neither step requires $k$ integral.
\end{proof}

\begin{proposition}[Boundary \(q\)-formal type and Stokes cocycle]
\label{prop:irregular-kzb-q-formal-type-stokes-cocycle}
\ClaimStatusConditional
Assume the boundary formal-type and mixed Stokes hypotheses
\textup{(K2)}--\textup{(K3)} of
Theorem~\ref{thm:irregular-kzb-composition}. In a ramified plumbing
coordinate \(q\) transverse to a nodal divisor, and in local
coordinates \(t_{\alpha}\) along the divisor, the KZB connection is
written in the licensed boundary chart \(\alpha=(q,t_{\alpha})\) as
\begin{equation}
\label{eq:irregular-kzb-boundary-connection-form}
\nabla
= d
-\left(
 \frac{A_{m}}{q^{m}}+\frac{A_{m-1}}{q^{m-1}}
 +\cdots+\frac{A_{1}}{q}+A_{0}
 \right)dq
-\sum_{\alpha}B_{\alpha}(q)\,dt_{\alpha}.
\end{equation}
The finite formal type in the \(q\)-direction is
\begin{equation}
\label{eq:irregular-kzb-boundary-formal-type}
\Theta_{\partial}(q)
=
\sum_{r=1}^{m-1}\frac{A_{r+1}}{r\,q^{r}}+A_{1}\log q,
\end{equation}
and, after a formal transformation
\[
 G(q)\in \operatorname{Aut}(V)[[q^{1/N}]],
\]
the \(q\)-part has the JMU/Levelt--Turrittin form
\begin{equation}
\label{eq:irregular-kzb-formal-gauge-form}
\nabla_{q}\simeq d_{q}-d_{q}\Theta_{\partial}
-\Lambda\,\frac{dq}{q}.
\end{equation}
For a Stokes ray \(\ell\) and a sector \(S_{\ell}\), the canonical
sectorial solution is
\begin{equation}
\label{eq:irregular-kzb-sectorial-solution}
Y_{\ell}(q)=H_{\ell}(q)e^{\Theta_{\partial}(q)}q^{\Lambda},
\end{equation}
and adjacent sectors are related by
\begin{equation}
\label{eq:irregular-kzb-stokes-matrix}
S_{\ell,\ell'}=Y_{\ell}^{-1}Y_{\ell'}.
\end{equation}
At a clutching corner \(\Gamma_{1}\#\Gamma_{2}\), compatibility of
the formal types and Stokes factors is the pair of identities
\begin{align}
\label{eq:irregular-kzb-clutching-formal-type}
\Theta_{\partial,\Gamma_{1}\#\Gamma_{2}}
&=
\Theta_{\partial,\Gamma_{1}}\oplus
\Theta_{\partial,\Gamma_{2}}\oplus
\Theta_{\mathrm{node}},\\
\label{eq:irregular-kzb-clutching-stokes-product}
S_{\ell,\ell'}^{\Gamma_{1}\#\Gamma_{2}}
&=
S_{\ell,\ell'}^{\Gamma_{1}}\,
S_{\ell,\ell'}^{\Gamma_{2}}\,
S_{\ell,\ell'}^{\mathrm{node}},
\end{align}
with the product order fixed by the tangential basepoint convention.
Associativity at generic nonrational level is exactly the Stokes
cocycle
\begin{equation}
\label{eq:irregular-kzb-stokes-cocycle}
S_{\ell_{1},\ell_{2}}S_{\ell_{2},\ell_{3}}S_{\ell_{3},\ell_{1}}=1.
\end{equation}
The licensing data are \(\alpha\) the boundary chart,
\(\gamma\) the formal/sectorial wild category, and \(\delta\) the
Stokes cocycle. This proposition supplies boundary composition data
only after the formal type and mixed Stokes matrices are supplied; it
does not prove curved-Dunn \(H^{2}\)-vanishing.
\end{proposition}

\begin{proof}
Hypothesis \textup{(K2)} gives a finite-pole meromorphic boundary
model after finite ramification of the plumbing coordinate. Separating
the normal coordinate \(q\) from the divisor coordinates
\(t_{\alpha}\) gives
\eqref{eq:irregular-kzb-boundary-connection-form}. The primitive
\eqref{eq:irregular-kzb-boundary-formal-type} records the chosen sign
convention for the polar part of this connection. The
Levelt--Turrittin theorem in the JMU normalisation, together with
Malgrange--Sibuya sectorial summation as used by
Jimbo--Miwa--Ueno~\cite{JMU81} and Boalch~\cite{Boalch2001,Boalch2014},
then supplies the formal gauge transformation \(G(q)\), the formal
normal form \eqref{eq:irregular-kzb-formal-gauge-form}, and the
sectorial fundamental solutions
\eqref{eq:irregular-kzb-sectorial-solution}. Their transition
matrices are precisely \eqref{eq:irregular-kzb-stokes-matrix}; the
formal type fixes the Stokes rays and groups, while the matrices are
additional analytic monodromy data. Hypothesis \textup{(K3)} is
exactly the mixed clutching compatibility at two-edge corners, hence
gives \eqref{eq:irregular-kzb-clutching-formal-type} and
\eqref{eq:irregular-kzb-clutching-stokes-product}. The three-sector
boundary of a small triangle in the wild groupoid gives
\eqref{eq:irregular-kzb-stokes-cocycle}; conversely, this cocycle is
the sector-crossing coherence needed for associativity of the
Stokes-regularized clutching composition.
\end{proof}

\begin{remark}[Formal-type range]
\label{rem:poincare-rank-range}
Theorem~\ref{thm:irregular-kzb-composition} is stated for
finite boundary formal type after ramification, not for a universal
Poincar\'e-rank formula. Pole order is part of the formal normal form.
A simple nodal logarithmic pole is regular singular; a genuine Stokes
problem requires higher pole order in the ramified coordinate and the
analytic Stokes matrices attached to that formal type. Multi-rank
extensions for irregular boundary data (Nekrasov partition functions
at $\epsilon_2 \to 0$, five-dimensional gauge theory) remain the
frontier item~\ref{frontier:multi-rank-kzb} below.
\end{remark}

\begin{remark}[Boundary formal type for Stokes composition]
\label{rem:stokes-gap-closed}
Composition at generic non-integral level and genus~\(\ge 1\)
requires boundary formal-type and mixed Stokes cocycle hypotheses.
Theorem~\ref{thm:irregular-kzb-composition} states exactly that
input. The JMU/Boalch framework is the correct
asymptotic language; it does not by itself construct the boundary
formal type.
\end{remark}
...
genus $\ge 1$ is resolved only under the boundary formal-type and
mixed Stokes cocycle hypotheses.
\end{corollary}
...
\begin{theorem}[Low-degree curved-Dunn comparison criterion]
\label{thm:curved-dunn-bridge}

Fix a genus \(g\). Assume there is a chain map
\[
 \Phi\colon (\mathfrak{g}^{\cA}_{\mathrm{mod}}, d_{\Theta^{(0)}})
 \longrightarrow (\mathfrak{h}^{\cA}_{\mathrm{Dunn}}, d_0)
\]
defined on the obstruction subcomplex by
\[
 \Phi_g(\Theta^{(g)})
...
genus-$g$ sewing circles. Assume:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\Phi$ is a chain map: $d_0 \circ \Phi = \Phi \circ d_{\Theta^{(0)}}$.
\item $\Phi$ sends the modular-bootstrap obstruction class to the
 curved-Dunn obstruction class.
\item \(H^2(\Phi)\) is an isomorphism, with surjectivity represented
 in cohomological degree \(\le 2\).
\end{enumerate}
If \(H^2(\mathfrak{g}^{\cA}_{\mathrm{mod}},d_{\Theta^{(0)}})=0\),
then \(H^2(\mathfrak{h}^{\cA}_{\mathrm{Dunn}},d_0)=0\).
\end{theorem}

\begin{proof}
Let \([\xi]\in H^2(\mathfrak{h}^{\cA}_{\mathrm{Dunn}},d_0)\).
By the degree-\(\le2\) surjectivity hypothesis, choose a
degree-\(2\) modular-bootstrap representative \(\tilde\xi\) whose
image represents \([\xi]\). The chain-map property and the
assumed \(H^2\)-comparison allow \(\tilde\xi\) to be chosen
\(d_{\Theta^{(0)}}\)-closed. If the modular-bootstrap \(H^2\)
vanishes, then \(\tilde\xi=d_{\Theta^{(0)}}\eta\), and hence
\(\xi=d_0\Phi(\eta)\). Thus \([\xi]=0\). The obstruction-class
compatibility is exactly the statement that the same calculation
annihilates the curved-Dunn obstruction, not merely an arbitrary
degree-\(2\) class.
\end{proof}

\begin{remark}[Precise bridge: two complexes and explicit map]
\label{rem:curved-dunn-complex-bridge}
The bridge is a single datum between two complexes:
\begin{itemize}\setlength\itemsep{2pt}
...
\emph{Degree-\(2\) range.} $\varphi$ is required only as a
degree-\(\le2\) comparison. Once it identifies the obstruction class and induces an
isomorphism on \(H^2\), modular-bootstrap acyclicity transfers to
the curved-Dunn twisting-cochain complex. The existence of this
comparison is the hypothesis of
Theorem~\ref{thm:curved-dunn-bridge} and
Proposition~\ref{prop:modular-bootstrap-to-curved-dunn-bridge};
it is not a full quasi-isomorphism, and no statement
in cohomological degree \(\ge3\) is asserted here.
\end{remark}

\begin{corollary}[Curved Dunn at all genera under comparison hypotheses]
\label{cor:curved-dunn-all-genera}

For \(g=1\), assume the genus-one twisting datum of
Proposition~\ref{prop:genus1-twisted-tensor-product}. For
\(g\ge2\), assume modular-bootstrap \(H^2\)-vanishing and the
degree-\(\le2\) comparison of
Theorem~\ref{thm:curved-dunn-bridge}. Then there exists a twisting
cochain $\tau_g$ such that
\[
\Bmod(\mathcal{O}^{\Ainf\text{-ch}})|_g \simeq
\Bmod(\SC^{\mathrm{ch,top}})|_g \otimes^{\tau_g}
\mathbf{B}(E_1^{\mathrm{tr}}).
\]
Under these hypotheses the degree-\(2\) curved-Dunn obstruction
vanishes. No full higher-degree formality statement is asserted.
\end{corollary}
...
isolating the $H^2$-vanishing hypothesis on which
...
The canonical formulation is \Cref{def:modular-bootstrap-complex}
in the curved-Dunn higher-genus chapter, consumed here as
the source of the low-degree comparison map
$\varphi \colon C^\bullet_{\mathrm{MB}}(g)
\to C^\bullet_{\mathrm{TC}}(g)$.

\begin{theorem}[$H^2$ vanishing for the modular bootstrap complex under bootstrap hypotheses]
...
Theorem~4.3.6 and \S 4.3; \cite{FBZ04}, Chapter~18). This
centre serves as the horizontal replacement for the missing
topologisation factor, yielding the critical-level
...
\begin{proof}
Parts (i)--(iii) follow in turn. Throughout, $\fg$ is simple,
...
The curvature vanishing $\kappaChHodge(\mathfrak{z}_X)=0$ follows from the
...
$\kappaChHodge(V_k(\fg)) = \dim(\fg)(k+h^\vee)/(2h^\vee)$, evaluated at
...
original in \cite{FF92}): kernels of the screening charges
...
lanes, chain-level and $(\infty,1)$-categorical:
\begin{itemize}
\item \emph{Chain-level lane.} The Wakimoto screening complex
(\cite{FBZ04}, Theorem~18.5.1; \cite{FF92}) is an explicit complex
...
\textup{(}\textsf{HYPOTHESIS}: affine Kac--Moody $\cA = V_{-h^\vee}(\fg)$,
Class~L; does \emph{not} apply to Vir$_c$, $\cW_N$, or other
families with nonvanishing complementarity sum
($13$ for Vir$_c$, $250/3$ for $\cW_3$; the
Bershadsky--Polyakov slot is open).\textup{)}%
\par\smallskip\noindent
At $k=-h^\vee$, the modular obstruction tower of $V_{-h^\vee}(\fg)$
is trivial at every genus:
\[
 \operatorname{obs}_g(V_{-h^\vee}(\fg))
 \;=\; \kappaChHodge(V_{-h^\vee}(\fg))\cdot\lambda_g^{\mathrm{FP}}
...
\[
\kappaChHodge(V_k(\fg)) + \kappaChHodge(V_{k'}(\fg))
...
\begin{proof}
The vanishing $\kappaChHodge(V_{-h^\vee}(\fg)) = \dim(\fg)\cdot 0/(2h^\vee)
= 0$ is immediate from the affine $\kappa$-formula
$\kappaChHodge(V_k(\fg)) = \dim(\fg)(k+h^\vee)/(2h^\vee)$. The vanishing
$\operatorname{obs}_g = 0$ then follows from the
Faber--Pandharipande formula
$\operatorname{obs}_g(V_k(\fg))
=\kappaChHodge(V_k(\fg))\lambda_g^{\mathrm{FP}}$
(Vol.~I Theorem~D), applied at
\(\kappaChHodge(V_{-h^\vee}(\fg))=0\). For the Koszul-dual
sum on the affine KM sublocus, linearity of
$\kappaChHodge(V_k(\fg))$ in the shifted level $t := k+h^\vee$, combined
with the Feigin--Frenkel duality $t \mapsto -t$
(i.e.\ $k \mapsto -k-2h^\vee$), produces the identity displayed
above. This cancellation is a property of the
\emph{affine KM $\kappa$-linearity}, not a universal statement
across families: for $\cA = \mathrm{Vir}_c$, $\kappaChHodge(\mathrm{Vir}_c)
+ \kappaChHodge(\mathrm{Vir}_{26-c}) = 13$ (Vol.~I Theorem~C), not $0$; the
...
under boundary formal-type hypotheses]
...
$\delta_\Gamma$, the finite-pole boundary formal type and
mixed Stokes clutching data assumed there, with wild-monodromy datum
\[
 (Q_\Gamma,\; \mathbb{T}_\Gamma,\; \hat M_\Gamma,\;
 \{S_\ell^\pm\}_\ell)
\]
in the sense of Jimbo--Miwa--Ueno~\cite{JMU81} Thm~4.3 and
Boalch~\cite{Boalch2001,Boalch2014}. Equivalently, in a ramified
plumbing coordinate it has the \(q\)-formal type, sectorial solutions,
and Stokes cocycle of
Proposition~\ref{prop:irregular-kzb-q-formal-type-stokes-cocycle}.
Composition of the modular operad $\mathcal{O}^{\Ainf\text{-ch}}$
descends to the cohomology of the wild fundamental groupoid
$\Pi_1^{\mathrm{wild}}$ with values in
$\End^{\mathrm{ch}}_{\cA}$-module families; operadic associativity
at $(g,n)$ follows from the assumed Stokes cocycle compatibility at
...
hypotheses supply the finite-pole formal type and the Stokes cocycle
compatibility. On the nonresonant generic locus, the
...
\item \emph{Generic nonrational level} $k \in \mathbb{C}
 \setminus \mathbb{Q}$, under the boundary formal-type and mixed
 Stokes cocycle hypotheses of
 Theorem~\ref{thm:irregular-kzb-composition}: via
 Theorem~\ref{thm:irregular-kzb-composition-generic-level}
 (irregular KZB formal type + Boalch wild groupoid).
...
resolved only under the boundary formal-type and mixed Stokes
cocycle hypotheses. Residual frontier: higher-pole irregular type
(Nekrasov
...
irregular KZB theorem under boundary formal-type hypotheses on the
generic nonrational locus, and
...
Combined with the degree-\(\le2\) comparison \(\varphi\) of
Remark~\ref{rem:curved-dunn-complex-bridge},
Theorem~\ref{thm:mb-H2-vanishing} transfers, under its hypotheses, to
\[
 H^2_{\mathrm{curved\text{-}Dunn}}(g) \;=\; 0
 \qquad (g \ge 2),
\]
at the cohomological level. Thus curved-Dunn \(H^2\)-vanishing follows
by transport under hypotheses: modular-bootstrap acyclicity plus the
degree-\(2\) comparison imply the curved-Dunn conclusion. KZB
Stokes-sector composition is a separate analytic input for boundary
composition and does not prove this \(H^2\)-vanishing. The genus-\(1\)
case is the separate twisted tensor criterion of
...
$E_1$-bar once a genus-one twisting cochain
\(\tau_1=\mathrm{Mon}(R)\) satisfying the curved tensor-product
Maurer--Cartan equation is supplied. The criterion appears as
\Cref{prop:genus1-twisted-tensor-product} in the curved-Dunn
higher-genus chapter, consumed here as the
seed for the modular uncurving of this section.

% =====================================
\subsection{Section D: Cross-genus Maurer--Cartan equation}
\label{subsec:cross-genus-mc}

\begin{theorem}[Cross-genus Maurer--Cartan under bootstrap acyclicity]
\label{thm:cross-genus-mc}

Let $\Theta = \sum_{g \ge 0} \hbar^{2g} \Theta^{(g)}
\in \mathfrak{g}^{\cA}_{\mathrm{mod}}\llbracket \hbar \rrbracket$
be the total modular datum assembled from the genus tower. Assume the
modular-bootstrap degree-\(2\) obstruction vanishes at every positive
genus needed in the recursion. Then
\[
...
$H^2(\mathfrak{g}^{\cA}_{\mathrm{mod}}) = 0$ in the relevant
degree. This is exactly the bootstrap acyclicity hypothesis; it is
not supplied by the curved-Dunn comparison or by KZB Stokes data.
\end{proof}
...
recursion, whose solvability is the stated modular-bootstrap
\(H^2\)-acyclicity input, not a Kan property.
\end{remark}
...
\item \emph{Equivariance:} compatibility with the $\Sigma_n$ action
 via the braided annular datum: quasi-triangularity of $R$, YBE
 braid coherence, pure-braid descent, and inverse orientation.
\item \emph{Unitality:} the vacuum/unit is a strict identity for every
 unit-normalised annular sewing datum; the unit seam is the
 counit-normalised diagonal bicomodule.
\end{enumerate}
\end{theorem}
...
nonrational level under the boundary formal-type and mixed Stokes
hypotheses:
...
multipliers under the same Stokes cocycle compatibility hypothesis;
quasi-triangularity, pure-braid descent, and inverse orientation are
not inferred from the formal type alone.
(iii) Proposition~\ref{prop:modular-operad-unitality} via the
unit/counit normalisation of the annular seam; unaffected by the
passage to irregular-singular data once the same normalisation is
included in the sewing datum.
\end{proof}
...
The two flavours of ``associativity'' separate, then reunify.
...
Theorem~\ref{thm:irregular-kzb-composition} and the degree-\(2\)
comparison of Theorem~\ref{thm:curved-dunn-bridge}, (Abs) implies
(Conc).
Explicitly: for any logarithmic $\SCmod$-algebra $\cA$ on a covered
level locus satisfying the boundary formal-type and mixed Stokes
hypotheses, \emph{if} \(D^2 = 0\) on \(\Bmod(\cA)\) and the
low-degree comparison identifies the obstruction class, \emph{then}
...
analytic data making $\tau_g$ well-defined at generic~$k$ under the
boundary formal-type hypothesis. The degree-\(2\) comparison of
\(\Phi\) transfers cocycle-level obstruction information to
composition-level obstruction information:
$D\Theta^{(g)} = 0 \Rightarrow d_0\tau_g +
\tfrac{1}{2}[\tau_g, \tau_g] = 0 \Rightarrow$ pentagon for
$B^{\mathrm{ann}}$-sewing.
\end{proof}

\begin{remark}[Infrastructure summary]
\label{rem:modular-infrastructure-summary}
Combining Sections~A--F:
\begin{enumerate}
\item Modular operad composition: under hypotheses at all genera on the covered
 level loci of Corollary~\ref{cor:fm68-all-levels-resolved}
 (Theorem~\ref{thm:concrete-operadic-composition}).
\item Curved Dunn additivity: the genus-\(1\) twisted tensor
 criterion and the \(g\ge2\) transport theorem under hypotheses
 (Corollary~\ref{cor:curved-dunn-all-genera}).
\item Genus-$1$ twisted tensor criterion
 (Proposition~\ref{prop:genus1-twisted-tensor-product}).
\item Cross-genus Maurer--Cartan: under modular-bootstrap
 acyclicity
 (Theorem~\ref{thm:cross-genus-mc}).
\item Abstract $D^2 = 0$ and concrete composition: separated and
 reunified under irregular-singular regularization plus the
 degree-\(2\) comparison
...
\begin{itemize}[nosep]
\item \textbf{(Genus-$1$ criterion and \(g\ge2\)
curved-Dunn \(H^2\).)}
 Theorem~\ref{thm:curved-dunn-bridge} +
 Corollary~\ref{cor:curved-dunn-all-genera} are the
 \emph{operadic-side} criterion; the \emph{factorization-side}
 criterion is the explicit chain-map $\Phi$ of
 Theorem~\ref{thm:curved-dunn-H2-vanishing-all-genera} +
 Proposition~\ref{prop:modular-bootstrap-to-curved-dunn-bridge}.
 Both routes require modular-bootstrap \(H^2\)-vanishing and a
 degree-\(2\) comparison. The Jimbo--Miwa irregular-singular KZB
 framework supplies boundary composition under formal-type and Stokes
 hypotheses; it is not the proof of curved-Dunn \(H^2\)-vanishing.
...
\item \textbf{(Rung-$\cW_N$ and rung-$\cW_\infty$.)}
 ...
 ratio $\beta_N = 12(H_N - 1)$ stated as a conjecture in
 Chapter~\ref{ch:beta-N-closed-form-all},
 Conjecture~\ref{conj:beta-N-harmonic-closed-form}; the
 finite-rank tempering statement uses the finite-envelope criterion of
 Theorem~\ref{thm:wn-tempered-all-N}. The
 modular-operadic composition above supplies the required
 associativity on the covered formal-type loci.
\end{itemize}
The local model of this chapter is the universal disc-and-corner
calculus on which the climax theorem applies. At $g \ge 2$ the
upgrade from modular-cohomological to chain-level requires
the same bootstrap acyclicity, degree-\(2\) comparison, and boundary
formal-type hypotheses.
\end{remark}

\begin{remark}[First-principles comparison of the repaired statements]
...
\item[\textbf{(Two $H^2$ obstructions, not one.)}] The modular-bootstrap
 ...
 complexes; when the low-degree comparison hypotheses hold, they are
 connected by the degree-\(2\) chain map
 (Theorem~\ref{thm:curved-dunn-bridge}).
\item[\textbf{(Generic non-integral level.)}] Modular operad axioms at
 integer level are classical; the generic-$k$ composition statement
 requires boundary formal type and analytic Stokes data in
 the Jimbo--Miwa--Ueno framework
 (Theorem~\ref{thm:irregular-kzb-composition}).
\item[\textbf{(Genus-$1$ twisted tensor criterion.)}] The genus-1
 twisted-tensor-product claim is a criterion
 (Proposition~\ref{prop:genus1-twisted-tensor-product}) for a
 supplied twisting cochain.
\item[\textbf{(Cross-genus Maurer--Cartan.)}] Solvability is supplied by
 Theorem~\ref{thm:cross-genus-mc} under modular-bootstrap
 \(H^2\)-acyclicity.
\item[\textbf{(Cross-genus coherence.)}] Kan horn-filling is
 insufficient; the replacement is KZB-regularized
 composition
...
\begin{remark}[$24$ M5-brane insertions and the chiral boundary locus]
\label{rem:msc-24-M5-punctures}
Twisted $11$d supergravity on $K3 \times T^2$ with $24$ M5-branes
wrapping the $I_1$ Kodaira fibres supplies the $24$ marked chiral
insertions on $\Ran(E^{\mathrm{nod,sm}}_{24})$. These points are not
the open colour of $\SCmod$: the open colour is the topological
parameter direction over $\overline{\mathcal A_2}$. The $4d$
$\mathcal{N}=2$ parent is class-$\mathcal S$ $A_1$ on the
$24$-punctured sphere $\Sigma_{0,24}$ with maximal regular
$\mathfrak{su}(2)$ punctures (Gaiotto $2008$; Chacaltana--Distler
$2010$: $c_{4d}=26$, $a_{4d}=16$, Coulomb rank $21$, flavour
$\mathfrak{su}(2)^{24}$). The Steiner system $S(5,8,24)$
(Conway--Sloane $1988$) records the $M_{24}$-symmetric
$24$-point configuration on $\mathbb{P}^1$.
\end{remark}

\section{K3 topologisation interface}
\label{sec:msco-deep}

\begin{remark}[Modular Swiss-cheese as the K3 topologisation interface]
\label{rem:msco-deep-1}
The modular Swiss-cheese operad $\mathsf{SC}^{\mathrm{mod}}$ is the
two-coloured interface between the curve-level
$E_1^{\mathrm{chiral}}$ algebra, its derived-centre
$E_2^{\mathrm{chiral}}$ structure, and any later
$E_3^{\mathrm{top}}$ structure obtained after an explicit
topological homotopy. It is not itself a single-coloured
$E_3$-operad, and the Siegel-modular deformation does not erase the
chiral colour. At the K3 base the numerical data are
\[
 \operatorname{wt}(\Delta_5)
 = c_{\phi_{0,1}^{K3}}(0)/2 = 10/2 = 5,\qquad
 K^{\kappaChHodge}=2c_+(\mathrm{Mukai}(K3))=8,
 \qquad
 \Phi_{10}^{\mathrm{un}}=\Delta_5^2.
\]
The order-$\hbar^3$ pentagon-heptagon obstruction is the class of
\[
 \phi^{(3)}
...
\]
in the $\SCmod$ deformation complex. Vanishing of this class is the
precise condition for passing from the two-coloured modular interface
to an $E_3^{\mathrm{top}}$ cohomology action.
\end{remark}
...
\section{Nakajima K3 Hilbert tower and local Maulik--Okounkov comparison}
\label{sec:msco-MO-yangian-K3-hilb}

\begin{remark}[Hilbert schemes of K3 and stable-envelope hypotheses]
...
$\mathrm{Hilb}^n(\mathrm{K3}) \to \mathrm{Sym}^n(\mathrm{K3})$.
Nakajima and Grojnowski construct the Heisenberg action on
$\bigoplus_n H^*(\mathrm{Hilb}^{[n]}(\mathrm K3))$.

The Maulik--Okounkov (MO) stable-envelope construction
(Maulik--Okounkov 2019, Ast\'erisque 408, \S13) applies to a
symplectic resolution $\pi\colon X\to X_0$ equipped with a
Hamiltonian torus action and suitable fixed-locus/polarisation data:
stable envelopes
...
Compact $\mathrm{K3}$ does not provide the affine Hamiltonian torus
geometry of $\mathbb A^2$ or a generic isolated-fixed-point chamber.
Thus the global K3 Hilbert tower supplies a Nakajima--Grojnowski
Heisenberg/Fock module, while MO $R$-matrices enter only after choosing
an equivariant local/quiver model or another symplectic-resolution
replacement with the MO hypotheses.
\end{remark}

\begin{remark}[Local MO Yangian and compact K3 Fock module]
\label{rem:msco-K3-hilb-MO-yangian}
For an equivariant symplectic-resolution model $X$ satisfying the MO
hypotheses, two generic chambers
$\mathfrak{c}_\pm \subset \mathrm{Lie}_{\mathbb R}(T)$ define the MO
$R$-matrix
\[
R^{\mathrm{MO}}_X(u) \;=\;
\mathrm{Stab}_{\mathfrak{c}_-}^{-1} \circ
\mathrm{Stab}_{\mathfrak{c}_+} \;\in\;
\mathrm{End}\bigl(H^*_T(X)\bigr)(u)
\]
where $u$ is the equivariant spectral parameter. Maulik--Okounkov
Thm.~13.1 gives the Yang--Baxter equation and the associated RTT
Yangian for this local/quiver model. Separately, the compact K3 tower
carries the Nakajima--Grojnowski Heisenberg action attached to the
Mukai lattice $H^{2*}(\mathrm{K3},\mathbb Z)$ of signature $(4,20)$.
The comparison object for Vol~II is therefore not a global MO Yangian
on $\mathrm{Hilb}^{[n]}(\mathrm K3)$; it is a comparison between local
MO chamber systems and the compact Mukai Fock module.
\end{remark}

\begin{remark}[Distinction from Schiffmann--Vasserot $\mathrm{CoHA}(\mathbb{A}^2)$]
\label{rem:msco-K3-hilb-vs-A2}
The compact K3 Hilbert tower is structurally distinct from the
Schiffmann--Vasserot construction on
...
\item \textbf{Ambient geometry.} $\mathbb A^2$ is non-compact affine
 ...
 action; compact $\mathrm{K3}$ carries no such global affine scaling
 action. Consequently the equivariant parameters
 $(\epsilon_1, \epsilon_2)$ on $\mathbb A^2$ have no direct analogue
 on compact $\mathrm{K3}$.
\item \textbf{CoHA output.} Schiffmann--Vasserot 2013 (Publ.~Math.~IHES
 ...
 $\mathcal{D}_\hbar(\mathrm{CoHA}_{\mathrm{K3} \times E})$.
 Thus $\mathrm{CoHA}$ is not a Yangian: it is the positive shuffle
 half, while the full quantum group is the doubled Hall object.
\item \textbf{Lie-algebra underlying.} On $\mathbb A^2$ the local MO Yangian
 ...
 compact K3 comparison target is therefore not
 $Y_\hbar(\mathfrak{g}_{\Delta_5})$ (no J-presentation); it is the Hall--Drinfeld
...
\begin{remark}[Modular Swiss-cheese as local-MO compatibility witness]
\label{rem:msco-K3-hilb-compatibility}
Let $\mathrm{Loc}_{\mathrm{MO}}$ be a local/quiver MO chamber system
attached to an equivariant symplectic-resolution model of a K3
puncture chart. A comparison with $\SCmod$ is a morphism from this
local chamber system to the two-coloured factorisation datum on
$(\Ran(E^{\mathrm{nod,sm}}_{24}),\overline{\mathcal A_2})$: the
closed colour records the $24$ chiral insertion loci, and the open
colour records the topological Siegel-parameter direction. The
averaging morphism
$\mathrm{av}=\mathrm{Torelli}_{\mathrm K3}\circ
\mathrm{KugaSatake}\circ j_{\mathrm{Kodaira}}$ supplies the candidate
pullback between the two bases. The comparison is defined exactly on
the locus where the following obstruction vanishes:
\[
 \mathfrak{o}_{\mathrm{MO/SC}}
 \in
 H^1\!\left(
 \overline{\mathcal A_2},
 \operatorname{Cone}\bigl(
 \mathrm{Loc}_{\mathrm{MO}}\to
 \mathrm{Loc}_{\SCmod}
 \bigr)
 \right),
\]
which measures failure of the averaging pullback to identify the MO
chamber local systems with the two colours of $\SCmod$.
\end{remark}

\begin{remark}[Reduced DT scalar on the $\overline{\mathcal A_2}$-face]
\label{rem:msco-K3xE-DT-partition}
The scalar character to be matched by the closed-colour sector of
$\mathsf{SC}^{\mathrm{mod}}$ on the K3$\times E$ factorisation base is
the reduced DT series pulled back to the Siegel parameter
...
Oberdieck--Pixton 2016 (arXiv:1706.10100). Here
$D_5=64^{-1}\Delta_5$ is the monic product and
$\Phi_{10}^{\mathrm{OP}}=D_5^2$ is the
Oberdieck--Pandharipande scalar square, while
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ is the unnormalised Igusa square.
The equality is a scalar character theorem; it becomes an
$\SCmod$ character statement only after the factorisation character map
and the obstruction $\mathfrak{o}_{\mathrm{MO/SC}}$ above are fixed.
Primary sources:
...
The compact K3 Hilbert tower has a strict Nakajima--Grojnowski
Heisenberg action. A local MO model, when present, has strict
Yangian $R$-matrices. The $\SCmod$ question is the compatibility of
these two pieces with the genus-completed K3$\times E$ character.

\begin{theorem}[Nakajima stabilisation criterion through $\mathsf{SC}^{\mathrm{mod}}$]
\label{thm:msco-GN-stabilisation-chain-level}

For each $n\geq 0$, the ``add a point at infinity'' transition map
$\iota_n\colon H^*(\mathrm{Hilb}^{[n]}(\mathrm K3))\to
H^*(\mathrm{Hilb}^{[n+1]}(\mathrm K3))$ of Nakajima 1997
\emph{Ann.\ Math.}~145 Thm.~3.10 (realised as the action of the
Heisenberg creator $\alpha_{-1}[\omega_{\mathrm K3}]$) lifts to a
chain-level map of $\mathsf{SC}^{\mathrm{mod}}$-modules exactly when
the local-MO comparison obstruction
\[
 \mathfrak{o}_n
 =
 \left[
 R_{n+1}^{\mathrm{loc}}(u)\circ(\iota_n\otimes\iota_n)
 -(\iota_n\otimes\iota_n)\circ R_n^{\mathrm{loc}}(u)
 \right]
 \in
 \operatorname{HH}^2(\mathcal P_n\otimes\mathcal P_n,
 \mathcal P_{n+1}\otimes\mathcal P_{n+1})
\]
vanishes. When $\mathfrak{o}_n=0$, a lift
\[
 ...
 \qquad
 \mathcal P_n := H^*(\mathrm{Hilb}^{[n]}(\mathrm K3)),
\]
can be chosen so that the square
\[
\begin{tikzcd}[column sep=1.5em]
\mathcal P_n\otimes\mathcal P_n
\arrow[r, "R^{\mathrm{loc}}_n(u)"]
\arrow[d, "\iota_n\otimes\iota_n"'] &
\mathcal P_n\otimes\mathcal P_n
\arrow[d, "\iota_n\otimes\iota_n"] \\
\mathcal P_{n+1}\otimes\mathcal P_{n+1}
\arrow[r, "R^{\mathrm{loc}}_{n+1}(u)"'] &
\mathcal P_{n+1}\otimes\mathcal P_{n+1}
\end{tikzcd}
\]
...
The only additional datum needed for an $\SCmod$-module lift is
compatibility with the local $R$-matrix system. That compatibility is
precisely the displayed Hochschild class $\mathfrak{o}_n$: if it
vanishes, the difference between the two paths around the square is a
Hochschild coboundary $\partial_{\mathrm{Hoch}}\eta_n$, and
$\eta_n$ supplies the chain homotopy defining $\widetilde\iota_n$.
Conversely, any chain-level $\SCmod$ lift gives such a homotopy, hence
annihilates $\mathfrak{o}_n$.

\emph{Verification paths:}
(i) Nakajima 1997 Thm.~3.10 gives the Heisenberg transition operator;
(ii) the local MO $R$-matrix is available only on a model satisfying
the MO stable-envelope hypotheses;
(iii) the obstruction class above is the Hochschild boundary of the
commutator square and is therefore the exact chain-level criterion.
\end{proof}
...
$\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$ if all classes
$\mathfrak{o}_n$ vanish and if the local dynamical associator data
solve the $\SCmod$ pentagon. Under these hypotheses the
$A_\infty$-bialgebra tower $\{m_k, \Delta_k\}_{k\geq 2}$ is governed by
\begin{itemize}
\item $m_2 =$ Nakajima--Grojnowski Heisenberg product on the compact
 Mukai Fock block;
\item $\Delta_2 =$ the local FRT coproduct transported through the
 chosen local-MO comparison;
...
\item $\{m_k, \Delta_k\}_{k\geq 4}$ given by the Etingof--Kazhdan
 ...
 \hbar^2\,\partial_Z\log\Phi_{10}^{\mathrm{un}}(Z)$.
\end{itemize}
Independently of this module-lift condition, the scalar character
target is
\[
 ...
 \;=\;
 \bigl(\Phi_{10}^{\mathrm{un}}(\Omega)\bigr)^{-1}.
\]
\end{theorem}

\begin{proof}
If all $\mathfrak{o}_n$ vanish, Theorem~\ref{thm:msco-GN-stabilisation-chain-level}
gives compatible transition maps through the local $R$-matrix system.
The completed colimit then inherits the closed-colour operations by
passage to the genus-completed tensor product. The higher operations
are exactly the chosen local dynamical associator data; their remaining
obstruction is the pentagon class of
Remark~\ref{rem:msco-deep-1}. Thus the statement is a module-lift
theorem under those hypotheses, not a consequence of the scalar
character alone.

Graded-character agreement with $(\Phi_{10}^{\mathrm{un}})^{-1}$: the $T$-equivariant Euler
...
identifies the full Mukai-equivariant product with
$(\Phi_{10}^{\mathrm{un}})^{-1}$. The OP/DT scalar conversion is the
separate normalisation recorded in
Remark~\ref{rem:msco-K3xE-DT-partition}.

\emph{Verification paths:}
(i) Grojnowski--Nakajima Fock character;
(ii) G\"ottsche's product for Hilbert schemes of points on K3;
(iii) Borcherds--Gritsenko--Nikulin
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$.
\end{proof}

\begin{remark}[BKM quasi-Hopf closure vs MO strict Yangian]
\label{rem:msco-BKM-vs-MO-range}
The local MO finite-$n$ action, when its hypotheses are met, is a
strict Yangian action. The compact K3 colimit is controlled by the
Mukai Fock module and by the BKM imaginary-root extension; the dynamical
Siegel-modular associator $\Phi^{\mathrm{Sieg,dyn}}$ is the obstruction
to strict coassociativity in the comparison target. Compare
Vol~III Chapter~\ref{V3-ch:modular-trace}, at the
super-quasi-Hopf comparison remark.
\end{remark}
