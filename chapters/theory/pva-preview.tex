...
\begin{itemize}[leftmargin=2em]
  ...
  \begin{equation}
    \label{eq:A-infty-chiral}
    m_k \colon A^{\otimes k} \longrightarrow A((\lambda_1))\cdots((\lambda_{k-1})) \qquad\text{of degree } 1-k,
  \end{equation}
  satisfying the (chiral) Stasheff relations, and with $m_1=Q$.
\end{itemize}

We state the analytic locality and renormalization hypotheses for completeness.
...
We begin by canonically splitting the binary operation $m_2$ into its regular and singular parts in the spectral parameter.
...
Observe that both $m_2^{\mathrm{reg}}$ and $m_2^{\mathrm{sing}}$ are of degree $-1$ as cochains. Furthermore, we will use graded symmetrization/antisymmetrization:
\[
...
\begin{definition}[Classical product on $H$]\label{def:product}
For classes $[a],[b]\in H$, choose representatives $a,b\in A$ and define
\begin{equation}\label{eq:product-def}
[a]\cdot [b] \;:=\; \big[\, \Sym\big(m_2^{\mathrm{reg}}(a,b),\, m_2^{\mathrm{reg}}(b,a)\big)\,\big] \;\in H.
\end{equation}
\end{definition}

\begin{remark}[Degree accounting]\label{rem:degree-shift}
The product $\cdot$ is obtained from $m_2^{\mathrm{reg}}$ by restricting to $\lambda = 0$ (the constant term in the regular Laurent expansion). Since the spectral parameter $\lambda$ carries degree $+1$, evaluating at $\lambda = 0$ shifts the degree by $+1$, giving a degree-$0$ product. Similarly, the $\lambda$-bracket has degree $0$ as a map to $V[\lambda]$ since each power $\lambda^n$ absorbs $n$ units of degree.
\end{remark}

\begin{lemma}[Well–definedness; \ClaimStatusProvedHere]\label{lem:product-welldefined}
...
\]
with no additional Koszul signs, since the sign $\varepsilon(s,j)=(1-1)\degree{a}=0$ for all terms at $n=2$. This uses the fact that $Q = m_1$ commutes with the Laurent projection, since $Q$ acts on the field-algebra coefficients and does not involve the spectral parameter $\lambda$. Projecting to the regular part, we see that the change in $\Sym(m_2^{\mathrm{reg}}(a,b),m_2^{\mathrm{reg}}(b,a))$ is $Q$–exact. The same holds for $b\mapsto b+Qy$. Thus the class \eqref{eq:product-def} is independent of representatives.
\end{proof}

\begin{lemma}[Graded commutativity on $H$; \ClaimStatusProvedHere]\label{lem:commutativity}
...
\begin{proposition}[Associativity on $H$; \ClaimStatusProvedHere]\label{prop:associativity}
...
\begin{definition}[Vertex Poisson bracket]\label{def:lambda-bracket}
For $[a],[b]\in H$, define
\begin{equation}\label{eq:lambda-bracket}
...
\end{definition}
\begin{lemma}[Well–definedness of the $\lambda$–bracket; \ClaimStatusProvedHere]\label{lem:bracket-welldefined}
...
\begin{proposition}[Sesquilinearity and skewsymmetry; \ClaimStatusProvedHere]\label{prop:sesqui-skew}
...
\begin{theorem}[Jacobi identity; \ClaimStatusProvedHere]\label{thm:Jacobi}
...
\end{proof}
\begin{theorem}[Leibniz rule; \ClaimStatusProvedHere]\label{thm:Leibniz}
...
\begin{corollary}[PVA structure; \ClaimStatusProvedHere]\label{cor:PVA-structure}
...
\begin{proposition}[Arnold relations and Jacobi; \ClaimStatusProvedHere]\label{prop:AOS-implies-Jacobi}
