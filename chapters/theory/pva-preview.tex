...
\]
where $F^{\ge0}(\lambda)\in A[\![\lambda]\!]$ is the regular (non–negative power) part and $F^{<0}(\lambda)\in \lambda^{-1}A[\![\lambda^{-1}]\!]$ is the singular (negative power) part. Define
\[
...
Both $m_2^{\mathrm{reg}}$ and $m_2^{\mathrm{sing}}$ have degree $-1$ as cochains. Use graded symmetrization/antisymmetrization:
\[
...
\begin{lemma}[Well-definedness]% label removed: lem:product-welldefined
The operation \eqref{eq:product-def} is independent of the choice of representatives of $[a]$ and $[b]$.
\end{lemma}

\begin{proof}
Suppose $a\mapsto a+Qx$; using the $A_\infty$ identity at degree $n=2$,
\[
...
\begin{lemma}[Graded commutativity on $H$]% label removed: lem:commutativity
...
\begin{proposition}[Associativity on $H$]% label removed: prop:associativity
The product $\cdot$ is associative on $H$.
\end{proposition}

\begin{proof}
Let \(\mu(a,b)=\mu_2^{\mathrm{reg}}(a,b;0)\) be the ordinary
unsuspended regular product, whose suspended bar component is
\(m_2^{\mathrm{reg}}\).  Consider the chain-level associator
\[
\alpha(a,b,c):=\mu(\mu(a,b),c)-\mu(a,\mu(b,c)).
\]
The $A_\infty$ identity at degree $3$ is first written in the
suspended \(m_2\)-coordinates, where the second binary composition
carries the Stasheff sign.  Transporting both binary vertices through
the bar desuspension to the \(\mu_2^{\mathrm{reg}}\)-coordinates turns
that signed binary part into the ordinary associator above.  The
regular--regular constant-term projection of the \(m_3\)-term is a
\(Q\)-boundary, so \(\alpha(a,b,c)\in\operatorname{im}Q\).  Hence
\([\alpha(a,b,c)]=0\) in \(H\), and the product is associative.
\end{proof}
...
\{\, [a] {}_\lambda [b] \,\} \;:=\; \Big[\, \tfrac{1}{2}\big( m_2^{\mathrm{sing}}(a,b;\lambda) \;-\; (-1)^{\degree{a}\degree{b}}\, e^{-(\lambda+\partial)\partial_\lambda}\, m_2^{\mathrm{sing}}(b,a;\lambda)\big)\,\Big] \;\in H[\![\lambda]\!].
\end{equation}
Here the operator $e^{-(\lambda+\partial)\partial_\lambda}$ implements the vertex algebra skew-symmetry transformation $\lambda\mapsto -\lambda-\partial$, which is \emph{not} the same as naive antisymmetrization for $\lambda$-dependent series.  The visible PVA2 sign is the ordinary Koszul sign; shifted signs belong to the nested residue orientation in the Jacobi calculation.
\end{definition}
\begin{lemma}[Well-definedness of the $\lambda$-bracket]% label removed: lem:bracket-welldefined
...
\begin{proposition}[Sesquilinearity and skewsymmetry]% label removed: prop:sesqui-skew
The bracket $\{\cdot {}_\lambda \cdot\}$ on $H$ satisfies:
\begin{enumerate}[label=(\alph*),leftmargin=2em]
 \item skewsymmetry: $\{\, [a] {}_\lambda [b] \,\} = - (-1)^{\degree{a}\degree{b}} \{\, [b] {}_{-\lambda-\partial} [a] \,\}$;
...
Skew-symmetry follows from the definition of the bracket via antisymmetrization with the exponential shift operator $e^{-(\lambda+\partial)\partial_\lambda}$, which by construction imposes $\{\, [a] {}_\lambda [b] \,\} = -(-1)^{\degree{a}\degree{b}}\{\, [b] {}_{-\lambda-\partial} [a] \,\}$.
\end{proof}
\subsection{Jacobi identity and derivation (Leibniz) property}
\begin{theorem}[Jacobi identity]% label removed: thm:Jacobi
...
\end{theorem}
\begin{proof}
At the chain level, the Jacobi identity is encoded by the $A_\infty$ identity at degree $3$, projected to the \emph{singular} part of three-point functions. The four boundary strata of $\FM_3(\C)$ are $D_{12}$, $D_{13}$, $D_{23}$, and $D_{123}$. The stratum $D_{123}$ corresponds to the total collision of all three points. Its contribution is $Q$-exact (it corresponds to $m_1(m_3(a,b,c))$ in the $A_\infty$ identity) and vanishes on cohomology classes. The three pairwise collision strata contribute the signed Jacobiator
\[
\{a_\lambda\{b_\mu c\}\}
-
...
\]
with the \(D_{13}\) sign coming from the incidence orientation and shifted transposition. Its cancellation follows from the classical Arnold relation among logarithmic 1-forms:
\[
...
\end{proof}
\begin{theorem}[Leibniz rule]% label removed: thm:Leibniz
...
\end{theorem}
\begin{proof}
At the chain level this is the statement that the signed mixed
regular--singular Leibniz defect
\[
...
b\cdot\{a{}_\lambda c\}
\]
is a \(Q\)-boundary.  Project the arity-\(3\) Stokes identity on
\(\FM_3(\C)\) to the sector where exactly one binary channel is
singular and the other is the regular constant term.  The oriented
faces \(D_{\{2,3\}},D_{\{1,2\}},D_{\{1,3\}}\) give the three displayed
summands of \(\mathcal L_\lambda\), with the \(D_{\{1,3\}}\) sign
coming from moving the degree-\((-1)\) bracket past \(b\).  The total
collision face contributes only the \(Q\)-exact \(m_3\)-homotopy, so
\(\mathcal L_\lambda(a;b,c)=0\) in cohomology.  Since this mixed
sector contains one singular channel and one regular channel, no
singular--singular Wick integral correction appears.
\end{proof}

\begin{corollary}[PVA structure]% label removed: cor:PVA-structure
...
\begin{proposition}[Arnold relations and Jacobi]% label removed: prop:AOS-implies-Jacobi
