...
(Hypothesis~H3(a)), and the contractibility of the ordered topological
...
By Hypothesis~H3(a), the weight form defining $m_k$ factors as
\begin{equation}
...
Hypothesis~H3(e)), and the cone $\Gamma_{k-1}$ has topological boundary
...
singularities (Hypothesis~H3(b)) and the topological cone $\Gamma_2$ has finite volume
...
of Hypothesis~H3(a).  Without this factorisation, the topological contraction
...
\item D4 is handled by the mixed three-face Stokes argument;
\item D5 is handled by the full three-face singular Stokes argument;
\item D6 is handled by the topological contraction of $\Conf_k^{<}(\R)$, using the
      factorisation $\widetilde\omega_k=\omega_k^{\mathrm{hol}}\wedge\omega_k^{\mathrm{top}}$
      (Hypothesis~H3(a)) to ensure compatibility with the holomorphic residue calculus.
\end{itemize}
