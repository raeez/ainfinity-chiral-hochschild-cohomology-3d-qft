\section{\texorpdfstring{Orientations on $\FM_k(\C)\times \Conf_m(\R)$ and Stokes' theorem}{Orientations on FM k(C) x Conf m(R) and Stokes' theorem}}
...
\subsection{\texorpdfstring{Orientation on $\FM_k(\C)$}{Orientation on FM k(C)}}
\label{subsec:orient-FM}

In this appendix \(\FM_k(\C)\) means the translation-reduced
Fulton--MacPherson compactification
\(\FM_k^{\mathrm{tr}}(\C)=\FM_{\C}(k)/\C\). It is a smooth manifold
with corners of real dimension \(2(k-1)\). The boundary cluster fibres
are scale-reduced spaces \(\FM_\ell^{\mathrm{red}}(\C)\); the full
complex-affine quotient is not used in the Stokes signs below. We
orient \(\FM_k(\C)\) using the complex structure: if
\(z_1,\ldots,z_k\) are holomorphic coordinates on \(\C^k\), the
quotient coordinates \(w_i=z_i-z_k\) for \(i=1,\ldots,k-1\) give an
orientation via
\begin{equation}
...
\subsection{\texorpdfstring{Orientation on $\Conf_m(\R)$}{Orientation on Conf m(R)}}
...
\subsection{\texorpdfstring{Explicit boundary orientation signs for $\FM_3(\C)$}{Explicit boundary orientation signs for FM 3(C)}}
\label{subsec:FM3-orientations}

This subsection computes the orientation signs $\epsilon_F$ for all four codimension-1 boundary faces of $\FM_3(\C)$ in complete detail.
...
\subsubsection{\texorpdfstring{Face $D_{\{1,2\}}$: collision of points 1 and 2}{Face D12: collision of points 1 and 2}}
...
\]
To track the sign, reorder the wedge product. Then
\[
...
\Omega_3 = -\varepsilon\,(-d\varepsilon)\wedge du_x\wedge du_y\wedge d\theta.
\]
The FM blow-up records the sign of the leading coefficient of the
orientation density.  The positive factor \(\varepsilon\) is the polar
Jacobian; it is discarded when comparing orientations, not evaluated
as a boundary volume.  Thus
\[
\operatorname{or}(D_{\{1,2\}})
=-\,du_x\wedge du_y\wedge d\theta
\]
with respect to the induced outward-normal-first boundary orientation.
The sign used in the Stokes-residue formula is the sign of the
oriented boundary term after this boundary orientation is paired with
the regularized residue convention
\(\Res(\omega)=(\varepsilon\,\iota_{\partial_\varepsilon}\omega)
|_{\varepsilon=0}\).
Thus the incidence number below is the sign of the Stokes-residue
summand, not merely the coefficient of the boundary volume form.  The
logarithmic residue itself extracts the coefficient of
\(d\log\varepsilon=d\varepsilon/\varepsilon\) as in
Definition~\ref{def:residue_divisor}.
...
\subsubsection{\texorpdfstring{Face $D_{\{2,3\}}$: collision of points 2 and 3}{Face D23: collision of points 2 and 3}}
...
\]
The induced boundary orientation $\iota_{-\partial_\varepsilon}\Omega_3 = -\varepsilon\,dx_1\wedge dy_1\wedge d\theta$ is compared with the standard \emph{product} orientation on
\[
D_{\{2,3\}}\cong
\FM_2^{\mathrm{tr}}(\C)\times\FM_2^{\mathrm{red}}(\C),
\]
namely $dx_1\wedge dy_1$ (outer, recording $w_1$) followed by $d\theta$ (inner $S^1$); the outer-then-inner reordering supplies the compensating sign, so that
\begin{equation}
\label{eq:eps-D23}
\boxed{\epsilon_{D_{\{2,3\}}} = +1.}
\end{equation}

\subsubsection{\texorpdfstring{Face $D_{\{1,3\}}$: collision of points 1 and 3}{Face D13: collision of points 1 and 3}}
...
\subsubsection{\texorpdfstring{Face $D_{\{1,2,3\}}$: total collision}{Face D123: total collision}}
...
\subsubsection{\texorpdfstring{Summary of orientation signs for $\FM_3(\C)$}{Summary of orientation signs for FM 3(C)}}

\begin{proposition}[Orientation signs for $k=3$]
...
These signs agree with those stated in Section~\ref{sec:FM_calculus}:
\(\epsilon_{D_{\{1,2\}}}=+1\),
\(\epsilon_{D_{\{1,3\}}}=-1\), and
\(\epsilon_{D_{\{2,3\}}}=+1\).  The total collision sign
\(\epsilon_{D_{\{1,2,3\}}}=-1\) determines the coefficient of the
\(m_1\circ m_3\) terms in the \(A_\infty\) identity.
\end{remark}

\subsection{\texorpdfstring{Worked example: Stokes' theorem on $\FM_3(\C)$}{Worked example: Stokes' theorem on FM 3(C)}}
...
The form is closed, $d\omega = 0$. Since $d\log f$ is closed ($d(d\log f) = d(df/f) = 0$ away from $\{f=0\}$), and the wedge product of closed forms is closed:
\[
...
\item $D_{\{1,3\}}$: the non-consecutive collision, zero only after
the open planar \(E_1\) projection,
...
\subsection{\texorpdfstring{General formula for $\epsilon_F$}{General formula for epsilon F}}
\label{subsec:general-eps}

The following gives the exact local prescription for the orientation sign
$\epsilon_F$ for an arbitrary collision divisor
$D_S \subset \FM_k(\C)$.
...
Choose a positively oriented chart
$(y_1,\ldots,y_{2k-3})$ on the boundary stratum $D_S$ and write the
ambient orientation form near $D_S$ as
\begin{equation}
\label{eq:general-eps-formula}
\Omega_k
=
\varepsilon_S^{2p-3}\,
d\varepsilon_S\wedge
J_S(y)\,dy_1\wedge\cdots\wedge dy_{2k-3}
+O(\varepsilon_S^{2p-2}).
\end{equation}
Then, relative to the outward-normal-first convention
\eqref{eq:outward-normal-rule}, the boundary sign is
\begin{equation}
\label{eq:sigma-S-formula}
\epsilon_{D_S}=-\operatorname{sgn}(J_S).
\end{equation}
Equivalently, the sign is the product of the Jacobian sign of the FM
coordinate change, the parity moving \(d\varepsilon_S\) to the normal
slot, the single outward-normal sign, and the comparison between the
induced boundary orientation and the ordered product orientation on the
outer and inner FM factors.
\end{proposition}

\begin{proof}
The FM chart expresses the ambient coordinates
$(x_1,y_1,\ldots,x_{k-1},y_{k-1})$ as functions of
$(\varepsilon_S,y_1,\ldots,y_{2k-3})$. Expanding the determinant of this
change of variables gives \eqref{eq:general-eps-formula}; the power
$\varepsilon_S^{2p-3}$ is the polar radial Jacobian for the relative
configuration of the $p$ colliding points modulo scale. Since
$\varepsilon_S$ is inward-pointing, the boundary convention is
$\Omega_k=(-d\varepsilon_S)\wedge\Omega_{D_S}$, which contributes the
minus sign in \eqref{eq:sigma-S-formula}. The coefficient \(J_S\)
is computed after the boundary coordinates have been ordered as the
outer FM factor followed by the relative FM factor; this is precisely
where the product-orientation comparison enters.
\end{proof}

\begin{corollary}[Pairwise collision signs for general $k$]
For $k$ points in $\C$ and a pairwise collision
$S=\{i,j\}$ with $1\le i<j\le k$, the sign
$\epsilon_{D_{\{i,j\}}}$ is obtained by applying
Proposition~\ref{prop:general-orient-sign} to the FM coordinate
system in which the relative coordinate is $z_i-z_j$ and the remaining
outer coordinates are ordered by the convention
\ref{conv:coord-ordering}. Thus no parity formula depending only on
$i$ is invariant under all admissible FM charts.

For the $A_\infty$ applications, the relevant invariant is the
\textbf{combined sign}
\[
