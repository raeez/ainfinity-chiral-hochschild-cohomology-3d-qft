...
An $\Ainf$ chiral algebra is a $C_\ast(W(\SCchtop))$-algebra presented in coordinates: multilinear operations $\{m_k\}_{k \ge 1}$ satisfying sesquilinearity (encoding holomorphic translation covariance), the Stasheff identities (Stokes' theorem on the Fulton--MacPherson compactification $\FM_k(\C)$), and the planar ordering convention (from the $E_1$ colour on~$\R$).  The relationship between this axiomatic presentation and the operadic definition is an equivalence of categories (Theorem~\ref{thm:rectification} and Proposition~\ref{prop:operad-implies-axioms}): the axioms are neither weaker nor stronger than the operad, but a coordinate expression of it.
