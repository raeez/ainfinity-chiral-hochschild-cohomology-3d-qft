...
\item Truncation ($m_k = 0$ for $k \gg 0$) holds for theories whose
  ...
  Proposition~\ref{prop:virasoro-truncation} for the Virasoro case), and
  for a theory with $n$-valent interactions, $m_k = 0$ for
  $k > 2n - 1$.  For theories with infinitely many interaction vertices
  (such as nonlinear sigma models), tameness condition~(c) must be
  verified by convergence arguments on the completed tensor algebra.
\end{enumerate}
\end{remark}
