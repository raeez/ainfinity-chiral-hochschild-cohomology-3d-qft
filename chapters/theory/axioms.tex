...
\item \textbf{At $n=1$:}\; $m_1^2(x) = -m_2(m_0, x) - (-1)^{|x|}\, m_2(x, m_0)$.  The differential does not square to zero; rather, it squares to minus the commutator (graded) with the curvature.
...
\item The chiral Hochschild cochain complex
  ...
  \textup{(}Volume~I, Theorem~\textup{\ref{thm:thqg-brace-dg-algebra}}\textup{)}.
\item The cohomology
  ...
  \textup{(}Volume~I, Theorem~\textup{\ref{thm:thqg-swiss-cheese}}\textup{)}.
\end{enumerate}
