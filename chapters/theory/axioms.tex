...
Equation \eqref{eq:ainfty-relation-raw} encodes the fact that \emph{when a block of inputs is fused first}, the correct spectral parameter inserted in the outer operation is the \emph{sum} of the inner $\lambda$'s, reflecting translation covariance and the convention that all $\lambda_i$ are differences relative to the last slot. This is the algebraic avatar of momentum–conservation for formal distributions.
\end{remark}
\end{definition}
...
\begin{proposition}[Definition \ref{def:ainfty_chiral} Captures Physical Structure]
\label{prop:physical_justification}
Under Hypotheses \textrm{(H1)--(H4)}, our Definition \ref{def:ainfty_chiral} correctly formalizes the $A_\infty$ chiral algebra structure implicit in Costello--Gwilliam--Gaiotto--Dimofte for the following reasons:

\begin{enumerate}[label=(\roman*)]
\item \textbf{Factorization algebra framework:} Costello--Gwilliam \cite{CG17} establish that local observables in holomorphic field theories form factorization algebras on $\C$. Our operations $m_k$ with locality axiom encode the factorization product on $\Conf_k(\C)$.

\item \textbf{BV formalism derivation:} All operations arise from the quantum master equation
\[
\frac{1}{2}\{S_{\text{eff}}, S_{\text{eff}}\}_{\text{BV}} + \hbar \Delta S_{\text{eff}} = 0,
\]
expanded as Feynman diagrams. The $A_\infty$ identity (Equation~\ref{eq:ainfty-relation-raw}) encodes the quantum master equation of the BV formalism at the level of individual operations.

\item \textbf{Chiral structures from contour integrals:} The $\lambda$-bracket formula and form integrands match constructions in \cite{CDG20} and Khan--Zeng \cite{KZ25}. Spectral parameters $\lambda_i$ encode derivatives via $e^{\lambda z}$ generating functions, as in vertex algebra theory.

\item \textbf{Verification in examples:} Our definition recovers known structures:
\begin{itemize}
\item For $m_{k \geq 3} = 0$: ordinary Poisson vertex algebras (Example \ref{ex:free_chiral});
\item For holomorphic theories with one-loop finiteness: structures of Gwilliam--Grady--Williams \cite{GGW21};
\item Cohomology $H^\bullet(\A,Q)$ yields PVAs matching expectations from \cite{CDG20} (Theorem \ref{thm:cohomology_PVA}).
\end{itemize}

\item \textbf{Consistency with dimensional ascent:} Our definition generalizes Tamarkin's chiral Hochschild complex \cite{Tam02} and provides the 3d analogue of 2d Hochschild theory from Poisson sigma models (Section \ref{sec:chiral_hochschild_expanded}).
\end{enumerate}
\end{proposition}

\begin{remark}[Status of $A_\infty$ Chiral Algebras]
\label{rem:status}
Costello--Dimofte--Gaiotto \cite{CDG20} explicitly state that ``an $A_\infty$ analog of a vertex algebra\ldots is a structure that has yet to be fully defined mathematically.'' Our Definition \ref{def:ainfty_chiral} represents the \textbf{first mathematically rigorous axiomatization} of this structure, built on solid operadic foundations. Proposition \ref{prop:physical_justification} provides systematic evidence that we have captured the correct physical structure.
\end{remark}

\subsection{Factorization Axiom and Cluster Limits}

A crucial feature of $A_\infty$ chiral algebras is how operations behave as insertion points collide---the \emph{factorization} or \emph{cluster} property. This axiom connects the operations to the operadic structure of $\OHT$.
...
To make the operadic definition of $\OHT$ completely precise, we now present the coend formula producing the operations $m_k$.
