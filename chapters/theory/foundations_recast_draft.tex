...
\begin{theorem}[Homotopy-Koszulity; {\cite{Hoe09,HL12,GK94}};
...
\cite{Hoe09,HL12,GK94}. Kontsevich formality gives a quasi-isomorphism
...
\item \emph{Derived center.} The \emph{chiral derived center}
 ...
 \]
 is the algebraic closed-sector vertex of the boundary chart. A
 physical bulk interpretation requires the HT open/closed comparison
 data. On the Koszul locus, a family datum $H_H(A;S)$ identifies
 $\cZ^n$ with $H^n(K_{A,S})$ and supports the Gerstenhaber algebra
 in~$S$ by Theorem~H of Volume~I.  At critical affine level the
...
The open colour of the Swiss-cheese pair $(A^{\mathrm{bulk}}, A^{\mathrm{bdy}})$ carries additional Hopf-algebraic structure formalized in Vol~III (\S7 of the $E_1$-chiral algebras chapter): the $E_1$-chiral bialgebra axioms~(H1)--(H5). The coproduct $\Delta_z \colon A \to A \otimes_{E_1,z} A$ lives on the open ($E_1$/topological) colour, not the closed ($E_2$/chiral) colour. The $E_\infty$ averaging map $\mathrm{av} \colon B^{\mathrm{ord}}(A) \to B^{\Sigma}(A)$ destroys the Hopf data and keeps only the degree-$2$ scalar shadow: in abelian branches $\mathrm{av}(r(z))=\kappaChHodge$, while for non-abelian affine Kac--Moody the Sugawara shift gives $\kappaChHodge=\mathrm{av}(r(z))+\dim(\fg)/2$ (Vol~III, Proposition~\ref{prop:averaging-forgets-hopf}). The braided ($E_2$) structure is recovered via the Drinfeld center $\cZ(\Rep^{E_1}(A))$, not by direct construction on the closed colour. The universal coproduct at all spins is $\Delta_z(e_s) = \sum (-1)^k \binom{N_R - b}{k} z^k \, e_a^L \cdot e_b^R$ (Vol~III, Proposition~\ref{prop:universal-coproduct}).
\end{remark}
...
\item Together, $(\barB(A_b), d_{\barB}, \Delta)$ is an $E_1$ dg
 coassociative coalgebra: $d_{\barB}$ is a coderivation encoding
 the $A_\infty$-chiral structure, $\Delta$ is deconcatenation.
 This is the bar-cobar adjunction of Volume~I, Theorem~A.
 The $\SCchtop$ structure does not live here; it emerges in
 the chiral derived center (Theorem~\ref{thm:universal-bulk}).
...
curvature $\kappaChHodge(\cA) \cdot \lambda_1$ in the modular hierarchy:
...
$\kappaChHodge(\cA) \cdot \omega_g$ (Volume~I, Theorem~D) is the leading
scalar of this iteration. The shadow obstruction tower
$(\kappa, \mathfrak{C}, \mathfrak{Q}, \ldots)$ is the degree
...
$\kappaChHodge(\cA) \cdot \lambda_1$ (one-loop correction); each higher
...
\item \emph{Genus $1$}: the one-loop correction. The curvature
 $d_{\barB}^2 = \kappaChHodge(\cA) \cdot \omega_1$ is the Hodge anomaly:
 ...
 class. The modular characteristic $\kappaChHodge(\cA)$ is the
 gravitational anomaly coefficient: it controls the strength
 of one-loop gravity.

\item \emph{Genus $g \ge 2$}: the higher-loop corrections. The
 shadow obstruction tower $(\kappa, \mathfrak{C}, \mathfrak{Q}, \ldots)$
 extracts the gravitational observables at each degree. The
...
\subsection{The completed modular Koszul datum}
...
$\cC$, the \emph{completed modular Koszul datum} is:
\[
 \Pi^{\mathrm{oc}}_X(\cA)
 \;:=\;
 \bigl(\,
...
% ===================================================================
\section{The $\Eone$--$\Etwo$--$\Ethree$ taxonomy}
\label{sec:en-taxonomy}

The geometric escalation from a point to a three-dimensional
holomorphic--topological theory passes through ten stages. At each
stage, a new topologico-combinatoric stratified space appears, a
new operad or operadic structure governs the algebraic data, and
the structure lives on a specific mathematical object. The
following proposition records the precise operadic identity at
each stage.

\begin{proposition}[The $\Eone$--$\Etwo$--$\Ethree$ operadic taxonomy;
\ClaimStatusProvedHere]
\label{prop:en-taxonomy}
\index{operadic taxonomy|textbf}
\index{geometric escalation|textbf}
The ten stages of the geometric escalation, together with their
governing operads and carrier objects, are:
...
\toprule
\textbf{Stage} & \textbf{Space} & \textbf{Operad / structure}
 & \textbf{Lives on} \\
\midrule
0 & Point
 & $\mathsf{Ass}$ \textup{(}classical Koszul duality\textup{)}
 & $A$ \\
1 & $\R$
 & $\mathsf{Ass}^c$ cooperad \textup{(}deconcatenation coproduct\textup{)}
 & $\barB(A)$ \\
2 & $[0,1]$, $\R_{\ge 0}$
 & $W(\mathsf{Ass}) = \Ainf$ \textup{(}modules via one-sided bar\textup{)}
 & $\barB(A)$, $\cC(I)$ \\
3 & $\C$
 & BD chiral operad $\{\FM_k(\C)\}$ \textup{(}OPE residues\textup{)}
 & bar differential $d_{\barB}$ \\
4 & $\C \times \R$
 & $(\mathrm{ChirAss})^!$ \textup{(}Koszul dual cooperad\textup{)}
 & $\barB(A)$ as $\Eone$ dg coalgebra \\
5 & $\mathbb{H}$ \textup{(}half-plane\textup{)}
 & $\SCchtop$ \textup{(}two-coloured; no open-to-closed\textup{)}
 & pair $(\cZ^{\mathrm{der}}_{\mathrm{ch}},\, A)$ \\
6 & $\widehat{D}$ \textup{(}formal disk\textup{)}
 & $\cE\!nd^{\mathrm{ch}}_A$ \textup{(}chiral endomorphism;
 $\Aut(\cO)$-equivariant\textup{)}
 & vertex algebra on $\widehat{D}$ \\
7 & Annulus, $\Sigma_g$
 & $\SCchtop_{\mathrm{mod}}$ \textup{(}partially modular;
 $d^2 = \kappa \cdot \omega_g$\textup{)}
 & $\Theta_\cA$, genus tower \\
8 & Drinfeld center
 & $\Etwo$-chiral Gerstenhaber \textup{(}chiral
 Deligne--Tamarkin\textup{)}
 & $\cZ^{\mathrm{der}}_{\mathrm{ch}}(A) =
 C^\bullet_{\mathrm{ch}}(A,A)$ \\
9 & Topologization
 & $\Ethree$-topological \textup{(}Sugawara; Dunn:
 $\Etwo^{\mathrm{top}} \otimes \Eone^{\mathrm{top}}$\textup{)}
 & $\cZ^{\mathrm{der}}_{\mathrm{ch}}(A)$ \\
\bottomrule
\end{tabular}
\end{center}
...
\smallskip\noindent
The following structural constraints hold:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Stage~$3$ vs Stage~$4$}: the BD chiral operad governs
 the bar \emph{differential} $d_{\barB}$
 \textup{(}OPE residues on $\FM_k(\C)$\textup{)}, while the
 coalgebra structure $(\barB(A), \Delta)$ over
 $(\mathrm{ChirAss})^!$ records the full $\Eone$-chiral
 coassociative data.

\item \emph{Stage~$5$ vs Stage~$9$}: $\SCchtop$ is the generic
 structure on the derived-center pair. The passage to
 $\Ethree$-topological at Stage~$9$ requires a conformal vector
 $T(z)$ at non-critical level \textup{(}Sugawara construction:
 $C$-translations become $Q$-exact, killing the dependence on
 complex structure in cohomology\textup{)}.
 Without a conformal vector, the structure remains at $\SCchtop$.
 At the critical level $k = -h^\vee$, the Sugawara construction
 degenerates and topologization fails.

\item \emph{Stage~$8$}: the $\Etwo$-chiral structure on the derived
 center $\cZ^{\mathrm{der}}_{\mathrm{ch}}(A)$ is automatic for
 $\Eone$-chiral algebras \textup{(}classical Deligne
 conjecture\textup{)}. For $E_\infty$-chiral algebras
 \textup{(}vertex algebras\textup{)}, the $\Etwo$ upgrades
 via the symmetric bar $\barB^\Sigma(A)$ and the higher
 Deligne conjecture.

\item \emph{Stage~$7$}: the modular extension
 $\SCchtop_{\mathrm{mod}}$ introduces curvature
 $d_{\barB}^2 = \kappaChHodge(\cA) \cdot \omega_g$ at genus $g \ge 1$.
 The modular characteristic $\kappaChHodge(\cA)$ is the leading
...
Chapter~4).
Stage~$7$: the modular extension and curvature identity are
Volume~I, Theorem~D.
Stage~$8$: the $\Etwo$-chiral Gerstenhaber structure on
