...
\S\ref{sec:universal-bulk-proofs}.  The key point: the
...
\item The Koszul dual $A^! = D_{\Ran}(\barB(A))$ is obtained
  by Verdier duality on the bar coalgebra.  It lives on the
  boundary~$\R$, governing the line-operator category
...
$D_{\Ran}$ & algebra $A^! = D_{\Ran}(\barB(A))$ & line-operator algebra \\
