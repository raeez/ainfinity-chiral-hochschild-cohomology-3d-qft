...
two are connected by the Thom--Sullivan de~Rham theorem:
the Thom--Sullivan functor sends the \v{C}ech cosimplicial
diagram to a dg algebra quasi-isomorphic to the de~Rham complex
of the total space.  Applied to FM compactifications, it yields
\[
\mathrm{TS}\bigl(\check{C}^\bullet(\mathcal{U};\, \Omega^*_{\FM_k(X)})\bigr)
\;\xrightarrow{\;\sim\;}
\Omega^*_{\FM_k(X)},
\]
and the $A_\infty$ operations constructed by FM integration on
the right agree (up to coherent homotopy) with the HCA
operations constructed algebraically on the left.  This
comparison is the operadic enrichment of the standard
\v{C}ech-to-de~Rham quasi-isomorphism, enhanced to an
$L_\infty$ morphism by the EZ operad.
\end{remark}
