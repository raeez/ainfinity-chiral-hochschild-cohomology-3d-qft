...
The Fulton--MacPherson compactification $\FM_k(\C)$ is the computational engine for extracting the arity-$k$ projections of~$\Theta^{\mathrm{oc}}$: each $\Ainf$ operation $m_k$ is a configuration-space integral over $\FM_k(\C) \times \Conf_k^{<}(\R)$, and the Stasheff identity $\sum_{i+j = k+1} m_i \circ m_j = 0$ is Stokes' theorem on this compactification, with codimension-2 corners canceling via Arnold--Orlik--Solomon relations.
