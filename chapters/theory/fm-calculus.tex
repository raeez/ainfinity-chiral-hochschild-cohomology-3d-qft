...
The Fulton--MacPherson compactification $\FM_k(\C)$ is the computational engine for extracting the arity-$k$ projections of~$\Theta^{\mathrm{oc}}$: each $\Ainf$ operation $m_k$ is a configuration-space integral over $\FM_k(\C) \times \Conf_k^{<}(\R)$, and the Stasheff identity $\sum_{i+j = k+1} m_i \circ m_j = 0$ is Stokes' theorem on this compactification, with codimension-2 corners canceling via Arnold--Orlik--Solomon relations.  The geometry is that of a manifold with corners: the codimension-1 faces of~$\FM_k(\C)$ are the collision divisors where subsets of points cluster, and the codimension-2 corners are the loci where two independent collision events occur simultaneously.  The Stokes identity on this boundary is the $A_\infty$ identity; the corner cancellation is the Arnold relation.  This section makes both precise, with explicit coordinates, orientations, and sign computations for~$k = 3$.
