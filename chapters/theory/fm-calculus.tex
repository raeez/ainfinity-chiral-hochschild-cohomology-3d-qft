...
$\FM_k(\C) \times \Conf_k(\R)$.  In the Steinberg analogy of
Remark~\ref{rem:lagrangian-H1H4}, the product
...
$D_S \cong \FM_r(\C) \times \FM_{|S|}(\C)$ in the
$\FM_k(\C)$-factor.
\end{remark}
...
factorization $D_S \cong \FM_r(\C) \times \FM_{|S|}(\C)$ expresses
...
Remark~\ref{rem:lagrangian-H1H4}, the divisors
...
\end{itemize}
By assumption (ii) and the factorization of weight forms (Hypothesis~\ref{hyp:H3}(c)), the residue decomposes as
\[
...
(the integral is understood in the distributional sense, with the propagator $G(z_i, z_v) = \bar\partial^{-1}\delta(z_i - z_v) = \frac{1}{2\pi i} \cdot \frac{1}{z_i - z_v}$ and the result follows from repeated application of the Cauchy integral formula; see e.g.\ \cite{CG17} \S5.4), the result is
\[
...
(Volume~I, Appendix~\ref*{app:homotopy-transfer}), applied to
...
sums of the transfer formula (Volume~I,
Theorem~\ref*{thm:tree-formula}) express each $m_k$ as an
