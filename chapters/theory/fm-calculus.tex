...
\item $D_{\{1,2,3\}}$: all three points collide $\cong \FM_1(\C) \times \FM_3(\C) \cong \FM_3(\C)$, since $\FM_1(\C) \cong \mathrm{pt}$ (a single point modulo translation; see Remark~\ref{rem:FM-convention}).
\end{itemize}
...
Points 1,2 collide. The factorization gives $D_{\{1,2\}} \cong \FM_2(\C) \times \FM_2(\C)$, with inner operation $m_2(a,b;\lambda_1)$ and outer operation $m_2(\bullet, c;\Lambda_{\{1,2\}})$ where $\Lambda_{\{1,2\}} = \lambda_1 + \lambda_2$. The Koszul sign is $(-1)^{\epsilon(0,2)} = (-1)^0 = +1$ (no elements passed). The spectral substitution inserts $\Lambda_{\{1,2\}}$ as the effective parameter of the merged slot, consistent with the boundary identification $z_1, z_2 \to u_{\{1,2\}}$ sending $\lambda_1 + \lambda_2 \mapsto \Lambda_{\{1,2\}}$. Contribution: $+m_2(m_2(a,b),c)$.
...
Every $A_\infty$ identity is a manifestation of Stokes' theorem on the corresponding configuration space.
...
\end{itemize}
For $m_k$ to be well-defined as a map between cochain complexes, we must have $Q \circ m_k = \pm m_k \circ Q^{\otimes k}$.  For an abstract logarithmic $\SCchtop$-algebra this chain-differential compatibility is part of the $C_\ast(W(\SCchtop))$-algebra structure; for physical realizations it is verified by Theorem~\ref{thm:physics-bridge}.  This is the input used in Claim~1.
...
For abstract logarithmic $\SCchtop$-algebras, the cluster factorization follows from Stokes' theorem on FM boundaries (Appendix~\ref{app:FM_Stokes}); the physics bridge theorem (Theorem~\ref{thm:physics-bridge}) is needed only to verify that physical HT theories satisfy Definition~\ref{def:log-SC-algebra}.  More generally, the FM calculus theorem is a purely geometric statement about logarithmic forms and residues on $\FM_k(\C)$.  When the structure comes from a physical theory, one-loop finiteness enters only through Theorem~\ref{thm:physics-bridge}: it is the bridge-theorem input ensuring that the renormalized Feynman forms satisfy the logarithmic conditions needed to define a logarithmic $\SCchtop$-algebra in the first place.
\end{remark}
...
$\mathrm{TS}$, which we now recall.  Given a cosimplicial
