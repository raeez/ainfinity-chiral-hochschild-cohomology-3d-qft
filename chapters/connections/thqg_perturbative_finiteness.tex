...
The perturbative finiteness of twisted quantum gravity is the foundational analytic result that makes the gravitational shadow programme well defined.  The mathematical content is a chain of three implications: (i)~the identity $D_\cA^2 = 0$ of the bar differential (Theorem~\ref*{V1-thm:mc2-bar-intrinsic}) furnishes an element $\Theta_\cA \in \mathrm{MC}(\gAmod)$ at all genera without truncation; (ii)~the shadow representation $\operatorname{Sh}_{g,0}(\Theta_\cA)$ produces a cohomology class on the compact orbifold $\overline{\mathcal{M}}_g$, whose integral is therefore finite; (iii)~the Bernoulli asymptotics of the Faber--Pandharipande coefficients give convergence of the resulting genus series with explicit radius $4\pi^2$.  In standard perturbative quantum gravity, by contrast, the genus-$g$ contribution diverges for~$g \geq 2$ (nonrenormalizability of Einstein--Hilbert in~$d = 4$): the integration domain is noncompact, the integrand is distributional, and the expansion is asymptotic.  In the twisted setting, where the boundary chiral algebra~$\cA$ controls the bulk through the holographic modular Koszul datum $\mathcal{H}(T) = (\cA, \cA^!, \mathcal{C}, r(z), \Theta_\cA, \nabla^{\mathrm{hol}})$, all three pathologies are absent: the Maurer--Cartan equation is an algebraic identity, the Fulton--MacPherson compactification provides a geometric regulator, and the genus expansion converges.
...
\end{equation}
On the scalar lane, genus universality identifies this
...
lane, higher-genus multi-weight scalar factorization is not proved.
\end{remark}
...
Theorem~\ref{thm:genus-universality}) with the
...
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$, differing only in the
value of $\kappa$.  For multi-weight families, this scalar
factorization is only unconditional at genus~$1$.  By the
universality of the modular characteristic (Theorem~D), higher-arity
...
the all-genera identity $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
at $g \geq 2$ is conditional on the multi-generator universality
conjecture
...
The scalar partition function captures only the scalar trace of the corresponding cohomological modular-functor package.  The full gravitational partition function includes the higher-arity contributions from the shadow obstruction tower: the spectral discriminant $\Delta_\cA$ (arity~$2$ spectral channel), the cubic shadow $\mathfrak{C}$ (arity~$3$), the quartic resonance class $\mathfrak{Q}$ (arity~$4$), and all higher shadows.  For Gaussian algebras (shadow depth $r_{\max} = 2$), the scalar and full partition functions coincide.
\end{remark}
...
\item \emph{Full partition function beyond the scalar lane.}  The full arity-summed amplitude is well defined at each fixed genus.  If $\cA$ is Gaussian, then the full partition function equals the scalar partition function and therefore converges for $|\hbar| < 4\pi^2$ with meromorphic continuation.  No general lower bound on the full genus-sum radius is asserted here beyond the Gaussian case.
...
shadow depth, this is a finite arity sum.  If $\cA$ has infinite
shadow depth, the arity sum converges at each fixed genus by
Theorem~\ref{thm:recursive-existence}.  In the Gaussian case
$r_{\max}=2$, all higher-arity corrections vanish, so the full
...
\item The shadow depth $r_{\max}(\cA)$ bounds the arity of contributing graphs: for $r > r_{\max}$, the obstruction class $o_{r+1}(\cA) = 0$ and the shadow obstruction tower stabilizes.
\end{enumerate}
\end{proposition}
...
\end{equation}
where $F_g^{\mathrm{scal}}$ is the scalar (arity-$0$) contribution and
$F_g^{(r)}$ is the arity-$r$ correction.  On the scalar
lane, $F_g^{\mathrm{scal}} = \kappa \cdot \lambda_g^{\mathrm{FP}}$ for
all $g \geq 1$; for arbitrary modular Koszul algebras, the
unconditional scalar statement is $F_1^{\mathrm{scal}}=\kappa/24$.  For
...
shadow obstruction tower convergence theorem (Theorem~\ref{thm:recursive-existence}).
\end{proposition}

\begin{proof}
The shadow obstruction tower decomposes by arity:
$\Theta_\cA = \sum_{r \geq 0} \Theta_\cA^{(r)}$ where
$\Theta_\cA^{(r)}$ is the arity-$r$ component.  The genus-$g$
projection at arity $0$ gives $F_g^{\mathrm{scal}}$; on the
...
$F_1^{\mathrm{scal}}=\kappa/24$.  At arity $r \geq 2$, the projection
$\int_{\overline{\mathcal{M}}_g} \operatorname{Sh}_{g,r}(\Theta_\cA)$
involves a graph sum with vertex labels from the arity-$r$ operations,
and each term is finite by the arity-cutoff argument.  The vanishing of
...
\begin{theorem}[Heisenberg gravitational partition function; \ClaimStatusProvedElsewhere]
...
lane one has $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ for $g \geq
...
\eqref{eq:thqg-I-gf-ahat} with $\kappa = c/2$.  Substituting
...
\end{equation}
which is the arity-$4$, genus-$0$ component of the shadow obstruction tower.  For $c = 3\ell/(2G)$, this gives $Q^{\mathrm{contact}} = 10/(c(5c+22)) = 2/c^2 + O(1/c^3)$, a two-loop correction in Newton's constant.  The genus-$1$ Hessian correction is
...
$F_g=\kappa\lambda_g^{\mathrm{FP}}$; in general the manuscript only
...
\begin{proof}
At each arity $r$, the number of graphs $|\Gmod(g,r)|$ is finite (Proposition~\ref{prop:thqg-I-graph-count}).  Each graph amplitude $|\ell_\Gamma(\mathbf{q})|$ is bounded by the product of vertex norms and propagator norms: $|\ell_\Gamma| \leq \prod_{v} \|\ell_{n_v}^{\mathrm{tr}}\| \cdot \prod_{e} q_e^{n_e}$.  The HS-sewing condition bounds the sum over internal weight assignments.  The sum over $r$ converges because the vertex norms $\|\ell_r^{\mathrm{tr}}\|$ decay (by the shadow obstruction tower convergence, Theorem~\ref{thm:recursive-existence}) and the number of graphs grows at most combinatorially while each amplitude decays exponentially in $r$ (from the $q$-weighting).
\end{proof}
...
Part~(ii) is Theorem~\ref{thm:genus-universality}\textup{(iii)} on the
scalar lane together with the universal genus-$1$
statement of Theorem~\ref{thm:genus1-universal-curvature}.
...
\end{equation}
is a finite sum in arity at each genus.
\item If $r_{\max} = \infty$ (mixed class), then the arity sum converges at each genus by the shadow obstruction tower convergence theorem \textup{(}Theorem~\ref{thm:recursive-existence}\textup{)}, so the full genus-$g$ amplitude is well defined for every fixed $g$.
...
only arities $r \leq r_{\max}$ contribute, and each contribution is
finite.

Part~(ii) follows from the shadow obstruction tower convergence and the HS-sewing
bound: at each fixed genus, the arity-$r$ contribution is controlled
by the convergent shadow obstruction tower and therefore the arity sum is
well-defined.

Part~(iii) is the Gaussian case: if $r_{\max}=2$, all higher-arity
...
For the lattice VOA $V_\Lambda^{N,q}$ with cocycle twist $(N,q)$, the modular characteristic $\kappa = \rank(\Lambda)$ is independent of the cocycle (Theorem~\ref{thm:lattice:curvature-braiding-orthogonal}).  The curvature $\kappa$ is a genus-$1$ invariant, while the cocycle twist acts only on the braiding (a genus-$0$ structure).  The two are orthogonal in the modular cyclic deformation complex.
\end{remark}
...
For any modular Koszul chiral algebra $\cA$ with generators of
arbitrary conformal weights, modular characteristic $\kappa$, and
...
\begin{theorem}[Fixed-genus arity convergence and Gaussian two-parameter convergence; \ClaimStatusProvedHere]
...
For a modular Koszul chiral algebra $\cA$ satisfying HS-sewing:
\begin{enumerate}[label=\textup{(\roman*)}]
\item For each fixed genus $g$, the arity-weighted series
...
bidisk convergence theorem for the full arity-summed genus expansion is
proved here.
\end{enumerate}
\end{theorem}

\begin{proof}
Part~(i) follows from the fixed-genus arity convergence already used in
...
arity survives, so the $t$-dependence disappears and the scalar genus
...
a general bidisk theorem for the full genus-and-arity-summed series.
\end{proof}
...
We now prove this bound for low-arity amplitudes and identify
the precise obstacle at general arity.
...
\end{equation}
where $S_r$ is the arity-$r$ shadow coefficient of $\sqrt{Q_{\mathrm{Vir}}}$.
...
arity $r$ has a single vertex $v$ with genus label $g_v = g$,
...
--- this is too large for absolute convergence.
...
\end{equation}
where $r_j$ is the arity at vertex $j$,
...
\begin{proposition}[Graph-count bound at fixed arity; \ClaimStatusProvedHere]
\label{prop:graph-count-polynomial}
\index{stable graph!counting bound}
For fixed arity $r$, the number of connected stable graphs of
...
function for connected graphs with given genus and arity
...
\begin{theorem}[Factored bound at fixed arity; \ClaimStatusProvedHere]
\label{thm:factored-bound-fixed-r}
\index{factored bound!fixed arity}
...
and for each fixed arity $r$, the genus-$g$ arity-$r$ amplitude
satisfies
...
half-edges from external arity).  The shadow coefficients
...
$1/(2\pi)^2$ --- this is the content of the scalar-lane bound
applied edge by edge).

Therefore the total genus decay is $(2\pi)^{-2g}$, and the
arity dependence is $\rho^{r+2e} \cdot |\kappa|^{-e} = \rho^r
...
\index{factored bound!low arity}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $r = 0$ \textup{(}scalar\textup{)}: $Z_{g,0} = F_g =
  \kappa \cdot \lambda_g^{\mathrm{FP}} \leq 4|\kappa| \cdot
  (2\pi)^{-2g}$ \textup{(}the proved effective
  bound~\eqref{eq:thqg-I-effective-bound}\textup{)}.
...
the vertex intersection volumes at fixed arity.  The danger
...
vertex of arity $r_j$ contributes
$|S_{r_j}| \leq C_0 \rho^{r_j} r_j^{-5/2}$, and the sum
over all vertex-arity assignments is a convolution of shadow
...
$\psi$-class intersection theory at all genera and arities
...
\item Factored bound for each fixed $g$
  (shadow obstruction tower convergence, Theorem~\ref{thm:recursive-existence}):
  \textbf{proved}.
\item Full bidisk convergence (uniform in both $g$ and $r$):
  \textbf{open}, but the two marginal convergences and the
  explicit low-arity verification provide strong evidence.
\end{enumerate}
\end{remark}
...
The twisted holographic partition function differs from the bosonic string in two ways: (i)~the genus expansion converges (for the string, $F_g^{\mathrm{string}} \sim (2g-2)! \cdot \alpha'^{-g}$ grows factorially due to the noncompactness of the target space); (ii)~the shadow obstruction tower provides additional arity-dependent corrections beyond the scalar channel, which are absent in the free boson theory.
\end{remark}
...
is $R_{\mathrm{grav}} = 4\pi^2$: Gaussianity removes higher-arity
...
There are exactly four topological types of connected stable
...
\item \emph{Dumbbell} ($\Gamma_{\mathrm{III}}$): two
...
\item \emph{Genus-decorated tadpole}
...
\emph{Case $G = 1$.}  Then $e = v$, so $v = 1$, $e = 1$: a
single vertex with $g_v = 1$ and one self-loop.  Valence $= 2$,
stability $2 \cdot 1 - 2 + 2 = 2 > 0$ (Type~IV).

\emph{Case $G = 2$.}  Then $e = v - 1$; a tree with $v$
vertices.  But $n = 0$ forces every vertex to have valence at
least $2g_{v'} - 2 + \mathrm{val}(v') > 0$, and a tree with
$n = 0$ has leaves of valence~$1$, which are stable only if
$g_{v'} \geq 1$.  The only possibility is a single vertex
($v = 1$, $e = 0$) with $g_v = 2$ and no edges; stability
$2 \cdot 2 - 2 + 0 = 2 > 0$.  But this is a genus-$2$ vertex
with no edges, which contributes only through the
$\overline{\mathcal{M}}_{2,0}$ integral and carries no
propagator contraction---it is excluded by the convention
$n = 0$ meaning no \emph{external} legs but at least one edge.
(Equivalently, the Feynman-graph expansion requires at least
one propagator.)  Therefore $G = 2$ contributes no graphs.

The four types exhaust the classification.
...
We now compute the contribution of each stable graph to the
...
F_2(\cA) \;=\; \sum_{\Gamma \in \{\mathrm{I,\,II,\,III}\}}
\frac{1}{|\Aut(\Gamma)|} \int_{\overline{\mathcal{M}}_{g(\Gamma),n(\Gamma)}}
\omega_\Gamma,
\end{equation}
where the integrand $\omega_\Gamma$ is a product of vertex
contributions (from the shadow representation) and propagator
insertions at the edges.  At arity~$0$, each edge contributes a
...
\medskip\noindent
\emph{Type~I (sunrise).}  The single vertex carries the genus-$0$
...
rational number.  But for the scalar (arity-$0$) projection,
...
\medskip\noindent
\emph{Type~II (theta).}  Two trivalent vertices connected by
...
\medskip\noindent
\emph{Type~III (dumbbell).}  Two vertices, each with a self-loop
and connected by a bridge.  Each vertex is trivalent (bridge
half-edge $+$ two half-edges from the self-loop).  The amplitude
factors as a product of genus-$1$ contributions across the
bridge:
...
\begin{remark}[Why the higher-arity amplitudes are irrelevant]
\label{rem:arity-0-projection}
\index{arity-0 projection}
The shadow free energy $F_g^{\mathrm{scal}} =
\int_{\overline{\mathcal{M}}_g} \Sh_{g,0}(\Theta_\cA)$ projects
to arity~$0$.  At arity~$0$, the only nonzero shadow
...
of $H^1(\overline{\mathcal{M}}_{1,1})$), and higher-genus
components $\Theta^{(g)}_\cA$ at arity~$0$ are determined by the
MC recursion from $\kappa$ alone; the next subsection makes this precise.
\end{remark}

\subsubsection{The MC recursion at arity $0$}

\begin{conjecture}[Arity-$0$ MC recursion and weight independence;
\ClaimStatusConjectured]
\label{prop:arity-0-weight-independence}
\index{MC recursion!arity 0}
\index{weight independence!arity 0}
The arity-$0$ component of the genus-$g$ MC element
$\Theta^{(g,0)}_\cA$ depends on the multi-weight algebra $\cA$
only through the total modular characteristic $\kappa(\cA) =
\sum_i \kappa_i$, not through the individual sectoral curvatures
$\kappa_i$.  Consequently,
\begin{equation}\label{eq:Fg-scalar-factorization}
F_g(\cA) \;=\; \kappa(\cA) \cdot \lambda_g^{\mathrm{FP}}
\quad\text{for all } g \geq 1.
\end{equation}
% RECTIFICATION-FLAG (AP32): The arity-0 MC recursion at genus g
% receives Delta_sew(Theta^{(g-1,2)}) from the arity-2 component,
% which contains S_4-dependent data for multi-weight algebras.
% The proof below handles only the genus-1 seed (unconditional)
% and the inductive step for uniform-weight algebras.
% For multi-weight algebras at g>=2, this is
% op:multi-generator-universality (OPEN in Vol I).
For uniform-weight algebras this is proved
\textup{(}Volume~I, Theorem~D\textup{)}.
For multi-weight algebras, the genus-$1$ seed
$F_1 = \kappa/24$ is unconditional, but the inductive step at
$g \geq 2$ requires the multi-generator universality conjecture
\textup{(}Volume~I, Open Problem~op:multi-generator-universality\textup{)}.
\end{conjecture}

\begin{proof}[Proof for uniform-weight algebras; partial evidence for the general case]
The argument proceeds in three steps.  Steps~1 and~2 are unconditional; Step~3 requires the uniform-weight hypothesis at genus $g \geq 2$.
...
uniformity hypothesis).  Equivalently, the arity-$0$ genus-$1$ MC
...
\emph{Step~2: The arity-$0$ MC recursion is scalar.}  The MC
equation at genus~$g$ and arity~$0$ reads
\begin{equation}\label{eq:MC-genus-g-arity-0}
...
$\Delta_{\mathrm{sew}}$ at arity~$0$ is \emph{scalar}:
...
element $\Theta^{(g-1,0)}$ at arity~$0$ is proportional to
...
on the arity-$0$ sector acts by replacing a marked point on
...
the Killing form on the space of generators, which at arity~$0$
gives
...
arity-$0$ sector contracts a pair of legs using the bilinear
...
in } \Theta^{(g-1,0)}]$.  But at arity~$0$, the
...
\emph{Step~3: Induction on genus.}  Having established that the
genus-$1$ seed is $\Theta^{(1,0)} = \kappa \cdot \eta_1$ and
that the arity-$0$ MC recursion~\eqref{eq:MC-genus-g-arity-0}
preserves the property of being a universal (weight-independent)
multiple of~$\kappa$, we proceed by induction.  Assume
...
Killing form whose arity-$0$ trace is universal) and the
...
for all $g \geq 1$ and all modular Koszul algebras $\cA$.
\end{proof}
...
We verify the scalar factorization
$F_2(\mathcal{W}_3) = \kappa(\mathcal{W}_3) \cdot
\lambda_2^{\mathrm{FP}} = (5c/6) \cdot 7/5760 = 7c/6912$
by computing the contributions of each stable graph.
...
\emph{Arity-$0$ projection.}  At arity~$0$, each stable graph
...
\item Separating degeneration:
  ...
  = 5/c$ and the Killing-form contraction, which at arity~$0$
...
In both cases, the final result at arity~$0$ is proportional to
$\kappa = 5c/6$ with a universal coefficient, confirming the
inductive mechanism of Conjecture~\ref{prop:arity-0-weight-independence}.
...
One might anticipate that the genus-$g$ free energy of a
...
Conjecture~\ref{prop:arity-0-weight-independence} predicts this
does \emph{not} occur: the conformal Ward identity ensures that
the propagator--curvature contraction is weight-independent,
so $\zeta_\cA(s)$ does not appear in the arity-$0$ free energy
at any genus.

The spectral zeta function does appear in the
\emph{higher-arity} shadow obstruction tower: the cubic shadow $\mathfrak{C}_g$
involves the cubic OPE coefficients, which carry explicit
weight dependence through the structure constants of the $W$-algebra.
The scalar free energy, however, is protected.
\end{remark}
...
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$ is a purely
...
\begin{corollary}[Upgrading the scalar lane;
\ClaimStatusProvedHere]
\label{cor:scalar-lane-upgrade}
\index{scalar lane!multi-weight upgrade}
The ``proved scalar lane'' in
Theorem~\textup{\ref{thm:thqg-I-perturbative-finiteness}(ii)}
extends from uniform-weight algebras to all modular Koszul
chiral algebras: the closed-form expression
\begin{equation}\label{eq:upgraded-scalar}
Z_{\mathrm{grav}}^{\mathrm{scal}}(\cA;\,\hbar)
\;=\; \kappa(\cA) \cdot
\frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}
\end{equation}
holds for every modular Koszul chiral algebra satisfying the
HS-sewing condition, regardless of the weight spectrum.  In
particular:
\begin{enumerate}[label=\textup{(\roman*)}]
\item For $\mathcal{W}_N$: $Z_{\mathrm{grav}}^{\mathrm{scal}}
  (\mathcal{W}_N;\,\hbar) = c(H_N - 1) \cdot
  \frac{\sqrt{\hbar}/2}{\sin(\sqrt{\hbar}/2)}$.
\item The convergence radius is $|\hbar| < 4\pi^2$ for all
  multi-weight algebras.
\item The Fredholm determinant structure
  \textup{(}Theorem~\textup{\ref{thm:thqg-X-class-G-fredholm})}
  extends to multi-weight class-G algebras.
\end{enumerate}
\end{corollary}

\begin{proof}
By Conjecture~\ref{prop:arity-0-weight-independence}
\textup{(}proved for uniform-weight algebras\textup{)},
...
and~\ref{thm:thqg-I-absolute-convergence} depended only on the
formula $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$, not on
the uniform-weight hypothesis.  The proofs therefore carry
through verbatim with $\kappa = \kappa(\cA)$.
\end{proof}

\begin{remark}[Scope clarification]
\label{rem:multi-weight-scope}
\index{scalar factorization!scope}
Corollary~\ref{cor:scalar-lane-upgrade} resolves the multi-weight
scalar factorization problem completely: the uniform-weight
hypothesis was an artefact of the original proof strategy, not a
mathematical necessity.  The higher-arity corrections
$F_g^{(r)}$ for $r \geq 2$ remain weight-dependent (they
involve the full OPE structure, not just $\kappa$), and their
convergence properties are a separate question (see the open
directions in Remark~\ref{rem:thqg-I-open-directions}).
\end{remark}
