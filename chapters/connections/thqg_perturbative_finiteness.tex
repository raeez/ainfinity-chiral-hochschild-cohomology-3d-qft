...
\begin{theorem}[Absolute convergence of the scalar generating function on the scalar lane; \ClaimStatusProvedHere]
% label removed: thm:thqg-I-absolute-convergence
\index{absolute convergence!shadow free energy}
If $\cA$ has generators of uniform conformal weight, then the
...
For arbitrary modular Koszul algebras, the unconditional scalar
statement is the genus-$1$ identity $F_1(\cA)=\kappa(\cA)/24$.
\end{theorem}

\begin{proof}
On the scalar lane, the asymptotic behavior of
Bernoulli numbers is controlled by Euler's formula
$|B_{2g}| = 2 \cdot \zeta(2g) \cdot (2g)! / (2\pi)^{2g}$, where
$\zeta(2g) \to 1$ as $g \to \infty$.  Therefore
\begin{align}
...
The pole structure is determined by the zeros of $\sin(\sqrt{\hbar}/2)$,
which occur at $\sqrt{\hbar}/2 = n\pi$, i.e.,
$\hbar = 4\pi^2 n^2$.  Writing $s(\hbar)=\sin(\sqrt{\hbar}/2)$, one
...
For higher-spin gravity in AdS$_3$ with $\mathcal{W}_N$ symmetry, the boundary algebra is $\mathcal{W}_k(\mathfrak{sl}_N)$ at $c = (N-1)(1 - N(N+1)/(k+N))$.  The modular characteristic is $\kappa(\mathcal{W}_N) = c \cdot (H_N - 1)$ where $H_N = \sum_{j=1}^N 1/j$ is the $N$-th harmonic number ($\kappa = c/2$ for $N=2$, $\kappa = 5c/6$ for $N=3$).  The perturbative partition function is
\[
...
MC recursion from $\kappa$ alone.
\end{remark}

\subsubsection{The MC recursion at arity $0$}

\begin{proposition}[Arity-$0$ MC recursion and weight independence;
\ClaimStatusConjectured]
\label{prop:arity-0-weight-independence}
% RECTIFICATION-FLAG: The proof below has a gap in Step 2 (genus-1 does not imply all-genera).
% The arity-0 MC recursion receives contributions from
% Delta_sew(Theta^{(g-1,2)}), which feeds arity-2 data into arity 0.
% For multi-weight algebras, Theta^{(g-1,2)} can depend on S_4 and
% higher shadow data, not just kappa.  Vol I prop:f2-quartic-dependence
% proves that the banana graph injects S_4-dependence into F_2.
% This proposition contradicts Vol I op:multi-generator-universality (OPEN).
% Status downgraded from ProvedHere to Conjectured pending resolution.
...
\end{equation}
The scalar factorization holds for \textbf{all} modular Koszul
chiral algebras, not only those with uniform conformal weight.
\end{proposition}
...
The proof reduces to equation~\eqref{eq:kappa-i-propagator}:
...
inductive mechanism of Proposition~\ref{prop:arity-0-weight-independence}.
...
A natural guess is that the genus-$g$ free energy of a
...
Proposition~\ref{prop:arity-0-weight-independence} shows this
...
% RECTIFICATION-FLAG: Depends on prop:arity-0-weight-independence,
% which has a gap.  Status downgraded pending resolution.
...
\begin{proof}
By Proposition~\ref{prop:arity-0-weight-independence},
...
Corollary~\ref{cor:scalar-lane-upgrade}, if correct, would resolve
the multi-weight scalar factorization problem.  However, the proof
of Proposition~\ref{prop:arity-0-weight-independence} has a gap:
the arity-$0$ MC recursion also receives input from
$\Delta_{\mathrm{sew}}\Theta^{(g-1,2)}$, which feeds arity-$2$
data (potentially $S_4$-dependent for multi-weight algebras)
into the arity-$0$ component.  The status of this claim remains
open (Open Problem~\ref*{V1-op:multi-generator-universality}).  The higher-arity corrections
