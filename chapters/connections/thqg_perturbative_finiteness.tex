...
contribution is a sum over the shadow tower.  If $\cA$ has finite
...
shadow tower convergence theorem (Theorem~\ref{thm:recursive-existence}).
\end{proposition}

\begin{proof}
The shadow tower decomposes by arity:
...
\item If $r_{\max} = \infty$ (mixed class), then the arity sum converges at each genus by the shadow tower convergence theorem \textup{(}Theorem~\ref{thm:recursive-existence}\textup{)}, so the full genus-$g$ amplitude is well defined for every fixed $g$.
...
Part~(ii) follows from the shadow tower convergence and the HS-sewing
bound: at each fixed genus, the arity-$r$ contribution is controlled
by the convergent shadow tower and therefore the arity sum is
well-defined.
...
\textup{(}shadow tower convergence\textup{)}, the contact contribution
...
\textup{(}so that $\rho < 1$ and the shadow tower converges\textup{)},
...
shadow tower at genus~$0$, where $\sqrt{Q_{\mathrm{Vir}}}$
...
involve convolutions of the shadow tower with itself: a
...
\item Factored bound for each fixed $g$
  (shadow tower convergence, Theorem~\ref{thm:recursive-existence}):
  \textbf{proved}.
...
\emph{higher-arity} shadow tower: the cubic shadow $\mathfrak{C}_g$
