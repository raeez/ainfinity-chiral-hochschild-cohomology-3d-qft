...
The two ways to compose the local exchanges
$R_{12}(z_{12})$, $R_{13}(z_{13})$, $R_{23}(z_{23})$ are the two
boundary trivialisations of the same flat three-point object on
$\FM_3(\C)$, with associator \(\Phi_A\).

\begin{theorem}[Associator-controlled Yang--Baxter relation;
\ClaimStatusProvedHere]
% label removed: thm:YBE
Assume \textrm{(H1)--(H4)}.
The spectral exchange family and the three-point associator satisfy
\[
R_{12}(z_{12})\,R_{13}(z_{13})\,R_{23}(z_{23})
\;=\;
\Phi_A^{-1}R_{23}(z_{23})\,R_{13}(z_{13})\,R_{12}(z_{12})\Phi_A
\]
as morphisms $L_1\otimes_z L_2\otimes_w L_3 \to
L_3\otimes_{-w}L_2\otimes_{-z}L_1$ for $z_i\neq z_j$. On a strictified
comparison surface with \(\Phi_A=1\), this is the strict quantum
...
\medskip
\noindent\textbf{Step 1: Boundary strata of \(\FM^{\mathrm{tr}}_3(\C)\).}
By the FM boundary structure (Definition~\ref{def:boundary_divisors}), the codimension-1 faces of \(\partial\FM^{\mathrm{tr}}_3(\C)\) are the collision divisors
\[
D_{\{1,2\}},\quad D_{\{1,3\}},\quad D_{\{2,3\}},\quad D_{\{1,2,3\}}.
\]
For \(S\subset\{1,2,3\}\) with \(|S|\ge2\),
\[
D_S\cong
\FM^{\mathrm{tr}}_{3-|S|+1}(\C)\times
\FM^{\mathrm{red}}_{|S|}(\C).
\]
Thus \(D_{\{1,2,3\}}\cong\FM^{\mathrm{red}}_3(\C)\).  The total
collision is not erased; it records the three-point
parenthesisation/associator.  The pairwise divisors give the binary
residue part of the Yang--Baxter identity.
...
\noindent\textbf{Step 3: Stokes' theorem, associator, and corner cancellation.}
By Stokes' theorem on the manifold with corners
\(\FM^{\mathrm{tr}}_3(\C)\),
\[
0 = \int_{\FM^{\mathrm{tr}}_3(\C)} d\omega_3^{\partial}
 = \sum_{\{i,j\}} (-1)^{\sigma_{\{i,j\}}}
 \int_{D_{\{i,j\}}} \Res_{D_{\{i,j\}}}(\omega_3^{\partial}).
\]
The displayed sum is the pairwise-residue component after the
associator contribution from \(D_{\{1,2,3\}}\) has been moved to the
chosen parenthesisation.  Codimension-2 corners arise from nested
collisions; they are codimension-2 in the ambient FM space and
codimension-1 faces of the corresponding total-collision divisor.  By
Arnold cancellation (Lemma~\ref{lem:Arnold_cancellation}), the
pairwise corner contributions cancel: for each pair of faces sharing a
codimension-2 stratum, the two orderings of taking residues produce
opposite signs via the Koszul sign rule on the normal covectors
\(d\varepsilon_{\{i,j\}}\).
...
\begin{proof}
Proof: both the Laplace-transform formula and the classical Yang--Baxter equation in four steps.
...
The formula $r(z) = \int_0^\infty e^{-\lambda z} \{\cdot{}_\lambda \cdot\}\, d\lambda$ is the local bridge between the PVA world (spectral variable~$\lambda$) and the $R$-matrix world (position variable~$z$). Under DK-0, after the evaluation surface and normalization are fixed, it recovers the classical $r$-matrix from the $\lambda$-bracket at the evaluation locus. At the operadic level, this is the passage between the two colors of the Swiss-cheese operad: the $\lambda$-bracket encodes holomorphic (closed-color) singularities; $R(z)$ records the local topological exchange shadow on the open color. The topological direction supplies the integration contour; the holomorphic direction supplies the meromorphic structure.
\end{remark}
...
Let $\mathbf B$ be the bar cooperad of
$\mathsf{SC}^{\mathrm{ch,top}}$ and let $A$ be a
$W(\mathsf{SC}^{\mathrm{ch,top}})$--algebra.
The bar construction $\mathsf{Bar}^{\mathrm{ch,top}}(A)$ is a colored
coalgebra over $\mathbf B$. On the boundary/open colour, the associated
Koszul-dual package determines a filtered local bialgebra shadow
$\mathcal H$ encoding the line-operator OPE and higher products. A Hopf
or factorisation-quantum-group interpretation requires the additional
spectral coproduct, associator, descent, and global comparison data
isolated below.

\begin{theorem}[Spectral exchange family and categorical braiding; \ClaimStatusProvedHere]
% label removed: thm:braided-category
Assume \textrm{(H1)--(H4)}.
Let $\mathcal C_{\partial}$ be the (derived) category of boundary line
operators with the tensor structure induced by the \(W\)-module
structure of~$M$. Then \(R(z)\) defines a meromorphic family of
exchange isomorphisms on the factorisation line category. After
choosing evaluation data at insertion points and descending along an
exchange path, this family yields the ordinary categorical braiding
natural transformation on the corresponding centre-side category. The
classical limit \(r(z)\) gives the filtered quasi-triangular local
shadow on the Koszul-dual boundary object~\(\mathcal H\).
\end{theorem}

\begin{proof}
We establish the meromorphic exchange family and the filtered
...
\]
for generic $z \in \C^\times$, arising from the exchange of insertion points in $\FM_2(\C)$. Thus $\{\beta_z\}_{z\in\C^\times}$ is a meromorphic exchange family between the factorisation products $\otimes_z$ and $\otimes_{-z}^{\mathrm{op}}$, not a single categorical braiding on the fixed tensor product. The ordinary braiding
\[
\beta_{M_1,M_2}\colon
M_1\otimes_{E_1}M_2 \xrightarrow{\ \sim\ }
M_2\otimes_{E_1}M_1
\]
is obtained only after evaluation/descent along an exchange path or,
equivalently, after passing to the derived/Drinfeld centre where the
half-braiding is a natural transformation. By Theorem~\ref{thm:YBE},
$\beta_z$ and the three-point associator satisfy the braid relation for
the meromorphic family; the categorical hexagon is the descended
centre-side statement.
...
holomorphic weight filtration $F^\bullet$ with
$\gr^F \mathcal{H}\cong U(\mathfrak{g}[z])$ (the enveloping algebra of
the loop algebra, on the closed color) tensored with an associative
algebra (on the open color).
...
local shadow}: $r(z)$ lives in filtration degree
...
classical shadow to the completed object $\widehat{\mathcal{H}}$.
The all-orders Yang--Baxter statement belongs to that chosen quantized
surface.

Theorem~\ref{thm:lines_as_modules} identifies perturbative line
operators with modules for the Koszul-dual boundary algebra, so the
local factorisation line category from Part~A is modelled by
...
\begin{proof}
Verification: each piece of structure in Definition~\ref{def:dg_Yangian}
...
The OPE of two parallel line operators $L_1(z_1)$, $L_2(z_2)$ is governed by configuration integrals over $\FM_2(\C) \times E_1(2)$. Setting $z = z_1 - z_2$, the quantum corrections to the free tensor product define $r(z) \in \A^! \widehat{\otimes}\, \A^!((z^{-1}))$ as in Theorem~\ref{thm:spectral_R_YBE}. The degree is $(1, \text{odd}, 0)$: the first component is the degree of the induced binary bar/configuration-integral term; the bar desuspension $s^{-1}$ itself lowers cohomological degree by~$1$. The second component is odd under the $\Z/2$-grading from the fermion number, and the third is weight~$0$ because the leading pole $1/z$ carries no holomorphic weight shift.
...
coproduct language, obtained by substituting the model-dependent
shorthand \(R(z)=\exp(r(z))\) and expanding the associator-controlled
three-line exchange identity to leading nontrivial order.
...
Assume \textrm{(H1)--(H4)} and choose the spectral tensor datum
\((\otimes_z,R_{V,W}(z),\Phi_A)\) of
Definition~\ref{def:factorisation_QG}.  The category
$\mathcal{C}_{\text{line}}$ of line operators in the chosen local
3d HT realization forms a local factorisation quantum-group shadow.
The full Latyntsev Ran-cosheaf and representation-category statement is
a global comparison problem, not a consequence of the local spectral
kernel alone.
\end{theorem}

\begin{proof}
Verification: the local spectral tensor datum of
...
boundary line operators and morphisms given by boundary-localized local
operators (intertwiners). The monoidal product is concatenation in the
$E_1$-direction along the boundary $t=0$: given lines $L_1,L_2$
placed at distinct points in $\C$, the product $L_1\otimes L_2$ is
defined by the $W(\SCchtop)$-module structure
(Section~\ref{subsec:boundary-module}). Associativity and unit
constraints follow from the \(E_1\)-coherence encoded in the
\(W\)-resolution. On the chirally Koszul locus,
Theorem~\ref{thm:lines_as_modules} further identifies
\(\mathcal C_{\mathrm{line}}\) with modules for the open-colour
Koszul dual, but that comparison is not the global Latyntsev
cosheaf.
...
$R_{L_1,L_2}(z)\colon L_1\otimes_z L_2\xrightarrow{\sim}
L_2\otimes_{-z}L_1$ (Definition~\ref{def:spectral-braiding}) is an
isomorphism between \(\otimes_z\) and
\(\otimes_{-z}^{\mathrm{op}}\). By Theorem~\ref{thm:YBE},
\(R_{L_1,L_2}(z)\) satisfies the associator-controlled
Yang--Baxter relation for this meromorphic exchange family, with the
three-point associator \(\Phi_A\). It becomes an ordinary categorical
braiding only after the evaluation/descent step described above.
...
The present theorem proves only the local shadow: on disks inside the
chosen product-formal model, the prefactorization algebra
...
For abelian $U(1)$ Chern--Simons at level~$k$ (Example~\ref{prop:abelian_CS_complete}), the boundary current chart is the Heisenberg/affine Kac--Moody algebra $\widehat{\mathfrak{u}(1)}_k$. Under the scoped bulk--Hochschild comparison, the associated physical bulk object is the chiral derived centre $C^\bullet_{\mathrm{ch}}(\widehat{\mathfrak{u}(1)}_k,\widehat{\mathfrak{u}(1)}_k)$; the open-colour Koszul dual line algebra is the Heisenberg Yangian $Y(\mathfrak{u}(1))$. The spectral $R$-matrix is
\begin{equation}
r(z) = k \, \frac{J \otimes J}{z} + \text{(higher terms)},
\end{equation}
satisfying the classical Yang--Baxter equation; the kernel is read from the boundary current OPE, not from an identification of the boundary chart with bulk. For non-abelian $G$, the Koszul dual becomes the Yangian $Y(\mathfrak{g})$ or quantum affine algebra $U_q(\widehat{\mathfrak{g}})$, depending on the CS level and boundary conditions.
\end{example}
...
$\kappaChHodge(Y_\hbar(\fg)) = -\dim(\fg)(k{+}h^\vee)/(2h^\vee)$. The complementarity
...
which is exactly the classical $r$-matrix $r_0 = k\,\Omega_{\mathrm{tr}}/z$
...
\item \emph{Complementarity.}
 $\kappaChHodge(\cA) + \kappaChHodge(\cA^!) = 0$ for Kac--Moody and free-field algebras
...
(iv): Complementarity $\kappaChHodge(\cA) + \kappaChHodge(\cA^!) = 0$ for KM and free-field algebras
...
curvatures $\kappaChHodge(\cA) \cdot \omega_g$ and
...
\begin{remark}[Kontsevich comparison scope]
...
\begin{remark}[Hall--Borcherds recognition gate]
In this synthesis $\mathbf H_{\Delta_5}^{\mathrm{tgt}}$ denotes the
K3 target datum produced by the super-Etingof--Kazhdan quantisation
of the BKM Manin pair
\[
...
Mukai-Heisenberg/Miki evaluation block. The unsuperscripted notation
$\mathbf H_{\Delta_5}$ is reserved for a compact Hall source only after
finite Hall--Borcherds recognition supplies compact Hall/CoHA source
provenance, positive and negative halves with Cartan, completed
coproduct, nondegenerate Hopf pairing, parity/root fixture, radical
quotient, PBW/no-extra-relations, Mittag--Leffler compatibility, and
the Borcherds--Heegner comparison. Before these gates pass, all
spectral-braiding statements below are target-package statements.
\end{remark}

\begin{remark}[Siegel-elliptic dynamical $R$-matrix as spectral braiding]
\label{rem:specbraid-1}
The K3 Siegel-elliptic dynamical $R$-matrix
$R_{\mathrm{Sieg,dyn}}(z_1, z_2;\tau, w)$ via Pasol--Zagier 2013
universal Igusa form is the spectral-braiding datum of the target
package $\mathbf H_{\Delta_5}^{\mathrm{tgt}}$ at the K3 base point.
With spectral parameters $(z_1, z_2)$ on the K3 configuration and
dynamical parameters $(\tau, w)$ on the Siegel upper half-space
$\mathbb H_2$, its rank-$24$ Mukai-Heisenberg/Miki evaluation block
satisfies the dynamical Yang--Baxter equation on the paramodular
$K(1)$ locus \cite{PasolZagier2013,Felder1995}. A global braid on
modules for the compact Hall source is a recognition consequence, not
part of the target datum. Compare Vol.~III
...
At the Lusztig specialisation $\hbar^2 = -1/8$ the target braiding of
$R_{\mathrm{Sieg,dyn}}$ has the expected order-$8$ monodromy on the
rank-$24$ evaluation block, with $8 = K^{\kappaChHodge}$ the
Mukai-doubling root-of-unity order. If the finite Hall--Borcherds
recognition gate and the integral braid comparison are supplied, this
target monodromy lifts to a braid group action on
$\mathbf H_{\Delta_5}$-modules factoring through
$B_{24}/\{\sigma_i^8 = 1\}$. Before that lift, the Humbert divisor
$H_1\cup H_4$ is the spectral singular target
\cite{Lusztig1990,Bruinier2002}. Compare
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[rational-times-theta factorisation]
\label{rem:specbraid-3-supplement}
The K3 target factorisation promotes the implicit $R_{\mathrm{Sieg,dyn}}$
on the Mukai-Heisenberg/Miki evaluation block to the explicit
...
series \cite{PasolZagier2013,ES98}. On the
rank-$24$ block, the quantum Yang--Baxter equation for
$R_{\mathrm{Sieg,dyn}}$ on the paramodular locus $K(1)$ follows from
rational QYBE for
$R^{\mathrm{rat}}_{\mathrm{Yang}}$ together with the Pasol--Zagier
cyclic Siegel--Kronecker identity (the shifted pentagon for
$\theta^{\mathrm{K3}}$). Extension from this evaluation block to the
completed BKM object is the super-Etingof--Kazhdan Manin-pair
quantisation plus Hall--Borcherds recognition problem. Compare Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}
...
The RTT-style matrix is defined on the finite evaluation block,
\[
T_{\mathrm{Muk}}(u,Z)\in\mathrm{End}(V_{24})\otimes
\mathbf H_{\Delta_5}^{\mathrm{tgt}}[[u^{-1}]],
\]
where $V_{24}$ is the Mukai-Heisenberg/Miki block. Molev's quantum
determinant $\mathrm{qdet}\,T_{\mathrm{Muk}}(u,Z)$ is a central element
of this block \cite{Mol07}. If the finite Hall--Borcherds
recognition gate lifts this target series to the completed compact
Hall--Drinfeld source and preserves the centre, its image satisfies
\[
\mathrm{qdet}\,T(u,Z)=C(u)\cdot\Delta_5(Z)\cdot\mathrm{Id},
\qquad
C(u)\in 1+u^{-1}\mathbb C[[u^{-1}]],
\]
with $C(u)$ independent of $Z$. Before that centre-preserving lift,
$\Delta_5(Z)$ is the scalar Borcherds--Heegner target fixed by the
Gritsenko vanishing on $2H_1+H_4$
\cite{Gritsenko1999,GritsenkoNikulin1998}, not a proven generator of
the full centre. Compare Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}
