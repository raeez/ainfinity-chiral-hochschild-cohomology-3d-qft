...
\begin{remark}[Laplace transform = spectral duality]
\label{rem:laplace-spectral-duality}
The formula $r(z) = \int_0^\infty e^{-\lambda z} \{\cdot{}_\lambda \cdot\}\, d\lambda$ exhibits the Laplace transform as the precise bridge between the PVA world (parametrized by the spectral variable~$\lambda$) and the $R$-matrix world (parametrized by the position variable~$z$). Under the monograph's Drinfeld--Kohno ladder (DK-0), this recovers the classical $r$-matrix from the $\lambda$-bracket at the evaluation locus. The topological direction supplies the integration contour; the holomorphic direction supplies the meromorphic structure. In this sense, the Laplace transform \emph{is} the spectral duality of the holomorphic-topological theory.

At the operadic level, the Laplace transform mediates between the
two colors of the Swiss-cheese operad: the $\lambda$-bracket encodes
holomorphic (closed-color) singularities, while $R(z)$ encodes
topological (open-color) braiding.  The transform is the passage
from momentum space ($\lambda$) to position space ($z$).
\end{remark}
...
The spectral $R$-matrix $r(z)$ and the meromorphic tensor product of line operators fit naturally into the framework of \textbf{factorisation quantum groups} developed by Latyntsev \cite{Lat23}.
...
The key observation: the holomorphic weight forms entering~$m_n$
