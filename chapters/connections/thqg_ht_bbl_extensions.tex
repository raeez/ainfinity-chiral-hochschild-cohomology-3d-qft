...
characteristic of the $M2$ boundary algebra.  The value of
$\kappa_{M2}$ has not been computed from first principles:
the DDCA is not a tensor product of standard chiral algebras,
so the computation requires the full self-sewing trace
(Definition~\ref*{def:modular-curvature-direct})
on the cyclic pairing of $A(K)$.  \ClaimStatusOpen
...
(Vol~I, Definition~\ref*{def:modular-curvature-direct}).
The value of $\kappa_{M2}$ is open: the DDCA
is not a tensor product of standard chiral algebras, so
the self-sewing trace requires the full cyclic pairing
on~$A(K)$, which has not been computed from first principles.
...
\begin{remark}[Why $\kappa_{M2}$ is genuinely open]
\label{rem:kappa-m2-genuinely-open}
\index{M2-brane!modular characteristic open}
...
three obstructions prevent a direct computation.
...
it converges only if the summands decay sufficiently
fast in $w$.
...
\emph{What would resolve it.}
A computation of $\kappa_{M2}$ requires either:
\textup{(a)}~the full OPE of the DDCA as a vertex algebra,
including the bilinear form at each conformal weight
and a regularization of the infinite trace; or
\textup{(b)}~a physical argument (e.g.\ anomaly cancellation
in the eleven-dimensional context, or a spectral
$\zeta$-function regularization of the Casimir energy
of the five-dimensional theory) that determines the
genus-$1$ partition function of the boundary theory
and hence~$\kappa$.
Until one of these is available, $\kappa_{M2}$ remains
a well-defined but uncomputed invariant. \ClaimStatusOpen
\end{remark}
...
characteristic $\kappa_{M2}$ is open; the shadow tower
...
mirror symmetry at the level of modular characteristics (this vanishing is specific to the CY/free-field lineage; for $\mathcal{W}$-algebras, $\kappa+\kappa^! = K(\fg) \neq 0$).  The
...
Self-loop & $\Theta_{M2}$
& $\kappa_{M2}$ open, infinite shadow depth
\end{tabular}
\end{center}
