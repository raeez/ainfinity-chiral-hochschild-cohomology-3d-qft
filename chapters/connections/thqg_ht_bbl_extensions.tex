...
\end{equation}
where $\kappa_{M2}:=\kappaChHodge(A_{M2,\infty})$ is the
unregularized modular characteristic of the $M2$ boundary algebra.
The theorem does not identify this characteristic with a number.
The zeta-regularized benchmark
\[
\kappa_{M2}^{\zeta}:=K^2\zeta(-1)=-K^2/12
\]
is computed separately in Proposition~\ref{prop:m2-zeta-regularized-kappa};
identifying $\kappa_{M2}$ with $\kappa_{M2}^{\zeta}$ is the
comparison conjecture~\ref{conj:m2-zeta-comparison}.
...
$\kappa_{M2} := \kappaChHodge(A_{M2,\infty})$, the self-sewing trace
(Vol~I, Definition~\ref*{V1-def:modular-curvature-direct}).  The
display records the tautological genus-$1$ component of the
bar-intrinsic MC element.  The numerical zeta-regularized benchmark is
the separate computation of Proposition~\ref{prop:m2-zeta-regularized-kappa};
the assertion that the DDCA self-sewing trace is computed by that
regularization is Conjecture~\ref{conj:m2-zeta-comparison}.
...
\begin{definition}[$M2$ zeta-regularization datum]
\label{def:m2-zeta-regularization-datum}
\index{M2-brane!modular characteristic regularization}
Let \(A_{M2,\infty}\) be the completed double-differential chiral
algebra at matrix rank \(K\).  The \emph{\(M2\) zeta-regularization
datum} consists of the following two pieces:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=1.8em]
\item the Weyl cyclic trace on \(\Diff(\C)\otimes\Mat_K\), normalized
so that the anti-diagonal pair
\[
E^{(m,n)}_{\alpha\beta}\otimes E^{(n,m)}_{\beta\alpha}
\]
has Weyl pairing
\[
\tr_{\mathrm{Weyl}}\bigl(z^m\partial^n\cdot z^n\partial^m\bigr)
=(-1)^{m+n}m!\,n!;
\]
\item the spectral zeta functional on the conformal-weight counting
series, sending \(\sum_{n\ge1} n\) to \(\zeta(-1)\).
\end{enumerate}
It defines the zeta-regularized scalar
\[
\kappa_{M2}^{\zeta}
:=\sum_{w\ge0}^{\zeta} K^2(w+1).
\]
\end{definition}

\begin{proposition}[Zeta-regularized \(M2\) scalar;
\ClaimStatusProvedHere]
\label{prop:m2-zeta-regularized-kappa}
For the datum of Definition~\ref{def:m2-zeta-regularization-datum},
\[
\kappa_{M2}^{\zeta}=K^2\zeta(-1)=-\frac{K^2}{12}.
\]
Consequently the zeta-regularized genus-one scalar is
\[
F_1^{\zeta}(A_{M2,\infty})
=\frac{\kappa_{M2}^{\zeta}}{24}
=-\frac{K^2}{288}.
\]
\end{proposition}

\begin{proof}
At conformal weight \(w+1\) the monomials \(z^m\partial^n\) with
\(m+n=w\) give \(w+1\) differential-operator generators, and the
matrix factor contributes \(K^2\).  After orthogonalizing the
anti-diagonal Weyl pairing in Definition~\ref{def:m2-zeta-regularization-datum},
the weight-\(w+1\) contribution to the self-sewing scalar is therefore
\(K^2(w+1)\).  Hence
\[
\kappa_{M2}^{\zeta}
=K^2\sum_{w\ge0}^{\zeta}(w+1)
=K^2\sum_{n\ge1}^{\zeta}n
=K^2\zeta(-1)
=-\frac{K^2}{12}.
\]
The displayed value of \(F_1^{\zeta}\) is the scalar Hodge-lane
normalisation
\(F_1^{\zeta}=\kappa_{M2}^{\zeta}/24\).
\end{proof}

\begin{conjecture}[DDCA zeta-comparison;
\ClaimStatusConjectured]
\label{conj:m2-zeta-comparison}
The actual cyclic self-sewing trace defining
\[
\kappa_{M2}:=\kappaChHodge(A_{M2,\infty})
\]
is computed by the \(M2\) zeta-regularization datum:
\[
\kappa_{M2}=\kappa_{M2}^{\zeta}=-\frac{K^2}{12}.
\]
Equivalently, the BV cyclic pairing of the infinite-type DDCA is
compatible with the Weyl trace and spectral zeta functional of
Definition~\ref{def:m2-zeta-regularization-datum}.
\end{conjecture}

\begin{remark}[What is proved]
Proposition~\ref{prop:m2-zeta-regularized-kappa} proves the value of
the regularized scalar once the regularization datum has been installed.
Conjecture~\ref{conj:m2-zeta-comparison} is the remaining mathematical
input: it asks that the regularized Weyl trace be the actual
genus-one self-sewing trace of \(A_{M2,\infty}\).  The rejected values
\(K^2\), \(-K\), and \(-1/2\) fail respectively by using only the
weight-zero matrix trace, by having the wrong matrix scaling, and by
failing the \(K=1\) identity-supertrace normalization.
\end{remark}
...
$\kappa_{M2}^{\zeta}=-K^2/12$ and
$\kappa_{M2}=\kappa_{M2}^{\zeta}$ conjectural \\
$\nabla^{\mathrm{hol}}_{M2}
...
\(\kappa_{M2}^{\zeta}=-K^2/12\) is the proved zeta-regularized
benchmark of Proposition~\ref{prop:m2-zeta-regularized-kappa};
the identification \(\kappa_{M2}=\kappa_{M2}^{\zeta}\) is
Conjecture~\ref{conj:m2-zeta-comparison}.  The shadow obstruction
tower is infinite regardless of the value of \(\kappa_{M2}\).
\end{computation}
...
\bigr).
\end{equation}
When $Y$ has $h^{2,1}=0$, the infinitesimal complex-structure
space has zero dimension and the displayed quadratic map has zero
source. The case $h^{2,1}=1$ is one-dimensional, not rigid.
...
the $A_\infty$ structure on $H^\bullet(Y,T_Y)$. The
Bogomolov--Tian--Todorov theorem makes the deformation
dg Lie algebra homotopy abelian and kills the Kuranishi
obstruction classes. It does not identify the full chiral bar
complex with a Gaussian Koszul model.
\end{computation}
...
\begin{enumerate}[label=(\roman*),leftmargin=1.8em]
\item The dg Lie algebra controlling deformations of complex
structure is homotopy abelian; hence the Kuranishi obstruction map
vanishes.
\item Chiral Koszul concentration and Gaussian shadow depth for
$V_{\partial,\mathrm{KS}}(Y)$ are extra hypotheses on the
factorization-algebra model; they are not consequences of BTT alone.
\item The weight-one Hodge supertrace is
\begin{equation}% label removed: eq:ks-kappa
\kappa_{\mathrm{KS}}^{(1)}(Y) = \frac{\chi(Y)}{2}
= h^{1,1}-h^{2,1}.
\end{equation}
...
\begin{proof}
(i)~Tian--Todorov unobstructedness, in the Barannikov--Kontsevich
form, follows from the $\partial\bar\partial$-lemma for polyvector
fields. This is a statement about the Maurer--Cartan deformation
functor, not about every operation in the chiral bar construction.

(ii)~The chiral bar complex contains the vertex-algebraic collision
data. A Gaussian truncation is therefore additional factorization
data, or a chosen scalar projection, rather than a formal consequence
of unobstructed complex-structure deformation.

(iii)~The weight-one supertrace is
computed from the row $H^\bullet(Y,T_Y)$:
...
With the B-model sign convention
\[
\kappa_{\mathrm{KS}}^{(1)}
=-\operatorname{str}(\mathrm{id}|_{V^{(1)}})
= h^{1,1} - h^{2,1}.
\]
For a Calabi--Yau threefold, $\chi(Y)=2(h^{1,1}-h^{2,1})$.

(iv)~The BCOV anomaly row is the one-loop scalar of
Bershadsky--Cecotti--Ooguri--Vafa and Costello--Li. Its coefficient
is $\chi(Y)/24$, and it is not the same invariant as the
weight-one Hodge supertrace.
\end{proof}
...
\subsection{Line operators and Morita generators}
...
to the derived category of coherent sheaves. If $G_Y$ is a
split-generator and
\[
 E_Y:=\RHom_Y(G_Y,G_Y),
\]
then derived Morita theory gives
...
$\RHom_Y(G_Y,F)$. The special object $\cO_Y$ gives only
$\RHom_Y(\cO_Y,\cO_Y)\simeq H^\bullet(Y,\cO_Y)$; it represents the
whole line category only on loci where $\cO_Y$ is a split-generator.
If a dg Morita equivalence
\[
 \Psi_Y\colon V_{\partial,\mathrm{KS}}^!(Y)\longrightarrow E_Y
\]
is fixed, then the same category may be written as
...
of coherent sheaves. Smooth proper schemes admit classical
split-generators, and Bondal--Van den Bergh derived Morita theory
identifies $D^b\mathrm{Coh}(Y)$ with perfect modules over
$\RHom_Y(G_Y,G_Y)$. The algebra
$\RHom_Y(\cO_Y,\cO_Y)$ is $H^\bullet(Y,\cO_Y)$ and is not a Morita
model for $D^b\mathrm{Coh}(Y)$ unless $\cO_Y$ split-generates. A
Koszul-dual module presentation is therefore the derived Morita
presentation transported along $\Psi_Y$.
\end{proof}
...
Let $Y$ be a compact Calabi--Yau threefold, let $G_Y$ be a
split-generator of $D^b\mathrm{Coh}(Y)$, and put
$E_Y=\RHom_Y(G_Y,G_Y)$. Equip the Kodaira--Spencer factorization
algebra with a scalar sewing projection and, when the line category is
written as $V_{\partial,\mathrm{KS}}^!(Y)$-modules, a Morita
equivalence $\Psi_Y\colon V_{\partial,\mathrm{KS}}^!(Y)\to E_Y$.
Then the holographic modular Koszul datum is
...
$\cC = D^b\mathrm{Coh}(Y)\simeq\Perf(E_Y)$
...
& Gauss--Manin projection of the shadow connection at
genus~$0$ \\
\hline
\end{tabular}
\end{center}
On this ambient the weight-one Hodge supertrace is
$\kappa_{\mathrm{KS}}^{(1)}(Y)=h^{1,1}-h^{2,1}$, while the BCOV
one-loop scalar row is
...
$V_{\partial,\mathrm{KS}}$; no Gaussian truncation is asserted before
the scalar sewing projection is fixed),
and $\nabla^{\mathrm{hol}}_{\mathrm{KS}}$ from the shadow
connection formula after projection to the deformation complex.
...
The two scalar rows are those of the preceding theorem.
\end{proof}
...
then
\[
\kappa_{\mathrm{KS}}^{(1)}(Y) + \kappa_{\mathrm{KS}}^{(1)}(\check{Y})
= (h^{1,1}(Y)-h^{2,1}(Y)) + (h^{1,1}(\check{Y})-h^{2,1}(\check{Y}))
= 0.
\]
For Kodaira--Spencer/Calabi--Yau mirror pairs, this is the
anti-invariance of the weight-one Hodge row under mirror symmetry
(this vanishing is specific to the CY/free-field lineage; for
$\mathcal{W}$-algebras, $\kappa+\kappa^! = \varrho_{\cA}\,K \neq 0$
...
The line category $D^b\mathrm{Coh}(Y)$ is Morita equivalent to
$\Perf(E_Y)$ for any split-generator $G_Y$, with
$E_Y=\RHom_Y(G_Y,G_Y)$. The object $\cO_Y$ gives the point-vacuum
algebra $H^\bullet(Y,\cO_Y)$, not the endomorphism algebra of the
whole line category.
\end{computation}
...
\subsection{Holonomy-Trivial Globalization}
% label removed: subsec:conditional-global

\begin{corollary}[Holonomy-trivial globalization; \ClaimStatusProvedHere]
% label removed: cor:conditional-globalization
If the defect holonomy trivializes, the global Koszul triangle
\eqref{eq:global-triangle-theorem} holds with no residual monodromy
obstruction.
...
& \textbf{scalar row} & \textbf{Genus-$0$ holonomy packet} \\
\hline
...
& companion $\mathcal{W}_3{}_{100-c}$ & $5c/6$
& characteristic comparison surface; same-family duality conditional \\
KS on CY $Y$ & $\PV(Y)$ & $\Omega^\bullet(Y)$
& $\kappa_{\mathrm{KS}}^{(1)}=h^{1,1}-h^{2,1}$
& Gauss--Manin comparison surface \\
$M2$ at rank $K$ & $A_{M2,\infty}$ & $Y^{(2)}(\mathfrak{gl}_K)$
& $\kappa_{M2}^{\zeta}=-K^2/12$; comparison conjectural
& benchmark M2 comparison surface \\
...
For the \(M2\) row, the scalar entry is the zeta-regularized benchmark
of Proposition~\ref{prop:m2-zeta-regularized-kappa}; replacing it by
the actual \(\kappa_{M2}\) requires
Conjecture~\ref{conj:m2-zeta-comparison}.  The global triangle is
unconditional only when that packet is
...
$\Bbound$ (the higher chiral operations from \cite{GKW25});
\item $\nabla^{\mathrm{hol}}_{T,0,n}
...
then the mixed projection, the derived centre is the closed-colour
comparison vertex, and the genus tower is the closed self-loop
projection. A physical bulk reading requires the same HT
prefactorization and exact-sector comparison data as in the active
bulk--Hochschild theorem.
...
legs contributes a term proportional to $\kappaChHodge(\Bbound)\cdot\omega_1$
...
& $\Omega_{\PV}^{\sn}/z$ on the deformation leading term \\
$(1,1)$-mixed & $\beta_{\der}$
& HKR quasi-isomorphism \\
Self-loop & $\Theta_{\mathrm{KS}}$
& $\kappa_{\mathrm{KS}}^{(1)}=h^{1,1}-h^{2,1}$ and
$\kappa_{\mathrm{BCOV}}=\chi(Y)/24$
\end{tabular}
\end{center}
The convolution algebra is simpler than the $M2$ case at the
deformation-complex projection, where the Tian--Todorov homotopy
abelian model controls the Kuranishi problem. The full chiral
boundary algebra still contains Schouten collision data and BCOV
loop data, so no Gaussian termination is asserted here.
\end{computation}
...
\section{Synthesis}
\label{sec:ht-bbl-ext-supplement}

\begin{remark}[HT bulk--boundary--line extension at the base]
\label{rem:ht-bbl-ext-1}
The HT bulk--boundary--line extension programme, when specialised to
the K3 bialgebra $\mathbf H_{\Delta_5}$, exhibits the
...
\end{itemize}
The three layers are related by the bulk--boundary--line
Koszul-duality tower: Bulk $=$ centre of Boundary, Boundary $=$ centre
of Line. This is the realisation of the ${\sf BBL}$ operadic
...
Mukai-doubling $K^{\kappaChHodge} = 8$. Compare Vol.~III,
