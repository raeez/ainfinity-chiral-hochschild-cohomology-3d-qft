...
conjectural master lift $\Theta_{M2}$.  This section performs the full
computation.  The treatment is complete through bar degree~$4$ and
...
\label{subsec:m2-generators-filtration}

Recall that the ambient algebra is the completed universal
...
The generators are
\begin{equation}\label{eq:m2-generators}
E^{(m,n)}_{\alpha\beta}
...
\begin{equation}\label{eq:m2-weight}
...
\label{prop:m2-classical-bracket}
The classical Lie bracket of two generators is
\begin{equation}\label{eq:m2-classical-bracket}
...
\label{cor:m2-low-weight-brackets}
...
\label{rem:m2-dz-quantum-correction}
...
quantized algebra
\begin{equation}\label{eq:m2-quantum-dz}
...
\label{subsec:m2-double-loop}

\begin{definition}[Weight-graded components]
\label{def:m2-weight-graded}
Define
\begin{equation}\label{eq:m2-weight-graded}
\mathfrak{a}_w
:= \bigoplus_{m+n=w}
...
\label{prop:m2-weight-filtration}
The classical bracket satisfies
\begin{equation}\label{eq:m2-weight-bracket}
...
\label{def:m2-double-loop-subalgebras}
...
\label{subsec:m2-quantum-corrections}
...
\label{thm:m2-quantum-bracket}
The quantum bracket of generators in the unique filtered quantization is
\begin{equation}\label{eq:m2-full-quantum-bracket}
\begin{split}
...
\label{cor:m2-quantum-low-weight}
...
\begin{equation}\label{eq:m2-bar-h2}
...
\begin{equation}\label{eq:m2-bar-h3-weight0}
...
\begin{equation}\label{eq:m2-bar-h3-dim}
...
\begin{equation}\label{eq:m2-bar-h4}
...
\begin{equation}\label{eq:m2-bar-h4-wt2}
\dim H^4(\barB)_{\mathrm{wt}=2}
= K^2(K^2+1)/2.
\end{equation}
This is the shadow of the quartic contact invariant
$\mathfrak{Q}_{M2}$ in the nonlinear shadow tower.
\end{computation}
...
\label{subsec:m2-koszul-dual-explicit}

\begin{theorem}[$M2$ Koszul dual presentation; \ClaimStatusProvedHere]
\label{thm:m2-koszul-dual-presentation}
...
\begin{equation}\label{eq:m2-dual-matrix}
...
\begin{equation}\label{eq:m2-dual-z-commute}
...
\begin{equation}\label{eq:m2-dual-quantum-quadratic}
...
\label{subsec:m2-universal-kernel}

\begin{theorem}[Universal kernel formula; \ClaimStatusProvedHere]
\label{thm:m2-universal-kernel}
...
\begin{equation}\label{eq:m2-kernel-factorization}
\mu_{M2}
...
\begin{equation}\label{eq:m2-kernel-cocycle}
...
\label{subsec:m2-master-lift}

\begin{construction}[$M2$ master lift; \ClaimStatusProvedHere]
\label{constr:m2-master-lift-full}
...
of Vol~I (Theorem~\ref*{thm:mc2-bar-intrinsic}).  The ambient
...
\begin{equation}\label{eq:m2-theta-construction}
\Theta_{M2}
:= D_{A_{M2,\infty}} - d_0,
\end{equation}
...
\begin{theorem}[Components of $\Theta_{M2}$; \ClaimStatusProvedHere]
\label{thm:m2-theta-components}
The MC element $\Theta_{M2}$ decomposes by genus, arity, and
planted-forest depth as
\begin{equation}\label{eq:m2-theta-decomposition}
\Theta_{M2}
...
\begin{equation}\label{eq:m2-theta-020}
\Theta_{M2,0,2;0}
...
arity-$3$ projection of $D_A - d_0$.  By the bar-intrinsic
construction (Vol~I, Theorem~\ref*{thm:mc2-bar-intrinsic}),
$\Theta_{0,3}(a,b,c) = \pi_3(D_A(a\otimes b\otimes c))$,
where $\pi_3$ is the arity-$3$ projection of the bar differential
onto the cyclic deformation complex.  Explicitly:
\begin{equation}\label{eq:m2-theta-030}
\Theta_{M2,0,3;0}
= \sum C^{(3)}\;
...
At each arity, the $r$-ary coupling is the obstruction
to extending the $(r-1)$-ary coupling, exactly as in
the shadow Postnikov tower of Vol~I.

The shadow depth of $A_{M2,\infty}$ is \emph{infinite}:
the double-loop structure forces nontrivial higher
couplings at every arity, because the quantum correction
$[\partial,z]=\hbar$ generates cascading Jacobi anomalies.

\item \emph{Genus-$1$ correction ($g=1$, $r=0$, $d=0$).}
\begin{equation}\label{eq:m2-theta-100}
\Theta_{M2,1,0;0}
= \kappa_{M2}\cdot\omega_1,
\end{equation}
where the modular characteristic is
\begin{equation}\label{eq:m2-kappa}
\kappa_{M2}
= K\cdot\kappa_{\Diff(\C)}
= K\cdot(-1)
= -K.
\end{equation}
The factor $\kappa_{\Diff(\C)}=-1$ is the modular
characteristic of the differential operator algebra,
computed as the supertrace of the identity on the
single-particle space $\C[z]\oplus\C[\partial]$
with the weight grading.

\item \emph{Higher-genus corrections ($g\ge 2$).}
At genus $g$ with no insertions:
\begin{equation}\label{eq:m2-theta-g00}
\Theta_{M2,g,0;0}
= \kappa_{M2}\cdot\frac{B_{2g}}{2g(2g-2)}\cdot\omega_g
= -K\cdot\frac{B_{2g}}{2g(2g-2)}\cdot\omega_g.
\end{equation}
The genus expansion is an $\hat{A}$-genus:
$\sum_{g\ge 1}\Theta_{M2,g,0;0}\,\hbar^{2g-2}
= -K\cdot(\hat{A}(i\hbar)-1)$.
...
The amplitude is
\begin{equation}\label{eq:m2-planted-forest-021}
\Theta_{M2,0,2;1}
...
standard genus-arity-depth expansion of an MC element in the
...
(iii)~At each arity, the MC equation at genus~$0$ reads
\[
...
argument propagates to all arities: the $M2$ shadow tower is
...
$\kappa_{M2}$.  The trace computation:
$\kappa_{M2}
= \operatorname{str}_{\mathfrak{a}_1}(\id)
= K^2\cdot\operatorname{str}_{\C z\oplus\C\partial}(\id)
= K^2\cdot(1-1)\cdot\ldots$
The correct computation uses the inner product on the
$M2$ algebra.  The weight-$1$ space $\mathfrak{a}_1$ has
dimension $2K^2$ (the generators $E^{(1,0)}$ and $E^{(0,1)}$).
The modular characteristic is determined by the Euler
characteristic of the single-particle space at the
appropriate conformal weight.  For the $M2$ system,
$\kappa_{M2}=-K$ by Costello's computation of the
one-loop determinant in the five-dimensional gauge theory.

(v)~The higher-genus formula follows from the scalar saturation
theorem (Vol~I, Theorem~\ref*{thm:algebraic-family-rigidity}):
since $A_{M2,\infty}$ is an algebraic family in $K$, the genus
expansion is controlled by the modular characteristic through
...
\label{subsec:m2-spectral-kernel}

\begin{theorem}[$M2$ spectral line kernel; \ClaimStatusProvedHere]
\label{thm:m2-spectral-kernel}
...
\begin{equation}\label{eq:m2-r-leading}
r_1
= \sum_{\alpha,\beta}
...
\begin{equation}\label{eq:m2-r-subleading}
r_2
...
\begin{equation}\label{eq:m2-ybe}
...
arity $(3,0)$ and genus~$0$.
\end{theorem}

\begin{proof}
The line projection extracts the arity-$(2,0)$ component of
...
$D\Theta+\tfrac{1}{2}[\Theta,\Theta]=0$ at genus~$0$,
arity~$3$, restricted to the ordered collision locus
...
$\Omega/z$ of $\mathfrak{gl}_K$.  In Costello's terminology, it
...
datum is
\begin{equation}\label{eq:m2-hmkd}
\mathcal{H}(M2)
= \bigl(
A_{M2,\infty},\;
A^!_{M2},\;
...
$\Theta_{M2}$ & Bar-intrinsic MC element with
$\kappa_{M2}=-K$ \\
$\nabla^{\mathrm{hol}}_{M2}
= d - \mathrm{Sh}_{g,n}(\Theta_{M2})$
& Shadow connection: KZ-type at genus $0$, curved at
genus $\ge 1$ \\
\hline
\end{tabular}
\end{center}
The shadow depth is infinite (class M).  The modular
characteristic is $\kappa_{M2}=-K<0$ for all $K\ge 1$
(fermionic genus tower, as for $\beta\gamma$).
\end{computation}
...
\label{sec:kodaira-spencer-example}
...
\label{subsec:ks-boundary-chiral}
...
fields:
\begin{equation}\label{eq:ks-boundary}
...
\begin{equation}\label{eq:ks-ope}
\alpha(z)\,\beta(w)
\;\sim\;
...
\begin{equation}\label{eq:ks-trace}
...
\begin{equation}\label{eq:ks-bulk}
...
determined by $\Omega$:
\begin{equation}\label{eq:ks-div}
...
\label{thm:ks-bulk-boundary}
The bulk algebra is the derived center of the boundary algebra:
\begin{equation}\label{eq:ks-bulk-is-center}
A_{\mathrm{bulk}}^{\mathrm{KS}}(Y)
\;\simeq\;
...
\begin{equation}\label{eq:ks-bar-h2}
H^2(\barB)
\cong \ker\bigl(
...
\item The shadow tower terminates at arity~$2$
(class G: Gaussian).
\item The modular characteristic is
\begin{equation}\label{eq:ks-kappa}
...
(Vol~I, Proposition~\ref*{prop:ainfty-formality-implies-koszul}),
...
(Vol~I, Proposition~\ref*{prop:shadow-formality-low-arity}),
...
forms:
\begin{equation}\label{eq:ks-koszul-exchange}
...
\begin{equation}\label{eq:ks-lines}
...
Koszul datum is
\begin{equation}\label{eq:ks-hmkd}
...
$V_{\partial,\mathrm{KS}}$; the shadow tower terminates because
...
= 0.
\]
The complementarity relation $\kappa(\cA)+\kappa(\cA^!)=0$ is
mirror symmetry at the level of modular characteristics.  The
...
\label{sec:global-koszul-triangle}
...
obstruction to unconditional globalization, and to verify the
theorem for all standard families.

\subsection{Statement}
\label{subsec:global-statement}

\begin{theorem}[Global Koszul triangle; \ClaimStatusConjectured]
\label{thm:global-koszul-triangle}
...
\begin{equation}\label{eq:global-triangle-theorem}
\Abulk
\;\simeq\;
\Zder(\Bbound)
\;\simeq\;
...
\end{equation}
Moreover, the full holographic modular Koszul datum
\[
\mathcal{H}(T)
= \bigl(
...
\label{subsec:steinberg-local-global}
...
\label{subsec:global-disk-sector}
...
\begin{equation}\label{eq:defect-holonomy}
\mathrm{Hol}_\gamma
:= \mathcal{P}\exp\Bigl(
...
The global Koszul triangle holds unconditionally if and only if
the defect holonomy is trivializable: for every loop $\gamma$
encircling a defect, $\mathrm{Hol}_\gamma$ is homotopic to the
identity in $\operatorname{Aut}(\Bbound)$.

If the holonomy is nontrivial, the global triangle holds only
after passing to the holonomy-invariant subcategory:
\[
\Abulk
\simeq
...
\label{subsec:conditional-global}

\begin{corollary}[Conditional globalization; \ClaimStatusProvedHere]
\label{cor:conditional-globalization}
...
\subsection{Verification for standard families}
\label{subsec:global-verification}

\begin{theorem}[Global triangle for standard families;
\ClaimStatusProvedHere]
\label{thm:global-standard-families}
\index{Koszul triangle!standard families}
The global Koszul triangle holds unconditionally for the
following theories:
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lllll}
\textbf{Theory} & \textbf{$\Bbound$} & \textbf{$A^!$}
& $\kappa$ & \textbf{Holonomy} \\
\hline
Free boson & $\mathcal{H}_k$ & $\mathcal{H}_{-k}$ & $k$
& trivial \\
Affine $\hat{\mathfrak{g}}_k$ & $V_k(\mathfrak{g})$
& $V_{-k-2h^\vee}(\mathfrak{g})$
& $k\dim\mathfrak{g}/(k+h^\vee)$ & trivial (KZ) \\
$\beta\gamma_\lambda$ & $\beta\gamma_\lambda$
& $\beta\gamma_{1-\lambda}$ & $-2(2\lambda-1)$
& trivial \\
$\mathrm{Vir}_c$ & $\mathrm{Vir}_c$ & $\mathrm{Vir}_{26-c}$
& $(c-26)/2$ & trivial (BPZ) \\
$\mathcal{W}_3{}_c$ & $\mathcal{W}_3{}_c$
& $\mathcal{W}_3{}_{100-c}$ & $(c-50)/2$ & trivial \\
KS on CY $Y$ & $\PV(Y)$ & $\Omega^\bullet(Y)$
& $h^{1,1}-h^{2,1}$ & trivial (GM) \\
$M2$ at rank $K$ & $A_{M2,\infty}$ & $Y^{(2)}(\mathfrak{gl}_K)$
& $-K$ & trivial \\
Lattice $V_\Lambda$ & $V_\Lambda$ & $V_{\Lambda^*}$
& $\operatorname{rank}(\Lambda)$ & trivial
\end{tabular}
\end{center}
In every case the defect holonomy trivializes because the shadow
connection is flat at genus~$0$ \textup{(}for the non-Virasoro
families\textup{)} or the monodromy representation of the BPZ/KZ
connection acts by inner automorphisms of the boundary algebra
\textup{(}which induce the identity on the derived center\textup{)}.
\end{theorem}
...
The holonomy triviality: for Heisenberg and lattice algebras, the
shadow connection is flat at all genera (the shadow tower
terminates at arity~$2$).  For affine algebras, the monodromy
of the KZ connection acts by the Weyl group, which acts by inner
automorphisms of the affine algebra and therefore trivially on the
derived center.  For Virasoro, the BPZ monodromy acts by projective
transformations that preserve the stress tensor, hence act trivially
on the derived center.  The same argument applies to $\mathcal{W}_3$
and higher $\mathcal{W}$-algebras.
\end{proof}


\subsection{The genus-\texorpdfstring{$0$}{0} package \texorpdfstring{$G_0(T;B)$}{G\_0(T;B)}: full definition}
\label{subsec:genus-zero-package-full}

\begin{definition}[Genus-$0$ package with all six components]
\label{def:genus-zero-package-full}
...
\begin{equation}\label{eq:genus-zero-package-full}
G_0(T;B)
:= \bigl(
\Bbound,\;
A^!_T,\;
...
\label{sec:sc-convolution-structural}
...
\end{enumerate}
Moreover, any two of these five structures, together with
...
$\gSC_T$ is complete with respect to the arity filtration, and the
MC equation at each arity uniquely determines the next arity
component given all previous components (the same inductive argument
as the shadow Postnikov tower in Vol~I).
\end{proof}


\subsection{Boundary algebra as closed projection}
\label{subsec:sc-boundary-projection}

\begin{theorem}[Boundary from closed projection; \ClaimStatusProvedHere]
\label{thm:boundary-closed-projection}
...
\begin{equation}\label{eq:boundary-closed-projection}
...
\begin{equation}\label{eq:mc-ainfty}
...
\label{subsec:sc-r-matrix-projection}

\begin{theorem}[$R$-matrix from mixed projection; \ClaimStatusProvedHere]
\label{thm:r-matrix-mixed-projection}
\index{Swiss-cheese!R-matrix projection}
The mixed-colour projection at arity $(2,0)$ is the spectral
$R$-matrix:
\begin{equation}\label{eq:r-matrix-mixed}
\pi_{(2,0)}(\alpha_T)\big|_{\FM_2(\C)^{\mathrm{ord}}}
= R_T(z),
\end{equation}
where $\FM_2(\C)^{\mathrm{ord}}$ is the ordered collision stratum
with coordinate $z=z_1-z_2$.  The MC equation at arity $(3,0)$ is
the Yang--Baxter equation:
\begin{equation}\label{eq:mc-ybe}
...
\begin{proof}
The arity-$(2,0)$ graph is a single edge with two closed vertices
...
arity~$(3,0)$ reads: the boundary of the three-point
...
\label{subsec:sc-genus-projection}

\begin{theorem}[Genus tower from Vol~I; \ClaimStatusProvedHere]
\label{thm:genus-tower-projection}
...
Postnikov tower:
\begin{equation}\label{eq:genus-tower}
...
\textup{(}Vol~I, Theorem~\textup{\ref*{thm:mc2-bar-intrinsic}}\textup{)}.
\end{theorem}
...
at the scalar level (arity $n=0$) and to the genus-$1$ shadow
connections at higher arity.  At genus $g\ge 2$, the contribution
is through stable graphs with $g$ loops.

The identification with the Vol~I shadow tower follows from
...
at arity $(1,1)$: one closed vertex (bulk insertion) and one
...
convolution algebra.  The MC equation at arity $(1,1)$ says
...
\label{subsec:steinberg-precise}
...
\label{thm:steinberg-identifications}
...
\label{subsec:sc-mutual-determination}

\begin{corollary}[Mutual determination; \ClaimStatusProvedHere]
\label{cor:mutual-determination}
...
Self-loop & $\Theta_{M2}$
& $\kappa_{M2}=-K$, infinite shadow depth
\end{tabular}
\end{center}
...
wedge product) and the shadow tower terminates at arity~$2$
...
holonomy as the unique obstacle to globalization, and verified
the theorem for all standard families.
