...
The non-principal Drinfeld--Sokolov reduction carries a gap in the
...
half-integer Kazhdan weight. The individual summands lack a definition
as sections of a line bundle on the curve~$X$ until a spin structure
or a branched cover is specified. The route is to pull the construction
back to a degree-$d_f$ cyclic cover $\pi\colon \widetilde X \to X$ on
which the Kazhdan grading is integer, execute the Sugawara identity
upstairs, and descend to~$X$ via $\mathrm{Deck}(\pi)$-invariants. The
resulting $\Ethree$-topologization holds for all good-graded
nilpotents.
...
\item \textup{(Galois-invariant subcomplex)} the subcomplex
 ...
 sections is closed under the total DS differential
 \(Q_{\mathrm{DS},f}^{\widetilde X}\) and identifies via the pushforward
 $\pi_{*}$ with a subcomplex of the $X$-complex $\mathcal{E}_{X}$;
\item \textup{(descent of Sugawara antighost)}
 ...
 $[Q_{\mathrm{DS},f}, G'_{\mathrm{DS},f,X}] = T_{\mathrm{DS}}(f)$ on
 $Q_{\mathrm{DS},f}$-cohomology on~$X$.
\end{enumerate}
\end{theorem}
...
with $\fg_{\pm 1/2}$ of dimension $2(N-2) = 4$ (four roots
$e_{1}-e_{2}, e_{1}-e_{3}, e_{2}-e_{4}, e_{3}-e_{4}$ in
$\fg_{1/2}$). The
...
over with (i) four short-root fermionic ghost pairs instead of two,
...
makes holomorphic translations \(Q_{\mathrm{DS},f}\)-exact, so the
factorisation algebra on the $X$-direction is locally constant on
$Q_{\mathrm{DS},f}$-cohomology, hence $\Etwo^{\mathrm{top}}$. Combined
...
The branched-cover construction of
Theorem~\ref{thm:E3-topological-DS-general-all-good-graded} resolves
the fractional-weight ghost obstruction at non-principal DS.
Theorem~\ref{thm:E3-topological-DS-general} extends to non-principal
good-graded nilpotents via branched-cover integralization plus Galois
descent. The $\Ethree$-topological structure holds uniformly across:
\begin{itemize}[nosep]
...
\item minimal nilpotents in exceptional types $E_{6}, E_{7}, E_{8},
 F_{4}, G_{2}$ \textup{(}$d_{f} = 2$ in most cases, $d_{f} = 1$
 for the Langlands-dual long-root case\textup{)}.
\end{itemize}
\end{corollary}

\subsection{Three independent routes to the BP data}
\label{subsec:three-lane}

The branched-cover proof of
Theorem~\ref{thm:E3-topological-DS-general-all-good-graded} is one of
three independent constructions of the BP central charge and the
$\Ethree$-topological datum.

\begin{proposition}[three-route concordance for BP]
\label{prop:three-lane-fractional-ghost}
\ClaimStatusProvedHere{}

For the BP algebra $\cW_{3}^{(2)}$, the central charge and the
...
\item the DS branched-cover construction
 \textup{(}Theorem~\ref{thm:E3-topological-DS-general-explicit-BP}\textup{)};
\item the Khan--Zeng $3$d Poisson-vertex sigma model on the freely
 generated graded algebra
 $\operatorname{gr}_{\mathrm{Li}}(\cW_{3}^{(2)})$
 \textup{(}Theorem~\ref{thm:E3-topological-free-PVA}\textup{)};
\item the de Boer--Tjin 1993 screened-free-field realisation
 combined with Feigin--Frenkel intersection.
\end{enumerate}
All three recover the Fateev--Lukyanov BP central charge
\[
c^{\mathrm{FL}}_{\mathrm{BP}}(k)
=-\frac{(2k+3)(3k+1)}{k+3}
\]
and the
$\Z/2$-Galois-equivariant Sugawara antighost structure.
\end{proposition}

\begin{proof}
(R1) is Theorem~\ref{thm:E3-topological-DS-general-explicit-BP}.
(R2) is Theorem~\ref{thm:E3-topological-free-PVA} applied to
$\operatorname{gr}_{\mathrm{Li}}(\cW_{3}^{(2)})$, freely generated on
$T, G^{\pm}, J$ by de Boer--Tjin 1993; Khan--Zeng produces the 3d
Poisson sigma model. (R3) is the direct de Boer--Tjin realisation of
BP as a sub-VA of three screened free bosons with the same
Fateev--Lukyanov central charge; the Feigin--Frenkel intersection equips the Koszul-dual
chiral Hochschild with $\Ethree$-topological data via the abelian
Heisenberg template. The three central-charge expressions coincide
as rational functions of \(k\); the $\Ethree$-topological data agree at $k = 0$ by
matching conformal vectors and Sugawara antighost structures across
routes.
\end{proof}
...
The branched-cover construction supports
Theorem~\ref{thm:E3-topological-DS-general} for all
good-$(1/d_{f})$-graded nilpotents, with the fractional-weight
obstruction resolved by branched-cover integralization and
Galois-invariant descent. The three independent routes of
Proposition~\ref{prop:three-lane-fractional-ghost} agree on the BP
fibre.
\end{remark}
...
$E_{n}$-operadic circle in the same way as principal~$\cW$: the derived center
...
line-transverse $E_1$-topological direction. Non-principal DS does not alter the
...
\begin{remark}[Non-principal DS as universal-conductor input]%
\label{rem:fractional-ghost-non-principal-input}%
%
%
\index{branched-cover descent!chain-level avatar}%
The branched-cover integralisation and Galois-descent technology
above closes the fractional-ghost reconstitution and supplies the
non-principal stratum of the universal-holography master theorem
(\textsf{thm:universal-holography-master} in
\textsf{chap:universal-conductor}; \textsf{chap:climax-platonic}).

The corollary \textsf{cor:rung-w-N} of the climax theorem covers
principal $\cW_N$ via integer Kazhdan grading; the chain-level
antighost lift $T_{\mathrm{DS}}(f) = [Q_{\mathrm{tot}}, G_f]$ extends
to all good-$(1/d_f)$-graded nilpotents via the cover technology
(Theorem~\ref{thm:E3-topological-DS-general-all-good-graded}). The
BP companion chapter
(Vol~II Chapter~\ref{chap:bp-chain-level-strict-platonic}) supplies
the explicit chain-level lift for the BP fibre on the original
complex, strengthening the cohomological statement to a chain-level
identity.

The branched-cover construction is invisible to the abstract
$E_3$-algebra; the Galois descent introduces a Deck action that
becomes the explicit conductor datum in the climax:
$K^{\mathrm{cover}}_{f,\mathrm{Deck}}
 := |\mathrm{Deck}(\pi)|^{-1} \mathrm{tr}_{\mathrm{Deck}}$ records
the fractional-ghost orbifold weight. For BP ($d_f = 2$, Deck $=
\Z/2$) this is a $1/2$ trace; for subregular $D_n$ ($d_f = 3$, Deck
$= \Z/3$) a $1/3$ trace. The conductor reads from the Kazhdan
denominator $d_f$ via the Elashvili--Kac tabulation
(Example~\ref{ex:kazhdan-denominator-table}).

The non-principal DS stratum carries the climax uniformly across the
standard landscape of $\cW$-algebras: principal $\cW_N$ realises the
rung-$\cW_N$ corollary at integer Kazhdan grading; the minimal
families (BP, minimal $\cW^k(\mathfrak{sl}_4)$) and subregular
families occupy the non-principal stratum, where the cover
construction applies. The chain-level avatar of the climax extends
to the entire DS image of the affine landscape, supplying the
fractional-ghost input for the universal-holography comparison on
that image.
\end{remark}

% ============================================================================
% End of structural reconstitution of the fractional-ghost reconstitution heal.
