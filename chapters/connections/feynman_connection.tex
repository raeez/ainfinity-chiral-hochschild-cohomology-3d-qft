...
the comparison has three pieces: the chiral-residue chain
...
the anomaly-free locus (Theorem~\ref{thm:brst-bar-genus0}); the
higher-genus chain-level obstruction is the harmonic discrepancy
isolated in Theorem~\ref{thm:bv-bar-coderived}.  A Feynman drawing
is therefore a controlled dictionary: Arnold form
$=$ logarithmic collision form, vertex $=$ chiral residue, boundary
stratum $=$ stable-graph gluing.  Loop number does not equal genus.
...
lift carries labelled half-edges and Arnold forms on configuration
spaces; the symmetric modular quotient produces the factor
...
number only after a ribbon or fatgraph perturbative model is
chosen.  The equation
\[
...
$G(z,w)=\kappa^{\mathrm{Heis}}/(z-w)^2$, and there are no interaction
...
form along the diagonal.  The form
is not multiplied with the Wick kernel to form a new meromorphic
...
stable-graph recursion carries no cubic interaction term; the genus-$1$
...
model it is bounded by graph loop number as above. The
Heisenberg scalar obstruction is linear in the level:
\[
\operatorname{curv}_g(\mathcal{H}_{\kappa^{\mathrm{Heis}}})
= \kappa^{\mathrm{Heis}}\omega_g,\qquad
...
\]
which is modular invariant as a density on $\mathcal{M}_{1,0}$.  This
is the analytic determinant-line amplitude of the free theory. The bar
complex does not identify this analytic function with a homology group
or construct it as a field-space measure integral.  Its
zero-mode-reduced vacuum line is
\[
H^0\bigl(\bar{B}^{(1)}
...
\[
F_1(\cA)=\kappaChHodge(\cA)\deg(\lambda_1)=\frac{\kappaChHodge(\cA)}{24}
\]
...
$\Theta_\cA^{\leq 2}=\kappaChHodge(\cA)\cdot\eta\otimes\Lambda$ as in
...
surface.  The path-integral comparison is distinct from the three
structural identifications in the spine:
