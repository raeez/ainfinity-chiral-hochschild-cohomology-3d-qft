...
the all-loop PVA--quantum HT lifting problem: the classical
$\lambda$-Jacobi limit must lift to an $E_3$-structure on
$Q$-cohomology. Theorem statements that depend on the all-loop quantum
lift carry the four $\hyp$ tags; the genus-zero
$\lambda$-bracket and the modular MC obstruction recursion are
unconditional.
...
theory must already contain a primitive genus-raising one-edge operator. Construct a
...
\section{Assertion layers and scope}

The chapter separates three mathematical inputs.

\begin{convention}[Three assertion layers]
Throughout the chapter we distinguish three kinds of statements.
...
\item \textbf{Formal algebraic results.} These are theorems proved here once one
...
\item \textbf{KZ-locus comparison results.} These are statements linking the formal
algebra to the Poisson sigma model. On the finite-type freely generated
finite-jet locus with inner holomorphic translation, the classical BV
datum is constructed in Construction~\ref{cons:kz-bv-fields-action};
half-space perturbative quantization still requires the KZ analytic SDR
package of Assumption~\ref{assum:kz-analytic-sdr-package}.
\end{enumerate}
\end{convention}

\begin{remark}[Formal algebra and analytic extraction]
...
with the three-dimensional Poisson sigma model requires the analytic SDR package and the
resolved classical input datum named above.
\end{remark}
...
Throughout the chapter, concrete coderivation dg~Lie algebras such as
...
written in the strict modular complex is a computation in
...
\]
They also identify the full homotopy/Koszul treatment, parallel to the
associative story, as the homotopy/Koszul gap in quadratic chiral duality
\cite{GLZ22}. This gap motivates the modular bar construction below.
...
the modular refinement of disk-level chiral duality lives once the modular comparison data
are supplied.
...
that \emph{a single Maurer--Cartan object packages higher operations, anomalies,
...
results identify the correct receptacle for quantization of PVAs as not merely
...
\begin{proposition}[Necessity of a genus-raising primitive]
...
\begin{theorem}[Stable-graph modular bar construction]\label{thm:modular-bar}
...
graphs of total genus zero, i.e.\ trees all of whose vertices have genus zero. Denote
...
\begin{proposition}[The one-edge decomposition]\label{prop:D0D1}
...
therefore remembers the element $S$ geometrically.
...
\begin{proposition}[Convolution description]
...
\begin{construction}[Homotopy-chiral/log-FM coordinates]
...
\begin{computation}[Low-genus control terms]
...
\begin{construction}[PVA-coordinate residue formulas]
...
\begin{computation}[Archetype formulas in boundary coordinates]
...
\begin{proposition}[Quartic channels and the first loop term]
...
\begin{theorem}[Formal structure theorem]\label{thm:formal-structure}
Let $C$ be a modular bar datum. Then:
\begin{enumerate}[leftmargin=2.4em]
...
\begin{corollary}[No modular-Koszul theory without loops (T15 (iii))]
...
algebraic source of genuine modular-Koszul obstructions (T15 sense (iii)).
\end{corollary}
...
\begin{proposition}[Comparison with Gui--Li--Zeng]
...
\label{constr:vol2-bipartite-linfty}
...
\begin{theorem}[Universal genus-one obstruction]\label{thm:Ob1}
...
\begin{theorem}[Higher-genus recursion]\label{thm:Obg}
Suppose
\[
...
\begin{corollary}[Full modular lift criterion]
...
\begin{theorem}[KZ-locus genus-zero boundary quantization;
\ClaimStatusConditional{} on
$\hypKZSDR,\,\hypStokes,\,\hypReflWts,\,\hypTLift$]\label{thm:kz-locus-tree}
...
has inner holomorphic translation. Assume the four endpoint
hypotheses
$\hypKZSDR$ (Khan--Zeng analytic SDR package, recorded in
Assumption~\ref{assum:kz-analytic-sdr-package} \cite{KZ25}),
...
Let $\A=\widetilde A_{\cl}(V)$ satisfy the resolved classical datum
...
\begin{corollary}[KZ-locus modular quantization criterion;
\ClaimStatusConditional{} on
$\hypKZSDR,\,\hypStokes,\,\hypReflWts,\,\hypTLift$]
Under the hypotheses of \cref{thm:kz-locus-tree} (i.e., the four
endpoint hypotheses
$\hypKZSDR,\,\hypStokes,\,\hypReflWts,\,\hypTLift$), if all
obstruction classes vanish, then there exists
\[
...
the quantized topological homotopy $T=[Q_{\mathrm{tot}},G]$ (this is
$\hypTLift$), the lift can be chosen in $\Deftop(A_{\cl}(V))$.
\end{corollary}
...
obtained from $\Xloc^{\bullet+1}$. We pass between the two conventions,
...
\begin{theorem}[Coordinate formula for the genus-raising operator]\label{thm:D1formula}
...
\begin{lemma}[BV properties of $\Delta_{\mathrm{cyc}}$]\label{lem:BV}
...
\begin{theorem}[Variational modular class]\label{thm:modclass}
...
\begin{lemma}[Gauge-independence up to exact terms]\label{lem:gaugeind}
...
\begin{theorem}[Triangular vanishing]\label{thm:triangular-vanishing}
...
Theorem~\textup{\ref{thm:Obg}} vanish for all $g\ge 1$. Hence a full
modular Maurer--Cartan lift
\[
\Theta^{\mathrm{Vir}}
...
$\hbar$-expansion; all of its coefficients are determined by that
one-loop level shift.
...
$\hypKZSDR,\,\hypStokes,\,\hypReflWts,\,\hypTLift$
of Theorem~\ref{thm:central-charge-shift}: Khan--Zeng analytic SDR
\cite{KZ25}; Stokes-data choice on the Fulton--MacPherson compactification;
reflected weights under analytic continuation; chain-level lift of
$T = [Q_{\mathrm{tot}}, G]$ to the all-loop quantum object
\cite{Costello2011Renorm,CG17,LurieHA}.
...
translation invariance, filtration, and conformal behavior imposed by the one-loop boundary
...
obstructions, and higher-genus recursion.
...
\subsection{Physical interpretation of the remaining scalar}
...
results above explain why such a shift is the \emph{only available relevant
...
selected by the three-dimensional Poisson sigma model. The identification
$\kappa=50$ in the $W_3$ case requires the half-space one-loop determinant calculation
in addition to the formal obstruction theory, which determines only the
central-parameter line.

\section{Further mathematical problems}

The constructions and theorems above determine several precise problems.
...
$W_3$ from the half-space theory itself. Determining this scalar turns the rigidity
result of \cref{cor:unique-lift} into a one-loop quantization theorem.
\end{question}
...
type \cite{DNP25}. Construct a common modular formalism governing:
\begin{enumerate}[leftmargin=2.4em]
...
\end{enumerate}
The input-layer candidate is the stable-graph modular deformation complex.
\end{question}

\begin{remark}[Connection to modular state functionals]
\label{rem:modular-state-functional-connection}
...
A modular state functional is the additional datum
$\Psi_\cA$ satisfying the quantum master equation
$\widehat{\boldsymbol\Theta}_\cA\,\Psi_\cA=0$. Its semiclassical
expansion
$\Psi_\cA \sim \exp(\sum_{g} \hbar^{2g-2} F_g(\cA))$
recovers the genus tower when the quantization datum is fixed, with local
normal forms determined by the nonlinear shadow archetype (Gaussian for
Heisenberg, Airy for affine, Pearcey for~$\beta\gamma$, mixed catastrophe
for Virasoro).
...
\section{Modular descendant potentials and tautological comparison data}
...
spaces $H_{g,n}^{T;B}$. A tautological action and a descendant potential package these
operations into comparison data with intersection theory on $\ov{\M}_{g,n}$.
...
\subsection{The tautological boundary action}
\label{subsec:tautological-boundary-action}

\begin{definition}[Tautological boundary action]
\label{def:tautological-boundary-action}
The \emph{tautological boundary action} is the linear map
\begin{equation}
...
\begin{proposition}[Tautological relations act on the quotient]
\label{prop:tautological-boundary-relations}
Every tautological relation $\sum_i c_i \alpha_i = 0$ in $R^\bullet(\ov{\M}_{g,n})$
induces the boundary operator identity
\[
...
\begin{proof}
The map $\Theta_{g,n}^{T;B}$ has domain the tautological ring itself. A tautological
relation is the zero element of that quotient ring, and linearity sends the zero element
to zero in $\End(H_{g,n}^{T;B})$.
\end{proof}
...
\begin{definition}[Eynard--Orantin comparison datum]
\label{def:eo-comparison-datum}
An \emph{Eynard--Orantin comparison datum} for $(T,B)$ consists of:
\begin{enumerate}[leftmargin=2.4em]
\item a semisimple genus-zero polarization of the state spaces $H_{0,n}^{T;B}$;
\item a spectral curve or local recursion kernel attached to that polarization;
\item a quantization map sending tautological relations in $R^\bullet(\ov{\M}_{g,n})$
to differential operators on the Fock completion generated by the variables
$(t_a,s_k)$;
\item an identification of those differential operators with the loop equations of the
chosen Eynard--Orantin recursion kernel.
\end{enumerate}
\end{definition}

\begin{proposition}[Descendant constraints from a comparison datum]
\label{prop:descendant-constraints-from-comparison-datum}
Given an Eynard--Orantin comparison datum for $(T,B)$, the descendant potential
$F_{T;B}$ satisfies the corresponding loop-equation identities order by order in $\h$.
\end{proposition}

\begin{proof}
The quantization map in the comparison datum turns each tautological relation into a
differential operator on the Fock completion. The fourth item identifies these operators
with the loop equations of the chosen recursion kernel. Applying the operators to
$\exp(F_{T;B})$ gives the asserted order-by-order identities after taking the logarithm.
\end{proof}

\begin{remark}[Connection to modular state functionals]
\label{rem:descendant-state-functional}
The descendant potential $F_{T;B}$ exponentiates to a modular state functional
$\Psi_T := \exp(F_{T;B})$ valued in a Fock-type completion. Under an
Eynard--Orantin comparison datum, $\Psi_T$ is annihilated by the quantized tautological
relations, giving the comparison map from the algebraic modular bar construction to the
analytic theory of quantum curves and Eynard--Orantin recursion.
\end{remark}
...
\section{K3 Siegel-elliptic comparison}
\label{sec:mpvaq-supplement}

\begin{definition}[Finite Hall window for the K3 comparison]
\label{def:mpvaq-finite-hall-window}
A \emph{finite Hall window} $\mathcal W$ for the K3 comparison is a
finite saturated set of Mukai-charge labels for the $K3\times E$ Hall
correspondences, together with compact-support orientation data, positive
and opposite Hall halves, a Cartan line, a nondegenerate Hopf pairing on
the radical quotient, parity/root fixtures, PBW/no-extra-relations checks,
and an $H_1$-compatible restriction map.  It produces finite-window
objects
\[
\mathbf H_{\Delta_5}^{\mathcal W},
\qquad
\mathbf Q_{\hbar}^{\mathrm{K3},\mathcal W}.
\]
The completed Hall--Drinfeld double $\mathbf H_{\Delta_5}$ and any
partition-function recognition as $\Delta_5$ require a cofinal system of
such windows with Mittag--Leffler completion and Borcherds--Heegner
comparison.
\end{definition}

\begin{remark}[Modular PVA quantisation and Siegel-elliptic $R$-matrix]
\label{rem:mpvaq-1}
At a finite Hall window $\mathcal W$ satisfying
\cref{def:mpvaq-finite-hall-window}, modular PVA quantisation has a
Siegel-elliptic comparison chart with the dynamical $R$-matrix
$R_{\mathrm{Sieg,dyn}}(z_1, z_2;\tau, w)$ and the Pasol--Zagier
universal Igusa form.  The finite-window quantisation
$\mathbf Q_{\hbar}^{\mathrm{K3},\mathcal W}$ is a deformation chart for
the genus-$2$ Siegel PVA in the $(\tau,w)$-modular direction; it is not a
construction of the completed object $\mathbf H_{\Delta_5}$.  The
$\mathrm{Sp}_4(\mathbb Z)$ modular target is fixed only after the
Borcherds--Heegner recognition gate for the cofinal Hall-window system
\cite{PasolZagier2013,Felder1995}. Compare Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Quantisation depth and Humbert divisor]
\label{rem:mpvaq-2}
At a finite Hall window with $H_1$-compatible restriction, the modular
PVA quantisation depth is measured by the Humbert-divisor chart
$R_{\mathrm{Sieg,dyn}}|_{H_1}$.  Recognition of this chart as the
$\mathrm{Sym}^1$-component of the Saito--Kurokawa/Ikeda Arthur packet is
conditional on equality of the windowed Hecke traces with the Maass
relation
\[
\lambda_p(\Delta_{10}) = a_p(\Delta_{E_6}) + p^8 + p^9
\]
on the recognized Hall-window system.  The root-of-unity specialisation
uses the K3 conductor
\[
 \ell = K^{\kappaChHodge}
 =2c_+(\mathrm{Mukai}(K3))=8,
\]
not the Borcherds weight. The latter is
\[
 \operatorname{wt}(\Delta_5)
 =c_{\phi_{0,1}^{K3}}(0)/2=10/2=5,
\]
with $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$
\cite{Andrianov1974,Ikeda2001,Lusztig1990}. Compare
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}
...
\begin{remark}[Scalar Siegel weight of $\Delta_5$ at the K3 base point]
\label{rem:mpvaq-siegel-weight}
\index{Siegel modular form!weight $5$!K3 scalar comparator}
\index{mock modular!weight $107/12$ non-integrality}
The scalar Borcherds comparator at the K3 base point is the primitive
Gritsenko--Nikulin square root $\Delta_5$ on the genus-$2$ Siegel upper
half space $\mathbb H_2$ with the order-two multiplier on
$\mathrm{Sp}_4(\mathbb Z)$.  Its Siegel weight is
\[
w_{\mathrm{Sieg}} \;=\; 5
\]
...
\]
the unnormalised Igusa square of the Borcherds denominator
identity $\Phi_{10}^{\mathrm{un}}(Z) = \prod_{r\cdot\alpha > 0}(1-q^{r\alpha})^{c(r\alpha)}$
...
Chapter~\ref{V1-sec:thqg-open-closed-realization}, Proposition around
line~1890; Shapere--Tachikawa 2008 arXiv:0804.1957; BLLPRvR 2013
eq.~(3.14)) is a separate scalar in the external Virasoro line. It can
enter a Hall-window partition-function comparison only through the
...
\begin{proposition}[Scalar Siegel-vs-elliptic weight matching at $c_{2d} = -214$]
\label{prop:mpvaq-siegel-vs-elliptic}
\index{Siegel-vs-elliptic!weight matching}
\index{Humbert divisor!$H_1$ restriction}
For the scalar comparator $\Delta_5$, the Siegel weight
$w_{\mathrm{Sieg}} = 5$ and the apparent elliptic scalar
$w_{\mathrm{ell}} = -c_{2d}/24 = 107/12$ are separated by the
Gritsenko--Nikulin restriction to the Humbert divisor
$H_1 \subset \mathcal A_2$:
\[
w_{\mathrm{Sieg}}
...
base point does not admit a naive elliptic Borcherds lift. The
statement is a scalar automorphic weight computation, not a recognition
of a PVA partition function.
\end{proposition}
...
$\Delta_5$ with $w = 5$, $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$, and Gritsenko-lift
...
scalar comparator weight is supplied by the Siegel lift rather than by
the naive $-c/24$ formula.
\end{proof}

\begin{remark}[Conditional partition-function recognition]
\label{rem:mpvaq-conditional-partition-recognition}
\index{partition function!conditional K3 Hall-window recognition}
Let $\{\mathcal W_i\}$ be a cofinal system of finite Hall windows
satisfying the compact-support orientation, positive/opposite half,
Cartan, nondegenerate pairing, parity/root fixture, PBW/no-extra,
radical-quotient, $H_1$-restriction, and Mittag--Leffler hypotheses of
\cref{def:mpvaq-finite-hall-window}.  If the completed windowed
supertrace $Z_{\mathrm{Hall}}^{\mathrm{K3}}$ exists and its divisor,
character, first Fourier--Jacobi coefficient, and Borcherds--Heegner
multiplicities agree with the Gritsenko--Nikulin lift, then
\[
Z_{\mathrm{Hall}}^{\mathrm{K3}}\in \mathbb C^\times\Delta_5.
\]
The scalar normalization of this line is a separate one-loop determinant
or trace normalization.  Without these Hall-window recognition
hypotheses the chapter has only the scalar comparator
$\mathbb C\cdot\Delta_5$ and finite-window traces, not an identified
K3 partition function.
\end{remark}
...
The first modular obstruction in the local PVA model is the class of
$D_1P=\Delta_{\mathrm{cyc}}P$ on the stable-graph bar coalgebra. For
the triangular Virasoro and $W_3$ sectors treated in this chapter this
class vanishes. The K3 value $c_{2d}=-214$ instead enters as the
central-parameter scalar
\[
 \kappaChHodge(\Vir_{-214})=c_{2d}/2=-107.
\]
Thus $-107$ is the curvature/central-parameter coefficient in the
external Virasoro line, not a nonzero local PVA obstruction class in
the triangular sector. The integer reflects the prime factorisation
$-214=-2\cdot107$ and the first-principles class-$\mathcal S$ value
