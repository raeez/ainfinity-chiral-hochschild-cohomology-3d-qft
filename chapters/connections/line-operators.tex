...
\item Comodules over the bar construction $\overline{B}_{\mathrm{ch}}(\mathcal{A}_{\mathrm{bulk}})$, equivalently modules over the Koszul dual $\mathcal{A}^! := H^\bullet(\overline{B}_{\mathrm{ch}}(\mathcal{A}_{\mathrm{bulk}}))^\vee$ (the linear dual of bar cohomology, obtained via Verdier duality --- not the cobar $\Omega(\overline{B})$, which recovers~$\mathcal{A}$ itself by bar-cobar inversion).
\end{itemize}
The bar construction $\overline{B}_{\mathrm{ch}}$ converts the $A_\infty$ chiral algebra structure (with operations $m_k$ for $k \ge 2$) into a coalgebra, and $\mathcal{A}^!$ is its Verdier/linear dual algebra. The Quillen equivalence (Theorem~\ref{thm:homotopy-Koszul}) ensures this conversion preserves the homotopy type of the module category.
...
The abstract identification of line operators with $\mathcal{A}^!$-modules acquires
concrete computational content once one extracts the \emph{spectral Maurer--Cartan kernel}
...
\begin{remark}[From abstract identification to concrete kernel]
\label{rem:abstract-to-concrete-kernel}
The passage from the abstract line-operator identification
(Theorem~\ref{thm:lines_as_modules}) to the spectral kernel $r(z)$ is the essential
bridge between the categorical statement ``line operators are $\mathcal{A}^!$-modules''
and the computational statement ``line-operator OPE is governed by a universal $r$-matrix.''
The modular extension $R_T^{\mathrm{mod}}(z;\hbar)$ further upgrades this to a
genus-refined kernel whose higher coefficients $r_{T,g}$ encode the modular corrections
...
At $k=-2$ (critical level): $q=1$, the quantum group
degenerates to the classical universal enveloping algebra,
and the line-operator category becomes
...
Simple objects are $\mathcal W_3{}_{50-c}$-modules
