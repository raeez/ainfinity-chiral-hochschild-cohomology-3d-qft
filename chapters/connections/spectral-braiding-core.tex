...
product.  In the general (non-Koszul) case, the higher
...
The self-duality $(\SCchtop)^! \simeq \SCchtop$ is the
operadic shadow of the self-intersection structure
$\widetilde{\cN} \times_{\fg} \widetilde{\cN}$ of the
Springer resolution.  The convolution algebra and the
geometric restriction functors are defined by the same
correspondence viewed from two sides---and the
correspondence is symmetric.  Here, the bar complex
$\barB(\cA)$ on $\FM_k(\C) \times \Conf_k(\R)$ is a
two-sided correspondence whose convolution (differential)
and diagonal (coproduct) are both defined by the same
operadic structure; self-duality ensures that dualizing the
...
entanglement (both channels active) occurs for non-Koszul algebras:
the Lagrangian self-intersection has excess, higher
$\Ainf$ operations couple the curvature to the braiding, and each
genus level contributes genuinely new spectral data.  The dichotomy
is controlled by two independent invariants---the Lagrangian
intersection type (transverse vs.\ excess, i.e.\ Koszul vs.\
non-Koszul) and the Coisson bracket ($c_0 = 0$ vs.\ $c_0 \ne 0$)
---which refine the perturbative/non-perturbative divide
...
For a non-Koszul algebra, both channels produce corrections:
