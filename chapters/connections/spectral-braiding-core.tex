...
\begin{theorem}[Recognition: $\SCchtop$-algebras are dg-shifted Yangians;
\ClaimStatusProvedHere]
\label{thm:yangian-recognition}
\index{recognition theorem!Swiss-cheese = Yangian|textbf}
\index{dg-shifted Yangian!recognition|textbf}
Let\/ $\cA^!$ be the Koszul dual of a logarithmic
$\SCchtop$-algebra\/~$\cA$.  Then $\cA^!$ is a dg-shifted
Yangian in the sense of
Definition~\textup{\ref{def:dg_Yangian}}, and conversely,
every dg-shifted Yangian arises as the Koszul dual of a
logarithmic $\SCchtop$-algebra.  More precisely, the
functor $\cA \mapsto \cA^!$ restricts to an equivalence
\[
\bigl\{\text{chirally Koszul logarithmic } \SCchtop\text{-algebras}\bigr\}
\;\xrightarrow{\;\sim\;}
\bigl\{\text{dg-shifted Yangians}\bigr\}
\]
between the Koszul locus and the category of dg-shifted
Yangians.
\end{theorem}

\begin{proof}
The forward direction is
Theorem~\ref{thm:Koszul_dual_Yangian}: each datum of
Definition~\ref{def:dg_Yangian} is read off from the
$\SCchtop$-coalgebra structure of the bar
complex.  For the converse, given a dg-shifted Yangian
$(Y, \Delta_z, r(z), T, \varepsilon)$, the cobar
construction $\Omegach(Y)$ produces a logarithmic
$\SCchtop$-algebra whose Koszul dual recovers~$Y$, by the
bar-cobar Quillen equivalence
(Theorem~\ref{thm:bar-cobar-adjunction},
unconditional via homotopy-Koszulity,
...
Etingof--Kazhdan formality machinery---which is itself
...
\item \emph{Closed color:} the coalgebra structure on
  ...
  the Sklyanin $\lambda$-bracket on $\cA^!$
...
(iv): Complementarity $\kappa(\cA) + \kappa(\cA^!) = 0$
(for Kac--Moody and free-field algebras) is
