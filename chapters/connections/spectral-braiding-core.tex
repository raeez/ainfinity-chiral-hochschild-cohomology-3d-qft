...
are the $r$-matrix manifestation of the non-formality
$m_k \ne 0$ for all $k \ge 3$: the $\Ainf$ tower prevents
the $r$-matrix from satisfying the scalar constraints of
quantum group theory.  By contrast, every class~L algebra
(simple poles, formal $\SCchtop$ structure) has a scalar
unitarity product by the quadratic Casimir identity.
The pole-order dichotomy (class~L vs class~M) is thus
visible in the unitarity structure of the $r$-matrix:
simple poles give scalar unitarity; higher poles give
operator-valued failure.
\end{remark}
...
\end{equation}
This is precisely the KZ connection form $A_2$ of
...
pole order (AP19): the Casimir $\Omega/z$ becomes
$\Omega\,d\log(z)$, which has a logarithmic (not polar)
singularity.
\end{computation}
...
rational $r$-matrix $\Omega/u$, this relation reduces to
...
\item \emph{Koszul self-dual point ($c = 13$).}\enspace
  ...
  S-duality: the self-dual Virasoro algebra is its own
...
Indeed, the spectral-parameter composition law is the
...
\begin{align*}
  \zeta(z|\tau)
  &= \frac{1}{z}
  - \sum_{k \ge 1} G_{2k}(\tau)\, z^{2k-1},
  \\
  \wp(z|\tau)
  &= \frac{1}{z^2}
  + \sum_{k \ge 1} (2k+1)\, G_{2k+2}(\tau)\, z^{2k},
\end{align*}
where $G_{2k}(\tau) = \sum_{(m,n) \ne (0,0)}
(m\tau + n)^{-2k}$.  The zeta expansion has only odd
$z$-powers; the Weierstrass expansion has only even
$z$-powers.  The modular properties follow: $G_{2k}$ for
$k \ge 2$ is a modular form of weight~$2k$; $G_2$ is
...
for the three standard families, making the
entanglement/decoupling dichotomy explicit.
...
\bottomrule
\end{tabular}
\caption{Genus-$1$ spectral data for the standard families.
The Heisenberg has $c_0 = 0$ and disentangles; all others
have $c_0 \ne 0$ and entangle.  The leading $\tau$-dependent
correction is quasi-modular (involving $G_2$) precisely when
entanglement occurs.  For the Heisenberg, the leading
correction is the modular $G_4$ at order $z^2$, reflecting
the absence of Sector~I.}
...
\label{cor:genus-g-curvature-braiding}%
...
\frac{1}{(z-w)^{n+1}}
\;\leadsto\;
\frac{(-1)^{n-1}}{(n-1)!}\, \partial_z^{n-1} B(z,w),
\]
...
$\Phi_n(z,w) = \tfrac{(-1)^{n-1}}{(n-1)!}\, \partial_z^{n-1} B(z,w)$
