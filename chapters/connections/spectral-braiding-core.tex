...
$r(-z) = -r_{21}(z)$ \emph{manifest}: all surviving poles are
odd-powered.  For the Laplace kernel, antisymmetry holds but is
...
The underlying reason is that the spectral-parameter
composition law is the additive formal group
$\hat{\mathbb{G}}_a$, whose endomorphism ring over~$\C$
is $\End(\hat{\mathbb{G}}_a) = \C$ --- every constant,
including the Wick rotation $[i]$, is an endomorphism.
The Wick rotation is therefore an archimedean,
characteristic-$0$ phenomenon: it uses the continuous
topology of~$\C$ to rotate a contour, and this operation
has no direct $p$-local analogue.  (A $p$-adic
square root of $-1$ exists in~$\Z_p$ for
$p \equiv 1\!\pmod{4}$, but it does not provide
the analytic continuation between Lorentzian and
Euclidean signatures that the Wick rotation requires.)
...
The key observation is that the quasi-periodic phase
