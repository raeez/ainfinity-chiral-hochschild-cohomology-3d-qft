...
Recall that the spectral $R$-matrix $R(z)$ is defined as the path-ordered exponential (holonomy) of the spectral connection built from the classical $r$-matrix: $R(z) := \mathcal{P}\!\exp\!\bigl(\int_\gamma r(w)\,dw\bigr)$, where $r(z) \in \cA^! \widehat{\otimes}\, \cA^!((z^{-1}))$ encodes the leading-order OPE singularity between line operators (Definition~\ref{def:spectral_R}; see also Remark~\ref{rem:r-vs-R-relation} for the relation between $r(z)$ and $R(z)$ in the Koszul and non-formal cases).
...
The compactified moduli space $\FM_3(\C)$ has exactly four codimension-$1$ boundary divisors: $D_{\{1,2\}}$, $D_{\{2,3\}}$, $D_{\{1,3\}}$ (pairwise collisions) and $D_{\{1,2,3\}}$ (total collision).  This is standard; see e.g.\ Lambrechts--Voli\'c~\cite{LV14}, \S4, or Sinha~\cite{Sinha2004}, \S2.  By the FM boundary structure (Definition~\ref{def:boundary_divisors}), the codimension-1 faces of $\partial\FM_3(\C)$ are these collision divisors:
\[
...
Each divisor $D_{\{i,j\}} \cong \FM_2(\C) \times \FM_2(\C)$ parametrizes configurations where $z_i$ and $z_j$ collide while the third point stays separated.  The restriction of the $3$-point weight form $\omega_3^{\partial}$ to $D_{\{i,j\}}$ factorizes as $\omega_2^{\mathrm{inner}}(a_i, a_j) \wedge \omega_2^{\mathrm{outer}}(\ldots)$ by the cluster factorization theorem (Theorem~\ref{thm:cluster_factorization_homotopy}; complete computation in Appendix~\ref{app:FM_Stokes}, Proposition~\ref{prop:k3_identity}).  The inner factor $\omega_2^{\mathrm{inner}}$ computes the pairwise braiding $R_{ij}(z_{ij})$; the outer factor $\omega_2^{\mathrm{outer}}$ computes the braiding of the cluster center with the remaining point.  Explicitly:
...
The classical YBE (a three-term identity) is the infinitesimal form of the flatness condition $\nabla^2 = 0$ for the KZ connection on $\Conf_3(\C)$.  To obtain the multiplicative quantum YBE (a two-sided equation), we pass from the infinitesimal to the global via the monodromy representation of the fundamental group, as follows.
...
strands.  The generators $A_{ij}$ of $PB_3$ (the standard
Birman generators, corresponding to a full twist of strands $i$ and $j$)
satisfy the relation $A_{12}A_{13}A_{23} = A_{23}A_{13}A_{12}$
among others (cf.~Birman \cite{Birman74}, \S1.8).  The monodromy
representation $\rho\colon PB_3 \to
\mathrm{Aut}(L_1 \otimes L_2 \otimes L_3)$ sends each generator
$A_{ij}$ to $R_{ij}(z_{ij})$, and the braid relation yields the
...
\label{rem:YBE-scope}
The Yang--Baxter equation is proved unconditionally for any logarithmic $\SCchtop$-algebra: it follows from Stokes' theorem on $\FM_3(\C)$ and the flatness of the KZ connection.  When combined with Theorem~\ref{thm:lines_as_modules} ($\cC_{\mathrm{line}} \simeq \cA^!\text{-}\mathbf{mod}$) and Theorem~\ref{thm:bulk_hochschild} (bulk $\simeq$ chiral Hochschild), the spectral braiding participates in the full corrected bulk--boundary--line triangle.  That global triangle is proved rigorously only in the boundary-linear exact sector (Theorem~\ref{thm:boundary-linear-bulk-boundary}); for general HT theories it remains conjectural.  See Section~\ref{sec:ht-bulk-boundary-line} for the precise scope table.
\end{remark}
...
self-intersection.  The \emph{collapse correspondence} (not a
self-intersection span, since the source and target configuration
spaces differ) is the span
\[
\Conf_2(\C) \xleftarrow{\ (\mathrm{pr}_1, \mathrm{pr}_2)\ }
\Conf_2(\C) \times \Conf_1(\C)
\xrightarrow{\ \mathrm{merge}\ } \Conf_1(\C),
\]
where $\mathrm{pr}_1, \mathrm{pr}_2$ project onto the two factors of the configuration and $\mathrm{merge}$ records the cluster center.  This is the merge correspondence that collapses a two-point configuration to its center; the true self-intersection correspondence $\mathfrak{S}_b \to \cL \times \cL$ (both feet equal to $\cL$, middle $\cL \times_{\cM}^h \cL$) underlies the convolution algebra structure.  The spectral $R$-matrix $R(z)$ is the pushforward of the weight form $\omega_2^{\partial}$ along the merge map: $R(z) = (\mathrm{merge})_\ast(\omega_2^{\partial})$, computed as the holonomy of the spectral connection on $\mathfrak{S}_b$ along the fiber of this correspondence.
...
\item Line operators $L_i$ are modules for the convolution algebra
  ...
  Here $H_\ast^{\mathrm{BM}}(\mathfrak{S}_b)$ denotes the
  Borel--Moore homology of the underlying classical truncation of
  the derived self-intersection; the full derived convolution
  algebra \textup{(}using the dg enhancement\textup{)} is the
  Swiss-cheese convolution $L_\infty$-algebra
  $\mathfrak{g}^{\mathrm{SC}}_T$ of
...
both taken on the classical truncation as in~(i) above,
...
The closed-color operad~$E_2$ is Koszul
(Ginzburg--Kapranov~\cite{GK94}).
More precisely, $E_2^! \simeq E_2\{-2\}$ (with an operadic
desuspension shift); the self-duality in the $E_2$ case uses
Kontsevich formality \cite{Kon03} to promote the Koszulity of
\cite{GK94} to a quasi-isomorphism of the BV resolution.
...
self-orthogonal under the operadic pairing: the
relation subspace $R \subset V^{\otimes 2}$ (where
$V = V_{\mathsf{ch}} \oplus V_{\mathsf{top}}$ is the
space of binary generators) satisfies $R^\perp = R$ under
the pairing induced by the operadic bimodule structure.
Since the component operads are self-dual and the
distributive law dualizes to itself, the full Swiss-cheese
operad is self-dual: $\mathsf{SC}^! \simeq \mathsf{SC}$.
...
law through formality.  The transfer is justified by the observation
that formality of~$E_2$ (Kontsevich) provides a zigzag of operad
quasi-isomorphisms that preserves the distributive law structure,
since the mixed operations are determined by the closed-color
operations via the Leibniz rule~\eqref{eq:leibniz-self-dual}, which
is preserved by any operad map (the Leibniz identity is a consequence
of the operadic composition, not additional data).
\end{proof}
...
\noindent\textbf{Structural verifications.}
We verify that the constructed data form a consistent Yangian:

\emph{Coproduct coassociativity.}
The twisted coproduct $\Delta_z$ is coassociative because
the $E_1$-algebra structure on $\mathcal{Y}$ is strictly
associative: $(\Delta_z \otimes \id) \circ \Delta_{z+w}
= (\id \otimes \Delta_w) \circ \Delta_z$ is the operadic
associativity identity for $E_1(3)$, transported through
the bar-cobar duality.  The $z$-dependent twist by $r(z)$
preserves coassociativity because $r(z)$ satisfies the
quantum $\Ainf$ Yang--Baxter equation (axiom (d) above),
which is precisely the obstruction to
$(\Delta_z \otimes \id) \circ \Delta_{z+w}
\neq (\id \otimes \Delta_w) \circ \Delta_z$.

\emph{$R$-matrix compatibility with coproduct.}
The spectral $R$-matrix $r(z)$ intertwines the coproduct
and the opposite coproduct:
$r(z)\, \Delta_z(a) = \Delta_z^{\mathrm{op}}(a)\, r(z)$
for all $a \in \mathcal{Y}$.  This is the operadic identity
at profile $(\mathsf{ch},\mathsf{ch};\mathsf{top})$ for
$\FM_2(\C) \times E_1(2)$: the $\Z/2$-equivariance of the
closed-color operation (swapping $z_1, z_2$) intertwines
with the swap of factors in $\mathcal{Y} \otimes \mathcal{Y}$.

\emph{Spectral parameter dependence.}
All structure maps depend on spectral parameters through
the relative coordinates $z_{ij} = z_i - z_j$ on
$\FM_k(\C)$, never on absolute positions.  This is
guaranteed by the $\C$-equivariance of the closed-color
structure (axiom (b) above): translation
$z_i \mapsto z_i + c$ acts trivially on all operations.
...
The no-open-to-closed constraint is automatically satisfied when
both colors coincide: the mixed operations
$M_{k,m}$ for $k > 0$, $m > 0$ are defined as maps
$\mathcal{Y}_{\mathsf{top}}^{\otimes m} \otimes
\mathcal{Y}_{\mathsf{ch}}^{\otimes k} \to \mathcal{Y}_{\mathsf{ch}}$,
and when $\mathcal{Y}_{\mathsf{top}} = \mathcal{Y}_{\mathsf{ch}}
= \mathcal{Y}$ this reduces to maps
$\mathcal{Y}^{\otimes(m+k)} \to \mathcal{Y}$, which are
automatically well-defined without requiring an
open-to-closed map.
\end{proof}
...
Our convention is $R(u) = 1 - \hbar P/u$ (the Molev convention
\cite[Theorem~1.2.2]{Mol07}), so that the FRT relation reads
...
For $\cA = \widehat{\mathfrak{u}(1)}_k$, the line-operator Koszul dual is
$\cA^!_{\mathrm{line}} = \widehat{\mathfrak{u}(1)}_{-k}$
(Vol~I, Theorem~B; the chiral Koszul dual is $\cA^!_{\mathrm{ch}} = \mathrm{Sym}^{\mathrm{ch}}(V^*)$, the chiral symmetric algebra).  There is a single FRT generator $t(u)$,
