...
\item Under Remark~\ref{rem:tholog-hmkd-comparison}, the strict
 ...
 A_\partial=\cA=\cH_k,
 \qquad
 \Theta_{\cH}(z)=\frac{k\,\Omega_{\cH}}{z}
 \quad(\text{rank-one coefficient }k/z),
 \]
...
$\mathcal A^!$ &
 ...
 $\mathcal B^{-k-6}$ &
 $\mathcal W_3^{-k-6}$ \\[2pt]
$K^c=c+c'$ & $16$ & $50$ & $100$ \\[2pt]
BRST ghost sector & none & minimal & principal \\[2pt]
central midpoint & affine critical & $25$ & $50$ \\[2pt]
...
normalization, defined by this transformation law as the curvature
deficit of the reduction; its value at the minimal
($f_{\min}$, Bershadsky--Polyakov) slot is open. The deficit is
distinct from the level-independent ghost \emph{central charge}
entering the conductor cross-check $K^c = c + c' = 50$ at the
minimal vertex: the spin-$(2, \tfrac32, \tfrac32, 1)$ $bc$ system
contributes $-26 - 11 - 11 - 2 = -50 = -K^c$, independently
of~$k$.
...
$J_\infty S_{f_{i+1}}$. In particular, a level-dependence of
$\kappa_{\mathrm{ghost}}$ at the non-principal step (level~$1$)
--- whose value is the open Bershadsky--Polyakov
$\kappa^{\mathrm{Hodge}}$ slot --- would reflect the fact that the
minimal Slodowy slice is \emph{not} a
...
records $c_{\mathrm{sum}}(3,1) = 50$, using the central-charge
formula
\[
c(\mathcal{B}^k)
...
with \(c(\mathcal{B}^k)=25-6(k+3)-24/(k+3)\):
\[
...
$c+c'=50$, the shifted central charge \(c-25\) is odd under
\(k\mapsto -k-6\).  The fixed level is \(k=-3\), the critical
level; the midpoint fibre \(c=25\) consists of the complex levels
\(k=-3\pm 2i\), so no real regular level realizes it.
The scalar \(\kappa+\kappa^!\) for the BP pair is open: no
derivation of \(\kappaChHodge(\mathcal B^k)\) exists, and Vol~I
records the slot as open.
The explicit formula for $\kappaChHodge(\mathcal B^k)$ is an open
bar-complex computation: it would receive contributions from both
the $\widehat{\mathfrak{sl}}_2$ sector (double poles) and the
...
c(\mathcal B^k)+c(\mathcal B^{-k-6})=50;
\]
the companion $\kappa$-sum
$\kappaChHodge(\mathcal B^k)+\kappaChHodge(\mathcal B^{-k-6})$ is
open.
...
By Definition~\ref{def:tholog-ore-transgression-contraction}, the
transgression algebra is
...
\end{equation}
where \(\iota_\Theta\) is the anomaly contraction of
Definition~\ref{def:tholog-ore-transgression-contraction}. When the
chosen representative of \(\Theta\) is central and the contraction
vanishes, this specializes to the strict central-shadow relation
\(\eta b=(-1)^{|b|}b\eta\). The transgression algebra
\(B_\Theta\) is the universal neutralization of the genus-\(1\) class
on the affine bar model. Through the Koszul comparison
\(V_k(\mathfrak{sl}_2)^!\simeq
...
Koszul dual & $\operatorname{CE}^{\mathrm{ch}}(\widehat{\mathfrak{sl}}_2{}_{-k-4})$ \\
...
Transgression algebra &
 $B_\Theta = B\langle\eta\rangle/
 (\eta b-(-1)^{|b|}b\eta-\iota_\Theta(b))$, $d\eta=\Theta$ \\
...
Koszul dual & $\operatorname{CE}^{\mathrm{ch}}(\widehat{\mathfrak{sl}}_2{}_{-k-4})$ & $\mathrm{Vir}_{26-c(k)}$ \\[2pt]
...
For anomaly completion, the Mukai-doubling Koszul conductor $K^{\kappaChHodge}(\mathcal{H}_{\mathrm{Muk}}(K3)) = 2\,c_+(\mathrm{Mukai}(K3)) = 8$ identifies (via Bruinier $2002$ Proposition~$5.1$ Heegner Chern-class reciprocity) with: (a) order of monodromy of $\mathcal{L}^{\Delta_5}$ around Humbert divisor $H_1 \subset \overline{\mathcal{A}_2}$; (b) Lusztig specialisation $\ell = 8$ at $\zeta^8 = 1$, $\hbar^2 = -1/8$. Universal identity $\hbar^2 \cdot K^{\kappaChHodge} = -1$ on the Lorentzian-lattice-parametric $\mathcal{B}$-family. The conductor $8$ sits beside the Theorem~C complementarity spectrum $\{0, 13, 250/3\}$ (BP slot open) as a distinct sixth invariant, not a complementarity value: the $\mathcal{B}$-row Hodge sum itself is $\rho_K(\mathrm{K3}\times E) = 0$.
\end{remark}
