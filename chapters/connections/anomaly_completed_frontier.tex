...
of the anomaly-completed machine.  We now pass to the first non-abelian
...
\item $\kappa(V_k) + \kappa(V_{-k-4}) = 3(k+2)/4 + 3(-k-2)/4 = 0$
  ...
  Kac--Moody algebras (AP24 applies: this anti-symmetry is special
  to KM, not universal);
\item $\kappa\neq 0$ for all $k\neq -2$.
\end{enumerate}
...
the genus-$1$ tautological class (AP21: $\Theta$ is a \emph{class},
...
\item \emph{Genus-$0$:} the uncompleted convolution algebra on $\Sb$
  ...
  $V_{-k-4}(\mathfrak{sl}_2)$ (AP-OC: the bar complex classifies
...
framework applies with non-trivial curvature---though the
algebraic content is simple, the genus-$1$ modular characteristic
is non-vanishing.
