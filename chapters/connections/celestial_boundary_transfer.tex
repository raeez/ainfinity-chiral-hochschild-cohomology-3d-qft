...
\section{Input theorems and transfer geometry}

The construction rests on three input theorems.
...
The transfer package below uses these inputs as hypotheses and develops
the resulting filtered homotopy algebra: wedge quotients, nonlinear
obstruction classes, highest-weight ladder formulas, simplex
coefficients, and the Airy--Witt normal form.
...
\begin{equation}\label{eq:cbt-suspended-transfer-split}
 m_n
 =
 s_H^{-1}
...
Koszul signs forced by the suspension operators in the displayed
formula and the orientation line \(\mathfrak o(T)\).
...
\begin{proof}
By the suspended Kadeishvili formula above, each transferred operation
\(m_n\) is the signed sum over planar rooted trees with \(n\) leaves
whose vertices, internal edges, leaves, and root are decorated
respectively by \(\mu_r,h,i,p\). The normalized side conditions make
this the minimal homotopy transfer of the dg algebra
\((C,\mu_1,\mu_2,\ldots)\).
...
\begin{remark}[Witt-sector closure]
The odd positive Witt modes form a representation-theoretic sector of
the full transferred $\Linf$ deformation. Their Witt brackets land in
the even positive sector, so Lie closure of the transferred ladder
requires the even highest-weight components of $H_b^{0,0}(S^3)$
together with the odd sector computed above.
\end{remark}
...
The previous sections produced exact results in holomorphic BF and
Poisson BF. The same obstruction formalism applies to the $W$-type
PVA world of Khan--Zeng (2025) once the filtered tree-level boundary
transfer is fixed.
...
\begin{remark}
This is the algebraic source of the first higher-spin celestial
correction: after the tree-level filtered transfer, the linear
conformal algebra and the first quadratic nonlinear term determine the
first obstruction class.
\end{remark}

\begin{remark}[beyond $W_3$]
Khan--Zeng emphasize that their construction extends naturally to
other $W_n$ and $W_\infty$ algebras. For every such algebra equipped
with a tree-level filtered transfer, the theorem above produces a
hierarchy of first nonlinear higher-spin celestial invariants from the
lowest nonlinear homogeneous terms of the $\lambda$-brackets.
\end{remark}
...
\section{Transfer problems}

The exact transfer package leaves the following concrete constructions.

\begin{enumerate}[leftmargin=2.2em]
\item \textbf{Even-mode closure for the Poisson ladder.}
The Airy--Witt theorem computes the odd positive modes. The full
transferred $\Linf$ algebra on
$T^*[1]\operatorname{Ham}(\mathbb{C}^2)$ requires the even positive
modes and their brackets with the odd ladder.
...
The abstract first-obstruction theorem is already in place. The
remaining computation is an explicit contraction for the $W_3$ PVA,
followed by $\operatorname{Ob}_1$ and the first ladder sector.

\item \textbf{Homotopy invariance of the higher ladder sectors.}
The all-degree formulas were proved for the canonical transferred
model. The invariant content is measured by projecting each ladder
cochain to the filtered obstruction quotient
$H^1(\mathfrak g,\delta_0)^{(r)}$.

\item \textbf{Quantum corrections.}
The formulas above are tree-level. Loop corrections enter through the
quantum deformation of the boundary PVA and through the induced
corrections to the filtered coderivation $b=b_0+b_1+\cdots$.
\end{enumerate}
