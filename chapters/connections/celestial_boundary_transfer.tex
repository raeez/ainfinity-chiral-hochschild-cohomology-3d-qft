...
% label removed: ch:celestial-boundary-transfer

The Kaluza--Klein reduction of six-dimensional holomorphic theories to
three-dimensional holomorphic--topological theories, together with the
appearance of boundary chiral algebras in twisted and celestial
holography, produces a concrete algebraic package: transferred
$\Ainf$ and $\Linf$ structures on the tangential Cauchy--Riemann
cohomology $H_b^{0,\bullet}(S^3)$ and its natural Lie-theoretic
avatars.  The source papers of Zeng, Costello--Dimofte--Gaiotto, and
Khan--Zeng supply the structural input: boundary chiral algebras,
special deformation retracts, rooted-tree transfer formulas, the
identification of the binary Poisson bracket with
$T^*[1]\operatorname{Ham}(\mathbb{C}^2)$, and Poisson vertex algebra models of
$W$-type and higher-spin flavor.

This chapter consolidates the mathematics developed across the
celestial and twisted holography thread into one coherent account and
adds a further representation-theoretic theorem.  The core results
are:
...
\item closed all-arity highest-weight ladder formulas for holomorphic
  BF and for the induced higher Kac--Moody minimal model;
\item closed all-arity odd highest-weight ladder formulas and
...
% label removed: sec:cbt-orientation
...
The present chapter does \emph{not} attempt to re-prove these source facts. Instead it distills the algebra they suggest, pushes it in several new directions, and presents the resulting package in a form suitable for direct inclusion in a mathematical monograph.

\begin{convention}[provenance]
% label removed: def:cbt-provenance
...
% label removed: thm:cbt-single-particle-transfer
...
% label removed: def:cbt-wedge-ainfty
...
% label removed: cor:cbt-single-particle-rigidity
\ClaimStatusProvedHere
...
In the holographic language, the usual wedge or global symmetry Lie algebra is the strict shadow of $\mathbb{W}(C)$. The point is not merely that one may discard composites; rather, the quotient by composites is an intrinsic functorial object.
\end{remark}

\subsection{Nonlinear normal forms and obstruction classes}
% label removed: sec:cbt-nonlinear-normal

We now pass from the single-particle quotient to the first irreducibly nonlinear datum beyond it.
...
% label removed: def:cbt-r-normal-form
...
\begin{lemma}
% label removed: lem:cbt-br-closed
\ClaimStatusProvedHere
...
\begin{lemma}
% label removed: lem:cbt-gauge-change-order-r
\ClaimStatusProvedHere
...
% label removed: def:cbt-obstruction-classes
...
% label removed: thm:cbt-recursive-linearization
\ClaimStatusProvedHere
...
% label removed: cor:cbt-relative-formality
\ClaimStatusProvedHere
...
% label removed: sec:cbt-composite-spectral

The filtration on $(B(H),b)$ yields a spectral sequence.

\begin{proposition}
% label removed: prop:cbt-E1-page
\ClaimStatusProvedHere
...
% label removed: thm:cbt-spectral-obstruction
\ClaimStatusProvedHere
...
\begin{corollary}
% label removed: cor:cbt-spectral-noncollapse
\ClaimStatusProvedHere
...
% label removed: sec:cbt-lowest-nonlinearity
...
% label removed: thm:cbt-lowest-nonlinearity
\ClaimStatusProvedHere
...
% label removed: cor:cbt-quadratic-control
\ClaimStatusProvedHere
...
% label removed: sec:cbt-holomorphic-bf-obstruction
...
% label removed: def:cbt-unit-projected-cubic
...
% label removed: prop:cbt-cubic-coeffs
\ClaimStatusProvedHere
...
% label removed: thm:cbt-unit-projection-gauge-invariant
\ClaimStatusProvedHere
...
% label removed: thm:cbt-explicit-ob1
\ClaimStatusProvedHere
For every $r\ge0$ and $s\ge1$,
\[
...
% label removed: cor:cbt-lowest-mode
\ClaimStatusProvedHere
At $(r,s)=(0,1)$,
\[
...
% label removed: cor:cbt-non-strictifiability
\ClaimStatusProvedHere
...
% label removed: cor:cbt-asymmetry-law
\ClaimStatusProvedHere
...
% label removed: sec:cbt-bf-ladders

Beyond the first obstruction lies an all-arity exact sector.

\subsection{The ladder sector}
% label removed: sec:cbt-ladder-sector
...
% label removed: lem:cbt-ladder-closure-bf
\ClaimStatusProvedHere
For all $a,b,c\ge0$,
\[
...
\subsection{The ordered all-arity formula}
% label removed: sec:cbt-ordered-all-arity
...
% label removed: thm:cbt-bf-ladder
\ClaimStatusProvedHere
...
\begin{corollary}[all arities are nonzero]
% label removed: cor:cbt-all-arities-nonzero
\ClaimStatusProvedHere
For every $n\ge3$,
\[
...
\]
So the canonical transferred $\Ainf$-model is nontrivial in every arity.
\end{corollary}

\subsection{Simplex geometry}
% label removed: sec:cbt-simplex-geometry
...
% label removed: prop:cbt-simplex-integral
\ClaimStatusProvedHere
One has
\[
C_n(b_1,\dots,b_{n-1})
...
% label removed: thm:cbt-simplex-sum-bf
\ClaimStatusProvedHere
...
% label removed: sec:cbt-higher-km
...
% label removed: thm:cbt-km-ladder
\ClaimStatusProvedHere
For every $n\ge3$,
\[
\begin{aligned}
...
\begin{remark}
% label removed: def:cbt-km-remark
This gives an explicit all-arity family of higher brackets in a minimal model of the four-dimensional higher Kac--Moody algebra. The coefficients are not merely existential; they are closed rational functions determined by simplex geometry.
\end{remark}

\section{Poisson BF: odd highest-weight ladders and all-order $\pi_n$ coefficients}
% label removed: sec:cbt-poisson-bf

We now pass to the transferred $\Linf$-operations
\[
...
% label removed: sec:cbt-raw-hw-bracket
...
% label removed: lem:cbt-raw-poisson
\ClaimStatusProvedHere
...
% label removed: lem:cbt-propagator-poisson
\ClaimStatusProvedHere
...
% label removed: sec:cbt-ordered-comb
...
% label removed: prop:cbt-ordered-comb
\ClaimStatusProvedHere
...
% label removed: sec:cbt-shifted-simplex
...
% label removed: thm:cbt-shifted-simplex
\ClaimStatusProvedHere
...
% label removed: sec:cbt-poisson-ladder

\begin{theorem}[odd-degree highest-weight ladder]
% label removed: thm:cbt-poisson-ladder
\ClaimStatusProvedHere
...
\]
where $B=b_1+\cdots+b_{n-1}$. Moreover,
\[
...
% label removed: cor:cbt-pi-n-formula
\ClaimStatusProvedHere
On the sector
\[
(r,s)=(2n-3,0),
...
% label removed: cor:cbt-poisson-examples
\ClaimStatusProvedHere
\[
...
\begin{remark}
% label removed: def:cbt-slope-comparison
...
% label removed: sec:cbt-airy-witt
...
\]
The odd highest-weight ladder computed in Theorem~\ref{thm:cbt-poisson-ladder} is therefore a concrete sector of the transferred $\Linf$ deformation of this cotangent Lie algebra. We now convert that sector into a one-dimensional differential-operator model.

\subsection{A classification lemma for homogeneous first-order operators}
% label removed: sec:cbt-classification-lemma

\begin{proposition}[homogeneous first-order operators on $\mathbb{C}\lbrack t\rbrack$]
% label removed: prop:cbt-first-order-homogeneous
\ClaimStatusProvedHere
...
% label removed: sec:cbt-renormalized-odd
...
% label removed: thm:cbt-airy-witt
\ClaimStatusProvedHere
...
% label removed: cor:cbt-faithfulness
\ClaimStatusProvedHere
The assignment
\[
\xi_m\longmapsto D_m
...
% label removed: sec:cbt-density-line
...
% label removed: cor:cbt-witt-density
\ClaimStatusProvedHere
Define
\[
...
% label removed: def:cbt-witt-caveat
...
% label removed: def:cbt-airy-witt-name
The formula
\[
D_m=-\partial_t\circ M_{t^{2m+2}}
\]
...
% label removed: sec:cbt-w-type

The previous sections produced exact results in holomorphic BF and Poisson BF. The natural next step in the archive is the $W$-type PVA world of Khan--Zeng (2025).

\subsection{The abstract $W$-type principle}
% label removed: sec:cbt-abstract-w-type
...
% label removed: cor:cbt-w-type-quadratic
...
% label removed: sec:cbt-w3-example
...
\]
The key point for us is that the lowest nonlinear part is quadratic in $T$.

\begin{corollary}[first higher-spin obstruction for the classical \texorpdfstring{$W_3$}{W3} PVA]
% label removed: cor:cbt-w3-obstruction
\ClaimStatusConditional
...
\begin{remark}
% label removed: def:cbt-universal-first-correction
This is the precise algebraic reason one expects a \emph{universal first higher-spin celestial correction}: once the tree-level filtered transfer exists and the linear conformal algebra together with the first quadratic nonlinear term are fixed, the first obstruction is fixed.
\end{remark}

\begin{remark}[beyond $W_3$]
% label removed: def:cbt-beyond-w3
Khan--Zeng emphasize that their construction extends naturally to other $W_n$ and $W_\infty$ algebras. Conditional on the tree-level filtered transfer, the abstract theorem above therefore predicts a hierarchy of first nonlinear higher-spin celestial invariants before explicit transfer formulas are worked out.
\end{remark}

\section{Synthesis: the platonic picture}
% label removed: sec:cbt-synthesis
...
The situation can be read as follows.
...
That is the deepest simplification produced in this thread.

\section{Concrete open problems that remain after the present chapter}
% label removed: sec:cbt-open-problems

The purpose of this section is not to retreat, but to make the frontier sharper.
...
% label removed: sec:cbt-notation-concordance
...
% label removed: sec:cbt-inventory
