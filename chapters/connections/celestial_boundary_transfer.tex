...
This is the precise algebraic reason one expects a \emph{universal first higher-spin celestial correction}: once the linear conformal algebra and the first quadratic nonlinear term are fixed, the first obstruction is fixed.
\end{remark}

\begin{remark}[beyond $W_3$]
% label removed: def:cbt-beyond-w3
Khan--Zeng emphasize that their construction extends naturally to other $W_n$ and $W_\infty$ algebras. The abstract theorem above therefore already predicts a hierarchy of first nonlinear higher-spin celestial invariants, even before explicit transfer formulas are worked out.
\end{remark}
