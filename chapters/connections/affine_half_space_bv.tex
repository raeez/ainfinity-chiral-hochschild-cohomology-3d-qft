...
construction (Theorem~\ref{thm:doubling-rwi}), which embeds the half-space
...
\section{Verification of the logarithmic $\SCchtop$ hypotheses}
\label{sec:verification-SC-hypotheses}
...
standard examples satisfy the hypotheses of the physics bridge theorem
...
$X = \C_z \times \R_t$, satisfies the three hypotheses of
...
\smallskip\noindent
\emph{Hypothesis \textup{(a)}: BV data.}
...
\smallskip\noindent
\emph{Hypothesis \textup{(b)}: One-loop finiteness.}
...
\smallskip\noindent
\emph{Hypothesis \textup{(c)}: Polynomial interactions.}
...
\smallskip
All three hypotheses are verified.  By Theorem~\ref{thm:physics-bridge},
...
\item The line-operator category is $C_{\mathrm{line}} \simeq A^!$-$\mathrm{mod}$
\textup{(}Theorem~\textup{\ref{thm:lines_as_modules})}.
\item The spectral $R$-matrix $r(z)$ is computed by bulk-boundary
propagator exchange
...
\begin{proof}
Each statement's only hypothesis beyond its own theorem-specific
conditions is that the input be a logarithmic $\SCchtop$-algebra.
This is Proposition~\ref{prop:affine-is-log-SC}.
\end{proof}
...
$\DS_f(\cA)$, defined as the BRST cohomology
\[
\DS_f(\cA) := H^\bullet(\cA \otimes \cC_f,\, Q_{\mathrm{BRST}}),
\]
inherits the structure of a logarithmic $\SCchtop$-algebra.
Concretely, the weight forms of $\DS_f(\cA)$ are the BRST
reductions of the weight forms of $\cA$:
\[
\omega_k^{\DS} = H^\bullet\bigl(\omega_k \otimes \cC_f^{\otimes k},\,
Q_{\mathrm{BRST}}^{\otimes k}\bigr),
\]
and these inherit the product decomposition
$\omega_k^{\DS} = (\omega_k^{\DS})^{\mathrm{hol}} \otimes
(\omega_k^{\DS})^{\mathrm{top}}$
