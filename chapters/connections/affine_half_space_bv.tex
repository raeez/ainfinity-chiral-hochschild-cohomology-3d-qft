...
The half-space BV quantization of the Khan--Zeng Poisson sigma model on $\C \times \R_{\ge 0}$ produces the affine Kac--Moody boundary VOA; the genus-0 projection of $\Theta^{\mathrm{oc}}$ is one-loop exact.  The general case (Theorem~\ref{thm:general-half-space-bv}) reduces to the whole-space Kontsevich machinery via the doubling construction (Theorem~\ref{thm:doubling-rwi}).  The affine Poisson--vertex sigma model on $\C \times \R_{\ge 0}$ is the mixed BF/Chern--Simons theory, solved to all loop orders by the Gwilliam--Williams half-space BV theorem.
\end{remark}
...
condition admits a renormalized BV quantization to all orders in $\hbar$; the quantization
...
\begin{theorem}[General half-space BV theorem for Poisson vertex sigma models; preview]\label{thm:general-half-space-bv-preview}
...
\ClaimStatusProvedHere{} (realized by the $\sigma$-equivariant FM
compactification in Theorem~\ref{thm:doubling-rwi}).
...
\begin{strategy}[From the affine theorem to the general case]\label{str:remaining-frontier}
...
The doubling theorem (Theorem~\ref{thm:doubling-rwi}) resolves this: the half-space calculus is the
$\sigma$-equivariant restriction of the standard whole-space calculus on the doubled space.
\end{strategy}
...
of Kontsevich formality due to Dolgushev.  The
...
construction (Theorem~\ref{thm:doubling-rwi}).
\end{proof}
...
independent lines of evidence, each addressing a different aspect:
...
(Definition~\ref{def:log-SC-algebra}).  Then the reflected weight
...
Let $V$ be a freely generated Poisson vertex algebra with inner
holomorphic translation, and let the half-space theory arise from
a logarithmic $\SCchtop$-algebra (Definition~\ref{def:log-SC-algebra}).
The Poisson--vertex sigma model on $\C\times\R_{\ge 0}$ with Neumann
boundary condition admits a renormalized perturbative BV quantization.
The boundary factorization algebra is a vertex algebra deforming~$V$.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:doubling-rwi}, the reflected weight identity holds
for any logarithmic $\SCchtop$-algebra.  By
Theorem~\ref{thm:half-space-reduction} (implication
$(2)\Rightarrow(1)$), the boundary factorization algebra is a vertex
algebra.
\end{proof}
...
\section{Verification of the bridge-theorem inputs}
\label{sec:verification-physics-bridge}

The abstract theorems of this chapter
(Theorems~\ref{thm:doubling-rwi}~and~\ref{thm:general-half-space-bv})
hold for any logarithmic $\SCchtop$-algebra.  We now verify that the
standard examples satisfy the three inputs of the physics bridge theorem
(Theorem~\ref{thm:physics-bridge}), closing the logical chain from the
abstract framework to the named families.

\begin{proposition}[The affine PV sigma model is a logarithmic $\SCchtop$-algebra]%
\label{prop:affine-is-log-SC}
\ClaimStatusProvedHere.
Let $\fg$ be semisimple and $k \in \C$.  The three-dimensional
Poisson--vertex sigma model of Khan--Zeng attached to the affine
Poisson vertex algebra $V_{\mathrm{aff}}(\fg, k)$, on
$X = \C_z \times \R_t$, satisfies the three inputs of
Theorem~\textup{\ref{thm:physics-bridge}}:
\begin{enumerate}[label=\textup{(\alph*)}]
\item BV data with HT gauge fixing and $Q_{\mathrm{BRST}}^2 = 0$;
\item one-loop finiteness;
\item polynomial interactions.
\end{enumerate}
...
Alternatively, the Gwilliam--Rabinovich--Williams mechanism applies
...
\smallskip
All three inputs are verified.  By Theorem~\ref{thm:physics-bridge},
the BV--BRST complex is a logarithmic $\SCchtop$-algebra.
\end{proof}

\begin{corollary}[The full half-space programme applies to the affine family]%
\label{cor:affine-full-programme}
\ClaimStatusProvedHere.
Since the affine PV sigma model is a logarithmic $\SCchtop$-algebra
\textup{(}Proposition~\textup{\ref{prop:affine-is-log-SC})},
the following hold unconditionally for the affine family:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The reflected weight identity holds for every admissible graph
\textup{(}Theorem~\textup{\ref{thm:doubling-rwi})}.
\item The half-space BV quantization exists with boundary vertex
algebra $V^{K_{\mathrm{eff}}}(\fg)$
\textup{(}Theorems~\textup{\ref{thm:affine-half-space-bv}}
and~\textup{\ref{thm:general-half-space-bv})}.
...
item~(iii) additionally requires chirally Koszul, which the affine
...
\begin{corollary}[Virasoro and $W_N$ are logarithmic $\SCchtop$-algebras]%
...
logarithmic $\SCchtop$-algebras.  In particular, the full half-space
programme \textup{(}boundary vertex algebra, line-operator
identification, spectral $R$-matrix\textup{)} applies to them.
\end{corollary}
...
The half-space programme is complete for any logarithmic $\SCchtop$-algebra (Definition~\ref{def:log-SC-algebra}):
\begin{gather*}
\text{PVA}
...
This chapter resolves the \emph{mixed color} of the Swiss-cheese
