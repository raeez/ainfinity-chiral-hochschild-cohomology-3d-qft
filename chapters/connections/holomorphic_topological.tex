...
Bar-cobar duality (Part~I) applies to $\mathcal{A}_{\mathcal{B}}$: the bar construction $\bar{B}(\mathcal{A}_{\mathcal{B}})$ \emph{is} the factorization coalgebra on $\Ran(X)$ that encodes PW boundary data (Convention~\ref{conv:bar-coalgebra-identity}).  Its Verdier dual $\mathbb{D}_{\Ran}\bar{B}(\mathcal{A}_{\mathcal{B}}) \simeq \bar{B}(\mathcal{A}_{\mathcal{B}}^!)$ identifies the dual boundary condition, and the cobar--bar counit $\Omega\bar{B}(\mathcal{A}_{\mathcal{B}}) \xrightarrow{\sim} \mathcal{A}_{\mathcal{B}}$ recovers the original algebra.  This allows one to study PW boundary VOAs via configuration space geometry.
\end{remark}
