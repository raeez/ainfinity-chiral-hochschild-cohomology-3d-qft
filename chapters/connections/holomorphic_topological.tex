...
The bar-cobar adjunction, from this perspective, provides the
resolution machinery: the bar construction $B(\cA)$ classifies
\emph{twisting morphisms} (universal couplings between $\cA$ and
$\cA^!$), while the \emph{chiral derived centre}
$\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)
= C^\bullet_{\mathrm{ch}}(\cA, \cA)$ computes the universal
bulk algebra.  Bar-cobar inversion
$\Omega(B(\cA)) \simeq \cA$ recovers the boundary algebra
from its own bar coalgebra, not from the bulk.
