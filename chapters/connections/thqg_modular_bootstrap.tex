% Twisted holography and quantum gravity: modular bootstrap as MC recursion.
%
% Labels used by the recursion:
...
The conformal bootstrap imposes crossing symmetry and modular
invariance; these equations constrain correlators without
determining them uniquely. The MC recursion is algebraically
complete for the fixed bar--modular shadow: it determines the
connected scalar shadow class at each genus from the OPE data at
...
(these follow from $D^2 = 0$ by genus grading).
\end{definition}
...
\medskip\noindent\emph{Expansion of $D\,\Theta$.}
...
\medskip\noindent\emph{Expansion of $[\Theta,\Theta]$.}
...
\medskip\noindent\emph{The tree-twisted differential squares to zero.}
The identity $d_{\Theta^{(0)}}^2 = 0$ follows from the genus-$0$ MC equation:
...
\item the solution is unique up to gauge equivalence
 (hence the genus-$g$ scalar shadow class is determined).
\end{enumerate}
...
\medskip
\noindent
\textbf{Given} $\Theta^{(0)}, \Theta^{(1)}, \ldots,
\Theta^{(g-1)}$.

\medskip
\noindent
\textbf{Obstruction class.}
...
d_{\Theta^{(0)}}(\Ob_g) = 0.
\end{equation}
This is an identity; it must hold by the inductive
hypothesis.

\textbf{Contracting homotopy.}
...
\Theta^{(1)}
\;=\;
\kappaChHodge(\cA) \cdot [\delta_{\mathrm{irr}}] \otimes \Delta,
\end{equation}
...
node). The scalar $\kappaChHodge(\cA)$ is the modular characteristic
...
$\Delta(\Theta^{(0)}) = \kappaChHodge(\cA) \cdot \omega_1$, where
...
The genus-$2$ recursion is completely explicit.

\medskip\noindent\emph{The obstruction.}
...
\medskip\noindent\emph{Shell decomposition.} The
...
contact flag $\chi_{\mathrm{ct}}$ and the mixed-shell discriminant
$\Delta_{\mathrm{mix}}=8\kappa S_4$:
\begin{center}
\begin{tabular}{cccccc}
$\sigma$ & $\pi$ & $\chi_{\mathrm{ct}}$ & $\Delta_{\mathrm{mix}}$ & Class & Example \\
\hline
$0$ & $0$ & $0$ & $0$ & G (Gaussian) & $\cH_k$ \\
$1$ & $1$ & $0$ & $0$ & L (Lie/tree) & $V_k(\fg)$ \\
$0$ & $0$ & $1$ & $0$ & C (contact) & $\beta\gamma$ \\
...
\begin{corollary}[Scalar degeneration for Gaussian support;
\ClaimStatusProvedHere]
...
For a Gaussian algebra $\cA$ \textup{(}support shadow depth
\(r^\Theta_{\max}(\cA)=2\), e.g.\ the Heisenberg algebra\textup{)},
the connected scalar line of the genus spectral sequence degenerates
at $E_1$:
...
\begin{proposition}[No finite support cutoff for mixed-type algebras;
\ClaimStatusProvedHere]
...
For a mixed-type algebra $\cA$ with support shadow depth
\(r^\Theta_{\max}(\cA)=\infty\)
\textup{(}e.g.\ the Virasoro algebra\textup{)}, no finite degree
bound on the bar-intrinsic support packet can force degeneration of
the genus spectral sequence. For every \(N\) there is a support
component \(\Theta_{\cA,n}\) with \(n>N\), hence a potential source for
higher genus-recursion terms after projection to the genus filtration.
Actual nonvanishing of a differential \(d_r\) remains a separate
cohomological statement: it can fail by exactness, projection to zero,
or cancellation in the chosen spectral-sequence page.
\end{proposition}
...
$r$-fold nested brackets, both of which have nonzero source terms when
the support packet is infinite.

More precisely, the \(d_r\) differential at the \((0,*)\) column maps
tree-level data through the \(r\)-loop recursion. Infinite support
depth says that the support filtration does not stabilize:
\(\Theta_{\cA}^{\leq N}\neq \Theta_{\cA}\) for every finite \(N\).
It does not say \(o_{N+1}(\cA)\neq0\), and it does not imply
\(d_N\neq0\) after passage to \(E_N\). Thus the proposition is the
support-envelope statement, not a non-cancellation theorem for every
page.
\end{proof}

\subsubsection{The spectral sequence and the support packet}
% label removed: subsubsec:thqg-VII-ss-shadow

\begin{theorem}[Spectral sequence differentials and support
projections; \ClaimStatusProvedHere]
\label{thm:thqg-VII-ss-shadow-obstruction}
\index{genus spectral sequence!support-packet correspondence}
The differentials of the genus spectral sequence receive their
non-scalar sources from the bar-intrinsic support packet:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $d_1$ encodes the modular characteristic $\kappaChHodge(\cA)$:
 it is the scalar genus-raising map
 $\Theta^{(0)} \mapsto \kappa \cdot \omega_1$.
\item $d_2$ receives the cubic support projection $\fC(\cA)$:
 it measures the failure of the genus-$2$ element to be
 determined solely by the scalar data.
\item $d_3$ receives the quartic support projection $\mathfrak{Q}(\cA)$:
 it contains the genus-$3$ obstruction contribution coming from the
 quartic contact invariant.
\item $d_r$ for $r \geq 4$ receives the degree-$(r{+}1)$
 support projection together with the corresponding $r$-loop
 obstruction term.
\end{enumerate}
If \(r^\Theta_{\max}(\cA)<\infty\), then no support component of
degree \(>r^\Theta_{\max}(\cA)\) can occur in these source terms.
This support cutoff is stronger than eventual vanishing of obstruction
classes, but it is not equivalent to a statement that every
corresponding spectral-sequence differential is nonzero or that the
spectral sequence collapses on a prescribed page.
\end{theorem}
...
and its non-scalar input is the degree-$(r{+}1)$ support projection
of \(\Theta_\cA\), together with the obstruction class
\(o_{r+1}(\cA)\) that tests extension of the truncation.

The support-depth assertion is the definition of
\(r^\Theta_{\max}\): if \(r>r^\Theta_{\max}(\cA)\), then
\(\Theta_{\cA,r}=0\). The equation \(o_{r+1}(\cA)=0\) has a different
meaning; it says that a degree-\(r\) truncation extends. It does not
erase an \(H^1\)-lift coordinate already chosen in the MC packet.
After forming \(E_r\), exactness and projection can further kill a
differential, so support depth supplies a source envelope rather than
an equality with the page of degeneration.
\end{proof}
...
The conformal bootstrap (see \cite{BPRS15} for a review)
...
they exclude regions of parameter space that violate them. The
remaining allowed region is the
...
support depth \(r^\Theta_{\max}\) gives the closure order of the
standard bar-intrinsic support packet, and the
...
provides unique determination together with a support-degree measure.
\end{remark}
...
scalar shadow class is uniquely determined \textup{(}up to gauge
...
the support packet is concentrated in degree~$2$ and
$\Theta_{\cH_k} = k \cdot \eta \otimes \Lambda$. The MC
recursion for $\cH_k$ is entirely scalar. Its connected genus
projection agrees with the Faber--Pandharipande $\lambda_g$-formula
...
$\kappaChHodge(\cH_k) = k$. The bar complex $\barB(\cH_k)$ is
...
chiral symmetric algebra $\Sym^{\mathrm{ch}}(V^*)$ on the dual
current space. The sign-flipped Heisenberg model $\cH_{-k}$ is the
open-colour line-side comparison, not the chiral Koszul dual.

Let $\omega_g$ denote the connected genus-$g$ Hodge class whose marked
pairing against $\psi_1^{2g-2}$ is the Faber--Pandharipande number.
The connected genus expansion has the form
\begin{equation}% label removed: eq:thqg-VII-heis-genus-exp
\Theta_{\cH_k}^{\mathrm{conn}}
\;=\;
\sum_{g \geq 1} \hbar^g\,\Theta^{(g),\mathrm{conn}}_{\cH_k},
\qquad
\Theta^{(g),\mathrm{conn}}_{\cH_k}
\;=\;
k\cdot\omega_g,\quad g\ge1,
\end{equation}
where the scalar free energy is determined by the moduli-space
integration:
\begin{equation}% label removed: eq:thqg-VII-heis-Fg
F_g(\cH_k)
\;=\;
k\,\lambda_g^{\mathrm{FP}},
\qquad
\lambda_g^{\mathrm{FP}}
=
...
is the OPE structure map. The genus-$g$ Gaussian determinant-line
amplitude of the gauge-fixed free-boson factorization algebra is the
$k$-th determinant-line power; hence its logarithm, i.e. its connected
free energy, is linear in~$k$. This is the perturbative determinant
model for the Heisenberg sector, not an unqualified measure-theoretic
path integral. Products of lower-genus terms are disconnected
determinant-line amplitudes and disappear after the connected
logarithm is taken. Thus the connected class is $k\omega_g$.

The numerical coefficient is the marked Hodge integral
\[
\lambda_g^{\mathrm{FP}}
=\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}\lambda_g.
\]
Faber--Pandharipande evaluate this integral as the Bernoulli number
shown above. Therefore
\[
F_g^{\mathrm{sc}}(\cH_k)=k\lambda_g^{\mathrm{FP}}.
\]
The equality is the scalar Hodge coefficient of the connected
determinant-line sector, not an unqualified full multi-channel genus
coefficient.
\end{proof}
...
connected scalar free energy is:
\begin{equation}% label removed: eq:thqg-VII-rank-d-Fg
F_g^{(d)}
\;=\;
d k\cdot\lambda_g^{\mathrm{FP}},
\end{equation}
where $\lambda_g^{\mathrm{FP}}$ is given by
\eqref{V1-eq:thqg-VII-heis-Fg}.
The zero-mode Gaussian determinant-line scalar carried by a
period matrix $\Omega_g$ is:
\begin{equation}% label removed: eq:thqg-VII-rank-d-det
\chi_{g,\mathrm{det}}^{(d)}(\Omega_g)
\;=\;
\frac{1}{\bigl(\det(\mathrm{Im}\,\Omega_g)\bigr)^{dk/2}},
\end{equation}
where $\Omega_g \in \mathfrak{H}_g$ is the period matrix of
the genus-$g$ surface. This analytic determinant-line scalar is a
representative of the Hodge determinant line; it is not the
exponential of the Faber--Pandharipande scalar free energy
$F_g^{(d)}$.
\end{corollary}
...
$\kappaChHodge(\cH_k^{\oplus d}) = d \cdot k$. The Gaussian
universality principle gives
$\Theta^{(g),\mathrm{conn}}_{\cH_k^{\oplus d}}=dk\,\omega_g$,
hence $F_g^{(d)}=dk\,\lambda_g^{\mathrm{FP}}$.
The Fredholm determinant computation
(Theorem~\ref{thm:heisenberg-sewing}) gives the determinant-line
representative
\[
\chi_{g,\mathrm{det}}^{(d)}(\Omega_g)
=(\det(\mathrm{Im}\,\Omega_g))^{-dk/2}.
\]
Its logarithmic curvature is $dk$ times the Hodge form; applying
the scalar trace
$\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}(-)$ gives
$F_g^{(d)}$. Thus the corollary records the integrated connected
Hodge free energy and the zero-mode determinant-line scalar, not a
...
\begin{tabular}{cllll}
\hline
$g$ & $\Ob_g$ & $\Theta^{(g),\mathrm{conn}}$ & $\lambda_g^{\mathrm{FP}}$ & Check \\
\hline
...
$1$ & $\Delta(\Theta^{(0)})$ &
 $k \cdot [\delta_{\mathrm{irr}}] \otimes \Delta$ &
 $\frac{1}{24}$ & $\checkmark$ \\
$2$ & connected part of $\frac{1}{2}[\Theta^{(1)},\Theta^{(1)}]$ &
 $k \cdot \omega_2$ &
...
% label removed: rem:thqg-VII-derivation
\index{Faber--Pandharipande formula!derivation}
...
The objects of the MC recursion carry a gravitational reading
in the HT setting. The dictionary is a heuristic
(\ClaimStatusHeuristic): the algebraic statements are the theorems
of the preceding sections; their gravitational names below presuppose a
fixed HT construction in which $\Theta_\cA$ is the genus expansion of a
bulk amplitude.

\subsubsection{The gravitational interpretation}
% label removed: subsubsec:thqg-VII-grav-interpretation

In the chosen HT construction the genus expansion
$\Theta_\cA = \sum_g \hbar^g \Theta^{(g)}$ reads as a
\emph{gravitational loop expansion}: $\hbar$ plays the role of
Newton's constant $G_N$ in the bulk, and $\Theta^{(g)}$ that of
the $g$-loop gravitational amplitude. This identification is a
dictionary entry, not a theorem of $\Phi_{\mathrm{hol}}$; matching
$\Theta^{(g)}$ to a metric-saddle loop expansion is the additional
physical input.

\begin{principle}[Gravity-bootstrap dictionary; \ClaimStatusHeuristic]
% label removed: princ:thqg-VII-gravity-bootstrap
\index{gravity-bootstrap correspondence}
Under the fixed HT construction, the MC recursion reads as a
\emph{boundary-to-bulk reconstruction} of the gravitational loop
expansion:
\begin{center}
\begin{tabular}{ll}
...
\begin{theorem}[One-loop scalar shadow from genus-$1$ MC;
\ClaimStatusProvedHere]
% label removed: thm:thqg-VII-one-loop-gravity
\index{one-loop gravitational scalar shadow}
The genus-$1$ MC element $\Theta^{(1)}$ determines the one-loop
gravitational scalar shadow:
\begin{equation}% label removed: eq:thqg-VII-one-loop
Z_{1}^{\mathrm{scal}}(\cA)
\;=\;
\exp\bigl(\Tr(\Theta^{(1)})\bigr)
\;=\;
\exp\bigl(\kappaChHodge(\cA) \cdot F_1\bigr),
\end{equation}
where $\Tr$ denotes the scalar trace, namely integration over the
genus-$1$ moduli space $\overline{\cM}_{1,1}$. For the Heisenberg
algebra at level~$k$ the scalar trace and the torus determinant
are different functions:
\begin{equation}% label removed: eq:thqg-VII-one-loop-heis
Z_{1}^{\mathrm{scal}}(\cH_k)
\;=\;
\exp\!\bigl(k/24\bigr),
\qquad
\chi_{1}^{\mathrm{det}}(\cH_k;\tau)
\;=\;
\eta(\tau)^{-k},
\end{equation}
and only the first expression is the genus-$1$ scalar Hodge trace.
\end{theorem}

\begin{proof}
The genus-$1$ scalar shadow contribution is
$Z_1^{\mathrm{scal}} = \exp(F_1)$ where
$F_1 = \kappaChHodge(\cA) \cdot
...
gives \(F_1(\cA)=\kappaChHodge(\cA)/24\)
(which equals \(k/24\) for the Heisenberg \(\cH_k\)).
\end{proof}
...
\item \emph{finite}, by the HS-sewing criterion
 \textup{(}Theorem~\textup{\ref{thm:general-hs-sewing}}) after the
 Fulton--MacPherson boundary extension;
...
For a Gaussian algebra $\cA$ \textup{(}\(r^\Theta_{\max}(\cA)=2\)\textup{)},
the connected scalar shadow expansion has the following
non-renormalization property: the $g$-loop amplitude
$\mathcal{A}_g$ depends on the genus-$0$ data only through
the single scalar $\kappaChHodge(\cA)$:
...
\(\lambda_g^{\mathrm{FP}}
=\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}\lambda_g\)
is the Faber--Pandharipande scalar.
In particular:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
The Gaussian support condition \(r^\Theta_{\max}=2\) says that the
bar-intrinsic support packet has only the scalar quadratic component;
the convolution algebra is formal on the connected scalar line, and
the contracting homotopy is scalar there. The recursion
\(\Theta^{(g)}=-\hCont(\Ob_g)\) therefore produces the same
Hodge class at each genus, multiplied by the single scalar
\(\kappaChHodge(\cA)\). Applying the marked scalar trace gives
\(\kappaChHodge(\cA)\lambda_g^{\mathrm{FP}}\), and exponentiating
the connected scalar coefficients gives the disconnected scalar
shadow series displayed above. No claim about a physical
gravitational partition function is made here.
\end{proof}

\subsubsection{Beyond the Gaussian: gravitational complexity}
% label removed: subsubsec:thqg-VII-beyond-gaussian

\begin{remark}[Gravitational corrections from the support packet]
% label removed: rem:thqg-VII-grav-corrections
\index{gravitational corrections!support packet}
For non-Gaussian algebras \textup{(}\(r^\Theta_{\max}\geq3\) on the
finite standard branches, or \(r^\Theta_{\max}=\infty\)\textup{)}, the MC recursion
produces genuine multi-loop gravitational corrections that are
\emph{not} determined by the single scalar $\kappa$. These arise from the
higher support data:
\begin{enumerate}[label=(\alph*)]
\item For the Lie/tree archetype \textup{(}\(r^\Theta_{\max}=3\),
 ...
 of~$\fg$.
\item For the contact archetype \textup{(}\(r^\Theta_{\max}=4\),
...
\item For the mixed archetype \textup{(}\(r^\Theta_{\max}=\infty\),
 e.g.\ $\mathrm{Vir}_c$): the support packet has no finite cutoff.
 No support-depth argument can force finite-page degeneration. The
 gravitational amplitude at genus~$g$ receives contributions
 from all degrees up to $g + 2$ (by the weight bound
 $w = 2g - 2 + r + d$).
\end{enumerate}
These corrections measure the support-degree complexity of the
boundary algebra: higher \(r^\Theta_{\max}\) allows richer bulk
comparison channels.
\end{remark}

\begin{proposition}[Support-degree bounds on gravitational amplitudes;
\ClaimStatusProvedHere]
% label removed: prop:thqg-VII-complexity-bounds
\index{gravitational amplitude!complexity bounds}
For a modular Koszul chiral algebra $\cA$ with finite support shadow
depth \(r^\Theta_{\max}(\cA)<\infty\), the $g$-loop shadow amplitude
has no support-degree component above \(r^\Theta_{\max}(\cA)\). More
precisely:
...
from the bar-intrinsic support packet, contracted with the
tautological classes of the moduli space. This is a support-degree
bound on the fixed HT shadow model, not a graph-degree theorem for an
independently constructed metric-gravity path integral.
\end{proposition}

\begin{proof}
Finite support depth means
\(\Theta_{\cA,r}=0\) for every \(r>r^\Theta_{\max}(\cA)\). The
recursive formula for \(\Theta^{(g)}\) is built from this chosen
bar-intrinsic support packet by the BV operator, brackets, and the
contracting homotopy. These operations can contract or combine
available support components, but they cannot introduce a primitive
support coordinate above the support cutoff. The displayed direct sum
is therefore the support envelope for the genus-\(g\) shadow amplitude.
The obstruction equation \(o_{r+1}=0\) is not used: it controls
extendability of a truncation, not the vanishing of a previously
chosen \(H^1\)-lift coordinate.
\end{proof}

\subsubsection{The gravitational modular bootstrap}
% label removed: subsubsec:thqg-VII-grav-bootstrap

\begin{remark}[Gravitational modular bootstrap]
...
from boundary OPE data. The required data are:
...
Z_{\mathrm{sh}}^{\mathrm{scal}}(\cA;\hbar)
=\exp\bigl(\sum_{g \geq 1}\hbar^g
\mathcal{A}_g^{\mathrm{scal,conn}}(\cA)\bigr),
\]
determined by the boundary data without free parameters inside the
fixed HT shadow model.

Within the fixed HT construction this realizes, as a boundary-CFT
reading rather than a statement about metric quantum gravity, the
heuristic that the bulk loop expansion is reconstructed from its
boundary conditions: the MC equation on the convolution algebra
$\gAmod$ is the algebraic mechanism, and the gravitational reading of
its output is conditional on a separate comparison between
\(\Theta_\cA\), a bulk saddle expansion, and the completed physical
partition function.
\end{remark}
...
\subsubsection{The role of support depth in gravitational
complexity}
% label removed: subsubsec:thqg-VII-depth-gravity

\begin{theorem}[Support depth bounds modular-bootstrap complexity;
\ClaimStatusProvedHere]
% label removed: thm:thqg-VII-depth-classification
\index{support shadow depth!gravitational classification}
The support shadow depth \(r^\Theta_{\max}(\cA)\) determines the
support-degree envelope of the MC recursion:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(r^\Theta_{\max}=2\) \textup{(}Gaussian\textup{)}:
 the connected scalar line of the genus spectral sequence collapses
 at \(E_1\); the
 $g$-loop amplitude $\mathcal{A}_g$ depends on the genus-$0$
 data only through the scalar~$\kappaChHodge(\cA)$.
 Example: $\cH_k$.
\item \(r^\Theta_{\max}=3\) \textup{(}Lie/tree, canonical affine
 branch\textup{)}: the cubic support component \(\fC(\cA)\) is the
 last nonzero support coordinate. The genus-$2$ amplitude receives
 cubic corrections. Example: $V_k(\fg)$.
\item \(r^\Theta_{\max}=4\) \textup{(}contact, canonical contact
 branch\textup{)}: the quartic support component
 \(\mathfrak{Q}(\cA)\) is present and there are no higher support
 coordinates on this branch. The planted-forest shell is activated at
 genus~$2$. Example: $\beta\gamma$.
\item \(r^\Theta_{\max}=\infty\) \textup{(}mixed\textup{)}:
 there is no finite support-degree envelope, and no depth argument can
 force finite-page degeneration. Example: $\mathrm{Vir}_c$.
\end{enumerate}
In the holographic interpretation \textup{(}\ClaimStatusHeuristic{}\textup{)},
the four classes correspond to HT boundary models with increasing
available bulk-comparison vertex complexity: free \textup{(}G\textup{)},
cubic \textup{(}L, Chern--Simons\textup{)},
quartic \textup{(}C, topological $B$-model\textup{)},
and all-order \textup{(}M, the Virasoro gravity line\textup{)}.
\end{theorem}

\begin{proof}
The classification follows from the support-packet statement
...
The vertex envelope is read off from the support packet: a nonzero
degree-\(r\) support coordinate can feed an \((r{-}1)\)-valent
comparison vertex after a bulk comparison datum is supplied. The
...
\item \textbf{G (Gaussian):} The recursion is trivial
 on the connected scalar projection:
 $\Theta^{(g),\mathrm{conn}}
 = \kappaChHodge(\cA)\,\omega_g$, with scalar
 coefficient
 $\kappaChHodge(\cA)\lambda_g^{\mathrm{FP}}$. The gravitational
comparison theory is free. The scalar shadow series factorizes.
\item \textbf{L (Lie/tree):} The recursion is nontrivial
 starting at genus~$2$ and the canonical affine support packet has no
 component above degree~$3$. The gravitational comparison theory has
 cubic interactions. Chern--Simons
...
\item \textbf{C (contact):} The recursion is nontrivial
 ...
 invariant. The gravitational comparison theory has quartic
 interactions.
\item \textbf{M (mixed):} The recursion is fully nontrivial
 on arbitrarily high support degrees. The gravitational comparison
 theory has interactions of all orders.
\end{itemize}
The Virasoro algebra, as the universal stress-tensor algebra,
sits in the M~class: its infinite support packet reflects the
all-order nature of the Virasoro gravity line. The finite classes
(G, L, C) correspond to support-truncated HT comparison models: they
are the topological or semistrict approximations that capture the
lowest-order
interactions.
\end{remark}
...
The modular bootstrap for twisted HT quantum gravity, at the K3 base
...
close onto $\Phi_{10}^{\mathrm{un}}(Z)$ in four duality frames (cf.~Vol.~II,
\S\ref{sec:line-operators-supplement}):
\begin{itemize}
\item heterotic on $T^4$ \textrm{(Harvey--Moore 1996)}: modular bootstrap recovers $\Phi_{10}^{\mathrm{un}}$
 as the elliptic-genus generating series;
\item IIB on $K3$ \textrm{(Dijkgraaf--Verlinde--Verlinde 1996--97)}: $D1$--$D5$ bootstrap
 recovers the $\Phi_{10}^{\mathrm{un}}$ denominator of the BPS index;
\item IIA on $K3$ \textrm{(Dijkgraaf--Moore--Verlinde--Verlinde 1997)}: string bootstrap
 recovers $\Phi_{10}^{\mathrm{un}}$ as the second-quantised Euler-characteristic series;
\item M-theory on $K3\times T^2$ \textrm{(Witten 1995)}: M$2$--M$5$ bootstrap
 recovers $\Phi_{10}^{\mathrm{un}}$ as the twisted one-loop determinant.
\end{itemize}
All four converge to the same scalar automorphic object
$\Phi_{10}^{\mathrm{un}}$.  A lift from this scalar equality to a K3
bar--cobar adjunction on $\mathbf H_{\Delta_5}$ requires the finite
Hall--Borcherds recognition datum stated below; without that datum the
bootstrap assertion is only the equality of scalar Fourier packages.
\end{remark}

\section{Three-dimensional TQFT state spaces from the non-semisimple MTC}
\label{sec:modbootstrap-3d-tqft}

The scalar modular bootstrap closes onto $\Phi_{10}^{\mathrm{un}}$
(Remark~\ref{rem:modbootstrap-1}).  Every use below of
$\mathbf H_{\Delta_5}$, of the Kerler--Lyubashenko category
$\mathcal C$, of DGGPR state spaces, of state sums, of transport, and
of conformal blocks is conditional on the Vol~III finite
Hall--Borcherds recognition datum: compact source primitive bases with
provenance, full parity blocks, product and coproduct matrices $M,D$,
super-bracket matrices $B$, Hopf-pairing matrices $G$, radical kernels
$K$, quotient maps $Q$, comparison matrices $A_\beta$, checked
Borcherds--Kac relation rows, Hopf radical/coideal descent,
no-extra-relations kernel equality, parity-refined PBW associated
graded comparison, centre control, associator control, and strict
finite-window transition compatibility.

Under that recognition datum, the bar--cobar adjunction on
$\mathbf H_{\Delta_5}$ supplies the Kerler--Lyubashenko
non-semisimple MTC
...
Theorem~\ref{V3-thm:modtr-Plancherel-KerlerLyubashenko}. This category
is the categorical substrate on which the lifted bootstrap is realised.
The
...
Fourier--Jacobi expansion of $\Phi_{10}^{\mathrm{un}}$, (iii) the
Oberdieck--Pandharipande reduced DT scalar in its own convention
...
\begin{theorem}[MC recursion on $Z^{\mathrm{DGGPR}}(\Sigma_2)$ reproduces
the bootstrap; \ClaimStatusConditional]
...
$\cA = \mathbf H_{\Delta_5}$ after finite Hall--Borcherds recognition,
coincides with the bulk--boundary sewing
axiom for $Z^{\mathrm{DGGPR}}(\Sigma_2)$:
\[
 \Theta^{(2)}_{\mathbf H_{\Delta_5}}
 \;=\;
 \iota\bigl((\Phi_{10}^{\mathrm{un}})^{-1}\bigr),
\]
...
$(\Phi_{10}^{\mathrm{un}})^{-1}$. Matching the two: the MC recursion builds
...
$\Phi_{10}^{\mathrm{un}}$ is the unique weight-$10$ Siegel cusp form on
...
$\Theta^{(2)} = \iota((\Phi_{10}^{\mathrm{un}})^{-1})$.

\emph{Independent derivations.}
...
Assume the finite Hall--Borcherds recognition datum of
\S\ref{sec:modbootstrap-3d-tqft}.
...
lifts of Cheng--Duncan--Harvey 2014 umbral mock theta series. The K3
...
Theorem~\ref{thm:modbootstrap-Zsigma2-bootstrap}, under the finite
Hall--Borcherds recognition datum of
\S\ref{sec:modbootstrap-3d-tqft}, upgrades to a
...
$\Omega_{H_2} = \iota((\Phi_{10}^{\mathrm{un}})^{-1})\in
...
$\Omega_{H_2} = \iota((\Phi_{10}^{\mathrm{un}})^{-1})$ is
...
$(\Phi_{10}^{\mathrm{un}}(Z))^{-1}$ after vacuum normalisation
...
$(\Phi_{10}^{\mathrm{un}}(Z))^{-1}$, and, at other canonical 3-manifolds (lens spaces,
...
3d TQFT partition function under Heegaard surgery after the finite
Hall--Borcherds recognition datum has supplied the source category, as
first observed
...
\begin{theorem}[Scalar genus-two Fourier dictionary; \ClaimStatusProvedHere]
\label{thm:modbootstrap-genus2-fourier-dictionary}
Write
\[
 \phi_{0,1}^{K3}(\tau,z)=\sum_{a\geq0,\ r\in\mathbb Z}f(a,r)q^a y^r,
 \qquad
 f(0,\pm1)=1,\quad f(0,0)=10,\quad
 f(1,\pm1)=-64,\quad f(1,0)=108 .
\]
Around the standard cusp
$(\tau,z,\tau')\to i\infty$ of $\mathfrak H_2$, the monic
Gritsenko--Nikulin product and the unnormalised square convention give
\[
 D_5(Z)
 \;=\;
 p^{1/2}q^{1/2}y^{1/2}
 ...
 =64^{-1}\Delta_5(Z),
\]
where $y=e^{2\pi iz}$, $q=e^{2\pi i\tau}$, and
$p=e^{2\pi i\tau'}$.  Hence
\[
 \frac{1}{\Phi_{10}^{\mathrm{un}}(Z)}
 =
...
\]
The primitive $\Delta_5$ denominator exponents are $f(mn,r)$.  The
doubled exponents $2f(mn,r)$ belong to the scalar square
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$; Fourier coefficients of the
reciprocal, such as the standard DMVV coefficients
$276,11202,184024$, are convolution coefficients of this reciprocal
square and are not primitive BKM root multiplicities.
\end{theorem}

\begin{proof}
The infinite-product expansion is the Gritsenko--Nikulin 1998
\emph{Amer.~J.~Math.}~120 \S 3 denominator identity for
$\widehat{\mathfrak m}_{\Delta_5}$: the exponent is the
\(\phi_{0,1}^{K3}\) coefficient extracted from the singular theta lift
\(\phi_{0,1}^{K3}\mapsto\Delta_5\) (Borcherds 1998
arXiv:alg-geom/9609022 Thm.~10.1). The displayed leading coefficients
are the Eichler--Zagier coefficients of the normalised weak Jacobi form
\(\phi_{0,1}^{K3}\); the full K3 elliptic genus is
\(2\phi_{0,1}^{K3}\), while the Igusa square already doubles the
primitive exponents.  Since
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ and
$D_5=64^{-1}\Delta_5$, inversion gives the reciprocal product with the
factor $(4096\,p q y)^{-1}$.  The quoted DMVV numbers are obtained by
expanding this reciprocal square; they are coefficients of the inverse
partition function, not denominator exponents.

The identity is determined by the Gritsenko--Nikulin denominator
formula, the Eichler--Zagier coefficient table, Borcherds'
singular-theta lift, the DMVV reciprocal-square expansion, and the
normalisations \(\Phi_{10}^{\mathrm{un}}=\Delta_5^2\) and
\(D_5=64^{-1}\Delta_5\).
\end{proof}

\begin{remark}[Conditional TQFT reading of the scalar coefficients]
\label{rem:modbootstrap-genus2-fourier-dictionary-tqft}
Assume the finite Hall--Borcherds recognition datum of
\S\ref{sec:modbootstrap-3d-tqft}.  Then the DGGPR invariant on
$\Sigma_2\times S^1$ may be evaluated by the same reciprocal-square
Fourier coefficients:
\[
 \tau_{\mathcal C}(\Sigma_2\!\times\! S^1)(Z)
 =
 \sum a(m,n,r)q^m p^n y^r,\qquad
 \sum a(m,n,r)q^m p^n y^r=(\Phi_{10}^{\mathrm{un}}(Z))^{-1}.
\]
Without the recognition datum, Theorem~\ref{thm:modbootstrap-genus2-fourier-dictionary}
is only the scalar Fourier identity.
\end{remark}
...
\ClaimStatusConditional
Assume the finite Hall--Borcherds recognition datum of
\S\ref{sec:modbootstrap-3d-tqft}. The total quantum dimension of
$\mathcal C = \mathrm{Rep}^{\mathrm{fd}}(\mathfrak u_{\zeta_8}
...
$c_{2d} = -214$ of
Vol~I Proposition~\ref{V1-prop:stqp-312-factor}, and after the finite
Hall--Borcherds recognition datum of
\S\ref{sec:modbootstrap-3d-tqft}, the genus-one chiral conformal
blocks of $\mathbf H_{\Delta_5}$ form a
...
Assume the finite Hall--Borcherds recognition datum and a chosen
Humbert restriction functional on the scalar Fourier--Jacobi expansion.
...
\[
 \mathcal R_{-2,1}(\tau)
 :=
 \lim_{z\to0}\frac{\phi_{-2,1}(\tau,z)}{(2\pi iz)^2}
\]
be the renormalised Humbert residue of the weight-$(-2)$ Jacobi form.
The unrenormalised value $\phi_{-2,1}(\tau,0)$ is zero and is not used
as a block coefficient.
...
\begin{align}
 ...
 F_{\alpha_1}(\tau)
 &\;=\;\frac{\phi_{0,1}^{K3}(\tau,0)}{12}\,\eta(\tau)^{-4}
 \;=\;q^{-1/6}\bigl(1 + 4q + 14q^2 + 40q^3 + \cdots\bigr),
 \label{eq:modbootstrap-explicit-Fa1}\\
 F_{\alpha_2}(\tau)
 &\;=\;F_{\alpha_1}(\tau),
 \label{eq:modbootstrap-explicit-Fa2}\\
 F_{\alpha_3}(\tau)
 &\;=\;\mathcal R_{-2,1}(\tau)\,\eta(\tau)^{-4},
 \label{eq:modbootstrap-explicit-Fa3}\\
 F_{\alpha_1 + \alpha_2}(\tau)
 &\;=\;E_4(\tau)\,\eta(\tau)^{-12}
 \;=\;q^{-1/2}\bigl(1 + 252q + 5130q^2 + 54760q^3 + \cdots\bigr),
 \label{eq:modbootstrap-explicit-Fa1a2}\\
 F_{\alpha_1 + \alpha_3}(\tau)
 &\;=\;\frac{\phi_{0,1}^{K3}(\tau,0)}{12}\mathcal R_{-2,1}(\tau)\cdot\eta^{-8},
...
$-c/24 = 107/12$, agree with the displayed scalar and residue blocks
...
\[
 ...
 i\in\{1, 2, 3\},
\]
with $\phi^{(1)} = \phi^{(2)} = \phi_{0,1}^{K3}(\tau,0)/12$ and
$\phi^{(3)} = \mathcal R_{-2,1}(\tau)$ after the chosen Humbert
renormalisation.
\end{theorem}
...
\(\phi_{0,1}^{K3}\) times a weight-$(-12)$ form. Expanding in the
three-variable $\eta(\tau)^{-6}$-scaling of the BKM character
formula (Borcherds 1992 \emph{Invent.}~109 Theorem~10.1)
produces the displayed scalar and renormalised-residue blocks via the
Gritsenko--Nikulin
...
1998 \S 5.1 Remark~5.1.1). Since
\(\phi_{0,1}^{K3}(\tau,0)=12\), the unit-leading block is
\(\eta^{-4}\), whose Euler product gives
\[
\eta(\tau)^{-4}=q^{-1/6}(1+4q+14q^2+40q^3+105q^4+\cdots).
\]
\item $F_{\alpha_3}$: the hyperbolic simple root is distinguished
by $\langle\alpha_3, \alpha_3\rangle = -2$, so $F_{\alpha_3}$
receives the renormalised Humbert residue of the weight-$(-2,1)$
Jacobi form. Since
$\phi_{-2,1}(\tau,z)=\theta_1(\tau,z)^2/\eta(\tau)^6$ has a double
zero at $z=0$ (Eichler--Zagier 1985 \S 9.1 eq.~(9.1)), the raw value
$\phi_{-2,1}(\tau,0)$ is zero; a non-zero block requires the residue
$\mathcal R_{-2,1}$ fixed in the theorem.
...
\((\phi_{0,1}^{K3}(\tau,0)/12)\mathcal R_{-2,1}(\tau)\) is the corresponding
renormalised Humbert coefficient, scaled by $\eta^{-8}$.
...
The theta-factorisations of \(\phi_{0,1}^{K3}(\tau,0)/12\) and
\(\mathcal R_{-2,1}(\tau)\), multiplied by the displayed powers of
\(\eta\), give the \(\Delta_5^{-1}\) Fourier--Jacobi coefficients
tabulated in Gritsenko--Nikulin 1998 \S 3.5.

The formula is fixed by the Fourier--Jacobi expansion of
Gritsenko--Nikulin, the weak-Jacobi generators of Eichler--Zagier, the
Borcherds character formula, the Kac--Walton recursion, and the Euler
product of \(\eta(\tau)=q^{1/24}\prod_{n\geq1}(1-q^n)\).
\end{proof}
