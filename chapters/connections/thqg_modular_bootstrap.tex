...
\label{sec:thqg-modular-bootstrap}
...
The genus-$g$ partition function of a holomorphic-topological (HT)
gravitational theory is not a free parameter: it is determined
inductively from the genus-$0$ data by the Maurer--Cartan equation
in the modular convolution algebra~$\gAmod$
(Definition~\ref{def:modular-convolution-dg-lie}).  The purpose
of this section is to make this determination completely explicit.
...
\label{subsec:thqg-VII-mc-genus-expansion}
\index{Maurer--Cartan equation!genus expansion|textbf}

\subsubsection{The genus filtration on the convolution algebra}
\label{subsubsec:thqg-VII-genus-filtration}

We recall from Definition~\ref{def:modular-convolution-dg-lie}
that the modular convolution dg~Lie algebra
\begin{equation}\label{eq:thqg-VII-gAmod-def}
\gAmod
\;=\;
\prod_{g \geq 0}\;
\prod_{n \geq 1}\;
...
\begin{equation}\label{eq:thqg-VII-genus-filtration}
F^g\,\gAmod
\;:=\;
\prod_{g' \geq g}\;
\prod_{n \geq 1}\;
...
\begin{equation}\label{eq:thqg-VII-assoc-graded}
\gr^g_F\,\gAmod
\;=\;
F^g / F^{g+1}
\;\cong\;
...
\label{lem:thqg-VII-genus-shifts}
...
\begin{equation}\label{eq:thqg-VII-D1}
D_1
\;:=\;
d_{\mathrm{loop}}
+ d_{\mathrm{pf}}.
...
\label{rem:thqg-VII-three-identities}
...
&= 0,\label{eq:thqg-VII-D-squared-decomp}
\end{align}
...
\label{subsubsec:thqg-VII-genus-expansion}

The universal MC element $\Theta_\cA \in \MC(\gAmod)$
(Theorem~\ref{thm:mc2-bar-intrinsic}) admits a genus expansion
\begin{equation}\label{eq:thqg-VII-theta-genus-exp}
\Theta_\cA
\;=\;
...
\begin{equation}\label{eq:thqg-VII-mc-full}
D\,\Theta_\cA
...
\begin{equation}\label{eq:thqg-VII-d-twisted}
d_{\Theta^{(0)}}
\;:=\;
D_0 + \ad_{\Theta^{(0)}}
\;=\;
...
\begin{equation}\label{eq:thqg-VII-obs-class}
\Ob_g\bigl(\Theta^{(<g)}\bigr)
\;:=\;
\underbrace{
...
\label{eq:thqg-VII-D-theta-expand}
\end{align}
...
\begin{equation}\label{eq:thqg-VII-bracket-expand}
...
0 &= D_0(\Theta^{(g)})
  ...
  &\qquad
   ...
   + \tfrac{1}{2}
     \sum_{\substack{g_1 + g_2 = g \\ g_1, g_2 \geq 1}}
     [\Theta^{(g_1)},\, \Theta^{(g_2)}].
\label{eq:thqg-VII-mc-g-assembled}
\end{align}
Rearranging, with $d_{\Theta^{(0)}} = D_0 +
[\Theta^{(0)}, -]$:
\begin{equation}\label{eq:thqg-VII-mc-g-rearranged}
d_{\Theta^{(0)}}(\Theta^{(g)})
\;=\;
-\Delta(\Theta^{(g-1)})
- \dsew^{(g)} - \dpf^{(g)}
...
\begin{equation}\label{eq:thqg-VII-genus0-mc}
...
&= [D_0\Theta^{(0)}
    + \tfrac{1}{2}[\Theta^{(0)},\Theta^{(0)}], x]\notag\\
&= 0,
\label{eq:thqg-VII-d-twisted-squared}
\end{align}
...
\label{rem:thqg-VII-three-term-disjointness}
...
\label{rem:thqg-VII-obs-cocycle}
...
\begin{equation}\label{eq:thqg-VII-obs-closed}
...
\begin{equation}\label{eq:thqg-VII-obs-class-coh}
[\Ob_g]
\;\in\;
...
(Theorem~\ref{thm:mc2-bar-intrinsic}): the global element
...
\label{subsubsec:thqg-VII-low-genus}
...
At genus $g = 1$:
\begin{equation}\label{eq:thqg-VII-mc-g1}
d_{\Theta^{(0)}}(\Theta^{(1)})
\;=\;
-\Delta(\Theta^{(0)})
- \dsew^{(1)},
...
\begin{equation}\label{eq:thqg-VII-dsew1-explicit}
\dsew^{(1)}(\varphi)
\;=\;
...
planted forests with positive loop genus require at least two
internal loops; at genus~$1$ the only boundary stratum is the
non-separating node $\delta_{\mathrm{irr}} \in \partial\overline{\cM}_{1,n}$.
\end{proof}
...
At genus $g = 2$:
\begin{equation}\label{eq:thqg-VII-mc-g2}
d_{\Theta^{(0)}}(\Theta^{(2)})
\;=\;
...
At genus $g = 3$:
\begin{equation}\label{eq:thqg-VII-mc-g3}
d_{\Theta^{(0)}}(\Theta^{(3)})
\;=\;
-[\Theta^{(1)},\, \Theta^{(2)}]
- \Delta(\Theta^{(2)})
...
At genus $g = 4$:
\begin{equation}\label{eq:thqg-VII-mc-g4}
d_{\Theta^{(0)}}(\Theta^{(4)})
\;=\;
-[\Theta^{(1)},\, \Theta^{(3)}]
...
\label{rem:thqg-VII-general-pattern}
...
\begin{equation}\label{eq:thqg-VII-obs-general}
\Ob_g
\;=\;
\underbrace{
\sum_{\substack{g_1+g_2=g \\ g_1,g_2 \geq 1}}
...
\label{subsubsec:thqg-VII-graph-sum}
...
graph:
\begin{equation}\label{eq:thqg-VII-bracket-graph}
[\Theta^{(g_1)},\Theta^{(g_2)}]
\;=\;
...
\begin{equation}\label{eq:thqg-VII-bv-graph}
\Delta(\Theta^{(g-1)})
\;=\;
...
\label{rem:thqg-VII-feynman-interpretation}
...
\textup{(}Theorem~\textup{\ref{thm:mc2-bar-intrinsic})}.  Denote
...
\item The tree-twisted complex $(\gAmod, d_{\Theta^{(0)}})$
  ...
  satisfying
  \begin{equation}\label{eq:thqg-VII-homotopy}
...
\item The cohomology $H^*(\gAmod[\mathrm{genus\,}g],
  ...
  \begin{equation}\label{eq:thqg-VII-h2-vanishing}
...
\begin{equation}\label{eq:thqg-VII-hpl-homotopy}
\hCont
\;=\;
\hCont_0 \circ
\sum_{n \geq 0}
...
(Theorem~\ref{thm:mc2-bar-intrinsic}) exists at all genera.
...
\label{subsubsec:thqg-VII-recursion-algorithm}
...
\begin{equation}\label{eq:thqg-VII-recursion-obs}
\Ob_g
\;:=\;
\sum_{\substack{g_1+g_2=g \\ g_1,g_2 \geq 1}}
\tfrac{1}{2}[\Theta^{(g_1)},\Theta^{(g_2)}]
...
\begin{equation}\label{eq:thqg-VII-recursion-cocycle}
d_{\Theta^{(0)}}(\Ob_g) = 0.
\end{equation}
...
\begin{equation}\label{eq:thqg-VII-recursion-solve}
\Theta^{(g)}
\;:=\;
...
\begin{equation}\label{eq:thqg-VII-gauge-diff}
...
\label{subsubsec:thqg-VII-base-case}
...
\begin{equation}\label{eq:thqg-VII-theta1}
\Theta^{(1)}
\;=\;
-\hCont\bigl(
...
\begin{equation}\label{eq:thqg-VII-theta1-scalar}
\Theta^{(1)}
\;=\;
...
\label{subsubsec:thqg-VII-genus2-worked}

\begin{computation}[Genus-$2$ MC recursion; \ClaimStatusProvedHere]
\label{comp:thqg-VII-genus2}
...
\begin{equation}\label{eq:thqg-VII-obs2-full}
\Ob_2
\;=\;
\tfrac{1}{2}[\Theta^{(1)},\Theta^{(1)}]
+ \Delta(\Theta^{(1)})
...
\label{eq:thqg-VII-bracket-term-g2}
\end{align}
...
element:
\begin{equation}\label{eq:thqg-VII-bv-term-g2}
\Delta(\Theta^{(1)})
...
\begin{equation}\label{eq:thqg-VII-theta2-solution}
\Theta^{(2)}
...
\label{eq:thqg-VII-shell-loop2}\\
\Theta^{(2)}_{\mathrm{sep}\circ\mathrm{loop}}
&= -\hCont\bigl(\Delta(\Theta^{(1)}) + \dsew^{(2)}\bigr)
\neq 0 \text{ iff } \fC(\cA) \neq 0,
\label{eq:thqg-VII-shell-sep}\\
\Theta^{(2)}_{\mathrm{pf}}
&= -\hCont\bigl(\dpf^{(2)}\bigr)
\neq 0 \text{ iff } \mathfrak{Q}(\cA) \neq 0.
\label{eq:thqg-VII-shell-pf}
\end{align}
By the genus-$2$ shell activation theorem
(Theorem~\ref{rem:genus2-shell-activation}), the vanishing pattern
$(\sigma,\pi)$ of the last two shells is a complete depth
diagnostic:
\begin{center}
\begin{tabular}{cccc}
$(\sigma,\pi)$ & Class & $r_{\max}$ & Example \\
\hline
$(0,0)$ & G (Gaussian) & $2$ & $\cH_k$ \\
$(1,0)$ & L (Lie/tree) & $3$ & $V_k(\fg)$ \\
$(0,1)$ & C (contact) & $4$ & $\beta\gamma$ \\
$(1,1)$ & M (mixed) & $\infty$ & $\mathrm{Vir}_c$
\end{tabular}
\end{center}
\end{computation}

\begin{remark}[Genus-$2$ as the first diagnostic]
\label{rem:thqg-VII-genus2-diagnostic}
...
\label{subsec:thqg-VII-genus-spectral-sequence}
...
\label{subsubsec:thqg-VII-ss-construction}
...
spectral sequence
\begin{equation}\label{eq:thqg-VII-genus-ss}
E_1^{p,q}
\;\Longrightarrow\;
...
\item \emph{$E_1$~page:}
  \begin{equation}\label{eq:thqg-VII-E1}
  E_1^{p,q}
  \;=\;
...
\label{subsubsec:thqg-VII-E1-detail}

\begin{proposition}[$E_1$~page for a Koszul algebra;
\ClaimStatusProvedHere]
\label{prop:thqg-VII-E1-koszul}
...
\label{rem:thqg-VII-E1-classical}
...
\label{subsubsec:thqg-VII-convergence}
...
\begin{equation}\label{eq:thqg-VII-convergence}
E_\infty^{p,q}
\;\cong\;
...
\begin{equation}\label{eq:thqg-VII-gaussian-degeneration}
...
\subsubsection{The spectral sequence and the shadow tower}
\label{subsubsec:thqg-VII-ss-shadow}
...
$o_{r+1}(\cA)$ projected through the shadow tower.
...
\label{rem:thqg-VII-pbw-vs-genus}
...
\label{subsec:thqg-VII-bootstrap-comparison}
...
\ClaimStatusProvedHere]
\label{thm:thqg-VII-mc-vs-bootstrap}
...
\ClaimStatusProvedHere]
\label{thm:thqg-VII-crossing-from-mc}
\label{rem:thqg-VII-crossing-as-mc}
...
\begin{equation}\label{eq:thqg-VII-crossing-symmetry}
...
\ClaimStatusProvedHere]
\label{cor:thqg-VII-genus-g-bootstrap}
...
\begin{equation}\label{eq:thqg-VII-genus-g-bootstrap}
...
(Theorem~\ref{thm:mc2-bar-intrinsic}) guarantees that
...
\label{rem:thqg-VII-higher-arity-crossing}
...
\label{rem:thqg-VII-modular-as-mc}
...
(Remark~\ref{rem:modular-invariance-tower}), forces the
...
\label{rem:thqg-VII-mc-stronger}
...
constraints alone.  In the conformal bootstrap, this information
is \emph{lost} (there are no known bootstrap constraints at
genus $g \geq 2$); in the MC recursion, it is
\emph{determined} by the higher differentials.
\end{proposition}
...
are uniquely determined by the MC recursion but not by the
conformal bootstrap.  The conformal bootstrap has no general
mechanism for accessing genus $g \geq 2$ constraints beyond
specific examples (e.g.\ the genus-$2$ partition function
of the monster CFT, which is fixed by other means).
\end{proof}
...
\label{subsubsec:thqg-VII-heis-setup}

The rank-$1$ Heisenberg algebra $\cH_k$ has a single generator~$J$
with OPE
\begin{equation}\label{eq:thqg-VII-heis-ope}
...
\begin{equation}\label{eq:thqg-VII-heis-genus-exp}
\Theta_{\cH_k}
\;=\;
...
integration:
\begin{equation}\label{eq:thqg-VII-heis-Fg}
F_g
\;=\;
...
\label{subsubsec:thqg-VII-heis-g0}

\begin{computation}[Genus-$0$ Heisenberg; \ClaimStatusProvedHere]
\label{comp:thqg-VII-heis-g0}
...
\begin{equation}\label{eq:thqg-VII-heis-theta1}
\Theta^{(1)}_{\cH_k}
\;=\;
...
\end{equation}
The free energy is:
\begin{equation}\label{eq:thqg-VII-heis-F1}
F_1
\;=\;
...
\Delta$:
\begin{equation}\label{eq:thqg-VII-heis-bracket-g2}
\tfrac{1}{2}[\Theta^{(1)},\Theta^{(1)}]
\;=\;
...
\begin{equation}\label{eq:thqg-VII-heis-bv-g1}
\Delta(\Theta^{(1)})
\;=\;
...
\begin{equation}\label{eq:thqg-VII-heis-sep-shell-vanish}
...
\begin{equation}\label{eq:thqg-VII-heis-obs2}
\Ob_2
\;=\;
...
\begin{equation}\label{eq:thqg-VII-heis-theta2}
\Theta^{(2)}_{\cH_k}
\;=\;
-\hCont\bigl(\Ob_2\bigr)
\;=\;
...
The free energy is:
\begin{equation}\label{eq:thqg-VII-heis-F2}
F_2
\;=\;
...
\begin{equation}\label{eq:thqg-VII-heis-bracket-g3}
[\Theta^{(1)},\Theta^{(2)}]
\;=\;
k^3 \, F_2 \cdot
...
\medskip\noindent\emph{(b) The BV term.}
\begin{equation}\label{eq:thqg-VII-heis-bv-g2}
\Delta(\Theta^{(2)})
\;=\;
...
form:
\begin{equation}\label{eq:thqg-VII-heis-theta3}
\Theta^{(3)}_{\cH_k}
\;=\;
...
The free energy:
\begin{equation}\label{eq:thqg-VII-heis-F3}
F_3
\;=\;
...
The first bracket:
\begin{equation}\label{eq:thqg-VII-heis-bracket13}
[\Theta^{(1)},\Theta^{(3)}]
\;=\;
k^4\, F_3\,
...
The second bracket:
\begin{equation}\label{eq:thqg-VII-heis-bracket22}
\tfrac{1}{2}[\Theta^{(2)},\Theta^{(2)}]
\;=\;
\tfrac{1}{2}\, k^4\, F_2^2\,
[\lambda_2,\lambda_2]
...
\medskip\noindent\emph{(b) The BV term.}
\begin{equation}\label{eq:thqg-VII-heis-bv-g3}
\Delta(\Theta^{(3)})
\;=\;
...
\begin{equation}\label{eq:thqg-VII-heis-obs4}
\Ob_4
\;=\;
...
\begin{equation}\label{eq:thqg-VII-heis-theta4}
\Theta^{(4)}_{\cH_k}
\;=\;
...
The free energy:
\begin{equation}\label{eq:thqg-VII-heis-F4}
F_4
\;=\;
...
\begin{equation}\label{eq:thqg-VII-heis-F4-value}
F_4
\;=\;
...
\label{subsubsec:thqg-VII-consistency}
...
\begin{equation}\label{eq:thqg-VII-recursion-closed}
\Theta^{(g)}_{\cH_k}
\;=\;
...
&= k^g \cdot
   ...
   \Bigr)
   \otimes \Delta^g.
\label{eq:thqg-VII-inductive-obs}
\end{align}
...
(the shadow tower terminates at weight~$2$, so the convolution
...
\label{subsubsec:thqg-VII-heis-rank-d}

\begin{corollary}[Rank-$d$ Heisenberg; \ClaimStatusProvedHere]
\label{cor:thqg-VII-rank-d}
...
free energy is:
\begin{equation}\label{eq:thqg-VII-rank-d-Fg}
F_g^{(d)}
\;=\;
d \cdot k^g \cdot F_g,
\end{equation}
...
\begin{equation}\label{eq:thqg-VII-rank-d-Zg}
Z_g^{(d)}
\;=\;
...
\label{subsubsec:thqg-VII-table}

\begin{table}[ht]
\centering
\caption{MC recursion data for $\cH_k$ through genus $4$}
\label{tab:thqg-VII-heis-table}
\begin{tabular}{cllll}
\hline
...
\label{subsubsec:thqg-VII-extended-table}
...
\label{rem:thqg-VII-asymptotics}
...
\label{rem:thqg-VII-verification}
...
\label{subsec:thqg-VII-gravity-loops}
...
\label{subsubsec:thqg-VII-grav-interpretation}
...
\label{princ:thqg-VII-gravity-bootstrap}
...
\label{subsubsec:thqg-VII-one-loop}

\begin{theorem}[One-loop gravity from genus-$1$ MC;
\ClaimStatusProvedHere]
\label{thm:thqg-VII-one-loop-gravity}
...
\begin{equation}\label{eq:thqg-VII-one-loop}
Z_1(\cA)
\;=\;
...
algebra at level~$k$:
\begin{equation}\label{eq:thqg-VII-one-loop-heis}
Z_1(\cH_k)
\;=\;
\exp\!\bigl(k/24\bigr)
\;=\;
...
gives $\kappa \cdot F_1 = k/24$.
\end{proof}

\subsubsection{The genus-$g$ gravitational amplitude}
\label{subsubsec:thqg-VII-g-loop}
...
\begin{equation}\label{eq:thqg-VII-g-loop}
\mathcal{A}_g(\cA)
\;=\;
...
\label{subsubsec:thqg-VII-non-renormalization}

\begin{theorem}[Non-renormalization from the MC recursion;
\ClaimStatusProvedHere]
\label{thm:thqg-VII-non-renormalization}
...
\begin{equation}\label{eq:thqg-VII-non-renorm}
\mathcal{A}_g(\cA)
\;=\;
\kappa(\cA)^g \cdot F_g,
\end{equation}
...
\item The partition function at all genera is:
  \begin{equation}\label{eq:thqg-VII-non-renorm-Z}
  Z_g(\cA)
  \;=\;
...
shadow tower terminates at weight~$2$, the convolution algebra
...
\label{subsubsec:thqg-VII-beyond-gaussian}

\begin{remark}[Gravitational corrections from the shadow tower]
\label{rem:thqg-VII-grav-corrections}
...
precisely:
\begin{equation}\label{eq:thqg-VII-complexity-bound}
\mathcal{A}_g(\cA)
\;\in\;
...
from the shadow tower, contracted with the tautological classes
of the moduli space.
\end{proposition}

\begin{proof}
The shadow tower $\Theta^{\leq r_{\max}} = \Theta_\cA$
...
\label{subsubsec:thqg-VII-grav-bootstrap}

\begin{remark}[Towards a gravitational modular bootstrap]
\label{rem:thqg-VII-grav-bootstrap}
\index{gravitational modular bootstrap}
The MC recursion provides the foundation for a
\emph{gravitational modular bootstrap programme}: a systematic
determination of all-genus gravitational amplitudes from the
boundary OPE data.  The ingredients are:
\begin{enumerate}[label=(\arabic*)]
...
\label{rem:thqg-VII-string-comparison}
...
complexity}
\label{subsubsec:thqg-VII-depth-gravity}

\begin{theorem}[Shadow depth classifies gravitational theories;
\ClaimStatusProvedHere]
\label{thm:thqg-VII-depth-classification}
\index{shadow depth!gravitational classification}
The shadow depth $r_{\max}(\cA)$ classifies the
\emph{gravitational complexity} of the HT theory dual to~$\cA$:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $r_{\max} = 2$ \textup{(}Gaussian\textup{)}: the bulk
  gravity is \emph{free} --- the gravitational partition
  function is a Fredholm determinant, the genus expansion is
  controlled by a single scalar $\kappa$, and the genus
  spectral sequence collapses at $E_1$.  Example: Heisenberg
  $\cH_k$ (free boson in the bulk).
\item $r_{\max} = 3$ \textup{(}Lie/tree\textup{)}: the bulk
  gravity has \emph{cubic vertices} --- the genus-$2$ amplitude
  receives cubic shadow corrections, the genus spectral
  sequence collapses at $E_2$.  Example: affine $V_k(\fg)$
  (Chern--Simons theory in the bulk).
\item $r_{\max} = 4$ \textup{(}contact\textup{)}: the bulk
  gravity has \emph{quartic vertices} --- the planted-forest
  shell is activated at genus~$2$, the genus spectral
  sequence collapses at $E_3$.  Example: $\beta\gamma$
  (topological $B$-model in the bulk).
\item $r_{\max} = \infty$ \textup{(}mixed\textup{)}: the bulk
  gravity has \emph{all-order vertices} --- the genus
  spectral sequence does not degenerate.  Example:
  $\mathrm{Vir}_c$ (full gravity in the bulk).
\end{enumerate}
\end{theorem}
...
sits in the M~class: its infinite shadow tower reflects the
...
\label{rem:thqg-VII-uv-finiteness}
