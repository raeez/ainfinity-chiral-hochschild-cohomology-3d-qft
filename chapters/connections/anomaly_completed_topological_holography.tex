...
higher operations and Wess--Zumino consistency in holomorphic-topological perturbation theory \cite{GKW25};
...
\noindent\textbf{Strict algebraic core.}
The strict dg part of this appendix consists of:
\begin{enumerate}
...
\end{enumerate}
Each item is obtained below from the universal property of the skew
transgression algebra and from the one-sided resolution it forces.

\medskip

\noindent\textbf{Physical bridge data.}
The identifications \(B\simeq A^{!}\), \(\Theta=\Theta_{g}\), and
\(\mathfrak{G}_{g}(B_{\Theta})\) with the higher-genus line-side
package of a twisted field theory require the corresponding
bulk--boundary, anomaly, and line-operator bridge maps. The sources
\cite{Zeng23,CDG20,DNP25,costello-gaiotto,CP2020} supply the bridge
maps used in the applications below.
...
The map \(\mu_{M}\) is surjective, since \(1\otimes m\) maps to \(m\).
...
The algebra \(B_{\Theta}\) is a skew polynomial extension, not \(B\) tensored with an exterior algebra.
...
\[
\kappa_{\mathrm{eff}}
=\kappaChHodge(\mathrm{Vir}_{26})+\kappaChHodge(\mathrm{ghost})
=13+(-13)=0,
\]
and $\gamma$ extends to a flat connection at all genera.
For $\mathcal W_3$ at $c=\alpha_3=100$, the analogous anomaly
cancellation $\kappa_{\mathrm{eff}}=\kappaChHodge(\mathcal W_3{}_{100})
+\kappaChHodge(\mathrm{ghost})=0$ gives the same conclusion.
\end{remark}

\begin{remark}[$\Delta_{5}$ as the one-loop-forced output of paramodular anomaly cancellation]
...
The strict content is the uniqueness of \(B_{\Theta_{g}}\) and
\(\mathfrak{G}_{g}(B_{\Theta_{g}})\) as dg constructions with
the universal properties specified above.
They package:
\begin{itemize}
...
\begin{remark}[Universal $\Phi$-trace identity on the $\mathrm{K3}\times T^{2}$ specialisation]
...
\begin{remark}[Bi-based Ran-space datum with $24$ nearby-cycles components]
...
\subsection{Strict-to-physical bridge conjectures}
% label removed: subsec:tholog-frontier-conjectures

The strict algebra above leaves three bridge assertions between the
transgression model and the full holomorphic-topological field theory.

\begin{conjecture}[Koszul-dual bulk completion]
...
(Theorem~\textup{\ref{thm:lines_as_modules}} on its
essential-realizability locus).
Then:
...
\item \(\Theta_g\) is central because \(\kappaChHodge(\cA)\) is scalar
...
\item \(\Theta_g\) is primitive in \(B_\Theta^{(g)}(\cA)\)
 ...
 \(\kappaChHodge(\cA)\cdot\omega_g\) pushes forward to a primitive
...
The genus-\(g\) curvature \(\kappaChHodge(\cA) \cdot \omega_g\) is an
...
filtered curved comparison between
\(\mathrm{C}^\bullet_{\mathrm{ch,top}}(\cA)\) and the genus-\(g\)
chiral Hochschild descent object.  Its special fibre at
\(\omega_g=0\) is an ordinary filtered quasi-isomorphism, and it is
not an ordinary quasi-isomorphism at nonzero curvature.  The cyclic
cohomology class \(\kappaChHodge(\cA) \cdot \omega_g\) maps under the
Koszul duality functor \(\cA \mapsto \cA^!\) to a class
...
Centrality: \(\kappaChHodge(\cA)\) is scalar (acts as multiplication by a
...
\(\kappaChHodge(\cA)\cdot\omega_g\) acts on cogenerators by the scalar
\(\kappaChHodge(\cA)\) and is therefore primitive in
...
\begin{conjecture}[Surface-completed line-side algebra]
...
(Theorem~\ref{thm:lines_as_modules} on its
essential-realizability locus) lifts to the genus-completed setting:
at genus~\(g\), line operators are modules for
\(\mathfrak{G}_g((A_\partial^!)_{\Theta_g})\).
...
\begin{conjecture}[Boundary reconstruction]
...
\item The genus anomaly \(\Theta_{g}\) is reconstructed from a
 ...
 \(\kappaChHodge(\cA) \cdot \omega_g\) to
...
\begin{remark}[Anomaly-holographic bridge data]
...
state the bridge data for the anomaly-holographic machine of this chapter.
...
\begin{remark}[Strict and bridge layers]
% label removed: rem:tholog-layer-separation
The logical separation is:
\begin{itemize}
\item Sections~\ref{subsec:tholog-transgression-algebra}--\ref{subsec:tholog-handle-operators-on-morphisms} give the strict transgression, genus-Clifford, and localization algebra.
...
\item Conjectures~\ref{conj:tholog-koszul-dual-bulk-completion}--\ref{conj:tholog-boundary-reconstruction} state the additional bridge identifications needed to pass from the strict model to all HT theories.
\end{itemize}
\end{remark}

\begin{remark}[Class-$\mathcal S$ $A_{1}$ on $\Sigma_{0,24}$ as the $4d$ parent theory]
...
\begin{remark}[Bridge to the modular holography programme]
...
\(\Rring(\Mbar_{g,n})\) is expected to act on the modular state spaces
...
chapter is the expected algebraic target for this action: the handle
...
$B=\cA^!=\Sym^{\mathrm{ch}}(V^*)$ (with $\kappaChHodge(\cA^!)=-k$), $\Theta_{\cH}(z)=k\,\Omega_{\cH}/z$ (rank-one coefficient $k/z$),
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappaChHodge(\mathrm{Vir}_{13})=\tfrac{13}{2}$ (self-duality
 at $c=13$ fixes the curvature to the half-complementarity value);
\item $\kappaChHodge(\mathrm{Vir}_c)=c/2$, hence the curvature
...
\end{enumerate}
The shifted curvature
\(\kappaChHodge(\mathrm{Vir}_c)-13/2=(c-13)/2\) measures the
\emph{distance from self-duality}; the unshifted curvature is the
scalar obstruction to extending the genus-$0$ Koszul duality to
genus~$1$.
...
$\Theta=\kappaChHodge(\mathrm{Vir}_c)\cdot\omega_1$, and the
strict central-shadow transgression algebra
\[
B_\Theta
\;=\;
...
and $\mathrm{Vir}_{26-c}$. If instead one works directly in the
curved class-$\mathsf M$ bar complex, where
\[
d_{B}^{2}=[\Theta_{\mathrm{Vir}},-],
\]
the flattening generator satisfies the curved Maurer--Cartan equation
of Proposition~\ref{prop:tholog-curved-mc-convention},
\[
d\eta+\eta^{2}+\Theta_{\mathrm{Vir}}=0.
\]
The displayed \(d\eta=\kappa\omega_1\) formula is therefore the
strict central-shadow convention; the \(u=\eta^{2}\) term is retained
as a secondary central class, not forgotten. A $B_\Theta$-module $M$
...
Gauging ghost spins &
 ...
 $2, \tfrac32, \tfrac32, 1$ &
 $2, 3$ \\[2pt]
$c^{\mathrm{gauge}}_{\mathrm{ghost}} = -(c+c')$ &
 $-16$ &
 $-50$ &
 $-100$ \\[2pt]
$\kappa=0$ at & $k=-3$ (crit.) &
...
\end{equation}
where $\kappa_{\mathrm{ghost}}(f,k)$ is \emph{defined} by this
transformation law as the curvature deficit of the reduction; its
value at the minimal slot is the open Bershadsky--Polyakov
$\kappa^{\mathrm{Hodge}}$ problem, and no closed formula for it is
asserted here. The deficit must not be conflated with the ghost
\emph{central charges}: a $bc$ system's central charge is determined
by its conformal weights alone and is level-independent. The
conductor cross-check at each vertex is the level-independent total
$c^{\mathrm{gauge}}_{\mathrm{ghost}} = -(c+c')$ of the $bc$ system
gauging the $W$-symmetry: eight spin-$1$ pairs give $-16$ at
level~$0$; spins $(2, \tfrac32, \tfrac32, 1)$ give
$-26 - 11 - 11 - 2 = -50$ at level~$1$; spins $(2,3)$ give
$-26 - 74 = -100$ at level~$2$.
...
the affine-level datum. The fixed locus of the duality involution
varies across the diagram: at level~$0$ it is $k=-3$
(critical), at level~$1$ it is $c=25$, and at level~$2$ it
is $c=50$. This fixed locus is a zero of the shifted duality
coordinate, not necessarily a zero of the unshifted curvature.
...
$c_{\mathrm{sum}}(3,1)=50$
(Vol~I, Theorem~\ref*{V1-thm:bp-koszul-conductor-polynomial}).
...
Assume the BP Verdier-companion duality of Volume~I,
$(\mathcal B^k)^! \simeq \mathcal B^{-k-6}$; the central-charge
conductor identity is proved there:
\[
c(\mathcal B^k)+c(\mathcal B^{-k-6})=50.
\]
...
Since $(\mathcal B^k)^!=\mathcal B^{-k-6}$ and the central-charge
sum is $c+c'=50$, the midpoint $c=25$ is attained only at the
complex levels $k=-3\pm 2i$, and the curvature is generically
nonzero.
The explicit formula for $\kappaChHodge(\mathcal B^k)$ is an open
bar-complex computation:
it would receive contributions from both the
...
\begin{remark}[Mukai-doubling Koszul conductor $K^{\kappaChHodge}=8$ on the $\mathcal B$-family]
...
$\kappaChHodge$ denote the curvature class
$\kappaChHodge(\mathcal B^{k})$ entering $\Theta$. Then the
...
conductor $K^{\kappaChHodge}=8$ from the
...
\cite{Drinfeld1990,Borcherds1992,GritsenkoNikulin1998}. Compare
...
Mukai-doubling identity $K^{\kappaChHodge} = 8 = 2\cdot 4$
...
\label{rem:anomtop-class-s-central-charge-k3}
...
\label{rem:anomtop-c214-spectrum}
...
algebra at $\Sigma_{0,24}$, Vol~I's $c_{2d}=-214$
class-$\mathrm M$ shadow-tower decomposition specialises to
\begin{align*}
...
the holomorphic chiral partition function is the remaining
modular-closure hypothesis beyond the
$M_{24}$-averaged/$\Phi_{10}^{\mathrm{un}}$-denominator locus; the
gravitational-anomaly form obstructs exact closure (mock shadows
supply the compensator), consistent with Harvey--Moore 1996
threshold integrals. Compare Vol~I
...
1997 evaluation. Assuming full modular closure as in
Remark~\ref{rem:anomtop-T2-modular}, the BPS counting closes.
The effective value $c_{\mathrm{eff}} = -166$ factors as
