...
This appendix isolates that algebraic core, corrects it at first principles where necessary, and pushes it one step farther.

The input from the surrounding literature is the following constellation of ideas:
...
is the dg-shifted Yangian $\cA^!$, the primitive class
...
(Computations~\ref*{comp:heisenberg-holographic-datum},
\ref*{comp:affine-holographic-datum},
\ref*{comp:virasoro-holographic-datum},
\ref*{comp:w3-holographic-datum}).
...
For Virasoro at $c=26$, $\kappa=0$ and $\gamma$ extends to a
flat connection at all genera.
For $\mathcal W_3$ at $c=50$, the same holds with
$\kappa(\mathcal W_3{}_{50})=0$.
\end{remark}
...
\begin{theorem}[Koszul-dual bulk completion; \ClaimStatusProvedHere]
\label{thm:tholog-koszul-dual-bulk-completion}
...
\item \(\Theta_g\) is central because \(\kappa(\cA)\) is scalar
  (Volume~I, Theorem~D\textsubscript{scal}): the Hochschild bridge
  preserves centrality (scalar coderivations map to central
  cocycles).
\item \(\Theta_g\) is primitive because the bar coalgebra
  \(\barB(\cA)\) is a bialgebra and the curvature class respects
  the coproduct: \(\Delta(\kappa \cdot \omega_g) =
  \kappa \cdot \omega_g \otimes 1 + 1 \otimes \kappa \cdot \omega_g\)
  (the curvature is a scalar coderivation, hence primitive by
  Proposition~\textup{\ref{prop:R-canonical-vol2}(iii)}).
...
on the Koszul locus (Volume~I, Theorem~H).

Centrality: \(\kappa(\cA)\) is scalar (acts as multiplication by a
constant on every bar component), so \(\Theta_g\) commutes with
all elements of \(\cA^!\).  Primitivity: the curvature is a scalar
coderivation of the cofree coalgebra
(Proposition~\ref{prop:curved-R-factorization}(iii)), and scalar
coderivations of cofree coalgebras are primitive
(\(\Delta(\Theta) = \Theta \otimes 1 + 1 \otimes \Theta\) because
\(\Theta\) acts on cogenerators by a scalar).  The Hochschild
bridge preserves both properties because it is a filtered
...
\begin{theorem}[Surface-completed line algebra; \ClaimStatusProvedHere]
\label{thm:tholog-surface-completed-line-algebra}
In the setting of Theorem~\ref{thm:tholog-koszul-dual-bulk-completion},
the genus-\(g\) refinement of the line-operator algebra is
\[
...
\begin{theorem}[Boundary reconstruction; \ClaimStatusProvedHere]
\label{thm:tholog-boundary-reconstruction}
...
\begin{remark}[The complete anomaly-holographic machine]
\label{rem:tholog-complete-machine}
Theorems~\ref{thm:tholog-koszul-dual-bulk-completion}--\ref{thm:tholog-boundary-reconstruction}
complete the anomaly-holographic machine of this chapter.
The strict M-level algebra
(Sections~\ref{subsec:tholog-transgression-algebra}--\ref{subsec:tholog-handle-operators-on-morphisms})
now applies to every boundary chiral algebra arising from a 3d HT
theory satisfying (H1)--(H4), because:
...
\end{enumerate}
The boundary determines the bulk at all genera---the holographic
...
\item Theorems~\ref{thm:tholog-koszul-dual-bulk-completion}--\ref{thm:tholog-boundary-reconstruction} close the H-level programme: the M-level algebra applies to all HT theories via the bridges established in this volume.
\end{itemize}
\end{remark}
...
The anomaly-completed transgression theory of this chapter admits a
...
This action upgrades numerical descendant integrals to operator-valued
tautological-ring representations, and tautological relations on
\(\Mbar_{g,n}\) become \emph{protected operator identities} in the
holographic theory.
The genus-Clifford completion \(\mathfrak{G}_{g}(B_{\Theta})\) of this
chapter provides the algebraic target for this action: the handle
operators and secondary charge \(u=\eta^{2}\) are the simplest
...
$B=\cA^!=\cH_{-k}$, $\Theta=k/z$ (the $r$-matrix),
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappa(\mathrm{Vir}_{13})=0$ (self-duality implies
  vanishing curvature);
\item $\kappa(\mathrm{Vir}_c)\neq 0$ for $c\neq 13$
  generically;
\item $\kappa$ is anti-symmetric under the duality involution:
  $\kappa(\mathrm{Vir}_{26-c})=-\kappa(\mathrm{Vir}_c)$
  (Theorem~D, anti-symmetry of the modular characteristic).
\end{enumerate}
...
\item On the strict locus ($\eta^2=0$):
  the curvature vanishes ($c=13$), and the
  Virasoro is self-dual.  The genus-$g$ extension
  degenerates to an exterior algebra
...
(Vol~I, Computation~\ref*{comp:sl3-ds-hierarchy}), the
...
(critical), at level~$1$ it is $c=38$, and at level~$2$ it
...
(Vol~I, Proposition~\ref*{prop:transport-propagation-frontier}):
...
(Vol~I, Theorem~\ref*{thm:hook-transport-corridor}),
at generic level:
...
(Vol~I, Proposition~\ref*{prop:bp-duality}).
...
product).  The transgression algebra is
\[
B_\Theta
\;=\;
\frac{B*\Bbbk\langle\eta\rangle}
...
\quad|\eta|=1.
\]
The secondary anomaly is $u=\eta^2\in Z^2(B_\Theta)\cap Z(B_\Theta)$.

The new feature for non-principal $W$-algebras: the $B_\Theta$-modules
carry data from \emph{both} $\mathcal W_\lambda^k$ and its Koszul
dual $\mathcal W_{\lambda^t}^{k^\vee}$.  A $\Theta$-neutralisation
...
\item \emph{Away from the $u$-locus} ($\eta^2$ invertible):
  ...
  \cong\operatorname{Mat}_{2^g}(B_\Theta[\eta^{-2}])$
  (Morita triviality).
  Higher genus is invisible: the $\mathcal W_\lambda^k$-module
  structure at genus~$g$ is Morita-equivalent to genus~$0$.
  This is the regime where the cross-orbit duality is
  ``unobstructed'': both $\mathcal W_\lambda^k$ and
  $\mathcal W_{\lambda^t}^{k^\vee}$ contribute to the
  full module category without interference.

\item \emph{On the strict locus} ($u=\eta^2=0$):
  $\mathfrak G_g(B_\Theta)/(\eta^2)
  \cong B/(\Theta)\otimes\Lambda^{2g+1}$.
  The higher-genus structure degenerates to an exterior-algebra
  extension.  On this locus, the curvature vanishes
  ($\kappa=0$) and the cross-orbit duality collapses: the
  algebra and its dual become indistinguishable at the level
  of the anomaly-completed module category.
\end{enumerate}
\end{construction}

\begin{computation}[Bershadsky--Polyakov algebra:
$\mathfrak{sl}_3$, hook $(2,1)$; \ClaimStatusProvedHere]
...
Central charge identity: $c(\mathcal B^k)+c(\mathcal B^{-k-6})=76$.
...
central-charge sum is $c+c'=76$, the curvature vanishes
at the self-dual point $c=38$ and is generically nonzero.
The explicit formula for $\kappa(\mathcal B^k)$ is determined
by the bar complex computation
(Computation~\ref*{comp:sl3-ds-hierarchy} in Vol~I):
...
The anomaly class is $\Theta=\kappa\cdot\omega_1\in Z^2(B)$,
...
The transgression algebra $B_\Theta$ carries a single
differential $d\eta=\Theta=\kappa\cdot\omega_1$ that
...
\end{enumerate}
The neutralisation $h_M$ is the datum that resolves both
obstructions simultaneously: it is a chain-level witness
...
\end{tabular}
\end{center}
The cross-orbit anomaly completion is the geometrical
