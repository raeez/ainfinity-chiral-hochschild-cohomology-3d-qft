...
The common algebraic music running through the surrounding chapters and the source corpus is by now unmistakable:
bulk holomorphic-topological operator algebras are classical or shifted-Poisson/chiral objects, boundary algebras are their quantum and often noncommutative chiral shadows, line operators are controlled by Koszul-dual algebras, and higher operations encode the deformation and anomaly data that obstruct a naive strictification of the bulk--boundary dictionary.
The most economical mathematical form of this picture is not an equality of algebras but a \emph{duality plus anomaly completion}.  
The purpose of this appendix is to isolate that algebraic core, correct it at first principles where necessary, and push it one step farther.

The fundamental input coming from the surrounding literature is the now-standard constellation of ideas:
chiral algebras as the quantum avatars of coisson structures \cite{BD04};
higher-dimensional chiral/factorization Koszul duality \cite{FG12};
the appearance of derived centers and derived tensor products in topological field theory and interval compactification \cite{costello-gaiotto,CP2020,DNP25};
higher operations and Wess--Zumino type consistency in holomorphic-topological perturbation theory \cite{GKW24};
boundary chiral algebras in holomorphic twists \cite{CDG20};
the Koszul-dual interpretation of twisted holography \cite{Zeng23};
and the control of line operators by strict or homotopy-coherent Yangian-type structures \cite{DNP25}.  
The new theorem package below turns this constellation into a strict dg-algebraic machine.
...
Whenever we identify \(B\) with a bulk Koszul-dual algebra \(A^{!}\), or identify \(\Theta\) with a genuine physical genus anomaly \(\Theta_{g}\), or interpret \(\mathfrak{G}_{g}(B_{\Theta})\) as the full higher-genus line-operator algebra of a twisted field theory, we have crossed from strict algebra into a physical/topological-holographic programme.
Those identifications are powerfully suggested by \cite{Zeng23,CDG20,DNP25,costello-gaiotto,CP2020}, but outside the strict models below they should be read with the appropriate semantic caution.
...
The first correction is foundational and should be stated plainly.

A first impulse might be to adjoin to \(B\) an odd \emph{supercentral} generator \(\eta\) with \(d\eta=\Theta\).
That is the wrong object for our purposes.
...
The correct object is instead a \emph{skew polynomial extension}:
\(\eta\) is required to graded-commute with the original algebra \(B\), but it remains a genuinely free odd generator with respect to its own powers.
Equivalently, one adjoins an odd variable over \(B\) via the sign automorphism
\[
\sigma:B\longrightarrow B,\qquad \sigma(b)=(-1)^{|b|}b.
\]
This is the smallest first-principles repair that preserves the desired universal property while leaving room for a nontrivial square
\[
u:=\eta^{2}.
\]

A second correction is equally important.
Because the extension is a skew/Ore extension rather than an ordinary tensor product with an exterior algebra, the most honest exact triangle is \emph{one-sided} and module-theoretic.
The naive untwisted bimodule resolution is not the right first-principles object.
The correct replacement is a two-term free resolution of each module, from which the transgression formulas for \(R\!\operatorname{Hom}\) and derived tensor products follow cleanly.
This is the point at which the theory becomes both stronger and more correct.
...
We now add genuine surface data.
The guiding principle is simple: a genus-\(g\) surface carries \(2g\) basic one-cycles, and in the anomaly-completed setting their algebraic shadow should be an odd Clifford algebra whose anticommutator is controlled by the secondary class \(u\).
...
In particular,
\[
D\!\big(\mathfrak{G}_{g}(B_{\Theta})[u^{-1}]\text{\rm -mod}\big)
\simeq
D\!\big(B_{\Theta}[u^{-1}]\text{\rm -mod}\big).
\]
\end{theorem}
...
We now package it in the form most naturally suited to the holographic applications suggested by the source corpus.
...
Theorem~\ref{thm:tholog-dg-shifted-yangian-completion} says that if a strict dg-shifted Yangian governs line operators before anomaly completion, then the correct genus-anomaly-corrected line algebra is \(Y_{\Theta}\), not \(Y\).
...
This recovers, in a precise hypothesis-driven form, the vanishing phenomenon discussed heuristically earlier in the thread.
\end{remark}
...
\]
This is the clean Casimir law suggested earlier in the thread; its precise realization depends on the ambient affine model.
\end{remark}
...
The Heisenberg/Fock pattern discussed earlier in the thread is exactly of the form treated by Proposition~\ref{prop:tholog-scalar-shadow}: the genus anomaly is universal across the family.
...
This is, in a strict algebraic sense, the Platonic ideal of the anomaly-completed topological-holographic package that the surrounding source corpus points toward.
...
Let \(A_{\partial}\) be a boundary chiral/factorization algebra arising from a twisted or holomorphic-topological field theory, and let \(A_{\partial}^{!}\) be a strict dg Koszul-dual bulk/defect/line algebra.
Then the correct strict genus-\(g\) bulk algebra is
\[
(A_{\partial}^{!})_{\Theta_{g}}
\]
for a canonically attached primitive central class \(\Theta_{g}\in Z^{2}(A_{\partial}^{!})\cap Z(A_{\partial}^{!})\).
\end{conjecture}

\begin{conjecture}[Surface-completed line algebra]
\label{conj:tholog-surface-completed-line-algebra}
In the setting of Conjecture~\ref{conj:tholog-koszul-dual-bulk-completion}, the correct strict higher-genus refinement of the line-operator algebra is
\[
\mathfrak{G}_{g}\!\big((A_{\partial}^{!})_{\Theta_{g}}\big).
\]
Moreover, the full higher-genus line category is supported on the secondary-anomaly cone
\[
u_{g}=0
\]
and is derived Morita-trivial away from it.
\end{conjecture}

\begin{conjecture}[Boundary reconstruction]
\label{conj:tholog-boundary-reconstruction}
Under a derived bulk--boundary equivalence
\[
A_{\partial}^{!}\simeq HH^{\bullet}(A_{\partial}),
\]
the genus anomaly \(\Theta_{g}\) is reconstructed from a canonical boundary Hochschild class
\[
\Theta_{g,\partial}\in HH^{2}(A_{\partial}),
\]
and the induced surface-completed bulk algebra
\[
\mathfrak{G}_{g}\!\big((A_{\partial}^{!})_{\Theta_{g}}\big)
\]
is equivalent to the universal genus-completed endomorphism algebra of the boundary theory.
\end{conjecture}
...
\item Section~\ref{subsec:tholog-frontier-conjectures} isolates the genuinely programme-level holographic claims.
\end{itemize}
This keeps the M-level/H-level boundary mathematically sharp.
\end{remark}

\subsection{Coda}
\label{subsec:tholog-coda}

The moral is simple and strong.

Topological holography, at the strict algebraic level accessible here, is not best thought of as an isomorphism between bulk and boundary operator algebras.
It is best thought of as a \emph{Koszul-dual anomaly-completed transgression theory}.
...
The correct higher-genus story is therefore not uniformly new:
away from the secondary-anomaly cone it collapses Morita-theoretically,
while on the cone it blooms into an honest exterior handle algebra.

That is the strict algebraic music underlying the source corpus, now stated from first principles and repaired where repair was needed.
