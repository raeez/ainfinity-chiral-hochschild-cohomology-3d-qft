...
\]
The leading-order formula $R(z)=1+\Omega/z+O(z^{-2})$
identifies $\Omega$ as the collision residue, consistent with
...
Weierstrass function, exactly as predicted by the shadow tower:
...
\begin{remark}[Consistency with the modular shadow tower]
...
=k\dim\mathfrak{g}/(k+h^\vee)=3k/(k+2)$
...
The computations above identify the reduced HT monodromy for $\widehat{\fg}_k$ with the eigenvalues of the quantum group $R$-matrix. We now prove this identification as a theorem, valid for every simple Lie algebra $\fg$ at generic level, by combining three inputs: one-loop exactness of the affine half-space BV theory, the Drinfeld--Kohno theorem, and the spectral $R$-matrix of Section~\ref{sec:spectral_braiding}.
...
Let $\fg$ be a simple Lie algebra with dual Coxeter number $h^\vee$, Killing form $\kappa$, and split Casimir $\Omega \in \fg \otimes \fg$. Let $k \in \CC$ with $k \neq -h^\vee$. Consider the affine Kac--Moody vertex algebra $V^k(\fg)$ as a logarithmic $\SCchtop$-algebra via the physics bridge (Theorem~\ref{thm:physics-bridge}).

\begin{proposition}[Affine line operators are resolved in degree $0$; \ClaimStatusProvedHere]
\label{prop:affine-resolved}
Let $V_1, \dots, V_n$ be finite-dimensional $\fg$-modules, viewed as evaluation modules for the dg-shifted Yangian $\cA^!$ at spectral positions $z_1, \dots, z_n$. Then the tree-level BRST complex $(E_n, Q_{0,n})$ is resolved in degree $0$: $H^j(E_n, Q_{0,n}) = 0$ for $j \neq 0$.
\end{proposition}
...
\begin{enumerate}[label=\textup{(\roman*)}]
  ...
  \]
  \item By the Drinfeld--Kohno theorem, this monodromy factors through the quantum group\/ $U_q(\fg)$ at $q = e^{i\pi/(k + h^\vee)}$: the representation $\rho_n^{\mathrm{HT}}$ is equivalent to the braided tensor product representation of\/ $\mathrm{Rep}_q(\fg)$ on $V_1 \otimes \cdots \otimes V_n$.
  \item The spectral $R$-matrix $R_{V_i, V_j}(z)$ of Section~\textup{\ref{sec:spectral_braiding}}, restricted to evaluation modules, equals the quantum group $R$-matrix: the braided monoidal structure on the reduced HT line category coincides with the braided monoidal structure on $\mathrm{Rep}_q(\fg)$.
\end{enumerate}
\end{theorem}
...
\medskip
\noindent\textbf{(ii).} The Drinfeld--Kohno theorem (\cite{Dri89}, Theorem~3.2; \cite{KL93}) asserts that the monodromy representation of the KZ connection at level $k$, acting on tensor products of finite-dimensional $\fg$-modules, factors through the quantum group $U_q(\fg)$ at $q = e^{i\pi/(k + h^\vee)}$. The braiding $\sigma_{ij}^2 \mapsto \exp(-2\pi i \cdot \Omega_{ij}/(k + h^\vee))$ on elementary generators matches the universal $R$-matrix of $U_q(\fg)$ acting on $V_i \otimes V_j$. Since $\rho_n^{\mathrm{HT}} = \rho_n^{\mathrm{KZ}}$, the same factorization holds for the HT monodromy.

\medskip
\noindent\textbf{(iii).} The spectral $R$-matrix of Definition~\ref{def:spectral-braiding} computes the braiding on the boundary line category $\cC_\partial$ via configuration integrals over $\FM_2(\CC)$ (Theorem~\ref{thm:braided-category}). On evaluation modules, this $\FM_2$-integral reduces: by the same one-loop collapse mechanism as in Theorem~\ref{thm:reduced-equals-kz}, the full $A_\infty$ exchange path reduces to the binary exchange governed by the Casimir. The classical limit $r(z) = \Omega/z + O(z^{-2})$ (Proposition~\ref{prop:field-theory-r}) exponentiates to the half-monodromy $R_{ij}(z) = \exp(-\pi i \cdot \Omega_{ij}/(k + h^\vee))$ of the KZ connection. By Remark~\ref{rem:spectral-R-dk-specialization}, this specialization is exactly the Drinfeld $R$-matrix governing the DK equivalence.

Combining: on evaluation modules, the spectral $R$-matrix equals the KZ monodromy, which equals the quantum group $R$-matrix. The associator $\Phi_{\mathrm{HT}}$ of Definition~\ref{def:ht-assoc} likewise reduces to the Drinfeld associator $\Phi_{\mathrm{KZ}}$. The pentagon and hexagon identities (Theorems~\ref{thm:pentagon} and~\ref{thm:hexagon}) are therefore the pentagon and hexagon of $\mathrm{Rep}_q(\fg)$, and the two braided monoidal categories coincide.
\end{proof}
...
The bar complex of $V^k(\mathfrak{sl}_2)$, viewed as a logarithmic $\SCchtop$-algebra, produces a flat connection on configuration spaces by Theorem~\ref{thm:synthesis}. By Theorem~\ref{thm:affine-monodromy-identification}(i)--(ii), the monodromy of this connection on evaluation modules equals the KZ monodromy, which factors through $U_q(\mathfrak{sl}_2)$ at $q = e^{i\pi/(k+2)}$ via the Drinfeld--Kohno theorem. In particular, the bar complex computes the braid-group representation
\[
  \rho_n^{\mathrm{HT}} = \rho_n^{\mathrm{KZ}} \colon B_n \longrightarrow \Aut(V^{\otimes n})
\]
on tensor powers of any finite-dimensional $\mathfrak{sl}_2$-module~$V$. The integration domain $\FM_n(\CC) \times \Conf_n(\RR)$ is the Swiss-cheese operation space: the $\FM_n(\CC)$ factor carries the bar differential and the $\Conf_n(\RR)$ factor carries the $E_1$ coproduct implementing the group-like expansion.

\medskip
\noindent\textbf{Step 2 (Braid closure via the Reshetikhin--Turaev construction).}
The colored Jones polynomial of a framed knot~$K$, presented as the closure of a braid $\beta \in B_n$, is obtained by applying the quantum trace $\operatorname{tr}_q = \operatorname{tr}(K^{2\rho} \,\cdot\,)$ to the braid-group representation factoring through $U_q(\mathfrak{sl}_2)$:
\[
  J_K(q) \;=\; \operatorname{tr}_q\bigl(\rho_n^{\mathrm{KZ}}(\beta)\bigr).
\]
This is the Reshetikhin--Turaev construction \cite{RT90}: $\mathrm{Rep}_q(\mathfrak{sl}_2)$ is a ribbon category (the ribbon element is $v = K^{-1}u$ where $K = q^H$ and $u = \sum S(b_i)a_i$), and the quantum trace implements the Markov closure that converts a braid representation into a framed link invariant. This step is a classical theorem requiring no new input.

\medskip
\noindent\textbf{Conclusion.}
Since $\rho_n^{\mathrm{HT}} = \rho_n^{\mathrm{KZ}}$ (Step~1), the quantum trace applied to the bar-complex monodromy computes the same invariant as the quantum trace applied to the KZ monodromy---namely, $J_K(q)$.
\end{proof}
...
$E_1$ coproduct on $\Conf_k(\RR)$ & Tensor product & Monoidal structure \\
Reduced HT associator & KZ associator & Drinfeld associator \\
\end{tabular}
\end{center}
The bulk-to-boundary functor is the half-space quantization of Theorem~\textup{\ref{thm:affine-half-space-bv}}. The boundary-to-line functor is Koszul duality: $\cA^! \simeq Y_\hbar(\fg)$. The identification of the line category with the quantum group representation category is Theorem~\textup{\ref{thm:affine-monodromy-identification}}.
\end{corollary}
