...
\[
  \omega_\Gamma^{\mathrm{ren}} = \lim_{\varepsilon \to 0} R_\Gamma(\omega_{\Gamma,\varepsilon})
\]
exists. The key point is that the subtraction terms $T_F(\omega_{\Gamma,\varepsilon})$ for any forest $F$ involve only the renormalized forms $\omega_\gamma^{\mathrm{ren}}$ for proper divergent subgraphs $\gamma \subsetneq \Gamma$ (which exist by the inductive hypothesis, since $L(\gamma) < L(\Gamma)$ for every proper subgraph) composed with the reduced graph $\Gamma/\gamma$. This closes the induction.
