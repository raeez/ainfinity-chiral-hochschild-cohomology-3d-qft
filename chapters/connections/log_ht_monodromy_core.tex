...
\begin{enumerate}[label=\textup{(\arabic*)}]
 ...
 \item When the internal line complex is classically resolved in degree $0$ and a compatible Hodge contraction is chosen, the superconnection reduces to an ordinary logarithmic connection on reduced states.
...
The mechanism is packaged into a single synthesis theorem. External analytic or perturbative inputs are flagged at the point of use.

\begin{theorem}[Synthesis theorem; \ClaimStatusConditional]\label{thm:synthesis}
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 \item If the tree-level line complex is resolved in degree $0$ and a compatible tree-level Hodge contraction is chosen, then the full quantum line complex remains concentrated in degree $0$, admits an $\h$-adic strong deformation retract relative to that choice, and the reduced superconnection collapses to an ordinary logarithmic flat connection
 \[
 \A_n = p_n A_n^{(1)} i_n.
 \]
...
\begin{proof}[Routing of proof obligations]
This is a synthesis theorem conditional on the hypotheses displayed in
the six clauses.  Clause~\textup{(i)} is proved by
Theorem~\ref{thm:strict-flatness}, Lemma~\ref{lem:inf-braid}, and
Proposition~\ref{prop:strict-log-ext}.  Clause~\textup{(ii)} is the
bar-valued curvature package of Lemma~\ref{lem:bar-insertion},
Theorem~\ref{thm:master-curvature}, and
Theorem~\ref{thm:support-hierarchy}.  Clause~\textup{(iii)} is
conditional on the anomaly-free perturbative and compactified
renormalisation hypotheses used in
Theorems~\ref{thm:log-bphz-existence},
\ref{thm:analytic-curvature}, and~\ref{thm:analytic-flatness}.
Clause~\textup{(iv)} is conditional on the resolved degree-zero line
complex and compatible Hodge contraction and is proved under those
hypotheses by Theorem~\ref{thm:quantum-concentration},
Theorem~\ref{thm:hodge-hbar-retract}, and
Theorem~\ref{thm:one-form-rigidity}.  Clause~\textup{(v)} follows from
the reduced-residue and tangential-regularisation package
Corollary~\ref{cor:reduced-residues},
Lemma~\ref{lem:tangential-regularization},
Theorems~\ref{thm:pentagon}--\ref{thm:hexagon}, and
Theorem~\ref{thm:pure-braid}.  Clause~\textup{(vi)} is
Theorem~\ref{thm:smashing-localization} and
Corollary~\ref{cor:periodicity}.  Thus the theorem is not an
unconditional theorem: its analytic and reduction clauses stand exactly
on the stated anomaly-free, resolved-line, and contraction hypotheses.
\end{proof}

The clauses above are proved or reduced below as indicated; conjectural
bridges beyond these hypotheses are stated separately in the frontier
section rather than folded into Theorem~\ref{thm:synthesis}.
...
Two distinct compactifications of $\Conf_n$ run throughout the chapter: \emph{logarithmic HT monodromy} names log-FM geometry, not logarithmic VOAs. Within log-FM itself there is an algebraic/analytic split:
...
The ground field $\kk$ has characteristic zero. Grading conventions are cohomological; tensor products are completed when necessary.

\subsection{Suspended bar conventions}
Let $A$ be an $A_\infty$ algebra in the suspended convention: the structure maps are
\[
...
\]
for the ordered configuration space. Write
\[
...
The full notion of dg-shifted Yangian belongs to a broader programme \cite{DNP25}. For the strict rational theory the relevant structure is the following.
...
The strict theory exposes the geometric heart of the construction: pairwise collisions, logarithmic poles, and Yang-Baxter flatness.
...
When $r(z)=k\,\Omega_{\mathrm{tr}}/z$ (the affine collision residue at level~$k$), this is exactly the classical rational KZ connection.
...
If $r(z)=k\,\Omega_{\mathrm{tr}}/z+O(1)$ at level~$k$ near $z=0$ and $\Omega_{ij}$ denotes the action of $\Omega$ in factors $i,j$, then
\[
...
\[
 z_i-z_j = \Big(\prod_{e\in P(i,j)}t_e\Big)u_{ij},
\]
with $u_{ij}$ a unit. Since $r(z)=k\,\Omega_{\mathrm{tr}}/z+O(1)$ at level~$k$ one has
\[
...
\[
 (\ds+\Theta_n)\Gamma_n+\MC(\Gamma_n)=0.
\]
The support combinatorics is exactly as stated.
\end{proof}
...
Fix a perturbative three-dimensional holomorphic-topological theory on
\[
 M=\CC_z\times \RR_t
\]
...
The analytic data are the graph-valued forms computing the higher operations of the theory in the sense of \cite{GKW25,DNP25}. These forms live on singular configuration spaces. Compactify and logarithmically resolve the relevant collision strata.
...
\begin{proof}
Mok's logarithmic Fulton-MacPherson spaces are log smooth, and the ordered real compactification of ordered points on a line is a manifold with corners. The mixed incidence strata being blown up are clean intersections of boundary strata indexed by connected collision subgraphs. Clean blowup preserves log smoothness and normal-crossings/corners structures. The local coordinates take the mixed form of the standard tree coordinates for Fulton-MacPherson spaces: every holomorphic difference factors into the product of scales along a holomorphic collision tree times a unit, and similarly every time difference factors through topological collision scales. The mixed blowups enforce compatibility when the same divergent subgraph controls both.
\end{proof}
...
\]
with smooth coefficient forms $\eta_{\alpha,\beta,\mu,\nu}$. All scale exponents are non-negative by construction (the subtraction removes all negative-degree terms). The logarithmic powers are at most $\mu_e, \nu_f \le 1$ (simple logarithmic singularities).
...
Let $D_F$ be the boundary divisor corresponding to a connected collision forest $F$, with local scale coordinate $r_F$ (in the holomorphic direction; the topological case is parallel). The residue $\Res_{D_F}\,\omega_\Gamma^{\mathrm{ren}}$ is the coefficient of $d\log r_F = dr_F/r_F$.
...
\]
Each graph-weight coefficient of $\mathcal K_n^{\mathrm{an}}$ vanishes by three ingredients: Stokes' theorem on the compactified mixed configuration spaces $\mathfrak C_\Gamma(\mathbf z)$, the BV/BRST master equation in the interior, and the Arnold cancellation at codimension-two corners (the FM analogue of Lemma~\ref{lem:Arnold_cancellation}).
...
\subsection{Hodge $\h$-adic retract}

\begin{theorem}[Hodge $\h$-adic strong deformation retract; \ClaimStatusProvedHere]\label{thm:hodge-hbar-retract}
Assume $H^j(E_n,Q_{0,n})=0$ for $j\neq 0$ and $H^0(E_n,Q_{0,n})$ has constant rank. After choosing a Hermitian metric, equivalently a tree-level Hodge contraction
\[
 (H_{0,n},0)\underset{p_{0,n}}{\stackrel{i_{0,n}}{\rightleftarrows}}(E_n,Q_{0,n}),
 \qquad
 Q_{0,n}h_{0,n}+h_{0,n}Q_{0,n}=\id-i_{0,n}p_{0,n},
\]
there is an induced $\h$-adic strong deformation retract
\[
...
\]
This gives the chosen tree-level contraction
\[
...
\]
But $q_n$ has cohomological degree $+1$, while $H_{0,n}$ lies in degree $0$, so $q_n=0$. Thus the target complex is $(H_{0,n}[[\h]],0)$, which identifies with $H_n$ by Theorem~\ref{thm:quantum-concentration}. The construction is canonical only relative to the chosen tree-level Hodge contraction; different contractions give homotopic retract data.
\end{proof}
...
\[
 \|h_{0,n}\Delta_n\|<1
\]
on every finite-rank truncation, the formulas of Theorem~\ref{thm:hodge-hbar-retract} converge as honest analytic maps and not only as formal $\h$-adic series.
\end{proposition}
...
\]
Transfer this superconnection along the chosen Hodge $\h$-adic retract.
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 \item an $\h$-adic retract is induced by any chosen tree-level Hodge contraction;
 \item the reduced logarithmic flat connection is
 \[
 \A_n = p_nA_n^{(1)}i_n;
 \]
 \item the HT associator, braiding, pentagon, hexagon, and pure braid representations exist without any further quantum concentration hypothesis.
\end{enumerate}
\end{theorem}

\begin{proof}
Item (i) is Theorem~\ref{thm:quantum-concentration}. Item (ii) is Theorem~\ref{thm:hodge-hbar-retract} after choosing a tree-level Hodge contraction. Item (iii) is Theorem~\ref{thm:one-form-rigidity}. Item (iv) then follows from the transport constructions and coherence theorems of Section~\ref{sec:reduction}; no additional quantum concentration hypothesis is imposed.
\end{proof}
...
The reduced theory speaks in four registers: arithmetic geometry, enumerative geometry, periodicity, and chromaticity.

\subsection{Arithmetic geometry: logarithmic local systems and periods}
The reduced theory lives in the category of logarithmic local systems on logarithmic Fulton-MacPherson spaces.
...
\section{Resolved logarithmic HT data}\label{sec:platonic}
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 \item A tree-level degree-zero resolution condition on $(E_n,Q_{0,n})$, together with a compatible tree-level Hodge contraction inducing the $\h$-adic reduction datum
 \[
 (H_n,0) \underset{p_n}{\stackrel{i_n}{\rightleftarrows}} (E_n[[\h]],Q_n)
 \]
...
\begin{theorem}[Resolved logarithmic HT monodromy theorem; \ClaimStatusProvedHere]\label{thm:platonic-ideal}
Every resolved logarithmic HT datum determines, functorially in its chosen reduction data:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{enumerate}
The concrete quasi-linear perturbative HT construction of Sections~\ref{sec:analytic}--\ref{sec:resolved} provides an example of such a datum whenever the tree-level line operators are resolved in degree $0$ and a compatible tree-level Hodge contraction has been chosen.
\end{theorem}
...
The unresolved frontier is now sharp: not whether a retract exists after quantization, but which physically natural line operators admit classically resolved degree-zero BRST models. Once that classical algebra exists, any compatible Hodge contraction produces the quantum-logarithmic monodromy machine, and different choices give homotopic reduction data.
\end{remark}

\section{Supplementary computations}

\subsection{Support combinatorics in the Maurer-Cartan polynomial}
The support combinatorics behind Theorem~\ref{thm:support-hierarchy} is as follows. If each $D_S$ is supported on the tensor factors in $S$, then the support of a higher product
\[
 m_\ell(D_{S_1},\dots,D_{S_\ell})
\]
...
The full monodromy computation for the $\mathfrak{sl}_2$ KZ connection
at general level~$k$ proceeds from the Casimir, through
...
\]
Indeed, with the normalization $C(j)=2j(j+1)$ induced by $\Omega$, the split-Casimir formula $\Omega_{12}=\frac{1}{2}\bigl(C(j_{\mathrm{tot}})-2\,C(\tfrac{1}{2})\bigr)=\frac{1}{2}\bigl(2j_{\mathrm{tot}}(j_{\mathrm{tot}}+1)-3\bigr)$
...
\begin{equation}\label{eq:R-level-explicit}
 R(z)=1+k\,\Omega_{\mathrm{tr}}/z+O(z^{-2})
\end{equation}
...
(cf.\ Theorem~\ref{thm:affine-monodromy-identification}).
\end{computation}
...
is the genus-$1$ curvature $\kappaChHodge(\widehat{\mathfrak{sl}}_2)
=\dim\mathfrak{g}\cdot(k+h^\vee)/(2h^\vee)=3(k+2)/4$
times the period form $\omega_1$, in agreement with the formula
$\kappaChHodge(\cA)\cdot\omega_g$ established in Volume~I.
\end{remark}
...
The key mechanism is that one-loop exactness removes the wheel and contact corrections beyond the affine Lie-Jacobi layer. After reduction to degree-zero evaluation states, the higher collision homotopies $h_S$ ($|S| \geq 3$) of Section~\ref{sec:ainfty} are generated by binary residues and the Arnold--IHX relation, because the tree-level BRST complex is a regular-sequence Koszul model (Proposition~\ref{prop:regular-sequence}). The reduced logarithmic connection is therefore determined by the Casimir residues of the KZ connection, while its finite cubic shadow is the KZ associator.
...
\begin{proof}
The affine half-space BV quantization is one-loop exact (Theorem~\ref{thm:affine-half-space-bv}). By the one-loop Koszul criterion (Theorem~\ref{thm:one-loop-koszul}), the pre-reduction boundary algebra $V^k(\fg)$ is chirally Koszul: the bar cohomology is concentrated. On evaluation modules the only higher operation surviving in the reduced comparison is the finite class-$\mathbf L$ Lie-Jacobi transfer generated by the binary current bracket; wheel and contact corrections vanish. Consequently the tree-level line complex for evaluation modules is a Koszul complex of the type in Proposition~\ref{prop:regular-sequence}, with the regular sequence given by the spectral evaluation constraints. The concentration in degree $0$ follows.
\end{proof}
...
\begin{proof}
By Proposition~\ref{prop:affine-resolved}, the tree-level line complex is resolved in degree~$0$. After choosing a compatible tree-level Hodge contraction, the unconditionality theorem (Theorem~\ref{thm:unconditionality}) applies: an $\h$-adic retract exists, and the reduced superconnection is an ordinary logarithmic flat connection
\[
 \A_n = p_n A_n^{(1)} i_n
\]
...
Since the affine algebra is chirally Koszul, the reduced connection
has no Virasoro wheel correction. The class-$\mathbf L$ cubic layer is
the Lie-Jacobi transfer and is absorbed by the KZ associator; the
contact part of the \(A_\infty\) Yang--Baxter defect vanishes. After
projection to degree-zero states, the regular terms in \(r(z)\)
beyond the simple pole act on higher-degree components killed by
\(p_n\). Thus
\[
...
\begin{theorem}[Affine monodromy and quantum-group comparison; \ClaimStatusProvedHere]
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 \item After choosing the KZ/Drinfeld associator and the corresponding
 gauge twist, the spectral half-monodromy on evaluation modules is
 braided-monoidally equivalent to the quantum-group braiding. The
 pointwise meromorphic family $R_{V_i,V_j}(z)$ and the universal
 $U_q(\fg)$ $R$-matrix are compared through this monodromy functor, not
 by a bare equality of functions.
\end{enumerate}
\end{theorem}
...
\medskip
\noindent\textbf{(iii).} The comparison of the spectral family with
the quantum-group braiding has three steps: reduction of the
$A_\infty$ tower, passage from the raw collision residue to the
shifted KZ residue, and Drinfeld--Kohno monodromy.
...
exactness. The affine graph calculus has no independent
wheel/contact operations; its class-$\mathbf L$ higher part is the
finite Lie-Jacobi transfer, generated by the same binary Casimir
residues through the Arnold--IHX relation. Therefore the
path-ordered exponential reduces to the standard ordered
exponential of the flat KZ connection, and the cubic shadow is the
Drinfeld associator of that connection:
\[
...
\smallskip\noindent
\emph{Step (iii-b): Raw and reduced residues.}
...
structure constants). After $d\log$ extraction the singular
bar-collision kernel is
\[
 r^{\mathrm{raw}}_{\mathrm{coll}}(z)
 =
 \frac{k\,\Omega}{z}.
\]
Here $\Omega = \sum_a J^a \otimes J_a$ is the split Casimir and the
factor~$k$ is the affine level appearing in the central double pole.
The reduced line connection is not obtained by exponentiating this raw
kernel. Projection to degree-zero conformal blocks divides by the
Segal--Sugawara normalisation $(k+h^\vee)$, as in
Theorem~\ref{thm:reduced-equals-kz}, and its logarithmic residue is
\[
 r^{\mathrm{KZ}}_{ij}
 =
 \frac{\rho_{ij}(\Omega)}{k+h^\vee}.
\]
...
\[
 ...
 {k + h^\vee}\Bigr).
\]
This is the path-ordered exponential of the reduced KZ connection
along the half-circle. The raw level factor $k$ has already been
absorbed in the projection from the bar collision kernel to the
conformal-block connection.

By the Drinfeld--Kohno theorem (part~(ii)), this half-monodromy
is gauge-equivalent, after the chosen KZ associator and twist, to the
image of the universal $R$-matrix of $U_q(\fg)$ on
$V_i \otimes V_j$. Therefore
\[
 R_{V_i,V_j}^{\mathrm{spectral,\,mon}}
 \;\sim_F\;
...
\smallskip\noindent
\textbf{Combining (i)--(iii):} On evaluation modules, the reduced
spectral half-monodromy is the KZ half-monodromy, and
Drinfeld--Kohno compares it with the quantum-group braiding after the
associator/twist choice. The associator-controlled Yang--Baxter
structure of Theorem~\ref{thm:YBE} is carried to the
quasi-triangular structure of $U_q(\fg)$; the comparison on evaluation
modules is compatible
...
associator $\Phi_{\mathrm{KZ}}$ by the same one-loop wheel/contact
cancellation:
...
The inputs to Theorem~\ref{thm:affine-monodromy-identification} carry one named hypothesis. The one-loop exactness of the affine half-space BV theory is proved in Theorem~\ref{thm:affine-half-space-bv}. The Koszulness of $V^k(\fg)$ is Theorem~\ref{thm:one-loop-koszul}, conditional on the loop-order spectral sequence (Conjecture~\ref{conj:one-loop-koszul}). The unconditionality theorem (Theorem~\ref{thm:unconditionality}) guarantees the reduced monodromy machine, and the Drinfeld--Kohno theorem is classical. For any simple $\fg$ and any generic level $k$ the result therefore holds conditionally on Conjecture~\ref{conj:one-loop-koszul}, with no further standing hypothesis.
\end{remark}
...
\begin{enumerate}[label=\textup{(\roman*)}]
 ...
 \item \emph{Bar-complex identification}: By one-loop exactness of $V^k(\fsl_3)$ at generic $k$ \textup{(}Theorem~\textup{\ref{thm:one-loop-koszul})}, the reduced HT connection has no Virasoro wheel correction. Its finite class-$\mathbf L$ cubic transfer is the Lie-Jacobi associator, hence the reduced connection is the KZ connection. Therefore the bar-complex associator reproduces the Drinfeld--KZ associator identically, including the cubic Casimir contribution. By the Drinfeld--Kohno theorem, the monodromy identifies with the $U_q(\fsl_3)$ braiding and associator on evaluation modules.
\end{enumerate}
\end{proposition}

\begin{proof}
The computation is a direct verification. The split Casimir $\Omega = \sum_{i \neq j} E_{ij} \ot E_{ji} + \sum_{a,b} (A^{-1})_{ab} H_a \ot H_b$ has the stated eigenvalues by the formula $\Omega|_{V_\nu} = \frac{1}{2}(c_2(\nu) - c_2(\lambda) - c_2(\mu))$. For part~(ii), the Fierz decomposition is standard; the key point is that $\mathcal{D}_{123}^{(-)} \neq 0$ because the totally symmetric tensor $d_{abc}$ of $\fsl_3$ is nonvanishing. The off-diagonal matrix element on the multiplicity space of $V_{(1,1)}$ is proportional to $c_3(2,0) - c_3(0,1) = 35/9 + 10/9 = 5$. For part~(iii), one-loop exactness kills the wheel/contact corrections, and the finite Lie-Jacobi cubic layer is exactly the Drinfeld--KZ associator. Hence the reduced connection $\nabla_{\fS} = d + \Omega\,dz/((k+3)z)$ is the KZ connection for $\fsl_3$ at level $k$. The identification with $U_q(\fsl_3)$ then follows from Theorem~\ref{thm:affine-monodromy-identification} applied to $\fg = \fsl_3$.
\end{proof}
...
For $\fg = \mathfrak{sl}_2$ at positive integer level $k$, the bar
complex integrals over $\FM_n(\CC) \times \Conf_n(\RR)$, evaluated on
tilting colours in the Andersen--Paradowski alcove and followed by the
negligible-ideal quotient $\pi$ of
Proposition~\ref{prop:rt-functor-via-negligible-quotient} and
quantum-trace closure, compute the corresponding colored Jones value
$J_K(q)$ of a framed knot $K$ at $q = e^{i\pi/(k+2)}$.
\end{corollary}
...
\]
on tensor powers of finite-dimensional $\mathfrak{sl}_2$ modules before
root-of-unity semisimplification. At the root of unity the MTC colour
set is the alcove quotient of tilting modules; non-alcove colours are
seen only through their image after negligible morphisms are killed.
The integration domain $\FM_n(\CC) \times \Conf_n(\RR)$ parametrises
the $E_1$ bar coalgebra: the $\FM_n(\CC)$ factor carries the bar
differential and the $\Conf_n(\RR)$ factor carries the $E_1$ coproduct
implementing the group-like expansion.
...
\begin{enumerate}[label=\textup{(\alph*)}]
 ...
 \item \emph{Beyond the affine lineage}: for $\mathcal{W}$-algebras obtained by Drinfeld--Sokolov reduction, the Swiss-cheese transferred operations $m_k^{\mathrm{SC}}$ ($k \geq 3$) do not vanish. The one-loop wheel/contact cancellation breaks, and the full $A_\infty$ superconnection contributes. The reduced monodromy is then the $\mathcal{W}$-algebra $R$-matrix, which should equal the spectral $R$-matrix of the $\mathcal{W}$-Yangian; this is the content of Conjecture~\ref{conj:rmatrix} in the non-formal Swiss-cheese regime.
 \item \emph{Higher genus}: at genus $g \geq 1$, the KZ connection is replaced by the KZB connection (Computation~\ref{comp:genus1-kzb}), and the monodromy acquires modular corrections controlled by the curvature $\kappaChHodge(\cA) \cdot \omega_g$. The identification should lift to elliptic and higher-genus $R$-matrices, connecting to the chromatic tower (Conjecture~\ref{conj:chromatic}).
\end{enumerate}
\end{remark}
...
\begin{remark}[\texorpdfstring{Logarithmic HT monodromy and Humbert \(H_1\)}{Logarithmic HT monodromy and Humbert H1}]
\label{rem:loght-core-humbert}
The logarithmic HT-monodromy framework specialises at Mukai-enhanced K3 to the holonomic $\mathcal{D}$-module $\mathcal{L}^{\Delta_5}$ on $\overline{\mathcal{A}_2}$ with regular singularities on the Humbert divisors $H_1 \cup H_4$. The monodromy of $\mathcal{L}^{\Delta_5}$ around $H_1$ has order $8$, identified via Bruinier $2002$ Proposition~$5.1$ Heegner Chern-class reciprocity with the Mukai-doubling Koszul conductor $K^{\kappaChHodge} = 2\,c_+(\mathrm{Mukai}(K3)) = 8$ and the Lusztig specialisation $\ell = 8$ at $\zeta^8 = 1$, $\hbar^2 = -1/8$. Universal identity $\hbar^2 \cdot K^{\kappaChHodge} = -1$ on the Lorentzian-lattice-parametric $\mathcal{B}$-family.
\end{remark}
