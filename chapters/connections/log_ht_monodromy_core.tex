...
\begin{theorem}[Existence, logarithmicity, and residue factorization; \ClaimStatusHeuristic]\label{thm:log-bphz-existence}
...
\begin{theorem}[Compactified-Stokes flatness; \ClaimStatusHeuristic]\label{thm:analytic-flatness}
...
\begin{theorem}[Family version over semistable degenerations; \ClaimStatusHeuristic]\label{thm:family-version}
...
Let $V_1, \dots, V_n$ be finite-dimensional $\fg$-modules, viewed as evaluation modules for the dg-shifted Yangian $\cA^!$ at spectral positions $z_1, \dots, z_n$ (an \emph{evaluation module} is a finite-dimensional $\fg$-module $V$ pulled back to a $Y_\hbar(\fg)$-module via the evaluation homomorphism $\mathrm{ev}_z: Y_\hbar(\fg) \to U(\fg)$ sending $T(u) \mapsto 1 + \hbar J/(u-z)$). Then the tree-level BRST complex $(E_n, Q_{0,n})$ is resolved in degree $0$: $H^j(E_n, Q_{0,n}) = 0$ for $j \neq 0$.
\end{proposition}
...
$\{J^a {}_\lambda J^b\} = f^{ab}_c J^c + k\,\kappa^{ab}\lambda$
(where $\kappa$ is the Killing form and $f^{ab}_c$ are the
structure constants).  Evaluating the Laplace transform:
\[
  r(z) = \frac{\sum_{a,b} J^a \otimes J^b \cdot \kappa_{ab}}{z}
         + \frac{k\,\kappa}{z^2}
  = \frac{\Omega}{z} + \frac{k\,\kappa}{z^2},
\]
...
$\theta \in [0,\pi]$, gives
$\mathrm{P}\!\exp(\int_0^\pi (\Omega/z_{ij} e^{i\theta})
\cdot iz_{ij} e^{i\theta}\, d\theta)
= \exp(i\pi\, \Omega)$, with the normalization
...
the pentagon and hexagon of $\mathrm{Rep}_q(\fg)$, and the two
braided monoidal categories coincide.
\end{proof}
