...
\label{sec:classical-r-matrix-explicit}
...
\label{subsec:classical-limit-explicit}

\begin{definition}[Classical $r$-matrix via semiclassical expansion]
\label{def:classical-r-matrix-expansion}
...
\label{rem:collision-residue-identification}
...
\MC(\gAmod)$ (Volume~I, Theorem~\ref*{thm:mc2-bar-intrinsic}):
\begin{equation}
\label{eq:collision-residue-r}
...
full shadow Postnikov tower: the simplest nontrivial projection
...
\end{equation}
Moreover, the order-$\hh^2$ coefficient $s(z)$ satisfies the
...
+ O(\hh^3), \label{eq:R12-expand} \\
R_{13}(z_{13}) &= \id_{123} + \hh\, r_{13} + \hh^2\, s_{13}
+ O(\hh^3), \label{eq:R13-expand} \\
R_{23}(z_{23}) &= \id_{123} + \hh\, r_{23} + \hh^2\, s_{23}
+ O(\hh^3), \label{eq:R23-expand}
\end{align}
...
\label{subsec:antisymmetry-unitarity}

\begin{proposition}[Antisymmetry and unitarity of $r(z)$;
\ClaimStatusProvedHere]
\label{prop:r-antisymmetry-unitarity}
...
If $r(z) = \Omega/z + (\text{regular at } z = 0)$ with
...
$\Omega/z$ forces oddness if $\Omega$ is symmetric and
$r - \Omega/z$ is regular.

(iii) Write $r_{12}(z) = \Omega/z + r_{\mathrm{reg}}(z)$.
Then $r_{21}(-z) = \Omega_{21}/(-z) + r_{\mathrm{reg},21}(-z)
= -\Omega/z + r_{\mathrm{reg},21}(-z)$ (using
...
\label{subsec:explicit-r-matrices}
...
\label{subsubsec:heisenberg-r-matrix}
...
$r$-matrix is
\begin{equation}
\label{eq:heisenberg-r-explicit}
\boxed{
...
\medskip
\noindent\textbf{Step 3: Collision residue verification.}
Equivalently, the collision residue on $\FM_2(\C)$ is
\[
\Res^{\mathrm{coll}}_{0,2}(\Theta_\cA)
= \Res_{z_1 = z_2}
\bigl[\Theta_{\cH_k}^{(0,2)}(z_1, z_2)\bigr]
= \Res_{w = 0} \left[\frac{k}{w^2}\, J(z_2) \otimes J(z_2)
\, dw\right]
= \frac{k}{z^2}\, J \otimes J,
\]
where $w = z_1 - z_2 = z$ is the collision coordinate on
$\FM_2(\C)$.  The double pole reflects the fact that the
Heisenberg OPE $J(z_1) J(z_2) \sim k/(z_1 - z_2)^2$ has a
second-order singularity and no first-order pole.
...
\label{rem:heisenberg-r-shadow-depth}
...
classification of Volume~I): the shadow tower terminates at arity~$2$,
...
\label{subsubsec:affine-r-matrix}
...
$\Omega/((k+h^\vee)z)$ carries the Lie-algebraic data; the double
...
\medskip
\noindent\textbf{Step 3: Collision residue.}
...
+ O(w^0).
\]
The collision residue $\Res_{w=0}^{\mathrm{coll}}$ extracts the
polar part, giving~\eqref{eq:affine-r-explicit}.

\medskip
\noindent\textbf{Step 4: CYBE verification.}
We verify~\eqref{eq:cybe-proved} for $r(z) = \Omega/z$ (suppressing
the overall $(k+h^\vee)^{-1}$ and the double-pole term, which
satisfies the CYBE independently by the abelian argument of
Proposition~\ref{prop:heisenberg-r-matrix}).

For $r_{ij} = \Omega_{ij}/z_{ij}$, the CYBE reads
\[
...
\begin{equation}\label{eq:infinitesimal-braid}
[\Omega_{12}, \Omega_{13}]
+ [\Omega_{12}, \Omega_{23}]
+ [\Omega_{13}, \Omega_{23}]
\;=\; 0
...
for $r(z) = \Omega/z$ follows: the three terms in the CYBE
...
\label{rem:sl2-r-matrix-explicit}
...
\medskip
\noindent\textbf{Step 3: Collision residue.}
On $\FM_2(\C)$, the Virasoro OPE
$T(z_1)\, T(z_2) \sim c/(2w^4) + 2T(z_2)/w^2 + \partial T(z_2)/w$
(where $w = z_1 - z_2$) gives the collision residue
$\Res^{\mathrm{coll}}_{0,2}(\Theta_{\mathrm{Vir}_c})
= c/(2z^4) + 2T/z^2 + \partial T/z$,
matching~\eqref{eq:virasoro-r-explicit} (with the identification
$c/2 = c/12 \cdot 3!$).

\medskip
\noindent\textbf{Step 4: Pole structure and shadow depth.}
The quartic pole $c/(2z^4)$ is the signature of the Virasoro: no
simple Lie algebra produces a pole of order higher than~$2$ (the
affine $r$-matrix has at most a double pole).  The quartic pole is
the geometric manifestation of the conformal anomaly $c$: it
arises from the $\lambda^3$ term in the $\lambda$-bracket, which
encodes the failure of the energy-momentum tensor to transform as
a primary field.  In the shadow depth classification, the quartic
pole places the Virasoro in the $M$ (mixed) class: the shadow
tower does not terminate, and the full Maurer--Cartan element
...
$r$-matrix}
\label{subsubsec:betagamma-r-matrix}
...
\label{rem:pole-structure-comparison}
...
\textbf{Shadow class} \\
\hline
$\cH_k$ & $k/z^2$ & $\lambda$-bracket $k\lambda$ & $G$ \\
$\widehat{\fg}_k$ & $\Omega/z$ & structure constants $f^{ab}_c$ & $L$ \\
$\beta\gamma_\lambda$ & $1/z$ & contact OPE & $C$ \\
$\mathrm{Vir}_c$ & $c/(2z^4)$ & conformal anomaly $\lambda^3$ & $M$ \\
$\mathcal{W}_N$ & $c_{W_N}/z^{2N}$ & spin-$N$ self-OPE & $M$ \\
\end{tabular}
\end{center}
...
\label{sec:pronilpotent-yangian}
\index{Yangian!pronilpotent|textbf}
\index{pronilpotent Yangian|textbf}
\index{modular convolution algebra!Yangian}

The classical $r$-matrix is the genus-$0$, arity-$2$ projection of
$\Theta_\cA$.  The full universal Maurer--Cartan element encodes
corrections at all genera and all arities.  This section constructs
the pronilpotent completion $Y_T^{\mathrm{mod}}$ of the modular
convolution algebra in which $\Theta_\cA$ lives, and identifies the
stable-graph Yang--Baxter equation as its Maurer--Cartan equation.

\subsection{The modular convolution algebra}
\label{subsec:modular-convolution-yangian}

\begin{definition}[Modular convolution algebra for the spectral
problem; \ClaimStatusProvedHere]
\label{def:modular-convolution-spectral}
\index{modular convolution algebra!spectral definition}
Let $\cA$ be a chiral algebra satisfying (H1)--(H4), with bar
coalgebra $\barBch(\cA)$ and Koszul dual $\cA^!$.  The
\emph{modular convolution Lie algebra for the spectral problem} is
\begin{equation}
\label{eq:modular-convolution-spectral}
L_T^{\mathrm{mod}}
\;=\;
\prod_{g \ge 0,\, n \ge 1}
\Hom\bigl(
...
\label{def:lie-bracket-graph-composition}
...
\ClaimStatusProvedHere]
\label{prop:jacobi-graph-composition}
...
\label{def:pronilpotent-yangian}
...
\label{rem:spectral-sequence-convergence}
\index{spectral sequence!convergence of arity filtration}
The spectral sequence associated to the arity filtration
$F^\bullet$ on $Y_T^{\mathrm{mod}}$ converges.  The reason is
that at each total degree~$d$, only finitely many filtration
...
\label{rem:genus-expansion-yangian}
...
\subsection{The pro-Maurer--Cartan element $R_T^{\mathrm{mod}}$}
\label{subsec:pro-mc-element}

\begin{definition}[Stable-graph $R$-matrix]
\label{def:stable-graph-r-matrix}
\index{R-matrix!stable-graph}
\index{stable graph!R-matrix}
The \emph{stable-graph $R$-matrix} is the element
\begin{equation}
\label{eq:stable-graph-r-matrix}
R_T^{\mathrm{mod}}(z; \hh)
\;=\;
...
Y_T^{\mathrm{mod}}[[\hh]],
\end{equation}
where $r_{T,g}(z)$ is the genus-$g$ contribution.  The classical
$r$-matrix of \S\ref{sec:classical-r-matrix-explicit} is the
genus-$0$ term: $r_{T,0}(z) = r(z)$.  The genus-$1$ term
$r_{T,1}(z)$ encodes the one-loop correction to the spectral
braiding---the effect of the Arakelov form $\omega_1$ on the
$R$-matrix (Theorem~\ref{thm:elliptic-spectral-dichotomy}).
\end{definition}

\begin{theorem}[Stable-graph YBE is the MC equation;
\ClaimStatusProvedHere]
\label{thm:stable-graph-ybe-mc}
\index{Yang--Baxter equation!stable-graph}
\index{Maurer--Cartan equation!stable-graph YBE}
The stable-graph $R$-matrix $R_T^{\mathrm{mod}}(z; \hh)$ satisfies
the stable-graph Yang--Baxter equation if and only if it is a
Maurer--Cartan element of $Y_T^{\mathrm{mod}}[[\hh]]$:
\begin{equation}
\label{eq:stable-graph-ybe-mc}
d\, R_T^{\mathrm{mod}}
+ \frac{1}{2}\bigl[R_T^{\mathrm{mod}},\, R_T^{\mathrm{mod}}\bigr]
= 0
\quad \Longleftrightarrow \quad
\text{stable-graph YBE holds at all genera.}
\end{equation}
\end{theorem}

\begin{proof}
\noindent\textbf{Step 1: Expansion by genus.}
Write $R = R_T^{\mathrm{mod}} = \sum_{g \ge 0} \hh^{2g}\, r_g$.
The MC equation $dR + \frac{1}{2}[R, R] = 0$ expands as
\begin{equation}
\label{eq:mc-genus-expansion}
...
$r_0$.  The equation $d\, r_1 + [r_0, r_1]
+ \frac{1}{2}[r_0, r_0]^{g=1} = 0$ is the genus-$1$
Yang--Baxter equation: the first quantum correction to the
classical braiding, controlled by the Arakelov quasi-periodicity
and the curvature $\kappa(\cA) \cdot \omega_1$.

\medskip
\noindent\textbf{Step 4: General $g$.}
At general genus $g$, equation~\eqref{eq:mc-at-genus-g} is the
genus-$g$ Yang--Baxter equation: it asserts that the genus-$g$
correction $r_g$ to the $R$-matrix is determined (up to gauge
equivalence) by the lower-genus corrections $r_0, \ldots,
r_{g-1}$ via the inhomogeneity
$\sum_{g_1+g_2=g,\, g_i < g} [r_{g_1}, r_{g_2}]$.  This is
the inductive genus-by-genus determination of the spectral
braiding, paralleling the genus spectral sequence of Volume~I
...
\ClaimStatusProvedHere]
\label{cor:classical-yangian-truncation}
...
\label{subsec:gravitational-yangian-virasoro}
...
\ClaimStatusProvedHere]
\label{prop:gravitational-yangian-properties}
The gravitational Yangian
$Y_T^{\mathrm{mod}}(\mathrm{Vir}_c)$ has the following properties:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Genus-$0$ $r$-matrix:}
$r_0(z) = c/(2z^4) + 2T/z^2 + \partial T/z$
...
algebra.  The shadow tower $\Theta_{\mathrm{Vir}_c}^{(\le r)}$
...
$\kappa(\mathrm{Vir}_c) + \kappa(\mathrm{Vir}_{26-c}) = 0$
(Volume~I, Theorem~C), so
$c + (26-c) = 26 \ne 0$ for the gravitational Yangian---the
modular curvature is always nonzero (except at $c = 0$ where
$\kappa = 0$).  The complementarity potential
$F_1(\mathrm{Vir}_c) + F_1(\mathrm{Vir}_{26-c})
= c/24 + (26-c)/24 = 26/24$
does NOT vanish: the bulk gravitational theory carries a net
genus-$1$ anomaly proportional to the total central charge~$26$.
\end{enumerate}
\end{proposition}
...
as resonance shadow]
\label{rem:gravitational-yangian-resonance}
...
Remark~\ref*{rem:virasoro-resonance-model}): it is the image of
...
(Volume~I, Conjecture~\ref*{conj:platonic-completion}).  The
...
\label{sec:higher-genus-braiding}
...
(Volume~I, Construction~\ref*{const:vol1-modular-tangent-complex}).
\end{definition}

\begin{theorem}[KZB flatness from the genus-$1$ MC equation;
\ClaimStatusProvedHere]
\label{thm:kzb-flatness-from-mc}
...
+ (\text{lower order in }E_2), \label{eq:zeta-S-transform} \\
\wp(z/\tau | -1/\tau)
&= \tau^2\, \wp(z|\tau). \label{eq:wp-S-transform}
\end{align}
...
\label{subsec:genus-2-and-beyond}
...
\ClaimStatusProvedHere]
\label{prop:higher-genus-r-structure}
...
\begin{equation}
\label{eq:higher-genus-r-graph-sum}
r_g(z)
...
\begin{proposition}[Convergence of the stable-graph expansion;
\ClaimStatusProvedHere]
\label{prop:stable-graph-convergence}
\index{stable-graph expansion!convergence}
For chiral algebras $\cA$ satisfying the HS-sewing condition
\textup{(}Volume~I,
...
\begin{theorem}[Line operators as Yangian modules;
\ClaimStatusProvedHere]
\label{thm:line-operators-yangian-modules}
\index{line operators!as Yangian modules}
Assume \textrm{(H1)--(H4)}.  The category $\mathcal{C}_{\mathrm{line}}$
...
\begin{equation}
\label{eq:line-ops-yangian-mods}
...
spectral $R$-matrix $R(z)$ provides the braiding.

At genus $g \ge 1$, the higher-genus components $r_g$ of the
pro-MC element $R_T^{\mathrm{mod}}$ act on Yangian modules as
\emph{genus-$g$ corrections to the braiding}: the genus-$g$
spectral braiding $R_g(z)$ is the endomorphism of
$V_1 \otimes V_2$ obtained by evaluating $r_g(z)$ on a pair of
Yangian modules.
\end{theorem}
...
$\cA^!$-modules with genus-$0$ modules over $Y_T^{\mathrm{mod}}$
follows from Corollary~\ref{cor:classical-yangian-truncation}: the
genus-$0$ truncation of $Y_T^{\mathrm{mod}}$ is the classical
Yangian, and representations of the classical Yangian are
representations of $\cA^!$ by the Tannakian reconstruction
(Theorem~\ref{thm:Koszul_dual_Yangian}).

The genus-$g$ corrections: the pro-MC element
$R_T^{\mathrm{mod}} = \sum_g \hh^{2g}\, r_g$ acts on
$V_1 \otimes V_2$ via $\rho(r_g) \in
\End(V_1 \otimes V_2)[[z^{-1}]]$, which is the genus-$g$
spectral braiding.  The MC equation for $R_T^{\mathrm{mod}}$
ensures that the total braiding
$R(z; \hh) = \id + \sum_{g \ge 0} \hh^{2g+1}\, \rho(r_g)(z)$
satisfies the quantum YBE on
$V_1 \otimes V_2 \otimes V_3$ at all genera.
\end{proof}

\begin{proposition}[The evaluation representation;
\ClaimStatusProvedHere]
\label{prop:evaluation-representation}
...
\label{rem:virasoro-conical-btz}
...
\begin{remark}[Connection to MC3 in type A]
\label{rem:mc3-type-a-connection}
\index{MC3!type A connection}
For $\cA = \widehat{\mathfrak{sl}_N}_k$, the module category
$\cA^!\text{-}\mathbf{mod}$ is the category of modules over the
Yangian $Y(\mathfrak{sl}_N)$.  The MC3 theorem in type A
(Volume~I, Theorem~\ref*{thm:mc3-type-a-resolution}) provides the
categorical lift: the thick subcategory generated by the
prefundamental representations $L^\pm$ of $Y(\mathfrak{sl}_N)$
(Hernandez--Jimbo) is the entire module category
$\mathcal{O}_{\mathrm{poly}}$, identified with a completed
subcategory of $\cA^!\text{-}\mathbf{mod}$.  The extension to
arbitrary simple type (Volume~I,
Conjecture~\ref*{conj:mc3-arbitrary-type}) requires the
prefundamental CG closure for all Dynkin types.
\end{remark}
...
\label{sec:fredholm-partition-functions}
...
\label{subsec:sewing-envelope-heisenberg}
...
\ClaimStatusProvedHere]
\label{prop:restriction-eigenvalues}
...
\label{subsec:partition-function-derivation}
...
\label{thm:heisenberg-partition-fredholm}
...
$\|T_q\| = |q| < 1$ for $|q| < 1$.  The Fredholm determinant
\[
\det(1 - T_q)
= \exp\Bigl(-\sum_{n=1}^\infty \frac{1}{n}\, \Tr(T_q^n)\Bigr)
= \exp\Bigl(-\sum_{n=1}^\infty \frac{1}{n}\,
\frac{q^n}{1-q^n} \cdot n\Bigr)
\]
(using the eigenvalues $\{q^m\}$ of $T_q$: the trace of $T_q^n$
is $\sum_{m=1}^\infty q^{mn}$).

Wait---the trace of $T_q^n$ when the eigenvalues are $\{q^m :
m \ge 1\}$ is $\Tr(T_q^n) = \sum_{m=1}^\infty q^{mn}$.  Hence
\begin{align*}
\log\det(1 - T_q)
...
\begin{equation}
\label{eq:heisenberg-partition-product}
...
characteristic]
\label{rem:fredholm-modular-char}
...
Bergman reduction]
\label{rem:heisenberg-sewing-theorem}
...
Theorem~\ref*{thm:heisenberg-one-particle-sewing}: for the
...
$\sum_{g \ge 1} F_g(\cH_k)\, \hh^{2g-2}$ is computed by
Volume~I, Theorem~D: it equals $-k/2 \cdot (\hat{A}(i\hh) - 1)$,
