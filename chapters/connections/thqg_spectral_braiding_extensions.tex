...
Yangian $Y(\fg)$.  The MC3 theorem for all simple types
(Volume~I, Corollary~\ref*{cor:mc3-all-types}) provides the
categorical lift: the thick subcategory generated by the
prefundamental representations $L^\pm$ of $Y(\fg)$
is the entire module category
$\mathcal{O}_{\mathrm{poly}}$, identified with a completed
subcategory of $\cA^!\text{-}\mathbf{mod}$.  The proof uses
the multiplicity-free $\ell$-weight property (Chari--Moura, 2006),
which holds for all simple Lie algebras.
\end{remark}
