...
The Moonshine module $V^\natural$ arises from the class-G Leech
lattice VOA $V_\Lambda$ through the FLM finite $\Z/2$ orbifold. That VOA
classification input is separate from the HT descent theorem. The HT statement
requires three further data: the abelian Leech holomorphic-topological
model, finite-orbifold BV descent for the lifted action, and
trivialization of the Dijkgraaf-Witten obstruction
\[
 [\alpha_{\mathrm{orb}}]\in H^3(B\Z/2;\mathrm{U}(1))\cong \Z/2.
\]
For a finite group $G$ acting on a boundary VOA $V$, the descent object
is the homotopy fixed point totalisation
\[
 V^{hG}
 =
 \Tot\!\left[
 V \rightrightarrows \Map(G,V)
 \;\begin{matrix}\to\\[-0.65ex]\to\\[-0.65ex]\to\end{matrix}\;
 \Map(G^2,V)
 \longrightarrow \cdots
 \right],
\]
together with the twisted-sector completion required by the orbifold
VOA. Chain-level $E_3$ descent requires the obstruction class to vanish.
When a normalized $2$-cochain $\beta$ trivializes
$\alpha_{\mathrm{orb}}$, the associator is corrected by
\[
 \Phi^\beta_{g,h,k}
 =
...
anomaly datum or a separately verified local computation, separate
from the determinant identity.

The descent criterion records the conditional statement: the
generic class-M DS-Hochschild statement extends only in the
weight-completed ambient, and $V^\natural$ as
an abstract boundary VOA acquires a single-colour
original-complex $E_3$ structure only after the listed conditional data.
...
normalized representative, so the homotopy-fixed-point totalisation
above carries the corrected trivial associator;
...
statement is a finite-orbifold HT descent from the abelian Leech model;
the original-complex $E_3$ structure on the abstract Monster VOA
follows only conditionally on the data above.
\end{theorem}

\begin{proof}
FLM construct the boundary VOA:
$V_\Lambda^{\mathrm{tw}}$ has ground-state multiplicity $2^{12}$, and
FLM Chapters~8--10 establish the twisted Jacobi identity for
...
gauging. Hypotheses~(ii)--(iii) place the finite-orbifold BV
construction on the homotopy fixed point totalisation, free of
anomalous associator, and identify, via boundary restriction, the
descended boundary algebra with the FLM orbifold.
Costello--Gwilliam descent transports the $E_3$ operations from
$\cF_\Lambda$ to $\cF_\Lambda^{\mathrm{orb}}$. The conclusion confines
to the Leech finite-orbifold model and stays compatible with the
...
The theorem provides a descended HT model beyond the generic class-M
statement only after the finite-orbifold BV hypotheses hold. The
Monster reaches class~G before the orbifold. The criterion requires:
\begin{itemize}
...
Jacobi identity (FLM~1988), Dong--Mason uniqueness, and the local
verification of the BV anomaly datum. The formal
identity $\det(1-(-1)|_\Lambda)=2^{24}$ is a compatibility check
within this list, separate from the BV obstruction.
\end{remark}
...
\item \emph{Type B} (Leech $\mathbb{Z}/2 = V^{\natural}$, 1 case):
 ...
 lattice; Leech rootlessness identifies the moonshine entry, and the
 finite-orbifold BV descent reduces to the single local datum
 $\alpha_{\mathrm{DW}} = 0$ (Proposition~\ref{prop:monster-alpha-explicit-zero}).
...
2023, arXiv:2303.11823) makes the Type~C enumeration canonical. The
Type~B mechanism is the Leech $\Z/2$ descent established above; Types~A
and~C are established in the Schellekens classification chapter
(Theorem~\ref{thm:schellekens-71-all-alpha-zero}).
...
\item Finite-orbifold descent is a homotopy-fixed-point construction
$V^{hG}=\Tot[V\rightrightarrows\Map(G,V)\to\Map(G^2,V)\to\cdots]$,
with a $\beta$-corrected associator when
$[\alpha_{\mathrm{orb}}]=0$.
