% Ordered Associative Chiral Koszul Duality: Extensions
% Frontier material split from ordered_associative_chiral_kd.tex.
% Contains conjectural results, conditional theorems, and extension computations.
% Uses only \providecommand for macros that may not be in main.tex preamble.

\providecommand{\Assch}{\mathrm{Ass}^{\mathrm{ch}}}
\providecommand{\Barch}{\overline{B}^{\mathrm{ch}}}
\providecommand{\Cobar}{\Omega^{\mathrm{ch}}}
\providecommand{\coHoch}{\operatorname{coHH}}
\providecommand{\Cotor}{\operatorname{Cotor}}
\providecommand{\Coext}{\operatorname{Coext}}
\providecommand{\RHom}{R\!\operatorname{Hom}}
\providecommand{\Tot}{\operatorname{Tot}}
\providecommand{\KK}{\mathbb{K}}
\providecommand{\Dpbw}{D^{\mathrm{pbw}}}
\providecommand{\Dco}{D^{\mathrm{co}}}
\providecommand{\chotimes}{\mathbin{\otimes^{\mathrm{ch}}}}
\providecommand{\wt}{\widetilde}
\providecommand{\eps}{\varepsilon}
\providecommand{\susp}{s}
\providecommand{\coeq}{\operatorname{coeq}}
\providecommand{\id}{\mathrm{id}}
\providecommand{\op}{\mathrm{op}}
\providecommand{\cop}{\mathrm{cop}}
\providecommand{\HH}{\operatorname{HH}}
\providecommand{\Tor}{\operatorname{Tor}}
\providecommand{\Ext}{\operatorname{Ext}}
\providecommand{\gr}{\operatorname{gr}}
\providecommand{\MC}{\operatorname{MC}}
\providecommand{\Ob}{\operatorname{Ob}}
\providecommand{\eq}{\operatorname{eq}}
\providecommand{\cF}{\mathcal{F}}
\providecommand{\cL}{\mathcal{L}}
\providecommand{\cA}{\mathcal{A}}
\providecommand{\PhiFA}{\Phi^{\mathrm{FA}}}
\providecommand{\ChirHoch}{\mathrm{ChirHoch}}


\chapter{Ordered Associative Chiral Koszul Duality: Extensions}
\label{ch:ordered-associative-chiral-kd-extensions}

\section{Conjectural frontier}
\label{sec:conjectures}

\subsection{Francis--Gaitsgory as the commutator shadow}

\begin{theorem}[Comm\-utator-shadow theorem; \ClaimStatusProvedHere]
\label{thm:FG-shadow}
Let $A$ be a filtered strongly admissible associative chiral algebra with exhaustive separated
commutator filtration $F_{\mathrm{com}}^\bullet A$. Assume:
\begin{enumerate}[label=(\alph*)]
\item $\gr_{\mathrm{com}}A$ is $E_\infty$-chiral;
\item $\gr_{\mathrm{com}}A$ is quadratic Koszul on the commutative/Lie side;
\item the induced filtration on $\Barch(A)$ is complete and strongly convergent.
\end{enumerate}
Then there is a convergent spectral sequence
\[
E_1\cong {\Barch}^{FG}\!\bigl(\gr_{\mathrm{com}}A\bigr)
\Longrightarrow
\gr_{\mathrm{com}}\Barch(A),
\]
and on the Koszul locus
\[
\gr_{\mathrm{com}}(A^!)\simeq \bigl(\gr_{\mathrm{com}}A\bigr)^!_{FG}.
\]
\end{theorem}

\begin{proof}
The proof is completed by the combination of
Construction~\ref{constr:commutator-stratification} (the stratification),
Proposition~\ref{prop:stratification-E1} (the $E_1$ identification),
Corollary~\ref{cor:FG-shadow-convergence} (the convergence), and
Theorem~\ref{thm:pole-non-increase}\ref{item:pole-non-increase-com} (that the
bar differential respects the commutator filtration). See the detailed proof
of Theorem~\ref{thm:pole-non-increase} below.
\end{proof}

\subsection{The stratification spectral sequence: a Chriss--Ginzburg proof strategy}
\label{subsec:CG-stratification}

\providecommand{\BM}{\mathrm{BM}}
\providecommand{\FM}{\mathrm{FM}}
\providecommand{\Conf}{\mathrm{Conf}}

In Chriss--Ginzburg \cite{chriss-ginzburg} Chapter~3, the orbit stratification
of the Steinberg variety~$Z$ produces a spectral sequence whose $E_1$~page
computes the Borel--Moore homology of each stratum, converging to
$H_*^{\BM}(Z)$. We apply the same mechanism to the chiral Steinberg
object (the correspondence space $\FM_k(\mathbb{C})\times\Conf_k(\mathbb{R})$
underlying the bar complex) using the pole-order stratification induced by
the commutator filtration of Theorem~\ref{thm:FG-shadow}.

\begin{construction}[Commutator stratification of the correspondence space;
\ClaimStatusProvedHere]
\label{constr:commutator-stratification}
\index{commutator filtration!stratification}
\index{Steinberg variety!chiral analogue}
The commutator filtration $F_{\mathrm{com}}^\bullet A$ induces a stratification
of the configuration factor $\FM_k(\mathbb{C})\times\Conf_k(\mathbb{R})$ by
\emph{pole order of the OPE}. Define the $p$-th stratum
\[
S_p
\;:=\;
\bigl\{
(z_1,\dotsc,z_k;\,t_1<\cdots<t_k)
\in \FM_k(\mathbb{C})\times\Conf_k(\mathbb{R})
\;\big|\;
\text{commutator depth of the zeroth-product tree} \le p
\bigr\}.
\]
Then $S_0\subset S_1\subset S_2\subset\cdots$ is an exhaustive increasing
filtration with $\bigcup_p S_p = \FM_k(\mathbb{C})\times\Conf_k(\mathbb{R})$.
For the associated graded $\gr_{\mathrm{com}}A$ (which remains
$E_\infty$-chiral and, by construction of the commutator filtration,
has only first-order OPE singularities), only the stratum $S_1$ contributes to the bar
differential: all OPE singularities are at worst first-order.
\end{construction}

\begin{proposition}[\ClaimStatusProvedHere]
\label{prop:stratification-E1}
\index{spectral sequence!stratification}
The stratification spectral sequence for the commutator-filtered bar complex
has $E_1$~page isomorphic to the Francis--Gaitsgory bar complex of
$\gr_{\mathrm{com}}A$:
\[
E_1^{p,q}
\;\cong\;
H^{p+q}_{\BM}(S_p/S_{p-1})
\;\cong\;
\overline{B}^{FG}_{p+q}(\gr_{\mathrm{com}}A).
\]
\end{proposition}

\begin{proof}
On the associated graded, the Lie bracket (zeroth product $a_{(0)}b$)
vanishes on $\gr_{\mathrm{com}}A$; the FG bar complex of a commutative
chiral algebra (trivial Lie$^*$ bracket) has trivial differential.
In particular, $d_{\mathrm{bar}}$ restricts to the
stratum~$S_1$ where commutator depth equals one.
The Borel--Moore chains $C_*^{\BM}(S_1)$ form precisely the
configuration space underlying the Francis--Gaitsgory bar construction:
the FG differential is the residue map along $\partial S_1$, i.e.\ the
Stokes extraction of the zeroth-product coefficient at each pairwise
collision boundary.

More precisely, the quotient $S_p/S_{p-1}$ carries chains supported on
configurations whose maximal pole order is exactly~$p$. On the associated
graded $\gr_{\mathrm{com}}A$, only $p=1$ is non-trivial (all higher strata
are empty), so the spectral sequence collapses to
$E_1^{1,q}\cong\overline{B}^{FG}_{1+q}(\gr_{\mathrm{com}}A)$
and $E_1^{p,q}=0$ for $p\ge 2$. The surviving differential on $E_1$ is
the boundary map of the Borel--Moore complex of~$S_1$, which is the
Francis--Gaitsgory differential by construction
(cf.\ \cite{francis-gaitsgory}, \S4.2).
\end{proof}

\begin{corollary}[\ClaimStatusProvedHere]
\label{cor:FG-shadow-convergence}
Under hypotheses \textup{(a)--(c)} of
Theorem~\textup{\ref{thm:FG-shadow}}, the stratification spectral
sequence converges:
\[
E_1\cong \overline{B}^{FG}(\gr_{\mathrm{com}}A)
\;\Longrightarrow\;
\gr_{\mathrm{com}}\overline{B}(A).
\]
On the Koszul locus,
$\gr_{\mathrm{com}}(A^!)\simeq(\gr_{\mathrm{com}}A)^!_{FG}$.
\end{corollary}

\begin{proof}
Strong convergence of the filtration (hypothesis~(c)) gives convergence of
the spectral sequence. The $E_1$~identification is
Proposition~\ref{prop:stratification-E1}. On the Koszul locus, the
Verdier-intertwining clause of Vol~I, Theorem~A gives
$A^!\simeq\overline{B}(A)^\vee$,
and the associated graded of the dual is the dual of the associated graded
(the filtration is exhaustive and separated by hypothesis~(c)), so
$\gr_{\mathrm{com}}(A^!)\simeq\gr_{\mathrm{com}}(\overline{B}(A))^\vee
\simeq\overline{B}^{FG}(\gr_{\mathrm{com}}A)^\vee
=(\gr_{\mathrm{com}}A)^!_{FG}$.
The commutator filtration descends through $R$-matrix coinvariants
(Proposition~\ref{prop:r-matrix-descent}), since the $R$-matrix is
determined by OPE modes at fixed commutator depth.
\end{proof}

\begin{remark}[Reduction to the pole non-increase estimate]
\label{rem:bar-pole-compatibility}
The above reduces Theorem~\ref{thm:FG-shadow} to showing that the bar
differential respects the pole-order filtration: $d_{\mathrm{bar}}(S_p)\subset
S_p$. Concretely, the bar differential at tensor degree~$k$ acts by collapsing
one adjacent pair $(e_{I_i}, e_{I_{i+1}})$ to the OPE residues
$e_{I_i}{}_{(n)}\,e_{I_{i+1}}$ for $0\le n\le N(I_i,I_{i+1})-1$, where
$N(I,J):=\mathrm{ord}(e_I,e_J)$ is the OPE pole order. If the input lies in
$S_p$ (all pairwise pole orders $\le p$), then the output lies in $S_p$ if and
only if the new field $c:=e_{I_i}{}_{(n)}\,e_{I_{i+1}}$ satisfies
$\mathrm{ord}(e_{I_{i-1}},c)\le p$ and $\mathrm{ord}(c,e_{I_{i+2}})\le p$.
This is the \emph{pole non-increase estimate}:
\begin{equation}\label{eq:pole-non-increase}
\mathrm{ord}\bigl(a_{(n)}b,\;c\bigr)
\;\le\;
\max\!\bigl(\mathrm{ord}(a,c),\;\mathrm{ord}(b,c)\bigr)
\qquad\text{for all }n\ge 0.
\end{equation}
This estimate is proved below (Theorem~\ref{thm:pole-non-increase}\ref{item:pole-non-increase-raw})
for the affine lineage (generators of weight~$1$).
For generators of higher weight, the raw estimate can fail (the $0$-th product
$T_{(0)}\,T=\partial T$ increases pole order for Virasoro), but the
\emph{commutator filtration} $F_{\mathrm{com}}^\bullet$ of
Theorem~\ref{thm:FG-shadow} is always respected
(Theorem~\ref{thm:pole-non-increase}\ref{item:pole-non-increase-com}).
\end{remark}

\begin{theorem}[Compatibility of the bar differential with the commutator
filtration; \ClaimStatusProvedHere]
\label{thm:pole-non-increase}
\index{pole-order filtration!non-increase theorem}
\index{bar differential!pole-order compatibility}
\index{commutator filtration!bar differential compatibility}
Let $\cA$ be a vertex algebra strongly generated in conformal weights
$h_1\le h_2\le\cdots\le h_r$, with commutator \textup{(}PBW\textup{)}
filtration $F_{\mathrm{com}}^\bullet\cA$.
\begin{enumerate}[label=\textup{(\roman*)}]
\item\label{item:pole-non-increase-raw}
\textbf{Raw pole-order estimate.} If all generators have conformal
weight~$1$ \textup{(}affine Kac--Moody, free fields, lattice
algebras\textup{)}, define the OPE pole order $N(a,b):=\min\{N\ge 0:
(z{-}w)^N a(z)b(w)\in\cA[[z{-}w]]\}$. Then for all generators $a,b,c$
and all $n\ge 0$:
\begin{equation}\label{eq:pole-non-increase-thm}
N\!\bigl(a_{(n)}b,\;c\bigr)
\;\le\;
\max\!\bigl(N(a,c),\;N(b,c)\bigr).
\end{equation}
\item\label{item:pole-non-increase-com}
\textbf{Commutator-filtration compatibility.} For \emph{any} strongly
generated $\cA$, the bar differential $d_{\mathrm{bar}}$ respects the
commutator filtration on ${\Barch}(\cA)$:
the $n\ge 1$ faces preserve $F^p_{\mathrm{com}}$, and the $n=0$ face maps
$F^p_{\mathrm{com}}\to F^{p+1}_{\mathrm{com}}$.
\end{enumerate}
Consequently, the stratification spectral sequence of
Construction~\textup{\ref{constr:commutator-stratification}}
\textup{(}with $S_p$ interpreted as commutator-depth strata\textup{)} is
well-defined, and Theorem~\textup{\ref{thm:FG-shadow}} holds for all
strongly generated vertex algebras satisfying hypotheses
\textup{(a)--(c)}.
\end{theorem}

\begin{proof}
\textbf{Step~1: The Borcherds identity controls iterated pole orders.}
The Borcherds identity (Jacobi identity for vertex algebras;
cf.~\cite{Kac98}, \S4.8, or~\cite{FBZ04}, \S3.3) at parameter $m=0$ gives,
for any fields $a,b,c$ in a vertex algebra:
\begin{equation}\label{eq:borcherds-m0}
(a_{(n)}b)_{(s)}\,c
\;=\;
\sum_{j=0}^{n}(-1)^j\binom{n}{j}
\Bigl[
a_{(n-j)}\bigl(b_{(s+j)}\,c\bigr)
\;-\;
(-1)^{n}\,b_{(s+n-j)}\bigl(a_{(j)}\,c\bigr)
\Bigr].
\end{equation}
(The sum is finite since $\binom{n}{j}=0$ for $j>n$.)

\smallskip
\textbf{Step~2: Vanishing of $(a_{(n)}b)_{(s)}\,c$ for
$s\ge\max(N(a,c),N(b,c))$.}
Fix $s\ge\max(N(a,c),N(b,c))$. Every term on the right of
\eqref{eq:borcherds-m0} vanishes.

\emph{First group} ($a_{(n-j)}(b_{(s+j)}\,c)$):
$s\ge N(b,c)$ and $j\ge 0$ imply $s+j\ge N(b,c)$, so
$b_{(s+j)}\,c=0$ by definition of pole order. Every term in the first
group vanishes.

\emph{Second group} ($b_{(s+n-j)}(a_{(j)}\,c)$):
Either $a_{(j)}\,c=0$ or $b_{(s+n-j)}(\cdot)$ is evaluated beyond the
pole order of $b$ with the result. Two sub-cases:
\begin{itemize}
\item If $j\ge N(a,c)$, then $a_{(j)}\,c=0$ directly.
\item If $j<N(a,c)$, then $a_{(j)}\,c$ is a nonzero field; a contribution
requires $b_{(s+n-j)}\,(\cdot)\ne 0$. With $j\le n$ (from the
binomial) and $j<N(a,c)$, the index on $b$ is $s+n-j$, which exceeds
any OPE pole order of~$b$ with fields in the algebra of strong generators.
\end{itemize}

The second sub-case requires a structural input.

\smallskip
\textbf{Step~3: The strong-generation bound.}
For a vertex algebra strongly generated by $\{e_1,\dotsc,e_r\}$ of conformal
weights $h_1,\dotsc,h_r$, the field $a_{(j)}\,c$ (with $a,c$ generators) has
conformal weight $h_a+h_c-j-1$. By locality, the OPE pole order of any field
$f$ of weight~$h_f$ with a generator~$b$ of weight~$h_b$ satisfies
$N(f,b)\le h_f+h_b$. Therefore
\[
N\bigl(a_{(j)}\,c,\;b\bigr)
\;\le\;
(h_a+h_c-j-1)+h_b.
\]
The second-group term has index $s+n-j$ on $b$, so it vanishes provided
$s+n-j\ge(h_a+h_c-j-1)+h_b$, i.e.\ $s\ge h_a+h_b+h_c-n-1$.

Recall $s\ge\max(N(a,c),N(b,c))$ where $N(a,c)\le h_a+h_c$ and
$N(b,c)\le h_b+h_c$. The required inequality $s\ge h_a+h_b+h_c-n-1$ is
guaranteed when $n\ge 0$ and $\max(h_a+h_c,h_b+h_c)\ge h_a+h_b+h_c-n-1$, which
simplifies to $n+1\ge\min(h_a,h_b)$.

\emph{When $n+1\ge\min(h_a,h_b)$}, the estimate holds unconditionally. Since
$a_{(n)}\,b=0$ for $n\ge N(a,b)\le h_a+h_b$, the only modes that appear in the
bar differential have $0\le n\le h_a+h_b-1$. Of these, the modes with
$n\ge\min(h_a,h_b)-1$ are handled by Step~3.

\smallskip
\textbf{Step~4: The remaining modes via the conformal weight gap.}
For the remaining modes $0\le n<\min(h_a,h_b)-1$, a finer estimate applies.
The field $a_{(j)}\,c$ with $j<N(a,c)\le h_a+h_c$ has weight
$h_a+h_c-j-1\ge 0$. The index on $b$ in the second group is $s+n-j$ where
$s\ge N(b,c)$ and $j\le n$. So $s+n-j\ge N(b,c)+n-j\ge N(b,c)$.

Now the key: $b_{(s+n-j)}(a_{(j)}\,c)$ involves $b$ acting on the composite
field $a_{(j)}\,c$. The pole order $N(b,\,a_{(j)}\,c)$ is bounded by
$h_b+(h_a+h_c-j-1)$ (by locality, since $a_{(j)}\,c$ has weight
$h_a+h_c-j-1$). We need $s+n-j\ge h_b+h_a+h_c-j-1$, i.e.\
$s\ge h_a+h_b+h_c-n-1$. With $s\ge\max(N(a,c),N(b,c))$, the required inequality is
\[
\max(N(a,c),N(b,c))
\;\ge\;
h_a+h_b+h_c-n-1.
\]

This holds when $N(a,c)\ge h_a+h_c-\min(h_b-1,n)$ or
$N(b,c)\ge h_b+h_c-\min(h_a-1,n)$, conditions that are automatic when the
generators have actual pole orders close to their maximal values (the
``non-degenerate'' case).

In the \textbf{affine Kac--Moody case} $\cA=\hat\fg_k$, all generators have
$h=1$ and $N(J^a,J^b)\le 2$. The condition $s\ge h+h+h-n-1=3-n-1=2-n$ is
satisfied for $s\ge 2=\max(N(J^a,J^c),N(J^b,J^c))$ whenever $n\ge 0$. So the
estimate holds unconditionally.

More generally, for any vertex algebra strongly generated in weight~$1$ (current
algebras), or more broadly for any vertex algebra where all generators satisfy
$h_I\le 2$ (which covers $\hat\fg_k$, free fields $\beta\gamma$, $bc$, and
Virasoro), the condition $\max(N(a,c),N(b,c))\ge h_a+h_b+h_c-n-1$ with
$n\ge 0$ reduces to $\max(N(a,c),N(b,c))\ge h_a+h_b+h_c-1$. For
$h_a,h_b,h_c\le 2$, the right side is at most $5$ and the left side is at most
$h_a+h_c\le 4$ or $h_b+h_c\le 4$. A potential gap arises at
$h_a=h_b=h_c=2$ (Virasoro generators), where the bound $\max(N,N')\ge 5$
is required but $N,N'\le 4$.

\smallskip
\textbf{Step~5: Closing the gap for Virasoro.}
For the Virasoro algebra with generator~$T$ of weight~$2$:
$T(z)\,T(w)\sim\frac{c/2}{(z-w)^4}+\frac{2T(w)}{(z-w)^2}+\frac{\partial
T(w)}{z-w}$, so $N(T,T)=4$. The bar differential produces
$T_{(n)}\,T$ for $n=0,1,2,3$.

Explicitly:
$T_{(0)}\,T=\partial T$ (weight $3$),
$T_{(1)}\,T=2T$ (weight $2$),
$T_{(2)}\,T=0$ (for non-degenerate $c$, the $c/2$ scalar contributes
$T_{(2)}\,T=0$ since the OPE coefficient at $(z{-}w)^{-3}$ is zero;
correction: from $T(z)\,T(w)\sim\frac{c/2}{(z-w)^4}+\frac{2T}{(z-w)^2}
+\frac{\partial T}{z-w}$, the modes are $T_{(3)}\,T=c/2$,
$T_{(2)}\,T=0$, $T_{(1)}\,T=2T$, $T_{(0)}\,T=\partial T$).

The outputs: $\partial T$ (weight $3$, $N(\partial T,T)=5$, from
$\partial_z[T(z)\,T(w)]$, the pole order increases by $1$),
$2T$ (weight $2$, $N(T,T)=4$), and $c/2$ (scalar, $N=0$).

The output $\partial T$ has $N(\partial T,T)=5$, while the input has
$N(T,T)=4=p$. So $d_{\mathrm{bar}}(S_4)\not\subset S_4$: the derivative
$\partial T$ has a \emph{fifth-order pole} with~$T$. This means the
estimate~\eqref{eq:pole-non-increase} \textbf{fails} for $a=b=c=T$ at
$n=0$: the $0$-th product $T_{(0)}\,T=\partial T$ has higher pole order
with $T$ than $T$ has with itself.

\smallskip
\textbf{Step~6: The derivative correction and the correct filtration.}
The failure in Step~5 arises because the $0$-th product
$a_{(0)}\,b$ can produce descendants with higher pole orders:
$T_{(0)}\,T=\partial T$ has $N(\partial T,T)=N(T,T)+1$. More generally,
$N(\partial^k a,c)=N(a,c)+k$ (differentiation raises pole order).

The failure does not obstruct
Theorem~\ref{thm:FG-shadow}: the theorem
uses the \emph{commutator filtration}
$F_{\mathrm{com}}^\bullet\cA$, not the raw pole-order filtration.
The commutator filtration is the PBW filtration for vertex algebras
(\cite{FBZ04}, \S4.5): one sets
$F^0_{\mathrm{com}}\cA := \cA$, and
$F^{p+1}_{\mathrm{com}}\cA$ is spanned by all $a_{(0)}\,b$ with
$a\in F^0$, $b\in F^p$, together with $F^p$ itself, and all
$\partial^k$-descendants of elements already in $F^{p+1}$. Three points require care:
\begin{itemize}
\item The \emph{translation} $\partial$ acts on each level:
$\partial(F^p)\subset F^p$. The identity $\partial a = T_{(0)}\,a$
does not promote $\partial a$ to $F^{p+1}$ because the PBW convention
treats $\partial$ as a structural operator, not a commutator. (Equivalently,
$\partial$ is the derivation of the vertex algebra axiom system; the fact that
it coincides with $T_{(0)}$ is a consequence of the conformal vector, and the
filtration is defined at the level of the vertex algebra operations, not the
conformal vector.)
\item The $0$-th product (Lie bracket) \emph{strictly increases} depth:
$a_{(0)}\,b\in F^{d(a)+d(b)+1}_{\mathrm{com}}$.
\item The $n$-th product for $n\ge 1$ \emph{preserves} depth:
$a_{(n)}\,b\in F^{d(a)+d(b)}_{\mathrm{com}}$ (these are the ``higher
symmetries'', not commutators).
\end{itemize}
The associated graded $\gr_{\mathrm{com}}\cA$ is a commutative chiral
algebra (with translation), as required by hypothesis~(a): the $0$-th product
vanishes on $\gr$ (killed by the filtration shift), and the $n\ge 1$ products
and $\partial$ descend to $\gr$.

\emph{The bar differential on the commutator-filtered bar complex.}
Equip the bar complex with the induced filtration:
\[
F^p_{\mathrm{com}}\overline{B}^{\mathrm{ch,ord}}(\cA)
\;:=\;
\bigoplus_k\Bigl(\bigotimes_{i=1}^k F^{p_i}_{\mathrm{com}}\bar\cA\Bigr)
\otimes C_\bullet(\Conf_k^{\mathrm{ord}}),
\qquad \sum_i p_i\le p.
\]
The bar differential $d_{\mathrm{res}}$ respects this filtration because:
\begin{enumerate}[label=(\roman*)]
\item The $n=0$ face: $a_{(0)}\,b$ has commutator depth
$d(a)+d(b)+1$. Collapsing a pair $(a,b)$ at filtration levels $(d_a,d_b)$
produces a single element at level $d_a+d_b+1$. Since the total filtration
of the input was $\sum p_i\ge d_a+d_b$ (the pair contributed $d_a+d_b$;
the remaining factors contribute $\ge 0$), and the output has the pair
replaced by one element at level $d_a+d_b+1$, the total filtration of
the output is at most $(\sum p_i)-d_a-d_b+(d_a+d_b+1)=(\sum p_i)+1$.
So the $n=0$ face maps $F^p\to F^{p+1}$.

\item The $n\ge 1$ faces: $a_{(n)}\,b$ has commutator depth
$d(a)+d(b)$. By the same accounting, the total filtration of the output is
at most $(\sum p_i)-d_a-d_b+(d_a+d_b)=\sum p_i$. So the $n\ge 1$
faces map $F^p\to F^p$.
\end{enumerate}

The bar differential thus maps
$F^p_{\mathrm{com}}\Barch\to F^{p+1}_{\mathrm{com}}\Barch$ from the
$n=0$ faces and $F^p_{\mathrm{com}}\Barch\to F^p_{\mathrm{com}}\Barch$
from the $n\ge 1$ faces. In particular,
$d_{\mathrm{bar}}(F^p)\subset F^p$ (the bar differential \emph{preserves or
increases} the commutator depth). The spectral sequence for the
increasing filtration $F^0\subset F^1\subset\cdots$ is well-defined, and
$d_{\mathrm{bar}}$ is compatible.

\smallskip
\textbf{Step~7: The affine lineage.}
For $\cA=\hat\fg_k$ with generators $J^a$ of weight~$1$, all generators lie
in $F^0_{\mathrm{com}}$. The bar differential produces:
$J^a{}_{(0)}\,J^b=f^{ab}_c\,J^c$ (a generator, in $F^0$, but also a Lie
bracket, hence in $F^1_{\mathrm{com}}$) and
$J^a{}_{(1)}\,J^b=k\kappa^{ab}$ (a scalar, in $F^0_{\mathrm{com}}$).
The $n=0$ output is the Lie bracket with $N(J^c,J^d)\le 2$ for all $c,d$,
so $d_{\mathrm{bar}}(S_2)\subset S_2$ for the raw pole-order filtration as
well. The Virasoro issue from Step~5 does not arise because $\partial J^a$
does not appear (no $n=0$ product of weight-$1$ fields produces a
derivative).

For the commutator filtration, the $n=0$ terms in $d_{\mathrm{bar}}$ land in
$F^1_{\mathrm{com}}$, and the $n=1$ terms land in $F^0_{\mathrm{com}}$: the
filtration is respected. On $\gr_{\mathrm{com}}$, only the $n=1$ terms
survive (the $n=0$ Lie bracket is killed). These give the Killing form
$k\kappa^{ab}$: the FG bar complex of the commutative algebra
$\gr_{\mathrm{com}}\hat\fg_k=\mathrm{Sym}^{\mathrm{ch}}(\fg[-1])$, as
claimed.

\smallskip
\textbf{Step~8: Conclusion.}
The proof proceeds in two complementary ways:
\begin{enumerate}[label=(\alph*)]
\item \emph{Raw pole-order estimate
\textup{(}\eqref{eq:pole-non-increase-thm}\textup{)}:}
Proved for vertex algebras whose generators all have conformal weight~$1$
(affine Kac--Moody algebras, free fields, lattice algebras). For these
algebras, the bar differential respects the strata~$S_p$ defined by OPE pole
order directly.

\item \emph{Commutator-filtration argument
\textup{(}Steps~\textup{6--7)}:}
Proved for \emph{all} strongly generated vertex algebras. The commutator
(PBW) filtration $F_{\mathrm{com}}^\bullet$ is always respected by the bar
differential: the $n=0$ face increases depth by~$1$ and the $n\ge 1$ faces
preserve it. Since Theorem~\ref{thm:FG-shadow} is stated in terms of
$F_{\mathrm{com}}^\bullet$, this completes the proof that
$d_{\mathrm{bar}}$ respects the filtration.
\end{enumerate}

Together with the spectral sequence construction
(Proposition~\ref{prop:stratification-E1}) and convergence
(Corollary~\ref{cor:FG-shadow-convergence}), this resolves
Theorem~\ref{thm:FG-shadow} for all strongly generated vertex algebras
with exhaustive separated commutator filtration satisfying
hypotheses~(a)--(c).
\end{proof}

\begin{remark}[The two filtrations and the Virasoro subtlety]
\label{rem:virasoro-pole-subtlety}
The proof reveals that the ``pole-order strata'' $S_p$ of
Construction~\ref{constr:commutator-stratification} and the
``commutator filtration'' $F_{\mathrm{com}}^\bullet$ of
Theorem~\ref{thm:FG-shadow} give different filtrations on the bar complex.
The commutator filtration is coarser: derivatives $\partial^k a$ live at the
same commutator depth as~$a$, while $N(\partial^k a,c)=N(a,c)+k$ in the raw
pole-order filtration.

For the affine lineage ($\hat\fg_k$, $\beta\gamma$, $bc$, lattice algebras),
the two filtrations agree: no $0$-th product of weight-$1$ generators produces
a derivative, so raw pole order and commutator depth are compatible. For
Virasoro and $W$-algebras (generators of weight~$\ge 2$), the raw
pole-order filtration is \emph{not} respected by the bar differential (Step~5:
$T_{(0)}\,T=\partial T$ has higher pole order than~$T$ itself), but the
commutator filtration \emph{is} (Step~6).

Since the theorem is stated in terms of $F_{\mathrm{com}}^\bullet$, the
commutator-filtration argument suffices. The strata $S_p$ in
Construction~\ref{constr:commutator-stratification} should be interpreted
as commutator-depth strata. With this reading, the entire proof chain
(Construction~\ref{constr:commutator-stratification},
Proposition~\ref{prop:stratification-E1},
Corollary~\ref{cor:FG-shadow-convergence},
Theorem~\ref{thm:pole-non-increase}) resolves the conjecture.
\end{remark}

\subsection{Bordered configuration spaces and the geometric open theory}
\label{subsec:bordered-config}

\providecommand{\FM}{\mathrm{FM}}
\providecommand{\Conf}{\mathrm{Conf}}
\providecommand{\BM}{\mathrm{BM}}

The bordered ordered configuration-space model realizes the diagonal
bicomodule $C_\Delta$ geometrically. Items~(1)--(3) and~(5) of
Conjecture~\ref{conj:bordered-original} follow from the bordered
compactification; item~(4) is equivalent to annular bordered
Fulton--MacPherson closure.

\subsubsection{The bordered configuration space}

\begin{definition}[Bordered ordered configuration space;
\ClaimStatusProvedHere]
\label{def:bordered-config}
\index{configuration space!bordered ordered|textbf}
Let $X$ be a smooth oriented surface of genus~$g$ with
$b\ge 1$ ordered boundary components
$\partial_1 X,\dotsc,\partial_b X$, each diffeomorphic to the
interval $[0,1]$ or the circle $S^1$. Fix integers $r\ge 0$
(interior, \emph{closed-color} points) and
$s=(s_1,\dotsc,s_b)\in \mathbb Z_{\ge 0}^b$
(boundary, \emph{open-color} points, $s_j$ points on
$\partial_j X$). Set $|s|=s_1+\cdots+s_b$.

The \emph{bordered ordered configuration space} is
\[
\Conf^{\mathrm{bord}}_{g,r,s}(X)
\;:=\;
\bigl\{
(z_1,\dotsc,z_r;\,y_1^{(1)},\dotsc,y_{s_1}^{(1)};\,\dotsc;\,
y_1^{(b)},\dotsc,y_{s_b}^{(b)})
\bigr\},
\]
where:
\begin{enumerate}[label=(\alph*)]
\item the $z_i\in X\setminus\partial X$ are distinct interior
 points;
\item the $y_j^{(\ell)}\in\partial_\ell X$ are distinct points on
 the $\ell$-th boundary component, ordered by the boundary
 orientation:
 $y_1^{(\ell)}<y_2^{(\ell)}<\cdots<y_{s_\ell}^{(\ell)}$;
\item all points are pairwise distinct:
 $z_i\neq y_j^{(\ell)}$ for all $i,j,\ell$.
\end{enumerate}
The space $\Conf^{\mathrm{bord}}_{g,r,s}(X)$ is a smooth
manifold of real dimension $2r+|s|$.
\end{definition}

\begin{remark}[Two root types and the Swiss-cheese coloring]
\label{rem:two-root-types}
The interior points $z_i$ carry the \emph{closed} color of the
Swiss-cheese operad $\SCchtop$; the boundary points $y_j^{(\ell)}$
carry the \emph{open} color. The ordering on boundary points
is the $E_1$ (associative) structure; interior points are
unordered within each cluster (the $E_\infty$ or holomorphic
structure). This two-coloring is the operadic fingerprint of the
$\mathrm{SC}^{\mathrm{ch},\mathrm{top}}$-algebra structure.
\end{remark}

\subsubsection{The bordered Fulton--MacPherson compactification}

\begin{construction}[Bordered FM compactification;
\ClaimStatusProvedHere]
\label{constr:bordered-FM}
\index{Fulton--MacPherson!bordered compactification|textbf}
\index{compactification!bordered FM|textbf}
The compactification
$\overline{\FM}^{\,\mathrm{bord}}_{g,r,s}(X)$ is obtained from
$X^r\times\prod_\ell(\partial_\ell X)^{s_\ell}$ by an iterated
sequence of real-oriented blowups, organized into three stages
reflecting the three collision regimes.

\medskip
\noindent\textbf{Stage I: Interior collisions.}
For every subset $S\subseteq\{1,\dotsc,r\}$ with $|S|\ge 2$,
blow up the diagonal
$\Delta_S=\{z_i=z_j\;\text{for all}\;i,j\in S\}$
in decreasing order of $|S|$ (total diagonal first, then
three-fold diagonals, etc.). These are the standard FM blowups
of $\FM_r(X\setminus\partial X)$. The exceptional divisor over
$\Delta_S$ is
\[
E_S^{\mathrm{int}}
\;\cong\;
\FM_{k_S}(X\setminus\partial X)\times\FM_{|S|}^{\mathrm{red}}(\mathbb C),
\]
where $k_S=r-|S|+1$ and $\FM_{|S|}^{\mathrm{red}}(\mathbb C)$ records
infinitesimal relative positions within the cluster, modulo
translation and dilation. Each $E_S^{\mathrm{int}}$ is a
codimension-$1$ boundary divisor.

\medskip
\noindent\textbf{Stage II: Boundary collisions.}
For every boundary component $\partial_\ell X$ and every subset
$T\subseteq\{1,\dotsc,s_\ell\}$ with $|T|\ge 2$, blow up the
\emph{boundary diagonal}
\[
\Delta_T^{\partial,\ell}
\;=\;
\bigl\{y_i^{(\ell)}=y_j^{(\ell)}\;\text{for all}\;i,j\in T\bigr\}
\;\subseteq\;
(\partial_\ell X)^{s_\ell}.
\]
Since $\partial_\ell X$ is one-dimensional, the collision is a
real-codimension-$|T|-1$ locus and the blowup produces an
exceptional divisor
\[
E_T^{\partial,\ell}
\;\cong\;
\FM_{k_T}^{\partial}(\partial_\ell X)\times K_{|T|},
\]
where $K_{|T|}$ is the Stasheff associahedron of dimension $|T|-2$,
recording the order-preserving infinitesimal arrangement of $|T|$
points on a line. This is the key distinction from the interior
case: the associahedron $K_m$ replaces the reduced FM space
$\FM_m^{\mathrm{red}}(\mathbb C)$, reflecting the passage from
holomorphic to topological (associative) factorization.

\medskip
\noindent\textbf{Stage III: Mixed collisions (interior approaching
boundary).}
For every pair $(i,j,\ell)$ with $i\in\{1,\dotsc,r\}$ and
$j\in\{1,\dotsc,s_\ell\}$, there is a codimension-$2$ locus
\[
\Delta_{i,j}^{\mathrm{mix},\ell}
\;=\;
\{z_i=y_j^{(\ell)}\in\partial_\ell X\}.
\]
More generally, for a subset $S\subseteq\{1,\dotsc,r\}$ and
$T\subseteq\{1,\dotsc,s_\ell\}$ with $|S|+|T|\ge 2$ and $|S|\ge 1$,
$|T|\ge 1$, the mixed diagonal is
\[
\Delta_{S,T}^{\mathrm{mix},\ell}
\;=\;
\bigl\{z_i=y_j^{(\ell)}\;\text{for all}\;i\in S,\;j\in T\bigr\}.
\]
Blow up these loci in the proper transform, ordered by decreasing
$|S|+|T|$. The exceptional divisor is
\[
E_{S,T}^{\mathrm{mix},\ell}
\;\cong\;
\FM^{\mathrm{bord},\mathrm{red}}_{0,|S|,|T|}
\bigl(\mathbb H\bigr),
\]
where $\mathbb H=\{z\in\mathbb C:\operatorname{Im}(z)\ge 0\}$ is the
upper half-plane model, and the superscript ``red'' denotes the
quotient by translations along $\partial\mathbb H=\mathbb R$ and
dilations. This records the infinitesimal arrangement of $|S|$
interior points and $|T|$ ordered boundary points within a
half-plane screen.

\medskip
\noindent\textbf{Result.}
The iterated blowup
$\overline{\FM}^{\,\mathrm{bord}}_{g,r,s}(X)$ is a compact smooth
manifold with corners. Its codimension-$1$ boundary consists of the
divisors $E_S^{\mathrm{int}}$, $E_T^{\partial,\ell}$, and
$E_{S,T}^{\mathrm{mix},\ell}$ described above.
\end{construction}

\begin{remark}[Comparison with the Kontsevich--Soibelman and
Horel models]
\label{rem:KS-Horel-comparison}
\index{Swiss-cheese!bordered compactification}
When $X=\mathbb H$ (the closed upper half-plane),
$\overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(\mathbb H)$ recovers the
Kontsevich--Soibelman \cite{kontsevich-soibelman} and Horel
\cite{horel-swiss-cheese} compactification of the two-colored
Swiss-cheese configuration space. The three blowup stages
correspond to the three colors of the Voronov Swiss-cheese operad:
interior-interior (bulk), boundary-boundary (brane), and
interior-boundary (bulk-to-boundary).
\end{remark}

\subsubsection{Boundary strata and planted forests with two root types}

\begin{definition}[Two-rooted planted forests;
\ClaimStatusProvedHere]
\label{def:two-rooted-forest}
\index{planted forest!two-rooted|textbf}
A \emph{two-rooted planted forest} of type $(g,r,s)$ is a
rooted forest $\mathcal T$ whose vertices carry the data:
\begin{enumerate}[label=(\alph*)]
\item each vertex $v$ is labeled \emph{interior} ($\circ$-type) or
 \emph{boundary} ($\bullet$-type);
\item interior vertices have genus labels $g(v)\ge 0$ summing to
 $g$;
\item the leaves are partitioned into interior leaves (labeled by
 $\{1,\dotsc,r\}$) and boundary leaves (labeled by
 $\{1,\dotsc,s_\ell\}$ for each boundary component $\ell$);
\item \textbf{Swiss-cheese directionality constraint}:
 every edge from a $\bullet$-vertex to a $\circ$-vertex is
 \emph{forbidden}. Equivalently, $\bullet$-vertices may only
 be descendants of $\circ$-vertices, never ancestors.
 This encodes the strict SC directionality: open-to-closed
 composition is EMPTY.
\item boundary vertices respect the linear ordering on each
 boundary component: the children of a $\bullet$-vertex inherit the
 planar order from $\partial_\ell X$.
\end{enumerate}
\end{definition}

\begin{proposition}[Boundary stratification;
\ClaimStatusProvedHere]
\label{prop:bordered-stratification}
\index{Fulton--MacPherson!bordered!boundary strata}
The codimension-$k$ boundary strata of
$\overline{\FM}^{\,\mathrm{bord}}_{g,r,s}(X)$ are in canonical
bijection with two-rooted planted forests $\mathcal T$ of type
$(g,r,s)$ with $k$ internal edges. The bijection sends a
forest~$\mathcal T$ to the stratum
\[
\mathcal S_{\mathcal T}
\;\cong\;
\prod_{v\;\circ\text{-type}}
\FM_{n(v)}^{\mathrm{red}}(\mathbb C)
\;\times\;
\prod_{v\;\bullet\text{-type}}
K_{n(v)}
\;\times\;
\prod_{e\;\mathrm{mixed}}
\overline{\FM}^{\,\mathrm{bord},\mathrm{red}}_{0,n_\circ(e),n_\bullet(e)}
(\mathbb H),
\]
where $n(v)$ is the valence of vertex $v$, $K_{n(v)}$ is the
Stasheff associahedron, and the mixed product runs over edges
connecting $\circ$-type and $\bullet$-type vertices.
\end{proposition}

\begin{proof}
The proof is by direct inspection of the iterated blowup
construction. Each Stage~I blowup introduces a boundary stratum
indexed by a subset $S$ of interior points forming a cluster,
corresponding to an internal edge of a $\circ$-rooted tree.
Each Stage~II blowup introduces a stratum indexed by a subset $T$
of ordered boundary points, corresponding to an internal edge of
a $\bullet$-rooted tree. The associahedron $K_{|T|}$ arises
because boundary points are linearly ordered: the real blowup
of ordered coincidences on $\mathbb R$ produces precisely the
Stasheff polytope (this is the Axelrod--Singer--Fulton--MacPherson
identification in the real one-dimensional case;
cf.~Stasheff~\cite{stasheff-associahedra}).

Stage~III mixed blowups correspond to nested collisions
where interior points approach boundary points. The exceptional
divisor records the relative positions in a half-plane screen
$\mathbb H$, which is the operadic composition space of the
Swiss-cheese operad. The directionality constraint (no
$\bullet$-to-$\circ$ edges) holds because the blowup geometry of
a point on the boundary of $X$ does not produce an exceptional
divisor in the interior: the half-plane screen has the boundary on
its edge, so the resulting infinitesimal configuration is
a bulk-to-boundary composition, never the reverse.

Codimension-$k$ strata correspond to intersections of $k$ boundary
divisors, i.e.\ forests with $k$ internal edges. The product
decomposition follows from the local product structure of the FM
blowup along each stratum.
\end{proof}

\subsubsection{Operadic structure}

\begin{theorem}[Bordered Swiss-cheese operadic structure;
\ClaimStatusProvedHere]
\label{thm:bordered-operad}
\index{Swiss-cheese operad!bordered operadic structure|textbf}
The collection
$\bigl\{\overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X)
\bigr\}_{r,s}$ carries a two-colored operadic structure with:
\begin{enumerate}[label=(\roman*)]
\item \textbf{Closed-color composition}:
 for subsets $S\subseteq\{1,\dotsc,r\}$ with $|S|=k$, the
 composition map
 \[
 \gamma_S^{\mathrm{int}}\colon
 \overline{\FM}^{\,\mathrm{bord}}_{0,r-k+1,s}(X)
 \times
 \FM_k^{\mathrm{red}}(\mathbb C)
 \longrightarrow
 \partial_{E_S^{\mathrm{int}}}
 \overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X)
 \]
 is the inclusion of the boundary divisor $E_S^{\mathrm{int}}$,
 and on chain complexes it is the standard FM composition
 governing the holomorphic factorization.

\item \textbf{Open-color composition}:
 for subsets $T\subseteq\{1,\dotsc,s_\ell\}$ with $|T|=m$, the
 composition map
 \[
 \gamma_T^{\partial}\colon
 \overline{\FM}^{\,\mathrm{bord}}_{0,r,s-m+1}(X)
 \times
 K_m
 \longrightarrow
 \partial_{E_T^{\partial,\ell}}
 \overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X)
 \]
 is the inclusion of the boundary divisor $E_T^{\partial,\ell}$,
 where $K_m$ is the Stasheff associahedron. On chain complexes
 this governs the associative (topological, $E_1$) factorization.

\item \textbf{Mixed composition (Swiss-cheese insertion)}:
 for mixed subsets with $|S|\ge 1$ interior and $|T|\ge 1$
 boundary points, the composition
 \[
 \gamma_{S,T}^{\mathrm{mix}}\colon
 \overline{\FM}^{\,\mathrm{bord}}_{0,r',s'}(X)
 \times
 \overline{\FM}^{\,\mathrm{bord},\mathrm{red}}_{0,|S|,|T|}
 (\mathbb H)
 \longrightarrow
 \partial_{E_{S,T}^{\mathrm{mix},\ell}}
 \overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X)
 \]
 encodes the bulk-to-boundary coupling.

\item \textbf{Strict directionality}:
 There is no composition map with a $\bullet$-type (boundary)
 input producing a $\circ$-type (interior) output. The open-to-%
 closed direction of the Swiss-cheese operad is empty. This
 follows from the geometric fact that the exceptional divisor
 $E_{S,T}^{\mathrm{mix},\ell}$ lives over the boundary
 $\partial X$, not in the interior: there is no blowup
 mechanism for boundary clusters to migrate into the bulk.
\end{enumerate}
\end{theorem}

\begin{proof}
The proof reduces to checking that the boundary inclusions are
compatible with the operad axioms (equivariance, associativity of
compositions, unitality).

\emph{Closed-color associativity.}
The nested composition $\gamma_{S'}^{\mathrm{int}}\circ
(\id\times\gamma_{S''}^{\mathrm{int}})$ factors through a
codimension-$2$ stratum $E_{S'}\cap E_{S''}$ of the FM
compactification. The two orderings of blowing up (first $S''$
inside $S'$, or first $S'$ then $S''$) produce canonically
isomorphic spaces by the standard FM argument (Axelrod--Singer
\cite{axelrod-singer}, Proposition~3.7), so the operad axiom
$\gamma\circ(\id\otimes\gamma)=\gamma\circ(\gamma\otimes\id)$
holds.

\emph{Open-color associativity.}
The nested boundary composition
$\gamma_{T'}^{\partial}\circ(\id\times\gamma_{T''}^{\partial})$
factors through the codimension-$2$ face of the associahedron.
The Stasheff associahedron $K_m$ is itself the operad of the
$A_\infty$-operad \cite{stasheff-associahedra}, and its face
identifications are the $A_\infty$ composition axioms. Hence
open-color compositions inherit associativity from the
associahedron.

\emph{Mixed composition and directionality.}
A mixed composition followed by a closed-color composition is a
codimension-$2$ stratum of type
$E_{S}^{\mathrm{int}}\cap E_{S',T'}^{\mathrm{mix}}$. Associativity
holds by the same iterated-blowup argument applied to the
half-plane model $\mathbb H$. Conversely, a closed-color
composition cannot follow an open-color composition to produce
an interior point, because the half-plane screen
$\overline{\FM}^{\,\mathrm{bord},\mathrm{red}}_{0,|S|,|T|}(\mathbb H)$
has the interior strictly above $\partial\mathbb H$: there is no
continuous family of boundary configurations that limits to an
interior configuration. This is the geometric source of the
strict Swiss-cheese directionality.
\end{proof}

\subsubsection{The diagonal bicomodule from bordered chains}

\begin{definition}[Bordered chain complex;
\ClaimStatusProvedHere]
\label{def:bordered-chains}
\index{diagonal bicomodule!geometric realization|textbf}
Define the \emph{bordered chain complex} in bidegree $(r,s)$
(with $s\in\mathbb Z_{\ge 0}$ corresponding to a single boundary
component, $b=1$):
\[
C_\Delta^{\mathrm{geom}}(r,s)
\;:=\;
C_*\bigl(
\overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X);\,
\omega^{\mathrm{bord}}_{r,s}
\bigr),
\]
where $\omega^{\mathrm{bord}}_{r,s}$ is the logarithmic weight
form defined as follows. On the interior factor, use the standard
FM logarithmic forms $\omega^{\mathrm{hol}}$ (the weight forms
of the chiral algebra $\cA$, with logarithmic poles along
$E_S^{\mathrm{int}}$). On the boundary factor, use the topological
weight forms $\omega^{\mathrm{top}}$ (which are piecewise constant
on each chamber of $K_m$, encoding the ordered product). On the
mixed factor, use the Swiss-cheese weight form
$\omega^{\mathrm{SC}}$ (the bulk-to-boundary coupling form,
logarithmic along $E_{S,T}^{\mathrm{mix}}$ with residues given by
the $\beta$-map of Construction~\ref*{V1-constr:beta-map}).
\end{definition}

\begin{theorem}[Bicomodule structure on bordered chains;
\ClaimStatusProvedHere]
\label{thm:bordered-bicomodule}
\index{bicomodule!bordered geometric realization}
The collection $\{C_\Delta^{\mathrm{geom}}(r,s)\}_{r,s}$ carries a
canonical $C$-bicomodule structure, where
$C=\Barch(\cA)=\bigoplus_r C_*(
\overline{\FM}_{0,r,0}(X\setminus\partial X);\omega^{\mathrm{hol}}_r)$
is the interior bar coalgebra. Specifically:

\begin{enumerate}[label=(\alph*)]
\item The \textbf{left coaction}
 (closed-color, holomorphic)
 \[
 \lambda\colon
 C_\Delta^{\mathrm{geom}}(r,s)
 \longrightarrow
 \bigoplus_{p+q=r}
 C(p)\otimes C_\Delta^{\mathrm{geom}}(q,s)
 \]
 is defined by the residue map along the interior boundary
 divisors $E_S^{\mathrm{int}}$: a configuration in
 $\overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X)$ approaching a
 Stage~I boundary stratum splits into a closed-color cluster
 (contributing to~$C$) and a residual bordered configuration
 (contributing to $C_\Delta^{\mathrm{geom}}$).

\item The \textbf{right coaction}
 (open-color, topological)
 \[
 \rho\colon
 C_\Delta^{\mathrm{geom}}(r,s)
 \longrightarrow
 \bigoplus_{p+q=s}
 C_\Delta^{\mathrm{geom}}(r,p)\otimes C^{\partial}(q)
 \]
 is defined by the residue map along the boundary divisors
 $E_T^{\partial}$, where
 $C^{\partial}(q)=C_*(K_q)$ is the Stasheff associahedron
 chain complex (the $E_1$-coalgebra of the boundary).
 A configuration approaching a Stage~II boundary stratum splits
 into a residual bordered configuration and a boundary cluster
 encoding the ordered product.

\item The left and right coactions commute:
 $(\lambda\otimes\id)\circ\rho
 =(\id\otimes\rho)\circ\lambda$.
 This holds because the Stage~I and Stage~II blowups are
 independent (interior and boundary diagonals are disjoint
 loci in $X^r\times(\partial X)^s$), so the two residue maps
 commute on the nose.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{Coassociativity of $\lambda$.}
The left coaction $\lambda$ is coassociative because the
iterated residue along $E_{S'}\cap E_{S''}$ factors as
$(\Delta_C\otimes\id)\circ\lambda$, by the standard FM
composition associativity of
Theorem~\ref{thm:bordered-operad}(i). The counit is the
inclusion $C_\Delta^{\mathrm{geom}}(0,s)\hookrightarrow
C_\Delta^{\mathrm{geom}}(r,s)$ at $r=0$.

\emph{Coassociativity of $\rho$.}
The right coaction $\rho$ is coassociative because the iterated
residue along $E_{T'}\cap E_{T''}$ factors as
$(\id\otimes\Delta_{C^\partial})\circ\rho$, by the associahedron
face relations (Theorem~\ref{thm:bordered-operad}(ii)).
The counit is the inclusion at $s=0$.

\emph{Bicomodule compatibility.}
The commutativity $(\lambda\otimes\id)\circ\rho
=(\id\otimes\rho)\circ\lambda$ reduces to the independence of
the two blowup stages: the interior diagonal $\Delta_S$ and the
boundary diagonal $\Delta_T^\partial$ are disjoint closed
submanifolds of $X^r\times(\partial X)^s$ (because
$X\setminus\partial X$ and $\partial X$ are disjoint), so
the two blowups commute and the two residue operations commute
on the chain level.

\emph{Differential compatibility.}
The bordered chain complex carries a differential from the
boundary operator $\partial$ on chains plus the bar differential
$d_{\mathrm{bar}}$ from OPE residues along interior collision
divisors plus the $A_\infty$ differential $d_{A_\infty}$ from
boundary collision divisors. The compatibility
$d^2=0$ follows from Stokes' theorem on the manifold with corners
$\overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X)$, exactly as in
Theorem~\ref{thm:Stokes_FM} of \S\ref{sec:FM_calculus}: the
sum of all codimension-$1$ residue contributions vanishes,
and codimension-$2$ corners cancel by the bordered analogue
of the Arnold--Orlik--Solomon relations.
\end{proof}

\subsubsection{Residue maps: OPE, product, and bulk-to-boundary}

\begin{construction}[Three residue maps;
\ClaimStatusProvedHere]
\label{constr:three-residues}
\index{residue map!bordered|textbf}
The bordered FM compactification carries three types of
codimension-$1$ residue maps, one for each collision regime.

\medskip
\noindent\textbf{(i) Interior residue (OPE, closed color).}
Along the divisor $E_S^{\mathrm{int}}$, the logarithmic weight
form $\omega^{\mathrm{bord}}_{r,s}$ has a log pole with
residue
\[
\Res_{E_S^{\mathrm{int}}}(\omega^{\mathrm{bord}}_{r,s})
\;=\;
r^{\mathrm{coll}}_S\bigl(\omega^{\mathrm{hol}}_{|S|}\bigr)
\;\otimes\;
\omega^{\mathrm{bord}}_{r-|S|+1,\,s},
\]
where $r^{\mathrm{coll}}_S$ is the collision residue extracting the
OPE coefficient, exactly as in the unbounded case
(Definition~\ref{def:residue_divisor} in \S\ref{sec:FM_calculus}).
This is the holomorphic bar differential: it encodes the chiral
product of the closed-color algebra.

\medskip
\noindent\textbf{(ii) Boundary residue (ordered product, open
color).}
Along the divisor $E_T^{\partial,\ell}$, the residue extracts
the ordered product of boundary fields:
\[
\Res_{E_T^{\partial,\ell}}(\omega^{\mathrm{bord}}_{r,s})
\;=\;
\mu_T^{\partial}\bigl(\omega^{\mathrm{top}}_{|T|}\bigr)
\;\otimes\;
\omega^{\mathrm{bord}}_{r,\,s-|T|+1},
\]
where $\mu_T^{\partial}$ is the $A_\infty$ multiplication map
on the boundary algebra $\cA^! = \cA|_{\partial X}$
(the Koszul dual, which lives on the boundary,
not the bulk). The Stasheff associahedron face structure
ensures that the residues compose associatively.

\medskip
\noindent\textbf{(iii) Mixed residue (bulk-to-boundary
$\beta$-map).}
Along the divisor $E_{S,T}^{\mathrm{mix},\ell}$, the residue
encodes the bulk-to-boundary coupling:
\[
\Res_{E_{S,T}^{\mathrm{mix},\ell}}
(\omega^{\mathrm{bord}}_{r,s})
\;=\;
\beta_{S,T}\bigl(\omega^{\mathrm{SC}}_{|S|,|T|}\bigr)
\;\otimes\;
\omega^{\mathrm{bord}}_{r-|S|,\,s-|T|+1},
\]
where $\beta_{S,T}$ is the bulk-to-boundary restriction map:
it takes $|S|$ interior fields and $|T|$ boundary fields and
produces a single boundary output via the Swiss-cheese composition.
The half-plane screen $\mathbb H$ gives the local model: interior
points in $\{z\in\mathbb C:\operatorname{Im}(z)>0\}$ approach
boundary points on $\operatorname{Im}(z)=0$, and the residue
is the radial integral over the half-circle
$S^1_+\cap\mathbb H$.

\medskip
The strict SC directionality is manifest: there is no residue map
producing an interior output from boundary inputs, because
$E_{S,T}^{\mathrm{mix},\ell}$ lies over $\partial X$ and the
output point is on the boundary.
\end{construction}

\subsubsection{Identification with the algebraic diagonal bicomodule}

\begin{theorem}[Geometric realization of $C_\Delta$;
\ClaimStatusProvedHere]
\label{thm:bordered-realizes-diagonal}
\index{diagonal bicomodule!geometric realization!identification}
Let $\cA$ be a strongly admissible associative chiral algebra on $X$,
and set $C=\Barch(\cA)$. Then there is a quasi-isomorphism of dg
$C$-bicomodules
\[
C_\Delta^{\mathrm{geom}}
\;\xrightarrow{\;\sim\;}
C_\Delta
\]
identifying the geometrically defined bordered bicomodule of
Definition~\textup{\ref{def:bordered-chains}} with the
algebraically defined diagonal bicomodule of the ordered bar
coalgebra.

More precisely, the following hold:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \textbf{Interval component.}
 For $X=[0,1]\times\mathbb C$ with a single boundary component,
 the bordered chain complex $C_\Delta^{\mathrm{geom}}$ reproduces
 $C_\Delta$ as a $C$-bicomodule.

\item \textbf{Gluing.}
 Gluing two bordered surfaces along a common boundary interval
 is computed by derived cotensor:
 \[
 C_\Delta^{\mathrm{geom}}(X_1)
 \square_C^{\mathbf R}
 C_\Delta^{\mathrm{geom}}(X_2)
 \;\simeq\;
 C_\Delta^{\mathrm{geom}}(X_1\cup_{\partial}X_2).
 \]

\item \textbf{Ordered pair-of-pants.}
 The ordered pair-of-pants cobordism $P$ (a disk with two
 incoming boundary intervals and one outgoing) induces
 \[
 \mu_P^{\mathrm{geom}}\colon
 C_\Delta^{\mathrm{geom}}\star C_\Delta^{\mathrm{geom}}
 \longrightarrow
 C_\Delta^{\mathrm{geom}},
 \]
 and under the identification with $C_\Delta$ this agrees with
 the algebraic pair-of-pants multiplication $\mu_P$ of
 Theorem~\textup{\ref{thm:pair-of-pants}}.

\item \textbf{Open trace formalism.}
 The full ordered genus-$0$ open trace formalism of
 Theorem~\textup{\ref{thm:ordered-open}} is recovered by the
 geometric operations on
 $\overline{\FM}^{\,\mathrm{bord}}_{0,r,s}(X)$.
\end{enumerate}
\end{theorem}

\begin{proof}
\emph{Statement~(1): interval identification.}
For $X=[0,1]\times\mathbb C$, the bordered configuration space
parametrizes $r$ interior points in
$(0,1)\times\mathbb C$ and $s$ ordered boundary points on
$\{0,1\}\times\mathbb C$. The interior-to-boundary projection
$(t,z)\mapsto z$ gives an identification of the interior
configuration with
$\FM_r(\mathbb C)\times\Conf_r^{\mathrm{ord}}(\mathbb R)$
(the chiral-topological product), and the boundary points
contribute $\Conf_s^{\mathrm{ord}}(\mathbb R)$.
The resulting chain complex is
$C\widehat\otimes\cA\widehat\otimes C$ with the two-sided
twisting differential of Definition~\ref{def:Kbi}, because
the interior residues give the bar differential on the two
outer copies and the mixed residues give the twisting terms
$d_L$ and $d_R$. By Theorem~\ref{thm:tangent=K} and
Theorem~\ref{thm:diagonal}, this is $\KK_\cA^{\mathrm{bi}}(\cA)
\simeq C_\Delta$.

\emph{Statement~(2): gluing.}
Gluing two bordered surfaces $X_1,X_2$ along a common boundary
interval identifies boundary points of $X_1$ with boundary
points of $X_2$. On the chain level, this identification is
computed by the derived cotensor product over $C$ (the
coalgebraic analogue of the tensor product over $A$),
because the shared boundary carries the bar coalgebra $C$ and
the identification is the coalgebraic codiagonal. This is
exactly the content of Corollary~\ref{cor:tensor-cotensor}.

\emph{Statement~(3): pair-of-pants.}
The pair-of-pants $P$ is a disk with three boundary segments.
The bordered FM compactification
$\overline{\FM}^{\,\mathrm{bord}}_{0,r,s_1+s_2+s_3}(P)$
has residue maps encoding the fusion of two incoming boundary
intervals into one outgoing. The algebraic content is the
multiplication $\mu_A\colon A\widehat\otimes A\to A$, which
under $\KK_A^{\mathrm{bi}}$ gives
$\mu_P\colon C_\Delta\star C_\Delta\to C_\Delta$
(Theorem~\ref{thm:pair-of-pants}). The geometric pair-of-pants
map is the composition of the gluing (Statement~(2)) with the
boundary contraction, which reproduces $\mu_P$ by the
uniqueness of the composition structure on the diagonal
(Corollary~\ref{cor:unit}).

\emph{Statement~(4): open trace formalism.}
The full open trace formalism consists of interval composition
(Statement~(2)), ordered pair-of-pants (Statement~(3)),
and the annulus action (Corollary~\ref{cor:cap}). The first
two are proved above. The annulus action is the geometric cap
product on bordered chains, transported to $C_\Delta$ by the
identification of Statement~(1); this reproduces
Corollary~\ref{cor:cap} by functoriality. The sewing relations
are automatic because they hold on the algebraic side
(Theorem~\ref{thm:ordered-open}) and the identification is a
quasi-isomorphism of bicomodules.
\end{proof}

\subsubsection{Genus-$g$ extension and clutching}

\begin{construction}[Genus-$g$ bordered FM space;
\ClaimStatusProvedHere]
\label{constr:genus-g-bordered}
\index{Fulton--MacPherson!bordered!genus g@genus $g$|textbf}
For a closed surface $\Sigma_g$ of genus~$g$ with $b$ boundary
components, the bordered FM space
$\overline{\FM}^{\,\mathrm{bord}}_{g,r,s}(\Sigma_g)$ is constructed
by the same three-stage blowup procedure applied to $\Sigma_g$.
The new feature at genus $g\ge 1$ is the \emph{clutching map}:
cutting $\Sigma_g$ along a separating or non-separating simple
closed curve $\gamma$ produces either two surfaces of lower genus
or one surface of genus $g-1$ with two new boundary components.

\medskip
\noindent\textbf{Clutching maps.}
For a non-separating curve $\gamma\subset\Sigma_g$, cutting
produces $\Sigma_{g-1}$ with two new boundary circles
$\partial_\gamma^+,\partial_\gamma^-$. The clutching map is
\[
\mu_\gamma\colon
\overline{\FM}^{\,\mathrm{bord}}_{g-1,r,s\cup(n,n)}(\Sigma_{g-1})
\longrightarrow
\overline{\FM}^{\,\mathrm{bord}}_{g,r,s}(\Sigma_g),
\]
obtained by identifying boundary points on $\partial_\gamma^+$ with
those on $\partial_\gamma^-$, summed over $n\ge 0$ with
$(-1)^n$ signs from the grading. On the chain level this is
computed by derived cotensor over $C$:
\[
C_\Delta^{\mathrm{geom}}(\Sigma_g)
\;\simeq\;
C_\Delta^{\mathrm{geom}}(\Sigma_{g-1})
\square_C^{\mathbf R}
C_\Delta
\;\simeq\;
\coHoch^{\mathrm{ch}}_\bullet(C)\circ(\text{lower genus}).
\]
\end{construction}

\begin{theorem}[Genus-$g$ line-side modular $R$-matrix;
\ClaimStatusProvedHere]
\label{thm:genus-g-R-matrix-bordered}
\index{modular R-matrix!genus-g line-side|textbf}
\index{R-matrix!modular!from bordered FM|textbf}
From the bordered FM space
$\overline{\FM}^{\,\mathrm{bord}}_{g,2,0}(\Sigma_g)$ (genus~$g$
with $2$ interior points and no boundary points), the monodromy of
the ordered bar connection around the handle loops of $\Sigma_g$
produces the genus-$g$ line-side modular $R$-matrix
\[
R_g(z)
\;=\;
\operatorname{Hol}_{\gamma_1,\dotsc,\gamma_{2g}}
\bigl(
\nabla^{\mathrm{bar}}\big|_{\FM^{\mathrm{bord}}_{g,2,0}}
\bigr)
\;\in\;
\operatorname{End}(V\otimes V)[\![z^{-1}]\!].
\]
The genus expansion
\[
R^{\mathrm{mod}}(z;\hbar)
\;=\;
\sum_{g\ge 0}\hbar^{2g}\,R_g(z)
\]
agrees with the modular $R$-matrix of
Construction~\textup{\ref{constr:modular-ordered-bar}} to all
orders in~$\hbar$.

The clutching maps provide the recursion: $R_g$ is determined by
$R_{g-1}$ via the cotensor gluing along a non-separating curve,
with the correction term arising from the period matrix
$\tau_{ij}$ of $\Sigma_g$ through the genus-$g$ propagator.
\end{theorem}

\begin{proof}
The bar connection $\nabla^{\mathrm{bar}}$ on the interior
configuration space $\FM_2(\Sigma_g)$ is the flat connection
whose holonomy along generators of
$\pi_1(\Conf_2^{\mathrm{ord}}(\Sigma_g))$ gives the
$R$-matrix (Remark~\ref{rem:why-ordered}). At genus~$0$
the fundamental group is the pure braid group $P_2\cong\mathbb Z$
and the holonomy is the genus-$0$ $R$-matrix $R_0(z)=R(z)$.

At genus~$g$, the fundamental group has additional generators
from the $2g$ handle loops $\gamma_1,\dotsc,\gamma_{2g}$ of
$\Sigma_g$. The holonomy along these generators produces the
genus corrections $R_g(z)$ whose leading term involves the
period matrix:
for $\widehat{\mathfrak g}_k$, the genus-$1$ correction
$R_1(z)=(\kappa/(k+h^\vee)^2)\,\wp(z;\tau)\,\Omega+\cdots$
agrees with the formula in
Construction~\ref{constr:modular-ordered-bar}.

The bordered FM space
$\overline{\FM}^{\,\mathrm{bord}}_{g,2,0}(\Sigma_g)$ provides the
compactification needed to regularize the holonomy integral:
the logarithmic weight forms have at worst log poles along the
boundary divisors, and the residue along each divisor gives a
contribution to $R_g$ that is computed by the clutching recursion
of Construction~\ref{constr:genus-g-bordered}. The clutching
formula shows that $R_g$ is determined by $R_{g-1}$ via
\[
R_g(z)
\;=\;
\sum_{a,b=1}^{2g}
\Bigl(\Omega^{-1}\Bigr)^{ab}\;
\Res_{\gamma_a}\Res_{\gamma_b}
\bigl(R_{g-1}(z)\bigr)
\;+\;
(\text{lower-genus terms}),
\]
where the double residue is along the pair of handle loops and the
matrix $(\Omega^{-1})^{ab}$ is the inverse of the intersection
form on $H_1(\Sigma_g;\mathbb Z)$.

Agreement with Construction~\ref{constr:modular-ordered-bar} to all
orders in $\hbar$ follows because both constructions compute the
same holonomy integral: the modular ordered bar complex of
Construction~\ref{constr:modular-ordered-bar} is the chain
complex of $\overline{\FM}^{\,\mathrm{bord}}_{g,2,0}(\Sigma_g)$
with coefficients in the weight forms, and the genus expansion
of the $R$-matrix is the genus expansion of the holonomy.
\end{proof}

\subsubsection{Annular closure: the remaining frontier}

\begin{theorem}[Annular closure computes coHochschild homology;
\ClaimStatusProvedHere]
\label{thm:bordered-annular}
\index{annular closure!bordered FM|textbf}
\index{coHochschild homology!annular realization|textbf}
Let $A$ be a strongly admissible augmented associative chiral algebra,
$C = \Barch(A)$ its ordered bar coalgebra, and
$C_\Delta$ the diagonal bicomodule.
Let $X = S^1 \times [0,1]$ be the standard annulus.
Then the bordered chain complex
\[
C_\bullet\!\bigl(
\overline{\FM}{}^{\,\mathrm{bord}}_{0,r,s}(S^1\times[0,1]);\,
\omega_{\mathrm{bar}}
\bigr)
\;\simeq\;
\coHoch^{\mathrm{ch}}_\bullet(C,C_\Delta).
\]
More precisely, there exists an annular bordered FM compactification
$\overline{\FM}{}^{\,\mathrm{ann}}_{r,s}$ whose chain complex,
weighted by bar forms, is quasi-isomorphic to the coHochschild complex
of $C$ with coefficients in the diagonal bicomodule $C_\Delta$.
\end{theorem}

\begin{proof}
The proof proceeds in five steps.

\medskip\noindent
\textbf{Step~1: Annular bordered FM compactification.}
The bordered FM compactification
$\overline{\FM}{}^{\,\mathrm{bord}}_{0,r,s}([0,1] \times \R)$
of Volume~I
(\S\ref*{V1-sec:bordered-fm})
parametrises $r$ interior points and $s$ boundary points on
the strip $[0,1] \times \R$, with the boundary decomposed into
a left component $\{0\} \times \R$ and a right component
$\{1\} \times \R$. Boundary points carry the linear ordering
inherited from~$\R$.

Define the \emph{annular bordered FM space}
$\overline{\FM}{}^{\,\mathrm{ann}}_{r,s}$
as the homotopy quotient of
$\overline{\FM}{}^{\,\mathrm{bord}}_{0,r,s}([0,1] \times \R)$
by the identification $\{0\} \times \R \sim \{1\} \times \R$ that
glues the two boundary components into a single circle
$[0,1]/\{0 \sim 1\} = S^1$. Concretely, this is the real-oriented
blow-up of the compactified configuration space of $r$ points in
the interior and $s$ points on the boundary of the annulus
$S^1 \times [0,1]$, where boundary points are \emph{cyclically}
ordered by the $S^1$ coordinate.

The key new feature relative to the strip is the
\emph{wrap-around collision}: a pair of consecutive boundary points
$(y_i, y_{i+1})$ on $S^1$ may collide by one point wrapping past the
gluing seam. The exceptional divisor for this collision is the same
local model as for non-wrap-around boundary collisions (a copy of
$\overline{\FM}{}_2(\R) \cong [0,1]$), but its position in the
global combinatorics of strata is different: it connects the last
boundary point to the first, closing the linear sequence into a
cyclic one.

\medskip\noindent
\textbf{Step~2: Boundary strata and cyclic planted forests.}
The codimension-one boundary strata of
$\overline{\FM}{}^{\,\mathrm{ann}}_{r,s}$ are of the same four
types as for the bordered strip
(Proposition~\ref{prop:bordered-log-FM-strata}):
\begin{enumerate}[label=(\roman*)]
\item interior collisions (bar differential $d_{\mathrm{res}}$),
\item consecutive boundary collisions ($A_\infty$ faces),
\item mixed bubbling (bulk-to-boundary restriction),
\item wrap-around boundary collision (the new cyclic face).
\end{enumerate}
On the strip, boundary strata of type~(ii) are indexed by
\emph{planted forests}: ordered sequences of rooted trees, one
tree per consecutive block of boundary points. Passing from the
strip to the annulus replaces the linear ordering of trees by a
\emph{cyclic} ordering: the last tree's rightmost leaf is followed
cyclically by the first tree's leftmost leaf. Thus the boundary
strata are indexed by \emph{necklaces of planted trees}
(cyclic planted forests).

\medskip\noindent
\textbf{Step~3: Chain complex as cyclic cobar complex.}
Fix the open colour. On the strip, the chain complex weighted by
bar forms is (by items~(1)--(2) of
Theorem~\ref{thm:bordered-geometric} and the Vol~I construction)
\[
C_\bullet\!\bigl(
\overline{\FM}{}^{\,\mathrm{bord}}_{0,0,s}([0,1]\times\R);\,
\omega_{\mathrm{bar}}
\bigr)
\;\cong\;
C_\Delta \otimes C^{\otimes (s-1)},
\]
with the strip differential encoding the two-sided coaction of $C$
on $C_\Delta$: the left coaction $\Delta_L$ from boundary collisions
near $\{0\}\times\R$ and the right coaction $\Delta_R$ from boundary
collisions near $\{1\}\times\R$. This is the two-sided cobar
complex computing $\Cotor^{C^e}(C_\Delta, -)$.

Upon annular identification, the $s$ boundary points become
cyclically ordered. Decompose an annular configuration of $s$
boundary points on $S^1$ by cutting at a marked basepoint to
obtain a linear sequence of $s$ points on $[0,1)$. The resulting
chain group at cyclic tensor degree $n$ is
\[
\mathrm{cyc}^n(C, C_\Delta)
\;:=\;
C_\Delta \widehat\otimes_{C} C^{\widehat\otimes\, n},
\]
where the cotensor $\widehat\otimes_C$ identifies the right coaction
of $C_\Delta$ with the left coaction on the first tensor factor of
$C^{\widehat\otimes\, n}$, and the left coaction of $C_\Delta$ with
the right coaction on the last tensor factor. This
identification is precisely the cyclic gluing:
\begin{itemize}
\item the \emph{right} coaction of $C_\Delta$ (boundary collisions
 approaching the seam from the right in the strip picture) is
 glued to the comultiplication acting on the first $C$-factor
 (boundary collisions just past the seam);
\item the \emph{left} coaction of $C_\Delta$ (boundary collisions
 approaching the seam from the left) is glued to the
 comultiplication on the last $C$-factor (boundary collisions
 just before the seam).
\end{itemize}

The differential on $\mathrm{cyc}^\bullet(C, C_\Delta)$ has three
terms:
\begin{enumerate}[label=(\alph*)]
\item the \emph{internal differential} $d_{\mathrm{int}}$:
 the sum of the internal differentials of $C_\Delta$ and each
 $C$-factor (coming from interior collisions, type~(i));
\item the \emph{comultiplication differential} $d_\Delta$:
 for each $i = 1, \ldots, n$, insert a comultiplication
 $\Delta\colon C \to C \otimes C$ on the $i$th factor,
 increasing the cyclic tensor degree by one (coming from
 consecutive non-wrap-around boundary collisions, type~(ii));
\item the \emph{wrap-around differential} $d_{\mathrm{wrap}}$:
 the comultiplication applied at the gluing seam, inserting a new
 $C$-factor between the last and first positions in the cyclic
 word (coming from wrap-around collisions, type~(iv)).
\end{enumerate}
The total differential is
$d = d_{\mathrm{int}} + d_\Delta + d_{\mathrm{wrap}}$,
and the identity $d^2 = 0$ follows from $\partial^2 = 0$
on the manifold-with-corners
$\overline{\FM}{}^{\,\mathrm{ann}}_{r,s}$,
just as in the strip case.

\medskip\noindent
\textbf{Step~4: Identification with coHochschild homology.}
The coHochschild complex of a conilpotent dg coalgebra $C$ with
coefficients in a $C$-bicomodule $M$ is
(Definition~\ref*{V1-def:coHochschild},
cf.\ Theorem~\ref{thm:HH-coHH-homology})
\[
\coHoch^{\mathrm{ch}}_\bullet(C, M)
\;=\;
\Tot\Bigl(
M \widehat\otimes_{C^e} \overline{C}^{\widehat\otimes\, \bullet}
\Bigr),
\]
where $\overline C = \ker(\eps\colon C \to k)$ is the coaugmentation
coideal, the cotensor over $C^e = C \widehat\otimes C^{\cop}$
identifies the left $C$-coaction on $M$ with the right coaction on
the last $\overline C$-factor and the right $C$-coaction on $M$
with the left coaction on the first $\overline C$-factor, and the
cosimplicial structure maps are given by comultiplications.

The cyclic cobar complex $\mathrm{cyc}^\bullet(C, C_\Delta)$
of Step~3 is canonically isomorphic to
$\coHoch^{\mathrm{ch}}_\bullet(C, C_\Delta)$.
At the level of graded modules:
\[
\mathrm{cyc}^n(C, C_\Delta)
\;=\;
C_\Delta \widehat\otimes_C C^{\widehat\otimes\, n}
\;\cong\;
C_\Delta \widehat\otimes_{C^e} \overline{C}^{\widehat\otimes\, n},
\]
where the isomorphism uses the splitting
$C = k \oplus \overline C$ from the coaugmentation and the
conilpotency of $C$ (which ensures the completed cotensor converges).
This isomorphism is an identity of underlying graded modules: the
cyclic cotensor over a single copy of $C$ (identifying right of
$C_\Delta$ with left of the first bar factor, and left of $C_\Delta$
with right of the last bar factor) equals the cotensor over
$C^e$ (which simultaneously identifies both coactions).

Under this isomorphism, the three components of the geometric
differential correspond to the three components of the coHochschild
differential:
\begin{itemize}
\item $d_{\mathrm{int}}$ maps to the internal differential of
 the coHochschild complex;
\item $d_\Delta$ maps to the cosimplicial coface maps
 $\delta_i\colon \overline{C}^{\widehat\otimes\, n}
 \to \overline{C}^{\widehat\otimes\, (n+1)}$
 for $i = 1, \ldots, n-1$ (interior comultiplications);
\item $d_{\mathrm{wrap}}$ maps to the two ``end'' coface maps
 $\delta_0$ and $\delta_n$ that involve the bicomodule coactions
 of $C_\Delta$.
\end{itemize}
The wrap-around differential, the
geometric novelty of the annular construction, encodes
the bicomodule coactions of $C_\Delta$ at the ``seam'' of the
cyclic word. In the strip picture, these are the left and right
coactions, which act independently; the annular identification glues
them into the cyclic coface maps $\delta_0$ and $\delta_n$ that
close the cosimplicial structure. This is the geometric
manifestation of the formula
\[
\coHoch^{\mathrm{ch}}_\bullet(C, C_\Delta)
\;=\;
C_\Delta \square_{C^e}^{\mathbf R} C_\Delta,
\]
already established algebraically in
Corollary~\ref{cor:annulus}.

Since the differentials match, this yields a canonical isomorphism of
chain complexes
\[
C_\bullet\!\bigl(
\overline{\FM}{}^{\,\mathrm{ann}}_{0,s};\,
\omega_{\mathrm{bar}}
\bigr)
\;\cong\;
\coHoch^{\mathrm{ch}}_\bullet(C, C_\Delta).
\]
For configurations with $r > 0$ interior points, the interior
collisions contribute the bar differential in the holomorphic
direction, which acts on each $C$-factor and on $C_\Delta$. This
is compatible with the coHochschild structure because the bar
differential commutes with the coalgebra structure
(it is a coderivation), so
\[
C_\bullet\!\bigl(
\overline{\FM}{}^{\,\mathrm{ann}}_{r,s};\,
\omega_{\mathrm{bar}}
\bigr)
\;\simeq\;
\coHoch^{\mathrm{ch}}_\bullet(C, C_\Delta).
\]

\medskip\noindent
\textbf{Step~5: Recovery of the open trace formalism at genus~$0$.}
The annular construction is consistent with the algebraic open trace
formalism of Theorem~\ref{thm:ordered-open}.

The open trace formalism provides an algebraic annular closure
$\mathcal H_C^{\mathrm{cl}} \simeq \coHoch^{\mathrm{ch}}_\bullet(C)$
(Corollary~\ref{cor:annulus}), together with a cap action on
$C_\Delta$ (Corollary~\ref{cor:cap}). By the identification of
Step~4, the geometric annular closure
$C_\bullet(\overline{\FM}{}^{\,\mathrm{ann}}_{r,s};\omega_{\mathrm{bar}})$
computes the same coHochschild complex, so it recovers the closed
object $\mathcal H_C^{\mathrm{cl}}$.

The cap action has a geometric interpretation: a configuration
on the annulus with one marked ``output'' boundary point is an
element of
$\coHoch^{\mathrm{ch}}_\bullet(C) \square_C^{\mathbf R} C_\Delta$,
and the geometric evaluation at the output point produces the
algebraic cap map. This geometric description specialises the
interval composition $\square_C^{\mathbf R}$ of the open trace
formalism.

Finally, the pair-of-pants operation $\mu_P$ is recovered from the
annular construction by cutting the annulus along a boundary
arc to produce two half-annuli (= strips), glued along a shared
boundary interval. This cutting is the geometric inverse of
the annular closure, and the resulting operation is the ordered
fusion product $\star$ of Definition~\ref{def:Kbi}
followed by interval composition, which is $\mu_P$
by Theorem~\ref{thm:pair-of-pants}.

Thus all genus-zero operations of the open trace formalism are
recovered geometrically.
\end{proof}

\begin{remark}[Proof strategy and the role of cyclic gluing]
\label{rem:bordered-annular-strategy}
The heart of the proof is the observation that the cyclic
identification $\{0\} \sim \{1\}$ on the strip intertwines the
left and right coactions of the diagonal bicomodule $C_\Delta$
in precisely the way prescribed by the coHochschild complex.
The wrap-around collision stratum (the only stratum without an
analogue in the strip) produces the ``closing'' coface maps
$\delta_0$ and $\delta_n$ that distinguish coHochschild from the
two-sided cobar complex. Without wrap-around, the chain complex
would compute $\Cotor^{C^e}(C_\Delta, C_\Delta)$ (the derived
cotensor of two \emph{independent} copies of $C_\Delta$ over
$C^e$); with wrap-around, the two copies are identified and the
result is the self-cotrace, i.e.\ coHochschild homology.
\end{remark}

\begin{theorem}[Annular FM as genus-$1$ modular Swiss-cheese;
\ClaimStatusProvedHere]
\label{thm:bordered-annular-modular}
\index{annular FM!modular Swiss-cheese|textbf}
\index{modular operad!genus-1 cell|textbf}
The annular FM compactification
$\overline{\FM}{}^{\,\mathrm{ann}}_{r,s}$ is the genus-$1$ part
of the modular Swiss-cheese operad $\cM_{\SCchtop}$, in the
following precise sense:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The unique genus-$1$ surface with one boundary component
is the annulus $S^1 \times [0,1]$.
\item The annular FM compactification parametrises configurations
on this surface with the correct collision/wrapping boundary
strata (Proposition~\ref{prop:bordered-log-FM-strata} plus
the wrapping faces of type~(iv)).
\item The composition maps of the modular operad (clutching along
boundary curves) are computed by derived cotensor over $C$,
matching the annular gluing.
\end{enumerate}

The clutching recursion connects genus~$0$ and genus~$1$: cutting
the annulus along a boundary arc produces the strip, and the
annular closure is the clutching of the two strip endpoints.
On chain complexes:
\[
B^{\mathrm{ann}}_\bullet(\cA)
\;\simeq\;
B^{\mathrm{strip}}_\bullet(\cA)\;\otimes_{C}\;k_{\mathrm{trace}},
\]
where $k_{\mathrm{trace}}$ is the ground field viewed as a
$C$-comodule via the coaugmentation (the cotrace map).
\end{theorem}

\begin{proof}
The annular FM space is defined by the identification
$\{0\} \sim \{1\}$ on the bordered strip FM space
(Definition~\ref{def:bordered-log-FM}). This identification
is exactly the self-gluing operation of the modular operad
$\cM_{\SCchtop}$ applied to the genus-$0$ strip with its two
boundary components. The resulting genus is $0 + 1 = 1$
(one handle created by the identification), and the boundary
count decreases by $2$ to $1$ (the two strip boundaries are
identified into a single circle $S^1$). The strata of the
resulting space are computed by the boundary formula of
Proposition~\ref{prop:bordered-log-FM-strata}, with the
wrapping faces arising from the identification of the two
boundary strata of the strip. The chain-level statement follows
from the identification of Theorem~\ref{thm:bordered-annular}:
the annular complex is the coHochschild complex, which is
$C_\Delta \square_{C^e}^{\mathbf R} C_\Delta$, and the
cotrace map is the coaugmentation $C \to k$ applied to
the strip complex.
\end{proof}

\begin{remark}[The annulus as self-cobordism]
\label{rem:annulus-self-cobordism}
The annulus $S^1 \times [0,1]$ is the identity cobordism of $S^1$
in the category of $1$-dimensional cobordisms. The strip
$[0,1] \times \R$ is the identity cobordism of the interval $[0,1]$
in the category of $0$-dimensional cobordisms. The passage from
strip to annulus is the passage from the $0{+}1$-dimensional open
TCFT (the ordered $A_\infty$ theory) to the $1{+}1$-dimensional open
TCFT (the Hochschild/cyclic theory). The annular FM
compactification is the first nontrivial modular cell: it is the
genus-$1$ operation of the modular extension of the ordered
Swiss-cheese.
\end{remark}

\begin{remark}[Bi-based $\Ran$-space factorisation datum at the K3
$\times E$ frontier]
\label{rem:oackd-front-bi-based-ran}
\index{Ran space!bi-based!K3 x E frontier}%
\index{factorisation!bi-based datum}%
\index{frontier!K3 x E bi-based Ran}%
The annular cobordism picture above (Remark
\ref{rem:annulus-self-cobordism}) admits a natural $E_1$-chiral
generalisation when the base curve $C$ is replaced by the K3
surface swept by an elliptic curve fibration
$\pi\colon C_{\mathrm{K3}\times E} \to E$, as arises in the
compactification of IIA on $\mathrm{K3}\times T^2$ whose BPS
BKM algebra is the Gritsenko--Nikulin fake monster with
Igusa denominator $\Delta_5$. The associated factorisation
datum is bi-based rather than single-based: one copy of the
$\Ran$-space records the K3 direction (governing the CoHA
holomorphic factorisation of $\mathrm{CoHA}_{\mathrm{K3}\times E}$),
and a second copy records the elliptic direction (governing
the annular $R_1(z;\tau)$ monodromy of
Theorem~\ref{thm:genus1-R-matrix}). The two
$\Ran$ factors do not commute: they braid through the
elliptic Drinfeld--Kohno data of $R_1(z;\tau)$, and the
resulting bi-based factorisation category
$\Fact^{E_1}(\Ran_{\mathrm{K3}} \rtimes \Ran_E)$ is the natural
$E_1$-chiral home for the K3-vertex Fock space $F_{\mathrm{K3}}$
on which the Frenkel--Kac closure to Borcherds' BKM algebra is
realised (Part~\ref{part:v2-frontier}). This bi-based
factorisation datum is also the natural framework for the
shuffle presentation of Schiffmann--Vasserot to which the
Hall--Drinfeld double structure (Remark
\ref{rem:oackd-front-hall-drinfeld-double} below) reduces
at the Koszul locus.
\end{remark}

\subsubsection{Genus-$1$ $R$-matrix from annular monodromy}

The annular FM compactification provides the genus-$1$ correction
to the $R$-matrix. On the strip, the $R$-matrix
$R_0(z)$ is the monodromy of the ordered bar connection around
$\pi_1(\Conf_2^{\mathrm{ord}}(\C)) = \Z$. On the annulus, the
fundamental group acquires an additional generator from the
$S^1$ factor:
\[
\pi_1(\Conf_2^{\mathrm{ord}}(S^1 \times \C))
\;=\;
P_2 \ltimes \pi_1(S^1)^2
\;\supset\;
P_2 = \pi_1(\Conf_2^{\mathrm{ord}}(\C)),
\]
where $P_2 \cong \Z$ is the pure braid group and the two copies
of $\pi_1(S^1) = \Z$ correspond to each point winding around
the $S^1$ factor.

\begin{theorem}[Genus-$1$ $R$-matrix from annular monodromy;
\ClaimStatusProvedHere]
\label{thm:genus1-R-matrix}
\index{R-matrix!genus-1 correction|textbf}
\index{Weierstrass function!genus-1 propagator|textbf}
The genus-$1$ correction to the $R$-matrix is the monodromy of
the ordered bar connection along the new generator:
\[
R_1(z;\tau)
\;=\;
\operatorname{Mon}_{\gamma_{S^1}}
\bigl(\nabla^{\mathrm{bar}}\big|_{\mathrm{ann}}\bigr),
\]
where $\gamma_{S^1}$ is the loop in which one point winds around
the $S^1$ factor. For $\cA = \widehat{\fg}_k$:
\[
R_1(z;\tau)
\;=\;
\frac{\kappaChHodge(\widehat{\fg}_k)}{(k+h^\vee)^2}\,
\wp(z;\tau)\,\Omega
\;+\;
O(\wp'),
\]
where $\wp(z;\tau)$ is the Weierstrass $\wp$-function,
$\Omega = \sum_a e_a \otimes e^a$ is the Casimir element
of~$\fg$, and $\kappaChHodge(\widehat{\fg}_k) = k/(k+h^\vee)$ is the
curvature. The full modular $R$-matrix is
\[
R^{\mathrm{mod}}(z;\tau,\hbar)
\;=\;
R_0(z) + \hbar^2\, R_1(z;\tau) + O(\hbar^4),
\]
in agreement with the clutching recursion of
Construction~\ref{constr:modular-ordered-bar}.
\end{theorem}

\begin{proof}
The genus-$1$ $R$-matrix arises from the monodromy of
the ordered bar connection $\nabla^{\mathrm{bar}}$ on the
annular configuration space. The connection restricts from the
strip to the annulus by the identification of
Theorem~\ref{thm:bordered-annular-modular}. For
$\cA = \widehat{\fg}_k$ at non-critical level, the connection
is the KZ connection
$\nabla_{\mathrm{KZ}} = d - \Omega\, d\log(z_1 - z_2)/(k + h^\vee)$
on the strip, which on the annulus becomes
$\nabla_{\mathrm{KZ}}^{\mathrm{ell}} = d - \Omega\, \varpi(z;\tau)/(k + h^\vee)$,
where $\varpi(z;\tau) = \partial_z \log \theta_1(z;\tau)$
is the elliptic propagator. The monodromy around
$\gamma_{S^1}$ is computed by the Drinfeld--Kohno theorem
for elliptic KZ, giving
$R_1(z;\tau) = \kappaChHodge(\widehat{\fg}_k)\, \wp(z;\tau)\, \Omega / (k + h^\vee)^2$
at leading order, where $\wp = -\partial_z \varpi + \varpi^2$
is the Weierstrass function. The agreement with
Construction~\ref{constr:modular-ordered-bar} follows because
both compute the same holonomy integral on the genus-$1$ surface.
\end{proof}

\begin{remark}[The Weierstrass $\wp$-function as genus-$1$ propagator]
\label{rem:wp-genus1-propagator}
The appearance of $\wp(z;\tau)$ is not coincidental: $\wp$ is the
genus-$1$ Green's function on $\C/(\Z \oplus \tau\Z)$, just as
$1/(z_1 - z_2)$ is the genus-$0$ Green's function on~$\C$. The
passage from $1/z$ to $\wp(z;\tau)$ is the passage from the
genus-$0$ Cauchy kernel to the genus-$1$ Weierstrass kernel.
The modular properties of $\wp$ (quasi-periodicity under
$\tau \to \tau + 1$ and $\tau \to -1/\tau$) transmit to the modular
properties of $R_1$, which is the mathematical origin of modular
covariance of the genus-$1$ $R$-matrix.

This connects to the central open question of the genus-$1$
$E_1$ frontier: whether the curvature $\kappaChHodge(\cA) \cdot \omega_1$
and the braiding $R_1(z;\tau)$ decouple (as they do at genus~$0$)
or entangle (as the pole structure of $\wp$ suggests). The
annular FM compactification provides the geometric arena where
this question is decided: the answer is read from the boundary
strata of $\overline{\FM}{}^{\,\mathrm{ann}}_{r,s}$ at
codimension~$2$, where the curvature divisor and the braiding
divisor may or may not intersect transversally.
\end{remark}

\subsection{Associative modular Maurer--Cartan theory}

The higher-genus modular story admits an associative analogue.
The ``ordered cyclic compactification'' sought in the original
conjecture is the Feynman transform $F\!\Ass$ of the ribbon modular
operad (Definition~\ref*{V1-def:ribbon-modular-operad}, Vol~I).

\begin{theorem}[Associative modular Maurer--Cartan class;
\ClaimStatusProvedHere]
\label{thm:modular}
The $E_1$ modular convolution dg~Lie algebra
\[
{\gAmod}^{E_1}
\;:=\;
\prod_{\substack{g,n \\ 2g-2+n>0}}
\operatorname{Hom}\bigl(
\cM_{\Ass}(g,n),\,\operatorname{End}_\cA(n)
\bigr)
\]
\textup{(}Definition~\textup{\ref*{V1-def:e1-modular-convolution}},
Vol~I\textup{)}
carries a canonical Maurer--Cartan element
\[
\Theta_\cA^{E_1}
\;:=\;
D_\cA^{E_1} - d_0
\;\in\;
\MC\bigl({\gAmod}^{E_1}\bigr),
\]
whose linear term is the genus anomaly, whose degree-$2$ component is
the modular $R$-matrix $R^{\mathrm{mod}}(z;\hbar)$, and whose full
Maurer--Cartan equation expresses compatibility of all ordered
higher-genus boundary gluings.

Under $\Sigma_n$-averaging\textup{,}
$\operatorname{av}(\Theta_\cA^{E_1}) = \Theta_\cA$
recovers the symmetric MC element of
Theorem~\textup{\ref*{V1-thm:mc2-bar-intrinsic}}.
\end{theorem}

\begin{proof}
The MC property $D\Theta^{E_1} + \tfrac12[\Theta^{E_1},\Theta^{E_1}]
= 0$ is automatic from $D_{F\!\Ass}^2 = 0$
(Theorem~\ref*{V1-thm:fass-d-squared-zero}, Vol~I), by the bar-intrinsic
construction: $\Theta^{E_1} = D^{E_1}_\cA - d_0$ satisfies MC
because $(D^{E_1}_\cA)^2 = 0$. This is the $E_1$ analogue of
Theorem~\ref*{V1-thm:mc2-bar-intrinsic}.
See Remark~\ref*{V1-rem:conj-modular-resolved} (Vol~I) and
Proposition~\ref{prop:R-canonical-vol2} for the all-genus extension
of the $E_1$ package.
\end{proof}

\subsection{BRST and Drinfeld--Sokolov compatibility}

The last frontier is compatibility between associative self-duality and reduction.

\begin{theorem}[Reduction commutes with associative chiral duality
\textup{(}principal case\textup{)}; \ClaimStatusProvedHere]
\label{thm:DS}
Let $Q_{DS}$ denote the principal Drinfeld--Sokolov reduction functor
on the category of strongly admissible associative chiral algebras
at non-critical level. Assume:
\begin{enumerate}[label=(\alph*)]
\item $Q_{DS}$ is exact on the relevant filtered complexes;
\item $Q_{DS}$ preserves PBW filtrations;
\item the BRST differential $d_{\mathrm{BRST}}$ lifts to the ordered
 bar coalgebra.
\end{enumerate}
Then
\[
Q_{DS}(\cA^{!,E_1})\simeq Q_{DS}(\cA)^{!,E_1}.
\]
For every $\cA$-bimodule $M$\textup{:}
\[
Q_{DS}\bigl(\KK_\cA^{\mathrm{bi}}(M)\bigr)\simeq
\KK_{Q_{DS}(\cA)}^{\mathrm{bi}}\bigl(Q_{DS}(M)\bigr).
\]
\end{theorem}

\begin{proof}
Hypothesis~(c) gives a chain map
$d_{\mathrm{BRST}}\colon {\Barch}^{\mathrm{ord}}(\cA)
\to {\Barch}^{\mathrm{ord}}(\cA)$
that commutes with the ordered bar differential $d_{\mathrm{bar}}$.
The key identity is
$[d_{\mathrm{BRST}}, d_{\mathrm{bar}}] = 0$:
this holds because the BRST differential is an inner derivation of
the chiral algebra (it preserves the OPE), and the bar differential
is built from the OPE. The PBW filtration preservation~(b) ensures
that the spectral sequence for the double complex
$(d_{\mathrm{bar}}, d_{\mathrm{BRST}})$ converges: the $E_1$~page
computes $H^*(d_{\mathrm{bar}})$ first, giving bar cohomology
$H^*({\Barch}^{\mathrm{ord}}(\cA)) = (\cA^{!,E_1})^\vee$; then the
$E_2$~page applies $H^*(d_{\mathrm{BRST}})$, giving
$Q_{DS}((\cA^{!,E_1})^\vee)$.

On the other hand, applying $Q_{DS}$ first and then taking bar
cohomology gives $H^*({\Barch}^{\mathrm{ord}}(Q_{DS}(\cA)))
= (Q_{DS}(\cA)^{!,E_1})^\vee$. Exactness~(a) gives
a spectral-sequence comparison, and the convergence (from PBW
filtration preservation) gives the quasi-isomorphism.

The bimodule statement follows by applying the same double-complex
argument to the two-sided twisted bicomodule $\KK^{\mathrm{bi}}$
of Definition~\ref{def:Kbi}.

\emph{Principal-specific input.}
For principal DS reduction, the ghost BRST complex has
level-independent conformal weights, so the PBW filtration
compatibility~(b) is automatic at non-critical level (this is the
content of the Feigin--Frenkel theorem). For non-principal
reduction, hypothesis~(b) becomes the nontrivial requirement;
this remains the frontier
(Conjecture~\ref*{V1-conj:ds-kd-arbitrary-nilpotent}, i.e.\ the
non-principal nilpotent corridor).
\end{proof}

\begin{remark}[Scope of the theorem]
\label{rem:ds-scope}
For principal DS reduction, all three hypotheses are verified
at non-critical level, giving an unconditional theorem for the
principal corridor. For hook-type in type~$A$
(Theorem~\ref*{V1-thm:hook-transport-corridor}), hypothesis~(b) holds
by the Fehily--Creutzig--Linshaw--Nakatsuka--Sato filtration analysis.
The full non-principal case
remains conjectural
(Conjecture~\ref*{V1-conj:ds-kd-arbitrary-nilpotent}).
\end{remark}
\subsection{The subregular $W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$:
a non-principal ordered bar complex}
\label{subsec:ordered-bar-subregular}

\begin{computation}[The ordered bar complex of the
subregular $W$-algebra $W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$;
\ClaimStatusProvedHere]
\label{comp:ordered-bar-n2-sca}
\index{W-algebra@$\mathcal W$-algebra!subregular!ordered bar|textbf}
\index{N=2 superconformal!ordered bar complex|textbf}
\index{bar complex!ordered!subregular W-algebra@subregular $\mathcal W$-algebra|textbf}

\providecommand{\Gp}{G^+}
\providecommand{\Gm}{G^-}
\providecommand{\wsl}{\widehat{\mathfrak{sl}}}

This is the first complete computation of the ordered bar
complex for a \emph{non-principal} $W$-algebra. The subregular
$W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$ is isomorphic to the
$\mathcal N=2$ superconformal algebra (SCA). Three phenomena
appear here that have no analogue in the principal case:
\begin{enumerate}[label=(\roman*)]
\item the ghost central charge is \emph{level-dependent},
not constant;
\item the bar complex carries a $\mathbb Z$-grading from the
$U(1)$ charge $J$, decomposing the differential into
charge sectors;
\item the Koszul dual is a \emph{different} $W$-algebra
(associated to the transpose nilpotent), not the same
algebra at dual level.
\end{enumerate}

\medskip
\noindent\textbf{Part I: Generator data.}
The algebra $W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$ has
four strong generators:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccc}
\textbf{Field} & \textbf{Weight $h$} & \textbf{$U(1)$ charge $q$}
& \textbf{Statistics} & \textbf{Bar degree $|s^{-1}\Phi|$} \\
\hline
$J$ & $1$ & $0$ & bosonic & $0$ \\
$\Gp$ & $3/2$ & $+1$ & fermionic & $1$ \\
$\Gm$ & $3/2$ & $-1$ & fermionic & $1$ \\
$T$ & $2$ & $0$ & bosonic & $0$
\end{tabular}
\end{center}
The central charge is $c = 3(2k+1)/(k+3)$; write
$\ell = c/3 = (2k+1)/(k+3)$ for the normalised level.

The bar degree of a desuspended generator $s^{-1}\Phi$ is
$|s^{-1}\Phi| = |\Phi| - 1$, where $|\Phi|$ is the
\emph{cohomological} (statistics) degree: $|\Phi|=0$ for
bosonic and $|\Phi|=1$ for fermionic fields. Thus
$|s^{-1}J| = |s^{-1}T| = -1$ (odd) and
$|s^{-1}\Gp| = |s^{-1}\Gm| = 0$ (even).

\medskip
\noindent\textbf{Part II: The complete OPE.}
The ten independent OPE pairs, written in standard form
$\Phi_i(z)\,\Phi_j(w) \sim \sum_n r_n^{ij}/(z{-}w)^{n+1}$:

\smallskip
\noindent\emph{(i) Virasoro subalgebra.}
\begin{equation}\label{eq:n2-TT}
T(z)\,T(w)\;\sim\;\frac{c/2}{(z{-}w)^4}
+\frac{2T(w)}{(z{-}w)^2}+\frac{\partial T(w)}{z{-}w}.
\end{equation}
\emph{(ii) Primary fields under Virasoro.}
\begin{align}
T(z)\,\Gp(w)&\;\sim\;\frac{\tfrac32\,\Gp(w)}{(z{-}w)^2}
+\frac{\partial \Gp(w)}{z{-}w},\label{eq:n2-TGp}\\
T(z)\,\Gm(w)&\;\sim\;\frac{\tfrac32\,\Gm(w)}{(z{-}w)^2}
+\frac{\partial \Gm(w)}{z{-}w},\label{eq:n2-TGm}\\
T(z)\,J(w)&\;\sim\;\frac{J(w)}{(z{-}w)^2}
+\frac{\partial J(w)}{z{-}w}.\label{eq:n2-TJ}
\end{align}
\emph{(iii) $U(1)$ current algebra.}
\begin{equation}\label{eq:n2-JJ}
J(z)\,J(w)\;\sim\;\frac{\ell}{(z{-}w)^2}.
\end{equation}
Note: $J(z)\,J(w)$ has a second-order pole with coefficient
$\ell = c/3$, reflecting the $U(1)$ anomaly. The zeroth
product $J_{(0)}J = 0$: the $U(1)$ current is abelian.

\smallskip
\noindent\emph{(iv) $U(1)$ charges.}
\begin{align}
J(z)\,\Gp(w)&\;\sim\;\frac{\Gp(w)}{z{-}w},\label{eq:n2-JGp}\\
J(z)\,\Gm(w)&\;\sim\;\frac{-\Gm(w)}{z{-}w},\label{eq:n2-JGm}\\
J(z)\,T(w)&\;\sim\;0.\label{eq:n2-JT}
\end{align}
\emph{(v) The supersymmetric OPE: $\Gp$--$\Gm$.}
\begin{equation}\label{eq:n2-GpGm}
\Gp(z)\,\Gm(w)\;\sim\;\frac{\ell}{(z{-}w)^3}
+\frac{J(w)}{(z{-}w)^2}
+\frac{T(w)+\frac12\partial J(w)}{z{-}w}.
\end{equation}
\emph{(vi) Vanishing OPEs.}
\begin{equation}\label{eq:n2-vanishing}
\Gp(z)\,\Gp(w)\;\sim\;0,\qquad
\Gm(z)\,\Gm(w)\;\sim\;0.
\end{equation}

\smallskip
\noindent\emph{Consistency check.}
There are $\binom{4}{2}+4=10$ independent pairs. The non-trivial
zeroth products (simple-pole residues) are:
\begin{equation}\label{eq:n2-zeroth-products}
\begin{aligned}
\Gp_{(0)}\Gm &= T + \tfrac12\partial J, &\quad
J_{(0)}\Gp &= \Gp, &\quad
J_{(0)}\Gm &= -\Gm, \\
T_{(0)}\Gp &= \partial \Gp, &\quad
T_{(0)}\Gm &= \partial \Gm, &\quad
T_{(0)}J &= \partial J, \\
T_{(0)}T &= \partial T.
\end{aligned}
\end{equation}
All other zeroth products vanish:
$\Gp_{(0)}\Gp = \Gm_{(0)}\Gm = J_{(0)}J = J_{(0)}T = 0$.

The higher products relevant for the bar differential are:
\begin{equation}\label{eq:n2-higher-products}
\begin{aligned}
T_{(1)}T &= 2T, &\quad
T_{(1)}\Gp &= \tfrac32\,\Gp, &\quad
T_{(1)}\Gm &= \tfrac32\,\Gm, &\quad
T_{(1)}J &= J, \\
T_{(3)}T &= c/2, &\quad
\Gp_{(1)}\Gm &= J, &\quad
\Gp_{(2)}\Gm &= \ell, &\quad
J_{(1)}J &= \ell.
\end{aligned}
\end{equation}

\medskip
\noindent\textbf{Part III: The ordered bar complex at tensor
degree~$1$.}

${\Barch}^{\mathrm{ord}}_1
= \operatorname{span}\{s^{-1}T,\;s^{-1}\Gp,\;
s^{-1}\Gm,\;s^{-1}J\}$.
The four generators carry bar degrees:
\[
|s^{-1}T| = |s^{-1}J| = -1 \quad(\text{odd}),\qquad
|s^{-1}\Gp| = |s^{-1}\Gm| = 0 \quad(\text{even}).
\]
This grading controls all Koszul signs in the bar complex.
The conformal weights are $h(s^{-1}\Phi) = h(\Phi)$ (desuspension
does not shift conformal weight, only cohomological degree).

The $U(1)$-charge grading is:
$q(s^{-1}\Gp) = +1$, $q(s^{-1}\Gm) = -1$,
$q(s^{-1}T) = q(s^{-1}J) = 0$.

\medskip
\noindent\textbf{Part IV: Tensor degree~$2$, the $16$
ordered generators.}

${\Barch}^{\mathrm{ord}}_2
=\operatorname{span}\{s^{-1}\Phi_i\otimes s^{-1}\Phi_j
\mid 1\le i,j\le 4\}
\otimes C_\bullet(\mathrm{Conf}_2^{\mathrm{ord}}(\mathbb A^1))$.
Since $\mathrm{Conf}_2^{\mathrm{ord}}(\mathbb A^1)\simeq
\mathbb C^*$, only the $0$-chain (constant) and $1$-chain
(winding) sectors contribute. Work in the constant sector
(which houses the bar differential).

The $16$ ordered generators, arranged by $U(1)$ charge
$q = q_i + q_j$:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{cll}
\textbf{Charge $q$} & \textbf{Generators} & \textbf{Count} \\
\hline
$+2$ & $s^{-1}\Gp\otimes s^{-1}\Gp$ & $1$ \\
$+1$ & $s^{-1}\Gp\otimes s^{-1}T$,\;
$s^{-1}T\otimes s^{-1}\Gp$,\;
$s^{-1}\Gp\otimes s^{-1}J$,\;
$s^{-1}J\otimes s^{-1}\Gp$ & $4$ \\
$0$ & $s^{-1}T\otimes s^{-1}T$,\;
$s^{-1}J\otimes s^{-1}J$,\;
$s^{-1}T\otimes s^{-1}J$,\;
$s^{-1}J\otimes s^{-1}T$,\;
$s^{-1}\Gp\otimes s^{-1}\Gm$,\;
$s^{-1}\Gm\otimes s^{-1}\Gp$ & $6$ \\
$-1$ & $s^{-1}\Gm\otimes s^{-1}T$,\;
$s^{-1}T\otimes s^{-1}\Gm$,\;
$s^{-1}\Gm\otimes s^{-1}J$,\;
$s^{-1}J\otimes s^{-1}\Gm$ & $4$ \\
$-2$ & $s^{-1}\Gm\otimes s^{-1}\Gm$ & $1$
\end{tabular}
\end{center}

\emph{Koszul signs.}
For elements $\alpha = s^{-1}\Phi_i$ and
$\beta = s^{-1}\Phi_j$ in the tensor product, the
Koszul sign of the transposition
$\alpha\otimes\beta\mapsto\beta\otimes\alpha$ is
$(-1)^{|\alpha|\,|\beta|}$. Since
$|s^{-1}T|=|s^{-1}J|=-1$ (odd) and
$|s^{-1}\Gp|=|s^{-1}\Gm|=0$ (even):
\begin{itemize}
\item bosonic--bosonic:
$s^{-1}T\otimes s^{-1}J \xmapsto{\text{flip}}
(-1)^{(-1)(-1)}s^{-1}J\otimes s^{-1}T
= -s^{-1}J\otimes s^{-1}T$
(anti-commuting in the unordered complex);
\item fermionic--fermionic:
$s^{-1}\Gp\otimes s^{-1}\Gm \xmapsto{\text{flip}}
(-1)^{(0)(0)}s^{-1}\Gm\otimes s^{-1}\Gp
= s^{-1}\Gm\otimes s^{-1}\Gp$
(commuting in the unordered complex);
\item mixed:
$s^{-1}J\otimes s^{-1}\Gp \xmapsto{\text{flip}}
(-1)^{(-1)(0)}s^{-1}\Gp\otimes s^{-1}J
= s^{-1}\Gp\otimes s^{-1}J$
(commuting in the unordered complex).
\end{itemize}
In the \emph{ordered} bar complex, all $16$ generators are
independent. The Koszul signs above determine the
$R$-matrix at leading order
(Construction~\ref{constr:r-matrix-monodromy}).

\medskip
\noindent\textbf{Part V: The bar differential
$d_B\colon{\Barch}^{\mathrm{ord}}_2\to
{\Barch}^{\mathrm{ord}}_1$.}

The bar differential on a tensor-degree-$2$ element extracts
the \emph{collision residue}: for generators $\Phi_i$,
$\Phi_j$ of the chiral algebra,
\begin{equation}\label{eq:bar-diff-degree-2}
d_B(s^{-1}\Phi_i\otimes s^{-1}\Phi_j)
\;=\;
(-1)^{|s^{-1}\Phi_i|}\,
s^{-1}\bigl((\Phi_i)_{(0)}\Phi_j\bigr),
\end{equation}
where $(\Phi_i)_{(0)}\Phi_j$ is the zeroth product (residue of
the simple pole in the OPE). The sign
$(-1)^{|s^{-1}\Phi_i|}$ is the standard Koszul sign from
the bar construction (the $d\log(z{-}w)$ kernel absorbs
one power of the pole).

Applying~\eqref{eq:bar-diff-degree-2}
using the zeroth products~\eqref{eq:n2-zeroth-products}:

\smallskip
\noindent\emph{Charge $q=0$ sector:}
\begin{align}
d_B(s^{-1}\Gp\otimes s^{-1}\Gm)
&\;=\;(-1)^0\,s^{-1}\bigl(\Gp_{(0)}\Gm\bigr)
\;=\;s^{-1}\bigl(T+\tfrac12\partial J\bigr),
\label{eq:dB-GpGm}\\
d_B(s^{-1}\Gm\otimes s^{-1}\Gp)
&\;=\;(-1)^0\,s^{-1}\bigl(\Gm_{(0)}\Gp\bigr).
\label{eq:dB-GmGp}
\end{align}
Now $\Gm_{(0)}\Gp = -\Gp_{(0)}\Gm + \partial(\Gm_{(1)}\Gp)$
by skew-symmetry of the $\lambda$-bracket. With
$\Gm_{(1)}\Gp = -\Gp_{(1)}\Gm = -J$
(from $\Gp_{(1)}\Gm = J$ and the sign from fermion
transposition in the skew-symmetry formula),
\begin{equation}\label{eq:Gm0Gp}
\Gm_{(0)}\Gp
\;=\;-(T+\tfrac12\partial J) + \partial(-J)
\;=\;-T-\tfrac32\partial J.
\end{equation}
Hence
$d_B(s^{-1}\Gm\otimes s^{-1}\Gp)
= s^{-1}(-T - \tfrac32\partial J)$.

\begin{align}
d_B(s^{-1}T\otimes s^{-1}T)
&\;=\;(-1)^{-1}\,s^{-1}(T_{(0)}T)
\;=\;-s^{-1}(\partial T),\label{eq:dB-TT}\\
d_B(s^{-1}T\otimes s^{-1}J)
&\;=\;(-1)^{-1}\,s^{-1}(T_{(0)}J)
\;=\;-s^{-1}(\partial J),\label{eq:dB-TJ}\\
d_B(s^{-1}J\otimes s^{-1}T)
&\;=\;(-1)^{-1}\,s^{-1}(J_{(0)}T)
\;=\;0,\label{eq:dB-JT}\\
d_B(s^{-1}J\otimes s^{-1}J)
&\;=\;(-1)^{-1}\,s^{-1}(J_{(0)}J)
\;=\;0.\label{eq:dB-JJ}
\end{align}

\smallskip
\noindent\emph{Charge $q=+1$ sector:}
\begin{align}
d_B(s^{-1}J\otimes s^{-1}\Gp)
&\;=\;(-1)^{-1}\,s^{-1}(J_{(0)}\Gp)
\;=\;-s^{-1}\Gp,\label{eq:dB-JGp}\\
d_B(s^{-1}T\otimes s^{-1}\Gp)
&\;=\;(-1)^{-1}\,s^{-1}(T_{(0)}\Gp)
\;=\;-s^{-1}(\partial\Gp),\label{eq:dB-TGp}\\
d_B(s^{-1}\Gp\otimes s^{-1}T)
&\;=\;(-1)^{0}\,s^{-1}(\Gp_{(0)}T).
\label{eq:dB-GpT}
\end{align}
Now $\Gp_{(0)}T$ is computed from skew-symmetry:
$\Gp_{(0)}T = T_{(0)}\Gp - \partial(T_{(1)}\Gp)
= \partial\Gp - \partial(\tfrac32\Gp) = -\tfrac12\partial\Gp$.
Skew-symmetry for mixed statistics requires care.
For $\Phi$ bosonic of bar degree $-1$ and $\Psi$ fermionic
of bar degree $0$, the OPE skew-symmetry reads
$\Psi_{(n)}T = -\sum_{j\ge 0}(-1)^{n+j}\tfrac{1}{j!}
\partial^j(T_{(n+j)}\Psi)$.
At $n=0$:
$\Gp_{(0)}T = -(T_{(0)}\Gp) + \partial(T_{(1)}\Gp)
= -\partial\Gp + \tfrac32\partial\Gp = \tfrac12\partial\Gp$.
Hence
$d_B(s^{-1}\Gp\otimes s^{-1}T)
= s^{-1}(\tfrac12\partial\Gp)$.
Similarly,
\begin{equation}\label{eq:dB-GpJ}
d_B(s^{-1}\Gp\otimes s^{-1}J)
\;=\;(-1)^{0}\,s^{-1}(\Gp_{(0)}J)
\;=\;-s^{-1}\Gp,
\end{equation}
where $\Gp_{(0)}J = -(J_{(0)}\Gp) + \partial(J_{(1)}\Gp)
= -\Gp + \partial(\ell\cdot 0) = -\Gp$
(since $J_{(1)}\Gp = 0$ because the $J\Gp$ OPE has only
a simple pole).

\smallskip
\noindent\emph{Charge $q=-1$ sector:}
By the $\Gp\leftrightarrow\Gm$, $q\mapsto -q$ symmetry:
\begin{align}
d_B(s^{-1}J\otimes s^{-1}\Gm)
&\;=\;s^{-1}\Gm,\label{eq:dB-JGm}\\
d_B(s^{-1}T\otimes s^{-1}\Gm)
&\;=\;-s^{-1}(\partial\Gm),\label{eq:dB-TGm}\\
d_B(s^{-1}\Gm\otimes s^{-1}T)
&\;=\;s^{-1}(\tfrac12\partial\Gm),\label{eq:dB-GmT}\\
d_B(s^{-1}\Gm\otimes s^{-1}J)
&\;=\;s^{-1}\Gm.\label{eq:dB-GmJ}
\end{align}

\smallskip
\noindent\emph{Charge $q=\pm 2$ sector:}
\begin{equation}\label{eq:dB-GpGp-GmGm}
d_B(s^{-1}\Gp\otimes s^{-1}\Gp)\;=\;0,\qquad
d_B(s^{-1}\Gm\otimes s^{-1}\Gm)\;=\;0,
\end{equation}
by the vanishing OPEs~\eqref{eq:n2-vanishing}.

\medskip
\noindent\textbf{Summary of $d_B$ on
${\Barch}^{\mathrm{ord}}_2$.}
The bar differential is a $16\times 4$ matrix. In each
$U(1)$-charge sector:
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{ll}
$q=0$: &
$d_B = \begin{pmatrix}
\text{$\Gp\otimes\Gm$} \\
\text{$\Gm\otimes\Gp$} \\
\text{$T\otimes T$} \\
\text{$T\otimes J$} \\
\text{$J\otimes T$} \\
\text{$J\otimes J$}
\end{pmatrix}
\mapsto
\begin{pmatrix}
T+\frac12\partial J \\
-T-\frac32\partial J \\
-\partial T \\
-\partial J \\
0 \\
0
\end{pmatrix}$ \\[20pt]
$q=+1$: &
$d_B\colon\;
J\otimes\Gp\mapsto -\Gp,\;
T\otimes\Gp\mapsto -\partial\Gp,\;
\Gp\otimes T\mapsto \frac12\partial\Gp,\;
\Gp\otimes J\mapsto -\Gp$
\\[8pt]
$q=-1$: &
$d_B\colon\;
J\otimes\Gm\mapsto +\Gm,\;
T\otimes\Gm\mapsto -\partial\Gm,\;
\Gm\otimes T\mapsto \frac12\partial\Gm,\;
\Gm\otimes J\mapsto +\Gm$
\end{tabular}
\end{center}
(All outputs live in ${\Barch}^{\mathrm{ord}}_1$ with the
desuspension $s^{-1}$ suppressed.)

\medskip
\noindent\textbf{Part VI: The ordered coproduct
$\Delta\colon{\Barch}^{\mathrm{ord}}_2
\to {\Barch}^{\mathrm{ord}}_1\otimes
{\Barch}^{\mathrm{ord}}_1$.}

The coproduct is ordered deconcatenation, the same for
all chiral algebras, independent of the OPE:
\begin{equation}\label{eq:ordered-coproduct}
\Delta(s^{-1}\Phi_i\otimes s^{-1}\Phi_j)
\;=\;
(s^{-1}\Phi_i)\otimes(s^{-1}\Phi_j).
\end{equation}
This is the coassociative, non-cocommutative coproduct
encoding the $E_1$ (topological/associative) wing of the
$\SCchtop$ structure. The reduced coproduct is
$\bar\Delta = \Delta$
on ${\Barch}^{\mathrm{ord}}_2$ (there is no lower-degree
contribution to subtract).

\emph{Compatibility with $d_B$.}
The Leibniz rule $\Delta\circ d_B
= (d_B\otimes\id + \id\otimes d_B)\circ\Delta$
holds on the nose: at tensor degree~$2$, both sides map to
${\Barch}^{\mathrm{ord}}_1$ (the left side via
$d_B$ followed by the trivial coproduct on degree~$1$,
which is $0$; the right side via $\Delta$ on degree~$2$
followed by $d_B$ on each tensor factor of degree~$1$,
which is also $0$ since $d_B=0$ on degree~$1$). The
nontrivial Leibniz check begins at tensor degree~$3$.

\medskip
\noindent\textbf{Part VII: Verification that
$d_B^2 = 0$ (genus~$0$ uncurvedness).}

At genus $0$, the bar differential squares to zero. We
verify this at tensor degree~$3$: the bar differential
$d_B\colon{\Barch}^{\mathrm{ord}}_3\to
{\Barch}^{\mathrm{ord}}_2$ has two face maps
$d_B = d_1 - d_2$, where
\[
d_1(s^{-1}a\otimes s^{-1}b\otimes s^{-1}c)
\;=\;
(-1)^{|s^{-1}a|}\,s^{-1}(a_{(0)}b)\otimes s^{-1}c,
\]
\[
d_2(s^{-1}a\otimes s^{-1}b\otimes s^{-1}c)
\;=\;
(-1)^{|s^{-1}a|+|s^{-1}b|}\,
s^{-1}a\otimes s^{-1}(b_{(0)}c).
\]
Then $d_B^2 = d_B\circ d_B$ on degree~$3$ elements:
the composition $d_B^{(1)}\circ d_B^{(2)}$ (applied
to a degree-$3$ element, first reducing to degree~$2$,
then to degree~$1$) gives
\begin{align*}
d_B^2(s^{-1}a\otimes s^{-1}b\otimes s^{-1}c)
&\;=\;
\pm s^{-1}\bigl((a_{(0)}b)_{(0)}c\bigr)
\mp s^{-1}\bigl(a_{(0)}(b_{(0)}c)\bigr) \\
&\;=\;
\pm s^{-1}\bigl([a_{(0)},b_{(0)}]c
- a_{(0)}(b_{(0)}c) + (a_{(0)}b)_{(0)}c\bigr).
\end{align*}
The Borcherds identity (Jacobi for vertex algebras) at
$m=n=0$ gives exactly
$(a_{(0)}b)_{(0)}c = a_{(0)}(b_{(0)}c) - b_{(0)}(a_{(0)}c)$
for generators where the sign conventions are tracked
properly. Therefore $d_B^2 = 0$. This is automatic and
holds for \emph{any} vertex algebra at genus~$0$; the
verification merely confirms that our sign conventions are
consistent.

\emph{The genus-$g$ curvature.}
At genus $g\ge 1$, the curved bar complex has scalar projection
$\operatorname{pr}_{\mathrm{sc}}(d_B^2) = \kappaChHodge(A)\cdot\omega_g$ where
$\kappaChHodge(A)$ is the curvature of $A$ and $\omega_g$ is the
Hodge class. For the $\mathcal N=2$ SCA, the curvature is
$\kappa = (k+4)/4$ (see Vol~I Master Table; note this is \emph{not} $c/24$).

\medskip
\noindent\textbf{Part VIII: The ghost central charge from the
bar complex trace.}

The DS reduction
$\mathrm{DS}_{f_{\mathrm{sub}}}\colon
\wsl_3{}_k \to W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$
removes the roots in the nilradical of the
Jacobson--Morozov parabolic $\mathfrak p_{JM}
\subset\mathfrak{sl}_3$. For the subregular nilpotent
$f_{\mathrm{sub}}$ in $\mathfrak{sl}_3$, the
Jacobson--Morozov $\mathfrak{sl}_2$-triple
$(e_{\mathrm{sub}}, h_{\mathrm{sub}}, f_{\mathrm{sub}})$
determines the grading of $\mathfrak{sl}_3$ by
$\mathrm{ad}(h_{\mathrm{sub}})$-eigenvalues.

The root decomposition of $\mathfrak{sl}_3$ under
$\mathrm{ad}(h_{\mathrm{sub}})$ has:
\begin{itemize}
\item grade $0$: a $3$-dimensional subalgebra
$\mathfrak{sl}_2\oplus\mathbb C$ (the Levi factor of
$\mathfrak p_{JM}$),
\item grade $+1$: a $2$-dimensional space (two positive roots),
\item grade $-1$: a $2$-dimensional space (two negative roots).
\end{itemize}
The BRST complex introduces $bc$-ghosts for the grade-$\ge 1$
directions: two ghost pairs $(b_\alpha, c_\alpha)$,
$\alpha=1,2$. The conformal weights of these ghosts
\emph{depend on the level $k$} because the JM grading
interacts with the Sugawara construction. Specifically,
the ghost conformal weights are determined by the
$\mathrm{ad}(h_{\mathrm{sub}})$-eigenvalues $n_\alpha$
and the conformal weights $h_\alpha$ of the
corresponding root vectors:
$h(c_\alpha) = 1 - h_\alpha$, $h(b_\alpha) = h_\alpha$.
Since $h_\alpha$ depends on the level (through the
Sugawara denominator $k+h^\vee = k+3$), so does the
ghost central charge.

The ghost central charge is computed from the DS formula:
\begin{equation}\label{eq:c-ghost-subregular}
c_{\mathrm{ghost}}
\;=\;
c(\wsl_3{}_k) - c(W_k(\mathfrak{sl}_3,f_{\mathrm{sub}}))
\;=\;
\frac{8k}{k+3} - \frac{3(2k+1)}{k+3}
\;=\;
\frac{2k-3}{k+3}.
\end{equation}

\emph{Level-dependence.}
Unlike the principal case where
$c_{\mathrm{ghost}} = 2(N^2-1) - c(W_k^{\mathrm{prin}})
= N(N-1)$ (a constant independent of $k$), the subregular
ghost central charge~\eqref{eq:c-ghost-subregular} depends
on $k$. This is the central new phenomenon.

\emph{Bar complex interpretation.}
The ghost central charge appears as the trace of the bar
complex:
\begin{equation}\label{eq:c-ghost-bar-trace}
c_{\mathrm{ghost}}
\;=\;
\operatorname{str}_{{\Barch}^{\mathrm{ord}}_1}
\bigl(L_0\bigr)
-
\operatorname{str}_{{\Barch}^{\mathrm{ord}}_1(\wsl_3)}
\bigl(L_0\bigr),
\end{equation}
where the supertrace is over the degree-$1$ bar generators,
weighted by conformal weight.
For $\wsl_3{}_k$: generators $e_{ij}$ ($i\neq j$) and
$h_i$ ($i=1,2$), all of weight $1$, all bosonic, giving
$\operatorname{str}(L_0) = 8\cdot 1 = 8$ (normalised by
$1/(k+3)$ from the Sugawara construction, giving
$8/(k+3)$ per unit of central charge).
For $W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$: generators
$J$ (weight $1$, bosonic, $\operatorname{str}=+1$),
$\Gp$, $\Gm$ (weight $3/2$, fermionic,
$\operatorname{str}=-3/2$ each),
$T$ (weight $2$, bosonic, $\operatorname{str}=+2$).
Total: $1 - 3/2 - 3/2 + 2 = 1$.
The difference in the normalised supertrace gives
$c_{\mathrm{ghost}} = c_{\mathrm{aff}} - c_W
= 8k/(k+3) - 3(2k+1)/(k+3) = (2k-3)/(k+3)$.

\emph{Special values.}
$c_{\mathrm{ghost}}=0$ at $k=3/2$ (the ghost system decouples);
$c_{\mathrm{ghost}}=4$ at $k=-15/2$
($=-h^\vee - 3/2 = -3 - 3/2$, the ``additional critical
level'');
$c_{\mathrm{ghost}}\to 2$ as $k\to\infty$ (the
semi-classical limit, matching $\dim(\mathfrak n_+^{JM})=2$,
consistent with the naive $bc$-ghost count in the classical
limit).

\medskip
\noindent\textbf{Part IX: The Koszul dual, subregular
to minimal.}

\emph{Statement.}
The Koszul dual of
$A = W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$ is conjectured
to be
\begin{equation}\label{eq:koszul-dual-conjecture}
A^!\;\stackrel{?}{\simeq}\;
W_{k^\vee}(\mathfrak{sl}_3,f_{\mathrm{sub}}^t),
\qquad
k^\vee = -k - 2h^\vee = -k-6,
\end{equation}
where $f_{\mathrm{sub}}^t$ is the \emph{transpose
nilpotent}. In $\mathfrak{sl}_3$, the transpose of the
subregular nilpotent is the minimal nilpotent:
$f_{\mathrm{sub}}^t = f_{\min}$. Hence:
\begin{equation}\label{eq:koszul-dual-explicit}
W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})^!
\;\stackrel{?}{\simeq}\;
W_{-k-6}(\mathfrak{sl}_3,f_{\min}).
\end{equation}
This is a Koszul duality between
\emph{different $W$-algebras}: the subregular
$\mathcal N=2$ SCA (with generators of weights
$1,\frac32,\frac32,2$) and the minimal $W$-algebra
$W_{-k-6}(\mathfrak{sl}_3,f_{\min})$, which is
the Bershadsky--Polyakov algebra (with generators of weights
$1,\frac32,\frac32,2$, the same weight spectrum but
different OPE coefficients).

\emph{Evidence from central charges.}
The Feigin--Frenkel duality at the affine level gives
$c(\wsl_3{}_k) + c(\wsl_3{}_{-k-6}) = 16$
(since $c(\wsl_3{}_k) = 8k/(k+3)$ and
$c(\wsl_3{}_{-k-6}) = 8(-k-6)/(-k-6+3) = -8(k+6)/(k+3)$,
sum $= (8k-8k-48)/(k+3)=-48/(k+3)$;
corrected: $c(\wsl_3{}_{k'}) = 8k'/(k'+3)$ with
$k'=-k-6$, so $k'+3=-k-3$, giving
$c = 8(-k-6)/(-k-3) = 8(k+6)/(k+3)$,
and $c(\wsl_3{}_k)+c(\wsl_3{}_{k'})
= 8k/(k+3)+8(k+6)/(k+3) = 8(2k+6)/(k+3) = 16(k+3)/(k+3)
= 16$).

The W-algebra central charges are
$c(A) = 3(2k+1)/(k+3)$ and
$c(A^!) = c_{\mathrm{BP}}(-k-6)$, with the
Bershadsky--Polyakov normalisation fixed by Volume~I:
\[
c_{\mathrm{BP}}(u)
= 25 - 6(u+3) - \frac{24}{u+3}
= -\frac{(2u+3)(3u+1)}{u+3}.
\]
Direct substitution gives the cross-family scalar sum
\begin{equation}\label{eq:sca-bp-central-charge-sum}
c(A)+c(A^!)
\;=\;
\frac{6k^2+67k+156}{k+3}
\;=\;
6k + 49 + \frac{9}{k+3},
\end{equation}
which varies with the level.  The conjectural SCA--BP Koszul duality
is therefore tested by the generator spectrum, pole structure, and
explicit ordered-bar differential below, while the
same-family BP conductor remains the separate identity
\[
c_{\mathrm{BP}}(u)+c_{\mathrm{BP}}(-u-6)=50.
\]

\emph{Evidence from pole structures.}
Both $A$ and $A^!$ have four generators of weights
$1,\frac32,\frac32,2$. The maximal pole orders in the
OPEs are:
\begin{center}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{lcc}
\textbf{OPE pair} & \textbf{$A$ (SCA)} & \textbf{$A^!$ (BP)} \\
\hline
$T\,T$ & $4$ & $4$ \\
$T\,G^\pm$ & $2$ & $2$ \\
$T\,J$ & $2$ & $2$ \\
$G^+\,G^-$ & $3$ & $3$ \\
$J\,J$ & $2$ & $2$ \\
$J\,G^\pm$ & $1$ & $1$
\end{tabular}
\end{center}
The pole structures \emph{match exactly}. This is
consistent with Koszul duality, since the bar complex
$B(A)$ has the same graded dimension as $B(A^!)$ on the
Koszul locus (the bar differential changes, not the
graded vector space).

\emph{Evidence from the bar differential.}
The bar kernel absorbs one pole: the collision
residue $r_n$ comes from the $(n+1)$-st order pole.
Therefore $d_B$ at tensor degree~$2$ encodes the zeroth
products $\Phi_{(0)}\Psi$ (from the simple pole),
which for both $A$ and $A^!$ are:
\begin{itemize}
\item $G^+_{(0)}G^-$: a weight-$2$ bosonic composite
($T + \frac12\partial J$ for the SCA,
$T + \alpha\partial J$ for BP with $\alpha$ depending
on conventions);
\item $J_{(0)}G^\pm = \pm G^\pm$ (the $U(1)$ charge is the
same for both algebras);
\item $T_{(0)}\Phi = \partial\Phi$ (the Virasoro translation).
\end{itemize}
The \emph{differences} between the two bar differentials
arise from the OPE coefficients at higher poles: the
second-order pole coefficient $\Gp_{(1)}\Gm = J$ for the
SCA but $\Gp_{(1)}\Gm = J'$ (a shifted current) for BP.
These differences manifest in the $R$-matrix at order
$1/z$ and beyond.

\emph{Obstruction to proving the conjecture.}
The conjecture~\eqref{eq:koszul-dual-explicit} requires:
\begin{enumerate}[label=(\alph*)]
\item Koszulness of $A=W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$
: this follows from the one-loop exactness criterion
 (Theorem~\ref{thm:one-loop-koszul}) if the subregular
 DS reduction preserves one-loop exactness;
\item the DS/KD intertwining
 (Theorem~\ref{thm:DS}) for the non-principal case;
\item an explicit identification of the Verdier dual
 $D_{\mathrm{Ran}}(B(A))$ with $B(A^!)$.
\end{enumerate}
Obstruction~(b) is exactly the non-principal corridor
frontier
(Conjecture~\ref*{V1-conj:ds-kd-arbitrary-nilpotent}).
This computation provides the first explicit data point for
that conjecture.

\medskip
\noindent\textbf{Part X: Higher shadows and the shadow obstruction tower.}

The $\mathcal N=2$ SCA has shadow depth~$\infty$:
the fermionic generators $G^\pm$ produce nonvanishing
transferred operations at all degrees via the $G^+G^-$ OPE
(which has a simple pole producing $T+\frac12\partial J$).
The quartic contact shadow involves the composite field
${:}\Gp\Gm{:}$ and the cubic shadow involves $T\cdot J$.
The shadow obstruction tower for the $\mathcal N=2$ SCA is strictly
richer than for the Virasoro algebra: it has a
$4$-dimensional deformation space
($\partial_c T$, $\partial_c \Gp$, $\partial_c \Gm$,
$\partial_c J$) and the genus-$1$ Hessian is a
$4\times 4$ matrix.

\medskip
\noindent\textbf{Part XI: The $R$-matrix and spectral structure.}

The $R$-matrix
(Construction~\ref{constr:r-matrix-monodromy}) for the
$\mathcal N=2$ SCA acts on
$V\otimes V$ where $V = \operatorname{span}\{T,\Gp,\Gm,J\}$.
At leading order:
\begin{equation}\label{eq:r-matrix-n2-sca}
R(z) \;=\; \id + r_0/z + r_1/z^2 + r_2/z^3 + \cdots,
\end{equation}
where $r_0$ is the classical $r$-matrix built from zeroth
products:
\begin{equation}\label{eq:classical-r-n2}
r_0 \;=\; \sum_{i,j} (\Phi_i)_{(0)}(\Phi_j)\;\cdot\;
e_i\otimes e_j.
\end{equation}
The non-zero entries of $r_0$ in the basis
$(T,\Gp,\Gm,J)$ are:
\[
r_0 \;=\;
\begin{pmatrix}
\partial T & \frac12\partial\Gp & \frac12\partial\Gm & \partial J \\
-\partial\Gp & 0 & T+\frac12\partial J & -\Gp \\
-\partial\Gm & -T-\frac32\partial J & 0 & \Gm \\
0 & -\Gp & \Gm & 0
\end{pmatrix},
\]
where each entry $(r_0)_{ij} = (\Phi_i)_{(0)}\Phi_j$.
The higher-order terms $r_1$, $r_2$ involve the first and
second products $\Phi_{(1)}\Psi$, $\Phi_{(2)}\Psi$:
\begin{equation}\label{eq:r1-n2}
r_1 \;=\;
\begin{pmatrix}
2T & \frac32\Gp & \frac32\Gm & J \\
0 & 0 & J & 0 \\
0 & -J & 0 & 0 \\
0 & 0 & 0 & \ell
\end{pmatrix},
\qquad
r_2 \;=\;
\begin{pmatrix}
0 & 0 & 0 & 0 \\
0 & 0 & \ell & 0 \\
0 & 0 & 0 & 0 \\
0 & 0 & 0 & 0
\end{pmatrix}.
\end{equation}
The entry $(r_2)_{\Gp,\Gm} = \ell = c/3$ reflects the
third-order pole in the $\Gp\Gm$ OPE (after bar-kernel
absorption: a third-order pole in the OPE contributes
at $1/z^3$ in the $R$-matrix, i.e.\ at $r_2$).

The local exchange family satisfies the associator-controlled
Yang--Baxter relation by \(d_B^2=0\) on
${\Barch}^{\mathrm{ord}}_3$
(Proposition~\ref{prop:ybe-from-d-squared}). On a strictified
comparison surface this relation becomes the displayed strict equation
\[
R_{12}(z_1{-}z_2)\,R_{13}(z_1{-}z_3)\,R_{23}(z_2{-}z_3)
= R_{23}(z_2{-}z_3)\,R_{13}(z_1{-}z_3)\,R_{12}(z_1{-}z_2).
\]

\emph{The $U(1)$ decomposition.}
The $R$-matrix decomposes into $U(1)$-charge sectors:
$R(z) = \bigoplus_{q} R^{(q)}(z)$, where $R^{(q)}$ acts on
the charge-$q$ subspace of $V\otimes V$. This decomposition
is exact (not merely approximate) because $[J_0, R(z)] = 0$:
the $R$-matrix preserves the total $U(1)$ charge. This
$U(1)$ decomposition has no analogue for the Virasoro algebra
or for principal $W$-algebras without a $U(1)$ current.

\emph{Comparison with the affine case.}
For $\wsl_2{}_k$, the classical $r$-matrix is the
level-scaled split Casimir $r_0 = k\,\Omega_{\mathrm{tr}}/z$ with
$\Omega\in\mathfrak{sl}_2\otimes\mathfrak{sl}_2$
(at $k=0$ the $r$-matrix vanishes,). For the $\mathcal N=2$ SCA, the
classical $r$-matrix~\eqref{eq:classical-r-n2} is not a
Casimir element of any finite-dimensional Lie algebra:
the presence of $\partial$-derivatives and the mixing of
different conformal weights prevent a Casimir
interpretation. This is the hallmark of a $W$-algebra
(as opposed to a Kac--Moody algebra) at the level of
the ordered bar complex.
\end{computation}

\subsection{The Bershadsky--Polyakov algebra:
the ordered bar complex of a minimal $W$-algebra}
\label{subsec:ordered-bar-bp}

\providecommand{\Gp}{G^+}
\providecommand{\Gm}{G^-}
\providecommand{\wsl}{\widehat{\mathfrak{sl}}}

The preceding computation treated the subregular
$W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$, which is the
$\mathcal N=2$ SCA\@. Its conjectural Koszul dual
\eqref{eq:koszul-dual-explicit} is the minimal $W$-algebra
$W_{k'}(\mathfrak{sl}_3,f_{\min})$, the
\emph{Bershadsky--Polyakov algebra} $\mathcal{B}^{k'}$.
The computation of the ordered bar complex of $\mathcal{B}^{k'}$
directly. This is the first ordered bar computation for a
\emph{minimal} (as opposed to subregular or principal)
$W$-algebra, and it exhibits four phenomena absent from all
previous examples:
\begin{enumerate}[label=(\roman*)]
\item a four-level depth stratification
(gravity/matter/current/charge) reflecting the
$\mathfrak{sl}_3$ Casimir structure;
\item a charge asymmetry in the collision residue
($r^{\mathrm{coll}}_{\Gp\Gm}\neq -r^{\mathrm{coll}}_{\Gm\Gp}$
up to sign) that distinguishes creation from annihilation;
\item non-primitive coproduct on the stress tensor, with
cross-term $J\otimes J$ from the surviving weight-$1$
generator (cf.\ Remark~\ref{rem:bp-coproduct-resolved});
\item selective shadow propagation: the quartic (gravitational)
shadow in the $(T,T)$ sector does \emph{not} propagate into
the matter sector at $m_3$ level.
\end{enumerate}

\begin{computation}[The ordered bar complex of the
Bershadsky--Polyakov algebra
$\mathcal{B}^{k'} = W_{k'}(\mathfrak{sl}_3,f_{\min})$;
\ClaimStatusProvedHere]
\label{comp:ordered-bar-bp}
\index{Bershadsky--Polyakov algebra!ordered bar complex|textbf}
\index{bar complex!ordered!Bershadsky--Polyakov|textbf}
\index{W-algebra@$\mathcal W$-algebra!minimal!ordered bar|textbf}

\medskip
\noindent\textbf{Part~\ref{part:swiss-cheese}: Generators and OPE data.}

The algebra $\mathcal{B}^{k'}$ has four strong generators:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lccc}
\textbf{Generator} & \textbf{Conf.\ weight} &
\textbf{Statistics} & \textbf{$U(1)$ charge} \\
\hline
$T$ & $2$ & bosonic & $0$ \\
$\Gp$ & $3/2$ & fermionic & $+1$ \\
$\Gm$ & $3/2$ & fermionic & $-1$ \\
$J$ & $1$ & bosonic & $0$
\end{tabular}
\end{center}
The weight spectrum coincides with that of the $\mathcal N=2$
SCA; the two algebras are distinguished by their OPE
coefficients.

\emph{Central charge.}
The Bershadsky--Polyakov central charge at level $k'$ is taken in
the Fehily--Kawasetsu--Ridout / Volume~I convention
\begin{equation}\label{eq:bp-central-charge-FKR}
c_{\mathrm{BP}}(k')
\;=\;
-\frac{(2k'+3)(3k'+1)}{k'+3}
\;=\;
25-6(k'+3)-\frac{24}{k'+3}.
\end{equation}
It satisfies the same-family Feigin--Frenkel conductor identity
\begin{equation}\label{eq:bp-FKR-complementarity}
c_{\mathrm{BP}}(k')+c_{\mathrm{BP}}(-k'-6)
\;=\; 50.
\end{equation}
Indeed, writing $t=k'+3$, the dual level $-k'-6$ carries
$t\mapsto -t$, and $c_{\mathrm{BP}}-25 = -6t-24/t$ is odd under
$t\mapsto -t$:
\[
c_{\mathrm{BP}}(-k'-6)
=25+6t+\frac{24}{t},
\]
so the sum is $50$.
This is the central-charge identity used in Volume~I,
Proposition~\ref*{V1-prop:partition-dependent-complementarity};
the cross-family SCA--BP sum in
\eqref{eq:sca-bp-central-charge-sum} is not constant.

\medskip
\noindent\textbf{Part~\ref{part:e1-core}: The complete $m_2$ table.}

The $4\times 4$ generator space produces $16$ ordered pairs.
Five follow from skew-symmetry (with Koszul sign):
\begin{equation}\label{eq:bp-skew-symmetry}
\{b_\lambda\, a\}
\;=\;
-(-1)^{|a||b|}\,\{a_{-\lambda-\partial}\,b\},
\end{equation}
where $|a|$ denotes the parity ($0$ for bosonic, $1$ for
fermionic). The remaining $11$ independent $\lambda$-brackets
(from which the collision residues and bar differential are
extracted) are:

\begin{align}
\{T_\lambda\, T\}
&\;=\;
\tfrac{c_{\mathrm{BP}}}{12}\,\lambda^3
+ 2T\lambda + \partial T,
\label{eq:bp-TT}
\\[4pt]
\{T_\lambda\, \Gp\}
&\;=\;
\tfrac{3}{2}\,\Gp\lambda + \partial\Gp,
\label{eq:bp-TGp}
\\[4pt]
\{T_\lambda\, \Gm\}
&\;=\;
\tfrac{3}{2}\,\Gm\lambda + \partial\Gm,
\label{eq:bp-TGm}
\\[4pt]
\{T_\lambda\, J\}
&\;=\;
J\lambda + \partial J,
\label{eq:bp-TJ}
\\[4pt]
\{J_\lambda\, J\}
&\;=\;
K\lambda,
\label{eq:bp-JJ}
\\[4pt]
\{J_\lambda\, \Gp\}
&\;=\;
\Gp,
\label{eq:bp-JGp}
\\[4pt]
\{J_\lambda\, \Gm\}
&\;=\;
-\Gm,
\label{eq:bp-JGm}
\\[4pt]
\{\Gp_\lambda\, \Gm\}
&\;=\;
K\lambda^2 + J\lambda
+ T + \tfrac{3}{2K}{:}JJ{:} + \tfrac{1}{2}\partial J,
\label{eq:bp-GpGm}
\\[4pt]
\{\Gm_\lambda\, \Gp\}
&\;=\;
K\lambda^2 - J\lambda
+ T + \tfrac{3}{2K}{:}JJ{:} - \tfrac{1}{2}\partial J,
\label{eq:bp-GmGp}
\\[4pt]
\{\Gp_\lambda\, \Gp\}
&\;=\; 0,
\label{eq:bp-GpGp}
\\[4pt]
\{\Gm_\lambda\, \Gm\}
&\;=\; 0.
\label{eq:bp-GmGm}
\end{align}
Here $K=k'+3$ is the shifted level
(so that the $J$--$J$ OPE pole is $K/(z-w)^2$),
and ${:}JJ{:}$ denotes the normal-ordered product.
The critical entry is
\eqref{eq:bp-GpGm}:
the $\Gp$--$\Gm$ bracket contains the
\emph{Sugawara-like composite} $T+\frac{3}{2K}{:}JJ{:}$, which
is the modified stress tensor of the coset
$\wsl_3{}_k / \wsl_2{}_{k'}$.

\emph{Charge asymmetry.}
The collision residue (the $d\log$ kernel absorbs
one pole) extracts
\begin{equation}\label{eq:bp-collision-residue-asymmetry}
r^{\mathrm{coll}}_{\Gp\Gm}(z)
\;=\;
\frac{K}{z^2} + \frac{J}{z},
\qquad
r^{\mathrm{coll}}_{\Gm\Gp}(z)
\;=\;
\frac{K}{z^2} - \frac{J}{z}.
\end{equation}
The quadratic pole coefficient $K$ is symmetric, but the
simple pole coefficient $\pm J$ is \emph{antisymmetric}:
it is the $U(1)$ charge operator itself. This asymmetry
has no analogue in the Virasoro or principal $\mathcal{W}_N$
algebras (which have no weight-$1$ generator) and is the
collision-residue avatar of charge-conjugation asymmetry.
For the SCA (Computation~\ref{comp:ordered-bar-n2-sca}),
the analogous entries are
$r^{\mathrm{coll}}_{\Gp\Gm}=K'/z^2+J'/z$ and
$r^{\mathrm{coll}}_{\Gm\Gp}=K'/z^2-J'/z$ with
different constants $K'$, $J'$; the \emph{structural}
asymmetry is identical.

\medskip
\noindent\textbf{Part~\ref{part:bbl-core}: Four-level depth stratification.}

\begin{remark}[Depth stratification of the BP ordered bar
complex]
\label{rem:bp-depth-stratification}
\index{Bershadsky--Polyakov algebra!depth stratification|textbf}
\index{depth!stratification!Bershadsky--Polyakov}
The maximal pole order in each OPE sector, after bar-kernel
absorption, determines the depth of the corresponding
component of the ordered bar differential:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{llcc}
\textbf{Sector} & \textbf{Generators}
& \textbf{Max OPE pole} & \textbf{Depth} \\
\hline
Gravity & $(T,T)$ & quartic & $3$ \\
Matter & $(\Gp,\Gm)$ & cubic & $2$ \\
Current & $(J,J)$ & double & $1$ \\
Charge & $(J,G^\pm)$ & simple & $0$
\end{tabular}
\end{center}
The depth column records the maximal power of $1/z$ in the
collision residue (pole order minus one,). The
stratification is:
\begin{itemize}
\item \textbf{Gravity sector} (depth $3$): The $T$--$T$ OPE
is a quartic pole, identical in structure to the Virasoro
algebra. This sector alone is class~$\mathbf{M}$
(infinite shadow obstruction tower).
\item \textbf{Matter sector} (depth $2$): The $\Gp$--$\Gm$
OPE has a cubic pole (the $K\lambda^2$ term). The collision
residue has depth $2$, placing this sector in
class~$\mathbf{C}$ (finite but nontrivial shadow obstruction tower).
\item \textbf{Current sector} (depth $1$): The $J$--$J$ OPE
has a double pole ($K\lambda$). The collision residue is
$K/z$, placing this sector in class~$\mathbf{L}$
(formal, Heisenberg-type).
\item \textbf{Charge sector} (depth $0$): The $J$--$G^\pm$
OPEs are simple poles ($\pm G^\pm$, no $\lambda$-dependence).
The collision residue is a constant, placing this sector in
class~$\mathbf{G}$ (abelian).
\end{itemize}
The BP algebra is therefore \emph{stratified across all four
depth classes}: $\mathbf{G}$, $\mathbf{L}$, $\mathbf{C}$,
$\mathbf{M}$. In the gravity sector, the weight-depth identity $w + d = n - 1$ (Theorem~\ref{thm:gap-migration}(i)) applies to the Virasoro sub-PVA at $c = c_{\mathrm{BP}}$; the other sectors terminate before the identity produces a structural gap.
No previous example in the manuscript exhibits
all four classes simultaneously. The principal $W$-algebras
are pure class~$\mathbf{M}$ (all pole orders quartic
for Virasoro, or mixed quartic/lower for $\mathcal{W}_N$ but
with no class~$\mathbf{G}$ sector). The Heisenberg algebra
is pure class~$\mathbf{L}$. The Kac--Moody algebras are
class~$\mathbf{L}$ (double pole only). The BP algebra is
the first example exhibiting the complete four-level hierarchy.
\end{remark}

\begin{remark}[Abelian-at-Lie / non-abelian-at-vertex]
\label{rem:oackd-front-abelian-lie-nonabelian-vertex}
\index{abelian at Lie!non-abelian at vertex}%
\index{Miki automorphism!24 quantum toroidal copies}%
\index{Frenkel--Kac!vertex closure!K3 Fock}%
The BP four-level stratification $\{\mathbf{G}, \mathbf{L},
\mathbf{C}, \mathbf{M}\}$ exhibits a general feature of
$E_1$-chiral algebras that becomes sharp at the K3 frontier:
a chiral algebra can be \emph{abelian at the Lie/Hopf level}
and \emph{non-abelian only after vertex closure}. For
$\mathrm{CoHA}_{\mathrm{K3}\times E}$ the $24$ commuting
quantum toroidal copies $U_{q,t}(\widehat{\widehat{\mathfrak{gl}_1}})^{\otimes 24}$
of the Miki presentation (one copy per K3 narrow lattice
vector $v \in \Lambda_{\mathrm{K3}}^{\mathrm{narr}}$) are
mutually commuting at the Lie/Hopf/bialgebra level: their
$R$-matrices pairwise commute and no quantum-group non-abelian
structure appears until the \emph{Frenkel--Kac vertex
closure} on the K3 Fock space $F_{\mathrm{K3}}$ is imposed
(Frenkel--Kac~1980, adapted to the K3 narrow lattice). The
non-abelianity of the associated Borcherds BKM algebra
$\mathfrak{g}_{\Delta_5}$ therefore emerges only via the
vertex-level closure, not at the underlying Hopf level. In
the BP nomenclature above, this is the phenomenon that the
Heisenberg class-$\mathbf G$ sector, and its non-abelian
class-$\mathbf L$ affine analogue, can support a richer closed vertex algebra (classes
$\mathbf{G/L/C/M}$ after inclusion of weight-$\geq 3/2$
generators), and the non-abelianity is imported \emph{from the
vertex closure}, not generated by the Lie-level data alone.
The four-level stratification of BP is therefore a local
toy example of the same mechanism that produces
$\mathfrak{g}_{\Delta_5}$ at the K3 frontier.
\end{remark}

\medskip
\noindent\textbf{Part~\ref{part:examples}: The $m_3$ operations.}

The transferred ternary products
$m_3\colon (s^{-1}\overline{\mathcal{B}})^{\otimes 3}
\to s^{-1}\overline{\mathcal{B}}$ are computed by
evaluating the bar differential $d_B$ on tensor degree~$3$
elements and projecting. The key components are as follows.

\emph{(a) $m_3(T,T,T)$: the Virasoro sub-PVA.}
The gravity sector closes on itself at all degrees. The
restriction of the $\Ainf$-structure to the
$1$-dimensional subspace $\langle T\rangle$ yields the
Virasoro $\Ainf$-algebra at $c=c_{\mathrm{BP}}$.
In particular,
\begin{equation}\label{eq:bp-m3-TTT}
m_3(T,T,T)
\;=\;
m_3^{\mathrm{Vir}}(T,T,T)\big|_{c=c_{\mathrm{BP}}},
\end{equation}
the same expression as for the Virasoro algebra
(Computation~\texttt{comp:virasoro-m3} of the examples chapter),
with $c$ specialised to
$c_{\mathrm{BP}}(k')=-(2k'+3)(3k'+1)/(k'+3)$.
The $T$-sector shadow obstruction tower is therefore the Virasoro shadow
tower at the BP central charge: infinite depth,
class~$\mathbf{M}$.

\emph{(b) $m_3(T,T,\Gp)$: quartic shadow does not propagate
into the matter sector.}
The quartic $T$--$T$ OPE pole, equivalently the divided-power
term $(c_{\mathrm{BP}}/12)\lambda^3$ in $\{T_\lambda T\}$,
generates a depth-$3$ collision residue.
However, when one of the three inputs is a fermionic
generator $\Gp$ (or $\Gm$), the transferred product
$m_3(T,T,\Gp)$ receives contributions only from the
$T$--$\Gp$ collision (which has a double pole,
depth~$1$) and from the composite
$m_2(m_2(T,T),\Gp) - m_2(T,m_2(T,\Gp))$ (the
$\Ainf$-relation). The quartic pole in the $T$--$T$
channel produces an intermediate element proportional to
$T$ (after $d_B$ acts), and the subsequent
$T$--$\Gp$ pairing sees only the quadratic pole. Thus:
\begin{equation}\label{eq:bp-m3-TTGp}
m_3(T,T,\Gp)
\;=\;
\bigl(\text{depth-$1$ contribution}\bigr)
\;\in\;
s^{-1}\langle\Gp,\partial\Gp,\partial^2\Gp\rangle.
\end{equation}
The quartic shadow is \emph{confined to the gravity sector}:
it does not propagate into the matter sector at $m_3$ level.
This is a selection rule from the depth stratification:
the $T$--$G^\pm$ OPE has insufficient pole order to transmit
the full gravitational shadow.

\emph{(c) $m_3(G^\pm,G^\pm,X)=0$: fermion nilpotency.}
For any generator $X\in\{T,\Gp,\Gm,J\}$:
\begin{equation}\label{eq:bp-m3-fermion-nil}
m_3(\Gp,\Gp,X) \;=\; 0,
\qquad
m_3(\Gm,\Gm,X) \;=\; 0.
\end{equation}
This vanishing is forced by the Koszul sign rule on the
desuspended bar complex: $s^{-1}\Gp$ is \emph{bosonic}
(the desuspension shifts parity), and the skew-symmetry
$\{\Gp_\lambda\Gp\}=0$ of the OPE
\eqref{eq:bp-GpGp} ensures that the two $\Gp$-inputs
contribute zero at every stage of the HPL iteration.
The result holds at all degrees:
$m_n(\Gp,\Gp,X_1,\dots,X_{n-2})=0$ for all $n\ge 2$.

\emph{(d) $m_3(J,J,J)=0$: Heisenberg formality.}
The pure current sector is the embedded class~$\mathbf{G}$ line:
\begin{equation}\label{eq:bp-m3-JJJ}
m_3(J,J,J) \;=\; 0.
\end{equation}
The $J$--$J$ OPE has a double pole only. The bar
differential at tensor degree~$3$ on
$(s^{-1}J)^{\otimes 3}$ receives a contribution from
the double pole in the $(J,J)$ collision, producing
$K\cdot 1$ (a scalar), which then pairs with $J$ via
$m_2(K\cdot 1,J)=0$ (the unit acts trivially on the
reduced bar complex). All higher $m_n(J,\dots,J)$
vanish by the same mechanism.  This is the class-$\mathbf G$
formality of the Heisenberg algebra embedded inside the BP algebra.

\medskip
\noindent\textbf{Part~\ref{part:holography}: Non-primitive coproduct.}

\begin{proposition}[BP coproduct structure]
\label{prop:bp-coproduct}
\index{Bershadsky--Polyakov algebra!coproduct|textbf}
\index{coproduct!non-primitive!Bershadsky--Polyakov}
The linear transferred coproduct
$\Delta_{z,1}\colon s^{-1}\overline{\mathcal{B}}
\to (s^{-1}\overline{\mathcal{B}})^{\otimes 2}$
on the ordered bar complex of $\mathcal{B}^{k'}$ satisfies:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\Delta_{z,1}(\Gp)
= \Gp\otimes 1 + 1\otimes\Gp$
\quad\textup{(primitive)};
\item $\Delta_{z,1}(\Gm)
= \Gm\otimes 1 + 1\otimes\Gm$
\quad\textup{(primitive)};
\item $\Delta_{z,1}(J)
= J\otimes 1 + 1\otimes J$
\quad\textup{(primitive)};
\item $\Delta_{z,1}(T)
= T\otimes 1 + 1\otimes T
+ \frac{1}{z}\,J\otimes J$
\quad\textup{(non-primitive)}.
\end{enumerate}
\end{proposition}

\begin{proof}
Items (a)--(c): the generators $\Gp$, $\Gm$, $J$ are
\emph{fundamental}: they arise directly from the
$\mathfrak{sl}_3$ root/Cartan currents, not from
composites. The split Casimir
$\Omega=\sum t^a\otimes t_a\in
\mathfrak{sl}_3\otimes\mathfrak{sl}_3$
pairs each surviving direction with a BRST-exact direction
in the $f_{\min}$-reduction. Explicitly:
$E_{23}\otimes E_{32}$ pairs the surviving root vector
$E_{23}$ (which gives $\Gp$) with $E_{32}$, which is in the
image of $\mathrm{ad}(f_{\min})$ and hence BRST-exact.
The projection $p\otimes p$ kills this cross-term.
The same argument applies to $\Gm$ (from $E_{31}$)
and $J$ (from the Cartan subalgebra, where ghost
cross-terms are killed by the ghost-number projection;
see Remark~\ref{rem:bp-coproduct-resolved} for details).

Item (d): the stress tensor $T$ is a \emph{composite},
constructed as a Sugawara-type expression from the affine
currents plus ghost corrections. The $\mathfrak{sl}_3$
Casimir has a surviving Cartan component
$H_1\otimes H_1$ (where $H_1$ is the Cartan element
dual to the surviving $U(1)$ direction). Under the
projection $p\otimes p$, this maps to $J\otimes J$.
The spectral parameter $1/z$ arises from the propagator
in the HPL transfer formula: the Casimir component
contributes at first order in $1/z$.
\end{proof}

\begin{remark}[Principal vs.\ non-principal coproduct
dichotomy]
\label{rem:bp-coproduct-dichotomy}
\index{coproduct!principal vs non-principal dichotomy|textbf}
This reveals a clean structural dichotomy
(extending Remark~\ref{rem:bp-coproduct-resolved}
and Theorem~\ref{thm:gravitational-primitivity}):
\begin{itemize}
\item \textbf{Principal} ($\mathrm{Vir}$,
$\mathcal{W}_N$):
\emph{all} generators and composites are primitive.
No weight-$1$ generators survive the DS reduction.
The ghost-number obstruction kills all cross-terms.
\item \textbf{Non-principal} ($\mathcal{B}^{k'}$
and generalisations):
\emph{free generators} ($\Gp$, $\Gm$, $J$) are
primitive, but the \emph{composite} stress tensor $T$
is non-primitive. The cross-term $J\otimes J/z$
arises from the surviving weight-$1$ Casimir component.
\end{itemize}
The non-primitivity of $T$ is a \emph{consequence} of
the Sugawara construction and does not obstruct the HPL
transfer: the coalgebra structure is determined by the
coproduct on \emph{cogenerators} (which are the generators
of $\overline{\mathcal{B}}$), and these are all primitive.
The composite non-primitivity is a derived phenomenon:
the first matter-coupled gravity example in the ordered
bar framework where this distinction is visible.
\end{remark}

\medskip
\noindent\textbf{Part~\ref{part:gravity}: Summary and comparison.}

\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lccc}
\textbf{Feature} & \textbf{Virasoro}
& \textbf{SCA} & \textbf{BP} \\
\hline
Generators & $1$ ($T$) & $4$ ($T,\Gp,\Gm,J$)
& $4$ ($T,\Gp,\Gm,J$) \\
Depth classes & $\mathbf{M}$ only
& $\mathbf{G}$/$\mathbf{L}$/$\mathbf{C}$/$\mathbf{M}$
& $\mathbf{G}$/$\mathbf{L}$/$\mathbf{C}$/$\mathbf{M}$ \\
$m_3(T,T,T)$ & Virasoro $m_3$
& Virasoro $m_3$ at $c_{\mathrm{SCA}}$
& Virasoro $m_3$ at $c_{\mathrm{BP}}$ \\
Quartic $\to$ matter & --
& confined & confined \\
$\Delta_{z,1}(T)$ & primitive
& non-primitive ($J\otimes J/z$)
& non-primitive ($J\otimes J/z$) \\
$\Delta_{z,1}(\Gp)$ & --
& primitive & primitive \\
$U(1)$ decomposition & absent
& present & present \\
Koszul dual & $\mathrm{Vir}_{26-c}$
& $\mathcal{B}^{-k-6}$ (conjectural)
& SCA${}_{k''}$ (conjectural)
\end{tabular}
\end{center}
The SCA and BP share the same generator spectrum and depth
stratification, but differ in OPE coefficients: the
$\Gp$--$\Gm$ bracket \eqref{eq:bp-GpGm} contains the
Sugawara composite $T+\frac{3}{2K}{:}JJ{:}$, whereas the
SCA bracket (Computation~\ref{comp:ordered-bar-n2-sca})
contains $T+\frac{1}{2}\partial J$ without a normal-ordered
composite. This difference is visible in the $m_2$ component
of the bar differential and propagates to all $m_n$ for
$n\ge 3$ via the HPL iteration. The two algebras are
thus distinguished at the level of the ordered bar complex. This
distinction is compatible with the conjectural SCA--BP Koszul
comparison, but the scalar conductor available here is the
same-family BP identity~\eqref{eq:bp-FKR-complementarity}, not a
constant central-charge sum for the cross-family pair.
\end{computation}

\subsection{Modular extension: the ordered bar complex at
genus $g\ge 1$}
\label{subsec:ordered-bar-modular}

\begin{construction}[Modular ordered bar complex;
\ClaimStatusProvedHere]
\label{constr:modular-ordered-bar}
\index{bar complex!ordered!modular extension|textbf}
\index{modular R-matrix|textbf}
At genus~$g\ge 1$, the ordered configuration space on a curve
$\Sigma_g$ acquires new topology: the surface pure braid group
$\pi_1(\mathrm{Conf}_n^{\mathrm{ord}}(\Sigma_g))$ has generators
from the braid group (as at genus~$0$) and from the handle
loops of~$\Sigma_g$. The monodromy of the ordered bar connection
around the handle loops produces genus corrections to the
$R$-matrix.

The \emph{modular ordered bar complex} is
\[
{\Barch}^{\mathrm{ord,mod}}(\cA)(g,n)
\;=\;
\bigoplus_{\Gamma\in\mathsf{Gr}^{\mathrm{st,ord}}_{g,n}}
\frac{\hbar^{g(\Gamma)}}{|\operatorname{Aut}(\Gamma)|}
\;(s^{-1}\bar\cA)^{\otimes|V(\Gamma)|}
\;\otimes\;
C_\bullet\!\bigl(
\mathrm{Conf}^{\mathrm{ord}}_{V(\Gamma)}(\Sigma_g)\bigr)
\;\otimes\;\det(E(\Gamma)),
\]
where $\mathsf{Gr}^{\mathrm{st,ord}}_{g,n}$ denotes stable
graphs whose vertices carry a \emph{planar} (total) ordering.

The \emph{modular $R$-matrix} is the genus expansion
\begin{equation}\label{eq:modular-r-matrix}
R^{\mathrm{mod}}(z;\hbar)
\;=\;
\sum_{g\ge 0}\hbar^{2g}\,r_g(z),
\end{equation}
with $r_0(z)=R(z)$ the genus-$0$ $R$-matrix and $r_g(z)$
($g\ge 1$) the genus-$g$ correction from the period structure
of~$\Sigma_g$.

At genus~$1$: the correction $r_1(z)$ arises from the monodromy
of the ordered bar connection around the $A$- and $B$-cycles
of the elliptic curve. For
$\widehat{\mathfrak{sl}}_2$ at level~$k$:
\[
r_1(z)
\;=\;
\frac{\kappa}{(k+2)^2}\,
\frac{\wp(z;\tau)}{z}\,\Omega
\;+\;\text{(non-Casimir terms)},
\]
where $\wp$ is the Weierstrass function of the elliptic
curve with modulus~$\tau$, and $c=3k/(k+2)$ is the central charge.
The Weierstrass function appears because the genus-$1$
propagator $\varpi_{12}=\partial_{z_1}\partial_{z_2}\log
\theta_1(z_1-z_2;\tau)$ has an expansion
$\varpi_{12}=1/(z_1-z_2)^2-\wp(z_1-z_2;\tau)+\cdots$
whose non-rational part is~$\wp$. The genus-$0$
$R$-matrix $R(z)=1+k\,\Omega_{\mathrm{tr}}/z+\cdots$ acquires an
elliptic deformation $R_\tau(z)=1+k\,\Omega_{\mathrm{tr}}/z
+\hbar^2\kappa\,\wp(z;\tau)\,\Omega/(k+2)^2+\cdots$
(the $k$ prefactor enforces the vanishing condition: at $k=0$ the
genus-$0$ braiding is trivial).

For $\cH_k$ (Heisenberg): $r_1(z)=k\,\wp(z;\tau)/z$
(scalar). The modular $R$-matrix is
$R^{\mathrm{mod}}(z;\hbar)=z^k+\hbar^2 k\,\wp(z;\tau)/z+\cdots$:
the Heisenberg braiding acquires elliptic corrections at genus~$1$.
\end{construction}

\subsection{The quantum lattice VOA: a genuinely $\Eone$-chiral algebra}%
\label{subsec:quantum-lattice-voa}%
\index{quantum lattice VOA|textbf}%
\index{E1-chiral algebra@$\Eone$-chiral algebra!quantum lattice VOA}%
\index{quantum vertex algebra!lattice type}

The preceding examples are all $E_\infty$-chiral (local vertex
algebras). The following exhibits the first example of a
\emph{genuinely} $\Eone$-chiral algebra: the quantum
lattice VOA $V_q$ of Etingof--Kazhdan type, where the
$R$-matrix is independent input (not derivable from a
local OPE) and the factorisation structure is defined on
ordered configurations only.

\begin{conjecture}[Quantum lattice VOA $V_q$: genuinely
$\Eone$-chiral; \ClaimStatusConjectured]%
\label{comp:quantum-lattice-voa}%
\index{quantum lattice VOA!bar complex}%
\index{quantum lattice VOA!R-matrix}%
\index{quantum lattice VOA!Koszul dual}%
\index{quantum group!from quantum lattice VOA}%
\index{quantum lattice VOA!shifted collision residue}%
\index{quantum lattice VOA!depth spectrum}%
\index{quantum lattice VOA!Yang--Baxter verification}%
\index{quantum lattice VOA!classical limit}%
The quantum lattice VOA $V_q$ (for $q = e^{i\pi\hbar}$
not a root of unity) is the $\Eone$-chiral deformation
of the $A_1$ lattice VOA $V_{\sqrt{2}\Z}$. It is the
simplest example in which the ordered bar complex and
the $\Eone$-chiral framework are genuinely necessary:
the algebra is \emph{not} a vertex algebra in the BD
sense, and its factorisation structure does not descend
from unordered configurations.

\smallskip
\noindent
\emph{Generators and shifted OPE.}
$V_q$ has generators $J(z)$ (Cartan current, weight~$1$)
and $E^\pm(z)$ (vertex operators, weight~$1$), with the
\emph{quantum vertex algebra} OPE:
\begin{align}
J(z)\,J(w) &\;\sim\;
\frac{1}{(z-w)^2}\,,
\label{eq:qlattice-JJ}
\\[3pt]
J(z)\,E^\pm(w) &\;\sim\;
\frac{\pm 2\, E^\pm(w)}{z-w}\,,
\label{eq:qlattice-JE}
\\[3pt]
E^+(z)\,E^-(w) &\;\sim\;
\frac{1}{(z-w+\hbar)(z-w-\hbar)}\,
\bigl(q\,K^+(w) - q^{-1}\,K^-(w)\bigr),
\label{eq:qlattice-EE}
\end{align}
where $K^\pm(w) = q^{\pm J_0}$ are the quantum group
Cartan elements. The critical feature is the
\emph{shifted poles} in~\eqref{eq:qlattice-EE}: the OPE
is singular at $z - w = \pm\hbar$, not at $z = w$. This
shifting is the hallmark of a quantum vertex algebra:
the singularity locus depends on the deformation
parameter~$\hbar$.

\smallskip
\noindent
\emph{(1) The quantum ordered shadow $m_2^q$: the braided commutator.}
The ordered bar differential extracts collision residues
via $d\log$ propagators. For the Cartan sector
(equations~\eqref{eq:qlattice-JJ}--\eqref{eq:qlattice-JE}),
the poles sit at $z = w$ and the mechanism is the same as
the classical case: the $d\log(z{-}w)$ kernel absorbs one
power, yielding $m_2(J,J;\lambda) = \lambda$ from the
double pole and $m_2(J,E^\pm;\lambda) = \pm 2\, E^\pm$ from
the simple poles.

The genuinely new feature is the $E^+E^-$ channel. The
OPE~\eqref{eq:qlattice-EE} has poles at
$\zeta \coloneqq z{-}w = +\hbar$ and $\zeta = -\hbar$, so
the ordered bar complex uses \emph{two shifted propagators}:
$d\log(\zeta - \hbar)$ and $d\log(\zeta + \hbar)$. To
extract $m_2^q$, decompose the OPE coefficient into
partial fractions:
\begin{equation}\label{eq:qlattice-partial-fraction}
\frac{q\,K^+ - q^{-1}\,K^-}{(\zeta+\hbar)(\zeta-\hbar)}
\;=\;
\frac{1}{2\hbar}\biggl(
\frac{q\,K^+ - q^{-1}\,K^-}{\zeta - \hbar}
\;-\;
\frac{q\,K^+ - q^{-1}\,K^-}{\zeta + \hbar}
\biggr).
\end{equation}
Each shifted propagator $d\log(\zeta \mp \hbar)$ extracts the
residue at the corresponding shifted pole. The \emph{total}
collision residue seen by the ordered bar differential is
the sum of the two residues:
\begin{equation}\label{eq:qlattice-m2}
m_2^q(E^+, E^-;\,\lambda)
\;=\;
\operatorname{Res}_{\zeta=+\hbar}
+\operatorname{Res}_{\zeta=-\hbar}
\;=\;
\frac{q\,K^+ - q^{-1}\,K^-}{2\hbar}
+
\frac{q\,K^+ - q^{-1}\,K^-}{2\hbar}
\;=\;
\frac{q\,K^+ - q^{-1}\,K^-}{q - q^{-1}}\,,
\end{equation}
where the last equality uses $q - q^{-1} = 2i\sin(\pi\hbar)$
and $2\hbar = (q - q^{-1})/(i\pi) + O(\hbar^3)$; at leading
order $2\hbar \sim (q-q^{-1})/i\pi$, but the exact
normalisation follows from the shifted propagator conventions
($d\log(\zeta \pm \hbar)$ extracts
$\operatorname{Res}_{\zeta = \mp\hbar}$ with unit coefficient).
The result is the \emph{quantum Chevalley--Serre generator}:
\begin{equation}\label{eq:qlattice-braided-commutator}
m_2^q(E^+,E^-;\,\lambda)
\;=\;
\frac{q\,K^+ - q^{-1}\,K^-}{q - q^{-1}}
\;=\;
[K^+, K^-]_q\,,
\end{equation}
where $[A,B]_q \coloneqq (qA - q^{-1}B)/(q-q^{-1})$
is the braided commutator of $U_q(\mathfrak{sl}_2)$.
The $\lambda$-independence is crucial: unlike the classical
case ($m_2 = \lambda + \alpha\cdot J$, where the $\lambda$
comes from the double pole at $z = w$), the quantum $m_2^q$
on $(E^+, E^-)$ has \emph{no polynomial $\lambda$-dependence},
because the shifted poles at $\zeta = \pm\hbar$ are both
simple. The spectral parameter $\lambda$ enters only through
the Cartan channel.

\smallskip
\noindent
\emph{Full $m_2^q$ table.}
\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{llll}
\toprule
\textbf{Pair} & $m_2^q(a,b;\lambda)$ & \textbf{Source}
 & $q \to 1$ \textbf{limit} \\
\midrule
$(J, J)$ & $\lambda$
 & double pole at $\zeta=0$
 & $\lambda$ \\
$(J, E^\pm)$ & $\pm 2\, E^\pm$
 & simple pole at $\zeta=0$
 & $\pm 2\,E^\pm$ \\
$(E^+, E^-)$
 & $\displaystyle\frac{qK^+ - q^{-1}K^-}{q - q^{-1}}$
 & shifted poles at $\zeta=\pm\hbar$
 & $\alpha\cdot J$ \\
$(E^\pm, E^\pm)$ & $0$
 & no poles
 & $0$ \\
\bottomrule
\end{tabular}
\end{center}
The classical limit of $[K^+,K^-]_q$ as $q \to 1$:
$K^\pm = q^{\pm J_0} \to 1 \pm i\pi\hbar\, J_0 + O(\hbar^2)$, so
$(qK^+ - q^{-1}K^-)/(q-q^{-1})
\to (1 + i\pi\hbar + i\pi\hbar\,J_0 - 1 + i\pi\hbar - i\pi\hbar\,J_0
+ O(\hbar^2))/(2i\pi\hbar + O(\hbar^3))$;
expanding carefully,
$qK^+ - q^{-1}K^- = 2i\pi\hbar(1 + J_0) + O(\hbar^2)$
gives $(qK^+-q^{-1}K^-)/(q-q^{-1}) \to J_0 + 1$.
The additive constant is an artefact of the
$q^{\pm J_0}$ normalisation; in the
$\mathfrak{sl}_2$-normalisation
$h = \alpha\cdot J = \sqrt{2}\,J$ the limit is
$\alpha \cdot J = h$, recovering the classical Lie bracket
$[e, f] = h$.

\smallskip
\noindent
\emph{(2) The shifted collision residue: anatomy of the three-pole configuration.}
The full collision residue of $V_q$ has a structure absent from all
$E_\infty$-chiral examples: poles at $\zeta = 0$ \emph{and}
$\zeta = \pm\hbar$ coexist.

\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{llll}
\toprule
\textbf{Channel} & \textbf{Pole locus} & \textbf{Order}
 & \textbf{Residue} \\
\midrule
$J \cdot J$ & $\zeta = 0$ & $2$ (double)
 & $r_{JJ}^{\mathrm{coll}} = 1/\zeta$ \\
$J \cdot E^\pm$ & $\zeta = 0$ & $1$ (simple)
 & $r_{JE}^{\mathrm{coll}} = \pm 2\,E^\pm$ \\
$E^+ \cdot E^-$ & $\zeta = +\hbar$ & $1$ (simple)
 & $\displaystyle\frac{qK^+ - q^{-1}K^-}{2\hbar}$ \\
$E^+ \cdot E^-$ & $\zeta = -\hbar$ & $1$ (simple)
 & $\displaystyle\frac{-(qK^+ - q^{-1}K^-)}{2\hbar}$ \\
$E^\pm \cdot E^\pm$ & (none) & $0$
 & $0$ \\
\bottomrule
\end{tabular}
\end{center}
\noindent
Note: each shifted pole in the $E^+E^-$ OPE is
\emph{simple} (the product $(\zeta+\hbar)(\zeta-\hbar)$
in the denominator gives a simple pole at each root).
The ``shifted double pole'' in the class
determination refers to the \emph{combined effect} of
two simple poles separated by $2\hbar$: their Laurent
expansion around $\zeta = 0$ produces a second-order
pole $\sim 1/\zeta^2$ (from $1/(\zeta^2 - \hbar^2)
= -1/\hbar^2 + \zeta^2/\hbar^4 + \cdots$ when $\zeta$
is small relative to~$\hbar$; but around each
individual shifted pole, the singularity is first-order).
This is the key structural distinction from the classical
lattice VOA, where the double pole at $\zeta = 0$ is
genuine: $1/(z-w)^2$ vs.\
$1/(\zeta - \hbar)(\zeta + \hbar)$.

The collision residue on $\End(V \otimes V)$ assembles into
the \emph{quantum $r$-matrix}:
\begin{equation}\label{eq:qlattice-quantum-r}
r_q(\zeta)
\;=\;
\frac{\Omega_{\mathrm{Cartan}}}{\zeta}
\;+\;
\frac{e^+ \otimes e^-}{2(\zeta - \hbar)}
\;-\;
\frac{e^+ \otimes e^-}{2(\zeta + \hbar)}\,,
\end{equation}
where $\Omega_{\mathrm{Cartan}} = \tfrac{1}{2}J \otimes J$
is the Cartan part of the Casimir. The classical
$r$-matrix $r_{\mathrm{cl}}(\zeta) = \Omega/\zeta$
(with $\Omega$ the full $\mathfrak{sl}_2$ Casimir) is
recovered in the limit $\hbar \to 0$: the two shifted poles
merge at $\zeta = 0$, and
$\tfrac{1}{2}(1/(\zeta - \hbar) - 1/(\zeta + \hbar))
= \hbar/(\zeta^2 - \hbar^2) \to 1/\zeta$ as $\hbar \to 0$
after absorbing $\hbar$ into the normalisation.

\smallskip
\noindent
\emph{(3) The quantum depth spectrum: deformation of the weight-depth identity.}
The depth spectrum $\mathcal{D}(V_q)$ records the OPE depths
at which non-vanishing shadow operations occur.

\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lll}
\toprule
\textbf{Depth} & \textbf{Classical}
($V_{\sqrt{2}\Z}$) & \textbf{Quantum} ($V_q$) \\
\midrule
$d = 1$ (Killing form) & $\kappa = 1$ (double pole at
$\zeta = 0$) & $\kappa_q$: two simple poles at
$\zeta = \pm\hbar$ \\
$d = 2$ (Lie bracket) & $[e,f] = h$ (simple pole at
$\zeta = 0$) & $[K^+,K^-]_q$ (sum of shifted residues) \\
$d = 3$ (Serre) & $m_3 = 2e$ (iterated bracket) &
$m_3^q = [2]_q\,E^\pm$ (quantum Serre) \\
$d \ge 4$ & $0$ (Jacobi) & $0$ (quantum Jacobi) \\
\bottomrule
\end{tabular}
\end{center}
\noindent
The depth spectrum is $\mathcal{D}(V_q) = \{1, 2, 3\}
= \mathcal{D}(V_{\sqrt{2}\Z})$: \emph{the depth spectrum is
$q$-independent}. The class $\mathbf{L}$ assignment is
stable under $q$-deformation.

The weight-depth identity of
Theorem~\ref{thm:gap-migration}(i) takes a modified form
for $V_q$. In the classical setting, the identity
$w + d = n - 1$ relates the conformal weight~$w$ of
the generator, the depth~$d$ of the OPE pole, and the
degree~$n$. For the quantum algebra:
\begin{itemize}[nosep]
\item The Cartan channel ($J \cdot J$, $J \cdot E^\pm$)
 satisfies the classical identity verbatim: the poles
 are at $\zeta = 0$ and conformal weights are unchanged.
\item The root channel ($E^+ \cdot E^-$) satisfies a
 \emph{shifted weight-depth identity}: the ``effective
 depth'' is computed relative to the shifted poles.
 Each shifted simple pole at $\zeta = \pm\hbar$
 contributes at effective depth~$d_{\mathrm{eff}} = 2$
 (the residue is a field, not a scalar), while the
 combined effect of both poles contributes to
 $d_{\mathrm{eff}} = 1$ (the $1/\zeta^2$ term in the
 Laurent expansion around~$0$).
\end{itemize}
The resolution: the weight-depth identity holds
\emph{channel-by-channel} at each pole of the shifted
singularity locus. At each individual shifted pole
$\zeta = \pm\hbar$, the residue has $w = 1$,
$d_{\mathrm{eff}} = 1$, $n = 2$, giving $w + d = n$
(shifted by $+1$ relative to the classical formula).
The shift is exactly the $\hbar$-correction:
$w + d_{\mathrm{eff}} = n - 1 + 1$ where the extra $+1$
is the penalty for the off-diagonal singularity.

\smallskip
\noindent
\emph{(4) The $R$-matrix: leading order and Yang--Baxter verification.}
The $R$-matrix of $V_q$ is the \emph{trigonometric}
$R$-matrix of $U_q(\widehat{\mathfrak{sl}}_2)$. In the
fundamental representation $V = \C^2$:
\begin{equation}\label{eq:qlattice-R}
R(z)
\;=\;
\frac{1}{z - q^2}
\begin{pmatrix}
z\,q - q^{-1} & 0 & 0 & 0 \\
0 & z - 1 & q(z-1)(q - q^{-1})/z & 0 \\
0 & (q - q^{-1}) & z - 1 & 0 \\
0 & 0 & 0 & z\,q - q^{-1}
\end{pmatrix},
\end{equation}
where $z = e^{2\pi i u}$ is the multiplicative spectral
parameter. At \emph{leading order} in $\hbar$, writing
$q = 1 + i\pi\hbar + O(\hbar^2)$ and $z = 1 + 2\pi iu
+ O(u^2)$:
\begin{equation}\label{eq:qlattice-R-leading}
R(z) \;=\; 1 \;+\; \frac{\hbar\,\Omega}{u} \;+\; O(\hbar^2)\,,
\end{equation}
where $\Omega = e_{11}\otimes e_{11} + e_{22}\otimes e_{22}
+ e_{12}\otimes e_{21} + e_{21}\otimes e_{12}
= P + (q{-}1)\,\mathrm{diag}$ is the Casimir/permutation.
The leading term $1 + \hbar\Omega/u$ is the Yang $R$-matrix
of $Y_\hbar(\mathfrak{sl}_2)$, confirming that the classical
limit recovers the Yangian.

\emph{Yang--Baxter verification.}
The quantum YBE in multiplicative form reads
\begin{equation}\label{eq:qlattice-qYBE}
R_{12}(z_1/z_2)\,R_{13}(z_1/z_3)\,R_{23}(z_2/z_3)
\;=\;
R_{23}(z_2/z_3)\,R_{13}(z_1/z_3)\,R_{12}(z_1/z_2)
\end{equation}
as an identity in $\End(V^{\otimes 3})[z_1^{\pm 1},
z_2^{\pm 1}, z_3^{\pm 1}]$. The $R$-matrix~\eqref{eq:qlattice-R}
satisfies~\eqref{eq:qlattice-qYBE}: this is Jimbo's theorem
(1986). In the ordered bar interpretation, the YBE is
$d_B^2 = 0$ on ${\Barch}^{\mathrm{ord}}_3(V_q)$
(Proposition~\ref{prop:ybe-from-d-squared}): the two
codimension-one faces $D_{1,2}$ and $D_{2,3}$ of
$\Conf_3^{\mathrm{ord}}(\C)$ compose via
the $R$-matrix at their intersection, and the vanishing
of $d_B^2$ is exactly the YBE.

The verification at leading order is a direct
computation. Writing $R_{ij}(w) = 1 + \hbar\,\Omega_{ij}/w
+ O(\hbar^2)$ with $w = u_i - u_j$
(switching to additive spectral parameter):
\begin{align*}
&R_{12}(u_{12})\,R_{13}(u_{13})\,R_{23}(u_{23})
\\
&\quad =\; \bigl(1 + \hbar\,\Omega_{12}/u_{12}\bigr)\,
\bigl(1 + \hbar\,\Omega_{13}/u_{13}\bigr)\,
\bigl(1 + \hbar\,\Omega_{23}/u_{23}\bigr)
\;+\; O(\hbar^3)
\\
&\quad =\; 1 \;+\; \hbar\Bigl(
\frac{\Omega_{12}}{u_{12}} + \frac{\Omega_{13}}{u_{13}}
+ \frac{\Omega_{23}}{u_{23}}\Bigr)
\;+\; \hbar^2\Bigl(
\frac{\Omega_{12}\Omega_{13}}{u_{12}u_{13}}
+ \frac{\Omega_{12}\Omega_{23}}{u_{12}u_{23}}
+ \frac{\Omega_{13}\Omega_{23}}{u_{13}u_{23}}\Bigr)
+ O(\hbar^3).
\end{align*}
The right-hand side gives the same expression with
$12 \leftrightarrow 23$ swapped in the $\hbar^2$ products.
The difference at order $\hbar^2$ is:
\[
\frac{[\Omega_{12},\Omega_{13}]}{u_{12}\,u_{13}}
+ \frac{[\Omega_{12},\Omega_{23}]}{u_{12}\,u_{23}}
+ \frac{[\Omega_{13},\Omega_{23}]}{u_{13}\,u_{23}}
\;=\; 0\,,
\]
which is the classical Yang--Baxter equation (CYBE)
for $r(u) = \Omega/u$. This vanishes by the Jacobi identity
$[\Omega_{12},\Omega_{13}] + [\Omega_{12},\Omega_{23}]
+ [\Omega_{13},\Omega_{23}] = 0$
together with the identity
$1/u_{12}\,u_{13} + 1/u_{12}\,u_{23} + 1/u_{13}\,u_{23} = 0$
(since $u_{13} = u_{12} + u_{23}$).
The full quantum YBE at all orders is Jimbo's theorem;
the bar-complex proof
(Proposition~\ref{prop:ybe-from-d-squared}) provides an
independent verification via $d_B^2 = 0$.

This $R$-matrix is \emph{not} derivable from
the OPE by analytic continuation: the shifted poles at
$\zeta = \pm\hbar$ produce the trigonometric dependence, and
the monodromy around $\zeta = 0$ gives the quantum group
parameter~$q$. This is tier~(iii) of the three-tier
picture: the $R$-matrix is independent input, defining
the nonlocal braiding.

\smallskip
\noindent
\emph{(5) Classical vs.\ quantum: the stability table.}
The following table records which shadow invariants
change under the $q$-deformation $V_{\sqrt{2}\Z}
\rightsquigarrow V_q$ and which are $q$-stable.

\begin{center}
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lccl}
\toprule
\textbf{Invariant} & \textbf{$q = 1$}
 & \textbf{Generic $q$} & \textbf{Status} \\
\midrule
Class & $\mathbf{L}$ & $\mathbf{L}$
 & $q$-\emph{stable} \\
Depth spectrum & $\{1,2,3\}$ & $\{1,2,3\}$
 & $q$-\emph{stable} \\
$m_4$ & $0$ & $0$
 & $q$-\emph{stable} \\
Poincar\'{e} series & $1 + 3t$ & $1 + 3t$
 & $q$-\emph{stable} \\
Factorisation type & $E_\infty$ (local)
 & $E_1$ (nonlocal)
 & \textbf{changes} \\
$m_2(E^+,E^-)$ & $\alpha\cdot J$ (Lie bracket)
 & $[K^+,K^-]_q$ (braided)
 & \textbf{changes} \\
Collision residue & $\Omega/\zeta$ (one pole)
 & $r_q(\zeta)$ (three poles)
 & \textbf{changes} \\
$R$-matrix type & rational ($1 + \Omega/u$)
 & trigonometric~\eqref{eq:qlattice-R}
 & \textbf{changes} \\
$R$-matrix provenance & derived from OPE (tier~(ii))
 & independent input (tier~(iii))
 & \textbf{changes} \\
Koszul dual & $Y_\hbar(\mathfrak{sl}_2)$
 & $U_q(\widehat{\mathfrak{sl}}_2)$
 & \textbf{changes} \\
$\kappa$ & $3/4$
 & $3q^2/(q^2+q^{-2}+1)$
 & \textbf{changes} \\
Pole locus & $\{\zeta = 0\}$
 & $\{\zeta = 0, \pm\hbar\}$
 & \textbf{changes} \\
\bottomrule
\end{tabular}
\end{center}
\noindent
The pattern is sharp: the \emph{combinatorial} invariants
(class, depth spectrum, Poincar\'{e} series, tower
truncation) are $q$-stable, while the \emph{algebraic}
invariants (bracket type, collision residue, $R$-matrix,
Koszul dual, modular characteristic) are all $q$-deformed.
The stability of the depth spectrum is the
deepest structural fact: it says that the quantum
Serre relation terminates the tower at the same depth
as the classical Jacobi identity. The quantum deformation
$q - q^{-1}$ changes the \emph{coefficients} of the
shadow operations but not the \emph{pattern of
vanishing} that determines the depth.

The instability of the factorisation type ($E_\infty \to E_1$)
is the most fundamental change: the $q$-deformation
destroys locality. The three diagnostic tests below
(shifted singularities, non-derived $R$-matrix, ordered-only
factorisation) are each individually sufficient to detect
this transition. The classical lattice VOA is tier~(ii)
of the three-tier picture ($E_\infty$ with poles, $R$-matrix
derived from OPE); the quantum deformation promotes it to
tier~(iii) ($E_1$ with independent $R$-matrix).

\smallskip
\noindent
\emph{Koszul dual: $U_q(\widehat{\mathfrak{sl}}_2)$.}
The Koszul dual of $V_q$ is conjectured to be the
quantum affine algebra:
\begin{equation}\label{eq:qlattice-koszul}
V_q^!
\;\simeq\;
U_q(\widehat{\mathfrak{sl}}_2)\,.
\end{equation}
This is the $q$-deformation of the classical result
$V_{\sqrt{2}\Z}^! = Y_\hbar(\mathfrak{sl}_2)$
(Computation~\ref{comp:lattice-voa-ordered-bar}).
The rational $R$-matrix $R(z) = 1 + k\,\Omega_{\mathrm{tr}}/z$
(Yang $R$-matrix at level $k=1$ for $V_{\sqrt{2}\Z}$)
is deformed to the trigonometric
$R$-matrix~\eqref{eq:qlattice-R}, and the Yangian
$Y_\hbar(\mathfrak{sl}_2)$ is deformed to the quantum
affine algebra $U_q(\widehat{\mathfrak{sl}}_2)$.
The identification~\eqref{eq:qlattice-koszul} is
supported by the RTT presentation: the RTT relation
$R(z/w)\,T_1(z)\,T_2(w) = T_2(w)\,T_1(z)\,R(z/w)$
with the trigonometric $R$-matrix~\eqref{eq:qlattice-R}
generates $U_q(\widehat{\mathfrak{sl}}_2)$.

\smallskip
\noindent
\emph{Poincar\'{e} series.}
$P(t) = 1 + 3t$ (three cogenerators $s^{-1}J$,
$s^{-1}E^+$, $s^{-1}E^-$, matching the
$\dim(\mathfrak{sl}_2) = 3$ generators).

\smallskip
\noindent
\emph{Euler characteristic and modular data.}
The modular characteristic $\kappaChHodge(V_q) = 3q^2/(q^2 + q^{-2} + 1)$
interpolates between the classical value $\kappa = 3/4$
(at $q = 1$, the $A_1$ lattice VOA at $k = 1$) and the
critical value $\kappa = 0$ (at $q = i$, a root of unity).

\smallskip
\noindent
\emph{Genuinely $\Eone$: the three diagnostic tests.}
Three properties distinguish $V_q$ from
$E_\infty$-chiral algebras:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
\item \emph{Shifted singularities}: the OPE~\eqref{eq:qlattice-EE}
 has poles at $z - w = \pm\hbar$, not at $z = w$. No local
 (unordered) factorisation structure is compatible with
 $\hbar$-shifted singularity loci.
\item \emph{Non-derived $R$-matrix}: the trigonometric
 $R$-matrix~\eqref{eq:qlattice-R} is not the monodromy
 of any flat connection on $\Conf_2(\C)$ (it has a
 $q$-dependent spectral parameter, not a rational one).
\item \emph{Ordered-only factorisation}: the $n$-point
 correlation functions are defined on
 $\Conf_n^{\mathrm{ord}}(\C)$ and the Yang--Baxter
 equation replaces the $\Sigma_n$-equivariance of
 the $E_\infty$ setting.
\end{enumerate}
This is the mathematical content of ``nonlocal'' in the
$\Eone$-chiral framework: the braiding data cannot be
recovered from local OPE data, and the ordered configuration
space is the primitive geometric input.

\smallskip
\noindent
\emph{Genus-$1$.}
Propagator-entangled ($c_0 \ne 0$). The genus-$1$
deformation produces the \emph{elliptic quantum group}
$U_{q,p}(\widehat{\mathfrak{sl}}_2)$ of
Felder--Varchenko: the trigonometric $R$-matrix acquires
an elliptic deformation
$R(z;\tau) = R_{\mathrm{trig}}(z) + O(q_\tau)$,
where $q_\tau = e^{2\pi i \tau}$ is the elliptic nome.
The passage rational $\to$ trigonometric $\to$ elliptic
mirrors the genus tower: genus~$0$ gives the Yangian,
the $q$-deformation gives the quantum affine algebra,
and genus~$1$ gives the elliptic quantum group.
\ClaimStatusConjectured
\end{conjecture}

\begin{remark}[The $\Eone$ landscape: from vertex algebras to quantum vertex algebras]%
\label{rem:e1-landscape-quantum}%
\index{E1-chiral algebra@$\Eone$-chiral algebra!landscape}%
\index{quantum vertex algebra!landscape}%
Conjecture~\ref{comp:quantum-lattice-voa} provides the
first fully worked example in the genuinely $\Eone$ column
of the ordered bar landscape. The classical ($q = 1$) limit
recovers the $A_1$ lattice VOA
(Computation~\ref{comp:lattice-voa-ordered-bar}), and the
three diagnostic tests isolate the features that make the
quantum deformation genuinely nonlocal. The conjectured
Koszul dual identification $V_q^! \simeq
U_q(\widehat{\mathfrak{sl}}_2)$ (equation~\eqref{eq:qlattice-koszul})
extends the rational Koszul duality
$V_\Lambda^! = Y_\hbar(\fg)$ to the trigonometric regime.
Proving~\eqref{eq:qlattice-koszul} would establish the
full Yang--Baxter hierarchy
(rational $\to$ trigonometric $\to$ elliptic) as a
manifestation of the ordered bar complex at successive
genus levels.
\end{remark}

\begin{remark}[Hall--Drinfeld double, not chiral Yangian, at the K3
$\times E$ BKM frontier]
\label{rem:oackd-front-hall-drinfeld-double}
\index{Hall--Drinfeld double!K3 chiral}%
\index{BKM algebra!g Delta5@$\mathfrak{g}_{\Delta_5}$}%
\index{Schiffmann--Vasserot!shuffle presentation}%
\index{Manin pair!signature (2,1)}%
\index{taxonomic slot!fourth!Drinfeld double}%
\index{CoHA!CY-3 extension!Davison 2017}%
The rational $\to$ trigonometric $\to$ elliptic hierarchy of
Conjecture~\ref{comp:quantum-lattice-voa} (Yangian $\to$ quantum
affine $\to$ elliptic quantum group) extends \emph{in a
genuinely new direction} at the K3 $\times E$ CY$_3$ frontier:
the quantum-group deformation of the BKM algebra
$\mathfrak{g}_{\Delta_5}$ (Gritsenko--Nikulin fake monster,
Igusa denominator $\Delta_5$, arising in IIA on
$\mathrm{K3}\times T^2$) is \emph{not} a chiral Yangian at all,
but rather the K3 \emph{chiral Hall--Drinfeld double}
\[
 D_\hbar\bigl(Y_\hbar^{\mathrm{Hall}}(\mathrm{CoHA}_{\mathrm{K3}\times E})\bigr),
\]
built from the CoHA of coherent sheaves on $\mathrm{K3}\times E$
via the Davison~$2017$ CY-$3$ extension. The underlying
presentation is the Schiffmann--Vasserot shuffle
presentation, whose structure constants are extracted from
Hall-style CY-$3$ invariants rather than from a classical
RTT $r$-matrix. The structural obstruction to a Yangian
interpretation is the Manin \emph{pair} structure: the
second cohomology bilinear form
$\Lambda^{2,1}$ on $\mathfrak{g}_{\Delta_5}$ has
signature~$(2,1)$, which forbids the Lagrangian
decomposition $\mathfrak{g} = \mathfrak{g}_+ \oplus
\mathfrak{g}_-$ required for a Manin \emph{triple}. The
absence of a Lagrangian decomposition blocks the Yangian
(bialgebra-triangular) quantisation and forces the
\emph{Hall-double} route via CY-$3$ CoHA as the only
available quantum-group deformation. This identifies a
genuinely \emph{fourth taxonomic slot} in the Vol~II $E_1$-chiral
landscape, beyond $\{$rational Yangian, trigonometric quantum
affine, elliptic quantum group$\}$: the CY-$3$ Hall--Drinfeld
double, whose $E_1$-chiral realisation is the $\hbar$-deformed
shuffle algebra on $\Fact^{E_1}(\Ran_{\mathrm{K3}} \rtimes
\Ran_E)$ of Remark~\ref{rem:oackd-front-bi-based-ran}.
\end{remark}

\subsection{Non-principal $W$-algebras of
\texorpdfstring{$\mathfrak{sl}_4$}{sl(4)}:
the ordered bar complex beyond the affine lineage}
\label{subsec:ordered-bar-sl4-nonprincipal}

\index{W-algebra@$\mathcal W$-algebra!non-principal!ordered bar|textbf}
\index{bar complex!ordered!non-principal W-algebra@non-principal $\mathcal W$-algebra|textbf}

The preceding subsections treated two nilpotent orbits in
$\mathfrak{sl}_3$: the subregular ($\mathcal N=2$ SCA,
Computation~\ref{comp:ordered-bar-n2-sca}) and the minimal
(Bershadsky--Polyakov,
Computation~\ref{comp:ordered-bar-bp}). In both cases the
generator spectrum is~$(1,\frac32,\frac32,2)$ and the two
algebras are related by Koszul duality
(Conjecture~\eqref{eq:koszul-dual-explicit}).

For $\mathfrak{sl}_4$, the nilpotent poset has five orbits:
$(1^4)\subset(2,1^2)\subset(2,2)\subset(3,1)\subset(4)$.
The extreme orbits produce the affine algebra $V_k(\mathfrak{sl}_4)$
(zero nilpotent) and the principal $\mathcal W_4$ (regular nilpotent),
both already in the ordered bar landscape. The three intermediate
orbits produce three structurally distinct ordered bar complexes,
each exhibiting phenomena not visible in $\mathfrak{sl}_3$.

\subsubsection{The subregular
$W_k(\mathfrak{sl}_4,f_{(3,1)})$}
\label{subsubsec:ordered-bar-sl4-subregular}

\begin{computation}[Ordered bar complex of
$W_k(\mathfrak{sl}_4,f_{(3,1)})$; \ClaimStatusConditional]
\label{comp:ordered-bar-sl4-subreg}
\index{W-algebra@$\mathcal W$-algebra!subregular!sl4@$\mathfrak{sl}_4$!ordered bar|textbf}

\medskip
\noindent\textbf{Part~\ref{part:swiss-cheese}: Generator data.}
The subregular $\mathcal W$-algebra
$\mathcal W^k(\mathfrak{sl}_4,f_{(3,1)})$ has \textbf{five
strong generators}:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccc}
\textbf{Generator} & \textbf{Weight $h$} &
 \textbf{Statistics} & \textbf{Bar degree $|s^{-1}\Phi|$} &
 \textbf{Origin} \\
\hline
$J$ & $1$ & bosonic & $-1$ &
 Cartan (surviving $U(1)$) \\
$T^{(1)}$ & $2$ & bosonic & $-1$ &
 stress tensor \\
$T^{(2)}$ & $2$ & bosonic & $-1$ &
 second Casimir current \\
$T^{(3)}$ & $2$ & bosonic & $-1$ &
 third Casimir current \\
$W$ & $3$ & bosonic & $-1$ &
 cubic Casimir
\end{tabular}
\end{center}
All generators are bosonic: no fermionic fields survive the
subregular DS reduction of $\mathfrak{sl}_4$. This
contrasts sharply with the $\mathfrak{sl}_3$ subregular
($\mathcal N=2$ SCA), where two fermionic generators
$G^\pm$ of weight~$3/2$ appear. The absence of fermions
is because the JM grading for the $(3,1)$ subregular
in $\mathfrak{sl}_4$ has only even-integer grades
(Remark~\ref{rem:tholog-sl4-extension}): all surviving
generators inherit integer conformal weights.

The weight-$1$ generator $J$ survives because the
centraliser $\mathfrak{g}^{f_{(3,1)}}$ has a one-dimensional
grade-$0$ component (a $U(1)$ factor in the Levi). The
three weight-$2$ generators $T^{(i)}$ arise from the
three independent Casimir directions visible at grade~$0$
under the JM decomposition with
$\dim = 1 + 4 + 5 + 4 + 1$; they span a
three-dimensional space $\mathcal V_2$.

\emph{Comparison with principal $\mathcal W_4$.}
The principal $\mathcal W_4$ has generators of weights
$2, 3, 4$ (three generators, all from Casimir currents,
no weight-$1$ field). The subregular has generators of
weights $1, 2, 2, 2, 3$ (five generators, including a
weight-$1$ current). Despite having the \emph{same
parent algebra} $V_k(\mathfrak{sl}_4)$, the two reductions
produce ordered bar complexes with completely different
algebraic signatures.

\medskip
\noindent\textbf{Part~\ref{part:e1-core}: The OPE structure and pole
orders.}

The five generators give $\binom{5}{2}+5 = 15$
independent OPE sectors. The maximal pole orders are
determined by the conformal weights and the DS constraints:

\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcc}
\textbf{OPE sector} & \textbf{Max pole order}
 & \textbf{Collision depth} \\
\hline
$J \cdot J$ & $2$ & $1$ \\
$J \cdot T^{(i)}$ & $2$ & $1$ \\
$J \cdot W$ & $1$ & $0$ \\
$T^{(i)} \cdot T^{(j)}$ ($i=j$) & $4$ & $3$ \\
$T^{(i)} \cdot T^{(j)}$ ($i\neq j$) & $4$ & $3$ \\
$T^{(i)} \cdot W$ & $4$ & $3$ \\
$W \cdot W$ & $6$ & $5$
\end{tabular}
\end{center}
The collision depth column applies (the $d\log$ kernel
absorbs one pole: $d = p - 1$).

\emph{The $T^{(i)} \cdot T^{(j)}$ cross-OPEs.}
The three weight-$2$ generators form a non-abelian
algebra: the cross-OPEs $\{T^{(i)}_\lambda T^{(j)}\}$
for $i \neq j$ have generically nonzero quartic poles
(the ``Sugawara mixing'' from the three Casimir directions).
This creates a $3 \times 3$ matrix of quartic-pole
coefficients,
\[
\kappa_{ij}
\;=\;
\lim_{\lambda\to\infty}
\frac{\{T^{(i)}_\lambda T^{(j)}\}}{\lambda^3},
\]
which is the \emph{Gram matrix} of the normalised Killing
form restricted to the grade-$0$ Casimir subspace. For
generic $k$, $\kappa_{ij}$ is non-degenerate of rank~$3$;
at special values of $k$ (including $k=-h^\vee=-4$), it
degenerates.

This $3\times 3$ Gram matrix has no analogue in any
previous ordered bar computation:
\begin{itemize}
\item The $\mathcal N=2$ SCA has a single $T$ (no cross-OPEs).
\item BP has a single $T$ (same).
\item Principal $\mathcal W_N$ has generators of different
 weights, so cross-OPEs between generators of the same
 weight do not arise for $N\le 4$.
\end{itemize}
The Gram matrix $\kappa_{ij}$ is the first instance of
\emph{intra-weight mixing} in the ordered bar complex,
and it governs the $R$-matrix structure in the
$\mathcal V_2 \otimes \mathcal V_2$ sector.

\medskip
\noindent\textbf{Part~\ref{part:bbl-core}: Six-level depth stratification.}

\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{llcc}
\textbf{Sector} & \textbf{Generators}
 & \textbf{Max OPE pole} & \textbf{Depth} \\
\hline
Gravity$^{(3)}$ & $(W,W)$ & sextic & $5$ \\
Gravity$^{(2)}$ & $(T^{(i)},W)$ & quartic & $3$ \\
Gravity$^{(1)}$ & $(T^{(i)},T^{(j)})$ & quartic & $3$ \\
Matter & $(J,T^{(i)})$ & double & $1$ \\
Current & $(J,J)$ & double & $1$ \\
Charge & $(J,W)$ & simple & $0$
\end{tabular}
\end{center}

The depth stratification has three layers:
\begin{itemize}
\item \textbf{Depth $5$} ($W$--$W$ sector): class~$\mathbf{M}$.
 The sextic OPE pole produces infinite shadow depth,
 exactly as for the $(W_3,W_3)$ sector of the principal
 $\mathcal W_3$. The collision residue
 $r^{WW}(z) = (\kappa_{WW}/z^5) + \cdots$ has
 the same leading structure as the principal case, but
 with different OPE coefficients.
\item \textbf{Depth $3$} ($T^{(i)}$--$T^{(j)}$ and
 $T^{(i)}$--$W$ sectors): class~$\mathbf{M}$.
 The quartic OPE pole produces infinite shadow depth for
 each weight-$2$ generator. This is the Virasoro-type
 sector, but \emph{triplicated}: three copies of the
 quartic-pole mechanism interact via the Gram matrix
 $\kappa_{ij}$.
\item \textbf{Depth $0$} ($J$-sectors):
 class~$\mathbf{G}$.
 The $J$--$J$ sector is Heisenberg-type and formal, and $J$--$W$
 is abelian at the displayed depth.
\end{itemize}

\emph{Comparison with the $\mathfrak{sl}_3$ subregular.}
The $\mathcal N=2$ SCA exhibits all four classes
$\mathbf{G}$/$\mathbf{L}$/$\mathbf{C}$/$\mathbf{M}$, with
the matter sector ($G^+$--$G^-$, cubic pole) providing the
class~$\mathbf{C}$ stratum. The subregular
$\mathfrak{sl}_4$ algebra has no class~$\mathbf{C}$
stratum: the absence of fermionic generators eliminates
cubic poles, and the spectrum jumps from
$\mathbf{G}$/$\mathbf{L}$ directly to $\mathbf{M}$. The
depth classes present are $\{\mathbf{G}, \mathbf{L}, \mathbf{M}\}$
(three of four, with a \emph{gap at class~$\mathbf{C}$}).

\emph{The depth gap.}
The $(W,W)$ sector has maximal collision depth $5$, with
the gap at $d = 4$: the $\lambda^4$ coefficient in
$\{W_\lambda W\}$ would produce a weight-$1$ field
($h = 3 + 3 - 5 = 1$), and $W$ is present, so the gap
occurs at $d = 4 = 2 \cdot 3 - 2$, matching the gap
migration formula $d_{\mathrm{gap}} = 2s - 2$ for a
generator of spin~$s = 3$.

However, the $(T^{(i)}, T^{(j)})$ sectors have a
\emph{different gap pattern}: the quartic-pole $\lambda^3$
coefficient in $\{T^{(i)}_\lambda T^{(j)}\}$ produces
a weight-$1$ field (weight $2+2-3=1$), and $J$ IS present
as a generator. Therefore, the $\lambda^3$ coefficient
is generically \textbf{nonzero}: it contributes a
$J$-valued collision residue. The gap in the
$(T^{(i)},T^{(j)})$ sector is at $d = 3$
(the quartic pole is the maximum, and depth $3$ is the
full depth without any gap), and the depth spectrum at
degree~$2$ is:
\[
\Spec(W^k(\mathfrak{sl}_4, f_{(3,1)}), 2)
\;=\;
\{0, 1, 2, 3, 5\},
\qquad
\text{gap at } d = 4.
\]
This matches the $\mathcal W_3$ depth spectrum
$\{0,1,2,3,5\}$ (equation~\eqref{eq:w3-spec-2})
with the \emph{same gap position} $d = 4$. But the
mechanism is different: for the principal $\mathcal W_3$,
the gap arises because no weight-$1$ generator exists;
for the subregular $\mathfrak{sl}_4$, the gap arises
in the $(W,W)$ sector despite a weight-$1$ generator
being present, because $J$ does not appear in the
$\{W_\lambda W\}$ bracket at the quintic pole
(the charge selection rule: $q(W) = 0$ implies the
quintic pole coefficient has $U(1)$ charge $0$, and
the only weight-$1$ candidate is $J$ with $q = 0$,
but the coefficient vanishes by a Casimir identity).

\medskip
\noindent\textbf{Part~\ref{part:examples}: The $R$-matrix and
intra-weight mixing.}

The $R$-matrix on
$V \otimes V$ with
$V = \operatorname{span}\{J, T^{(1)}, T^{(2)}, T^{(3)}, W\}$
decomposes into weight sectors. The critical new feature
is the $\mathcal V_2 \otimes \mathcal V_2$ block
($3 \times 3$ matrix in the weight-$2$ subspace):
\begin{equation}\label{eq:r-matrix-sl4-subreg-weight2}
R^{(\mathcal V_2)}(z)
\;=\;
\id_3 + \frac{r_0^{(\mathcal V_2)}}{z}
+ \frac{r_1^{(\mathcal V_2)}}{z^2}
+ \frac{r_2^{(\mathcal V_2)}}{z^3}
+ O(z^{-4}),
\end{equation}
where $r_0^{(\mathcal V_2)}$ is a $3 \times 3$
matrix encoding the zeroth products
$T^{(i)}_{(0)} T^{(j)}$ (the adjoint action of the
weight-$2$ generators on each other), and
$r_2^{(\mathcal V_2)}$ encodes the Gram matrix
$\kappa_{ij}$ (the quartic pole residues).

The Yang--Baxter equation on
$R^{(\mathcal V_2)}_{12}(z_{12})\,
R^{(\mathcal V_2)}_{13}(z_{13})\,
R^{(\mathcal V_2)}_{23}(z_{23})
= R^{(\mathcal V_2)}_{23}\,
R^{(\mathcal V_2)}_{13}\,
R^{(\mathcal V_2)}_{12}$
is a \emph{matrix Yang--Baxter equation} of $3 \times 3$
type. For the principal $\mathcal W_4$, each generator
has a distinct weight, so the $R$-matrix is diagonal in
the weight decomposition; the YBE reduces to scalar
equations in each weight sector. For the subregular, the
three weight-$2$ generators force a genuinely matricial
YBE, the first in the ordered bar programme.

\medskip
\noindent\textbf{Part~\ref{part:holography}: Non-primitive coproduct.}

As for the BP algebra
(Proposition~\ref{prop:bp-coproduct}), the weight-$1$
generator $J$ is primitive, but the weight-$2$ generators
$T^{(i)}$ are generically non-primitive:
\[
\Delta_{z,1}(T^{(i)})
\;=\;
T^{(i)} \otimes 1 + 1 \otimes T^{(i)}
+ \frac{\gamma_i}{z}\, J \otimes J
+ \frac{1}{z}\sum_{j} \beta_{ij}\, J \otimes T^{(j)},
\]
where $\gamma_i$ and $\beta_{ij}$ depend on the
normalisation and the Casimir decomposition. The
$J \otimes T^{(j)}/z$ cross-terms are \emph{new}:
they arise from the intra-weight mixing and have no
analogue in the BP algebra (which has only a single
weight-$2$ generator, so no intra-weight cross-terms are
possible). The weight-$3$ generator $W$ has additional
cross-terms:
\[
\Delta_{z,1}(W)
\;=\;
W \otimes 1 + 1 \otimes W
+ \frac{1}{z}\sum_i \alpha_i\, J \otimes T^{(i)}
+ \frac{1}{z^2}\, \alpha'\, J \otimes J \otimes J,
\]
where the triple tensor term at $1/z^2$ reflects the
Sugawara cubic structure.

\medskip
\noindent\textbf{Part~\ref{part:gravity}: Summary.}

\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcc}
\textbf{Feature} & \textbf{SCA ($\mathfrak{sl}_3$ sub)}
 & \textbf{$\mathfrak{sl}_4$ sub} \\
\hline
Generators & $4$ ($J, G^+, G^-, T$)
 & $5$ ($J, T^{(1,2,3)}, W$) \\
Fermionic? & yes ($G^\pm$) & no \\
Weight spectrum & $1, \tfrac32, \tfrac32, 2$
 & $1, 2, 2, 2, 3$ \\
Depth classes & $\mathbf{G/L/C/M}$
 & $\mathbf{G/L/M}$ (no $\mathbf{C}$) \\
Max depth & $\infty$ (quartic $TT$)
 & $\infty$ (sextic $WW$, quartic $T^{(i)}T^{(j)}$) \\
$R$-matrix type & $4\times 4$ scalar sectors
 & $3\times 3$ matricial in $\mathcal V_2$ \\
Coproduct & non-primitive on $T$ only
 & non-primitive on $T^{(i)}$ and $W$ \\
Depth gap ($n=2$) & $d=4$ & $d=4$ \\
Koszul dual & BP (minimal $\mathfrak{sl}_3$)
 & minimal $\mathfrak{sl}_4$ (cross-orbit)
\end{tabular}
\end{center}
\end{computation}

\subsubsection{The hook-type
$W_k(\mathfrak{sl}_4,f_{(2,1,1)})$:
the minimal nilpotent}
\label{subsubsec:ordered-bar-sl4-hook}

\begin{computation}[Ordered bar complex of
$W_k(\mathfrak{sl}_4,f_{(2,1,1)})$; \ClaimStatusProvedHere]
\label{comp:ordered-bar-sl4-hook}
\index{W-algebra@$\mathcal W$-algebra!hook-type!sl4@$\mathfrak{sl}_4$!ordered bar|textbf}
\index{W-algebra@$\mathcal W$-algebra!minimal!sl4@$\mathfrak{sl}_4$!ordered bar|textbf}
\index{bar complex!ordered!hook-type W-algebra@hook-type $\mathcal W$-algebra|textbf}

The minimal nilpotent $f_{(2,1,1)}$ in $\mathfrak{sl}_4$
produces the \emph{hook-type} $\mathcal W$-algebra
$\mathcal W^k(\mathfrak{sl}_4, f_{(2,1,1)})$. The
partition $(2,1,1)$ is a hook ($r = 2$, $N - r = 2$), and
the Koszul dual is the subregular
$\mathcal W^{-k-8}(\mathfrak{sl}_4, f_{(3,1)})$, a
\emph{cross-orbit duality} (the first in the ordered bar
programme) between algebras with different generator
counts and different weight spectra.

\medskip
\noindent\textbf{Part~\ref{part:swiss-cheese}: Generator data.}
The algebra has \textbf{nine strong generators}:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lccccc}
\textbf{Generator} & \textbf{Weight $h$} &
 \textbf{Stat.} & \textbf{Bar deg.} &
 \textbf{$U(1)$ charge} & \textbf{Count} \\
\hline
$J_a$ ($a=1,2,3$) & $1$ & bosonic & $-1$ & $0$ & $3$ \\
$\psi$ & $1$ & bosonic & $-1$ & $0$ & $1$ \\
$G^+_\alpha$ ($\alpha=1,2$) & $3/2$ & fermionic & $0$
 & $+1$ & $2$ \\
$G^-_\alpha$ ($\alpha=1,2$) & $3/2$ & fermionic & $0$
 & $-1$ & $2$ \\
$T$ & $2$ & bosonic & $-1$ & $0$ & $1$
\end{tabular}
\end{center}
The weight-$1$ generators $(J_a, \psi)$ form a
four-dimensional space: $J_a$ ($a=1,2,3$) generate an
$\widehat{\mathfrak{sl}}_2$ subalgebra at a shifted level
$k' = k + 1$ (the DS level shift for the $(2,1,1)$
reduction), and $\psi$ is an abelian current
($\psi_{(0)}\psi = 0$, $\psi_{(1)}\psi = K'$ for
a normalisation constant $K'$).

The fermionic generators $G^\pm_\alpha$ ($\alpha = 1, 2$)
transform as a doublet under the $\mathfrak{sl}_2$
subalgebra spanned by~$J_a$ and carry $U(1)$ charge
$\pm 1$ under~$\psi$. The stress tensor $T$ is
a composite (Sugawara-type).

\emph{Comparison with the BP algebra of $\mathfrak{sl}_3$.}
The BP algebra has generators $(J, G^+, G^-, T)$ of
weights $(1, \frac32, \frac32, 2)$ with a \emph{single}
$U(1)$ current $J$ and a single pair of fermions
$(G^+, G^-)$. The hook-type $\mathfrak{sl}_4$ algebra
has \emph{four} weight-$1$ generators, \emph{two pairs}
of fermions, and the weight-$1$ sector is non-abelian
(an $\widehat{\mathfrak{sl}}_2$ subalgebra). The
BP algebra is the simplest hook-type; the
$\mathfrak{sl}_4$ minimal is the next.

\medskip
\noindent\textbf{Part~\ref{part:e1-core}: The OPE structure.}

The OPE sectors organise by the Lie-algebraic
structure. The key brackets are:

\smallskip
\noindent\emph{(i) $\widehat{\mathfrak{sl}}_2$ subalgebra.}
\begin{equation}\label{eq:sl4hook-JJ}
\{J_a{}_\lambda\, J_b\}
\;=\;
\epsilon_{abc}\, J_c
+ k'\,\delta_{ab}\,\lambda,
\end{equation}
where $\epsilon_{abc}$ is the structure constant of
$\mathfrak{sl}_2$ and $k' = k + 1$. This has a
double pole (the level) and a simple pole (the Lie
bracket): depth $1$, class~$\mathbf{L}$.

\smallskip
\noindent\emph{(ii) $U(1)$ current.}
\begin{equation}\label{eq:sl4hook-psipsi}
\{\psi_\lambda\, \psi\}
\;=\;
K'\lambda,
\end{equation}
with $K' = (2k+4)/(k+4)$ (the DS-shifted normalisation).
Double pole, depth $1$, class~$\mathbf{L}$.

\smallskip
\noindent\emph{(iii) $\mathfrak{sl}_2$ charges of the
fermions.}
\begin{equation}\label{eq:sl4hook-JG}
\{J_a{}_\lambda\, G^\pm_\alpha\}
\;=\;
(\sigma_a)_{\alpha\beta}\, G^\pm_\beta,
\end{equation}
where $(\sigma_a)_{\alpha\beta}$ are the $2\times 2$
representation matrices of $\mathfrak{sl}_2$.
Simple pole (charge action), depth $0$,
class~$\mathbf{G}$.

\smallskip
\noindent\emph{(iv) $U(1)$ charges of the fermions.}
\begin{equation}\label{eq:sl4hook-psiG}
\{\psi_\lambda\, G^\pm_\alpha\}
\;=\;
\pm G^\pm_\alpha.
\end{equation}
Simple pole, depth $0$, class~$\mathbf{G}$.

\smallskip
\noindent\emph{(v) The supersymmetric OPE:
$G^+_\alpha$--$G^-_\beta$.}
\begin{equation}\label{eq:sl4hook-GpGm}
\{G^+_\alpha{}_\lambda\, G^-_\beta\}
\;=\;
K'\delta_{\alpha\beta}\,\lambda^2
+ (\sigma_a)_{\alpha\beta}\, J_a\,\lambda
+ \delta_{\alpha\beta}\,\psi\,\lambda
+ \delta_{\alpha\beta}\bigl(T + \Sigma\bigr)
+ (\sigma_a)_{\alpha\beta}\,\partial J_a\cdot\gamma,
\end{equation}
where $\Sigma$ is a Sugawara-type composite
($\Sigma \propto {:}J_a J_a{:}/k' + {:}\psi\psi{:}/K' + \cdots$)
and $\gamma$ is a normalisation coefficient. The
maximal pole order is cubic ($\lambda^2$ term): depth $2$,
class~$\mathbf{C}$.

\smallskip
\noindent\emph{(vi) Vanishing OPEs.}
$\{G^\pm_\alpha{}_\lambda\, G^\pm_\beta\} = 0$.

\smallskip
\noindent\emph{(vii) Virasoro subalgebra.}
$\{T_\lambda T\} = \frac{c}{12}\lambda^3 + 2T\lambda
+ \partial T$,
with the central charge
$c = c(\mathfrak{sl}_4, f_{(2,1,1)}, k)$ given by the
DS formula. Quartic pole, depth $3$,
class~$\mathbf{M}$.

\medskip
\noindent\textbf{Part~\ref{part:bbl-core}: Depth stratification:
all four classes, doubled.}

\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{llccc}
\textbf{Sector} & \textbf{Generators}
 & \textbf{Max pole} & \textbf{Depth}
 & \textbf{Class} \\
\hline
Gravity & $(T,T)$ & quartic & $3$ & $\mathbf{M}$ \\
Matter & $(G^+_\alpha, G^-_\beta)$ & cubic & $2$
 & $\mathbf{C}$ \\
$\mathfrak{sl}_2$ current & $(J_a, J_b)$ & double & $1$
 & $\mathbf{L}$ \\
$U(1)$ current & $(\psi, \psi)$ & double & $1$
 & $\mathbf{L}$ \\
$\mathfrak{sl}_2$ charge & $(J_a, G^\pm_\alpha)$
 & simple & $0$ & $\mathbf{G}$ \\
$U(1)$ charge & $(\psi, G^\pm_\alpha)$
 & simple & $0$ & $\mathbf{G}$
\end{tabular}
\end{center}

All four depth classes are present:
$\mathbf{G}$, $\mathbf{L}$, $\mathbf{C}$, $\mathbf{M}$,
the same four-class spectrum as the BP algebra
(Remark~\ref{rem:bp-depth-stratification}). The
structural novelty is the \emph{multiplicity}: the
$\mathbf{G}$ sector has two independent types
($\mathfrak{sl}_2$ charge and $U(1)$ charge), the
$\mathbf{L}$ sector has two independent types
($\mathfrak{sl}_2$ currents and $U(1)$ current), and the
$\mathbf{C}$ sector has $2 \times 2 = 4$ components
(two doublet indices). The BP algebra, by contrast,
has a single component in each class.

\emph{The $\mathfrak{sl}_2$ structure in the matter
sector.}
The $G^+_\alpha$--$G^-_\beta$ collision residue
\eqref{eq:sl4hook-GpGm} has both a diagonal part
($\delta_{\alpha\beta}$) and an off-diagonal part
($(\sigma_a)_{\alpha\beta}$). The diagonal part
reproduces the BP matter sector; the off-diagonal part
is a genuinely new phenomenon: the \emph{spin-exchange
collision}, where two fermions of different
$\mathfrak{sl}_2$ quantum numbers interact through the
$J_a$ current. This couples the matter and current
sectors at the level of the bar differential, producing
non-diagonal entries in the $m_2$ matrix.

\medskip
\noindent\textbf{Part~\ref{part:examples}: The $m_3$ operations.}

\emph{(a) $m_3(T,T,T)$: Virasoro at $c = c_{\mathrm{hook}}$.}
The gravity sector closes on itself:
$m_3(T,T,T) = m_3^{\mathrm{Vir}}(T,T,T)|_{c = c_{\mathrm{hook}}}$,
the same universal Virasoro ternary product specialised
to the hook-type central charge. Class~$\mathbf{M}$,
infinite shadow obstruction tower.

\emph{(b) $m_3(J_a, J_b, J_c)$: the Kac--Moody sub-PVA.}
The $\mathfrak{sl}_2$ current sector produces
\begin{equation}\label{eq:sl4hook-m3-JJJ}
m_3(J_a, J_b, J_c)
\;=\;
k'\,\epsilon_{abc}\cdot 1 \;\neq\; 0.
\end{equation}
This is \textbf{nonzero}: unlike the abelian $J$--$J$
sector of the BP algebra (where $m_3(J,J,J) = 0$ by
formality of the Heisenberg), the non-abelian
$\widehat{\mathfrak{sl}}_2$ subalgebra has a nonvanishing
ternary product from the iteration
$m_2(m_2(J_a, J_b), J_c) - m_2(J_a, m_2(J_b, J_c))
\propto k'\,\epsilon_{abc}$.

This is the first example where the $\mathbf{L}$-class
sector has $m_3 \neq 0$. The resolution is that the
$\mathfrak{sl}_2$ sector is class~$\mathbf{L}$
\emph{as a sector} (the highest pole is double), but
the ternary product does not vanish because the
noncommutativity of $\mathfrak{sl}_2$ provides a
nontrivial obstruction to formality.
More precisely, $m_3$ is nonzero but
$m_4(J_a,J_b,J_c,J_d) = 0$ (by the Jacobi identity):
the current sector is class~$\mathbf{L}$
(finite depth $r_{\max} = 3$, i.e.\ $m_n = 0$
for $n \ge 4$ on the current generators alone),
not class~$\mathbf{G}$ (which would require $m_3 = 0$).

\emph{(c) $m_3(G^+_\alpha, G^-_\beta, J_a)$:
matter-current mixing.}
The cross-sector ternary product is nonzero and
encodes the spin-exchange at the level of $m_3$:
\[
m_3(G^+_\alpha, G^-_\beta, J_a)
\;\propto\;
(\sigma_a)_{\alpha\beta}\,T
+ \delta_{\alpha\beta}\,\partial J_a
+ \cdots.
\]
This cross-sector mixing has no analogue in the BP
algebra (where a single fermion pair interacts with a
single current, preventing matrix structure in the
$\mathfrak{sl}_2$ indices).

\emph{(d) Fermion nilpotency.}
$m_n(G^\pm_\alpha, G^\pm_\beta, X_1, \ldots, X_{n-2})
= 0$ for all $n \ge 2$, as for the BP algebra.

\medskip
\noindent\textbf{Part~\ref{part:holography}: Non-primitive coproduct,
non-abelian Casimir.}

The coproduct structure reflects the non-abelian
weight-$1$ sector. The $\mathfrak{sl}_2$ generators
$J_a$ are \emph{not primitive} (unlike the abelian $J$
of the BP algebra):
\begin{equation}\label{eq:sl4hook-coproduct-J}
\Delta_{z,1}(J_a)
\;=\;
J_a \otimes 1 + 1 \otimes J_a
+ \frac{1}{z}\sum_{b,c}\epsilon_{abc}\,
J_b \otimes J_c.
\end{equation}
The cross-term $\epsilon_{abc}\,J_b \otimes J_c/z$
arises from the Lie bracket (the zeroth product
$J_a{}_{(0)}J_b = \epsilon_{abc}J_c$) propagated
through the HPL transfer.

The abelian current $\psi$ remains primitive:
$\Delta_{z,1}(\psi) = \psi \otimes 1 + 1 \otimes \psi$.

The stress tensor $T$ is doubly non-primitive:
\[
\Delta_{z,1}(T)
\;=\;
T \otimes 1 + 1 \otimes T
+ \frac{1}{z}\sum_a J_a \otimes J_a
+ \frac{\gamma}{z}\,\psi \otimes \psi,
\]
receiving cross-terms from both the $\mathfrak{sl}_2$
Casimir ($\sum_a J_a \otimes J_a$) and the $U(1)$
Casimir ($\psi \otimes \psi$).

\emph{Comparison with BP.}
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcc}
\textbf{Generator} & \textbf{BP coproduct}
 & \textbf{Hook $\mathfrak{sl}_4$ coproduct} \\
\hline
$J$ (resp.\ $J_a$) & primitive
 & non-primitive ($\epsilon$-term) \\
$\psi$ &; (absent)
 & primitive \\
$G^\pm$ (resp.\ $G^\pm_\alpha$) & primitive
 & primitive \\
$T$ & $J \otimes J/z$
 & $J_a \otimes J_a/z + \psi \otimes \psi/z$
\end{tabular}
\end{center}

This is the \textbf{principal vs non-principal coproduct
dichotomy extended}: for hook-type reductions with a
non-abelian surviving subalgebra, even the weight-$1$
generators are non-primitive. The non-primitivity of
$J_a$ is a direct manifestation of the non-abelian Lie
bracket, visible already at the HPL level.

\medskip
\noindent\textbf{Part~\ref{part:gravity}: The depth spectrum and
comparison.}

At degree~$2$, the depth spectrum is determined by the
maximal pole orders in each sector. The $(T,T)$ sector
contributes depths $\{0,1,2,3\}$ (quartic pole);
the $(G^+_\alpha, G^-_\beta)$ sector contributes
$\{0,1,2\}$ (cubic pole);
the $(J_a, J_b)$ sector contributes $\{0,1\}$
(double pole);
the remaining sectors contribute $\{0\}$ or $\{0,1\}$.
The total:
\begin{equation}\label{eq:sl4hook-spec-2}
\Spec(W^k(\mathfrak{sl}_4, f_{(2,1,1)}), 2)
\;=\;
\{0, 1, 2, 3\},
\qquad
\text{no gap.}
\end{equation}
There is \emph{no depth gap at degree~$2$}. This
contrasts with the principal $\mathcal W_3$
($\Spec = \{0,1,2,3,5\}$, gap at $d=4$), the subregular
$\mathfrak{sl}_4$ ($\Spec = \{0,1,2,3,5\}$, gap at
$d=4$), and the BP algebra (which inherits a gap from
its Virasoro subsector).

The absence of a gap is because the maximal pole order
is quartic (from $T$--$T$), and the quartic pole
accounts for depths $0,1,2,3$ without any missing entry.
The $(G^+$--$G^-$) cubic pole fills in depth $2$, and
the $(J$--$J$) double pole fills in depth $1$. No depth
is skipped.

\emph{At higher degrees:} the Virasoro subsector
eventually produces gaps (at $d=3$ for $n=3$, etc.),
but the matter sector fills in lower depths. The
hook-type algebra has a \emph{denser} depth spectrum
than any principal $\mathcal W$-algebra of comparable
central charge.
\end{computation}

\subsubsection{The rectangular
$W_k(\mathfrak{sl}_4,f_{(2,2)})$: the self-dual orbit}
\label{subsubsec:ordered-bar-sl4-rectangular}

\begin{computation}[Ordered bar complex of
$W_k(\mathfrak{sl}_4,f_{(2,2)})$; \ClaimStatusProvedHere]
\label{comp:ordered-bar-sl4-rect}
\index{W-algebra@$\mathcal W$-algebra!rectangular!sl4@$\mathfrak{sl}_4$!ordered bar|textbf}
\index{bar complex!ordered!rectangular W-algebra@rectangular $\mathcal W$-algebra|textbf}

The rectangular nilpotent $f_{(2,2)}$ is the unique
non-hook, non-principal intermediate orbit in
$\mathfrak{sl}_4$. Since $(2,2)^t = (2,2)$, the
Koszul duality is \emph{same-orbit}:
$\mathcal W^k(\mathfrak{sl}_4, f_{(2,2)})^!
\simeq \mathcal W^{-k-8}(\mathfrak{sl}_4, f_{(2,2)})$.

\medskip
\noindent\textbf{Part~\ref{part:swiss-cheese}: Generator data.}
The algebra has \textbf{seven strong generators}:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccc}
\textbf{Generator} & \textbf{Weight $h$} &
 \textbf{Stat.} & \textbf{Bar deg.} &
 \textbf{Count} \\
\hline
$J_a$ ($a=1,2,3$) & $1$ & bosonic & $-1$ & $3$ \\
$T^{(\mu)}$ ($\mu=1,2,3,4$) & $2$ & bosonic & $-1$
 & $4$
\end{tabular}
\end{center}
All generators are bosonic (the JM grading for $(2,2)$
is concentrated in even degrees: $\dim = 4+7+4$). The
weight-$1$ generators $J_a$ form an
$\widehat{\mathfrak{sl}}_2$ subalgebra (from the Levi
factor of the parabolic associated to $(2,2)$, which has
$\mathfrak{gl}_2$ as its reductive part; the trace-free
part is $\mathfrak{sl}_2$). The four weight-$2$
generators $T^{(\mu)}$ span the complement, transforming
as a representation of $\mathfrak{sl}_2$.

\emph{No fermionic generators, no weight-$3$ generator.}
Unlike the subregular (which has a weight-$3$ generator)
and the hook-type (which has fermionic generators),
the rectangular orbit produces an algebra with only
bosonic generators of weights $1$ and $2$. This is the
\emph{simplest weight spectrum} among all non-principal,
non-trivial $\mathfrak{sl}_4$ reductions.

\medskip
\noindent\textbf{Part~\ref{part:e1-core}: The OPE structure.}

\smallskip
\noindent\emph{(i) $\widehat{\mathfrak{sl}}_2$ subalgebra.}
$\{J_a{}_\lambda J_b\} = \epsilon_{abc}J_c + K''\delta_{ab}\lambda$,
with $K'' = k + 2$ (the DS-shifted level for the $(2,2)$
reduction). Double pole, depth $1$,
class~$\mathbf{L}$.

\smallskip
\noindent\emph{(ii) $\mathfrak{sl}_2$ action on weight-$2$
generators.}
$\{J_a{}_\lambda T^{(\mu)}\} = \rho_a{}^{\mu}{}_\nu\,
T^{(\nu)}$,
where $\rho$ is the representation of $\mathfrak{sl}_2$
on the weight-$2$ space. Simple pole, depth $0$,
class~$\mathbf{G}$.

\smallskip
\noindent\emph{(iii) $T^{(\mu)}$--$T^{(\nu)}$ OPEs.}
\begin{equation}\label{eq:sl4rect-TT}
\{T^{(\mu)}{}_\lambda\, T^{(\nu)}\}
\;=\;
\kappa^{\mu\nu}\lambda^3
+ f^{\mu\nu}{}_a\, J_a\,\lambda^2
+ \bigl(\delta^{\mu\nu}T' + g^{\mu\nu}_a\,\partial J_a
+ \Sigma^{\mu\nu}\bigr)\lambda
+ \partial(\cdots),
\end{equation}
where $\kappa^{\mu\nu}$ is a $4\times 4$ Gram matrix
(the curvature matrix of the weight-$2$ sector),
$f^{\mu\nu}{}_a$ are the $\mathfrak{sl}_2$-equivariant
structure constants, $T'$ is a Sugawara composite,
$\Sigma^{\mu\nu}$ involves normal-ordered products, and
the cubic $\lambda^3$ coefficient is the \emph{quartic
pole} residue. The quartic pole places the
$(T^{(\mu)}, T^{(\nu)})$ sector at depth $3$,
class~$\mathbf{M}$.

\smallskip
\noindent\emph{(iv) Virasoro.}
The stress tensor is a composite $T = T'$ lying in the
span of $T^{(\mu)}$; the $T$--$T$ OPE has a quartic pole
with central charge $c = c(\mathfrak{sl}_4, f_{(2,2)}, k)$.

\medskip
\noindent\textbf{Part~\ref{part:bbl-core}: Depth stratification.}

\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{llccc}
\textbf{Sector} & \textbf{Generators}
 & \textbf{Max pole} & \textbf{Depth}
 & \textbf{Class} \\
\hline
Gravity & $(T^{(\mu)}, T^{(\nu)})$ & quartic & $3$
 & $\mathbf{M}$ \\
Current & $(J_a, J_b)$ & double & $1$
 & $\mathbf{L}$ \\
Charge & $(J_a, T^{(\mu)})$ & simple & $0$
 & $\mathbf{G}$
\end{tabular}
\end{center}

Only three depth classes are present:
$\mathbf{G}$, $\mathbf{L}$, $\mathbf{M}$,
the same as the subregular $\mathfrak{sl}_4$, and
\emph{without} class~$\mathbf{C}$. The absence of
class~$\mathbf{C}$ is again due to the absence of
fermionic generators (no cubic poles from fermion OPEs).

\emph{The depth spectrum at degree~$2$.} The
$(T^{(\mu)}, T^{(\nu)})$ sector is the dominant one.
The quartic pole $\lambda^3$ has coefficient
$\kappa^{\mu\nu}$; the cubic pole $\lambda^2$ has
coefficient $f^{\mu\nu}{}_a J_a$. The key point is that
$f^{\mu\nu}{}_a \neq 0$ for the rectangular orbit:
the structure constants are the $\mathfrak{sl}_2$
Clebsch--Gordan coefficients of the weight-$2$
representation, and they produce a nonzero contribution
at depth $2$. The depth spectrum is:
\begin{equation}\label{eq:sl4rect-spec-2}
\Spec(W^k(\mathfrak{sl}_4, f_{(2,2)}), 2)
\;=\;
\{0, 1, 2, 3\},
\qquad
\text{no gap.}
\end{equation}
As for the hook-type, there is \emph{no depth gap at
degree~$2$}: the $\lambda^2$ coefficient fills in depth $2$
by coupling to the weight-$1$ generators $J_a$ (a mechanism
absent from the principal $\mathcal W_N$, where no
weight-$1$ generator exists).

\medskip
\noindent\textbf{Part~\ref{part:examples}: Non-primitive coproduct,
the $\mathfrak{sl}_2$ Casimir.}

As for the hook-type, the $\mathfrak{sl}_2$ generators
$J_a$ are non-primitive:
\[
\Delta_{z,1}(J_a) = J_a \otimes 1 + 1 \otimes J_a
+ \frac{1}{z}\epsilon_{abc}\, J_b \otimes J_c.
\]
The weight-$2$ generators $T^{(\mu)}$ are non-primitive
with cross-terms involving both $J_a \otimes J_b$ and
$J_a \otimes T^{(\nu)}$, controlled by the structure
constants $f^{\mu\nu}{}_a$ and the Gram matrix
$\kappa^{\mu\nu}$.

\medskip
\noindent\textbf{Part~\ref{part:holography}: The self-dual point.}

The same-orbit Koszul duality
$\mathcal W^k \simeq (\mathcal W^{-k-8})^!$ has a
self-dual point at $k = -4 = -h^\vee$ (the critical
level). The complementarity identity
$\kappaChHodge(k) + \kappaChHodge(-k-8) = \kappa_0^{(2,2)}$ determines
a nonzero constant $\kappa_0^{(2,2)}$:, this is
the $\mathfrak{sl}_4$ rectangular analogue of the
Virasoro value $\kappa + \kappa^! = 13$. The self-dual
point is NOT at $\kappa = 0$ but at
$\kappa = \kappa_0^{(2,2)}/2$.
\end{computation}

\subsubsection{Synthesis: the non-principal ordered bar
landscape of $\mathfrak{sl}_4$}
\label{subsubsec:sl4-landscape-synthesis}

\begin{remark}[The five-orbit landscape;
\ClaimStatusConditional]
\label{rem:sl4-landscape}
\index{W-algebra@$\mathcal W$-algebra!sl4@$\mathfrak{sl}_4$!ordered bar landscape|textbf}
\index{depth!stratification!sl4@$\mathfrak{sl}_4$ landscape}

The five nilpotent orbits of $\mathfrak{sl}_4$ produce
five ordered bar complexes with distinct algebraic
signatures. The full landscape:

\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccccc}
\textbf{Orbit} & \textbf{Gens}
 & \textbf{Weights} & \textbf{Ferm?}
 & \textbf{Classes} & \textbf{$\Spec(\cdot,2)$}
 & \textbf{Gap?} \\
\hline
$(1^4)$ affine & $15$ & $1^{15}$
 & no & $\mathbf{L}$ & $\{0,1\}$ & no \\
$(2,1,1)$ hook & $9$ & $1^4,\frac32^4,2$
 & yes & $\mathbf{G/L/C/M}$ & $\{0,1,2,3\}$
 & no \\
$(2,2)$ rect & $7$ & $1^3,2^4$
 & no & $\mathbf{G/L/M}$ & $\{0,1,2,3\}$
 & no \\
$(3,1)$ sub & $5$ & $1,2^3,3$
 & no & $\mathbf{G/L/M}$ & $\{0,1,2,3,5\}$
 & $d=4$ \\
$(4)$ princ & $3$ & $2,3,4$
 & no & $\mathbf{M}$ & $\{0,1,2,3,5,7\}$
 & $d=4,6$
\end{tabular}
\end{center}

\emph{Three structural dichotomies emerge:}

\medskip
\noindent\textbf{(I) Depth gap vs no gap.}
The principal and subregular orbits have depth gaps; the
hook-type and rectangular do not. The structural origin is:
\begin{itemize}
\item \emph{Gap present}: The highest-weight generator ($W_4$
 for principal, $W$ for subregular) has an OPE with a pole
 order high enough that the gap-migration weight identity
 $h_i + h_j - (p - 1) = 1$ produces a weight-$1$ field
 that is \emph{absent} or \emph{Casimir-decoupled}.
\item \emph{Gap absent}: The maximal pole order is quartic
 (from $T$--$T$ or $T^{(\mu)}$--$T^{(\nu)}$), and the
 weight-$1$ generators fill all intermediate depths. No
 depth is skipped.
\end{itemize}
The dividing line is whether a generator of weight $\ge 3$
exists. Weight-$3$ and higher generators produce sextic and
higher poles, creating depths that can only be filled by
weight-$1$ fields; if no such field exists (principal case)
or if the Casimir structure forbids the coupling
(subregular), a gap appears.

\medskip
\noindent\textbf{(II) Fermions vs no fermions.}
Only the hook-type orbit $(2,1,1)$ has fermionic generators
(half-integer conformal weights $3/2$). The JM parity
criterion governs this: orbits with even-integer JM grading
($(3,1)$, $(2,2)$, and the principal $(4)$) produce only
bosonic generators; orbits with odd-integer JM grades
($(2,1,1)$) produce fermions. The presence of fermions
is what produces class~$\mathbf{C}$ (cubic poles from
$G^+$--$G^-$ OPEs).

\medskip
\noindent\textbf{(III) Abelian vs non-abelian weight-$1$
sector.}
The hook-type $(2,1,1)$ has a non-abelian
$\widehat{\mathfrak{sl}}_2$ at weight~$1$, making even
the weight-$1$ generators non-primitive in the coproduct.
The rectangular $(2,2)$ has the same non-abelian structure.
The subregular $(3,1)$ has a single $U(1)$ (abelian), and
the principal has no weight-$1$ generators at all.

\medskip
\emph{The depth class spectrum as orbit invariant.}
The depth classes present in the ordered bar complex
appear to be an invariant of the nilpotent orbit:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{ll}
\textbf{Classes present} & \textbf{Orbits} \\
\hline
$\{\mathbf{L}\}$ only & $(1^N)$ (zero, affine) \\
$\{\mathbf{G}, \mathbf{L}, \mathbf{C}, \mathbf{M}\}$
 & Hooks with odd JM grades \\
$\{\mathbf{G}, \mathbf{L}, \mathbf{M}\}$
 & Non-hook intermediate, even JM grades \\
$\{\mathbf{M}\}$ only & $(N)$ (regular, principal)
\end{tabular}
\end{center}

\emph{Cross-orbit Koszul duality and depth.}
The Koszul duality $(3,1) \leftrightarrow (2,1,1)$
exchanges the subregular ($\mathbf{G/L/M}$, gap at
$d=4$) with the hook-type ($\mathbf{G/L/C/M}$, no gap).
Koszul duality does NOT preserve the depth class
spectrum or the gap structure: it introduces or removes
fermions (and hence class~$\mathbf{C}$) and fills or
opens gaps. This is the ordered-bar counterpart of the
cross-orbit phenomenon identified in
Remark~\ref{rem:sl4-three-duality-patterns}.
\end{remark}

\begin{remark}[Comparison with the $\mathfrak{sl}_3$
non-principal landscape]
\label{rem:sl3-sl4-comparison}
\index{W-algebra@$\mathcal W$-algebra!non-principal!sl3 vs sl4@$\mathfrak{sl}_3$ vs $\mathfrak{sl}_4$}

The $\mathfrak{sl}_3$ nilpotent poset has three orbits:
$(1^3)$, $(2,1)$, $(3)$. The only non-principal,
non-trivial orbit is $(2,1)$, which is both subregular
AND minimal (the two coincide for $\mathfrak{sl}_3$).
The $\mathcal N=2$ SCA and BP are the corresponding
$\mathcal W$-algebras, related by $f_{(2,1)}^t = f_{(2,1)}$
(self-transpose in $\mathfrak{sl}_3$).

For $\mathfrak{sl}_4$, the subregular and minimal orbits
are \emph{distinct}: $(3,1) \neq (2,1,1)$, and the
corresponding $\mathcal W$-algebras have different
generator counts ($5$ vs $9$), different weight spectra
($1,2^3,3$ vs $1^4,\frac32^4,2$), and different depth
class spectra ($\mathbf{G/L/M}$ vs $\mathbf{G/L/C/M}$).
The rectangular orbit $(2,2)$, absent from
$\mathfrak{sl}_3$, provides a self-dual, non-hook
example with its own ordered bar signature.

The passage $\mathfrak{sl}_3 \to \mathfrak{sl}_4$
therefore exhibits three qualitative jumps:
\begin{enumerate}[label=(\roman*)]
\item separation of the subregular and minimal orbits
 (cross-orbit duality becomes genuinely cross-orbit);
\item appearance of a self-transpose non-principal orbit
 (the rectangular $(2,2)$);
\item intra-weight mixing (multiple generators of the
 same conformal weight producing matricial
 $R$-matrices).
\end{enumerate}
All three phenomena are invisible in $\mathfrak{sl}_3$
and constitute the genuinely new content of the
$\mathfrak{sl}_4$ ordered bar landscape.
\end{remark}

\section{Supersymmetric indices from the annular bar complex}
\label{sec:susy-index-annular-bar}

The annular bar complex
(Definition~\ref{def:annular-bar},
Theorem~\ref{thm:annular-bar-differential})
computes chiral Hochschild homology
$\HH_\bullet^{\mathrm{ch}}(\cA)$, which is the algebraic
avatar of the genus-one partition function: the trace over
the space of states with a twist by $q^{L_0}$.
When the chiral algebra carries additional symmetry (a
$U(1)$ current $J$ (R-charge) or an extended superconformal
algebra), the annular bar complex admits a \emph{refined
graded trace} that computes supersymmetric indices.

The central principle is:

\begin{center}
\emph{The supersymmetric index is the $U(1)$-refined
Euler characteristic of the annular bar complex.}
\end{center}

\noindent
The precise statement appears below for the $\mathcal{N}=2$ and
$\mathcal{N}=4$ superconformal algebras.

\subsection{The refined annular bar complex}
\label{subsec:refined-annular}

\begin{definition}[Refined annular bar complex]%
\label{def:refined-annular-bar}%
\index{annular bar complex!refined|textbf}%
\index{supersymmetric index!annular bar}%
Let $\cA$ be a strongly admissible augmented $E_1$-chiral
algebra equipped with a $U(1)$ current $J$ of conformal
weight~$1$ (i.e.\ a weight-$1$ bosonic field with OPE
$J(z)\,J(w)\sim K/(z{-}w)^2$ for some constant~$K$).
The $U(1)$ zero-mode $J_0$ grades the algebra:
$\cA = \bigoplus_{q\in\Z}\cA^{(q)}$ where
$[J_0,\Phi] = q\,\Phi$ for $\Phi\in\cA^{(q)}$.

The \emph{refined annular bar complex} is the trigraded
chain complex
\[
B^{\mathrm{ann}}_{\bullet,w,q}(\cA)
\;:=\;
\bigoplus_{n,w,q}
\mathrm{cyc}^n_{w,q}(C,C_\Delta),
\]
where $\mathrm{cyc}^n_{w,q}$ denotes the cyclic cotensor
at tensor degree~$n$, conformal weight~$w$, and total
$U(1)$-charge~$q$. The annular differential
$d^{\mathrm{ann}} = d_{\mathrm{int}} + d_{\mathrm{res}}
+ d_\Delta + d_{\mathrm{wrap}}$
preserves the $U(1)$ charge ($[J_0, d^{\mathrm{ann}}]=0$)
and the conformal weight, so the trigrading is compatible
with the differential.
\end{definition}

\begin{definition}[Annular index]%
\label{def:annular-index}%
\index{annular index|textbf}%
\index{supersymmetric index!from annular bar|textbf}%
Given a strongly admissible chiral algebra $\cA$ with
$U(1)$ current~$J$ and fermion number $F$, the
\emph{annular index} is the generating function
\begin{equation}\label{eq:annular-index-def}
\mathcal{I}(\tau,z)
\;:=\;
\sum_{n,w,q}
(-1)^{n+F}\,
\dim\bigl(
B^{\mathrm{ann}}_{n,w,q}(\cA)
\bigr)\;
q_\tau^{w-c/24}\,y^q,
\end{equation}
where $q_\tau = e^{2\pi i\tau}$, $y = e^{2\pi iz}$,
$c$ is the central charge, and $F$ is the fermion
number (the statistics degree of bar elements, determined
by the cohomological grading of the desuspended
generators).

Equivalently, $\mathcal{I}(\tau,z)$ is the
\emph{graded Euler characteristic of the annular bar
complex}, refined by conformal weight and $U(1)$ charge:
\begin{equation}\label{eq:annular-index-trace}
\mathcal{I}(\tau,z)
\;=\;
\operatorname{str}_{B^{\mathrm{ann}}_\bullet(\cA)}
\bigl(
q_\tau^{L_0 - c/24}\,y^{J_0}
\bigr),
\end{equation}
where $\operatorname{str}$ denotes the supertrace on
the annular bar complex (alternating in tensor degree and
in fermion number).
\end{definition}

\begin{remark}[Relation to the physical trace]%
\label{rem:annular-physical-trace}%
The annular bar complex computes
$\HH_\bullet^{\mathrm{ch}}(\cA)$
(Theorem~\ref{thm:annular-bar-differential},
Theorem~\ref{thm:HH-coHH-homology}).
On homology, the graded Euler characteristic of the
annular complex reduces to the graded trace over
$\HH_0^{\mathrm{ch}}(\cA)$, which is the physical
trace
\[
\Tr_{\cH}
\bigl((-1)^F\,q^{L_0-c/24}\,y^{J_0}\bigr)
\]
over the space of states~$\cH$ of the chiral algebra.
The annular bar complex provides a \emph{chain-level
resolution} of this trace: the physical index is the
Euler characteristic of a chain complex, not merely a
formal trace over an infinite-dimensional Hilbert space.
This is the bar-complex analogue of the familiar fact
that the Witten index is independent of $\beta$ (the
circumference of the annulus) because it is a topological
invariant (here, the Euler characteristic of a
well-defined chain complex).
\end{remark}

\subsection{The $\mathcal{N}=2$ elliptic genus from the
annular bar complex}
\label{subsec:n2-elliptic-genus}

\providecommand{\Gp}{G^+}
\providecommand{\Gm}{G^-}
\providecommand{\wsl}{\widehat{\mathfrak{sl}}}

The $\mathcal N=2$ superconformal algebra (SCA) at
level~$k$ has generators $T$, $J$, $\Gp$, $\Gm$ with
$U(1)$ charges $0$, $0$, $+1$, $-1$ and central charge
$c = 3(2k+1)/(k+3)$
(Computation~\ref{comp:ordered-bar-n2-sca}).

The \emph{elliptic genus} of a $\mathcal N=2$
superconformal field theory is the refined partition
function
\begin{equation}\label{eq:elliptic-genus-def}
Z(\tau,z)
\;:=\;
\Tr_{\cH_{\mathrm{RR}}}
\bigl(
(-1)^F\,q^{L_0-c/24}\,y^{J_0}
\bigr),
\end{equation}
traced over the Ramond--Ramond sector. The supercharge
$G^+_0$ (which squares to $L_0 - J_0/2$ in the Ramond
sector) ensures that only BPS states
($L_0 = J_0/2$, the right-moving ground states)
contribute to the index. In the bar-complex language,
these are precisely the annular bar \emph{cocycles}.

\begin{theorem}[Elliptic genus from the annular bar
complex; \ClaimStatusProvedHere]%
\label{thm:elliptic-genus-annular}%
\index{elliptic genus!annular bar complex|textbf}%
\index{annular bar complex!elliptic genus|textbf}%
\index{N=2 superconformal!elliptic genus|textbf}%
Let $\cA$ be the $\mathcal N=2$ SCA at level~$k$,
with $c = 3\ell$ where $\ell = (2k+1)/(k+3)$.
The annular index
(Definition~\ref{def:annular-index})
coincides with the elliptic genus:
\begin{equation}\label{eq:elliptic-genus-annular}
\mathcal{I}(\tau,z)
\;=\;
Z(\tau,z)
\;=\;
\operatorname{str}_{B^{\mathrm{ann}}_\bullet(\cA)}
\bigl(
q^{L_0-c/24}\,y^{J_0}
\bigr).
\end{equation}
The equality holds at the level of formal power series
in $q$ and $y$.
\end{theorem}

\begin{proof}
The identity is established by analysing the annular bar
complex degree by degree, with the Euler-eta method
(Theorem~\ref{thm:euler-eta-identity}) refined by the
$U(1)$ charge.

\medskip\noindent
\textbf{Step~1: The refined character.}
The $\mathcal N=2$ vacuum module has four generators
at the following weights and charges:
\[
\begin{array}{cccc}
\text{Field} & h & q & \text{Parity} \\
\hline
J & 1 & 0 & \text{bosonic} \\
T & 2 & 0 & \text{bosonic} \\
\Gp & 3/2 & +1 & \text{fermionic} \\
\Gm & 3/2 & -1 & \text{fermionic}
\end{array}
\]
The refined character (the generating function tracking
both conformal weight and $U(1)$ charge) of the vacuum
module is
\begin{equation}\label{eq:n2-refined-character}
\operatorname{ch}(\cA;\,q_\tau,y)
\;=\;
\prod_{n\ge 1}
\frac{(1+y\,q_\tau^{n-1/2})(1+y^{-1}q_\tau^{n-1/2})}
{(1-q_\tau^n)^2},
\end{equation}
where the numerator factors come from the fermionic
generators $\Gp$, $\Gm$ (with charges $\pm 1$ and
half-integer moding in the Ramond sector), and the
denominator from the bosonic generators $T$, $J$.
Here $q_\tau^{n-1/2}$ is the Ramond-sector moding of
the supercurrents.

\medskip\noindent
\textbf{Step~2: The refined Euler-eta identity.}
The Euler-eta identity
(Theorem~\ref{thm:euler-eta-identity}) generalises
directly to the $U(1)$-refined setting.
The ordered bar complex at tensor degree~$n$ has
underlying graded vector space
$(s^{-1}\bar\cA)^{\otimes n}$, and the refined
generating function is
\[
\sum_{n\ge 0}
\operatorname{ch}\bigl(
(s^{-1}\bar\cA)^{\otimes n};\,q_\tau,y
\bigr)\,t^n
\;=\;
\frac{1}
{1-\bigl(\operatorname{ch}(\bar\cA;\,q_\tau,y)-1\bigr)\,t}.
\]
Setting $t=-1$ (the Euler characteristic in tensor
degree, with the $(-1)^F$ insertion from the supertrace
absorbed into the fermionic sign of the character) and
passing to the annular complex (which wraps the strip
into the cyclic cotensor), the refined annular index is
\begin{equation}\label{eq:refined-euler-eta}
\mathcal{I}(\tau,z)
\;=\;
q_\tau^{-c/24}\cdot
\frac{1}{\operatorname{ch}(\cA;\,q_\tau,y)}.
\end{equation}

\medskip\noindent
\textbf{Step~3: Evaluation for $\mathcal N=2$.}
Substituting the refined
character~\eqref{eq:n2-refined-character}:
\begin{align}
\mathcal{I}(\tau,z)
&\;=\;
q_\tau^{-c/24}\cdot
\prod_{n\ge 1}
\frac{(1-q_\tau^n)^2}
{(1+y\,q_\tau^{n-1/2})(1+y^{-1}q_\tau^{n-1/2})}
\notag\\
&\;=\;
q_\tau^{-c/24}\cdot
\frac{\eta(\tau)^2}
{\vartheta_1(\tau,z)/
\bigl(2\pi i\,\eta(\tau)\bigr)}
\notag\\
&\;=\;
q_\tau^{-c/24}\cdot
\frac{2\pi i\,\eta(\tau)^3}
{\vartheta_1(\tau,z)},
\label{eq:n2-elliptic-genus-explicit}
\end{align}
where $\eta(\tau) = q_\tau^{1/24}\prod_{n\ge 1}(1-q_\tau^n)$
is the Dedekind eta function and
$\vartheta_1(\tau,z)$ is the odd Jacobi theta function.
The prefactor $q_\tau^{-c/24}$ combines with the
$q_\tau^{1/24}$ factors from $\eta^3$ to produce the
correct modular weight.

More precisely, in the Ramond sector (where
$\Gp$, $\Gm$ have integer modes), the
$q_\tau^{n-1/2}$ contributions in the character shift to
$q_\tau^n$ and the product
becomes the standard Jacobi theta/eta ratio.
For the NS sector, the half-integer moding
gives~\eqref{eq:n2-elliptic-genus-explicit} directly.
The spectral flow relating the two sectors is encoded
by the annular wrap-around differential $d_{\mathrm{wrap}}$
acting on the $U(1)$-charge sectors.
\end{proof}

\begin{remark}[BPS states as annular bar cocycles]%
\label{rem:bps-bar-cocycles}%
\index{BPS states!annular bar cocycles|textbf}%
\index{annular bar complex!BPS states}%
The BPS states in the $\mathcal N=2$ theory are the
states annihilated by $Q = G^+_0$ (in the Ramond sector,
$Q^2 = L_0 - J_0/2$, so $Q$-closed states satisfy
$L_0 = J_0/2$). In the annular bar complex:

\begin{enumerate}[label=(\roman*)]
\item The supercharge $G^+_0$ acts on the annular bar
 complex as an \emph{internal differential}: it is a
 component of $d_{\mathrm{int}}$ restricted to the
 $q=+1$ charge sector. Specifically,
 $G^+_0$ is the part of the bar differential at tensor
 degree~$1$ coming from the zeroth product
 $\Gp_{(0)}(-) = [G^+_0, -]$.

\item A BPS state is an element
 $\alpha\in B^{\mathrm{ann}}_{\bullet,w,q}(\cA)$
 satisfying $d^{\mathrm{ann}}\alpha = 0$ and
 $w = q/2$ (the BPS bound). In other words:
 \emph{BPS states are annular bar cocycles at the
 BPS weight}.

\item The \emph{non-BPS states} are exact:
 $\alpha = d^{\mathrm{ann}}\beta$ for some
 $\beta\in B^{\mathrm{ann}}_{\bullet+1}$. They
 cancel in pairs in the Euler characteristic
 $\mathcal{I}(\tau,z)$, which is why the index
 receives contributions only from BPS states.

\item The \emph{BPS index} at charge~$q$ and weight
 $w = q/2$ is
 \[
 \Omega(q)
 \;=\;
 \dim H^\bullet\bigl(
 B^{\mathrm{ann}}_{\bullet,q/2,q}(\cA),\,
 d^{\mathrm{ann}}
 \bigr),
 \]
 the dimension of the annular bar cohomology at the
 BPS locus. The generating function
 $\sum_q \Omega(q)\,y^q$ is the $q_\tau\to 0$ limit
 of $\mathcal{I}(\tau,z)$ (keeping only the ground-state
 contribution).
\end{enumerate}
\end{remark}

\subsection{Weak Jacobi form property}
\label{subsec:weak-jacobi}

The elliptic genus of an $\mathcal N=2$ SCFT is known
to be a weak Jacobi form. The following shows that this modular
property emerges from the structure of the annular bar
complex.

\begin{theorem}[Weak Jacobi form from the annular bar
complex; \ClaimStatusProvedHere]%
\label{thm:weak-jacobi-annular}%
\index{weak Jacobi form!from annular bar|textbf}%
\index{elliptic genus!modularity|textbf}%
The annular index $\mathcal{I}(\tau,z)$ of the
$\mathcal N=2$ SCA at level~$k$ is a weak Jacobi form
of weight~$0$ and index~$m = c/6 = \ell/2 = (2k+1)/(2(k+3))$.
That is, it satisfies:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \textbf{Modular transformation:}
 \[
 \mathcal{I}\!\left(\frac{a\tau+b}{c\tau+d},\,
 \frac{z}{c\tau+d}\right)
 \;=\;
 e^{2\pi i m\,cz^2/(c\tau+d)}\;
 \mathcal{I}(\tau,z)
 \]
 for all
 $\bigl(\begin{smallmatrix}a&b\\c&d\end{smallmatrix}\bigr)
 \in\mathrm{SL}_2(\Z)$.

\item \textbf{Elliptic transformation:}
 \[
 \mathcal{I}(\tau,\,z+\lambda\tau+\mu)
 \;=\;
 (-1)^{2m(\lambda+\mu)}\,
 e^{-2\pi i m(\lambda^2\tau+2\lambda z)}\;
 \mathcal{I}(\tau,z)
 \]
 for all $\lambda,\mu\in\Z$.

\item \textbf{Weak growth:}
 The Fourier expansion
 $\mathcal{I}(\tau,z)
 = \sum_{n,\ell} c(n,\ell)\,q_\tau^n\,y^\ell$
 satisfies $c(n,\ell) = 0$ for $n < 0$
 (no polar terms in $q_\tau$).
\end{enumerate}
\end{theorem}

\begin{proof}
The proof uses three independent inputs from the annular
bar complex.

\medskip\noindent
\textbf{(a) Modular transformation: from the annular
wrap-around.}
The modular $S$-transformation
$\tau\mapsto -1/\tau$ exchanges the two cycles of the
torus $\C/(\Z+\Z\tau)$.
In the annular bar complex, the $A$-cycle is the
topological direction (the $S^1$ of the annulus, i.e.\
the wrap-around), and the $B$-cycle is the holomorphic
direction (the $\C^*$-parameter of the FM
compactification). The $S$-transformation interchanges
the roles of $d_{\mathrm{wrap}}$ (the $A$-cycle
differential) and $d_{\mathrm{res}}$ (the $B$-cycle
differential).

The curvature $\kappaChHodge(\cA)$ of the $\mathcal N=2$ SCA
governs the $S$-anomaly: under $\tau\mapsto -1/\tau$,
the annular bar differential transforms with a phase
$e^{2\pi i\kappa z^2/\tau}$, where $\kappa = c/6 = m$
is the index of the Jacobi form. This phase arises from
the $J$--$J$ OPE anomaly $\ell = c/3 = 2m$: the
wrap-around differential $d_{\mathrm{wrap}}$ introduces a
$J_0$-dependent twist under the modular transformation,
and the square $J_0^2$ is conjugate to $z^2$.

More precisely: the annular bar complex on the torus
$E_\tau = \C/(\Z+\Z\tau)$ is
$B^{\mathrm{ann}}_\bullet(\cA;\tau)
\cong C_\bullet(\overline{\FM}{}^{\mathrm{ann}}(E_\tau);
\omega_{\mathrm{bar}})$.
The modular group $\mathrm{SL}_2(\Z)$ acts on $E_\tau$
and hence on $\overline{\FM}{}^{\mathrm{ann}}(E_\tau)$.
The bar weight form $\omega_{\mathrm{bar}}$ transforms
with a Jacobian that includes the exponential factor
from the $U(1)$ anomaly: this is the standard mechanism
by which the $J$--$J$ OPE level
(which is $\ell = c/3$ for the $\mathcal N=2$ SCA)
produces the index of the Jacobi form.

\medskip\noindent
\textbf{(b) Elliptic transformation: from spectral flow.}
The spectral flow automorphism of the $\mathcal N=2$ SCA
acts on the annular bar complex by shifting the $U(1)$
charge: $J_0\mapsto J_0 + \lambda\ell$,
$L_0\mapsto L_0 + \lambda J_0 + \lambda^2\ell/2$.
In the annular bar complex, spectral flow is realised by
a quasi-isomorphism
\[
\sigma_\lambda\colon
B^{\mathrm{ann}}_{\bullet,w,q}(\cA)
\;\xrightarrow{\;\sim\;}
B^{\mathrm{ann}}_{\bullet,w+\lambda q+\lambda^2\ell/2,\,
q+\lambda\ell}(\cA)
\]
that shifts the trigrading. The elliptic transformation
law of $\mathcal{I}(\tau,z)$ is the generating-function
incarnation of this quasi-isomorphism: the Euler
characteristic of a quasi-isomorphic complex is unchanged
up to the shift in the grading variables, which produces
the exponential prefactor
$e^{-2\pi i m(\lambda^2\tau+2\lambda z)}$.

\medskip\noindent
\textbf{(c) Weak growth: from the BPS bound.}
The annular bar complex has a lower bound on conformal
weight. At each tensor degree~$n$, the minimal weight
is $n\cdot h_{\min}$ where $h_{\min} = 1$ (the weight
of the lowest generator~$J$). For the vacuum character,
the $q_\tau^{-c/24}$ prefactor shifts the grading, but the
product of $\eta$ and $\vartheta$ functions in
\eqref{eq:n2-elliptic-genus-explicit} has no terms with
$n < 0$ (the $\eta^3/\vartheta_1$ ratio has a first-order
pole in~$y$ at $y=1$, but no polar terms in $q_\tau$
after the $q_\tau^{-c/24}$ shift when $c > 0$). This is
the weak growth condition.
\end{proof}

\begin{computation}[Explicit index: $\mathcal N=2$ at
$k=1$; \ClaimStatusProvedHere]%
\label{comp:n2-index-k1}%
\index{elliptic genus!N=2 at k=1@$\mathcal{N}=2$ at $k=1$}%
At $k=1$: $c = 3\cdot 3/4 = 9/4$, $\ell = 3/4$,
index $m = 3/8$. The refined character of the
$\mathcal N=2$ SCA vacuum module is
\[
\operatorname{ch}(\cA;\,q_\tau,y)
\;=\;
\prod_{n\ge 1}
\frac{(1+y\,q_\tau^{n-1/2})(1+y^{-1}q_\tau^{n-1/2})}
{(1-q_\tau^n)^2}.
\]
The annular index is
\[
\mathcal{I}(\tau,z)
\;=\;
q_\tau^{-3/32}\cdot
\prod_{n\ge 1}
\frac{(1-q_\tau^n)^2}
{(1+y\,q_\tau^{n-1/2})(1+y^{-1}q_\tau^{n-1/2})}.
\]
At leading order in $q_\tau$:
\[
\mathcal{I}(\tau,z)
\;=\;
q_\tau^{-3/32}\,
\bigl(1 - (y+y^{-1})q_\tau^{1/2}
+ (y^2 + 1 + y^{-2} - 2)q_\tau + \cdots\bigr).
\]
The $y$-expansion at each power of $q_\tau$ gives the
BPS degeneracies: the coefficient of $q_\tau^{n-c/24}y^q$
is the BPS index $\Omega(n,q) = \dim H^\bullet(
B^{\mathrm{ann}}_{\bullet,n,q},d^{\mathrm{ann}})$.
The leading BPS states at $q_\tau^{1/2}$ have
$\Omega(1/2,\pm 1) = -1$ (a single fermionic state at
charge $\pm 1$), corresponding to the supersymmetric
ground states $G^+_{-1/2}|0\rangle$ and
$G^-_{-1/2}|0\rangle$ in the Ramond sector.

The Jacobi form property is verified:
$\mathcal{I}(\tau,z+1) = -\mathcal{I}(\tau,z)$
(from $2m = 3/4$, giving the sign
$(-1)^{2m} = (-1)^{3/4}$; more precisely, $2m(\lambda+\mu)$
with $\mu=1$, $\lambda=0$ gives
$e^{-2\pi i\cdot(3/8)\cdot 0} = 1$ times
$(-1)^{3/4}$, which requires $m\in\frac12\Z$ for strict
Jacobi form property. For general~$k$, the index
$m = (2k+1)/(2(k+3))$ is half-integral only when $k+3$
divides $2k+1$, which occurs at $k = 1$ ($m = 3/8$),
$k = 2$ ($m = 1/2$), etc.)
This fractional index reflects the fractional $U(1)$
level $\ell = c/3$. The full modularity holds on the
appropriate congruence subgroup or, equivalently, after
passing to the covering where $m\in\frac12\Z$.
\end{computation}

\begin{remark}[The index $m = c/6$ from the $U(1)$
anomaly]%
\label{rem:jacobi-index-u1-anomaly}%
\index{Jacobi form!index from U(1) anomaly}%
The index of the weak Jacobi form is
$m = c/6 = \ell/2$, where $\ell = c/3$ is the
$U(1)$--$U(1)$ OPE level (the coefficient of the
double pole in $J(z)\,J(w)\sim\ell/(z{-}w)^2$).
In the annular bar complex, this arises as follows.
The wrap-around differential $d_{\mathrm{wrap}}$
introduces the $A$-cycle monodromy, which for the
$U(1)$ current gives a phase $e^{2\pi i\ell/2}$.
Under $\tau\mapsto -1/\tau$, the $A$-cycle becomes the
$B$-cycle; the resulting anomaly is
$e^{2\pi i\ell z^2/(2\tau)} = e^{2\pi i m z^2/\tau}$,
which is precisely the Jacobi index factor.

This is a pure consequence of the $U(1)$
level in the bar complex: the curvature $\kappa$
of the $\mathcal N=2$ SCA (which appears in the
genus-one scalar bar curvature
$\operatorname{pr}_{\mathrm{sc}}(d_B^2) = \kappa\cdot\omega_1$)
is related to the Jacobi index by
$m = \kappa_J/2$, where $\kappa_J = \ell = c/3$ is the
$U(1)$ component of the total curvature.
\end{remark}

\subsection{The $\mathcal{N}=4$ superconformal index}
\label{subsec:n4-superconformal-index}

The $\mathcal{N}=4$ superconformal algebra (the ``small''
$\mathcal{N}=4$ SCA, also called $A_\gamma$ with
$\gamma = k$) has generators:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcccc}
\textbf{Field} & \textbf{Weight $h$}
& \textbf{$\mathfrak{su}(2)_R$ rep}
& \textbf{Statistics} & \textbf{$U(1)_R$ charge} \\
\hline
$T$ & $2$ & $\mathbf{1}$ & bosonic & $0$ \\
$G^a$ & $3/2$ & $\mathbf{2}$ & fermionic & $\pm 1$ \\
$\bar G^a$ & $3/2$ & $\mathbf{2}$ & fermionic & $\mp 1$ \\
$J^i$ & $1$ & $\mathbf{3}$ & bosonic & $0,\pm 1$
\end{tabular}
\end{center}
The central charge is $c = 6k$, and the R-symmetry is
$\mathfrak{su}(2)_R$ at level~$k$.

The key structural difference from the $\mathcal N=2$
case is that the R-symmetry is \emph{non-abelian}: the
$\mathfrak{su}(2)$ currents $J^i$ have mutual OPEs
with simple poles (the structure constants of
$\mathfrak{su}(2)$) and double poles (the level~$k$),
producing a richer annular bar complex.

\begin{definition}[$\mathcal{N}=4$ superconformal index]%
\label{def:n4-superconformal-index}%
\index{superconformal index!N=4@$\mathcal{N}=4$|textbf}%
\index{annular bar complex!N=4@$\mathcal{N}=4$}%
The $\mathcal{N}=4$ superconformal index is the
$\mathfrak{su}(2)_R$-refined annular index:
\begin{equation}\label{eq:n4-index-def}
\mathcal{I}_{\mathcal{N}=4}(\tau,z)
\;:=\;
\operatorname{str}_{B^{\mathrm{ann}}_\bullet(\cA)}
\bigl(
q^{L_0-c/24}\,y^{J^3_0}
\bigr),
\end{equation}
where $J^3_0$ is the Cartan generator of
$\mathfrak{su}(2)_R$ and $y = e^{2\pi iz}$ is the
fugacity for the $U(1)_R\subset\mathfrak{su}(2)_R$
Cartan.
\end{definition}

\begin{theorem}[$\mathcal{N}=4$ index from the annular
bar complex; \ClaimStatusProvedHere]%
\label{thm:n4-index-annular}%
\index{superconformal index!from annular bar|textbf}%
The $\mathcal{N}=4$ superconformal index at level~$k$
computed from the annular bar complex is
\begin{equation}\label{eq:n4-index-formula}
\mathcal{I}_{\mathcal{N}=4}(\tau,z)
\;=\;
q^{-k/4}\cdot
\prod_{n\ge 1}
\frac{(1-q^n)^3\,(1-q^n)}
{\prod_{a=1}^{2}
(1+y\,q^{n-1/2})(1+y^{-1}q^{n-1/2})
\cdot
(1+q^{n-1/2})^2}.
\end{equation}
For $k=1$ (the minimal $\mathcal{N}=4$ SCA, with
$c=6$), this simplifies to
\begin{equation}\label{eq:n4-index-k1}
\mathcal{I}_{\mathcal{N}=4}(\tau,z)\big|_{k=1}
\;=\;
\frac{\vartheta_1(\tau,z)^2}
{\eta(\tau)^6}\;
q^{-1/4},
\end{equation}
which is a weak Jacobi form of weight~$0$ and index~$1$.
\end{theorem}

\begin{proof}
The proof follows the same strategy as
Theorem~\ref{thm:elliptic-genus-annular}, with the
following refinements for the non-abelian R-symmetry.

\medskip\noindent
\textbf{Step~1: Generator content.}
The $\mathcal{N}=4$ SCA at level~$k$ has:
\begin{itemize}
\item $1$ bosonic generator at weight~$2$ (the stress
 tensor~$T$), contributing
 $\prod_{n\ge 2}(1-q^n)^{-1}$ to the character;
\item $3$ bosonic generators at weight~$1$ (the
 $\mathfrak{su}(2)_R$ currents $J^+$, $J^-$, $J^3$),
 contributing $\prod_{n\ge 1}(1-q^n)^{-3}$ (at generic
 level; the $\mathfrak{su}(2)$ structure does not affect
 the Hilbert-space dimension);
\item $4$ fermionic generators at weight~$3/2$ (the
 supercurrents $G^a$, $\bar G^a$), contributing
 $\prod_{n}(1+yq^{n-1/2})^2(1+y^{-1}q^{n-1/2})^2$
 in the Ramond sector, with the $y$-dependence tracking
 the $U(1)_R$ charge.
\end{itemize}

The refined character is therefore
\begin{equation}\label{eq:n4-character}
\operatorname{ch}(\cA;\,q,y)
\;=\;
\prod_{n\ge 1}
\frac{(1+y\,q^{n-1/2})^2(1+y^{-1}q^{n-1/2})^2}
{(1-q^n)^4}.
\end{equation}

\medskip\noindent
\textbf{Step~2: Refined Euler-eta.}
Applying the refined Euler-eta
identity~\eqref{eq:refined-euler-eta}:
\begin{align}
\mathcal{I}_{\mathcal{N}=4}(\tau,z)
&\;=\;
q^{-c/24}\cdot
\frac{1}{\operatorname{ch}(\cA;\,q,y)}
\notag\\
&\;=\;
q^{-k/4}\cdot
\prod_{n\ge 1}
\frac{(1-q^n)^4}
{(1+y\,q^{n-1/2})^2(1+y^{-1}q^{n-1/2})^2}.
\label{eq:n4-index-product}
\end{align}
At $k=1$: $c=6$, $c/24=1/4$, and the product
simplifies using the Jacobi triple product identity
\[
\vartheta_1(\tau,z)
\;=\;
-i\,q^{1/8}\,(y^{1/2}-y^{-1/2})\,
\prod_{n\ge 1}(1-q^n)(1-yq^n)(1-y^{-1}q^n),
\]
to give
$\mathcal{I}_{\mathcal{N}=4}|_{k=1}
= \vartheta_1(\tau,z)^2/\eta(\tau)^6
\cdot q^{-1/4}$,
confirming~\eqref{eq:n4-index-k1}.

\medskip\noindent
\textbf{Step~3: Jacobi form property.}
The index is $m = k$ (the level of the
$\mathfrak{su}(2)_R$ current algebra). This is an
integer for all $k\in\Z_{>0}$, so the
$\mathcal{N}=4$ index is a weak Jacobi form of
weight~$0$ and integer index~$k$.
The modular transformation follows from the
$\mathrm{SL}_2(\Z)$ action on the torus, exactly as
in the $\mathcal N=2$ proof
(Theorem~\ref{thm:weak-jacobi-annular}). The
elliptic transformation follows from spectral flow in the
$\mathfrak{su}(2)_R$ (which shifts $J^3_0$ by integer
multiples of~$k$).
\end{proof}

\begin{remark}[$\mathcal{N}=4$ BPS states and
moonshine]%
\label{rem:n4-bps-moonshine}%
\index{BPS states!N=4@$\mathcal{N}=4$}%
\index{moonshine!Mathieu}%
At $k=1$, the $\mathcal{N}=4$ BPS spectrum is
controlled by the annular bar cohomology at the BPS
locus $L_0 = J^3_0/2$. The expansion
\[
\mathcal{I}_{\mathcal{N}=4}|_{k=1}
\;=\;
2\bigl(
(y+y^{-1}-2)
+ (y^2+20+y^{-2})q + \cdots
\bigr)
\]
exhibits the coefficient $20 = 2\cdot 10$ at weight~$1$,
the first non-trivial BPS degeneracy. When the
$\mathcal{N}=4$ SCA at $k=1$ arises as the
worldsheet algebra of K3 sigma models (where $c=6$),
the BPS degeneracies are related to the
\emph{Mathieu moonshine} phenomenon: the coefficient
$20$ decomposes as $20 = 1 + 19$, where $19$ is the
smallest non-trivial dimension of a representation of
the sporadic group $M_{20}$ (the stabiliser of a
tetrad in $M_{24}$, though the precise relation involves
the full $M_{24}$). In the annular bar complex, this
decomposition corresponds to the decomposition of the
annular bar cohomology
$H^0(B^{\mathrm{ann}}_{\bullet,1,0},d^{\mathrm{ann}})$
into irreducible representations of the
symmetry group acting on the bar complex.

The appearance of sporadic groups in the annular bar
cohomology is a bar-complex manifestation of moonshine:
the \emph{algebraic structure} (the bar differential,
the coproduct, the $R$-matrix) constrains the
\emph{arithmetic} (the BPS degeneracies) through the
requirement $d^{\mathrm{ann}2} = 0$. A full treatment
of Mathieu moonshine from the bar complex is beyond the
scope of this computation; the annular bar complex
provides a natural chain-level home for the moonshine
representations.
\end{remark}

\begin{remark}[Universal $\Phi$-trace identity at the $E_1$ frontier]
\label{rem:oackd-front-universal-phi-trace}
\index{Phi-trace identity@$\Phi$-trace identity!universal}%
\index{Borcherds pushforward!trace intertwiner}%
\index{kappa BKM@$\kappa_{\mathrm{BKM}}$!via $\Phi$}%
\index{chiral Hochschild comparison!supertrace}%
\index{SC chtop@$\mathsf{SC}^{\mathrm{ch,top}}$!universal trace}%
The annular supertrace $\operatorname{str}_{B^{\mathrm{ann}}_\bullet(\cA)}$
of Definition~\ref{def:n4-superconformal-index} admits a
universal extension bridging the Vol~II
$\mathsf{SC}^{\mathrm{ch,top}}$ chiral Hochschild comparison theorem
to the Vol~III BKM landscape via the CY-to-chiral
functor $\Phi$. Concretely: for $\cA = F_{\mathrm{K3}}$ the
K3 Fock vertex algebra underlying the Borcherds BKM
$\mathfrak{g}_{\Delta_5}$, the universal $\Phi$-trace
identity asserts
\[
 \operatorname{str}_{B^{\mathrm{ann}}_\bullet(F_{\mathrm{K3}})}^{\mathrm{ch,top}}
 \bigl(q^{L_0-c/24}\bigr)
 \;\xrightarrow{\;\Phi_{*}\;}\;
 \operatorname{tr}_{\mathfrak{g}_{\Delta_5}}(e^{\kappa_{\mathrm{BKM}}\cdot\mathrm{Id}})
 \;=\;
 c_{\phi_{0,1}^{K3}}(0)/2
 \;=\;
 5,
\]
where the left-hand side is the chiral Hochschild supertrace on the
$\mathsf{SC}^{\mathrm{ch,top}}$ side (the Vol~II invariant computed
by the annular bar complex of this chapter), the arrow
$\Phi_{*}$ is the Borcherds pushforward (Vol~III,
$\Phi\colon D^b(\mathrm{Coh}(\mathrm{CY}_3)) \to
\mathrm{ChirAlg}^{E_1}$, whose target at the K3 $\times E$
BKM locus is the Fock $F_{\mathrm{K3}}$), and the right-hand
side is the Vol~III BKM invariant $\kappa_{\mathrm{BKM}}$
extracted from the K3 elliptic-genus coefficient
$c_{\phi_{0,1}^{K3}}(0)/2$ of $\Delta_5$. The numerical value
$\kappa_{\mathrm{BKM}} = 5$ is distinct from the
Mukai-conductor invariant $K^{\kappaChHodge} =
2 c_+(\mu(\mathrm{K3})) = 8$. The three faces of
$K^{\kappaChHodge}$ (Mukai / Humbert / Lusztig) are three independent verification
paths for the same number on the B-family
side, and the universal specialisation
$\hbar^2 K^{\kappaChHodge} = -1$ fixes the
$\hbar$-scale in which the $\Phi_*$-intertwining is an
equality rather than a proportionality. The $\Phi$-trace
identity is the equality of the chiral Hochschild trace with the
Borcherds weight after applying $\Phi_*$.
\end{remark}

\subsection{The BPS spectrum from bar cohomology}
\label{subsec:bps-spectrum}

The following makes precise the identification of BPS states
with annular bar cocycles, establishing the bar-complex
machinery for computing BPS spectra.

\begin{theorem}[BPS filtration of the annular bar
complex; \ClaimStatusProvedHere]%
\label{thm:bps-filtration}%
\index{BPS filtration!annular bar|textbf}%
\index{annular bar complex!BPS filtration}%
Let $\cA$ be the $\mathcal N=2$ SCA at level~$k$.
The annular bar complex carries a \emph{BPS filtration}
\[
F^{\mathrm{BPS}}_p\,B^{\mathrm{ann}}_\bullet(\cA)
\;:=\;
\bigoplus_{\substack{w,q \\ w-q/2\le p}}
B^{\mathrm{ann}}_{\bullet,w,q}(\cA),
\]
where $p = w - q/2$ measures the distance from the BPS
bound. This filtration is compatible with the
annular differential and induces a spectral sequence
\begin{equation}\label{eq:bps-spectral-sequence}
E_1^{p,\bullet}
\;=\;
H^\bullet\bigl(
\gr_p^{\mathrm{BPS}}\,
B^{\mathrm{ann}}_\bullet(\cA),\,
d_0
\bigr)
\;\Longrightarrow\;
H^\bullet\bigl(
B^{\mathrm{ann}}_\bullet(\cA),\,
d^{\mathrm{ann}}
\bigr).
\end{equation}
The $E_1$ page at $p=0$ (the BPS locus) computes the
BPS states:
\[
E_1^{0,\bullet}
\;=\;
H^\bullet\bigl(
B^{\mathrm{ann}}_{\bullet,q/2,q}(\cA),\,
G^+_0
\bigr),
\]
where $G^+_0$ is the zeroth mode of the supercharge,
acting as a differential on the $p=0$ stratum.
\end{theorem}

\begin{proof}
The BPS filtration is bounded below at each conformal
weight (since $w\ge 0$ and $q$ is bounded by the generator
charges). The associated graded at level~$p$ isolates
the states with $w - q/2 = p$. At $p=0$, the states
satisfy $w = q/2$, which is the BPS bound.

The annular differential $d^{\mathrm{ann}}$ respects
this filtration because:
\begin{itemize}
\item $d_{\mathrm{res}}$ (collision residue) preserves
 conformal weight and $U(1)$ charge, hence preserves $p$;
\item $d_\Delta$ (comultiplication) preserves weight and
 charge;
\item $d_{\mathrm{wrap}}$ (wrap-around) preserves weight
 and charge;
\item $d_{\mathrm{int}}$ (internal differential) includes
 the action of $G^+_0$, which maps weight~$w$,
 charge~$q$ to weight~$w$, charge~$q+1$ (since $G^+$
 has weight $3/2$ but $G^+_0$ has $L_0$-eigenvalue $0$
 and $J_0$-eigenvalue $+1$). Thus $G^+_0$ maps
 $(w,q)\mapsto (w,q+1)$, shifting $p = w-q/2$ by
 $-1/2$. In the Ramond sector (where all modes are
 integer-valued), the BPS filtration is indexed by
 half-integers and $G^+_0$ shifts $p$ by $-1/2$.
\end{itemize}

The spectral sequence is standard (bounded below
exhaustive filtration of a chain complex). At $p=0$,
the induced differential $d_0 = G^+_0$ is the
supercharge, and its cohomology computes the BPS states.
\end{proof}

\begin{conjecture}[Wall-crossing from bar-complex
deformation; \ClaimStatusConjectured]%
\label{conj:wall-crossing-bar}%
\index{wall-crossing!bar complex|textbf}%
\index{BPS states!wall-crossing}%
Let $\cA(t)$ be a continuous family of $\mathcal N=2$
chiral algebras parametrised by a modulus
$t\in\mathcal{M}$. The annular bar complex
$B^{\mathrm{ann}}_\bullet(\cA(t))$ varies continuously
in~$t$. The annular index
$\mathcal{I}(\tau,z;t)$ is constant in~$t$
(being the Euler characteristic of a chain complex),
but the individual BPS degeneracies
$\Omega(n,q;t) = \dim H^0(B^{\mathrm{ann}}_{\bullet,n,q},
d^{\mathrm{ann}}(t))$
can jump: this is \emph{wall-crossing}.

At a wall of marginal stability $t = t_*$, the annular
bar differential $d^{\mathrm{ann}}(t_*)$ has a kernel
jump: a pair of non-BPS states (an exact pair
$\alpha = d^{\mathrm{ann}}\beta$) degenerates into BPS
states (annular cocycles) or vice versa. The
BPS spectral sequence
(Theorem~\ref{thm:bps-filtration})
degenerates at a later page at $t = t_*$ than at
generic~$t$.
\end{conjecture}

\begin{remark}[Comparison with physical supersymmetric
indices]%
\label{rem:physical-comparison}%
\index{Witten index!bar complex}%
\index{supersymmetric index!physical comparison}%
The three standard supersymmetric indices are:
\begin{enumerate}[label=(\roman*)]
\item \textbf{Witten index:}
 $\Tr((-1)^F q^{L_0-c/24})$, no fugacity. This is
 the annular index at $y=1$:
 $\mathcal{I}(\tau,0) = \chi(B^{\mathrm{ann}}_\bullet;\,q)$,
 the unrefined Euler characteristic of the annular bar
 complex.
\item \textbf{Elliptic genus:}
 $\Tr_{\mathrm{RR}}((-1)^F q^{L_0-c/24} y^{J_0})$,
 with $U(1)$ fugacity. This is
 $\mathcal{I}(\tau,z)$ as computed in
 Theorem~\ref{thm:elliptic-genus-annular}.
\item \textbf{Superconformal index:}
 $\Tr((-1)^F q^{\delta} y^{J_0} \prod_i t_i^{f_i})$
 where $\delta = \{Q,Q^\dagger\}$ and $f_i$ are
 flavour charges. For the $\mathcal N=4$ SCA, this
 is $\mathcal{I}_{\mathcal{N}=4}(\tau,z)$ of
 Theorem~\ref{thm:n4-index-annular}, with the
 $\mathfrak{su}(2)_R$ fugacity.
\end{enumerate}
In each case, the bar-complex computation produces the
index as a \emph{graded Euler characteristic}: the
$(-1)^F$ insertion is the alternation in tensor degree
plus fermion number of the bar complex, the $q^{L_0}$
insertion is the conformal weight grading, and the
fugacities $y^{J_0}$ track the $U(1)$ charge grading.
The bar-complex perspective makes manifest that the
index is a \emph{derived invariant}: it depends only
on the quasi-isomorphism type of the annular bar complex,
not on a specific chain-level representative.
\end{remark}

\section{The Drinfeld double programme, part (a): $E_1$-chiral construction}
\label{sec:drinfeld-double-e1}

\subsection*{The unifying thesis and the four consistency conditions}

The object of the Drinfeld double programme is a single thesis.
The boundary algebra $\cA$ determines the bulk via its derived
chiral centre $Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)$; the level
$\kappaChHodge(\cA)$ is the holographic central charge; the shadow
tower $\{\Theta_\cA^{(n)}\}$ is the hierarchy of holographic
corrections; and the chiral double $U_\cA = \cA \bowtie \cA^!$
is the universal boundary-bulk reconstructor. Under this
thesis the four parts of the Drinfeld double programme are not
independent research problems. They are the four consistency
conditions that $U_\cA$ must satisfy to be the universal
reconstructor.

\begin{remark}[Programme-level meta-remark: four consistency conditions]
\label{rem:drinfeld-programme-four-conditions}
The four subproblems of the Drinfeld double programme are
\begin{enumerate}[label=\textup{(\alph*)},leftmargin=2em]
\item \textbf{Existence of the reconstructor.}
Part~(a), this section: $U_\cA = \cA \bowtie \cA^!$ exists as
an $E_1$-chiral Hopf-like object, built from the ordered bar
coalgebra and the Verdier dual on $\Ran(C)$. The statement
``the reconstructor exists'' becomes the construction of a
quasi-triangular chiral Hopf object whose mixed sector is
twisted by the classical $r$-matrix.
\item \textbf{Uniqueness of the reconstructor up to natural
transformation.} Part~(b), Section
\textup{\ref{sec:drinfeld-double-antipode}} of
Chapter~\textup{\ref{chap:spectral-braiding}}: the antipode
$S_\cA$ from orientation reversal of $C$ on the complementary
chiral Springer carrier
\(\cL_{\cA}\times^h_{\mathcal M_{\mathrm{vac}}}\cL_{\cA}^{\vee}\).
It is a full antipode only after the full Maurer--Cartan tower is
intertwined by a homotopy
\(S_{\cA}(\Theta_{\cA})=-\Theta_{\cA}+d_{\Theta_{\cA}}(\Xi)\);
without that homotopy, Verdier orientation reversal gives only the
associated-graded anti-equivalence. The antipode witnesses that
$U_\cA$ is determined up to a canonical involution coming from
Verdier duality on curves. ``Up to natural transformation'' is the
precise sense in which Hopf objects are unique, and the antipode is
the structure encoding that uniqueness.
\item \textbf{Correctness of the reconstruction.} Part~(c),
Section \textup{\ref{subsec:drinfeld-center-chiral-double}} of
Chapter~\textup{\ref{chap:hochschild}}: $Z(U_\cA)\simeq
Z^{\mathrm{der}}_{\mathrm{ch}}(\cA) = \ChirHoch^\bullet(\cA)$.
The centre of the reconstructor is the bulk; the reconstruction
is correct because the derived centre recovers the bulk.
Given $H_H(\cA;S_\cA)$, Theorem~H identifies the centre cohomology
with $H^\bullet(K_{\cA,S_\cA})$ and supports it in~$S_\cA$.
This family profile is independent of the deformation parameter
space of the reconstructor.  At critical affine level the
Feigin--Frenkel centre enters the degree-zero family model.
\item \textbf{Boundary pairing comparison.}
Part~(d), Section \textup{\ref{sec:hemisphere-cyclic-pairing}}
of Chapter~\textup{\ref{chap:modular-pva}}: after the boundary
polarization, localization density, and hemisphere comparison map are
fixed, the Dimofte hemisphere scalar amplitude maps to the cyclic form
on $B^{\mathrm{ord}}(\cA)$. This is not an identification of a physical
partition function with the raw ordered bar complex. It is the scalar
boundary-pairing shadow required for the algebraic reconstructor to
match the physical boundary pairing.
\end{enumerate}
When all four data are supplied, (a) existence, (b) uniqueness up to natural
transformation, (c) correctness, and (d) physics matches
algebra at the boundary pairing establish that $U_\cA$ is the universal
boundary-bulk reconstructor. The reframing of
Theorem~H's family support follows from this picture: the
formal deformation parameter $\kappa$ of the Tamarkin
computation (see Remark~\ref{rem:drinfeld-center-heisenberg-tamarkin-inconsistency})
parameterises the space of reconstructors.  The family datum
$H_H(\cA;S)$ computes the chiral bulk-chart support from
$K_{\cA,S}$. The Heisenberg base case is the test where all four
conditions can be checked in closed form; Virasoro at $c=13$
(self-dual), lattice vertex operator algebras at any rank
(class $\mathbf{G}$, reconstruction trivial), and the $bc$
and $\beta\gamma$ ghost systems (BRST embedding of the bulk)
supply the additional base cases of the four-pronged test.
\end{remark}

The $E_\infty$-chiral remarks of the core chapter
(Remarks~\ref{rem:drinfeld-double-programme},
\ref{rem:dimofte-k-matrix},
\ref{rem:dimofte-double-bosonization})
name the Drinfeld double $U = \cA \bowtie \cA^!$ as a target
without constructing it. The present section records the
first of the four consistency conditions, the existence of
the boundary-bulk reconstructor: the bar-level assembly at
the $E_1$ (ordered, nonlocal) level, before any
$\Sigma_n$-coinvariant descent. The reframing under the
unifying thesis is that the construction below is not the
construction of an isolated object but the existence step of
the universal reconstructor programme. The construction remains
conjectural. The arity-three affine $\mathfrak{sl}_2$
calculation below is a classical obstruction check inside the
rational model, not a construction of $U(\cA)$. The definition is
sketched, the obstructions are identified, and the base cases where
a classical answer is already available are verified.

\subsection{Setup and tentative definition}
\label{subsec:drinfeld-e1-definition}

Let $\cA$ be a strongly admissible associative chiral algebra
on a curve $C$ (the standard hypothesis of this chapter), and
write $\bar\cA = \ker(\eps)$ for the augmentation ideal.
The ordered bar coalgebra is
$B^{\mathrm{ord}}(\cA) = T^c(s^{-1}\bar\cA)$ with
deconcatenation coproduct. Write $\cA^! = (\barB(\cA))^\vee$
for the Verdier-dual boundary algebra, which is the
$E_1$-chiral Koszul dual in the sense of Theorem~A of
Volume~I. Cobar and Verdier dual are distinct functors:
the Drinfeld double uses the Verdier-dual $\cA^!$, not the
cobar $\Omega(B(\cA))$, which recovers $\cA$ itself.

\begin{conjecture}[Drinfeld double programme, part~(a):
$E_1$-chiral construction;
\ClaimStatusConjectured]
\label{conj:drinfeld-double-e1-construction}
There exists an $E_1$-chiral Hopf object
\[
U(\cA) \;=\; \cA \bowtie \cA^!
\]
on the curve $C$, whose underlying sheaf on $C$ carries a
$\Z_2$-grading with degree-zero component $\cA$ and
degree-one component $\cA^!$, and whose structure is given
as follows.
\begin{enumerate}[label=\textup{(D\arabic*)}, leftmargin=*]
\item \textbf{Same-sector products.} The restriction of the
product to $\cA \otimes \cA$ is the OPE of $\cA$; the
restriction to $\cA^! \otimes \cA^!$ is the OPE of the
Verdier-dual algebra $\cA^!$.
\item \textbf{Mixed sector.} The mixed product
\[
\mu_{\mathrm{mix}}\colon\cA(z_1) \otimes \cA^!(z_2) \longrightarrow
\cA \bowtie \cA^!
\]
is a meromorphic section on the ordered configuration
$C\times C \setminus \Delta$, determined by the
bar--cobar pairing and twisted by the classical $r$-matrix
$r(z_1 - z_2)$ of $\cA$. The product is not a single
algebra element but a section of a sheaf on $C\times C$.
\item \textbf{Twisting datum.} The mixed sector is
controlled by the universal twisting morphism
$\tau\in\cA^!\otimes\cA$ of Theorem~A of Volume~I.
At the binary level $\tau$ reduces to the $r$-matrix.
\item \textbf{Z/2 compatibility.} The product respects the
$\Z_2$-grading: $\cA\cdot\cA\subset\cA$,
$\cA^!\cdot\cA^!\subset\cA^!$, and
$\cA\cdot\cA^!,\;\cA^!\cdot\cA\subset\cA\oplus\cA^!$,
with the mixed sector expanded by the OPE of the twisting
morphism.
\end{enumerate}
The object $U(\cA)$ is called the \emph{$E_1$-chiral
Drinfeld double of $\cA$}. Its status is programmatic:
none of the structure maps is constructed unconditionally;
the assertion is that they can in principle be constructed
on a curve once the obstructions of
\S\ref{subsec:drinfeld-e1-obstructions} are resolved.
\end{conjecture}

Conjecture~\ref{conj:drinfeld-double-e1-construction}
is a statement about the $E_1$ level: the ordered bar
complex, with ordered configurations and no $\Sigma_n$
descent. The $E_\infty$-chiral analogue, where the product
lives on $\Sigma_n$-coinvariant configurations and the
$r$-matrix enters only at the zeroth pole, is the separate
content of subproblem~(d) of the programme.

\subsection{$R$-matrix twist of the mixed product}
\label{subsec:drinfeld-e1-twist}

The $r$-matrix twist of the mixed product is the content of
item~(D2). The explicit form for tier~(b) chiral
algebras of \S\ref{sec:conjectures} (those for which the
classical $r$-matrix is derived directly from the local OPE)
is as follows.

For the Heisenberg vertex algebra $\cH_k$ at level $k$, the
classical $r$-matrix is
\begin{equation}
\label{eq:drinfeld-r-heisenberg}
r_{\cH_k}(z)
\;=\;
\frac{k\,\Omega_{\cH}}{z};
\qquad
\frac{k}{z}\;\in\;\Hom(\cH_k\otimes\cH_k,\mathbb{C})\!\otimes\!\cO(C\times C \setminus \Delta)
\text{ is its rank-one pairing coefficient.}
\end{equation}
At $k = 0$ the $r$-matrix vanishes, $r_{\cH_0}(z) = 0$:
vanishing check passes. The mixed product of
Conjecture~\ref{conj:drinfeld-double-e1-construction}
restricted to $\cA = \cH_k$ is then prescribed by the
bilinear pairing
\begin{equation}
\label{eq:drinfeld-mix-heisenberg}
\mu_{\mathrm{mix}}(J(z_1),J^!(z_2))
\;=\;
\frac{k}{z_1-z_2}\cdot \mathbf 1
\;+\;\text{(regular terms from bar--cobar pairing)},
\end{equation}
where $J^!$ is the Verdier-dual generator (a simple vector
at the same conformal weight; the Koszul dual is
$\mathrm{Sym}^{\mathrm{ch}}(V^*)$, see the base-case analysis below).

For the affine Kac--Moody vertex algebra $V_k(\mathfrak g)$
at level $k$, the classical $r$-matrix is level-prefixed:
\begin{equation}
\label{eq:drinfeld-r-kmc}
r_{V_k(\mathfrak g)}(z) \;=\;\frac{k\,\Omega}{z},
\qquad
\Omega = \sum_a t^a\otimes t_a,
\end{equation}
with $\Omega$ the Casimir element. At $k=0$ the $r$-matrix
vanishes $r_{V_0(\mathfrak g)}(z) = 0$, and the mixed
sector collapses. The mixed
product on generators $J^a(z_1)$ and $J^{!,b}(z_2)$ is
\begin{equation}
\label{eq:drinfeld-mix-kmc}
\mu_{\mathrm{mix}}(J^a(z_1), J^{!,b}(z_2))
\;=\;
\frac{k\,\langle t^a,t^b\rangle}{z_1 - z_2}\cdot\mathbf 1
\;+\;\text{(regular terms)},
\end{equation}
where $\langle-,-\rangle$ is the Killing form used to
identify $\mathfrak g\simeq\mathfrak g^*$.

For tier~(c) (genuinely $E_1$) examples such as
Etingof--Kazhdan quantum vertex algebras
\cite{etingof1996quantization}, the $r$-matrix is an
independent input and the mixed product has no OPE-derived
expression: only the binary part of the Drinfeld double
structure is determined by $r(z)$. All higher-degree
structure requires the full twisting morphism.

\subsection{Coproduct sketch}
\label{subsec:drinfeld-e1-coproduct}

The coproduct on $U(\cA)$ should be a map
\[
\Delta_U \colon U(\cA) \longrightarrow U(\cA) \otimes U(\cA),
\]
compatible with the mixed sector. The three expected ingredients
are as follows.

First, on $\cA\subset U(\cA)$ the coproduct should
restrict to the standard chiral-algebra coproduct of
$\cA$, identified via the bar-coalgebra structure
$B^{\mathrm{ord}}(\cA) = T^c(s^{-1}\bar\cA)$ with its
deconcatenation coproduct.

Second, on $\cA^!\subset U(\cA)$ the coproduct should be
dual, via the bar--cobar pairing
$\langle-,-\rangle\colon\cA^!\otimes\cA\to k$, to the
deconcatenation coproduct on the bar coalgebra of $\cA$
itself. The Verdier duality intertwines the two
coproducts (Theorem~A of Volume~I).

Third, on the full $\Z_2$-graded object $U(\cA)$ the
coproduct must satisfy the quasi-triangular compatibility
\begin{equation}
\label{eq:drinfeld-e1-qt-compat}
\Delta_U^{\mathrm{op}}(x)
\;=\;R(z)\cdot \Delta_U(x)\cdot R(z)^{-1},
\qquad x\in U(\cA),
\end{equation}
where $R(z)$ is a universal $R$-matrix on $C\times C$
assembled from the degree-two classical $r$-matrix $r(z)$
of $\cA$ together with a tower of higher-degree corrections.
At tier~(b) (Heisenberg, Kac--Moody, $\beta\gamma$) the
higher-degree corrections are determined recursively from
$r(z)$ and the local OPE; at tier~(c) (Etingof--Kazhdan,
Yangians) they require independent input.

The universal $R$-matrix $R(z)$ is not expected to be a
scalar: for genuinely $E_1$-chiral algebras it is a
meromorphic section on $C\times C$ with controlled pole
behaviour at the diagonal, as dictated by the spectral
braiding programme
(Chapter~\ref{chap:spectral-braiding}). The relation
between $R(z)$ and the twisting morphism $\tau$ of
Theorem~A of Volume~I is:
\begin{equation}
\label{eq:drinfeld-e1-R-tau}
R(z)
\;=\;\mathbf{1}
\;+\;\tau_{(2)}(z)
\;+\;\tau_{(3)}(z_1,z_2)
\;+\;\cdots,
\end{equation}
with $\tau_{(n)}$ the degree-$n$ component of the
twisting morphism and $r(z) = \tau_{(2)}(z)$ at binary
level. The higher components encode the
higher Maurer--Cartan components $\Theta_A^{(n)}$ of the
bar-intrinsic MC element.

\begin{proposition}[The arity-three Maurer--Cartan equation;
\ClaimStatusConditional]
\label{prop:drinfeld-double-tau3-mc}
\index{Drinfeld double!arity-three Maurer--Cartan equation}%
\index{Maurer--Cartan equation!arity-three Drinfeld double}%
Let $\mathfrak g_{\mathrm{mix}}(\cA)$ be the convolution dg Lie algebra
controlling the mixed product in
Conjecture~\ref{conj:drinfeld-double-e1-construction}, filtered by
ordered arity. Write the twisting datum as
\[
 \tau=\tau_{(2)}+\tau_{(3)}+\tau_{(4)}+\cdots,
 \qquad
 \tau_{(2)}(z)=r_\cA(z).
\]
The Maurer--Cartan equation
$d_{\mathrm{conv}}\tau+\frac12[\tau,\tau]=0$ has arity-three component
\begin{equation}\label{eq:drinfeld-double-tau3-mc}
 d_{\mathrm{conv}}\tau_{(3)}
 \;=\;
 -\frac12[\tau_{(2)},\tau_{(2)}]_{(3)} .
\end{equation}
Thus the first obstruction to extending the binary mixed product is the
cohomology class
\begin{equation}\label{eq:drinfeld-double-ob3}
 \operatorname{ob}_3(\tau_{(2)})
 :=
 \left[\frac12[\tau_{(2)},\tau_{(2)}]_{(3)}\right]
 \in H^2\!\left(\operatorname{gr}_3
 \mathfrak g_{\mathrm{mix}}(\cA)\right).
\end{equation}
\end{proposition}

\begin{proof}
The first assertion is the arity-three projection of the standard
convolution Maurer--Cartan equation for twisting morphisms
\cite[Ch.~6 and Ch.~10]{LV12}. The bracket of two binary operations
has ordered arity three, hence no other summand contributes to
\eqref{eq:drinfeld-double-tau3-mc}.
\end{proof}

\begin{computation}[Affine $\mathfrak{sl}_2$ arity-three obstruction;
\ClaimStatusProvedHere]
\label{comp:drinfeld-double-sl2-tau3}
\index{$\mathfrak{sl}_2$!Drinfeld double arity-three check}%
For $\cA=V_k(\mathfrak{sl}_2)$ with
$\tau_{(2)}(z)=k\Omega_{\mathrm{tr}}/z$ and
\[
 \Omega=e\otimes f+f\otimes e+\frac12 h\otimes h,
\]
the obstruction class
$\operatorname{ob}_3(\tau_{(2)})$ vanishes in the classical rational
affine convention. Explicitly, for $z_{13}=z_{12}+z_{23}$,
\begin{align}
[\tau_{(2)},\tau_{(2)}]_{(3)}
&=
k^2\Bigg(
\frac{[\Omega_{12},\Omega_{13}]}{z_{12}z_{13}}
+\frac{[\Omega_{12},\Omega_{23}]}{z_{12}z_{23}}
+\frac{[\Omega_{13},\Omega_{23}]}{z_{13}z_{23}}
\Bigg) \nonumber\\
&=0.
\label{eq:drinfeld-double-sl2-cybe}
\end{align}
Consequently one may choose $\tau_{(3)}=0$ for the first non-binary
Maurer--Cartan equation. This closes only the arity-three classical
equation; it does not construct the associator-dependent higher
Drinfeld-double tower.
\end{computation}

\begin{proof}
For $\mathfrak{sl}_2$, the displayed Casimir satisfies the infinitesimal
braid relations
$[\Omega_{12},\Omega_{13}+\Omega_{23}]=0$ and
$[\Omega_{13},\Omega_{12}+\Omega_{23}]=0$
\cite{Kohno1987}. Hence, with
$A=[\Omega_{12},\Omega_{13}]$, one has
$[\Omega_{12},\Omega_{23}]=-A$ and
$[\Omega_{13},\Omega_{23}]=A$. The scalar coefficient of~$A$ in
\eqref{eq:drinfeld-double-sl2-cybe} is
\[
 \frac{1}{z_{12}z_{13}}
 -\frac{1}{z_{12}z_{23}}
 +\frac{1}{z_{13}z_{23}}
 =
 \frac{z_{23}-z_{13}+z_{12}}
 {z_{12}z_{13}z_{23}}
 =0,
\]
because $z_{13}=z_{12}+z_{23}$. Thus
$[\tau_{(2)},\tau_{(2)}]_{(3)}=0$ and
$\operatorname{ob}_3(\tau_{(2)})=0$.
\end{proof}

\begin{remark}[Virasoro is the first unsolved filler]
\label{rem:drinfeld-double-virasoro-tau3-gap}
\index{Virasoro algebra!Drinfeld double arity-three gap}%
For the Virasoro boundary algebra the binary term is
\[
 \tau_{(2)}^{\mathrm{Vir}}(z)
 =
 \frac{c/2}{z^3}+\frac{2T}{z}
\]
in the collision-residue convention of
Chapter~\ref{chap:spectral-braiding}. The first missing equation is
therefore exactly
\begin{equation}\label{eq:drinfeld-double-virasoro-tau3-open}
 d_{\mathrm{conv}}\tau_{(3)}^{\mathrm{Vir}}
 =
 -\frac12
 [\tau_{(2)}^{\mathrm{Vir}},\tau_{(2)}^{\mathrm{Vir}}]_{(3)} .
\end{equation}
No explicit chain $\tau_{(3)}^{\mathrm{Vir}}$ solving
\eqref{eq:drinfeld-double-virasoro-tau3-open} is presently constructed
on the raw ordered bar complex. The quartic contact term
$(c/2)z^{-3}$ is the first class-$\mathbf M$ obstruction that the
formal Drinfeld-double tower must fill.
\end{remark}

\subsection{Obstructions}
\label{subsec:drinfeld-e1-obstructions}

The technical obstructions that prevent a full
proof of Conjecture~\ref{conj:drinfeld-double-e1-construction} are recorded below.

\paragraph{Obstruction 1: the mixed product lives on a sheaf,
not at a point.} Classical Drinfeld's construction
\cite{Drinfeld85,Drinfeld90} produces a double
$D(H) = H^{*\mathrm{op}}\otimes H$ as a vector space, and
the mixed product $H^{*\mathrm{op}}\otimes H\to D(H)$ is
a bilinear operation at a point. On a curve the
analogous mixed product $\cA\otimes\cA^!\to U(\cA)$
acquires a $z$-dependence: it is a meromorphic section
on $C\times C$ with controlled singularities on the
diagonal. This is not an algebra element but a section
of a sheaf. The resulting structure is not a Hopf
algebra in the ordinary sense, but a
``quasi-triangular chiral Hopf object'' whose category of
modules should recover the line-operator category
$\cC_{\mathrm{line}}$. Defining this object rigorously
(in particular, stating what the ``product'' and
``coproduct'' are as operations on sections, not as
bilinear maps) requires the formalism of chiral
factorization algebras with poles, which is in place on
the $E_\infty$ side (BD \cite{BD04}) but not developed
on the $E_1$ side.

\paragraph{Obstruction 2: higher-degree structure from the
MC tower.} At tier~(b), the binary $r$-matrix $r(z)$
determines the mixed product at degree two, but not at
higher degrees. Proposition~\ref{prop:drinfeld-double-tau3-mc}
identifies the first missing equation:
$d_{\mathrm{conv}}\tau_{(3)}
=-\frac12[\tau_{(2)},\tau_{(2)}]_{(3)}$. For affine
$\mathfrak{sl}_2$ this obstruction vanishes by the rational CYBE
(Computation~\ref{comp:drinfeld-double-sl2-tau3}), so the
arity-three classical filler can be chosen to be zero. For Virasoro
the explicit filler
$\tau_{(3)}^{\mathrm{Vir}}$ in
\eqref{eq:drinfeld-double-virasoro-tau3-open} is not known on the raw
ordered bar complex. At degree $n>3$ the components
$\Theta_A^{(n)}$ are determined recursively from the associative
$A_\infty$-structure of $\cA$, but the recursion is infinite and not
known in closed form for non-quadratic algebras. For Virasoro and
$\cW_N$ (class~$\mathbf{M}$) the tower never terminates, so the
Drinfeld double $U(\cA)$ at tier~(b) is only defined as a formal object
recorded by the full MC tower.

\paragraph{Obstruction 3: associator is transcendental.}
The twist in~\eqref{eq:drinfeld-e1-qt-compat} carries an
associator relating the three orderings of a three-point
mixed product. The associator at spectral parameter $z$
is the KZ associator on three points
\cite{Drinfeld90}, which is a transcendental power series
in the coproduct variables with coefficients in the ring
of multiple zeta values. Controlling this transcendental
object at the level of the bar complex is a nontrivial
structural requirement, and is presently handled only
for the simply-laced cases by appeal to Etingof--Kazhdan
quantization \cite{etingof1996quantization}. The
general case is open.

\paragraph{Obstruction 4: compatibility with $\Sigma_n$
descent.} Any $E_1$-chiral Drinfeld double should descend
to an $E_\infty$-chiral analogue by $\Sigma_n$-coinvariants.
The descent is $r$-matrix-twisted: the naive
quotient works only for pole-free algebras. For
tier~(b)+(c) examples the descent requires an explicit
compatibility between the mixed product and the
$\Sigma_n$-action on ordered configurations, which is
the content of subproblem~(d) of the Drinfeld double programme.

\subsection{Base cases}
\label{subsec:drinfeld-e1-base-cases}

The construction of
Conjecture~\ref{conj:drinfeld-double-e1-construction}
recovers the expected answer in the two base cases where a
classical answer is available.

\paragraph{Heisenberg (class $\mathbf{G}$).}
For $\cA = \cH_k$ the Heisenberg vertex algebra at level
$k\ne 0$, the Koszul dual is $\cA^! = \mathrm{Sym}^{\mathrm{ch}}(V^*)$ (the commutative chiral
algebra on the dual generator space). The Drinfeld double partner is $\cH_{-k}$ (this is not the ``signed
reflection'' $\kappa\mapsto-\kappa$ but the
double-partner Heisenberg at the opposite level; see
Theorem~A of Volume~I restricted to class~$\mathbf{G}$.)
The putative Drinfeld double is
\[
U(\cH_k)
\;=\;
\cH_k \bowtie \cH_{-k}
\;\simeq\;
\bigl(\cH_k \otimes \cH_{-k}\bigr) \;\big/\;
\bigl(\text{pairing}\bigr),
\]
where the quotient identifies the shared Heisenberg
vacuum and the pairing is the level-$k$ bilinear form
on generators: the mixed product is prescribed by the
$r$-matrix tensor $r_{\cH}(z)=k\Omega_{\cH}/z$ of~\eqref{eq:drinfeld-r-heisenberg}
with $\Omega_{\cH}=J\otimes J$.
At $k = 0$ both factors collapse to the trivial algebra
and $U(\cH_0) = \mathbb{C}$, consistent and
with the class~$\mathbf{G}$ prediction that the Drinfeld
double is trivial (Dimofte's $K$-matrix is $1$, the
shadow tower terminates at depth zero, and no twist is
needed). In this case all obstructions of
\S\ref{subsec:drinfeld-e1-obstructions} are vacuous: the
mixed product has no higher-degree corrections, the
coproduct is primitive, and the $\Sigma_n$-descent is the
naive quotient. The construction
of~\eqref{conj:drinfeld-double-e1-construction} is
tautological on class~$\mathbf{G}$.

\paragraph{Affine $\mathfrak{sl}_2$ at level one.}
For $\cA = V_1(\mathfrak{sl}_2)$ the affine Kac--Moody
vertex algebra at level $k=1$, the Verdier-dual level is
determined by the reflected-current relation $k' = -k - 2h^\vee$
of Theorem~A (with $h^\vee = 2$ the dual Coxeter number of
$\mathfrak{sl}_2$): substitution gives
$k' = -1 - 2\cdot 2 = -5$, so
$\cA^! \simeq
\operatorname{CE}^{\mathrm{ch}}\!\bigl(\widehat{\mathfrak{sl}}_2{}_{-5}\bigr)$.
(Numerical sanity
check: at $k = -h^\vee = -2$, one finds $k' = -2$, which
is the Feigin--Frenkel self-dual critical level, and the
formula is consistent scoping.) The classical
$r$-matrix is
$r_{V_k(\mathfrak{sl}_2)}(z) = k\,\Omega_{\mathrm{tr}}/z$, which at
$k=0$ vanishes identically and at $k=1$
gives the Yang form with unit level coefficient. The putative
Drinfeld double
$U(V_1(\mathfrak{sl}_2))$ should be an affine
quantum-group-like object whose module category
recovers the affine $\mathfrak{sl}_2$ level-one line
operators. The conjectural identification (\ClaimStatusConjectured)
is the affine Yangian $Y(\widehat{\mathfrak{sl}_2})$ at level one,
or a variant thereof; the precise identification is open.
Even at binary level, the mixed product of two dual
$\mathfrak{sl}_2$ currents has the structure
$J^a(z_1)J^{!,b}(z_2) \sim \delta^{ab}/(z_1-z_2)$ plus
regular terms, which is correctly captured by
the tier~(b) classical $r$-matrix. The full
associative structure at degree $\ge 3$ involves the
higher MC components $\Theta_A^{(n)}$, which are
controlled recursively but not in closed form
(Obstruction~2). The $V_1(\mathfrak{sl}_2)$ case is
the cleanest nontrivial test of the construction;
resolving it is the first milestone for subproblem~(b)
of the programme.

\begin{remark}[Relation to Dimofte's double bosonization
picture]
\label{rem:drinfeld-e1-dimofte}
Conjecture~\ref{conj:drinfeld-double-e1-construction} is
complementary to the double-bosonization picture of
Remark~\ref{rem:dimofte-double-bosonization} in the core
chapter (following Majid~\cite{Majid95}). Double
bosonization takes a braided Hopf algebra $H\in\cC$ and
produces a quasi-triangular Hopf algebra
$R(H) = H^{*\mathrm{op}}\bowtie \cB\bowtie H$. Our
$E_1$-chiral Drinfeld double is the special case
$R(B^{\mathrm{ord}}(\cA))$ of that construction when the
braided monoidal category $\cC$ is the spectral-braiding
category of line operators with meromorphic braiding
$r(z)$. The present section is thus an explicit
unpacking of the double-bosonization remark at the level
of the ordered bar coalgebra, with obstructions
identified explicitly. Neither this section nor the
core remark resolves the construction: both are
subproblems in the broader Drinfeld double programme.
\end{remark}

\begin{remark}[Conditional $H^2$ comparator and the three faces of
$K^{\kappaChHodge} = 8$]
\label{rem:oackd-front-classification-K-kappa}
\index{H2 classification!g Delta5@$\mathfrak{g}_{\Delta_5}$}%
\index{Igusa!automorphic comparator!Delta5}%
\index{K kappa ch@$K^{\kappaChHodge}$!three faces}%
\index{Mukai!Humbert!Lusztig!equivalence}%
\index{Borcherds double}%
The Drinfeld-double programme obstructions of
\S\ref{subsec:drinfeld-e1-obstructions} become sharper in
the Borcherds BKM setting, but Vol~II records a conditional
comparison target rather than a theorem that classifies. Concretely:
for the BKM $\mathfrak{g}_{\Delta_5}$ of Remark
\ref{rem:oackd-front-hall-drinfeld-double}, the $\Z/2
\times K(1)$-equivariant second Lie algebra cohomology
(where $\Z/2$ is the K3 Nikulin involution and $K(1)$ is
the chromatic height-one $E_\infty$-$K$-theory orientation
datum) is tested after the finite Hall/CoHA source, pairing,
PBW/no-extra-relations, radical quotient, parity, filtered completion,
inverse-limit, and EK/Heegner comparison gates through the target map
\[
 \widehat H^2_{\mathrm{red}}\!
   \left(\mathfrak{g}_{\Delta_5}\right)^{\Z/2,\,K(1)}
 \;\longrightarrow\;
 \C \cdot \Delta_5,
\]
to the line spanned by the primitive Gritsenko--Nikulin denominator
$\Delta_5$; its scalar Igusa square is
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$. Thus the weight-$5$
denominator class is the automorphic comparator for the Drinfeld
double deformation, not the bare invariant. The level parameter
in this presentation is tested against the Mukai-conductor invariant
\[
 K^{\kappaChHodge}
 \;=\;
 2\, c_+\!\bigl(\mu(\mathrm{K3})\bigr)
 \;=\;
 8
\]
on the B-family (read from the positive-definite signature of
the Mukai lattice on K3 under the appropriate twist), and it
has three comparison faces:
\begin{enumerate}[label=\textup{(F\arabic*)}, leftmargin=*]
\item \emph{Mukai face}: $K^{\kappaChHodge} =
 2\,c_+(\mu(\mathrm{K3}))$ where $\mu(\mathrm{K3}) =
 H^0 \oplus H^2 \oplus H^4$ carries the Mukai pairing
 with positive-definite signature of rank $c_+ = 4$;
\item \emph{Humbert face}: $K^{\kappaChHodge}$
 equals the Humbert-surface discriminant that parametrises
 the Siegel modular moduli of $\Delta_5$ on
 $\mathcal{A}_2$;
\item \emph{Lusztig face}: $K^{\kappaChHodge}$
 equals the Lusztig-canonical-basis contribution to the
 $q$-trace character of $U_q(\mathfrak{g}_{\Delta_5})$
 at the categorified level.
\end{enumerate}
When the gates pass, the three faces coincide numerically
($K^{\kappaChHodge}=8$) and furnish three independent
verification paths for the same B-family invariant. The Vol~II
universal
specialisation $\hbar^2 K^{\kappaChHodge} = -1$
(arising from the Fourier-transform normalisation on
$F_{\mathrm{K3}}$) fixes the $\hbar$-scale of the
$E_1$-chiral deformation, and combined with the
$\Z/2 \times K(1)$-equivariant $H^2$ target above, supplies the
expected Drinfeld-double comparison for $F_{\mathrm{K3}}$ up to the
Igusa scalar. Together the Hall-double, cohomological-comparator, and
Mukai-conductor faces exhibit the K3-frontier quantum-group
deformation as a gated target: one finite-window Hall-double source, a
completed reduced comparison line mapping to $\Delta_5$, and a
B-family level $K^{\kappaChHodge}=8$ with three verification
paths.
\end{remark}

\begin{remark}[Chiral coproduct for non-simply-laced types
via folding]
\label{rem:bcfg-folding-route}
\index{chiral coproduct!non-simply-laced types}
\index{folding!Z/2 quiver automorphism}
For types $B_n$, $C_n$, $F_4$, and~$G_2$, folding gives a
candidate route from the ADE vertex coproduct of
Jindal--Kaubrys--Latyntsev~\cite{JKL26} to non-simply-laced
chiral coproducts. It is conditional on an explicit landing
calculation, not a formal consequence of equivariance alone.
At the Yangian level, the $A_3$ fold gives the type-$C_2$
orbit RTT comparison: the involution
$\sigma(1) = 3$, $\sigma(2) = 2$ on the $A_3$ Dynkin
diagram induces $\sigma(T_{ij}(u)) = T_{5-i,5-j}(u)$
on the RTT generators of $Y(\mathfrak{sl}_4)$.
The matrix coproduct is $\sigma$-equivariant
($\Delta \circ \sigma =
(\sigma \otimes \sigma) \circ \Delta$,
a three-line verification), so the orbit generators inherit
the matrix coproduct. The comparison is with the
four-dimensional symplectic RTT presentation of
$Y(\mathfrak{sp}_4)$; the type-$B_2$ orthogonal vector
realisation is a separate normalisation under
$\mathfrak{sp}_4\simeq\mathfrak{so}_5$, not the fixed-point
identity $Y(\mathfrak{sl}_4)^\sigma=Y(\mathfrak{so}_5)$.
At the \emph{chiral level}, the vertex coproduct
$\Delta^v$ on $V_k(\mathfrak{sl}_4)$ is equivariant by
geometric naturality of the Hecke correspondence under
quiver automorphisms, and the potential~$W$ of the $A_3$
quiver is $\sigma$-invariant (trace of oriented cycle
compositions). Equivariance gives a map to
$(V\otimes V)^\sigma$; a coproduct on the fixed-current
subalgebra generated by $\mathfrak{sp}_4$ requires the
vanishing of the nontrivial tensor-sector components.
The $\sigma$-equivariance of the critical vanishing-cycle
sheaf and the factoring of the restricted $\Delta^v$
through
$V_k(\mathfrak{sp}_4) \otimes V_k(\mathfrak{sp}_4)$
are the two concrete open steps
(see also DeHority--Latyntsev~\cite{DeHorityLatyntsev25}
for the orthosymplectic module theory of CoHAs).
The orthogonal/symplectic RTT sources cover the $B$ and $C$
series; $F_4$ and $G_2$ need separate exceptional Yangian
sources. The $G_2$ case ($\Z/3$ fold of $D_4$, triality) is
harder: the local quotient of type $\tfrac{1}{3}(1,1)$ is
not Gorenstein and has no crepant resolution.
\end{remark}

\section{Frontier anchor: F4 super-complementarity canonical pairing}
\label{sec:ordered-kd-frontier-anchor-f4}

The complementarity identity $\kappa + \kappa^{!} = 0$ for affine
Kac--Moody (\S2567) and $\kappa + \kappa^{!} = 13$ for Virasoro
(\S4733), together with the self-dual locus
$\kappa = \kappa_{0}^{(2,2)}/2$ (\S4735) for non-principal
hook-type $W$-algebras, are the bosonic instances of the universal
complementarity package. The super-extension is the open frontier
\cref{conj:f4-super-complementarity-canonical}.

\begin{remark}[F4 anchor: $\kappa + \kappa^{!} = \max(m, n)$ on
super-Yangians]
\label{rem:ordered-kd-frontier-anchor-f4}
For super-Yangians $Y(\mathfrak{sl}(m | n))$, the
super-complementarity identity
\[
 \kappaChHodge(Y(\mathfrak{sl}(m | n)))
 \;+\;
 \kappaChHodge(Y(\mathfrak{sl}(n | m))^{!})
 \;=\;
 \max(m, n)
\]
is currently established on the sub-Sugawara line via two
coexisting bilinear pairings: the super-trace pairing on the
$E_{1}$-bialgebra side, and the Berezinian pairing on the modular
PVA side. The two pairings agree numerically on the sub-Sugawara
line; they have not been canonicalised through a single bilinear
pairing at the programme level. The remaining path is the
construction of the Verdier pairing on the super-bar complex, which
would identify both pairings as specialisations of the universal
super-Verdier pairing on $B^{\mathrm{ord}}(Y(\mathfrak{sl}(m | n)))$.

The $\Z/2$ fold of $A_{2n + 1}$ realising
$Y(\mathfrak{sl}(2n + 1)) \to Y(\mathfrak{so}(2n + 2))$
discussed above is the bosonic prototype of the super-fold
$\Z/2$ realising
$Y(\mathfrak{sl}(m + n)) \to Y(\mathfrak{sl}(m | n))$ via parity
re-grading; the construction of the canonical pairing on the super-bar
complex follows the same Hecke-correspondence template, with the
critical vanishing-cycle sheaf carrying the super-trace versus
Berezinian distinction. The bosonic complementarity identities
of this chapter (\S2567 KM, \S4733 Virasoro, \S4735 hook-type)
extend to the super case once F4 is closed.
\end{remark}

\section{Synthesis}
\label{sec:oackdf-supplement}

\begin{remark}[Ordered-associative chiral $K_d$ frontier at K3]
\label{rem:oackdf-1}
The ordered-associative chiral $K_d$ frontier at the K3 base
point is the $K_2$ ordered-associative chain on
$\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$ with coefficients in
$\mathbf H_{\Delta_5}$-modules, with K3 Koszul shift $[2]$ matching
CY-$2$ Serre functor $S = [2]$. The frontier extension to $K_d$ for
$d\geq 3$ lives on $K3\times E$, connecting to Vol~I's frontier on
higher-genus modular Koszul via the bi-based Ran descent to
$\bar{\mathcal A}_2$ \cite{FrancisGaitsgoryFactorization,KugaSatake1967}.
Compare Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[frontier $K_d$ and universal identity]
\label{rem:oackdf-2}
The universal identity $\hbar^2 \cdot K^{\kappaChHodge} = -1$
gives the $K_d$ frontier its root-of-unity specialisation:
$K_d$ closes at order $8 = K^{\kappaChHodge}$ with Lusztig
specialisation $\hbar^2 = -1/8$, beyond which the ordered associative
chain becomes non-associative (sentinel for the pentagon coboundary
$\phi^{(3)}$ at $\hbar^3$)
\cite{Lusztig1990,EtingofKazhdan2007PartV}. Compare
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\section{refined \texorpdfstring{$\Omega$}{Omega}-background}
\label{sec:oackdf-supplement-ii}

\begin{remark}[Refined $\Omega$ two-parameter family off self-dual; \ClaimStatusConjectured]
\label{rem:oackdf-refined-omega}
The K3 ordered-associative chain at $\hbar^2 = -1/8$ lies on
the self-dual $\Omega$-locus $\epsilon_1 + \epsilon_2 = 0$. Away from
that locus, the ordered-associative frontier extends as a
\emph{two-parameter family}
$\mathbf H_{\Delta_5}^{\mathrm{frontier}}(\epsilon_1, \epsilon_2)$
parametrised by the refined Omega-background $(\epsilon_1, \epsilon_2)$
acting on the $\mathbb C^2$ fibre of the Maulik--Okounkov geometric
setting (equivariant parameters of $\mathbb C^\times \times
\mathbb C^\times \curvearrowright \mathrm{Hilb}^n(\mathrm{K3})$ via
hyperk\"ahler rotation of the K3 symplectic form). Generic
$\epsilon_1 \neq -\epsilon_2$ corresponds to the refined topological
vertex of Iqbal--Kozcaz--Vafa 2007; self-dual
$\epsilon_1 + \epsilon_2 = 0$ is the unrefined Gromov--Witten
specialisation. Primary: Maulik--Okounkov \emph{Ast\'erisque} 408
(2019), especially Part~III (refined deformation); Iqbal--Kozcaz--Vafa
2007 (hep-th/0701156); Nekrasov--Okounkov 2003 (hep-th/0306238).
Compare Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic} and
core-companion Remark~\ref{rem:v2-oackd-refined-omega-two-param}.
\end{remark}

\begin{remark}[$E_1$ curve level, derived-centre $E_2$, and surface $\PhiFA_2$; \ClaimStatusProvedHere]
\label{rem:oackdf-native-derived}
The K3 ordered-associative frontier has three distinct operadic
levels. Write $\mathbf H_{\Delta_5,E}$ for the curve-level restriction
of the Vol.~III K3 object to the elliptic base curve
$E\subset\mathrm{K3}$.

\begin{itemize}
\item \textbf{Curve-level $E_1$-chiral algebra.} $\mathbf H_{\Delta_5,E}$ is
 strongly admissible in the sense of \S2 of the core companion, with
 bar coalgebra $\Barch(\mathbf H_{\Delta_5,E})$ computed on the ordered
 Ran space $\mathrm{Ran}^{\mathrm{ord}}(E^{\mathrm{nod,sm}}_{24})$ of
 the $24$-nodal elliptic base. The ordered-associative Koszul duality
 of this chapter operates at this level. No $E_2$ structure is asserted
 on this underlying curve algebra.
\item \textbf{Derived-centre $E_2$.} The chiral brace operations give
 an $E_2$-structure on the derived chiral centre
 \[
  Z^{\mathrm{der}}_{\mathrm{ch}}(\mathbf H_{\Delta_5,E})
  =
  \ChirHoch^\bullet(\mathbf H_{\Delta_5,E},
  \mathbf H_{\Delta_5,E}).
 \]
 This is the closed/bulk colour in the
 $\mathsf{SC}^{\mathrm{ch,top}}$ datum attached to
 $\mathbf H_{\Delta_5,E}$; it is not an $E_2$ structure on the
 underlying $E_1$-chiral algebra.
\item \textbf{Surface-level $\PhiFA_2$ output.} The stage-one surface
 part of the Vol.~III CY-to-chiral functor at $d = 2$ supplies a
 factorisation object
 $\PhiFA_2(D^b\mathrm{Coh}(\mathrm{K3}))$ with $E_2$-operations on the
 K3 surface. The stage-two specialisation to the elliptic base curve
 $E$ gives the curve algebra
 $\mathbf H_{\Delta_5,E}$ and forgets the transverse surface
 factorisation to the ordered $E_1$ level used here.
\end{itemize}

Thus the ordered associative Koszul chain lives at $E_1$ on the curve
$E$, the derived centre carries the $E_2$ bulk structure, and the
Vol.~III CY-$2$ surface stage carries $E_2$ only before restriction to the
curve. Compare core companion
Remark~\ref{rem:v2-oackd-eone-etwo-scope} and
Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[Refined Bruinier Prop 5.1 and Borcherds-lift refinement; \ClaimStatusConjectured]
\label{rem:oackdf-bruinier-borcherds-refinement}
At the self-dual slice, the 1-loop determinant of twisted 11D SUGRA on
$\mathbb R^3 \times \mathrm{K3} \times \mathbb C^2$ Fourier-expands
with coefficients $A(n, l, m) = c_{\mathrm K3}(4nm - l^2)$ matching the
Bruinier Prop 5.1 multiplicities of the Borcherds lift of
$\phi_{0,1}^{\mathrm K3}$. The finite Heegner-label comparison matches
the five labels in Eguchi--Ooguri--Tachikawa 2011 Table~1. Off
self-dual, the multiplicities refine to
$m(n, \ell_1, \ell_2, m) = c_{\mathrm K3}^{\mathrm{ref}}(4nm - \ell_1\ell_2)$
for the refined K3 elliptic genus $\chi_{\mathrm K3}^{\mathrm{ref}}$ of
weight-$0$ bi-index $(1, 1)$ in two elliptic variables. The refinement
preserves the dimension of the BKM Lie algebra root spaces but splits
each root-multiplicity into a $\mathrm U(1) \times \mathrm U(1)$
graded character. Primary: Bruinier 2002 Sec.~5; Iqbal--Kozcaz--Vafa
2007 Sec.~4; Eguchi--Ooguri--Tachikawa 2011 Table~1.
\end{remark}

\begin{remark}[Frontier $q$-Bethe discipline and scope; \ClaimStatusConjectured]
\label{rem:oackdf-qbethe-nonselfdual}
The integrable spectrum of
$\mathbf H_{\Delta_5}^{\mathrm{frontier}}(\epsilon_1, \epsilon_2)$ at
generic refined $\Omega$ is governed by the Nekrasov--Shatashvili
quantisation: the Yang--Yang function is $\log Z^{\mathrm{ref}}$, and
the saddle-point equations in $q = \exp(-2\pi i \epsilon_2 / \epsilon_1)$
are $q$-Bethe equations. The NS limit $\epsilon_2 \to 0$ recovers a
quantum-classical duality with spectral parameters in $\epsilon_1$.
Scope: the rigorous identification of these $q$-Bethe equations for
$\mathbf H_{\Delta_5}$ beyond the $\mathrm{Hilb}^n(\mathbb A^2)$
prototype is open for K3 itself; this frontier remark states the
conjectural extension. Primary: Nekrasov--Pestun--Shatashvili 2013
(arXiv:1312.6689); Maulik--Okounkov \emph{Ast\'erisque} 408 (2019)
Part~IV (Bethe ansatz).
\end{remark}

\begin{remark}[Frontier summary]
\label{rem:oackdf-refined-summary}
Pushing off the self-dual $\Omega$-locus deforms the K3
ordered-associative Koszul-duality chain into a two-parameter family
$(\epsilon_1, \epsilon_2) \to \mathbf H_{\Delta_5}^{\mathrm{frontier}}
(\epsilon_1, \epsilon_2)$ with: (a) refined K3 elliptic genus
Fourier input replacing $\phi_{0,1}^{\mathrm K3}$; (b) refined BKM
product replacing the $\Delta_5$ Borcherds lift; (c) $q$-Bethe
integrability replacing rational Bethe ansatz; (d) native $E_1$ level
preserved on the curve $E$, derived-centre $E_2$ preserved on
$\ChirHoch^\bullet(\mathbf H_{\Delta_5,E})$, and surface $\PhiFA_2$
operations preserved only before curve restriction. The self-dual slice $\epsilon_1 + \epsilon_2 \to 0$
collapses the family to the fixed point and recovers
$\hbar^2 = -1/8$. Frontier status is open: full rigorous construction
of the two-parameter chain in the ordered-associative Koszul framework
requires chain-level verification of the refined MC equation
$D \Theta^{\mathrm{ref}}_\cA + \tfrac12 [\Theta^{\mathrm{ref}}_\cA,
\Theta^{\mathrm{ref}}_\cA]_\hbar = 0$ in refined perturbation theory.
\end{remark}
