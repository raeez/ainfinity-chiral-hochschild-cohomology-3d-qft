...
S_p$.  A plausible route is to upgrade the standard vertex-algebra estimate that
the OPE cannot create poles of higher order than the input
to the commutator-filtered ordered bar differential
...
words, the bar differential, built from iterated OPE residues, should not
increase the maximal pole order of a configuration.  What remains to be checked
is that this pole-order estimate is compatible with the ordered bar differential
and the commutator filtration used to define the strata~$S_p$.  That
verification is not carried out here, so the remark supplies only evidence for
Conjecture~\ref{conj:FG-shadow}, not a completed proof within the
...
inherently a bulk-to-boundary composition, never the reverse.
...
\item $d_{\mathrm{wrap}}$ maps to the two ``end'' coface maps
  ...
  of $C_\Delta$.
\end{itemize}
The crucial point is that the wrap-around differential, which is the
geometric novelty of the annular construction, encodes precisely
...
followed by interval composition---which is precisely $\mu_P$
...
the bar construction (AP19: the $d\log(z{-}w)$ kernel absorbs
...
Wait --- skew-symmetry for mixed statistics requires care.
...
For affine KM algebras (AP24), $\kappa + \kappa^! = 0$.
...
The bar kernel absorbs one pole (AP19): the collision
...
third-order pole in the $\Gp\Gm$ OPE (after AP19
