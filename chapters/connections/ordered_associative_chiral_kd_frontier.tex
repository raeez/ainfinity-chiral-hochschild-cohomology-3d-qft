...
Theorem~\ref{conj:ordered-log-fm-modular}) and the
...
$\FM_n^{\mathrm{ord}}(\Sigma_g\mathbin{|}D)
= \FM_n(\Sigma_g\mathbin{|}D)$
(Theorem~\ref{conj:ordered-log-fm-modular}, proof).
...
theorem (Theorem~\ref{conj:ordered-log-fm-modular}) are
...
\item For the annulus $\bA = S^1\times[0,1]$:
  ...
  $\bA_\epsilon\to\{0\}$, but this is a degeneration of the
...
+ (-1)^{\dim\FM}\id_{\FM}\otimes\partial_{E_1}$.  The $E_1$
...
\item $\mathrm{C}^{E_1}(g)$: the ordered complementarity
  $\kappa(\cA)+\kappa(\cA^!)=0$ gives net-zero curvature in
  the total bulk theory at genus~$g$.  The ordered Frobenius
  structure
...
admits strict spectral factorisation realisation in
the $\hbar$-completed algebra.  The descent from the
...
= 0$ for all $n\ge 2$.  The gauge twist
$G^{\mathrm{mod}}(z;\hbar)$ that strictifies
$R^{\mathrm{mod}}$ is constructed inductively on the
$\hbar$-filtration: at each order $\hbar^{2g}$, the
obstruction to extending the twist from order
$\hbar^{2(g-1)}$ to order $\hbar^{2g}$ lives in
$H^3_{\mathrm{spec}}(\mathrm{gr}^n\widehat{Y})$, which
vanishes by the same root-space argument.

The descent $\barB^{\mathrm{ord}}(\cA)
\xrightarrow{R^{\mathrm{mod}}} \barBch(\cA)$
is the $\Sigma_n$-averaging
$\operatorname{av}(\Theta_\cA^{E_1}) = \Theta_\cA$
of Theorem~\ref{conj:modular}, mediated by the
strictified $R^{\mathrm{mod}}$.  Since strictification
is well-defined at all $\hbar$-orders, the descent is
