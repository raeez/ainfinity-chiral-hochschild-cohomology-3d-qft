...
\begin{conjecture}[Higher-genus ordered log-FM and the modular
ordered bar complex]%
\label{conj:ordered-log-fm-modular}%
\ClaimStatusConjectured%
\index{bar complex!ordered!modular from log-FM}%
\index{Fulton--MacPherson!ordered logarithmic!higher genus}%
At genus~$g\ge 1$, the ordered log-FM space
$\FM_n^{\mathrm{ord}}(\Sigma_g\mathbin{|}D)$ on a smooth
curve $\Sigma_g$ with divisor~$D$ carries the following
structure:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The codimension-$1$ boundary decomposes into
  \emph{three families} of faces:
  \begin{enumerate}[label=\textup{(\alph*)}]
  \item \emph{ordered collision faces}: consecutive clusters
    collide, contributing the genus-$0$ ordered bar
    differential~$d_{\mathrm{bar}}$\textup{;}
  ...
  \end{enumerate}
\item The boundary faces of type~\textup{(b)} and~\textup{(c)}
  factor via Mok's cutting map~\textup{\cite{Mok25}}
  \[
    \FM_n^{\mathrm{ord}}(W/B)(\rho)
    \;\longrightarrow\;
    \prod_{v\in V(S_\rho)}
    \FM_{I_v}^{\mathrm{ord}}(Y_v\mathbin{|}D_v),
  \]
  where the product is taken in the category of ordered
  log-FM spaces \textup{(}the ordering on each factor is
  induced by the planar structure of the degeneration
  graph\textup{)}.
\item The identity $\partial^2 = 0$ on
  $\FM_n^{\mathrm{ord}}(\Sigma_g\mathbin{|}D)$
  recovers the identity $D^2 = 0$ of
  Construction~\textup{\ref{constr:modular-ordered-bar}},
  including the curved identity
  $d_{\mathrm{bar}}^2 + [D_{\mathrm{nsep}},
  d_{\mathrm{bar}}] + D_{\mathrm{nsep}}^2 = 0$.
...
Conjecture~\ref{conj:ordered-log-fm-modular}) and the
...
extend log-FM to the $E_1$ direction.  It extends log-FM to
the \emph{ordered component} of the closed-colour FM space.
The handle-loop monodromy that forces
$\FM_k^{\mathrm{ord}}(\Sigma_g\mathbin{|}D)$ to have
nontrivial boundary at $g\ge 1$ is monodromy in the
\emph{surface} pure braid group of $\Sigma_g$, not
...
$\FM_n(X\mathbin{|}D)$ (Chapter~\ref{chap:fm3-planted-forest})
...
$\FM_n^{\mathrm{ord}}(\bA_\epsilon\mathbin{|}\partial\bA_\epsilon)$
for the family $\bA_\epsilon = S^1\times[0,\epsilon]$
...
\begin{proof}
The Swiss-cheese operation space is a product
$\FM_k(\Sigma_g) \times E_1(m)$.  The boundary operator on
the total space decomposes as
\begin{equation}\label{eq:boundary-product-decomposition}
  \partial_{\mathrm{SC}}
  \;=\;
  \partial_{\FM}\otimes\id_{E_1}
  \;+\;
  (-1)^{\dim\FM}\,
  \id_{\FM}\otimes\partial_{E_1}\,,
\end{equation}
with no mixed codimension-$1$ term: a boundary face of the
product is a face of one factor times the interior of the
other.  (Mixed boundaries appear at codimension~$2$ as
corners, contributing to $\partial^2 = 0$ but not
to~$\partial$.)

The bar differential $\dfib$ and the genus-raising operators
$D_{\mathrm{sep}}$, $D_{\mathrm{nsep}}$ come from
$\partial_{\FM}$: they are residues on collision and
degeneration faces of $\FM_k(\Sigma_g)$.  The deconcatenation
coproduct $\Delta$ comes from $\partial_{E_1}$: it is the
cutting of an ordered interval into two subintervals.

Since $E_1(m)$ does not depend on~$g$---the space~$\bR$ is
contractible at all genera, and line operators are ordered
on~$\bR_{\ge 0}$ independent of~$\Sigma_g$---the coproduct
$\Delta$ is genus-independent.  In particular:
\begin{itemize}[nosep]
...
genus-independent.  The ``no open-to-closed'' rule of
$\SCchtop$ (Definition~\ref{def:SC-colors}) is the operadic
expression of this product decomposition: the second factor
\emph{cannot receive} genus-dependent information from the
first, because the product has no cross-terms at
codimension~$1$.
\end{remark}
...
conjecture (Conjecture~\ref{conj:ordered-log-fm-modular}) are
...
\label{rem:rigidity-open-questions}%
\index{topological rigidity!open questions}%
The topological rigidity principle settles the foundational
question for the $E_1$~wing but leaves three downstream
problems:
\begin{enumerate}[label=\textup{(\alph*)}]
\item \emph{The bordered FM compactification}
  (Conjecture~\ref{conj:bordered}) remains open.  The
  topological rigidity theorem says the $E_1$ factor is
  genus-independent, but the \emph{bordered} compactification
  (for intervals and annuli with specified boundary
  conditions) is a genus-$0$ problem that has not yet been
  solved as a manifold-with-corners construction.
\item \emph{The five $E_1$ main theorems at higher genus.}
  Theorems~$\mathrm{A}^{E_1}$--$\mathrm{H}^{E_1}$
  (\S\ref{sec:e1-theorems}) are stated at genus~$0$.  The
  topological rigidity principle implies they extend to all
  genera, but the precise formulation of the genus-$g$
  analogues (especially Theorem~$\mathrm{C}^{E_1}$
  at $g\ge 1$, where the ordered Frobenius structure
  interacts with curvature) has not been written.
\item \emph{Ordered-to-unordered descent at higher genus.}
  The descent from $\barB^{\mathrm{ord}}(\cA)$ to
  $\barBch(\cA)$ at genus~$g\ge 1$ requires the modular
  $R$-matrix $R^{\mathrm{mod}}(z;\hbar)$.  The convergence
  and well-definedness of this descent for all simple Lie
  algebras is guaranteed by the complete strictification
  theorem
  (Theorem~\ref{thm:complete-strictification}), but the
  explicit comparison with the curved bar complex at
  $g\ge 2$ has not been carried out.
\end{enumerate}
\end{remark}
