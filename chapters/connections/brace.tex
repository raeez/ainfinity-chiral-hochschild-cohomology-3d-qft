...
the two-coloured operad $\SCchtop$: closed inputs act on the open
colour $\cA$, with the directional convention forbidding open inputs producing closed outputs. The brace package is not a Dunn-additivity
identification of $\SCchtop$ with a single-colour $E_3$ operad.
After fixing a Drinfeld associator and a two-coloured cobar homotopy
satisfying Proposition~\ref{prop:chd-two-coloured-cobar-obstruction},
the pair
\[
...
and the chain-level identities
\eqref{eq:chd-hoc-contraction}--\eqref{eq:chd-hoc-delta-mix}
for the two-coloured $(\SCchtop)^!$-cobar complex. Write
...
\begin{proof}
The homotopy-category assertion is
Theorem~\ref{thm:chiral-higher-deligne}(2), with closed colour
identified with the chiral derived centre via
...
$(\SCchtop)^!$-cobar convolution complex. The gauge statement is
deformation theory of operadic twisting morphisms
\cite[Chs.~6, 10]{LV12}: two choices of strictification represent the
same point of $\pi_0$ when connected by a
...
chain category requires a distinguished point in the
...
Let $\cA$ be an $E_1$-chiral algebra and
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA)=C^\bullet_{\mathrm{ch}}(\cA,\cA)$.
The Deligne--Tamarkin content for this chapter:
\begin{enumerate}
...
\begin{proof}
(i) is the brace-to-Gerstenhaber passage for Hochschild
cochains, applied to the chiral Hochschild model of
Proposition~\ref{prop:derived-center-E2}. (ii) is
Theorem~\ref{thm:chd-deligne-tamarkin}: the map
$C_\ast(E_2)\to\mathrm{Br}$ comes from Tamarkin's formality
theorem after choosing $\Phi$; the chiral formal-disc model is
identified with the geometric model by
Proposition~\ref{thm:chiral-hochschild-models-equivalent}. (iii)
is obstruction theory for filtered operad morphisms, written
...
is the obstruction to solving the Maurer--Cartan equation one
filtration step further in the convolution dg Lie algebra
\cite[Ch.~6]{LV12}. If $[r,r]=0$, the length-three obstruction is the
triple Massey product from choices of null-homotopy for the
binary bracket; changing the null-homotopy adds
$r\cdot H+H\cdot r$. The final sentence is
Convention~\ref{conv:chd-e3-chiral-meaning}: $\SCchtop$ is
two-coloured until explicit inner-stress topologisation replaces the chiral direction by a topological one.
\end{proof}
...
$Y_\hbar(\mathfrak{g})$. Set $\mathcal{E}:=\mathrm{End}^{\mathrm{ch}}_{Y_\hbar(\mathfrak{g})}$,
the chiral endomorphism operad of Remark~\ref{rem:e1-chiral-deligne-chain-level-obstruction}.
Then:
\begin{enumerate}
 ...
 part~\textup{(a)}. CYBE defines the Massey coset; vanishing of the image of $\theta_3$ in the particular chiral
 endomorphism operad is a separate question.
 \end{enumerate}
\end{proposition}

\begin{proof}
CYBE is the Maurer--Cartan equation for the binary class $r$ in
the convolution dg Lie algebra; the triple Massey
coset and its indeterminacy follow the standard construction
...
equations and Tamarkin's formality theorem assert that a
...
The displayed quotient is the target in which the
...
\begin{proof}
The indeterminacy formula is Massey indeterminacy for a triple
...
and applying $t_{ij}\mapsto\Omega_{ij}$ gives the scalar
$\sigma_3(\Phi)$ multiplying $\theta_3$. Passing to the quotient by
$\mathrm{Ind}$ yields the displayed criterion. No injectivity statement
for $\mathfrak t_3\to H^\bullet(\mathcal E)$ enters.
\end{proof}
...
length-three projections are linear combinations of the
displayed Lie words. Drinfeld computes these coefficients for KZ
...
field $\Bbbk$ with non-empty $\mathrm{Assoc}(\Bbbk)$, the set is a torsor under
$\mathrm{GRT}_1(\Bbbk)$; the KZ associator is the distinguished
complex point from regularised KZ parallel transport, with
multiple-zeta coefficients. Therefore:
\begin{enumerate}
...
\cite{Drinfeld90QuasiHopf}; Furusho proves compatibility of
pentagon and hexagon
\cite{Furusho2010PentagonImpliesHexagon}. Tamarkin's formality
construction uses an associator to produce a chain map
$C_\ast(E_2)\to\mathrm{Br}$ \cite{Tamarkin2003}. The rational
Yangian $r$-matrix supplies the infinitesimal braid representation;
a point of the associator torsor is extra data.
\end{proof}

\begin{remark}[Primary anchors for the associator obstruction]%
\label{rem:massey-rrr-yangian-verification}%
Three external inputs feed the local obstruction package:
\begin{enumerate}
...
\]
with differential induced by the coderivation on $O^\vee$ and the internal differential of $\mathrm{End}_A$.
...
\]
by grafting trees in $O^\vee$ (\cite{GJ94,Vor99}).
The resulting structure on $\mathrm{C}^\bullet_O(A)$ is a homotopy Gerstenhaber (brace) algebra; its induced Gerstenhaber bracket is
\[
...
The brace algebra is a strict presentation of the $E_2$/Gerstenhaber package on
Hochschild cochains, not the invariant deformation object. The Gerstenhaber bracket on
$\mathrm{C}^\bullet_O(A)[1]$ is the binary Taylor coefficient of the
filtered $L_\infty$-structure from the bar-coderivation complex.
\end{remark}
...
chiral Gerstenhaber brace operations, distinct from classical
topological Gerstenhaber brace operations. The discipline:
\begin{itemize}[nosep]
\item (Three Hochschild theories, one brace each.) The brace acts on
 ...
 Each Hochschild theory carries its own brace; the three coincide after the comparison maps of
 Theorem~\ref{thm:three-hochschild-unification} (\S\ref{sec:chiral_hochschild}).
\item (Chiral $\ne$ topological brace.) The chiral brace is
 integration over $\FM_k(\C) \times \Conf_m(\R)$
 with logarithmic kernels; the topological brace is the McClure--Smith
 brace~\cite{McClureSmith02} on the little 2-disks operad. The two agree on
...
\item The unqualified ``Hochschild brace'' is ambiguous;
 the intended brace is always chiral, topological, or categorical.
\end{itemize}
\end{remark}

\begin{remark}[Degree conventions]
Cohomological grading. For $O=\OHT$ the internal degrees reflect the form-degrees on $\FM(\C)$ and the $E_1$-degrees on $\Conf(\R)$; brace degree shifts follow the Gerstenhaber sign rule.
The brace operations are the transferred $L_\infty$-brackets
...
\]
the integral over the fiber product of configuration spaces determined by the brace tree (sign and degree formulas in Appendix~\ref{app:brace-signs}; the geometric content is operadic composition of correlation kernels via fiber integration over the tree parameter space).

\begin{proposition}[Well-definedness and functoriality]
\label{prop:geometric-braces-well-defined-brace}
For $A$ a logarithmic $\SCchtop$-algebra, the geometric brace operations on $\mathrm{C}^\bullet_{\mathrm{ch,top}}(A)$ are well-defined, strictly multilinear, and satisfy the brace identities. The Gerstenhaber bracket from these braces matches the one induced by the cooperad $O^\vee$.
\end{proposition}

\begin{proof}
Integrability is Definition~\ref{def:log-SC-algebra} (logarithmic weight forms have controlled singularities): the weight forms have at most logarithmic singularities along the boundary divisors of the FM compactification, integrable on compact manifolds with corners. The brace identities follow from Stokes on FM boundaries with corner cancellation by Theorem~\ref{thm:Arnold_full_proof} (Appendix~\ref{app:FM_Stokes}): boundary strata reproduce the brace relations, Arnold cancellations at codimension-$2$ corners absorbing spurious terms (Appendix~\ref{app:brace-signs} for the sign verification). Compatibility with the cooperad differential is the operadic cochain construction for a $W(\SCchtop)$-algebra.
\end{proof}
...
\begin{proof}
By the Kontsevich--Soibelman deformation theorem~\cite[Thm.~3.1]{KS09} (also \cite[Thm.~10.3.1]{LV12}), deformations are classified by twisting morphisms from the bar cooperad into $\mathrm{End}_A$; a twisting morphism is an MC element in the homotopy convolution object associated to $(O^\vee,\mathrm{End}_A)$. On a bar-cofree model this homotopy object is strictified by the convolution Lie algebra $\mathrm{Hom}(O^\vee,\mathrm{End}_A)$ (\cite{GK94,Val07}). The brace model identifies this strictification with $\mathrm{C}^\bullet_{\mathrm{ch,top}}(A)[1]$.
\end{proof}
