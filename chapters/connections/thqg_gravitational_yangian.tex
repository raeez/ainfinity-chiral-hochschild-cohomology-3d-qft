...
one lower than the OPE poles at $z^{-4}$ and~$z^{-2}$ (AP19).
...
infinite shadow tower: the higher-order poles generate an infinite
...
a Ward identity (Theorem~\ref{thm:thqg-VI-leading-soft}).
\end{remark}
...
computes the explicit gravitational $r$-matrix, the three-graviton
vertex, and the representation-theoretic data.
...
operators in the HT theory only on the affine/proved dg-shifted-Yangian
lane.  Independently, the underlying line-operator category is the
open-colour module category $\cC \simeq \cA^!\text{-}\mathsf{mod}$
...
\subsubsection{The line-operator category and its Yangian enhancement}

\begin{definition}[Line-operator category and conditional Yangian enhancement]
...
\cA^!\text{-}\mathsf{mod}
\end{equation}
On the affine/proved Yangian lane, this category is presented by
...
braiding on $\cC_\cA$ is the \emph{$R$-matrix braiding}:
...
$\cC_\cA$ is \emph{meromorphic} in the spectral
...
\item The genus-zero binary shadow
  ...
  independently, the algebraic line-side model is
  $\cA^!\text{-}\mathsf{mod}$.
...
RTT relation & Evaluation-core graviton factorization
...
boundary operators.  Its infinite shadow tower mirrors the expected
infinite graviton-vertex tower, while the full higher-genus modular
completion remains frontier.
\end{computation}
...
\item The line-operator category
  $\cC_\cA = \cA^!\text{-}\mathsf{mod}$ is proved independently,
