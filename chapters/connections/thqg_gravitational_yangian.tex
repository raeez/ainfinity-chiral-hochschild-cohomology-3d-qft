...
statements (Theorems \texttt{mc2-bar-intrinsic},
\texttt{collision-residue-twisting},
\texttt{collision-depth-2-ybe},
\texttt{bar-nilpotency-complete},
\texttt{convolution-d-squared-zero},
\texttt{nms-virasoro-quintic-forced},
\texttt{completed-bar-cobar-strong},
\texttt{stabilized-completion-positive},
\texttt{resonance-filtered-bar-cobar},
\texttt{bar-modular-operad},
\texttt{recursive-existence},
\texttt{kazhdan-lusztig-equivalence},
\texttt{derived-dk-affine};
Definitions \texttt{collision-filtration},
\texttt{shadow-postnikov-tower},
\texttt{yangian}, \texttt{yangian-rtt},
\texttt{holographic-modular-koszul-datum};
Corollaries \texttt{nms-betagamma-mu-vanishing},
\texttt{heisenberg-postnikov-termination},
\texttt{affine-postnikov-termination};
Lemma \texttt{degree-cutoff}) whose scope qualifiers propagate
to the proofs below. Load-bearing cross-volume citations in proof
bodies below carry the explicit ``Volume~I'' prose prefix; the
semantic content (bar-intrinsic MC element, collision residue,
depth-2 YBE, modular-operad structure) is inherited without
restatement.
\end{remark}

The single organizing identity is the MC equation
$D_\cA\Theta_\cA + \tfrac{1}{2}[\Theta_\cA,\Theta_\cA] = 0$
(Volume~I Theorem~\ref*{V1-thm:mc2-bar-intrinsic}), restricted to
...
Specialization to the gravitational kernel proceeds from the organizing
...
over the triple datum $(k,f,\mu)$. One universal MC class, one spectral
$R$-matrix, one Koszul duality, three legs of proof (MC, Koszul,
...
\begin{theorem}[Unified chiral bialgebra datum]
\label{thm:gravitational-yangian-unified-chiral-quantum-group}
\index{theorem!unified chiral quantum group}
Fix a triple datum $(k,f,\mu)$ as in
Definition~\ref{def:triple-datum}. There exists a quasi-triangular
chiral bialgebra datum
\begin{equation}\label{eq:unified-Q}
Q_\fg^{k,f,\mu}
\;=\;\bigl(
A_\fg^{k,f,\mu},\;\Delta_z,\;R(z),\;\Phi,\;
(A_\fg^{k,f,\mu})^!\bigr),
\end{equation}
unique up to spectral gauge isomorphism on the verified fibre, with
...
\item \textup{(Chiral bialgebra.)}
 ...
 A_\fg^{k,f,\mu}((z))$. A counit exists only after an augmentation
 is fixed, and an antipode is extra Hopf or Drinfeld-double
 structure, not part of the general datum.
...
\begin{proof}
\emph{Leg~1 (MC).} Applied to the boundary-reduced chiral algebra
$A_\fg^{k,f,\mu}$ constructed by BRST reduction at $(f,\mu)$, the
genus-zero universal MC class controls the classical kernel and the
associator. Theorem~\ref*{V1-thm:mc2-bar-intrinsic} gives the MC
element, the binary collision residue extracts $r(z)$, and the Arnold
relation on $\overline{\mathcal{M}}_{0,4}$ gives the CYBE constraint.
The spectral coproduct and its quasi-triangular $R$-matrix are the
separate Drinfeld/RTT input; the MC calculation rigidifies their
classical kernel and associator rather than deriving Hopf data from
the bar coalgebra alone.
...
The finite-window RTT packet in Theorem~\ref{thm:unified-chiral-QG}
carries the lacing-scaled Koszul-dual readout
\[
        Y_\hbar(\fg)^! \simeq Y_{-\hbar r_\fg}(\fg^\vee)
\]
only after the RTT/FRT packet, PBW theorem, inverse-kernel
orthogonal-complement identity, Drinfeld/RTT comparison, and completed
spectral tower have been fixed. For the classical non-simply-laced
lanes $B_n,C_n$ this is the Molev/Finkelberg--Tsymbaliuk/
Frenkel--Hernandez finite-window statement. For $F_4$ and $G_2$ it
remains conditional on the exceptional finite RTT obstruction package
isolated in Vol.~I; a lacing number alone does not supply the Yangian
Koszul-dual theorem.
\end{remark}
...
(Volume~I Theorem~\ref{V1-thm:convolution-d-squared-zero}), and the associated
...
A self-contained verification of the Arnold relation on
$\Conf_3(\C)$ runs as follows. Set $z_3 = 0$ by translation invariance. Then
...
(Volume~I Theorem~\ref{V1-thm:bar-nilpotency-complete}).
\end{remark}
...
the dg-shifted Yangian $r$-matrix. The arguments in
\S\ref{V1-subsec:collision-filtration-recovery}
(Theorem~\ref{V1-thm:collision-residue-twisting}) supply the
underlying reduction; the detailed structure and the explicit
$r$-matrices for the standard landscape follow.
...
\begin{proof}
The identification~\eqref{V1-eq:thqg-V-twisting-id} is Volume~I
...
\emph{Property~(iii).}
This is the content of Volume~I Theorem~\ref{V1-thm:collision-depth-2-ybe},
...
\partial T(z),
\end{equation}
the regular term. The $r$-matrix requires the completed
Koszul-dual branch; on the strict same-family Virasoro comparison
surface this branch is represented by $\mathrm{Vir}_{26-c}$
(Remark~\ref{V1-rem:virasoro-resonance-model}). On the corresponding
...
the bracket on $(\cA^!)^{\otimes 3}$ yields:
\begin{align}
...
independent theorem about tree-level graviton
scattering.
\end{remark}
...
clearing denominators in \eqref{V1-eq:thqg-V-cybe} with
$r_{ij}(z) = \Omega_{ij}/z$, substituting $z_{23} = z_{13} - z_{12}$,
and extracting the leading coefficient as $z_{12} \to 0$ gives the
IBR\@.
\end{corollary}
...
representation-theoretic applications below
(line-operator modules, spectral braiding, the RTT
relation), the bialgebra structure suffices; the
antipode would be needed for dualization of modules
and for the construction of a ribbon structure, neither of
which is required.

\smallskip\noindent
\emph{(ii) The name ``Yangian.''}
The term ``dg-shifted Yangian'' marks the structural
...
(Computation~\ref{comp:thqg-V-affine-yangian}). For
...
Theorem~\ref{thm:thqg-V-cybe-from-arnold}. The modular
...
\;+\;
\kappaChHodge(\cA) \cdot r_{\cA,0}^{(2)}
\;=\; 0,
\end{equation}
...
structure. Existence of such a
kernel is not asserted here; only the ambient modular dg Lie package
in which that higher-genus completion would live is isolated.
\end{remark}
...
For $\mathrm{Vir}_c$, the strict same-family representative of the
completed Koszul-dual branch is $\mathrm{Vir}_{26-c}$, with Virasoro
generators $\{L_n\}_{n \in \Z}$ satisfying
$[L_m, L_n] = (m-n)\,L_{m+n}
...
\begin{proof}
On the strict same-family Virasoro comparison surface, the completed
Koszul-dual branch of $\mathrm{Vir}_c$ is represented by
$\mathrm{Vir}_{26-c}$
\textup{(}Remark~\ref{V1-rem:virasoro-resonance-model}\textup{)}.
...
$K = \kappaChHodge(\mathrm{Vir}_c) + \kappaChHodge(\mathrm{Vir}_{26-c})
= c/2 + (26-c)/2 = 13$
for all~$c$.
...
\S\ref{V1-chap:yangians} and the gravitational language here:
...
\begin{remark}[The gravitational Yangian as a projection of $\Theta_\cA$]
...
$Q_\fg^{k,f,\mu}$ (Theorem~\ref{thm:unified-chiral-QG}),
...
$\mathrm{av}(r^{\mathrm{grav}}(z)) = \kappaChHodge(\mathrm{Vir}_c) = c/2$
...
$c$ is load-bearing and never omitted (verified
...
gravitational-Yangian collapse established above.
\end{remark}

\section{Synthesis}
\label{sec:thqgy-supplement}

\begin{remark}[Gravitational Yangian scope: Hall--Drinfeld double at K3]
\label{rem:thqgy-hall-drinfeld-scope}
The BKM-side K3 object is the K3 chiral Hall--Drinfeld double
\[
 \mathcal D_\hbar\bigl(
 \mathcal Y^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3\times E})
 \bigr)
\]
with Schiffmann--Vasserot shuffle kernel. It is not a Drinfeld
Yangian: the BKM algebra has no finite Kac--Moody Cartan datum, no
Drinfeld $J$-presentation, and imaginary simple roots whose Weyl
action is governed by the Borcherds denominator. Hence
$Y_\hbar(\mathfrak g_{\Delta_5})$ is not an object in the Drinfeld
Yangian sense. The self-mirror K3 Yangian
$Y(\mathfrak g_{K3})$ on the Mukai lattice
$\Lambda_{\mathrm{Muk}}=\mathrm{II}_{4,20}$ is a separate integrable
branch.
\end{remark}
