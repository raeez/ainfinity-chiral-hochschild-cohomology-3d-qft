...
\item The modular dg-shifted Yangian
  ...
  ambient pronilpotent completion and the formal genus-refined binary
  shadow package; the actual modular kernel and stable-graph
  Yang--Baxter equation are explicitly fenced as frontier data.
\item The gravitational Yangian for the Virasoro algebra
  (\S\ref{subsec:thqg-V-virasoro-yangian}), computing the explicit
  Virasoro binary kernel $\rgrav_{\mathrm{Vir}}(z)$ and the
  ternary shadow, whose gravitational reading gives the cubic
  graviton vertex.
...
of the shadow Postnikov obstruction tower.  The arity filtration stratifies
...
cohomological content of the shadow obstruction tower at arity $r+1$:
...
comes from the double-pole Laplace-kernel contribution (AP19:
collision residue gives zero from the double pole, but the
spectral r-matrix receives $\kappa\delta^{ij}/z$ from the
double-pole Laplace kernel $\int_0^\infty e^{-\lambda z}
\cdot\kappa\delta^{ij}\lambda\,d\lambda = \kappa\delta^{ij}/z^2$
evaluated on the Koszul dual generators via the natural pairing).
% RECTIFICATION-FLAG: The "therefore" in the original was
% misleading — r_H(z) does NOT follow from the collision residue
% being zero. It follows from the Casimir structure on the Koszul
% dual, using the double-pole Laplace kernel. Both facts (collision
% residue = 0 AND r_class = Omega/z) are correct; the logical
% connection between them via "therefore" was removed.
...
of~$z$; Volume~I, Remark~\ref{rem:three-r-matrices}):
$r^{\mathrm{coll}}(z) = (c/2)/z^3 + 2T/z$.
...
sequence of non-trivial higher-arity corrections
...
CYBE~\eqref{eq:thqg-V-cybe} then states that the three possible
...
For class~$G$: the shadow obstruction tower terminates at arity~$2$
(Corollary~\ref{cor:heisenberg-postnikov-termination}), so all
higher-depth $r$-matrices vanish.
For class~$L$: the shadow obstruction tower terminates at arity~$3$
...
For class~$C$: the shadow obstruction tower terminates at arity~$4$, and the
...
formal $\hbar$-adic series~\eqref{eq:thqg-V-pro-mc}.
\end{proof}
...
At genus~$0$, arity~$3$: the sgYBE would reduce to the classical
CYBE (three stable graphs, each a single edge connecting two
trivalent vertices).

At genus~$1$, arity~$2$: the sgYBE would give the
...
the Volume~I type-$A$ MC3 reduction theorem additionally proves the
...
comparison from Volume~I.  In type~$A$, the Volume~I type-$A$ MC3 reduction theorem additionally
proves the shifted-prefundamental-generation and pro-Weyl-recovery
...
package for $\mathfrak{so}_5$ would additionally require the
