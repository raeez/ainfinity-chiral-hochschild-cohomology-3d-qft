...
The shadow Postnikov obstruction tower
...
\begin{corollary}[Mittag--Leffler for the shadow obstruction tower; \ClaimStatusProvedHere]
...
\end{equation}
The shadow Postnikov obstruction tower stabilizes immediately:
...
shadow obstruction tower terminates at arity~$3$ because there are
...
\subsubsection{The Virasoro shadow obstruction tower through arity~$4$}
...
\end{equation}
The shadow obstruction tower data through arity~$4$, on the
...
\qquad \text{as } r \to \infty.
\end{equation}
Since the cubic source dominates the full recursion at
large~$r$ (the $\{\mathrm{Sh}_j, \mathrm{Sh}_k\}$
cross-terms contribute at lower order in~$1/c$),
the asymptotic ratio $|S_{r+1}|/|S_r| \to 6/|c|$ as
$r \to \infty$ is exact.  The prefactor
$240/|5c+22|$ is approximate; the exact prefactor receives
corrections from the quartic and higher bracket terms.
The convergence/divergence trichotomy is:
\begin{itemize}
\item For $|c| > 6$: the shadow obstruction tower converges
  geometrically.
\item For $|c| < 6$: the shadow obstruction tower diverges in
...
The divergence of the shadow obstruction tower for $|c| < 6$ in
...
arity~$10$, cubic-source approximation;
\ClaimStatusProvedHere]
...
\begin{remark}[The bosonic string as a convergent shadow obstruction tower]
...
The shadow obstruction tower converges rapidly.
...
\end{equation}
The shadow Postnikov obstruction tower is
...
shadow obstruction tower; \ClaimStatusProvedHere]
...
multi-generator shadow obstruction tower at arity~$6$ and above,
...
affect the \emph{extended} shadow obstruction tower (including
...
(Volume~I, Theorem~\ref{thm:propagator-variance}).
\end{computation}
...
The shadow obstruction tower converges rapidly.  The Koszul dual
...
even in the non-perturbative regime, the shadow obstruction tower
...
Costello--Li programme \cite{Costello-Li19,Costello-Gaiotto18}.
...
\item The boundary VOA is affine $\mathfrak{gl}_N$ at level $k = 1$
  ...
  = (N^2{-}1)(N{+}1)/(2N) + 1$.
\item The holographic R-matrix is $r(z) = \Omega/z$
  \textup{(}classical Yang--Baxter\textup{)}.
\item The genus expansion $F_g = \kappa\cdot\lambda_g^{\mathrm{FP}}$
  computes twisted amplitudes.
\item The anomaly matching
  $\kappa_{\mathrm{eff}} = \kappa(\mathrm{matter})
  + \kappa(\mathrm{ghost}) = 0$
  is a consistency check \textup{(}AP29: $\kappa_{\mathrm{eff}}$
  is a system invariant, not an algebra invariant\textup{)}.
\end{enumerate}
...
\item In the large-$N$ limit, the shadow connection degenerates
  to the Airy connection: the ABJM partition function is
...
equation matches the $\mathrm{SU}(2)$ Seiberg--Witten
...
The instanton coefficients $F_k$ of the SW prepotential
are Taylor coefficients of the flat sections of the
shadow connection.
Computational verification:
