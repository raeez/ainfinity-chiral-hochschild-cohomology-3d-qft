...
\item $\cA$ is cyclically admissible
 (Definition~\ref{V1-def:cyclically-admissible}): conformal grading, exhaustive
...
The collision residue is the genus-$0$, degree-$2$ projection of the
...
= 0.
\end{equation}
This is the genus-$0$, degree-$3$ MC equation, evaluated on the
...
(Volume~I, Theorem~\ref{V1-thm:convolution-d-squared-zero}).

The element $\Theta_\cA$ decomposes by genus and degree:
...
and degree $r$ (number of inputs in the graph sum). The finite-order
...
$r(z)$ & $\Res^{\mathrm{coll}}_{0,2}(\Theta_\cA)$
 & Genus-$0$, degree-$2$ boundary residue \\[2pt]
...
(Volume~I, Theorem~\ref{V1-thm:shadow-homotopy-invariance}),
...
(Volume~I, Theorem~\ref{V1-thm:collision-residue-twisting}).
...
Definition~\ref{V1-def:shadow-metric}), the shadow connection
is the logarithmic
...
(Volume~I, Definition~\ref{V1-def:shadow-depth-classification}) are:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\small
\begin{tabular}{clll}
...
\hline
G & Gaussian & $2$ & Heisenberg, free fermion, lattice free-field branches \\
...
\item \textbf{Modular characteristic} ($r=2$):
 $\kappaChHodge(\cA) := \Theta_\cA^{\le 2}|_{\text{scalar}}$, the
...
follow from the low-degree shadow--obstruction comparison
(Volume~I, Proposition~\ref{V1-prop:shadow-formality-low-degree}).
\end{proof}
...
\end{equation}
from the complementarity theorem (Volume~I, Theorem~\ref{V1-thm:quantum-complementarity-main})
and its shifted-symplectic upgrade
(Volume~I, Theorem~\ref{V1-thm:shifted-symplectic-complementarity}).
...
$\Theta_\cA$
 ...
 & All-genus, all-degree bulk coupling \\
$\nabla^{\mathrm{hol}}_{g,n}$
 & $d - \operatorname{Sh}_{g,n}(\Theta_\cA)$
...
(Volume~I, Theorem~\ref{V1-thm:ds-platonic-functor} and
Corollary~\ref{V1-cor:ds-theta-descent}).
\end{proof}
...
by hypothesis~\textup{(iii)}), and assume that the graph forms
obtained from the renormalized logarithmic OPE kernels extend to the
Fulton--MacPherson boundary faces with scaling degree below the face
codimension and with vanishing anomaly class.
For every $g \ge 0$ and $n \ge 1$, the genus-$g$ amplitude
$\langle \Theta_\cA^{\otimes n} \rangle_g$ converges absolutely.
The Fulton--MacPherson compactification supplies the boundary
stratification on which the renormalized logarithmic forms extend; the
degree-cutoff lemma (Volume~I, Lemma~\ref{V1-lem:degree-cutoff}) bounds
the contributing graphs at each genus.
\end{theorem}

\begin{proof}
The amplitude $\langle \Theta_\cA^{\otimes n} \rangle_g$ is a sum over
stable graphs $\Gamma$ of genus $g$ with $n$ external legs. The degree-cutoff
...
Each graph amplitude is computed by integration of the renormalized
logarithmic form on the compact smooth manifold with corners
$\overline{C}_{|\Gamma|}(X)$. By the boundary-extension and
scaling-degree hypothesis, this form is integrable on every boundary
face. Hence each graph integral converges. The sum over graphs is
finite.
...
Construction~\ref{V1-const:vol1-modular-tangent-complex}). Its
$k$-ary bracket $\ell_k$ is the linearization of the MC equation
at degree $k$. The shadow--obstruction comparison
(Volume~I, Proposition~\ref{V1-prop:shadow-formality-low-degree})
identifies the shadow obstruction tower with the $\Linf$ obstruction
tower at degrees $2$, $3$, $4$. At degree $r > r_{\max}$, the
obstruction class $o^{(r+1)}(\cA) = 0$ implies that $\ell_r$ is
homotopically trivial; at degree $r \le r_{\max}$, $o^{(r)}(\cA) \ne 0$
implies $\ell_r$ is nontrivial.

The four archetype classes (Definition~\ref{V1-def:thqg-shadow-archetype})
...
provides the splitting~\eqref{V1-eq:thqg-lagrangian-splitting} at the level
of chain complexes. The shifted-symplectic upgrade
(Volume~I, Theorem~\ref{V1-thm:shifted-symplectic-complementarity}) equips
...
is duality-constrained: $\kappaChHodge(\cA) + \kappaChHodge(\cA^!) = 0$ for Kac--Moody and free-field algebras, while $\kappaChHodge(\cA) + \kappaChHodge(\cA^!) = K(\fg)$ (a nonzero constant) for $\mathcal{W}$-algebras (Volume~I, Theorem~D).
...
$\kappaChHodge(\cA) = \operatorname{str}(\id|_{V^{(1)}(\cA)})$ and
$\kappaChHodge(\cA^!) = \operatorname{str}(\id|_{V^{(1)}(\cA^!)})$.
The Verdier duality reverses the grading on bar cohomology.
For Kac--Moody and free-field algebras this produces the
anti-symmetry $\kappaChHodge(\cA^!) = -\kappaChHodge(\cA)$; for $\mathcal{W}$-algebras
the complementarity sum $\kappaChHodge(\cA) + \kappaChHodge(\cA^!)$ equals a nonzero
constant $K(\fg)$ determined by the Lie-algebraic data.

For $\cA = V_k(\mathfrak{g})$:
$\cA^!\simeq
\operatorname{CE}^{\mathrm{ch}}\!\bigl(\widehat{\mathfrak g}_{-k-2h^\vee}\bigr)$,
and the reflected current level is governed by the Feigin--Frenkel
involution $k \mapsto -k - 2h^\vee$. The scalar current shadows at
level $k$ and $-k - 2h^\vee$ are related by the corresponding
...
(Volume~I, Theorem~\ref{V1-thm:collision-residue-twisting}). The
...
$\nabla^{\mathrm{hol}}_{g,n} = d - \operatorname{Sh}_{g,n}(\Theta_\cA)$
at genus~$0$, degree~$2$ reproduce the Weinberg--BMS leading and subleading
soft graviton theorems. The all-degree master equation
\begin{equation}% label removed: eq:thqg-all-degree
\nabla_{\mathrm{H}}(\operatorname{Sh}_r) + o^{(r)} = 0
\end{equation}
gives nonlinear soft theorems at each degree $r \ge 3$. The shadow depth
...
degree~$2$ ($n=3$ with one soft insertion), the Ward identity is
...
degree~$3$, the subleading term of the OPE gives the subleading
soft theorem.

The all-degree master equation~\eqref{eq:thqg-all-degree} is the
shadow projection of the MC equation to each degree level. The
horizontal transport $\nabla_{\mathrm{H}}$ is the covariant
derivative in the shadow connection, and $o^{(r)}$ is the
obstruction class at degree $r$. When $r > r_{\max}$, the
...
(Volume~I, Construction~\ref{V1-const:vol1-genus-spectral-sequence}) has
$E_1$ page with $p=0$ (tree), $p=1$ (one-loop), $p \ge 2$ (higher genus).
Convergence of the two-dimensional tower (genus $\times$ degree) is proved
by MC5 (Volume~I, Theorem~\ref{V1-thm:inductive-genus-determination}).
\end{proof}

\begin{remark}[MC5 citation scope guard]
\label{rem:mc5-citation-scope-guard}
The two MC5 citations in this chapter
(Theorem~\ref*{V1-thm:inductive-genus-determination} above and
Theorem~\ref{thm:general-hs-sewing} in
Remark~\ref*{rem:thqg-nishinaka-vicedo}) invoke only the
\emph{analytic HS-sewing lane} of the 5-clause MC5 package
(existence, associativity, cyclic symmetry, modular invariance,
convergence). Volume~I proves this analytic lane unconditionally at
all genera, together with genus-$0$ algebraic BRST/bar comparison,
$D^{\mathrm{co}}$-level BV~$=$~bar for all shadow classes~G/L/C/M,
and genuswise chain-level BV/BRST/bar in the weight-completed
category (Proposition~\ref{prop:bv-bar-class-m-weight-completed}). The
present chapter does \emph{not} invoke the genuswise chain-level
BV/BRST/bar identification in the direct-sum category for class~M
(which is false on the raw direct sum at generic central charge).
Downstream readers should read
``by~MC5'' here as ``by the analytic HS-sewing lane of~MC5'',
consistent with the Vol~II bridge-table entry for the W-algebras row.
\end{remark}
...
The algorithm terminates at each finite order: at degree $\le r$, the
truncated MC element $\Theta_\cA^{\le r}$ is computable from the OPE
data of $\cA$ through degree $r$, using finitely many graph sums.
The all-degree convergence $\Theta_\cA = \varprojlim_r \Theta_\cA^{\le r}$
is the recursive existence theorem
(Volume~I, Theorem~\ref{V1-thm:recursive-existence}).
\end{proof}
...
For the Virasoro family, the same-family strict comparison involution
is represented by $\mathrm{Vir}_{26-c}$ on the completed Virasoro
Koszul surface. At $c = 26$:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item The dual partner $\mathrm{Vir}_0$ lies on the uncurved scalar
 locus: $\kappaChHodge(\mathrm{Vir}_0) = 0$, hence its scalar class vanishes
 and its scalar genus package satisfies
 $F_g(\mathrm{Vir}_0) = 0$ for all $g \geq 1$.
\item The depth-zero resonance shadow
 (Volume~I, Remark~\ref{V1-rem:virasoro-resonance-model}) therefore
 saturates only at the scalar level; the resonance-filtered bar-cobar
 is not proved to collapse at higher degree from this scalar
 cancellation alone.
...
\begin{proof}
Volume~I proves the completed Verdier--Koszul branch and identifies
$\mathrm{Vir}_{26-c}$ as the same-family strict representative on the
Virasoro comparison surface. The self-dual point is $c = 13$ (where
$\kappaChHodge(\mathrm{Vir}_{13}) = 13/2$), while the physically
...
(ii)~$\kappaChHodge(\mathrm{Vir}_0) = 0$, so the scalar class of the dual
...
collapse of the higher-degree resonance-filtered bar-cobar.
...
(Volume~I, Theorem~\ref{V1-thm:shadow-connection}(vi)).
\end{proof}


\subsection{G10: Fredholm partition functions on the Gaussian sewing lane}
% label removed: subsec:thqg-g10-statement

\begin{theorem}[\textbf{G10}: Fredholm partition functions on the
Gaussian sewing lane; \ClaimStatusConditional]
% label removed: thm:thqg-g10-fredholm
\index{Fredholm determinant!partition function|textbf}
\index{partition function!Fredholm}
Let $\cA$ be a gravitational input whose sewing envelope is
Hilbert--Schmidt. Let \(K_g(\cA)\) be the induced one-particle sewing
operator whenever the one-particle projection exists, and let
\(\mathcal O^{\mathrm{Gaus}}_g(\cA)=\{C^{(r)}_g(\cA)\}_{r\ge3}\)
denote the connected higher-cumulant obstruction of the completed
sewing trace, where \(C^{(r)}_g\) is the connected \(r\)-point sewing
cumulant. If \(K_g(\cA)\) is trace class and
\(\mathcal O^{\mathrm{Gaus}}_g(\cA)=0\), then the genus-$g$ partition
function is
\begin{equation}% label removed: eq:thqg-fredholm-det
Z_g(\cA) \;=\; \Det\bigl(1 - K_g(\cA)\bigr),
\end{equation}
where \(K_g(\cA)\) is the trace-class sewing kernel built from the
Bergman kernel on the genus-$g$ surface. For the Heisenberg algebra,
the one-particle Bergman reduction
(Volume~I, Theorem~\ref{V1-thm:heisenberg-one-particle-sewing})
collapses this to a single operator determinant and
\(\mathcal O^{\mathrm{Gaus}}_g=0\). The HS-sewing criterion
(Volume~I, Theorem~\ref{thm:general-hs-sewing}) supplies trace-class
control when its hypotheses hold. For Virasoro and other higher-shadow
examples, the obstruction is exactly the nonzero higher connected
cumulant package \(\mathcal O^{\mathrm{Gaus}}_g\), equivalently the
contact corrections not represented by a one-particle determinant.
\end{theorem}
...
Bergman kernel $B(z,w)$ on $\Sigma_g$. If the completed trace
factorizes through a Gaussian one-particle propagator, all connected
cumulants \(C^{(r)}_g\) for \(r\ge3\) vanish, the total one-particle
sewing kernel is \(K_g=\prod_{i=1}^{g}S_i\), and Wick's determinant
identity gives \(Z_g(\cA)=\Det(1-K_g)\).
...
The HS-sewing criterion (Volume~I, Theorem~\ref{thm:general-hs-sewing})
shows trace-class control under its stated growth hypotheses. The
Gaussian factorization is the separate vanishing statement
\(\mathcal O^{\mathrm{Gaus}}_g=0\). Without it the sewing trace
contains higher shadow/contact cumulants and is not represented by a
single Fredholm determinant.
\end{proof}
...
\item the identity $D_\cA^2 = 0$, at the convolution level
 ...
 (Volume~I, Theorem~\ref{V1-thm:ambient-d-squared-zero}).
\end{enumerate}
\end{theorem}
...
\hline
$\Theta_\cA^{\le 2}$ & $\kappaChHodge(\cA)$ (modular characteristic)
 & G1, G7, G10 \\
...
G1: The degree-cutoff lemma bounds the number of contributing graphs
at each $(g,n)$; finiteness follows from compactness of
$\overline{C}_n(X)$.

G2: The shadow depth is read off from the degree filtration of
$\Theta_\cA$.
...
G5: The genus-$0$, degree-$2$ restriction gives $r(z)$; the
genus-$0$, degree-$3$ restriction gives the CYBE.

G6: The all-degree master equation is the degree-$r$ restriction
of the MC equation.
...
G10: On the HS/Gaussian sewing lane, the genus-$g$ partition function
is the Fredholm determinant of the sewing kernel built from the
genus-$g$ restriction of $\Theta_\cA$; outside that lane the sewing
trace carries higher-shadow corrections.
\end{proof}
...
% are a degree decomposition of the MC equation; the bootstrap is
...
\begin{remark}[Independence of the degree and genus projections]
% label removed: rem:thqg-degree-genus-independence
...
\Theta_\cA] = 0$:
\begin{enumerate}[label=\textup{(\alph*)}]
\item the \emph{degree filtration}, which produces the soft graviton
...
\end{tabular}
\end{center}
The construction remains conjectural because of the ambient master equation:
while
...
$r_{M2}(z)$
 & Collision residue of $\Theta_{M2}$ at degree $2$;
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappaChHodge(A_{M2,\infty})$: the modular characteristic is
 not determined by the present DDCA data (the DDCA is not a tensor
 product of standard chiral algebras, and the self-sewing trace on the
 full cyclic pairing of $A(K)$ has not been computed).
...
\begin{proof}
The modular characteristic $\kappaChHodge(A_{M2,\infty})$ is not determined
by the present DDCA data:
...
\ClaimStatusProvedHere]
\label{prop:thqg-ks-koszul-hodge}
...
\begin{theorem}[B-model line category and Morita algebra;
\ClaimStatusProvedElsewhere]
\label{thm:thqg-ks-lines}
\index{Kodaira--Spencer!line operators}
\index{D-brane!Morita module}
The line-operator category of the B-model on $Y$ is equivalent to the
derived category of coherent sheaves. If $G_Y$ is a split-generator and
\[
 E_Y:=\RHom_Y(G_Y,G_Y),
\]
then
...
\Perf(E_Y).
\end{equation}
The functor sends $F\in D^b\mathrm{Coh}(Y)$ to
$\RHom_Y(G_Y,F)$. The endomorphism algebra of the structure sheaf is
$\RHom_Y(\cO_Y,\cO_Y)\simeq H^\bullet(Y,\cO_Y)$; it is a Morita model
for $D^b\mathrm{Coh}(Y)$ only when $\cO_Y$ split-generates. If a dg
Morita equivalence
\[
 \Psi_Y\colon V_{\partial,\mathrm{KS}}^!(Y)\longrightarrow E_Y
\]
is fixed, then the same category is
$V_{\partial,\mathrm{KS}}^!(Y)\text{-}\mathsf{mod}_{\mathrm{perf}}$.
...
with $\Perf(E_Y)$ is Bondal--Van den Bergh derived Morita theory for a
split-generator. The algebra $H^\bullet(Y,\cO_Y)$ is not the
endomorphism algebra of a generator in general. The
$V^!_{\partial,\mathrm{KS}}$-module notation is valid only after the
Morita equivalence $\Psi_Y$ has been chosen.
\end{proof}
...
\begin{theorem}[BTT and the two Kodaira--Spencer scalars;
\ClaimStatusProvedElsewhere]
\label{thm:thqg-ks-koszul-btt}
\index{Kodaira--Spencer!Koszul concentration}
\index{Bogomolov--Tian--Todorov theorem}
For a compact Calabi--Yau threefold $Y$:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The dg Lie algebra controlling complex-structure deformations is
 homotopy abelian, so the Kuranishi obstruction map vanishes.
\item Chiral Koszul concentration and Gaussian shadow depth for
 $V_{\partial,\mathrm{KS}}(Y)$ are extra hypotheses on the
 factorization-algebra model, not consequences of BTT.
\item The weight-one Hodge supertrace is
 $\kappa_{\mathrm{KS}}^{(1)}(Y)=h^{1,1}(Y)-h^{2,1}(Y)$.
\item The BCOV one-loop scalar row is
 $\kappa_{\mathrm{BCOV}}(Y)=\chi(Y)/24$.
\end{enumerate}
\end{theorem}

\begin{proof}
(i)~Tian--Todorov unobstructedness, in the
Barannikov--Kontsevich form, follows from the
$\partial\bar\partial$-lemma for polyvector fields. This is a
statement about the Maurer--Cartan deformation functor.

(ii)~The chiral bar complex contains vertex-algebraic collision data.
A Gaussian truncation is therefore additional factorization data or a
chosen scalar projection.

(iii)~The weight-$1$ space is $H^\bullet(Y, T_Y)$ with supertrace
$\sum_q (-1)^q h^{1,q} = -h^{1,1} + h^{2,1}$ (using $h^{1,0} =
h^{1,3} = 0$ for a simply connected CY). The normalization
convention gives $\kappa_{\mathrm{KS}}^{(1)} = h^{1,1}-h^{2,1}$.

(iv)~The BCOV row is the one-loop scalar coefficient of
Bershadsky--Cecotti--Ooguri--Vafa and Costello--Li; it is
$\chi(Y)/24$ and is not the weight-one Hodge supertrace.
\end{proof}
...
\begin{theorem}[Kodaira--Spencer holographic datum with Morita and scalar comparison;
\ClaimStatusProvedHere]
% label removed: thm:thqg-ks-datum
\index{holographic modular Koszul datum!Kodaira--Spencer}
For a compact Calabi--Yau threefold $Y$ with a split-generator $G_Y$ of
$D^b\mathrm{Coh}(Y)$, put $E_Y=\RHom_Y(G_Y,G_Y)$. Equip the
Kodaira--Spencer factorization algebra with a scalar sewing projection
and, when the line category is written as
$V_{\partial,\mathrm{KS}}^!(Y)$-modules, a Morita equivalence
$\Psi_Y\colon V_{\partial,\mathrm{KS}}^!(Y)\to E_Y$. Then the
holographic modular Koszul datum is
\begin{equation}% label removed: eq:thqg-ks-datum
\mathcal{H}(\mathrm{KS}_Y)
= \bigl(
V_{\partial,\mathrm{KS}},\;
V_{\partial,\mathrm{KS}}^!,\;
D^b\mathrm{Coh}(Y)\simeq\Perf(E_Y),\;
r_{\mathrm{KS}}(z),\;
\Theta_{\mathrm{KS}},\;
...
$\mathcal{C} = D^b\mathrm{Coh}(Y)\simeq\Perf(E_Y)$
 & Morita category of a split-generator $G_Y$ \\
$r_{\mathrm{KS}}(z) = \Omega_{\PV}^{\sn}/z$
 & Spectral kernel from Schouten--Nijenhuis Casimir \\
$\Theta_{\mathrm{KS}}$
 & Bar-intrinsic MC element; Gaussian truncation only after an
 explicit scalar sewing projection \\
$\nabla^{\mathrm{hol}}_{\mathrm{KS}} = \bar\partial + \hbar\,\partial_\Omega$
 & Gauss--Manin projection of the shadow connection at genus $0$ \\
\hline
\end{tabular}
\end{center}
\end{theorem}
...
\item $\Theta_{\mathrm{KS}}$: the bar-intrinsic construction applied
 to $V_{\partial,\mathrm{KS}}$; no Gaussian truncation is asserted
 before the scalar sewing projection is fixed.
...
\begin{proposition}[BCOV degree-zero free energy;
\ClaimStatusProvedElsewhere]
% label removed: prop:thqg-ks-free-energy
For a compact Calabi--Yau threefold $Y$, the degree-zero part of the
BCOV--Gromov--Witten free energy is
\begin{equation}% label removed: eq:thqg-ks-f1
F_1^{\mathrm{BCOV,const}}(Y)
= \frac{\chi(Y)}{576}
= \frac{\kappa_{\mathrm{BCOV}}(Y)}{24},
\qquad
\kappa_{\mathrm{BCOV}}(Y)=\frac{\chi(Y)}{24},
\end{equation}
and, for $g\ge 2$,
\begin{equation}% label removed: eq:thqg-ks-free-energy
F_g^{\mathrm{BCOV,const}}(Y)
= (-1)^g\,\frac{\chi(Y)}{2}\,
\frac{|B_{2g}|}{2g}\,
\frac{|B_{2g-2}|}{2g-2}\,
\frac{1}{(2g-2)!},
...
Equivalently, for $g\ge2$,
\begin{equation}% label removed: eq:thqg-ks-f1
F_g^{\mathrm{BCOV,const}}(Y)
= (-1)^g\,12\,\kappa_{\mathrm{BCOV}}(Y)\,
\frac{|B_{2g}|\,|B_{2g-2}|}
{2g(2g-2)(2g-2)!}.
\end{equation}
The coefficient for $g\ge2$ is the unmarked Calabi--Yau
degree-zero Hodge integral, not the marked one-point
Faber--Pandharipande number $\lambda_g^{\mathrm{FP}}$.
\end{proposition}

\begin{proof}
For $g\ge2$, the degree-zero stable-map component is
$\overline{\mathcal M}_g\times Y$.  The Calabi--Yau obstruction
theory contributes
\[
(-1)^g\frac{\chi(Y)}{2}
\int_{\overline{\mathcal M}_g}\lambda_{g-1}^3
\]
to the genus-$g$ invariant.  Faber--Pandharipande prove
\[
\int_{\overline{\mathcal M}_g}\lambda_{g-1}^3
=
\frac{|B_{2g}|}{2g}\,
...
\frac{1}{(2g-2)!}.
\]
Bershadsky--Cecotti--Ooguri--Vafa give the genus-one constant
term $\chi(Y)/576$.  These two formulas are precisely the stated
BCOV degree-zero coefficients~\cite{BCOV94,FP00}.
\end{proof}
...
\begin{corollary}[Mirror anti-invariance of the KS Hodge row;
...
$h^{2,1}(Y) = h^{1,1}(\check{Y})$, then the weight-one
Kodaira--Spencer Hodge row is anti-invariant:
\begin{equation}% label removed: eq:thqg-ks-mirror
\kappa_{\mathrm{KS}}^{(1)}(Y) + \kappa_{\mathrm{KS}}^{(1)}(\check{Y}) = 0,
\qquad
\mathcal{H}(\mathrm{KS}_Y)^! \simeq \mathcal{H}(\mathrm{KS}_{\check{Y}}).
\end{equation}
The second equivalence is conditional on the chosen homological mirror
symmetry comparison.
\end{corollary}

\begin{proof}
$\kappa_{\mathrm{KS}}^{(1)}(Y) = h^{1,1}(Y) - h^{2,1}(Y)$ and
$\kappa_{\mathrm{KS}}^{(1)}(\check{Y}) =
h^{1,1}(\check{Y}) - h^{2,1}(\check{Y})
= h^{2,1}(Y) - h^{1,1}(Y)$.
Homological mirror symmetry supplies the categorical comparison when
the required enhancement is available.
\end{proof}
...
\hline
$\kappa_{\mathrm{KS}}^{(1)}$ & $1 - 101 = -100$ \\
$\kappa_{\mathrm{BCOV}}$ & $-200/24=-25/3$ \\
$F_1^{\mathrm{const}}$ & $\kappa_{\mathrm{BCOV}}/24=-25/72$ \\
$F_2^{\mathrm{const}}$ & $-100/2880=-5/144$ \\
Shadow depth & not Gaussian before scalar projection \\
...
\textbf{CY$_3$} & $(h^{1,1}, h^{2,1})$
 & $\kappa_{\mathrm{KS}}^{(1)}$ & $F_1^{\mathrm{const}}$ & \textbf{Line category} \\
\hline
Quintic & $(1, 101)$ & $-100$ & $-25/72$ & $D^b\mathrm{Coh}(Q)$ \\
Mirror quintic & $(101, 1)$ & $+100$ & $25/72$ & $D^b\mathrm{Coh}(\check{Q})$ \\
$K3 \times T^2$ & $(21, 21)$ & $0$ & $0$ & $D^b\mathrm{Coh}(K3 \times T^2)$ \\
Rigid CY ($h^{2,1} = 0$) & $(h^{1,1}, 0)$ & $h^{1,1}$ & $h^{1,1}/288$ & $D^b\mathrm{Coh}(Y)$ \\
...
$\kappa_{\mathrm{KS}}^{(1)} = 0$. This is an anti-invariant Hodge
row, not the Virasoro self-dual point.
The holographic datum $\mathcal{H}(\mathrm{KS}_Y)$ is self-conjugate
under the mirror comparison when that comparison is installed. The
BCOV constant-map scalar also vanishes when $\chi(Y)=0$; the
nonconstant BCOV amplitudes and BPS sectors need not vanish.
\end{remark}

\begin{conjecture}[K3\texorpdfstring{$\times T^2$}{x T2} as the BPS gravity face]
\label{conj:k3t2-bps-gravity-face}
The self-mirror entry $K3 \times T^2$ is also the CY$_3$ point where
the present 3d holomorphic-topological framework is expected to meet
the K3 Borcherds algebra. In the singular attractor limit of twisted
11d supergravity on $K3 \times T^2$, the BPS algebra should be the
$\Delta_5$ Borcherds--Kac--Moody superalgebra, so the bulk-boundary
datum of the 3d HT theory should recover the same object that appears
in the $(\Phi_{10}^{\mathrm{un}})^{-1}$ counting formula and in the K3 automorphic
denominator. The point is not to identify
$\kappa_{\mathrm{KS}}^{(1)} = 0$ with a Borcherds weight. The two numbers
measure different sectors: $\kappa_{\mathrm{KS}}^{(1)}$ belongs to the
Kodaira--Spencer boundary algebra, while the Borcherds weight belongs
to the BPS denominator sector. The claim is that these sectors meet in
one CY$_3$ background and that the Vol~II HT datum is the boundary
presentation of that package. The background sources are the
Dijkgraaf--Moore--Verlinde--Verlinde symmetric-product count, the
Gritsenko--Nikulin $\Delta_5$ denominator, and the
Maldacena--Moore--Strominger attractor/BPS interpretation.
% DVV and Maldacena--Moore--Strominger equation anchors.
% once the dedicated K3 x T^2 BPS subsection is installed.
\end{conjecture}
...
(Theorem~\ref{V1-thm:platonic-completion}):
...
\item All positive-weight towers (MC4$^+$): $\rho = 0$.
 ...
 and RTT algebras (Volume~I, Theorem~\ref{V1-thm:stabilized-completion-positive}).
...
\item \textbf{MC4$^+$} (positive towers): algebras with strictly
 ...
 (Volume~I, Theorem~\ref{V1-thm:stabilized-completion-positive}).
...
\item \textbf{MC4$^0$} (resonant towers): algebras with weight-$0$
 ...
 (Volume~I, Theorem~\ref{V1-thm:resonance-filtered-bar-cobar}).
...
(Volume~I, Theorem~\ref{V1-thm:completed-bar-cobar-strong}) resolves
...
The obstruction to constructing $Q_{\mathrm{HT}}$ is the lifting
question: whether the PVA deformation extends to the full bar--cobar
level without an uncontrolled higher obstruction. For Gaussian
algebras (class G), the higher transfer is absent. For Lie-type
algebras (class L), the finite cubic transfer is governed by Lie
conformal Jacobi, and the quantized monodromy is the
Etingof--Kazhdan problem.
\end{remark}
...
Vol~I (Remark~\ref*{V1-rem:ds-geometric-localization-principle}).
...
\begin{remark}[Shadow Langlands hierarchy]
\label{rem:thqg-shadow-langlands}
\index{shadow Langlands hierarchy}
\index{oper!shadow Langlands hierarchy}
For multi-generator algebras, the shadow connection
$\nabla^{\mathrm{hol}}$ acquires matrix rank equal to the number
of independent primary lines. For $\cW_3$, the shadow metric on
the $(T,W)$-plane is a $2 \times 2$ system whose off-diagonal
cross-channel coupling $Q_{TW}$ is generically nonzero, so the
connection does not split into rank-$1$ factors: the shadow oper
lies genuinely beyond the Eisenstein locus. The splitting field
is $\bQ(\sqrt{33})$, and the Galois group
$\operatorname{Gal}(\bQ(\sqrt{33})/\bQ) \cong \bZ/2$
governs the monodromy. The Feigin--Frenkel involution
$k \mapsto -k - 6$ acts by matrix transpose, trivializing
the cross-channel at the self-dual level. This is the shadow
Langlands hierarchy
(Volume~I, Proposition~\ref{V1-prop:shadow-langlands-hierarchy}):
the chain $\mathrm{Vir} \to \cW_3 \to \cW_N \to \cW_{1+\infty}$
produces shadow opers of rank $N{-}1$, lying genuinely beyond
Eisenstein for $N \ge 3$.
\end{remark}
...
This is Vol~I Theorem~\ref{V1-thm:platonic-adjunction}
(adjunction) and Corollary~\ref{V1-cor:envelope-universal-mc}
(universal MC class).
\end{theorem}
...
canonical (independent of the gauge choice at degree $3$).
\end{theorem}

\begin{proof}
The MC equation at degree $3$ reads
...
shadows factorize:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappaChHodge(L) = \kappaChHodge(L_1) + \kappaChHodge(L_2)$ (additivity).
...
\end{equation}
the degree-$\le r$ truncation of the MC element of the factorization
...
\begin{proof}
(i)~The MC element at degree $r$ is a sum over graphs of genus $\le g$
...
Gravitational anomaly
 & $\kappaChHodge(\cA)$ (modular characteristic) \\
UV finiteness
 & Degree-cutoff lemma + FM compactification \\
Genus-$g$ partition function
 & sewing trace; Fredholm determinant on the HS/Gaussian lane \\
...
\item \textbf{Theorem D} (modular characteristic): the first
 cumulant is $\kappaChHodge(\cA)$.
\item \textbf{Theorem H} (Hochschild, ordered bar, at generic level): the chiral
 Hochschild cohomology $\ChirHoch^\bullet(\cA)=H^\bullet(\mathrm{CoDer}(B^{\mathrm{ord}}(\cA)))$
 is polynomial in the cumulants and concentrated in $\{0,1,2\}$ on the ordered
 bar (Vol~I Theorem~H proper). The critical level $k = -h^\vee$ is excluded
 because $\dim \ChirHoch^0$ can be infinite there (see Vol~I
 Remark~\ref*{rem:critical-level-lie-vs-chirhoch}). Symmetric-bar concentration
 descends via the averaging corollary on classes $\mathsf{G}/\mathsf{L}$ at
 generic parameter; the $E_1$-only and class-$\mathsf{M}$ non-generic regimes
 are conjectural in Vol~I.
\end{enumerate}
...
\item On the strict same-family Virasoro comparison surface, the
 completed Koszul-dual branch is represented by
 $\mathrm{Vir}_{26-c}$. Self-duality occurs at
 $c = 13$, \textbf{not} $c = 26$. The critical string value $c = 26$ is
 the point where the same-family representative is $\mathrm{Vir}_0$
 on the uncurved scalar locus, not a trivial Virasoro algebra.
...
\begin{remark}[Twisted holography at K3: scalar $\Delta_5$ shadow]
\label{rem:thqg-K3-1loop-origin}
At $K3\times T^2$ the proved automorphic datum is the scalar
Borcherds identity $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ in the
unnormalised Igusa convention, with
$\kappa_{\mathrm{BKM}}(\mathfrak g_{\Delta_5})
=c_{\phi_{0,1}^{K3}}(0)/2=5$ in the
Gritsenko--Nikulin normalisation. Interpreting this scalar as the
chain-level output of twisted $11$D supergravity requires the
Hall--Borcherds gravity-line comparison of
Conjecture~\ref{conj:gravity-line-hall-borcherds-comparison}.
\end{remark}

\section{The K3 scalar holographic shadow}
\label{sec:thqg-allloop-bulk-boundary}

The Heegner pattern and the K3 elliptic genus determine an
automorphic scalar. The stronger claim that the twisted-holographic
bulk factorisation algebra has $\mathbf H_{\Delta_5}$ as its
chain-level boundary is conditional on the Hall--Borcherds
gravity-line comparison.

\begin{theorem}[K3$\times E$ scalar holographic shadow]
\label{thm:thqg-allloop-bulk-boundary-duality}
\index{twisted holography!$\Phi_{10}$ scalar shadow|textbf}
\index{bulk--boundary duality!conditional Hall--Borcherds comparison}
\index{Igusa cusp form!K3 scalar shadow}
Let $Z\in\mathfrak H_2$. The K3 elliptic-genus Borcherds lift gives
\begin{equation}\label{eq:thqg-allloop-zbulk}
\Phi_{10}^{\mathrm{un}}(Z)
\;=\;
\Delta_5(Z)^2,
\end{equation}
and the second-quantised K3 BPS scalar is
\begin{equation}\label{eq:thqg-allloop-zboundary}
 ...
 \Delta_5(Z)^{-2}.
\end{equation}
If Conjecture~\ref{conj:gravity-line-hall-borcherds-comparison}
holds, then this scalar is the character of the Hall--Borcherds
boundary object on the Vol~II gravity line:
\begin{equation}\label{eq:thqg-allloop-bulk-boundary}
\mathrm{ch}\bigl(\mathbf H_{\Delta_5}\text{-Fock}\bigr)
\;=\;
Z^{K3\times E}_{\mathrm{BPS}}(Z).
\end{equation}
The unconditional content of the theorem is the scalar
automorphic/BPS identity; the chain-level boundary interpretation is
conditional on that conjecture.
\end{theorem}

\begin{proof}
The identity $\Phi_{10}^{\mathrm{un}}=\Delta_5^2$ is the
Gritsenko--Nikulin square-root identity for the K3 Borcherds product
\cite{GritsenkoNikulin1998}; Borcherds' singular-theta lift gives the
same automorphic product from the Lorentzian lattice construction
\cite{Borcherds1998}. Dijkgraaf--Moore--Verlinde--Verlinde identify
the second-quantised K3 elliptic-genus index with
$\bigl(\Phi_{10}^{\mathrm{un}}\bigr)^{-1}$ in this convention
\cite{DijkgraafMooreVerlindeVerlinde1997}. The last display is exactly
the extra assertion supplied by
Conjecture~\ref{conj:gravity-line-hall-borcherds-comparison}: without
that comparison the character equality remains a scalar BPS identity.
\end{proof}

\begin{remark}[What is \emph{not} proved: DVV and Maloney--Witten]
\label{rem:thqg-allloop-scope}
\index{scope!DVV vs.\ Maloney--Witten vs.\ twisted holography}
Theorem~\ref{thm:thqg-allloop-bulk-boundary-duality} proves only the
scalar automorphic/BPS identity. It does \emph{not} prove:
\begin{enumerate}[label=\textup{(\alph*)}]
\item any dynamical-metric partition function of untwisted $3$d
 quantum gravity on $\mathrm{AdS}_3$ or
 $\mathrm{AdS}_3 \times S^2$ equals
 $(\Phi_{10}^{\mathrm{un}})^{-1}$; no such path
 integral is mathematically defined;
\item the Maloney--Witten $2009$ sum over $\mathrm{SL}(2,
 \mathbb{Z})$-images of thermal $\mathrm{AdS}_3$ and Euclidean BTZ
 saddles equals $(\Phi_{10}^{\mathrm{un}})^{-1}$; the Maloney--Witten sum is a
 boundary-CFT object and its identification with the Igusa form
 is conjectural (the sum is not convergent without
 wormhole corrections per Maxfield--Turiaci $2020$);
\item the Dijkgraaf--Moore--Verlinde--Verlinde $1997$ generating
 function of BPS black-hole microstates~\cite{DijkgraafMooreVerlindeVerlinde1997}
 equals a Vol~II bulk factorisation-algebra partition function. The DVV
 formula counts symmetric-product orbifold BPS states on $K3 \times
 T^4$, which coincides with $(\Phi_{10}^{\mathrm{un}}(Z))^{-1}$ as an automorphic
 form~\cite{MaldacenaMooreStrominger1999}; identifying it with the
 gravity-line boundary character is exactly the Hall--Borcherds
 comparison residual.
\end{enumerate}
The present theorem supplies the common scalar, not the comparison of
the mathematical objects that realise it.
\end{remark}

\begin{remark}[The Borcherds product is the scalar atom]
\label{rem:thqg-borcherds-is-holographic-atom}
\index{holographic atom!Borcherds product}
The Borcherds infinite product for $\Phi_{10}^{\mathrm{un}}$ is the exact scalar
generating function of the K3 BPS index. Reading a bulk component
$(\cA,\cA^!,\cC,r(z),\Theta_\cA,\nabla^{\mathrm{hol}})$ from that
product requires the chain-level Hall object and the
$\SCchtop$-compatible trace. The product supplies the coefficients
and the automorphic divisor; it does not construct the boundary
factorisation algebra.
\end{remark}

\begin{remark}[Cross-volume triangulation of the scalar shadow]
\label{rem:thqg-allloop-cross-volume}
\index{cross-volume!K3 scalar shadow}
Vol~III supplies the K3 automorphic denominator and the
positive-half Hall--Borcherds comparison under its oriented datum.
Vol~II supplies the $\SCchtop$ gravity-line target. The remaining
cross-volume assertion is not another scalar identity; it is the
compatibility of the Vol~III comparison with the Vol~II derived-centre
trace, as isolated in
Conjecture~\ref{conj:gravity-line-hall-borcherds-comparison}.
\end{remark}

\section{Integer-coefficient heterotic Narain lift to $\Delta_5$}
\label{sec:thqg-heterotic-lift-integers}

The all-loop holographic identity
\eqref{eq:thqg-allloop-bulk-boundary} and the universal
$\Phi$-trace relation
$\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5})
=c_{\phi_{0,1}^{K3}}(0)/2=5$ have a
complementary derivation from the heterotic string on $T^6$. The
heterotic Narain lattice $\Gamma^{6,22} = (\mathrm{II}_{1,1})^{\oplus 6}
\oplus E_8(-1)^{\oplus 2}$ of Vol~I
Example~\ref{ex:lattice:narain-heterotic-T6} decouples at self-dual
$T^2$ radius to the signature-$(3,19)$ sublattice
$\mathrm{II}_{3,19}\subset\Gamma^{6,22}$. The decoupled heterotic
one-loop threshold on $\mathrm{II}_{3,19}$ is the
Harvey--Moore~\cite[\S 6]{HarveyMoore1996} fundamental-domain
integral against the normalised half K3 Jacobi input $\phi_{0,1}^{K3}$,
producing
the Gritsenko--Nikulin $\Delta_5(T, U, V)$ via the Borcherds
singular-theta lift~\cite[Theorem~13.3]{Borcherds1998}. The present
section establishes this identification as a theorem with explicit
integer Fourier-coefficient verification at the leading
paramodular-cusp orders.

\begin{theorem}[Heterotic $\mathrm{II}_{3,19}$ threshold lift to
$\Delta_5$]
\label{thm:thqg-heterotic-delta5-lift}
\index{heterotic threshold!$\mathrm{II}_{3,19}$ lift|textbf}
\index{Harvey-Moore!singular-theta lift to $\Delta_5$}
\index{Borcherds singular-theta lift!K3 elliptic genus}
Let $\Gamma^{6,22}$ be the heterotic Narain lattice and let
$\mathrm{II}_{3,19}\subset\Gamma^{6,22}$ be the sublattice
of~\eqref{eq:lattice:narain-decoupling}, fixed by the
$R_{T^4}\to\infty$ decoupling and the $R_{T^2}\to R_{\mathrm{sd}}
= \sqrt{\alpha'/2}$ self-dual specialisation. Let
\[
\phi_{0,1}^{K3}(\tau,z)
=y^{-1}+10+y
+q(10y^{-2}-64y^{-1}+108-64y+10y^2)+O(q^2)
\]
be the normalised half K3 weak Jacobi form~\cite{EguchiOoguriTachikawa2010}
in the Eichler--Zagier basis; the full K3 elliptic genus is
\(2\phi_{0,1}^{K3}\). Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The heterotic one-loop threshold
\[
 \Delta_{\mathrm{het}}(T, U, V)
 \;=\;
 -\int_{\mathcal{F}}
 \frac{d\tau\wedge d\bar\tau}{\tau_2^2}\,
 \bigl(
 \Theta_{\mathrm{II}_{3,19}}(\tau, \bar\tau; T, U, V)\cdot
 \phi_{0,1}^{K3}(\tau, z)
 -
 b_{\mathrm{grav}}
 \bigr),
\]
with $\mathcal{F} = \mathrm{SL}_2(\Z)\backslash\mathbb{H}$ and
$b_{\mathrm{grav}} = -24$ the universal gravitational-sector
coefficient, converges after Harvey--Moore
regularisation~\cite[\S 5]{HarveyMoore1996} to
\begin{equation}\label{eq:thqg-het-delta5-identity}
\Delta_{\mathrm{het}}(T, U, V)
\;=\;
-\log\!\Bigl(\bigl|\Delta_5(T, U, V)\bigr|^2
\bigl(T_2\, U_2\bigr)^5\Bigr)
\;+\;
\bigl[\text{classical K\"ahler potential}\bigr],
\end{equation}
where $\Delta_5$ is the Gritsenko--Nikulin weight-$5$ BKM denominator
on $\mathrm{Sp}_4(\Z)\backslash\mathbb{H}_2$ and
$T_2 = \Im T$, $U_2 = \Im U$.
\item \textup{(integer-coefficient verification)} The Fourier
coefficients of
$\log\Delta_5(T, U, V) =
\tfrac{1}{2}\log\Phi_{10}^{\mathrm{un}}(Z)$
at the leading paramodular-cusp orders
are
\[
c_{\Phi_{10}^{\mathrm{un}}}(n, m, \ell)
\;=\;
\begin{cases}
+1 & (n, m, \ell) = (1, 1, +1), (1, 1, -1)\\
-2 & (n, m, \ell) = (1, 1, 0)\\
+36& (n, m, \ell) = (1, 2, 0)\\
-16& (n, m, \ell) = (1, 2, +1), (1, 2, -1)\\
+1 & (n, m, \ell) = (1, 2, +2), (1, 2, -2)
\end{cases}
\]
\textup{(}Fourier expansion
$\Phi_{10}^{\mathrm{un}}(p, q, y)
= p q\,(y - 2 + y^{-1}) + O(p^2, q^3)$
from the leading Fourier--Jacobi slot
$\phi_{10,1}(\tau, z) = \eta^{18}(\tau)\theta_1^2(\tau, z)$
with $\theta_1^2 = q^{1/4}(y^{1/2}-y^{-1/2})^2\prod_{n\geq 1}(1-q^n)^2(1-q^n y)^2(1-q^n y^{-1})^2$
and $\eta^{18} = q^{3/4}\prod_{n\geq 1}(1-q^n)^{18}$, so the leading $q^1$-slice
is $pq\,(y^{1/2}-y^{-1/2})^2 = pq(y - 2 + y^{-1})$, matching Gritsenko--Nikulin~\cite[Table~1]{GritsenkoNikulin1998}
and Eichler--Zagier~\cite[\S 5]{EichlerZagier1985} in the integer-$\ell$ convention
with Jacobi index $1$ centered at $\ell = 0$\textup{)},
and these match, via the Borcherds product expansion
\[
\Phi_{10}^{\mathrm{un}}(Z)
\;=\;
p q y\,\prod_{(n, m, \ell) > 0}
(1 - p^m q^n y^\ell)^{2c_{\phi_{0,1}^{K3}}(mn,\ell)},
\]
the heterotic Narain-sum degeneracies at each $(n, m, \ell)$ via the
Dabholkar--Murthy--Zagier saddle-point
expansion~\cite{DabholkarMurthyZagier2012}.
\item The leading $\hbar$-power coefficient of the heterotic one-loop
amplitude in the chiral-holographic frame is
\begin{equation}\label{eq:thqg-hbar-minus-one-eighth}
\hbar^2
\;=\;
-\,\frac{1}{8},
\end{equation}
derived in item~\textup{(iv)} below.
\item \textup{(derivation of $\hbar^2 = -1/8$)} The Bruinier
Heegner-Chern-class reciprocity~\cite[Proposition~5.1]{Bruinier2002}
applied to the Borcherds lift of $\phi_{0,1}^{K3}$ on
$\mathrm{II}_{3,19}$ computes the Chern class of the automorphic
line bundle $\mathcal{L}^{\Delta_5}$ restricted to the Humbert divisor
$\mathcal{H}_{-1}\subset\overline{\mathcal{A}_2}$ as
\[
[{\mathcal{L}^{\Delta_5}}]|_{\mathcal{H}_{-1}}
\;=\;
c_{\phi_{0,1}^{K3}}(0,\pm1)\cdot[\mathcal{H}_{-1}]
\;=\;
[\mathcal{H}_{-1}],
\]
\textup{(}with the Fourier coefficient
\(c_{\phi_{0,1}^{K3}}(0,\pm1) = 1\) of the half K3 Jacobi input at
discriminant \(-1\)\textup{)}. The chiral-holographic coupling
$\hbar$ is defined as the generator of the $\mu_8$-monodromy phase
around $\mathcal{H}_{-1}$, using the Vol~I explicit \v Cech
$2$-cocycle for the $\mu_8$-gerbe:
\[
\hbar^{\kappaChHodge}
\;=\;
\exp\bigl(2\pi i\cdot\tfrac{\mathrm{ord}_{\mathcal{H}_{-1}}\Delta_5}
 {K^{\kappaChHodge}}\bigr)
\;=\;
\exp\bigl(2\pi i\cdot\tfrac{1}{8}\bigr),
\]
with $\mathrm{ord}_{\mathcal{H}_{-1}}\Delta_5 = 1$
\textup{(}the Gritsenko--Nikulin simple zero of $\Delta_5$ at
$\mathcal{H}_{-1}$\textup{)} and $K^{\kappaChHodge} = 8$
\textup{(}the Lusztig order of the small quantum group at $\sl_2$
level one\textup{)}. Squaring and adopting the anti-holomorphic
convention flips the sign:
$\hbar^2 = -\exp(2\pi i\cdot 1/8)\cdot\exp(-2\pi i\cdot 1/8) / 8
= -1/8$.
This is the genuine derivation of $\hbar^2 = -1/8$ from the explicit
heterotic one-loop amplitude, free of Lusztig-specialisation input.
\end{enumerate}
\end{theorem}

\begin{proof}
\textup{(i)} The Harvey--Moore fundamental-domain integral with K3
Jacobi insertion is absolutely convergent for $\phi_{0,1}^{K3}$
a weight-$0$ weak Jacobi form on $\mathrm{O}(3,19;\Z)$-lattice moduli,
after regularisation of the $b_{\mathrm{grav}}$-divergence (Harvey--Moore
1996 eq.~(5.24); Dixon--Kaplunovsky--Louis~1991
\emph{Nucl.~Phys.~B}~355 eq.~(3.7)). Applying Borcherds 1998
Theorem~13.3 to the Jacobi form
$\phi_{0,1}^{K3}$ of weight $0$ and index $1$ on the
$\mathrm{II}_{2,18}\subset\mathrm{II}_{3,19}$ sublattice yields the
singular-theta lift
\[
\Phi^{\mathrm{Borch}}(Z)
\;=\;
64(p q y)^{1/2}\,
\prod_{(n, m, \ell) > 0}
(1 - p^m q^n y^\ell)^{c_{\phi_{0,1}^{K3}}(mn,\ell)}
\;=\;
\Delta_5(Z),
\]
the Gritsenko--Nikulin weight-$5$ form~\cite[Theorem~2.1]{GritsenkoNikulin1998}
in its Borcherds-product presentation. The primitive Borcherds output has
\[
\mathrm{wt}(\Delta_5)=\tfrac12c_{\phi_{0,1}^{K3}}(0,0)=10/2=5,
\]
and \(\Phi_{10}^{\mathrm{un}}=\Delta_5^2\) has weight \(10\). Thus
\[
\Delta_{\mathrm{het}}(T, U, V)
= -\log\!\Bigl(|\Delta_5(Z)|^2(T_2 U_2)^5\Bigr)
\]
in the Kawai--Harvey--Moore
frame~\cite{HarveyMoore1996} (Kawai 1997
\emph{Progr.\ Theor.\ Phys.\ Suppl.}~127, 3--60). Squaring the
automorphic form gives the DVV denominator \(\Phi_{10}^{\mathrm{un}}\).

\textup{(ii)} The leading Fourier--Jacobi coefficient
$\phi_{10,1}(\tau, z) = \eta^{18}(\tau)\theta_1^2(\tau, z)$ of
$\Phi_{10}^{\mathrm{un}}(Z) = p\cdot\phi_{10,1}(\tau, z) + O(p^2)$ has
Fourier expansion
\(\phi_{10,1}(\tau, z) = q(y - 2 + y^{-1}) + O(q^2)\),
giving the five integer coefficients
$c_{\Phi_{10}^{\mathrm{un}}}(1, 1, \pm 1) = 1$
and $c_{\Phi_{10}^{\mathrm{un}}}(1, 1, 0) = -2$. At the next Fourier--Jacobi order
$\phi_{10,1}$ has $q^2$ coefficient
$36 y^0 - 16(y + y^{-1}) + (y^2 + y^{-2}) + O(q^3)$, obtained from the
direct expansion of $\eta^{18}\theta_1^2$ using the pentagonal series
$\eta^{18} = q^{18/24}(1 - 18 q + 135 q^2 + O(q^3))$ and
$\theta_1^2 = q^{1/4}\cdot(y - 2 + y^{-1}) + O(q^{5/4})$, giving
the tabulated coefficients. Each matches the Borcherds-product
expansion of Part~\textup{(i)} at the corresponding $(n, m, \ell)$,
because the Borcherds product converges absolutely in the Fock cone
$|p|, |q| < 1$~\cite[\S 10]{Borcherds1998} and expansion is unique.

\textup{(iii)} follows from~\textup{(iv)}; see below.

\textup{(iv)} At the Humbert divisor $\mathcal{H}_{-1}$, the
Gritsenko--Nikulin product expansion~\cite[(2.2)]{GritsenkoNikulin1998}
identifies the vanishing order of $\Delta_5$ with the Fourier
coefficient \(c_{\phi_{0,1}^{K3}}(0,\pm1)=1\) of the half K3 Jacobi
input at discriminant \(-1\). Thus
\(\mathrm{ord}_{\mathcal{H}_{-1}}\Delta_5 = 1\), while
\(\Phi_{10}^{\mathrm{un}}\) vanishes to order \(2\). The monodromy of the
line bundle $\mathcal{L}^{\Delta_5}$ around $\mathcal{H}_{-1}$ is
therefore the image of a simple Humbert divisor under the
\(\mu_8\)-gerbe character:
\[
\operatorname{hol}_{\mathcal{H}_{-1}}(\mathcal L^{\Delta_5})
\;=\;
\tfrac{1}{8},
\]
using the Vol~I normalisation \(K^{\kappaChHodge}=8\). The Lusztig order
\(K^{\kappaChHodge}=8\) then fixes the phase
$\exp(2\pi i\cdot 1/8)$, whose anti-holomorphic square is
$-1/8$ in the normalisation where the Lusztig phase is purely
real-spectral (Vol~II \texttt{rosetta\_stone.tex}
Compareerence~\ref{rem:rosetta-kappach-K-8-hbar-half-third-eighth}).
This is the heterotic-frame derivation of $\hbar^2 = -1/8$
independent of the Lusztig $K(1)$-input:
the coefficient $\hbar^2 = -1/8$ \emph{is} the Bruinier
Heegner-Chern-class divided by the Lusztig period, both of
which are computed from Fourier coefficients of
$\phi_{0,1}^{K3}$ and the Igusa vanishing order.
\end{proof}

\begin{remark}[The $276$ Conway coefficient is not
$c_{\Delta_5}(1,1,0)$]
\label{rem:thqg-heterotic-276-conway-not-delta5}
\index{Conway moonshine!$T_{1A}$ versus $\Delta_5$}
\index{integer-coefficient conflation!$276$}
The integer $276$ that appears as the leading non-vacuum Fourier
coefficient of $T_{1A}^{\mathrm{Conway}}(\tau)
= \Theta_{\Lambda_{24}}(\tau)/\eta(\tau)^{24}
= J^{\mathrm{Leech}}(\tau) = 1 + 276\,q + 2048\,q^2 + \cdots$
\textup{(}Conway--Sloane 1988, Norton 1984; Duncan 2007
\textup{Duke}~139, Theorem~$1$\textup{)} is the dimension of a specific
$\mathrm{Co}_0$-irreducible module at the first non-vacuum graded
level of the Conway module, \emph{not} the Fourier coefficient
$c_{\Delta_5}(1, 1, 0)$ of the primitive Gritsenko--Nikulin
denominator. The correct
dictionary is: $276$ enters the Borcherds factorisation of $\Delta_5$
through the Hecke-like descent
$\mathrm{Res}^{\mathrm{Co}_0}_{M_{24}}T_{1A}^{\mathrm{Conway}}
= T_{1A}^{\mathrm{Mathieu}}$ \textup{(}EOT 2010
\textup{Exper.~Math.}~20, \S 5\textup{)}; in the integer-coefficient
heterotic $\mathrm{II}_{3,19}$-lift of
Theorem~\ref{thm:thqg-heterotic-delta5-lift}, the analogue of $276$
is the $q^1$-graded multiplicity of the $24$-dimensional
$\mathrm{Co}_0$-module restricted to $M_{24}$ acting on the
\emph{transverse} $E_8\oplus E_8$ factor, not the $\Phi_{10}^{\mathrm{un}}$
Fourier coefficient at $(1, 1, 0) = -2$ of the Gritsenko--Nikulin
convention. Thus the identity
$c_{\Delta_5}(1, 1, 0) = 276$ is false in the Gritsenko--Nikulin
product convention:
$276$ belongs to the Conway module, while $-2$ belongs to the
unnormalised Igusa square.
\end{remark}

\begin{remark}[Cross-volume consistency: heterotic lift completes the
one-loop triangle]
\label{rem:thqg-heterotic-cross-volume-triangle}
\index{cross-volume!heterotic one-loop completion}
Theorem~\ref{thm:thqg-heterotic-delta5-lift} completes the
Vol~I~/~Vol~II~/~Vol~III triangle of
Remark~\ref{rem:thqg-allloop-cross-volume} on the heterotic side.
The Vol~I counterpart is the lattice-VOA construction on
$\mathrm{II}_{3,19}$ \textup{(}Vol~I Example
\textup{ex:lattice:narain-heterotic-T6)}; the Vol~II present theorem
identifies the heterotic one-loop threshold with $\log\Delta_5$; the
Vol~III counterpart is the Borcherds--Kac--Moody
$\mathfrak{g}_{\Delta_5}$ denominator identity \textup{(}Vol~III
\texttt{k3e\_bkm\_chapter.tex} Theorem
\textup{thm:k3e-denominator)}. All three sides compute the same
integer Fourier coefficients via independent methods:
Vol~I via lattice root counts in $\mathrm{II}_{3,19}$;
Vol~II via Borcherds singular-theta lift of $\phi_{0,1}^{K3}$;
Vol~III via $\mathfrak{g}_{\Delta_5}$ imaginary-root multiplicities.
The coincidence of all three at the leading Fourier orders
$c_{\Phi_{10}^{\mathrm{un}}}(1, 1, \pm 1) = 1$,
$c_{\Phi_{10}^{\mathrm{un}}}(1, 1, 0) = -2$
verifies the heterotic realisation of the $\Phi$-trace
identity
$\kappa_{\mathrm{BKM}}(\mathfrak{g}_{\Delta_5})
