...
\label{subsec:thqg-swiss-cheese-conv}
...
\label{prop:thqg-gSC-factorization}
The Swiss-cheese convolution algebra decomposes as a direct product
\begin{equation}\label{eq:thqg-gSC-factorization}
\mathfrak{g}^{\mathrm{SC}}_\cA
\;\cong\;
...
\item $\gAmod = \Convstr(\mathcal{C}^!_{\mathrm{ch}},
  ...
  algebra (Definition~\ref{def:modular-convolution-dg-lie}),
...
\label{subsec:thqg-six-components}
...
\label{item:thqg-component-b}
\index{Koszul dual!holographic}
The Koszul dual is
\begin{equation}\label{eq:thqg-koszul-dual}
...
\begin{equation}\label{eq:thqg-collision-residue}
...
\begin{equation}\label{eq:thqg-cybe}
...
\label{item:thqg-component-e}
\index{Maurer--Cartan element!universal|textbf}
The bar-intrinsic MC element
(Volume~I, Theorem~\ref{thm:mc2-bar-intrinsic}) is
\begin{equation}\label{eq:thqg-theta}
\Theta_\cA \;:=\; D_\cA - d_0
\;\in\;
\MC\bigl(\gAmod\bigr),
\end{equation}
...
\begin{equation}\label{eq:thqg-mc-equation}
...
\begin{equation}\label{eq:thqg-theta-decomposition}
\Theta_\cA
...
\begin{remark}[Genus-$0$ flatness is the KZ connection]
\label{rem:thqg-kz}
\index{KZ connection!from shadow connection}
At genus~$0$, the shadow connection with $n$ insertions is
\begin{equation}\label{eq:thqg-kz}
...
On a single primary line, the shadow connection is the logarithmic
...
\label{subsec:thqg-shadow-tower}
...
\label{prop:thqg-named-shadows}
\index{modular characteristic!as shadow}
\index{cubic shadow}
\index{quartic resonance class}
The first three layers of the shadow Postnikov tower are:
\begin{enumerate}[label=\textup{(\roman*)}]
...
(Volume~I, Corollary~\ref{cor:shadow-extraction}), each finite-order
...
\label{prop:thqg-phase-space-genus0}
At genus $0$, the gravitational phase space is
\begin{equation}\label{eq:thqg-phase-genus0}
\mathbf{C}_0(\cA)
\;\simeq\;
...
\label{subsec:thqg-summary-table}
...
\item \textbf{Drinfeld--Sokolov reduction}: For principal DS reduction
  $\mathcal{W}_k(\mathfrak{g}) = H^0_{\mathrm{DS}}(V_k(\mathfrak{g}))$,
  the holographic datum transforms as
  $\mathcal{H}(\mathcal{W}_k) = \mathrm{DS}(\mathcal{H}(V_k))$.
\end{enumerate}
\end{theorem}
...
(iii)~follows from the compatibility of bar-cobar with principal DS
reduction (Volume~I, Chapter~\ref{chap:w-algebras}).
\end{proof}
...
\label{subsec:thqg-g1-statement}
...
\label{subsec:thqg-g2-statement}
...
(Volume~I, Proposition~\ref{prop:shadow-formality-low-arity})
identifies the shadow tower with the $\Linf$-formality obstruction
...
\label{subsec:thqg-g3-statement}
...
\label{subsec:thqg-g4-statement}
...
transforms as $\kappa(\cA^!) = -\kappa(\cA)$ (Volume~I, Theorem~D: anti-symmetry).
...
The Verdier duality reverses the grading on bar cohomology, which
produces the sign: $\kappa(\cA^!) = -\kappa(\cA)$.
...
\label{subsec:thqg-g5-statement}
...
satisfies the classical Yang--Baxter equation~\eqref{eq:thqg-cybe}.
It defines a dg-shifted Yangian $\Ydg(\cA^!)$ whose RTT realization is the
gravitational scattering matrix.  The line-operator category is
$\mathcal{C} \simeq \cA^!\text{-}\mathsf{mod}$.
\end{theorem}
...
The dg-shifted Yangian $\Ydg(\cA^!)$ is defined by the RTT
formalism applied to $r(z)$
(Volume~I, Definition~\ref{def:modular-yangian-pro}).
Its module category coincides with $\cA^!\text{-}\mathsf{mod}$
by the line-operator theorem (Chapter~\ref{chap:ht-bulk-boundary-line},
Theorem~\ref{thm:lines_as_modules}).
\end{proof}


\subsection{G6: Soft graviton theorems}
\label{subsec:thqg-g6-statement}
...
\begin{equation}\label{eq:thqg-all-arity}
...
\label{subsec:thqg-g7-statement}
...
\label{subsec:thqg-g8-statement}
...
\item $\cA \;\xmapsto{\text{bar-intrinsic}}\; \Theta_\cA$
  (Volume~I, Theorem~\ref{thm:mc2-bar-intrinsic});
...
theorems cited.  The key point is that no additional input beyond
the boundary OPE is needed: the bar-intrinsic construction
(Volume~I, Theorem~\ref{thm:mc2-bar-intrinsic}) produces $\Theta_\cA$
...
\label{subsec:thqg-g9-statement}
...
\item The depth-zero resonance shadow
  (Volume~I, Remark~\ref{rem:virasoro-resonance-model}) saturates;
\item The resonance-filtered bar-cobar
  (Volume~I, Theorem~\ref{thm:resonance-filtered-bar-cobar}) collapses;
...
$\kappa(\mathrm{Vir}_{13}) = 0$), but the physically distinguished
point is $c = 26$ (where $\mathrm{Vir}_{26}^! = \mathrm{Vir}_0$, the
trivial Virasoro algebra).

At $c = 26$: (i)~$\mathrm{Vir}_{26}^! = \mathrm{Vir}_0$, so the
$S$-duality acts trivially on the bulk sector.
(ii)~The depth-zero resonance shadow $\mathrm{Vir}_{26-c}$ at $c = 26$
is $\mathrm{Vir}_0$, which carries no dynamics.
(iii)~The resonance rank $\rho(\mathrm{Vir}_{26}) = 0$, so the
resonance-filtered bar-cobar has no resonance corrections.
(iv)~$\mathbf{Q}_g(\mathrm{Vir}_{26}) = \mathbf{Q}_g(\mathrm{Vir}_0)
= 0$ for $g \ge 1$, so the complementarity splitting degenerates.

The analogous statement for affine algebras: the self-dual level is
$k = -h^\vee$ (the critical level), where the Koszul involution has
a fixed point.  For $\mathcal{W}_N$-algebras: the self-dual central
charge depends on $N$.
...
\label{subsec:thqg-g10-statement}
...
representability to the entire standard landscape.
\end{theorem}
...
\label{subsec:thqg-dependency-statement}
...
\item the bar-intrinsic MC element $\Theta_\cA := D_\cA - d_0 \in \MC(\gAmod)$
  (Volume~I, Theorem~\ref{thm:mc2-bar-intrinsic}); and
...
\label{subsec:thqg-dependency-graph}
...
\label{rem:thqg-arity-genus-independence}
...
\begin{equation}\label{eq:thqg-m2-generating-function}
...
\ClaimStatusProvedHere]
\label{comp:thqg-m2-quantum-correction}
...
correction:
\begin{equation}\label{eq:thqg-m2-quantum}
...
\label{subsec:thqg-m2-koszul-dual}
...
\begin{equation}\label{eq:thqg-m2-koszul-dual}
...
\begin{equation}\label{eq:thqg-m2-universal-kernel}
\mu_{M2}
:= \sum_{\alpha,\beta,m,n}
...
\ClaimStatusProvedHere]
\label{prop:thqg-m2-bar-cohomology}
...
\item The bar cohomology does not concentrate in degrees $1$ and $2$:
  ...
  of $\Diff(\C)$.  The shadow tower does not terminate.
\end{enumerate}
\end{proposition}
...
\label{subsec:thqg-m2-six-components}

\begin{theorem}[$M2$ holographic datum; \ClaimStatusConjectured]
\label{thm:thqg-m2-datum}
...
\begin{equation}\label{eq:thqg-m2-datum}
\mathcal{H}(M2)
= \bigl(
A_{M2,\infty},\;
A^!_{M2},\;
...
$\Theta_{M2}$
  & Bar-intrinsic MC element; shadow tower is infinite
    (class M) \\
$\nabla^{\mathrm{hol}}_{M2}$
...
\subsection{Shadow tower for $M2$}
\label{subsec:thqg-m2-shadow-tower}

\begin{proposition}[Shadow depth of $M2$; \ClaimStatusProvedHere]
\label{prop:thqg-m2-shadow-depth}
The $M2$ boundary algebra has shadow depth $r_{\max}(A_{M2,\infty}) = \infty$
(class M: mixed).  The first four shadow layers are:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappa(A_{M2,\infty}) = K^2$: the modular characteristic equals
  $\dim(\mathfrak{gl}_K) = K^2$ (the weight-$0$ generators contribute
  $K^2$ and higher weights contribute zero by cancellation).
...
\end{enumerate}
The shadow tower does not terminate: $A_{M2,\infty}$ has both
...
$\kappa = \operatorname{str}(\id|_{V^{(0)}}) = \dim(\mathfrak{gl}_K)
= K^2$.  (The weight-$0$ space is $\mathfrak{gl}_K$ with trivial
ghost number; higher weights do not contribute to $\kappa$ because
the supertrace over pairs $(E_{\alpha\beta}z^m\partial^n,
E_{\alpha\beta}z^n\partial^m)$ cancels for $m+n \ge 1$.)
...
\label{subsec:thqg-m2-collision-residue}

\begin{computation}[$M2$ collision residue; \ClaimStatusProvedHere]
\label{comp:thqg-m2-collision-residue}
...
\begin{equation}\label{eq:thqg-m2-collision-residue}
r_{M2}(z)
...
\label{subsec:thqg-m2-costello}

\begin{remark}[Brane/bulk duality as an MC problem]
\label{rem:thqg-m2-brane-bulk}
...
quotient of $\mathcal{H}(M2)$ by this ideal.  The shadow tower
...
\begin{equation}\label{eq:thqg-ks-boundary}
...
\begin{equation}\label{eq:thqg-ks-ope}
\alpha(z)\, \beta(w) \;\sim\;
\frac{[\alpha, \beta]_{\sn}(w)}{z - w} + \text{regular}.
\end{equation}
The trace pairing
\begin{equation}\label{eq:thqg-ks-trace}
...
determined by $\Omega$:
\begin{equation}\label{eq:thqg-ks-div}
...
\subsection{The Koszul dual}
\label{subsec:thqg-ks-koszul-dual}
...
field theory:
\begin{equation}\label{eq:thqg-ks-koszul-dual}
V_{\partial,\mathrm{KS}}^!(Y)
\;\simeq\;
\bigoplus_{p,q}
...
\begin{equation}\label{eq:thqg-ks-koszul-exchange}
...
\begin{equation}\label{eq:thqg-ks-lines}
...
\label{subsec:thqg-ks-concentration}
...
\item The shadow tower terminates at arity $2$ (class G: Gaussian).
...
$\Theta_{\mathrm{KS}}$
  & Bar-intrinsic MC element; shadow tower terminates at
    arity $2$ (Gaussian) \\
...
(Theorem~\ref{thm:thqg-g5-yangian}), which for the Schouten--Nijenhuis
...
\label{subsec:thqg-ks-free-energies}

\begin{proposition}[KS genus-$g$ free energy; \ClaimStatusProvedHere]
\label{prop:thqg-ks-free-energy}
For a compact Calabi--Yau threefold $Y$, the genus-$g$ free energy
of the Kodaira--Spencer holographic system is
\begin{equation}\label{eq:thqg-ks-free-energy}
F_g^{\mathrm{KS}}(Y)
= \kappa_{\mathrm{KS}}(Y) \cdot \frac{B_{2g}}{2g(2g-2)},
\qquad g \ge 2,
\end{equation}
where $B_{2g}$ are the Bernoulli numbers and
$\kappa_{\mathrm{KS}}(Y) = h^{1,1} - h^{2,1}$.
At genus $1$:
\begin{equation}\label{eq:thqg-ks-f1}
F_1^{\mathrm{KS}}(Y) = -\frac{\kappa_{\mathrm{KS}}}{12}.
\end{equation}
\end{proposition}

\begin{proof}
Since the KS shadow tower terminates at arity $2$ (Gaussian), all
...
determinant.  For a Gaussian theory, $F_g = \kappa \cdot B_{2g}/[2g(2g-2)]$
...
\begin{equation}\label{eq:thqg-ks-mirror}
...
\label{subsec:thqg-ks-quintic}

\begin{computation}[The quintic; \ClaimStatusProvedHere]
\label{comp:thqg-ks-quintic}
...
$F_1$ & $-\kappa/12 = 100/12 = 25/3$ \\
$F_2$ & $\kappa^2 / 240 = 10000/240 = 125/3$ \\
...
\label{subsec:thqg-ks-landscape}
...
W-algebra duality, and the platonic adjunction.

\subsection{The modular completion conjecture}
\label{subsec:thqg-modular-completion}

\begin{conjecture}[Platonic completion; \ClaimStatusConjectured]
\label{conj:thqg-platonic-completion}
\index{platonic completion conjecture}
Every positive-energy chiral algebra $\cA$ has finite resonance rank:
\begin{equation}\label{eq:thqg-platonic-completion}
\rho(\cA) < \infty.
\end{equation}
...
\label{rem:thqg-completion-evidence}
...
\label{subsec:thqg-mc4-splitting}

\begin{definition}[MC4$^+$ and MC4$^0$]
\label{def:thqg-mc4-splitting}
...
\begin{equation}\label{eq:thqg-strong-filtration}
...
\label{subsec:thqg-pva-sigma}

\begin{definition}[$3$d PVA sigma model]
\label{def:thqg-pva-sigma}
...
\end{enumerate}
The action is
\begin{equation}\label{eq:thqg-pva-action}
S = \int_{\C \times \R}
...
\label{subsec:thqg-non-principal-w}

\begin{definition}[Hook-type $\mathcal{W}$-algebras in type A]
\label{def:thqg-hook-type}
...
\begin{equation}\label{eq:thqg-hook-duality}
...
\label{conj:thqg-transport-transpose}
...
\begin{equation}\label{eq:thqg-transport-transpose}
...
\label{rem:thqg-transport-evidence}
The conjecture is proved for:
\begin{itemize}
...
\subsection{The platonic adjunction}
\label{subsec:thqg-platonic-adjunction}

\begin{definition}[Cyclically admissible Lie conformal algebra]
\label{def:thqg-cyclically-admissible}
...
\begin{construction}[Platonic package]
\label{constr:thqg-platonic-package}
\index{platonic package|textbf}
Given a cyclically admissible Lie conformal algebra $L$, the
\emph{platonic package} is the six-fold datum
\begin{equation}\label{eq:thqg-platonic-package}
\Pi_X(L) = \bigl(
\mathcal{F}_X(L),\;
\bar{B}_X(L),\;
\Theta_L,\;
...
\begin{theorem}[Platonic adjunction; \ClaimStatusProvedElsewhere]
\label{thm:thqg-platonic-adjunction}
\index{platonic adjunction}
\index{modular factorization envelope}
There exists a universal modular factorization envelope
\begin{equation}\label{eq:thqg-modular-envelope}
...
\begin{equation}\label{eq:thqg-platonic-adjunction}
...
\label{rem:thqg-nishinaka-vicedo}
...
non-chiral setting.  The modular extension to all genera is the
frontier: it requires the full sewing machinery of MC5.
\end{remark}


\subsection{Gauge triviality and the quartic canonical class}
\label{subsec:thqg-gauge-triviality}

\begin{theorem}[Cubic gauge triviality; \ClaimStatusProvedHere]
\label{thm:thqg-cubic-gauge-triviality}
...
\label{subsec:thqg-frontier-envelope-shadow}

\begin{definition}[Envelope-shadow functor]
\label{def:thqg-envelope-shadow}
...
at arity $\le r$ is
\begin{equation}\label{eq:thqg-envelope-shadow}
...
\label{prop:thqg-finite-jet-rigidity}
...
\subsection{MC3 categorical lift: type A and beyond}
\label{subsec:thqg-mc3-frontier}

\begin{theorem}[MC3 in type A; \ClaimStatusProvedElsewhere]
\label{thm:thqg-mc3-type-a}
\index{MC3!type A resolution}
For $\mathfrak{g} = \mathfrak{sl}_N$, the thick generation of
$D^b(\cO_q(\mathfrak{sl}_N))$ by prefundamental representations
$L^\pm$ is proved at the character level:
\begin{equation}\label{eq:thqg-mc3-character}
[M(\lambda)] \in K_0\text{-}\mathrm{span}\{[V_n] \cdot [L^-]\}
\quad \text{for all } \lambda.
\end{equation}
The DK construction gives a derived equivalence
\begin{equation}\label{eq:thqg-mc3-dk}
D^b(\cO_q(\mathfrak{sl}_N))
\;\simeq\;
\Perf(\Ydg(\widehat{\mathfrak{sl}}_N^!))
\end{equation}
on compact objects, once the character-level generation is lifted
to a categorical statement.
\end{theorem}

\begin{conjecture}[MC3 for arbitrary simple type; \ClaimStatusConjectured]
\label{conj:thqg-mc3-arbitrary}
\index{MC3!arbitrary type conjecture}
For all simple $\mathfrak{g}$, the prefundamental Clebsch--Gordan
closure (the analogue of Theorem~\ref{thm:thqg-mc3-type-a}) holds,
giving DK equivalences for the dg-shifted Yangian $\Ydg(\cA^!)$ for
all chirally Koszul $\cA$ in the affine family.
\end{conjecture}

\begin{remark}[The Hern\'andez--Jimbo obstruction]
\label{rem:thqg-hernandez-jimbo}
The obstruction to extending MC3 beyond type $A$ is the absence of
a suitable set of prefundamental representations for the quantum
affine algebra $U_q(\hat{\mathfrak{g}})$ in types $B$, $C$, $D$,
$E$, $F$, $G$.  The Hern\'andez--Jimbo construction provides
candidates, but the Clebsch--Gordan closure has not been established
at the character level for these types.
\end{remark}


\subsection{The holographic dictionary}
\label{subsec:thqg-dictionary}
...
\label{rem:thqg-five-theorems-cumulants}
...
\label{subsec:thqg-grand-completion}

\begin{conjecture}[Grand completion; \ClaimStatusConjectured]
\label{conj:thqg-grand-completion}
...
\begin{equation}\label{eq:thqg-grand-completion}
...
\item $\mathrm{Vir}_c^! \simeq \mathrm{Vir}_{26-c}$.  Self-duality at
  ...
  the point where $\cA^! \simeq \mathrm{Vir}_0$ (trivial Virasoro).
...
\item The shadow Postnikov tower is the primary nonlinear object:
