...
$\kappa(\mathrm{Vir}_{13}) = 13/2$), but the physically distinguished
...
Recall that Drinfeld--Sokolov reduction on the algebraic side
...
We emphasize that the algebraic and geometric infrastructure
...
% label removed: thm:thqg-cubic-gauge-triviality
...
\begin{theorem}[MC3 for all simple types; \ClaimStatusProvedElsewhere]
...
closure holds
(Vol~I, Corollary~\ref*{cor:mc3-all-types}),
giving DK equivalences for the dg-shifted Yangian $\Ydg(\cA^!)$ for
all chirally Koszul $\cA$ in the affine family.
The proof replaces the minuscule hypothesis of the type-$A$ argument
with the strictly weaker multiplicity-free $\ell$-weight property
(Chari--Moura, 2006), which holds for all simple types.
\end{theorem}

\begin{remark}[The Hern\'andez--Jimbo obstruction: resolved]
...
(Vol~I, Theorem~\ref*{thm:categorical-cg-all-types}): the
...
hypothesis entirely, and holds for all simple Lie algebras.
\end{remark}
