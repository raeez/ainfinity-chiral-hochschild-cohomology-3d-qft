...
Anomaly completion proceeds through three successive projections of
$\Theta^{\mathrm{oc}}$: the transgression algebra, the genus-Clifford
completion, and the anomaly cancellation
$\kappa_{\mathrm{eff}} = \kappaChHodge(\text{matter}) + \kappaChHodge(\text{ghost}) = 0$.
The spectral Drinfeld class is controlled by the root-sector scalar
classes
$[\epsilon_{\alpha,n}]\in
\mathscr K_n/\operatorname{im}(\delta_{\alpha,n})$
(Theorem~\ref{thm:complete-strictification}); its vanishing is a
scalar strictification condition, separate from the genus-dependent
anomaly data.
...
higher operations in HT perturbation theory~\cite{GKW25};
...
\noindent\textbf{Strict algebraic spine.}
The construction has four algebraic outputs.
\begin{enumerate}
...
\noindent\textbf{Comparison data.}
Identifying \(B\) with a bulk Koszul-dual algebra \(A^{!}\),
identifying \(\Theta\) with a geometric genus anomaly
\(\Theta_{g}\), or identifying
\(\mathfrak{G}_{g}(B_{\Theta})\) with a line-side surface package
requires explicit bridge data:
a Koszul-dual model for \(A^{!}\), a Hochschild or cyclic comparison
map carrying \(\Theta_g\) to a central \(2\)-cocycle, and a
bulk--boundary comparison functor.  The strict theorems below apply
once those maps and their centrality, primitivity, and functoriality
hypotheses are part of the input.

\subsection{The first-principles correction}
\label{subsec:tholog-first-principles-correction}

Adjoining an odd \emph{supercentral} generator \(\eta\) with \(d\eta=\Theta\) fails. An odd supercentral element in characteristic \(0\) satisfies
\[
\eta^{2}=-\eta^{2},
\]
hence \(\eta^{2}=0\). The secondary class \(u=\eta^2\) and the genus-Clifford relation vanish before the construction starts.

The correct object is a \emph{skew polynomial extension}: \(\eta\) graded-commutes with \(B\) but remains a free odd generator with respect to its own powers. Equivalently, adjoin an odd variable over \(B\) via the sign automorphism
\[
\sigma:B\longrightarrow B,\qquad \sigma(b)=(-1)^{|b|}b.
\]
This is the smallest extension preserving the universal property while leaving room for a nontrivial square
\[
u:=\eta^{2}.
\]

A second correction is parallel. Because the extension is a skew/Ore extension rather than a tensor product with an exterior algebra, the natural exact triangle is \emph{one-sided} and module-theoretic. The replacement for the untwisted bimodule resolution is a two-term free resolution of each module, from which the transgression formulas for \(R\!\operatorname{Hom}\) and derived tensor products follow.

\begin{remark}[Inevitability]
\label{rem:tholog-inevitability}
The transgression algebra $B_\Theta$ is the \emph{universal} resolution of the curvature obstruction. Its universal property (Proposition~\ref{prop:tholog-universal-property}) states that every dg~algebra map $B_\Theta \to C$ is equivalent to a pair $(f\colon B \to C,\; h \in C^1)$ with $dh = f(\Theta)$ and $hf(b) = (-1)^{|b|}f(b)h$. Equivalently, $B_\Theta$ is the moduli problem ``resolve $\Theta$ at chain level,'' incarnated as an algebra. The secondary anomaly $u = \eta^2$ and the genus-Clifford completion $\mathfrak G_g(B_\Theta)$ are then forced by the same universality: $u$ is the by-product of $\eta^2 \ne 0$, and the Clifford algebra is the universal algebra in which $2g$ odd generators square to~$u$. The entire tower (transgression, neutralisation, secondary anomaly, genus completion, Morita triviality) is a single deductive chain from the datum $(B, \Theta)$.
\end{remark}
...
The surrounding story compresses into one strict question.
...
The answer has two passes:
\begin{enumerate}
\item from \(B\) to the transgression algebra \(B_{\Theta}\);
\item from \(B_{\Theta}\) to the genus-Clifford completion \(\mathfrak{G}_{g}(B_{\Theta})\), when surface data is to be retained.
\end{enumerate}

Both passes are universal, exact, functorial, and compatible with the monoidal structures of line-operator theories.

\subsection{The transgression algebra}
\label{subsec:tholog-transgression-algebra}

Let \(B\) be a dg algebra. In a genuinely non-commutative bar model the
anomaly may act not only by central multiplication but also by a
degree-\(1\) contraction/sigma-derivation
\[
\iota_{\Theta}\colon B\longrightarrow B[1].
\]

\begin{definition}[Ore transgression with anomaly contraction]
\label{def:tholog-ore-transgression-contraction}
Assume that the anomaly datum consists of a degree-\(2\) cocycle
\(\Theta\in Z^{2}(B)\) and a degree-\(1\) anomaly contraction
\(\iota_{\Theta}\) compatible with the differential, so that the
relation below is stable under \(d\). The \emph{non-commutative Ore
transgression algebra} is
\begin{equation}
\label{eq:tholog-ore-transgression-contraction}
B_{\Theta}^{\mathrm{Ore}}
:=
\frac{B\langle \eta\rangle}
{\left\langle
\eta b-(-1)^{|b|}b\eta-\iota_{\Theta}(b)
\;\middle|\; b\in B \text{ homogeneous}
\right\rangle},
\qquad |\eta|=1,\qquad d\eta=\Theta.
\end{equation}
Equivalently, \(B_{\Theta}^{\mathrm{Ore}}\) is the Ore extension of
\(B\) by the sign automorphism \(\sigma(b)=(-1)^{|b|}b\) and the
\(\sigma\)-derivation \(\iota_{\Theta}\).
\end{definition}

If \(\iota_{\Theta}=0\) and \(\Theta\) is central, the Ore
transgression reduces to the central-shadow transgression used in the
computations below. Thus let
\[
\Theta\in Z^{2}(B)\cap Z(B)
\]
...
Thus \(B_{\Theta}\) is the specialization of
\eqref{eq:tholog-ore-transgression-contraction} with
\(\iota_{\Theta}=0\): the skew polynomial/Ore extension of \(B\) by the
sign automorphism \(\sigma(b)=(-1)^{|b|}b\), equipped with a
differential transgressing \(\Theta\).

\begin{proposition}[Curved Maurer--Cartan convention]
\label{prop:tholog-curved-mc-convention}
Let \(B\) be a curved dg algebra with derivation \(d\) and curvature
\(\Theta\in B^{2}\):
\[
d^{2}=[\Theta,-],
\qquad d\Theta=0.
\]
For a degree-\(1\) element \(\eta\) in an associative enlargement of
\(B\), set \(d_{\eta}=d+[\eta,-]\). Then
\[
d_{\eta}^{2}
=
\left[\Theta+d\eta+\frac12[\eta,\eta],-\right]
=
\left[\Theta+d\eta+\eta^{2},-\right].
\]
Thus \(d_{\eta}\) is square-zero exactly when the curved Maurer--Cartan
equation
\[
d\eta+\frac12[\eta,\eta]+\Theta=0
\]
holds, equivalently \(d\eta+\eta^{2}+\Theta=0\) in the associative
commutator convention. The strict transgression algebra
\(B_{\Theta}\) of Definition~\ref{def:tholog-transgression-algebra}
uses a different, central-shadow convention: \(\Theta\) is a closed
central cocycle in an honest dg algebra and \(d\eta=\Theta\). In that
convention no twist \(d+[\eta,-]\) is being made; the square
\(u=\eta^{2}\) is therefore not absorbed into the differential but
survives as the secondary central class.
\end{proposition}
...
\[
(d+[\eta,-])^{2}
=d^{2}+[d\eta,-]+\frac12[[\eta,\eta],-].
\]
Since \(d^{2}=[\Theta,-]\) and \(|\eta|=1\), the associative
commutator satisfies \([\eta,\eta]=2\eta^{2}\) in characteristic
\(0\). The displayed identity follows. The last assertion is the
definition of the strict Ore transgression convention: the differential
is extended by \(d\eta=\Theta\), while the algebra remains free in the
\(\eta\)-direction, so \(\eta^{2}\) is an element of the algebra rather
than a term in the twisting equation.
\end{proof}

\begin{definition}[Curved Maurer--Cartan anomaly completion]
\label{def:tholog-curved-mc-anomaly-completion}
\index{anomaly completion!curved Maurer--Cartan convention|textbf}
Let \((B,d,\Theta)\) be a curved dg algebra with
\[
d^{2}=[\Theta,-].
\]
The curved-Maurer--Cartan anomaly completion is the associative
enlargement
\[
B_{\Theta}^{\mathrm{curv}}
:=
B\langle\eta,u\rangle/(u-\eta^{2}),
\qquad |\eta|=1,\quad |u|=2,
\]
equipped with the twisted differential
\[
d_{\eta}:=d+[\eta,-]
\]
and the defining flatness equation
\begin{equation}\label{eq:curved-mc-anomaly-completion}
d\eta+\frac12[\eta,\eta]+\Theta=0,
\qquad\text{equivalently}\qquad
d\eta+\eta^{2}+\Theta=0.
\end{equation}
Then \(d_{\eta}^{2}=0\). This is the curved-twist convention; it is
not the strict central-shadow transgression algebra of
Definition~\ref{def:tholog-transgression-algebra}, where \(d\eta=\Theta\)
is used only after \(\Theta\) has been made a closed central cocycle in
an honest dg algebra and no twist \(d+[\eta,-]\) is being formed.
\end{definition}

\begin{example}[Virasoro class-\(\mathbf M\) anomaly completion]
\label{ex:virasoro-curved-mc-anomaly-completion}
For the Banach-completed Virasoro bar complex, the curved completion is
\[
B_{\Theta}^{\mathrm{Vir}}
=
\Bch(\mathrm{Vir}_{c})^{\wedge}_{\rho}
...
\]
The secondary anomaly \(u=\eta^{2}\) is meaningful only after this
associative transgression algebra has been fixed; it is not an
independent replacement for the curved Maurer--Cartan equation.
\end{example}
...
\begin{corollary}[Modules as neutralizations]
...
\begin{corollary}[Obstruction class]
...
\begin{corollary}[Moduli of neutralizations]
...
\begin{theorem}[Free two-term resolution of a neutralized module]
...
\begin{proof}
First, $D_{M}$ is well defined over \(B\).
...
The algebra \(B_{\Theta}\) is not \(B\) tensored with an exterior algebra; it is a skew polynomial extension.
...
\begin{theorem}[Transgression exact triangle for derived Hom]
...
\begin{corollary}[Long exact sequence in Ext]
...
For a right \(B_{\Theta}\)-module, the neutralization operator is not strictly right \(B\)-linear; it is twisted by the sign automorphism. The right-handed notion follows.
...
Such \(h_{L}\) is a \emph{right neutralization}.
\end{definition}

\begin{theorem}[Transgression exact triangle for derived tensor products]
...
This is the cone form needed for comparing boundary conditions through a bulk or line algebra endowed with a nontrivial central genus class.
\end{remark}
...
\begin{theorem}[Gauge invariance of transgression]
...
\begin{theorem}[Primitive lift theorem]
...
\begin{corollary}[Fusion of neutralizations]
...
\begin{proposition}[The secondary central class]
...
\begin{theorem}[Quadratic refinement on the space of neutralizations]
...
\begin{theorem}[Curvature identity]
...
\begin{corollary}[Orthogonality of sectors with distinct secondary anomaly]
...
\begin{corollary}[Equal-secondary sector]
...
\begin{theorem}[Hypersurface long exact sequence]
...
\begin{theorem}[Universal property of genus completion]
...
\begin{theorem}[One handle becomes a matrix factor after inverting \(u\)]
...
\begin{theorem}[Morita triviality of higher genus after inverting \(u\)]
...
\begin{corollary}[Localization principle]
...
\begin{theorem}[Exterior degeneration modulo \(u\)]
...
\begin{corollary}[Handle action in the strict sector]
...
\begin{theorem}[Handle action on morphism complexes]
...
\begin{proposition}[Universal boundary lift]
...
\hline
Heisenberg & $\Sym^{\mathrm{ch}}(V^*)$ & $k\Omega_{\cH}/z$ & $\cH_k$ \\
...
$\kappaChHodge(\mathrm{Vir}_{26})=13$, but the effective curvature
...
\begin{corollary}[Surface-completed holographic lift]
...
The strict algebra above gives the following hypothesis-driven
Koszul-dual package.
...
The physical identification is exactly the input \(B\simeq A_\partial^!\)
and the central primitive cocycle \(\Theta_g\).  Under that input,
\(B_{\Theta_{g}}\) and \(\mathfrak{G}_{g}(B_{\Theta_{g}})\) are the
strict dg constructions characterized by the universal properties
proved above.
They package:
\begin{itemize}
...
\begin{theorem}[Strict dg-shifted Yangian completion]
...
\begin{proposition}[Boundary reconstruction of the anomaly]
...
\begin{corollary}[Interval transgression formula]
...
\begin{proposition}[Semisimple vanishing criterion]
...
This gives the vanishing statement under the displayed hypothesis.
\end{remark}

\begin{proposition}[Casimir shadow lemma]
...
\begin{proposition}[Scalar shadow lemma]
...
One may now summarize the whole construction in one paragraph.
...
\subsection{Koszul-dual anomaly comparison}
\label{subsec:tholog-koszul-dual-anomaly-comparison}

\begin{theorem}[Koszul-dual bulk comparison]
\label{thm:tholog-koszul-dual-bulk-comparison}
Let \(A_{\partial}\) be a boundary chiral algebra arising from a
3d holomorphic-topological theory
(Theorem~\ref{thm:physics-bridge}), and let \(A_{\partial}^{!}\)
be an open-colour Koszul dual on the chirally Koszul locus of
Theorem~\textup{\ref{thm:lines_as_modules}}.  Assume the genus-\(g\)
curvature class
\[
\kappaChHodge(\cA)\cdot\omega_g\in H^2_{\mathrm{cyc}}(\cA,\cA)
\]
is transported by the Hochschild bridge
(Theorem~\textup{\ref{thm:hochschild-bridge-higher-genus}}) and the
Koszul-dual comparison to a primitive central cocycle
\[
\Theta_g\in Z^2(A_\partial^!)\cap Z(A_\partial^!).
\]
Then the genus-\(g\) bulk algebra determined by this comparison is the
transgression algebra
\[
(A_\partial^!)_{\Theta_g}.
\]
Its modules are precisely \(A_\partial^!\)-modules equipped with a
chain homotopy neutralizing \(\Theta_g\), and its morphism and tensor
products are computed by
Theorems~\ref{thm:tholog-derived-hom} and
\ref{thm:tholog-derived-tensor}.
\end{theorem}

\begin{proof}
The hypotheses put the pair \((A_\partial^!,\Theta_g)\) in the
scope of Definition~\ref{def:tholog-transgression-algebra}.
Proposition~\ref{prop:tholog-universal-property} identifies maps out
of the transgression algebra with maps out of \(A_\partial^!\) plus
a degree-\(1\) null-homotopy of \(\Theta_g\).  The Hom and tensor
statements are the two transgression exact triangles.
\end{proof}

\begin{theorem}[Surface-completed line-side comparison]
\label{thm:tholog-surface-completed-line-comparison}
In the setting of
Theorem~\ref{thm:tholog-koszul-dual-bulk-comparison}, the surface
refinement associated to \(\Theta_g\) is
\[
\mathfrak{G}_{g}\!\big((A_{\partial}^{!})_{\Theta_{g}}\big).
\]
After inverting \(u_g=\eta^2\) it is derived Morita equivalent to
\((A_\partial^!)_{\Theta_g}[u_g^{-1}]\).  Modulo \(u_g\) it is the
exterior handle extension of
\((A_\partial^!)_{\Theta_g}/(u_g)\).
\end{theorem}

\begin{proof}
Apply Definition~\ref{def:tholog-genus-clifford},
Theorem~\ref{thm:tholog-morita-triviality}, and
Theorem~\ref{thm:tholog-exterior-degeneration} to the algebra
\((A_\partial^!)_{\Theta_g}\).
\end{proof}

\begin{theorem}[Boundary reconstruction comparison]
\label{thm:tholog-boundary-reconstruction-comparison}
Assume a quasi-isomorphism of dg algebras
\[
\beta_{\mathrm{der}}\colon A_\partial^!
\stackrel{\sim}{\longrightarrow} HH^\bullet(A_\partial)
\]
compatible with the Hochschild bridge at genus \(g\).  Let
\[
\Theta_{g,\partial}:=\beta_{\mathrm{der}}(\Theta_g)
\in HH^2(A_\partial).
\]
Then \(\Theta_g\) is recovered from the boundary Hochschild class
\(\Theta_{g,\partial}\), and for every \(A_\partial\)-module \(M\) the
obstruction to extending \(M\) across the genus-\(g\) anomaly is the
image of \(\Theta_{g,\partial}\) in
\(\operatorname{Ext}^2_{A_\partial}(M,M)\).
\end{theorem}

\begin{proof}
This is Proposition~\ref{prop:tholog-boundary-reconstruction}
applied to \(B=A_\partial^!\) and transported through
\(\beta_{\mathrm{der}}\).
\end{proof}

\begin{remark}[Formal-local bulk model]
\label{rem:tholog-formal-local-bulk-model}
The homotopy-Koszulity of \(\SCchtop\)
(Theorem~\ref{thm:homotopy-Koszul}) supplies the open-colour Koszul
dual \(A_\partial^!\); the Hochschild bridge supplies the class
\(\Theta_g\); the transgression and genus-Clifford theorems supply the
formal-local bulk and line algebra.  A physical realization adds the
comparison functor identifying this strict model with observables of
the chosen field theory.
\end{remark}

\begin{remark}[$\mathfrak{sl}_4$ cross-orbit Koszul duality]
\label{rem:tholog-sl4-extension}
\index{anomaly completion!sl4@$\mathfrak{sl}_4$}
The $\mathfrak{sl}_3$ Bershadsky--Polyakov computation
(Computation~\ref{comp:bp-anomaly-completion}) has an
$\mathfrak{sl}_4$ analogue once the orbit-wise BRST and Koszul-dual
comparison maps are fixed.
...
\item \emph{Rectangular $(2,2)$}: seven strong generators of
 ...
 the first non-hook, non-principal orbit in this comparison.
...
cross-orbit Koszul duality in the anomaly-completed comparison:
the two $\mathcal W$-algebras have different rank, different
generator spectra, and different ghost systems ($3$ vs $4$ pairs).
The transgression algebra $B_\Theta$ bridges them whenever the
cross-orbit Koszul map carries the genus class to the same primitive
central cocycle on both bar models.

The curvature for all orbits satisfies
$\kappaChHodge(\mathcal W_\lambda^k)=\frac{15(k+4)}{8}
-\kappa_{\mathrm{ghost}}^{(\lambda)}(k)$,
with the ghost term determined by the chosen DS reduction.
A \emph{JM parity criterion} governs level-dependence:
orbits with even-integer Jacobson--Morozov grading ($(3,1)$, $(2,2)$)
have level-independent $\kappa_{\mathrm{ghost}}$ in the orbit-wise
BRST model; the minimal orbit $(2,1,1)$ has odd-integer JM grades and
level-dependent
$\kappa_{\mathrm{ghost}}(k)$.
The orbit data are listed in
Construction~\ref{constr:sl4-hook-anomaly-completion}.
\end{remark}
...
The classical/commutative/coisson bulk class \(\Theta\) is not merely an obstruction; it is the source of a universal odd transgression \(\eta\), a central secondary charge \(u=\eta^{2}\), and a higher-genus Clifford/exterior construction.
...
\subsection{The anomaly-completed Yangian comparison}
\label{subsec:anomaly-completed-yangian-comparison}

\begin{theorem}[Anomaly-completed Yangian comparison]
...
monodromy comparison of
Theorem~\ref{thm:affine-monodromy-identification}
identifies the line side with modules over a strict dg-shifted
Yangian \(Y_\hbar(\fg)\).  Let
\[
\Theta_Y\in Z^2(Y_\hbar(\fg))\cap Z(Y_\hbar(\fg))
\]
be the image of the genus anomaly, and assume
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(\tau_z(\Theta_Y)=\Theta_Y\);
\item \(\Delta_z(\Theta_Y)=\Theta_Y\otimes 1+1\otimes \Theta_Y\);
\item \(\varepsilon(\Theta_Y)=0\).
\end{enumerate}
Then the transgression algebra
\[
Y_{\hbar,\Theta}:=(Y_\hbar(\fg))_{\Theta_Y}
\]
is a strict dg-shifted Yangian extending \(Y_\hbar(\fg)\).  Its
secondary class \(u=\eta^2\) is central.  The quotient
\(Y_{\hbar,\Theta}/(u)\) is the strict-locus specialization, while
the genus completion satisfies
\[
\mathfrak G_g(Y_{\hbar,\Theta})[u^{-1}]
\simeq
\operatorname{Mat}_{2^g}\bigl(Y_{\hbar,\Theta}[u^{-1}]\bigr)
\]
and is therefore derived Morita equivalent to
\(Y_{\hbar,\Theta}[u^{-1}]\).
\end{theorem}

\begin{proof}
The first assertion is Theorem~\ref{thm:tholog-dg-shifted-yangian-completion}
applied to \(Y=Y_\hbar(\fg)\) and \(\Theta=\Theta_Y\).  Centrality
of \(u\) is Proposition~\ref{prop:tholog-secondary-central-class}.
The localized genus statement is Theorem~\ref{thm:tholog-morita-triviality}.
\end{proof}

\begin{remark}[Class-\(\mathbf C\) specialization obstruction]
\label{rem:anomaly-yangian-one-pair}
A comparison with the class-\(\mathbf C\) celestial algebra requires
an evaluation morphism
\[
\operatorname{ev}_{\mathbf C}\colon
Y_{\hbar,\Theta}\longrightarrow \mathcal A_{\mathbf C}
\]
carrying \(u\) to the quartic celestial shadow class of
Theorem~\ref{thm:class-C-quartic-bms} and carrying the genus
Clifford relation to the relation of
Theorem~\ref{V1-thm:g9-clifford-structure}.  The obstruction is the
pair of defects
\[
\operatorname{ev}_{\mathbf C}(\eta^2)-u_{\mathbf C},
\qquad
\operatorname{ev}_{\mathbf C}
(\alpha_i\beta_j+\beta_j\alpha_i-\delta_{ij}u).
\]
The Yangian comparison above supplies the source algebra; the
celestial specialization is exactly the vanishing of these two
defects.
\end{remark}

\section{Synthesis}
\label{sec:ac-core-supplement}

\begin{remark}[Anomaly completion at $K3 \times T^2$]
\label{rem:ac-core-paramodular}
The Vol~II anomaly-completion framework specialises at twisted
11D-SUGRA on $K3 \times T^2$ with $24$ M5-branes on $I_1$ Kodaira
fibres to a Deligne cohomology anomaly class in
$H^2(\overline{\mathcal{A}_2}, \mathcal{O}^*)$; the
Gritsenko--Nikulin Igusa cusp form $\Delta_5$ gives the canonical
trivialiser in the normalization used here,
with
\[
 \mathrm{wt}(\Delta_5)
...
\]
The equality $5=2+3$ is only the $N=1$ K3 mnemonic. Four duality
frames give the same form (Harvey--Moore $1996$; DVV
$1996$--$1997$; DMVV $1997$; Witten $1995$). Universal
$\Phi$-trace identity:
