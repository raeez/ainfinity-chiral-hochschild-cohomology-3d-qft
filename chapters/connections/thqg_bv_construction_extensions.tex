...
$\FM_k(\bC) \times \Conf_k(\bR)$, whose Stasheff identities are
consequences of Stokes' theorem on FM boundaries.  That discussion
was genus-$0$ and tree-level.  This chapter has two objectives.
First (Sections~\ref{sec:thqg-bv-ext-tree-feynman}--\ref{sec:thqg-bv-ext-mk-derivation}):
derive the tree-level $m_k$ from the 3d HT action, the propagator,
and Feynman diagrams on $\FM_k(\bC) \times \Conf_k(\bR)$, showing
that the quantum master equation forces every $\Ainf$ relation to
appear as a boundary integral.  Second
...
and show that the general stable-graph expansion of amplitudes
...
decomposes as
\begin{equation}
\label{eq:thqg-bv-ext-field-decomp}
...
\label{subsec:thqg-bv-ext-propagator}

\begin{definition}[HT propagator; \ClaimStatusProvedHere]
\label{def:thqg-bv-ext-propagator}
The gauge-fixed propagator is
\begin{equation}
\label{eq:thqg-bv-ext-prop}
G(z_1, z_2;\, t_1, t_2)
\;=\;
...
\label{rem:thqg-bv-ext-factored-propagator}
...
$E_1$-operad.  The product space
$\FM_k(\bC) \times \Conf_k(\bR)$ thus receives a form--chain
pairing from each propagator edge.
\end{remark}

\subsection{Feynman diagrams and tree amplitudes}
\label{subsec:thqg-bv-ext-tree-amplitudes}

\begin{definition}[Feynman diagrams for $m_k$]
\label{def:thqg-bv-ext-feynman-diag}
...
\label{prop:thqg-bv-ext-degree-count}
...
\label{rem:thqg-bv-ext-qme-factor}
...
\label{subsec:thqg-bv-ext-mk-tree}
...
\begin{align}
\label{eq:thqg-bv-ext-tree-factorize}
...
\label{subsec:thqg-bv-ext-low-arity}

\begin{example}[$m_1$: the BRST differential]
\label{ex:thqg-bv-ext-m1}
...
\label{subsec:thqg-bv-ext-binary-expansion}

\begin{proposition}[Binary tree expansion; \ClaimStatusProvedHere]
\label{prop:thqg-bv-ext-binary-expansion}
...
\label{rem:thqg-bv-ext-general-vertices}
...
\label{subsec:thqg-bv-ext-trees-to-bar}
...
\item The bar differential $b$ acts by summing over all consecutive
  ...
  $m_\ell$ to that block---this is the boundary of $\FM_k(\bC)$
  at the divisor $D_S$;
...
\label{cor:thqg-bv-ext-bar-c-direction}
...
\label{subsec:thqg-bv-ext-one-loop-diagrams}

\begin{definition}[One-loop diagrams]
\label{def:thqg-bv-ext-one-loop-diag}
...
\label{prop:thqg-bv-ext-one-loop-amp}
The one-loop diagram with $k$ external legs and one internal loop
has amplitude
\begin{equation}
\label{eq:thqg-bv-ext-one-loop-amp}
...
\label{subsec:thqg-bv-ext-kappa}
...
scalar
\begin{equation}
\label{eq:thqg-bv-ext-kappa-def}
\kappa(A)
...
$L_0$ on $A$:
\begin{equation}
\label{eq:thqg-bv-ext-kappa-supertrace}
\kappa(A) = \operatorname{str}_{A}(L_0).
\end{equation}

The genus-$1$ bar differential acquires a curvature:
\begin{equation}
\label{eq:thqg-bv-ext-curved-bar}
...
\label{rem:thqg-bv-ext-kappa-higher-loop}
...
\label{rem:thqg-bv-ext-curved-flat}
...
\label{sec:thqg-bv-ext-two-loop}

\subsection{Two-loop Feynman diagrams}
\label{subsec:thqg-bv-ext-two-loop-diagrams}

\begin{definition}[Two-loop diagrams]
\label{def:thqg-bv-ext-two-loop-diag}
...
\ClaimStatusProvedHere]
\label{prop:thqg-bv-ext-two-loop-amp}
The two-loop contribution to the $k$-point amplitude is
\begin{equation}
\label{eq:thqg-bv-ext-two-loop-amp}
...
\label{subsec:thqg-bv-ext-three-shell}
...
\begin{equation}
\label{eq:thqg-bv-ext-three-shell-id}
...
\label{cor:thqg-bv-ext-genus2-curvature}
At $k = 0$ (vacuum amplitude), the three-shell identity
reduces to:
\begin{equation}
\label{eq:thqg-bv-ext-genus2-vac}
\cA^{(2)}_{\mathrm{vac}}
...
\label{subsec:thqg-bv-ext-stable-graphs}

\begin{definition}[Stable graphs]
\label{def:thqg-bv-ext-stable-graph}
...
\label{const:thqg-bv-ext-stable-amp}
For $\Gamma \in \mathrm{Graph}(g, k)$, the \emph{amplitude} is
\begin{equation}
\label{eq:thqg-bv-ext-stable-amp}
...
stable graphs:
\begin{equation}
\label{eq:thqg-bv-ext-total-amp}
...
\label{subsec:thqg-bv-ext-stokes-stable}
...
\begin{equation}
\label{eq:thqg-bv-ext-stable-stokes}
...
\label{subsec:thqg-bv-ext-modular-bar}
...
\begin{equation}
\label{eq:thqg-bv-ext-modular-bar-id}
\cA_k^{(g)}(a_1, \ldots, a_k)
...
\item the differential on $B^{(g,k)}(A)$ (the modular bar
  ...
  \begin{equation}
  \label{eq:thqg-bv-ext-modular-diff}
  D_{\mathrm{mod}}
...
\label{subsec:thqg-bv-ext-modular-diff}
...
distinct operations:
\begin{equation}
\label{eq:thqg-bv-ext-three-comp}
D_{\mathrm{mod}} = d_{\mathrm{int}} + d_{\mathrm{sew}}
+ \h\,\Delta_{\mathrm{BV}},
\end{equation}
where:
...
\begin{align}
\label{eq:thqg-bv-ext-d-squared}
d_{\mathrm{int}}^2 &= 0,
&
...
\label{subsec:thqg-bv-ext-mc-genus}
...
\label{rem:thqg-bv-ext-cg-principle}
...
\label{rem:thqg-bv-ext-shadow-projections}
The shadow Postnikov tower of Vol~I (the finite-order truncations
...
\label{rem:thqg-bv-ext-algebraic-analytic}
...
\label{rem:thqg-bv-ext-dictionary}
