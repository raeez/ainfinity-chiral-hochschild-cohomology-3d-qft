...
\label{sec:thqg-bv-ext-tree-feynman}
...
\item internal vertices of valence $\ge 3$, each carrying a vertex
 factor $V_n$ of the appropriate degree;
...
\label{def:thqg-bv-ext-tree-amplitude}
...
\label{prop:thqg-bv-ext-qme-closedness}
...
trees}
\label{sec:thqg-bv-ext-mk-derivation}
...
\label{const:thqg-bv-ext-mk-feynman}
...
\subsection{Explicit tree-level operations at low degree}
% label removed: subsec:thqg-bv-ext-low-degree

\begin{example}[$m_1$: the BRST differential]
% label removed: ex:thqg-bv-ext-m1
At degree $k = 1$, $\mathrm{Tree}(1)$ contains a single diagram
...
At degree $k = 2$, $\mathrm{Tree}(2)$ contains a single binary
...
At degree $k = 3$, $\mathrm{Tree}(3)$ contains:
...
$m_3$ satisfies the degree-$3$ Stasheff identity. On the FM boundary
...
\ClaimStatusProvedHere]
\label{thm:thqg-bv-ext-trees-bar}
...
\begin{corollary}[Bar differential $=$ \emph{operadic}
C-direction factorization on $\FM_k(\bC) \times \Conf_k(\bR)$;
\ClaimStatusProvedHere]
% label removed: cor:thqg-bv-ext-bar-c-direction
\emph{Scope.} The identification stated here is \emph{operadic}:
it holds on the local little-discs / Fulton--MacPherson
configuration spaces $\FM_k(\bC) \times \Conf_k(\bR)$ underlying
the Swiss-cheese operad $\SCchtop$. The global promotion to a
Beilinson--Drinfeld factorization coalgebra on $\Ran(X)$ for a
curve $X$ is a separate statement, proved in
Construction~\ref{constr:bar-fact-coalgebra} and
Theorem~\ref{thm:BD-CG-equivalence} of
\texttt{theory/factorization\_swiss\_cheese.tex}
under the hypotheses recorded there
\textup{(}holomorphic translation invariance,
pro-ambient / weight-completed conilpotency for class-M
algebras, genus-$0$ flat vs.\ genus-$g \geq 1$ curved
Feynman-transform reconstruction\textup{)}. The present
corollary does \emph{not} import those hypotheses; it is a
statement about the FM-operadic boundary stratification.

\emph{Assertion.} The bar differential $b$ encodes operadic
factorization along the holomorphic ($\bC$) boundary
stratification of $\FM_k(\bC)$:
\[
...
\]
with the sum over consecutive sub-blocks indexing the codimension-$1$
boundary divisors $D_S \subset \FM_k(\bC)$. Dually, the
deconcatenation coproduct $\Delta$ on $B(A) = T^c(s^{-1}\bar A)$
encodes the topological ($\bR$) boundary stratification of
$\Conf_k(\bR)$: a split of the ordered sequence
$(sa_1, \ldots, sa_k)$ at position $p$ corresponds to a wall
$\{t_p = t_{p+1}\}$ in $\Conf_k(\bR)$. Together,
$(b, \Delta)$ realize the two-coloured
$\SCchtop$-operadic boundary calculus on
$\FM_k(\bC) \times \Conf_k(\bR)$.
\end{corollary}

\begin{proof}
Part~(i) (identification of $b$ with $\bC$-boundary contraction)
is Theorem~\ref{thm:thqg-bv-ext-trees-bar}(ii): the codimension-$1$
boundary of $\FM_k(\bC)$ at the divisor $D_S$ (colliding a
consecutive block $S$) contributes precisely $m_\ell$ on that
block, and the alternating sum over $S$ is $b$.

Part~(ii) (identification of $\Delta$ with $\bR$-deconcatenation)
is definitional: $B(A) = T^c(s^{-1}\bar A)$ is the tensor
coalgebra, whose coassociative coproduct is deconcatenation,
\[
...
\]
The geometric interpretation is the partition of
$\Conf_k(\bR) = \{t_1 > \cdots > t_k\}$ by a cut at $t_p$ into
a left interval $\{t_1 > \cdots > t_p\}$ and a right interval
$\{t_{p+1} > \cdots > t_k\}$.

Part~(iii) (compatibility $\Delta \circ b =
(b \otimes \id + \id \otimes b) \circ \Delta$). This is the
\emph{defining} coderivation property: $b$ extends the collection
$\{m_\ell\}_{\ell \geq 1}$ uniquely to a coderivation of the
tensor coalgebra $T^c(s^{-1}\bar A)$ with respect to
deconcatenation \textup{(}standard fact; see e.g.\
Loday--Vallette \cite{LV12}, Proposition~1.2.3 on the
universal property of the cofree coalgebra\textup{)}.
The geometric content is that the FM boundary $D_S$
(a $\bC$-collision) is disjoint from any deconcatenation
cut in $\Conf_k(\bR)$ (a $\bR$-collision): the two strata
intersect transversally in
$\FM_k(\bC) \times \Conf_k(\bR)$, and Fubini on the product
integral factors the total $D = d_{\FM_k(\bC)}
+ d_{\Conf_k(\bR)} + Q$ into commuting summands.
\end{proof}

\begin{remark}[What this corollary does \emph{not} claim;
ambient-qualifier and holomorphic-factorisation-scope guard]
% label removed: rem:thqg-bv-ext-c-direction-scope
The phrase \emph{holomorphic factorization} is reserved in the
literature \textup{(}Beilinson--Drinfeld \cite{BD04};
Francis--Gaitsgory \cite{FG12}; Costello--Gwilliam
\cite{CG17,CG21}\textup{)} for a $\cD$-module structure on
the Ran space $\Ran(X)$ of a curve $X$, or equivalently a
prefactorization algebra on the topological space $X$. The
corollary above is \emph{not} a statement about
$\Ran(X)$: it is a statement about the local FM operad
$\FM_k(\bC)$ at a single formal disc. The two are bridged,
but \emph{only under} the hypotheses of
Construction~\ref{constr:bar-fact-coalgebra}:
\begin{enumerate}[label=\textup{(\alph*)}]
\item the chiral OPE on $A$ makes
$j_* j^* A^{\boxtimes n} \to \Delta_* A$ into a
$\cD_{X^n}$-module structure compatible with the bar
differential;
\item the cooperadic structure on $(\SCchtop)^{\vee}$ is
homotopy-Koszul self-dual \textup{(}Theorem~\ref{thm:homotopy-Koszul}
of \texttt{theory/factorization\_swiss\_cheese.tex}\textup{)};
\item the ambient is the pro-object /
weight-completed bar complex \textup{(}the raw direct sum for
class-M algebras is conilpotent by finite bar length but fails the
finite-type/completion hypotheses needed for continuous OPE-mode
families; the BD-factorization structure requires the limit
ambient, cf.\ the Virasoro weight-completion example of Vol~I\textup{)}.
\end{enumerate}
Absent (a)--(c), the corollary is an identification of the
bar differential with FM-boundary contraction, \emph{not} with
factorization cohomology on a curve.
\end{remark}
...
\label{sec:thqg-bv-ext-one-loop}
...
\kappaChHodge(A) = \operatorname{str}_{A}(L_0).
\end{equation}
...
Theorem~D): $\kappaChHodge(A) = \sum_n (-1)^n \cdot n \cdot
...
The curvature $\kappaChHodge(\cA)$ is determined by the genus-$1$ tadpole
(one-loop vacuum diagram) alone. Higher-loop vacuum diagrams
on the connected scalar lane evaluate to
...
are determined recursively from $\kappaChHodge(\cA)$ and the tree-level data
by the MC recursion, but they do not modify the value of
\(\kappaChHodge(\cA)\)
itself. This is because $\kappaChHodge(\cA)$ is a genus-$0$-to-genus-$1$
...
\label{def:thqg-bv-ext-three-shells}
\begin{enumerate}[label=(\roman*)]
...
\label{thm:thqg-bv-ext-three-shell}
...
\label{sec:thqg-bv-ext-stable-graph}
...
\label{thm:thqg-bv-ext-stable-stokes}
...
\ClaimStatusProvedHere]
\label{thm:thqg-bv-ext-modular-bar}
...
\ClaimStatusProvedHere]
\label{cor:thqg-bv-ext-three-components}
...
\ClaimStatusProvedHere]
\label{thm:thqg-bv-ext-mc-projections}
The total amplitude
...
Specifically:
\begin{enumerate}[label=(\roman*)]
\item At genus $0$, projection to degree $k$ gives
 $m_k = \Theta_A^{(0,k)}$: the tree-level $\Ainf$ operations;
\item At genus $0$, degree $2$: $\kappa = \Theta_A^{(0,2)}$ is
...
At degree $r = 2$: $\kappa = \operatorname{str}(L_0)$ is the
one-loop vacuum tadpole. At degree $r = 3$: the cubic shadow
$C_A$ is the three-point one-loop amplitude on
$\overline{\cM}_{1,3}$. At degree $r = 4$: the quartic resonance
...
theories by the degree at which their BV amplitudes first receive
...
\section{Synthesis}
\label{sec:bvconstr-ext-supplement}

\begin{remark}[BV construction on $\mathbf H_{\Delta_5}$]
\label{rem:bvconstr-ext-1}
Set \(\Phi_{10}^{\mathrm{sep}}(Z)=
\Phi_{10}^{\mathrm{un}}(Z)/(\eta(\tau)^{24}\eta(\sigma)^{24})\).
The BV construction applied to the K3 bialgebra produces a
homotopy-BV algebra
$(\mathbf H_{\Delta_5},\; \Delta_{\mathrm{BV}},\; \{-,-\}_{\mathrm{BV}},\; \cdot)$
with BV Laplacian
\[
\Delta_{\mathrm{BV}}^{K3} \;=\; \Delta_{\mathrm{Hall}} \;+\; \hbar\, \Delta_{\mathrm{Sieg}} \;+\; \hbar^2\, \Delta_{\Phi_{10}^{\mathrm{sep}}},
\]
a $\hbar$-formal operator on $\mathbf H_{\Delta_5}$ that squares to zero
modulo the bar--cobar coboundary. The BV bracket $\{-,-\}_{\mathrm{BV}}$
recovers the Poisson bracket $\{-,-\}_{\mathrm{Sieg}}$ at
$\hbar\to 0$ (Vol.~II, \S\ref{sec:modpva-ext-supplement}), and deforms
to the full Lie bracket of $\mathfrak g_{\Delta_5}$ through the
BV master equation $\Delta_{\mathrm{BV}}(e^{S/\hbar}) = 0$ at the
specialisation $\hbar^2 = -1/8$. Primary-lit: Batalin--Vilkovisky 1981,
Kontsevich--Soibelman 2008, Cattaneo--Mnev--Reshetikhin 2012,
