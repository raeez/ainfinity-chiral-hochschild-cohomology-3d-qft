...
\chapter{Shifted RTT duality, pairing-annihilator descent, and the rank-one Kleinian test case}
...
\item the \emph{rank-one test case}, where the pairing-annihilator mechanism can
 be checked for coideal stability and matched to a Kleinian-type algebra.
\end{enumerate}
...
construction of Dimofte--Niu--Py \cite{DNP25}. Every
statement below is to be read literally at the level at which it is stated:
construction versus inversion, local kernel versus boundary quotient, finite
stage versus completed limit, and theorem versus conjecture remain sharply
separate.
...
$A_\infty$-algebra, together with exact functors
\[
...
\begin{definition}[Boundary quotient and pairing-annihilator]
...
\]
The associated \emph{orthogonal pairing-annihilator} on the $-\mu$ side is
\[
C_{-\mu,\lambda}^{(N)}
:=
...
\begin{theorem}[Quotient pairing-annihilator descent]
\label{thm:quotient-coideal-descent}
The orthogonal annihilator $C_{-\mu,\lambda}^{(N)}$ is a weightwise
dual dg submodule of
\(\overline{B}(A_{-\mu,N}^{\mathrm{pre}})\), canonically identified
with the quotient dual. It is a coideal or subcoalgebra only after
the relevant spectral coproduct is proved to preserve it. There is a
canonical
identification
\[
...
\begin{theorem}[Shifted RTT pairing-annihilator and rank-one Kleinian test \ClaimStatusProvedHere]
\label{thm:shifted-rtt-pairing-annihilator-kleinian-test}
Fix a chamber for the coweight
\(\mu=(\mu_1,\dots,\mu_N)\), and let
\(\widehat Y_{\hbar}^{\mathrm{RTT}}\) be the completed RTT algebra.
Let \(Y_{\hbar}^{\mathrm{RTT},\vee}_{\ge\mu}\) denote the dual shifted
half selected by the chamber and the transported RTT pairing. The shifted
boundary ideal is the pairing-annihilator
\begin{equation}
\label{eq:shifted-rtt-pairing-annihilator}
I_\mu
=
\left\{
...
\langle x,y\rangle=0
\ \text{for every }\
y\in Y_{\hbar}^{\mathrm{RTT},\vee}_{\ge\mu}
\right\}.
\end{equation}
The quotient
\[
\widehat Y_{\hbar,\mu}^{\mathrm{RTT}}
:=
\widehat Y_{\hbar}^{\mathrm{RTT}}/I_\mu
\]
carries a descended RTT coproduct precisely when
\begin{equation}
\label{eq:shifted-rtt-coideal-condition}
\Delta(I_\mu)
\subset
...
\end{equation}
In this chamber the shifted currents are written
\begin{equation}
\label{eq:shifted-rtt-local-generators}
T_{ij}^{(\mu)}(u)
=
u^{-\mu_i+\mu_j}
\left(
\delta_{ij}+\sum_{r\ge1}T_{ij}^{(r)}u^{-r}
\right).
\end{equation}
The boundary polynomial is imposed through the quantum determinant
\begin{equation}
\label{eq:shifted-rtt-qdet-polynomial}
...
T_{\sigma(2),2}(u-\hbar)
\cdots
T_{\sigma(N),N}(u-(N-1)\hbar)
=P_\mu(u).
\end{equation}

In rank one, a candidate shifted boundary quotient passes the Kleinian
test only if it has the determinantal/Casimir form
\begin{equation}
\label{eq:rank-one-quantized-kleinian-boundary}
A_{\hbar}^{(m)}
:=
...
\right),
\end{equation}
for some \(m\ge2\), together with the coideal condition
\eqref{eq:shifted-rtt-coideal-condition} for the ambient RTT coproduct.
Its PBW associated graded algebra is
\begin{equation}
\label{eq:rank-one-kleinian-associated-graded-general}
\operatorname{gr}A_{\hbar}^{(m)}
\cong
\mathbb C[x,y,z]/(xy-z^m),
\end{equation}
the affine Kleinian surface of type \(A_{m-1}\). Thus a proposed shifted
RTT quotient that either collapses the root sector, fails
\eqref{eq:shifted-rtt-coideal-condition}, or fails
\eqref{eq:rank-one-kleinian-associated-graded-general} is not the
shifted line algebra.
\end{theorem}

\begin{proof}
Equation~\eqref{eq:shifted-rtt-pairing-annihilator} is the literal
orthogonal complement of the chamber-selected dual shifted half under
the RTT pairing. A coproduct descends to a quotient by an ideal \(I_\mu\)
exactly when the image of every element of \(I_\mu\) vanishes in the
quotient tensor product. This is precisely
\eqref{eq:shifted-rtt-coideal-condition}. Formula
\eqref{eq:shifted-rtt-local-generators} records the chamber shift of the
RTT currents; formula \eqref{eq:shifted-rtt-qdet-polynomial} is the
standard antisymmetrized RTT quantum determinant with its shifted
boundary value fixed to \(P_\mu(u)\).

For \eqref{eq:rank-one-kleinian-associated-graded-general}, place
\(x,y,z\) in PBW degree \(1\) and \(\hbar\) in degree \(0\). The two
commutator relations in \eqref{eq:rank-one-quantized-kleinian-boundary}
have commutative associated graded. The two determinantal relations have
leading terms
\[
yx-z^m,
\qquad
xy-z^m.
\]
After passing to the commutative associated graded these coincide, so
\[
\operatorname{gr}A_{\hbar}^{(m)}
\cong
\mathbb C[x,y,z]/(xy-z^m).
\]
The images of \(x\) and \(y\) are nonzero in this associated graded
ring, so the root sector is not killed. The final sentence is the
contrapositive of the quotient-coproduct criterion and the associated
graded computation.
\end{proof}

\section{The rank-one test case: from pairing-annihilators to Kleinian surfaces}
...
Proposition~\ref{prop:stage-one-cartan-collapse-appendix} shows that the first genuinely
...
\begin{definition}[Rank-one pairing-annihilator]
\label{def:rank-one-orthogonal-coideal}
Let
\[
q_c\colon U(\mathfrak{sl}_2)\to A_c
\]
be the quotient map. The associated orthogonal pairing-annihilator is
\[
...
\begin{theorem}[Explicit rank-one coideal stability and curved cobar model \ClaimStatusProvedHere]
\label{thm:explicit-rank-one-orthogonal-coideal}
The pairing-annihilator $C_c$ is the curved dg subcoalgebra of
...
The rank-one pairing-annihilator already captures the local algebraic music of
the boundary layer. The local RTT kernel is unchanged at $N=1$;
nontriviality enters by a central quadratic boundary quotient; the dual object
becomes a curved coideal after the explicit stability calculation; and its
cobar algebra is the quantized rank-one
...
Coulomb-branch normalization, the rank-one pairing-annihilator $C_c$ should agree
with the finite-stage $A_1$ instance of the general pairing-annihilator
...
\item The genuinely new shifted/truncated datum lives in the boundary quotient,
 whose dual avatar is the pairing-annihilator sub-dg-module
 \textup{(}Theorem~\ref{thm:quotient-coideal-descent}\textup{)}.
\item In the first nontrivial rank-one case, that pairing-annihilator
 satisfies the extra coideal-stability calculation and produces a curved
 cobar model whose associated graded is the affine Kleinian surface of type $A_1$
...
orthogonal pairing-annihilator, and rank-one test case.
\end{proof}
...
\item Identify the pairing-annihilator $C_{-\mu,\lambda}^{(N)}$ with the
 expected dg-shifted or Coulomb-branch dual presentation and prove the
 relevant coproduct stability.
...
Theorem~\ref{thm:unified-chiral-QG}.
\end{enumerate}
...
The shifted RTT presentation, the pairing-annihilator mechanism,
...
(Theorem~\ref{thm:explicit-rank-one-orthogonal-coideal}) and must be
verified case-by-case at higher rank as the construction is lifted to
the completed tower.
\end{remark}
...
In what follows, ``orthogonal'' refers exclusively to the
bar-pairing-annihilator (Definition above), an entirely different
structure: a pairing-annihilator subobject in the ambient bar coalgebra
or chiral bialgebra, defined as the orthogonal complement of a
subalgebra under the bar pairing. It should be called a coideal only
after coproduct stability has been proved.
...
$Q_\fg^{k,f,\mu}$; \ClaimStatusConditional]
\label{rem:shifted-rtt-uhm}
The shifted RTT and pairing-annihilator/coideal package of this appendix
is the $\mu \ne 0$ specialisation of the unified chiral quantum
group $Q_\fg^{k,f,\mu}$
(Theorem~\ref{thm:unified-chiral-QG}) at
