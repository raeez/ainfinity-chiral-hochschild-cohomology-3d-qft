...
\item The hero: \emph{root multiplicity one}.  Every root of a
  simple Lie algebra has multiplicity one
  (Theorem~\ref{thm:root-space-one-dim}).  This single fact forces
  the multilinear Lie space controlling each filtration sector to be
  one-dimensional.  A cohomological obstruction valued in a
  one-dimensional space is a scalar, determined by one equation, which
  the coproduct rigidity supplies.  The explicit BCH coefficient
  $1/n$ at filtration~$n$ (computed by the beta integral
  $\int_0^1(1-t)^{n-1}\,dt=1/n$ in
  Theorem~\ref{thm:dynkin-bch-path}) identifies the scalar but is
  irrelevant to the vanishing of the obstruction class: it is the
  one-dimensionality that does the work.
...
The BCH formula at filtration~$n$ involves intricate nested
...
(Theorem~\ref{thm:steinberg-presentation}).  The complete
strictification theorem says that, once the spectral
quasi-factorization package has been constructed, the passage from
quasi to strict is not an additional datum but an automatic
consequence of the root structure.  Just as the Decomposition
Theorem (purity of IC complexes) makes the Hecke algebra arise
inevitably from the geometry of~$G/B$, root multiplicity one makes
the strictification step arise inevitably from the Lie theory
of~$\fg$.
...
Drinfeld obstructions could survive.  The frontier admits a
precise stratification (Conjecture~\ref{conj:affine-strictification}).
\end{remark}
