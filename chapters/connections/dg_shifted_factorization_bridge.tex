...
\item A quasi-factorization theorem: every residue-bounded complete
  dg-shifted Yangian equipped with a bar-horizontal strictification
  canonically yields a spectral
...
For non-path sectors (e.g., the $D_4$ star), the BCH coefficient on the product side is determined as follows: root-space one-dimensionality (Theorem~\ref{thm:root-space-one-dim}) forces the multilinear BCH contribution to be a scalar multiple of the unique generator of the root space $\fg_\alpha$.  The scalar is computed by applying both sides to a test vector in the root space and using the Jacobi collapse lemma (Lemma~\ref{lem:jacobi-collapse}), which shows that the only surviving contribution is the iterated bracket $[e_{\alpha_1}, [e_{\alpha_2}, \ldots]]$, whose coefficient in the BCH expansion is $1$ by the Dynkin formula.
...
obstruction class, so the first nontrivial spectral Drinfeld class
vanishes at every filtration.  Starting from any spectral
quasi-factorization datum for $A$, Theorem~\ref{thm:strictification-spectral-cohomology}(3)
kills that first nontrivial term by a filtered twist.  Iterating this
filtration by filtration produces a twist with trivial quasi-associator,
...
(Theorem~\ref{thm:steinberg-presentation}).  The complete strictification
...
datum but an automatic consequence of the root structure.  Just as the Decomposition Theorem
(purity of IC complexes) makes the Hecke algebra arise inevitably from
the geometry of $G/B$, root multiplicity one makes the strictification
step arise inevitably from the Lie theory of $\fg$.
...
the higher-dimensional root spaces.  The frontier admits a
precise stratification.
\end{remark}

\begin{conjecture}[Affine strictification; \ClaimStatusConjectured]%
\label{conj:affine-strictification}%
\index{strictification!affine conjecture}%
Complete strictification extends to all untwisted affine Lie
algebras~$\widehat\fg$.  Specifically:
\begin{enumerate}
...
\item \emph{Mixed sectors} (real $+$ imaginary constituents):
  a sum of roots $\beta_1 + \cdots + \beta_p$ involving both
  real and imaginary constituents lands in a root
  $\gamma$ of $\widehat\fg$, which is either real
  (multiplicity~$1$, covered by~(1)) or imaginary
  (covered by~(2)).  The precise mechanism for the
  reduction to these two cases --- showing that the
  multilinear Lie monomial computation in mixed sectors
  is controlled by the real and imaginary sub-computations
  separately --- requires a detailed analysis of how
  iterated brackets of real and imaginary root vectors
  interact in $\widehat\fg$; this analysis is not carried
  out here.
\end{enumerate}
\emph{Caveat.}  The Cartan gauge-twist theorem requires that
...
sector the commutation hypothesis is satisfied.  The
hypothesis should be verified explicitly for the full
spectral connection before the conjecture is promoted
to a theorem.
...
doubling theorem of Chapter~\ref{chap:affine-half-space-bv}
(Theorem~\ref{thm:general-half-space-bv}), which embeds the half-space
reflected weight identity into the standard whole-space Stokes
identity.  The \emph{closed color} (chiral Koszulness) is the only
one that can fail, controlled by the loop complexity of the specific
algebra (Theorem~\ref{thm:width-bound}).  Two of the three Swiss-cheese
colors are universally rigid; the third is algebraically free.  The bar
complex carries all three.
...
residue $r$-matrix (bar-theoretic, cf.~AP19).  The bracket at
...
\end{align}
Crucially, $\mathsf{T}_n^{(r)} = \binom{n+r-1}{r}\,L_{-n-r}$ is a
...
$A_\infty$ shadow tower exposed: the entire scattering content of
