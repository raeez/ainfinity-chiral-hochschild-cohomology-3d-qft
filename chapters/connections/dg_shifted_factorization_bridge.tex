...
\begin{theorem}[Conditional strictification criterion; \ClaimStatusConditional]
Assume:
\begin{enumerate}
...
\]
so that $r(z) = \sum_{r \ge 0}\Omega_r\,z^{-r-1}$ is the bar-theoretic
collision-residue $r$-matrix. The bracket at
...
finite DS/KZ HPL datum
$(Q_{\mathrm{DS}},m_2,i,p,h_{\mathrm{DS}},\delta_{\mathrm{DS}})$
via the resolvent tree formula, but is not finitely generated:
$m_k \ne 0$ for all $k \ge 3$ at generic~$c$ on the wheel-residue
survival surface.
\end{remark}
...
open for logarithmic $W(p)$). The concrete comparison obstruction is
...
\cite{Mukai1987,Lusztig1990}. Compare
