...
The first is the operadic language: $\cA^!$ is a dg-shifted
Yangian (Theorem~\ref{thm:Koszul_dual_Yangian}), whose
axioms \emph{are} the $\SCchtop$-operadic identities
decomposed by color.  The physical
...
a dg-shifted Yangian (i.e.\ an $\SCchtop$-algebra) is
\emph{already} a strict factorization quantum group.  The
correct claim is subtler and, in fact, stronger: the
...
bar complex encodes the sewing kernel; the
$\bR$-direction factorization encodes the line-operator
