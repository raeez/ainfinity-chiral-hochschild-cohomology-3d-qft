...
landscape:
\begin{itemize}
\item Heisenberg: $p_{\max}=2$, so
$r_{\cH}(z)=k\Omega_{\cH}/z$ with $\Omega_{\cH}=J\otimes J$; the
rank-one coefficient $k/z$ is a single pole.
\item Affine $\widehat{\fg}_k$: $p_{\max}=2$.  The trace-form
 presentation is $r_{\mathrm{tr}}(z)=k\,\Omega_{\mathrm{tr}}/z$.  The KZ/Sugawara
...
comparison with the quantised Calogero--Moser Hamiltonians requires an explicit
...
the quantised Calogero--Moser Hamiltonians requires the normalization dictionary of
...
affine-KM trace-form representative $k\Omega_{\mathrm{tr}}/z$ and the
...
The trace-form $r_{\mathrm{tr}}(z)=k\,\Omega_{\mathrm{tr}}/z$, KZ/Sugawara
...
\section{The primary-state Virasoro Ward factor}
\label{sec:dnp-master-virasoro-r}

\begin{computation}[Primary-state Virasoro Ward factor]
\label{comp:dnp-master-virasoro-r-matrix}
\index{Virasoro!primary-state Ward factor}
\index{Ward identity!Virasoro primary-state factor}
Fix a highest-weight $\mathrm{Vir}_c$-module and let
$\pi_h$ denote projection to the highest-weight line on which
$L_0$ acts by the conformal weight~$h$ and $L_n$, $n>0$, acts by
zero.  On this one-dimensional primary-state quotient, the
Virasoro collision-residue connection is
\begin{equation}
\label{eq:dnp-master-primary-ward-connection}
\nabla_h
\;=\;
d-\left(\frac{2h}{z}+\frac{c}{2z^3}\right)\,dz .
\end{equation}
On a chosen branch of $\log z$, with normalization fixed by the
condition that the $c=0$ factor is $z^{2h}$, the scalar parallel
transport is
\begin{equation}
...
Only even powers of $1/z$ appear after the primary factor is removed.
\end{computation}

\begin{proof}
The Virasoro collision residue in
\S\ref{sec:dnp-master-collision-residue} is
\[
r_{\mathrm{Vir}}^{\mathrm{coll}}(z)
=\frac{c/2}{z^3}\,\mathbf 1\otimes\mathbf 1+\frac{2T}{z}.
\]
The conformal Ward identity for a primary field $\phi_h$ is
\[
T(u)\phi_h(0)
\sim
\frac{h}{u^2}\phi_h(0)+\frac{1}{u}\partial\phi_h(0).
\]
After projection to the highest-weight line, the descendant term
$\partial\phi_h=L_{-1}\phi_h$ is removed and $T$ contributes the
$L_0$ eigenvalue~$h$.  Thus $\pi_h(2T/z)=2h/z$.  The central
collision term is scalar and contributes $c/(2z^3)$.  This gives
\eqref{eq:dnp-master-primary-ward-connection}.

The horizontal-section equation for
\eqref{eq:dnp-master-primary-ward-connection} is
\[
\frac{dR}{dz}
=
\left(\frac{2h}{z}+\frac{c}{2z^3}\right)R .
\]
Its logarithm is
\[
\log R = 2h\log z-\frac{c}{4z^2},
\]
on the chosen logarithm branch.  Exponentiating gives
\eqref{eq:dnp-master-virasoro-r-closed}.  The remaining expansion is
the ordinary exponential series for the single monomial
$-c/(4z^2)$:
\[
...
\]
Every exponent is of the form $-2k$, hence no odd inverse powers occur.
\end{proof}

The scalar factor
\eqref{eq:dnp-master-virasoro-r-closed} is not the full Virasoro
spectral $R$-matrix.  The full line-sector object is the completed
operator-valued parallel transport with residue
$r_{\mathrm{Vir}}^{\mathrm{coll}}(z)$, descendants, channel
decomposition, and the higher $\Ainf$ corrections; on the
non-perturbative comparison locus it is compared with the
Ponsot--Teschner fusion kernel, not with this one-dimensional scalar
quotient.

\section{The $\Walg_3$ Gaudin Hamiltonians: rank-$2$ local calculation}
\label{sec:dnp-master-w3-hamiltonians}

For $\Walg_3$ with $p_{\max} = 6$ and $k_{\max} = 5$, the GZ26
local Hamiltonian candidates are differential operators of order
$4 = k_{\max} - 1$ on the conformal block space.  On a supplied
$\Walg_3$ Gaudin/$RLL$ surface these candidates become commuting
Hamiltonians.  The collision-depth expansion reads
\[
H_i^{(\Walg_3)}
\;=\;
\sum_{j\neq i}
...
The local PVA/Jacobi calculation supplies the coefficient package in
the displayed expansion; commutativity is an additional theorem of the
chosen $\Walg_3$ Gaudin/$RLL$ algebra and is not a consequence of this
coefficient list alone.
...
$k\Omega_{\mathrm{tr}}/z$; the KZ/Sugawara representative is $\Omega/((k+3)z)$.
\end{computation}
...
$r(z)$ are realised on the diagonal
...
\item[(c)] \emph{Volume~III $\Phi$ functor.} The CY-to-chiral
 ...
 for $Y_{\hbar}(\mathfrak{so}(4|20))$ (scope $n=2$);
	$\bC^{3}$ through the positive half
	$Y_\hbar(\widehat{\mathfrak{gl}}_1)^+$ and its completed
	Drinfeld double as F5 of the Schiffmann--Vasserot CoHA
	$r$-matrix; toric CY${}_{3}$ through
