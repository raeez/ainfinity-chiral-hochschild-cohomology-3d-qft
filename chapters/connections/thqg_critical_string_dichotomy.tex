...
This section constructs the algebraic mechanism by which these
three facts coalesce into a single structural dichotomy,
...
$u = 0$ is the \emph{no-ghost theorem} expressed in bar-cobar language;
...
(Definition~\ref{def:modular-curvature-direct}) and $\omega_g$ the
...
In~\eqref{eq:g9-transgression-def}, the algebra $B\langle \eta \rangle$
...
\end{align}
Substituting into~\eqref{eq:g9-dsq-simplify}:
\[
d_{B_\Theta}^{\,2}(b)
= d^2(b) - [\lambda, b]
= [\lambda, b] - [\lambda, b]
= 0,
\]
using $d^2(b) = [\lambda, b]$ from~\eqref{eq:g9-curved-relation}.
...
\begin{proof}
From~\eqref{eq:g9-BT-decomposition}, the ideal $(\eta) = B\eta + B\eta^2
...
(Convention~\ref{conv:higher-genus-differentials}) is a
...
\item \emph{Shadow tower level.}\; Under the identification
$\kappa(\cA) = \Theta_\cA^{\leq 2}$
(Definition~\ref{def:modular-curvature-direct}), the
secondary anomaly is the arity-$2$ shadow evaluated on the genus-$g$
...
(Proposition~\ref{prop:genus-g-curvature-package}), so $u = 0$
iff $\kappa(\cA) = 0$.

\emph{(iv)} By Definition~\ref{def:modular-curvature-direct},
...
= u.
\end{equation}
The last equality uses the normalization~\eqref{eq:g9-intersection}
of the symplectic basis.  The factor~$u$ appears because the
integrand is $(\eta^{(g)})^2 = u$, and the fiber integral computes
the intersection number.

We verify~\eqref{eq:g9-clifford-compute} directly from the
fiber integral calculus.  By definition,
\[
\oint_{A_k} \eta^{(g)} \cdot \oint_{B_l} \eta^{(g)}
+ \oint_{B_l} \eta^{(g)} \cdot \oint_{A_k} \eta^{(g)}
= \oint_{A_k} \oint_{B_l} d\bigl(\eta^{(g)} \cdot \eta^{(g)}\bigr)
= \oint_{A_k} \oint_{B_l} d(u)
= \delta_{kl} \cdot u,
\]
where the first equality uses the Leibniz rule
$d(\eta^{(g)} \cdot \eta^{(g)}) = (d\eta^{(g)}) \cdot \eta^{(g)}
+ \eta^{(g)} \cdot (d\eta^{(g)})$ and the identification
$(\eta^{(g)})^2 = u$, and the last equality uses the
intersection pairing
$\langle A_k, B_l \rangle = \delta_{kl}$.
...
Relations~\eqref{eq:g9-clifford-ab}--\eqref{eq:g9-clifford-bb}
...
to~\eqref{eq:g9-clifford-ab}--\eqref{eq:g9-clifford-bb}.
...
\emph{(ii)} Set $u = 0$
in~\eqref{eq:g9-clifford-ab}--\eqref{eq:g9-clifford-bb}: all
...
In the BRST language (Remark~\ref{thm:bv-bar-bridge}), the generators
...
\end{equation}
(Theorem~\ref{thm:vir-genus1-curvature}), arising from the quartic
...
\subsubsection{Effective curvature}
% label removed: subsubsec:effective-curvature
\index{effective curvature|textbf}
...
\emph{effective curvature} is
\begin{equation}% label removed: eq:g9-kappa-eff
\kappa_{\mathrm{eff}} \;:=\; \kappa - \kappa'
\;=\; \kappa(\cA) - \kappa(\cA^!).
\end{equation}
For the Virasoro: $\kappa_{\mathrm{eff}} = c/2 - (26-c)/2 = c - 13$.
\end{definition}

\begin{remark}[Effective curvature vs.\ complementarity sum]
% label removed: rem:g9-complementarity-constraint
The sum $\kappa + \kappa'$ is a topological invariant of the Koszul
pair.  The effective curvature $\kappa_{\mathrm{eff}} = \kappa - \kappa'$
measures the asymmetry: $\kappa = (\kappa + \kappa' +
\kappa_{\mathrm{eff}})/2$, $\kappa' = (\kappa + \kappa' -
\kappa_{\mathrm{eff}})/2$.

For affine Kac--Moody, $\kappa + \kappa' = 0$
(Remark~\ref{rem:vir-vs-km-complementarity}), so
$\kappa_{\mathrm{eff}} = 2\kappa$.  For the Virasoro,
$\kappa + \kappa' = 13$ and $\kappa_{\mathrm{eff}} = c - 13$ vanishes
at $c = 13$, not $c = 26$.
\end{remark}
...
\end{equation}
recovering Theorem~\ref{thm:vir-all-genera}.
...
Theorems~\ref{thm:topological-regime}
and~\ref{thm:gravitational-regime} to $\cA = \mathrm{Vir}_c$ with
$\kappa = c/2$.  The free energy
formula~\eqref{eq:g9-partition-topo} is Theorem~\ref{thm:vir-all-genera}.
...
Proposition~\ref{prop:vir-complementarity}: $F_g(\mathrm{Vir}_{26}) +
F_g(\mathrm{Vir}_0) = 13 \cdot \lambda_g^{\mathrm{FP}} + 0 =
13 \cdot \lambda_g^{\mathrm{FP}}$.
\end{proof}

\begin{remark}[The no-ghost theorem in bar-cobar language]
...
framework, this is the statement that the Koszul dual
$\mathrm{Vir}_{26}^! \simeq \mathrm{Vir}_0$ is in the gravitational
regime: $u_0 = 0$, so the ghost sector is the exterior algebra
$\bigwedge^*(\Bbbk^{2g})$ with no Clifford entanglement.  The
``physical states'' are the bar cohomology
$H^*(\barB^{(g)}(\mathrm{Vir}_0))$, a genuine cohomology theory
(no curvature correction needed).  The ``ghosts'' are
$\alpha_k, \beta_k$, corresponding to $b$-$c$ ghost oscillators.
...
\frac{|B_{2g}|}{(2g)!}$.  These agree with
Theorem~\ref{thm:vir-all-genera} and
Computation~\ref{comp:vir-physical-cc}.
\end{computation}
...
\begin{proof}
\emph{(i)} is Theorem~\ref{thm:gravitational-regime}(iii) applied to
...
\item Effective curvature vanishes: $\kappa_{\mathrm{eff}} = 0$.
\end{enumerate}
\end{theorem}
...
\item \emph{Uncurved self-duality at $c = 0$}
(Theorem~\ref{thm:virasoro-self-duality}).  Here $\kappa = 0$: the bar
complex is a genuine cochain complex, self-duality is of \emph{classical}
(quadratic) type, and the Clifford completion is in the gravitational
regime.

\item \emph{Curved self-duality at $c = 13$}
(Theorem~\ref{thm:vir-self-duality-c13}).  Here $\kappa = 13/2 \neq 0$:
...
$\kappa_{\mathrm{eff}} = c - 13$, and falls into the following
pattern:
...
\begin{proof}
Compilation of
Theorems~\ref{thm:critical-string-dichotomy},
\ref{thm:ghost-sector-c26},
and~\ref{thm:vir-self-duality-c13}, together with
Proposition~\ref{prop:vir-complementarity}.
\end{proof}
...
\subsection{The shadow tower at the critical points}
% label removed: subsec:shadow-critical
\index{shadow tower!critical points}

The critical string dichotomy interacts with the shadow Postnikov tower
$\Theta_\cA^{\leq r}$ through the modular characteristic
$\kappa = \Theta_\cA^{\leq 2}$.

\begin{proposition}[Shadow tower at critical central charges;
\ClaimStatusProvedHere]
% label removed: prop:shadow-critical
\index{shadow tower!critical central charges}
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{At $c = 0$}: $\kappa = 0$.  The shadow tower is
identically zero: the OPE is purely quadratic
($T_{(3)}T = 0$), the bar complex is uncurved, and all higher
shadows vanish.

\item \emph{At $c = 13$}: $\kappa = 13/2 \neq 0$.  The shadow tower
...
The dual shadow tower
$\Theta_{\mathrm{Vir}_0}^{\leq r} = 0$ for all $r \geq 2$.
Thus $\mathrm{Vir}_{26}$ alone generates the shadow tower of
the Koszul pair.
\end{enumerate}
\end{proposition}
...
vanishes and the OPE is quadratic.  All higher shadows are zero.
...
\emph{(iii)} $\kappa(\mathrm{Vir}_0) = 0$ implies
$\Theta_{\mathrm{Vir}_0}^{\leq r} = 0$ for all $r$.
\end{proof}
...
(Remark~\ref{rem:virasoro-resonance-model}).  At $c = 26$, this
partner is $\mathrm{Vir}_0$, the trivial algebra.  The depth-zero
resonance shadow saturates: there is nothing simpler than the
uncurved algebra.  This is the algebraic content of the critical
string: $\mathrm{Vir}_{26}$ has pushed all complexity into itself.
\end{remark}
...
\item The period matrix $\tau_{kl} = \oint_{B_l}\omega_k$ appears as
the overlap $\langle \alpha_k, \beta_l \rangle = \tau_{kl}$ in the
...
(\mathrm{Im}\,\tau)^{-1}_{kl}$ in the Arakelov normalization.
Integration against $u = \kappa \cdot \omega_g$ yields
$\kappa \cdot \delta_{kl} = \delta_{kl}\, u$, matching the
Clifford relation.

\emph{(iv)} $\tau_{kl} = \oint_{B_l}\omega_k$ determines the
...
(Proposition~\ref{prop:genus-g-curvature-package}) is the trace of the
...
maximally asymmetric: $|\kappa_{\mathrm{eff}}| = 13$.
...
(i) Theorem~\ref{thm:vir-all-genera};
(ii) complementarity;
(iii) Theorem~\ref{thm:ghost-sector-c26};
(iv) Remark~\ref{rem:vir-trichotomy-genera};
(v) Definitions~\ref{def:transgression-algebra}
and~\ref{def:bar-transgression-complex};
(vi) Theorems~\ref{thm:topological-regime}
and~\ref{thm:gravitational-regime};
(vii) Theorem~\ref{thm:topological-regime}(iii).
\end{proof}
...
\begin{proof}
The sequence~\eqref{eq:g9-localization} is the standard localization
...
relations~\eqref{eq:g9-clifford-ab}--\eqref{eq:g9-clifford-bb} gives
the exterior algebra, confirming (i).  Part (ii) is
Theorem~\ref{thm:topological-regime}.  Part (iii) is the long exact
...
\begin{proof}
The coproduct~\eqref{eq:g9-eta-coproduct} is forced by the
...
1 \otimes \lambda$.  Since $\lambda = \kappa \cdot \omega_g$ is
already primitive in $\barB^{(g)}(\cA)$ (it is a scalar coderivation:
$\Delta(\lambda) = \lambda \otimes 1 + 1 \otimes \lambda$), this is
consistent.  The counit $\varepsilon(\eta) = 0$ follows from
$\varepsilon(\lambda) = 0$.  Coassociativity:
$(\Delta \otimes \id)\Delta(\eta) = \eta \otimes 1 \otimes 1 +
1 \otimes \eta \otimes 1 + 1 \otimes 1 \otimes \eta =
(\id \otimes \Delta)\Delta(\eta)$.
\end{proof}

\begin{corollary}[Coproduct of the secondary anomaly;
\ClaimStatusProvedHere]
% label removed: cor:g9-u-coproduct
The secondary anomaly element has coproduct
\begin{equation}% label removed: eq:g9-u-coproduct
\Delta(u) = u \otimes 1 + 2\eta \otimes \eta + 1 \otimes u.
\end{equation}
...
coalgebra, the cross term survives, giving~\eqref{eq:g9-u-coproduct}.
\end{proof}
...
indecomposables of the associated Lie algebra.  The element
...
multiplication~\eqref{eq:g9-BT-multiplication} because $f$ is an
...
\Dg{g})$ of Remark~\ref{rem:g9-mc-relation} is the unique factorization
...
(Theorem~\ref{thm:genus-universality}).  The factor $1/2^g$ accounts
...
extending the BV/bar dictionary of Remark~\ref{thm:bv-bar-bridge}?  The quantum master
...
(Proposition~\ref{prop:hodge-clifford}) suggests a connection to the
Mukai Hodge structure on $K(\Sigma_g)$.  Does the spectral sequence of
Construction~\ref{constr:clifford-spectral} compute the Mukai pairing?
...
(Remark~\ref{rem:g9-wall-crossing}) should have a categorical lift:
...
(Theorem~\ref{thm:localization-sequence}) suggests a
