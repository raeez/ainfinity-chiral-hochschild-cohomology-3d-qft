...
Theorems \texttt{vir-all-genera}, \texttt{topological-regime},
\texttt{gravitational-regime}, \texttt{critical-string-dichotomy},
\texttt{ghost-sector-c26}, \texttt{vir-self-duality-c13} and
Propositions \texttt{vir-complementarity},
\texttt{virasoro-c26-selfdual},
\texttt{genus-g-curvature-package}; their scope qualifiers
propagate. Load-bearing cross-volume citations carry the explicit
``Volume~I'' prefix; the semantic content
(complementarity tallies, regime dichotomy, secondary-anomaly
data) is inherited without restatement.
\end{remark}

The curvature $\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g$
...
$\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g \cdot \id$
(Proposition~\ref{V1-prop:genus-g-curvature-package}),
with $\kappaChHodge(\cA) = \mathrm{Tr}_\cA$
...
\begin{proof}
Verification: $d_{B_\Theta}^{\,2}$ on generators.
...
d := \dfib,
\quad
\lambda := \kappaChHodge(\cA) \cdot \omega_g.
\end{equation}
...
\big/ (\eta^2 - \kappaChHodge(\cA) \cdot \omega_g),
\end{equation}
...
The curvature $\kappaChHodge(\cA) \cdot \omega_g$ is the genus-$g$ scalar
shadow of the universal MC element $\Theta_\cA$:
$\kappaChHodge(\cA) \cdot \omega_g = \mathrm{Sh}_{g,0}(\Theta_\cA)$
...
u \;:=\; \eta^2 \;=\; \kappaChHodge(\cA) \cdot \omega_g
\;\in\; B_\Theta^{(g)}(\cA).
\end{equation}
...
\item \emph{Factorization.}\; $u = \kappaChHodge(\cA) \cdot \omega_g$, where
$\kappaChHodge(\cA) \in \Bbbk$ is the scalar modular characteristic and
$\omega_g \in H^{1,1}(\Sigma_g)$ is the Arakelov class. The element
$u$ vanishes if and only if $\kappaChHodge(\cA) = 0$.

\item \emph{Shadow obstruction tower level.}\; Under the identification
$\kappaChHodge(\cA) = \Theta_\cA^{\leq 2}$
...
iff $\kappaChHodge(\cA) = 0$.

\emph{(iv)} By Definition~\ref{V1-def:modular-curvature-direct},
$\kappaChHodge(\cA) = \Theta_\cA^{\leq 2}$ is the weight-$2$ shadow.
\end{proof}
...
\kappaChHodge(\cA) \cdot c_1(\mathbb{E})\bigr).
\]
Its vanishing $\{u = 0\}$ is the
\emph{uncurved locus} $\{\kappaChHodge(\cA) = 0\}$: the algebraic
...
\end{align}
where $u = \kappaChHodge(\cA) \cdot \omega_g$ is the secondary anomaly
...
$u = \kappaChHodge(\cA) \cdot \omega_g$.
...
Suppose $\kappaChHodge(\cA) = 0$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
...
pole $T_{(3)}T = c/2$ in the Virasoro OPE. The completed
Koszul-dual branch has strict same-family representative
$\mathrm{Vir}_{26-c}$
...
$\kappa := \kappaChHodge(\cA)$ and $\kappa' := \kappaChHodge(\cA^!)$, the
...
\[
\kappa_{\mathrm{eff}} \;:=\;
\kappaChHodge(\mathrm{matter}) + \kappaChHodge(\mathrm{ghost}),
\]
...
$\kappaChHodge(\mathrm{Vir}_{26-c}) = (26-c)/2$ are positive:
the Koszul pair lives entirely on the anti-de~Sitter side
of the anomaly budget. At $c = 26$ the dual characteristic
$\kappaChHodge(\mathrm{Vir}_{26-c}) = (26-c)/2$ vanishes;
the Koszul dual is uncurved, and the matter system
absorbs the full anomaly $\kappaChHodge(\mathrm{Vir}_{26}) = 13$.

For $c > 26$ the Koszul dual has
$\kappaChHodge(\mathrm{Vir}_{26-c}) = (26-c)/2 < 0$:
the secondary anomaly $u_{26-c} = \kappaChHodge(\mathrm{Vir}_{26-c})\cdot\omega_g$
reverses sign, and the fiberwise curvature
$\dfib^{\,2} = \kappaChHodge(\mathrm{Vir}_{26-c})\cdot\omega_g$
...
There $\kappaChHodge(\mathrm{Vir}_{13}) = \kappaChHodge(\mathrm{Vir}_{13}^!) = 13/2$,
...
\otimes \bigwedge\nolimits^*(\Bbbk^{2g}).
\end{equation}
The scalar genus-$g$ obstruction budget is carried by
$\mathrm{Vir}_{26}$; the dual $\mathrm{Vir}_0$ has zero scalar
curvature and still carries its uncurved bar complex together with
the exterior ghost factor.
\end{enumerate}
\end{theorem}
...
scalar complementarity statement is
...
$c = 26$ bosonic string are transverse oscillators. In the
...
\item $\kappaChHodge(\mathrm{Vir}_{13}) = 13/2$: symmetric anomaly budget,
$\kappa = \kappa' = 13/2$.
...
\begin{proof}
\emph{(i)} On the strict same-family Virasoro comparison surface,
$\mathrm{Vir}_{26-c}$ equals $\mathrm{Vir}_{13}$ at $c = 13$, giving
the Virasoro fixed point.
...
higher-degree tower is not fixed by $\kappaChHodge(\mathrm{Vir}_0)=0$ alone.
\end{enumerate}
\end{proposition}

\begin{proof}
\emph{(i)} $\kappaChHodge(\mathrm{Vir}_0) = 0$; at $c = 0$ the quartic pole
...
follows from $\kappaChHodge(\mathrm{Vir}_0)=0$ alone.
...
\emph{(iii)} $\kappaChHodge(\mathrm{Vir}_0) = 0$ gives the vanishing of the
...
At $c = 26$:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Scalar anomaly budget.}\;
...
The modular characteristic is $\kappaChHodge(\mathcal{W}_{N,c}) = c \cdot
...
\item The \emph{gravitational regime} ($u = 0$) has vanishing scalar
curvature. The boundary bar complex is uncurved; its higher
cohomological content is not killed by the scalar shadow, and the
ghost sector is $\bigwedge^*(H_1(\Sigma_g))$.
...
sector ($\mathrm{Vir}_0$, gravitational) has zero scalar curvature,
and the bulk sector ($\mathrm{Vir}_{26}$, topological) carries the
nonzero scalar anomaly. This is the bosonic string from the
...
\otimes \bigwedge\nolimits^{2g}.
\end{equation}
At $c = 0$ the localization by \(u^{-1}\) is absent, since
\(u=0\); the uncurved complex
$\barB^{(g)}(\mathrm{Vir}_0) \otimes \bigwedge^{2g}$ is the
remaining term. At $c = 26$ the dual \(\mathrm{Vir}_0\) term is
uncurved, so the connecting homomorphism $\partial$ measures the
scalar-curvature passage from the topological regime to the
gravitational regime and not a disappearance of the bar complex.
\end{corollary}
...
parameter through $u = \kappaChHodge(\cA) \cdot \omega_g$. The variation of
...
For a Koszul chiral algebra $\cA$ with $\kappaChHodge(\cA) \neq 0$, the
...
$F_g(\mathrm{Vir}_0) = 0$: the protected scalar trace is zero because
the supertrace over the exterior algebra vanishes.
\end{remark}
...
\kappaChHodge(\text{matter},c=26)+\kappaChHodge(\text{ghost},c=-26)$ is zero, so
...
$\kappaChHodge(\text{matter}) = c^{\mathrm{matter}}/2 = 13$ (or
$\kappa = c$ in the convention of
\S~\ref{subsec:tholog-coda}); the $bc$-ghost system contributes
$\kappaChHodge(\text{ghost}) = -13$. Total $\kappa_{\mathrm{eff}}=0$,
