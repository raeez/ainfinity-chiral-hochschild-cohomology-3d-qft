...
framework, the proved statement is narrower: the Koszul dual
...
ghost zero-mode bookkeeping.  The manuscript does \emph{not} prove that
the physical state space of the full bosonic string is
$H^*(\barB^{(g)}(\mathrm{Vir}_0))$, nor does it derive the classical
no-ghost theorem from bar-cobar methods alone.  The ``ghosts'' are
...
$\kappa = 0$ does not by itself determine the higher-arity shadow
tower.
...
dual higher-arity shadow tower.  Thus the nonzero scalar and quartic
data exhibited here come from $\mathrm{Vir}_{26}$, while the dual
higher-arity tower is not fixed by $\kappa(\mathrm{Vir}_0)=0$ alone.
\end{enumerate}
\end{proposition}
...
term vanish, but no conclusion about the full higher-arity tower
...
$F_1(\mathrm{Vir}_0)$; the higher-arity dual tower is not determined
...
saturates only at the scalar/genus-$1$ level; higher-arity data of the
...
closes at $c = 26$, while the non-scalar shadow tower still requires
separate control.
\end{remark}
...
(Corollary~\ref{cor:g9-u-coproduct}), its cohomology class
