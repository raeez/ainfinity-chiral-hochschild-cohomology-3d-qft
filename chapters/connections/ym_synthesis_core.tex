...
\section{Yang--Mills boundary theory and bar--cobar structure}
\label{sec:ym-boundary-bar-cobar-core}

The holomorphic-topological twist of 4d $\cN = 2$ gauge theory on $\C \times \R_{\ge 0}$ produces a boundary VOA whose twisting morphisms are classified by the $E_1$ chiral coassociative bar coalgebra~$\barB(\cA)$: the differential encodes holomorphic collisions on~$\FM_k(\C)$, the deconcatenation coproduct encodes topological splittings along~$\R$. The curvature $\kappa$, line-sector operations on $\cC_{\mathrm{line}}$, and spectral $R$-matrix are the gauge-theoretic projections of $\Theta^{\mathrm{oc}}$.

\subsection{Twisted Yang--Mills boundary package}
\label{subsec:twisted-ym-boundary-package}
...
\begin{remark}[Which examples are Koszul-admissible]
\label{rem:koszul-admissible-examples}
The Koszul-admissibility condition is satisfied by the standard class of examples: generic-level affine Kac--Moody algebras (by Theorem~\ref{thm:one-loop-koszul}) and principal $\cW$-algebras obtained by Drinfeld--Sokolov reduction (by Theorem~\ref{thm:ds-koszul-obstruction}). Beyond the affine lineage, Koszul-admissibility must be verified case by case.
\end{remark}

\begin{notation}[Open sector, boundary-dual shadow, and tangent space]
...
\qquad
\mathsf{Boundary}^{\vee}_{\cB} := \Omegach(\barBch(\cA_{\cB}^!)),
\qquad
...
\begin{theorem}[Boundary BRST recovery for twisted Yang--Mills data]
...
factorization algebra; on cohomology (using Koszul-admissibility)
this algebra is the Koszul dual:
\[
H^\bullet\bigl(\mathbb{D}_{\Ran}\, \mathsf{Open}_{\cB}\bigr)
\;\simeq\; \cA_{\cB}^!.
\]
The dual boundary shadow $\mathsf{Boundary}^{\vee}_{\cB} = \Omegach(\barBch(\cA_{\cB}^!))$
is the cobar of the bar of the Koszul dual, which recovers
$\cA_{\cB}^!$ by bar-cobar inversion (Theorem~B, inversion on the Koszul locus). It therefore
packages the boundary Koszul dual rather than the bulk Hochschild
model.
\end{enumerate}
\end{theorem}
...
$\barBch(\cA_{\cB}) \simeq C^{\frac{\infty}{2}+\bullet}(\cA_{\cB})$.
For the Verdier-dual statement: $\mathbb{D}_{\Ran}$ maps the
bar coalgebra to a factorization algebra whose cohomology, by
Koszul-admissibility and the Verdier-dual bar/cobar comparison theorem of Volume~I,
is $\cA_{\cB}^!$.
\end{proof}

\begin{theorem}[Tangent-to-center principle]
...
\begin{proof}
Apply the genus-zero deformation/obstruction exchange for Koszul pairs
proved in Volume~I (the Koszul--Hochschild comparison theorem, which establishes
$HH^n_{\mathrm{chiral}}(\cA) \cong HH^{2-n}_{\mathrm{chiral}}(\cA^!)^{\vee}\otimes \omega_X$
for chirally Koszul pairs) to $(\cA_{\cB}, \cA_{\cB}^!)$.
\end{proof}

\begin{corollary}[Center-vanishing rigidity criterion]
...
\begin{corollary}[One-parameter criterion]
...
\begin{theorem}[The tangent functor factors through the dual center]
...
For morphisms, the bar construction is functorial by the BV functoriality theorem of Volume~I,
and the main Koszul--Hochschild comparison of Volume~I is functorial because it is built
from natural operations: bar/cobar, derived \(\mathrm{Hom}\), and Verdier duality. Passing to
cohomological degree~$2$ produces the required natural transformation.
\end{proof}

\begin{corollary}[Interface invariance of tangent data]
...
\begin{corollary}[Comparison without full boundary equivalence]
...
\label{subsec:twisted-ym-central-formality}
...
\begin{theorem}[Dual unobstructedness criterion]
...
\begin{proof}
By the deformation-obstruction dictionary, primary obstructions to extending a first-order deformation lie in $HH^{3}_{\mathrm{chiral}}(\cA_{\cB})$. Apply the Koszul--Hochschild comparison theorem of Volume~I (the degree shift $n \mapsto 2-n$ for chirally Koszul pairs) with $n=3$ to obtain
\[
HH^{3}_{\mathrm{chiral}}(\cA_{\cB})
\cong
HH^{-1}_{\mathrm{chiral}}(\cA_{\cB}^{!})^{\vee}\otimes \omega_X.
\]
The assumed vanishing of $HH^{-1}_{\mathrm{chiral}}(\cA_{\cB}^{!})$ forces $HH^{3}_{\mathrm{chiral}}(\cA_{\cB})=0$, so the primary obstruction vanishes. The final sentence is the standard interpretation of $HH^3$ in deformation theory.
\end{proof}

\begin{theorem}[Central formality theorem]
...
\begin{proof}
The argument proceeds in three stages: (i) identify the controlling dg Lie algebra and its cohomology, (ii) verify the vanishing hypotheses for the affine lineage via the Koszul property, and (iii) apply the Schlessinger--Stasheff criterion to conclude formality.

\medskip\noindent\textbf{Stage 1: Controlling dg Lie algebra.}
By Theorem~\ref{thm:MC-deformations}, the formal deformations of $\cA_{\cB}$ are governed by Maurer--Cartan elements of the dg Lie algebra $\mathfrak{g}^{\mathrm{def}}_{\cB} = \tau_{\ge 1}RHH_{\mathrm{ch}}(\cA_{\cB})[1]$. Its cohomology satisfies
\[
H^0(\mathfrak{g}^{\mathrm{def}}_{\cB}) = HH^{1}_{\mathrm{chiral}}(\cA_{\cB}),
\qquad
H^1(\mathfrak{g}^{\mathrm{def}}_{\cB}) = HH^{2}_{\mathrm{chiral}}(\cA_{\cB}),
\qquad
H^2(\mathfrak{g}^{\mathrm{def}}_{\cB}) = HH^{3}_{\mathrm{chiral}}(\cA_{\cB}).
\]
For central formality we need $H^0 = 0$ (no infinitesimal automorphisms) and $H^2 = 0$ (no obstructions).

\medskip\noindent\textbf{Stage 2: Koszul resolution and Hochschild vanishing.}
For the affine lineage (generic-level affine Kac--Moody and principal $\cW$-algebras obtained by Drinfeld--Sokolov reduction), the Koszul property (Theorem~\ref{thm:one-loop-koszul}) ensures that the chiral bar complex $\barBch(\cA_{\cB})$ is a resolution of $\cA_{\cB}^!$. Chiral Hochschild cohomology is then computed by the bar resolution:
\[
HH^n_{\mathrm{chiral}}(\cA_{\cB},\cA_{\cB})
\;\cong\;
\mathrm{Ext}^n_{\cA_{\cB}\text{-}\mathrm{bimod}}(\cA_{\cB},\cA_{\cB}),
\]
where the Ext is taken in the category of chiral bimodules using the bar resolution $\barBch(\cA_{\cB}) \to \cA_{\cB}^!$. The Koszul--Hochschild comparison (Volume~I, degree shift $n \mapsto 2-n$ for chirally Koszul pairs) gives
\[
HH^n_{\mathrm{chiral}}(\cA_{\cB})
\;\cong\;
HH^{2-n}_{\mathrm{chiral}}(\cA_{\cB}^!)^{\vee} \otimes \omega_X.
\]
For $n=1$: $HH^1_{\mathrm{chiral}}(\cA_{\cB}) \cong HH^{1}_{\mathrm{chiral}}(\cA_{\cB}^!)^{\vee} \otimes \omega_X$. In the affine lineage, the Koszul dual $\cA_{\cB}^!$ has no degree-$1$ chiral Hochschild classes (the quadratic-dual algebra inherits a weight grading from the root-space decomposition that excludes degree-$1$ self-extensions), so $HH^1_{\mathrm{chiral}}(\cA_{\cB}) = 0$. For $n=3$: Theorem~\ref{thm:twisted-ym-dual-unobstructedness} gives $HH^3_{\mathrm{chiral}}(\cA_{\cB}) = 0$ from the vanishing $HH^{-1}_{\mathrm{chiral}}(\cA_{\cB}^!) = 0$, which holds because the bar complex is concentrated in non-negative degrees.

\medskip\noindent\textbf{Stage 3: Schlessinger--Stasheff formality.}
With $H^0(\mathfrak{g}^{\mathrm{def}}_{\cB}) = 0$ and $H^2(\mathfrak{g}^{\mathrm{def}}_{\cB}) = 0$ established, choose a minimal $L_\infty$ model on $H^\bullet(\mathfrak{g}^{\mathrm{def}}_{\cB})$; existence follows from the homotopy transfer theorem applied to the Lie operad, and uniqueness up to $L_\infty$ isomorphism is standard \cite{LV12}. Since the only nonzero cohomology relevant to the Maurer--Cartan problem is in degree $1$, every higher bracket
\[
\ell_n \colon (H^1)^{\otimes n} \longrightarrow H^{2}
\]
has zero target, because $\ell_n$ has cohomological degree $2-n$. Thus all $\ell_n$ vanish identically and the minimal model is abelian. This is the Schlessinger--Stasheff criterion (see \cite{LV12}, \S12): a formal moduli problem with $H^0 = 0$ (trivial gauge group) and $H^2 = 0$ (no obstructions) has smooth, hence affine, formal neighborhood.
...
\begin{lemma}[Center of a chiral tensor product]
\label{lem:twisted-ym-center-tensor}
Let $\cB_1,\cB_2$ be quadratic Koszul-admissible boundary data with chiral envelopes $\cA_1,\cA_2$, and assume that the chiral tensor product $\cA_1\otimes^{\mathrm{ch}}\cA_2$ has trivial mixed OPE, so that the quadratic Koszul-dual tensor-product theorem of Volume~I applies. Then
\[
...
\begin{remark}[Triviality on the dual side]
\label{rem:mixed-ope-dual-side}
Lemma~\ref{lem:twisted-ym-center-tensor} assumes trivial mixed OPE on the original side $\cA_1\otimes^{\mathrm{ch}}\cA_2$ in order to invoke the Koszul-dual tensor-product theorem of Volume~I, which then computes $(\cA_1\otimes^{\mathrm{ch}}\cA_2)^! \cong \cA_1^!\otimes^{\mathrm{ch}}\cA_2^!$. For the center computation to descend to the dual side, one needs that the mixed OPE is also trivial on the Koszul-dual tensor product $\cA_1^!\otimes^{\mathrm{ch}}\cA_2^!$. This holds because triviality of the mixed OPE is a quadratic-data condition: it depends only on the quadratic relations in the chiral envelope, and the Koszul-dual tensor-product theorem preserves the quadratic splitting. Explicitly, if the generators of $\cA_1$ and $\cA_2$ have mutually regular OPE (no singular terms), then the dual generators (which are determined by the orthogonal complement of the quadratic relations) inherit the same mutual regularity.
\end{remark}

\begin{theorem}[Binary mixed-coupling theorem]
...
\]
Since the boundary data are quadratic, the quadratic Koszul-dual
tensor-product theorem of Volume~I gives
\[
\cA_{12}^{!}
\cong
...
\begin{corollary}[Multi-body coupling expansion]
...
\begin{corollary}[Mixed rigidity criterion]
...
\begin{remark}[Interaction content]
\label{rem:twisted-ym-interaction-content}
Theorems~\ref{thm:twisted-ym-dual-unobstructedness}--\ref{thm:twisted-ym-binary-mixed-couplings}
add three structures to the tangent-to-center principle:
\begin{enumerate}[label=(\roman*)]
...
\end{enumerate}
Together these statements identify the local geometry of boundary moduli.
\end{remark}

\subsection{Relative Hochschild duality and the mixed Kodaira--Spencer class}
\label{subsec:relative-hochschild-mixed-ks}
...
\begin{theorem}[Relative duality theorem]
...
\begin{corollary}[Relative tangent-to-center principle]
...
\begin{corollary}[Relative dual unobstructedness]
...
\begin{theorem}[Relative central formality]
...
\begin{lemma}[Well-definedness and invariance]
...
\begin{theorem}[First-correction theorem]
...
\]
the class of a quadratic-limit mixed coupling survives to the next page if and only if it lies in the kernel of
\[
[\kappa_{\mathrm{mix}},-] :
...
\begin{corollary}[Persistence criterion for the quadratic law]
\label{cor:persistence-criterion-quadratic-law}
The entire quadratic mixed-coupling subspace
\[
Q_{\mathrm{mix}}
:=
\widetilde Z_\infty(M_1)\widehat\otimes \widetilde Z_\infty(M_2)
\]
survives to first non-quadratic order if and only if
\[
[\kappa_{\mathrm{mix}},-]\big|_{Q_{\mathrm{mix}}}=0.
\]
If the adjoint action of
$HH^1_{\mathrm{rel}}(M_{12};M_1,M_2)$ on $Q_{\mathrm{mix}}$ is faithful in mixed weight one, this condition is equivalent to
\[
\kappa_{\mathrm{mix}}(M_{12};M_1,M_2)=0.
\]
More generally, if the mixed coderivation components $m^{(1)},\dots,m^{(r)}$ act trivially on the corresponding associated-graded quadratic-coupling terms, then the mixed-weight spectral sequence degenerates on those terms through $E_{r+1}$.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:first-correction-theorem} identifies the first
non-quadratic differential on $Q_{\mathrm{mix}}$ with
$[\kappa_{\mathrm{mix}},-]$. Thus every quadratic-limit coupling
survives exactly when this restricted operator is zero. Faithfulness
of the adjoint action converts vanishing of the operator into
vanishing of the class $\kappa_{\mathrm{mix}}$. The higher statement
is the same filtered-complex argument: the $j$th mixed-weight piece
of the coderivation induces $d_j$ on the associated spectral sequence,
so trivial action for $1\le j\le r$ forces $d_1,\dots,d_r$ to vanish
on the indicated terms.
\end{proof}

\begin{theorem}[Universal interaction brackets on mixed couplings]
...
\label{subsec:w-boundary-cech-theory}
...
\begin{theorem}[Higher-body \v{C}ech duality]
...
\begin{theorem}[Tensor-product model for the augmented center complex]
...
\begin{corollary}[Pure $m$-body coupling theorem]
...
\begin{theorem}[Higher Mayer--Vietoris descent for couplings]
...
\begin{theorem}[Filtered stability of pure $m$-body couplings]
...
\subsection{Scope: the HT-twist of $4$d gauge theory}
\label{subsec:ym-ht-scope}

The results of this chapter apply uniformly to every $4$d gauge theory
admitting a holomorphic--topological twist. This includes, but is not
limited to:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\cN=1$ super-Yang--Mills with holomorphic twist (Costello--Li);
\item $\cN=2$ super-Yang--Mills in the holomorphic twist
(Beem--Rastelli, Costello--Gaiotto~\cite{CostelloGaiottoBoundaryVOA});
\item $\cN=4$ super-Yang--Mills in the HT-twist
(Costello--Paquette~\cite{CostelloPaquetteTwistedSugraStates},
Costello--Costello--Paquette~\cite{CL16});
\item Pure (non-supersymmetric) $4$d gauge theory on a null slab
(the twist is absent but the HT reduction is available on
$S^2\times \R$).
\end{enumerate}
The universal celestial holography theorem
(Theorem~\ref{thm:universal-celestial-holography} in
\texttt{celestial\_holography\_core.tex}) implies that the boundary
chiral algebras of (a)--(d) all fit in a common
$\mathrm{SC}^{\mathrm{ch,top}}$ framework. The distinction between
gauge and gravity is the shadow class: pure gauge theories are
class~$\mathbf G$ (Koszul, $m_k^{\mathrm{SC}} = 0$ for $k\geq 3$);
gravitationally coupled theories are class~$\mathbf M$ (infinite
shadow depth).

\begin{theorem}[Unification: YM + gravity + $\cN=4$ share a single $\mathrm{SC}^{\mathrm{ch,top}}$ skeleton]
\label{thm:ym-gravity-N4-unification}
\index{celestial holography!unification}
Let $T_4^{\mathrm{YM}}$, $T_4^{\mathrm{grav}}$, $T_4^{\cN=4}$ be an
HT-twistable $4$d gauge theory, $4$d gravity, and $\cN=4$ SYM
respectively. Their celestial chiral algebras
$\cA^{\mathrm{cel}}_{\mathrm{YM}}$, $\cA^{\mathrm{cel}}_{\mathrm{grav}}$,
$\cA^{\mathrm{cel}}_{\cN=4}$ are algebras over the same Swiss-cheese
operad $\mathrm{SC}^{\mathrm{ch,top}}$, and the celestial
bulk-to-boundary data
\[
\bigl(\cA^{\mathrm{cel}}_\bullet,\,
 Z^{\mathrm{der}}_{\mathrm{ch}}(\cA^{\mathrm{cel}}_\bullet)\bigr)
\]
defines a single functor
$T_4 \mapsto \bigl(\text{boundary, bulk}\bigr)$ from HT-twistable $4$d
theories to $\mathrm{SC}^{\mathrm{ch,top}}$-pairs. The shadow class
and the level/Brown--Henneaux central charge distinguish the three
cases:
\begin{center}
\begin{tabular}{l|l|l}
Theory & Shadow class & Defining datum \\\hline
$T_4^{\mathrm{YM}}$ & $\mathbf G$ & level $k\in\Z$, Koszul\\
$T_4^{\cN=4}$ & $\mathbf L$ or $\mathbf C$ & level $k$ + conformal data\\
$T_4^{\mathrm{grav}}$ & $\mathbf M$ & Brown--Henneaux $c=3\ell/(2G_N)$
\end{tabular}
\end{center}
The Costello--Paquette celestial algebra for $\cN=4$ SYM is the
class~$\mathbf L$ (or $\mathbf C$, at the self-dual anomaly point)
specialization of this construction.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:universal-celestial-holography}(i)--(ii), each
HT-twistable $T_4$ produces an
$\mathrm{SC}^{\mathrm{ch,top}}$-algebra pair functorially. The shadow
class is read off the OPE data (Part~II, G/L/C/M classification):
pure gauge theories have Lie-algebra commutator-only OPE
(class~$\mathbf G$), gravitational theories have
$\mathfrak{w}_{1+\infty}$-type OPE at infinite depth
(class~$\mathbf M$), $\cN=4$ SYM's HT-twist was computed by
Costello--Paquette to produce the logarithmic HT-monodromy algebra
of \S~\ref{subsec:affine-monodromy-identification} in
\texttt{log\_ht\_monodromy\_core.tex} (class~$\mathbf L$, upgraded to
class~$\mathbf C$ at the anomaly self-dual point).
Functoriality in $T_4$ is the Costello--Li universal property of
the HT-twist.
\end{proof}

\begin{remark}[Costello--Paquette as a specialization]
\label{rem:costello-paquette-specialization}
The Costello--Paquette celestial chiral algebra for $\cN=4$ SYM is
a specific class~$\mathbf L$/$\mathbf C$ algebra in our framework.
The novelty of the universal theorem is not the $\cN=4$
computation, which we cite, but the observation that a single
$\mathrm{SC}^{\mathrm{ch,top}}$ skeleton controls all HT-twistable
$4$d QFTs, with the shadow class stratifying the landscape.
\end{remark}

\subsection{Algebraic mass-gap reduction: from reduction to theorem}
\label{subsec:ym-mass-gap-theorem}

The instanton-completed bar complex $B^\Lambda(\cA)$
(Theorem~\ref{thm:novikov-completion-theorem} in
\texttt{ym\_instanton\_screening.tex}) encodes non-perturbative
contributions to the boundary chiral algebra of $4$d gauge theory.
The algebraic reduction of the mass-gap problem has force only after
the boundary algebra is realized on a Hilbert screening space and the
untwisted Hamiltonian is compared with the screening Laplacian.

\begin{theorem}[Algebraic mass-gap reduction]
\label{thm:ym-mass-gap-reduction}
\index{mass-gap problem!algebraic reduction}
\index{Yang--Mills!mass-gap}
Let $T_4^{\mathrm{YM}}$ be pure $4$d Yang--Mills with compact
simple gauge group $G$. Assume:
\begin{enumerate}[label=\textup{(H\arabic*)}]
\item $T_4^{\mathrm{YM}}$ admits an HT-twist on $S^2\times \R$,
producing a celestial chiral algebra
$\cA^{\mathrm{cel}}_{\mathrm{YM}}$ with instanton-completed bar
complex $B^\Lambda(\cA^{\mathrm{cel}}_{\mathrm{YM}})$.
\item The screening operators
$S_\alpha^{\mathrm{scr}}\in \cA^{\mathrm{cel}}_{\mathrm{YM}}$, one
for each simple root $\alpha$, are weight-$1$ primary fields
(Felder~\cite{FelderVOA}), carried to the Wakimoto
realization~\cite{WakimotoFree}
of $V_k(\fg)$ under the half-space BV reduction
(Theorem~\ref{thm:affine-half-space-bv}).
\item The visible centre $Z_{\mathrm{vis}}$ of
Definition~\ref{def:visible-center-and-couplings} admits the
\emph{defect-realization datum}
\[
\rho_{\mathrm{def}}\colon
B^\Lambda(\cA^{\mathrm{cel}}_{\mathrm{YM}})
\longrightarrow
\mathcal A_{\mathrm{def}},
\]
where $\mathcal A_{\mathrm{def}}$ is a unital $*$-algebra equipped
with a positive normalized state $\tau$ and a central screening
sequence $\mathbf s=(s_1,\dots,s_r)$ whose GNS screening operators
realize the images of the $S_\alpha^{\mathrm{scr}}$.
\item The defect realization supplies a positive visible-centre gap:
the triple $(\mathcal A_{\mathrm{def}},\tau,\mathbf s)$ is a
screening state datum in the sense of
Definition~\ref{def:screening-state-datum}, its GNS screening
operators form a Hilbert-screening datum, and the internal screening
Hamiltonian satisfies
\[
H_{\mathrm{scr}}^{\mathrm{int}}\ge \mu^2(1-P_0)
\]
for some $\mu>0$.
\item The untwisted Yang--Mills Hamiltonian and the Hilbert-screening
datum form an untwisting bridge datum: $H_{\mathrm{YM}}$ is realized
on the same GNS Hilbert space as a nonnegative self-adjoint operator,
satisfies $H_{\mathrm{YM}}\Omega_\tau=0$, and has the gauge-invariant
physical Hilbert space as a closed subspace.
\item The algebraic screening domination estimate holds on the
GNS algebraic core:
\[
...
\end{enumerate}
Then the untwisted Hamiltonian has a spectral gap:
\[
\operatorname{Spec}(H_{\mathrm{YM}})
\cap(0,\alpha\mu^2-\varepsilon)=\varnothing.
\]
The algebraic mass-gap input is the positive visible-centre gap (H4),
transported through the defect realization and compared with
$H_{\mathrm{YM}}$ by (H5)--(H6).
\end{theorem}

\begin{proof}
(H1) gives the HT-twist reduction; (H2) identifies the screening
operators as Felder-type weight-$1$ primaries, placing the
construction inside the Wakimoto realization. Hypothesis (H3) places
the instanton-completed bar complex in the physical defect algebra and
produces the GNS Hilbert space. Hypothesis (H4) is the screening gap
of Definition~\ref{def:internal-screening-gap-domination}(a), while
(H5)--(H6) are the untwisting and domination data of
Definition~\ref{def:internal-screening-gap-domination}(b). Applying
Theorem~\ref{thm:mass-gap-reduction-internal-screening} gives
\[
\operatorname{Spec}(H_{\mathrm{YM}})
\cap(0,\alpha\mu^2-\varepsilon)=\varnothing.
\]
The final assertion is the reading of (H4) through the
defect-realization map.
\end{proof}

\begin{remark}[Bridge-data separation]
\label{rem:mass-gap-bridge-data}
The theorem separates three inputs: the algebraic HT/Felder
realization (H1)--(H2), the defect-realization and Hilbert-screening
data (H3)--(H4), and the untwisting plus domination data (H5)--(H6).
The infrared conclusion is made only after all three inputs are part
of the datum.
\end{remark}
