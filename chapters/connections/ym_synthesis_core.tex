...
The holomorphic-topological twist of 4d $\cN = 2$ gauge theory on $\C \times \R_{\ge 0}$ produces a boundary VOA whose twisting morphisms are classified by the bar complex with $\SCchtop$-coalgebra structure.  The curvature $\kappa$, line-sector operations on $\cC_{\mathrm{line}}$, and spectral $R$-matrix are the gauge-theoretic projections of $\Theta^{\mathrm{oc}}$.
...
\item \emph{Bar/cobar construction.}  Apply the chiral bar and cobar functors of Part~I to the
boundary envelope.
...
\item the assumption that $\cA_{\cB}$ lies on the Koszul locus of Part~I and belongs to a class
...
\begin{theorem}[Tangent-to-center principle; \ClaimStatusProvedElsewhere]
...
\]
Moreover, the first nonzero $\lambda_n$ is a derived invariant of the filtered $A_\infty$ quasi-isomorphism class of the cospan $(M_1\to M_{12}\leftarrow M_2)$.
\end{theorem}
