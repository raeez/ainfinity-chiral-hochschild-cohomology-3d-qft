...
The complementarity theorem (Theorem~\ref{thm:quantum-complementarity-main})
...
Theorem~\ref{thm:shifted-symplectic-complementarity} upgrades this to a
$(-1)$-shifted symplectic polarization via the BV formalism.
This section develops the full holographic content of that upgrade:
...
shifted-symplectic Lagrangian structure~(C2) becomes the natural
geometric home of the shadow tower
...
Appendix~\ref{app:nonlinear-modular-shadows}, and that at genus~$1$
...
The five main theorems (A--D+H) of Part~I supply the following
...
\begin{remark}[H/M/S layers]\label{rem:thqg-III-hms}
Following Convention~\ref{conv:hms-levels}:
...
\item \emph{Verdier self-duality.}
  ...
  $Z(\cA) \cong Z(\cA^!)$
  (Lemma~\ref{sublem:center-isomorphism}) induce a
...
(Corollary~\ref{cor:duality-bar-complexes-complete} and
Lemma~\ref{sublem:center-isomorphism}).
...
via Lemma~\ref{sublem:center-isomorphism}, and $\mathrm{ev}$
...
equation~\eqref{eq:verdier-ks-anticommute}).  At the level of
...
\begin{equation}\label{eq:thqg-III-theta-intertwine}
...
\begin{proof}
The MC element $\Theta_\cA := D_\cA - d_0$ is MC because
$D_\cA^2 = 0$
(Theorem~\ref{thm:mc2-bar-intrinsic}).
Under Verdier--Koszul duality, the bar coderivation $D_\cA$
maps to the $\cA^!_\infty$ algebra structure differential $D_{\cA^!}$
(Theorem~\ref{thm:verdier-bar-cobar}), and the genus-$0$
collision differential $d_0$ is invariant.  Therefore
\[
\sigma(\Theta_\cA)
= \sigma(D_\cA - d_0)
= D_{\cA^!} - d_0
...
\label{rem:thqg-III-chain-level}
\index{complementarity!chain-level mechanism}
Proposition~\ref{prop:thqg-III-mc-compatibility} is the
chain-level incarnation of the complementarity slogan
$\Theta_\cA + \Theta_{\cA^!} = 0$
(Remark~\ref{rem:theorem-C-decomposition}).
At the S-level, the relation becomes
$\kappa(\cA) + \kappa(\cA^!) = 0$: the modular characteristic
$\kappa(\cA)$ is anti-symmetric under Koszul duality
(Theorem~\ref{thm:kappa-anti-symmetry}).
...
bilinear form
\begin{equation}\label{eq:thqg-III-holographic-pairing-def}
\langle -, - \rangle_g
\colon
...
\eqref{eq:thqg-III-holographic-pairing-def} is nondegenerate.
\end{lemma}
...
\ClaimStatusProvedHere]
\label{prop:thqg-III-degree-shift}
...
\begin{equation}\label{eq:thqg-III-C1-homotopy}
\mathbf{C}_g(\cA)
\;\simeq\;
\mathbf{Q}_g(\cA) \;\oplus\; \mathbf{Q}_g(\cA^!),
\end{equation}
...
maps
\begin{equation}\label{eq:thqg-III-projectors}
p^+ := \tfrac{1}{2}(\id + \sigma),
\qquad
p^- := \tfrac{1}{2}(\id - \sigma)
\end{equation}
...
p^+ + p^- &= \id, &
p^+ \circ p^- &= 0.
\label{eq:thqg-III-projector-properties}
\end{align}
...
\label{rem:thqg-III-def-obs-exchange}
...
\label{subsubsec:thqg-III-ptvv-review}
...
\begin{equation}\label{eq:thqg-III-ptvv-nondegen}
\omega^{\sharp}
\colon
...
\label{rem:thqg-III-two-structures}
...
Nondegeneracy follows from the Verdier duality identification
$\mathbb{D}(\barB(\cA)) \simeq \cA^!_\infty$
(Theorem~\ref{thm:bv-functor}): on the Koszul locus, $\cA^!_\infty$
is formal and equivalent to~$\cA^!$, so bar-cobar
inversion (Theorem~\ref{thm:bar-cobar-isomorphism-main})
...
\emph{Part (iii).}
By Lemma~\ref{lem:eigenspace-decomposition-complete} and
Lemma~\ref{lem:obs-def-split-complete}, the $+1$-eigenspace
...
\label{subsubsec:thqg-III-verdier-shifted}
...
\begin{equation}\label{eq:thqg-III-lagrangian-decomp}
\mathbf{Q}_g(\cA)
\;\simeq\;
...
\eqref{eq:thqg-III-anti-symmetry}.  Therefore
...
By the anti-symmetry \eqref{eq:thqg-III-anti-symmetry}:
$\langle \sigma(v), \sigma(w) \rangle_g
= -\langle v, w \rangle_g$.
...
\label{rem:thqg-III-conditionality}
...
cohomology (Lemma~\ref{lem:perfectness-criterion}).
...
\label{cor:thqg-III-lagrangian-intersection}
...
\begin{equation}\label{eq:thqg-III-lagrangian-intersection}
\mathbf{Q}_g(\cA)
...
\textup{(}Definition~\textup{\ref{def:nms-complementarity-potential})}.
\end{corollary}

\begin{proof}
This follows from the shifted cotangent normal form
(Theorem~\ref{thm:nms-cotangent-normal-form}) applied to
...
Theorem~\ref{thm:nms-derived-critical-locus} then identifies
...
\label{prop:thqg-III-compatibility}
...
\textup{(}Theorem~\textup{\ref{thm:ss-quantum})} induces a
...
form.  Concretely:
\begin{equation}\label{eq:thqg-III-compatibility-diagram}
\begin{tikzcd}
...
(Theorem~\ref{thm:fiber-center-identification}), the
...
Having established the shifted-symplectic Lagrangian structure,
we now construct the complementarity potential $S_\cA$ and prove
that its Taylor jets recover the shadow tower of
Appendix~\ref{app:nonlinear-modular-shadows}.

\subsubsection{Construction of the potential}
\label{subsubsec:thqg-III-potential-construction}

\begin{theorem}[Holographic complementarity potential;
\ClaimStatusConditional]
\label{thm:thqg-III-complementarity-potential}
\index{complementarity potential!construction|textbf}
Under the hypotheses of
Theorem~\textup{\ref{thm:thqg-III-lagrangian-polarization}},
there exists a formal function
\begin{equation}\label{eq:thqg-III-potential-existence}
S_\cA
\;\in\;
...
(Theorem~\ref{thm:nms-cotangent-normal-form}), the
...
\emph{Part (ii).}
Theorem~\ref{thm:nms-derived-critical-locus}: the derived
...
The MC equation
$D_\cA \Theta_\cA + \tfrac{1}{2}[\Theta_\cA, \Theta_\cA] = 0$
(Theorem~\ref{thm:mc2-bar-intrinsic}) projects to the
...
arity-$r$ shadow jet of
Definition~\textup{\ref{def:nms-arity-r-shadow-jet}}.
The first three jets are:
\begin{align}
\Sh_2(\cA) &= H_\cA
  && \text{\textup{(}Hessian\textup{)}},
\label{eq:thqg-III-sh2}
\\
\Sh_3(\cA) &= \mathfrak{C}_\cA
  && \text{\textup{(}cubic shadow\textup{)}},
\label{eq:thqg-III-sh3}
\\
\Sh_4(\cA) &= \mathfrak{Q}_\cA
  && \text{\textup{(}quartic shadow\textup{)}}.
\label{eq:thqg-III-sh4}
\end{align}
These are the arity-$r$ projections of the bar-intrinsic MC
element $\Theta_\cA$
\textup{(}Theorem~\textup{\ref{thm:mc2-bar-intrinsic})}:
\begin{equation}\label{eq:thqg-III-shadow-projection}
\Sh_r(\cA)
= \sum_{\mathrm{cyc}}
...
jets of
Definition~\ref{def:nms-shadow-jets} follows because both
are Taylor coefficients of the same formal function: $S_\cA$
is the complementarity potential of
Definition~\ref{def:nms-complementarity-potential}, and
$S_\cA^{\le r} = \sum_{k=2}^r \frac{1}{k!} \Sh_k(\cA)$
is the truncated potential of
Definition~\ref{def:nms-arity-r-shadow-jet}.  The explicit
formula \eqref{eq:thqg-III-shadow-projection} is
equation~\eqref{eq:nms-arity-r-shadow-jet}.
\end{proof}

\begin{corollary}[Master equations for shadow jets;
\ClaimStatusConditional]
\label{cor:thqg-III-shadow-master-equations}
\index{master equations!shadow jets}
The classical master equation $\{S_\cA, S_\cA\}_{H_\cA} = 0$
decomposes by arity into the all-arity master equation
\textup{(}Theorem~\textup{\ref{thm:nms-all-arity-master-equation})}:
\begin{equation}\label{eq:thqg-III-all-arity-master}
\nabla_{H_\cA} \Sh_r(\cA) + \mathfrak{o}_\cA^{(r)} = 0,
\qquad r \ge 3,
\end{equation}
where
$\nabla_{H_\cA} := \{H_\cA, -\}_{H_\cA}$ is the Hessian
differential and $\mathfrak{o}_\cA^{(r)}$ is the arity-$r$
obstruction
\textup{(}Definition~\textup{\ref{def:nms-arity-r-obstruction})}.

At the first three arities:
\begin{align}
r = 3\colon\qquad
\nabla_{H_\cA} \mathfrak{C}_\cA &= 0,
\label{eq:thqg-III-cubic-master}
\\
r = 4\colon\qquad
\nabla_{H_\cA} \mathfrak{Q}_\cA
+ \tfrac{1}{2}\{\mathfrak{C}_\cA, \mathfrak{C}_\cA\}_{H_\cA}
&= 0,
\label{eq:thqg-III-quartic-master}
\\
r = 5\colon\qquad
\nabla_{H_\cA} \Sh_5(\cA)
+ \{\mathfrak{C}_\cA, \mathfrak{Q}_\cA\}_{H_\cA}
&= 0.
\label{eq:thqg-III-quintic-master}
\end{align}
\end{corollary}

\begin{proof}
Expand $\{S_\cA, S_\cA\}_{H_\cA} = 0$ by arity.
At arity $r$, the quadratic term contributes
$\{H_\cA, \Sh_r\}_{H_\cA} = \nabla_{H_\cA} \Sh_r$;
the remaining terms contribute $\mathfrak{o}_\cA^{(r)}$.
This is Theorem~\ref{thm:nms-all-arity-master-equation}.
The explicit cases follow by substitution.
\end{proof}

\begin{proposition}[Legendre duality of the potential;
\ClaimStatusConditional]
\label{prop:thqg-III-legendre}
...
$S_{\cA^!} = S_\cA^{\vee}$.
Through cubic order:
\begin{equation}\label{eq:thqg-III-legendre-cubic}
S_{\cA^!}(p)
...
Theorem~\ref{thm:nms-finite-termination} characterizes
...
\label{prop:thqg-III-sewing-recursion}
...
\textup{(}Theorem~\textup{\ref{thm:nms-all-arity-separating-boundary})}:
\begin{equation}\label{eq:thqg-III-sewing-recursion}
\xi^* \Sh_r(\cA)
=
...
\textup{(}Construction~\textup{\ref{constr:nms-sewing-product})}
with propagator $P_\cA = H_\cA^{-1}$.
\end{proposition}

\begin{proof}
The shadow jets are arity-$r$ projections of $\Theta_\cA$,
which satisfies the modular clutching identity
$\xi^*(\Theta_\cA) = \Theta_\cA \star \Theta_\cA$.
Projecting to arity $r$ gives the stated recursion
(Theorem~\ref{thm:nms-all-arity-separating-boundary}).
\end{proof}
...
partition function}
\label{subsubsec:thqg-III-btz}

\begin{setup}[Holographic genus-$1$ data]
\label{setup:thqg-III-genus-1}
...
over moduli by
Lemma~\ref{lem:fiber-cohomology-center}), we have
...
\label{rem:thqg-III-holographic-dictionary}
\index{holographic dictionary!genus 1}
The decomposition~\eqref{eq:thqg-III-genus-1-boundary}--\eqref{eq:thqg-III-genus-1-bulk}
...
\begin{equation}\label{eq:thqg-III-genus-1-partition}
Z_1(\cA; \tau)
= \Tr_{\cA}(q^{L_0 - c/24})
...
\label{rem:thqg-III-higher-genus}
...
The shadow tower $\Theta_\cA^{\le r}$ controls the
nonperturbative corrections to the higher-genus partition
function: the arity-$r$ shadow jet $\Sh_r(\cA)$ contributes
to the $r$-point function on $\overline{\mathcal{M}}_g$.
For the Virasoro algebra, the shadow tower is infinite
(Theorem~\ref{thm:nms-virasoro-quintic-forced}), reflecting
the infinite tower of gravitational quantum corrections.
\end{remark}

\subsubsection{Entanglement entropy from the complementarity
potential}
\label{subsubsec:thqg-III-entanglement}

\begin{proposition}[Entanglement entropy from the Hessian;
\ClaimStatusHeuristic]
\label{prop:thqg-III-entanglement-entropy}
...
\begin{equation}\label{eq:thqg-III-entanglement}
S_{\mathrm{EE}}^{(1)}
...
\label{rem:thqg-III-ryu-takayanagi}
...
\ClaimStatusProvedHere]
\label{thm:thqg-III-landscape-census}
...
& $k \dim\mathfrak{g} / 2$ & L & $3$ & cubic
& $\mathfrak{C}_{\mathrm{aff}}$
\\
$\beta\gamma$
& $-1$ & C & $4$ & quartic & $\mathfrak{Q}_{\beta\gamma}$
\\
Lattice $V_\Lambda$
...
(Theorem~\ref{thm:nms-affine-cubic-normal-form}), but the
quartic shadow vanishes in minimal gauge:
$\mathfrak{Q}_{\mathrm{aff}} = 0$
(Theorem~\ref{thm:nms-finite-termination}(ii)).
Class~L, $S_\cA$ cubic.

The modular characteristic is
$\kappa(\widehat{\mathfrak{g}}_k) = k \dim\mathfrak{g} / 2$
(Theorem~\ref{thm:kappa-kac-moody}).
The Koszul dual is
$\widehat{\mathfrak{g}}_k^! =
\widehat{\mathfrak{g}}_{-k-2h^{\vee}}$
(Feigin--Frenkel duality), giving
$\kappa(\widehat{\mathfrak{g}}_k^!)
= (-k - 2h^{\vee}) \dim\mathfrak{g} / 2$
and $\kappa + \kappa^! = -h^{\vee} \dim\mathfrak{g}
= -\dim\mathfrak{g} \cdot h^{\vee}$, consistent with the
anti-symmetry of $\kappa$ under duality.

\emph{$\beta\gamma$ system.}
The $\beta\gamma$ system has central charge $c = -2$ and
modular characteristic $\kappa(\beta\gamma) = -1$.
...
$\mu_{\beta\gamma} = 0$
(Corollary~\ref{cor:nms-betagamma-mu-vanishing}), but the
...
The shadow tower terminates at arity $2$ because
...
(Theorem~\ref{thm:virasoro-c26-duality}).
...
(Theorem~\ref{thm:nms-virasoro-quartic-contact}).
...
$\kappa(W_N) = c(W_N)/2$ where $c(W_N)$ is the central
charge of the $W_N$ algebra at level $k$.
The shadow tower is infinite for $N \ge 3$ (the $W_3$ cubic
...
\ClaimStatusProvedHere]
\label{prop:thqg-III-independent-sum}
...
\begin{proof}
By Proposition~\ref{prop:independent-sum-factorization}, the
...
action]
\label{rem:thqg-III-gravitational-action}
...
\end{itemize}
The Virasoro shadow tower being infinite reflects the
...
\label{rem:thqg-III-holographic-datum}
...
(Proposition~\ref{prop:collision-residue-twisting}).
\end{remark}

\begin{theorem}[Universality of the complementarity package;
\ClaimStatusProvedHere]
\label{thm:thqg-III-universality}
...
\item Choice of curve $X$ at genus $0$
  \textup{(}Proposition~\textup{\ref{prop:genus0-curve-independence})}.
...
\item Modular transformations $\gamma \in
  ...
  \textup{(}Corollary~\textup{\ref{cor:modular-properties})}.
\end{enumerate}
...
\emph{Part (iii).}
Corollary~\ref{cor:modular-properties}: the complementarity
...
(Theorem~\ref{thm:mc2-bar-intrinsic}).  Since $\Theta_\cA$ is
...
the modular shadow tower $\Theta_\cA^{\le r}$: each finite
