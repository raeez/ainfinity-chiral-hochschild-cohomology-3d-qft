...
(Theorem~\ref*{V1-thm:verdier-bar-cobar}), and the genus-$0$
...
(Volume~I, Theorem~D; for $\mathcal{W}$-algebras, $\kappa + \kappa^! = K(\fg)$).
...
inversion (Theorem~\ref*{V1-thm:bar-cobar-isomorphism-main})
...
symplectic form on the modular functor $Z_\cA(\Sigma_g)$
...
equips the modular functor with a symplectic structure via the
...
components.  At genus~$1$, the map $\Phi_1$ sends the
...
axioms).  The comparison should follow from the identification
...
\mathcal{Z}(\cA))$ with the trace pairing on the modular functor,
which is classical (Zhu's theorem).
\end{remark}
