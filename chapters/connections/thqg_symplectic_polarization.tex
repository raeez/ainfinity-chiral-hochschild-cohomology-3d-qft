...
Theorem~\ref{V1-thm:shifted-symplectic-complementarity} upgrades
this to a $(-1)$-shifted symplectic polarisation via the BV
formalism. The ambient complex carries a Verdier involution whose
eigenspace decomposition is unconditional on the Koszul
locus~(C1); the shifted-symplectic Lagrangian structure~(C2) is
the geometric home of the shadow obstruction tower
$\Theta_\cA^{\le 2} \to \Theta_\cA^{\le 3} \to \Theta_\cA^{\le 4}
\to \cdots$. The complementarity potential $S_\cA$ generates the
dual Lagrangian as a formal graph; its Taylor jets are the shadow
jets $H_\cA$, $\mathfrak{C}_\cA$, $\mathfrak{Q}_\cA$ of
Appendix~\ref*{V1-app:nonlinear-modular-shadows}; at genus~$1$
the construction reduces to a one-line Lagrangian-capacity
computation.  A physical BTZ or entanglement interpretation requires
the AdS$_3$/CFT$_2$ comparison datum, Hilbert completion, modular
...
\emph{conditional} on the following Volume~I ingredients, cited
rather than reproved here: the perfectness
...
but $\kappaChHodge(\cA) + \kappaChHodge(\cA^!) = 13$ for Virasoro and
$\kappa + \kappa^! = \varrho_{\cA}\,K$ for principal
$\mathcal{W}_N$-algebras
(with $\varrho_{\cW_N} = H_N - 1$ and $K = 4N^3 - 2N - 2$; the
Bershadsky--Polyakov $\kappa$-sum is open, its central-charge
conductor being $K_{\BP} = 50$ with no derived
generator-profile coefficient):
...
Koszul locus, requiring no perfectness or nondegeneracy
hypothesis.
...
$\dim \mathcal{H}_g(\cA)
= 2 \dim Q_g(\cA)$
(Theorem~\ref{V1-thm:self-dual-halving}).
\end{proof}
...
$\dfib^{\,2} = \kappaChHodge(\cA) \cdot \omega_g$ vanishes
...
The essential definitions of \cite{PTVV13} take the following
form.
...
(Theorem~\ref{V1-thm:bv-functor}): on the chirally Koszul locus,
bar cohomology is concentrated, and under the line-face formality
hypothesis the homotopy Koszul dual~$\cA^!_\infty$ reduces to~$\cA^!$.
Without this hypothesis the same pairing is valued in the homotopy
dual~$\cA^!_\infty$. Bar-cobar inversion
(Theorem~\ref*{V1-thm:bar-cobar-isomorphism-main}) ensures the induced
...
Verification: the two PTVV Lagrangian conditions
...
structures; \ClaimStatusConditional]
...
\subsection{Holographic comparison datum: genus-$1$ capacity and BTZ}
...
The complementarity theorem has a holographic reading only after a
physical AdS$_3$/CFT$_2$ comparison datum is supplied. Such a datum
consists of a completed boundary-CFT genus-$g$ partition function
together with a chosen bulk saddle expansion, or with a gravitational
functional integral after the bulk measure and contour prescription
have been specified, over geometries with conformal boundary of genus
$g$. At genus~$1$, one permitted high-temperature saddle is the BTZ
black hole \cite{BTZ92}. Complementarity supplies the algebraic
bulk--boundary polarization entering such a comparison; it does not
construct the bulk measure, prove modular invariance, select saddles,
or imply a physical entropy formula.
...
\mathbb{C} \cdot \kappaChHodge(\cA)$, where $\kappaChHodge(\cA)$ is the
...
\item The complementarity decomposition is:
 \begin{align}
 Q_1(\cA) &= \mathbb{C} \cdot \kappaChHodge(\cA)
...
\item The Verdier pairing at genus $1$ is the standard
 ...
 \langle \kappaChHodge(\cA), \lambda_1 \rangle_1 = 1,
 \qquad
 \langle \kappaChHodge(\cA), \kappaChHodge(\cA) \rangle_1 = 0,
 \qquad
 \langle \lambda_1, \lambda_1 \rangle_1 = 0.
 \end{equation}

\item The genus-$1$ complementarity package has a single scalar
 degree-$2$ channel and no nonlinear shadow jets:
 \begin{equation}% label removed: eq:thqg-III-genus-1-capacity
 \rank H_\cA^{(1)} = \dim Q_1(\cA)=1,
 \qquad
...
Since $\kappaChHodge(\cA) \in H^0$ and
$\lambda_1 \in H^2 = H^{3 \cdot 1 - 3}$,
the integration pairing gives
$\langle \kappaChHodge(\cA), \lambda_1 \rangle_1
= \int_{\overline{\mathcal{M}}_{1,1}} \kappaChHodge(\cA) \cdot
...
\emph{Part (iv).}
The line $Q_1(\cA)$ is one-dimensional, and the Verdier pairing
identifies its dual with the line $Q_1(\cA^!)$.  Hence the
degree-$2$ channel $H_\cA^{(1)}$ is a rank-one bilinear form after the
center-pairing normalization.  The cohomology of
$\overline{\mathcal{M}}_{1,1}$ has no products beyond the top Hodge
line, so the genus-$1$ cubic and quartic shadow projections vanish.
\end{proof}
...
\end{tabular}
\end{center}
The central charge $\kappaChHodge(\cA)$ controls the
boundary conformal anomaly; its Lagrangian complement
$\lambda_1$ controls the bulk gravitational curvature.
The nondegenerate pairing between them is the algebraic datum that may
enter a bulk--boundary entanglement construction only after a
Hilbert-space comparison datum has been supplied.
\end{remark}
...
\begin{theorem}[Genus-$1$ complementarity factorization with BTZ
comparison package; \ClaimStatusConditional]
% label removed: thm:thqg-III-btz-partition
\index{BTZ black hole!comparison package}
...
Assume the completed physical genus-$1$ partition function
\(Z_1^{\mathrm{phys}}(\cA;\tau,\bar\tau)\) satisfies:
\begin{enumerate}[label=\textup{(B\arabic*)}]
\item it admits an algebraic factorization morphism
 \(Z_1^{\mathrm{phys}}=
  Z_1^{\mathrm{bdry}}\cdot Z_1^{\mathrm{bulk}}\)
 whose logarithmic differential is the Verdier pairing on
 \(\mathcal H_1(\cA)=Q_1(\cA)\oplus Q_1(\cA^!)\);
\item its boundary factor has vacuum exponent \(-c/24\) and state
 coefficients \(d(h)\);
\item its bulk factor carries the Hodge automorphy factor determined
 by \(\lambda_1\);
\item it is modular invariant and vacuum dominated in the
 high-temperature \(S\)-transformed chamber.
\end{enumerate}
Then complementarity identifies the two factors with the two
eigenspaces:
\begin{enumerate}[label=\textup{(\alph*)}]
...
\item The bulk sector
 ...
 Hodge-class datum that carries the modular weight of the completed
 physical boundary CFT under \textup{(B3)--(B4)}.
\end{enumerate}
The obstruction to such a product formula is the failure of
\textup{(B1)}, equivalently the connected mixed cumulant of the two
eigenspaces in the completed trace. Complementarity proves the
polarization and the pairing; \textup{(B1)--(B4)} are the exact extra
inputs needed to turn that algebraic polarization into a BTZ-compatible
...
separates the algebraic genus-$1$ state space into two eigenspaces.
The boundary sector $Q_1(\cA)$
contains $\kappaChHodge(\cA)$, which determines the vacuum energy
$-c/24$ via the Sugawara construction. The bulk sector
$Q_1(\cA^!)$ contains $\lambda_1$, whose action on the
scalar trace is multiplication by the Hodge class;
this is the modular weight.

The product decomposition
\(Z_1^{\mathrm{phys}}=
Z_1^{\mathrm{bdry}}\cdot Z_1^{\mathrm{bulk}}\)
is precisely hypothesis \textup{(B1)}. Its obstruction is the connected
mixed cumulant between \(Q_1(\cA)\) and \(Q_1(\cA^!)\) in the completed
trace. The Lagrangian property kills the symplectic self-pairings of
the two eigenspaces; it does not kill that analytic mixed cumulant by
itself.

\emph{BTZ identification.}
In three-dimensional gravity, the high-temperature genus-$1$ saddle is
the BTZ black hole geometry \cite{BTZ92, Maloney-Witten07}.
The BTZ entropy $S_{\BTZ} = 2\pi\sqrt{c \cdot h / 6}$
is the Cardy formula under the usual modular-invariance,
vacuum-dominance, and saddle hypotheses. The Hodge line
$Q_1(\cA^!)$ is the algebraic place where the modular-weight datum
lives. Under those additional hypotheses one obtains the dictionary:
\[
...
$\mathcal{H}_g(\cA) = Q_g(\cA) \oplus Q_g(\cA^!)$
separates the algebraic higher-genus state space into boundary
and bulk sectors. A physical higher-genus black-hole partition
function requires an additional handlebody/non-handlebody saddle
prescription and modular-invariance theorem. The $(-(3g-3))$-shifted
...
algebraic corrections to the higher-genus scalar trace: the degree-$r$
shadow jet $\Sh_r(\cA)$ contributes
...
the infinite tower of algebraic shadow corrections. The
nonperturbative gravitational interpretation is a separate physical
hypothesis.
\end{remark}

\subsubsection{Lagrangian capacity bound from the complementarity
potential}
...
\index{Lagrangian capacity!genus 1}
\index{entanglement entropy!capacity bound}
For a unitary chiral algebra $\cA$ with central charge $c$,
let
\begin{equation}% label removed: eq:thqg-III-entanglement
C_1(\cA) := \rank H_\cA^{(1)}=\dim Q_1(\cA)=1
\end{equation}
be the genus-$1$ Lagrangian capacity.  Assume in addition that a
physical comparison datum realizes \(Q_1(\cA)\) and \(Q_1(\cA^!)\) as
trace-class Hilbert factors whose dimensions are bounded by
\(C_1(\cA)\) and \(C_1(\cA^!)\), and supplies a normalized positive
trace-class state \(\rho\) on their completed tensor product.  Then
the von Neumann entropy of either reduced state satisfies
\begin{equation}% label removed: eq:thqg-III-capacity-bound
0 \leq S_{\mathrm{vN}}(\rho)
:= -\Tr(\rho\log\rho)
\leq \log \min\{C_1(\cA),C_1(\cA^!)\}
=0.
\end{equation}
Thus any entropy in this realized genus-$1$ capacity lane vanishes.
Complementarity itself proves only the algebraic capacity and
pairing; it does not construct a density matrix, a partial trace, or a
Hilbert tensor product.
\end{proposition}

\begin{proof}
The genus-$1$ computation above gives
\(C_1(\cA)=C_1(\cA^!)=1\).  A density matrix on a Hilbert factor of
dimension at most one has rank at most one.  The standard entropy
bound \(0\leq S_{\mathrm{vN}}(\rho)\leq \log \rank(\rho)\) gives the
displayed inequality.  The last assertion is a scope statement: the
Verdier pairing is a perfect algebraic pairing of finite-dimensional
lines, not a positive trace-class operator on a Hilbert tensor
product.
\end{proof}

\begin{remark}[Scope]
The Lagrangian polarization gives a direct-sum decomposition
\(\mathcal{H}_1 = Q_1(\cA)\oplus Q_1(\cA^!)\), not a tensor product.
A physical entropy requires a completion that turns the algebraic
summands into Hilbert factors, a positive normalized state, and a
partial-trace operation.  At genus \(g\ge2\), complementarity gives the
capacity \(C_g(\cA)=\dim Q_g(\cA)\) when finite; it bounds possible
entropies after such a realization, but it does not compute them.
\end{remark}

\begin{remark}[Ryu--Takayanagi comparison datum]
% label removed: rem:thqg-III-ryu-takayanagi
\index{Ryu--Takayanagi formula!comparison datum}
The Ryu--Takayanagi formula
$S_{\mathrm{EE}} = \frac{\mathrm{Area}(\gamma_A)}{4G_N}$
relates entanglement entropy to the area of the minimal
surface $\gamma_A$ in the bulk \cite{Ryu-Takayanagi06}.  In the
present algebraic surface, the rank of the Hessian is a Lagrangian
capacity, not an area:
\[
\rank(H_\cA^{(g)}) = \dim Q_g(\cA).
\]
A physical RT or quantum-extremal-surface interpretation additionally
requires a boundary subregion, a bulk metric, a homology constraint,
and a variational minimal-surface problem.  Under such a comparison
datum the capacity can bound the number of algebraic channels
available to the entropy calculation; it is not itself
\(\mathrm{Area}(\gamma_A)/(4G_N)\).
\end{remark}
...
Verification: the full complementarity package (C1 + C2) for
...
\textbf{Family} & $\kappaChHodge(\cA)$ & \textbf{Class}
...
$\kappaChHodge(\widehat{\mathfrak{g}}_k) = \dim(\mathfrak{g})\cdot(k+h^\vee)/(2h^\vee)$
(Volume~I, Theorem~D).
The Koszul dual is
...
modular characteristic $\kappaChHodge(\beta\gamma) = c_{\beta\gamma}/2$
...
The scalar quartic contraction on the weight-changing line is
$\mu_{\beta\gamma} = 0$
(Corollary~\ref*{V1-cor:nms-betagamma-mu-vanishing}), but the
quartic shadow itself is nonzero on the full envelope.
Class~C, with a terminal quartic contact layer in the full composite
envelope.
...
characteristic $\kappaChHodge(V_\Lambda) = \rank(\Lambda)$
...
The Virasoro algebra has $\kappaChHodge(\mathrm{Vir}_c) = c/2$
...
$\kappaChHodge(W_N) = c(W_N) \cdot (H_N - 1)$ where $H_N = \sum_{j=1}^{N} 1/j$
...
failure of the finite trace polarization at the critical level.
\end{remark}
...
$\dim Q_2 = \dim Q_2^! = 4$
\textup{(}Theorem~\textup{\ref{V1-thm:self-dual-halving})}.
\end{proposition}

\begin{proof}
By Theorem~\ref{V1-thm:self-dual-halving},
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\kappaChHodge(\cA_1 \oplus \cA_2)
 = \kappaChHodge(\cA_1) + \kappaChHodge(\cA_2)$
...
\begin{remark}[The complementarity potential under a gravitational
comparison datum]
% label removed: rem:thqg-III-gravitational-action
\index{complementarity potential!comparison datum}
The complementarity potential $S_\cA$ is the formal function whose
...
interpretation of $S_\cA$ as an action requires a comparison datum:
a bulk field space, BV/gauge-fixing data, boundary polarization, a
normalization of observables, and a renormalization scheme.  Only after
such data have been fixed can the algebraic shadow channels be read as
metric or Chern--Simons vertices.  The algebraic dictionary is:
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{l@{\quad}l@{\quad}l}
\textbf{Jet} & \textbf{Algebraic role} &
\textbf{After comparison} \\
\midrule
$H_\cA = \Sh_2$ & quadratic shadow channel &
 Newton-coupling channel \\
$\mathfrak{C}_\cA = \Sh_3$ & cubic shadow &
 possible cubic vertex channel \\
$\mathfrak{Q}_\cA = \Sh_4$ & quartic shadow &
 possible quartic/contact channel \\
$\Sh_5, \Sh_6, \ldots$ & higher shadows &
 higher vertex or composite channels \\
\end{tabular}
\end{center}
The shadow depth class determines the algebraic length of the
obstruction tower, not by itself the perturbative complexity of a
metric gravitational dual:
\begin{itemize}
\item G class: only the quadratic shadow channel survives.
\item L class: a cubic shadow channel is present.
\item C class: a quartic/contact shadow channel is present.
\item M class: the shadow obstruction tower is unbounded.
\end{itemize}
For Virasoro and \(W_N\), the infinite tower is an algebraic
class-\(\mathsf M\) statement.  It is not a theorem that
three-dimensional gravity is perturbatively non-renormalizable, and it
does not produce gravitational counterterms unless a physical
renormalization problem and comparison map have first been specified.
\end{remark}
...
The complementarity package above is the third component of
the holographic modular Koszul datum $\mathcal{H}(T)$
...
\section{Synthesis}
\label{sec:sympol-supplement}

\begin{remark}[Symplectic polarization on $\mathbf H_{\Delta_5}$]
\label{rem:sympol-1}
The symplectic-polarization programme developed here singles out, at
the K3 base, the Mukai pairing on
$\Lambda_{\mathrm{Muk}} = II_{4,20}$ as the canonical symplectic datum.
The polarisation splits $\Lambda_{\mathrm{Muk}}\otimes\mathbb R$ into
the positive cone $V_+$ of dimension $4$ and the negative cone $V_-$
of dimension $20$, and the symplectic form on the $\mathbf H_{\Delta_5}$-module
category is
\[
\omega_{K3} \;=\;
\mathrm{Im}\bigl(\langle-, -\rangle_{\mathrm{Muk}}\bigr)\bigr|_{V_+\otimes V_-}
\;\in\; \Omega^2(\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})),
\]
a symplectic form on the period-domain tangent space
\(\operatorname{Hom}(V_+,V_-)\), of real dimension \(80\). The universal
\(8\) is not the rank of this form; it is the chiral normalisation
\(2c_+\) attached to the positive four-plane and hence the scalar
\(K^{\kappaChHodge}=8\) of Vol.~I,
\S\ref{V1-sec:chiralmod-supplement}. Geometric quantisation of
\(\omega_{K3}\), with this scalar normalisation, recovers
\(\mathbf H_{\Delta_5}\) as the Hilbert space of polarised sections at
the Lusztig specialisation
$\hbar^2 = -1/8$. Primary-lit: Mukai 1987, Bridgeland--Maciocia 2001,
Kontsevich--Soibelman 2008 (symplectic reductions).
\end{remark}
