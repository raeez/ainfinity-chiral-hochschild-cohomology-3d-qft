...
the bar-cobar adjunction alone.  Movement~VI derives this
duality at the level of the full MC element, the shadow tower,
the genus expansion, and the holographic connection.  The Verdier involution
...
Witt shadow tower remains open.
...
higher-genus scalar contributions through the shadow tower, and
...
it is the shadow tower's genus projection, controlled by $D_\cA^2 = 0$ rather than by
...
decomposes by arity into an infinite sequence of celestial soft
factors.  In the celestial/gravitational reading, this hierarchy is
read as the corresponding soft hierarchy.  Each $S_r$ is a Ward identity constraining the
$(n+1)$-point function in terms of the $n$-point function,
weighted by the arity-$r$ shadow.  The first three terms are read as
follows:
$S_2$ is read as the leading Weinberg soft factor;
$S_3$ is gauge-trivial (a consequence of cubic gauge triviality);
$S_4$ is read as the first quartic-contact correction, controlled by the
quartic contact invariant $\mathfrak{Q}^{\mathrm{contact}}
= 10/(c(5c+22))$;
$S_5$ is read as the first genuinely new higher-arity Ward identity in that
reading.
...
$T(z_0)$, read as a soft graviton insertion, into an $n$-point
correlator is determined by the sum over single
...
as the soft graviton, and the leading-order scattering statement is
...
gives a new Ward identity read as constraining a soft-graviton
insertion into a five-point function.  It involves the full cubic-quartic interaction
...
$r = 2$ & $S_2 = (c/2)\,\omega^{(2)} + \cdots$
  & Proved & Read as the leading Weinberg soft statement \\[2pt]
$r = 3$ & $S_3 = d_2\,\sigma_3$
  & Gauge-trivial & Read as no new independent soft factor \\[2pt]
$r = 4$ & $S_4 = \frac{10}{c(5c+22)}\,\omega^{(4)}$
  & Proved & Read as the quartic-contact correction in the soft hierarchy \\[2pt]
$r = 5$ & $S_5 \ne 0$
  & Proved (existence) & Read as the first genuinely new arity-$5$ soft Ward identity \\[2pt]
$r \ge 6$ & $S_r$
  & Infinite tower & Read as higher soft Ward identities in the celestial/gravitational hierarchy
\end{tabular}
\end{center}
...
\item The celestial soft factors are the $t$-integrated
  versions of the boundary celestial soft factors:
  $\widetilde{S}_2$ and $\widetilde{S}_4$ are the corresponding
  integrated arity-$2$ and arity-$4$ factors.
\end{enumerate}
\end{proposition}
...
the resulting soft algebra at null infinity through the Virasoro
algebra at the same central charge.  In the flat-space limit
$\ell \to \infty$ ($c \to \infty$), this soft algebra is read as the
BMS package, and the celestial OPE is correspondingly read as
reproducing the BMS Ward identities.  The shadow connection then
provides the all-arity extension: the full celestial soft hierarchy is
read as extending that BMS package through the projections of
$\Theta_{\mathrm{Vir}_c}$ along the celestial direction.  In this
reading, the integrated factors $\widetilde{S}_2$ and
$\widetilde{S}_4$ are read as the integrated leading Weinberg and
...
$\Theta = \kappa \cdot \omega_1$ (with $\kappa = c/2$) packages the
...
Celestial soft hierarchy: $S_2$ is read as the leading Weinberg soft statement, $S_3$ is
gauge-trivial, $S_4 = \frac{10}{c(5c+22)}\omega^{(4)}$, and
$S_5 \ne 0$ is read as the first genuinely new higher-arity Ward
identity.
