...
\label{ch:thqg-3d-gravity-movements}% alias for dangling \ref{ch:...}
\label{ch:thqg-3d-gravity-vi-x}% alias for dangling \ref{ch:...}
...
(Proposition~\ref{prop:gravity-koszul-dual}) is exact on the
gravitational theory, not a formal consequence of bar-cobar
adjunction. The Verdier involution $\mathbb{D}$ on
$\operatorname{Ran}(X)$ sends $\Theta_{\mathrm{Vir}_c}$ to
$\Theta_{\mathrm{Vir}_{26-c}}$; every gravitational observable
transforms predictably under $c \mapsto 26 - c$. The duality
extends to the full MC element, the shadow obstruction tower,
the genus expansion, and the holographic connection.
...
The Koszul dual has $\kappaChHodge(\mathrm{Vir}_{26-c}) = (26-c)/2$.
Sum: $(c + 26 - c)/2 = 13$.
\end{proof}

The total one-loop anomaly of the boundary theory plus its
Koszul dual is always~$13$. This Virasoro specialisation of
the complementarity theorem (Volume~I,
Theorem~\ref{V1-thm:quantum-complementarity-main}) reads, in
Lagrangian-geometric form, as a splitting of $\mathbf{C}_g$:
the pair $(\mathbf{Q}_g(\mathrm{Vir}_c),\,
\mathbf{Q}_g(\mathrm{Vir}_{26-c}))$ are Lagrangians with total
curvature $K \cdot \omega_g = 13\,\omega_g$, exchanged
by~$\mathbb{D}$.
...
\item The Koszul dual $\mathrm{Vir}_{26}^!
 ...
 higher-degree shadow obstruction tower.
...
\item $F_g^{\mathrm{eff}}(\mathrm{Vir}_{26} \otimes bc_2) = 0$ for all $g \ge 1$:
 ...
 since $\kappa_{\mathrm{eff}} = \kappaChHodge(\mathrm{Vir}_{26}) + \kappaChHodge(bc_2)
 = 13 - 13 = 0$.
\item The effective scalar genus tower of the dual side is
 $F_g^{\mathrm{eff}}(\mathrm{Vir}_0 \otimes bc_2) =
 -13\,\lambda_g^{\mathrm{FP}}$, where
 $\lambda_g^{\mathrm{FP}}
 =\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}\lambda_g$ and
 $\kappa_{\mathrm{eff}} = 0 + (-13) = -13$.
\end{enumerate}
\end{proposition}
...
(no quartic pole). Hence $\kappaChHodge(\mathrm{Vir}_0) = 0$, so the
scalar class vanishes and the bar complex is uncurved on the scalar
channel. This does not prove the full higher-degree tower vanishes.
(iii): $\kappaChHodge(\mathrm{Vir}_0) = 0$ and $\kappaChHodge(\mathrm{Vir}_{26})
= 13$. The Lagrangian $\mathbf{Q}_g(\mathrm{Vir}_0)$ has zero
fiber curvature; $d_{\mathrm{fib}}^2 = 0$ on $\barB(\mathrm{Vir}_0)$.
(iv): $F_g^{\mathrm{eff}}(\mathrm{Vir}_{26}\otimes bc_2)
= \kappaChHodge(\mathrm{Vir}_{26}\otimes bc_2)\lambda_g^{\mathrm{FP}}
= \kappa_{\mathrm{eff}}\lambda_g^{\mathrm{FP}} = 0$.
(v): $\kappa_{\mathrm{eff}}(\mathrm{Vir}_0) = (0-26)/2 = -13$,
so $F_g^{\mathrm{eff}}(\mathrm{Vir}_0 \otimes bc_2)
= -13\,\lambda_g^{\mathrm{FP}}$.
\end{proof}

At $c = 26$, the matter Virasoro anomaly cancels the ghost
contribution at the effective scalar level. The Koszul dual
$\mathrm{Vir}_0$ is the Witt algebra: the symmetry of the
classical closed-string worldsheet with vanishing scalar
curvature, though not a proved trivial higher-degree tower.
The scalar complementarity package passes to the dual side;
identification of the full closed-string genus expansion with
the full Witt shadow obstruction tower remains a construction
problem.
...
\begin{proof}
The shadow connection is the degree-$(n,0)$ genus-$0$ projection of
$\Theta_\cA$. By Theorem~\ref{thm:gravity-verdier-intertwining}(ii),
$\mathbb{D}(\Theta_{\mathrm{Vir}_c})
= \Theta_{\mathrm{Vir}_{26-c}}$. Projecting to the same degree
...
For affine Kac--Moody $\hat{\mathfrak{g}}_k$, the chiral Koszul target is
\[
\hat{\mathfrak{g}}_k^!
\simeq
\operatorname{CE}^{\mathrm{ch}}\!\bigl(\hat{\mathfrak{g}}_{-k-2h^\vee}\bigr),
\]
and the scalar complementarity constant of the reflected current shadow
vanishes:
\[
K_{\hat{\mathfrak{g}}}
\;=\;
\kappaChHodge(\hat{\mathfrak{g}}_k)
+ \kappaChHodge(\hat{\mathfrak{g}}_{-k-2h^\vee})
\;=\;
...
Each row gives the chiral algebra, its Koszul involution, the scalar
complementarity $\varkappa := \kappa + \kappa^!$ (distinct from the Trinity
ghost-charge conductor $K := c + c^! = -c_{\mathrm{ghost}}(\BRST)$; the
two are related by $\varkappa = \varrho\cdot K$), and the self-dual
point where the gravitational theory is its own $S$-dual.

\subsubsection*{The duality map on scalar partition functions}
...
$F_g(\mathrm{Vir}_c)
= \kappaChHodge(\mathrm{Vir}_c) \cdot \lambda_g^{\mathrm{FP}}
...
This scalar-shadow product is independent of~$c$.
\end{remark}
...
The duality of Movement~VI maps every shadow at central
charge~$c$ to its partner at $26 - c$ but says nothing about
how \emph{many} shadows exist. The tower is bottomless.

The support shadow depth \(r^\Theta_{\max}(\cA)\) of the boundary chiral algebra
determines the minimal homotopy-algebraic complexity of the dual
gravitational theory. For Virasoro:
\(r^\Theta_{\max}(\mathrm{Vir}_c) = \infty\) at generic~$c$. The tower
...
\begin{proof}
Construction: an explicit nonzero cocycle representative.

\emph{Step 1: the obstruction source.}
The quartic shadow
$\Theta_{\mathrm{Vir}_c}^{\le 4} \in F^2\gAmod$
satisfies the MC equation through degree~$4$:
\[
D\,\Theta^{\le 4}
...
+ \tfrac{1}{2}[\Theta_3, \Theta_3]_{\mathrm{degree}\,5}
\;\in\; F^5\gAmod/F^6\gAmod,
\]
where $\Theta_r$ denotes the degree-$r$ component.

\emph{Step 2: chain-level computation.}
Work on $\FM_5(\C) \cong K_5$ (the Stasheff associahedron).
The bracket $[\Theta_3, \Theta_3]_{\mathrm{degree}\,5}$ is a
...
The computation gives the relevant $E_1$~page dimensions explicitly.
...
map $\xi \mapsto [\Theta_2, \xi]_{\mathrm{degree}\,5}$, induced
...
tower terminates at degree~$4$: the gravitational theory has a
...
one-dimensional $E_1$~space at the relevant degree.

(iv): the differential $d_1$ at degree~$4$ computes the bracket
...
$E_2^{r-1,0} \ne 0$ or $d_1^{r-1,0} \ne 0$. By induction,
the existence of nonzero obstructions at all degrees (guaranteed by
...
Let $\gAmod$ carry the degree filtration
...
degree~$\le r$. The element $\Theta_\cA^{\le r}$ satisfies the
...
\]
and the failure at the next degree is the obstruction class
...
modulo $F^{r+1}$; the failure at the next degree is precisely the
...
\hline
$r = 2$ & $\kappa = c/2$ & Read as the complete scalar genus tower \\
...
The shadow obstruction tower of Movement~VII is infinite: class~M,
nonvanishing at every degree. The matching genus expansion of
3d gravity nevertheless converges with radius~$2\pi$. The
infinity of vertices is tamed by the Bernoulli decay of the
Faber--Pandharipande numbers: the general HS-sewing theorem
(Theorem~\ref{thm:general-hs-sewing}) applied to Virasoro,
combined with the explicit $\hat A$-genus generating function.
...
\begin{theorem}[Convergence of the effective scalar gravitational genus tower;
\ClaimStatusProvedHere]
\label{thm:gravity-genus-convergence}
\index{genus tower!convergence!gravitational}
The all-genera effective scalar free energy (incorporating both
the matter curvature $\kappaChHodge(\mathrm{Vir}_c) = c/2$ and the
ghost curvature $\kappaChHodge(bc_2) = -13$ into the effective
...
= \kappaChHodge(\mathrm{Vir}_c\otimes bc_2)\lambda_g^{\mathrm{FP}}.
\]
This
...
The scalar-shadow interpretation: $\hbar = 2\pi$ is the
first pole of $\hbar/(2\sin(\hbar/2))$, corresponding to the
first nonzero zero of $\sin(\hbar/2)$. The convergence radius
$2\pi$ in the scalar genus-counting parameter is finite
but nonzero. This theorem proves convergence of the FP scalar tower;
it does not supply a Gevrey-$1$ Page/Stokes transseries.
\end{proof}
...
\lambda_g^{\mathrm{FP}}$
\textup{(}uniform-weight\textup{)}, where
$\lambda_g^{\mathrm{FP}}
= \frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!}$
is the Faber--Pandharipande number.
...
\begin{remark}[Renormalized UV finiteness]
\label{rem:gravity-no-uv}
\index{UV finiteness!3d gravity}
The convergence of the genus expansion is achieved after extension to
the Fulton--MacPherson boundary. The boundary strata of $\FM_n(\C)$
separate the short-distance singularities into local counterterms
and finite residues. The degree cutoff
(Lemma~\ref{V1-lem:degree-cutoff}) bounds the contributing graphs
at each genus, and the HS-sewing condition ensures the sums over
internal states converge. This is the algebraic form of perturbative
UV finiteness for the renormalized 3d gravity sector.
\end{remark}
...
formulation gives the same semiclassical one-loop datum after a flat
connection and boundary polarization have been fixed: the quadratic
BV determinant is the Ray--Singer/Reidemeister torsion. For the
thermal AdS$_3$ vacuum this torsion is the eta/vacuum-character
determinant factor after the chosen boundary-mode normalization. It is
not the full Chern--Simons path integral over all flat connections or
the complete modular sum.
...
The intrinsic genus-$g$ free energy
$F_g(\mathrm{Vir}_c)=\kappaChHodge(\mathrm{Vir}_c)\lambda_g^{\mathrm{FP}}$
\textup{(}uniform-weight\textup{)}
with $\kappaChHodge(\mathrm{Vir}_c)=c/2$ satisfies:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $|F_g(\mathrm{Vir}_c)| \le |\kappaChHodge(\mathrm{Vir}_c)| \cdot
 \frac{4}{(2\pi)^{2g}}$ for all $g \ge 2$.
\item $F_g(\mathrm{Vir}_c)$ is rational:
 \(F_g(\mathrm{Vir}_c) \in
 \Q \cdot \kappaChHodge(\mathrm{Vir}_c)\).
...
Beyond the convergence of the genus tower (Movement~VIII)
lies the question of degree-by-degree shadow content:
each shadow $S_r$ is a celestial soft factor.
...
\subsubsection*{Degree-$2$: the celestial soft factor and its gravitational reading}
...
The degree-$2$ component of the shadow connection is
...
\begin{proof}
The shadow connection at genus~$0$ and degree~$2$ is the
...
\subsubsection*{Degree-$3$: gauge triviality}
...
The degree-$3$ component $S_3$ of the shadow connection is
gauge-trivial:
\[
S_3 \;=\; d_2\,\sigma_3
\]
...
independent new degree-$3$ contribution to the celestial soft
hierarchy.
\end{theorem}

\begin{proof}
Verification: $H^1(F^3\gAmod/F^4\gAmod, d_2) = 0$.
The graded piece $F^3/F^4$ consists of cyclic ternary
operations on $\mathrm{Vir}_c$. The $E_1$~page at degree~$3$
...
degree-$2$ deformation $\delta m_2$ to the induced ternary
change: $d_2(\delta m_2) = [m_2, \delta m_2]_{\mathrm{degree}\,3}$.
...
degree-$2$ sector rather than as a new independent soft factor.

\subsubsection*{Degree-$4$: the quartic celestial soft factor and its gravitational reading}
...
The degree-$4$ component of the shadow connection is
...
\begin{proof}
The computation gives the quartic contact integral on $\FM_4(\C)$.
...
The degree-$4$ shadow is determined by the Stasheff relation at
$n = 4$:
\[
d_{\FM_4}\,m_4
\;=\;
...
\right\rangle.
\end{equation}
At $c \to \infty$ (semiclassical gravity): $S_4 \sim 2/c^2
...
\subsubsection*{Degree-$5$: the first new higher-degree celestial soft factor}
...
The degree-$5$ component $S_5$ of the shadow connection is
...
\end{equation}
Thus $S_5$ is the first genuinely new degree-$5$ component of the
celestial soft hierarchy: it is not determined by the lower-degree
data $S_2$ and $S_4$.
\end{theorem}
...
MC equation at degree~$5$ would give $o_5 = d_2(D\sigma_5 +
...
higher-degree soft insertion in a five-point function. It involves the full cubic-quartic interaction
...
$r = 5$ & $S_5 \ne 0$
 & Proved (existence) & Encodes the first genuinely new degree-$5$ soft Ward identity \\[2pt]
$r \ge 6$ & $S_r$
...
\noindent
The pattern: even-degree shadows carry the physical content (contact
invariants), while odd-degree shadows tend to be gauge-trivial or
...
appears at degree~$4$, not degree~$3$; in the
...
\item Accordingly, the integrated celestial soft factors
 ...
 $t$-integrated degree-$2$ and degree-$4$ factors attached to the
...
provides the all-degree extension: the full celestial soft hierarchy is
...
The detailed derivation of the soft graviton comparison
(the degree-$2$ Virasoro Ward package and Weinberg comparison,
the Cachazo--Strominger superrotation channel from the degree-$3$
shadow together with the $S_2$ weight term, the quartic
sub-subleading contact channel, the quintic prediction from $S_5$,
and the infinite tower theorem) is developed in
...
The gravitational duality $c \leftrightarrow 26-c$~(VI), the
infinite complexity of the shadow obstruction tower~(VII), the
convergence of the genus expansion~(VIII), and the celestial soft
hierarchy~(IX) all depend on the single parameter~$c$. All four
change character at two distinguished values: $c = 13$ (the
self-dual point, where the Koszul involution is the identity) and
$c = 26$ (the critical string, where the effective anomaly
vanishes). The collision of these two values produces the
...
specialization.
(iii): $\kappa_{\mathrm{eff}} = 0$, hence the scalar coefficient
...
the full higher-degree Witt tower. The residual
...
of the sewing amplitudes at degree~$2$ (the propagator $K$).
The expansion involves only \emph{binary} contractions, traces
of products of the propagator.

\emph{Step 2: Virasoro requires higher-degree sewing.}
...
expressible as a sum of binary traces. But the degree-$4$
contribution
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\delta_2^{(g)}(\mathrm{Vir}_c) = 0$: at degree~$2$, the
...
exactly by the Fredholm approximation at degree~$2$.

(ii): the degree-$3$ correction involves
...
(Proposition~\ref{prop:gravity-hs-sewing}): the degree-$r$
...
M & $\mathrm{Vir}_c$ & $\infty$
 & No ($\delta_r \ne 0$ for all $r \ge 4$)
 & $\kappaChHodge(\mathrm{Vir}_c{\otimes}bc_2)(\hat A(i\hbar)-1)$ & $\mathfrak{Q}_{\mathrm{Vir}}$
\end{tabular}
\end{center}
...
= \kappaChHodge(\mathrm{Vir}_c\otimes bc_2)(\hat A(i\hbar)-1)$
...
other complexity classes require higher-degree corrections, with
...
effective scalar shadow series; \ClaimStatusProvedHere]
\label{prop:gravity-u-eigenvalue}
\index{spectral decomposition!genus scalar shadow series}
The genus-$g$ effective scalar shadow series decomposes as
...
\end{enumerate}
For generic~$c$, $u \ne 0$ and
$Z_g^{\mathrm{strict,scal}} = 0$: the effective scalar shadow
series is computed by the curved scalar sector alone.
\end{proposition}
...
\subsubsection*{The effective scalar Virasoro genus-$g$ shadow: explicit form}

Combining the gravitational transgression algebra with the genus
tower~\eqref{eq:gravity-generating}:

\begin{theorem}[Effective scalar gravitational shadow generating function;
\ClaimStatusProvedHere]
\label{thm:gravity-partition-function-explicit}
\index{partition function!Virasoro!explicit form}
The all-genera effective scalar shadow generating function of the
Virasoro gravity lane is
...
\;+\; O(\hbar^{2g+2}).
\]
This is the FP scalar shadow. It is not the physical BTZ/Cardy
partition function unless a modular-invariant completion, vacuum
dominance, and saddle analysis are supplied separately.
\end{theorem}
...
$Z_{\mathrm{grav}}^{\mathrm{scal}} = \exp(0) = 1$: the scalar
shadow series is trivial. This records vanishing of the effective
...
the effective scalar shadow series is the exponential series generated
by the normalized $\hat A$-class weighted by $-13/2$. Self-duality
means invariance under the reflection
$\cS\colon c\mapsto 26-c$, not inversion of the scalar series. Thus
$\cS(Z_c^{\mathrm{scal}}) = Z_{26-c}^{\mathrm{scal}}$ and at $c = 13$:
...
grows as $3k\,\lambda_g^{\mathrm{FP}}$, the standard
...
\emph{connected gravitational vertices} at genus~$g$ and degree~$r$.
\end{remark}
...
$|\hbar| < 2\pi$ after Fulton--MacPherson boundary extension.
Celestial soft hierarchy: $S_2$ encodes the leading Weinberg soft statement, $S_3$
is gauge-trivial and encodes no new independent soft statement,
$S_4$ encodes the first quartic-contact
correction, controlled by $\frac{10}{c(5c+22)}\omega^{(4)}$, and
$S_5 \ne 0$ encodes the first genuinely new higher-degree Ward
identity.
...
\section{Synthesis}
\label{sec:3dgravvix-supplement}

\begin{remark}[The ten movements at the K3 base: mathematical content and physical scope]
\label{rem:3dgravvix-1}
The ten movements realise the following mathematical constructions on
the K3 base, with each physical phrase translated into the
rigorous mathematical object it names.
\begin{enumerate}[label=\textup{(\Roman*)}]
\item \textbf{(Movement~VI, geometric.)} The Costello--Li
 holomorphic-topological twist of $11$D supergravity on
 $\mathbb{R}^3 \times K3 \times \mathbb{C}^2$ is the
 Costello--Gaiotto--Paquette $2018$~\cite{CostelloGaiottoPaquette2018}
 $E_3$-holomorphic factorisation algebra on $\mathbb{R}^3 \times
 (K3 \times \mathbb{C}^2)$; its factorisation-homology
 specialisation along the $S^2$-fibre of a hyperk\"ahler rotation
 $\Sigma_3 = S^2$ gives an $E_1$-chiral factorisation algebra on
 the curve $\mathbb{C} \subset \mathbb{R}^3$. The phrase
 ``dimensional reduction of twisted $11$D SUGRA on $K3 \times T^2$
 to $3$d gravity on $\mathrm{AdS}_3 \times S^2$'' is the physical
 reading of this two-stage factorisation-algebra specialisation at
 the level of the holomorphic twist, which requires the
 Costello--Gaiotto--Paquette twisted-holography comparison; the
 dynamical-metric reading on $\mathrm{AdS}_3 \times S^2$ remains a
 physical heuristic, not a constructed path integral.
\item \textbf{(Movement~VII, algebraic.)} The
 $\mathbf{H}_{\Delta_5}$-module category is the category of
 modules over the Borcherds--Kac--Moody algebra
 $\mathfrak{g}_{\Delta_5}$ of Gritsenko--Nikulin
 $1998$~\cite{GritsenkoNikulin1998}; its complexified Grothendieck
 group carries the character
 $\mathrm{ch}(\mathbf{H}_{\Delta_5}\text{-Fock})
 = 1/\Phi_{10}^{\mathrm{un}}(Z)$
 via the denominator identity. The phrase ``boundary Hilbert
 space'' denotes this category, not a physical Hilbert space in the
 Wightman sense; the holographic dictionary identifies it with the
 asymptotic-symmetry module category at the
 Costello--Gaiotto--Paquette twisted-holography frame.
 The algebraic character statement is the Borcherds denominator
 computation.
\item \textbf{(Movement~VIII, automorphic; scope-qualified.)} The
 bulk factorisation-algebra partition function of the Costello--Li
 holomorphic twist at the Lusztig locus $\hbar = 1$ is
 $\Phi_{10}^{\mathrm{un}}(Z)/\eta(q)^{24}$
 (Theorem~\ref{thm:thqg-allloop-bulk-boundary-duality}). The phrase
 ``$\mathrm{AdS}_3$-partition function equals
 $\Phi_{10}^{\mathrm{un}}(Z)$''
 denotes this identity under the Costello--Gaiotto--Paquette
 twisted-holography dictionary and \emph{not} the Maloney--Witten
 $2009$ sum over smooth Euclidean saddles (BTZ and $\mathrm{SL}(2,
 \mathbb{Z})$-images of thermal $\mathrm{AdS}_3$), which evaluates
 a different formal path integral. The Dijkgraaf--Moore--Verlinde--Verlinde
 $1997$~\cite{DijkgraafMooreVerlindeVerlinde1997} generating
 function counts BPS black-hole microstates of type~IIA on
 $K3 \times T^2$ (equivalently, symmetric-product orbifold
 elliptic genera): this is \emph{also} $1/\Phi_{10}^{\mathrm{un}}(Z)$ by the
 Maldacena--Moore--Strominger $1999$ identification, but it equals
 the twisted-holography boundary character, not the dynamical-metric
 bulk partition function. The three identifications
 (twisted-holography bulk, DVV BPS counting, Maloney--Witten smooth
 saddle sum) coincide at the level of the automorphic form
 $\Phi_{10}^{\mathrm{un}}$ and diverge at the level of what integrates
 to produce it. The twisted-holography identification is proved at the
 factorisation-algebra scope; the Maloney--Witten identification
 requires the full off-shell path integral and remains conjectural.
\item \textbf{(Movement~IX, cross-volume.)} The gravitational
 $S$-duality $c \mapsto 26 - c$ on the Virasoro family and the
 $24$-fold soft-graviton structure on the K3 pair are
 established in Vol.~II
 \S\ref{sec:grav-sdual-supplement} and \S\ref{sec:softgrav-supplement};
 the topological count ``$24$'' is the arithmetic identity
 $24 = \chi_{\mathrm{top}}(K3)$, coinciding under
 Gopakumar--Vafa with the $24$ M$5$-branes wrapping the Kodaira
 $I_1$ singular fibres of the elliptic $K3$
 (Vafa $1996$~\cite{Vafa1996F}); the identification is a theorem
 at the twisted-holography scope (Costello $2021$ five-brane vertex
 algebra) and remains heuristic at the untwisted scope.
\item \textbf{(Movement~X, algebraic.)} Heptagon coherence at order
 $\hbar^3$ with pentagon coboundary
 ...
 is proved in the seven-colour $\mathsf{SC}^{\mathrm{ch,top}}$
 heptagon; its physical reading as ``gravitational $E_8$ anomaly
 cancellation'' is the Green--Schwarz mechanism for the
 $\mathfrak{g}_{\Delta_5}$-paramodular anomaly at the twisted-holography
 scope. The chain-level heptagon coherence is the proved statement;
 the Green--Schwarz reading is a physical interpretation at the scope
 fixed by Costello--Gaiotto--Paquette~\cite{CostelloGaiottoPaquette2018}.
\end{enumerate}
Primary literature: Witten $2007$~\cite{Witten2007}
($3$d gravity boundary-CFT datum, not a dynamical-metric construction);
Maloney--Witten $2009$ (formal sum over smooth Euclidean
$\mathrm{AdS}_3$ saddles, not equal to $\Phi_{10}^{\mathrm{un}}$ on the nose);
Maxfield--Turiaci $2020$ (wormhole contributions at the boundary-CFT
level); Dijkgraaf--Moore--Verlinde--Verlinde
$1997$~\cite{DijkgraafMooreVerlindeVerlinde1997} (symmetric-product
orbifold BPS degeneracies, distinct object from the $3$d gravity
partition function but sharing the automorphic $1/\Phi_{10}^{\mathrm{un}}$ via
the twisted-holography dictionary of
Costello--Gaiotto--Paquette $2018$~\cite{CostelloGaiottoPaquette2018}).
Compare Vol.~III,
