...
to $\kappa_{\mathrm{eff}} = (H_N - 1)\,c - \alpha_N/2$, where
$H_N = \sum_{j=1}^N 1/j$ is the harmonic number:
the matter curvature $(H_N - 1)\,c$ sums spin-$s$ contributions
$c/s$ for $s = 2, \ldots, N$, and $-\alpha_N/2$ is the total
ghost curvature.
Matter-ghost cancellation occurs at
$c_{\mathrm{crit}} = \alpha_N/\!\left(2(H_N - 1)\right)$,
the \emph{critical string} point --- distinct from the
self-dual point $c^* = \alpha_N/2$.
For Virasoro, $c_{\mathrm{crit}} = 26$
(the zero of $\kappa_{\mathrm{eff}} = (c - 26)/2$);
for $\mathcal{W}_3$, $c_{\mathrm{crit}} = 60$
(the zero of $\kappa_{\mathrm{eff}} = 5(c - 60)/6$).
