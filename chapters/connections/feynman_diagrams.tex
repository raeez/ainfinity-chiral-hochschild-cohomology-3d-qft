...
\begin{remark}[Logarithmic Feynman--bar comparison; \ClaimStatusHeuristic]
...
operations satisfy the recursive constraint:
\[
m_1 \circ m_n =
-\sum_{\substack{r+s+t=n,\ s\geq 1\\ (r,s,t) \neq (0,n,0)}}
(-1)^{rs+t} m_{r+1+t}(\mathrm{id}^{\otimes r} \otimes m_s \otimes
\mathrm{id}^{\otimes t}).
\]
...
\begin{proof}
This is a rewriting of the uncurved $A_\infty$ relations (Keller convention):
\[\sum_{\substack{r+s+t=n\\s\geq1}} (-1)^{rs+t} m_{r+1+t}(\mathrm{id}^{\otimes r} \otimes m_s \otimes
...
\begin{theorem}[Kontsevich formality; \ClaimStatusProvedElsewhere{} \cite{Kon03}]
...
regularized BRST-anomaly brackets of Gaiotto--Kulp--Wu~\cite{GKW25}
...
\begin{proof}
On the perturbative side, Gaiotto--Kulp--Wu~\cite{GKW25} prove that the
...
\begin{proposition}[Two-channel reduction after compactifying the
 ternary discrepancy; \ClaimStatusProvedHere]
...
\begin{corollary}[Standard-chart form of the genus-\texorpdfstring{$0$}{0}
 ternary discrepancy; \ClaimStatusConditional]
...
\ClaimStatusProvedElsewhere{} \cite{costello-renormalization}]
