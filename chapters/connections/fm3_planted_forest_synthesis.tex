...
The subject rests on a single piece of structure: a chiral algebra
...
D^2=0.\footnotemark
\]
\footnotetext{At both the convolution and ambient levels; the ambient identity is proved in Volume~I via Mok's log-FM normal-crossings theorem.}%
...
algebra (strict model) indexed by those forests.  Compressing this semicosimplicial object to a
...
&=
\left([
\alpha,\alpha'],\ \frac12\bigl([\beta,\chi(\alpha')] - (-1)^{|\alpha||\alpha'|}[\beta',\chi(\alpha)]\bigr)\right),\\
\label{eq:syn-l3}
l_3((0,\beta_1),(0,\beta_2),(\alpha,0))
&=
\frac1{12}\Bigl(0,
[\beta_1,[\beta_2,\chi(\alpha)]]+(-1)^{|\beta_1||\beta_2|}[\beta_2,[\beta_1,\chi(\alpha)]]\Bigr),
\end{align}
...
Equation~\eqref{eq:pf-cubic-overlap-synthesis} has no
...
The logical structure simplifies.
