...
(Theorem~\ref*{V1-thm:verdier-bar-cobar}) is the chain-level mechanism
from which gravitational $S$-duality descends: Verdier duality sends
the bar coalgebra to the homotopy Koszul dual \emph{algebra}~$\cA^!_\infty$,
which on the Koszul locus (e.g.\ Virasoro) is formal and equivalent
to~$\cA^!$.  This section derives
$S$-duality as a formal consequence of that identification together with
the bar-intrinsic construction of the universal MC
element~$\Theta_\cA$ (Theorem~\ref*{V1-thm:mc2-bar-intrinsic}).
...
% label removed: lem:thqg-IV-verdier-graph-composition
...
% label removed: def:thqg-IV-verdier-convolution
...
% label removed: prop:thqg-IV-verdier-dg-lie
...
% label removed: thm:thqg-IV-theta-duality
\index{S-duality@$S$-duality!gravitational|textbf}
...
$\Ran(X)$ sends the bar coalgebra $\barB_X(\cA)$ to the homotopy
Koszul dual algebra $\cA^!_\infty$, intertwining the total
differentials:
...
sends $\barB_X(\cA)$ to the homotopy Koszul dual algebra
$\cA^!_\infty$, whose underlying $\cD$-module complex is equivalent
to $\barB_X(\cA^!)$
...
% label removed: prop:thqg-IV-fundamental-diagram
The diagram
...
% label removed: cor:thqg-IV-twisted-tangent
...
% label removed: thm:thqg-IV-shadow-duality
...
% label removed: prop:thqg-IV-kappa-duality
...
% label removed: cor:thqg-IV-shadow-depth
...
\hline
$\mathsf{G}$ (Gaussian) & $2$ &
$\cH_\kappa \leftrightarrow \cH_{-\kappa}$ \\
$\mathsf{L}$ (Lie/tree) & $3$ &
...
on the conformal block bundle over configuration space.  The Verdier
...
\begin{theorem}[KZ duality; \ClaimStatusProvedHere]
% label removed: thm:thqg-IV-kz-duality
...
\sigma(\nabla^{\mathrm{sh}}_\cA) = \nabla^{\mathrm{sh}}_{\cA^!}.
\end{equation}
For affine $\widehat{\fg}_k$, this is the Feigin--Frenkel involution
...
For affine $\widehat{\fg}_k$: the collision residue gives the KZ
connection with coupling $1/(k + h^\vee)$.  Under
$k \mapsto k' = -k - 2h^\vee$:
$1/(k' + h^\vee) = 1/(-k - h^\vee) = -1/(k + h^\vee)$.
Therefore
$\nabla_{\mathrm{KZ}}^{(k')} = d + \frac{1}{k+h^\vee}
\sum_{i<j}\Omega_{ij}\,d\log(z_i - z_j)$,
which is the KZ connection with negated coupling, the Feigin--Frenkel
dual.
\end{proof}
...
% label removed: prop:thqg-IV-bpz-duality
...
% label removed: prop:thqg-IV-monodromy-duality
...
% label removed: subsec:thqg-IV-complementarity-constant
\index{complementarity!constant $K(\cA)$|textbf}

\begin{definition}[Complementarity constant]
% label removed: def:thqg-IV-complementarity-constant
...
\qquad
\kappa(\cH_\kappa^!) = \kappa(\cH_{-\kappa}) = -\kappa,
\qquad
K(\cH_\kappa) = 0.
\end{equation}
\end{proposition}
...
% label removed: thm:thqg-IV-km-K
...
% label removed: prop:thqg-IV-bc-bg-K
...
% label removed: thm:thqg-IV-virasoro-K
For $\mathrm{Vir}_c$:
...
$c(k) = (N-1)(1 - N(N+1)/(k+N))$,
and the Feigin--Frenkel involution $k \mapsto k' = -k - 2N$ gives
$c' = c(k') = (N-1)(1 - N(N+1)/(k'+N))
= (N-1)(1 + N(N+1)/(k+N))$.  Therefore:
\begin{align*}
c + c' &= (N-1)\bigl(2 - N(N+1)/(k+N) + N(N+1)/(k+N)\bigr) \\
&\quad + (N-1) \cdot 2N(N+1)/(k+N) \cdot \text{(correction)}.
\end{align*}
Direct computation:
$c + c' = 2(N-1) + 2(N-1)N(N+1) \cdot 0$; this is level-independent
by the cancellation of the $1/(k+N)$ terms.  The exact value is
$K_N = 2(N-1)(2N^2 + 2N + 1)$
(Theorem~\ref{thm:central-charge-complementarity}).
...
\begin{proof}
$K_N = 2(N-1)(2N^2+2N+1) = 4N^3 + 2N^2 - 2N - 2$.
...
\item $\mathrm{Vir}_{26}^! = \mathrm{Vir}_0$: the dual is uncurved.
% RECTIFICATION-FLAG (AP20): Theta_{Vir_0} ≠ 0 (the OPE still has quadratic+simple poles;
% "uncurved" means d_B^2=0, not Theta=0). The arity-2 SCALAR shadow kappa=0 but the full MC
% element has nontrivial higher-arity components (class M, r_max = infinity).
\item $\kappa(\mathrm{Vir}_0) = 0$: the bar complex is uncurved ($d_B^2 = 0$),
  but the full MC element $\Theta_{\mathrm{Vir}_0}$ retains nontrivial
  higher-arity components (the Witt algebra is class~M with $r_{\max} = \infty$).
\item The scalar shadow free energy $F_g(\mathrm{Vir}_0) = \kappa(\mathrm{Vir}_0) \cdot \lambda_g^{FP} = 0$
  for all $g \geq 1$.
\item After ghost subtraction, the combined system
  $\mathrm{Vir}_{26} \otimes bc_2$ has $\kappa_{\mathrm{total}} = 0$:
  the BRST differential squares to zero at all genera
  without period corrections.
\end{enumerate}
\end{theorem}
...
(c)~$\kappa(\mathrm{Vir}_0) = 0$, so $\Theta_{\mathrm{Vir}_0} = D - \dzero = 0$:
the bar complex is uncurved, $d_B^2 = 0$ at all genera.

(d)~$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}} = 0$.

(e)~$\kappa(\mathrm{Vir}_{26}) = 13$ and $\kappa(bc_2) = -13$
(Proposition~\ref{prop:thqg-IV-bc-bg-K} at $\lambda = 2$).
By additivity: $\kappa_{\mathrm{total}} = 0$.  Therefore
$d_{\mathrm{BRST}}^2 = \kappa_{\mathrm{total}} \cdot \omega_g = 0$.
\end{proof}
...
and $\kappa = 0$).  At $c = 0$, the shadow datum is trivial:
$\Theta = 0$ and both sides of $\sigma(\Theta) = \Theta$ vanish,
but this is not the Koszul self-duality of the algebra.
At $c = 13$, the self-duality is nontrivial: the shadow tower is
genuinely self-dual with nonzero data at every arity.
\end{remark}
...
% label removed: thm:thqg-IV-critical-degeneration
\index{critical level!degeneration|textbf}
At $k = -h^\vee$:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\kappa(\widehat{\fg}_{-h^\vee}) = 0$.
\item $\Theta_{\widehat{\fg}_{-h^\vee}} = 0$.
\item $D_{\widehat{\fg}_{-h^\vee}} = \dzero$
  (the bar complex is uncurved).
...
(b)~$\Theta = D - \dzero = 0$ since $D = \dzero$ when $\kappa = 0$:
the period corrections vanish because the curvature that triggers
them is zero.
(c)~Immediate from (b).
...
$Z(\widehat{\fg}_{-h^\vee}) \cong \mathrm{Fun}(\mathrm{Op}_{\fg^\vee}(X))$,
the algebra of functions on the space of ${}^L\fg$-opers.
\end{proof}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item Verdier duality: $\VD(\dzero) = \dzero$ (the only content).
\item Feigin--Frenkel: $\kappa = 0 = \kappa'$.
\item Level-rank: $K = 0$ (a fortiori).
\item Complementarity: $\mathbf{C}_g(\cA) \simeq \mathbf{Q}_g(\cA)$
  (the dual contribution vanishes).
\end{enumerate}
...
% label removed: prop:thqg-IV-self-dual-classification
...
% label removed: thm:thqg-IV-four-facets
...
\begin{theorem}[Free energy complementarity; \ClaimStatusProvedHere]
% label removed: thm:thqg-IV-free-energy
...
(Theorem~\ref{thm:genus-universality}).  Adding the two contributions:
$F_g(\cA) + F_g(\cA^!) = (\kappa + \kappa') \lambda_g^{\mathrm{FP}}
= K \lambda_g^{\mathrm{FP}}$.
The generating function identity follows from
$\phi_\cA(x) = \sum_{g \geq 1} F_g(\cA)\, x^{2g}
= \kappa \cdot (\hat{A}(ix) - 1)$.
\end{proof}

\begin{corollary}[Self-dual generating function; \ClaimStatusProvedHere]
% label removed: cor:thqg-IV-self-dual-generating
At the self-dual point $\cA \simeq \cA^!$:
$F_g(\cA) = (K/2) \cdot \lambda_g^{\mathrm{FP}}$.
\end{corollary}

\begin{proposition}[Partition function functional equation; \ClaimStatusProvedHere]
% label removed: prop:thqg-IV-partition-duality
...
\end{equation}
For $K = 0$: $Z_g(\cA^!) = Z_g(\cA)^{-1}$ (duality inverts the
partition function).
For $K \neq 0$: $Z_g(\cA) \cdot Z_g(\cA^!)$ is a universal constant
depending only on $K$ and~$g$, not on the specific algebra within the
family.
\end{proposition}

\begin{proof}
Exponentiate $F_g + F_g' = K \lambda_g^{\mathrm{FP}}$.
\end{proof}
...
$Z_g(\mathrm{Vir}_c) \cdot Z_g(\mathrm{Vir}_{26-c})
= \exp(13 \lambda_g^{\mathrm{FP}})$.
At genus~$1$: $\lambda_1^{\mathrm{FP}} = 1/24$, so
$Z_1 \cdot Z_1' = e^{13/24}$.
At genus~$2$: $\lambda_2^{\mathrm{FP}} = 7/5760$, so
$Z_2 \cdot Z_2' = e^{91/5760} = e^{7/443.08\ldots}$.

At the self-dual point $c = 13$:
$Z_g(\mathrm{Vir}_{13})^2 = \exp(13 \lambda_g^{\mathrm{FP}})$,
i.e., $Z_g(\mathrm{Vir}_{13}) = \exp(13 \lambda_g^{\mathrm{FP}}/2)$.

At $c = 26$: $Z_g(\mathrm{Vir}_{26}) \cdot Z_g(\mathrm{Vir}_0) = e^{13 \lambda_g^{\mathrm{FP}}}$.
Since $Z_g(\mathrm{Vir}_0) = 1$ (uncurved):
$Z_g(\mathrm{Vir}_{26}) = e^{13 \lambda_g^{\mathrm{FP}}}$.
\end{computation}
...
% label removed: prop:thqg-IV-holographic-datum
...
% label removed: prop:thqg-IV-clutching
...
\hline
$\cH_\kappa$ & $\kappa$ & $\cH_{-\kappa}$ & $-\kappa$ & $0$ &
$\kappa \mapsto -\kappa$ \\
\hline
$\widehat{\fg}_k$ &
...
\end{align}
The total curvature vanishes.  At genus~$g$:
$F_g^{\mathrm{matter}} + F_g^{\mathrm{ghost}} = 0$:
the combined system is uncurved at all genera.
...
% label removed: rem:thqg-IV-no-ghost
...
At $c = 26$, $\kappa_{\mathrm{total}} = 0$, so $Q^2 = 0$ at all
genera without curvature corrections.  The positivity of the physical
Hilbert space (the no-ghost theorem) follows from the Lagrangian
structure of the complementarity decomposition
(Theorem~\ref{thm:shifted-symplectic-complementarity}): the
physical space is the intersection of two transverse Lagrangians,
hence inherits a nondegenerate pairing.
\end{remark}
...
\[
c(k) = (N-1)\left(1 - \frac{N(N+1)}{k+N}\right).
\]
Under $k \mapsto k' = -k - 2N$: $k' + N = -(k+N)$, so
$c' = (N-1)(1 + N(N+1)/(k+N))$.  Adding:
\[
c + c' = 2(N-1) + 0 = 2(N-1),
\]
but this is wrong; the DS formula has additional terms.
The complete $\mathcal{W}_N$ central charge is
$c(k) = (N-1) - N(N^2-1)(N+1)/(k+N) + N(N^2-1)/(k+N)$; more
precisely:
\[
c(k) = (N-1)\left(1 - \frac{N(N+1)}{t}\right), \qquad t = k + N.
\]
Then $c + c' = 2(N-1) - (N-1)N(N+1)(1/t + 1/(-t)) = 2(N-1)$.
This gives $c + c' = 2(N-1)$, but the known value is
$2(N-1)(2N^2+2N+1)$.

The discrepancy is resolved by using the full DS central charge formula
including the ghost contribution from the BRST reduction:
\[
c_{\mathrm{DS}}(k) = c_{\mathrm{Sug}}(k) - c_{\mathrm{DS-ghost}} - \dim(\fn_+),
\]
where $\fn_+$ is the nilpotent radical.  The complete formula
