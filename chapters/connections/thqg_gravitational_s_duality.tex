...
(Theorem~\ref{thm:verdier-bar-cobar}) is the chain-level mechanism
...
to $\barB_X(\cA^!)$
(Theorem~\ref{thm:verdier-bar-cobar}); hence it interchanges the
...
The Koszul dual is $\cH_\kappa^! = \cH_{-\kappa}$: Verdier duality
...
\item $\Theta_{\mathrm{Vir}_0} = 0$.
\item The genus tower of $\mathrm{Vir}_0$ vanishes identically:
...
\begin{remark}[Self-duality at $c = 13$ vs.\ uncurved self-duality at $c = 0$]
% label removed: rem:thqg-IV-c13-vs-c0
\index{Virasoro!self-duality comparison}
The Virasoro family has two self-dual points:
$c = 13$ (curved, complex level, $\kappa = 13/2$) and
$c = 0$ (uncurved, real level $k = -2$, $\kappa = 0$).
At $c = 0$, the self-duality is trivial: $\Theta = 0$ and both sides
of $\sigma(\Theta) = \Theta$ are zero.
...
$\phi_\cA(x) = \sum_{g \geq 1} F_g(\cA)\, x^{2g-2}
