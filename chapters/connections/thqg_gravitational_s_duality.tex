...
This section derives the algebraic gravitational $S$-duality of the
chiral Koszul--Verdier surface as a formal consequence of that
identification together with the bar-intrinsic construction of
$\Theta_\cA$ (Theorem~\ref*{V1-thm:mc2-bar-intrinsic}).
...
\begin{theorem}[Algebraic gravitational $S$-duality; \ClaimStatusProvedHere]
...
\sigma(\kappaChHodge(\cA) \cdot \eta \otimes \Lambda)
= \kappaChHodge(\cA^!) \cdot \eta \otimes \Lambda.
\end{equation}
\end{proposition}
...
characteristic:
$\Theta_\cA^{(1,2)} = \kappaChHodge(\cA) \cdot \eta \otimes \Lambda$,
...
The deformation complex component $\VD_{\Ran}(\kappaChHodge(\cA) \cdot \eta)$
...
$\kappaChHodge(\cA)$ is replaced by $\kappaChHodge(\cA^!)$
...
\begin{tabular}{lcccc}
\toprule
\textbf{Algebra} & $\kappaChHodge(\cA)$ & $\kappaChHodge(\cA^!)$ &
...
For affine Kac--Moody, the class-$\mathbf L$ cubic term is the Lie
Jacobi transfer, not an associative defect.  Its obstruction class
vanishes by the Lie conformal Jacobi identity and the invariance of the
level form.  Hence no Virasoro scalar cubic shadow is attached to
$\widehat{\fg}_k$, and the scalar cubic complementarity has zero affine
summand.
\end{computation}
...
\end{equation}
the chiral Koszul target is
\[
\widehat{\fg}_k^!
\;\simeq\;
\CE^{\mathrm{ch}}\!\bigl(\widehat{\fg}_{k'}\bigr),
\qquad
k' = -k - 2h^\vee,
\]
and the reflected current presentation satisfies
...
Combining: $\kappaChHodge(\widehat{\fg}_k)
= kd/(2h^\vee) + d/2 = (k + h^\vee)d/(2h^\vee)$.

Under $k \mapsto k' = -k - 2h^\vee$:
$k' + h^\vee = -k - 2h^\vee + h^\vee = -(k + h^\vee)$, so
$\kappaChHodge(\widehat{\fg}_{k'})
= (k'+h^\vee)d/(2h^\vee)
= -(k + h^\vee)d/(2h^\vee) = -\kappaChHodge(\widehat{\fg}_k)$.
Therefore the scalar current shadow has
$K = \kappaChHodge(\widehat{\fg}_k) + \kappaChHodge(\widehat{\fg}_{k'})
...
\toprule
$\fg$ & $d$ & $h^\vee$ & $\kappaChHodge(\widehat{\fg}_k)$ & $k'$ &
$\kappaChHodge(\widehat{\fg}_{k'})$ & $K$ \\
\midrule
...
At the standard ghost values $\lambda = 2$: $\kappaChHodge(bc_2) = -13$,
$\kappaChHodge(\beta\gamma_2) = 13$.
At $\lambda = 1$: $\kappaChHodge(bc_1) = -1$, $\kappaChHodge(\beta\gamma_1) = 1$.
\end{proof}
...
$\kappaChHodge(V_\Lambda) = r$, $\kappaChHodge(V_\Lambda^!) = -r$, $K = 0$.
\end{proposition}
...
$\kappaChHodge(V_\Lambda) = r \cdot \kappaChHodge(\cH_1) = r$.
The Koszul dual negates each Heisenberg factor: $\kappaChHodge(V_\Lambda^!) = -r$.
\end{proof}
...
The genus-$1$ curvature is $\kappaChHodge(\mathrm{Vir}_c) = c/2$
...
\kappaChHodge(\mathcal{W}_N^k) = c(k) \cdot (H_N - 1),
\end{equation}
...
$\kappaChHodge(\mathcal{W}_N^k) = c(k) \cdot \varrho(\fsl_N) = c(k)(H_N - 1)$.
...
$K(\mathrm{BP}) = 50$, $K(V_k(\fg)) = 2\dim\fg$, $K(\cW_N) = 4N^3-2N-2$),
...
At $c = 26$:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\kappa_{\mathrm{eff}} = \kappaChHodge(\mathrm{Vir}_{26}) - 13 = 0$.
\item $\mathrm{Vir}_{26}^! = \mathrm{Vir}_0$.
\item $\kappaChHodge(\mathrm{Vir}_0) = 0$: the bar complex is uncurved
...
$\kappa_{\mathrm{eff}} = \kappaChHodge(\mathrm{Vir}_c) + \kappaChHodge(bc_2)
...
(c)~$\kappaChHodge(\mathrm{Vir}_0) = 0$, so the scalar curvature term
...
$F_1(\mathrm{Vir}_0) = \kappaChHodge(\mathrm{Vir}_0)/24 = 0$.

(e)~$\kappaChHodge(\mathrm{Vir}_{26}) = 13$ and $\kappaChHodge(bc_2) = -13$
...
$\kappaChHodge(\mathrm{Vir}_c) = \kappaChHodge(\beta\gamma_2)$, hence
...
\item $\mathrm{Vir}_{13}^! = \mathrm{Vir}_{13}$: the algebra is
 self-dual.
\item $\kappaChHodge(\mathrm{Vir}_{13}) = 13/2$: the bar complex is curved.
...
At $k = -h^\vee$:
\begin{enumerate}[label=\textup{(\alph*)}]
\item $\kappaChHodge(\widehat{\fg}_{-h^\vee}) = 0$.
...
\subsection{Algebraic $S$-duality as exact symmetry: the all-genera theorem}
...
\item \textbf{Feigin--Frenkel duality} (genus~$1$, degree~$2$):
 $\sigma(\kappaChHodge(\cA)) = \kappaChHodge(\cA^!)$.
...
complementarity theorem proved in Volume~I (chapter on higher-genus
...
$\kappaChHodge(\cA) \cdot \eta \otimes \Lambda$.
Applying $\sigma$ gives $\kappaChHodge(\cA^!) \cdot \eta' \otimes \Lambda$
...
$F_1^{\mathrm{sc}}(\cA) = \kappaChHodge(\cA)/24$ and
$F_1^{\mathrm{sc}}(\cA^!) = \kappaChHodge(\cA^!)/24$.
Adding and using $\kappaChHodge(\cA) + \kappaChHodge(\cA^!) = K(\cA)$ yields the
stated genus-$1$ formula.

Part~(b): on the scalar lane,
Theorem~\ref{V1-thm:genus-universality} gives
$F_g^{\mathrm{sc}}(\cA)=\kappaChHodge(\cA)\lambda_g^{\mathrm{FP}}$ and
...
The differences:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The algebraic gravitational $S$-duality constructed in this
 chapter is exact on the chiral Koszul--Verdier surface:
 $\sigma(\Theta_\cA)=\Theta_{\cA^!}$ is Theorem~\ref{thm:thqg-IV-theta-duality}.
 This is not a Montonen--Olive-style physical duality conjecture; real
 forms, analytic sewing, and dynamical-metric interpretations are
 separate inputs.
...
(b)~$K(\cA^!) = \kappaChHodge(\cA^!) + \kappaChHodge((\cA^!)^!) = \kappaChHodge(\cA^!) + \kappaChHodge(\cA) = K(\cA)$.
...
\hline
\textbf{Algebra $\cA$} & $\kappaChHodge(\cA)$ & \textbf{Dual $\cA^!$} &
$\kappaChHodge(\cA^!)$ & $K$ & \textbf{Type} \\
\hline
...
\hline
$\mathcal{W}_3^c$ & $5c/6$ & companion $\mathcal{W}_3^{100-c}$ &
...
\begin{align}
\kappa_{\mathrm{matter}} &= \kappaChHodge(\mathrm{Vir}_{26})
= 26/2 = 13, % label removed: eq:thqg-IV-bosonic-matter\\
\kappa_{\mathrm{ghost}} &= \kappaChHodge(bc_2)
...
definition of algebraic gravitational $S$-duality in this framework. The
...
\section{K3 \texorpdfstring{$T^2$}{T2} parameter and Verdier duality}
\label{sec:grav-sdual-supplement}

\begin{proposition}[$K3\times T^2$ $S$-parameter compatibility;
\ClaimStatusProvedHere]
\label{rem:grav-sdual-1}
Let $\pi\colon\mathcal H\to\mathfrak H_1$ be a family of chiral
bialgebras assigned to a $K3\times T^2$ frame, equipped with an
$\mathrm{SL}_2(\mathbb Z)$-equivariant structure
\[
\rho\colon \mathrm{SL}_2(\mathbb Z)\longrightarrow
\operatorname{Aut}(\mathcal H)
\]
and fibrewise chiral Koszul pairs
$(\mathcal H_\tau,\mathcal H_\tau^!)$. Suppose the generator
$S=\begin{psmallmatrix}0&-1\\1&0\end{psmallmatrix}$ satisfies the
Verdier-intertwining condition
\[
\rho(S)_\tau(\Theta_{\mathcal H_\tau})
\;=\;
\Theta_{\mathcal H^!_{-1/\tau}}.
\]
Then $\rho(S)$ realizes the gravitational $S$-duality functor of this
chapter on the $K3\times T^2$ family. Moreover
\[
\rho(S)^2=\rho(-I);
\]
hence the induced duality is a $\mathbb Z/2$-involution on any quotient
where $-I$ acts trivially, and it has an order-four lift at the fixed
elliptic point $\tau=i$ when $\rho(-I)$ acts as the nontrivial central
sign.
\end{proposition}

\begin{proof}
The first assertion is Theorem~\ref{thm:thqg-IV-theta-duality} applied
fibrewise: the displayed Verdier-intertwining condition is exactly the
identity $\sigma(\Theta_\cA)=\Theta_{\cA^!}$ with
$\cA=\mathcal H_\tau$ and $\sigma=\rho(S)_\tau$. The group-theoretic
part is the defining relation
$S^2=-I$ in $\mathrm{SL}_2(\mathbb Z)$. Applying $\rho$ gives
$\rho(S)^2=\rho(-I)$. At $\tau=i$, the modular transformation $S$
stabilizes the base point, so its lift acts as an automorphism of the
single fibre; the square is central, giving order two or four according
to the action of $\rho(-I)$.
\end{proof}

\begin{remark}[Automorphic realization]
An automorphic model based on the Andrianov spinor factorization
$L(s,\Phi_{10})=L(s,\Delta\otimes E_6)\zeta(s-9)\zeta(s-8)$ supplies
one source of the equivariant structure~$\rho$ when the corresponding
chiral-bialgebra family has been constructed. The proposition above is
the formal Verdier consequence of that equivariant input. Scalar
partition-function identities remain the functional equations of
Proposition~\ref{prop:thqg-IV-partition-duality}.
\end{remark}
