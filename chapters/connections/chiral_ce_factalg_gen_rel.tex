...
Subsection~\S\ref{subsec:costello-e3-chiral-ce-gen-rel}
establishes the skeleton: the
...
develops the full generators-and-relations presentation: explicit tridegree conventions,
...
factorisation-homology Fubini descent from the transverse $\Disk_3$
sector to the chiral CE algebra on $C$. Every coefficient in every OPE is tracked to its
...
the factorisation-homology Fubini pushforward to the chiral CE algebra
on $C$ via Ayala--Francis--Tanaka
...
\eqref{gen:ch-dA}--\eqref{gen:ch-dB} are the Taylor modes of
...
reference curve $C$;
\item the chiral coefficient system is topologised by the non-critical
Sugawara vector together with the antighost primitive of
...
\Disk_3^{\mathrm{red}}
\;=\;
\Disk_1^{R_1}\otimes_D \Disk_1^{R_2}\otimes_D \Disk_1^{R_3}
\end{equation}
via Dunn additivity, where the three Dunn factors correspond to
the three real topological directions in $\R^3$. The holomorphic
directions enter the coefficient system after Dolbeault pushforward
and BRST topologisation; they are not counted as real dimensions of
the transverse $\Disk_3$.
...
after the stated reductions. The three $\R^3$ directions are already
...
\begin{theorem}[Fubini comparison to the chiral CE algebra on $C$]
...
$(\R^3\times\C^2)$-direction produces a factorisation algebra on $C$
...
\begin{theorem}[Topologisation of the chiral coefficient system at
generic bulk]
\label{thm:ch-ce-topologization-E3-top}
\index{topologisation!chiral CE}%
\ClaimStatusConditional{} on the existence of the HT antighost
primitive.
The chiral coefficient system on $C$ is holomorphic before
topologisation and is not one of the three Dunn factors in
\eqref{eq:disk3-decomp}. At non-critical level $k$, the Sugawara
construction on $\widehat\fg_k$ gives a conformal vector
$T(z)\in\cF^{\mathrm{ch}}_{\fg,M;C}$. If the HT BV complex contains an
antighost zero mode $G_0$ with
\[
[Q_{\mathrm{tot}},G_0]=L_{-1},\qquad L_{-1}=\Res_{z=0}T(z),
\]
then holomorphic translation on $C$ is $Q_{\mathrm{tot}}$-exact, and
the coefficient system becomes topological on
...
holomorphic translation $\partial_z$ on local sections. The stated
antighost identity makes this translation a $Q_{\mathrm{tot}}$-
coboundary. Hence the coefficient factorisation algebra is locally
constant along $C$ after passing to $Q_{\mathrm{tot}}$-cohomology. The
transverse $\Disk_3$ structure remains the Dunn product of the three
real directions in \eqref{eq:disk3-decomp}.
\end{proof}
...
motivic coefficient of the leading subleading correction, the same
rational period $-1/24$ that the second Bernoulli number contributes to
the Todd class of $\P^1$.
\end{theorem}
...
Fix the Costello renormalisation scheme and the Bergman propagator.
The coefficient is the renormalised BV Feynman-rule residue
\cite[\S11]{CostelloRenormalization}.  It is computed by the following
...
\end{enumerate}
The coefficient $\zeta(2)/(2\pi i)^2 = (\pi^2/6)/(-4\pi^2) = -1/24$ is
the motivic representative of the weight-two period attached to this
pole (Brown 2014 motivic-MZV decomposition). The relation is
chain-level rigorous; the right-hand side is the single representative
of the $(z-w)^{-1}$ pole in the motivic basis.
\end{proof}
...
-\delta^{be}\delta^{cf}$, then again contracting the remaining pair of
$\epsilon$-symbols. Direct scalar evaluation of the triple
...
weight on the Bruinier--Heegner locus. At
$K^{\kappaChHodge} = 8$
...
the K3 Heisenberg witness $K^{\kappaChHodge}=8$), this identity forces
...
the Feynman-integral route. Three non-overlapping sources concur on
$\hbar^2 = -1/8$: the diagrammatic bare residue $-1/216$, the motivic
identity $\hbar^2 = -1/8$, and the Chern-class reciprocity
$K^{\kappaChHodge} = 8$. Matching the diagrammatic side to
...
$\hbar^2 K^{\kappaChHodge} = -1$ at $K^{\kappaChHodge} = 8$ forces
...
\(K^{\kappaChHodge}=2c_+(\mathrm{Mukai}(K3))=8\), supplying the
$K^{\kappaChHodge}$ input to~(2) from a non-Feynman,
non-motivic route.
\end{enumerate}
...
weight at most $n+1$ (Brown 2014). The two-loop coefficient is governed
by the Bruinier--Etingof identity on the $\mathsf{B}$-family at fixed
exponent two,
\[
\hbar^2\cdot K^{\kappaChHodge} = -1,
\qquad K^{\kappaChHodge} = 2c_+(L),
\]
where $L$ is the defining Lorentzian lattice of the relevant
$\mathsf{B}$-family witness. The K3 Mukai lattice
$L = \mathrm{II}_{4,20}$ gives $K^{\kappaChHodge} = 2c_+ = 8$ and
$\hbar^2 = -1/8$, the value computed diagrammatically above. The same
identity reads $\hbar^2 = -1/2$ on the Monster witness
($L = \mathrm{II}_{1,1}$, $K^{\kappaChHodge} = 2$) and
$\hbar^2 = -1/50$ on the Fake Monster witness
($L = \mathrm{II}_{25,1}$, $\Phi_{12}$ denominator,
$K^{\kappaChHodge} = 50$); these are distinct lattice members at the
two-loop level, not higher loop orders.
\end{remark}
...
and the finite Hall--Borcherds recognition package supplies the compact
Hall source, PBW/no-extra-relations, parity/root fixture, radical
quotient, completed double, and Heegner comparison, then factorisation
homology over \(E\) identifies with the recognised K3 target
$\mathbf H_{\Delta_5}$:
\begin{equation}
...
comparison with the BKM presentation. Under the additional
Hall--Borcherds recognition hypotheses in the statement, the
factorisation-homology integral combines the Dolbeault Fubini on
$E\setminus\{p_i\}$ with the Sym-part-CE closure and identifies with the
recognised target $\mathbf H_{\Delta_5}$ by
...
\item\label{id:14}
$[Q_{\mathrm{tot}},G_0]=L_{-1}$ on the HT BV complex
(topologisation antighost witness;
Theorem~\ref{thm:ch-ce-topologization-E3-top}).
\item\label{id:15}
$\hbar^2\cdot K^{\kappaChHodge} = -1$ under the
Bruinier--Heegner normalisation of the $\mathsf{B}$-family witness,
fixing $\hbar^2 = -1/8$ at the K3 value $K^{\kappaChHodge} = 8$
