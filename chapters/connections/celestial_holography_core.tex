...
The disk-level story is already visible in the existing literature.
...
\begin{theorem}[Celestial gluon OPE from the bar complex; \ClaimStatusHeuristic]
...
\begin{theorem}[Celestial graviton OPE from the bar complex; \ClaimStatusHeuristic]
...
The celestial OPE computations above exhibit a fundamental structural constraint: the bar complex sees only one helicity.  The following theorem makes precise the structural observation of the Introduction (Observation~\ref{obs:helicity-directionality}): the two colours of $\SCchtop$ are the two helicities, and the no-open-to-closed rule is the helicity selection rule.  What was visible from the operad structure alone is now realised concretely in the celestial OPE.
...
architecture developed in Part~\ref{part:ordered}: the ordered
bar complex $\Barch^{\mathrm{ord}}(\cA)$ carries the colour-ordered
...
\begin{remark}[$\mathfrak{w}_{1+\infty}$ shadow tower and celestial genus expansion]
