...
\section{Yang--Mills boundary-to-infrared synthesis}

\begin{summary}[Boundary-to-infrared chain]
The synthesis is the following chain of reductions and bridge data:
\[
...
\longrightarrow
\text{mass-gap theorem under bridge data}.
\]
The arrows through the screening Laplacian are algebraic or Hilbert-theoretic constructions below.
The last arrow is a theorem for an untwisting bridge datum whose Hamiltonian satisfies the stated
quadratic-form domination estimate.
\end{summary}

\begin{theorem}[Grand synthesis bridge theorem]
...
\item every untwisting bridge datum whose screening operators satisfy
\[
\sum_i S_i^*S_i\ge \mu^2(1-P_0)
\]
and whose Yang--Mills Hamiltonian satisfies
\[
H_{\mathrm{YM}}\ge \alpha L_{\mathrm{scr}}
\]
on the vacuum complement has
\[
...
Item~(iv) is the screening Hodge theorem together with the mass-gap transfer theorem for bridge data.
\end{proof}

\begin{remark}[Boundary and bridge data]
The synthesis separates three kinds of data.
\begin{enumerate}[label=(\alph*)]
\item \emph{Algebraic core:} boundary BRST recovery, tangent-to-center, central formality,
...
\item \emph{Defect-realization data:} a functor from the localized dual-module category to the
physical line-operator category, identifying the vacuum object and carrying screened modules to invisible sectors.
\item \emph{Untwisting domination data:} a Hilbert-screening datum, a closed realization of the
gauge-invariant Hilbert space, a nonnegative self-adjoint Hamiltonian $H_{\mathrm{YM}}$, and positive
constants $\alpha,\mu$ satisfying the screening gap and domination inequalities.
\end{enumerate}
\end{remark}
...
\begin{summary}[Boundary algebra input]
Holomorphic-topological Yang--Mills theory enters the bar-cobar framework through a
Koszul-admissible boundary chiral algebra.
...
\item \emph{AGT/holography outputs.} Treat AGT and twisted holography as downstream comparison
maps out of the same boundary package.
\end{enumerate}
\end{summary}
...
\begin{summary}[Instanton and screening data]
The preceding boundary packages organize perturbative and twisted Yang--Mills sectors.
Instantons, confinement, and the untwisted infrared bridge require three additional data:
\begin{enumerate}[label=(\roman*)]
...
\item a \emph{Hilbert-screening bridge} consisting of screening operators, a vacuum projection,
an untwisted Hamiltonian, and a domination estimate comparing that Hamiltonian with the screening Laplacian.
\end{enumerate}
With these data, instanton sectors, confinement criteria, and mass-gap transfer are internal
objects of the same formalism.
\end{summary}
...
\begin{theorem}[Novikov completion theorem]
...
\begin{theorem}[Instanton-completed tangent-to-center principle]
...
\begin{theorem}[Screened tangent-to-center principle]
...
\begin{proposition}[Koszul identification of the visible center]
...
\begin{theorem}[Instanton-screening spectral sequence]
...
\begin{theorem}[Central localization criterion for confinement]
...
This physical reading requires a defect-realization functor from physical line operators to modules over the
dual algebra. The localization theorem itself is purely algebraic.
\end{remark}
...
\begin{theorem}[Higher-body \v{C}ech duality after screening]
...
\begin{theorem}[Screening Hodge theorem]
...
\begin{corollary}[Screening spectral gap criterion]
...
The datum is part of the hypothesis for the untwisted comparison; it is not produced by the screening
construction alone.
\end{definition}

\begin{theorem}[Mass-gap transfer via screening domination]
\label{thm:mass-gap-transfer-screening-domination}
...
\begin{corollary}[Stable untwisting under bounded error]
Assume the setup of Theorem~\ref{thm:mass-gap-transfer-screening-domination}, except that
\[
...
Vol~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}, hosted on the
...
$(\mathfrak{g}_{\Delta_5}, \mathfrak{n}_+^{\mathrm{imag}}\oplus
		...
		\(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\sigma)^{24})\)). On the Koszul locus of the
...
K^{\kappaChHodge} = -1$ holds with Mukai-doubled Koszul
conductor $K^{\kappaChHodge} = 2 c_+(\Lambda_{\mathrm{Muk}}) =
8$.
\end{theorem}
...
$\mathrm{CH}^1(H_1)\otimes \mathbb{Z}/8$; the Vol~III finite
calculation verifies the Humbert $H_1$ monodromy order.
\end{proof}
...
1964 multiplier system). Compareerences: Vol~I
...
\begin{remark}[Algebraic core and bridge obligations]
Theorems~\ref{thm:novikov-completion-theorem}--\ref{cor:stable-untwisting-bounded-error} construct:
\begin{enumerate}[label=(\roman*)]
...
\item a theorem-level transfer from screening positivity and Hamiltonian domination to a positive mass gap.
\end{enumerate}
The bridge obligations are analytic and physical: construct a natural untwisting bridge datum for the
actual untwisted four-dimensional Yang--Mills Hamiltonian, and verify the screening domination estimate in
Theorem~\ref{thm:mass-gap-transfer-screening-domination}.
\end{remark}

\section{Synthesis}
\label{sec:yms-supplement}

\begin{remark}[Yang-Mills class-$\mathcal S$ synthesis on $\Sigma_{0,24}$]
\label{rem:yms-1}
The Yang--Mills class-$\mathcal S$ synthesis at the K3 base point is
class-$\mathcal S$ $A_1$ on $\Sigma_{0,24}$: $4d$ $\mathcal N=2$ theory
