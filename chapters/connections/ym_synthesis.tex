...
\item the Verdier dual of the open sector is the cobar complex of the Koszul dual:
\[
\mathbb{D}_{\Ran}\, \mathsf{Open}_{\cB} \simeq \mathsf{Bulk}^{\vee}_{\cB} = \Omegach(\cA_{\cB}^!).
\]
\end{enumerate}
\end{theorem}
...
Theorem~\ref{thm:verdier-bar-cobar} specialized to $\cA_{\cB}$.
\end{proof}
...
For morphisms, Theorem~\ref{thm:bv-functor} supplies functoriality of the bar construction,
and the construction of Theorem~\ref{thm:main-koszul-hoch} is functorial because it is built
...
\begin{proof}
By the deformation-obstruction dictionary, primary obstructions to extending a first-order deformation lie in $HH^{3}_{\mathrm{chiral}}(\cA_{\cB})$.  Apply Theorem~\ref{thm:main-koszul-hoch} with $n=3$ to obtain
\[
...
\begin{proof}
By Theorem~\ref{thm:deformations-maurer-cartan}, the formal deformations of $\cA_{\cB}$ are governed by Maurer--Cartan elements of the dg Lie algebra $\mathfrak{g}^{\mathrm{def}}_{\cB}$.  Its cohomology satisfies
\[
...
% ===== Begin twisted_ym_frontier_new_math.tex =====
% New frontier mathematics package: central formality and mixed couplings.
% Suggested insertion point: immediately after the subsection
% "Twisted Yang--Mills boundary package" in Chapter 31.
...
\begin{definition}[Instaton charge monoid and Novikov ring]
...
\begin{theorem}[Instaton-completed tangent-to-center principle; \ClaimStatusProvedHere]
...
\begin{theorem}[Instaton-screening spectral sequence; \ClaimStatusProvedHere]
