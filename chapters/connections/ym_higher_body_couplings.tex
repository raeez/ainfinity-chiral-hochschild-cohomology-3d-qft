...
Several boundary sectors interacting force a relativised Hochschild theory.
The multi-body problem assembles a simplicial package whose nerve
organises all pairwise and higher mixed deformations into a single
coherent datum. The relative Hochschild cochain complex
$CC^\bullet_{\mathrm{rel}}(M_{12};M_1,M_2)$ is the homotopy cofibre
of the restriction maps; its cohomology classifies the mixed couplings
invisible from either boundary alone. The higher-body \v{C}ech complex
extends this to an arbitrary finite collection of boundary conditions,
assembled via a simplicial boundary nerve whose cohomology controls
the simultaneous deformability of the configuration.
...
higher-body structures are coherent corrections to the decoupling
ansatz in which each boundary deforms independently. The relative duality theorem
(Theorem~\ref{thm:relative-duality-theorem}) is a projection of Verdier
intertwining in $\mathfrak{g}^{\mathrm{mod}}_\cA$; its long exact
sequence is the Mayer--Vietoris sequence of the modular
...
test case: the mixed Kodaira--Spencer class and its higher-body
\v{C}ech generalisation are computed explicitly and matched against
complementarity. The simplicial boundary package defined here feeds
...
\begin{definition}[Complete reduced derived center]
\label{def:complete-reduced-derived-center}
For a unital complete filtered minimal \(A_\infty\)-algebra \(M\), set
\[
\widetilde Z_\infty(M)
:=
\ker\bigl(HH^0(M)\longrightarrow \Bbbk\mathbf 1\bigr),
\]
with the induced complete filtration.
\end{definition}
...
\]
In particular, write $Q_{\mathrm{mix}}\subset E_1^0$ for the
quadratic-limit summand identified by
\[
Q_{\mathrm{mix}}
\cong
\widetilde Z_\infty(M_1)\widehat\otimes \widetilde Z_\infty(M_2).
\]
Then the class of a quadratic-limit mixed coupling survives to the next
page exactly when it lies in the kernel of
\[
[\kappa_{\mathrm{mix}},-] :
Q_{\mathrm{mix}}
...
Let $Q_{\mathrm{mix}}\subset E_1^0$ be the quadratic-limit summand
identified by
\[
Q_{\mathrm{mix}}
\cong
\widetilde Z_\infty(M_1)\widehat\otimes \widetilde Z_\infty(M_2).
\]
The quadratic mixed-coupling law survives to first non-quadratic order
exactly when
\[
[\kappa_{\mathrm{mix}},-]\big|_{Q_{\mathrm{mix}}}=0.
\]
If the adjoint-action map
\[
HH^1_{\mathrm{rel}}(M_{12};M_1,M_2)
\longrightarrow
\operatorname{Hom}\bigl(Q_{\mathrm{mix}},E_1^1\bigr),
\qquad
\eta\longmapsto [\eta,-]\big|_{Q_{\mathrm{mix}}},
\]
is faithful, this condition is equivalent to
$\kappa_{\mathrm{mix}}(M_{12};M_1,M_2)=0$. Without this faithfulness
hypothesis, the vanishing of $\kappa_{\mathrm{mix}}$ is sufficient for
persistence but is not necessary.

More generally, if the filtered cospan is gauge-equivalent, relative to
the two boundary faces, to one whose mixed coderivation has
$m^{(1)}=\cdots=m^{(r)}=0$, then no mixed-weight spectral-sequence
differential of length $1,\dots,r$ occurs.
\end{corollary}

\begin{proof}
Theorem~\ref{thm:first-correction-theorem} identifies the first
differential on $Q_{\mathrm{mix}}$ with the restricted adjoint action
$[\kappa_{\mathrm{mix}},-]\big|_{Q_{\mathrm{mix}}}$. Persistence of the
whole quadratic law means precisely that every class in
$Q_{\mathrm{mix}}$ lies in the kernel of this map. The faithfulness
statement is the definition of faithfulness for the displayed
adjoint-action representation. If the lower mixed coderivation pieces
are killed by a relative filtered gauge transformation, the Hochschild
differential has no terms raising mixed weight by $1,\dots,r$; the
construction of the filtered spectral sequence then gives
$d_1=\cdots=d_r=0$.
\end{proof}
...
\qquad n\ge 2.
\]
A formal mixed coupling $\alpha\in \mathbb T_{\mathrm{mix}}\widehat\otimes \mathfrak m$ extends to all orders exactly when
\[
...
\]
Call $C_Z^\bullet(\mathfrak N)$ the \emph{derived-center \v{C}ech complex} of the simplex.
\end{definition}
...
\]
Pure $m$-body first-order couplings are the dual tensor product of the reduced
facewise derived centers.
\end{corollary}
...
\section{Architectural synthesis: higher-body couplings and the universal holography ladder}
\label{sec:ym-higher-body-holography-synthesis}

\begin{definition}[Higher-body YM bridge datum]
\label{def:higher-body-ym-bridge-datum}
For a boundary simplex $\mathfrak N=\{M_S\}$, a \emph{higher-body YM
bridge datum} consists of the following five pieces of structure:
\begin{enumerate}[label=(\roman*)]
\item defect-realization data, namely factorization-algebra defects
$\mathcal D_S$ and quasi-isomorphisms from their transferred boundary
endomorphism algebras to the corresponding $M_S$, compatible with all
face maps;
\item Hilbert-screening bridge data, namely a functor from the
Novikov-completed deformation complex of $\mathfrak N$ to the
positive-energy Hilbert complex which sends mixed Hochschild classes to
screening operators and intertwines the face restrictions;
\item untwisting bridge data, namely a filtered quasi-isomorphism from
the twisted BRST or holomorphic-topological complex to the untwisted YM
complex on a common dense domain;
\item screening positivity, namely a Hermitian form for which the
screening operator attached to each effective mixed class is
non-negative and has kernel equal to the screened sector;
\item domination estimates controlling the Novikov sums and higher-body
operator products uniformly over the boundary simplex.
\end{enumerate}
The physical comparison functor from
$T^{1,[m]}_{X,\mathrm{pure}}(\mathfrak N)$ to screening, mass-gap, and
celestial soft-factor data is defined on the subcategory of boundary
simplices equipped with these five structures.
\end{definition}

The pure $m$-body coupling spaces $T^{1,[m]}_{X,\mathrm{pure}}(\mathfrak N)$ classified
in this chapter are the algebraic source for the YM rung of the
universal holography ladder. After a higher-body YM bridge datum is
chosen, the comparison map sends them to the higher-body component of
the bulk derived chiral center. In the binary case
(Theorem~\ref{thm:w-augmented-center-tensor-model} with $m=2$) this
recovers the binary tensor decomposition of
$Z^{\mathrm{der}}_{\mathrm{ch}}(\cA_{\cB_1\sqcup\cB_2})$; in arity
$m$ the comparison lands in the $m$-th mixed sector of the
SC$^{\mathrm{ch,top}}$ heptagon (Chapter~\ref{ch:sc-chtop-heptagon},
edge $(3)\leftrightarrow(4)$).

\begin{remark}[Higher-body couplings as SC$^{\mathrm{ch,top}}$ mixed-sector data]
\label{rem:ym-higher-body-mixed-sector}
The mixed-sector dimension formula
$\dim \SC^{\mathrm{mix}}_{k,m} = (k-1)!\binom{k+m}{m}$
(see the SC$^{\mathrm{ch,top}}$ heptagon edge analysis) counts the target
channels for the bridge comparison map. The filtered-stability theorem
(Theorem~\ref{thm:w-filtered-stability-pure-mbody}) identifies the
$E_1$-stable source multiplicity on the affine + principal $\cW$
sublocus equipped with the bridge datum of
Definition~\ref{def:higher-body-ym-bridge-datum}.
\end{remark}

\begin{remark}[$\chi$-functor descent and Beem--Rastelli factorization]
\label{rem:ym-higher-body-chi-functor-descent}
Under the defect-realization and untwisting parts of
Definition~\ref{def:higher-body-ym-bridge-datum}, Beem and Rastelli's
$\chi$-functor \cite{BR15} applied to the multi-component boundary
simplex $\mathfrak N$ is compatible with the bar-cobar reduction of
Chapter~\ref{ch:ym-boundary-theory}. The resulting comparison sends a
pure $m$-body coupling to the $\chi$-image of an $m$-fold composite of
bulk operators in the universal holography target. With the
Hilbert-screening bridge, screening positivity, and domination estimates
in place, Theorem~\ref{thm:uch-main}
(Chapter~\ref{sec:universal-celestial-holography}) applies to the YM
rung and identifies the filtered-stability theorem above with the
chain-level source of the celestial $m$-point soft-factor hierarchy.
\end{remark}

\begin{remark}[Universal conductor at higher body order]
\label{rem:ym-higher-body-universal-conductor}
Assume the BRST resolution of
$\cA_{\cB_1\sqcup\cdots\sqcup\cB_m}$ splits, on the decoupled-graded
sublocus, as the completed direct sum of the component ghost complexes.
Then the Koszul conductor identity $K=-c_{\mathrm{ghost}}(\BRST)$
(Vol.~I, the Vol~I universal-conductor chapter) is additive in the
components and recovers the binary conductor identity of
Chapter~\ref{ch:ym-boundary-theory} at $m=2$. The collapse at $E_1$ in
Theorem~\ref{thm:w-filtered-stability-pure-mbody} is the corresponding
conductor-additivity statement for the deformation spectral sequence.
\end{remark}
