...
\subsection{The $R$-matrix from monodromy}
...
loop $\gamma\colon z_1\mapsto z_2\,e^{2\pi i t}+z_1(1-e^{2\pi it})$
(the first point circles the second).

The ordered bar complex
$\Barch^{\mathrm{ord}}_2=\bar\cA\otimes\bar\cA\otimes
C_\bullet(\mathrm{Conf}_2^{\mathrm{ord}})$
carries a flat connection from the bar differential:
the residue differential $d_{\mathrm{res}}$ is a
connection $1$-form on $\mathrm{Conf}_2^{\mathrm{ord}}$
with values in $\operatorname{End}(\bar\cA\otimes\bar\cA)$.
The \emph{$R$-matrix} is the monodromy of this connection
around~$\gamma$:
\begin{equation}\label{eq:r-matrix-monodromy}
R(z)
\;:=\;
\operatorname{Mon}_\gamma
\bigl(d_{\mathrm{res}}\big|_{\mathrm{Conf}_2^{\mathrm{ord}}}\bigr)
\;\in\;
\operatorname{End}(\bar\cA\otimes\bar\cA)[\![z^{-1}]\!],
\end{equation}
where $z=z_1-z_2$ is the relative coordinate and the
formal expansion in $z^{-1}$ comes from the OPE pole
structure.

The $R$-matrix intertwines the two orderings:
\[
R(z)\colon
\Barch^{\mathrm{ord}}_2(a\otimes b;\,z_1,z_2)
\;\xrightarrow{\;\sim\;}
\Barch^{\mathrm{ord}}_2(b\otimes a;\,z_2,z_1)
\]
for $a,b\in\bar\cA$.  For $E_\infty$-algebras,
$R(z)=\tau$ (the flip), and the ordered and unordered
bar complexes are identified.  For genuine $E_1$-algebras,
$R(z)\neq\tau$ and carries nontrivial spectral dependence.

\emph{Explicit formula.}
$R(z)=\mathcal P\exp\!\bigl(\oint_\gamma
\sum_{n\ge 0}r_n\,z^{-n-1}\,dz\bigr)$
where $r_n=\sum_{I,J}c^?_{IJ;n}\,e_I\otimes e_J$ is the
OPE matrix at pole order~$n$.  At leading order:
$R(z)=1+r_0/z+O(z^{-2})$, where
$r_0=\sum_{I,J}c^?_{IJ;0}\,e_I\otimes e_J$ is the matrix
of $0$-th products---the \emph{classical $r$-matrix}.
\end{construction}
...
\begin{proof}
$\mathrm{Conf}_3^{\mathrm{ord}}(\mathbb C)$ has fundamental
group generated by three loops: $\gamma_{12}$ ($z_1$ circles
$z_2$), $\gamma_{13}$ ($z_1$ circles $z_3$), $\gamma_{23}$
($z_2$ circles $z_3$).  The relation in~$P_3$ is
$\gamma_{12}\gamma_{13}\gamma_{23}
=\gamma_{23}\gamma_{13}\gamma_{12}$
(the pure braid relation).  The monodromy representation
$\pi_1(\mathrm{Conf}_3^{\mathrm{ord}})\to
\operatorname{GL}(\bar\cA^{\otimes 3})$
sends $\gamma_{ij}\mapsto R_{ij}(z_i-z_j)$.  The pure
braid relation becomes~\eqref{eq:ybe-from-bar}.

Alternatively: $d^2$ on $\Barch^{\mathrm{ord}}_3$ involves
composing two boundary faces of
$\overline{\mathrm{FM}}_3^{\mathrm{ord}}$.  The two
codimension-one faces $D_{1,2}$ and $D_{2,3}$ meet in one
codimension-two face $D_{1,2}\cap D_{2,3}
=\{z_1=z_2=z_3\}$.  The two ways of reaching this face
($D_{1,2}$ then $D_{2,3}$, or $D_{2,3}$ then $D_{1,2}$)
produce the two sides of YBE\@.  Their equality is
$d^2=0$.
\end{proof}
...
The Koszul dual is $(\cH_k)^!=\cH_{-k}$
(the same algebra at negated level).  The $R$-matrix is
...
\subsection{The dg-shifted Yangian from the ordered bar complex}
\label{subsec:dg-shifted-yangian-from-bar}

\begin{construction}[dg-shifted Yangian as ordered bar
cohomology; \ClaimStatusProvedHere]
...
affine line is the cohomology of the ordered bar complex:
\[
\cA^!
\;=\;
H^*\!\bigl(\Barch^{\mathrm{ord}}(\cA)\bigr)^\vee.
\]
For $\cA=\widehat{\mathfrak g}_k$:
$\cA^!=Y_\hbar(\mathfrak g)$, the Yangian at deformation
parameter $\hbar=1/(k+h^\vee)$.
...
curve coordinate).  The Yangian relations
$[H_i(u),\,E_j(v)]=\cdots$ arise from the degree-$2$
bar cohomology relations: the bar differential on
...
$u$-expansion come from higher-pole OPE coefficients
$h_{i(n)}e_j$ ($n\ge 1$).

The RTT presentation $R(u{-}v)\,T_1(u)\,T_2(v)
=T_2(v)\,T_1(u)\,R(u{-}v)$
follows from the associativity of the ordered bar complex:
$T(u)=\sum_I e_I\otimes t^I(u)$ is the generating matrix
with $t^I(u)\in\cA^![\![u^{-1}]\!]$, and the RTT relation
is $d^2=0$ on the three-fold ordered bar complex
$\Barch^{\mathrm{ord}}_3$
