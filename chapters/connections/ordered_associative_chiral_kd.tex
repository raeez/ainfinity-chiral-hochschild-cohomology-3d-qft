...
homological algebra is clean and state the established inputs used below. In \S\ref{sec:shuffle} we
...
Koszul duality, and the enveloping algebra formalism are taken as already established
(Hypothesis~\ref{hyp:inputs}).
...
Work over a field $k$ of characteristic $0$. Let $X$ be a smooth curve over $k$, and let
...
The following are assumed results.
...
One-sided module/comodule duality is already established (Hypothesis~\ref{hyp:inputs}). We
bootstrap it to the two-sided theory by passing from $A$ to its enveloping algebra $A^e$ and
then using Corollary~\ref{cor:enveloping}.
...
It is convenient to package the preceding results into a single statement.
...
\begin{theorem}[Geometric realization of the diagonal bicomodule; \ClaimStatusProvedHere]
% legacy label retained in this superseded file
\label{conj:bordered}
Let \(A\) be a strongly admissible augmented \(E_1\)-chiral algebra.
Licensing data: \(\alpha+\beta+\gamma\), for the boundary chart,
bordered-FM comparison, and logarithmic-chain ambient.
For \(p,q\geq0\), the bordered Fulton--MacPherson compactification
\[
\overline{\mathrm{Conf}}_{p,q}(\mathbb H)
\]
of \(p\) interior points and \(q\) ordered boundary points has
codimension-one residue types
\[
z_i\to z_j,\qquad x_a\to x_b,\qquad
\Im z_i\to0,\ \Re z_i\to x_a .
\]
The diagonal boundary condition is the bimodule
\(\Delta_A={}_AA_A\), with left action from the negative boundary
approach and right action from the positive boundary approach. Its
bordered logarithmic chain model is
\[
D_A^{p,q}
=
C_\bullet^{\log}\!\bigl(
\overline{\mathrm{Conf}}_{p,q}(\mathbb H)
\bigr)\otimes A^{\otimes p}\otimes A^{\otimes q},
\]
with residue differential
\[
d_D=d_{\mathrm{int}}+d_{\partial}+d_{\mathrm{mix}},
\quad
d_{\mathrm{int}}=\sum_S\operatorname{Res}_{D_S^{\mathrm{int}}}\otimes m_S,
\quad
d_\partial=\sum_I\operatorname{Res}_{D_I^\partial}\otimes\mu_I,
\]
and
\[
d_{\mathrm{mix}}
...
\bar a_i=|a_i|-1.
\]
Equivalently, the older bordered ordered configuration-space model
\[
\mathfrak C^{\mathrm{bord}}_{g,r,s}(X)
\]
has a functorial system of chain-level operations on the diagonal
bicomodule \(C_\Delta\) such that:
\begin{enumerate}[label=(\arabic*)]
...
\begin{remark}
This superseded copy records the resolved status. The active statement
is \texttt{thm:bordered-diagonal-bicomodule-chain-model} in
\texttt{ordered\_associative\_chiral\_kd\_core.tex}; the annular
closure is Theorem~\ref{thm:bordered-annular}.
\end{remark}
...
\begin{remark}[Bordered configuration spaces]
The bordered configuration-space construction is in Volume~I,
\S\ref*{V1-sec:bordered-fm}, and in
Theorem~\ref{thm:bordered-diagonal-bicomodule-chain-model}. Items~(1)--(5)
of Theorem~\ref{conj:bordered} are resolved; the annular closure is
the bordered-FM/coHochschild Theorem~\ref{thm:bordered-annular}.
\end{remark}
...
associative product \(\mu_2\) of the low-degree construction above.
...
the open-colour product \(\mu_2\). The monodromy part
...
(the residue of the double pole, with the unique degree-$2$ ordered
face carrying sign \(+1\)). Bar cohomology:
...
$\hbar=1/(k+2)$:
$R(z)=1+\hbar\,k\,\Omega_{\mathrm{tr}}/z+O(z^{-2})$, where
...
associative chiral analogue of the commutative current-level shadow
\[
\widehat{\mathfrak g}_k
\longmapsto
\operatorname{CE}^{\mathrm{ch}}\!\bigl(\widehat{\mathfrak g}_{-k-2h^\vee}\bigr)
\]
coming from Verdier-level duality: the ordered bar complex replaces
...
Bershadsky--Polyakov algebra: its spin-$3/2$ generators are
\emph{bosonic}, its $G^+G^-$ OPE carries the nonlinear
${:}JJ{:}$ term, and
$c(k)=-(2k+3)(3k+1)/(k+3)$. It is \emph{not} isomorphic to the
$\mathcal N=2$ superconformal algebra --- the statistics of the
spin-$3/2$ generators differ --- but the two families share the
generator spectrum $(2,\tfrac32,\tfrac32,1)$ and are expected to
be related through the Heisenberg coset of the former together
with a free-field extension, in the Kazama--Suzuki pattern. The
ordered bar complex below is computed for the $\mathcal N=2$
superconformal algebra itself, at central charge $c$, generated
by:
\begin{center}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{lcl}
...
In $\mathfrak{sl}_3$ the partition $[2,1]$ is self-transpose and
the subregular and minimal orbits coincide, so the conjectured
Koszul dual is the \emph{same} family at the dual level:
$W_{-k-6}(\mathfrak{sl}_3,f_{\mathrm{sub}})$. This self-duality
of the family under $k\mapsto -k-6$ is exactly the conductor
identity $c(k)+c(-k-6)\equiv 50$.
...
\emph{DS reduction.}
The subregular (for $\mathfrak{sl}_3$, equal to the minimal)
DS reduction
$\mathrm{DS}_{f_{\mathrm{sub}}}\colon
\widehat{\mathfrak{sl}}_3{}_k
\to W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})$
attaches ghosts to the positive-grade subspace
$\mathfrak{g}_{1/2}\oplus\mathfrak{g}_{1}$ of dimension
$2+1=3$. Naive central-charge additivity
$c_{\mathrm{aff}} = c_{W} + c_{\mathrm{ghost}}$ fails for every
DS reduction: the $W$-algebra stress tensor is the improved
$T_{\mathrm{Sug}} + \partial x_{f}$, and the improvement term
shifts the central charge by a level-dependent amount. With the
value
$c\bigl(W_k(\mathfrak{sl}_3,f_{\mathrm{sub}})\bigr)
= 8k/(k+3) - 6k - 1$, the central-charge deficit is
\[
c_{\mathrm{aff}} - c_{W,\mathrm{sub}}
= \frac{8k}{k+3} - \Bigl(\frac{8k}{k+3} - 6k - 1\Bigr)
= 6k + 1,
\]
level-dependent through the improvement. The deficit is not a
ghost central charge: a $bc$ pair's central charge is fixed by
its conformal weights and is level-independent, and the
level-independent conductor cross-check at this vertex is the
gauging-ghost total for spins $(2, \tfrac32, \tfrac32, 1)$,
$-26 - 11 - 11 - 2 = -50 = -(c + c')$. The principal case has
the same shape: the deficit
$c_{\mathrm{aff}} - c_{W,\mathrm{prin}}$ is level-dependent,
while the number of ghost fields $N(N-1)$ and the gauging total
$-(c+c') = -100$ at $N = 3$ are not.
\end{conjecture}
...
\begin{definition}[Hall window for the K3 ordered-associative lane]
\label{def:v2-oackd-finite-hall-window}
A \emph{Hall window} $\mathcal W$ is a finite saturated set of
Mukai-charge labels for the $K3\times E$ Hall correspondences, closed
under the extension diagrams used by the ordered-associative bar
truncation.  The window carries compact-support orientation data,
positive and opposite Hall halves, a Cartan line, a nondegenerate Hopf
pairing after radical quotient, parity/root fixtures, PBW/no-extra
relations, and an $H_1$-compatible trace.  It gives only finite-window
objects
\[
\mathbf H_{\Delta_5}^{\mathcal W},\qquad
\mathbf Q_{\hbar}^{\mathrm{K3},\mathcal W}.
\]
A completed $\mathbf H_{\Delta_5}$, a scalar partition function, an
Arthur-packet identification, or a refined $\Omega$/$q$-Bethe structure
requires a cofinal recognized system of such windows with
Mittag--Leffler completion and Borcherds--Heegner comparison
\cite{kontsevich-soibelman,PTVV13,BBJ19,MaulikOkounkov12}.
\end{definition}
...
pairs with Vol~I's twin only after choosing a Hall window
$\mathcal W$: the $K_2$ ordered-associative chain on
$\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$ has
$\mathbf H_{\Delta_5}^{\mathcal W}$ coefficients and Koszul shift
$[2]$ CY-$2$. The chain-level chiral quantum group before completion is
\[
\mathbf Q^{\mathrm{K3},\mathcal W}_\hbar
=D_\hbar\!\bigl(
C_{\star,\mathcal W}^{\mathrm{CE,crv}}(\mathfrak g_{\Delta_5});
\omega_{\Delta_5}
\bigr),
\]
with $\hbar^2_{\mathrm{K3}} = -1/8$ Lusztig specialisation
\cite{GritsenkoNikulin1998,Borcherds1992}. Compare Vol.~III
...
For a Hall window $\mathcal W$ satisfying
\cref{def:v2-oackd-finite-hall-window}, the Arthur statement is a
recognition condition on the windowed Hecke trace.  If the traces of
$\mathbf Q^{\mathrm{K3},\mathcal W}_\hbar$ agree in the window with
\[
\lambda_p(\Delta_{10}) =
a_p(\Delta_{E_6}) + p^8 + p^9,
\]
then this finite range is compatible with the
Saito--Kurokawa/Ikeda parameter
$\psi_{\Delta_{10}}=\phi_{\Delta_{E_6}}\boxtimes\mathrm{Sym}^1$.
The $p^8+p^9$ term is a nonquadratic Hall--Borcherds target trace; it
becomes evidence for $R_{\mathrm{Borcherds}}\neq0$ only after the
PBW/no-extra-relations, radical-quotient, and pairing gates hold in the
recognized Hall-window system \cite{Andrianov1974,Ikeda2001}. Compare
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[refined $\Omega$ two-parameter extension of $\mathbf H_{\Delta_5}$; \ClaimStatusConjectured]
\label{rem:v2-oackd-refined-omega-two-param}
Assume that the Hall window $\mathcal W$ carries two equivariant
parameters $(\epsilon_1,\epsilon_2)$ preserving the Hall correspondences
and the Maulik--Okounkov stable-envelope $R$-matrix.  The K3
ordered-associative chain at $\hbar^2=-1/8$ lies on the self-dual
$\Omega$-locus $\epsilon_1+\epsilon_2=0$ with
$\epsilon_1=-\epsilon_2=1/(2\sqrt2)$ and
$\hbar^2=\epsilon_1\epsilon_2=-1/8$.  Off the self-dual locus the
expected object is the finite-window family
$\mathbf H_{\Delta_5}^{\mathcal W}(\epsilon_1,\epsilon_2)$, not a
completed two-parameter Hall--Drinfeld double.

The finite-window refined trace has the expected form
\[
Z^{\mathrm{ref}}_{\mathcal W}(\tau_{\mathrm K3}, \epsilon_1, \epsilon_2)
 \;=\; \prod_{(n, l, m)\in \mathcal W_+}
...
2007).  The specialization $Z^{\mathrm{ref}}\to\Delta_5$ is conditional:
it requires a cofinal recognized Hall-window system whose self-dual
limit has the Gritsenko--Nikulin divisor, character, first
Fourier--Jacobi coefficient, and Borcherds multiplicities.
Primary: Maulik--Okounkov, \emph{Ast\'erisque} 408 (2019);
Nekrasov--Okounkov 2003 (hep-th/0306238); Iqbal--Kozcaz--Vafa 2007
(hep-th/0701156). Compare Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[refined K3 elliptic genus and refined BKM product; \ClaimStatusConjectured]
\label{rem:v2-oackd-refined-bkm-product}
Under a refined Hall window $\mathcal W$, the candidate refined K3
elliptic genus
$\chi_{\mathrm{K3},\mathcal W}^{\mathrm{ref}}(\tau,z_1,z_2)$ is required
to restrict at $z_1=z_2=z$ to the finite range of the
Eguchi--Ooguri--Tachikawa Jacobi form $\phi_{0,1}^{\mathrm{K3}}$. The
windowed refined BKM denominator is
\[
\prod_{(m, n, \ell_1, \ell_2)\in \mathcal W_+}
 (1 - p^m q^n y_1^{\ell_1} y_2^{\ell_2})^{c(mn, \ell_1, \ell_2)}
\]
with multiplicity exponents governed in that finite range by the
refined K3 genus Fourier coefficients. At $y_1 = y_2^{-1}$ (self-dual
slice), $y_1^{\ell_1} y_2^{\ell_2}
= y_1^{\ell_1 - \ell_2}$ and the multiplicities reduce to the
$\phi_{0,1}^{\mathrm K3}$ discriminant-indexed $c_{\mathrm K3}(4nm - \ell^2)$,
matching the Bruinier Prop.~5.1 multiplicities in the checked finite
Heegner labels (\cite[Prop.~5.1]{Bruinier2002}). Passing from this
finite check to the $\Delta_5$ BKM denominator is precisely the
Borcherds--Heegner recognition gate for a cofinal Hall-window system.
Primary: Iqbal--Kozcaz--Vafa 2007 Sec.~4;
...
\label{rem:v2-oackd-qbethe-nonselfdual}
At the self-dual slice, a Maulik--Okounkov Hall window with stable
envelopes gives rational Bethe equations for the finite-window
$\mathrm{Hilb}^n(\mathrm{K3})$ module category.  Off self-dual, the
Nekrasov--Shatashvili template predicts a $q$-deformation with
$q:=\exp(-2\pi i\epsilon_2/\epsilon_1)$, but the assertion that the
K3 Hall-window trace $\log Z^{\mathrm{ref}}_{\mathcal W}$ is the
Yang--Yang function is conditional on constructing commuting
Hamiltonians and Baxter/transfer operators compatible with the same
Hall window. Primary:
...
The rigorous identification of $q$-Bethe equations for the completed
$\mathbf H_{\Delta_5}$ beyond the $\mathrm{Hilb}^n(\mathbb A^2)$
prototype is open for K3 itself.
\end{remark}

\begin{remark}[native $E_1$ vs.\ derived $E_2$ discipline]
\label{rem:v2-oackd-eone-etwo-scope}
For a Hall window $\mathcal W$, the K3 comparator
$\mathbf H_{\Delta_5}^{\mathcal W}$ carries three distinct operadic
levels before completion:
\begin{itemize}
\item \textbf{Native $E_1$-chiral}: $\mathbf H_{\Delta_5}^{\mathcal W}$ as an
 ordered-associative $E_1$-chiral algebra on the curve $E$ (elliptic
 factorisation base, $\mathrm{Ran}(E^{\mathrm{nod,sm}}_{24})$), with
 bar coalgebra $\Barch(\mathbf H_{\Delta_5}^{\mathcal W})$ in the sense of this
...
\item \textbf{Derived $E_2$ via factorisation on K3}: the K3 factorisation
 structure lifts a recognized finite Hall window to an $E_2$-chiral
 algebra \emph{on the K3 surface} only when the factorisation maps
 preserve the compact-support orientation, Hall product, and radical
 quotient. The $E_2$ operadic structure is derived from the
 2-dimensional factorisation geometry, not native to the curve-level
 chain.
\item \textbf{Vol.~III CY-$2$ $\Phi$ output}: the functor
 $\Phi_2 : D^b\mathrm{Coh}(\mathrm{K3}) \to E_2\text{-}\mathrm{ChirAlg}(\mathrm{K3})$
 produces the windowed output
 $\Phi_2^{\mathcal W}(D^b\mathrm{Coh}(\mathrm{K3}))$ as a finite
 $E_2$-chiral object.  Identifying its cofinal completion with
 $\mathbf H_{\Delta_5}$ is a Hall--Borcherds recognition problem.
 Factorisation over a curve $E$ intersecting K3 (e.g.\ an elliptic
 fibre of an elliptic K3) reverts the level to native $E_1$.
\end{itemize}
The conflation between native and derived levels is the relevant
typing hazard. The ordered-associative Koszul-duality of this chapter
is unambiguously at the \emph{native
$E_1$} level: $A$ is $E_1$-chiral, $A^!$ is Yangian-like, and $E_2$
appears only on the derived centre $Z(\mathrm{Rep}(A))$. Compare
Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic} and
\S\ref{sec:oackdf-supplement-ii} of the frontier companion.
\end{remark}

\begin{remark}[Summary]
\label{rem:v2-oackd-refined-summary}
The K3 ordered-associative lane constructs finite Hall-window
comparators
$\mathbf H_{\Delta_5}^{\mathcal W}(\epsilon_1,\epsilon_2)$ and
$\mathbf Q_{\hbar}^{\mathrm{K3},\mathcal W}$ before completion.  Refined
partition functions, $q$-Bethe equations, refined BKM products, Arthur
packets, and the completed $\Delta_5$ denominator are conditional on a
cofinal recognized Hall-window system. The operadic level is native
$E_1$ on the curve~$E$ and derived $E_2$ on the K3 surface only after
the factorisation maps preserve the same window data. Bruinier
Prop.~5.1 leading multiplicities at the self-dual slice give the finite
Heegner-label check; they do not by themselves construct the completed
partition function.
\end{remark}
