...
Yang--Baxter equation as linearisation, and the dg-shifted Yangian as ordered bar cohomology
with Drinfeld generators and RTT presentation.
...
(Construction~\ref{constr:r-matrix-covering}), the dg-shifted Yangian
as ordered bar cohomology
...
\begin{construction}[$E_1$ shadow tower]
% label removed: constr:vol2-e1-shadow-tower
The $E_1$-chiral MC element $\Theta^{E_1}_\cA \in \mathrm{MC}(\mathfrak{g}^{E_1}_\cA)$ generates the ordered shadow tower $(\Theta^{E_1}_\cA)^{\leq r}$, with projections:
\begin{enumerate}[label=\textup{(\roman*)}]
\item At arity~$2$: the spectral $R$-matrix $r(z) = \mathrm{Res}^{\mathrm{coll}}_{0,2}(\Theta^{E_1}_\cA)$.
\item At arity~$3$: the CYBE $[r_{12}, r_{13}] + [r_{12}, r_{23}] + [r_{13}, r_{23}] = 0$ as MC truncation.
\item At genus~$1$: the KZ associator from the genus-$1$ component $\Theta^{(1),E_1}_\cA$.
\item At all genera: the Drinfeld--Kohno hierarchy (DK-$0$ through DK-$5$) as successive shadow extractions (Vol~I, Construction~\ref{constr:dk-shadow-projections}).
\end{enumerate}
...
(the chiral symmetric algebra on the dual; Vol~I, \S\ref*{sec:heisenberg-koszul}).  The $R$-matrix is
