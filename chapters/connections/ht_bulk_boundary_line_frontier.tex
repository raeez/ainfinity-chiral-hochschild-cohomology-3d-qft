...
The local theorem package is in place. The global theorem reduces to
the chain-level bulk/boundary map, the compact generator of the line
category, the line-module comparison for the absolute open-colour
algebra, and the Hopf/Steinberg endpoint required for a
...
\end{itemize}
The local exact sector realises each of these moves.

\subsection{The absolute line algebra versus boundary-adapted local line algebras}

Two distinct line objects appear:
\begin{enumerate}[label=(\roman*),leftmargin=1.8em]
\item the \emph{absolute} open-colour line algebra
$A^!_{\mathrm{line}}$ of the global theory, presenting the completed
perturbative line category on the chirally Koszul and quasi-linear
loci (\cite{DNP25}; for the affine lineage,
...
\end{enumerate}
The latter are local exact models: boundary charts on the absolute line
category $\cC_{\mathrm{line}}$, not globally equal to
$A^!_{\mathrm{line}}$, and not substitutes for the Hopf/Steinberg
comparison. They are the local testing ground in which the global
theorem becomes rigid.

\subsection{The bar coalgebra and the bulk/boundary/line triangle}

The Steinberg variety $Z = \widetilde{\cN} \times_{\mathfrak g}
\widetilde{\cN}$ presents the Hecke algebra via
$H_*^{BM}(Z) \cong \cH_q(W)$. The bar complex $\barB(\cA)$ is
an $E_1$ dg coassociative coalgebra: the differential $d_{\barB}$
encodes the chiral product on $\FM_k(\C)$, and the deconcatenation
coproduct $\Delta$ encodes the topological factorisation on
$\Conf_k^{<}(\R)$. The $\SCop$ structure does not live on
$\barB(\cA)$ itself; it emerges in the chiral derived center.
The chiral Hochschild cochain complex
$C^\bullet_{\mathrm{ch}}(\cA,\cA)$, defined via the chiral
endomorphism operad $\mathrm{End}^{\mathrm{ch}}_{\cA}$, carries brace
operations, and the pair $(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$
is the $\SCop$ datum (bulk acting on boundary). For the affine
lineage, the line category arising from $\cA^!$-modules recovers the
quantum group:
$\cC_{\mathrm{line}}^{\mathrm{red}}|_{\mathrm{eval}} \simeq
\mathrm{Rep}_q(\fg)$
(Theorem~\ref{thm:affine-monodromy-identification}).

\begin{theorem}[SC convolution presentation of the bulk-boundary-line triangle; \ClaimStatusProvedHere]
...
Let $(\openFactCat\big|_{\logCurve}, \cD^{\mathrm{cl}}\big|_X; b, A_b, \dots)$
be the bicoloured primitive package on a smooth curve~$X$, write
$\cA = A_b = \mathrm{End}_{\openFactCat}(b)$ for its
open-colour endomorphism algebra, and let
...
\item \textbf{$E_1$ coalgebra structure on $\barB(\cA)$.}
 ...
 &&\text{\textup{(}deconcatenation coproduct from
 $\Conf_k^{<}(\R)$\textup{)}}.
 \end{align*}
 The pair $(d_{\barB},\Delta)$ makes $\barB(\cA)$ an $E_1$
 coalgebra classifying twisting morphisms
 $\barB(\cA)\to\cA$
 \textup{(}Theorem~\textup{A}\textup{)}.
 The $\SCop$ structure does \emph{not} live on $\barB(\cA)$:
 $d_{\barB}$ and $\Delta$ are the $E_1$ \emph{coalgebra} data,
 dual to the $A_\infty$-algebra structure on~$\cA$.

\item \textbf{$\SCop$ datum from the chiral derived center.}
 The $\SCop$ structure lives on the pair
 $(C^\bullet_{\mathrm{ch}}(\cA,\cA),\;\cA)$:
 \begin{itemize}[leftmargin=1.5em]
 \item \emph{closed operations}: the chiral product on
 $C^\bullet_{\mathrm{ch}}(\cA,\cA)$ from composition in
 $\mathrm{End}^{\mathrm{ch}}_{\cA}$;
 \item \emph{open operations}: the $A_\infty$-structure on
 $\cA$ from $\Conf_k^{<}(\R)$;
 \item \emph{mixed operations}: the brace action
 $C^\bullet_{\mathrm{ch}}(\cA,\cA)\otimes\cA\to\cA$
 (bulk acting on boundary).
 \end{itemize}
 The no-open-to-closed rule is the directionality of the
 $\SCop$ operad: braces act from bulk to boundary, not
 conversely. Together with the canonical MC element $\alpha_T$
 of Theorem~\textup{\ref{thm:3d-universal-mc}}, these determine
 the full $\SCop$-datum by homotopy transfer
 \textup{(}Theorem~\textup{\ref{thm:homotopy-Koszul}}\textup{)}.

 \item \textbf{Line algebra from $\cA^!$-modules.}
 The open-colour sector of the $\SCop$-datum gives the algebraic
 model of the line category as the module category:
 \[
 \cC_{\mathrm{line}}
 \;\simeq\;
 \mathrm{Rep}(\gSC_T).
 \]
 The equivalence is the definition of the open-colour sector
 of $\alpha_T$
...
\item \textbf{Bulk-to-boundary from the mixed face.}
 The mixed sector of the $\SCop$-datum yields the
...
\begin{proof}
The theorem assembles six proved results into a single package.
Statement~(i) is Theorem~A (the bar-cobar adjunction), which
gives $\barB(\cA)$ the structure of an $E_1$ dg coassociative
coalgebra with differential $d_{\barB}$ and deconcatenation
coproduct~$\Delta$.
...
locus\textup{)}.
Statement~(v) is Proposition~\ref{prop:alpha-projections}(iii)
together with Theorem~\ref{thm:YBE}.
Statement~(vi) is Proposition~\ref{prop:alpha-projections}(v)
together with the genus-tower identification from Vol~I
(Theorem~D, the modular characteristic, bridge MC5).

The $E_1$ coalgebra data $(d_{\barB},\Delta)$ and the
$\SCop$ brace data on $(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$
are faces of a \emph{single} MC element
$\alpha_T \in \mc(\gSC_T)$: the boundary operations, line-sector
operations, braiding, and genus tower are not assembled by hand,
but read off one correspondence. The line-module and
corrected-triangle identifications are then the comparison theorems
that interpret those faces on the scoped theorem surface.
\end{proof}

\begin{remark}[The Steinberg analogy]
\label{rem:steinberg-dictionary}
\index{Steinberg!analogy}
The correspondence between geometric representation theory
and the HT bulk/boundary/line triangle is \emph{analogical},
not operadic:
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lll}
\hline
\textbf{Steinberg geometry} & \textbf{HT triangle} & \textbf{Role} \\
\hline
...
Hecke algebra $\cH_q(W)$
 & $\cA^!_{\mathrm{line}}$ (Koszul dual) & Line algebra \\
$\mathrm{Rep}(\cH_q) = \cO_q$
 & $\cC_{\mathrm{line}} \simeq \cA^!_{\mathrm{line}}\text{-mod}$ & Module category \\
Center $Z(\cH_q)$
...
\end{tabular}
\end{center}
The two colours of the $\SCop$ datum are
$(C^\bullet_{\mathrm{ch}}(\cA,\cA),\,\cA)$: bulk (closed) and
boundary (open). The bar complex $\barB(\cA)$ is the $E_1$
coalgebra underlying the boundary; the $\SCop$ brace operations
live on the chiral Hochschild cochains, not on~$\barB(\cA)$.
The no-open-to-closed rule
is the directionality of the brace action (bulk acts on boundary,
not conversely), analogous to the directionality of the Springer
...
The algebraic bulk/boundary/line triangle
(Theorem~\ref{thm:steinberg-presentation-extended}) is unconditional.
...
The algebraic bulk/boundary/line triangle
\textup{(}Theorem~\textup{\ref{thm:steinberg-presentation-extended}}\textup{)}
...
\item The dg-shifted-Yangian package of \cite{DNP25} has exactly
 ...
 $r$-matrix
 (and Theorem~\ref{thm:complete-strictification} identifies the
 spectral Drinfeld obstruction with the scalar classes
 $[\epsilon_{\alpha,n}]\in
 \mathscr K_n/\operatorname{im}(\delta_{\alpha,n})$; path sectors
 vanish, and general finite sectors strictify on their scalar
 vanishing locus).
\end{itemize}

\begin{example}[Geometric Steinberg for Heisenberg: all three ingredients verified;
 \ClaimStatusProvedHere]
\label{ex:geometric-steinberg-heisenberg}
\index{Steinberg!geometric conjecture!Heisenberg verification}
\index{Heisenberg algebra!geometric Steinberg}
The Heisenberg algebra $\cH_k$ (boundary chart of abelian
Chern--Simons on $\bC_z \times \bR_{\ge 0}$) provides an explicit
verification of ingredients (a)--(c) of
Conjecture~\ref{conj:geometric-steinberg}:
\begin{enumerate}[label=\textup{(\alph*)}]
\item The derived moduli of vacua is the shifted cotangent bundle
 $\cM_{\mathrm{vac}}(\cH_k) =
 T^*[-2]\bigl(\operatorname{Maps}(D^2, V)\bigr)$
 with $(-2)$-shifted symplectic form
 $\omega_{-2} = k \cdot \omega_{\mathrm{can}}$
 (the BV action is quadratic, so the critical locus is smooth
 and the Liouville form is canonical).
\item The Dirichlet boundary condition defines the zero section
 $\cL_b = \operatorname{Maps}(D^2, V)
 \hookrightarrow \cM_{\mathrm{vac}}(\cH_k)$,
 which is Lagrangian (the zero section of any shifted cotangent
 is automatically Lagrangian).
\item The derived self-intersection is the $(-1)$-shifted cotangent
 $\mathfrak{S}_b = T^*[-1]\bigl(\operatorname{Maps}(D^2, V)\bigr)$,
 and convolution recovers the Heisenberg:
 $H_*^{\mathrm{BM}}(\mathfrak{S}_b) \simeq \cH_k \simeq
 Z^{\mathrm{der}}_{\mathrm{ch}}(\cH_k)$
 (the last equivalence by Theorem~H of Volume~I at generic level,
 since $\cH_k$ is formally smooth; Heisenberg has no critical level,
 so the generic-level restriction of Theorem~H is satisfied).
\end{enumerate}
This is the chiral analogue of the abelian Steinberg: for $G = T$ a
torus the classical Steinberg variety degenerates to a point, the
Hecke algebra is commutative, and the Springer correspondence is the
identity. For the Heisenberg the self-intersection is clean
($m_k = 0$ for $k \ge 3$), the convolution is commutative up to the
level pairing, and all higher Swiss-cheese operations vanish. The
non-trivial test begins with the Virasoro algebra, where the
fourth-order pole creates excess intersection.
\end{example}
...
\begin{conjecture}[{$(-2)$-shifted symplectic structure: general HT sigma model}; \ClaimStatusConjectured]
\label{conj:minus-two-shifted-general}
...
\begin{remark}[Boundary calculation]
...
(Proposition~\ref{prop:minus-two-shifted-examples}), each verified
in its geometric incarnation. A fully rigorous argument for a general
theory $T$ requires establishing that the BV data define a
$(-1)$-shifted symplectic structure on $\mathcal{F}$ in the sense of
derived algebraic geometry; that step lies outside the present
chapter.
\end{remark}
...
and~\ref{conj:minus-two-shifted-general}) and the algebraic data
...
Proposition~\textup{\ref{conj:minus-two-shifted-general}}, and let $b$ be a rich
...
Proposition~\textup{\ref{conj:minus-two-shifted-general}}.
\end{proposition}
...
\[
 ...
 \mathbb{T}_{\cL_b}[-1]
\]
is an equivalence rather than merely a map. The
``richness'' condition on~$b$ enters here. A rich boundary condition,
...
The following makes the Koszul resolution explicit.
...
produce a coalgebra with this structure?} The usual answer is
...
of cofree coalgebras. The geometric answer is different: the bar
...
singularities; \ClaimStatusConjectured]
...
Theorem~\textup{\ref{thm:bar-is-self-intersection}}, the qualified
characterizations of Koszul concentration
\textup{(}Theorem~\textup{\ref{thm:koszul-dodecahedron}}\textup{)}
detect whether a Lagrangian self-intersection has no off-diagonal
excess Tor. The principal faces decompose as:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2em]
\item \textbf{Concentration} $=$ Borel--Moore homology of
 ...
 without excess.
\item \textbf{Diagonal quadraticity} $=$ the conormal complex is
 quadratic on the Koszul diagonal. Strict SC-formality, namely
 vanishing of all higher $\Ainf$ operations, is a smaller locus inside
 the Koszul locus.
...
\end{enumerate}
The non-formal wheel/contact sector is \emph{excess intersection}: the
Lagrangians meet along a positive-dimensional locus with a nontrivial
excess bundle~$E$, and the excess operations are the Taylor
coefficients of the class $e(E)$. The class-$\mathbf L$ Lie-Jacobi
operation is different: it is the cubic shadow of transverse Lie
collision. The curved case $d^2 = \kappaChHodge(\cA)\cdot\omega_g$ is the
...
$\Steinb$ has the cohomological profile of a smooth intersection:
no off-diagonal cohomology from excess tangencies. Strict
SC-formality, namely $m_k=0$ for all $k\geq3$, is stronger; it also
removes the finite Lie-Jacobi shadow and therefore is not equivalent to
Koszul concentration.
Each remaining face of
Theorem~\ref{thm:koszul-dodecahedron} translates by the same
dictionary; the five most geometric translations appear above.
The curved statement is the corresponding formal-local reading
over $\overline{\cM}_g$: at genus~$g \geq 1$, the Lagrangian
$\cL$ deforms with first-order displacement $\kappaChHodge(\cA)\cdot
...
\item \textbf{Theorem~D} \textup{(}leading coefficient\textup{)}.
 The curvature $\kappaChHodge(\cA) \cdot \omega_g$ is the first-order
 ...
 Kodaira--Spencer class $\kappaChHodge(\cA) \cdot \omega_g
...
the explicit computation gives $\kappaChHodge(\cA) \cdot \omega_g$.
...
Lagrangian $\cL$ together with the additional Darboux datum. The
precise formulation:
...
Let $(\openFactCat\big|_{\logCurve}, \cD^{\mathrm{cl}}\big|_X; b, A_b, \dots)$
be a bicoloured primitive package, and write
$\cA = A_b = \mathrm{End}_{\openFactCat}(b)$ for its open-colour
...
first. The open-colour endomorphism algebra $A_b$ extracted from the
bicoloured primitive package (the boundary datum) fixes
...
\textup{(}as summarized in the derived-symplectic conclusion of Volume~I\textup{)}, the open-colour endomorphism algebra $A_b$ of the bicoloured primitive package determines a
...
Proposition~\ref{conj:minus-two-shifted-general}). Geometrically,
...
\begin{conjecture}[Convolution is the bar composition;
\ClaimStatusConjectured]
\label{conj:convolution-bar}
...
\item \textbf{Bar differential from convolution.}
 ...
 OPE residue on $\FM_2(\C)$. This is the bar
...
Proposition~\ref{conj:minus-two-shifted-general}). Under
...
\begin{conjecture}[Swiss-cheese directionality from correspondence
geometry; \ClaimStatusConjectured]
\label{conj:directionality-correspondence}
...
\begin{conjecture}[Geometric Steinberg convolution identification; \ClaimStatusConjectured]
\label{conj:steinberg-convolution}
...
\item the convolution product corresponds to the bar differential
 $d_{\barB}$ \textup{(}Proposition~\textup{\ref{conj:convolution-bar}(i)}\textup{)};
\item the diagonal coproduct corresponds to the bar coproduct
 $\Delta$ \textup{(}Proposition~\textup{\ref{conj:convolution-bar}(ii)}\textup{)};
\item the Swiss-cheese directionality is enforced by the product
 decomposition of the iterated Steinberg
 \textup{(}Proposition~\textup{\ref{conj:directionality-correspondence}}\textup{)};
...
are established in Proposition~\ref{conj:minus-two-shifted-general}
(the $(-2)$-shifted symplectic structure) and the Lagrangian
analysis of the boundary locus. Ingredient~(c) (that convolution
on the Steinberg reproduces the $E_1$ dg coalgebra structure) is the
content of
Propositions~\ref{conj:convolution-bar}
and~\ref{conj:directionality-correspondence} above.
...
\section{Six distinctions: third vertex, absolute versus boundary-adapted, complement independence, exact elimination, trivial blocks, formal type}

Six distinctions structure the local theory.

\subsection*{Identifying the third vertex}
The third vertex of the triangle is not $\cA^!_{\mathrm{line}}$ itself: it is the derived center of the line category, equivalently the chiral Hochschild cochains of $\cA^!_{\mathrm{line}}$. The correction appears globally in \eqref{eq:global-corrected-triangle} and locally in Theorem~\ref{thm:local-bulk-line}.

\subsection*{Absolute versus boundary-adapted line algebras}
The exact local line algebras $K_{F,p}$ and $K_\kappa$ are boundary-adapted and pointed; they are local charts on the line sector, distinct from the global absolute open-colour algebra $A^!_{\mathrm{line}}$.

\subsection*{Independence from the chosen complement}
The intrinsic boundary algebra $B^\intr_{L,W}$ removes the complement dependence at the root. The boundary-linear exact sector then appears as the strict part of the intrinsic theory.

\subsection*{Exact elimination in place of homological perturbation}
Boundary Kuranishi elimination is an exact algebraic theorem, not a perturbative transfer. It isolates the singular residue canonically.

\subsection*{Universal trivial blocks made visible}
Contractible tangent-normal blocks and hyperbolic Clifford blocks separate cleanly: the former are contractible; the latter are Morita-trivial after spinor stabilization. The remaining data is the genuine local content.

\subsection*{Pointed formal type versus stable category}
The one-step Jacobi coalgebra classifies the pointed local formal germ. The stabilization theorems characterise the invariants of the stable open sector. The line Mather--Yau theorem classifies the reduced microlocal formal type.

\subsection*{The resulting shape}

After these corrections, the theory takes the form:
\[
...
\]
That microlocal backbone supports the global theorem.


\section{Bulk-boundary-line triangle and its boundary-linear exact heart}

On the global side, the corrected theorem target has a precise
hypothesis set: compact generation of the boundary vacuum, the
...
\begin{conjecture}[Universal correlator from shadow obstruction tower;
 \ClaimStatusConjectured]
\label{conj:correlator-from-shadow-tower}
...
\section{Notation: boundary--bulk--line dictionary}
...
\item On the perturbative chirally Koszul/quasi-linear locus with
 pointed completion, the line category is equivalent to modules for
 the open-colour Koszul dual~$\cA^!_{\mathrm{line}}$
...
\begin{remark}
The theorem identifies the structural reach of the correct object: a
modular envelope alone does not suffice. The literature forces four
structures to coexist: cyclic deformation theory, Koszul duality,
modular sewing, and meromorphic line-operator transport. A bare
modular envelope captures only the sewing part.
\end{remark}
...
coefficient $k/z$, $m_k=0$ for $k\ge 3$, and
...
$\nabla^{\mathrm{hol}}_{0,n}$ identifies with the KZ connection
...
At the scalar level,
$F_1(\cA)=\kappaChHodge(\cA)\lambda_1^{\mathrm{FP}}
=\kappaChHodge(\cA)/24$.
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{llll}
\textbf{Family} & $\kappaChHodge$ & $F_1=\kappaChHodge/24$
...
%: Conjectural definition. Existence asserted in Conjecture~\ref{conj:m2-direct-realization}.
\begin{definition}[$M2$ master lift (conjectural); \ClaimStatusConjectured]
\label{def:m2-master-lift}
\emph{Conditionally on existence (Conjecture~\ref{conj:m2-direct-realization})}, the full $M2$ ambient lift is the Maurer--Cartan element
\[
\Theta_{M2}
\in
...
the reflected current presentation has level $-k-2h^\vee=-1-6=-7$.
...
$F_1(\beta\gamma_1)
=\kappaChHodge(\beta\gamma_1)\lambda_1^{\mathrm{FP}}
=1 \cdot 1/24=1/24$.
The positive sign reflects $\kappa>0$: the $\beta\gamma_1$ system has
a bosonic genus tower. The full genus expansion
$\sum_{g\ge 1}F_g\,x^{2g}=(\hat A(ix)-1)$
...
\section{K3 Mukai-doubling conductor and the Hall--Drinfeld double bridge}
\label{sec:htbb-frontier-supplement}

\begin{remark}[HT bulk-boundary line frontier at K3]
\label{rem:htbb-frontier-K3}
At the HT bulk-boundary line frontier, $K3 \times E$ targets the
Vol~III K3 chiral Hall--Drinfeld double on the boundary side after
the compact CoHA source, orientation system, Hopf pairing, and Hall
bracket comparison are supplied. The Mukai-doubling Koszul conductor
$K^{\kappaChHodge}(\mathcal{H}_{\mathrm{Muk}}(K3))
=2\,c_+(\mathrm{Mukai}(K3))=8$ has three faces via Bruinier $2002$
Proposition~$5.1$: Mukai doubling, Humbert $H_1$ monodromy order,
Lusztig $\ell=8$ at $\zeta^8=1$. Universal identity
$\hbar^2\cdot K^{\kappaChHodge}=-1$.
\end{remark}
