...
of $\cL$---the formal completion at the vacuum.  The cofree
coalgebra is the symmetric algebra on the fibers of the conormal
bundle.  Nothing is imposed; everything is inherited.
\end{remark}
...
equivalence at the vacuum).  The conjecture is not a statement to be
verified---it is a \emph{definition} of $\cM_{\mathrm{vac}}(T)$ and
$\cL_b$ in terms of the bar complex.  The content of the conjecture
reduces to the statement that this definition agrees with the
physical construction of $\cM_{\mathrm{vac}}(T)$ from BV data.
...
$\mathrm{Crit}(\alpha) = \cL_\cA$ (the zero section).  By the
formal Darboux theorem for $(-2)$-shifted symplectic stacks
(Brav--Bussi--Joyce~\cite{BBJ19}, Theorem~4.1), every $(-2)$-shifted
symplectic derived stack is, in the formal neighborhood of a
Lagrangian, equivalent to such a local model: $T^*[-1]\cL$
with a $(-1)$-shifted closed $1$-form whose critical locus
is~$\cL$.  In the zero-form case ($\alpha = 0$), $\cM(\cA)$
reduces to $T^*[-1]\cL_\cA$ with the $(-1)$-shifted Liouville
structure; the $(-2)$-shifted symplectic structure on the
physical vacuum moduli $\cM_{\mathrm{vac}}$ is encoded in the
non-trivial $1$-form $\alpha$ on this local model.
