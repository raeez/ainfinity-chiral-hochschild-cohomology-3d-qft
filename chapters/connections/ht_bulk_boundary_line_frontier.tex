...
The local theorem package is in place.  The global theorem reduces to two comparison maps and a geometric endpoint for the full line algebra.
...
\item \textbf{Genus tower from curvature.}
  ...
  and the shadow Postnikov obstruction tower
...
\item The derived self-intersection is the $(-1)$-shifted cotangent
  ...
  (the last equivalence by Theorem~H of Volume~I, since $\cH_k$ is
  formally smooth).
\end{enumerate}
...
\item \emph{Genus face.}  At genus~$g \ge 1$, the restriction
  ...
  shadow Postnikov obstruction tower $\Theta_{\Bbound}^{\le r}$ of Vol~I.
...
\item \emph{The shadow Postnikov obstruction tower.}
...
(flat at all genera).
Shadow obstruction tower terminates at arity~$2$; all nonlinear
shadows vanish.
\end{computation}
...
% RECTIFICATION-FLAG: This is a conjectural definition — the object Theta_{M2}
% is defined by its expected properties, not by an explicit construction.
% Per AP43, a central object must be defined before use. The conjecture
% that follows (conj:m2-direct-realization) is what asserts existence.
...
$F_1(\beta\gamma_1)=\kappa \cdot \lambda_1^{\mathrm{FP}} = 1 \cdot 1/24=1/24$.
