...
(the Lagrangian condition: the conormal is the shifted tangent).
...
\]
The augmentation ideal $\bar B = \ker(\varepsilon\colon B \to \kk)$
provides a coalgebra generator via $s^{-1}\bar B \simeq
\mathbb{L}_{\cL/\cM}[1]^\vee[-1]$, and the symmetric coalgebra
on the fibers is the cofree tensor coalgebra:
$\Sym^c(\mathbb{T}_\cL) \simeq T^c(s^{-1}\bar B)$.
The derived tensor product is therefore computed by the bar complex:
\[
B \otimes^{\mathbb{L}}_{\cO(\cM)} B
\;\simeq\;
T^c(s^{-1}\bar B, \, d_{\barB}).
\]
This is the standard identification of the derived self-intersection
...
(iii) The HKR identification is the classical statement that
Hochschild cohomology of a smooth commutative algebra equals
polyvector fields, generalized to the derived setting.  For
a Lagrangian $\cL \hookrightarrow \cM$, the Lagrangian condition
$\mathbb{L}_{\cL/\cM} \simeq \mathbb{T}_\cL[-1]$ gives
\[
T^*[-1]\cL
\;\simeq\;
\cL \times^h_\cM \text{(first-order neighborhood of $\cL$ in $\cM$)}
\;\simeq\;
\cM|_{\cL},
\]
where the last equivalence uses that the Lagrangian determines the
ambient symplectic stack locally.  This is
Theorem~\ref{thm:boundary-linear-bulk-boundary} of
