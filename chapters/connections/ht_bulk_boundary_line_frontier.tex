...
\item the \emph{absolute} line algebra $A^!$ of the global theory, presenting all line operators on the Koszul locus (\cite{DNP25}; for the affine lineage, $\cC_{\mathrm{line}}^{\mathrm{red}}|_{\mathrm{eval}} \simeq \mathrm{Rep}_q(\fg)$ by Theorem~\ref{thm:affine-monodromy-identification});
...
\end{enumerate}
The latter are local exact models: boundary charts on the absolute line category $\cC_{\mathrm{line}}$, not globally equal to $A^!$, but the local testing ground in which the global theorem becomes rigid.
...
\item The dg-shifted-Yangian package of \cite{DNP25} has exactly
  ...
  $r$-matrix
  (the dg-shifted Yangian is a strict factorization quantum group
  by Theorem~\ref{thm:complete-strictification}).
\end{itemize}
...
three separate constructions assembled toward a conjecture.  They
are three readings of a single fact, and that fact is already proved.

\begin{theorem}[The bar complex is the Lagrangian self-intersection;
\ClaimStatusProvedHere]
...
\item The structure sheaf of $\Steinb$ is the
  \emph{bar construction} on~$B$:
  ...
  transverse\textup{)}, and the cofree coalgebra structure encodes
  the diagonal
...
\item Under the HKR identification, Hochschild cochains of~$B$
  ...
  \cO(\cM)|_{\cL}.
  \end{equation}
  This recovers the ambient stack from the Lagrangian: the bulk
  is the derived center of the boundary.
\end{enumerate}
\end{theorem}
...
structure.  The derived tensor product is therefore computed
by the bar complex:
\[
...
structure sheaf of $\Steinb$ as a \emph{formal groupoid}: the
...
$\cL$.  In particular, the \emph{functions} on the formal
neighborhood of $\cL$ in $\cM$ are controlled by $\cO(T^*[-1]\cL)$:
\[
\cO(\hat{\cM}_\cL)
\;\simeq\;
\cO(T^*[-1]\cL)
...
\cO(\cM)|_{\cL}$, where the restriction is understood as the
formal completion at $\cL$.  The shifted symplectic structure on
...
construction produces a coalgebra because it is computing the
structure sheaf of a \emph{self-intersection}, and self-intersections
...
self-intersection}, arising because the ambient space is a product
...
Theorem~\ref{thm:bar-is-self-intersection} inverts the logic: the
bar complex \emph{defines} the self-intersection, which
\emph{determines} the Lagrangian and the ambient stack (up to formal
equivalence at the vacuum).  The \emph{existence} of the geometric data is not a
statement to be verified---it is a \emph{definition} of
$\cM_{\mathrm{vac}}(T)$ and $\cL_b$ in terms of the bar complex.
What remains nontrivial is the \emph{agreement} between this
defined stack and the physical construction of
$\cM_{\mathrm{vac}}(T)$ from BV data: the conjecture reduces to
this agreement.
For the three worked examples
(Proposition~\ref{prop:minus-two-shifted-examples}), this agreement
is verified directly.  In general, it is the content of axiom~(H1).
\end{remark}
...
\item \textbf{Spectral collapse} $=$ PBW exactness $=$ no excess
  ...
  $\cO(\Steinb)$ from the associated graded collapses at $E_2$
...
\end{enumerate}
Non-Koszulness is \emph{excess intersection}: the Lagrangians
...
$\cO(\Steinb) \simeq \barB(B)$, so the bar cohomology computes
the sheaf cohomology of the structure sheaf of the
self-intersection.  Concentration of $H^{p,q}(\barB)$ in a single
degree is exactly the statement that $\Steinb$ has the
cohomological profile of a smooth (clean) intersection: no
higher cohomology from excess tangencies.  Formality ($m_k = 0$
for $k \geq 3$) means the $\Ainf$-structure on $\cO(\Steinb)$
...
The curved statement follows from
Theorem~\ref{thm:bar-is-self-intersection} applied fiberwise
...
Under the Lagrangian self-intersection dictionary of
...
\item \textbf{Theorem~H} \textup{(}Hochschild ring\textup{)}.
  ...
  on $T^*[-1]\cL \simeq \cM|_\cL$: the bulk algebra is the
  formal neighborhood of the boundary inside the ambient
  symplectic stack.  The BV bracket on $\HH^\bullet(B)$ is the
...
$\cL \to \cL \times_\cM^h \cL$.  Under $\cO(\Steinb) \simeq
\barB(B)$, the groupoid diagonal is the deconcatenation
coproduct $\Delta$ and the unit is the augmentation.  The
adjunction between $\cO(\cL)$-modules and $\cO(\Steinb)$-comodules
is the bar-cobar adjunction of Vol~I, Theorem~A\@.

(B) When the self-intersection is clean ($\Steinb$ smooth), the
groupoid is \'{e}tale, and Morita theory gives
$\mathrm{QCoh}(\cL) \simeq \mathrm{CoMod}(\cO(\Steinb))$.  The
...
\mathbb{T}_\cL[-1]$ identifies $T^*[-1]\cL$ with the formal
neighborhood of $\cL$ in~$\cM$, giving $\HH^\bullet(B) \simeq
\cO(\cM)|_\cL$.  The BV bracket is the $(-1)$-shifted Poisson
...
%% ================================================================
\subsection{Holographic reconstruction: the boundary creates the bulk}
\label{ssec:holographic-reconstruction}

The five-theorem proposition reveals that the ambient
$(-2)$-shifted symplectic stack $\cM$ is \emph{determined}
by the Lagrangian $\cL$.  We now make this precise.
...
Then $\cA$ determines, up to formal equivalence at the vacuum, a
$(-2)$-shifted symplectic derived stack $\cM(\cA)$ and a Lagrangian
embedding $\cL_\cA \hookrightarrow \cM(\cA)$ such that
$\cO(\cL_\cA \times_{\cM(\cA)} \cL_\cA) \simeq \barB(\cA)$.
\end{theorem}

\begin{proof}
The reconstruction is an application of Theorem~\ref{thm:bar-is-self-intersection}(iii) read in reverse.
...
We define the ambient stack $\cM(\cA)$ as the total space of
$T^*[-1]\cL_\cA$ equipped with the additional datum of a
$(-1)$-shifted closed $1$-form $\alpha$ such that
$\mathrm{Crit}(\alpha) = \cL_\cA$ (the zero section).
The formal Darboux theorem for shifted symplectic stacks
(Brav--Bussi--Joyce~\cite{BBJ19}, Theorem~4.1) establishes
a bijection: $(-2)$-shifted symplectic structures on a formal
neighborhood of a Lagrangian $\cL$ correspond to
$(-1)$-shifted closed $1$-forms on $T^*[-1]\cL$ whose
derived critical locus is~$\cL$.  In particular, every such
pair $(T^*[-1]\cL, \alpha)$ determines a $(-2)$-shifted symplectic
structure, and conversely.  The $(-2)$-shifted structure is
\emph{not} the canonical $(-1)$-shifted Liouville form on
$T^*[-1]\cL$ (which has the wrong shift); rather, it is the
additional datum encoded by~$\alpha$.  When $\alpha = 0$, the
$(-2)$-shifted structure degenerates and $\cM(\cA)$ reduces to
$T^*[-1]\cL_\cA$ with its $(-1)$-shifted symplectic structure
alone; the non-trivial physical content of vacuum moduli
$\cM_{\mathrm{vac}}$ requires $\alpha \neq 0$.

The zero section $\cL_\cA \hookrightarrow \cM(\cA)$
is Lagrangian: the conormal of the zero section satisfies
$\mathbb{L}_{\cL/\cM} \simeq \mathbb{T}_\cL[-1]$
(the Lagrangian condition for $(-2)$-shifted symplectic targets).
By Theorem~\ref{thm:bar-is-self-intersection}(ii), the
self-intersection of this zero section is $\cO(\cL \times_\cM \cL)
\simeq T^c(s^{-1}\bar B) = \barB(\cA)$.

For uniqueness: any $(-2)$-shifted symplectic $\cM'$ with
Lagrangian $\cL' \simeq \cL_\cA$ and $\cO(\cL' \times_{\cM'}
\cL') \simeq \barB(\cA)$ has formal completion at $\cL'$
determined by the formal Darboux data $(T^*[-1]\cL', \alpha')$,
and is therefore formally equivalent to $\cM(\cA)$ at the
vacuum (Brav--Bussi--Joyce~\cite{BBJ19}, Theorem~4.1).
The uniqueness is up to formal equivalence in the formal
neighborhood of $\cL$, not up to global equivalence.
\end{proof}
...
first.  The chiral algebra $\cA$ (the boundary datum) \emph{creates}
its own ambient $(-2)$-shifted symplectic stack $\cM(\cA)$ via the
HKR/shifted-cotangent construction.  Axioms~(H1)--(H4) are not
the existence of the bulk---that is guaranteed---but the
\emph{agreement} of the formal reconstruction with the physical
BV construction.

In the language of the derived symplectic category
(\S\ref{sec:derived-symplectic-conclusion}), the chiral algebra is a
\emph{point} $\cL_\cA$ in $\mathsf{Symp}^{\mathrm{der}}_{-2}$,
and the point determines its ambient object $\cM(\cA)$ up to
formal equivalence.  The holographic principle---that boundary
data determines bulk data---is the formal Darboux theorem for
...
$\cA = \cA_\partial(\cT)$ satisfying \textup{(H1)--(H4)}, and suppose
...
for the Koszul dual~$A^!$---a dg-shifted Yangian
(Theorem~\ref{thm:Koszul_dual_Yangian})---with $r$-matrix
