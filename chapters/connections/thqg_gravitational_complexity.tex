...
\label{sec:thqg-gravitational-complexity}
\label{ch:thqg-gravitational-complexity}% alias for dangling \ref{ch:...}
\index{gravitational complexity|textbf}
\index{support shadow depth!gravitational interpretation}
...
is the sequence of degree projections of the bar-intrinsic MC element
...
(Definition~\ref{V1-def:nms-genus-loop-operator}).

\begin{definition}[Gravitational obstruction class]
\label{def:thqg-gravitational-obstruction}
\index{gravitational obstruction class|textbf}
\index{obstruction class!gravitational}
The \emph{gravitational obstruction class at degree~$r{+}1$} is
...
\end{equation}
vanishes if and only if the tower extends from degree~$r$ to
degree~$r{+}1$ without new data.
\end{definition}

\begin{proposition}[Two sources of gravitational obstruction]
\label{prop:thqg-two-sources}
...
\item The \emph{bracket term}
 $\bigl[\Theta_\cA^{\leq r},\,\Theta_\cA^{\leq r}\bigr]_{r+1}$:
 a sum over connected stable graphs of total degree~$r{+}1$ with
...
\item The \emph{genus-loop term}
 ...
 the BV operator $\Lambda_P$ applied to the lower-degree shadow,
...
\begin{proof}
Connected graphs contributing to degree~$r{+}1$ are of two types:
multi-vertex graphs (all edges internal, total degree = sum of
vertex degrees minus number of internal edges), and single-vertex
graphs with one self-loop. These are the only contributions to
the MC equation at weight~$r{+}1$ in the modular convolution
algebra (Lemma~\ref{V1-lem:graph-sum-truncation}).
\end{proof}

\begin{remark}[The first four obstruction classes]
% label removed: rem:thqg-first-four
\index{obstruction class!explicit low-degree}
...
% ======================================================================
% §2. SUPPORT DEPTH AND THE COMPLEXITY INVARIANT
% ======================================================================

\subsection{Support depth as a gravitational invariant}
...
\index{support shadow depth!as gravitational invariant}
Let $\cA$ be a cyclically admissible chiral algebra
(Definition~\ref{V1-def:cyclically-admissible}).
...
r^\Theta_{\max}(\cA)
\;=\;
\inf\{r \geq 2 : \Theta_{\cA,n}=0\ \text{for all}\ n>r\}
\;\in\;
\{2, 3, 4, \ldots\} \cup \{\infty\},
\end{equation}
where \(\Theta_{\cA,n}\) is the degree-\(n\) component of the
bar-intrinsic Maurer--Cartan packet in the chosen gauge component;
the infimum of the empty set is \(\infty\).
The associated obstruction depth is
\[
d_{\mathrm{obs}}(\cA):=
\inf\{r\ge2:o_{r'+1}(\cA)=0\ \text{for every}\ r'\ge r\}.
\]
It satisfies \(d_{\mathrm{obs}}(\cA)\le r^\Theta_{\max}(\cA)\).
Obstruction depth only records eventual \(H^2\)-vanishing; it does
not set the higher \(H^1(J^n(\cA),d_2)\)-lift coordinates to zero.

Two related homotopy-theoretic support invariants are the
\(A_\infty\)-support depth \(d^\Theta_\infty(\cA)\) and the
\(L_\infty\)-support depth \(f^\Theta_\infty(\cA)\).
...
f^\Theta_\infty(\cA),
\end{equation}
where \(d^\Theta_\infty\) is the last nonzero transferred
\(A_\infty\)-operation in the bar-intrinsic packet and
\(f^\Theta_\infty\) is the last nonzero transferred genus-$0$
\(L_\infty\)-bracket in the same packet.
\end{definition}

\begin{theorem}[Operadic complexity identification]
\label{thm:operadic-complexity}
\ClaimStatusConditional
\index{operadic complexity!identification theorem|textbf}%
\index{support shadow depth!equals $A_\infty$-support depth equals $L_\infty$ support depth}%
For any chirally Koszul algebra~$\cA$ satisfying the
shadow--formality comparison hypotheses of
Theorem~\textup{\ref*{V1-thm:operadic-complexity-detailed}},
\begin{equation}\label{eq:operadic-complexity-id}
r^\Theta_{\max}(\cA)
\;=\;
d^\Theta_\infty(\cA)
\;=\;
...
\(d_{\mathrm{obs}}\) is a separate invariant.
\end{theorem}

\begin{proof}
By induction on degree. At genus~$0$, stable graphs are trees.
The HPL tree formula computes the transferred $A_\infty$-structure
$\{m_k^H\}$ on the minimal model~$H$: each tree with $k$ leaves has
$k-1$ vertices decorated by~$m_2$ and $k-2$ internal edges
decorated by the contracting homotopy~$h$. This is the same
combinatorial sum as the shadow recursion
$S_r = \sum_{j} (-1)^{r-j} S_j S_{r+1-j}$
(Theorem~\ref{thm:period-2-parity}). Hence
\(m_k^H = 0 \Leftrightarrow \Theta_{\cA,k}=0\) in the selected
bar-intrinsic packet, giving
\(d^\Theta_\infty = r^\Theta_{\max}\).

For the second equality, the transferred $L_\infty$-brackets
$\{\ell_k^{(0),\mathrm{tr}}\}$ on cohomology are obtained from
the $A_\infty$-structure by antisymmetrisation (Kadeishvili).
On the Koszul locus, the bar spectral sequence degenerates at
$E_2$ (PBW), so the antisymmetrisation is an isomorphism on the
relevant weight spaces. Therefore \(\ell_k^{(0),\mathrm{tr}} = 0\)
if and only if \(m_k^H = 0\) in the cyclic-invariant sector, giving
\(f^\Theta_\infty = d^\Theta_\infty\).
\end{proof}
...
stabilises at degree \(r^\Theta_{\max}(\cA)\):
\begin{equation}% label removed: eq:thqg-stabilization
\Theta_\cA^{\leq r}
\;=\;
\Theta_\cA^{\leq r^\Theta_{\max}(\cA)}
\quad \text{for all } r \geq r^\Theta_{\max}(\cA),
\qquad
o_{r+1}(\cA) = 0.
\end{equation}
The gravitational $L_\infty$-algebra of the bulk HT theory
has interaction vertices in this packet of degree at most
\(r^\Theta_{\max}(\cA)\).
Conversely: a non-vanishing $r$-ary bracket in the bulk
forces \(r^\Theta_{\max}(\cA) \geq r\).
\end{theorem}

\begin{proof}
By the operadic complexity theorem of Vol~I
\textup{(}Theorem~\textup{\ref*{V1-thm:operadic-complexity-detailed}}\textup{)},
\(r^\Theta_{\max}(\cA) = f^\Theta_\infty(\cA)\), so the transferred genus-$0$
$L_\infty$ brackets satisfy
\(\ell_s^{(0),\mathrm{tr}} = 0\) for every
\(s > r^\Theta_{\max}(\cA)\).
By the all-degree shadow--obstruction comparison of Vol~I
...
every degree, so the bracket term of the obstruction tower vanishes
beyond \(r^\Theta_{\max}(\cA)\). The genus-loop term vanishes because
$\Lambda_P$ maps $\operatorname{Sym}^s(V^*) \to
\operatorname{Sym}^{s-2}(V^*)$ and the source is zero for
\(s > r^\Theta_{\max}(\cA)\). Hence the packet stabilises at degree
\(r^\Theta_{\max}(\cA)\), and the bulk HT theory has no interaction
vertices above that degree in the chosen gauge component. Conversely,
a non-vanishing $r$-ary bulk bracket
forces $\ell_r^{(0),\mathrm{tr}} \neq 0$, hence
\(f^\Theta_\infty(\cA) \ge r\), and therefore
\(r^\Theta_{\max}(\cA) \ge r\) by the same operadic complexity theorem.
\end{proof}

\begin{proposition}[Support depth is a filtered quasi-isomorphism invariant]
% label removed: prop:thqg-depth-qi-invariant
\ClaimStatusProvedHere
\index{support shadow depth!quasi-isomorphism invariance}
Let \(f\colon \cA \xrightarrow{\sim} \cA'\) be a filtered
quasi-isomorphism of cyclic chiral algebras which identifies the
chosen bar-intrinsic Maurer--Cartan gauge components. Then
\[
r^\Theta_{\max}(\cA)=r^\Theta_{\max}(\cA').
\]
\end{proposition}
...
By the operadic complexity theorem of Vol~I
\textup{(}Theorem~\textup{\ref*{V1-thm:operadic-complexity-detailed}}\textup{)},
\(r^\Theta_{\max}=d^\Theta_\infty\). Filtered cyclic homotopy
transfer identifies the minimal \(A_\infty\)-models and their
transferred operations in the selected MC component. Hence
\(d^\Theta_\infty(\cA)=d^\Theta_\infty(\cA')\), and therefore
\(r^\Theta_{\max}(\cA)=r^\Theta_{\max}(\cA')\).
\end{proof}
...
The classification uses the support of the bar-intrinsic
Maurer--Cartan packet.  The obstruction classes \(o_n\) decide
extendability; the vanishing of the higher \(H^1\)-lift coordinates
decides whether the packet itself has finite support.

\begin{lemma}[Cubic source permanence]
\label{lem:thqg-cubic-source}
\ClaimStatusProvedHere
...
\begin{proof}
Two-vertex graphs with degrees $(3, r)$ contribute
...
\begin{theorem}[Standard-family support-depth classification]
\label{thm:thqg-four-class}
\ClaimStatusProvedHere
\index{gravitational complexity!four-class theorem|textbf}
\index{support shadow depth!classification!gravitational}
On the canonical bar-intrinsic branches of the standard landscape,
the support-depth classes are:
\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{clcp{0.30\textwidth}p{0.24\textwidth}}
\toprule
\emph{Class} & \emph{Name} & \(r^\Theta_{\max}\) &
\emph{support packet} & \emph{lift-coordinate condition} \\
\midrule
$\mathbf{G}$ & Gaussian &
 $2$ &
 \(\Theta_{\cA,n}=0\) for \(n>2\) &
 \(\lambda_n=0\) for \(n>2\). \\
$\mathbf{L}$ & Lie &
 $3$ &
 \(\Theta_{\cA,3}\neq0\), \(\Theta_{\cA,n}=0\) for \(n>3\) &
 \(\lambda_n=0\) for \(n>3\). \\
$\mathbf{C}$ & Contact &
 $4$ &
 cubic gauge-trivial, \(\Theta_{\cA,4}\neq0\), later support zero &
 \(\lambda_n=0\) for \(n>4\). \\
$\mathbf{M}$ & Mixed &
 $\infty$ &
 infinitely many nonzero \(\Theta_{\cA,n}\) &
 no finite lift cutoff.\\
\bottomrule
\end{tabular}
\end{center}
In the first three rows the obstruction depth is finite and agrees
with the displayed support depth only because the canonical
gauge-slice \(H^1\)-lift coordinates above the displayed degree
vanish. In the mixed row the Virasoro cubic source gives nonzero
support components in arbitrarily high degree at generic central
charge. This is not a classification of arbitrary components of
the full filtered MC moduli problem; outside these canonical rows
obstruction depth and support depth may differ.
\end{theorem}

\begin{proof}
Case analysis is made on the canonical standard-family packets,
not on the obstruction classes alone. Proposition~\ref{prop:thqg-two-sources}
and Lemma~\ref{lem:thqg-cubic-source} control the obstruction
sources; the displayed \(H^1\)-lift-coordinate vanishings are the
additional support-depth input.

\smallskip
\noindent\emph{Gaussian row.}
\(r^\Theta_{\max}=2\). The bracket term at degree~$3$ vanishes
($\ell_3^{(0),\mathrm{tr}} = 0$: abelian OPE).
With $\mathfrak{C} = 0$ and $\mathfrak{Q} = 0$, both
sources vanish at all higher degrees by induction, and the
canonical lift coordinates above degree~$2$ are zero.
Class~$\mathbf{G}$.

\smallskip
\noindent\emph{Lie row.}
\(r^\Theta_{\max}=3\). The cubic shadow \(\mathfrak{C}\neq0\).
The quartic obstruction $o_4 = 0$: the two contributions
at degree~$4$ (the bracket term and the loop) cancel by
the Jacobi identity. With $\mathrm{Sh}_4 = 0$, the cubic
source gives $\{\mathfrak{C}, 0\}_H = 0$ at degree~$5$.
The canonical lift coordinates above degree~$3$ vanish, so the
support depth is \(3\). Class~$\mathbf{L}$.

\smallskip
\noindent\emph{Contact row.}
\(r^\Theta_{\max}=4\). After cubic gauge trivialisation the
quartic contact packet \(\mathfrak{Q}\neq0\) is the first
nonlinear support component. The cubic source permanence is
inactive. The remaining source at degree~$5$ is
$[\Theta^{\leq 4}, \Theta^{\leq 4}]_5$: with
$\mathfrak{C} = 0$, the only available vertex of
degree~$> 2$ is $\mathfrak{Q}$ at degree~$4$. For
rank-one cyclic slices, the contributions vanish by the
rank-one rigidity theorem (Theorem~\ref{V1-thm:nms-rank-one-rigidity},
proved in Vol~I\@). The canonical lift coordinates above degree~$4$
vanish. Hence the support depth is \(4\).
Class~$\mathbf{C}$.

\smallskip
\noindent\emph{Mixed row.}
\(r^\Theta_{\max}=\infty\). Both $\mathfrak{C} \neq 0$ and
$\mathfrak{Q} \neq 0$.
...
\smallskip
\noindent\emph{Exhaustiveness on the standard landscape.}
The standard rows are the Heisenberg/lattice, affine, contact
\(\beta\gamma\), and Virasoro/\(\mathcal W_N\) branches. The
rank-one contact branch excludes a surviving quintic support
coordinate after cubic trivialisation; the mixed Virasoro branch
has the cubic source and therefore never reaches a finite support
cutoff. Other components of the filtered MC moduli problem require
their own obstruction and lift-coordinate analysis.
\end{proof}
...
Under the scoped holographic comparison, on the canonical
...
\begin{proof}
The degree-$r$ component $\Theta_{\cA,r}$ determines the $r$-ary
$L_\infty$ bracket of the bulk theory via
the low-degree shadow--formality dictionary of Vol~I
\textup{(}Proposition~\textup{\ref*{V1-prop:shadow-formality-low-degree}}\textup{)}.
Stabilization at \(r^\Theta_{\max}\) truncates the \(L_\infty\)-structure
in the chosen packet.
Class~$\mathbf{G}$: $\Theta_\cA = \kappaChHodge(\cA) \cdot \eta \otimes
\Lambda$ is purely quadratic; the potential
$S_\cA(x) = \frac{1}{2}\kappaChHodge(\cA)\,x^2$ is Gaussian.
Class~$\mathbf{L}$: the cubic shadow gives $\ell_3 \neq 0$
(Chern--Simons-type vertices); the Jacobi identity kills $\ell_4$.
Class~$\mathbf{C}$: after cubic gauge trivialisation, the quartic
contact packet \(\mathfrak{Q} \neq 0\) gives \(\ell_4\neq0\);
the potential is quartic. Class~$\mathbf{M}$: both $\mathfrak{C} \neq 0$ and
$\mathfrak{Q} \neq 0$; infinite tower, non-polynomial potential.
By the operadic complexity theorem of Vol~I,
\(r^\Theta_{\max}(\cA)=\infty\) forces
\(f^\Theta_\infty(\cA)=\infty\), so the bulk theory has
non-vanishing higher brackets at infinitely many degrees in this
canonical branch.
\end{proof}
...
$S_\cA(x) = \frac{1}{2}\kappaChHodge(\cA)\,x^2$.
...
\emph{Algebra} & $\kappa$ & $o_3$ & $o_4$ & $o_5$ &
 $o_6$ & $\mathrm{Sh}_3$ & $\mathrm{Sh}_4$ & \(r^\Theta_{\max}\) \\
\midrule
...
\end{tabular}
\end{center}
The lattice value $\kappaChHodge(V_\Lambda) = \mathrm{rank}(\Lambda)$
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\mathfrak{C}(\cA) = \kappaChHodge(\cA)\cdot f_\fg\cdot x^3$,
 where $f_\fg$ is the normalised structure constant.
\item $o_4(\cA) = 0$: the Jacobi identity kills all
 degree-$4$ graph contributions.
\item \(o_r(\cA)=0\) for all \(r\ge4\), and
 \(r^\Theta_{\max}(\cA)=3\) on the canonical affine branch.
\end{enumerate}
\end{theorem}
...
degree~$5$ gives $\{\mathfrak{C}, 0\}_H = 0$. Induction.
\end{proof}
...
\item $\mathfrak{Q}(\beta\gamma) \neq 0$ on the
 ...
 \textup{(}Theorem~\ref{V1-thm:betagamma-quartic-birth}\textup{)}.
\item $o_5(\beta\gamma) = 0$: rank-one rigidity
 \textup{(}Theorem~\ref{V1-thm:nms-rank-one-rigidity}\textup{)}.
\item \(o_r(\beta\gamma)=0\) for \(r\ge5\) on the
 weight-changing line, and \(r^\Theta_{\max}(\beta\gamma)=4\)
 on the canonical contact branch.
\end{enumerate}
\end{theorem}
...
$:\!\beta\partial\gamma\!:$
(Theorem~\ref{V1-thm:betagamma-quartic-birth}).
Part~(iii): With $\mathfrak{C} = 0$, the cubic source is
inactive. The remaining degree-$5$ source is
...
\textup{(}abelian OPE forces all degree-$3$ cocycles to be
...
$\mathfrak{g} = \gAmod$ with the degree filtration.
\end{proof}
...
\label{thm:thqg-virasoro-quintic}
\ClaimStatusProvedHere
...
\label{thm:thqg-virasoro-infinite}
\ClaimStatusProvedHere
\index{shadow tower!Virasoro infinite}
For generic $c$: $\mathrm{Sh}_r(\mathrm{Vir}_c) \neq 0$
for all $r \geq 2$, so \(r^\Theta_{\max}(\mathrm{Vir}_c)=\infty\).
\end{theorem}
...
each degree-$r$ component lives in a finite-dimensional quotient
...
\label{subsec:thqg-virasoro-tower}
\index{Virasoro!shadow tower computation}

The all-degree master equation on the one-generator primary line
...
\label{thm:thqg-virasoro-tower-explicit}
\ClaimStatusProvedHere
...
formula, verified by the Volume~II Virasoro shadow-tower engine.
\end{proof}

\begin{proposition}[Structural properties of the Virasoro tower]
\label{prop:thqg-virasoro-structure}
\ClaimStatusProvedHere
...
\item \emph{Sign alternation}:
 ...
 at $r = 5$.
\item \emph{Linear numerators at degree~$\geq 6$}:
...
$1/\kappa = 2/c$ at each recursion degree; the factor $(5c+22)$
arises from $Q_0 = 10/[c(5c+22)]$ and propagates with period~$2$.
...
\mathrm{Sh}_r(\cA)(x,\ldots,x)$.
For classes $\mathbf{G}$, $\mathbf{L}$, $\mathbf{C}$:
polynomial of degree \(r^\Theta_{\max}\).
For class $\mathbf{M}$: formal power series.
On a one-dimensional primary line write
$\mathrm{Sh}_r(\cA)(x,\ldots,x)=s_r(\cA)x^r$.
The scalar Euler primitive
$E_\cA(x)=\sum_{r\geq 2}s_r(\cA)x^r/r$
is a radial contraction of the same jets, not the Taylor potential.
\end{definition}

\begin{theorem}[Virasoro complementarity potential]
% label removed: thm:thqg-virasoro-potential
\ClaimStatusProvedHere
\begin{align}\label{eq:thqg-virasoro-potential-explicit}
S_{\mathrm{Vir}}(x)
&= \frac{c}{4}\,x^2
+ \frac{1}{3}\,x^3
...
\begin{proof}
For the Virasoro primary line the entries of
Theorem~\ref{thm:thqg-virasoro-tower-explicit} have the form
$\mathrm{Sh}_r=s_rx^r$.
The coefficient of $x^r$ in the Taylor potential is therefore
$s_r/r!$.
\end{proof}
...
Corollary~\ref{V1-cor:nms-genus-one-hessian-correction}:
...
diagnostic requires the contact flag $\chi_{\mathrm{ct}}$ and the
mixed-shell discriminant $\Delta_{\mathrm{mix}} = 8\kappa S_4$:
\begin{center}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{cccccc}
\toprule
$\sigma$ & $\pi$ & $\chi_{\mathrm{ct}}$ & $\Delta_{\mathrm{mix}}$ & \emph{Class} & \emph{Archetype} \\
\midrule
$0$ & $0$ & $0$ & $0$ & $\mathbf{G}$ & Heisenberg \\
$1$ & $1$ & $0$ & $0$ & $\mathbf{L}$ & Affine \\
$0$ & $0$ & $1$ & $0$ & $\mathbf{C}$ & $\beta\gamma$ \\
...
$\mathfrak{C}(\cA) \neq 0$
\textup{(}Vol~I, Proposition~\textup{\ref*{V1-prop:shadow-formality-low-degree}(ii)}\textup{)}:
...
$S_3 \neq 0$, so $\pi = 1$. The contact flag separates $G$
from~$C$. The mixed-shell discriminant separates $M$ from the
finite Lie shell after the $(\sigma,\pi)$ pattern has isolated
$\{L,M\}$.
\end{proof}
...
\begin{theorem}[Complete obstruction data through degree~$6$]
...
modular characteristic is $\kappaChHodge(\beta\gamma) = 6\lambda^2 - 6\lambda + 1$
(Proposition~\ref{V1-prop:betagamma-obstruction-coefficient}).
...
\S\ref{V1-sec:heisenberg-shadow-gaussianity},
\S\ref{V1-sec:affine-cubic-shadow},
\S\ref{V1-sec:betagamma-quartic-birth},
\S\ref{V1-sec:mixed-cubic-quartic-shadows},
Theorem~\ref{V1-thm:w-virasoro-quintic-forced}.
\end{proof}
...
$o_3 = 0$ &
 ...
 $\mathbf{G}$, $\mathbf{C}$ \\
$o_4 = 0$ &
 Jacobi identity kills degree-$4$ graphs. &
 $\mathbf{L}$ \\
 & Abelian propagation. &
 $\mathbf{G}$ \\
$o_5 = 0$ &
...
\begin{proof}
By Proposition~\ref{V1-prop:independent-sum-factorization}:
$\Theta_L = \Theta_{L_1} + \Theta_{L_2}$ with no cross-terms.
The degree-$r$ shadow is additive. A shadow is non-vanishing
iff at least one summand is, giving the max formula.
\end{proof}


% ======================================================================
% §16. SUPPORT DEPTH VS.\ KOSZULNESS
% ======================================================================

\subsection{Support depth is not Koszulness}
% label removed: subsec:thqg-depth-vs-koszulness
\index{support shadow depth!vs.\ Koszulness}
...
\begin{tabular}{lcc}
\toprule
\emph{Family} & \(r^\Theta_{\max}\) & \emph{Koszul?} \\
\midrule
...
\end{tabular}
\end{center}
Support shadow depth classifies \emph{complexity} within the Koszul
locus, not Koszulness itself.
\end{proposition}

\begin{proof}
Each algebra is Koszul by PBW universality
(Proposition~\ref{V1-prop:pbw-universality}) or explicit
computation. Their support depths are \(2,3,4,\infty\).
\end{proof}
...
quasi-isomorphism
$\Defcyc(\cA) \simeq \Defcyc(\cA^!)$ preserving the degree filtration.
Therefore each genus-$0$, degree-$r$ shadow cohomology group agrees for
$\cA$ and $\cA^!$, so the set of non-vanishing shadow degrees is the
...
\emph{Algebra} & \emph{Koszul dual} &
 \emph{Duality} & \emph{Class} & \(r^\Theta_{\max}\) \\
\midrule
...
$\lambda$-bracket) receives a degree-$r$ contribution as a
$z^{-r}$ pole; the collision residue
$r^{\mathrm{coll}}(z) = \Res^{\mathrm{coll}}_{0,2}(\Theta_\cA)$
has pole orders one lower.
Stabilization at \(r^\Theta_{\max}\) truncates the Laurent expansion.
\end{proof}
...
\index{support shadow depth!boundary-holographic interpretation}
...
\end{equation}
where the uniform-weight scalar projection is
$\kappaChHodge(\cA)\lambda_g^{\mathrm{FP}}$ with
$\lambda_g^{\mathrm{FP}}
=\int_{\overline{\cM}_{g,1}}\psi_1^{2g-2}\lambda_g$.
The boundary data determine this integral: the OPE of
...
The support shadow depth \(r^\Theta_{\max}(\cA)\) classifies the nonlinear
...
$\bigl(\mathrm{Sh}_3 = \alpha\,x^3\bigr)$ and the critical
discriminant is $\Delta = 8\kappa S_4$ on the mixed primary line.
At $\Delta = 0$ the quadratic
$Q_L = (2\kappa + 3\alpha t)^2$ is a perfect square on that line.
The contact class is finite because its contact flag carries no
mixed Virasoro wheel.
The degree-$r$ shadow first contributes to $F_g$ at genus
$g_{\min}(r) = \lfloor r/2 \rfloor + 1$
\textup{(}Corollary~\textup{\ref{V1-cor:shadow-visibility-genus}}\textup{)}.
The four complexity classes are:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Gaussian} ($\mathbf{G}$, \(r^\Theta_{\max}=2\);
 $\Delta = 0$, $\alpha = 0$).
 ...
 \[
 F_g(\cT) = \kappaChHodge(\cA)\cdot\lambda_g^{\mathrm{FP}},
 \qquad
 \lambda_g^{\mathrm{FP}} =
 \frac{2^{2g-1}-1}{2^{2g-1}}\cdot
 \frac{|B_{2g}|}{(2g)!},
 \]
 where $B_{2g}$ are Bernoulli numbers; no higher-degree
...
\item \emph{Lie} ($\mathbf{L}$, \(r^\Theta_{\max}=3\);
 $\Delta = 0$, $\alpha \neq 0$).
 The boundary OPE produces a cubic shadow
 $\mathfrak{C} = \kappaChHodge(x,[y,z]) \neq 0$;
 ...
 higher-degree shadows contribute.
 \textup{(}Affine $V_k(\fg)$ at generic level.\textup{)}

\item \emph{Contact} ($\mathbf{C}$, \(r^\Theta_{\max}=4\);
 stratum separation\textup{)}.
The boundary $A_\infty$ structure has
$\mathfrak{C} = 0$ and a non-trivial contact flag
$\chi_{\mathrm{ct}} = 1$.
...
\item \emph{Mixed} ($\mathbf{M}$, \(r^\Theta_{\max}=\infty\);
 $\Delta \neq 0$).
 Both $\mathfrak{C} \neq 0$ and $\mathfrak{Q} \neq 0$;
 ...
 at every genus from every degree: two new shadow
...
is the restriction of the all-degree master equation to a primary
line
...
generating function algebraic of degree~$2$. On a mixed primary
line the discriminant $\Delta = 8\kappa S_4$ controls the analytic
type: $\Delta = 0$ gives $\sqrt{Q_L}$ polynomial on that line,
while $\Delta \neq 0$ gives $\sqrt{Q_L}$ irrational and the
Virasoro wheel has infinite depth.  The contact class is separated
by $\chi_{\mathrm{ct}}$, not by this scalar alone.
...
For class~$\mathbf{G}$, only the quadratic shadow
\(\kappaChHodge(\cA)\)
contributes, giving
$F_g^{\mathrm{sc}}(\cA)=\kappaChHodge(\cA)\lambda_g^{\mathrm{FP}}$
\textup{(}uniform-weight; the scalar genus tower, Theorem~D\textup{)}.
...
\begin{proposition}[$\mathcal{W}_N$ support complexity stabilises]
% label removed: prop:thqg-wn-stabilization
\ClaimStatusProvedHere
For all $N \geq 2$: $\mathcal{W}_N^k$ is class~$\mathbf{M}$
with \(r^\Theta_{\max}(\mathcal W_N^k)=\infty\). The coefficients
...
Stabilisation: the degree-$r$ shadow involves generators of
...
\toprule
\emph{Family} & \emph{Class} & \(r^\Theta_{\max}\) &
 \emph{Potential} &
 \emph{Bulk gravity} \\
\midrule
Heisenberg $\cH_k$ &
...
Let $\cA$ lie on one of the canonical bar-intrinsic branches of the
standard gravitational landscape and satisfy the HS-sewing condition.
The support shadow depth \(r^\Theta_{\max}(\cA)\) determines the
minimal homotopy-algebraic complexity of the bulk gravitational
theory in that branch.  The standard-family classification has four
support-depth classes:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textbf{Gaussian} (\(r^\Theta_{\max}=2\)):
 Free bulk theory. All obstructions vanish.
 Quadratic potential. Archetype: Heisenberg.

\item \textbf{Lie} (\(r^\Theta_{\max}=3\)):
 Semistrict higher-spin gravity with cubic vertices.
 Jacobi identity closes the tower beyond the cubic degree.
 Archetype: affine $V_k(\fg)$.

\item \textbf{Contact} (\(r^\Theta_{\max}=4\)):
 Quartic contact vertices, no cubic.
 Cubic gauge triviality, rank-one rigidity, and the vanishing
 higher lift coordinates terminate the support packet.
 Archetype: $\beta\gamma$.

\item \textbf{Mixed} (\(r^\Theta_{\max}=\infty\)):
 Full $L_\infty$ with vertices at all degrees.
...
parameter deformation on the canonical standard-family strata.  No
classification of arbitrary components of the filtered MC moduli
problem is asserted here.
\end{maintheorem}
...
\textup{(}chiral envelope, \cite[\S3.7]{BD04}\textup{)}:
...
\item \emph{Finite-jet rigidity}
 \textup{(}Proposition~\ref{V1-prop:finite-jet-rigidity}\textup{)}:
 the degree-$r$ shadow depends only on the $(r{+}1)$-jet
 $\Uvert(R)/F_{r+1}$.
\item \emph{Polynomial level dependence}
 \textup{(}Proposition~\ref{V1-prop:polynomial-level-dependence}\textup{)}:
 the degree-$r$ shadow is polynomial of degree $\leq r{-}1$ in
 a central-extension parameter.
\item \emph{Gaussian collapse}
 \textup{(}Proposition~\ref{V1-prop:gaussian-collapse-abelian}\textup{)}:
...
and \(d^\Theta_\infty=2\); conversely, \(d^\Theta_\infty=2\) forces
...
class~$\mathbf{L}$ (\(r^\Theta_{\max}=3\)), while its DS reduction
$\mathrm{Vir}_c$ is class~$\mathbf{M}$
(\(r^\Theta_{\max}=\infty\)).
...
\item \emph{Higher transferred operations detect the loss.}
 ...
 degrees~$k \geq 3$. For $N = 3$, the homotopy~$h$ routes
...
Whether the chain-level BRST data is recoverable from the
cohomology-level $\Ainf$ structure separates principal from
non-principal reductions. For principal reductions of simply
laced algebras, bar-cobar inversion (Theorem~A) reconstructs
the chain-level algebra from its transferred $\Ainf$ structure
up to quasi-isomorphism on the Koszul locus. For non-principal
reductions, reconstruction requires that the Koszul property
survive the reduction (Theorem~\ref*{V1-thm:ds-koszul-obstruction}):
proved for the hook-type corridor in type~A, open in general.
\end{remark}
...
\index{support shadow depth!deformation stability}

\begin{proposition}[Generic constancy on a standard support stratum]
% label removed: prop:thqg-generic-constancy
\ClaimStatusProvedHere
\index{support shadow depth!generic constancy}
Let \(\{\cA_t\}_{t\in\C}\) be an algebraic one-parameter family
inside a fixed canonical standard-family gauge slice, and suppose
the support packet is known on a finite window \(2\le n\le N\).
Then the vanishing pattern of the support coordinates
\(\Theta_{\cA_t,n}\), \(2\le n\le N\), is constant on the
complement of a finite algebraic discriminant. On that complement,
finite support depth \(r^\Theta_{\max}\le N\) is locally constant.
At the discriminant the support depth may change; no monotonicity
statement is made without controlling the \(H^1\)-lift coordinates.
\end{proposition}

\begin{proof}
In the fixed gauge slice each support coordinate
\(\Theta_{\cA_t,n}\) is obtained by a finite graph-sum expression
in the structure constants and propagator entries of \(\cA_t\).
Hence each coordinate on the finite window is an algebraic function
of \(t\). The zero set of a nonzero algebraic function is finite,
and an identically zero coordinate is zero on the whole branch.
Removing the union of the nontrivial zero loci gives a Zariski-open
set on which the support/non-support pattern in degrees \(2,\ldots,N\)
is constant. The last nonzero degree in this finite window is
therefore constant there. The final sentence follows because
vanishing of obstruction classes controls extendability only; the
support coordinate also includes the choice of \(H^1\)-lift.
\end{proof}
...
a nondegenerate support packet is not defined (the cyclic pairing
fails to be nondegenerate). At $c = -22/5$: $Q_0$ diverges, signalling
...
$\kappa = 0$). At $k = -2$: the support shadow depth jumps from
...
Euler--Lagrange equation for the scalar Euler primitive
$E_\cA(x)=\sum_{r\geq2}s_r(\cA)x^r/r$:
\begin{equation}% label removed: eq:thqg-euler-lagrange
\frac{\partial E_\cA}{\partial x}
\;=\;
\kappa\,x + \sum_{r=3}^{r^\Theta_{\max}(\cA)}
s_r(\cA)\,x^{r-1}
\;=\; 0
\end{equation}
at the critical point $x = 0$. The second variation is
$E''_\cA(0) = \kappaChHodge(\cA) = H_\cA$. The genus-$g$
partition function arises from the formal saddle-point
expansion of the Taylor action $S_\cA$.
\end{theorem}

\begin{proof}
Write $\mathrm{Sh}_r(\cA)(x,\ldots,x)=s_r(\cA)x^r$.
The all-degree master equation
$\nabla_H(\mathrm{Sh}_r) + \mathfrak{o}^{(r)} = 0$,
restricted to the primary line, is the recursion for the
Taylor coefficients of $S_\cA$ and for the radial coefficients of
$E_\cA$. The MC equation for
...
\end{tabular}
\end{center}
Polynomial degree of the gravitational effective action
mirrors the EFT truncation hierarchy: renormalisable ($r \leq 4$)
versus non-renormalisable ($r = \infty$). Class~$\mathbf{M}$ is
the algebraic image of a non-renormalisable theory requiring
infinitely many counterterms in the Wilsonian sense; the full
tower is determined by the finite OPE data of the boundary
algebra, so no independent counterterms arise.
\end{remark}
...
\end{equation}
the degree-$r$ piece of the convolution algebra.
\end{construction}
...
(Theorem~\ref{V1-thm:spectral-sequence-filtered-dg}). The
...
\item \emph{Class~$\mathbf{G}$}: The spectral sequence
 ...
 $r \geq 1$. The MC element is purely scalar in degree:
...
for $r \geq 3$: every class at degree~$r$ is the image of
the degree-$(r{-}1)$ class under $d_1$ (up to a nonzero
scalar), and is simultaneously in the kernel of $d_1$ at
degree~$r$ (since $o_{r+2} \neq 0$ means $d_1$ is injective).
\end{proof}
...
\index{support shadow depth!curve independence}
...
(Proposition~\ref{V1-prop:genus0-curve-independence}):
...
The degree-$6$ obstruction $\mathfrak{o}^{(6)}$ receives
contributions from all two-vertex graphs with degrees
...
$\mathfrak{o}^{(5)}$; the degree-$6$ component requires
...
bracket term. The degree-$2$ piece $\mathrm{Sh}_2 = (c/2)x^2$
paired with degree-$5$ gives $5S_5 x^4\cdot(2/c)\cdot cx
= 10S_5\,x^5$, but this produces degree $5 + 2 - 1 = 6$
with different symmetry.

\smallskip\noindent
Collecting the three dominant contributions at degree~$6$:
\begin{align*}
\mathfrak{o}^{(6)}
&= \{\mathfrak{C}, \mathrm{Sh}_5\}_H
...
The shadow obstruction tower has a ``zero-crossing'' at degree~$6$ but remains
...
\label{open:thqg-gap}
\index{support shadow depth!gap question}
Does there exist a modular Koszul chiral algebra with
\(r^\Theta_{\max}\in \{5,6,7,\ldots\}\setminus\{\infty\}\)?
All known standard examples have
...
$o_3 = 0$, $o_4 \neq 0$, $o_5 \neq 0$, $o_6 = 0$: a
quartic-plus-quintic tower that terminates at degree~$5$.
No known algebraic mechanism produces this pattern.
\end{openproblem}

% =====================================
% Support depth as stratification depth of the Steinberg
% =====================================

\subsection{Support depth as stratification depth of the Steinberg}
% label removed: subsec:shadow-steinberg-stratification
\index{Steinberg object!stratification depth}
\index{support shadow depth!Steinberg interpretation}

The support-depth classification has the following conjectural
geometric lift through the Chriss--Ginzburg picture: support
shadow depth should be the stratification depth of the Steinberg
object $\mathfrak{S}_b$ associated to the bar complex, after the
same gauge slice and lift-coordinate choices have been fixed.
...
\begin{proposition}[Support depth = stratification depth]
% label removed: prop:shadow-equals-stratification
\ClaimStatusConjectured
The support shadow depth \(r^\Theta_{\max}\) of a modular Koszul
chiral algebra equals the stratification depth of the associated
Steinberg object~$\mathfrak{S}_b$ in the corresponding bar-intrinsic
gauge component. Concretely:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textbf{Class~$\mathbf{G}$}
 (\(r^\Theta_{\max}=2\)): $\mathfrak{S}_b$ has two strata,
 the smooth locus plus one singular stratum.
 Examples: Heisenberg, lattice VOA.
\item \textbf{Class~$\mathbf{L}$}
 (\(r^\Theta_{\max}=3\)): three strata.
 Example: affine $V_k(\fg)$, where the Lie bracket
 creates one additional singular stratum.
\item \textbf{Class~$\mathbf{C}$}
 (\(r^\Theta_{\max}=4\)): four strata
 (the gap at depth~$3$ from cubic gauge triviality).
 Example: $\beta\gamma$.
\item \textbf{Class~$\mathbf{M}$}
 (\(r^\Theta_{\max}=\infty\)): infinitely many strata.
...
strata. The three standard classes
(items~(i)--(iii) above) verify the identification directly; the
general statement requires the FM residue completeness axiom (H3).
\end{remark}
...
in~$d$. Finite support depth \(>4\) therefore requires a
very specific OPE pole pattern: poles of exactly orders
\(1\) through \(r^\Theta_{\max}\) with all higher poles and lift
...
\(r^\Theta_{\max}=r\) with \(r\) finite, one needs non-vanishing
poles at orders $1, \ldots, r$ and vanishing at all orders
\(>r\), together with vanishing higher lift coordinates. For
\(r>4\), this means the OPE and the bar-intrinsic gauge slice must
have a sharp cutoff at order~$r$, a condition not satisfied by any
...
what is the support-depth class? The hook-type corridor
in type~A is proved
(Volume~I); the general case
is open. Expectation: all non-abelian non-principal
W-algebras are class~$\mathbf{M}$ (the DS reduction
introduces composite fields at all degrees).
\end{openproblem}

\begin{openproblem}[Categorical complexity]
\label{open:thqg-categorical}
\index{categorical support shadow depth}
Is there a categorical lift of the support shadow depth that
detects \(r^\Theta_{\max}\) at the level of derived categories
...
\index{support shadow depth!$t$-structure detection}

\providecommand{\Steinberg}{\mathfrak{S}}

\begin{conjecture}[Support depth as $t$-structure amplitude]
\label{conj:shadow-t-amplitude}
\ClaimStatusConjectured
Let $\cA$ be a modular Koszul chiral algebra with
Steinberg object $\Steinberg_b$. Then the support shadow depth
\(r^\Theta_{\max}\) equals the \emph{amplitude} of the natural
...
bar support in the chosen component.
\end{conjecture}

\begin{remark}[Intrinsic detection of complexity class]
% label removed: rmk:t-structure-intrinsic
Conjecture~\ref{conj:shadow-t-amplitude} provides a
categorical (intrinsic) detection of \(r^\Theta_{\max}\) without
...
orbit stratification; their number is the support shadow depth
\(r^\Theta_{\max}\). The passage from $|W|$ to
\(r^\Theta_{\max}\) is the
...
\section{Synthesis}
\label{sec:gravcomplex-supplement}

\begin{remark}[Gravitational complexity of $\mathbf H_{\Delta_5}$]
\label{rem:gravcomplex-1}
The gravitational-complexity analysis applied to the K3 background has
a Schiffmann--Vasserot-type target measure
\[
\mathcal C_{\mathrm{grav}}(\mathbf H_{\Delta_5}^{\mathrm{src}})
\;=\; \dim\, \mathrm{Ext}^\bullet_{\mathrm{CoHA}_{K3\times E}}
\bigl(\mathbf H_{\Delta_5}^{\mathrm{src}}, \mathbf H_{\Delta_5}^{\mathrm{src}}\bigr)^{\mathbb Z/2,\, K(1)},
\]
where $\mathbf H_{\Delta_5}^{\mathrm{src}}$ denotes a compact
Hall--Borcherds source only after the Vol~III recognition gates have
been supplied.  The leading Ext direction is compared, not constructed,
by the Heegner characteristic map
\[
H^2_{\mathrm{Lie}}\bigl(\mathfrak q_{K(1)};\mathrm{grt}_1\bigr)
\;\cong\; \mathbb C\cdot \Delta_5 .
\]
Thus the $\hbar=0$ leading scalar and the approach to
$\hbar_{\mathrm{Lusztig}}=\sqrt{-1/8}$ are target comparisons through
the Gritsenko-additive-lift direction; they are not consequences of a
direct BKM Lie-cohomology computation nor of the Vol~II
$\mathsf{SC}^{\mathrm{ch,top}}$ sewing diagram alone.  Primary-lit:
Etingof--Kazhdan 2007 Part~V, Bruinier 2002, Borcherds 1992/1998,
Gritsenko--Nikulin 1996--98, Susskind 2014 (complexity conjecture).
\end{remark}
