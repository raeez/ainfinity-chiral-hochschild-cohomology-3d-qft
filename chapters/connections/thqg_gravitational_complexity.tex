...
The shadow Postnikov obstruction tower stabilises at arity $r_{\max}(\cA)$:
...
(Proposition~\ref{prop:thqg-depth-qi-invariant}).
The complementarity theorem (Theorem~C) gives a quasi-isomorphism
$\Defcyc(\cA) \simeq \Defcyc(\cA^!)$ preserving the arity filtration,
hence $d_\infty(\cA) = d_\infty(\cA^!)$.  Alternatively: the
shadow depth is determined by the OPE pole structure
(Definition~\ref{def:thqg-gravitational-complexity}),
and the Feigin--Frenkel involution preserves the pole
pattern of the OPE while exchanging the level parameter;
this is verified explicitly for the Koszul pairs
