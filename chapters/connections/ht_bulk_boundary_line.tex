...
boundary itself; line operators are modules for the open-colour Koszul
...
\section{Literature backbone, formal consequences, and status ledger}% label removed: sec:backbone
...
\begin{enumerate}[label=(\roman*),leftmargin=1.8em]
\item the \emph{absolute} open-colour line algebra $A^!_{\mathrm{line}}$ of the global theory, conjecturally presenting all line operators and appearing in \cite{DNP25};
...
\end{enumerate}
The latter are local exact models attached to a chosen boundary vacuum.  They should be viewed as boundary charts on the absolute line category.  They are not globally equal to $A^!_{\mathrm{line}}$, but they are the correct local testing ground in which the shape of the global theorem becomes rigid.
...
$\mathrm{Rep}(\cH_q) = \cO_q$
  & $\cC_{\mathrm{line}} = A^!_{\mathrm{line}}\text{-mod}$ & Module category \\
Center $Z(\cH_q)$
...
(Vol~I, Proposition~\ref{prop:lifts-as-relative-mc}).
...
=\frac{4(k+3)+4(-k-3)}{3}
=0$,
so the complementarity $\kappa(\cA)+\kappa(\cA^!)=0$ holds
(cf.\ Remark~\ref*{rem:kappa-shift-convention}
of Vol~I). The central charges likewise satisfy
