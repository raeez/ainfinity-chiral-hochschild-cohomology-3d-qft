...
\begin{construction}[Formality route to DS--Koszul intertwining]
\label{constr:vol2-ds-kd-formality}
The DS--Koszul intertwining $\mathrm{DS}_f(B(\hat{V}_k(\mathfrak{g}))) \simeq B(\mathcal{W}_k(\mathfrak{g}, f))$ holds at generic level when $\mathfrak{n}_+ = \bigoplus_{j>0} \mathfrak{g}_j$ is abelian: the BRST charge $Q_{\mathrm{DS}}$ has no ghost-ghost terms, the BRST complex is Koszul, and the Kazhdan filtration gives formality $\mathbf{R}\mathrm{DS}_f \simeq H^0_{Q_{\mathrm{DS}}}$ (Vol~I, Proposition~\ref{prop:ds-bar-formality}).  The structural dividing line is:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Abelian} $\mathfrak{n}_+$: all hook and rectangular partitions in type~$A$.  Formality automatic.
...
\item \emph{The critical discriminant.}
  ...
  $\Delta = 0$ iff the shadow tower terminates (classes
...
The unshifted characteristic $\kappa=k\dim\mathfrak g/(k+h^\vee)
=8k/(k+3)$ satisfies
$\kappa_k+\kappa_{-k-2h^\vee}
=\frac{8k}{k+3}+\frac{8k+48}{k+3}
=\frac{16k+48}{k+3}
=2\dim\mathfrak{sl}_3=16$,
so the complementarity $\kappa(\cA)+\kappa(\cA^!)=0$ holds for
the \emph{normalised} characteristic after subtracting the vacuum
contribution (cf.\ Remark~\ref*{rem:kappa-shift-convention}
