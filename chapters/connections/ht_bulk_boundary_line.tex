...
\newcommand{\Cobar}{\operatorname{\Omega}}
...
\newcommand{\dCrit}{\operatorname{dCrit}}
\newcommand{\Zder}{\operatorname{Z}_{\mathrm{der}}}
\newcommand{\mc}{\operatorname{MC}}
\newcommand{\op}{\mathrm{op}}
\newcommand{\der}{\mathrm{der}}
\newcommand{\Abulk}{A_{\mathrm{bulk}}}
\newcommand{\Bbound}{B_{\partial}}
\newcommand{\Kline}{K_{\mathrm{line}}}
\newcommand{\intr}{\mathrm{intr}}
\newcommand{\free}{\mathrm{free}}
\newcommand{\eff}{\mathrm{eff}}
\newcommand{\st}{\mathrm{st}}
\newcommand{\ex}{\mathrm{ex}}
\newcommand{\minmod}{\mathrm{min}}
\newcommand{\expmod}{\mathrm{exp}}
\newcommand{\hkR}{\mathrm{HKR}}
\newcommand{\sn}{\mathrm{SN}}
\newcommand{\lines}{\mathsf{Lines}}
\newcommand{\hSym}{\widehat{\Sym}}

\title{\vspace{-1em}\textbf{The 3d Holomorphic--Topological Bulk/Boundary/Line Program from First Principles}\\
\large Intrinsic boundary $A_\infty$ algebras, polarized Landau--Ginzburg geometry, Kuranishi line algebras,\\
microlocal classification, and the fortified route toward the global Koszul triangle}
\author{}
\date{March 15, 2026}


\begin{abstract}
This manuscript is a repository-grade synthesis and fortification of the entire bulk/boundary/line program developed in this thread.  It has two intertwined aims.

The first aim is foundational.  Starting from first principles of locality, BV--BRST deformation theory, Koszul duality, Hochschild theory, shifted symplectic geometry, and the open/closed philosophy of topological field theory, we rebuild the 3d holomorphic--topological story so that each layer is placed in its correct home.  The bulk vertex is not the same object as the line algebra; rather the corrected global triangle is
\[
\Abulk
\;\simeq\;
\Zder(\Bbound)
...
\]
with $A^!$ the Koszul-dual algebra controlling lines and $\Zder$ the derived center.  We explain how this corrects several common confusions at once: bulk versus line, boundary-adapted local line algebras versus the absolute line algebra $A^!$, strict dg models versus genuine $A_\infty$ structures, and complement-dependent presentations versus intrinsic formal-neighborhood constructions.

The second aim is new mathematics.  We consolidate and reprove, in one coherent narrative, the theorem packages developed across the previous notes:

\begin{enumerate}[label=(\arabic*),leftmargin=1.8em]
\item the intrinsic boundary $A_\infty$ algebra attached to a pair $(L\subset V,W)$ with $W|_L=0$ via intrinsic normal Taylor classes;
\item the boundary Thom--Sebastiani theorem and the two stabilization theorems (contractible tangent-normal stabilization and hyperbolic normal/Clifford stabilization);
\item the polarized Landau--Ginzburg theorem
\[
\dCrit(W)\simeq T^*[-1]Z_F^{\der},
\qquad
\HH^\bullet(B_{L,W})\cong H^\bullet\!\bigl(\cO(\dCrit(W))\bigr)
\]
for boundary-linear superpotentials $W(x,y)=\langle y,F(x)\rangle$;
\item the one-step Jacobi coalgebra $\cJ_{F,p}$, the exact pointed line dg algebra
\[
K^{\ex}_{F,p}\simeq \RHom_{B_{F,p}}(\kk,\kk),
\]
the Maurer--Cartan/vacuum theorem, and polarized reconstruction;
\item the boundary Kuranishi elimination theorem, giving an explicit nonlinear Schur complement for the boundary dg algebra and a minimal pointed line algebra governed by the effective obstruction map $\kappa$;
\item the local bulk/line theorem
\[
\HH^\bullet(K_{F,p})\cong H^\bullet\!\left(\cO(\dCrit(W_{\eff}))\right),
\qquad
W_{\eff}(u,\gamma)=\langle \gamma,\kappa(u)\rangle;
\]
\item and a new synthesis theorem proved here: the \emph{boundary-linear line Mather--Yau theorem}, asserting that the polarized minimal pointed line $A_\infty$ algebra is a complete invariant of the reduced microlocal formal germ.
\end{enumerate}

The manuscript is explicit at every stage about status.  Literature-backed input is labeled as such.  Formal consequences are separated from new theorems proved in this manuscript.  The global, all-theory bulk/boundary/line Koszul triangle remains conjectural, but the microlocal algebraic core is now much more rigid: in the exact boundary-linear sector the local boundary vacuum is governed by a canonical coalgebra, the local pointed line algebra is its cobar, its minimal model is the Kuranishi algebra, and its derived center recovers the completed local bulk algebra.  This is the fortified local backbone from which the global theorem should be attacked.
\end{abstract}

\tableofcontents

\section{Overture: what is being corrected, what is being built}

The 2020 boundary-chiral-algebra paper \cite{CDG20} already contains the seed of the whole 3d holomorphic--topological story:
\begin{itemize}[leftmargin=1.8em]
\item bulk local operators form a commutative chiral algebra with shifted Poisson bracket;
\item boundary local operators form a module over the bulk and carry higher $A_\infty$-like structure;
\item the derived bulk--boundary map
\[
\beta_{\der}:\Abulk \longrightarrow \Zder(\Bbound)
\]
should exist and should be an isomorphism for sufficiently rich boundary conditions.
\end{itemize}

The 2025 line-operator paper \cite{DNP25} raises the categorical level: perturbative lines should be modules over a Koszul-dual $A_\infty$ algebra $A^!$, and their OPE is governed by a dg-shifted Yangian.

The deepest theorem-level target is therefore not
\[
\text{bulk}\;\simeq\;\text{boundary}\;\simeq\;\text{line},
\]
which is false as stated, but rather the corrected triangle
...
\cC_{\mathrm{line}}\simeq A^!\text{-mod}.
\end{equation}
That is the 3d holomorphic--topological analogue of the strongest open/closed statements in 2d TFT and the B-model.

This manuscript rebuilds the whole thread with three principles kept visibly in force.
\begin{enumerate}[label=\textbf{P\arabic*}. ,leftmargin=2em]
\item \textbf{Intrinsic before presented.}  Whenever a construction was formerly written using a splitting $V=L\oplus N$, we first reformulate it intrinsically in terms of the quotient $Q=V/L$ and the formal neighborhood of $L$.
\item \textbf{Derived before cohomological.}  Cohomology-level OPE data are too small.  The right objects are dg or $A_\infty$ models, derived centers, bar and cobar constructions, and formal moduli problems.
\item \textbf{Reduction before comparison.}  The path to a theorem is not to compare three gigantic global objects at once.  One first isolates a small exact sector, proves an exact local theorem there, then identifies the pieces that are contractible, Morita-trivial, or stabilization-invariant, and only then returns to the global categorical triangle.
\end{enumerate}

\subsection*{What this manuscript contains}

The manuscript is organized so that the mathematics tightens progressively:
\begin{itemize}[leftmargin=1.8em]
\item Sections~\ref{sec:backbone}--\ref{sec:intrinsic} lay out the literature backbone and build the intrinsic boundary $A_\infty$ algebra.
\item Section~\ref{sec:ts-knorrer} proves multiplicativity and stabilization: Thom--Sebastiani, contractible stabilization, and hyperbolic Kn\"orrer/Clifford stabilization.
\item Section~\ref{sec:boundary-linear} treats the exact boundary-linear Landau--Ginzburg sector, including the bulk/boundary theorem and the free polarization theorem as a special case.
\item Sections~\ref{sec:exact-lines}--\ref{sec:kuranishi} construct the exact pointed line algebra, the one-step Jacobi coalgebra, the Kuranishi reduction, and the minimal line algebra.
\item Section~\ref{sec:classification} proves the new classification theorem for the reduced microlocal formal germ and exhibits the fortified local bulk/boundary/line triangle.
\item Section~\ref{sec:global} returns to the global conjectural envelope and formulates the geometric Steinberg-style endpoint that should ultimately present the full line algebra by convolution.
\end{itemize}

We work over a field $\kk$ of characteristic $0$.  All tensor products are understood in the completed sense whenever formal power series appear.
...
\begin{proposition}[Formal reduction of the global triangle]\label{prop:formal-global-triangle}
...
\hline
Universal defect evidence for the bulk/boundary comparison & literature & Zeng \cite{Zen23}. \\
\hline
...
\begin{theorem}[Intrinsic boundary $A_\infty$ algebra]\label{thm:intrinsic-boundary}
...
\begin{corollary}[Formal-neighborhood invariance]\label{cor:formal-neighborhood}
...
\begin{corollary}[Boundary-linear strictness]\label{cor:boundary-linear-strict}
...
The intrinsic boundary construction should behave like a robust algebraic object, not a delicate local trick.  Three operations test that robustness: external sums, tangent-normal hyperbolic stabilizations, and pure-normal hyperbolic stabilizations.
...
\begin{theorem}[Boundary Thom--Sebastiani]\label{thm:TS}
...
\begin{theorem}[Contractible stabilization]\label{thm:contractible-stab}
...
\begin{theorem}[Hyperbolic Clifford stabilization]\label{thm:clifford-stab}
...
The preceding theorems become powerful when combined with a formal Morse lemma relative to the boundary.

\begin{theorem}[Relative formal Morse splitting]\label{thm:relative-morse}
...
\begin{theorem}[Derived critical locus theorem]\label{thm:dcrit-shiftedcot}
...
\begin{theorem}[Boundary-linear bulk/boundary theorem]\label{thm:boundary-linear-bulk-boundary}
...
\begin{corollary}[Free polarization theorem]\label{cor:free-polarization}
...
\begin{theorem}[Exact presentation theorem]\label{thm:exact-line}
...
\begin{corollary}[Explicit pointed line algebra]\label{cor:explicit-line}
...
\begin{theorem}[Maurer--Cartan vacuum theorem]\label{thm:mc-vacua}
...
\begin{theorem}[Polarized reconstruction]\label{thm:polarized-reconstruction}
...
\begin{lemma}[Formal implicit solution]\label{lem:formal-implicit}
...
\begin{lemma}[Factorization of the massive equations]\label{lem:factorization}
...
\begin{theorem}[Boundary Kuranishi elimination]\label{thm:boundary-kuranishi}
...
\begin{theorem}[Minimal pointed line algebra]\label{thm:minimal-pointed-line}
...
\begin{proposition}[Recursive Kuranishi coefficients]\label{prop:recursive-kuranishi}
...
\begin{corollary}[Quadratic and cubic effective couplings]\label{cor:low-order-kappa}
...
\begin{corollary}[Smooth-point formality]\label{cor:smooth-formality}
...
\begin{theorem}[Local bulk/line theorem]\label{thm:local-bulk-line}
...
\begin{theorem}[Boundary-linear line Mather--Yau theorem]\label{thm:line-mather-yau}
...
\begin{corollary}[Microlocal classification theorem]\label{cor:microlocal-classification}
The assignment
\[
...
\begin{corollary}[Stable invisibility of universal blocks]\label{cor:stable-invisibility}
...
\begin{theorem}[Fortified local bulk/boundary/line triangle]\label{thm:fortified-local-triangle}
...
\begin{conjecture}[HT Steinberg presentation]
...
\end{enumerate}
Only when these are all in place will the global triangle stop being a powerful analogy and become a theorem with a home.
...
The bulk/boundary/line Koszul triangle is therefore no longer merely a slogan.  Its local algebraic heart has been built.  What remains is the global geometric act: to produce the rich boundary conditions, the comparison functors, and the holomorphic--topological Steinberg object on which the full line algebra should appear by convolution.  If that act succeeds, the theory will have reached the point where the algebra is no longer merely computed---it is \emph{presented}.
...
Used here as background for the open/closed/factorization viewpoint.
...
2020, arXiv:2005.00083.

\bibitem[DNP25]{DNP25}
T.~Dimofte, W.~Niu, and V.~Py,
\emph{Line Operators in 3d Holomorphic QFT},
2025.
Used here for the Koszul-dual line algebra and dg-shifted Yangian perspective.

\bibitem[DTT09]{DTT09}
V.~Dolgushev, D.~Tamarkin, and B.~Tsygan,
\emph{Formality theorems for Hochschild complexes and their applications},
2009, arXiv:0901.0069.

\bibitem[GKW24]{GKW24}
D.~Gaiotto, J.~Kulp, and J.~Wu,
...
\bibitem[Tam00]{Tam00}
D.~Tamarkin,
\emph{The deformation complex of a $d$-algebra is a $(d+1)$-algebra},
2000, arXiv:math/0010072.

\bibitem[Zen23]{Zen23}
K.~Zeng,
\emph{Universal Defects and Boundary Chiral Algebras in Holomorphic Twists},
2023.
Used here as evidence for the bulk/boundary comparison.
