...
Three input theorems supply the algebraic background.
...
Khan and Zeng construct mixed holomorphic-topological gauge theories from freely generated Poisson vertex algebras; the $\lambda$-bracket supplies the structure constants, gauge invariance is equivalent to the Jacobi identity, with Virasoro, super-Virasoro, and $W_3$ examples carrying explicit higher-spin interpretations (Khan--Zeng, 2025).
\end{enumerate}
...
\]
Call $F^1C$ the \emph{composite ideal}. The quotient
\[
L:=C/F^1C
\]
...
\]
with $i,p,h$ all filtration-preserving, normalized by
\[
p i=\id_H,\qquad p h=0,\qquad h i=0,\qquad h^2=0.
\]
Let $m_n$ be the transferred minimal $\Ainf$-structure on $H$, and define
\[
F^rH:=p(F^rC),
\qquad
H_{\mathrm{sp}}:=H/F^1H.
...
The transfer uses the following signed convention.  Put
\(\widetilde C=sC\) and \(\widetilde H=sH\), and define
\[
\widetilde i=s\,i\,s^{-1},\qquad
\widetilde p=s\,p\,s^{-1},\qquad
\widetilde h=s\,h\,s^{-1},\qquad
\widetilde\mu_r=s\,\mu_r\,(s^{-1})^{\otimes r}.
\]
For \(T\in\operatorname{PT}_n\), order the internal edges
\[
E_{\mathrm{int}}(T)=\{e_1<\cdots<e_N\}
\]
by the left-to-right depth-first planar order, and orient the tree by
\(\mathfrak o(T)=\det(\Bbbk^{E_{\mathrm{int}}(T)})\).  Let
\(\widetilde\mu_T\) be the composite in graded vector spaces obtained
by putting \(\widetilde i\) at leaves, \(\widetilde\mu_{\operatorname{val}(v)}\)
at each internal vertex \(v\), \(\widetilde h\) on every internal
edge, and \(\widetilde p\) at the root, composed in the above edge
order.  The transferred operation is
\begin{equation}\label{eq:cbt-suspended-transfer}
 m_n
 =
 s_H^{-1}
 \left(\sum_{T\in\operatorname{PT}_n}\widetilde\mu_T\right)
 s_H^{\otimes n}.
\end{equation}
Thus the signs are not an extra \(\pm\)-choice: they are exactly the
Koszul signs forced by the suspension operators in
\eqref{eq:cbt-suspended-transfer} and the orientation line
\(\mathfrak o(T)\).
...
\begin{proof}
By the suspended Kadeishvili formula
\eqref{eq:cbt-suspended-transfer}, each transferred operation \(m_n\)
is the signed sum over planar rooted trees with \(n\) leaves whose
vertices, internal edges, leaves, and root are decorated respectively
by \(\mu_r,h,i,p\).  The normalized side conditions make this the
minimal homotopy transfer of the dg algebra \((C,\mu_1,\mu_2,\ldots)\).
...
Beyond the single-particle quotient lies the first irreducibly nonlinear datum, governed by the obstruction classes constructed here.
...
Filter $A_{\mathrm{BF}}$ by the composite ideal
\[
...
Isolate the highest-weight ladder
\[
L_a:=w_1^a,
\qquad
...
Poisson BF theory induces transferred $\Linf$-operations
\[
L_n:=\{\cdot,\dots,\cdot\}_{\bar\pi,n}
\]
on $H_b^{0,\bullet}(S^3)$, whose odd highest-weight ladder is computed below.
...
\]
The odd highest-weight ladder computed in Theorem~\ref{thm:cbt-poisson-ladder} is therefore a concrete sector of the transferred $\Linf$ deformation of this cotangent Lie algebra, and the construction below converts that sector into a one-dimensional differential-operator model.
...
\begin{proof}
Evaluate on basis vectors. Let $y_i=v_{b_i}$. Writing $B=b_1+\cdots+b_{m+1}$ and using Theorem~\ref{thm:cbt-poisson-ladder} with $n=m+2$, one obtains
\[
...
The span of the odd positive Witt modes fails to close as a Lie subalgebra of the Witt algebra: the bracket of two odd modes is even. The theorem states a representation-theoretic identification of a computed sector of the full transferred $\Linf$ deformation, not closure of the odd ladder as a Lie algebra. The missing even modes arise from brackets involving further degree-$0$ highest-weight sectors of $H_b^{0,0}(S^3)$.
\end{remark}
...
\section{MHV amplitudes from a four-point residue datum}
\label{sec:cbt-mhv-bar-complex}

\begin{definition}[four-point Parke--Taylor residue datum]
\label{def:cbt-pt-residue-datum}
\index{Parke--Taylor residue datum|textbf}
Fix the colour order \((1,2,3,4)\) and helicity word
\((+,-,-,+)\).  A \emph{four-point Parke--Taylor residue datum} for
the bar weight form \(\omega_4^{(+,-,-,+)}\) consists of:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the colour-order projection
\[
P_{\mathrm{cyc}}\colon C_\bullet(\overline{\FM}_4(\C))
\longrightarrow C_\bullet(\overline{\FM}^{\mathrm{cyc}}_4(\C))
\]
which kills the non-cyclic pair channels
\(D_{\{1,3\}}\) and \(D_{\{2,4\}}\);
\item a spinor-helicity residue character on the cyclic boundary
divisors
\[
\rho_{i,i+1}\colon
\Res_{D_{\{i,i+1\}}}
(\omega_4^{(+,-,-,+)}\wedge\nu_4)
\longmapsto
\langle i\,i{+}1\rangle^{-1},
\qquad i\in\Z/4\Z,
\]
where the index \(i+1\) is read cyclically, so \(4+1=1\);
\item a negative-helicity pairing normalization
\[
\rho_{--}(\tilde J_2,\tilde J_3)=\langle 23\rangle^4;
\]
\item the orientation convention on
\(\overline{\FM}^{\mathrm{cyc}}_4(\C)\) for which the four cyclic
boundary residues multiply in the order
\((12),(23),(34),(41)\).
\end{enumerate}
The resulting residue functional is
\[
\mathcal R^{(+,-,-,+)}_{4,\mathrm{cyc}}
\;=\;
\rho_{--}\prod_{i\in\Z/4\Z}\rho_{i,i+1}
\quad
\text{on }P_{\mathrm{cyc}}
(\omega_4^{(+,-,-,+)}\wedge\nu_4).
\]
This datum is a comparison datum between the bar weight form and the
spinor-helicity normalization.  It is not supplied by the abstract
\(\Ainf\) transfer alone.
\end{definition}

The celestial and boundary transfer machinery developed above gives a
precise residue problem for scattering amplitudes: after the datum of
Definition~\ref{def:cbt-pt-residue-datum} is supplied, the
colour-ordered four-point MHV scalar is the cyclic boundary residue of
the bar weight form.  Without that datum the statement is only an
analogy with scattering, not a theorem about physical amplitudes.

\begin{theorem}[Four-point Parke--Taylor residue identity;
\(\ClaimStatusConditional\); licensing \(\alpha+\beta+\gamma\)
via colour order, spinor-helicity residue comparison, and cyclic FM
orientation]
\label{thm:cbt-mhv-bar-integral}
Assume a four-point Parke--Taylor residue datum in the sense of
Definition~\ref{def:cbt-pt-residue-datum}.  Then the colour-ordered
bar residue of the \((+,-,-,+)\) weight form is
\[
\mathcal R^{(+,-,-,+)}_{4,\mathrm{cyc}}
\bigl(\omega_4^{(+,-,-,+)}\wedge\nu_4\bigr)
\;=\;
...
\]
Equivalently, after the usual external-state and colour-order
normalisations are identified, this scalar is the four-gluon
Parke--Taylor expression \(A_4^{\mathrm{PT}}(1^+,2^-,3^-,4^+)\).
The theorem is a residue identity on the cyclic FM boundary.  It does
not construct the Lorentzian scattering theory, the LSZ map, or the
Mellin/celestial transform.

Specifically:
\begin{enumerate}[label=\textup{(\alph*)}]
\item The degree-$4$ bar weight form factorises over the five planar
binary trees with \(4\) leaves before the colour-order projection.
...
\item The cyclic residue functional extracts the adjacent boundary
channels:
\[
\mathcal R_{4,\mathrm{cyc}}
\;=\;
\rho_{12}\rho_{23}\rho_{34}\rho_{41}.
\]
\item Each adjacent residue contributes the prescribed
spinor-helicity factor \(\langle i\,i{+}1\rangle^{-1}\), and the
negative-helicity pairing contributes \(\langle23\rangle^4\).
\item Their product is the Parke--Taylor scalar:
\[
\mathcal R^{(+,-,-,+)}_{4,\mathrm{cyc}}
\bigl(\omega_4^{(+,-,-,+)}\wedge\nu_4\bigr)
\;=\;
...
\begin{proof}
Work in the holomorphic-topological twist of \(4\)d
\(\mathcal{N}=2\) Yang--Mills.  The boundary theory on
\(\mathbb{C}\times\{0\}\) is \(\widehat{\mathfrak{g}}_k\), and the
bulk theory on \(\mathbb{C}\times\mathbb{R}_{>0}\) is holomorphic
Chern--Simons fibred over~\(\mathbb{R}_+\).  The proof below is a
calculation in the residue datum, not an independent construction of
the physical scattering map.
...
By Stokes' theorem and the FM residue calculus, the boundary
contribution of a collision divisor is the OPE residue of the
colliding pair.  The colour-order projection \(P_{\mathrm{cyc}}\)
keeps only the adjacent cyclic channels.  Under the residue character
\(\rho_{i,i+1}\) of Definition~\ref{def:cbt-pt-residue-datum}, each
adjacent cyclic collision contributes
\(\langle i\,i{+}1\rangle^{-1}\).
...
\emph{Assembly.}
Multiplying the four adjacent residue characters and the
negative-helicity pairing gives
\[
\rho_{--}\rho_{12}\rho_{23}\rho_{34}\rho_{41}
=
\frac{\langle 23\rangle^4}
{\langle 12\rangle\,\langle 23\rangle\,
\langle 34\rangle\,\langle 41\rangle}.
...
For general \(n\), the bar-complex integral over
\(\overline{\mathrm{FM}}_n(\mathbb{C})\) has boundary strata indexed
by labelled stable trees.  A Parke--Taylor theorem at general \(n\)
requires a cyclic residue datum on those strata, compatibility with
the bar differential, and the BCFW recursion identifying the resulting
residue sum with
\(\prod_i\langle i\,i{+}1\rangle^{-1}\).  This is a construction
problem unless that recursive datum is supplied; it is not a
consequence of the four-point residue identity alone.
\end{remark}
