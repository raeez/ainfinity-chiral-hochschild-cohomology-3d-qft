...
of~$\Theta^{\mathrm{oc}}$ at each arity.  The core results are:
...
\item The arity-$4$ bar weight form factorises over the five planar binary trees with $4$~leaves in the colour-ordered sector.
...
\]
tensored with the arity-$4$ weight form.

\emph{Weight form and tree sum.}
The arity-$4$ weight form is $\omega_4 = \sum_{\alpha=1}^{5} \omega_{T_\alpha}$, a sum over the five planar binary trees.  For example, the tree with subtrees $\{1,2\}$ and $\{3,4\}$ contributes $\omega_{T_{12|34}} = d\log(z_1 - z_2) \wedge d\log(z_3 - z_4) \wedge d\log(z_{12} - z_{34})$.
