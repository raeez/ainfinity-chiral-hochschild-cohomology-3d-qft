...
\begin{theorem}[Bulk $=$ Chiral Hochschild Cochains (boundary-linear sector);
\ClaimStatusProvedHere]
\label{thm:bulk_hochschild}
Let\/ $\cA$ be a logarithmic\/ $\SCchtop$-algebra and let $A_{\partial}$ be the boundary chiral algebra obtained by imposing Dirichlet or Neumann boundary conditions.  In the boundary-linear exact sector, bulk local operators are quasi-isomorphic to chiral Hochschild cochains:
\begin{equation}
\mathcal{O}_{\textup{bulk}} \simeq C^\bullet_{\textup{ch}}(A_{\partial}, A_{\partial}).
\end{equation}
The identification is proved unconditionally for any logarithmic\/ $\SCchtop$-algebra.
...
only in the boundary-linear case
\textup{(}Theorem~\textup{\ref{thm:boundary-linear-bulk-boundary}}\textup{)};
the global case is conjectural---see Section~\textup{\ref{sec:ht-bulk-boundary-line}}
for a detailed scope table.
\end{theorem}
...
\]
where $\cC_\partial$ is the dg-category of boundary conditions (Definition~\ref{def:oc-factorization-category}).  This identification is \emph{Morita invariant}: it depends only on the category $\cC_\partial$, not on the choice of vacuum object $b$ used to present the boundary algebra $A_\partial = \End(b)$.  The passage from $Z_{\mathrm{der}}(\cC_\partial)$ to $C^\bullet_{\mathrm{ch}}(A_\partial, A_\partial)$ is via the standard identification of the derived center of a compactly generated dg-category with the Hochschild cochain complex of any compact generator's endomorphism algebra \cite{Kel06}.
\end{remark}

\begin{remark}[Scope of the bulk--Hochschild identification]
\label{rem:bulk-hochschild-scope}
The quasi-isomorphism $\mathcal{O}_{\mathrm{bulk}} \simeq C^\bullet_{\mathrm{ch}}(A_{\partial}, A_{\partial})$ is proved unconditionally for any logarithmic $\SCchtop$-algebra; the scope qualifier in the theorem header refers to the \emph{triangle composition} (that bulk $\simeq$ derived center of boundary), which further requires compact generation and the derived center quasi-isomorphism.  These are verified rigorously only in the boundary-linear exact sector (Theorem~\ref{thm:boundary-linear-bulk-boundary}).  The global triangle for all HT theories remains conjectural; see Section~\ref{sec:ht-bulk-boundary-line} for a detailed scope table.
\end{remark}
...
\medskip
\noindent\textbf{Polynomial structure.}
...
$H^1=\Bbbk\langle\partial_c T,\,\partial_c W_3,\,
\partial_c W_4\rangle$ (three-dimensional: central-charge
variation for each generator of spin $s=2,3,4$).
The bulk algebra is
$V_T\cong\Bbbk[[c]]\otimes
...
The three-dimensional $H^1$ produces the $3\times 3$
...
conformal weight is inner except for the one-parameter
family $\partial_c$ acting independently on each generator:
$\partial_c T$, $\partial_c W_3$, $\partial_c W_4$ record
the infinitesimal shifts of the OPE central terms under
$c \to c + \varepsilon$.  These are outer (they shift
structure constants without being generated by an inner
automorphism), giving $H^1 = \Bbbk^3$.
\end{computation}
