...
\begin{theorem}[CDG Compatibility]
...
\begin{theorem}[Bulk--cochains identification]
\label{thm:bulk-CHC}
Assume \textrm{(H1)--(H4)}.
...
\]
parametrized by points in $\mathbb{X}_{k,m}$, i.e., by integration of propagator kernels over $\FM_k(\C) \times \Conf_m(\R)$.  By the Swiss--cheese recognition (Conjecture~\ref{conj:recognition}), these operations assemble into a $W(\SCchtop)$--algebra, and the totality of such operations is precisely the operadic cochain complex
\[
