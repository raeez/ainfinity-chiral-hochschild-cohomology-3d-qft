...
(Computation~\ref*{comp:kz-from-graph-sum} of Vol~I).
\end{computation}
...
Theorem~\ref*{thm:cech-hca}).  The bridge quasi-isomorphism
...
Hochschild complex (Volume~I,
\S\ref*{sec:cosimplicial-hochschild}).
...
brace bracket, $F_3$ is the Jacobiator homotopy
(Proposition~\ref*{prop:jacobiator-nullhomotopic} of Volume~I),
...
The upshot is that the polynomial structure
$\ChirHoch^*(\mathcal{W}^k(\fg)) \cong
\mathbb{C}[\Theta_1,\ldots,\Theta_r]$ is not merely a
cohomological accident: it reflects the \emph{strict} HCA
structure of $\mathcal{W}$-algebras at generic level, in which
all homotopy corrections $F_n$ ($n \geq 3$) are contractible
by the same mechanism that produces PBW concentration on the
bar side (Volume~I, MC1).
\end{remark}
...
=\Bbbk\langle\partial_c T\rangle$ (one-dimensional,
generated by the central-charge variation).
