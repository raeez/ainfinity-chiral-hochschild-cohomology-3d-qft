...
We now develop the geometric interpretation of bulk observables via chiral Hochschild cohomology, following the pattern of dimensional descent from 2d Poisson sigma models and connecting to Khan--Zeng's 3d HT Poisson sigma model \cite{KZ25}.
...
The key insight (Cattaneo--Felder, Kontsevich \cite{Kon03}) is that the space of fields can be rewritten as:
\begin{equation}
...
We now establish the precise relationship between bulk local operators and chiral Hochschild cochains.
...
\begin{conjecture}[Hochschild bridge to Volume~I; \ClaimStatusConjectured]
\label{conj:hochschild-bridge}
Let $\cA$ be a chiral Koszul algebra on a curve~$X$ (in the sense of
Volume~I, Theorem~B).  The bulk--cochains identification
(Theorem~\ref{thm:bulk-CHC}) specializes to give a canonical
quasi-isomorphism
\[
...
Volume~I.  On the Koszul locus, this complex is polynomial in
$r$~generators of degrees $m_i + 1$ (Volume~I, Theorem~H).  The
Connes periodicity operator~$S$ on $\ChirHoch^\bullet$ corresponds
to the holomorphic weight shift on the filtration $F^\bullet$ above.
\end{conjecture}

\begin{remark}
The conjecture reduces the physical computation (bulk observables
of the 3d HT theory) to the algebraic computation (chiral
Hochschild cohomology of the vertex algebra).  Evidence:
(i)~the associated graded decomposition above matches the
BD--chiral Hochschild structure of Volume~I;
(ii)~for the Heisenberg algebra, both sides are polynomial in
one generator of degree~$2$ (the stress tensor);
(iii)~the Koszul duality functor acts on both sides via the
Connes operator.
\end{remark}
