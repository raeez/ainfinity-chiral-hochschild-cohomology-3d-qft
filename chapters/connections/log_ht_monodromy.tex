...
From that point of view, the different worlds that appeared throughout the thread---chiral and factorization algebra, $A_\infty$ deformation theory, dg-shifted Yangians, Fulton-MacPherson compactifications, perturbative holomorphic-topological field theory, logarithmic geometry, pure braid groups, periods, and spectral/chromatic stabilization---stop being parallel themes and become successive shadows of one mechanism.
...
The purpose of this article is to develop that mechanism from first principles, fortify each stage, and package the result into a single theorem machine. The content is new to the extent that it is not merely a restatement of any single source: it synthesizes ideas and extends them into explicit constructions and theorem packages not present, as far as we know, in the current literature. Whenever a result depends on analytic or perturbative inputs external to the manuscript, we say so explicitly.

\begin{theorem}[Synthesis theorem]\label{thm:synthesis}
...
The present manuscript is designed to be self-contained in its formal development, but it is motivated by and builds on several currents in the literature: Beilinson--Drinfeld's chiral/factorization viewpoint \cite{BD}; Ayala--Francis on factorization homology \cite{AF}; Costello--Dimofte--Gaiotto on holomorphic twists and boundary chiral algebras \cite{CDG}; Gaiotto--Kulp--Wu on higher operations and compactified configuration-space regularization \cite{GKW}; Khan--Zeng on three-dimensional Poisson-vertex gauge theory \cite{KZ}; Mok on logarithmic Fulton-MacPherson spaces \cite{Mok}; Dimofte--Niu--Py on line operators and dg-shifted Yangians \cite{DNP}; and Latyntsev on factorisation quantum groups \cite{Lat}. The results of this article should be read as a structural extension and synthesis of that material rather than a summary of it.

\section{First principles and conventions}\label{sec:foundations}
...
\begin{lemma}[Bar insertion identity]\label{lem:bar-insertion}
...
\]
for Mok's logarithmic Fulton-MacPherson compactification of the pair $(\PP^1,\infty)$ \cite{Mok}. Near a boundary stratum indexed by a collision tree $T$, there exist local scale coordinates $t_e$ indexed by internal edges $e\in E(T)$ such that pairwise differences factor as
\[
...
The full notion of dg-shifted Yangian belongs to a broader program \cite{DNP}. For the strict rational theory developed below we isolate only the structure actually used.
...
\begin{theorem}[Flatness of the shifted KZ/FM connection]\label{thm:strict-flatness}
...
\begin{lemma}[Residues and infinitesimal braid operators]\label{lem:inf-braid}
...
\begin{proposition}[Logarithmic extension and edge residues]\label{prop:strict-log-ext}
...
\begin{corollary}[Local monodromy around a boundary divisor]\label{cor:strict-monodromy}
...
\begin{theorem}[Master curvature formula]\label{thm:master-curvature}
...
\begin{theorem}[Support hierarchy]\label{thm:support-hierarchy}
...
\begin{corollary}[The first three curvature layers]\label{cor:first-three}
...
\begin{theorem}[First obstruction theorem]\label{thm:first-obstruction}
...
\begin{corollary}[Recursive support filtration]\label{cor:recursive-support}
...
\[
  M=\CC_z\times \RR_t
\]
in the quasi-linear class highlighted in \cite{DNP}. Concretely, this includes holomorphic-topological twists of many $3$d $\mathcal N=2$ gauge theories with arbitrary Chern-Simons level, linear matter, and superpotential, under the assumptions for which the perturbative line OPE is exactly resumable. Let $L_1,\dots,L_n$ be perturbative line operators supported on parallel copies of the topological direction, with state spaces $V_1,\dots,V_n$.

The analytic data we need are the graph-valued forms computing the higher operations of the theory in the sense of \cite{GKW,DNP}. The issue is that these forms live on singular configuration spaces. The cure is to compactify and logarithmically resolve the relevant collision strata.
...
\begin{proposition}[Local coordinates and log smoothness]\label{prop:mixed-log-smooth}
...
\]
constructed from regularized propagators in the sense of \cite{GKW,DNP}. Pulling back to a boundary chart of $\mathfrak C_\Gamma(\mathbf z)$, the form has an asymptotic expansion of the shape
\[
...
\begin{theorem}[Existence, logarithmicity, and residue factorization]\label{thm:log-bphz-existence}
...
\begin{theorem}[Analytic curvature formula]\label{thm:analytic-curvature}
...
\begin{theorem}[Compactified-Stokes flatness]\label{thm:analytic-flatness}
...
\begin{theorem}[Binary residues and the analytic $A_\infty$ Yang-Baxter identity]\label{thm:analytic-yb}
...
\begin{theorem}[Family version over semistable degenerations]\label{thm:family-version}
...
\begin{theorem}[Quantum concentration from classical concentration]\label{thm:quantum-concentration}
Suppose
\[
...
\begin{corollary}[Flat $\h$-adic deformation of reduced states]\label{cor:flat-hbar}
...
\begin{theorem}[Canonical $\h$-adic strong deformation retract]\label{thm:canonical-retract}
...
\begin{proposition}[Analytic convergence under norm smallness]\label{prop:analytic-retract}
...
\begin{theorem}[One-form rigidity]\label{thm:one-form-rigidity}
...
\begin{corollary}[Reduced residues]\label{cor:reduced-residues}
...
\begin{lemma}[Commuting corner residues]\label{lem:commuting-residues}
...
\begin{lemma}[Tangential regularization]\label{lem:tangential-regularization}
...
\begin{theorem}[Hexagon identities]\label{thm:hexagon}
...
\begin{corollary}[Monodromic braided tensor structure]\label{cor:braided-category}
...
\begin{theorem}[Local monodromy and pure braid representations]\label{thm:pure-braid}
...
\begin{corollary}[Logarithmic periods]\label{cor:periods}
...
\begin{proposition}[Regular-sequence resolution criterion]\label{prop:regular-sequence}
...
\begin{lemma}[Explicit linear homotopy]\label{lem:linear-homotopy}
...
\begin{theorem}[Unconditionality for resolved quasi-linear lines]\label{thm:unconditionality}
...
\begin{theorem}[Boundary factorization after reduction]\label{thm:reduced-boundary-factorization}
...
\begin{proposition}[Arithmetic form of the reduced theory]\label{prop:arithmetic-form}
...
\begin{proposition}[Decorated degeneration formula]\label{prop:decorated-degeneration}
...
\begin{theorem}[Smashing localization commutes with factorization homology]\label{thm:smashing-localization}
...
\begin{corollary}[Periodicity is local-to-global stable]\label{cor:periodicity}
...
\begin{conjecture}[Monodromy equals spectral $R$-matrix]\label{conj:rmatrix}
...
\begin{conjecture}[Logarithmic CoHA envelope]\label{conj:coha}
...
\begin{conjecture}[Chromatic lift of the HT associator]\label{conj:chromatic-lift}
...
\begin{theorem}[Platonic ideal theorem]\label{thm:platonic-ideal}
...
\emph{A factorization homology primer},
arXiv:1903.10961.

\bibitem{BD}
A.~Beilinson and V.~Drinfeld,
\emph{Chiral Algebras},
American Mathematical Society Colloquium Publications, vol.~51, 2004.
