...
Work over a characteristic-zero ground field $\kk$. Unless stated otherwise, grading conventions are cohomological and tensor products are completed when necessary.

\subsection{Suspended bar conventions}
Let $A$ be an $A_\infty$ algebra. Use the suspended convention: the structure maps are
\[
...
\]
for the ordered configuration space. Set
\[
...
\begin{theorem}[Hodge $\h$-adic strong deformation retract; \ClaimStatusProvedHere]% label removed: thm:hodge-hbar-retract
...
\section{Resolved logarithmic HT data}% label removed: sec:platonic
...
\begin{theorem}[Resolved logarithmic HT monodromy theorem; \ClaimStatusProvedHere]% label removed: thm:platonic-ideal
...
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}), the
...
\]
whose \(\bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\sigma)^{24})\bigr)c_{\Phi_{10}}\) summand transgresses
...
equal to $K^{\kappaChHodge}$ and $2K^{\kappaChHodge}$
respectively under the Mukai-doubling identity
$K^{\kappaChHodge} = 2c_+ = 8$ on the Lorentzian
...
genus-$2$ face of the logarithmic-HT spine, and it is
the site where the five-level chain engages with the Siegel-modular
content of the K3 Hall--Borcherds construction. Primary-lit:
...
\]
The leading-order formula $R(z)=1+k\,\Omega_{\mathrm{tr}}/z+O(z^{-2})$
...
\section{Synthesis}
\label{sec:lhm-supplement}

\begin{remark}[Log-HT monodromy at the Humbert divisor]
\label{rem:lhm-1}
The K3 log-HT monodromy is controlled by the Humbert divisor
