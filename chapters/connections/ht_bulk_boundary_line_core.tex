...
\end{equation}
Here $\cC_\partial$ denotes the open-sector factorization category (Definition~\ref{def:oc-factorization-category}); $B_\partial = \End_{\cC_\partial}(b)$ is the boundary algebra for a chosen vacuum $b$; and $\cC_{\mathrm{line}} \simeq \cA^!\text{-}\mathbf{mod}$ is the line-operator category (Theorem~\ref{thm:lines_as_modules}).  All three identifications are Morita invariant (Corollary~\ref{cor:bulk-derived-center-categorical}).
...
Closed-color Koszul duality identifies the bulk:
$A_{\mathrm{bulk}} \simeq \mathrm{ChirHoch}^\ast(\cA^!)$
(Theorem~\ref{thm:bulk_hochschild}).
Open-color Koszul duality identifies line operators:
...
The triangle is the statement that $\cA^!$ is a dg-shifted
Yangian (Theorem~\ref{thm:Koszul_dual_Yangian}), with
$(\cA^!)^! \simeq \cA$
(Theorem~\ref{thm:duality-involution}): all three vertices
and all connecting maps are recovered from this single
...
arity~$2$ to the spectral $R$-matrix $r(z)$, at arity~$3$ to
...
condition.
The equivalence
$\cC_{\mathrm{line}} \simeq A^!\text{-mod}$ is the
convolution-module identification: modules for the
self-intersection algebra are modules for the Koszul dual.
...
Volume~I (Theorem~A), whose projections recover
...
= 10/[c(5c+22)]$ (Volume~I,
Theorem~\ref{thm:quartic-contact-shadow}) is a concrete instance
...
algebra, the bar complex carries no differential beyond the
...
relation).
For a cubic algebra, the arity-$3$ Stasheff relation---which
expresses $m_3$ as a homotopy for the failure of $m_2$ to be
...
binary vertices---these are already accounted for by
