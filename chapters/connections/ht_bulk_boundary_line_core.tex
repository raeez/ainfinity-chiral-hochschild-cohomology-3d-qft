...
The open/closed MC element $\Theta^{\mathrm{oc}} = \Theta_A + \sum \mu^{M_j}$ has three algebraic faces: the universal bulk $\cZ^{\mathrm{der}}_{\mathrm{ch}}(\cA)$ (chiral derived center), the boundary algebra~$\cA$ (open-color chart), and the line-sector operations on~$\cC_{\mathrm{line}}$ (open-color face).  On the chirally Koszul locus, $\cC_{\mathrm{line}} \simeq \cA^!_{\mathrm{line}}\text{-mod}$.  The corrected triangle, under compact-generation and derived-center hypotheses, is:
...
(Theorem~\ref{thm:lines_as_modules}).  The
center descriptions are Morita invariant
...
cochains of the boundary, while the displayed derived-center and
line-side module models retain the extra scope just stated.
In that scoped setting, the bulk is the \emph{derived center} of the
boundary, not the boundary itself; the line category is modeled by
modules for the open-colour Koszul dual~$\cA^!_{\mathrm{line}}$, not for the
bulk algebra.  This is the 3d
...
The corrected triangle admits a two-color refinement.
Closed-color Hochschild theory identifies the bulk:
$A_{\mathrm{bulk}} \simeq C^\bullet_{\mathrm{ch}}(\Bbound,\Bbound)$
(Theorem~\ref{thm:bulk_hochschild}, where
$\Bbound = A_\partial$ in that theorem's notation).
...
filtration on $\SCchtop$
(Theorem~\ref{thm:spectral-parameter-from-closed-color}).
On that same chirally Koszul locus, the triangle packages the
statement that $\cA^!_{\mathrm{line}}$ carries the dg-shifted
Yangian package (Theorem~\ref{thm:yangian-recognition}), while the full
two-colour Koszul duality involution returns~$\cA$
(Theorem~\ref{thm:duality-involution}).  Together with the
boundary-linear derived-center theorem recalled above, these
results organize the scoped corrected triangle: the Yangian package and
line-side package control the line corner, while the
bulk/derived-center corner still uses the extra compact-generation
and derived-center hypotheses just stated.

Algebraically, the open-color structure is controlled by the
$\mathsf{E}_1$ modular convolution algebra
$\mathfrak{g}_\cA^{\mathrm{mod},E_1}$
of Vol~I (\S\ref{sec:e1-holographic-bridge}): the
Maurer--Cartan element $\Theta_\cA^{E_1}$ specialises at
arity~$2$ to the spectral kernel $r(z)$, at arity~$3$ to
the Drinfeld associator, and its $\Sigma_r$-coinvariant
recovers the closed-color shadow tower $\kappa, C, Q, \ldots$
of the bulk algebra.
\end{remark}

\begin{remark}[Steinberg presentation of the corrected triangle]
\label{rem:steinberg-triangle}
\index{bulk-boundary-line!Steinberg presentation}
The three vertices of the corrected triangle live on three
different spaces related to the Lagrangian inclusion
$\cL \hookrightarrow \cM$ and its self-intersection
$\mathfrak{S}_b = \cL \times_{\cM} \cL$.
Precisely:
\begin{itemize}
\item The bulk algebra is
  $A_{\mathrm{bulk}} \simeq \cO(T^\ast[-1]\cL)$, the algebra of
  polyvector fields on the Lagrangian (equivalently, the
  Hochschild cochains of $\cO(\cL)$).  Note that the
  restriction $\cO(\cM)|_{\cL} = \cO(\cL)$ is the
  \emph{boundary}, not the bulk; the bulk retains the full
  $(-1)$-shifted cotangent fibre.
\item The boundary $B_\partial = \cO(\cL)$ is functions
  on the Lagrangian itself, on $\cL$, not on
  $\mathfrak{S}_b$.
\item The line category
  $\cC_{\mathrm{line}} = \mathfrak{S}_b\text{-mod}$ is
  modules for the convolution algebra on the
  self-intersection; this is the vertex naturally
  associated to $\mathfrak{S}_b$.
\end{itemize}
The connection between these three different loci is the
Lagrangian condition
$T_{\cL}\cM / T\cL \simeq T^*\cL[-1]$, which mediates
between $\cM|_{\cL}$ and $\cL$ and
$\mathfrak{S}_b$ as follows.
The equivalence $Z_{\der}(B_\partial) \simeq A_{\mathrm{bulk}}$
is the HKR theorem: Hochschild cochains of $\cO(\cL)$
recover polyvector fields, which are functions on the shifted
cotangent $T^*[-1]\cL \simeq \cM|_{\cL}$ by the Lagrangian
condition.
The equivalence
$\cC_{\mathrm{line}} \simeq \cA^!_{\mathrm{line}}\text{-mod}$ is the
convolution-module model: the line category is modeled by modules for
the self-intersection algebra, equivalently by modules for the
open-colour Koszul dual.
Both equivalences require the Lagrangian condition, which makes
the normal bundle self-dual and the Steinberg algebra Koszul
(this requires additional geometric input, such as purity of the
relevant IC sheaves; the Lagrangian condition alone is necessary
but not sufficient).
\end{remark}
...
\]
which vanishes when the $w_i$ are coordinate functions, since $\partial_{\psi_i}(w_j)=\delta_{ij}$ gives cancelling terms.  At higher arity, $\Theta_W=\sum_{n\ge 1}\sum_{a_1,\ldots,a_n}f_{a_1\cdots a_n}(x)\,\partial_{\psi_{a_1}}\wedge\cdots\wedge\partial_{\psi_{a_n}}$ where the coefficients $f_{a_1\cdots a_n}(x)$ depend only on the tangent coordinates~$x$ and thus lie in the kernel of every $\partial_{\psi_a}$.  The bracket $[\Theta_W,\Theta_W]_{\sn}$ reduces to a sum of terms each involving $\partial_{\psi_a}(f_{b_1\cdots b_m})=0$, so
\[
...
Completeness holds because $\mathrm{Conv}^{\mathrm{mod}}$ is defined as a product over genera: $\mathrm{Conv}^{\mathrm{mod}} = \prod_{g \geq 0} \mathrm{Conv}^{(g)}$, where each $\mathrm{Conv}^{(g)}$ is finite-dimensional (only finitely many stable graphs of genus $g$ with fixed arity contribute).  The product topology is complete by definition, and $\mathrm{Conv}^{\mathrm{mod}} = \varprojlim_g \mathrm{Conv}^{\mathrm{mod}} / F^{\leq g}$.
The
...
$\kappa(\cA)\cdot\omega_g$ by Volume~I Theorem~D
(leading coefficient).
...
(the genus-$0$, arity-$\leq 3$ component of~$\Theta_\cA$).
...
arity filtration.  The class $\mathfrak{q}_4(\cA)$ is the
obstruction to extending the local genus-$0$ open/closed theory
from arity~$\leq 3$ across the first codimension-$2$ logarithmic
Fulton--MacPherson strata at arity~$4$.
\end{definition}
...
\item[\textbf{M} (Mixed, $d_{\mathrm{alg}}=\infty$):]
  ...
  higher operations contributes at every arity.
  \emph{Example:} Virasoro at generic~$c$; $\cW_3$ at non-generic central charge.
\end{description}
\end{proposition}

\begin{proof}
The shadow depth $d(\cA)$ is defined by the arity filtration on
...
quantum Yang--Baxter equation (the arity-$2$ MC equation in the
$E_1$-modular convolution algebra already encodes the quantum
relation).
For a cubic algebra, the arity-$3$ Stasheff relation (expressing $m_3$ as a homotopy for the failure of $m_2$ to be
...
operations at every arity (as happens for $\cW_3$ at
...
\medskip
\noindent\textbf{Step 2: Bar differential at arity 2.}
The bar differential at arity~$2$ encodes both the
...
structure constants are determined by lower-arity
operations, so $d_{\mathrm{alg}}\ge 1$.

\medskip
\noindent\textbf{Step 3: Vanishing of independent
  higher operations.}
At arity~$3$, the bar differential computes the triple
...
iterating $m_2$.  The arity-$3$ Stasheff relation
...
arity: if $m_2,\ldots,m_{n-1}$ are all determined by
$m_2$, the arity-$n$ Stasheff relation expresses $m_n$ in
terms of compositions of $m_2,\ldots,m_{n-1}$, hence in
terms of $m_2$ alone.  Therefore the bar coderivation is
determined by its arity-$2$ component, giving
$d_{\mathrm{alg}}=1$.
...
r(z) \;=\; \frac{\Omega}{z}
\;=\; \frac{1}{z}\sum_{a} J^a \otimes J_a,
\end{equation}
where $\Omega = \sum_a J^a\otimes J_a$ is the quadratic
Casimir element of $\mathfrak{sl}_2$.  One checks directly
...
= k$, and the genus-$1$ obstruction takes the form
\begin{equation}\label{eq:sl2-genus1}
\Theta^{(1)} \;=\;
-k \cdot \Gamma_{\text{self-loop}} \otimes \Lambda,
\end{equation}
...
$d_0\,\Theta^{(1)} = -k\,\omega_1$ together with
$d_0(\Gamma_{\text{self-loop}} \otimes \Lambda) = \omega_1$.)
The genus-$1$ free energy for
$\hat{\mathfrak{sl}}_2$ at level~$k$ is
\[
F_1(\hat{\mathfrak{sl}}_2,\, k)
\;=\;
-3 \cdot \frac{k}{k+2} \cdot \log\,\eta(\tau),
\]
where $3 = \dim(\mathfrak{sl}_2)$ and the factor
$k/(k{+}2)$ is the ratio of the level to the shifted
level $k + h^\vee$ (with $h^\vee = 2$).  This is the
standard one-loop partition function of the WZW model:
$\dim(\mathfrak{g})$ free bosons, each contributing
$-\frac{1}{24}\log\eta(\tau)$, rescaled by the Sugawara
ratio $k/(k{+}h^\vee)$.  The structural form matches the
...
Furthermore, each of the one-dimensional subspaces
...
complementary free energy $F_1(\cH_{-k}) = (k/24)\log\eta(\tau)$
satisfies $F_1(\cH_k) + F_1(\cH_{-k}) = 0$: the genus-$1$
contributions of Koszul-dual Heisenberg algebras cancel exactly,
...
appearing first at weight~$2$ (arity~$4$).  The corresponding
...
lower-arity operations.  They witness the irreducible
