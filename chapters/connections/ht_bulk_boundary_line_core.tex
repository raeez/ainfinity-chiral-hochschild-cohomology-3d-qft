...
The triangle is the statement that $\cA^!$ carries a full
...
The three vertices of the corrected triangle are three sheaves
on the Steinberg self-intersection
$\mathfrak{S}_b = \cL \times_{\cM} \cL$,
where $\cL \hookrightarrow \cM$ is the Lagrangian inclusion
of the boundary into the ambient derived stack.
The bulk $A_{\mathrm{bulk}} = \cO(\cM)|_{\cL}$ is the
restriction of the ambient structure sheaf to the Lagrangian.
The boundary $B_\partial = \cO(\cL)$ is functions on the
Lagrangian itself.
The line category
$\cC_{\mathrm{line}} = \mathfrak{S}_b\text{-mod}$ is modules
for the convolution algebra on the self-intersection.
The equivalence $Z_{\der}(B_\partial) \simeq A_{\mathrm{bulk}}$
is the HKR theorem: Hochschild cochains of functions on $\cL$
...
Both equivalences are consequences of the Lagrangian condition
$T_{\cL}\cM / T\cL \simeq T^*\cL[-1]$, which makes the
normal bundle self-dual and the Steinberg algebra Koszul.
\end{remark}
