...
(extended along $\mathbb{R}_t$). Bulk operators form a commutative
algebra; boundary operators are noncommutative; line operators carry
a monoidal structure from fusion. The three operator classes live at
three different categorical levels, and the correct relation among
them is forced by the geometry of the slab. Place $T$ on
...
The functor $F$ is the candidate fiber functor for a Tannakian
reconstruction of the line category. Its tensor endomorphism algebra
$U_F:=\End^\otimes(F)$ is a Drinfeld double only after the Hopf
structure, pairing, completion, and meromorphic braiding have been
constructed. The precise theorem is the following.

\begin{theorem}[Slab Hopf reconstruction; \ClaimStatusProvedHere]
\label{thm:slab-hopf-reconstruction}
Let $T$ be a perturbative $3$d HT theory on
$\mathbb C_z\times I$ with transverse boundary conditions $D,N$.
Let $\widehat{\cC}_{\mathrm{line}}$ be the conilpotent completed,
compactly generated line category, and let
$F:\widehat{\cC}_{\mathrm{line}}\to\mathrm{Vect}$ be the slab functor
\eqref{eq:slab-fiber-functor}. Assume:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=1.8em]
\item $F$ is exact, conservative, monoidal, and preserves compact
generators;
\item the wall algebras $\cA_D$ and $\cA_N$ are completed Hopf
$E_1$-chiral algebras in the same tensor category, with
$\cA_N\simeq\cA_D^!$ on the chirally Koszul locus;
\item hemisphere amplitudes give a non-degenerate Hopf pairing
$\langle-,-\rangle:\cA_D\otimes\cA_N\to\kk$;
\item interval orientation reversal supplies antipodes and line-cutting
supplies coproducts compatible with the pairing;
\item the meromorphic braiding $R(z)$ satisfies the quasi-triangular
identities for a chosen Drinfeld/KZ associator.
\end{enumerate}
Then Tannakian reconstruction identifies
\[
 U_F=\End^\otimes(F)
 \;\simeq\;
 \cA_D\bowtie\cA_N
 ...
 \cA_D\bowtie\cA_D^!
\]
as a completed quasi-triangular Hopf $E_1$-chiral algebra, and
\[
\widehat{\cC}_{\mathrm{line}}
\;\simeq\;
(\cA_D\bowtie\cA_D^!)\textup{-}\mathbf{mod}.
\]
The bulk is recovered from this line side only after the separate
derived-centre comparison
$\beta_{\der}:A_{\mathrm{bulk}}\to Z_{\der}(B_\partial)$ is a
quasi-isomorphism.

If the five inputs above are not supplied, the obstruction to this
Drinfeld-double reconstruction is the MC lifting class
\[
 o_{\mathrm{slab}}\in
 H^2\!\left(
 \operatorname{Cone}\!\left(
 \operatorname{Def}_{\mathrm{Hopf}}
   (\cA_D,\cA_N,\langle-,-\rangle,R)
 \longrightarrow
 \operatorname{Def}_{\otimes}
   (\widehat{\cC}_{\mathrm{line}},F)
 \right)[1]\right).
\]
Vanishing of $o_{\mathrm{slab}}$, together with a choice of lift, is
exactly the missing chain-level Hopf/module-category comparison.
\end{theorem}

\begin{proof}
The hypotheses make $F$ a completed monoidal fiber functor; standard
Tannakian reconstruction gives the completed tensor endomorphism Hopf
algebra $U_F=\End^\otimes(F)$ and identifies
$\widehat{\cC}_{\mathrm{line}}$ with $U_F$-modules. A non-degenerate
Hopf pairing between the two wall Hopf algebras, together with
compatible antipodes, coproducts, and the quasi-triangular
$R$-matrix, is precisely Drinfeld's double construction in the
completed chiral tensor category, hence $U_F\simeq\cA_D\bowtie\cA_N$.
The chirally Koszul hypothesis gives $\cA_N\simeq\cA_D^!$. The
obstruction complex is the deformation-theoretic cone comparing
deformations of the Hopf-double datum with deformations of the
monoidal fiber functor; its $H^2$ class is the usual obstruction to
lifting an infinitesimal monoidal reconstruction to an MC solution.
The final bulk statement is independent: it is the Morita-invariant
derived-centre comparison, not a consequence of the Drinfeld double.
\end{proof}

Thus the slab forces a bimodule reconstruction problem because two
transverse boundary conditions produce two algebraic inputs; it does
not by itself construct the global Drinfeld double. Complementarity
(Theorem~C of Volume~I) is the algebraic manifestation of the
transversality:
...
\label{rem:ht-global-de-rham-ht-bulk-boundary-line-core}
The corrected global triangle below, together with every downstream
identification of bulk chiral Hochschild cohomology with the
derived centre of the open sector, presupposes a passage from the
formal-local slab geometry to a chiral-algebra statement on the
global bulk source curve $C$. This promotion is conditional on
$\hypHTGlobalDR$: vanishing $H^1(X,\C_X) = 0$ for the holomorphic
de~Rham complex of the global manifold, descent of the local
Hamiltonian sheaf~\cite[\S4--5]{CG17}, satisfaction of the
quantum master equation, vanishing of the anomaly polynomial, and
...
3232--3233}). The compact-generation, line-module, and
derived-centre hypotheses below are the categorical avatars of the
same global obstruction.
\end{remark}

The slab geometry produces three vertices. Under compact generation,
the line-module comparison, and the derived-centre comparison, the
...
\begin{corollary}[Boundary--defect slab package;
\ClaimStatusConditional; licensing $\alpha+\beta+\gamma+\delta$]
\label{cor:boundary-defect-slab-package}
Under the hypotheses of Theorem~\ref{thm:slab-hopf-reconstruction},
Theorem~\ref{thm:lines_as_modules}, and the derived-centre comparison
in~\eqref{eq:global-corrected-triangle}, the slab
\(\mathbb C_z\times[0,1]\) carries the four-part datum
\[
\left(
B_\partial=\End_{\cC_\partial}(b),\quad
\Abulk\simeq\Zderch{B_\partial},\quad
\widehat{\cC}_{\mathrm{line}},\quad
(\beta^{\mathrm{hol}}_z,\beta^{\mathrm{cat}})
\right).
\]
Here \(\beta^{\mathrm{hol}}_z\) is the meromorphic spectral exchange
family supplied by the closed-colour collision kernel \(R(z)\), and
\(\beta^{\mathrm{cat}}\) is its transported categorical braiding after
choosing the Drinfeld/KZ associator, the exchange path in
\(\Conf_2(\C)\), and the line-module comparison
\[
\widehat{\cC}_{\mathrm{line}}\simeq
\cA^!_{\mathrm{line}}\textup{-}\mathbf{mod}^{A_\infty}.
\]
The transport
\[
\beta^{\mathrm{hol}}_z\longmapsto\beta^{\mathrm{cat}}
\]
is the spectral-to-categorical half-monodromy of
Proposition~\ref{prop:spectral-to-categorical}; on the affine
evaluation surface it is the KZ--Drinfeld--Kohno--Kazhdan--Lusztig
transport of Proposition~\ref{prop:kz-dk-kl-line-braiding-proof-path}.
Thus the slab theorem relates boundary algebra, bulk derived centre,
line category, and braiding only after these comparison data are
installed; no one of the four objects determines the other three by
itself.
\end{corollary}
...
Higher operations from \cite{GKW25} and the line-operator
...
On the chirally Koszul locus, open-color Koszul duality models the
completed line category:
...
\item Boundary: $B_\partial = \cO(\cL)$, functions on the
 Lagrangian itself.
\item Lines:
 the Steinberg self-intersection gives the module model
 $\cC_{\mathrm{line}}\simeq\mathfrak{S}_b\text{-mod}$ once the
 geometric Steinberg object and compact-generation comparison are
 supplied.
\end{itemize}
...
open-colour Koszul model for this self-intersection algebra on the
completed perturbative locus. These equivalences require the
Lagrangian condition, compact generation, and the geometric
Steinberg comparison; purity of the relevant IC~sheaves is the
remaining input for the full correspondence model.
\end{remark}

\begin{remark}[The slab fiber functor and the Drinfeld double]
\label{rem:slab-fiber-functor}
The slab geometry of the preceding subsection gives the fiber functor
whose Hopf reconstruction is Theorem~\ref{thm:slab-hopf-reconstruction}.
The resulting Drinfeld double $\cA\bowtie\cA^!$ exists only after the
Hopf pairing, antipodes, compatible coproducts, meromorphic braiding,
and completed module-category comparison have been supplied. The slab
is a bimodule with two boundary components ($D$-wall and $N$-wall), not
an $\SCchtop$ disk (which has one open boundary and one closed
interior). This remark records the translation to the monograph's
algebraic framework.
...
descend from the slab geometry when the hypotheses of
Theorem~\ref{thm:slab-hopf-reconstruction} hold: the $D$-wall gives
the boundary, the Hopf-reconstructed double gives the line model, and
the derived center of the boundary gives the bulk after the
...
When the relative analogues of the five inputs of
Theorem~\ref{thm:slab-hopf-reconstruction} are supplied, the Hopf
algebras $\cA$ and $\cA^!$ become \emph{relative} Hopf algebras
internal to $\cC_\cF$. On the chiral algebra side, the
...
\item There is a quasi-equivalence of dg-categories
 ...
 \]
 where $\pi$ is the Koszul comparison obtained by passing
 $\mu$ through $\barB(\cA)$, taking bar cohomology
 $A^i=H^\bullet\barB(\cA)$, and applying Verdier evaluation
 $A^i\otimes\cA^!\to\omega_{\operatorname{Ran}}$.
...
\item The inverse quasi-equivalence
 ...
 where $\tau_{\mathrm{univ}} \in \cA^! \widehat\otimes \cA$
 is the Verdier coevaluation kernel induced by the universal
 twisting morphism $\barB(\cA)\to\cA$.
\end{enumerate}
\end{theorem}
...
has Verdier dual $\cA^! := (H^\bullet(\barB(\cA)))^\vee_{\mathrm{Verd}}$.
Bar-cobar inversion (Volume~I, Theorem~B) gives a quasi-isomorphism
$\Omega(\barB(\cA)) \xrightarrow{\sim} \cA$. The universal
twisting morphism $\barB(\cA)\to\cA$, after bar cohomology and
Verdier duality, gives the coevaluation kernel
$\tau_{\mathrm{univ}} \in \cA^! \widehat\otimes \cA$ mediating
between MC elements and module structures:
an MC coupling $\mu \in \End(V) \widehat\otimes \cA$ is carried by
this comparison to an $\cA^!$-action $\rho$ on~$V$, and this
comparison is a quasi-isomorphism of dg-categories by the
bar-cobar Quillen equivalence applied to modules
(Theorem~\ref{thm:homotopy-Koszul}).

Part~(iii) applies the inverse comparison: given $(V, \rho)$,
the formula $\mu = (\rho \otimes \mathrm{id})(\tau_{\mathrm{univ}})$
produces an MC element because $\tau_{\mathrm{univ}}$ is the Verdier
image of the universal twisting morphism $\barB(\cA)\to\cA$; its MC
equation is the image of the coderivation identity $d_{\barB}^2=0$.
\end{proof}
...
not be concentrated, and the Koszul comparison may not exist.
The theorem refines
...
\section{Literature backbone and formal consequences}\label{sec:backbone}

The foundational inputs are fixed first.

\subsection{Inputs from the literature}

Costello--Dimofte--Gaiotto \cite{CDG20} establishes the cohomology-level bulk algebra, its shifted Poisson bracket, the action of bulk on boundary operators, and the expectation of higher derived operations and of a derived bulk--boundary comparison map. Zeng~\cite{Zeng23} identifies boundary chiral algebras with universal defect algebras in families of examples. Gaiotto--Kulp--Wu~\cite{GKW25} supplies the perturbative technology constructing higher operations and proving the quadratic identities they satisfy in a broad holomorphic--topological setting. Dimofte--Niu--Py~\cite{DNP25} models the line category by modules over a Koszul-dual $A_\infty$ algebra and constructs the dg-shifted-Yangian structures organizing their OPE.
...
The proposition supplies the formal envelope inside which the construction proceeds.
...
\subsection{Microlocal algebraic core}

The microlocal algebraic core strengthens the literature. Inside a large but exact local sector, the boundary, line, and bulk vertices admit explicit constructions and theorem-level comparisons.
...
The boundary and line sectors do not stand beside the bulk as parallel
commutative algebras. They sit one categorical level lower. The local
operator algebra is the chosen bulk observable algebra. The algebra of
endomorphisms of a boundary vacuum or of a line defect is an open-sector
presentation. The bridge from open data back to bulk data, when it
...
\subsection{Centers from open sectors}
...
\]
This is the precise sense in which a model bulk can be compared with
the centre of the line sector while lines themselves are controlled by
...
\subsection{The exact local sector}

A global theorem comparing bulk, boundary, and line categories in every holomorphic--topological theory requires a local exact sector in which the objects admit literal formulas. The boundary-linear Landau--Ginzburg sector plays that role. In that sector:
...
Choose, only locally for writing formulas, a splitting $V=L\oplus N$. Then $Q\cong N$ and one may write
\[
...
The intrinsic boundary construction is a stable algebraic object under three test operations: external sums, tangent-normal hyperbolic stabilizations, and pure-normal hyperbolic stabilizations.
...
algebra with polyvector fields. In the dg setting here, the derived
center of the boundary dg algebra is the shifted cotangent algebra of
the derived boundary zero locus. Since the boundary-linear LG bulk
...
\begin{remark}
This theorem is the first local incarnation of the principle that the line sector does not merely receive data from the boundary equations; it \emph{presents} them.
\end{remark}
...
\eqref{eq:massive-field-i2}; the \(C\)-component has no massive
...
\]
which gives \eqref{eq:massive-field-i3}.  The \(C\)-component receives
the direct cubic term \(F_3^C\) and the indirect exchange term obtained
by feeding two tangent inputs into the massive channel through \(F_2^I\),
propagating by \(A^{-1}\), and returning through \(F_2^C\).  Substituting
\(\varphi_2=-A^{-1}F_2^I\) gives \eqref{eq:kappa3}.
\end{proof}
...
The previous section produces the minimal pointed line algebra $K_\kappa$ attached to a boundary vacuum. Two culmination theorems follow:
...
\begin{equation}\label{eq:weff-derived-critical-dga}
...
\end{equation}
where
\begin{equation}\label{eq:weff-derived-critical-differential}
d_{W_{\eff}}
=
\sum_i
...
\subsection{Stable invariants and invisible data}
...
The exact local picture summarizes in one theorem.
...
\begin{definition}[Modular homotopy type; \ClaimStatusDefinitional]
...
$\kappaChHodge(\cA)\cdot\omega_1$ at genus~$1$ and, on the proved scalar
higher-genus lane, $\kappaChHodge(\cA)\cdot\omega_g$, by Volume~I Theorem~D
(leading coefficient).
...
$\mathfrak{q}_4$ need not vanish. The Gaussian row has
$\mathfrak{q}_4=0$, and the affine Lie row has no independent
quartic contact class after the Jacobi cubic layer. The contact row is
different: the $\beta\gamma$ system carries a finite quartic contact
layer, terminal at shadow depth~$4$. In the Virasoro family, the
quartic resonance class
$\mathfrak{Q}^{\mathrm{contact}}_{\mathrm{Vir}}=10/[c(5c+22)]$ from
the quartic-contact analysis of Volume~I is a nonzero
$\mathfrak{q}_4$, and the same wheel mechanism continues in
unbounded arity for class~M.
\end{remark}
...
The genus-$1$ obstruction for the Heisenberg chiral
...
\tfrac{1}{2}[\Theta^{(g_1)},\Theta^{(g_2)}]
\;-\;
\kappaChHodge(\cH_k) \cdot \omega_1.
\end{equation}
...
$\kappaChHodge(\cH_k) = k$: the modular characteristic equals the
...
The genus-$1$ free energy is the scalar
$-\int_{\Mbar_{1,1}}\Theta^{(1)}$ in the boundary-orientation
convention. Since
...
k/24,
\end{equation}
where the determinant-line shadow at fixed modular parameter is
$\eta(\tau)^{-k}$.
...
curvature $\kappaChHodge(\cH_k) = k$ and the binary operation $m_2$:
...
\begin{proposition}[Shifted depth as structural complexity; \ClaimStatusProvedHere]
\label{prop:shadow-depth}
\index{shadow depth}
The shifted algebraic depth $d_{\mathrm{alg}}(\cA)$ is the
finite coordinate
\[
d_{\mathrm{alg}}=r_{\mathrm{sh}}-2
\]
on the Gaussian, Lie, and contact atlas, and is $\infty$ on the
mixed tower. The four classes are:
...
\item[\textbf{L} (Lie, $d_{\mathrm{alg}}=1$):]
 Current algebras. The finite Lie--Jacobi cubic layer is
 determined by the binary current bracket; no independent wheel
 or contact term survives in higher arity. On a comparison surface
 with chosen quantization and associator, the quantum exchange family
 $R(z) = \mathrm{Pexp}\!\bigl(\int r(w)\,dw\bigr)$ satisfies the
 associator-controlled Yang--Baxter relation; after strictification it
 becomes the usual quantum Yang--Baxter equation. The local class is
 determined by the classical $r$-matrix together with the Lie--Jacobi
 relation and the chosen associator datum.
 \emph{Example:} affine $\hat{\mathfrak{g}}_k$ at generic level.
\item[\textbf{C} (Contact, $d_{\mathrm{alg}}=2$):]
 Contact algebras. The quantum exchange family satisfies the same
 associator-controlled Yang--Baxter relation on a chosen quantization
 surface, but the bar transfer contains an independent quartic contact
 layer beyond the classical $r$-matrix.
...
\begin{proof}
The ordered bar filtration gives the shadow depth
$r_{\mathrm{sh}}\in\{2,3,4,\infty\}$. Subtracting the Gaussian
base value gives the finite coordinate $d_{\mathrm{alg}}$ on
classes $\mathbf G,\mathbf L,\mathbf C$. For a Gaussian algebra,
no operation beyond $m_2$ remains. For a current algebra, the
simple-pole residue is the Lie bracket, and the Jacobi identity
is the cubic Stasheff relation; the layer is present but finite.
For a contact algebra, one quartic contact layer remains. For a
mixed Virasoro or $W$-algebra, the wheel residue survives in
unbounded arity, hence no finite truncation suffices.
\end{proof}

\begin{computation}[Shifted depth of $\hat{\mathfrak{sl}}_2$ at generic level; \ClaimStatusProvedHere]
\label{comp:shadow-depth-sl2}
\index{shadow depth!affine $\hat{\mathfrak{sl}}_2$}

$\hat{\mathfrak{sl}}_2$ at generic level $k$
(i.e.\ $k\neq -2$) has shifted algebraic depth
$d_{\mathrm{alg}}=1$ and shadow depth $r_{\mathrm{sh}}=3$.
The independent datum is the current bracket and level; the
finite cubic layer is the Lie--Jacobi transfer.
...
\medskip
\noindent\textbf{Step 3: The finite cubic layer.}
At degree~$3$, the ordered bar differential records the triple
current collision. The simple-pole term in \eqref{eq:sl2-m2}
gives the Lie--Jacobi cubic layer, and the identity
\[
f^{ab}{}_{e}f^{ec}{}_{d}
+f^{bc}{}_{e}f^{ea}{}_{d}
+f^{ca}{}_{e}f^{eb}{}_{d}=0
\]
closes the Stasheff relation in arity~$3$. The Sugawara
construction is non-degenerate at $k\neq-2$, and the one-loop
exactness theorem (Theorem~\ref{thm:one-loop-koszul}) excludes an
independent Virasoro wheel tower before Drinfeld--Sokolov
reduction. Thus the class~$\mathbf L$ layer is finite:
$r_{\mathrm{sh}}=3$ and $d_{\mathrm{alg}}=1$.
...
by $(z_1{-}z_2)(z_1{-}z_3)(z_2{-}z_3)$. The quantum exchange family
\begin{equation}\label{eq:sl2-R-matrix}
R(z) \;=\; \mathcal{P}\exp\!\Bigl(\int_0^z
r(w)\,dw\Bigr)
\end{equation}
satisfies the associator-controlled Yang--Baxter relation of
Theorem~\ref{thm:YBE}; on a strictified comparison surface this is the
usual quantum Yang--Baxter equation. The local class is controlled by
$r(z)$, the Jacobi identity, and the chosen associator datum,
confirming class~$\mathbf L$.

\medskip
\noindent\textbf{Step 5: Genus-$1$ obstruction.}
The modular characteristic is $\kappaChHodge(\hat{\mathfrak{sl}}_2)
= \frac{3(k+2)}{4}
...
\;=\;
\frac{\kappaChHodge(\hat{\mathfrak{sl}}_2)}{24}
\;=\;
...
The $(-1)$-shifted symplectic pairing on the genus-$g$
...
$+(p+q-1)$. For the pairing to be nonzero, $p = q$ is required
...
is isotropic: for $\alpha,\,\beta \in Q_g(\cA)$,
...
The cyclic structure is the datum that makes modular gluing possible: genus-$g$ amplitudes arise from composing $g$ clutching operations with the trace $\Tr_\cC$. The modular characteristic $\kappaChHodge(\cA)$ measures the obstruction to the trace being a \emph{chain map} (i.e., compatible with the bar differential at genus $g \geq 1$). When $\kappa = 0$, the trace is a chain map and the theory is ``anomaly-free''; when $\kappa \neq 0$, the trace acquires a curvature correction $\kappa \cdot \omega_g$, which is the conformal anomaly.
\end{remark}
...
The Lagrangian decomposition reads
...
Work on $\Mbar_{1,1}$ (one marked point), where the Hodge
...
the modular characteristic $\kappaChHodge(\cH_k) = k$
...
$\kappaChHodge(\cH_k)\cdot\omega_1 = k\,\omega_1$ is boundary-accessible
...
The scalar genus-$1$ free energy
$F_1(\cH_k) = k/24$
...
characteristic $\kappaChHodge(\cH_k)$, and the eta function belongs
to the determinant-line character of the free boson on $E_\tau$. The
complementary scalar free energy $F_1(\cH_k^!) = -k/24$
...
$\kappaChHodge(\cH_k) + \kappaChHodge(\cH_k^!) = 0$
...
\begin{remark}[Abelian and non-abelian Chern--Simons worked cases]
\label{rem:moved-preface-abelian-nonabelian-cs}
The abelian and non-abelian Chern--Simons worked examples are the
minimal exact-sector tests of the bulk--boundary--line triangle.

\smallskip
\noindent\emph{Abelian Chern--Simons.}
For $G = U(1)$: $\Bbound = \cH_k$, the infinitesimal kernel is
$r_{\cH}(z)=k\Omega_{\cH}/z$ with coefficient $k/z$, the logarithmic
holonomy is $z^k$, and
$\Abulk=Z^{\mathrm{der}}_{\mathrm{ch}}(\cH_k)$. The classical
$r$-matrix satisfies the $k=0$ vanishing check trivially.

\smallskip
\noindent\emph{Non-abelian Chern--Simons.}
For $G = SL_2$: $\Bbound = \widehat{\mathfrak{sl}}_2{}_k$, $\cA^!_{\mathrm{ch}} = \widehat{\mathfrak{sl}}_2{}_{-k-4}$, $r(z) = k\,\Omega_{\mathrm{tr}}/z$ (vanishing at $k = 0$ satisfied). The KZ connection is the degree-$(2,0)$ projection of $\alpha_T$; on evaluation modules, the reduced HT spectral $R$-matrix agrees with the quantum-group $R$-matrix of $U_q(\mathfrak{sl}_2)$ at $q = e^{i\pi/(k+2)}$ (Theorem~\ref{thm:affine-monodromy-identification}). Line category:
\[
...
The $K3 \times E$ specialization targets the Vol~III K3 chiral
Hall--Drinfeld double
$\mathcal{D}_\hbar(\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))$
on the boundary side only after the compact CoHA source, orientation
system, Hopf pairing, and Hall bracket comparison are supplied. The
bulk source is twisted 11D-SUGRA on $K3 \times T^2$ with $24$
M5-brane line defects on the Kodaira $I_1$ fibres. The resulting
conditional Bruinier--Heegner characteristic line has scalar image
\(\mathbb C\cdot\Delta_5\); the primitive weight-$5$
Gritsenko--Nikulin denominator is the bulk--boundary gauge-class target on
this compact Hall locus, and its scalar Igusa square is
$\Phi_{10}^{\mathrm{un}}=\Delta_5^2$. No intrinsic
\(H^2(\mathfrak g_{\Delta_5})\) class is asserted before this comparison
package is supplied.
\end{remark}
...
$K3\times\mathbb{C}^2$ at $\hbar^3$; \ClaimStatusConditional]
...
$\mathbf{H}_{\Delta_5}$ denote the recognised $\Delta_5$ boundary target
after the Vol~III compact-source package, and let
$\mathcal{W}^{\mathrm{BKM}}_\lambda$
($\lambda\in\mathrm{Mod}(\mathfrak{g}_{\Delta_5})$)
denote the Wilson-line defect labelled by a BKM module target. Then the
...
\end{array}
\]
is controlled, after applying the Bruinier--Heegner characteristic map
\(\chi_{\mathrm{Heg}}\), by the reduced BV obstruction characteristic
\[
	 \chi_{\mathrm{Heg}}(c^{\mathrm{BV}}_3)
	 = c_{\phi_{0,1}^{K3}}(1,\pm1)\,[H_3]
	 = -64\,[H_3];
\]
Theorem~\ref{thm:bvbrst-3loop-obstruction} supplies the same class in
the BV complex. The triangle commutes
at $\hbar^3$ if and only if $c^{\mathrm{Br}}_3$ is trivialised by a Wilson-line
...
\]
with $c_3^{\lambda}$ the restriction of this Heegner characteristic to
the BKM module target \(\lambda\), and $A$ the gauge-field pullback from the bulk. The
trivialisation is possible precisely when
$\mathrm{tr}_\lambda(\chi_{\mathrm{Heg}}(c^{\mathrm{BV}}_3)) = 0$ in
the defect coupling complex of the
$\mathcal{W}^{\mathrm{BKM}}_\lambda$-coupling
complex. The generic Koszul comparison locus for this statement is
the open sublocus
$\mathcal{A}_2^{\mathrm{gen}} :=
\overline{\mathcal{A}_2}\setminus(H_1\cup H_3\cup H_4)$ together with
the Wilson sectors satisfying this trace-vanishing condition; sectors
with non-zero trace on the Heegner divisor $H_3$ require the
counterterm.
\end{theorem}
...
\[
 \langle \Gamma_{\theta\times\theta}\,|\,\lambda\rangle_{\mathrm{Br}}
 \;=\;
	 c_{\phi_{0,1}^{K3}}(1,\pm1)\cdot\mathrm{tr}_\lambda([H_3])
	 = -64\,\mathrm{tr}_\lambda([H_3]),
\]
and vanishes iff $\lambda$ is perpendicular, after the assumed
Bruinier--Heegner comparison, to the divisor class $[H_3]$. On the open
Koszul locus of this theorem, the admitted Wilson sectors are those
with trivial $[H_3]$-pairing; on the Heegner divisor $H_3$, sectors
with non-trivial $[H_3]$-pairing require a counterterm. The counterterm
$S^{(3)}_{\mathrm{c.t.}}$ is dictated by
BV locality and the requirement of gauge invariance, and its
coefficient is exactly $c_3^\lambda$ by
Theorem~\ref{thm:bvbrst-3loop-obstruction}. The scalar
$176256=p_{24}(5)=\chi(\operatorname{Hilb}^5(K3))$ belongs to the
Fock/Hilbert-sector dimension used in the Cartan-matrix trace
normalisation; it is not the Bruinier reduced obstruction coefficient.
\end{proof}

\begin{corollary}[Bulk-boundary-line line defects and BKM-module
Wilson lines; \ClaimStatusConditional]
\label{cor:htbb-core-wilson-BKM}
\index{Wilson lines!BKM modules}
Assuming the same compact-source recognition and BKM-module comparison,
...
\[
 ...
 \;=\;
 \mathrm{tr}_{\lambda_1\otimes\lambda_2}
 \bigl(\Delta_{\mathrm{BKM}}(c^{\mathrm{Br}}_3)\bigr)
 \;\in\;
 H^2\!\left(
 \operatorname{End}(
 \mathcal{W}_{\lambda_1}\otimes\mathcal{W}_{\lambda_2})
 \right),
\]
...
\[
	 -64\,
 \mathrm{tr}_{\lambda_1\otimes\lambda_2}
 \bigl(\Delta_{\mathrm{BKM}}[H_3]\bigr).
\]
There is no square of the three-loop coefficient in degree~$2$; a
cup-square would live in degree~$4$ and belongs to the separate
...
Assume the compact-source recognition and BKM-module comparison package
of Theorem~\ref{thm:htbb-core-3loop-triangle}.  The $24$ M$5$-brane
defects of the bulk-boundary-line setup on $K3\times T^2$ with Kodaira
$I_1$ fibres then decompose into $24$ recognised Wilson-line target
classes at $\hbar^3$: the $i$th defect is compared with the BKM
simple-root Wilson line
$\mathcal{W}^{\mathrm{BKM}}_{\alpha_i}$, where \(\alpha_i\) is a
simple-root label in the recognised \(\mathfrak g_{\Delta_5}\) target.
At $\hbar^3$ the $24$-fold decomposition supplies the root-stalk labels
for Wilson-line insertions in the graph calculus of
Theorem~\ref{thm:bvbrst-3loop-feynman}. The resulting obstruction map
has Heegner characteristic
\[
 o^{(3)}_{24}\colon
 \bigl(\mathbb C^{24}\bigr)^{\otimes 3}
 \longrightarrow
 \operatorname{Pic}_{\mathrm{Heg}}(\mathcal A_2)\otimes\mathbb C,
 \qquad
	 o^{(3)}_{24}=c_{\phi_{0,1}^{K3}}(1,\pm1)[H_3]=-64[H_3]
\]
after the Bruinier reduced projection. The integer
$176256=p_{24}(5)=\chi(\operatorname{Hilb}^5(K3))$ is the dimension of
the fifth Fock/Hilbert sector; it is a true K3 scalar, but it does not
count three-loop Wilson-line weavings and it is not the coefficient of
the BV obstruction class.
\end{remark}
...
\begin{remark}[The HT bulk-boundary-line $2$-loop vanishing]
\label{rem:htbb-core-2loop-vanish}

Assume the compact-source recognition and BKM-module comparison package
of Theorem~\ref{thm:htbb-core-3loop-triangle}, restricted to the
$\hbar^2$ defect-coupling complex. In the compared
bulk-boundary-line problem, the $\hbar^2$-order BV obstruction on
$K3\times\mathbb{C}^2$ is
\[
 c^{\mathrm{triangle}}_2
 =0
 \quad\text{after the Heegner characteristic comparison},
\]
because discriminant~$2$ is not in the Eichler--Zagier admissible
support $n\equiv 0,3\pmod 4$. The same vanishing is seen diagrammatically as
the $24-24=0$ two-loop Feynman cancellation. Under the same recognition
and comparison hypotheses, this $\hbar^2$-vanishing says that the HT
bulk-boundary-line triangle requires no inserted $\hbar^2$ counterterm:
no compared $2$-loop coupling between bulk twisted $11$D SUGRA,
boundary $\mathbf{H}_{\Delta_5}$, and line
$\mathcal{W}^{\mathrm{BKM}}_\lambda$ is forced. The
	$\hbar^3$ obstruction \(c^{\mathrm{Br}}_3=-64[H_3]\neq 0\) marks the first non-trivial
...
and conditional BKM counterterm comparison]
\label{rem:htbb-core-synthesis}

Assume compact-source recognition and BKM-module comparison. The
combination of
Theorem~\ref{thm:htbb-core-3loop-triangle} (bulk-boundary-line
$3$-loop triangle) and
Theorem~\ref{thm:bvbrst-3loop-obstruction} (explicit $c_3$ via
Gritsenko-Nikulin $p^3$ Fourier coefficient) gives the following
conditional K3 triangle comparison:
\begin{enumerate}[leftmargin=1.8em]
\item The $3$-loop BV obstruction class is
	 $c_3 = [\log(\Delta_5/(\eta(q)^{12}\eta(p)^{12}))]_{p^3}$, with
	 Bruinier reduced value
	 \(c^{\mathrm{Br}}_3=c_{\phi_{0,1}^{K3}}(1,\pm1)[H_3]=-64[H_3]\);
\item the dominant $3$-loop Feynman graph is the theta-times-theta
 graph with three internal $K3$ loops;
\item after compact-source recognition and BKM-module comparison, the
 $3$-loop bulk-boundary-line triangle commutes on the generic locus and
 fails on the Heegner divisor $H_3$ without the corresponding
 Wilson-line counterterm;
\item consistency with the pentagon-$\hbar^3$ cocycle
	 $\phi^{(3)} = \zeta(3) c_{\mathrm{symm}} + (25/3) c_{\mathrm{timelike}}
	 + \bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\sigma)^{24})\bigr)c_{\Phi_{10}}$: the non-trivial
	 \(\bigl(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\sigma)^{24})\bigr)c_{\Phi_{10}}\) summand is precisely $c_3$.
\end{enumerate}
The progression $c_0 = 0$ (classical) $\to c_1 = [\Delta_5]$
(paramodular anomaly) $\to c_2 = 0$ (two-loop vanishing)
	$\to c^{\mathrm{Br}}_3 = -64\cdot[H_3]$ (first non-trivial Heegner
obstruction) traces the
...
\index{weak Jacobi form!\phi_{0,1}^{K3}@$\phi_{0,1}^{K3}$ core reading}
After applying the same Bruinier--Heegner characteristic comparison, the
order-by-order obstruction tower $\{c_n\}_{n\geq 1}$ closes into a
single all-order scalar identity:
\[
\chi_{\mathrm{Heg}}\bigl(c_n^{\mathrm{triangle}}\bigr)
= c_{\phi_{0,1}^{K3}}(a_n,r_n)\cdot [H_n],
\]
where
\(\phi_{0,1}^{K3}\) is the normalised half K3 weak Jacobi form,
\(4a_n-r_n^2=n\) is the chosen positive-cone root label, and $[H_n]$ is
the Humbert divisor class of discriminant $n$
\textup{(}Theorem~\ref{thm:bvbrst-heegner-all-order-vol2}; proof-side
twin in Vol~I
...
\cite[Theorem 9.3]{EichlerZagier1985}; leading values \(c_3=-64\),
\(c_4=108\), \(c_7=513\), \(c_8=-808\) are the coefficients of
\(\phi_{0,1}^{K3}\) at root labels \((1,\pm1),(1,0),(2,\pm1),(2,0)\)
\cite[Table 1]{EichlerZagier1985}. The entire formal tower is a single
object: the weak Jacobi form lifting to \(\Delta_5\), whose square is
\(\Phi_{10}^{\mathrm{un}}\), under the
Borcherds~$1998$ singular-theta
lift~\cite[Theorem 10.1]{Borcherds1998}. The Bruinier reduced
three-loop value is \(c^{\mathrm{Br}}_3=-64[H_3]\). The integer
$176256=p_{24}(5)$ remains the fifth K3 Fock/Hilbert-sector dimension
and may appear after applying a Cartan-matrix trace normalisation, but
it is not a Fourier coefficient of \(\phi_{0,1}^{K3}\) and not the reduced
BV obstruction.
\end{remark}
