% Extensions to bv_ht_physics.tex: boundary correspondence,
% W-algebras, and BV integration.
% Assumes theorem environments from main.tex.
...
(BV integration as bar-cobar pairing). For each we construct boundary data
in the standard families, identify the algebraic mechanism governing
the correspondence, and prove partial results in the
...
Conjecture~\ref{conj:bar-cobar-ht-boundary} acquires a sharp formulation once the boundary data is specified
family by family:
...
The four standard families (Heisenberg, affine, Virasoro, $\beta\gamma$)
...
\begin{proposition}[Bar differential as boundary BRST]
...
\begin{theorem}[Heisenberg boundary correspondence]
...
The Heisenberg case is exact, not perturbative, for three
...
condition is the unique boundary condition compatible with
$U(1)$-current conservation. Together, these ensure that
$\barBch(\cH)$ encodes the boundary data completely.
\end{remark}
...
\begin{theorem}[Affine boundary correspondence]
...
\item The bar cohomology $H^\bullet(\barBch(\hat{\fg}_k))$ is the
 ...
 Proposition~\ref*{V1-prop:pbw-universality} of Volume~I), and the bar
...
\begin{proposition}[Bar differential and gauge-theory BRST for affine]
...
\item The bar differential $d_{\barB}$ has infinitely many nonvanishing
 ...
 support shadow depth \(r^\Theta_{\max}(\mathrm{Vir}_c)=\infty\): the Virasoro is in
 class~M (mixed).
...
\begin{proposition}[Virasoro duality and boundary correspondence]
...
The relation $\mathrm{Vir}_c^! = \mathrm{Vir}_{26-c}$ means the
Koszul dual remains in the same family: the
depth-zero resonance shadow of Volume~I
(Remark~\ref*{V1-rem:virasoro-resonance-model}). The
...
In the boundary correspondence, the dual boundary
...
\item The bar differential has components $d_2, d_3, d_4$ and terminates
 at degree 4. This is the shadow depth $r_{\max}(\beta\gamma) = 4$:
 the $\beta\gamma$ system is in class~C (contact).
\item The quartic shadow $Q^{\mathrm{contact}}_{\beta\gamma}$ computes
 the quartic contact invariant. Corollary~\ref*{V1-cor:nms-betagamma-mu-vanishing}
 of Volume~I gives only the scalar contraction
 $\mu_{\beta\gamma}=0$ on the weight-changing line; the class-C
 quartic contact layer of the full bar complex remains.
...
depth 4 (class~C, contact) with nontrivial cubic
and quartic bar components. Both are
Koszul, and the boundary correspondence is exact in both cases. The
contact structure manifests as additional
...
\begin{theorem}[Abelian boundary correspondence]
...
\begin{corollary}[Rank-$N$ free field boundary correspondence]
...
\begin{proof}
Item (1) is immediate: $\kappaChHodge(\cH_N) = N$ because the curvature is
...
\begin{theorem}[Boundary chiral algebra from gauge theory]
% label removed: thm:boundary-chiral-from-gauge
Costello--Li and Costello--Dimofte--Gaiotto supply the
boundary chiral-algebra construction used here~\cite{CL16,CDG20}.
...
$\hat{\fg}_{-k-2h^\vee}$ (the Dirichlet algebra): it encodes
...
The derivation is proved in Chapter~\ref{chap:affine-half-space-bv}
...
The three steps above summarize that proof.
\end{remark}
...
\begin{theorem}[$\cW^k(\fg, f)$ from modified boundary condition]
% label removed: thm:w-algebra-from-boundary
Costello--Gaiotto's twisted-holography construction supplies this
DS boundary condition~\cite{costello-gaiotto}. For a nilpotent
$f \in \fg$ equipped with a good DS grading and a boundary condition
to which the Costello--Gaiotto construction applies, at level
...
\begin{theorem}[Principal DS commutes with bar-cobar]
...
(Proposition~\ref*{V1-prop:pbw-universality} of Volume~I). The principal
...
\begin{theorem}[Obstruction-dimension theorem for the $(3,2)$
subregular DS reduction;
licensing $\alpha + \gamma + \delta + \varepsilon$;
\ClaimStatusProvedHere]
\label{thm:non-principal-ds-bar-cobar-obstruction-32}
For $\fg = \mathfrak{sl}_5$ and $f = f_{(3,2)}$ the subregular
nilpotent, the bar construction does not commute with Drinfeld--Sokolov
reduction at the chain level. The obstruction is the non-admissibility
of the BFFGM character $\chi_{(3,2)} : \fn_+ \to \C$ on the good
even Dynkin $\fn_+$ at $h = \mathrm{diag}(2, 0, -2, 1, -1)$
$(\dim \fn_+ = 10,\ \mathrm{LCS\ depth} = 3)$: the cocycle
$d\chi \in H^2_\chi(\fn_+, \C)$ has dimension three, witnessed by
the grade-$1$ quadrilateral
$\{E_{0,3},\, E_{1,4},\, E_{3,1},\, E_{4,2}\}$ pairing into the
grade-$2$ BFFGM-charged triple
$\{E_{0,1},\, E_{1,2},\, E_{3,4}\}$ via
\[
 [E_{0,3}, E_{3,1}] = E_{0,1},\qquad
 [E_{1,4}, E_{3,1}] = -E_{3,4},\qquad
 [E_{1,4}, E_{4,2}] = E_{1,2},
\]
on each of which $\chi_{(3,2)}([\cdot,\cdot]) \in \{+1,-1,+1\}$
respectively is non-zero. The Kazhdan-filtered BRST spectral sequence
fails to degenerate at $E_1$:
\[
 \dim H^2_\chi(\fn_+, \C) \;=\; 3,
\]
and the obstruction lifts to an explicit non-vanishing class in
the chain-level non-principal DS bar--cobar comparison.
\end{theorem}

\begin{proof}
The principal sl$_5$ character $\chi_{\mathrm{princ}}$ is admissible
in the sense of Kac--Roan--Wakimoto \cite{KRW03} Theorem~2.1: zero
violating brackets on $[\fn_+, \fn_+]$, so $Q^2 = 0$ on the naive
BRST complex and the Kazhdan filtration delivers $E_1$-degeneration.
For the subregular character $\chi_{(3,2)}$, direct Chevalley--Eilenberg
differential computation on $\Lambda^\bullet \fn_+^\ast$ produces
exactly the three displayed violating brackets, all of the form
(grade-$1$) $\wedge$ (grade-$1$) $\to$ (grade-$2$, BFFGM-charged).
Three routes agree on $\dim H^2_\chi(\fn_+, \C) = 3$:
(a) the Chevalley--Eilenberg differential $Q^2$ on
$\Lambda^\bullet \fn_+^\ast$ has rank $3$ on the grade-$2$ BFFGM-charged
stratum;
(b) the Kac--Roan--Wakimoto admissibility criterion
$\chi([\cdot,\cdot]) = 0$ holds on $[\fn_+, \fn_+]$ except at the
three displayed brackets;
(c) the combinatorial count of grade-$1$ pair-to-grade-$2$
$\chi$-charged brackets gives three.
The Kazhdan-filtration $E_1$-degeneration argument used in the
principal case (\cite{Arakawa07Wcoset}, \cite{KRW03}) fails at exactly
these three brackets; the $(3, 2)$ partition is the first non-abelian
type-A case ($\fn_+$ is $2$-step nilpotent with four nonzero
commutators), and the obstruction is independent of the
Kac--Wakimoto level shift (the three grade-$1$ + grade-$1$
violations are level-independent).
\end{proof}
...
\begin{theorem}[Hook-type DS commutes with bar-cobar]
...
hook-type corridor, proved by Fehily~\cite{Feh23} and
Creutzig--Linshaw--Nakatsuka--Sato~\cite{CLNS25}. The key input is
...
\cite{CLNS25}), then the transport closure generates all partitions
...
\begin{conjecture}[DS reduction as functor on bar-cobar data]
...
%% ====================================================================
%% SECTION 3: PERTURBATIVE BV GRAPH FUNCTIONALS
%% ====================================================================

\section{Perturbative BV graph functionals}
% label removed: sec:bv-integration-partition-functions

The comparison problem asks for a gauge-fixed perturbative BV
functional whose Feynman expansion is modeled by the bar-cobar pairing.
The construction below is narrower and algebraic: it defines a
graph-weight density on the genus-$g$ chiral bar complex from
configuration-space Feynman rules. After a moduli-space integration
functional has been fixed, this density gives a scalar graph shadow.

\subsection{Graph-weight density on the bar complex}

\begin{definition}[Graph-weight BV density]
% label removed: def:graph-weight-bv-measure
Let $\cA$ be a chiral algebra with curvature $\kappa = \kappaChHodge(\cA)$.
The \emph{graph-weight BV density on $\barBch(\cA)$ at genus $g$} is the linear
functional
\[
...
The graph-weight series is convergent only after the hypotheses of the
HS-sewing criterion (Theorem~\ref*{thm:general-hs-sewing} of Volume~I)
have been verified: polynomial OPE growth, subexponential sector
growth, and the required completion of the genus-$g$ bar sector. The
standard landscape examples enter the construction only at the points
where these hypotheses are checked.
\end{remark}

\begin{definition}[Genus-$g$ perturbative BV graph shadow]
% label removed: def:bv-partition-function-bar
The \emph{genus-$g$ perturbative BV graph shadow} is
\begin{equation}
% label removed: eq:bv-partition-function
Z_g^{\mathrm{BV,gr}}(\cA)
 := \int_{\ov{\cM}_g} \mu^{\mathrm{BV}}_g(\Theta_\cA),
\end{equation}
...
\begin{proposition}[Perturbative graph shadow as bar-complex integral]
% label removed: prop:bv-partition-bar-integral
Assume the BV/bar-cobar pairing of
Conjecture~\ref{conj:bv-integration-bar-cobar} identifies the
residue-distribution integral with the genus-$g$ graph-weight BV
density. Then the perturbative graph shadow
$Z_g^{\mathrm{BV,gr}}(\cA)$ is the genus-$g$ evaluation of the
bar-cobar pairing
\[
Z_g^{\mathrm{BV,gr}}(\cA)
 = \langle \barBch(\cA)^{(g)},
...
\]
not an independent non-perturbative BV path integral.
\end{proposition}
...
Definition~\ref{def:graph-weight-bv-measure}. The graph-weight BV density
$\mu^{\mathrm{BV}}_g$ is the restriction of the bar-cobar pairing to
the genus-$g$ sector.
\end{proof}

\subsection{Heisenberg: the Gaussian determinant shadow}

\begin{theorem}[Heisenberg graph shadow]
% label removed: thm:heisenberg-partition-function
For the rank-$N$ Heisenberg algebra $\cH_N$ at genus $g \geq 2$, the
perturbative BV graph shadow is
\[
Z_g^{\mathrm{BV,gr}}(\cH_N)
...
The graph-weight sum reduces to a Gaussian determinant calculation because the
...
from the zero-mode volume factor of the Jacobian $\mathrm{Jac}(\Sigma_g)
...
factorization, Proposition~\ref*{V1-prop:independent-sum-factorization}
of Volume~I), and the graph shadow is the $N$-th power:
\[
Z_g^{\mathrm{BV,gr}}(\cH_N)
 = Z_g^{\mathrm{BV,gr}}(\cH_1)^N
 = (\det \mathrm{Im}\, \Omega)^{-N/2}
 \cdot (\det{}' \Delta)^{-N/2}.
\]
\end{proof}
...
The result matches the standard determinant-line scalar of the
free-boson CFT on $\Sigma_g$ (see e.g.\ D'Hoker--Phong~\cite{DhokerPhong}).
The factor $(\det \mathrm{Im}\, \Omega)^{-N/2}$ is the zero-mode volume
and $(\det{}' \Delta)^{-N/2}$ is the non-zero-mode functional
determinant. The bar-complex computation reproduces this scalar through
the graph-weight density on $\ov{\cM}_g$; it does not construct an
ordinary field-space measure on maps
$\phi \colon \Sigma_g \to \bR^N$.
\end{remark}

\begin{corollary}[Genus-1 Heisenberg graph shadow]
% label removed: cor:genus-1-heisenberg
At genus 1 with modular parameter $\tau$:
\[
Z_1^{\mathrm{BV,gr}}(\cH_N) = \frac{1}{|\eta(\tau)|^{2N}},
\]
...
unit-area zeta-regularized oscillator determinant is
\(\det{}' \Delta = |\eta(\tau)|^4\). Substituting:
\[
Z_1^{\mathrm{BV,gr}}(\cH_N)
 = (\mathrm{Im}\, \tau)^{-N/2}
 \cdot |\eta(\tau)|^{-2N}.
\]
The volume normalization convention absorbs $(\mathrm{Im}\,
\tau)^{-N/2}$, leaving
$Z_1^{\mathrm{BV,gr}}(\cH_N) = |\eta(\tau)|^{-2N}$.
\end{proof}
...
$\kappaChHodge(\hat{\fg}_k) = \dim \fg \cdot (k + h^\vee) / (2h^\vee)$ integrated over
$\ov{\cM}_g$.
...
\begin{theorem}[Verlinde formula from bar-complex graph sum]
...
$\bR$-direction \emph{operadic} factorization (topological
coproduct on $\Conf_k(\bR)$), and the trace is the
$\bC$-direction \emph{operadic} factorization (holomorphic
differential on $\FM_k(\bC)$). This is the $\SCchtop$-operadic
Swiss-cheese structure of Volume~II
(Part~\ref{part:swiss-cheese}). The promotion to BD
factorization on $\Ran(\Sigma_g)$ requires the hypotheses of
Construction~\ref{constr:bar-fact-coalgebra}
\textup{(}chiral OPE, homotopy-Koszul self-duality of
$\SCchtop$, pro-ambient / weight-completion\textup{)} and is
recorded separately in
\texttt{theory/factorization\_swiss\_cheese.tex}.
\end{remark}

\subsection{Virasoro: gravity scalar shadow}

\begin{construction}[Virasoro graph shadow from bar data]
% label removed: constr:virasoro-partition-function
The Virasoro graph shadow at genus $g$ presents a qualitative
...
At genus $g$, the scalar graph shadow is:
\begin{equation}
% label removed: eq:virasoro-partition-function
Z_g^{\mathrm{BV,gr}}(\mathrm{Vir}_c)
 = \int_{\ov{\cM}_g}
 \exp\!\left(
...
\item $\omega_{\mathrm{WP}}$ is the Weil--Petersson form on
 $\ov{\cM}_g$;
\item $\Theta^{(r)}_{\mathrm{Vir}}$ is the degree-$r$ shadow of the
...
curvature $\kappaChHodge(\mathrm{Vir}_c) = c/2$. The higher terms $r \geq 3$
come from the shadow obstruction tower, which does not terminate for Virasoro
(quintic obstruction is nonzero:
$o^{(5)}_{\mathrm{Vir}} \neq 0$).
\end{construction}

\begin{proposition}[Genus-1 Virasoro partition function]
% label removed: prop:genus-1-virasoro
At genus 1 the determinant-line Gaussian shadow and the Virasoro vacuum
character are distinct. The determinant-line shadow integral is
\[
Z^{\mathrm{det}}_1(\mathrm{Vir}_c) = \int_{\cF}
 \frac{d^2 \tau}{(\mathrm{Im}\, \tau)^2}
 \, |\eta(\tau)|^{-c},
\]
where $\cF$ is the fundamental domain for $\mathrm{SL}_2(\bZ)$ in the
upper half-plane. The vacuum-module character is instead
\[
\chi_{\mathrm{vac}}(\mathrm{Vir}_c;\tau)
...
The scalar non-zero-mode determinant contributes
\(|\eta(\tau)|^{-c}\), since
\(\kappaChHodge(\mathrm{Vir}_c)=c/2\). The Virasoro vacuum module has
one oscillator in each mode \(n\ge2\), so its character is the displayed
product and not a \(c\)-fold Gaussian determinant.
\end{proof}

\begin{remark}[Maloney--Witten and the genus-$g$ programme]
% label removed: rem:maloney-witten
At genus $g \geq 2$, the Virasoro graph shadow is a scalar comparison
surface for the Maloney--Witten proposal~\cite{MW10}. The
bar-complex formula \eqref{eq:virasoro-partition-function} is an
algebraic shadow of the modular-sum problem: the graph-weight sum over
stable graphs is a combinatorial comparison datum, not a construction
of the gravitational path integral over hyperbolic $3$-manifolds.

The shadow obstruction tower contributions at degree $r \geq 3$ are
perturbative shadow corrections in the bar complex. The
...
The curvature $\kappaChHodge(\cA)$ plays the role of the BV anomaly. At
genus 0, $\omega_0 = 0$ and there is no anomaly. At genus 1,
$\omega_1 = [\mathrm{pt}] \in H^0(\ov{\cM}_{1,1})$ is the fundamental
class, so the anomaly is $\kappaChHodge(\cA)$ itself.
...
\begin{construction}[Graph density and Mumford isomorphism]
% label removed: constr:bv-mumford
For a chiral algebra $\cA$ of central charge $c$ and curvature
$\kappa = \kappaChHodge(\cA)$, the genus-$g$ perturbative BV graph
shadow is related to the Mumford isomorphism. The Mumford isomorphism
states:
\[
...
The graph-weight BV density $\mu^{\mathrm{BV}}_g$ is a section of a
power of the dual Hodge bundle determined by $\kappaChHodge(\cA)$, and
the scalar shadow
$Z_g^{\mathrm{BV,gr}}(\cA) = \int_{\ov{\cM}_g}
\mu^{\mathrm{BV}}_g(\Theta_\cA)$ is the corresponding integral.
Note that $\kappaChHodge(\cA)$ is in general distinct from $c$: for Virasoro,
...
scalar lane,
$F_g(\cH_N)=\kappaChHodge(\cH_N)\lambda_g^{\mathrm{FP}}$
is linear in $\kappaChHodge(\cH_N)=N$ at every genus.
\end{construction}

\subsection{Anomaly-cancelled scalar shadows}

\begin{proposition}[Ghost-matter cancellation from bar-cobar]
...
The anomaly-cancelled scalar graph shadow is:
\[
Z_g^{\mathrm{string,sh}}
 = \int_{\ov{\cM}_g}
...
\[
Z_g^{\mathrm{string,sh}}
 = Z_g^{\mathrm{BV,gr}}(\cH_{26})
 \cdot Z_g^{\mathrm{BV,gr}}(\mathrm{ghost}).
\]
\end{proposition}
...
(Proposition~\ref*{V1-prop:independent-sum-factorization} of Volume~I):
for $\cA = \cA_1 \oplus \cA_2$ with vanishing mixed OPE, the shadows
separate and the scalar graph shadow factorizes.
\end{proof}
...
the identifications constructed above.
...
abelian and affine boundary computations \\
\hline
...
\item The perturbative BV graph shadow at genus $g \geq 2$ for the
Virasoro algebra, where the infinite shadow obstruction tower
...
\end{enumerate}
Together they measure the gap between the explicit family computations
above and a uniform genus-$g$ boundary/BV theorem.
\end{remark}

\section{Synthesis}
\label{sec:bvht-ext-supplement}

\begin{remark}[BV--HT extension and the K3 anomaly line]
\label{rem:bvht-ext-1}
The BV--HT extension technology applied to twisted 11D SUGRA on
$K3\times T^2$ has a one-dimensional anomaly target only after the
Vol~III compact Hall--Borcherds source has been recognised.  With that
recognition hypothesis in force, the BV functional is
\[
Z^{\mathrm{BV}}_{K3} \;=\;
\int_{\mathcal L_{\mathrm{BV}}} e^{S_{\mathrm{HT}}^{K3}/\hbar}
\]
evaluated on an HT Lagrangian in the recognised target datum.  Its
first quantum obstruction maps, by the Heegner characteristic comparison
of Remark~\ref{rem:anom-ext-1}, to
\[
H^2_{\mathrm{Lie}}\bigl(\mathfrak q_{K(1)};\mathrm{grt}_1\bigr)
\;\cong\; \mathbb C\cdot\Delta_5 .
\]
The one-loop exact BV statement is therefore conditional: after fixing
the $\Delta_5$ anomaly counterterm, higher loop terms are absent exactly
when the completed HT graph-complex obstruction classes vanish outside
the recognised comparison line.  This is the BV--HT scalar form of the
elliptic-genus one-loop mechanism for M-theory on $K3\times T^2$,
refined by the additive Borcherds lift normalisation of Gritsenko.
Primary-lit: Witten 1995, Costello--Gwilliam 2017,
Gwilliam--Williams 2020, Bruinier 2002, Gritsenko 1995,
