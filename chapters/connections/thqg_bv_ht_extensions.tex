...
\label{chap:bv-ht-extensions}

\begin{summary}[Scope and targets]
The three conjectures of Chapter~\ref{chap:bv-ht-extensions}%
---Conjecture~\ref{conj:bar-cobar-ht-boundary} (bar-cobar from HT
...
(BV integration as bar-cobar pairing)---are developed here into
evidence-rich programmes.  For each we construct explicit boundary data
in the standard families, identify the precise algebraic mechanism that
makes the correspondence work or fail, and prove partial results in the
abelian and affine cases.  The overall architecture is dictated by three
principles: (P1)~construct boundary conditions family by family before
...
\label{sec:bar-cobar-ht-boundary}

The conjecture that bar-cobar duality of a boundary chiral algebra
encodes HT boundary conditions (Conjecture~\ref{conj:bar-cobar-ht-boundary})
has a sharp formulation once the boundary data is specified
family by family.  The general picture is:
...
\noindent
The point is not an abstract analogy.  Each row has calculable content,
...
\label{def:boundary-bar-complex}
...
\label{prop:bar-differential-boundary-brst}
...
\label{rem:interacting-bar-brst}
...
\label{thm:heisenberg-boundary-correspondence}
...
The bar complex $\barBch(\cH)$ computes the space of boundary states.
Explicitly:
\begin{enumerate}
...
\label{constr:heisenberg-boundary-state}
...
\label{thm:affine-boundary-correspondence}
Consider holomorphic Chern--Simons with simple gauge group $G$ on
$\bC \times \bR_+$.  The bar complex $\barBch(\hat{\fg}_k)$ computes
the space of conformal boundary conditions.  Specifically:
\begin{enumerate}
\item The bar differential $d_{\barB}$ decomposes as
  $d_{\barB} = d_2 + d_3$, where $d_2$ encodes the affine OPE and
  $d_3$ encodes the cubic interaction
  $f^{abc} J^a J^b J^c / (z - w)$.
...
\]
The double-pole term contributes $d_2$ (the quadratic part of the bar
differential), and the simple-pole term contributes $d_3$ (since the
structure constants $f^{abc}$ produce a ternary operation in the
$\Ainf$-structure after desuspension).
...
\label{prop:affine-bar-brst-identification}
...
$\bar{\partial}$ & $\longleftrightarrow$ & internal differential $d_1$
  on bar elements \\
$[J^a, \cdot] \cdot c^a$ & $\longleftrightarrow$ & bar differential
  $d_2$ (binary OPE) \\
$k \, \partial c^a c^a$ & $\longleftrightarrow$ & bar differential
  $d_3$ (ternary structure constant)
\end{tabular}
\end{center}
\end{proposition}
...
\label{rem:critical-level-obstruction}
...
\label{constr:virasoro-boundary-correspondence}
...
\label{prop:virasoro-boundary-duality}
...
\label{rem:virasoro-resonance-boundary}
...
\label{constr:betagamma-boundary}
...
\item The quartic shadow $Q^{\mathrm{contact}}_{\beta\gamma}$ computes
  ...
  Corollary~\ref*{cor:nms-betagamma-mu-vanishing} of Volume~I,
...
\label{rem:betagamma-vs-heisenberg}
...
\label{thm:abelian-boundary-correspondence}
...
\label{cor:rank-n-free-boundary}
...
\label{thm:boundary-chiral-from-gauge}
...
\label{rem:bar-cobar-gauge-theory-content}
The bar construction $\barBch(\hat{\fg}_k)$ computes the transition from
Neumann to Dirichlet boundary conditions.  This is the precise
gauge-theoretic content of bar-cobar duality: the bar complex is the
open-string state space connecting two boundary conditions, and the
cobar construction recovers the closed-string algebra from the
open-string data.
\end{remark}
...
\label{rem:affine-from-gauge-theorem}
...
\label{thm:w-algebra-from-boundary}
...
\label{thm:principal-ds-bar-cobar}
...
\label{rem:non-principal-obstruction}
...
\label{thm:hook-type-ds-bar-cobar}
...
\label{constr:w-algebra-kd-gauge}
...
\begin{proposition}[DS reduction as functor on bar-cobar data; \ClaimStatusConjectured]
\label{prop:ds-functor-bar-cobar}
...
\item Compatible with the shadow tower: the shadow $\Theta_{\cW}$ is the
...
\label{sec:bv-integration-partition-functions}
...
\label{def:graph-weight-bv-measure}
...
\label{rem:bv-measure-convergence}
...
criterion is satisfied by the entire standard landscape.
\end{remark}

\begin{definition}[BV partition function]
\label{def:bv-partition-function-bar}
The \emph{BV partition function at genus $g$} is
\begin{equation}
\label{eq:bv-partition-function}
Z_g(\cA) := \int_{\ov{\cM}_g} \mu^{\mathrm{BV}}_g(\Theta_\cA),
\end{equation}
where $\Theta_\cA \in \barBch(\cA)^{(g)}$ is the universal MC element
(the shadow tower projection at genus $g$, a sum over all stable graphs
...
\label{prop:bv-partition-bar-integral}
...
\label{thm:heisenberg-partition-function}
...
\label{rem:heisenberg-cft-comparison}
...
\label{constr:affine-partition-function}
...
\label{rem:verlinde-factorization}
...
\label{constr:virasoro-partition-function}
...
\begin{equation}
\label{eq:virasoro-partition-function}
Z_g(\mathrm{Vir}_c)
  = \int_{\ov{\cM}_g}
  \exp\!\left(
...
curvature $\kappa(\mathrm{Vir}_c) = c$.  The higher terms $r \geq 3$
come from the shadow tower, which does not terminate for Virasoro
(quintic obstruction is nonzero:
$o^{(5)}_{\mathrm{Vir}} \neq 0$).
\end{construction}

\begin{proposition}[Genus-1 Virasoro partition function; \ClaimStatusProvedHere]
\label{prop:genus-1-virasoro}
At genus 1:
\[
...
The shadow tower contributions at arity $r \geq 3$ are the
...
\label{thm:bv-anomaly-bar-curvature}
...
\label{rem:bv-anomaly-cancellation}
...
\label{constr:bv-mumford}
For a chiral algebra $\cA$ of central charge $c$ and curvature
$\kappa = c$, the BV partition function at genus $g$ is related to the
Mumford isomorphism.  The Mumford isomorphism states:
\[
...
The BV measure $\mu^{\mathrm{BV}}_g$ is a section of $\lambda_c^{-1}$,
and the partition function $Z_g(\cA) = \int_{\ov{\cM}_g}
\mu^{\mathrm{BV}}_g(\Theta_\cA)$ is the integral of a section of the
dual Hodge bundle.

For the Heisenberg algebra at rank $N$:
\[
Z_g(\cH_N) = \int_{\ov{\cM}_g} \lambda_N^{-1}
  = \frac{B_{2g}}{2g(2g-2)} \cdot N^g + \text{lower-order terms},
\]
where $B_{2g}$ is the $2g$-th Bernoulli number.  The leading term
comes from the Bernoulli contribution to the Hodge integral
$\int_{\ov{\cM}_g} \lambda_g^{2g-2}$.
\end{construction}
...
\label{prop:ghost-matter-cancellation}
In the bosonic string, the matter sector has $c = 26$ and the ghost
sector has $c_{\mathrm{ghost}} = -26$.  The total curvature is
\[
...
\label{rem:extended-bv-dictionary}
...
\label{rem:what-remains-conjectural}
...
\item The BV partition function at genus $g \geq 2$ for the Virasoro
  algebra, where the infinite shadow tower contributes non-trivially.
