...
We now resolve the structural parts of Conjectures
...
\]
and recall that the selection rule (Theorem~\ref{thm:one-vertex-trace-selection-rule}) forces
\begin{equation}
...
\textup{(}in the sense conjectured in Volume~I\textup{)}.
\end{remark}
...
principles; \ClaimStatusConjectured{} (the identification $\cA_{\mathrm{cel}}^!_{\mathrm{line}} = Y_\hbar(\fg)$ is proved for $\fg = \mathfrak{gl}_N$ in $\cN=4$ SYM \cite{Cos13}; the general statement is conjectural)]
