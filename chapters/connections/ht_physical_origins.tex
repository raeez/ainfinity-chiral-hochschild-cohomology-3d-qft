...
$\cA$ arising from a 3d HT theory satisfying (H1)--(H4), is $\cA$
...
Let $\cA$ be the $A_\infty$ chiral algebra of a 3d HT theory satisfying
(H1)--(H4).  Each $A_\infty$ operation $m_k$ receives contributions from
...
Let $\cA$ be the $A_\infty$ chiral algebra of a 3d HT theory satisfying
(H1)--(H4) such that the bulk BV quantization is one-loop exact (i.e.,
...
Drinfeld--Sokolov reduction with respect to a nilpotent element
$f\in\fg$.  Then:
\begin{enumerate}
\item The $A_\infty$ structure on $\cA_{\mathrm{DS}}$ is obtained by
  homological transfer from $\cA$ through the semi-infinite BRST
  complex $C^{\infty/2+\bullet}(\fn_+,\cA)$.
\item The transferred $A_\infty$ operations $m_k^{\mathrm{DS}}$ are
  given by sums over decorated trees in the BRST homotopy retract, and
  generically satisfy $m_k^{\mathrm{DS}}\neq 0$ for all $k\ge 2$.
\item The bar cohomology of $\cA_{\mathrm{DS}}$ satisfies
  \[
  H^{p,q}(\overline{B}^{\mathrm{ch}}(\cA_{\mathrm{DS}}))
  \neq 0
  \qquad\text{for infinitely many }q
  \]
  whenever the BRST homotopy $h$ has nontrivial image in the
  transferred operations.  In particular, $\cA_{\mathrm{DS}}$ is
  generically \emph{not} chirally Koszul.
\end{enumerate}
\end{theorem}

\begin{proof}
(1) By the homological perturbation lemma, the DS reduction is a
deformation retract, and the $A_\infty$ transfer theorem
(Kadeishvili--Merkulov) gives the transferred operations.

(2) The transferred $m_k^{\mathrm{DS}}$ are sums over planar binary
trees with internal edges labeled by the BRST homotopy $h$ and
leaves labeled by the original $m_2$ (and higher $m_k$ if present).
For the principal nilpotent $f$ in $\mathfrak{sl}_2\subset\fg$, the
BRST homotopy has a nontrivial kernel in every conformal weight, so
each tree contributes.

(3) The nontriviality of $m_k^{\mathrm{DS}}$ for $k\gg 0$ means the
bar differential has components in arbitrarily high tensor degree.
The bar cohomology therefore spreads across multiple diagonals.  For
Virasoro (the DS reduction of $\widehat{\mathfrak{sl}}_2$), the
operations $m_k\neq 0$ for all $k\ge 2$ (each $m_k$ receives a
contribution from the $(k-1)$-vertex tree in the BRST transfer), and
the bar cohomology is genuinely non-concentrated.
\end{proof}
...
(before DS reduction) is chirally Koszul.  DS reduction generically
destroys Koszulness by introducing infinitely many transferred higher
operations from the BRST homotopy.}
...
\emph{exists} for all theories satisfying (H1)--(H4).  The remaining
...
\begin{theorem}[Width bound for bar cohomology; \ClaimStatusConditional]
\label{thm:width-bound}
Under (H1)--(H4), the bar cohomology of $\cA$ satisfies
\[
H^{p,q}(\overline{B}^{\mathrm{ch}}(\cA))=0
\qquad\text{for }|q|>\mathfrak{L}(\cA).
\]
In particular, $\mathfrak{L}(\cA)=0$ is necessary for chiral
Koszulness, and $\mathfrak{L}(\cA)=0$ together with formality of the
tree-level bar complex is sufficient.
\end{theorem}
...
$E_{\mathfrak{L}(\cA)+1}$.  The width of the $E_\infty$ page
(= bar cohomology) is bounded by $\mathfrak{L}(\cA)$.
