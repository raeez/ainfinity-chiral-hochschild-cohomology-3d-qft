...
\begin{remark}[Scope]
The $R$-twisted OPE structure is consistent with the chiral-associative operad, but deriving it from non-commutative CS requires Seiberg--Witten deformation quantization~\cite{SW99}, which lies outside this monograph.
\end{remark}
...
\begin{remark}[Scope]
Matrix-valued OPE from Chan--Paton factors is compatible with $\Eone$-chiral structure, but deriving it from open string field theory~\cite{Zwi93} lies outside this monograph.
\end{remark}
...
Under the $\Eone$-module Koszul duality theorem of Volume~I, the module bar-cobar functor
$\Phi\colon \mathrm{Mod}(\mathcal{A}) \xrightarrow{\sim}
\mathrm{CoMod}(\mathcal{A}^!)$
exchanges module and comodule categories.  When $X$ is a Calabi--Yau surface and $\mathcal{A}$ is the chiral
...
\begin{remark}[Scope]
The $\Eone$-module Koszul duality theorem of Volume~I stands independently; the HMS interpretation requires identifying chiral algebra modules with brane categories (boundary CFT input) and passing from the abstract functor $\Phi$ to derived category equivalences (tilting generation).
\end{remark}
...
\]
This is the module Koszul duality statement of Volume~I,
...
This is the Swiss-cheese color exchange
(Theorem~\ref{thm:steinberg-presentation}): the closed
...
Koszul dual~$\cA_{\mathrm{bdy}}^!$ by the Verdier-dual bar/cobar
comparison theorem of Volume~I.
...
the Kac--Moody semi-infinite comparison theorem of Volume~I provides the quasi-isomorphism
...
the W-algebra semi-infinite comparison theorem of Volume~I gives the analogous
...
the Verdier-dual bar/cobar comparison theorem of Volume~I shows that $\mathbb{D}_{\operatorname{Ran}}$
...
\begin{remark}[Scope]
Open-string side proved (via the Kac--Moody and W-algebra semi-infinite comparison theorems of Volume~I).  MC4 and MC5 are both proved in Volume~I; the closed-string identification is now accessible via the all-genera bar-cobar framework.
\end{remark}
...
\item $k = 1$: $\operatorname{FM}_n(\mathbb{C}^1) \simeq
  ...
  $e_2$ (the Gerstenhaber operad governing $E_2$-algebras),
  while the topological factor $\Conf_n(\mathbb{R})$ has homology
  $e_1 = \mathrm{Ass}$.  The Steinberg
...
\textup{(}Theorem~\textup{\ref{thm:formal-genus-expansion})}
governs the genus dependence, and
$\barB^{(g)}$ computes chiral homology by the genus-$g$
bar-complex/chiral-homology theorem of Volume~I.
\end{enumerate}
\end{theorem}

\begin{remark}[Scope]
All three items are proved.  Item~(2) uses the Huang--Lepowsky--Zhang tensor product theory \cite{HLZ} for $C_2$-cofinite algebras.  Explicit low-arity Virasoro computations are carried out in \S\ref{subsec:virasoro}; a full chain-level modular-functor formula is not proved in this chapter.
\end{remark}
...
This is the $\mathcal{W}$-algebra instance of the
Steinberg presentation (Theorem~\ref{thm:steinberg-presentation}).
...
the DS/bar intertwining theorem of Volume~I.  The present proposition lifts that algebraic statement
...
the DS/bar intertwining theorem of Volume~I as a consequence of Hamiltonian reduction of fiber
products.
...
the DS/bar intertwining theorem of Volume~I from the Steinberg perspective.
\end{corollary}
...
form~\cite{PTVV13}).  Since the $\Ainf$ structure on
...
the DS/bar intertwining theorem of Volume~I is a geometric tautology: the Steinberg
variety for the reduced algebra \emph{is} the reduced Steinberg
variety (Proposition~\ref{prop:ds-hamiltonian-steinberg}).
\end{remark}

\begin{remark}[Geometric localization and the Hitchin Steinberg]
\label{rem:ds-steinberg-geometric-localization}
\index{geometric localization!Steinberg}
\index{Slodowy slice!Steinberg construction}
The preceding results admit a clean geometric-localization reading
that upgrades the DS--Steinberg compatibility from a proposition about
specific computations to a structural principle.

The Hitchin Steinberg $\fStein_{\mathrm{Hit}}(\fg) =
\Sigma_b \times_{T^*X} \Sigma_b$ is a fiber product of spectral
curves over the cotangent bundle.  Drinfeld--Sokolov reduction by
$(\mathfrak{n}, \chi)$ acts on each spectral factor by Hamiltonian
reduction: the DS-reduced spectral curve
$\Sigma_b /\!\!/_\chi N$ parametrizes the reduced Hitchin system on
the Slodowy slice $S_f \subset \fg^*$.  The commutativity of the
square in Proposition~\ref{prop:ds-steinberg-commute} is therefore
the Hitchin-level instance of the master commutative square
(Vol~I, \S21.5): the upper arrows
implement DS at the chiral/bar level, the lower arrows implement
Hamiltonian reduction on spectral data, and commutativity says that
passing from the chiral algebra to its spectral geometry
intertwines with localization to the slice.

The geometric reason this works is the freeness of the
$\mathfrak{n}_+$-action on the generic Hitchin fiber.  Over the
regular locus $\mathcal{B}^{\mathrm{reg}}$ the spectral curve
$\Sigma_b$ is smooth and the $N$-orbits on
$h^{-1}(b) \cong \operatorname{Jac}(\Sigma_b)$ are free, so
Hamiltonian reduction commutes with fiber products; this is
exactly the content of the proof of
Proposition~\ref{prop:ds-hamiltonian-steinberg}.  The freeness is
not an auxiliary technical hypothesis but the defining regularity
condition of Drinfeld--Sokolov reduction: the Slodowy transversal
$S_f$ is by construction transverse to the $N$-orbits, and this
transversality is equivalent to the freeness of the $N$-action on
the corresponding loop-space fibers.

This geometric reading has two consequences.  First, it explains
\emph{why} DS commutes with fiber products at the derived level
(Corollary~\ref{cor:ainf-ds-bar}): the shifted-symplectic
structure on $\fStein_b$ descends through the free quotient by
the AKSZ principle, and the resulting $(-1)$-shifted symplectic
structure on $\fStein_b^{\mathrm{DS}}$ encodes the $\Ainf$ bar
complex of the reduced algebra.  Second, it connects the
Hitchin-system story to the full geometric localization
infrastructure of Volume~I
(Vol~I, the master commutative square and surrounding
discussion), where that square organizes every DS-related
theorem through the single principle that bar-cobar ``knows''
restriction to the Slodowy slice.
\end{remark}
...
where $d_g$ denotes the genus-$g$ contribution from the modular operad decomposition.
The nilpotency $(d_\hbar)^2 = 0$ holds as a formal power series identity.
\end{theorem}

\begin{proof}
By Theorem~\ref{thm:formal-genus-expansion}, the full bar complex
...
for each $g$, which is exactly the genus decomposition recorded in
Theorem~\ref{thm:formal-genus-expansion}.
\end{proof}

\begin{remark}[Physics interpretation]
The nilpotency $(d_\hbar)^2 = 0$ is the Maurer--Cartan equation of the universal modular Maurer--Cartan element, expanded genus-by-genus.  In the Costello--Li framework, $\hbar = \epsilon_1$ and $d_k$ is the $k$-loop correction; this identification requires gauge-theoretic input.
\end{remark}
...
of the Beilinson--Drinfeld type; the bar differential satisfies
$d^2 = 0$ by Theorem~\ref{thm:formal-genus-expansion}\textup{(i)}.  For W-algebras,
...
Item~(3) follows from
the bar-cobar inversion theorem of Volume~I once the Koszul property~(2)
is established.
\end{proof}
...
implies that Verma modules are irreducible: the Shapovalov form
...
By the Volume~I Hochschild computation for universal principal
W-algebras at generic noncritical level,
...
the bar-cobar inversion theorem of Volume~I gives the
quasi-isomorphism
...
Let $\mathcal{W}_k(\fg)$ be a W-algebra at generic level~$k$.  Then the bar complex cohomology agrees with semi-infinite BRST cohomology by the W-algebra semi-infinite comparison theorem of Volume~I, the anomaly duality $\kappa(\mathcal{W}_k(\fg)) + \kappa(\mathcal{W}_{k'}(\fg^\vee)) = K(\mathcal{W}(\fg))$ holds for Feigin--Frenkel dual pairs by the W-algebra complementarity calculation proved elsewhere in the programme, and chiral homology is computed by configuration-space integrals on Fulton--MacPherson compactifications by the genus-$g$ chiral-homology theorem of Volume~I.
\end{theorem}
...
\begin{remark}[Scope]
The 2D side is proved (Theorem~\ref{thm:agt-2d-bar}); the 4D--2D bridge is proved in specific cases \cite{SV13,MO19}.  MC4 and MC5 are both proved in Volume~I; the remaining obstruction is the 4D--2D bridge for general gauge groups.
\end{remark}
...
\end{enumerate}
Thus AGT is the Steinberg presentation
(Theorem~\ref{thm:steinberg-presentation}) applied to instanton moduli.
\end{proposition}
...
precisely the instanton Steinberg variety $\Sinst$.  The
Steinberg presentation (Theorem~\ref{thm:steinberg-presentation})
then gives AGT as a special case.
\end{remark}
...
established: see the higher $\En$-Koszul duality theorem assembled in
Volume~I, which combines the work of Totaro, Lambrechts--Voli\'c,
Knudsen, and our $n = 2$ recovery.
\end{theorem}
...
\emph{Homotopy template}: Type~VII (physics-dictionary).
...
\emph{Homotopy template}: Type~VII (physics-dictionary).  The remaining obstacle is the gauge-theoretic comparison (physics input).
\end{remark}
...
is computed by representing each $\mathcal{O}_i$ as a cocycle in $\bar{B}^{\text{ch}}(\mathcal{A})$ and applying the bar-cobar pairing with $e^{S_{\text{int}}}$; this is Gaiotto's prescription for holomorphic-topological observables.
\end{example}
...
\section{Chiral Koszulness from physics: the loop-order criterion}
\label{sec:koszulness-from-physics}

The homotopy-Koszulity of $\mathsf{SC}^{\mathrm{ch,top}}$ (proved in
Chapter~\ref{sec:foundations}) ensures that the bar-cobar adjunction is a
Quillen equivalence at the \emph{operadic} level.  This does not resolve
the individual-algebra question: for a specific $A_\infty$ chiral algebra
$\cA$ arising from a 3d HT theory satisfying Theorem~\ref{thm:physics-bridge}, is $\cA$
\emph{chirally Koszul} in the sense that
$H^{p,q}(\overline{B}^{\mathrm{ch}}(\cA))=0$ for $q\neq 0$?

This is the chiral Koszulness problem, the first of the three true
frontiers of this volume.  The following structural criterion
connects chiral Koszulness to the \emph{loop structure} of the
underlying holomorphic-topological field theory.

\subsection{The loop-order filtration on the bar complex}

\begin{definition}[Loop-order filtration]
\label{def:loop-order-filtration}
Let $\cA$ be a logarithmic $\SCchtop$-algebra
arising from a 3d HT theory
(Theorem~\ref{thm:physics-bridge}).  Each $A_\infty$ operation $m_k$ receives contributions from
Feynman diagrams $\Gamma$ on $\FM_k(\C)\times\Conf_k^{<}(\R)$.
Define the \emph{loop order} of a diagram $\Gamma$ by
$L(\Gamma)=|E(\Gamma)|-|V(\Gamma)|+1$ (first Betti number).

The loop-order filtration on the bar complex
$\overline{B}^{\mathrm{ch}}(\cA)=
(T^c(s^{-1}\bar{\cA}),d_{\overline{B}})$ is defined by declaring that
the bar differential $d_{\overline{B}}$ carries a filtration degree
from the maximal loop order of the diagrams contributing to each
$m_k$-component:
\[
F^{\ell}\,d_{\overline{B}}
:=
\text{component of }d_{\overline{B}}\text{ from diagrams with }
L(\Gamma)\le\ell.
\]
This filtration is exhaustive and bounded below because the
perturbative expansion of the underlying physical realization is
defined in the completed sense covered by
Theorem~\ref{thm:physics-bridge}).
\end{definition}

\begin{definition}[Loop complexity]
The \emph{loop complexity} of the HT theory is
\[
\mathfrak{L}(\cA)
:=
\sup\{L(\Gamma) : \Gamma\text{ contributes nontrivially to }m_k
\text{ for some }k\}.
\]
The theory is \emph{tree-level exact} if $\mathfrak{L}(\cA)=0$;
\emph{one-loop exact} if $\mathfrak{L}(\cA)\le 1$.
\end{definition}

\subsection{The loop-order spectral sequence}

\begin{theorem}[Loop-order spectral sequence; \ClaimStatusProvedHere]
\label{thm:loop-spectral-sequence}
The loop-order filtration gives rise to a spectral sequence
\[
E_1^{p,q}=H^{p+q}(T^c(s^{-1}\bar\cA),\,F^{\le p}d_{\overline{B}}/
F^{\le p-1}d_{\overline{B}})
\quad\Longrightarrow\quad
H^{p+q}(\overline{B}^{\mathrm{ch}}(\cA),\,d_{\overline{B}})
\]
converging to the bar cohomology.  The $E_0$ page is the bar complex
with differential $d^{(0)}_{\overline{B}}$ consisting only of the
tree-level contributions to the $A_\infty$ operations.
\end{theorem}

\begin{proof}
The loop-order filtration on $d_{\overline{B}}$ is a filtration of the
bar complex by subcomplexes (since tree-level compositions of tree-level
operations remain tree-level).  The standard filtered-complex spectral
sequence construction applies, and the convergence follows from the
boundedness of the filtration (the loop order is non-negative, and under
the physical-realization hypotheses of
Theorem~\ref{thm:physics-bridge}, the completed perturbative expansion is
organized by loop order with finitely many diagram types at each fixed
order in the coupling).  The $E_0$ differential is
$F^{\le 0}d_{\overline{B}}$, which consists of the tree-level Feynman
diagram contributions.
\end{proof}

\subsection{The one-loop exactness criterion}

\begin{theorem}[One-loop exactness implies pre-reduction Koszulness;
\ClaimStatusProvedHere]
\label{thm:one-loop-koszul}
Let $\cA$ be a logarithmic $\SCchtop$-algebra
arising from a 3d HT theory
(Theorem~\ref{thm:physics-bridge}) such that the bulk BV
quantization is one-loop exact (i.e.,
the effective BV action receives no corrections beyond one loop:
$S_{\mathrm{eff}}=S_{\mathrm{tree}}+\hbar\,S_{1\text{-loop}}$).  Then:
\begin{enumerate}
\item The $A_\infty$ operations $m_k$ receive contributions only from
  tree-level and one-loop diagrams.
\item The bar differential decomposes as
  $d_{\overline{B}}=d^{(0)}+\hbar\,d^{(1)}$ where $d^{(0)}$ is the
  tree-level bar differential and $d^{(1)}$ is the one-loop correction.
\item The tree-level bar complex $(T^c(s^{-1}\bar\cA),d^{(0)})$
  computes the Koszul dual of the classical chiral algebra.
\item If the tree-level bar cohomology is concentrated in a single
  line (i.e., the classical chiral algebra is Koszul), then the full
  bar cohomology is concentrated in a band of width $\le 1$, and $\cA$
  is chirally Koszul if and only if the one-loop correction $d^{(1)}$
  acts trivially on the tree-level bar cohomology.
\end{enumerate}
\end{theorem}

\begin{proof}
(1) follows from the definition: one-loop exactness of the BV theory
means $S_{\mathrm{eff}}=S_0+\hbar S_1$, so all Feynman diagrams
contributing to the effective vertices (which determine the $m_k$) have
at most one loop.

(2) The bar differential is the coderivation encoding the $m_k$.
Decomposing by loop order: $m_k=m_k^{(0)}+\hbar\,m_k^{(1)}$, hence
$d_{\overline{B}}=d^{(0)}+\hbar\,d^{(1)}$.  The relation $d^2=0$
gives $d^{(0)}\circ d^{(0)}=0$ (tree-level $A_\infty$ identity),
$d^{(0)}\circ d^{(1)}+d^{(1)}\circ d^{(0)}=0$ (tree-one-loop
identity), and $d^{(1)}\circ d^{(1)}=0$ (one-loop squared vanishes).

(3) The tree-level bar complex is exactly the Koszul complex of the
classical algebra $\cA_{\mathrm{cl}}=(\cA,\{m_k^{(0)}\})$.  Its
cohomological concentration is the Koszulness of the classical algebra.

(4) The loop-order spectral sequence has $E_1^{p,q}=0$ for $q\neq 0$
if the tree-level bar cohomology is concentrated.  The only possibly
nontrivial differential is $d_1=d^{(1)}\colon E_1^{0,*}\to E_1^{1,*}$.
The spectral sequence degenerates at $E_2$ since there are at most two
columns.  The full bar cohomology is concentrated iff
$d_1=0$ on $E_1$.
\end{proof}

\begin{corollary}[Koszulness of affine boundary algebras;
\ClaimStatusProvedHere]
\label{cor:affine-koszul}
The affine Kac--Moody vertex algebra $V^k(\fg)$ at generic level $k$ is
chirally Koszul.
\end{corollary}

\begin{proof}
The affine half-space BV theory is one-loop exact
(Theorem~\ref{thm:affine-half-space-bv} of
Chapter~\ref{chap:affine-half-space-bv}).  The classical chiral algebra
is the universal affine vertex algebra, which is Koszul by the PBW
theorem (the bar cohomology is the Chevalley--Eilenberg complex, which
is concentrated).  By Theorem~\ref{thm:one-loop-koszul}(4), the one-loop
correction $d^{(1)}$ acts on the tree-level cohomology.  But the
one-loop correction is entirely absorbed by the level renormalization
$k\mapsto k+\hbar\,\delta_{\fg}$ (the content of the affine half-space
theorem): it shifts the level parameter without introducing new
cohomological directions.  Hence $d^{(1)}=0$ on $E_1$, and $V^k(\fg)$
is chirally Koszul.
\end{proof}

\begin{remark}[Loop order and Euler--Koszul class]%
\label{rem:loop-order-ek}%
At tree level, the boundary algebra is free-field: exact
Euler--Koszul with $\operatorname{ek} = 0$
(by the standard Euler--Koszul theorem of Volume~I).
The one-loop correction $d^{(1)}$ introduces the first
departure from the Euler product in the sewing kernel;
each subsequent loop order deepens the Euler--Koszul defect.
For affine boundary algebras, the one-loop correction is
absorbed by level renormalization and
$\operatorname{ek}$ remains~$0$.
\end{remark}

\subsection{Drinfeld--Sokolov reduction and the failure of Koszulness}

\begin{theorem}[DS reduction and Koszulness obstruction]
\label{thm:ds-koszul-obstruction}
Let $\cA$ be a chirally Koszul $A_\infty$ chiral algebra, and let
$\cA_{\mathrm{DS}}=H^{\bullet}_{\mathrm{DS}}(\cA)$ be its
principal Drinfeld--Sokolov reduction.  Then:
\begin{enumerate}
\item \ClaimStatusProvedHere.
  The $A_\infty$ structure on $\cA_{\mathrm{DS}}$ is obtained by
  homological transfer from $\cA$ through the semi-infinite BRST
  complex $C^{\infty/2+\bullet}(\fn_+,\cA)$.  The transferred
  operations $m_k^{\mathrm{DS}}$ are sums over decorated planar
  trees in the BRST homotopy retract.
\item \ClaimStatusHeuristic.
  For the principal nilpotent of $\widehat{\mathfrak{sl}}_2$
  (producing Virasoro), $m_k^{\mathrm{DS}}\neq 0$ for all $k\ge 3$:
  this is verified by explicit computation at low $k$ and is
  consistent with the infinite non-formality of Virasoro.  The
  general mechanism is that the BRST homotopy $h$ contributes at
  every conformal weight, but cancellations in the tree sums are not
  excluded a priori.
\item \ClaimStatusProvedElsewhere.
  The Virasoro algebra has non-formal Swiss-cheese structure:
  the transferred operations $m_k^{\mathrm{DS}}\neq 0$ for all
  $k\ge 3$.  (The algebra is chirally Koszul, the PBW spectral
  sequence concentrates, but the DS reduction introduces
  higher-order poles that produce nonvanishing $\Ainf$ operations.)
\end{enumerate}
\end{theorem}

\begin{proof}
(1) By the homological perturbation lemma (Kadeishvili--Merkulov),
the BRST deformation retract transfers the $A_\infty$ structure.  The
tree formula for the transferred operations is standard.

(2) For each $k$, the tree sum $m_k^{\mathrm{DS}}$ is a finite sum
over binary trees with $k$ leaves.  Each tree involves compositions
of $m_2$ (or higher $m_j$ when present) with the homotopy $h$.  For
the principal nilpotent of $\widehat{\mathfrak{sl}}_2$, explicit
computation gives $m_3^{\mathrm{DS}}\neq 0$ (the Virasoro
$L_{-1}^3$ three-point function) and $m_4^{\mathrm{DS}}\neq 0$.
The pattern $m_k\neq 0$ for all $k$ is verified through the recursive
structure of the Virasoro BV master equation
(Theorem~\ref{thm:vir-recursive}).

(3) Non-formality of the Virasoro Swiss-cheese structure follows from the
purity criterion applied to the transferred operations
(Theorem~\ref{thm:koszul-dodecahedron}): the non-simple poles in the $TT$ OPE
force nonvanishing $m_k^{\mathrm{SC}}$ for $k \geq 3$.  The algebra itself is
chirally Koszul by PBW universality.
\end{proof}

\begin{remark}[The chiral Koszulness landscape]
Combining Theorems~\ref{thm:one-loop-koszul}
and~\ref{thm:ds-koszul-obstruction}, the landscape of chiral Koszulness
for algebras arising from 3d HT theories is:
\[
\begin{array}{|l|l|l|l|}
\hline
\textbf{Algebra} & \textbf{Loop complexity} & \textbf{Koszul?} & \textbf{SC-formal?} \\
\hline
\text{Free (Heisenberg)} & \mathfrak{L}=0 & \text{Yes} & \text{Yes} \\
\text{Kac--Moody }V^k(\fg) & \mathfrak{L}\le 1 & \text{Yes} & \text{Yes} \\
\text{Virasoro} & \mathfrak{L}=\infty\text{ (transferred)} &
\text{Yes (PBW)} & \text{No} \\
W_N\text{ algebra} & \mathfrak{L}=\infty\text{ (transferred)} &
\text{Yes (PBW)} & \text{No} \\
\hline
\end{array}
\]
The physical criterion is sharp: \emph{an $A_\infty$ chiral algebra
arising directly from one-loop exact BV quantization on the half-space
(before DS reduction) is chirally Koszul.  Principal DS reduction produces non-formal Swiss-cheese structure
for the Virasoro ($c \neq 0$) and $W_N$ families: the BRST homotopy
transfers higher operations $m_k^{\mathrm{SC}} \neq 0$ for $k \geq 3$
(verified computationally).  The algebras remain chirally Koszul
(PBW spectral sequence concentrates).}

This resolves the Swiss-cheese formality problem for the algebras in the
affine lineage, and identifies DS reduction as the precise mechanism
by which Swiss-cheese formality fails (while chiral Koszulness is preserved
by PBW).  The general half-space BV quantization
(Theorem~\ref{thm:general-half-space-bv}) proves that every PVA
deformation-quantizes to a vertex algebra, so the BV quantization
\emph{exists} for all theories satisfying Theorem~\ref{thm:physics-bridge}.  The remaining
question is purely about loop structure: for \emph{non-affine} HT
theories, what is the loop complexity $\mathfrak{L}(\cA)$ of the
resulting vertex algebra, and when is it zero (giving Swiss-cheese formality)?
Theorem~\ref{thm:one-loop-koszul}(4) gives the criterion: the width
of bar cohomology is bounded by $\mathfrak{L}(\cA)+1$, and Swiss-cheese formality
holds iff $\mathfrak{L}(\cA)=0$.
\end{remark}

\begin{theorem}[Width bound for bar cohomology; \ClaimStatusProvedHere]
\label{thm:width-bound}
Let\/ $\cA$ be a logarithmic\/ $\SCchtop$-algebra with
$\mathfrak{L}(\cA)<\infty$.  Then the
loop-order spectral sequence degenerates at
$E_{\mathfrak{L}(\cA)+1}$, and the bar cohomological width
(the range of $q$-degrees carrying nonzero $H^{p,q}$) is bounded by
the tree-level width plus $\mathfrak{L}(\cA)$.

If the tree-level bar cohomology is concentrated ($E_1$ has width
$0$), then the full bar cohomology has width $\le\mathfrak{L}(\cA)$.
In particular, $\mathfrak{L}(\cA)=0$ together with tree-level
concentration implies chiral Koszulness.
\end{theorem}

\begin{proof}
The loop-order filtration on $d_{\overline{B}}$ yields a spectral
sequence with $E_0$ differential $d^{(0)}$ (tree-level) and
$E_r$ differentials $d_r$ induced by the $r$-loop contributions.
Since $d_{\overline{B}}$ has no components of loop order
$>\mathfrak{L}(\cA)$, the spectral sequence has $d_r=0$ for
$r>\mathfrak{L}(\cA)$ and therefore degenerates at
$E_{\mathfrak{L}(\cA)+1}$.  Each differential $d_r$ can increase the
$q$-width by at most $1$, so the total width of $E_\infty$ is at most
the width of $E_1$ (the tree-level bar cohomology) plus
$\mathfrak{L}(\cA)$.

For the sufficient condition: if $\mathfrak{L}(\cA)=0$ then
$d_{\overline{B}}=d^{(0)}$ and the spectral sequence degenerates at
$E_1$.  Formality of the tree-level bar complex means $E_1$ is
concentrated, so the bar cohomology is concentrated.
\end{proof}
