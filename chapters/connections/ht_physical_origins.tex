...
Chapter~\ref{ch:foundations}) ensures that the bar-cobar adjunction is a
...
L(\Gamma)\le\ell.
\]
This filtration is exhaustive and bounded below (under (H1), the
perturbative expansion converges in the completed sense).
\end{definition}
...
(H1) the perturbative expansion has finitely many diagrams at each
...
(Vol~I, Theorem~\ref{thm:euler-koszul-standard}).
...
\item \ClaimStatusProvedElsewhere.
  The Virasoro algebra is not chirally Koszul: $m_k\neq 0$ for all
  $k\ge 3$ (non-formality), and the bar cohomology is non-concentrated
  (the PBW spectral sequence does not degenerate; see
  Section~\ref{sec:virasoro-non-koszul}).
\end{enumerate}
\end{theorem}
...
(3) Non-Koszulness of Virasoro follows from the purity criterion
(Theorem~\ref{thm:koszul-dodecahedron}): the bar cohomology carries
mixed weights from the non-simple poles in the $TT$ OPE.
\end{proof}
...
\begin{array}{|l|l|l|}
\hline
\textbf{Algebra} & \textbf{Loop complexity} & \textbf{Koszul?} \\
\hline
\text{Free (Heisenberg)} & \mathfrak{L}=0 & \text{Yes} \\
\text{Kac--Moody }V^k(\fg) & \mathfrak{L}\le 1 & \text{Yes} \\
...
(before DS reduction) is chirally Koszul.  Principal DS reduction
destroys Koszulness for the Virasoro ($c \neq 0$) and $W_N$ families by
introducing transferred higher operations from the BRST homotopy
(verified computationally; the non-Koszulness itself follows from
purity failure in the bar complex).}

This resolves the chiral Koszulness problem for the algebras in the
affine lineage, and identifies DS reduction as the precise mechanism
by which Koszulness fails.  The general half-space BV quantization
...
resulting vertex algebra, and when is it zero (giving Koszulness)?
Theorem~\ref{thm:one-loop-koszul}(4) gives the criterion: the width
of bar cohomology is bounded by $\mathfrak{L}(\cA)+1$, and Koszulness
