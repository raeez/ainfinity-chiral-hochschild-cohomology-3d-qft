...
\[
\mathrm{Obs}_4=
m_2(m_3(T,T,T),T)-m_2(T,m_3(T,T,T))
+m_3(m_2(T,T),T,T)
-m_3(T,m_2(T,T),T)
+m_3(T,T,m_2(T,T)),
\]
...
gauge convention gives~\eqref{eq:gravity-m4}.  The source-tree witness
is `compute/lib/symbolic_stasheff.py`, functions
`stasheff_rhs_arity4` and `m4_virasoro_symbolic`; the independent
factorisation guard is
...
\(-1\).  The source-tree witness is
`compute/lib/symbolic_stasheff.py`, functions `stasheff_rhs_arity5`
and `m5_virasoro_symbolic`; the guard
`compute/tests/test_gravity_m5_stasheff_gauge.py` checks the six
coefficients and the symmetric specialization.
\end{proof}
...
\(\partial T\) at depth~$2\).  At degree~$6$, the same cancellation is
detected by the source-tree recursion in
`compute/m7_m10_depth_frontier.py::StasheffEngine`: for generic
spectral parameters the coefficients of \(d^5T\) and \(d^4T\) vanish,
while \(d^3T,d^2T,dT,T\) and the scalar term are nonzero.  The focused
guard is `compute/tests/test_gap_migration_m6.py`.
...
\end{tabular}
\end{center}
The executable witness is `compute/m7_m10_depth_frontier.py`,
class `StasheffEngine`; the guard
`compute/tests/test_gap_migration_m6.py` checks the absence of the
depth-$0$ and depth-$1$ coefficients at generic spectral parameters.
\end{computation}
...
\end{enumerate}
The source-tree guard verifies the rightmost-reduction consequences and
...
\HalfBraid_{E \times \R},\,\mathrm{tr}^{\mathrm{cl}}_{K3 \times E}
= \Phi_{10}^{\mathrm{un}},\,\TraceC_{\mathrm{open}},\,
\Zderch{A_g}\bigr)$
...
\(\SpCh_{E,C}\) is the elliptic-to-reference-curve specialisation, and
