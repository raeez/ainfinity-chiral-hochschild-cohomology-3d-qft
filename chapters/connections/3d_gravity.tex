...
\item \textbf{Lines} $\simeq \mathrm{Vir}_{26-c}\text{-mod}$:
  ...
  defect of angular deficit $2\pi(1 - h/c)$ (valid to leading order in $h/c \ll 1$; the exact Ba\~nados--Teitelboim--Zanelli formula is $2\pi(1-\sqrt{1-8Gh})$).
...
%====================================================================
% MOVEMENT III.5: DS-HPL TRANSFER
%====================================================================
...
Drinfeld--Sokolov reduction.  The affine dg-shifted Yangian
$Y^{\mathrm{dg}}(\widehat{\mathfrak{sl}}_2)$ carries a coproduct
$\Delta_z$ and classical $r$-matrix $r^{\mathrm{aff}}(z) = \Omega/z$.
Homological perturbation transfers this entire structure to a
\emph{Virasoro dg-shifted Yangian} on the BRST cohomology.

\subsubsection*{The strong deformation retract}

Write the DS-adapted generators of the BRST complex as
...
$d_0(U) = 2c$, and $d_0 = 0$ on $W$, $V$, $c$.  The cohomology
is spanned by $W$ and its derivatives (the Virasoro sector).

Define the SDR $(i, p, h)$:
\begin{align*}
...
The triple $(i, p, h)$ satisfies the SDR conditions:
...
On $U$: $(\operatorname{id} - ip)(U) = U$;
$(d_0 h + h d_0)(U) = d_0(U/2) + h(2c)
= c + U/2 \cdot 2 = U$
(using $d_0(U/2) = c$ and $h(2c) = U$, which sum to $U + 0$;
more precisely $d_0(U) = 2c$ gives
$h d_0(U) = h(2c) = 2 \cdot U/2 = U$, and
$d_0 h(U) = d_0(0) = 0$, so $(d_0 h + h d_0)(U) = U$).
On $c$: $(\operatorname{id} - ip)(c) = c$;
$(d_0 h + h d_0)(c) = d_0(U/2) + h(0) = c$.
\end{proof}
...
(ghost interactions, OPE corrections from normal ordering).
The homological perturbation lemma transfers the $\Ainf$ structure
through the SDR.
...
\item A transferred coproduct
  ...
  $\Delta_z^{\mathrm{aff}}$ through the SDR and its tensor product.
...
The theorem establishes the structural framework.
Three verifications remain:
\textup{(a)}~full sign verification of the transferred
products and coproduct at arity~$\ge 3$;
\textup{(b)}~explicit computation of the first nonlinear
coproduct correction $\Delta_{z,2}^{\mathrm{Vir}}(T,T)$
(the first gravitational vertex);
\textup{(c)}~verification that the transferred structure
...
Z_1^{\mathrm{scal}}(\tau)
\;=\;
\exp\!\left(-F_1 \cdot \log\eta(\tau)\right)
\;=\;
\eta(\tau)^{-c/48},
\end{equation}
using $\log\eta(\tau) = \frac{2\pi i\tau}{24}
- \sum_{n=1}^{\infty}\frac{q^n}{n(1-q^n)}$
and the fact that $F_1 = c/48 = (c/2)\cdot 1/24$ matches the
exponent $-\kappa/24$ in the scalar partition function
$Z_1 = \eta(\tau)^{-\kappa}$ with $\kappa = c/2$.
...
corrections.
Here the expansion parameter is $x$ (not $\hbar$); the genus-$g$
correction enters at $x^{2g} = \hbar^g$.
...
The genus-$1$ computation reveals three fundamental connections.
