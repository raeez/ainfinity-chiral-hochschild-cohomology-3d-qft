...
satisfying the hypotheses of Theorem~\ref{thm:physics-bridge}).  By the argument of
...
(note: $\cH_k^! \neq \cH_{-k}$; see Vol~I, \S\ref{sec:frame-koszul-dual}).
...
For the $A_\infty$ claim: the shadow Postnikov obstruction tower
...
The physical interpretation is narrower.  The infinite
$A_\infty$-hierarchy on Virasoro modules reflects the
infinite mixed shadow depth of the Virasoro side and should
be regarded generically as a higher derived transport /
Ward hierarchy on the corresponding module package.  On the
degenerate-representation comparison surface, where null
vectors truncate the hierarchy, the resulting finite-order
specializations recover the corresponding BPZ
...
(Theorem~\ref{thm:yangian-recognition}); in the standard affine HT
gauge realization, Theorem~\ref{thm:Koszul_dual_Yangian}
identifies it explicitly.  This section defines the corresponding
dg-shifted Yangian intrinsically from the convolution algebra,
constructs the evaluation homomorphism, and proves the RTT
formalism governs the evaluation-module compatibility package on the
affine/proved lane.
...
\item The Laplace kernel $r^L_c(z)$ has a fourth-order pole
  ...
  $r(z)\sim\Omega/z$; the Virasoro Laplace kernel has the
...
\item \textbf{DK-5} (full monoidal equivalence):
  ...
  proved for all simple types.  In type~$A$, the remaining post-core
  obstruction is the compact-completion packet isolated by the
  Volume~I reduction theorem; for other types the analogous post-core
  mechanism is conditional on the later shifted/completed inputs.
\end{enumerate}
\end{definition}
...
corrections, curvature $\kappa$, shadow Postnikov obstruction tower).
...
\item The shadow Postnikov obstruction tower
