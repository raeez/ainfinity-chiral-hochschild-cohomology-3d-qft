...
and the monoidal product from the $R$-matrix.
Throughout, $\cA$ denotes a logarithmic $\SCchtop$-algebra
(Definition~\ref{def:log-SC-algebra}), with bar complex
...
the action of $\cA^!$ on $M$ is associative up to
...
because each degree-$n$ component $(\partial f)_n$ depends
only on $f_{n-1}$, $f_n$, and $f_{n+1}$. The series
stabilizes degree-by-degree, requiring no analytic convergence
condition. Composition
$(g\circ f)_n = \sum_{i+j=n}g_i\circ(\mathrm{id}^{\otimes i}
\otimes f_j)$ is likewise a finite sum at each degree.
\end{remark}
...
\end{itemize}
Composition is associative and compatible with
$\partial$ by the standard bar-cobar argument
(cf.~\cite{LH03}, \S3.2).
\end{definition}
...
(Theorem~\ref{thm:scchtop-bar-cobar-adjunction}) extends to module
...
Koszul (Hoefel and Hoefel--Livernet~\cite{Hoe09,HL12}; the binary quadratic
...
satisfying the hypotheses of Theorem~\ref{thm:physics-bridge}). The
comparison above identifies $\cC_{\mathrm{line}}$ with the essential
image of the physically realizable $\cA^!$-modules. When the
essential-realizability hypothesis of
Theorem~\ref{thm:lines_as_modules} holds, this image is the entire
...
category as $H^0(\cA^!\text{-}\mathbf{mod})$. The
full $A_\infty$-enrichment records finer data:
Massey products in $\operatorname{Ext}^*_{\cA^!}(M,N)$,
formality obstructions, and
the $A_\infty$-Yoneda embedding
$\cA^!\text{-}\mathbf{mod}\hookrightarrow
\mathrm{Fun}(\cA^!,\mathrm{Ch})$. The minimal model theorem
recovers the strict model: every
$A_\infty$-module is $A_\infty$-quasi-isomorphic to a
minimal one ($\mu_1^M=0$), and the minimal
$A_\infty$-module category is the Kadeishvili
...
We extract the $A_\infty$-module structures for three
families.
...
(note: $\cH_k^! \neq \cH_{-k}$; see Vol~I, \S\ref{V1-sec:frame-koszul-dual}).
The line category $\cC_{\mathrm{line}} \simeq \cH_{-k}\text{-}\mathbf{mod}$
is an $A_\infty$-module category over $\cH_k^!$,
equivalent to $\cH_{-k}\text{-}\mathbf{mod}$
via the Yangian identification $Y(\mathfrak{u}(1)) \simeq \cH_{-k}$.
...
(Vol~I, \S\ref*{V1-sec:heisenberg-shadow-gaussianity}).
\end{proposition}
...
\S\ref*{V1-sec:heisenberg-shadow-gaussianity}): the only
...
\[
\cA^!\simeq
\operatorname{CE}^{\mathrm{ch}}\!\bigl(\widehat{\mathfrak{sl}}_2{}_{-k-4}\bigr).
\]
The affine algebra at level $-k-4$ is the reflected current
presentation, with $k\mapsto -k-2h^\vee=-k-4$ for $\mathfrak{sl}_2$.
...
After fixing the KZ/Drinfeld associator, the affine evaluation
descent path, and the Kazhdan--Lusztig tensor transport, the descended
evaluation subcategory of
$\widehat{\mathfrak{sl}}_2{}_{-k-4}\text{-}\mathbf{mod}$
has descended braided monoidal structure with exchange
$R(z)=\exp(k\,\Omega/(z(k{+}2)))$, where
$\Omega=e\otimes f+f\otimes e+\tfrac12 h\otimes h$.
This descended braided monoidal category is equivalent to a
...
(the affine algebra terminates at degree $3$;
Vol~I, \S\ref*{V1-sec:affine-cubic-shadow}). The non-vanishing
...
\begin{proof}
The local spectral tensor datum has binary part the Yangian
coproduct $\Delta_z$ on $\cA^!=Y_\hbar(\mathfrak{sl}_2)$
(Theorem~\ref{thm:Koszul_dual_Yangian}) and three-point part the
chosen KZ/Drinfeld associator. The
$R$-matrix $R(z)=\exp(k\,\Omega/(z(k{+}2)))$ is the
explicit local exchange kernel associated with the KZ connection
$\nabla=d-\Omega\,d\log(z)/(k+2)$
(Computation~\ref{comp:line-op-sl2}). After descent along the
evaluation exchange path, this monodromy
...
For the $A_\infty$ claim: the shadow obstruction tower
of $\widehat{\fg}_k$ terminates at degree $3$
...
$\Hom_{\fg}(V_\lambda,V_\mu)_{\mathrm{loc}}$
is the local system induced by the affine Kac--Moody
comparison-surface KZ monodromy on the coefficient space
$\Hom_{\fg}(V_\lambda,V_\mu)$.
...
(Vol~I, Remark~\ref*{V1-rem:virasoro-resonance-model}): the
...
for any finite~$n$: the Virasoro shadow obstruction tower is infinite
($r_{\max}=\infty$, class~M;
Vol~I, \S\ref*{V1-sec:mixed-cubic-quartic-shadows}), so the
...
depth; Vol~I, \S\ref*{V1-sec:mixed-cubic-quartic-shadows}).
...
$A_\infty$-structure is nontrivial at every degree: there exist
...
degree pair nontrivially with the generic highest-weight
structure.

Physically, the infinite
$A_\infty$-hierarchy on Virasoro modules reflects the
infinite mixed shadow depth. On the
degenerate-representation locus, where null
vectors truncate the hierarchy, the finite-order
specializations recover the BPZ
...
(Definition~\ref{def:universal-mc-kernel}) controls the monoidal structure on $\cA^!\text{-}\mathbf{mod}$.
...
is the associator-controlled Yang--Baxter relation
(Theorem~\textup{\ref{thm:YBE}}) for the chosen local exchange datum.
\end{proposition}
...
\item Mixed terms involving both $M$ and $N$ structure
 maps, connected by the exchange family: these vanish when
 the leading term \(r(z)\) satisfies the classical Yang--Baxter
 equation
 \([r_{12},r_{13}]+[r_{12},r_{23}]+[r_{13},r_{23}]=0\)
 and the quantized exchange family satisfies the
 associator-controlled Yang--Baxter relation. The strict equation
 \(R_{12}R_{13}R_{23}=R_{23}R_{13}R_{12}\) belongs only to a
 strictified comparison surface.
\end{enumerate}
The associator-controlled relation is proved in
Theorem~\ref{thm:YBE}; with the chosen associator data, the
$A_\infty$-module relations hold.
\end{proof}
...
(Theorem~\ref{thm:yangian-recognition}); Theorem~\ref{thm:Koszul_dual_Yangian}
identifies it explicitly in the standard affine HT
gauge realization. This section defines the
dg-shifted Yangian intrinsically from the convolution algebra,
constructs the evaluation homomorphism, and proves that the RTT
formalism governs evaluation-module compatibility on the
affine/proved lane.
...
Yangian correctly parametrizes the strict genus-$0$
deformation data on the dg-shifted-Yangian lane. This does not by
itself identify the full generic line-operator deformation theory with
the dg-shifted Yangian outside that lane.
\end{remark}
...
\begin{proposition}[Evaluation modules and the affine/evaluation core;
\ClaimStatusProvedHere]
% label removed: prop:evaluation-modules-dense
\index{evaluation modules!affine core}
...
$\Ydg(\cA^!)$-module whenever the evaluation homomorphism is
available. On the affine/proved dg-shifted-Yangian lane, these
evaluation modules furnish the distinguished evaluation-generated core
used in the theorematic DK comparison. No general statement is made
here that every $\Ydg(\cA^!)$-module is built from evaluation modules.
\end{proposition}

\begin{proof}
This is immediate from the definition of the evaluation homomorphism:
pullback along $\mathrm{ev}_{z_0}$ turns any $\cA^!$-module into a
$\Ydg(\cA^!)$-module. On the affine/proved lane, the later
evaluation-core DK comparison is stated precisely on the thick
subcategory generated by such evaluation objects.
\end{proof}

\subsection{The RTT formalism}
% label removed: subsec:rtt-formalism

\begin{theorem}[RTT compatibility on the evaluation-module lane;
\ClaimStatusProvedHere]
% label removed: thm:yangian-controls-fusion
\index{RTT relation!controls fusion|textbf}
Let\/ $\cA$ be a logarithmic\/ $\SCchtop$-algebra, and suppose we are on
a dg-shifted-Yangian lane where evaluation modules
$\mathrm{ev}_{z_i}^*(M_i)$ are available. Then the RTT relation
$R(z_1{-}z_2)\,T_1(z_1)\,T_2(z_2)
=T_2(z_2)\,T_1(z_1)\,R(z_1{-}z_2)$
is the compatibility condition for fusion / intertwining among these
evaluation modules. This theorem does not claim that all morphisms in
the full generic line-operator category arise in this way.
\end{theorem}
...
\textbf{Step 2: Compatibility on evaluation modules.}
For evaluation modules at spectral parameters $z_1,z_2,z_3$, the Yangian
actions are transported by the evaluation homomorphisms
$\mathrm{ev}_{z_i}$. The RTT relation then gives the braid/fusion
compatibility of the corresponding actions: if
$R(z_1{-}z_2)\,T_1(z_1)\,T_2(z_2)
=T_2(z_2)\,T_1(z_1)\,R(z_1{-}z_2)$,
then any intertwiner compatible with these evaluation actions must
respect that RTT constraint. This is exactly the evaluation-module
compatibility statement claimed above. No exhaustiveness statement for
the full line-operator category is asserted here.
\end{proof}
...
\emph{gravitational Yangian}: the dg-shifted-Yangian model encoding the
Virasoro RTT data on the affine / evaluation-module lane.
...
\begin{enumerate}[label=(\roman*)]
\item The Laplace kernel $r^L_c(z)$ has a fourth-order pole
 at $z=0$ (equation~\eqref{eq:virasoro-r-matrix-explicit}),
 not a simple pole. Standard Yangians have
 $r(z)\sim k\,\Omega_{\mathrm{tr}}/z$; the Virasoro Laplace kernel has the
 additional terms $2T/z^2+c/(2z^4)$ from the conformal
 anomaly (the collision residue $r^{\mathrm{coll}}_c(z) =
 (c/2)/z^3 + 2T/z$ has a cubic pole).
...
\end{enumerate}
Both departures are gravitational analogues of gauge-theoretic
Yangian features: the fourth-order pole replaces the gauge
simple pole $k\,\Omega_{\mathrm{tr}}/z$, and the infinite-dimensional algebra
...
the CYBE for $r(z)=k\,\Omega_{\mathrm{tr}}/z$, proved in
Proposition~\ref{prop:ybe-from-d-squared}).
...
$k=-2$ & undefined & undefined
 & critical level; Feigin--Frenkel centre and opers \\[3pt]
$k\to\infty$ & $0$ & $q\to 1$
 & $U(\mathfrak{sl}_2)$ (large-level limit)
\end{tabular}
\end{center}
At $k=-2$ (critical level), the parameters
$\hbar=1/(k+2)$ and $q=e^{i\pi/(k+2)}$ are not limiting values in
the KZ comparison. The KZ connection exits its non-critical domain,
and the Feigin--Frenkel centre/oper description replaces the
quantum-group comparison. The classical enveloping algebra in the
table is the large-level limit $k\to\infty$, equivalently
$\hbar\to0$, not the critical level.
\end{computation}
...
\begin{itemize}
\item MC1 (PBW/Koszul input + ordered-bar concentration):
 $H^n(\barB_{\mathrm{ch}}(\cA))=0$ for $n\gg 0$. PBW supplies the
 Koszul filtration input; ordered-bar Arnold--Orlik--Solomon/FM
 concentration supplies the vanishing. \textbf{Proved} for all
 standard families.
...
\item MC3 (categorical CG): tensor products of
 $\cA^!$-modules admit the categorical Clebsch--Gordan package, and
 the DK comparison is proved on the evaluation-generated core.
 \textbf{Partially proved for all simple types;} in type~$A$, the
 remaining post-core obstruction is reduced to the
...
\begin{enumerate}[label=(\roman*)]
\item the DK comparison is proved on the evaluation-generated core of
 tensor products of evaluation modules;
\item beyond that core, the shifted-prefundamental-generation and
 pro-Weyl-recovery packets are established on the type-$A$
 shifted/completed side; and
\item the only remaining post-core step is the compact-completion
 packet extending the evaluation-core comparison to the compact
 shifted-prefundamental core and comparing it with the desired
 completed / pro-nilpotent category.
\end{enumerate}
\end{theorem}

The all-types theorematic surface is narrower: Volume~I proves the
categorical Clebsch--Gordan package for all simple types and the DK
comparison on the evaluation-generated core. Only type~$A$ further
reduces the post-core packets to a single compact-completion gap;
beyond that core in general type, the later shifted/completed
mechanism remains conditional.
...
\textup{(Vol~I, Proposition~\ref*{V1-prop:prefundamental-clebsch-gordan}.)}
...
\begin{proof}
The character identity~\eqref{V1-eq:prefundamental-cg} is a
...
on both sides (Vol~I, Proposition~\ref*{V1-prop:prefundamental-clebsch-gordan}; the computation is verified numerically
...
\textup{(Vol~I, Corollary~\ref*{V1-cor:universal-character-containment}.)}
...
\textup{(Vol~I, Corollary~\ref*{V1-cor:k0-generation-OY}.)}
...
decomposition~\eqref{V1-eq:prefundamental-cg}. The projectors
...
\begin{proposition}[Type-$A$ categorical Clebsch--Gordan ingredient;
\ClaimStatusProvedElsewhere]
% label removed: prop:categorical-lift-type-a
\index{MC3!categorical lift!type A}
\textup{(Vol~I, the categorical-CG part of the type-$A$ reduction.)}
For $\fg=\mathfrak{sl}_N$ and generic~$k$, the categorical
Clebsch--Gordan decomposition for the relevant
evaluation/prefundamental tensor products lifts from the
character level to an actual splitting in the derived category.
This is the categorical-CG part of the type-$A$ story. It does not,
by itself, identify the full shifted/completed category; the later
shifted-prefundamental, pro-Weyl, and compact-completion statements
belong to the separate post-core packets. The proof uses:
\begin{enumerate}[label=(\roman*)]
...
\item \emph{Efimov completion} (\cite{Efi20}):
 ...
 descent.
\item \emph{Post-core shifted/completed analysis}:
 the later shifted-prefundamental and pro-Weyl arguments are
 handled separately in the type-$A$ reduction theorem, rather than
 following formally from a generic finite-dimensional extension
 theorem.
\end{enumerate}
\end{proposition}
...
\begin{theorem}[All-types MC3 on the evaluation-generated core;
\ClaimStatusProvedElsewhere]
...
plus the all-types evaluation-core DK comparison.)}
For any simple Lie algebra $\fg$ and generic level~$k$:
\begin{enumerate}[label=(\roman*)]
\item The multiplicity-free $\ell$-weight property
 (Chari--Moura) gives the categorical Clebsch--Gordan part of
 MC3 for all Dynkin types, replacing the minuscule hypothesis.
\item The DK comparison is proved on the evaluation-generated core.
\item Beyond that core, only type~$A$ currently has a theorematic
 reduction of the remaining packets to a single compact-completion
 gap; in other types the analogous shifted-prefundamental,
 pro-Weyl, and compact-completion mechanism is conditional on
 additional inputs.
\end{enumerate}
\end{theorem}

\begin{remark}[Multiplicity-free $\ell$-weights resolve only the categorical-CG obstruction]
...
property, which holds universally for all simple types. What this
resolves is the categorical-CG part of MC3 and the resulting
evaluation-generated-core comparison. It does not by itself close the
later shifted/completed post-core packets.
\end{remark}
...
\item \textbf{DK-0} (Laplace shadow):
 ...
 \textbf{Proved} (Vol~I; Proposition~\ref*{V1-thm:collision-residue-twisting}).
\item \textbf{DK-1} (KZ monodromy / quantum-group comparison):
 the monodromy of the KZ connection at level~$k$ is compared by
 the classical Drinfeld--Kohno theorem with the corresponding
 braided tensor-product representation of
...
\item \textbf{DK-4} (completed bar-cobar equivalence):
 ...
 on pro-categories.
 \textbf{Conjectural}, part of
 Conjecture~\ref{conj:dk-compacts-completion}
 (Vol~I, \texttt{yangians\_computations.tex}).
 The algebraic side is supplied by the MC4 strong
 completion-tower theorem
 (Vol~I, Theorem~\ref*{V1-thm:completed-bar-cobar-strong}),
 but DK-4 itself is the \emph{representation-theoretic}
 pro-category equivalence on the line-operator/quantum-group
 side; this is a stronger statement than the chiral-algebra
 completion theorem and is not discharged by it.
\item \textbf{DK-5} (full monoidal equivalence):
 ...
 \textbf{Frontier.} The evaluation-generated-core DK comparison is
 proved for all simple types. The first rank-$2$ degree-$2$ seed
 comparison closes for $\mathfrak{sl}_3$
 \textup{(}Theorem~\ref{thm:sl3-evaluation-ext-degree-2-seed}
 below\textup{)}: at resonance
 $a - b = h^\vee \hbar = 3\hbar$ with $\hbar = 1/(k + h^\vee)$ at
 $k = h^\vee = 3$, the evaluation-module Ext data is
 $(\mathrm{Ext}^0, \mathrm{Ext}^1, \mathrm{Ext}^2) = (1, 9, 1)$,
 matching the Cartan-weight-zero projection of the $17$-dimensional
 $Y^{\mathrm{dg}}(\mathfrak{sl}_3)$ degree-$2$ seed. The remaining
 type-$A$ post-core obstruction is the compact-completion packet
 isolated by the Volume~I reduction theorem; for other types the
 analogous post-core mechanism is conditional on the later
 shifted/completed inputs.
\end{enumerate}
\end{definition}

\begin{theorem}[$\mathrm{Ext}^*$ of $\mathfrak{sl}_3$ evaluation
modules at resonance: the first rank-$2$ DK-$4$ seed;
licensing $\alpha + \beta + \gamma + \delta + \varepsilon$;
\ClaimStatusProvedHere]
\label{thm:sl3-evaluation-ext-degree-2-seed}
\index{Drinfeld--Kohno!sl3 evaluation Ext}
\index{Yangian!dg-shifted!sl3 degree-2 seed}
Let $V_{\omega_1}(a), V_{\omega_2}(b)$ denote the evaluation modules of
$\widehat{\mathfrak{sl}}_3$ at the fundamental weights with spectral
parameters $a, b$, at level $k = h^\vee = 3$ and
$\hbar = 1/(k + h^\vee) = 1/6$. At the resonance locus
$a - b = h^\vee \hbar = 1/2$, the evaluation-module Ext is
\[
 \mathrm{Ext}^0(V_{\omega_1}(a), V_{\omega_2}(b)) = 1,
\qquad
 \mathrm{Ext}^1 = 9,
\qquad
 \mathrm{Ext}^2 = 1,
\]
with $\mathrm{Ext}^0$ the canonical contraction
$V_{\omega_1} \otimes V_{\omega_2} \to \C$,
$\mathrm{Ext}^1 = \dim \mathfrak{sl}_3 + 1 = 9$ (adjoint plus central),
$\mathrm{Ext}^2 = 1$ (the Massey-$1$ obstruction class). Generically
in $(a, b)$ all three Ext groups vanish. The total
$\mathrm{Ext}$-dimension $11$ is the Cartan-weight-zero projection of
the $17$-dimensional degree-$2$ seed
$F_2 Y^{\mathrm{dg}}(\mathfrak{sl}_3) = 8 + 8 + 1$
(level-$1$ adjoint, level-$2$ adjoint, level-$2$ central quantum
determinant); the missing $6$ are non-Cartan weight classes that
vanish in Ext between fixed-weight evaluation modules. The Casimir
eigenvalues on $\mathrm{Ext}^1$ partition as
$1/3$ (adjoint octet) and $-8/3$ (trivial singlet), consistent with
$\mathrm{tr}(\Omega) = 0$ and $\mathrm{tr}(\Omega^2) = \dim \mathfrak{sl}_3 = 8$.
\end{theorem}

\begin{proof}
Three independent routes:
\emph{Route A} (direct evaluation-module Ext): explicit BGG resolution
of $V_{\omega_1}(a)$ paired with the dual of $V_{\omega_2}(b)$. The
Casimir difference operator
$(\Omega_a - \Omega_b - (a - b))$ has kernel of dimension $1$ at
resonance and $0$ generically. Higher Ext via the Koszul resolution
of the Cartan diagonal: the rank-$2$ Cartan delivers
$\mathrm{Ext}^2 = 1$ (Massey-$1$).
\emph{Route B} (Kohno--Drinfeld KZ monodromy at $k = h^\vee = 3$):
the level $k + h^\vee = 6$ KZ connection on
$\mathbb{C} \setminus \{0, 1\}$ has trace of the level-$2$ spectral
residue $(1/2) \cdot 8 \cdot 3 = 12$ on the adjoint, consistent with
the dimension count.
\emph{Route C} (factorisation algebra of evaluations on
$\R^2 \setminus \mathrm{pts}$): the Costello--Witten--Yamazaki 4d
Chern--Simons translation identifies the Ext via the local-system
cohomology of the punctured plane with values in the adjoint
representation, recovering the resonance locus as the diagonal
on which the local system reduces.
All three routes agree on $(1, 9, 1)$; generic vanishing is the
Beilinson falsification probe — any non-resonance shift kills the
spectral kernel.
\end{proof}
...
DK-2 & MC1 (PBW/Koszul input) + DK-1 \\
DK-3 & MC3 (categorical CG) \\
DK-4 & MC4 (completion towers) \\
DK-5 & MC3 + MC4 + MC5 (all genera; analytic, $D^{\mathrm{co}}$, and strict completed/pro chain lanes proved; raw direct-sum class~M false)
\end{tabular}
\end{center}
...
The Laplace kernel for $\mathcal{W}_3$ has poles of orders
$1$ through $6$:
\begin{equation}
...
The transport-closure of hook vertices in the type~$A$ reduction graph $\Gamma_N$ equals the full partition set $\operatorname{Par}(N)$ for all $N \leq 7$ (Vol~I, Theorem~\ref{V1-thm:transport-closure-type-a}). Every non-hook partition is reachable from a hook vertex via the dominance-order Hasse diagram; the DS--Koszul intertwining propagates along these edges by the transport propagation lemma (Vol~I, Proposition~\ref{V1-prop:transport-propagation}).
\end{construction}
...
$r(z)=k\,\Omega_{\mathrm{tr}}/z$. The DS functor sends this to
...
Creutzig--Linshaw--Nakatsuka--Sato~\cite{CLNS25}.
...
\item \emph{Transport propagation}
 (Proposition~\ref{V1-prop:transport-propagation}):
...
\begin{remark}[Line operators and the modular Koszul datum]
...
projection of the modular Koszul datum $\Pi_X(L)$
(Vol~I, Construction~\ref*{V1-constr:platonic-package}):
the six-fold datum
...
module core on the affine/proved dg-shifted-Yangian lane,
...
corrections, curvature $\kappa$, shadow obstruction tower).

The genus-$0$ line-operator data is the tip of the
platonic iceberg. The full modular Koszul datum adds:
\begin{itemize}
...
\item The shadow obstruction tower
 ...
 on $\cA^!$ at each degree.
\item The modular tangent complex
 (Vol~I, Construction~\ref*{V1-const:vol1-modular-tangent-complex}),
...
\begin{tabular}{llll}
\textbf{Algebra} & \textbf{Companion / Koszul-dual lane}
...
$\mathcal{W}_3{}_c$
 ...
 & Characteristic proved; same-family algebra conditional \\[3pt]
$\mathcal{W}_N{}_c$
...
\textup{(}Vol~I, Definition~\textup{\ref*{V1-def:vol1-rigid-planted-forest-depth-filtration}}\textup{)}.
...
genus $g$ by $1$ and decreases the degree $r$ by $2$ (one
self-gluing):
...
two degree slots (the two endpoints of the sewn edge), giving
...
Construction~\ref*{V1-const:vol1-boundary-operators-residue}).
...
(Vol~I, \S\ref*{V1-sec:concordance-koszulness-programme})
...
\section{Synthesis}
\label{sec:lineop-ext-supplement}

\begin{remark}[Line-operator extensions at the $24$-punctured K3 base]
\label{rem:lineop-ext-1}
The line-operator extensions of this chapter reach their K3-canonical
realisation in the class-$\mathcal S$ $A_1$-on-$\Sigma_{0,24}$
theory: each of the $24$ Kodaira-I$_1$ punctures $p_a\in\Sigma_{0,24}$
carries a half-BPS line operator $L_a^{\mathrm{half\text{-}BPS}}$ with
OPE
\[
L_a^{\mathrm{half\text{-}BPS}}(z_1)\cdot L_b^{\mathrm{half\text{-}BPS}}(z_2) \;=\; \sum_{c=1}^{24}\, \mathcal N^c_{ab}(z_1-z_2)\, L_c^{\mathrm{half\text{-}BPS}}(z_2) \;+\; \text{regular},
\]
where the fusion coefficients $\mathcal N^c_{ab}$ realise the
Steiner system $S(5,8,24)$ as the structure constants of their fusion
algebra: $\mathcal N^c_{ab} = 1$ iff $\{a,b,c\}$ extends to a Golay
octad, otherwise $0$ in a distinguished gauge. This fusion algebra is
the $\hbar\to 0$ classical limit of the coproduct
$\Delta_{K3}:\mathbf H_{\Delta_5}\to\mathbf H_{\Delta_5}\otimes\mathbf H_{\Delta_5}$
restricted to the line-operator subalgebra. Primary-lit: Gaiotto--Moore--Neitzke
2013, Beem et al.\ 2015, Conway--Sloane 1988.
\end{remark}
