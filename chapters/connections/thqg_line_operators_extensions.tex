...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Module categories over the Koszul dual}
\label{sec:module-categories-koszul-dual}
...
\label{subsec:dg-category-amod}

\begin{definition}[$A_\infty$-modules over $\cA^!$]
\label{def:ainfty-module-koszul-dual}
...
\begin{equation}
\label{eq:ainfty-module-relations}
...
\label{prop:enriched-hom-d-squared}
...
\label{rem:enriched-hom-topology}
...
\ClaimStatusProvedHere]
\label{thm:dg-categorical-equivalence}
...
\label{rem:strict-vs-ainfty-modules}
...
\label{subsec:explicit-module-categories}

We extract the $A_\infty$-module structures for the three
principal families.

\subsubsection{Heisenberg: curved symmetric modules}
\label{subsubsec:heisenberg-modules}

For $\cA=\cH_k$ (Heisenberg at level $k$),
the Koszul dual is $\cA^!=\cH_{-k}$: the Heisenberg
algebra at level $-k$.  The bar complex
...
\ClaimStatusProvedHere]
\label{prop:heisenberg-modules-fock}
...
\begin{equation}
\label{eq:heisenberg-module-category}
...
\label{subsubsec:affine-sl2-modules}
...
\label{prop:affine-module-category}
...
\begin{equation}
\label{eq:affine-module-quantum-group}
\widehat{\mathfrak{sl}}_2{}_{-k-4}\text{-}\mathbf{mod}
\;\simeq\;
...
\label{comp:enriched-hom-affine}
...
\label{subsubsec:virasoro-modules}
...
\ClaimStatusProvedHere]
\label{prop:virasoro-module-category}
...
\label{subsec:tensor-product-r-matrix}
...
\label{def:spectral-tensor-product}
...
\begin{equation}
\label{eq:spectral-tensor-product}
...
\label{prop:tensor-product-module-relations}
...
\ClaimStatusProvedHere]
\label{comp:spectral-tensor-heisenberg}
...
\label{comp:spectral-tensor-affine}
...
\label{subsec:yangian-convolution}

\begin{definition}[dg-shifted Yangian from convolution]
\label{def:yangian-convolution}
...
(Vol~I, Definition~\ref*{def:modular-convolution-dg-lie}).
...
\begin{equation}
\label{eq:yangian-convolution-def}
\Ydg(\cA^!)
\;:=\;
...
\label{rem:yangian-strict-model}
...
\label{subsec:evaluation-homomorphism}
...
\label{prop:evaluation-modules-dense}
...
\label{subsec:gravitational-yangian}

\begin{construction}[Gravitational Yangian;
\ClaimStatusProvedHere]
\label{constr:gravitational-yangian}
...
\begin{equation}
\label{eq:gravitational-yangian-generators}
T(z)
\;=\;
...
with the $r$-matrix
\begin{equation}
\label{eq:virasoro-r-matrix-explicit}
r_c(z)
\;=\;
\frac{\partial T}{z}
+\frac{2T}{z^2}
...
\begin{equation}
\label{eq:gravitational-rtt-modes}
[T_{(m)},T_{(n)}]
\;=\;
(m-n)\,T_{(m+n)}
...
\begin{equation}
\label{eq:gravitational-coproduct}
\Delta_z(T_{(n)})
\;=\;
...
\label{rem:gravitational-yangian-not-standard}
...
and $q$-parameter}
\label{subsec:affine-quantum-group}

\begin{theorem}[$q$-parameter derivation;
\ClaimStatusProvedHere]
\label{thm:q-parameter-derivation}
...
\ClaimStatusProvedHere]
\label{comp:q-parameter-small-levels}
...
\label{subsec:mc3-statement}

\begin{definition}[MC3: categorical Clebsch--Gordan]
\label{def:mc3-problem}
...
\item MC3 (categorical CG): tensor products of
  $\cA^!$-modules decompose.  \textbf{Proved in type $A$};
  conjectural for general type.
...
\label{subsec:mc3-type-a}

\begin{theorem}[MC3 resolution in type $A$;
\ClaimStatusProvedElsewhere]
\label{thm:mc3-type-a-line-ops}
...
\label{subsubsec:chromatic-filtration}

\begin{definition}[Chromatic filtration on the Yangian]
\label{def:chromatic-filtration}
...
\ClaimStatusProvedHere]
\label{prop:chromatic-compatibility}
...
\label{subsubsec:prefundamental-cg}

\begin{proposition}[Prefundamental Clebsch--Gordan;
\ClaimStatusProvedElsewhere]
\label{prop:prefundamental-cg-line-ops}
...
\ClaimStatusProvedElsewhere]
\label{cor:universal-character-containment-line-ops}
...
\label{cor:k0-generation-line-ops}
...
\label{subsubsec:categorical-lift-obstruction}

\begin{definition}[Categorical CG projectors]
\label{def:categorical-cg-projectors}
...
\ClaimStatusProvedElsewhere]
\label{prop:categorical-lift-type-a}
...
\subsection{General type: the conjectural extension}
\label{subsec:mc3-general-type}

\begin{conjecture}[MC3 for arbitrary simple type;
\ClaimStatusConjectured]
\label{conj:mc3-arbitrary-type-line-ops}
\index{MC3!arbitrary type|textbf}
\textup{(Vol~I, Conjecture~\ref*{conj:mc3-arbitrary-type}.)}
For any simple Lie algebra $\fg$ and generic level~$k$:
\begin{enumerate}[label=(\roman*)]
\item The prefundamental representations $L^\pm_i$
  ($i=1,\ldots,\mathrm{rank}(\fg)$) satisfy CG formulas
  analogous to~\eqref{eq:prefundamental-cg} for all
  Dynkin types.
...
\label{rem:hernandez-jimbo-obstruction}
\index{Hernandez--Jimbo!prefundamental CG}
The key missing ingredient for
Conjecture~\ref{conj:mc3-arbitrary-type-line-ops} is
the prefundamental CG closure for general Dynkin type.
In type~$A$: the prefundamental representation $L^-$
is a Fock-type module whose character is a ratio of
theta functions, and the CG formula follows from
$q$-binomial identities
(Proposition~\ref{prop:prefundamental-cg-line-ops}).
For types $B$, $C$, $D$, $E$, $F$, $G$: the
Hern\'andez--Jimbo prefundamental representations
exist (\cite{HJ12}), but the explicit CG formulas
involve twisted theta functions and $q$-hypergeometric
identities that have not been verified at the
character level.

The obstruction is combinatorial, not structural: the
four-step argument of
Proposition~\ref{prop:categorical-lift-type-a} applies
verbatim once the character-level CG formulas are
established.
\end{remark}

\subsection{The DK ladder}
\label{subsec:dk-ladder}
...
\item \textbf{DK-5} (full monoidal equivalence):
  ...
  \textbf{Accessible in type $A$} (via MC3 resolution;
  conjectural for general type).
\end{enumerate}
\end{definition}

\begin{remark}[DK ladder and the MC hierarchy]
\label{rem:dk-mc-relationship}
...
\label{sec:w-algebra-line-operators}
...
\label{subsec:principal-wn-lines}

\begin{definition}[$\mathcal{W}_N$ line-operator category]
\label{def:wn-line-category}
...
\label{subsec:non-principal-w-lines}

\begin{definition}[Non-principal DS reduction]
\label{def:non-principal-ds}
...
\label{constr:vol2-transport-closure}
...
\ClaimStatusConjectured]
\label{conj:transport-to-transpose}
...
\begin{equation}
\label{eq:transport-to-transpose}
...
\label{subsec:ds-functor-lines}

\begin{theorem}[DS as a dg functor on line categories;
\ClaimStatusProvedHere]
\label{thm:ds-dg-functor}
...
\label{subsec:non-principal-obstruction}

\begin{proposition}[Non-principal bar-cobar obstruction;
\ClaimStatusProvedHere]
\label{prop:non-principal-obstruction}
...
\begin{equation}
\label{eq:non-principal-obstruction-diagram}
\begin{tikzcd}
...
is a class
\begin{equation}
\label{eq:non-principal-obstruction-class}
o_f
\;\in\;
H^1\bigl(
...
\label{rem:non-principal-frontier}
...
\label{rem:lines-platonic-package}
...
\item The shadow Postnikov tower
  $\Theta_\cA^{\le r}$ (Vol~I, Theorem~\ref*{thm:mc2-bar-intrinsic}),
...
\label{subsec:planted-forest-genus-filtration}
...
\label{rem:shadow-depth-module-complexity}
