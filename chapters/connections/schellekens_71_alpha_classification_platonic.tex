...
alone does not trivialize a finite-orbifold BV associator.
...
M\"oller--Scheithauer supply the $71$ VOA presentations. Original-complex
$E_3$ or HT descent requires further input. The stratum-by-stratum
criterion: Type~A is the abelian lattice case; Types~B and~C require a
trivialized Dijkgraaf-Witten class and finite-orbifold BV descent for the
chosen cyclic action. The $\mathbb{Z}/3$ Coxeter--Todd case is a
representative Type~C local datum; the general BV hypothesis remains
separate.

For a non-trivial cyclic presentation $G=\langle g\rangle$, the
finite-orbifold descent object is the homotopy fixed point
totalisation
\[
 V_\Lambda^{hG}
 =
 \Tot\!\left[
 V_\Lambda \rightrightarrows \Map(G,V_\Lambda)
 \;\begin{matrix}\to\\[-0.65ex]\to\\[-0.65ex]\to\end{matrix}\;
 \Map(G^2,V_\Lambda)
 \longrightarrow \cdots
 \right],
\]
with twisted sectors included by the cyclic-orbifold VOA
presentation.  The obstruction class lives in
$H^3(BG;\mathrm{U}(1))$ and must vanish for chain-level $E_3$ descent.
If $\alpha_{\mathrm{orb}}=d\beta$, the descent associator is
\[
 \Phi^\beta_{g,h,k}
 =
 \beta(g,h)\,\beta(gh,k)\,\beta(h,k)^{-1}\,
 \beta(g,hk)^{-1}\,\Phi_{g,h,k}.
\]
Level matching is a VOA presentation condition; it is not the
Dijkgraaf-Witten trivialization.
...
Schellekens 1993 enumerates the possible weight-one
...
$V_1 = 0$, represented by the Moonshine module
...
trivialized in the non-vacuous strata:
 ...
 $[\alpha_{\mathrm{orb}}]=0$ in
 $H^3(B\mathbb{Z}/n;\mathrm{U}(1))$, equivalently with a local
 trivializing $2$-cochain $\beta$ correcting the associator by the
 displayed formula.
 \end{itemize}
\item \textbf{Finite-orbifold BV descent.} The corresponding
finite-orbifold BV construction applies to the homotopy fixed point
totalisation of the abelian lattice HT model and its boundary
restriction identifies the descended boundary chiral algebra with $W$.
\end{enumerate}
...
Schellekens classification supplies the VOA presentations; the BV
anomaly trivializations and the HT descent are separate input.
\end{theorem}

The verification proceeds stratum by stratum.
Section~\ref{sec:schellekens-typeA-pure-niemeier} treats the vacuous
Type~A anomaly and abelian HT model;
Section~\ref{sec:schellekens-typeB-leech-z2} records the Type~B local
datum from Chapter~\ref*{ch:monster-chain-level-e3-top-platonic};
...
classification input; chain-level HT descent follows once
the anomaly trivialization, corrected associator, and BV-descent data
in the theorem are supplied. The
Monster shortcut $\Lambda^\sigma = 0$ is restricted to Type~B.
\end{remark}
...
\begin{remark}[Scope of the Type B mechanism]
\label{rem:schellekens-typeB-non-generalisable}
The identity $\Lambda^\sigma=0$ for $\sigma=-1$ is a torsion-free
lattice fact, not unique to the Leech lattice. The distinguishing
feature of Type~B is the Schellekens presentation: the Leech
$\mathbb Z/2$ orbifold gives the unique weight-one-zero entry
$V^\natural$. Type~A carries no orbifold anomaly; Type~C uses cyclic
automorphisms with generally non-trivial fixed lattices. The Type~B
argument applies only to its stratum; the $71$ entries require the
stratum-by-stratum data.
\end{remark}
...
The lattice calculation stops here.  The FLM cocycle
\(\epsilon\colon \Lambda\times\Lambda\to\{\pm1\}\) controls the
ordinary lattice VOA product, but its restriction to the
Coxeter--Todd fixed lattice is not, by itself, a computation of the
finite-orbifold BV associator in
\(H^3(B\mathbb Z/3;\mathrm U(1))\).  The missing datum is a normalized
local \(3\)-cocycle \(\alpha_{\mathrm{orb}}\) for the cyclic gauging,
or equivalently a \(2\)-cochain \(\beta\) with
\(\alpha_{\mathrm{orb}}=d\beta\), identifying the BV associator with
the zero Dijkgraaf--Witten class.
...
orbifold product is a VOA chain identity.  This proves the
cyclic-orbifold VOA presentation and the level-matching input.  It
does not prove the HT finite-gauging associator.  Chain-level
...
\begin{remark}[Type C local data]
...
representative of the local data: for every
...
(non-trivial eigenvalues of $g$ on $\Lambda \otimes \mathbb{R}$
come in complex-conjugate pairs contributing positive absolute
values).  These are compatibility checks, not a computation of
the Dijkgraaf--Witten class.  The Type~C entries differ in the lattice
$\Lambda^\sigma$ and the twisted-vacuum conformal weight
$h_{\mathrm{tw}}$. The listed entries are precisely the
level-matched cyclic orbifolds; HT descent adds the finite-orbifold BV
step, including a normalized local cocycle and its trivializing
cochain.
\end{remark}
...
the parent HT model; the finite cyclic quotient is the homotopy fixed
point totalisation and inherits the $E_3$ operations only after the BV
anomaly has been trivialized, the associator has been corrected, and
...
Type~A: no orbifold, anomaly class vacuous. The underlying lattice is
even unimodular and the VOA is a boundary of abelian holomorphic CS.

Type~B: $\Lambda^\sigma = 0$ is the lattice input for the Leech
$\Z/2$ calculation. HT descent uses the local sign
$\alpha_{\mathrm{orb}}(\sigma,\sigma,\sigma)=+1$ and finite-orbifold
BV descent.

Type~C: $\Lambda^\sigma \ne 0$; the $H^3$-cocycle restriction is a
genuine cohomology class. The cyclic-orbifold level-matching criterion
is the VOA condition; HT descent requires the corresponding local
BV trivialization.

The three mechanisms are disjoint: HT descent on the full list requires
the stratum-specific data, with the
$\Lambda^\sigma$-triviality argument restricted to Type~B and the
even-unimodular argument restricted to Type~A.
\end{remark}
...
requirement of strong rationality, holomorphy, and central charge exactly
$24$ forces $V_1$ into the classified finite list. Extension to
higher central charges $c \in 8\mathbb{Z}$ (the values at which even
unimodular lattices exist in positive definite rank:
$8, 16, 24, 32, 40, \ldots$) lies outside the stated criterion.
At $c = 8$ there is one lattice VOA ($V_{E_8}$, with
$V_1 = \mathfrak{e}_8$ at level~$1$); at $c = 16$, two
($V_{E_8 \oplus E_8}$ and $V_{D_{16}^+}$); at $c = 24$, the classified
$71$; at $c = 32$, the count is conjectural (millions of even
unimodular lattices of rank~$32$).
\end{remark}
...
framed-VOAs (Miyamoto 2004; Lam-Shimakura 2015) presents each entry
as an extension of the Virasoro tensor product
$\bigotimes L(c = 1/2, 0)^{\otimes 48}$. The framed-VOA presentation
gives a VOA route to anomaly analysis via the
$\mathbb{Z}/2$-simple-current extension theory of Dong-Li-Mason 2000;
it serves as a cross-check for the cyclic presentation and
simple-current phases. The finite-orbifold BV trivialization required
in Theorem~\ref{thm:schellekens-71-all-alpha-zero} is a separate
input.
\end{remark}
...
\item In Types~B and~C, the descent object is the homotopy fixed point
totalisation $V^{hG}$, and a trivialization
$\alpha_{\mathrm{orb}}=d\beta$ corrects the associator by
$\Phi^\beta_{g,h,k}=
\beta(g,h)\beta(gh,k)\beta(h,k)^{-1}\beta(g,hk)^{-1}\Phi_{g,h,k}$.
\item The $\mathbb{Z}/3$ Coxeter--Todd calculation appears in
Proposition~\ref{prop:z3-coxeter-todd-alpha-zero-explicit}: it gives
the fixed-lattice, determinant-sign, and level-matching checks, while
the local BV cocycle and trivializing cochain remain supplied data.
Every other Type~C case has the same formal shape with different
...
\item The three mechanisms are distinct; the
$\Lambda^\sigma = 0$ shortcut of the Monster covers Type~B alone.
