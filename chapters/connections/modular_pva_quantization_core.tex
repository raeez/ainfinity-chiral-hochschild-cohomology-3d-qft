...
This axiom is a consequence of $\partial^2 = 0$ on the moduli space
of stable curves: the boundary of the boundary of
$\overline{\cM}_{g,n}$ is empty, so the composition of two boundary
operations vanishes.  The axiom records this topological fact at the
algebraic level.
\end{remark}
...
The $E_2$ quasi-modularity (Arnold defect) obstructs the holomorphic bar differential
at genus~$1$: the propagator coefficient $E_2(\tau)$ is quasi-modular with anomaly
$12\tau/(2\pi i)$.  Resolution requires the non-holomorphic completion
$\widehat{E}_2 = E_2 - 3/(\pi\,\mathrm{Im}\,\tau)$, which restores true modular
covariance at the cost of holomorphicity.

The upshot is a clean dichotomy: \emph{trees govern chirality; loops govern
modularity}.  The genus-$0$ Maurer--Cartan element governs the chiral algebra
structure; the genus-$1$ lift governs the modular structure; and the lift is unique
(up to equivariant gauge) once the obstruction class
...
\end{enumerate}
The shadow depth $d_{\mathrm{alg}}(\cA)$ is the minimal $k$ such that $m_{k+1}, m_{k+2}, \ldots$ are all determined by $m_2, \ldots, m_k$ via the Stasheff relations.
\end{theorem}

\begin{proof}
The $A_\infty$ relations at arity $n$ express $m_n$ in terms of compositions of $m_i$ for $i < n$, modulo a $d_0$-coboundary (the $m_1 \circ m_n + m_n \circ m_1^{\otimes n}$ terms).  When this coboundary is exact, i.e., when the obstruction class in $H^\bullet(d_0)$ vanishes, the operation $m_n$ is determined by lower-arity data.  The shadow depth is the arity beyond which all obstructions vanish, equivalently the arity beyond which the bar coderivation is determined by its lower components.
\end{proof}
...
\begin{warning}[The $S$-transform is not the Wick rotation]%
\label{warn:s-transform-not-wick}%
\index{S-transform@$S$-transform!not Wick rotation}%
\index{Wick rotation!vs $S$-transform}%
The modular $S$-transform $\tau \mapsto -1/\tau$ exchanges
the $A$-cycle and $B$-cycle of $E_\tau$: it acts on the
\emph{closed colour} (the complex structure of the worldsheet).
The Wick rotation of the topological direction $\R_t$ acts on
the \emph{open colour} (the $E_1$ ordering).  These are
operadically distinct: they are sourced by the same
$B$-cycle quasi-periodicity, but the $S$-transform couples
to~$\kappa(\cA)$ (highest-pole coefficient) while the
braiding monodromy couples to $c_0 = \{a_{(0)}b\}$
(lowest-pole coefficient).  They act on different factors of
$\FM_k(\C) \times \Conf_k^{<}(\R)$, though the two colours
can entangle at genus~$\ge 1$ when $c_0 \ne 0$
\textup{(}Theorem~\textup{\ref{thm:elliptic-spectral-dichotomy})}.
The tempting identification of $S$ with ``Wick rotation of
the modular parameter'' is a $2$d worldsheet perspective
(where exchanging the two cycles of $E_\tau$ looks like
exchanging space and time); it does not survive the lift to
the $3$d holomorphic-topological framework, where the two
operations act on different Swiss-cheese colours.
\end{warning}
...
The propagator expansion $h(u;\tau)
= u^{-1} + \sum_{k \ge 1} c_k(\tau)\,u^{2k-1}$
has only odd powers of~$u$ (by the oddness of the
Jacobi $\theta_1$-function).
An OPE pole of even order~$2k$ therefore pairs with
$c_k(\tau) \propto E_{2k}(\tau)$
in the genus-$1$ fibre differential:
\[
  a_{(2k-1)} b \;\mapsto\;
  c_k(\tau)\,a_{(2k-1)} b\,.
\]
Odd-order OPE poles pair with even powers of~$u$, which
vanish.  The genus-$1$ corrections are thus controlled
entirely by the even-order poles:
the double pole contributes an $E_2(\tau)$-weighted
correction (quasi-modular --- the Arakelov defect);
the quartic pole contributes $E_4(\tau)$ (truly
modular); the sextic pole contributes $E_6(\tau)$;
and so on.  For
Heisenberg (max pole $= 2$): only~$E_2$.
For Virasoro (max pole $= 4$): $E_2$ and~$E_4$.
For $\cW_3$ (max pole $= 6$): $E_2$, $E_4$,
and~$E_6$.  The truncation by pole order is the
genus-$1$ shadow of the pole-order classification.
\end{observation}
...
that a $1/N$ expansion exists without computing any coefficient.  We now
make it quantitative by evaluating the modular characteristic
...
(Vol~I, eq.~\eqref*{eq:modular-char-gf};
...
\begin{theorem}[Structure of the scalar-lane $1/N$ expansion;
\ClaimStatusProvedHere]
\label{thm:scalar-lane-1overN-structure}
\index{t Hooft expansion@'t~Hooft expansion!scalar lane}
The scalar-lane free energy
