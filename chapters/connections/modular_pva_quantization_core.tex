...
(Vol~I, Construction~\ref*{const:modular-homotopy-deformation-object}).
\end{convention}
...
Definition~\textup{\ref{def:primitive-log-modular-kernel}} in
Volume~I.
\end{definition}
...
Theorem~\ref{thm:primitive-to-global-reconstruction} in Volume~I.
\end{proof}
...
$\Delta_{\mathrm{ns}}$ of Volume~I
(Proposition~\ref{prop:primitive-shell-equations}): both denote the
...
\begin{proof}
Coordinate specialization of Proposition~\ref{prop:branch-master-equation}
in Volume~I.
\end{proof}
...
\begin{proof}
Coordinate specialization of
Corollary~\ref{cor:metaplectic-square-root} in Volume~I.
\end{proof}
...
\item The full expansion is the Feynman transform of the modular operad
  ...
  $\bar B^{\mathrm{full}}(\cA) \simeq \FT_{\cM\!od}(\cA)$
  \textup{(}Theorem~\textup{\ref{thm:prism-higher-genus}}
  of Vol~I\textup{)}.
\end{enumerate}
\end{theorem}
...
specialise the genus decomposition.  Part~(iii) is
Theorem~\ref{thm:prism-higher-genus} of Vol~I.
\end{proof}
...
gauge theory with gauge group $\GL_N$:
\begin{itemize}
...
\begin{conjecture}[Holographic genus identification for $\GL_N$ Chern--Simons]%
...
quantization of the $\GL_N$ Chern--Simons theory on
...
\begin{remark}[Why $\GL_N$ and not $\SL_N$]
The $\GL_N$ theory has $U(1)$ center that decouples in the
...
For $\SL_N$, the center is $\bZ/N$ and the counting is more
subtle.
\end{remark}
