...
${\Barch}^{\mathrm{ord}}(\cH_k)\simeq\bigoplus_{n\ge 1}
...
deconcatenation make~${\Barch}^{\mathrm{ord}}(\cH_k)$
...
This chapter is organized as follows.  In \S\ref{sec:setup} we isolate the exact regime in which the
...
associativity alone at arities~$3$--$5$), the Euler-eta identity for
...
All five items of the original bordered conjecture
(Theorem~\ref{conj:bordered}) are resolved:
...
decomposition at arities $3$--$5$ (Proposition~\ref{prop:ordered-aos}).
...
${\Barch}^{\mathrm{ord}}_n(\cA)$ for $n=3,4,5$ follows from
...
At each arity~$n$, the terms in $d^2$ decompose into two
classes:
\begin{enumerate}[label=\textup{(\roman*)}]
...
Explicit verification at low arities:

\emph{Arity~$3$}: Two faces $D_{1,2}$ and $D_{2,3}$.
...
\emph{Arity~$4$}: Three faces $D_{1,2}$, $D_{2,3}$,
...
\emph{Arity~$5$}: Four faces $D_{1,2},\ldots,D_{4,5}$.
...
\begin{proof}
By induction on arity, using the two cancellation mechanisms.

At arity~$n$, the operator $d^2$ on
$[a_1|\cdots|a_n]\in{\Barch}^{\mathrm{ord}}_n$ is
\[
...
consecutive ones).  At arity~$3$, $d^2[e|f|h]$ acquires
...
(the residue of the double pole).  The weight-depth identity $w + d = n - 1$ (Theorem~\ref{thm:gap-migration}(i)) is trivially satisfied: $w = 0$, $d = 1$, $n = 2$.
Bar cohomology:
...
The ordered bar complex ${\Barch}^{\mathrm{ord}}(\cA)_n$
...
$\Delta_n := \dim{\Barch}^{\mathrm{ord}}_n
...
\widehat{\mathfrak{sl}}_2
  & 3 & 27 & 10 & 17
  & \text{braid representation } B_3\to\GL(V^{\otimes 3}) \\
\widehat{\mathfrak{sl}}_2
  & 4 & 81 & 15 & 66
...
$\dim{\Barch}^{\mathrm{ord}}_n = d^n = 3^n$
...
$\dim{\Barch}^{\mathrm{ord}}_n = 1 = \dim\Barch_n$
...
$d_{\mathrm{res}}$ on
${\Barch}^{\mathrm{ord}}_n(\mathrm{Vir})$
...
additionally satisfies \emph{strong unitarity}
$R_{12}(z)\,R_{21}(-z)=\id$.
...
$\Omega^{\mathrm{ord}}({\Barch}^{\mathrm{ord}}(\cA))$,
...
The $\SCchtop$-bar complex $B^{\mathrm{SC}}(\cA)$
carries two colour projections, each with its own
duality operation:
\begin{itemize}[nosep]
...
\item \emph{Open colour.}
  ...
  This is the $E_1$-chiral direction: the open-colour
  projection of $\SCchtop$ self-duality
  (Theorem~\ref{thm:SC-self-duality}) preserves the
  $E_1$-algebra structure on the dual.
\end{itemize}
The $R$-matrix is the cross-colour datum:
$\Barch(\cA)_n \simeq
({\Barch}^{\mathrm{ord}}(\cA)_n)^{R\text{-}\Sigma_n}$
...
Theorem~\ref{conj:FG-shadow} gives the controlled
...
\caption{Two-colour Koszul duals for the standard families.
  ...
  ${\Barch}^{\mathrm{ord}}\to\Barch$; the class column
...
$\Barch^{\mathrm{ord}}(Y_\hbar(\fg))$ recovers the
...
\[
  \Barch^{\mathrm{ord}}\bigl(Y_\hbar(\fg)\bigr)
  \;\xrightarrow{\;\sim\;}
  U(\fg[t]).
\]
...
For the full recovery: the closed-colour projection of the
$\SCchtop$-bar complex retains the symmetric bar
$\Barch(\cA)_n\simeq
(\Barch^{\mathrm{ord}}(\cA)_n)^{R\text{-}\Sigma_n}$
...
The $R$-matrix is $R(z)=1+\Omega/z+O(z^{-2})$ with
$\Omega=e\otimes f+f\otimes e+\tfrac12 h\otimes h$
...
Let $\cA$ be a strongly admissible $E_1$-chiral algebra with
ordered bar complex ${\Barch}^{\mathrm{ord}}(\cA)$.  Define
the \emph{graded bar Euler characteristic} by
\[
\chi({\Barch}^{\mathrm{ord}};\,q)
\;:=\;
\sum_{n\ge 1}(-1)^n\dim {\Barch}^{\mathrm{ord}}_{n,w}\;q^w,
\]
where ${\Barch}^{\mathrm{ord}}_{n,w}$ is the weight-$w$,
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item $H^0({\Barch}^{\mathrm{ord}}(\cA))=\Bbbk$ (the
  coaugmentation), $H^1({\Barch}^{\mathrm{ord}}(\cA))=V$
  (the strong generating space of~$\cA$), and
  $H^n({\Barch}^{\mathrm{ord}}(\cA))=0$ for all $n\ge 2$.
...
\item The \emph{depth spectral sequence}
  ...
  zeroth products) converges to $H^*({\Barch}^{\mathrm{ord}})$.
...
Statement~(iv): filter ${\Barch}^{\mathrm{ord}}$ by
...
The $E_4$ collapse for $\cW_N$ is controlled by the weight-depth identity $w + d = n - 1$ and the structural gap at $d = n$ (Theorem~\ref{thm:gap-migration}(i)--(ii)): the depth-$n$ differential vanishes because no weight-$1$ generator exists.
\end{computation}
...
The ordered bar complex ${\Barch}^{\mathrm{ord}}(\cH_k)$ has
a single weight-$1$ generator $\alpha$.  The space
${\Barch}^{\mathrm{ord}}_{n,w}$ consists of bar words
$[\alpha^{(m_1)}|\cdots|\alpha^{(m_n)}]$ with
$\sum m_i=w$ and each $m_i\ge 1$: thus
$\dim{\Barch}^{\mathrm{ord}}_{n,w}=\binom{w-1}{n-1}$
...
\sum_{n,w\ge 1}
\dim{\Barch}^{\mathrm{ord}}_{n,w}\;t^n q^w
\;=\;
\frac{tq}{1-tq/(1-q)}
\;=\;
\frac{tq(1-q)}{1-q-tq}.
...
\end{array}
\]
This table gives $\dim{\Barch}^{\mathrm{ord}}_{1,w}=d$
...
not at $u=v$) but never produces a double pole.  By AP19,
the $d\!\log$ kernel absorbs one pole power, so the collision
...
\end{enumerate}
Together these give
$\Barch^{\mathrm{ord}}(Y_\hbar(\mathfrak{sl}_N))
...
\S\ref{sec:setup}.  We now compute its ordered bar complex,
...
has collision residue $r^{\mathrm{coll}}(z) =
\Omega/z$ (a simple pole with the Casimir).
...
double bar
$\Barch^{\mathrm{ord}}(Y_\hbar(\mathfrak{sl}_2))
...
\;=\;
H^*\!\bigl(\Barch^{\mathrm{ord}}(Y_\hbar(\mathfrak{sl}_2))
\bigr)^\vee
\;=\;
...
the poles are all simple, so the $d\!\log$ kernel (AP19)
...
on $\Barch^{\mathrm{ord}}_2(Y_\hbar)$ reproduces exactly
...
$\Barch^{\mathrm{ord}}_3(Y_\hbar)$ has elements
...
\begin{array}{c|l|l}
\text{Degree}
  & \Barch^{\mathrm{ord}}_n(Y_\hbar(\mathfrak{sl}_2))
  & \text{Identification} \\
\hline
...
The $\Ainf$ structure of the open-colour Koszul dual
sharply separates gauge and gravitational theories:
\[
\begin{array}{c|cc}
...
\end{array}
\]
DS reduction transports class~$\mathbf{L}$ to class~$\mathbf{M}$:
it preserves Koszulness but destroys formality.  The resolvent
tree formula
$m_k^{\cW} = \sum_{\mathrm{Catalan}}(m_2^{\mathrm{aff}}
\circ h_{\mathrm{DS}})$
generates the infinite tower of higher products from the
single quadratic product of the affine input.
