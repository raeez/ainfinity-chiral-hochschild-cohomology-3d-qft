...
the modular characteristic $\kappaChHodge(\cA)$ and the shadow obstruction tower.
In particular, $\kappaChHodge(\cA)$ is the degree-$2$ image of $\mathrm{av}$
...
The open-color projection of $\Theta^{\mathrm{oc}}$ is the
$E_1$-chiral Koszul duality: the ordered bar coalgebra $\Barch(A)$
with its deconcatenation coproduct, the diagonal bicomodule
$C_\Delta$, and the duality involution sending $A$ to~$A^!$. The
commutative shadow of chiral Koszul duality is classical. The
associative story, the open-color projection of the
$\SCchtop$-algebra structure on the Koszul dual, is distinct. It is
not the Francis--Gaitsgory equivalence between chiral Lie algebras
and cocommutative factorization coalgebras. It is a theory in which
ordered configuration spaces survive, shuffles do not collapse under
symmetric-group quotients, and linear duality returns chiral
algebras on the far side (the duality involution of
Theorem~\ref{thm:duality-involution}).

The simplest case. The Heisenberg algebra~$\cH_k$
...
$d(s^{-1}\alpha\otimes s^{-1}\alpha)=k\,s^{-1}(1)$:
the unique ordered degree-$2$ face has sign \(+1\), and the scalar
\(k\) comes from the double-pole OPE
$\alpha_{(1)}\alpha=k$. The ordering is load-bearing:
the collision residue $r_{\cH}(z)=k\Omega_{\cH}/z$ (one pole absorbed
by the $d\log$ kernel, per the bar construction) changes sign under
...
A single question fixes the associative picture.

\medskip

\emph{What is the coalgebra-side avatar of the diagonal bimodule ${}_A A_A$?}

\medskip

For $C=\Barch(A)$ the ordered bar coalgebra of an associative chiral
algebra $A$, the boundary object is the \emph{diagonal bicomodule}
\[
C_\Delta,
\]
...
The chapter results are summarised by the dictionary
\[
...
Three classes of statements appear.

\begin{enumerate}[label=(\roman*)]
\item \textbf{Established inputs.}
The associative chiral bar--cobar equivalence, the one-sided
module/comodule Koszul duality, and the enveloping algebra formalism
are taken as established (Hypothesis~\ref{hyp:inputs}).

\item \textbf{Theorems proved here.}
...
Fix a field $k$ of characteristic $0$. Let $X$ be a smooth curve over
$k$, and let $\mathcal D(X)$ denote the relevant derived category of
$D$-modules on $X$ (or on the Ran space of $X$) endowed with the
chiral tensor product
\[
...
\begin{remark}
This is the regime in which every homological-algebra step is
licensed: totalisation, completion, normalisation, and passage to
coderived categories are all available. The arguments below require
no input beyond these hypotheses.
\end{remark}

\begin{hypothesis}[Established inputs]
\label{hyp:inputs}
The following are taken as established.
...
In the associative chiral regime the basic bar object is ordered.
The theory diverges from the commutative shadow at the first step:
every shuffle survives as a distinct summand.
...
\begin{remark}
The theorem is associative. In a commutative or $E_\infty$ setting
the shuffle summands collapse under the symmetric-group quotient;
here they remain distinct. The ordered configuration space retains
their full combinatorics.
\end{remark}

\section{Opposite-duality, anti-involutions, and enveloping duality}
\label{sec:opposite}

Order reversal transforms opposite multiplication on the algebra
side into opposite deconcatenation on the bar side.
...
The coalgebra-side avatar of an $A$-bimodule admits two
constructions: differentiating the ordered bar construction at a
square-zero extension; twisting a two-sided tensor product with the
universal twisting cochain. The two constructions agree.
...
One-sided module/comodule duality holds by Hypothesis~\ref{hyp:inputs}.
Passing from $A$ to its enveloping algebra $A^e$ and using
Corollary~\ref{cor:enveloping} bootstraps to the two-sided theory.
...
\begin{remark}
Corollary~\ref{cor:tensor-cotensor} is the coalgebraic gluing law
for intervals. The diagonal bicomodule $C_\Delta$ is the universal
boundary condition for interval composition.
\end{remark}

\section{Hochschild--coHochschild duality}
\label{sec:HH-coHH}

Annular traces and centers on the algebra side identify with
coHochschild theories of the bar coalgebra.
...
\begin{remark}
The cohomology theorem is the center theorem; the homology theorem
is the annulus theorem. The algebraic center and algebraic trace of
$A$ are read directly from the coalgebraic diagonal $C_\Delta$.
\end{remark}

\section{The diagonal bicomodule as universal boundary condition}
\label{sec:open-trace}

Two operations live on the boundary side. Interval composition is
derived cotensor (Corollary~\ref{cor:tensor-cotensor}). The ordered
pair-of-pants operation is the transport of the external tensor
product of bimodules; it is inherently ordered, and symmetric only
after extra structure.
...
\begin{remark}
The ordering is essential. Without additional symmetry data, $\star$
is a monoidal product reflecting the ordering of incoming open
boundaries: the algebraic shadow of an ordered pair-of-pants, not
the fully symmetric open cobordism category.
\end{remark}
...
\begin{remark}[Scope of the ordered open package]
The theorem supplies the algebraic ordered open package. The lift
to a fully symmetric one-brane open TCFT requires extra structure
that symmetrizes ordered fusion: a boundary symmetry datum or a
direct geometric realization by bordered configuration spaces.
...
The master theorem describes the algebra side of associative chiral
Koszul duality: the diagonal bicomodule $C_\Delta$, its composition
algebra, and the Hochschild--coHochschild dictionary. The
constructions of Appendix~\ref{sec:ordered-bar-explicit} supply the
spectral side: the $R$-matrix as descent datum
...
The unifying principle is the Chriss--Ginzburg paradigm: every
algebraic structure is a Maurer--Cartan element in a convolution
dg~Lie algebra. The $R$-matrix $R(z)$ is the MC element in the
Gerstenhaber complex $C^\bullet_{\mathrm{GS}}(\cA,\cA)$ governing
$\Sigma_n$-equivariant descent (Remark~\ref{rem:r-matrix-mc}); the
Yang--Baxter equation is the MC equation; the classical $r$-matrix
$r_0$ is its linearisation (Corollary~\ref{cor:classical-ybe}); the
Yangian $Y_\hbar(\mathfrak g)$ is the algebra of symmetries of this
MC datum. The passage from the master theorem's algebraic core to
the spectral data of ordered configuration spaces is the passage
from the abstract MC moduli to its explicit coordinatisation.
\end{remark}
...
Volume~I establishes five main theorems for $E_\infty$-chiral
algebras: bar-cobar adjunction, Koszul inversion, complementarity,
leading coefficient, and the Hochschild ring. Each has an
$E_1$-chiral counterpart, obtained by replacing symmetric
coinvariants with the ordered bar complex and its braid-group
equivariance. The five $E_1$ theorems at genus zero are consequences
of the master theorem (Theorem~\ref{thm:master}) together with the
constructions of Appendix~\ref{sec:ordered-bar-explicit}; they are
collected here to mirror Vol~I.
...
Under averaging, $\operatorname{av}(r(z))$ is the degree-$2$
scalar shadow entering $\kappaChHodge(\cA)$. For abelian branches this
is $\kappaChHodge(\cA)$ itself; for non-abelian affine
Kac--Moody $V_k(\fg)$ the trace-form average is
$k\dim(\fg)/(2h^\vee)$ and the Sugawara normal-ordering shift gives
$\kappaChHodge(V_k(\fg))=\operatorname{av}(r(z))+\dim(\fg)/2$.
\end{theorem}
...
\begin{remark}[Drinfeld double obstruction criterion]
\label{rem:drinfeld-double-programme}
The boundary algebras $\cA$ (Dirichlet) and $\cA^!$ (Neumann) are
linked by the Verdier coevaluation kernel
$\tau_{\mathrm{Verd}}\in \cA^! \widehat\otimes \cA$ induced from the
universal twisting morphism $\barB(\cA)\to\cA$, and by the Verdier
intertwining
$\mathbb{D}_{\Ran}(\barB(\cA)) \simeq \barB(\cA^!)$ (Theorem~A of
Volume~I). The bar-level Drinfeld double
$U=\cA\bowtie\cA^!$ is the Hopf object obtained from four data:
boundary-operator composition, deconcatenation on
$B^{E_1,\mathrm{ord}}_{\mathrm{ch}}(\cA)$, the complementarity Hopf
pairing, and the opposite-duality map of Theorem~\ref{thm:opposite}.
These data assemble if and only if the bialgebra compatibility defect
\[
 \mathfrak{o}_{\mathrm{DD}}(\cA)
 \in
 H^3\!\left(
   \Def_{\mathrm{Hopf}}\bigl(\cA,\cA^!,\tau_{\mathrm{Verd}},
   B^{E_1,\mathrm{ord}}_{\mathrm{ch}}(\cA)\bigr)
 \right)
\]
vanishes. When it vanishes, the resulting Hopf object satisfies
$\cC_{\mathrm{line}}\simeq U\text{-mod}$ and gives the bar-cobar form
of Dimofte's geometric Tannakian reconstruction \cite{Dimofte25}.
\end{remark}
...
At $K_\cA(z) = 1$, equation~\eqref{eq:dimofte-k-matrix} reduces to
$z^0 \cdot 1 \cdot 1 = 1$: all non-zero modes $\alpha_n$ with
$n\neq 0$ act trivially and $\alpha_0 = 0$. The twisted coproduct then
returns to the primitive coproduct
$\Delta(a)=a\otimes 1+1\otimes a$ on boundary generators, exactly the
class~$\mathbf G$ case. Thus the Volume~I shadow-depth partition
$\{\mathbf{G},\mathbf{L},\mathbf{C},\mathbf{M}\}$ with
$d_{\mathrm{alg}}\in\{0,1,2,\infty\}$ is the same partition as
$K$-matrix complexity.
\begin{center}
...
At the $E_1$-chiral level, the ordered bar coalgebra
$B^{\mathrm{ord}}(\cA)=T^c(s^{-1}\bar\cA)$ carries the $R$-matrix
$r(z)$ in degree two as the binary collision residue of the local OPE
on $\bar\cA$. The higher collision tower defines a braided Hopf object
once the meromorphic line-operator category
$\cC_{\mathrm{line}}^{\mathrm{mer}}$ is equipped with a spectral
braiding whose pole part is $r(z)$. The obstruction to this lift is the
Yang--Baxter defect
\[
 \mathfrak{o}_{\mathrm{mer}}(\cA)
 =
 \operatorname{Alt}_{123}
 \bigl([r_{12}(z_{12}),r_{13}(z_{13})]
      +[r_{12}(z_{12}),r_{23}(z_{23})]
      +[r_{13}(z_{13}),r_{23}(z_{23})]\bigr)
\]
viewed in the pole-controlled third collision complex. When
\(\mathfrak{o}_{\mathrm{mer}}(\cA)=0\), Majid double bosonization gives
the HT universal algebra
\[
 R\bigl(B^{\mathrm{ord}}(\cA)\bigr),
\]
a quasi-triangular Hopf algebra whose module category is a rigidified
enhancement of \(\cC_{\mathrm{line}}\). This is the meromorphic
refinement of the Drinfeld-double obstruction criterion in
Remark~\ref{rem:drinfeld-double-programme}; its Calabi--Yau analogue is
the BPS-algebra double-bosonization construction of Volume~III.
\end{remark}

\section{Bordered configuration-space input}
\label{sec:integration}

The bordered configuration-space construction of Volume~I,
\S\ref*{V1-sec:bordered-fm}, supplies the compactified geometric input
used above.

\begin{theorem}[Bordered diagonal bicomodule chain model;
\ClaimStatusProvedHere]
\label{thm:bordered-diagonal-bicomodule-chain-model}
Let \(A\) be a strongly admissible augmented \(E_1\)-chiral algebra.
Licensing data: \(\alpha+\beta+\gamma\).  Here \(\alpha\) is the
choice of upper-half-plane boundary chart and boundary ordering,
\(\beta\) is the comparison with the bordered FM construction of
Volume~I, and \(\gamma\) is the logarithmic-chain / completed
configuration-space ambient.
For \(p,q\geq 0\), let
\(\overline{\mathrm{Conf}}_{p,q}(\mathbb H)\) be the real oriented
blowup compactification of configurations of \(p\) labelled interior
points \(z_1,\ldots,z_p\in\mathbb H\) and \(q\) ordered boundary
points \(x_1<\cdots <x_q\in\partial\mathbb H=\mathbb R\). Its
boundary strata are indexed by planted forests
\[
F=(V_{\mathrm{int}},V_{\partial},E;\rho_{\mathrm{bulk}},\rho_{\partial}),
\]
where vertices over \(\rho_{\mathrm{bulk}}\) record interior
collisions and vertices over \(\rho_{\partial}\) record boundary
collisions. The codimension-one divisors are exactly
\[
D_{ij}^{\mathrm{int}}:\ z_i\to z_j,\qquad
D_{ab}^{\partial}:\ x_a\to x_b,\qquad
D_{i|a}^{\mathrm{mix}}:\ \Im z_i\to0,\ \Re z_i\to x_a.
\]

The diagonal boundary condition is the \(A\)-bimodule
\[
\Delta_A={}_AA_A\in A\text{-}\mathrm{Bimod}\text{-}A,
\]
with left action obtained from approach along the negative boundary
direction and right action from approach along the positive boundary
direction. A logarithmic chain model for this diagonal bicomodule is
\begin{equation}\label{eq:bordered-diagonal-chain}
D_A^{p,q}
:=
C_\bullet^{\log}\!\bigl(\overline{\mathrm{Conf}}_{p,q}(\mathbb H)\bigr)
\otimes A^{\otimes p}\otimes A^{\otimes q}.
\end{equation}
The residue part of its differential decomposes as
\begin{equation}\label{eq:bordered-diagonal-differential}
...
d_{\mathrm{int}}
=
\sum_{S\subset\{1,\ldots,p\}}
\operatorname{Res}_{D_S^{\mathrm{int}}}\otimes m_S,\qquad
d_{\partial}
=
\sum_{I\subset\{1,\ldots,q\}}
\operatorname{Res}_{D_I^{\partial}}\otimes\mu_I,
\]
and
\[
d_{\mathrm{mix}}
=
\sum_{S,I}
\operatorname{Res}_{D_{S,I}^{\mathrm{mix}}}\otimes\nu_{S,I}.
\]
Here \(m_S\) is the interior chiral collision operation, \(\mu_I\) is
the ordered boundary multiplication, and \(\nu_{S,I}\) is the
bulk-to-boundary operation obtained by bringing the interior fields in
\(S\) to the boundary cluster \(I\). Hence \(D_A^{p,q}\) is the
geometric model for the statement that the diagonal is the universal
boundary condition.

The annular closure replaces the boundary interval by \(S^1\):
\begin{equation}\label{eq:bordered-annular-closure}
D_A^{\mathrm{ann}}
=
\bigoplus_{n\geq0}
C_\bullet^{\log}\!\bigl(
\overline{\mathrm{Conf}}_n(S^1\times\mathbb R_{\geq0})
\bigr)\otimes_{\mathbb Z/n}A^{\otimes n}.
\end{equation}
The cyclic coinvariants are taken with the desuspended Koszul action
\begin{equation}\label{eq:annular-cyclic-sign}
...
\bar a_i=|a_i|-1.
\end{equation}
The annular monodromy is the first bordered stratum where the
genus-one \(R\)-matrix and the trace pairing appear.
\end{theorem}

\begin{proof}
The compactification and its planted-forest stratification are the
bordered Fulton--MacPherson construction of Volume~I,
\S\ref*{V1-sec:bordered-fm}. Taking the boundary of the logarithmic
chain \([\overline{\mathrm{Conf}}_{p,q}(\mathbb H)]\) gives precisely
the three codimension-one degenerations above: interior collisions,
boundary collisions, and mixed interior-to-boundary collisions. The
Stokes residue along these divisors gives the three displayed
operators \(d_{\mathrm{int}}\), \(d_\partial\), and \(d_{\mathrm{mix}}\);
\(\partial^2=0\) on the manifold with corners gives \(d_D^2=0\), with
the usual Arnold cancellation on the interior faces and the
associative Stasheff cancellation on the ordered boundary faces.

The two boundary approach directions are the two \(A\)-actions on
\({}_AA_A\). Therefore the bordered chain object models the diagonal
bimodule, and Theorem~\ref{thm:diagonal} identifies its bar image with
\(C_\Delta\). Gluing the two ends of the boundary interval gives the
annular closure. Moving the last desuspended input past the preceding
desuspended inputs produces the exponent in
\eqref{eq:annular-cyclic-sign}; this is exactly the cyclic action used
in the coHochschild trace of Theorem~\ref{thm:bordered-annular}.
\end{proof}

Thus the bordered FM compactification gives the collision strata,
boundary composition maps, operadic gluing, and compatibility with the
ordered bar coproduct. The annular closure identifies the cyclic
boundary trace with coHochschild homology by
Theorem~\ref{thm:bordered-annular}. These properties are exactly the
geometric hypotheses used in Theorems~\ref{thm:opposite},
\ref{thm:tangent=K}, and \ref{thm:bimod-bicomod}.
...
For quick reference, here is the list of new results proved here rather than taken as
established inputs (Hypothesis~\ref{hyp:inputs}).
...
The diagonal bicomodule $C_\Delta$, the Hochschild--coHochschild
dictionary, and the $E_1$ five main theorems form the algebraic
skeleton. The geometry driving the skeleton lives on ordered
configuration spaces: the chiral factor is an integral over labelled
complex configurations, the open factor is the real-time
associahedral compactification, pairwise monodromy supplies the
$R$-matrix, and the associator-controlled Yang--Baxter relation is
$d^2=0$,
and for affine Kac--Moody input the resulting associative algebra
with spectral parameter is the Yangian. The ordered bar complex for
the Heisenberg and $\widehat{\mathfrak{sl}}_2$ admits checks by
hand; from there the $R$-matrix, its Yang--Baxter equation, and the
dg-shifted Yangian follow, closing with the Euler-eta identity and
bar cohomology concentration theorem.
...
\begin{remark}[Ordered spaces for $E_1$-algebras]
\label{rem:why-ordered}
\index{E1 algebra@$E_1$-algebra!ordered configuration spaces}
An $E_\infty$-chiral algebra has $\Sigma_n$-equivariant operations,
and its bar complex uses the unordered space $\mathrm{Conf}_n(X)$;
the pole-free BD-commutative subclass is a special case. An
$E_1$-algebra (associative chiral algebra) has operations indexed
by ordered compositions, and its bar complex uses
$\mathrm{Conf}_n^{\mathrm{ord}}(X)$. The ordering records the
topological direction: in $\mathbb R_+\times\mathbb C$, points
along the $\mathbb R_+$-axis are ordered by time, and the
$E_1$-structure is the time-ordered product. The $R$-matrix
$R(z)\in\operatorname{End}(V\otimes V)(\!(z)\!)$ is the data
passing from ordered to unordered: the monodromy of the local
system on $\mathrm{Conf}_2^{\mathrm{ord}}(\mathbb C)$ around the
generator of $\pi_1=\mathbb Z$.
\end{remark}

\begin{construction}[Complex FM and open-colour Stasheff compactifications;
\ClaimStatusProvedElsewhere]
\label{constr:ordered-fm}
\index{Fulton--MacPherson!ordered compactification}
The labelled complex Fulton--MacPherson compactification
$\overline{\mathrm{FM}}_n^{\mathrm{lab}}(X)$ is the iterated real
oriented blowup of all partial diagonals in
$\mathrm{Conf}_n^{\mathrm{ord}}(X)$. Hence every pair collision
$D_{i,j}=\{z_i=z_j\}$ is a codimension-one boundary face. The
open-colour $E_1$ compactification is a different factor,
$K_n^{E_1}$: the real-time Stasheff compactification of ordered
points on the topological line. Its codimension-one faces are the
consecutive collapses $D^{E_1}_{i,i+1}$.

For $n=2$:
$\overline{\mathrm{FM}}_2^{\mathrm{lab}}(\mathbb C)
\cong\overline{\mathbb C}$ (the Riemann sphere), with
boundary divisor $D_{1,2}=\{z_1=z_2\}$ (one point at
infinity).

For $n=3$:
$\overline{\mathrm{FM}}_3^{\mathrm{lab}}(\mathbb C)$ has the three
pairwise divisors $D_{1,2}$, $D_{1,3}$, and $D_{2,3}$. The
associahedron $K_3^{E_1}$ has two endpoints, corresponding to the
two consecutive real-time collapses. The ordered chiral bar complex
uses both pieces: the complex FM factor supplies all holomorphic
collision residues and Arnold relations, while the $E_1$ factor
supplies the consecutive deconcatenation faces.
\end{construction}
...
Let $\{e_I\}$ be a homogeneous basis for $\bar\cA$ and let
\[
\mu_2(e_I,e_J)=\sum_K\mu^K_{IJ}e_K
\]
be the open-colour binary product.  This symbol is not the
vertex-algebra \(0\)-mode.  In the affine current shadow one writes
\[
e_I{}_{(n)}e_J=\sum_K c^K_{IJ;n}e_K ;
\]
then \(n=0\) is the Lie bracket and belongs to the symmetric
chiral/Arnold calculation, not to the associative \(E_1\) proof below.
For the genuine open-colour ordered bar differential,
\[
d_{\mathrm{res}}[s^{-1}a_1|\cdots|s^{-1}a_m]
=
\sum_{i=1}^{m-1}(-1)^{i-1}
[s^{-1}a_1|\cdots|
s^{-1}\mu_2(a_i,a_{i+1})|\cdots|s^{-1}a_m]
\otimes[\delta_{D_{i,i+1}}].
\]
Thus a sign is attached to the ordered face, not chosen term by term.
...
\;=\;
\sum_K\mu^K_{IJ}\;s^{-1}e_K
\;\otimes\;
[\delta_{D_{1,2}}],
\end{equation}
where $[\delta_{D_{1,2}}]$ is the ordered boundary face and the
coefficient is the associative product \(\mu_2\).  If \(\cA\) is
instead a local vertex algebra, replacing \(\mu_2\) by the \(0\)-mode
Lie bracket \(e_I{}_{(0)}e_J\) is a different, symmetric chiral
calculation; it is not an \(E_1\)-associative differential by itself.
...
\(\mu_2(\mu_2(e_I,e_J),e_K)
=\mu_2(e_I,\mu_2(e_J,e_K))\), the associative \(E_1\) axiom.
For a local vertex algebra the \(0\)-mode is a Lie bracket; its
Jacobi identity is enforced by the symmetric chiral bar together with
the Arnold relation, not by this open-colour argument.
\end{construction}

\begin{proposition}[Open-colour ordered AOS relations: $d^2=0$ from
...
\emph{associativity of the open-colour product \(\mu_2\)} and
\emph{sign alternation on independent face pairs}. No Jacobi
identity, no Arnold relation, and no symmetric-group
equivariance are required for this open-colour differential.

For the open-colour $E_1$ bar differential, at each degree~$n$ the
terms in $d^2$ decompose into two
classes:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Overlapping terms}: two consecutive collisions
 ...
 \(\mu_2(a,\mu_2(b,c))=\mu_2(\mu_2(a,b),c)\).
...
and
\[
d^2[a|b|c]=\mu_2(\mu_2(a,b),c)
-\mu_2(a,\mu_2(b,c))=0
\]
by associativity.
Open-colour face count: $2$ faces of the associahedron~$K_3^{E_1}$
(a line segment).
...
Open-colour face count: $3$ consecutive collision divisors of
$K_4^{E_1}$.
...
Open-colour face count: $4$ codimension-one face types of the
linear word differential inside the associahedral cellular chain model
(the Stasheff polytope with $14$ vertices).

The geometric content is the separation of factors. The complex
labelled FM compactification has all pairwise collision divisors and
therefore carries the Arnold relations. The open-colour Stasheff factor
has the consecutive deconcatenation faces governing the associative
bar differential. Associativity alone proves $d_{E_1}^2=0$ on this
open-colour factor; the full chiral differential also uses the complex
FM Arnold cancellation.
\end{proposition}

\begin{proof}
By induction on degree, using the two cancellation mechanisms.

At degree~$n$, write \(\partial_i\) for the unsigned operation
collapsing the consecutive pair \((i,i+1)\). The differential is
\[
d=\sum_{i=1}^{n-1}(-1)^{i-1}\partial_i .
\]
Decompose the sum according to whether $j=i+1$
(overlapping) or $j\ge i+2$ (independent).

\emph{Overlapping pairs} ($j=i+1$):
\(\partial_i\) followed by the induced \(\partial_i\) collapses
$a_i,a_{i+1}$ first, then the result with $a_{i+2}$,
producing \(\mu_2(\mu_2(a_i,a_{i+1}),a_{i+2})\).
Its coefficient is \((-1)^{i-1}(-1)^{i-1}=+1\). The reverse order,
\(\partial_{i+1}\) followed by the induced \(\partial_i\), produces
\(\mu_2(a_i,\mu_2(a_{i+1},a_{i+2}))\).
Its coefficient is \((-1)^i(-1)^{i-1}=-1\). Their sum is
\[
\mu_2(\mu_2(a_i,a_{i+1}),a_{i+2})
-\mu_2(a_i,\mu_2(a_{i+1},a_{i+2})),
\]
...
in $\overline{\mathrm{FM}}_n^{\mathrm{ord}}$. The algebraic
contributions of the two composites are identical because the two
collapses act on disjoint tensor slots, but
the ordered face signs are opposite. Collapsing \(i\) first and then
the induced \(j-1\) face has coefficient
\((-1)^{i-1}(-1)^{j-2}=(-1)^{i+j-3}\), while collapsing \(j\) first
and then \(i\) has coefficient
\((-1)^{j-1}(-1)^{i-1}=(-1)^{i+j-2}\). The two differ by \(-1\), the
orientation sign of the two boundary orders at the codimension-two
corner.
\end{proof}
...
Proposition~\ref{prop:ordered-aos} treats the open-colour component
\(d_\mu\) of the ordered bar differential.  This
is not the vertex-algebra \(0\)-mode component \(d_0\) of an affine
current algebra.  For the pole-order depth filtration, see the proof
of Theorem~\ref{thm:bar-cohomology-concentration}(iv).  The
\emph{full} chiral differential for a local vertex algebra decomposes
as \(d=d_0+d_1+d_2+\cdots\),
where \(d_p\) is the component involving \(n\)-th products
\(a_{(n)}b\) at pole depth \(n=p\).  Its identity \(d^2=0\) decomposes
by total depth and uses the symmetric chiral/Arnold mechanism:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Open \(E_1\) depth}: \(d_\mu^2=0\). This is associativity
 of \(\mu_2\) plus sign alternation on independent ordered faces,
 precisely the content of the proposition.
\item \emph{Vertex depth~$0$}: \(d_0^2=0\) is not associativity of the
 \(0\)-mode.  It is the Lie Jacobi identity together with the Arnold
 cancellation on the symmetric chiral configuration factor.
...
alternation) is the open-colour \(E_1\) mechanism.  The higher-depth
conditions \(\sum_{i+j=p}d_i\,d_j=0\) for a local vertex algebra use
the same collision stratification only after the complex FM/Arnold
terms are included.  The spectral-parameter generalization therefore
does not alter the fundamental distinction between the ordered
Stasheff bar (associahedron faces, \(\mu_2\)-associativity) and the
symmetric chiral bar (FM faces, Lie Jacobi plus Arnold).
\end{remark}

\begin{remark}[The Lie bracket is not the open \(E_1\) product]
\label{rem:jacobi-not-needed}
\index{bar complex!ordered!Jacobi not needed}
On the unordered (Chevalley--Eilenberg) bar complex, $d^2=0$
requires the Jacobi identity.  On the open-colour ordered bar complex,
\(d^2=0\) requires associativity of \(\mu_2\).  These are different
statements; the \(0\)-mode Lie bracket of a local vertex algebra cannot
be inserted as \(\mu_2\).
...
Now consider the naive $E_1$ word differential in which \emph{all}
pair collapses are included without the complex FM Arnold relation.
At degree~$3$, $d^2[e|f|h]$ acquires
contributions from the non-consecutive collision $D_{1,3}$
(first and third slots collide), giving
$d^2[e|f|h]\ni\varepsilon_{13}[e,h]\ne 0$, where
\(\varepsilon_{13}\in\{\pm1\}\) is the orientation sign of that
non-consecutive face. This is the
$\mathfrak{sl}_2$ counterexample: $12$ of the $27$
ordered triples have $d^2\ne 0$ in the non-consecutive
theory.

The open-colour word differential uses only consecutive real-time
collisions, and $d^2=0$ follows from associativity. The complex chiral
factor uses every pair collision, and $d^2=0$ there requires the Jacobi
identity together with the Arnold relation. For $E_\infty$-chiral
algebras both are available; for genuinely $E_1$-chiral algebras the
open-colour differential is the associative one.
\end{remark}
...
The distinction between ordered-consecutive and ordered-all-pair
is operative. For $V_k(\mathfrak{sl}_2)$ with generators
$\{e,f,h\}$, the ordered bar complex with all pair collapses
...
is genuinely $E_1$-chiral: its open-colour operations are defined on
the real-time ordered Stasheff compactification.
...
(the chiral symmetric algebra on the dual; Vol~I, \S\ref*{V1-sec:heisenberg-koszul}). The collision residue is the rank-one tensor
$r_{\cH}(z)=k\Omega_{\cH}/z$, $\Omega_{\cH}=J\otimes J$, produced from the double-pole OPE by $d\log$ absorption. After evaluation on Fock charges the $R$-matrix is the scalar $R(z)=z^k$, nontrivial as a function of~$z$. The ordered and unordered bar complexes are therefore equivalent up to the $R$-twisted sign character of~$\Sigma_n$.
\end{computation}
...
We use the ordered bar convention
\[
d[s^{-1}a_1|\cdots|s^{-1}a_n]
=
\sum_{i=1}^{n-1}(-1)^{i-1}
[s^{-1}a_1|\cdots|s^{-1}(a_i{}_{(0)}a_{i+1})|\cdots|s^{-1}a_n]
\]
and the standard \(\mathfrak{sl}_2\)-normalisation
\([e,f]=h,\ [h,e]=2e,\ [h,f]=-2f\).  Hence the degree-\(2\)
bar differential has fixed signs:
\begin{align*}
...
the $\Sigma_2$-coinvariants identify
\[
s^{-1}e\otimes s^{-1}h
\;=\;
-\,s^{-1}h\otimes s^{-1}e
\]
because the two desuspended weight-one currents are odd; in the ordered complex, they
...
$R(z)=1+\hbar\,k\,\Omega_{\mathrm{tr}}/z+O(z^{-2})$ with
$\hbar=1/(k+2)$, where
...
$k\,\Omega_{\mathrm{tr}}/z$, which vanishes at $k=0$.) The Yang--Baxter equation
...
For genuinely $E_1$ algebras $\cA$, the ordered bar complex
$\Barchord(\cA)$ carries strictly more information than the
unordered complex $\Barch(\cA)$. The surplus is the descent datum
for the $\Sigma_n$-covering
$\mathrm{Conf}_n^{\mathrm{ord}}(X)\to\mathrm{Conf}_n(X)$. The
$R$-matrix is this datum.
...
$r_n=\sum_{I,J}c^K_{IJ;n}\,e_I\otimes e_J$ is the
vertex-shadow OPE matrix at pole order~$n$, and $\mathcal P$ denotes
path-ordering.
At leading order:
...
\end{equation}
where $r_0$ is the \emph{classical $r$-matrix} of the local
vertex-algebra shadow: the matrix of vertex \(0\)-modes, hence of Lie
brackets in the affine current case.  It is not the open-colour
associative product \(\mu_2\) of
Construction~\ref{constr:ordered-bar-low-degree}.
...
ordered bar coalgebra $C=\Barchord(\cA)$, local exchange family
$R(z)$, and a braid-descent datum consisting of a chosen associator
trivialization on triple overlaps and the unitary exchange
normalization \(R_{12}(z)R_{21}(-z)=\id\) on adjacent exchange
collars. Then the unordered bar complex is the
...
underlying \(B_n\)-representation is supplied by the
associator-controlled Yang--Baxter relation
\textup{(}Proposition~\ref{prop:ybe-from-d-squared}\textup{)} after
the chosen associator trivialization; it factors through \(\Sigma_n\)
by the unitary exchange normalization.
For the pole-free BD-commutative subclass of
...
=\sigma_{i+1}\sigma_i\sigma_{i+1}$ is satisfied on the chosen
strictified comparison surface because the local exchange family
satisfies the associator-controlled Yang--Baxter relation
(Proposition~\ref{prop:ybe-from-d-squared}).
...
the unitary exchange normalization \(R_{12}(z)\,R_{21}(-z)=\id\).
This relation is not a formal consequence of the local exchange family
or of symmetry of the leading Casimir term; it is exactly the
normalization included in the braid-descent datum.  In the pole-free
BD-commutative case \(R=\tau\) it is automatic, while for vertex
algebras with OPE poles it must be supplied by the chosen comparison
surface.
...
The identity $d^2=0$ on $\Barchord_3(\cA)$ gives the
associator-controlled Yang--Baxter relation. On a strictified
ordered comparison surface, where the three-point associator is the
identity, this relation is the strict equation
...
\begin{proof}
Two proofs: topological and combinatorial.
...
The labelled complex compactification
$\overline{\mathrm{FM}}_3^{\mathrm{lab}}(\mathbb C)$ has the three
pairwise divisors $D_{1,2}$, $D_{1,3}$, and $D_{2,3}$. The open-colour
Stasheff factor $K_3^{E_1}$ has two consecutive boundary points.
The full-collision face supplies the associator; after strictification
it is the identity.
...
\item $D_{1,2}$ then $D_{2,3}$: collapse $z_1\to z_2$
 (applying \(\mu_2(a,b)\)), then the composite collides
 with $z_3$ (applying \(\mu_2(\mu_2(a,b),c)\)).
 The monodromy of the two-step path is
 $R_{12}(z_{12})\,R_{13}(z_{13})$.
\item $D_{2,3}$ then $D_{1,2}$: collapse $z_2\to z_3$
 first (applying \(\mu_2(b,c)\)), then $z_1$ collides with
 the composite (applying \(\mu_2(a,\mu_2(b,c))\)).
 The monodromy is
 $R_{23}(z_{23})\,R_{13}(z_{13})$.
\end{enumerate}
The algebraic part of $d^2$ vanishes by associativity of
the open-colour product \(\mu_2\).  The monodromy part gives the
associator-controlled Yang--Baxter relation; under strictification the
two sides of~\eqref{eq:ybe-from-bar} are the two pathways, and their
equality is $d^2=0$.
\end{proof}
...
The modular characteristic $\kappaChHodge(\cA)$ lives at degree~$2$
...
Direct coefficient verification gives $67$ independent Eulerian
weight identities.
\end{remark}
...
The $R$-matrix governs descent from ordered to unordered; the
Yang--Baxter equation is $d^2=0$. The $E_\infty$-chiral Koszul dual
$\cA^!_{\mathrm{ch}}$ is another chiral algebra on the same curve.
The ordered ($E_1$) Koszul dual is an associative algebra with
spectral parameter, living on the affine line of spectral
parameters rather than the curve. For affine Kac--Moody input this
algebra is the Yangian.
...
\item \emph{Open colour.}
 ...
 (Proposition~\ref{prop:sc-koszul-duality-nonselfdual}), so the
...
\mathrm{Vir}_c
 ...
 & \mathbf{M} \\
\mathcal{W}_{3,c}
 & \text{companion }\mathcal{W}_{3,\,100-c}
...
\begin{proposition}[Associated-graded current shadow of the Yangian bar;
\ClaimStatusProvedHere]
...
Equip \(Y_\hbar(\fg)\) with Drinfeld's PBW filtration.  The
associated graded algebra is the current enveloping algebra
\[
\operatorname{gr}_F Y_\hbar(\fg)
\;\simeq\;
U(\fg[t]).
\]
The ordered bar of \(Y_\hbar(\fg)\) has a primitive collision-residue
projection whose associated graded degree-\(2\) and degree-\(3\)
pieces are the current Lie bracket and Jacobi identity:
\[
 \operatorname{gr}_F
 \operatorname{Prim}_{\mathrm{coll}}
 d_{\Barchord(Y_\hbar(\fg))}
 \;=\;
 [-,-]_{\fg[t]} .
\]
Thus the computation recovers the \emph{current shadow}
\(U(\fg[t])\) only after taking the PBW associated graded or the
primitive collision-residue projection.  It is not an isomorphism
\(\Barchord(Y_\hbar(\fg))\simeq U(\fg[t])\) of full ordered bar
objects.
The central extension
$k\,n\,\delta_{n+m,0}\in[\widehat{\fg}_k,\widehat{\fg}_k]$
is absent from this associated-graded current shadow.
...
\begin{proof}
The Yangian spectral OPE in the Drinfeld presentation has singular part
\[
 [E_i(u),\,F_j(v)]
 \;=\;
 ...
 \;+\;\text{(regular)},
\]
with only \emph{simple} poles in \(u-v\).  After passing to the PBW
associated graded and projecting the ordered bar differential to the
primitive collision-residue part, the degree-\(2\) component extracts
the current Lie bracket:
\[
 ...
 A_{ij}\,s^{-1}E_j.
\]
This is the degree-\(2\) shadow of Drinfeld's PBW theorem
\(\operatorname{gr}_F Y_\hbar(\fg)\simeq U(\fg[t])\).  The full
associative bar differential of \(Y_\hbar(\fg)\) is the bar
differential of the Yangian product; its bar-cobar recovery is the
Yangian, not the associated-graded current algebra.
...
\(\operatorname{gr}_F d^2=0\) on the primitive collision shadow
reduces to the classical Yang--Baxter equation for
...
The $R$-matrix is $R(z)=1+\hbar\,k\,\Omega_{\mathrm{tr}}/z+O(z^{-2})$ with
$\hbar=1/(k+2)$ and
$\Omega=e\otimes f+f\otimes e+\tfrac12 h\otimes h$
...
The low-degree computations display the ordered bar complex entry
by entry. The generating function for bar dimensions across tensor
degrees and conformal weights admits a closed form connecting the
bar complex to Dedekind $\eta$-functions.

The ordered bar complex carries a bigrading by tensor degree~$n$
(number of bar slots) and conformal weight~$w$ (total weight of
insertions). The Euler characteristic in this bigrading is
computable in closed form for the standard families; for affine
Kac--Moody algebras at generic non-critical level it connects to
Dedekind $\eta$-functions and modular forms.
...
\item Weight-refined: if the generators have conformal
 ...
 \end{equation}
\item The open-colour ordered bar filtration has first differential
 \(d_\mu\), the coderivation determined by the associative product
 \(\mu_2\); it does not have Lie algebra homology as its first page
 unless the chosen \(E_1\) product itself is an enveloping-algebra
 product.  If \(\cA\) also carries a local vertex-algebra shadow, then
 the separate pole-depth filtration of the symmetric chiral shadow has
 \[
 E_1^{p,*}=H_*^{\mathrm{Lie}}(\fg;\Bbbk),
 \qquad [a,b]=a_{(0)}b,
 \]
 and converges to the symmetric chiral\slash Francis--Gaitsgory shadow,
 not to the open-colour \(E_1\) proof.  For affine Kac--Moody shadows
 this vertex-depth spectral sequence collapses at \(E_2\).  For the
 Virasoro shadow, higher differentials are nonzero through \(E_4\)
 because of the quartic OPE pole.
\end{enumerate}
\end{theorem}

\begin{proof}
Statement~(i): Koszulness means the open-colour bar cohomology
...
\Barchord_1$ extracts the open product \(\mu_2(a,b)\); its image is
the space of open-colour decomposable elements.  The quotient is
\[
V=Q_{\mu_2}(\bar\cA)
  =\bar\cA/\mu_2(\bar\cA,\bar\cA),
\]
identified with the strong generating space by the strongly
admissible Koszul hypothesis.
...
Statement~(iv): the open-colour filtration is the tensor-length
filtration for the associative coderivation \(d_\mu\).  Its first
page is the homology of the ordered associative bar
\(\Barchord(\bar\cA,\mu_2)\), not Lie algebra homology of vertex
\(0\)-modes.

For a local vertex-algebra shadow one may instead filter the
symmetric chiral differential by \emph{pole-order depth}: assign
filtration degree~$p$ to the component involving OPE products
$a_{(n)}b$ with \(n\le p\). The $E_0$ page uses only the vertex
\(0\)-mode, so
\[
E_1^{p,*}=H_*(\fg;\Bbbk),
\qquad [a,b]=a_{(0)}b .
\]
This spectral sequence belongs to the symmetric chiral\slash
Francis--Gaitsgory shadow described in
Remark~\ref{rem:depth-decomposition-d-squared}; it is not the
open-colour \(E_1\) associativity argument.  The differentials
\(d_r\colon E_r^{p,q}\to E_r^{p+r,q-r+1}\) are controlled by the
\(r\)-th pole OPE coefficients. For affine shadows
\textup{(}pole order~\(\le 2\)\textup{)}, the spectral sequence has at
most one nontrivial differential and collapses at \(E_2\). For the
Virasoro shadow \textup{(}pole order~\(4\)\textup{)}, the central
charge and its descendants produce nonzero differentials through
\(E_4\).
\end{proof}
...
\end{array}
\]
The last column records the separate vertex-shadow spectral sequence,
not the open-colour \(E_1\) concentration proof.  The affine family has
all generators at weight~$1$ (currents), and the vertex-shadow depth
sequence collapses at $E_2$ because the only higher pole is the double
pole (the Killing form term $k\,\kappa^{ab}/(z{-}w)^2$), which
...
The generating functions record the ordered bar complex at the
dimensional level. The structural claim verified at the first
nontrivial rank is the associated-graded one: the primitive
collision-residue shadow of the Yangian ordered bar is the current
Lie algebra, hence its enveloping algebra is the PBW associated graded
\(U(\mathfrak{sl}_3[t])\).

\begin{computation}[Associated-graded Yangian bar shadow for
$\mathfrak{sl}_3$: explicit verification]%
\label{comp:double-bar-sl3}%
\index{double bar!sl3@$\mathfrak{sl}_3$!explicit verification}%
Verification: Proposition~\ref{prop:open-colour-double-bar}
for $\fg=\mathfrak{sl}_3$ at degrees~$1$--$3$ by explicit
computation of the Chevalley differential and the primitive
collision-residue projection in the Chevalley basis.  The computation
identifies the associated-graded current shadow; it does not assert an
isomorphism between the full ordered bar of the Yangian and
\(U(\mathfrak{sl}_3[t])\).
...
\end{enumerate}
Together these give the PBW associated-graded current shadow
\[
\operatorname{gr}_F Y_\hbar(\mathfrak{sl}_N)
\simeq U(\mathfrak{sl}_N[t])
\]
and identify the primitive collision-residue projection of
\(\Barchord(Y_\hbar(\mathfrak{sl}_N))\) with the Lie bracket of
\(\mathfrak{sl}_N[t]\) for all \(N\).
...
The preceding computation treats the Yangian as the output:
the open-colour Koszul dual of the affine algebra. With the
Yangian as input -- a genuinely $E_1$-chiral algebra -- its
ordered bar complex computes directly.  Proposition~\ref{prop:open-colour-double-bar}
identifies only the associated-graded primitive collision shadow with
the current algebra \(U(\fg[t])\) without central extension.  The full
bar-cobar object remains the Yangian \(Y_\hbar(\fg)\).
...
(Construction~\ref{constr:dg-shifted-yangian-from-bar}).
It is an associative algebra with spectral parameter, a genuinely
$E_1$-chiral algebra in the sense of \S\ref{sec:setup}. The
ordered bar complex of $Y_\hbar(\mathfrak{sl}_2)$ is computed
below, with the Yangian taken as the input chiral algebra.
...
k\,\Omega_{\mathrm{tr}}/z$ (a simple pole with the Casimir, scaled by
...
The Yangian $Y_\hbar(\mathfrak{sl}_2)$ is the open-colour Koszul
...
\]
The open-colour Koszul dual of the Yangian itself is not computed by
discarding the deformation parameter.  Proposition~\ref{prop:open-colour-double-bar}
computes only the associated-graded primitive collision shadow
\[
\operatorname{gr}_F Y_\hbar(\mathfrak{sl}_2)
\simeq U(\mathfrak{sl}_2[t]).
\]
The corresponding shadow of the small linear dual is
\begin{equation}\label{eq:yangian-koszul-dual}
\operatorname{gr}_F
\cA^{!,\mathrm{small}}_{\mathrm{line}}\bigl(Y_\hbar(\mathfrak{sl}_2)\bigr)
\;\leadsto\;
U(\mathfrak{sl}_2[t])^\vee,
\end{equation}
where \(\leadsto\) denotes the associated-graded current-shadow
calculation, not an isomorphism of full \(E_1\)-bar objects.  The
Yangian is not identified with this shadow. The duality
chain:
\[
...
\text{Yangian, }E_1}
\;\xrightarrow{\;\text{associated graded shadow}\;}
\underbrace{U(\mathfrak{sl}_2[t])^\vee}_{
\text{current coalgebra dual, }E_1}
\;\xrightarrow{\;\text{not full KD}\;}
\;\cdots
\]
This chain records a shadow, not a second full Koszul duality.
Open-colour Koszul duality is an involution on the $E_\infty$ side
(for Kac--Moody:
$\widehat{\fg}_k \mapsto \widehat{\fg}_{-k-2h^\vee} \mapsto
\widehat{\fg}_k$); the open \(E_1\) side requires the full
bar-cobar object, not just the primitive collision shadow. The reason
the shadow loses data is:
\begin{itemize}[nosep]
...
\item The Yangian $Y_\hbar(\fg)$ has only simple poles
 (the RTT singularities), so the primitive collision-residue
 projection sees the current Lie bracket.
\item The deformation parameter $\hbar$ is invisible only after
 passing to the PBW associated graded or to the primitive
 collision-residue shadow; it is present in the full RTT product and
 in the full ordered bar.
\end{itemize}
The current shadow is not the Yangian.  Self-duality lives at the level of
the full $\SCchtop$-algebra, where the two-colour Koszul duality
(Theorem~\ref{thm:duality-involution}) acts as an involution on
the pair $(\cA^!_{\mathrm{ch}},\,\cA^!_{\mathrm{line}})$. The open
colour shadow alone does not close.

At $\hbar = 0$ (the PBW associated graded/classical limit), the
shadow becomes the current enveloping algebra:
\(Y_0(\mathfrak{sl}_2)=U(\mathfrak{sl}_2[t])\), and
\[
\cA^{!,\mathrm{small}}_{\mathrm{line}}(U(\mathfrak{sl}_2[t]))
\;=\;
U(\mathfrak{sl}_2[t])^\vee
\;\simeq\;
U(\mathfrak{sl}_2[t])
\]
only after choosing the usual graded restricted dual/PBW pairing.  The
display is a graded shadow calculation; it is not a canonical
self-duality of the completed enveloping algebra as an unfiltered
topological algebra.

\medskip
\noindent\emph{(5) The associated-graded primitive shadow of
...
\emph{Degree~$2$: the associated-graded primitive residue extracts
the Lie bracket of $\mathfrak{sl}_2[t]$.}\enspace
The primitive collision-residue projection of the bar differential,
after passing to the PBW associated graded, extracts the
current Lie bracket. By
...
H^{(r+s-1-p)} - H^{(p)}\bigr)
\\
&\xrightarrow{\;\operatorname{gr}_F,\ \hbar^{-1}\;}
...
\textbf{Summary at degree $2$:} the associated-graded primitive
collision-residue projection of the ordered bar differential
reproduces exactly the Lie bracket of the polynomial current algebra
$\mathfrak{sl}_2[t]$:
\[
\operatorname{gr}_F d_{\mathrm{bar}}^{\mathrm{prim,coll}}
\;=\;
[-,-]_{\mathfrak{sl}_2[t]}.
\]
The level \(k\) (hence \(\hbar = 1/(k{+}2)\)) remains in the full RTT
product and in the filtered Yangian, but it disappears from this
associated-graded current shadow.  The shadow is the
\(\mathfrak{sl}_2[t]\) bracket \textbf{without central extension},
confirming
Proposition~\ref{prop:open-colour-double-bar}
at degree~$2$.
...
identity. This is the current-shadow classical \(r\)-matrix parallel
to the one obtained from
$\widehat{\mathfrak{sl}}_{2,k}$
(Computation~\ref{comp:ordered-bar-sl2}), confirming
that the associated-graded primitive shadow sees only the current
structure.
...
1 & \{s^{-1}X^{(r)} : X \in \{E,F,H\},\;r\ge 0\}
 & \mathfrak{sl}_2[t] \\
2 & \operatorname{gr}_F d^{\mathrm{prim,coll}} = [-,-]_{\mathfrak{sl}_2[t]},\;\text{no central ext.}
 & \text{Lie bracket of } \mathfrak{sl}_2[t] \\
3 & d^2 = 0 \iff \text{Jacobi for } \mathfrak{sl}_2[t]
 & \text{CYBE for } r_Y(z)=\hbar\Omega/z
\end{array}
\]
This confirms the associated-graded current-shadow clause of
Proposition~\ref{prop:open-colour-double-bar} at the first three
degrees. The computation extends to all degrees at the level of
Drinfeld PBW associated graded: the Yangian spectral OPE has only
simple poles at \(u=v\), so the primitive collision-residue shadow
extracts the Lie bracket of the current algebra, and \(d^2=0\)
follows from the Jacobi identity.

\medskip
\noindent\emph{(6) Where the deformation parameter is lost.}
The associated-graded primitive collision shadow sees only residues at
the collision diagonal \(u=v\).  The Yangian deformation parameter
\(\hbar\) is retained by the full RTT product, but it disappears after
passing to \(\operatorname{gr}_F\) and projecting to the current
residue.  This is the precise analogue of the central-extension
statement in Proposition~\ref{prop:open-colour-double-bar}: the level
\(k\) is not recovered by the associated-graded current shadow.

\medskip
\noindent\emph{(7) The \(R\)-matrix of the Yangian,
viewed as input.}
The Yangian is itself an \(E_1\)-chiral algebra, so it has its own
RTT \(R\)-matrix input.  The associated-graded current shadow has
leading term
\begin{equation}\label{eq:yangian-r-matrix-own}
R_Y^{\operatorname{gr}}(z)
\;=\;
1 + \hbar\,\Omega_{\mathfrak{sl}_2}/z + O(z^{-2}),
\end{equation}
where $\Omega_{\mathfrak{sl}_2}$ is the Casimir of
\(\mathfrak{sl}_2\).  The full Yangian \(R\)-matrix is the RTT input;
the display records only its current-shadow leading term.  Forgetting
\(\hbar\) is not a statement about the full Yangian \(R\)-matrix.

This is a precise illustration of the loss phenomenon:
passing from \(Y_\hbar(\mathfrak{sl}_2)\) to the associated-graded
current shadow gives \(U(\mathfrak{sl}_2[t])\), not
\(\widehat{\mathfrak{sl}}_{2,k}\). The central extension (the datum
that distinguishes the affine algebra from the current algebra) is
absent from this shadow. Recovery requires the closed-colour direction
(Verdier/Ran duality on the full chiral bar complex \(\barBch(\cA)\)).
\end{computation}
...
$\kappaChHodge(\Omega) = \dim(\mathfrak{sl}_3) = 8$
...
genus-one curvature $\kappaChHodge(\mathrm{Vir}_c)\cdot\omega_1$
...
\hline
\text{Gauge (CS)} & \text{finite Lie transfer; no wheel tower} &
 \text{nontrivial Yangian} \\
\text{Gravity (Vir)} & m_k \neq 0 \;\forall\, k &
 \text{strictly primitive}
\end{array}
\]
For gauge theories (Chern--Simons type, class~$\mathbf{L}$), the open-colour dual is a genuine Yangian $Y_\hbar(\fg)$ with a nontrivial spectral coproduct $\Delta_z$. Its finite cubic layer is the Lie-Jacobi transfer; the Virasoro wheel tower is absent. For gravitational theories (Virasoro type, class~$\mathbf{M}$), the two columns swap: the higher products $m_k$ are all nonzero (an infinite $\Ainf$ tower), but the coproduct $\Delta_z$ degenerates to $\Delta_z(x) = x \otimes 1 + 1 \otimes x$ because the Virasoro algebra admits no finite-type Hopf structure.

Drinfeld--Sokolov reduction transports finite class~$\mathbf{L}$ transfer to class~$\mathbf{M}$: it preserves Koszulness and creates the Virasoro wheel tower. The resolvent tree formula
...
\begin{definition}[$E_1$-chiral quantum group datum]%
...
An \textbf{$E_1$-chiral quantum group datum} on a curve $X$ is an
...
A chiral Hopf object is the stronger datum obtained by adding an
augmentation and an antipode satisfying the Hopf compatibility.
\end{definition}
...
itself: it is an independent bialgebraic datum, not to be
...
are independent structure not recoverable from a local OPE: the
discriminant is provenance, not value.
...
Let $(\cA, \Delta, R(z))$ be an $E_1$-chiral quantum group datum on
a curve~$X$
...
The explicit comparison obstruction is
Theorem~\ref{thm:strong-weak-chiral-yangian-comparison-obstruction}.
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item Trace-form: $r(z) = k\,\Omega_{\mathrm{tr}}/z$; at \(k=0\) only this
singular central trace-form pole vanishes.
\item KZ normalisation: $r(z) = \Omega/((k+h^\vee)z)$, equal to
$\Omega/(h^\vee z)$ at $k=0$.
\item Sugawara/KZ comparison: the two representatives are related on
the non-critical comparison surface. This is not the scalar identity
\(k=1/(k+h^\vee)\).
\end{enumerate}
For Heisenberg $\cH_k$, the tensor kernel
$r_{\cH}(z)=k\Omega_{\cH}/z$ has rank-one coefficient $k/z$ and gives the
ordered bar differential
$d(s^{-1}\alpha\otimes s^{-1}\alpha) = k\, s^{-1}(1)$
...
$r(z)$ to the modular characteristic $\kappaChHodge(\cA)$ at degree $2$,
the binary collision projection of the universal MC element
$\Theta_\cA$. For $\cA$ abelian or scalar (Heisenberg, Virasoro),
$\mathrm{av}(r(z)) = \kappaChHodge(\cA)$; for $\cA$ non-abelian
Kac--Moody, $\mathrm{av}(r(z)) + \dim(\fg)/2 = \kappaChHodge(V_k(\fg))$
...
$\varkappa(\cA) := \kappaChHodge(\cA) + \kappaChHodge(\cA^!)$ is a DISTINCT
family-specific quantity ($\varkappa = 0$ for KM/free, $\varkappa = 13$
for Vir, $\varkappa = 250/3$ for $\cW_3$, as catalogued in Vol~I
landscape census; the BP slot is open, no derivation of
$\kappaChHodge(\mathrm{BP}_k)$ existing), related to the Trinity
conductor on the computed families by
$\varkappa = \varrho_{\cA}\cdot K$ with
$\varrho_{\mathrm{KM}} = \varrho_{\mathrm{free}} = 0$ and
$\varrho_{\cW_N} = H_N - 1$; no such coefficient has been derived
for BP. The
Trinity takes values $K(\Vir) = 26$, $K(\cW_3) = 100$,
$K(\mathrm{BP}) = 50$, $K(V_k(\fg)) = 2\dim\fg$. The cohomological
...
\section{Core \(E_1\)-chiral Hall--Drinfeld double}
\label{sec:oackd-core-supplement}

\begin{remark}[\texorpdfstring{\(E_1\)-native at \(d = 3\) with \(E_2\) via Drinfeld centre}{E1-native at d = 3 with E2 via Drinfeld centre}]
\label{rem:oackd-core-E1-E2}
...
\begin{remark}[Hall--Drinfeld double as fourth quasi-Hopf taxon]
\label{rem:oackd-core-hall-drinfeld}
The Vol~III K3 chapter treats the core quantum-group deformation of
$\mathfrak{g}_{\Delta_5}$ as a conditional Hall--Drinfeld target
\[
\mathcal{D}_\hbar^{\mathrm{red},\mathrm{comp}}
  (\mathcal{Y}^{\mathrm{Hall}}_\hbar(\mathrm{CoHA}_{K3 \times E}))
\;\dashrightarrow\;
\mathbf H_{\Delta_5},
\]
with Schiffmann--Vasserot $2012$ shuffle (Davison $2017$ CY-$3$
extension). This is a fourth taxonomic slot beyond Drinfeld--Jimbo,
Yangians, RTT, and Manin-triple quasi-Hopf, but it is gated by the
finite Hall/CoHA source, pairing, PBW/no-extra-relations, radical,
parity, completion, inverse-limit, and EK/Heegner comparison tests.
The underlying Manin datum is a pair, not a triple, because the
signature-$(2,1)$ Cartan forbids Lagrangian decomposition. The
cohomological comparator is the target map
\[
\widehat H^2_{\mathrm{red}}\!
  \left(\mathfrak{g}_{\Delta_5}\right)^{\mathbb{Z}/2, K(1)}
\;\longrightarrow\;
\mathbb{C} \cdot \Delta_5,
\]
so the Igusa cusp form is the automorphic target line, not the bare
deformation class.
\end{remark}

\section{Synthesis (continued)}
\label{sec:oackd-deep}

\begin{remark}[Ordered associative $E_1$-bar on $\mathbf H_{\Delta_5}$]
\label{rem:oackd-deep-1}
The ordered associative $E_1$-bar core of this chapter applies to the
K3 bialgebra on each finite Hall/CoHA window and gives the target
\[
...
Schiffmann-Vasserot 2012). The full object is reached only after the
PBW/no-extra-relations, radical, parity, completion, and inverse-limit
gates. This is the source being tested for the non-abelian chiral
structure. Compare Vol.~I,
\S\ref{V1-sec:barconstr-supplement}.
\end{remark}

\begin{remark}[Averaging and the symmetric shadow]
\label{rem:oackd-deep-2}
...
Lie-algebra structure emerges from the Hall Yangian. The K3
...
Compare Vol.~I, \S\ref{V1-sec:e1mkd-supplement}.
\end{remark}
