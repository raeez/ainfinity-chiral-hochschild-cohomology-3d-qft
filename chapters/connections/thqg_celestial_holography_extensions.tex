...
decomposition by genus. The following makes the genus-by-genus recursion fully explicit and
...
Let $\cA$ be a modular Koszul chiral algebra with curvature $\kappaChHodge(\cA)$, and let
...
The degree-$r$ component $\Theta_\cA^{(r)}$ (when denoted by degree
...
\item \emph{Internal vertex insertion}: a higher-degree vertex $m_n$ with $n \geq 3$
...
\label{cor:contracting-homotopy-koszul}
If $\cA$ is chirally Koszul, then $H^2(\mathfrak{g}^{(G)}, d_0) = 0$ for all
$G \geq 1$ (Vol~I, Theorem~\ref*{V1-thm:koszul-equivalences-meta}, condition~(vii):
...
The computation of the genus recursion explicitly at genera $1$ through~$4$.
...
\Theta_\cA^{(1)} = \kappaChHodge(\cA) \cdot \eta \otimes \Lambda_1,
\end{equation}
where $\eta$ is the cyclic trace form and $\Lambda_1 \in H^1(\ov{\M}_{1,1})$ is
the Hodge class. The free energy is
\begin{equation}
% label removed: eq:f1
F_1(\cA) = \kappaChHodge(\cA) \cdot \frac{1}{24}.
\end{equation}
\end{computation}

\begin{proof}
The trace $\sum_i \langle e_i, m_2(e^i, -) \rangle$ computes the supertrace
of the identity on the cyclic complex, which equals $\kappaChHodge(\cA)$ by definition
...
normalization \(F_1(\cA)=\kappaChHodge(\cA)/24\).
\end{proof}
...
function (Vol~I equation~\texttt{eq:modular-char-gf}) uses the Bernoulli
number convention $B_2 = 1/6$, and
\[
F_1(\cA)=\kappaChHodge(\cA)\cdot
(2^{2\cdot 1 - 1} - 1)/(2^{2\cdot 1 - 1}) \cdot |B_2|/(2!) =
\kappaChHodge(\cA) \cdot \frac{1}{2} \cdot \frac{1}{12}
=\kappaChHodge(\cA)/24,
\]
confirming the
normalization.
...
\label{comp:genus-2-three-shell}
...
On the proved scalar lane, the free energy is
\begin{equation}
% label removed: eq:f2
F_2(\cA) = \kappaChHodge(\cA) \cdot \frac{7}{5760}.
\end{equation}
\end{computation}

\begin{proof}
Using the universal generating function (Vol~I equation~\texttt{eq:modular-char-gf}),
\[
...
\]
Therefore
$F_1(\cA)=\kappaChHodge(\cA)/24$ and
$F_2(\cA)=7\kappaChHodge(\cA)/5760$.

The three-shell decomposition independently verifies the coefficient.
On the scalar lane, $F_2$ is linear in~$\kappa$: the $\kappa^2$
contributions from the loop-loop and separating shells cancel by
$D^2 = 0$, and the surviving planted-forest coefficient is
$\alpha_{\mathrm{pf}} = 7/5760$ by the codimension-$2$
cancellation identity on $\ov{\M}_2$.
\end{proof}
...
% label removed: eq:f3
F_3(\cA)
= \kappaChHodge(\cA) \cdot \frac{31}{967680}.
\end{equation}
...
% label removed: eq:f4
F_4(\cA)
= \kappaChHodge(\cA) \cdot \frac{127}{154828800}.
\end{equation}
...
= \frac{127}{154828800}$.
This is the Faber--Pandharipande marked Hodge integral on
$\ov{\M}_{4,1}$,
\(\int_{\ov{\M}_{4,1}}\psi_1^6\lambda_4
=127/154828800\).
\end{proof}
...
\(\kappaChHodge(\mathcal H_k)=k\)) and the Virasoro algebra
\(\mathrm{Vir}_c\) (with \(\kappaChHodge(\mathrm{Vir}_c)=c/2\))
...
6 & \frac{1414477k}{2678117105664000} & \frac{1414477c}{5356234211328000} \\[3pt]
7 & \frac{8191k}{612141052723200} & \frac{8191c}{1224282105446400} \\[3pt]
8 & \frac{16931177k}{49950709902213120000} & \frac{16931177c}{99901419804426240000} \\[3pt]
9 & \frac{5749691557k}{669659197233029971968000} & \frac{5749691557c}{1339318394466059943936000} \\[3pt]
10 & \frac{91546277357k}{420928638260761696665600000} & \frac{91546277357c}{841857276521523393331200000}
\end{array}
\]
...
\label{eq:fg-general}
F_g(\cA)
= \kappaChHodge(\cA) \cdot \frac{2^{2g-1} - 1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!},
\end{equation}
...
(Vol~I equation~\texttt{eq:modular-char-gf}), expanded as
\[
\frac{x/2}{\sin(x/2)} - 1
...
Heisenberg column (at matching central charge $c$), reflecting $\kappaChHodge(\mathrm{Vir}_c) = c/2$
versus $\kappaChHodge(\mathcal{H}_c) = c$.
\end{proof}

\begin{remark}[Asymptotics and convergence]
% label removed: rem:free-energy-asymptotics
The scalar genus series $\sum F_g\,\h^{2g}$ converges absolutely
for $|\h| < 2\pi$. The reason is the asymptotic
$F_g(\cA) \sim 2\kappaChHodge(\cA)/(2\pi)^{2g}$: the Bernoulli factorial
$|B_{2g}| \sim 2\,(2g)!/(2\pi)^{2g}$ is exactly cancelled by the
$(2g)!$ in the denominator of~$\lambda_g^{\mathrm{FP}}$, leaving
exponential decay where matrix-model genus expansions have
factorial divergence. Genuine Gevrey-$1$ growth and resurgent
trans-series enter through the shadow obstruction tower~$S_r$, $r \ge 3$
(\S\ref*{subsec:gravity-shadow-tower}).
\end{remark}
...
\begin{theorem}[Spectral-sequence support on the chirally Koszul surface; \ClaimStatusProvedHere]
% label removed: thm:spectral-sequence-convergence
If $\cA$ is chirally Koszul, then $E_1^{p,q} = 0$ for $q \neq 0$, and the spectral
sequence is supported on the degree-$0$ line. Consequently all higher
differentials $d_r$ for $r \geq 2$ vanish, and the genus-by-genus scalar package is
controlled by the $q=0$ line together with the non-separating BV differential
$d_1$ on that line. This does not by itself imply curvature-only reconstruction:
additional finite or infinite shadow corrections may still survive on the
degree-$0$ line.
\end{theorem}
...
acting on the degree-$0$ line. On the Gaussian/class~G surface this reduces to
the universal curvature/Bernoulli formula \eqref{eq:fg-general}, while for the
finite-depth and infinite-depth shadow regimes the same $q=0$ line may still carry
additional shadow corrections.
\end{proof}

\begin{remark}[Swiss-cheese non-formality versus bar concentration]
% label removed: rem:non-koszul-spectral
Non-formal Swiss-cheese structure does \emph{not} by itself force failure of
the $E_1$-degeneration statement above. Bar concentration and Swiss-cheese
formality are different properties. On the standard chirally Koszul families,
including class~M examples such as Virasoro and $\mathcal{W}_N$, the spectral
sequence can still be supported on the degree-$0$ line. What the support depth
measures is the amount of shadow data needed on that line: classes~G/L/C are
controlled by finite jets, while class~M has no finite support cutoff on the
same $q=0$ reconstruction surface.  Whether those support channels give
distinct physical soft factors is a separate celestial comparison question.
\end{remark}

\begin{proposition}[Finite support shadow depth controls the $q=0$ reconstruction; \ClaimStatusProvedHere]
% label removed: prop:spectral-shadow-depth
Assume $\cA$ is chirally Koszul and the bar-intrinsic shadow packet has
support depth \(r_\Theta:=r^\Theta_{\max}(\cA)\), i.e. all obstruction
classes and all \(H^1\)-lift coordinates above degree \(r_\Theta\) vanish in the
chosen gauge slice. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the genus spectral sequence is supported on the $q=0$ line and has
 $d_s = 0$ for all $s \geq 2$;
\item no shadow datum of degree \(>r_\Theta\) enters the $q=0$ reconstruction;
\item equivalently, the genus-$g$ scalar package is determined for all $g$ by the
 finite jet \(J^{r_\Theta}(\cA)\) together with the BV differential $d_1$ on the
 degree-$0$ line.
\end{enumerate}
\end{proposition}

\begin{proof}
Part \textup{(i)} is exactly the previous theorem. If the support
shadow depth is \(r_\Theta\), then
\(\Theta_\cA^{\leq r_\Theta}=\Theta_\cA\) in the bar-intrinsic gauge
slice and all shadow-packet components of degree \(>r_\Theta\) vanish.
Hence only the finite jet
\(J^{r_\Theta}(\cA)\) contributes to the $d_1$-controlled reconstruction on the
degree-$0$ line, giving \textup{(ii)} and \textup{(iii)}.
\end{proof}
...
The support-shadow packet (Vol~I, \S\ref*{sec:shadow-postnikov-tower}) encodes the nonlinear
OPE data of a chiral algebra as a sequence of finite-order invariants.
These invariants form a graded vector space; reconstruction asks when
finite truncations determine the completed bar-intrinsic packet.
...
\end{equation}
where $\cA^{\mathrm{sh}}_{s,0}$ is the degree-$s$, genus-$0$ component of the shadow
algebra (Vol~I, Definition~\ref*{V1-def:shadow-algebra}). Explicitly:
\begin{itemize}
...
\begin{theorem}[Finite-jet characterization of support shadow depth; \ClaimStatusProvedHere]
% label removed: thm:finite-jet-shadow-depth
Let \(r_\Theta=r^\Theta_{\max}(\cA)\) be the support depth of the
bar-intrinsic shadow packet.
The following are equivalent:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the support shadow depth \(r^\Theta_{\max}(\cA)\) is finite;
\item \(\Theta_\cA^{\leq r_\Theta} = \Theta_\cA\);
\item the augmented reconstruction packet has no nonzero component
 above degree \(r_\Theta\);
\item the obstruction classes \(o_n(\cA)\) and the lift coordinates
 \(\lambda_n(\cA)\) vanish for all \(n>r_\Theta\).
\end{enumerate}
\end{theorem}

\begin{proof}
This is the support-depth definition applied to the compatible shadow
reconstruction packet.  Obstruction classes alone give only
liftability; the \(H^1\)-lift coordinates select the actual finite MC
lift.  Hence the finite-jet characterization requires vanishing of
both \(o_n\) and \(\lambda_n\) above the support bound.
\end{proof}
...
work:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The \emph{shadow obstruction tower} $\Theta_\cA^{\leq r}$ denotes the
 degree-truncation (finite-order projection of the MC element).
...
\item The \emph{degree-$r$ shadow} $\operatorname{Sh}_r(\cA) =
 \cA^{\mathrm{sh}}_{r,0}$ is the genus-$0$, degree-$r$ component
 of the shadow algebra $\cA^{\mathrm{sh}}$.
\end{enumerate}
The shadow jet space $J^r(\cA)$ assembles the degree-$s$ shadows for
...
(This is a Gaussian algebra with support shadow depth~$2$ and, for the
Heisenberg family, lies on the proved scalar lane in Vol~I.)
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(r^\Theta_{\max}(\mathcal{H}_k) = 2\) (Gaussian support shadow depth).
\item The completed support packet equals its quadratic truncation:
 \(\Theta_{\mathcal H_k}=\Theta_{\mathcal H_k}^{\le2}\), and
 \(J^2(\mathcal{H}_k)\) is the line
 \(\C \cdot \kappaChHodge(\mathcal H_k)\) with
 \(\kappaChHodge(\mathcal{H}_k) = k\).
\item A single datum, the level $k$, determines the scalar modular shadow series:
 \begin{equation}
 % label removed: eq:heisenberg-full-reconstruction
 \mathcal Z^{\mathrm{sc}}_{\mathcal{H}_k}(\h)
 = k \cdot \sum_{g=1}^{\infty} \h^{2g-2} \cdot
 \frac{2^{2g-1}-1}{2^{2g-1}} \cdot \frac{|B_{2g}|}{(2g)!}.
 \end{equation}
\item The shadow connection
\(\nabla^{\mathrm{hol}}_{0,n}
= d - \kappaChHodge(\mathcal H_k)\cdot \omega^{(2)}\)
...
one-dimensional, spanned by \(\kappaChHodge(\mathcal H_k)=k\).

(iii) Since the support packet terminates at degree $2$, the full MC element is
\(\Theta_{\mathcal{H}_k} =
\kappaChHodge(\mathcal H_k) \cdot \eta \otimes \Lambda\), where
$\Lambda = \sum_{g \geq 1} \h^g \Lambda_g$ is the Hodge generating class. The
scalar shadow series is determined by the universal generating function
evaluated at \(\kappaChHodge(\mathcal H_k)=k\).
...
$\mathrm{Sh}_{0,n}$ reduces to the degree-$2$ shadow, which is
\(\kappaChHodge(\mathcal H_k)\) times the
sum of Arnold classes. Flatness $(\nabla_{0,n})^2 = 0$ follows from the MC equation
for $\Theta$, which at degree $2$ is the identity
\(d(\kappaChHodge(\mathcal H_k)\omega^{(2)}) = 0\) (the
Arnold class is closed).

(v) The degree-$r$ soft factor $S_r$ is the $r$-point shadow
$\mathrm{Sh}_{0,r}(\Theta)$. Since $\Theta$ has no component at degree $\geq 3$,
$S_r = 0$ for $r \geq 3$.
\end{proof}
...
\label{thm:class-l-reconstruction}
Let $\cA = \widehat{\mathfrak{g}}_k$ be the universal affine Kac--Moody algebra at
non-critical level $k \neq -h^\vee$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(r^\Theta_{\max}(\widehat{\mathfrak{g}}_k) = 3\) (Lie/tree support shadow depth).
\item The completed support packet is exhausted by degree~$3$. The jet data
 consists of:
 \begin{itemize}
 \item $\kappaChHodge(\widehat{\mathfrak{g}}_k) = \dim(\mathfrak{g})(k + h^\vee)/(2h^\vee)$,
...
enveloping vertex algebras (Vol~I, Proposition~\ref*{V1-prop:pbw-universality}). The
...
degree~$4$. For the universal affine algebra, the degree-$4$ cyclic cochains are
...
\begin{corollary}[Three data determine the affine scalar modular shadow series; \ClaimStatusProvedHere]
% label removed: cor:affine-three-data
The scalar modular shadow series of $\widehat{\mathfrak{g}}_k$ is determined by three
data: the level $k$, the dimension \(\dim(\mathfrak{g})\), and the structure constants
$f^c_{ab}$. The degree-$3$ jet
\[
J^3(\widehat{\mathfrak{g}}_k)
= \bigl(\kappaChHodge(\widehat{\mathfrak{g}}_k), \mathfrak{C}\bigr)
\]
is a complete invariant for the scalar modular problem on this reconstruction surface.
\end{corollary}
...
\label{thm:class-c-reconstruction}
Let $\cA = V_{\beta\gamma}(\lambda)$ be the $\beta\gamma$ system at conformal weight
$\lambda$. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(r^\Theta_{\max}(V_{\beta\gamma}) = 4\) (contact/quartic support shadow depth).
\item The completed support packet is exhausted by degree~$4$. The jet data consists of:
 \begin{itemize}
 \item $\kappaChHodge(V_{\beta\gamma}) = 6\lambda^2 - 6\lambda + 1$ (on the weight-changing
 line: \(\kappaChHodge(V_{\beta\gamma}) = 0\) at
...
$m_5 = 0$, the only source for $o_5$ is the interaction of lower-degree operations.
...
at degree~$5$. Therefore \(o_5=0\), and the canonical contact support
packet has no primitive degree-\(\ge5\) component on this branch.
\end{proof}
...
Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item \(r^\Theta_{\max}(\mathrm{Vir}_c) = \infty\) (mixed/infinite support shadow depth).
\item The shadow jet space has infinitely many nonzero degree pieces:
 $\dim \cA^{\mathrm{sh}}_{r,0} \geq 1$ for all $r \geq 2$.
\item The jet data through degree $8$ is:
\end{enumerate}
\end{theorem}

\begin{proof}[Proof of {\textup{(iii)}}]
The computation gives $J^r(\mathrm{Vir}_c)$ for $r = 2, \ldots, 8$.

\medskip\noindent\textbf{Degree $2$ ($J^2$).}
The curvature is $\kappaChHodge(\mathrm{Vir}_c) = c/2$. The quadratic jet is
one-dimensional:
\[
J^2(\mathrm{Vir}_c) =
\C \cdot \kappaChHodge(\mathrm{Vir}_c), \qquad
\kappaChHodge(\mathrm{Vir}_c) = c/2.
\]

\medskip\noindent\textbf{Degree $3$ ($J^3$).}
...
\[
J^4(\mathrm{Vir}_c) =
\C \cdot \kappaChHodge(\mathrm{Vir}_c)
\oplus \C \cdot \mathfrak{Q}^{\mathrm{contact}},
\qquad \dim J^4 = 2.
\]

\medskip\noindent\textbf{Degree $5$ ($J^5$).}
...
the Virasoro composite $:(L_{-2})^2 L:$ generates a nonvanishing degree-$5$ cyclic
trace. Therefore:
\[
\dim \cA^{\mathrm{sh}}_{5,0} \geq 1, \qquad o_5 \neq 0.
\]

\medskip\noindent\textbf{Degree $6$ ($J^6$).}
The degree-$6$ shadow receives contributions from two sources: trees with six
leaves decorated by $m_2$ and $m_3$, and the first planted-forest correction at
degree $6$. The dimension is $\dim \cA^{\mathrm{sh}}_{6,0} \geq 1$.

\medskip\noindent\textbf{Degree $7$ ($J^7$).}
Similar analysis: $\dim \cA^{\mathrm{sh}}_{7,0} \geq 1$. The degree-$7$ shadow
is the first level at which a genuinely new geometric stratum (the codimension-$5$
boundary of $\ov{\M}_{0,8}$) contributes.

\medskip\noindent\textbf{Degree $8$ ($J^8$).}
At degree $8$, the shadow has $\dim \cA^{\mathrm{sh}}_{8,0} \geq 2$: the
quartic-quartic interaction $\mathfrak{Q} \cdot \mathfrak{Q}$ generates a new
independent shadow class beyond the cascading contributions from lower degrees.
...
The nonvanishing of $o_5$ (proved above) implies
\(r^\Theta_{\max} \geq 5\). The mechanism
that forces $o_5 \neq 0$ persists at all higher degrees: the nonlinear OPE
$L(z) L(w) \sim (c/2)(z-w)^{-4} + 2L(w)(z-w)^{-2} + \partial L(w)(z-w)^{-1}$
generates nontrivial cyclic traces at every degree through the composite
$:L^n:$ which has nonvanishing self-contraction for all $n$. Therefore
$o_{r+1} \neq 0$ for all $r \geq 4$, and
\(r^\Theta_{\max} = \infty\).
\end{proof}

\subsection{Finite-jet scalar reconstruction}
% label removed: subsec:four-step-reconstruction

\begin{construction}[Finite-jet scalar reconstruction; \ClaimStatusProvedHere]
% label removed: constr:four-step-reconstruction
Given a chirally Koszul algebra $\cA$ with known support shadow depth
class (G, L, C, or M), the scalar modular shadow series is
reconstructed from the following
finite-jet datum.
(Algebras in class~W are not chirally Koszul and fall outside this
reconstruction; see Volume~I, Remark~\ref*{V1-rem:wild-quiver-boundary}.)

\medskip\noindent\textbf{Curvature.}
Extract $\kappaChHodge(\cA)$ from the cyclic trace of $m_2$ (the genus-$1$ BV operator).
This is the quadratic jet $J^2(\cA)$.

\medskip\noindent\textbf{Shadow depth.}
\begin{itemize}
\item If $m_n = 0$ for $n \geq 3$: class G (\(r^\Theta_{\max} = 2\)).
\item If $m_3 \neq 0$ but $m_n = 0$ for $n \geq 4$ and Jacobi holds: class L
 (\(r^\Theta_{\max} = 3\)).
\item If $m_4 \neq 0$ but $\mathfrak{C} = 0$ (gauge-trivial cubic) and $o_5 = 0$:
 class C (\(r^\Theta_{\max} = 4\)).
\item Otherwise: class M (\(r^\Theta_{\max} = \infty\)). The reconstruction requires the
 completed shadow reconstruction packet, including the lift coordinates.
\end{itemize}

\medskip\noindent\textbf{Finite nonlinear jet.}
...
\medskip\noindent\textbf{Universal scalar formula.}
For classes G, L, C: the scalar shadow series is determined by the finite jet
\(J^{r^\Theta_{\max}}(\cA)\) via the generating function \eqref{eq:fg-general} (for the
quadratic component) and explicit corrections from the nonlinear shadows.

For class M: the scalar shadow series requires the full inverse limit
$J^\infty(\cA) = \varprojlim J^r(\cA)$, which is an infinite-dimensional datum.
The completed shadow reconstruction packet provides a systematic approximation scheme: each truncation
\(\Theta^{\leq r}\) gives an approximation to
\(\mathcal Z^{\mathrm{sc}}_\cA\) that is exact through a computable
genus range.
\end{construction}

\begin{proposition}[Class G applied to Heisenberg]
% label removed: prop:class-g-heisenberg-applied
For $\cA = \mathcal{H}_k$, the curvature is
\(\kappaChHodge(\mathcal H_k)=k\) and the
shadow class is G. The universal scalar formula gives
\[
\mathcal Z^{\mathrm{sc}}_{\mathcal{H}_k}(\h)
=
k\,\h^{-2}\left(\frac{\h/2}{\sin(\h/2)}-1\right).
\]
All higher shadow data are zero. The scalar reconstruction is exact from one number.
\end{proposition}

\begin{proposition}[Class L applied to $\widehat{\mathfrak{sl}}_2$]
% label removed: prop:class-l-sl2-applied
For $\cA = \widehat{\mathfrak{sl}}_{2,k}$, the curvature is
\(\kappaChHodge(\widehat{\mathfrak{sl}}_{2,k})=3(k+2)/4\).
The cubic shadow is the cyclic trace of the
structure constants $f^c_{ab}$ of $\mathfrak{sl}_2$ (with
$f^{12}{}_3 = 1$, etc.), and the Jacobi identity gives $o_4 = 0$.
The universal formula applies with the cubic correction: the genus
expansion receives finite corrections from the degree-$3$ shadow
...
For $\cA = V_{\beta\gamma}(\lambda)$, the curvature is
\(\kappaChHodge(V_{\beta\gamma}(\lambda))
=6\lambda^2 - 6\lambda + 1\). The cubic shadow is
gauge-trivial, $\mathfrak{C}=0$, and
$\mathfrak{Q}^{\mathrm{contact}}$ is the quartic cyclic contraction;
the obstruction $o_5$ vanishes. The universal formula applies with
the quartic correction.
The finite datum
\((\kappaChHodge(V_{\beta\gamma}(\lambda)),
\mathfrak{Q}^{\mathrm{contact}})\) determines the scalar modular
shadow series on this reconstruction surface.
\end{proposition}

\begin{proposition}[Class M applied to Virasoro]
% label removed: prop:class-m-virasoro-applied
For $\cA = \mathrm{Vir}_c$, the curvature is
\(\kappaChHodge(\mathrm{Vir}_c)=c/2\) and the
nonvanishing obstruction $o_5$ places the algebra in class~M. The
universal formula requires the completed support packet: each finite truncation
$\Theta^{\leq r}$ determines the modular data to some finite genus range, but
no finite set of invariants suffices for the full scalar shadow series. The
...
\section{Soft theorems from the shadow obstruction tower}
% label removed: sec:soft-theorems-shadow-tower

\subsection{The shadow connection decomposed by degree}
% label removed: subsec:shadow-connection-degree
...
Its homogeneous decomposition produces the degree-by-degree soft
identities below.
...
\end{equation}
where $\mathrm{Sh}_{0,n}$ is the degree-$n$ genus-$0$ shadow extraction map
(Vol~I, Corollary~\ref*{V1-cor:shadow-extraction}). Decompose by degree:
\begin{equation}
% label removed: eq:shadow-connection-degree-decomposition
\mathrm{Sh}_{0,n}(\Theta_\cA)
\;=\;
\sum_{r=2}^{n} S_r(z_1, \ldots, z_n),
\end{equation}
where \(S_r\) is the degree-\(r\) algebraic support channel.  After a
celestial comparison datum is supplied, its image is the corresponding
soft operator if the comparison does not kill or identify that channel:
\begin{equation}
...
\begin{remark}[Degree-by-degree flatness]
% label removed: rem:degree-by-degree-flatness
Expanding the flatness equation \eqref{eq:flatness-equation} by degree gives a
hierarchy of identities. At the lowest degrees:
\begin{itemize}
\item \textbf{Degree $2$}: $dS_2 = 0$. The leading support channel is closed.
\item \textbf{Degree $3$}: $dS_3 + [S_2, S_2] = 0$. The cubic support channel is
 constrained by the quadratic self-interaction of $S_2$.
\item \textbf{Degree $4$}: $dS_4 + [S_2, S_3] + [S_3, S_2] + [S_2, S_2, S_2]_3 = 0$.
 The quartic support channel involves both the cubic shadow and the triple interaction
 of $S_2$.
\item \textbf{Degree $r$}: $dS_r + \sum_{j+k=r} [S_j, S_k] + \cdots = 0$. The degree-$r$
 extension problem is controlled by the lower-degree data, the obstruction
 class \(o_r\), and, when the obstruction vanishes, an \(H^1\)-lift
 coordinate.  The physical soft factor is the image of this algebraic
 channel under the chosen celestial comparison.
\end{itemize}
\end{remark}
...
The degree-$2$ soft factor is
\begin{equation}
% label removed: eq:s2-explicit
S_2(z_1, \ldots, z_n)
\;=\;
\kappaChHodge(\cA) \cdot \omega^{(2)}(z_1, \ldots, z_n),
\end{equation}
...
\begin{proof}
The degree-$2$ component of the MC element $\Theta_\cA$ is
$\Theta^{(0)}_2 = \kappa \cdot \eta$, where $\eta$ is the cyclic trace form.
The shadow extraction at degree $2$ on $\mathrm{Conf}_n(\C)$ maps $\eta$ to the
...
For affine algebras $\widehat{\mathfrak{g}}_k$, on the affine Kac--Moody
comparison surface the leading soft factor gives the KZ connection form:
\[
...
$i$-th and $j$-th factors. The identity $S_2 = \kappa \cdot \omega^{(2)}$
supplies the universal Arnold-form part of the leading soft factor; on the
affine Kac--Moody comparison surface, the shadow representation of
$\kappaChHodge(\widehat{\mathfrak{g}}_k)$ on the corresponding module package
specializes to the KZ connection form above.
The flatness $dS_2 = 0$ is the standard closure of the Arnold form, and the
KZ equation $\nabla^{\mathrm{KZ}} \Phi = 0$ is, on that same comparison
surface, the statement that the corresponding conformal blocks are
horizontal sections.
\end{remark}
...
The degree-$3$ soft factor $S_3$ satisfies the flatness identity
\begin{equation}
...
\end{enumerate}
In the celestial context, this supplies the Virasoro-side benchmark
comparison surface for the Cachazo--Strominger subleading soft
graviton package.
\end{theorem}
...
\omega_3(z_1, z_2, z_3),
\]
where $\omega_3$ is the degree-$3$ Arnold form on $\mathrm{Conf}_3(\C)$.
...
This is the benchmark celestial operator to be compared with the
gauge-trivial Virasoro cubic shadow on the corresponding
gravitational comparison surface.
\end{proof}
...
The degree-$4$ soft factor satisfies the flatness identity
\begin{equation}
\label{eq:s4-flatness}
...
\]
The triple self-interaction $[S_2, [S_2, S_2]]$ is the degree-$4$ component of
...
\begin{theorem}[Quintic support channel for Virasoro; \ClaimStatusProvedHere]
...
\]
Therefore the Virasoro support packet has no finite cutoff beginning at the
quintic witness.  A celestial soft theorem sees the corresponding degree-$5$
operator only after the chosen celestial comparison detects this support
channel.
\end{theorem}

\begin{proof}
The quintic obstruction is computed from the flatness equation at degree~$5$:
\[
dS_5 + [S_2, S_4] + [S_4, S_2] + [S_2, [S_2, S_3]]_{\text{degree } 5}
+ \frac{1}{24}[S_2, [S_2, [S_2, S_2]]] = 0.
\]
After gauge-trivializing $S_3$, the obstruction to solving for $S_5$ is the class
$[dS_4 \text{ component at degree } 5]$ in the relevant cohomology. For Virasoro,
...
curvature \(\kappaChHodge(\mathrm{Vir}_c)=c/2\) to produce a nontrivial
degree-$5$ cocycle.

Explicitly, the quintic source term includes the bracket
\([\kappaChHodge(\mathrm{Vir}_c) \cdot \omega^{(2)},
\mathfrak{Q}^{\mathrm{contact}} \cdot \omega^{(4)}]\),
...
the relevant cohomology group, which is one-dimensional at degree~$5$).
\end{proof}

\begin{corollary}[Class M has no finite celestial support cutoff; \ClaimStatusConditional; licensing tags \(\beta+\gamma\)]
% label removed: cor:infinite-tower-class-m
For every class~M algebra $\cA$ (i.e.\
\(r^\Theta_{\max}(\cA)=\infty\)), there is no finite arity bound on
the algebraic celestial support packet.  If a celestial comparison datum
\(\Xi_{\mathrm{cel}}\) is fixed and its comparison map detects infinitely
many of these support channels, then the resulting soft-operator package is
infinite.  The infinitude is not a consequence of support depth alone.
\end{corollary}

\begin{proof}
The equality \(r^\Theta_{\max}(\cA)=\infty\) says precisely that no finite
truncation \(\Theta_\cA^{\le R}\) exhausts the bar-intrinsic support
packet on the declared branch.  Projection to physical celestial soft
operators is an additional comparison map.  If this map detects infinitely
many support channels, their images form an infinite soft-operator package;
if it kills or identifies channels, support depth alone does not count the
physical operators.
\end{proof}

\subsection{Support termination for finite-depth classes}
% label removed: subsec:tower-termination-finite

\begin{proposition}[Class G: support packet terminates at \(S_2\); \ClaimStatusProvedHere]
...
\begin{proof}
All degree-$r$ MC components vanish for $r \geq 3$ because $m_n = 0$ for $n \geq 3$.
The displayed multivalued functions are exactly the abelian KZ flat sections on
the Heisenberg comparison surface.
\end{proof}

\begin{proposition}[Class L: support packet terminates at \(S_3\); \ClaimStatusProvedHere]
...
\]
Thus the affine support packet has no nonzero terms beyond the
leading KZ-form term and the cubic shadow:
$\nabla = d - \frac{1}{k+h^\vee}\sum_{i<j} \Omega_{ij} d\log(z_i - z_j)
- S_3(\widehat{\mathfrak{g}}_k)$.
On the affine Kac--Moody comparison surface, the leading degree-$2$
term is the KZ connection form, and the resulting finite-depth affine
support packet furnishes the bar-side comparison surface for the
corresponding Tsuchiya--Kanie conformal-block package.
\end{proposition}

\begin{proof}
The vanishing of $o_4$ (Theorem~\ref{thm:class-l-reconstruction}(iii)) implies
$S_4 = 0$. By induction, $S_r = 0$ for all $r \geq 4$: once the support packet has
no primitive component above degree~$3$,
the flatness identity at degree $r+1$ has source zero, so $S_{r+1}$ is both closed and
zero in the chirally Koszul quotient.
\end{proof}

\begin{proposition}[Class C: support packet terminates at \(S_4\); \ClaimStatusProvedHere]
...
support channels vanish.  A physical soft-factor statement requires the
celestial comparison datum as above.
\end{proof}
...
$\C\mathrm{P}^1_{\mathrm{cel}}$ as the corresponding celestial transport package.
On benchmark celestial comparison surfaces, this transfer is the
composition of Mellin transform, soft residue, and Ward identity.

\medskip\noindent\textbf{Mellin transform.}
On benchmark comparison surfaces, replace conformal-weight eigenstates
$\phi_\Delta(z)$ by their Mellin transforms:
\[
...
connection and passes to the corresponding celestial transform package.

\medskip\noindent\textbf{Soft residues.}
On a benchmark celestial comparison surface where the transformed package is
represented by benchmark correlators $\mathcal{M}_n$, the soft factor at
level $p$ is the residue of an $(n+1)$-point benchmark correlator package
when one external state has reduced conformal dimension $\Delta \to 1-p$:
\begin{equation}
% label removed: eq:celestial-soft-limit
\operatorname*{Res}_{\Delta=1-p}
\mathcal{M}_{n+1}(\Delta, z; \Delta_1, z_1; \ldots)
=
\sum_i
\rho_i\!\left(\pi_1D_\cA\big|_{(s^{-1}\cA)^{\otimes(p+2)}}\right)
\mathcal{M}_n(\Delta_1, z_1; \ldots),
\end{equation}
where $p+2$ is the arity of the corresponding transferred chiral operation.
For the gravitational stress-tensor normalisation this reduced pole is shifted
to $\Delta^{\mathrm{st}}=2-p$.

\medskip\noindent\textbf{Ward identity.}
The flatness $(\nabla^{\mathrm{hol}})^2 = 0$ yields the celestial Ward identity on
the transferred transport package; on those same benchmark comparison surfaces it
is written as:
\begin{equation}
...
\end{equation}
This is, on those benchmark comparison surfaces, the infinite hierarchy of soft
theorem consistency relations.
\end{construction}
...
\item The subleading soft algebra ($S_2 + S_3$) includes the Virasoro generators
 (stress tensor) for gravitational theories; on the corresponding
 celestial comparison surface, the Cachazo--Strominger subleading
 soft theorem is the benchmark operator package to be compared with
 this Virasoro-side subleading shadow data.
\item The sub-subleading comparison surface ($S_2 + S_3 + S_4$) includes
 the $\mathcal{W}_3$ generators for higher-spin theories: the quartic
 contact invariant $\mathfrak{Q}^{\mathrm{contact}}$ is the first
 higher-spin shadow class whose equality with a celestial soft
 coefficient is governed by the Mellin comparison cone.
\item For class~M theories, the shadow algebra controlling the associated
 graded of the soft sector is infinite-dimensional and is controlled by
 the inverse limit $J^\infty(\cA)$.
\end{itemize}
On the theorematic and benchmark comparison surfaces where the corresponding soft
operators are constructed, the shadow-depth language provides a
structural dictionary for the complexity of the soft algebra: class~G aligns
with finite abelian soft data, class~L with finite-dimensional Lie benchmark
soft data, class~C with finite-dimensional nonlinear soft data (with a quartic
correction), and class~M with an infinite-dimensional associated-graded
shadow algebra.
\end{remark}
...
calculus to degrees $3$, $4$, $5$ and beyond. The goal is to
analyze the sectorwise and low-mode structure of the completed one-wheel
representative $W_1(A_{\mathrm{BF}})$, the associated boundary modular class
on chosen regularization surfaces, and the resulting evidence for whether the
BF model lifts to all genera.
...
\label{prop:mixed-cubic-coefficients}
...
\begin{equation}
\label{eq:cubic-antisymmetry-verified}
...
$|\beta_1| = |\beta_2| = 1$ (the $C_\infty$ relation at degree~$3$). Translating to
...
(Theorem~\ref{thm:quartic-support-theorem-lowest-modes}). The computation gives the two charged
coefficients.
...
The pattern from degrees $2$ through $5$ reveals a rigid arithmetic structure:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item At each degree $n$, the number of nonvanishing low-mode channels is
...
\item The $SU(2)$ charge conjugation symmetry halves the number of independent
 coefficients at each degree.
\end{enumerate}
The generating function for the number of independent low-mode one-vertex coefficients
at degree $n$ is therefore bounded by
$\lceil \frac{1}{2}(\lfloor (n-1)^2/4 \rfloor + 1) \rceil$, which grows quadratically
in $n$: the low-mode sector remains finite-dimensional at every degree.
\end{remark}

\subsection{Completed one-wheel formal sum and regularized modular-class package for BF}
...
Fix a regularization scheme assigning regularized wheel weights $w_\Gamma$ for the
holomorphic BF boundary algebra whenever the completed one-wheel sum is defined on
the chosen surface. We then denote by
\begin{equation}
...
(Definition~\ref{def:one-wheel-sum-and-class}). On that same chosen regularization
surface, its cohomology class
$\mathfrak W(A_{\mathrm{BF}}) = [W_1(A_{\mathrm{BF}})]$ is the corresponding
boundary modular class. We decompose the representative by vertex number:
\begin{equation}
...
\begin{theorem}[One-vertex tadpole representative for BF; \ClaimStatusProvedHere]
% label removed: thm:one-vertex-w1-bf
The one-vertex contribution to the completed one-wheel representative is the
tadpole:
\begin{equation}
...
KK-completed deformation complex and requires regularization before any numerical
evaluation. No canonical regularized value is proved here.
\end{theorem}
...
product of two such sums gives the finite part $(-1/2)^2 = 1/4$ in that simple
factorized scheme.
...
is a canonical infinite formal sum that requires regularization before any
numerical evaluation, unlike the higher-vertex sectors.
\end{proof}

\begin{theorem}[Two-vertex sector of the BF one-wheel representative; \ClaimStatusProvedHere]
% label removed: thm:two-vertex-w1-bf
The two-vertex contribution to the completed one-wheel representative decomposes
by vertex types:
\begin{equation}
...
wheel), and $B_{n_1,n_2}$ is the wheel contraction using vertices of degrees $n_1$ and
...
Each vertex has degree $n_i$, where $n_i - 1$ legs are external and $1$ leg connects
to each of the two internal edges, plus possibly additional internal edges. For
the simplest case $n_1 = n_2 = 3$: each vertex has degree $3$, two legs are internal
(connecting to the loop edges), and one leg is external. But for the one-wheel sum,
we also have external legs from the cyclic trace, so the effective degree is $3$ with
...
\begin{computation}[Three-vertex sector of the BF one-wheel representative; \ClaimStatusProvedHere]
...
The simplest type has all vertices of degree~$2$ (binary), but since $m_2$ has no
$A^1 \to A^1$ component in the one-vertex trace, the first nonvanishing three-vertex
wheel has vertex degrees $(3, 2, 3)$ or $(3, 3, 2)$ (by cyclic symmetry of the wheel,
...
The three-vertex contribution to the lowest-mode completed one-wheel
representative is
\begin{equation}
...
\begin{theorem}[BF genus-$1$ obstruction on the loop-generated regularization surface; \ClaimStatusProvedHere]
...
\item The bulk BF theory is topological and has a well-defined partition function at
 ...
 satisfies $H^1(\Cone(F_\pi)) = H^2(\Cone(F_\pi)) = 0$, then the genus-by-genus
 boundary and bulk lift problems are equivalent up to gauge.
\item The boundary algebra $A_{\mathrm{BF}}$ is commutative on cohomology, so it lies
 near the free-commutative benchmark. The conditional Koszul route is the
 implication: if $A_{\mathrm{BF}}$ satisfies the chirally Koszul hypothesis of
 Corollary~\ref{cor:contracting-homotopy-koszul}, then the obstruction
 cohomology vanishes. Commutativity alone does not imply that hypothesis.
\item The finite-jet analysis shows that the charged modular data is concentrated
 in a rigid, finite-dimensional locus. The Wess--Zumino identities and
 stable-graph factorization define the low-mode obstruction equations on this
 locus; their common zero locus is the finite-jet vanishing locus.
\end{enumerate}
\end{remark}

\subsection{Topological Steinberg heuristic for BF}
...
The geometric route to Conjecture~\ref{conj:bf-modular-lifting} is the
following conditional statement: if the shifted symplectic structure on the
BF vacuum moduli is globally exact in the needed sense, and if that exact
primitive trivializes the associated Steinberg self-intersection compatibly
with the obstruction complex, then the modular lifting problem is acyclic.

\begin{proposition}[Exact BF symplectic structure under a global AKSZ primitive; \ClaimStatusProvedHere]
\label{prop:bf-exact-symplectic}
Assume the AKSZ one-form $\alpha=\langle B,\delta A\rangle$ descends to a
global $(-3)$-shifted primitive on $\Mvac(\mathrm{BF})$. Then the shifted
symplectic structure on $\Mvac(\mathrm{BF})$ is exact, $\omega=d\alpha$.
On this exact surface, the Steinberg object
$\Steinberg_b^{\mathrm{BF}}$ is the derived self-intersection of a
Lagrangian in an exact symplectic space.
\end{proposition}

\begin{remark}[Primitive computation]
...
\begin{proposition}[Exact Steinberg contraction under gauge-trivialization; \ClaimStatusProvedHere]
...
Assume in addition that the exact primitive and the shifted Darboux model can be
matched compatibly so that the induced Maurer--Cartan element of the
self-intersection is gauge-equivalent to zero in the completed governing
deformation complex. Then the derived self-intersection
...
The map $\varphi$ transports $\alpha$ to a primitive
...
is formally contractible onto $\mathcal{L}$ under the gauge-trivialized
exact-Steinberg hypotheses of Proposition~\ref{prop:exact-steinberg-contractible}.
\end{remark}

\begin{corollary}[Gauge-trivialized BF modular lifting package; \ClaimStatusProvedHere]
\label{cor:bf-modular-lifting-resolved}
Assume the gauge-trivialized exact-Steinberg hypotheses of
Proposition~\ref{prop:exact-steinberg-contractible} hold for each
genus-$g$ BF Steinberg and that the obstruction-deformation map
identifies $\mathfrak W_g$ with the corresponding Steinberg obstruction
class. Then the full obstruction tower $\{\mathfrak{W}_g\}_{g\ge 1}$ vanishes,
hence in particular $\mathfrak{W}(A_{\mathrm{BF}})=0$, together with a
full solution $\Pi(\h)$ of the modular Maurer--Cartan problem and
contractibility of each genus-$g$ Steinberg
$\Steinberg_b^{\mathrm{BF},g}$ for $g \geq 1$.
\end{corollary}

\begin{proof}
Proposition~\ref{prop:exact-steinberg-contractible} gives a contraction
of each gauge-trivialized genus-$g$ BF Steinberg onto the boundary
Lagrangian. The obstruction complex is therefore acyclic. Since the
obstruction-deformation map identifies $\mathfrak W_g$ with the
corresponding Steinberg obstruction class, every $\mathfrak W_g$
vanishes. The $\hbar$-adic obstruction tower then lifts inductively to
a solution $\Pi(\h)$ of the modular Maurer--Cartan equation, and the
Steinberg contractibility is the contraction just constructed.
\end{proof}

\begin{remark}[Exact-Steinberg obstruction class]
Under the gauge-trivialized exact-Steinberg hypotheses above, the $(-2)$-shifted
...
contractibility mechanism of Proposition~\ref{prop:exact-steinberg-contractible}
therefore applies at each genus, yielding
$\Steinberg_b^{\mathrm{BF},g} \simeq \mathcal{L}_b$ for all $g$.

The corresponding genus-$g$ obstruction complex is acyclic. Hence the
genus-by-genus obstruction--deformation sequence forces the genus-$g$
obstruction class $\mathfrak{W}_g$ to vanish for all $g \geq 1$.
Thus the full obstruction tower vanishes; in particular
$\mathfrak{W}(A_{\mathrm{BF}})=0$, and the full solution $\Pi(\h)$ exists by
inductive lifting under the stated exact-Steinberg hypotheses.
\end{remark}

\begin{remark}[BF exact-Steinberg obstruction]
% label removed: rem:bf-too-easy
The obstruction to applying Proposition~\ref{prop:exact-steinberg-contractible}
to BF is the gauge-trivialization class
\[
\Obs^{\mathrm{BF}}_{\mathrm{St},g}\in
H^2\!\left(\Cone\!\left(C^\bullet(\Steinberg_b^{\mathrm{BF},g})
\longrightarrow C^\bullet(\mathcal L_b)\right)\right).
\]
In the general HT framework, the
...
Steinberg acquires source terms for the higher $\mathfrak{W}_g$.
For the abelian BF benchmark, the topological character of the bulk
together with the compatible vanishing scalar genus-$1$ package gives
the exact-Steinberg comparison problem governed by
$\Obs^{\mathrm{BF}}_{\mathrm{St},g}$.
\end{remark}
...
\begin{theorem}[Higher-genus scalar compatibility for abelian BF; \ClaimStatusProvedHere]
\label{thm:bf-boundary-bulk-consistency}
Any lifted boundary modular package for $A_{\mathrm{BF}}$ that is compatible with
the external abelian BF bulk partition formula on $S^3$ must satisfy the
higher-genus scalar vanishing constraint
\begin{equation}
\label{eq:bf-consistency}
\sum_{g=1}^{\infty} \h^{2g-2} F_g^{\mathrm{bdry}}(A_{\mathrm{BF}}) = 0.
\end{equation}
In particular, every such compatible lifted boundary package has
$F_g^{\mathrm{bdry}} = 0$ for $g \geq 1$. This is consistent with the later
boundary-side analysis of Theorem~\ref{thm:bf-shadow-depth}, where the abelian BF
boundary algebra is shown to have commutative binary product together with the
same compatible scalar vanishing on the abelian benchmark surface.
\end{theorem}

\begin{proof}
By Proposition~\ref{prop:cs-bf-partition}, the external abelian BF bulk formula on
$S^3$ gives $Z_{\mathrm{BF}}(S^3)=1$, hence there is no nontrivial higher-genus scalar
contribution to match on the bulk side. Therefore any lifted boundary modular
package compatible with that external bulk formula must satisfy
\eqref{eq:bf-consistency}, and coefficientwise all
$F_g^{\mathrm{bdry}}$ vanish for $g\geq 1$.

The internal boundary analysis is compatible with this: later in
Theorem~\ref{thm:bf-shadow-depth} the abelian BF boundary algebra is shown to have
commutative binary product, and the compatible genus-$1$ scalar term again
vanishes on the abelian benchmark surface. No theorematic full higher-degree
shadow collapse is used here.
\end{proof}
...
correct comparison-surface reading is therefore that the non-abelian boundary theory
is expected to carry nontrivial modular data. What this section isolates is
the charged modular jet $\mathfrak{J}^{\mathrm{low}}_{\mathrm{ch}}$ of
Definition~\ref{def:low-mode-charged-modular-jet} as the low-mode benchmark packet
for that expected non-abelian modular information.

The interacting sector of the non-abelian boundary algebra has $m_n \neq 0$ for
$n \geq 3$ (from the non-abelian cubic vertex), so the non-abelian boundary theory
is no longer on the abelian commutative benchmark surface. What this section shows is
more limited: the lowest-mode charged jet carries nontrivial modular data, and the
standard class~L reconstruction gives the benchmark comparison model for how one
would organize that data if an affine-type comparison theorem were established for
the non-abelian BF boundary algebra. No such comparison theorem is proved here.
\end{remark}
...
internal-degree distributions.
\end{proposition}

\begin{proof}
Each vertex on the wheel has degree $n_i \geq 2$, with $2$ legs internal (connecting
...
partition factor $p(k)$ counts the unordered multisets of degrees $(n_1, \ldots, n_k)$,
...
is the analytic sewing condition of
(Vol~I, \S\ref*{V1-sec:concordance-analytic-sewing}).
\end{remark}

\begin{proposition}[Recursive determination of wheel amplitudes; \ClaimStatusProvedHere]
\label{prop:recursive-wheel-determination}
...
loop) with a free leg of degree $n$ at one end, and $\Tr_{\mathrm{loop}}$ closes the
...
a $(k-1)$-vertex open wheel $W_1^{(k-1,n)}$ with one additional vertex of degree $n$
...
countable algebraic system with finitely many coefficients at each fixed wheel order:
the unknowns are the stagewise amplitudes $W_1^{(k)}\big|_L$ for $k = 1, 2, 3, \ldots$,
and the constraints are the quadratic identities from the $D^2 = 0$ relation
restricted to the low-mode sector. The
compatibility condition of Theorem~\ref{thm:bf-boundary-bulk-consistency}, that any
boundary package compatible with the external abelian BF bulk formula has vanishing
higher-genus scalar free energies, supplies only the expected abelian scalar constraint on this
bootstrap system; it does not by itself solve the full low-mode resummation problem.
\end{remark}
...
with vertex degrees summing to $2 \cdot 6 + e$, where $e$ is the total number of
external legs. The minimal wheel has degrees $(2,2,2,2,2,2)$ (all binary), but
...
The first nonvanishing six-vertex wheel on the lowest modes has degrees
...
\]
for large even $k$. The one-wheel sum is absolutely convergent if all eigenvalues
satisfy $|\lambda_i| < 1$. Boundary cases on the unit circle require separate
conditional-convergence or regularization analysis. This is the spectral criterion
available here for the lowest-mode one-wheel representative.
\end{remark}

\begin{theorem}[Spectral criterion for absolute one-wheel convergence; \ClaimStatusProvedHere]
\label{thm:spectral-one-wheel-convergence}
Let $A$ be a cyclic minimal $A_\infty$ algebra, and let $L$ be a finite-dimensional
lowest-mode space on which the traced operator series
\begin{equation}
% label removed: eq:loop-transfer-operator
T := \sum_{n \geq 2} m_n^{\mathrm{traced}} : L \longrightarrow L,
\end{equation}
converges in $\End(L)$ to a well-defined linear operator. Here
$m_n^{\mathrm{traced}}$ is the partially traced operation obtained by
contracting one input and one output of $m_n$ using the cyclic pairing. Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The one-wheel sum converges absolutely on $L$ if and only if
 the spectral radius of $T$ satisfies $\rho(T) < 1$.
\item If $\rho(T) = 0$ (nilpotent transfer), the one-wheel sum is a finite sum.
\item If $\rho(T) > 1$, absolute convergence fails; if $1$ is an eigenvalue, the
 series diverges. Boundary cases with $|\lambda| = 1$ but $\lambda \neq 1$
 require separate conditional-convergence analysis or regularization.
\end{enumerate}
\end{theorem}

\begin{proof}
Under the convergence hypothesis, the loop transfer operator $T$ is a well-defined
endomorphism of the finite-dimensional space $L$. The $k$-vertex wheel amplitude on
$L$ is $\frac{1}{k} \Tr(T^k)$. The total
...
= -\log\det(\id - T).
\]
Absolute convergence of $\sum_k \Tr(T^k)/k$ is equivalent in finite dimension to
$\rho(T)<1$, i.e.\ all eigenvalues of $T$ lying strictly inside the unit disk.
If $T$ is nilpotent ($T^N = 0$ for some $N$), then the sum terminates at $k = N$,
and the result is a finite polynomial in the eigenvalues. If $\rho(T)>1$, absolute
convergence fails. If $1$ is an eigenvalue, the harmonic contribution along that
eigenspace forces divergence. Boundary cases with $|\lambda|=1$ but $\lambda\neq 1$
may converge only conditionally and are therefore outside the theorematic absolute-
convergence surface recorded here.
\end{proof}

\begin{corollary}[Determinant formula for the one-wheel representative; \ClaimStatusProvedHere]
...
satisfied, the completed one-wheel representative on the lowest modes has the closed form
\begin{equation}
...
and $\det(\id_L - T)$ is a polynomial of degree~$3$ in the matrix entries of the
restricted transfer operator $T|_L$.
\end{corollary}
...
$\dim L = 3$, so $\det(\id_L - T)$ is cubic in the entries of the $3\times 3$
matrix of $T|_L$.
\end{proof}
...
an ordinary polynomial. For the BF boundary algebra, the passage from this
finite-mode determinant to a genuine infinite-mode Fredholm determinant is
governed by the HS-sewing obstruction
\[
\Obs^{\mathrm{BF}}_{\mathrm{HS}}\in
H^2\!\bigl(\Cone(\mu^{\mathrm{BF}}_{\mathrm{HS}})\bigr).
\]
Vanishing of this class is exactly the condition that the relevant KK spectrum
satisfy the HS-sewing condition of Vol~I,
Theorem~\ref*{thm:general-hs-sewing}.
\end{remark}

\subsection{Abelian BF boundary benchmark}

\begin{theorem}[Commutative abelian BF benchmark and compatible scalar vanishing; \ClaimStatusProvedHere]
\label{thm:bf-shadow-depth}
The holomorphic BF boundary algebra $A_{\mathrm{BF}}$ on $S^3$ satisfies:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the binary product $m_2$ on CR cohomology is commutative;
\item on any lifted boundary modular package for $A_{\mathrm{BF}}$ compatible with the
 external abelian BF bulk formula on $S^3$, the universal genus-$1$ scalar term
 vanishes.
\end{enumerate}
This theorem records the commutative abelian benchmark together with the
boundary-side scalar vanishing statement. The full class~G collapse of the
higher-degree shadow obstruction tower is the comparison condition
$\Obs^{\mathrm{BF}}_{\mathrm{G}}=0$.
\end{theorem}
...
$\Omega_b^{0,\bullet}(S^3)$ is commutative). In particular the binary product
$m_2$ on CR cohomology is commutative.

By Theorem~\ref{thm:bf-boundary-bulk-consistency}, any lifted boundary modular
package compatible with the external abelian BF bulk formula on $S^3$ has
$F_g^{\mathrm{bdry}}=0$ for all $g \geq 1$. In particular the universal genus-$1$
scalar term vanishes on that compatible benchmark surface. The remaining
higher-degree shadow tower is controlled by the comparison class
$\Obs^{\mathrm{BF}}_{\mathrm{G}}$.
\end{proof}

\begin{remark}[Non-abelian BF class~L comparison]
% label removed: rem:non-abelian-promotion
For non-abelian BF with gauge group $G$, the interacting boundary theory has
charged shadow data. Its comparison with the affine class~L reconstruction
surface of Theorem~\ref{thm:class-l-reconstruction} is governed by the class
\[
\Obs^{\mathrm{BF}}_{\mathrm{L}}(G)\in
H^2\!\bigl(\Cone(\mu^{\mathrm{BF}}_{\mathrm{L}}(G))\bigr).
\]
On the vanishing locus of this class the affine curvature formula
$\kappaChHodge(\widehat{\fg}_k)=\dim(\fg)(k+h^\vee)/(2h^\vee)$ and the exact
class~L shadow-depth statement apply to $A_{\mathrm{BF},G}$.
...
representations of $G$. The shadow obstruction tower furnishes the comparison
map from the affine class~L scalar series to this partition formula; the exact
recovery is again equivalent to $\Obs^{\mathrm{BF}}_{\mathrm{L}}(G)=0$.
At genus~$1$: $Z_1(G) = |\mathrm{Irr}(G)|$ (the number of irreducible representations).
The affine class~L formula
$F_1(\cA)=\kappaChHodge(\cA)/24$ agrees with the genus-$1$
non-abelian BF computation exactly on the vanishing locus of the same
comparison class.
\end{remark}
...
\begin{theorem}[Shadow-depth comparison strata for the holographic atoms; \ClaimStatusProvedHere]
% label removed: thm:shadow-depth-holographic
The three holographic atoms of Section~\ref{sec:three-holographic-atoms} organize into
the following comparison strata:
\[
\renewcommand{\arraystretch}{1.3}
\begin{array}{c|c|c|c}
\text{Model} & \text{Boundary algebra} & \text{proved comparison datum} & \text{obstruction datum} \\
\hline
\text{Free } \beta\gamma
& V_{\beta\gamma}
& \(r^\Theta_{\max}=4,\ \kappaChHodge=6\lambda^2-6\lambda+1\)
& 0 \\
\text{Kodaira--Spencer}
& U_{\mathrm{KS}}
& \text{Gauss--Manin/open-closed comparison}
& \Obs^{\mathrm{cel}}_{\mathrm{KS}}\in H^2(\Cone(\mu_{\mathrm{KS}})) \\
\mathcal{W}_3
& \mathcal{W}_3^q(c_{\mathrm{eff}})
& \text{mixed/infinite } \mathcal{W}_3\text{ comparison}
& \Obs^{\mathrm{cel}}_{\mathcal W_3}\in H^2(\Cone(\mu_{\mathcal W_3}))
\end{array}
\]
The comparison data determine the reconstruction surface:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\beta\gamma$ (class~C): the finite contact packet determines
 the scalar modular shadow series.
\item Kodaira--Spencer: the Gauss--Manin/open-closed comparison becomes a class~L
 assignment for $U_{\mathrm{KS}}$ exactly on the vanishing locus of
 $\Obs^{\mathrm{cel}}_{\mathrm{KS}}$.
\item $\mathcal{W}_3$: the mixed/infinite higher-spin comparison becomes an exact
 support-depth theorem with the stated \(\kappaChHodge\)-line exactly on the
 vanishing locus of $\Obs^{\mathrm{cel}}_{\mathcal W_3}$.
\end{enumerate}
\end{theorem}

\begin{proof}
The $\beta\gamma$ classification follows from Theorem~\ref{thm:class-c-reconstruction}.
For Kodaira--Spencer, the universal defect algebra $U_{\mathrm{KS}}$
and the open-closed Gauss--Manin datum define a comparison map
$\mu_{\mathrm{KS}}$ from the celestial shadow connection to the class~L
reconstruction surface. Standard cone obstruction theory gives the
class $\Obs^{\mathrm{cel}}_{\mathrm{KS}}\in H^2(\Cone(\mu_{\mathrm{KS}}))$;
vanishing is precisely the condition that the comparison lift to a
literal class~L shadow-depth assignment.

For $\mathcal{W}_3$, the higher-spin celestial reduction defines a
comparison map $\mu_{\mathcal W_3}$ from the mixed $\mathcal W_3$
shadow tower to the infinite class~M surface. Its obstruction class
$\Obs^{\mathrm{cel}}_{\mathcal W_3}\in H^2(\Cone(\mu_{\mathcal W_3}))$
is exactly the lifting obstruction for the infinite shadow-depth
statement and for the central scalar $\kappa=c_{\mathrm{eff}}/2$.
\end{proof}

\begin{remark}[Holographic complexity hierarchy]
% label removed: rem:holographic-complexity-hierarchy
The support-depth classification produces a natural complexity hierarchy for holographic
models:
\[
\text{G (free)} \subset \text{L (current algebra)} \subset \text{C (contact)}
\subset \text{M (mixed/infinite)}
\quad\bigl|\quad
\text{W (wild, non-Koszul)}.
\]
Free holographic models (such as abelian BF) provide the commutative
vanishing-curvature benchmark on this celestial surface.
Contact models (such as $\beta\gamma$ holography) have finite modular content with an
additional quartic invariant. Kodaira--Spencer contributes the Gauss--Manin
and open-closed comparison controlled by
$\Obs^{\mathrm{cel}}_{\mathrm{KS}}$. Mixed models such as the
$\mathcal{W}_3$ comparison require the full shadow obstruction tower,
with exactness controlled by $\Obs^{\mathrm{cel}}_{\mathcal W_3}$.

This hierarchy parallels the physical distinction between free, gauge, and
gravitational holographic sectors: free sectors are Gaussian, gauge/open-closed
benchmark sectors interpolate toward Lie-theoretic structure, and gravitational
higher-spin sectors are expected to have no finite algebraic support cutoff.
The support-depth language quantifies this distinction only on the theorematic
surfaces where that depth has been proved; it does not count physical soft
operators without the celestial comparison datum.
\end{remark}

\subsection{The modular $R$-matrix for BF}
% label removed: subsec:modular-r-matrix-bf

\begin{definition}[Genus-zero BF benchmark kernel]
% label removed: def:genus-zero-r-matrix-bf
Let $k_{\mathrm{BF}}$ denote the BF coupling. Denote by
\begin{equation}
% label removed: eq:r-matrix-bf
r_{\mathrm{BF}}(z) = \frac{k_{\mathrm{BF}}\,\Omega}{z},
\end{equation}
where $\Omega = \sum_{p,q \geq 0} a_{p,q} \otimes b_{p,q}$ is the coevaluation
element of the cyclic pairing. On the abelian BF benchmark surface, $\Omega$
acts as the identity operator on the polynomial ring $\C[w_1, w_2]$. At the
abelian limit $k_{\mathrm{BF}} = 0$ the kernel $r_{\mathrm{BF}}(z)$ vanishes
identically, providing the vanishing check.
\end{definition}

\begin{proposition}[Abelian BF benchmark: trivial CYBE surface; \ClaimStatusProvedHere]
% label removed: prop:cybe-bf
On the abelian BF benchmark surface, the genus-zero kernel
$r_{\mathrm{BF}}(z)=k_{\mathrm{BF}}\,\Omega/z$ satisfies the classical Yang--Baxter equation
\begin{equation}
...
\begin{proof}
For abelian BF, the underlying Lie bracket is trivial, so the pairwise commutators
$[r_{12},r_{13}]$, $[r_{12},r_{23}]$, and $[r_{13},r_{23}]$ all vanish. Hence the
CYBE holds identically on this benchmark surface.
\end{proof}

\begin{theorem}[Compatible modular $R$-matrix benchmark for abelian BF; \ClaimStatusProvedHere]
% label removed: thm:modular-r-matrix-bf
For the abelian holomorphic BF model on $S^3$, on any lifted boundary modular
package compatible with the external abelian BF bulk formula, the modular
$R$-matrix benchmark has no higher-genus corrections:
\begin{equation}
% label removed: eq:modular-r-matrix-bf
R_{\mathrm{BF}}^{\mathrm{mod}}(z; \h) = r_{\mathrm{BF}}(z) = \frac{k_{\mathrm{BF}}\,\Omega}{z}.
\end{equation}
All genus corrections vanish: $r_g(z) = 0$ for $g \geq 1$. This is consistent with
the commutative abelian benchmark and compatible scalar vanishing recorded above.
\end{theorem}

\begin{proof}
The genus-$g$ correction $r_g(z)$ to the $R$-matrix is the degree-$2$ component of
$\Theta_\cA^{(g)}$, which is $\kappa \cdot \eta \otimes \Lambda_g$ by the universal
formula. On any compatible lifted boundary package,
Theorem~\ref{thm:bf-shadow-depth} gives vanishing genus-$1$ scalar term; by the
universal relation $F_1(\cA)=\kappaChHodge(\cA)/24$, the corresponding scalar curvature
parameter on that comparison surface is therefore $\kappa=0$. Substituting this
into the universal degree-$2$ genus-correction formula shows that all genus
corrections vanish there, so the modular $R$-matrix benchmark reduces to the
classical kernel.
\end{proof}

\begin{remark}[Non-abelian modular $R$-matrix comparison]
% label removed: rem:non-abelian-modular-r
For non-abelian BF with gauge group $G$ at coupling $k_{\mathrm{BF}}$,
the higher-genus comparison with the affine class~L surface is governed by
the cone classes
\[
\Obs^{\mathrm{BF}}_{G,g}\in H^2(\Cone(\mu^{\mathrm{BF}}_{G,g})).
\]
On the locus where these classes vanish, the modular $R$-matrix has
nontrivial genus corrections of the form
\[
R^{\mathrm{mod}}_{G}(z; \h) = \frac{k_{\mathrm{BF}}\,\Omega_G}{z} + \sum_{g \geq 1} \h^{2g}\,r_g(z),
\]
with genus corrections controlled by the same scalar generating series that
governs the class~L shadow package:
\[
R^{\mathrm{mod}}_G(z; \h)
= \left(1 + \sum_{g \geq 1} \h^{2g}\, \kappaChHodge(G) \Lambda_g\right)
\cdot \frac{k_{\mathrm{BF}}\,\Omega_G}{z}.
\]
At $k_{\mathrm{BF}} = 0$ the leading kernel vanishes, providing the vanishing check.
\end{remark}
...
\begin{enumerate}[label=\textup{(\arabic*)}]
\item The abelian BF model has commutative boundary product, and any
 lifted boundary package compatible with the external abelian bulk formula has
 vanishing higher-genus scalar free energies. The exact class~G /
 trivial-shadow-tower identification is the vanishing condition
 $\Obs^{\mathrm{BF}}_{\mathrm{G}}=0$ for the corresponding comparison cone.
\item The non-abelian BF model has a genuine interacting sector beyond the abelian
 commutative benchmark surface. Its low-mode charged modular data and the
 corresponding affine class~L benchmark comparison package give the finite
 comparison surface; the
 full non-abelian BF boundary algebra satisfies the affine class~L reconstruction
 theorem exactly on the vanishing locus of
 $\Obs^{\mathrm{BF}}_{\mathrm{L}}$.
\item The one-vertex calculus (degrees $2$--$5$) produces a rigid support structure on
...
\item The absolute convergence of the lowest-mode one-wheel representative is controlled
 by the spectral radius of the loop transfer operator on its theorematic surface,
 leading there to a determinant formula for the representative rather than the
 cohomology class.
\item The holographic-atom comparison surface consists of a proved class~C
 $\beta\gamma$ benchmark, a Kodaira--Spencer Gauss--Manin/open-closed benchmark,
 and a mixed/infinite $\mathcal{W}_3$ benchmark whose exact celestial
 coefficient comparison is governed by the Mellin comparison cone. It gives the
 free/gauge-benchmark/gravitational-benchmark complexity hierarchy on this
 celestial surface; the exact shadow-depth classification of all three atoms is
 the simultaneous vanishing of the displayed comparison classes.
\end{enumerate}
Within the modular Koszul framework, the charged modular jet
$\mathfrak{J}^{\mathrm{low}}_{\mathrm{ch}} =
(Q_{110}, Q_{011}, B_{\mathrm{mix}})$ of Definition~\ref{def:low-mode-charged-modular-jet}
is the first nonlinear invariant of the boundary algebra, and its vanishing on the
abelian BF benchmark surface is the
boundary-side vanishing low-mode benchmark compatible with the external bulk
partition formula $Z_{\mathrm{BF}}(S^3) = 1$. The exact class~G and
bulk-boundary shadow identifications are the vanishing condition for
$\Obs^{\mathrm{BF}}_{\mathrm{G}}$.
\end{remark}

\section{Synthesis}
\label{sec:thqgche-supplement}

\begin{remark}[Celestial holography at K3: universal $\Phi$-trace]
\label{rem:thqgche-phi-trace}
\index{central charge!$c_{2d}=-214$!K3 celestial holography}
On the $K3\times E$ input the chiral conductor identity
\[
K(\cA)=\mathrm{tr}_{\mathfrak Z(\cA)}(\mathfrak K_\cA)
\]
intertwines under the Borcherds singular-theta pushforward with
\[
...
=5.
\]
This is the Borcherds-weight scalar of the $\Delta_5$ denominator,
not the class-$\mathcal S$ central charge. The class-$\mathcal S$
$A_1$ surface $\Sigma_{0,24}$ with maximal punctures supplies the
Beem--Rastelli celestial-boundary vertex algebra
\[
\chi[\mathcal T[A_1,\Sigma_{0,24}]],
\qquad
c_{4d}=\frac{5\cdot 24-13}{6}=\frac{107}{6},
\qquad
c_{2d}=-12c_{4d}=-214,
\]
using the Shapere--Tachikawa normalization and the Beem--Rastelli
$c_{2d}=-12c_{4d}$ convention. The $M_{24}$ action and Steiner/Golay
incidence data enter as equivariance of the K3 elliptic-genus
coefficient system; they are not identified term-by-term with
celestial soft coefficients. The exact coefficient comparison is the
vanishing condition for the cone class
\[
\Obs^{K3}_{\mathrm{Steiner}}
\in
H^2\!\left(
\Cone\!\left(
\gr_{\mathrm{Steiner}}\mathrm{Soft}^{\mathrm{cel}}_{K3}
\longrightarrow
\gr_{\mathrm{Steiner}}\mathrm{Jac}_{0,1}^{M_{24}}
\right)\right).
\]
\end{remark}
