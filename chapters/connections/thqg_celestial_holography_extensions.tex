...
\section{The modular bootstrap equation}
\label{sec:modular-bootstrap-equation}

\subsection{The genus-graded MC recursion}
\label{subsec:genus-graded-mc-recursion}
...
\label{def:modular-partition-function}
...
(Vol~I, Theorem~\ref*{thm:mc2-bar-intrinsic}).  The \emph{modular partition function}
is the formal power series
\begin{equation}
\label{eq:modular-partition-function-def}
Z_\cA(q,\tau)
\;:=\;
...
\label{rem:theta-notation-convention}
Throughout this section, $\Theta_\cA$ denotes the bar-intrinsic MC element
of Vol~I, Theorem~\ref*{thm:mc2-bar-intrinsic}, which is a formal
...
\label{rem:three-geometric-operations}
...
\label{thm:mc-recursion-from-d-squared}
...
(Vol~I, Theorem~\ref*{thm:mc2-bar-intrinsic}: $\Theta_\cA := D_\cA - d_0$
...
$G \geq 1$ (Vol~I, Theorem~\ref*{thm:koszul-equivalences-meta}, condition~(ix):
FH concentration).  In particular, the contracting homotopy
\begin{equation}
\label{eq:contracting-homotopy}
...
\label{subsec:low-genus-amplitudes}
...
\label{rem:hodge-class-normalization-genus1}
...
\label{rem:free-energy-asymptotics}
...
\label{subsec:genus-spectral-sequence-bootstrap}

\begin{construction}[Genus spectral sequence; \ClaimStatusProvedHere]
\label{constr:genus-spectral-sequence-bootstrap}
...
\label{thm:spectral-sequence-convergence}
...
\label{prop:spectral-shadow-depth}
...
\section{Holographic reconstruction from boundary data}
\label{sec:holographic-reconstruction-boundary}

\subsection{The shadow jet space}
\label{subsec:shadow-jet-space}
...
\label{thm:finite-jet-shadow-depth}
The following are equivalent:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\label{rem:three-uses-of-shadow}
...
\label{subsec:class-g-reconstruction}

\begin{theorem}[Class G reconstruction; \ClaimStatusProvedHere]
\label{thm:class-g-reconstruction}
...
\item A single datum---the level $k$---determines the entire modular partition function:
  \begin{equation}
  \label{eq:heisenberg-full-reconstruction}
  Z_{\mathcal{H}_k}(\h)
...
\label{subsec:class-l-reconstruction}
...
\label{subsec:class-c-reconstruction}
...
contact invariant vanishing theorem (Vol~I, Corollary~\ref*{cor:nms-betagamma-mu-vanishing}).
...
\label{rem:betagamma-cubic-gauge}
...
\label{subsec:class-m-reconstruction}

\begin{theorem}[Class M reconstruction; \ClaimStatusProvedHere]
\label{thm:class-m-reconstruction}
...
\label{subsec:four-step-reconstruction}

\begin{construction}[Four-step reconstruction; \ClaimStatusProvedHere]
\label{constr:four-step-reconstruction}
...
\label{prop:class-g-heisenberg-applied}
...
\label{prop:class-l-sl2-applied}
...
\label{prop:class-c-betagamma-applied}
...
\label{prop:class-m-virasoro-applied}
...
\section{Soft theorems from the shadow tower}
\label{sec:soft-theorems-shadow-tower}

\subsection{The shadow connection decomposed by arity}
\label{subsec:shadow-connection-arity}
...
\label{rem:arity-by-arity-flatness}
...
\label{subsec:leading-soft-weinberg}

\begin{theorem}[Leading soft theorem; \ClaimStatusProvedHere]
\label{thm:leading-soft-theorem}
...
extraction map (Vol~I, Corollary~\ref*{cor:shadow-extraction}).
\end{proof}

\begin{remark}[KZ connection]
\label{rem:kz-connection}
...
\label{subsec:sub-subleading-soft}

\begin{theorem}[Sub-subleading soft theorem; \ClaimStatusProvedHere]
\label{thm:sub-subleading-soft-theorem}
...
\label{rem:quartic-contact-poles}
...
\label{subsec:higher-soft-theorems}

\begin{theorem}[Quintic obstruction for Virasoro; \ClaimStatusProvedHere]
\label{thm:quintic-obstruction-virasoro}
...
\label{cor:infinite-tower-class-m}
...
\label{prop:class-l-termination}
...
\label{prop:class-c-termination}
...
\label{rem:soft-algebra-null-infinity}
...
\label{subsec:bf-boundary-review}
...
\label{comp:quartic-contact-residue}
...
\label{prop:su2-charge-conjugation}
...
\label{subsec:five-vertex-sector}

\begin{theorem}[Quintic support on lowest modes; \ClaimStatusProvedHere]
\label{thm:quintic-support-lowest-modes}
...
\label{comp:quintic-internal-modes}
...
\label{subsec:full-modular-class-bf}

\begin{definition}[Completed one-wheel sum for BF]
\label{def:completed-one-wheel-bf}
...
\begin{equation}
\label{eq:w1-vertex-decomposition}
W_1(A_{\mathrm{BF}})
...
\label{subsec:bf-obstruction-tower}

\begin{theorem}[BF obstruction analysis; \ClaimStatusProvedHere]
\label{thm:bf-obstruction-analysis}
...
\label{subsec:bf-triviality-steinberg}
...
\label{subsec:bf-partition-comparison}
...
\begin{equation}
\label{eq:bf-partition-function}
...
the trivial partition function: the Gaussian shadow depth is $r_{\max} = 2$ with
$\kappa = 0$, so the entire shadow tower vanishes.
\end{proof}

\begin{remark}[Non-abelian BF and the interacting sector]
\label{rem:non-abelian-bf}
...
\label{subsec:higher-vertex-recursive}

\begin{proposition}[Wheel vertex growth; \ClaimStatusProvedHere]
\label{prop:wheel-vertex-growth}
...
\label{rem:one-wheel-convergence}
...
\label{rem:finiteness-low-mode-problem}
...
\label{subsec:six-vertex-sector}

\begin{computation}[Six-vertex wheels; \ClaimStatusProvedHere]
\label{comp:six-vertex-wheels}
...
\label{rem:symmetric-function-structure}
...
$L$, defined by
\begin{equation}
\label{eq:loop-transfer-operator}
...
\label{cor:determinant-modular-class}
...
\label{rem:fredholm-determinant-connection}
...
Since $\kappa = 0$, the shadow tower is identically zero by the universal formula
\eqref{eq:fg-general}: $F_g = \kappa \cdot c_g = 0$ for all $g$.  The higher-arity
shadows inherit this vanishing through the MC recursion: with $\Theta^{(1)} = 0$
(because $\kappa = 0$), all subsequent genus corrections vanish, and hence all
arity-$r$ shadows vanish.
\end{proof}

\begin{remark}[Non-abelian promotion]
\label{rem:non-abelian-promotion}
...
\label{rem:holographic-complexity-hierarchy}
...
\label{rem:non-abelian-modular-r}
...
\label{subsec:bf-synthesis}

\begin{remark}[Summary of BF computations]
\label{rem:bf-summary}
