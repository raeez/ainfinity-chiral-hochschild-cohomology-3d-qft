...
\begin{equation}
% label removed: eq:f2
F_2(\cA) = \kappa(\cA) \cdot \frac{7}{5760}.
\end{equation}
\end{computation}
...
\]
Therefore $F_1 = \kappa/24$ and $F_2 = 7\kappa/5760$.
...
$F_g(\cA) \sim 2\kappa(\cA)/(2\pi)^{2g}$: the Bernoulli factorial
...
sequence can still be supported on the degree-$0$ line.  What the shadow depth
measures is the amount of shadow data needed on that line: classes~G/L/C are
controlled by finite jets, while class~M carries an infinite shadow obstruction tower on the
same $q=0$ reconstruction surface.
\end{remark}

\begin{proposition}[Finite shadow depth controls the $q=0$ reconstruction; \ClaimStatusProvedHere]
% label removed: prop:spectral-shadow-depth
Assume $\cA$ is chirally Koszul and the shadow obstruction tower stabilizes at arity
$r_{\max}$.  Then:
\begin{enumerate}[label=\textup{(\roman*)}]
\item the genus spectral sequence is supported on the $q=0$ line and has
  $d_s = 0$ for all $s \geq 2$;
\item no shadow datum of arity $> r_{\max}$ enters the $q=0$ reconstruction;
\item equivalently, the genus-$g$ scalar package is determined for all $g$ by the
  finite jet $J^{r_{\max}}(\cA)$ together with the BV differential $d_1$ on the
  degree-$0$ line.
\end{enumerate}
\end{proposition}

\begin{proof}
Part \textup{(i)} is exactly the previous theorem.  If the shadow obstruction tower
stabilizes at arity $r_{\max}$, then $\Theta_\cA^{\leq r_{\max}} = \Theta_\cA$ and
all shadow terms of arity $> r_{\max}$ vanish.  Hence only the finite jet
$J^{r_{\max}}(\cA)$ contributes to the $d_1$-controlled reconstruction on the
...
The shadow obstruction tower (Vol~I, \S\ref*{sec:shadow-postnikov-tower}) encodes the nonlinear
...
(iii) Since the shadow obstruction tower terminates at arity $2$, the full MC element is
...
The shadow obstruction tower provides a systematic approximation scheme: each truncation
...
$\kappa(\widehat{\mathfrak{g}}_k)$ on the corresponding module package
...
\]
Thus the affine soft tower has no nonzero terms beyond the
leading KZ-form term and the cubic shadow:
$\nabla = d - \frac{1}{k+h^\vee}\sum_{i<j} \Omega_{ij} d\log(z_i - z_j)
- S_3(\widehat{\mathfrak{g}}_k)$.
On the affine Kac--Moody comparison surface, the leading arity-$2$
term is the KZ connection form, and the resulting finite-depth affine
shadow obstruction tower furnishes the bar-side comparison surface for the
...
On benchmark celestial comparison surfaces, this transfer is written by the
following procedure.
...
represented by benchmark correlators $\mathcal{M}_n$, the soft factor $S_r$ at
arity $r$ governs the behavior of an $(n+1)$-point benchmark correlator package
when one external state has conformal dimension $\Delta \to 1$:
\begin{equation}
...
\end{equation}
where the right-hand side is the soft factor acting on the remaining $n$-point
benchmark correlator package.
...
operators are actually constructed, the shadow-depth language provides a
bookkeeping dictionary for the complexity of the soft algebra: class~G aligns
...
correction), and class~M with an expected infinite-dimensional soft algebra.
\end{remark}
...
in $n$: the low-mode sector remains finite-dimensional at every arity.
\end{remark}
...
\item The boundary algebra $A_{\mathrm{BF}}$ is commutative on cohomology, so it lies
  near the free-commutative benchmark where a Koszul vanishing route would be
  available.  However, the chirally Koszul hypothesis needed for
  Corollary~\ref{cor:contracting-homotopy-koszul} is not proved here for
  $A_{\mathrm{BF}}$.  Thus the Koszul argument is only a conditional route to
  vanishing of the obstruction cohomology, not a theorematic consequence of
  commutativity alone.
\item The finite-jet analysis shows that the charged modular data is concentrated
  ...
  Wess--Zumino identities and stable-graph factorization are highly restrictive,
  which suggests a plausible low-mode vanishing mechanism but does not by itself
  prove vanishing.
\end{enumerate}
\end{remark}
...
self-intersection becomes exact in the governing deformation complex.
Then one expects the derived self-intersection
$\Steinberg = \mathcal{L} \times_X \mathcal{L}$ to be formally equivalent
...
Under the exact-Steinberg heuristic above, one would obtain
vanishing of the full obstruction tower $\{\mathfrak{W}_g\}_{g\ge 1}$,
...
A contractible Steinberg would suggest that the corresponding genus-$g$
obstruction complex is acyclic, hence that the relevant obstruction class
should vanish.  On that heuristic reading, the genus-by-genus
obstruction--deformation sequence would force the genus-$g$
...
surface.  That expectation is suggestive, but it is not turned into a
...
vanishes on the abelian benchmark surface.  No theorematic full higher-arity
...
is expected to carry nontrivial modular data.  What this section actually isolates is
...
is no longer on the abelian commutative benchmark surface.  What this section actually shows is
...
finite-mode determinant to a genuine infinite-mode Fredholm determinant would
require establishing the HS-sewing condition for the relevant KK spectrum
(Vol~I, Theorem~\ref*{thm:general-hs-sewing}), which is not proved here.
\end{remark}
...
strongest scalar vanishing currently justified on the boundary side.  It does
not by itself prove a full class~G collapse of the higher-arity shadow obstruction tower.
\end{theorem}
...
scalar term vanishes on that compatible benchmark surface.  No stronger statement
about the full higher-arity shadow obstruction tower follows from these inputs alone.
\end{proof}

\begin{remark}[Non-abelian promotion]
% label removed: rem:non-abelian-promotion
For non-abelian BF with gauge group $G$, one expects a nontrivial interacting
boundary theory with nonzero charged shadow data.  What is not proved in this
chapter is a literal identification of the non-abelian BF boundary algebra with
the affine class~L reconstruction surface of
Theorem~\ref{thm:class-l-reconstruction}; in particular, the affine curvature
formula $\kappa(\widehat{\fg}_k)=\dim(\fg)(k+h^\vee)/(2h^\vee)$ and the exact
class~L shadow-depth statement are not established here for $A_{\mathrm{BF},G}$.
...
representations of $G$.  The shadow obstruction tower is therefore expected to furnish a
benchmark comparison package for this formula, but that recovery is not proved in
this section.
At genus~$1$: $Z_1(G) = |\mathrm{Irr}(G)|$ (the number of irreducible representations).
The affine class~L formula $F_1=\kappa/24$ should therefore be read here only as
the benchmark comparison model for the non-abelian BF interacting sector, not as a
theorematic genus-$1$ computation for $A_{\mathrm{BF},G}$ itself.
\end{remark}

\subsection{The shadow-tower stratification of holographic models}
% label removed: subsec:shadow-stratification-holographic

\begin{theorem}[Shadow-depth and status packet for the holographic atoms; \ClaimStatusProvedHere]
...
the following theorematic/comparison packet:
\[
\renewcommand{\arraystretch}{1.3}
\begin{array}{c|c|c|c|c}
\text{Model} & \text{Boundary algebra} & \text{Current surface} & r_{\max} & \kappa \\
\hline
\text{Free } \beta\gamma & V_{\beta\gamma} & \text{C} & 4 & 6\lambda^2 - 6\lambda + 1 \\
\text{Kodaira--Spencer} & U_{\mathrm{KS}} & \text{Gauss--Manin/open-closed benchmark} & \text{comparison only} & \text{comparison only} \\
\mathcal{W}_3 & \mathcal{W}_3^q(c_{\mathrm{eff}}) & \text{expected mixed/infinite envelope} & \text{expected } \infty & \text{expected } c_{\mathrm{eff}}/2
\end{array}
\]
The packet determines the current reconstruction status:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\beta\gamma$ (class~C): four data determine the full modular partition function.
\item Kodaira--Spencer: the manuscript currently provides the Gauss--Manin/open-closed
  benchmark package, not a proved class assignment for the universal defect algebra
  $U_{\mathrm{KS}}$.
\item $\mathcal{W}_3$: the mixed/infinite higher-spin envelope is presently an expected
  modular package, not a proved shadow-depth theorem on this celestial surface.
\end{enumerate}
\end{theorem}

\begin{proof}
The $\beta\gamma$ classification follows from Theorem~\ref{thm:class-c-reconstruction}.
For Kodaira--Spencer, the celestial chapter imports the universal defect algebra
$U_{\mathrm{KS}}$ and the open-closed benchmark package from the Kodaira--Spencer
holography discussion, while the HT/BBL chapter gives the Gaussian
Gauss--Manin/shadow-connection datum on its separate theorematic surface.  What is
not proved here is a literal shadow-depth identification of the celestial
large-$N$ defect algebra $U_{\mathrm{KS}}$ with a class~L reconstruction surface.

For $\mathcal{W}_3$, the underlying celestial source chapter only constructs the
expected modular envelope on a conjectural slab-reduction surface.  The mixed/infinite
designation therefore remains an expected higher-spin package rather than a proved
classification theorem in this file.
\end{proof}
...
additional quartic invariant.  Kodaira--Spencer currently contributes a Gauss--Manin
and open-closed benchmark packet rather than a proved current-algebra shadow-depth
classification on this celestial surface.  Mixed models (such as the expected
$\mathcal{W}_3$ holography package) are anticipated to require the full machinery of
...
higher-spin sectors are expected to be infinite-tower.  The shadow depth language
quantifies this distinction only on the theorematic surfaces where that depth has
...
acts as the identity operator on the polynomial ring $\C[w_1, w_2]$.
\end{definition}
...
$r_{\mathrm{BF}}(z)=\Omega/z$ satisfies the classical Yang--Baxter equation
\begin{equation}
...
universal relation $F_1(\cA)=\kappa(\cA)/24$, the corresponding scalar curvature
parameter on that comparison surface is therefore $\kappa=0$.  Substituting this
into the universal arity-$2$ genus-correction formula shows that all genus
...
For non-abelian BF with gauge group $G$, one expects a modular $R$-matrix with
nontrivial genus corrections:
\[
R^{\mathrm{mod}}_{G}(z; \h) = \frac{\Omega_G}{z} + \sum_{g \geq 1} \h^{2g}\,r_g(z),
\]
but this chapter does not prove an exact formula for the higher-genus coefficients
$r_g(z)$ of the non-abelian BF boundary algebra.  On the benchmark affine
comparison surface, one would instead expect genus corrections controlled by the
same scalar generating series that governs the class~L shadow package, so the
modular $R$-matrix would take the comparison form
\[
R^{\mathrm{mod}}_G(z; \h)
= \left(1 + \sum_{g \geq 1} \h^{2g}\, \kappa(G) \Lambda_g\right)
\cdot \frac{\Omega_G}{z}.
\]
The displayed series should therefore be read only as the affine benchmark
comparison model, not as a proved formula for non-abelian BF itself.
\end{remark}
...
\item The abelian BF model has commutative boundary product, and any
  ...
  vanishing higher-genus scalar free energies.  A full class~G / trivial-shadow-tower identification
  is not proved here.
\item The non-abelian BF model has a genuine interacting sector beyond the abelian
  commutative benchmark surface.  What this chapter isolates is its low-mode charged modular
  data and the corresponding affine class~L benchmark comparison package; it does
  not prove that the full non-abelian BF boundary algebra itself satisfies the
  affine class~L reconstruction theorem.
...
\item The absolute convergence of the lowest-mode one-wheel representative is controlled
  ...
  cohomology class.
\item The current holographic-atom packet consists of a proved class~C
  $\beta\gamma$ row, a Kodaira--Spencer Gauss--Manin/open-closed benchmark row,
  and an expected mixed/infinite $\mathcal{W}_3$ row.  It therefore suggests the
  free/gauge-benchmark/gravitational-benchmark complexity hierarchy on this
  celestial surface, rather than proving a blanket shadow-depth classification of
  all three atoms here.
\end{enumerate}
...
partition formula $Z_{\mathrm{BF}}(S^3) = 1$, not an internally proved
class~G identification or bulk-boundary shadow identity.
\end{remark}
