...
\begin{theorem}[Boundary-bulk compatibility condition for abelian BF; \ClaimStatusProvedHere]
...
the external abelian BF bulk partition formula on $S^3$ must satisfy
\begin{equation}
...
boundary algebra is class~G with $\kappa = 0$.
\end{theorem}

\begin{proof}
By Proposition~\ref{prop:cs-bf-partition}, the external abelian BF bulk formula on
$S^3$ gives $Z_{\mathrm{BF}}(S^3)=1$, hence $\log Z_{\mathrm{BF}}(S^3)=0$.
Therefore any lifted boundary modular package compatible with that external bulk
formula must satisfy \eqref{eq:bf-consistency}, and coefficientwise all
$F_g^{\mathrm{bdry}}$ vanish for $g\geq 1$.

The internal boundary analysis is compatible with this: later in
Theorem~\ref{thm:bf-shadow-depth} the abelian BF boundary algebra is shown to lie
on the class~G surface with $\kappa=0$, so the scalar genus terms vanish and no
higher-arity shadow data survive.
\end{proof}
...
is no longer on the abelian class~G surface.  What this section actually shows is
...
free energies, is the expected abelian solution of this bootstrap system.
\end{remark}
...
\item The non-abelian BF model has a genuine interacting sector beyond the abelian
  class~G surface.  What this chapter isolates is its low-mode charged modular
