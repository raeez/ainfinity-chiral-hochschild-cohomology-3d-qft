...
The scalar free energies decay exponentially:
$F_g(\cA) \sim 2\kappa(\cA)/(2\pi)^{2g}$
as $g \to \infty$, by the Faber--Pandharipande formula
and the Bernoulli asymptotic $|B_{2g}| \sim 2\,(2g)!/(2\pi)^{2g}$
--- the factorial in~$|B_{2g}|$ is \emph{exactly cancelled}
by the $(2g)!$ in the denominator of~$\lambda_g^{\mathrm{FP}}$.
The scalar genus series $\sum F_g\,\h^{2g}$ therefore
\emph{converges absolutely} for $|\h| < 2\pi$
(equivalently, $|\h^2| < 4\pi^2$), and the Borel
transform is entire.  This is \emph{not} the factorially
divergent Gevrey-$1$ behaviour of matrix-model genus
expansions.  The convergence reflects the compactness
of~$\overline{\cM}_g$ and the exponential decay of
$\lambda_g^{\mathrm{FP}} \sim 2/(2\pi)^{2g}$: the Hodge integrals
that define the scalar lane are tame.
Factorial growth and resurgent trans-series enter
only through the \emph{shadow tower}
(the non-scalar higher-arity corrections $S_r$,
$r \ge 3$), which for class~$\mathbf{M}$ algebras
at $|c| < 6$ diverges geometrically
with growth rate~$\rho > 1$
...
controlled by finite jets, while class~M carries an infinite shadow tower on the
...
Assume $\cA$ is chirally Koszul and the shadow tower stabilizes at arity
$r_{\max}$.  Then:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\begin{proof}
Part \textup{(i)} is exactly the previous theorem.  If the shadow tower
...
shadow tower furnishes the bar-side comparison surface for the
...
not by itself prove a full class~G collapse of the higher-arity shadow tower.
\end{theorem}
...
about the full higher-arity shadow tower follows from these inputs alone.
\end{proof}
...
representations of $G$.  The shadow tower is therefore expected to furnish a
