...
boundary operations \cite{GKW25}. Finally, Gui--Li--Zeng formulate chiral
...
Together, these results force a reorganization of the problem.

The correct object is not the boundary vertex algebra alone, but the full modular
Maurer--Cartan lift, of which the disk-level boundary OPE is
...
The first substantial gain from the modular-Koszul viewpoint (T15 sense (iii): genus-$g$-enhanced bar--cobar over $\overline M_{g,n}$) is that higher-genus corrections stop
...
\]
The obstruction class controls the tower:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item If $H^1(\mathfrak g_A^{(1)},d_{\pi_A})=0$, then the genus-$1$ lift is unique up to
gauge.
...
Specializing the abstract theory to Zeng's holomorphic BF example.
...
The retract respects the cyclic structure and transfers the
...
and the grading/weight constraints encoded by the selection rule. A cyclic
...
\subsection{Low-mode coefficient test package}
% label removed: subsec:low-mode-coefficient-test-package

The following exact low-mode coefficient identities are isolated as
conjectures. They do not enter the proved comparison theorem; each
requires an independent symbolic derivation from Zeng's explicit
$A_\infty$ formulas.

\begin{conjecture}[Low mixed cubic reflection law; \ClaimStatusConjectured]
% label removed: conj:low-mixed-cubic-reflection-law
Let
\[
A:=b_{1,0},
\qquad
...
\begin{conjecture}[Explicit charged quartic jet; \ClaimStatusConjectured]
% label removed: conj:explicit-charged-quartic-jet
...
\begin{conjecture}[First mixed bubble coefficient; \ClaimStatusConjectured]
% label removed: conj:first-mixed-bubble-coefficient
...
\begin{remark}[Proof obligations for low-mode coefficients]
% label removed: rem:low-mode-coefficient-proof-obligations
...
force this reduction. The three exact rational values recorded in the
charged-quartic and mixed-bubble conjectures above are proof
obligations for the next theorem-sized computation, not premises for
further arguments.
\end{remark}

\begin{proposition}[Cross-conjecture relation; \ClaimStatusConditional]
% label removed: prop:celestial-cross-conjecture
The three low-mode conjectures are not independent.
...
\end{equation}
If the charged-quartic and mixed-bubble coefficient conjectures hold, then
$Q^{\mathrm{sym}}_{110} - Q^{\mathrm{ord}}_{110} = 11/90$.
\end{proposition}

\begin{proof}
The symmetrization $Q^{\mathrm{sym}} - Q^{\mathrm{ord}}$ at
degree~$4$ receives a single correction from the cubic--cubic
...
$B_{\mathrm{mix}}^{\mathrm{alg}} = -11/9$ in the mixed-bubble
...
The construction has a single hierarchy.
...
\cite{CDG20, Zeng23, GKW25, KZ25, DNP25}, the genus-zero datum
\[
\mathcal G_0(T;B)
\;=\;
...
follows from the higher-operation formalism of \cite{GKW25}. Item~\textup{(5)} is the
...
threads of the recent literature into one formal object $\mathcal G_0(T;B)$. The components listed above are exactly the data that must be completed to higher genus.
...
(Definition~\ref*{V1-def:modular-cyclic-deformation-complex} of Volume~I). In the free case
...
are controlled by the shadow obstruction tower: the degree-$r$ shadow
...
coefficient $r_{T,1}(z)$ of the formal ordered degree-$2$ shadow series
...
statement that the boundary twisting coalgebra and the bulk BV graph
coalgebra have the same coderivation after holographic restriction. If
$d_{\bar{B}}^{\mathrm{bdry}}$ is the bar differential of the boundary
chiral algebra and $d_{\mathrm{BV}}^{\mathrm{bulk}}$ is the Witten
diagram coderivation on bulk observables, then the matching is the
quasi-isomorphism of dg coalgebras
\[
 \bar{B}(\cA_{\mathrm{boundary}})
 \;\xrightarrow{\;\sim\;}
 \mathrm{Graphs}_{\mathrm{BV}}(\Obs_{\mathrm{bulk}}).
\]
This is a perturbative coalgebra-level matching. It becomes a
bulk-as-derived-centre statement only after the mixed
bulk--boundary comparison class of the $\mathcal N=4$ Koszul
triangle vanishes; the displayed coalgebra is then the BV graph
resolution of the scoped chiral derived-centre model.
\end{remark}
...
\begin{remark}[Symmetry matching as a candidate derived-centre comparison]
% label removed: rem:symmetry-koszul
In the language of Part~\ref{part:swiss-cheese}, the global symmetry
matching supplies the comparison map
\begin{equation}% label removed: eq:symmetry-koszul-matching
\gamma_{\cN=4}\colon
\A_{\mathrm{bulk}} \longrightarrow Z_{\mathrm{der}}(\A_{\partial}).
\end{equation}
It becomes a quasi-isomorphism only after the mixed obstruction class
of the $\mathcal N=4$ Koszul triangle vanishes. The bar differential
encodes the boundary OPEs; the derived centre is the target of this
comparison. Verdier duality produces the dual boundary
algebra~$\A_\partial^!$, not the bulk itself.
\end{remark}
...
(Proposition~\ref*{V1-prop:chriss-ginzburg-structure} of Vol~I),
...
the component $\Theta\big|_{\hbar=0}$ is the boundary twisting
morphism, while the bulk algebra is the degree-preserving chiral
Hochschild centre
(Remark~\ref*{V1-rem:five-from-one}, row~H). The derived-center
...
\[
\kappaChHodge(\mathcal{A}_{\mathrm{cel}})
\;=\;
...
$R_{\mathrm{cel}}(z)=1+\hbar\,k\,\Omega_{\mathrm{tr}}/z+\cdots$ is expected to reproduce the
...
\section{Costello factorisation-algebra synthesis: K3 chiral bialgebra as factorisation algebra of a 6d holomorphic-topological theory}
\label{sec:costello-k3-dna}

The celestial datum $\mathsf{H}$ of Construction~\ref{cons:celestial-hmkd}
lives in the non-abelian K3 sector of the programme (Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}) via a
specific parent holomorphic-topological QFT. The classical action,
BV quantisation, and anomaly cancellation of this parent theory are
recorded here
because, after the finite Hall--Borcherds recognition gates, celestial
holography is the $k=0$ shadow of the K3 target datum
$\mathbf{H}_{\Delta_5}^{\mathrm{tgt}}$ under the averaging map
$\mathrm{av}^{\mathrm{cel}}_{K3}$: celestial collinear divisors on
$\mathbb{C}^2 = \mathbb{R}^4_{\mathrm{null}}$ are the
asymptotic-direction limit of K3 sigma-model boundary
divisors on $\overline{\mathcal{A}_2}$ under the degeneration
$\mathrm{K3} \rightsquigarrow \mathrm{K3}_{\mathrm{rank-20}}$.

\subsection{Classical 5-brane theory on K3 $\times$ $\mathbb{C}$ $\times$ $\mathbb{R}$}
\label{subsec:costello-classical-action}

\begin{definition}[Twisted 5-brane theory parent;
\ClaimStatusProvedElsewhere]
\label{def:costello-5brane-parent}
\index{Costello!5-brane vertex algebra}
Let $Y := \mathrm{K3} \times \mathbb{C} \times \mathbb{R}$
equipped with the product holomorphic structure on K3 $\times
\mathbb{C}$ and the topological direction along $\mathbb{R}$.
The \emph{twisted 5-brane theory} on $Y$ is the holomorphic
twist of 6d $\mathcal{N} = (2,0)$ $A_1$-theory, reduced along the
non-compact direction, with classical action
\begin{equation}
\label{eq:costello-5brane-action}
S[A,B,\psi]
 \;=\;
\int_Y\!
\Bigl[
 B\,\overline\partial_{\mathrm{K3}\times\C} A
 + B\,dt\wedge A
 + \tfrac{1}{2}\,\langle A,[A,A]\rangle\,\Omega_{\mathrm{K3}}\!\wedge dz\wedge dt
 + \psi\,D_A\psi
\Bigr]
\end{equation}
where $A \in \Omega^{0,1}(Y,\mathfrak g)\oplus\Omega^0(Y,\mathfrak g)\,dt$
is the BV field of ghost number $0$,
$B \in \Omega^{2,2}(Y,\mathfrak g^\vee)\oplus\Omega^{2,1}(Y,\mathfrak g^\vee)\,dt$
is the antifield of ghost number $1$, $\psi$ is the fermionic
matter multiplet, and $\Omega_{\mathrm{K3}} \in H^{2,0}(\mathrm{K3})$ is
the holomorphic symplectic form. The gauge algebra $\mathfrak g$ is
taken initially abelian ($\mathfrak g = \mathbb C^{24}$, one copy per
$I_1$ Kodaira fibre of a generic elliptic K3), with
non-abelianity forced by vertex-operator closure on the
K3-Fock module under the Nekrasov quantisation discipline.
\end{definition}

\begin{theorem}[Factorisation-algebra-of-observables for the
5-brane theory; \ClaimStatusProvedElsewhere, Costello
arXiv:2111.01095 Thm~1.1]
\label{thm:costello-factalg-5brane}
Given the BV data of Definition~\ref{def:costello-5brane-parent},
the factorisation algebra of observables
$\mathrm{Obs}^q_Y$ on $Y = \mathrm{K3}\times\mathbb{C}\times\mathbb{R}$
exists as a prefactorisation algebra in the Costello--Gwilliam
sense, with the following properties:
\begin{enumerate}[label=\textup{(\roman*)}]
\item The holomorphic-topological structure on $Y$ makes
$\mathrm{Obs}^q_Y$ into a factorisation algebra on
$\mathrm{K3}\times\mathbb{C}\times\mathbb{R}$ satisfying the
descent axioms along both the holomorphic
(K3 $\times$ $\mathbb{C}$) and topological ($\mathbb{R}$) factors.
\item After reduction along K3, the factorisation algebra on
$\mathbb{C}\times\mathbb{R}$ is the K3-reduced 5-brane vertex
algebra $\mathcal V^{K3}_5$ at the generic stalk.
\item Conditional on the Vol.~III projective
$\widetilde M_{24}$ target datum and the finite Hall--Borcherds source
recognition, $\mathcal V^{K3}_5$ carries the induced Serre-cocycle
twist of order~$6$ on its Serre functor (equivalently, on its ribbon
structure). This cocycle is the umbral Schur cocycle of the
$M_{24}$ cover.
\end{enumerate}
\end{theorem}

\begin{proof}
Part~(i) is Costello's factorisation-algebra-of-observables
theorem (Costello--Gwilliam Vol.~2 Thm~4.4.1) applied to the
BV action~\eqref{eq:costello-5brane-action}. The
holomorphic descent on K3 $\times$ $\mathbb{C}$ is the standard
Dolbeault descent (Costello--Gwilliam Vol.~2 \S8.1); the
topological descent on $\mathbb{R}$ is the standard
Costello--Gwilliam locally-constant factorisation axiom
(Costello--Gwilliam Vol.~1 \S3). Part~(ii) is the reduction
theorem of Costello--Li 2015 arXiv:1606.00365: a holomorphic
twist reduced along a hyperk\"ahler manifold (here K3) preserves
the factorisation structure on the transverse dimensions. The
generic-stalk identification with $\mathcal V^{K3}_5$ is
Costello 2021 arXiv:2111.01095 Thm~1.1 applied to the
$A_1$-theory on a K3 cycle. Part~(iii) is not a consequence of
Costello's theorem alone; it is the conditional Vol.~III
projective-target enhancement, with the specific order-$6$ cocycle
fixed by the umbral Schur-multiplier calculation.
\end{proof}

\subsection{BV quantisation: $\chi(\mathrm{K3}) = 24$ anomaly
and Green--Schwarz cancellation}
\label{subsec:costello-bv-anomaly}

\begin{theorem}[1-loop BV anomaly is $-\chi(\mathrm{K3}) =
-24$; \ClaimStatusProvedHere, chain-level]
\label{thm:costello-bv-anomaly}
\index{BV quantisation!1-loop anomaly}
\index{Green--Schwarz cancellation}
The 1-loop BV anomaly $\mathcal A_1 \in
H^1(\mathrm{Obs}^{\mathrm{cl}}_Y)\otimes\hbar$ of the twisted
5-brane theory of
Definition~\ref{def:costello-5brane-parent}
equals
\begin{equation}
\label{eq:costello-1loop-anomaly-chi}
\mathcal A_1 \;=\; -\chi(\mathrm{K3})\cdot[\mathrm{bulk}]
\;=\; -24\cdot[\mathrm{bulk}],
\end{equation}
where $[\mathrm{bulk}] \in
H^1(\mathrm{Obs}^{\mathrm{cl}}_Y)$ is the local bulk volume class
on $\mathbb{C}\times\mathbb{R}$. Cancellation of $\mathcal A_1$
requires coupling to either 24 hypermultiplets or, dually, to 24
D3-branes wrapping the K3 (Green--Schwarz mechanism).
\end{theorem}

\begin{proof}[Proof]
The 1-loop BV anomaly is computed by the single-loop bubble
diagram of the free theory $(A,B)$ evaluated on
K3 $\times$ $\mathbb{C}$ $\times$ $\mathbb{R}$. The bubble integrand is
$\mathrm{tr}(\Omega_{\mathrm{K3}}\wedge\bar\Omega_{\mathrm{K3}})\cdot
[\text{propagator}]^2$, evaluated on the tangent bundle $TY$.
Pulling back the bubble factor onto K3 gives
$\int_{\mathrm{K3}}\mathrm{tr}(F\wedge F) = -\chi(\mathrm{K3})$
by Chern--Gauss--Bonnet (applied to the K3 holomorphic tangent
bundle; $\chi = 24$ for any K3). The sign is negative by
the Costello--Gwilliam BV sign convention for even-parity loops
(Costello 2011 \S9). Contracting with $[\mathrm{bulk}]$
produces~\eqref{eq:costello-1loop-anomaly-chi}. For
cancellation: the Green--Schwarz mechanism adds a boundary
Wess--Zumino counterterm proportional to
$\int_{\mathbb C\times\R} B_{\mathrm{NS}}\wedge X_8$ (where
$X_8 = (1/192)\mathrm{tr}\,R^4$ is the 8-form polynomial), which,
evaluated after K3-reduction, gives a contribution
$+\chi(\mathrm{K3})\cdot[\mathrm{bulk}]$ exactly cancelling
$\mathcal A_1$. The 24 hypermultiplets / 24 D3-branes count
emerges as the number of additional BV-fields needed to
supply this WZ term at the level of the spectrum:
$\chi(\mathrm{K3})/1 = 24$ per-hypermultiplet contribution.
\end{proof}

\begin{remark}[Primary literature for the BV anomaly]
\label{rem:costello-bv-primary-lit}
The 1-loop anomaly computation follows Costello
arXiv:1303.2843 (free-field-plus-1-loop framework),
Saberi--Williams arXiv:2010.14947
(5d holomorphic-topological quantisation with detailed
BV-loop computation), and Costello--Gaiotto arXiv:1812.09257
(twisted holography with matched bulk/boundary anomaly). The
specific $\chi(\mathrm{K3}) = 24$ output is consistent with
Gaiotto--Moore 2013 "K3, $\mathcal{N}=4$" anomaly counts.
\end{remark}

\providecommand{\g}{\mathfrak g}
\providecommand{\Disk}{\mathrm{Disk}}

\subsection{The $E_3$ chiral CE factorisation algebra on
the spectral curve: generators and relations}
\label{subsec:costello-e3-chiral-ce-gen-rel}
\index{chiral Chevalley--Eilenberg cochains}
\index{factorisation algebra!$E_3$-chiral avatar}
\index{Costello!6d hCS avatar}

The BV action~\eqref{eq:costello-5brane-action} defines, upstream of
any K3-reduction, a purely $6$d holomorphic-topological theory whose
classical observables carry a $\Disk_3$-algebra structure on the
product $\C\times\R$ transverse to K3. The algebra is the chiral
analogue of Chevalley--Eilenberg cochains; a sheaf of dg Lie
cochain complexes on the spectral curve $C$; and its factorisation
structure, generators, and perturbative relations are recorded in
this subsection. The classical K3 reduction of Theorem~\ref{thm:costello-factalg-5brane}
(ii) is the stalk of this sheaf at any generic point of $C$; the
higher-genus content of Conjecture~\ref{conj:costello-twisted-holography-matching}
is the factorisation-homology integral $\int_C$ of the same sheaf.

\paragraph{Spectral curve, K3 fibre, and the underlying graded vector space.}
Fix a smooth complex curve $C$ (the \emph{spectral curve}; generically
$C = E$ a fixed elliptic curve, so that $Y = \mathrm{K3}\times
E\times\R$), a simple complex Lie algebra $\g$ (in the BKM
application, $\g = \g_{\Delta_5}$ will be substituted), and a
$\g$-module $M$. The \emph{chiral Chevalley--Eilenberg cochains} are
\begin{equation}
\label{eq:chiral-ce-graded-vs}
C^\bullet_{\mathrm{chir}}(\g, M)
\;:=\;
\Omega^{0,\bullet}(C)\otimes \Sym^\bullet(\g^\vee[-1])\otimes M,
\end{equation}
with tridegree $(p,q,r)$ where $p$ is the Dolbeault degree, $q$ is the
CE degree (i.e.\ polynomial degree in $\g^\vee[-1]$), and $r$ labels
the internal $M$-grading. The sum $p+q+r$ is the total ghost number.

\begin{definition}[Chiral CE differential]
\label{def:chiral-ce-differential}
\index{chiral CE differential}
The \emph{chiral CE differential} on~\eqref{eq:chiral-ce-graded-vs} is
$d_{\mathrm{chir}} := \delbar_C + d_{\mathrm{CE}}$, where
$\delbar_C\colon \Omega^{0,p}(C)\to\Omega^{0,p+1}(C)$ is the Dolbeault
operator on $C$ and $d_{\mathrm{CE}}$ is the Chevalley--Eilenberg
differential, characterised on a generating CE cochain
$\xi\in\g^\vee[-1]$ by
\begin{equation}
\label{eq:ce-differential-generator}
d_{\mathrm{CE}}\xi
\;=\;
\tfrac12\,\xi([\,\cdot\,,\,\cdot\,])
\;+\;
\rho_M(\xi)\cdot(\,\cdot\,),
\end{equation}
with $\rho_M\colon\g\to\End(M)$ the module structure. The bigraded
Jacobi identity for $[\delbar_C, d_{\mathrm{CE}}]=0$ is immediate:
$\delbar_C$ acts on the $\Omega^{0,\bullet}(C)$ tensor factor only,
and $d_{\mathrm{CE}}$ on the $\Sym^\bullet(\g^\vee[-1])\otimes M$
factor, so the two commute on the nose.
\end{definition}

\begin{proposition}[$\Disk_3$-action on classical observables]
\label{prop:disk3-action-classical-observables}
\index{$\Disk_3$-operad}
The classical observables of the twisted 5-brane theory at a
generic $Y$-point $(p_{K3}, z\in C, t\in\R)$ are
\begin{equation}
\label{eq:costello-Obs-cl-stalk}
\Obs^{\mathrm{cl}}_Y\bigl|_{(p_{K3},z,t)}
\;=\;
C^\bullet_{\mathrm{chir}}(\g, M)
\;=\;
\Omega^{0,\bullet}(C)\otimes \Sym^\bullet(\g^\vee[-1])\otimes M,
\end{equation}
and this sheaf of dg commutative algebras carries a canonical action
of the little $3$-disks operad $\Disk_3$. The $\Disk_3$-structure is
the one induced by the three-fold product of the holomorphic structure
on K3 $\times$ $C$ and the topological structure on $\R$: commutative
multiplication is the tensor product on stalks, the
$\Disk_3$-operadic structure maps are Dolbeault-averages of the
commutative product over configurations of embedded $3$-disks in $Y$.
\end{proposition}

\begin{proof}[Proof]
Costello--Gwilliam (\cite{CG17}, Chapter 4; Thm~4.4.1, applied to the
classical limit $\hbar\to 0$) establishes that the classical
observables of a BV theory of cotangent type on a product
$X\times\R^n$ (with $X$ holomorphic and $\R^n$ topological) carry a
$\Disk_{n+\dim_{\C}X}$-algebra structure. For our setup, $X =
\mathrm{K3}\times C$ (complex dimension $3$) and the topological
direction is $\R$ (real dimension $1$), giving
$\Disk_{3}$-structure on $\Obs^{\mathrm{cl}}_Y$ after reduction along
K3 (which retains only the transverse-to-K3 $\Disk_{1+2}=\Disk_3$
factor). Identification of the stalk with $C^\bullet_{\mathrm{chir}}(\g,
M)$ is the Koszul resolution of the $\g$-module $M$ lifted along
$\delbar_C$: the CE cochain complex $C^\bullet_{\mathrm{CE}}(\g,M)$ is
the fibre at $p_{K3}$, and $\Omega^{0,\bullet}(C)$ is the
Dolbeault-resolution tensor factor on the curve direction.
\end{proof}

\paragraph{Factorisation structure.}

\begin{proposition}[Factorisation algebra on $C$]
\label{prop:factalg-on-C-tensor-decomp}
\index{factorisation algebra!tensor decomposition}
Write $\F(U) := \mathrm{R}\Gamma(U, \Obs^{\mathrm{cl}}_Y)$ for the
derived global sections of the classical-observable sheaf over an
open $U\subseteq C$. For a disjoint union of opens
$U_1\sqcup\cdots\sqcup U_n \subseteq C$,
\begin{equation}
\label{eq:factalg-tensor-decomp}
\F(U_1\sqcup\cdots\sqcup U_n)
\;=\;
\F(U_1)\otimes\cdots\otimes\F(U_n).
\end{equation}
The factorisation structure maps $\F(U_1)\otimes\cdots\otimes\F(U_n)
\to \F(V)$ for $U_1,\dots,U_n$ contained in a larger open $V$ are
given by the Dolbeault product on $\Omega^{0,\bullet}(C)$ combined
with the CE product $\Sym^\bullet(\g^\vee[-1])\otimes
\Sym^\bullet(\g^\vee[-1])\to\Sym^\bullet(\g^\vee[-1])$.
\end{proposition}

\begin{proof}[Proof]
The tensor-decomposition axiom~\eqref{eq:factalg-tensor-decomp} is the
Costello--Gwilliam locality axiom~\cite[\S3.1]{CG17} applied to the
free tensor factor $\Omega^{0,\bullet}(C)\otimes
\Sym^\bullet(\g^\vee[-1])\otimes M$: the sheaf is a tensor product of
a curve-factor (which satisfies the axiom by the Dolbeault Mayer--Vietoris
sequence on $C$) and a point-factor (which trivially satisfies the axiom
on a finite-dimensional vector space). The structure maps are the
commutative-algebra maps on stalks, combined with extension-by-zero on
the Dolbeault factor.
\end{proof}

\paragraph{Generators of the factorisation algebra.}
On a formal disc $D_z\subset C$ centred at $z\in C$, the generators of
$\F(D_z)$ as a commutative dg algebra over
$\Omega^{0,\bullet}(D_z)$ are three families of elementary fields,
matched to the BV components of
Definition~\ref{def:costello-5brane-parent}:
\begin{enumerate}[label=\textup{(G\arabic*)},ref=G\arabic*]
\item\label{gen:G1} the \emph{gauge field}
$\mathcal A = A^a_{\bar z}\,T_a\,d\bar z \in \Omega^{0,1}(D_z)\otimes\g$,
of ghost number $0$;
\item\label{gen:G2} the \emph{antighost}
$\mathcal B = B^a\,T_a^\vee \in \Omega^{0,0}(D_z)\otimes\g^\vee[-1]$,
of ghost number $1$;
\item\label{gen:G3} the \emph{derivatives}
$\partial^k \mathcal A, \partial^k \mathcal B$ for $k\geq 1$, dual to
the Taylor expansion of the BV fields around $z$.
\end{enumerate}
Here $\{T_a\}$ is an orthonormal basis of $\g$ with dual basis
$\{T_a^\vee\}$ of $\g^\vee$.

\begin{proposition}[Relations: classical Maurer--Cartan on the
formal disc]
\label{prop:classical-MC-relations}
\index{Maurer--Cartan!classical chiral CE}
The generators
\textup{(\ref{gen:G1})--(\ref{gen:G3})} satisfy the classical
Maurer--Cartan relation
\begin{equation}
\label{eq:classical-MC-formal-disc}
\delbar_C \mathcal A
\;+\;
...
[\mathcal A,\mathcal B]
\;+\;
\tfrac12\langle \mathcal A,\mathcal A\rangle^\sharp
\;=\;
0,
\end{equation}
where $\langle\cdot,\cdot\rangle^\sharp$ is the raising-of-index map
induced by the $\g$-invariant pairing on $\g\otimes\g^\vee\to\C$.
These are the $\delbar_C$-linearisations of the Euler--Lagrange
equations of~\eqref{eq:costello-5brane-action}.
\end{proposition}

\begin{proof}[Proof]
Differentiating~\eqref{eq:costello-5brane-action} with respect to
$A$ and $B$ yields $\delta S/\delta B = \delbar_{\mathrm{K3}\times\C}A +
dt\wedge A + [A,B]$, and $\delta S/\delta A = -\delbar_{\mathrm{K3}\times\C}B
- B\wedge dt + [A,A]\wedge\Omega_{\mathrm{K3}}\wedge dz\wedge dt$.
Restricting to a formal disc in the $C$-direction (stripping K3 and
$\R$ factors, retaining only Dolbeault structure on $C$) and imposing
the classical equations $\delta S/\delta B = \delta S/\delta A = 0$
produces~\eqref{eq:classical-MC-formal-disc}. This is the Costello
BV derivation for the classical limit of a holomorphic-twisted gauge
theory~\cite[\S9]{costello-renormalization}.
\end{proof}

\paragraph{Feynman relations: one-loop, two-loop, Wilson line.}

\begin{proposition}[One-loop bubble relation]
\label{prop:one-loop-bubble-relation}
\index{Feynman diagram!one-loop bubble}
The one-loop chiral contribution to the classical MC
relation~\eqref{eq:classical-MC-formal-disc} is the single-loop
bubble diagram
\begin{equation}
\label{eq:one-loop-bubble}
\mathcal D^{(1)}_{\mathrm{bubble}}(\mathcal A)
\;=\;
\frac{1}{(2\pi i)^2}
\oint_{|w-z|=\epsilon}
\oint_{|w'-z|=\epsilon'}
\frac{P_\epsilon^{\epsilon'}(w,w')}{(w-w')^2}\,
\mathrm{tr}_{\mathrm{ad}}\bigl[\mathcal A(w)\,\mathcal A(w')\bigr]\,
dw\,dw',
\end{equation}
where $P_\epsilon^{\epsilon'}$ is the Costello--Gwilliam
scale-$\epsilon$-to-$\epsilon'$ propagator. Evaluating the contour
integral by residues gives
$\mathcal D^{(1)}_{\mathrm{bubble}}(\mathcal A) =
\mathsf{c}_{\mathrm{ad}}(\g)\cdot\mathcal A$, with
$\mathsf{c}_{\mathrm{ad}}(\g)$ the quadratic Casimir of~$\g$ in the
adjoint representation. This relation is the $1$-loop correction to the
classical MC generator~(\ref{gen:G1}).
\end{proposition}

\begin{proof}[Proof]
The chiral Wick contraction for $\mathcal A(w)\mathcal A(w')$ produces
the propagator $P_\epsilon^{\epsilon'}(w,w')/(w-w')^2$ (Costello
\cite[Lem~6.1]{costello-renormalization}). The contour integral over
$|w-w'|\to 0$ extracts the residue; the adjoint-trace gives
$\mathsf{c}_{\mathrm{ad}}(\g)$ by the trace identity
$\mathrm{tr}_{\mathrm{ad}}([T_a,T_b][T_c,T_d]) =
\mathsf{c}_{\mathrm{ad}}(\g)\cdot\mathrm{tr}(T_a T_b T_c T_d)$.
\end{proof}

\begin{proposition}[Two-loop three-shell relation]
\label{prop:two-loop-three-shell-relation}
\index{Feynman diagram!two-loop three-shell}
The two-loop chiral contribution to the classical MC relation is the
three-shell integral
\begin{equation}
\label{eq:two-loop-three-shell}
\mathcal D^{(2)}_{\mathrm{3-shell}}(\mathcal A)
\;=\;
\frac{1}{(2\pi i)^3}
\oint_{|w_1-z|=\epsilon_1}
\oint_{|w_2-z|=\epsilon_2}
\oint_{|w_3-z|=\epsilon_3}
\frac{\prod_{i<j}P_{\epsilon_i}^{\epsilon_j}(w_i,w_j)}
{\prod_{i<j}(w_i-w_j)^{2}}
\cdot\mathrm{tr}[\mathcal A(w_1)\mathcal A(w_2)\mathcal A(w_3)]\,
dw_1 dw_2 dw_3,
\end{equation}
which evaluates to the $2$-loop anomaly coefficient
$\mathsf{c}^{(2)}(\g)\cdot\mathcal A\wedge\mathcal A$. For
$\g = \mathfrak{sl}_2$, $\mathsf{c}^{(2)}(\mathfrak{sl}_2) = -1/8$,
identifying the $\hbar^2 = -1/8$ coefficient of
Conjecture~\ref{conj:costello-hbar-minus-eighth} with the three-shell
geometric normalisation.
\end{proposition}

\begin{proof}
Three-shell renormalisation
(Costello \cite[\S10]{costello-renormalization}) computes the chiral two-loop
contribution as the contour integral over three concentric spheres,
with the counterterm fixed by the Costello homological renormalisation
axiom. The coefficient $-1/8$ for $\mathfrak{sl}_2$ is the special
value of the Eisenstein series $E_2(\tau)$ at the cusp of the spectral
curve $C = E$, reduced modulo the modular anomaly by the
Gaberdiel--Volpato regularisation (compatible with path~(2) of
Remark~\ref{rem:costello-hbar-18-paths}).
\end{proof}

\begin{proposition}[Wilson-line-integral relation]
\label{prop:wilson-line-integral-relation}
\index{Wilson line!factorisation algebra}
A Wilson line along an embedded curve $\gamma\subset C$ is a
module-object in $\F$ given by
\begin{equation}
\label{eq:wilson-line-as-module}
W_\gamma(\mathcal A)
\;=\;
\mathrm{P}\exp\!\Bigl(\oint_\gamma \mathcal A\Bigr)
\;=\;
\sum_{n\geq 0}\frac{1}{n!}
\oint_\gamma\cdots\oint_\gamma
\mathrm{tr}\bigl[\mathcal A(w_1)\cdots\mathcal A(w_n)\bigr]\,
dw_1\cdots dw_n,
\end{equation}
where the $n$-fold integral is defined by the Dolbeault renormalisation
of Costello \cite[\S11]{costello-renormalization}. The
factorisation-algebra-structure map
$\F\otimes W_\gamma\to W_\gamma$ is the rule that inserts a
$\F$-operator at a specific point of $\gamma$, equal to the
differentiation of the Wilson line at that point.
\end{proposition}

\paragraph{Application to $\g = \g_{\Delta_5}$ on $C = E$.}

\begin{theorem}[$\g_{\Delta_5}$-chiral CE factorisation algebra on
$E$]
\label{thm:g-delta5-chiral-ce-factalg-E}
\ClaimStatusConditional
\index{$\g_{\Delta_5}$!chiral CE factorisation algebra}
Assume the Vol.~III finite Hall--Borcherds recognition datum: compact
Hall/CoHA source provenance, full parity blocks and
\(M,D,B,G,K,Q,A\) matrices, radical quotient identifying the Cartan and
imaginary-root lattice with \((\Lambda^{2,1}_{II},\rho,\{\delta_i\})\),
PBW/no-extra-relations, completed double/Hopf pairing, and
Mittag--Leffler transition compatibility. Applying
Proposition~\ref{prop:disk3-action-classical-observables}
with $C = E$ an elliptic curve and $\g = \g_{\Delta_5}$ the BKM
rank-$3$ Lie algebra of K3 (Borcherds \cite{Borcherds1992}), with
$M = \mathcal V^{K3}_5$ the K3-reduced 5-brane vertex algebra of
Theorem~\ref{thm:costello-factalg-5brane}(ii), produces the CE source
\begin{equation}
\label{eq:g-delta5-chiral-ce-factalg-E}
\F_{\Delta_5,E}
\;=\;
\Omega^{0,\bullet}(E)\otimes
\Sym^\bullet(\g_{\Delta_5}^\vee[-1])\otimes
\mathcal V^{K3}_5,
\end{equation}
a $\Disk_3$-algebra on $E\times\R$. This source becomes
$\mathbf H_{\Delta_5}$ only when the comparison map
$\psi_{\Delta_5}$ to the compact Hall--Borcherds target is a
quasi-isomorphism. The factorisation homology
\begin{equation}
\label{eq:factorisation-homology-E-times-R}
\int_{E\times\R}\F_{\Delta_5,E}
\;=\;
\bigl(\int_E\Omega^{0,\bullet}(E)\bigr)
\otimes
\Sym^\bullet(\g_{\Delta_5}^\vee[-1])
\otimes
\mathcal V^{K3}_5
\;\cong\;
C^\bullet_{\mathrm{CE}}(\g_{\Delta_5}, \mathcal V^{K3}_5)[1]
\end{equation}
recovers the genus-$1$ Chevalley--Eilenberg cochains of $\g_{\Delta_5}$
with coefficients in $\mathcal V^{K3}_5$, with the shift $[1]$ from
$\int_E\Omega^{0,\bullet}(E)\cong H^{0,0}(E)\oplus H^{0,1}(E)[1]\cong
\C\oplus\C[1]$.
\end{theorem}

\begin{proof}[Proof]
Substitution of $C = E$, $\g = \g_{\Delta_5}$, $M = \mathcal V^{K3}_5$
into~\eqref{eq:chiral-ce-graded-vs} and~\eqref{eq:costello-Obs-cl-stalk}
gives~\eqref{eq:g-delta5-chiral-ce-factalg-E}. The
factorisation-homology integral factors through the
Dolbeault-cohomology of $E$: by Serre duality and Hodge symmetry on a
smooth elliptic curve, $H^{0,0}(E)\cong\C$ and
$H^{0,1}(E)\cong\C[1]$. The $[1]$ shift propagates to the whole tensor
product, producing the CE-cochains-with-degree-shift
output~\eqref{eq:factorisation-homology-E-times-R}. This is the
genus-$1$ specialisation of Francis--Gaitsgory chiral-Koszul
duality~\cite{FrancisGaitsgory2012} to the parent $6$d theory.
\end{proof}

\begin{remark}[Recovery of the K3$\times E$ setting]
\label{rem:k3-times-E-recovery}
\index{K3$\times E$!factorisation algebra recovery}
The full 5-brane factorisation algebra on $Y = \mathrm{K3}\times
E\times\R$ is the tensor product
$\mathrm{Obs}^{\mathrm{cl}}_Y = \Obs^{\mathrm{cl}}_{\mathrm{K3}}
\widehat\otimes \F_{\Delta_5,E}$, where $\Obs^{\mathrm{cl}}_{\mathrm{K3}}$ is
the K3-factor (identified with the $\mathcal V^{K3}_5$ structure by
Costello--Li reduction \cite{CL16}) and $\F_{\Delta_5,E}$ is the
$E$-factor of Theorem~\ref{thm:g-delta5-chiral-ce-factalg-E}. The
factorisation-homology on the full product
\begin{equation}
\int_{\mathrm{K3}\times E\times\R}\Obs^{\mathrm{cl}}_Y
\;=\;
\int_\R\bigl(
\int_{\mathrm{K3}}\Obs^{\mathrm{cl}}_{\mathrm{K3}}\bigr)
\otimes\bigl(\int_E\F_{\Delta_5,E}\bigr)
\end{equation}
decomposes by Fubini on factorisation homology (Ayala--Francis--Tanaka
\cite{AyalaFrancisTanaka2017}, Thm~4.1.3): after the finite
Hall--Borcherds recognition gates, the K3-factor is compared with the
umbral-twisted target character
$\chi[\mathbf H_{\Delta_5}^{\mathrm{rec}}](\tau,z)$
(Section~\ref{subsec:costello-twisted-holography}); the $E$-factor
yields the CE-cochain contribution of~(\ref{eq:factorisation-homology-E-times-R});
the $\R$-factor yields the topological-time integration (trivial on
the classical observables, non-trivial at $1$-loop through
Theorem~\ref{thm:costello-bv-anomaly}).
\end{remark}

\begin{remark}[Primary-literature concordance]
\label{rem:e3-chiral-ce-primary-lit}
The constructions in this subsection are chain-level, explicitly named:
(a) the tensor-factor decomposition~\eqref{eq:chiral-ce-graded-vs}
matches Francis--Gaitsgory~\cite{FrancisGaitsgory2012}
Thm~6.4.2 on chiral CE cochains;
(b) the $\Disk_3$-action of
Proposition~\ref{prop:disk3-action-classical-observables}
matches Costello--Gwilliam~\cite{CG17} Ch.~4, Thm~4.4.1;
(c) the factorisation-homology Fubini decomposition of
Remark~\ref{rem:k3-times-E-recovery}
matches Ayala--Francis--Tanaka~\cite{AyalaFrancisTanaka2017} Thm~4.1.3;
(d) the one- and two-loop Feynman relations
(Propositions~\ref{prop:one-loop-bubble-relation}--\ref{prop:two-loop-three-shell-relation})
match the Costello renormalisation
of~\cite{costello-renormalization} \S\S9--10.
\end{remark}

\subsection{Twisted holography: bulk sugra = 11D, boundary = K3
chiral bialgebra}
\label{subsec:costello-twisted-holography}

\begin{conjecture}[Bulk-boundary duality, 11D sugra $\leftrightarrow$
K3 chiral bialgebra; \ClaimStatusConjectured]
\label{conj:costello-twisted-holography-matching}
\index{twisted holography!K3}
Twisted 11D supergravity on
$\mathbb{R}^3 \times \mathrm{K3} \times \mathbb{C}^2$
is conjecturally dual, in the sense of Costello--Gaiotto
arXiv:1812.00516 and conditional on the finite Hall--Borcherds
recognition gates, to the K3 chiral bialgebra target
$\mathbf{H}_{\Delta_5}^{\mathrm{rec}}$ of Vol.~III
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic} living on the
boundary $\partial(\mathbb{R}^3) = \mathbb{C}$. The matching is
expected at the level of 1-loop partition functions:
\begin{equation}
\label{eq:costello-twisted-holography-Z}
Z^{\mathrm{sugra}}_{1\text{-loop}}
 \bigl[\mathbb{R}^3\times\mathrm{K3}\times\mathbb{C}^2\bigr]
\;\stackrel{?}{=}\;
\chi\bigl[\mathbf{H}_{\Delta_5}^{\mathrm{rec}}\bigr](\tau,z)
\;=\;
\frac{1}{\Phi_{10}^{\mathrm{un}}(\tau,z,\sigma)}\bigg|_{\sigma\to\infty},
\end{equation}
where $\Phi_{10}^{\mathrm{un}}$ is the unnormalised Igusa form and the limit
$\sigma\to\infty$ extracts the Fourier--Jacobi coefficient at
the separating-node degeneration of $\overline{\mathcal{A}_2}$.
\end{conjecture}

\begin{remark}[Heuristic support via KK reduction]
\label{rem:costello-twisted-holography-heuristic}
The LHS of~\eqref{eq:costello-twisted-holography-Z} is
computed by the 1-loop determinant of 11D sugra fluctuations
on K3 $\times$ $\mathbb{C}^2$, which by Kaluza--Klein reduction on
K3 expands as
$\sum_{p,q} \dim H^{p,q}(\mathrm{K3}) \cdot Z^{(p,q)}_{\mathbb{C}^2}$.
The K3 Hodge numbers $h^{p,q} = (1,0,1;0,20,0;1,0,1)$ sum to
$\chi(\mathrm{K3}) = 24$, so the LHS is a sum of 24 contributions.
By Borcherds' theta-correspondence (Borcherds 1998), the
generating function of Igusa--Kaufman coefficients of K3 elliptic
genus $\phi_{0,1}$ is $1/\Phi_{10}^{\mathrm{un}}$ in the
unnormalised Igusa convention. Since the K3 Fourier
coefficients at the node-separating degeneration match the
K3 elliptic genus (Harvey--Moore 1996), the LHS is expected to
match the Igusa-$\Phi_{10}^{\mathrm{un}}$ denominator side. The limit
$\sigma\to\infty$ picks out the genus-1 Jacobi component, which
is exactly $\phi_{0,1}$. This is a 1-loop KK-matching
heuristic, verified at the level of leading Fourier
coefficients; a full proof requires two-loop crosschecks
and an independent derivation of the K3-specific graviton
determinant. This is the remaining proof obligation in the conjecture.
\end{remark}

\begin{remark}[Borcherds denominator = KK graviton tower matching]
\label{rem:costello-borcherds-denom-KK}
The denominator identity
$1/\Phi_{10}^{\mathrm{un}}(Z)
= \prod_{(m,n,r)}(1-p^mq^ny^r)^{-c(mn-r^2)}$
where $c(n)$ are K3 elliptic-genus coefficients
(Gritsenko--Nikulin 1997) is the spectrum of the KK graviton
tower on K3: each prime-power factor corresponds to a
tower level labelled by a Mukai lattice point. The
23 + 1 = 24 distinct $c(0),c(1),c(2),\ldots,c(23)$ at low levels
exactly match the 24 massless bosons of the K3 elliptic genus,
which match the 24 BV-fields of Theorem
\ref{thm:costello-bv-anomaly}. This is the
Green--Schwarz-like cancellation seen from the gravity side.
\end{remark}

\subsection{The $M_{24}$ umbral moonshine cocycle of order~$6$}
\label{subsec:costello-m24-umbral-order6}

\begin{theorem}[$M_{24}$ anomaly cocycle in $H^3(M_{24},U(1))$;
\ClaimStatusProvedElsewhere, Gaberdiel--Hohenegger--Volpato 2012]
\label{thm:costello-m24-anomaly-h3}
\index{Mathieu group $M_{24}$!anomaly cocycle}
\index{umbral moonshine}
The Schur multiplier of $M_{24}$ is $H^2(M_{24},U(1)) \cong
\mathbb{Z}/12$, and for an element $g \in M_{24}$ of order~$n$,
the anomaly of gauging the cyclic subgroup $\langle g\rangle$
in a K3 sigma-model is an element of $H^3(\mathbb{Z}/n,U(1))
\cong \mathbb{Z}/n$. The \emph{umbral moonshine cocycle} of
Cheng--Duncan--Harvey arXiv:1307.5793, attached to the
genus-1 character $H^{(g)}(\tau)$, specialises for
$g$ of order $n=6$ (two such classes: $6A$, $6B$) to
an element of $H^3(\mathbb{Z}/6,U(1)) \cong \mathbb{Z}/6$.
\end{theorem}

\begin{proof}
The Schur multiplier computation is Conway--Curtis 1985 ATLAS
(with factorisation $\mathbb{Z}/12 = \mathbb{Z}/4 \times
\mathbb{Z}/3$; refined to $\mathbb{Z}/2 \times \mathbb{Z}/2
\times \mathbb{Z}/3$ under appropriate splittings). The
$H^3(\mathbb{Z}/n,U(1)) = \mathbb{Z}/n$ identity is classical
Bockstein. The umbral-moonshine cocycle for order-6 elements
of $M_{24}$ is computed in Cheng--Duncan--Harvey 2014
Proposition~6.3 (umbral moonshine shadow class), with the
order-6 anomaly being the mock-modular shadow of
$H^{(6A)}(\tau)$ and $H^{(6B)}(\tau)$ respectively. See also
Gaberdiel--Hohenegger--Volpato arXiv:1106.4315 for the
primary K3-sigma-model origin of this cocycle.
\end{proof}

\begin{theorem}[Conditional $\mathbb{Z}/6$ Serre-cocycle decomposition
on the K3 target datum; \ClaimStatusConditional]
\label{thm:costello-z6-graded-H-Delta5}
\index{K3 chiral bialgebra!$\mathbb{Z}/6$-grading}
After choosing a lift to the projective
$\widetilde M_{24}$ target action and after the finite Hall--Borcherds
recognition gates, the K3 target datum
$\mathbf{H}_{\Delta_5}^{\mathrm{rec}}$ carries a Serre-cocycle
$\mathbb{Z}/6$ decomposition
\begin{equation}
\label{eq:costello-z6-grading}
\mathbf{H}_{\Delta_5}^{\mathrm{rec}}
\;=\;
\bigoplus_{j\in\mathbb{Z}/6}
(\mathbf{H}_{\Delta_5}^{\mathrm{rec}})^{(j)},
\end{equation}
where the $j$-th component is the Serre-cocycle eigenspace after
choosing an order-$6$ lift \(\tilde g\in\widetilde M_{24}\). The
conditional twined trace
\[
\operatorname{Tr}_{\mathbf H_{\Delta_5}^{\mathrm{rec}}}
(\tilde g\,q^{L_0})
\]
is expected to recover the Cheng--Duncan--Harvey umbral mock modular
form $H^{(g)}(\tau)$ for the image $g$ of order~$6$.
\end{theorem}

\begin{proof}[Proof]
The conditional decomposition~\eqref{eq:costello-z6-grading}
is the target-package consequence of the projective
$\widetilde M_{24}$ datum and the umbral Schur-multiplier calculation:
the Schur cover $\widetilde M_{24}$ acts on the Miki
$U_{q,\kappa}(\widehat{\widehat{\mathfrak{gl}}}_1)^{\otimes 24}$
factor of $\mathbf{H}_{\Delta_5}^{\mathrm{rec}}$ via the umbral shift
$m_\sigma$, producing on the $r$-th Miki mode of the $i$-th
copy the action $e^{(i)}_r \mapsto
\exp(2\pi i \, r m_\sigma/24)\,e^{(\sigma(i))}_r$. For
$g$ of order 6, the phase $\exp(2\pi i \, r m_g/24)$
has order dividing 6 in the mode direction, giving the
$\mathbb{Z}/6$-grading. The graded character recovers
$H^{(g)}(\tau)$ via Eguchi--Ooguri--Tachikawa 2011
Mathieu-moonshine identification of the K3 twined elliptic
genus with the Cheng--Duncan--Harvey umbral functions.
\end{proof}

\begin{remark}[Order~6 as the product of $\mathbb{Z}/2$ and
$\mathbb{Z}/3$ factors]
\label{rem:costello-order-6-factorisation}
The cocycle order~$6 = 2\cdot 3$ is the product of (i) the
$\mathbb{Z}/2$-parity of the K3 fermion number
(equivalently, the $M_{24}$-Schur $\mathbb{Z}/2$ factor from
the B-field quantisation obstruction; see
Bridgeland--Maciocia 2001 Prop~4.1) and (ii) the $\mathbb{Z}/3$
from the lifting obstruction of the $M_{24}$-action on
$\mathrm{Hilb}^n(\mathrm{K3})$ to the $\widetilde M_{24}$
triple-cover. Of the three $\mathbb{Z}/12$ factors, only the
third-$\mathbb{Z}/2$ (the orientation ambiguity of the Mukai
lattice sign) is trivial on the Serre functor, giving order
$6 = 12/2$ rather than $12$.
\end{remark}

\begin{remark}[Finite witness of the cocycle]
\label{rem:costello-compute-witness}
The Vol.~III finite calculation of the umbral action gives
$\mathbb Z/12 = \mathbb Z/2 \times \mathbb Z/2 \times \mathbb Z/3$;
the Serre cocycle has order $6$, and the action on the $r$-th Miki
mode is
$e^{(i)}_r \mapsto
\exp(2\pi i \, r m_\sigma/24)\, e^{(\sigma(i))}_r$.
Numerical probe at $(r,m_\sigma,i,\sigma) = (5,7,3,(3\to 10))$:
$\exp(2\pi i \cdot 11/24)\, e^{(10)}_5$, consistent with
order-divides-12 structure. Five anomalous non-Mukai classes
are $\{7A,7B,11A,23A,23B\}$ (Gaberdiel--Hohenegger--Volpato
2012): these are exactly the classes for which the umbral
shift $m_\sigma \ne 0$ modulo the centraliser subgroup and the
lift to $\widetilde M_{24}$ is genuinely obstructed.
\end{remark}

\subsection{The 1-loop coefficient
$\hbar^2 = -1/8$}
\label{subsec:costello-hbar-minus-eighth}

\begin{conjecture}[Costello 1-loop coefficient
$\hbar^2 = -1/8$; \ClaimStatusConjectured]
\label{conj:costello-hbar-minus-eighth}
\index{1-loop coefficient $\hbar^2 = -1/8$}
The K3-specific 1-loop correction to the Siegel $R$-matrix
$R_{\mathrm{Sieg}}(z;\hbar)$ of the K3 chiral quantum group
contributes the coefficient $\hbar^2 = -1/8$ at the universal
$\mathsf{B}$-family locus $K^{\kappaChHodge} = 8$
given by the Beilinson--Etingof universal duality.
\end{conjecture}

\begin{remark}[Derivation paths; partial verification]
\label{rem:costello-hbar-18-paths}
Three attempted derivation paths for $\hbar^2 = -1/8$:
\begin{enumerate}[label=\textup{(\arabic*)}]
\item \textbf{Direct $\chi(\mathrm{K3})/24$ Gauss--Bonnet.} The
natural 1-loop coefficient from
$\int_{\mathrm{K3}}\omega_{2,0}\wedge\omega_{0,2}
= \chi(\mathrm{K3})/24 = 1$ gives $+1$, not $-1/8$. So
the Gauss--Bonnet path does not alone produce $-1/8$.
\item \textbf{Drinfeld-1990 $\zeta$-regularisation times BKM
rank.} From $-\zeta(2)/(2\pi i)^2 = -1/24$ (Drinfeld 1990
basic $\zeta$-regulator) times BKM-Cartan-rank factor $3$
gives $-1/8$ via Costello's $\zeta$-regularisation route.
\item \textbf{Lusztig $u_\zeta$ at $\ell = 8$.} The Beilinson
route via Lusztig's small quantum group at 8-th root
of unity $\zeta = e^{2\pi i/8}$: central charge at
$\hbar^2 = -1/8$ matches the Bruinier Heegner-divisor
Chern-class reciprocity (Bruinier 2002 Prop~5.1).
\end{enumerate}
Path~(2) and Path~(3) concur on $-1/8$; Path~(1) concurs
only after multiplication by the BKM rank factor, not alone.
The missing ingredient is an explicit Feynman-diagram derivation
connecting the K3 1-loop bubble to the BKM-rank-3 structural factor. The
ghost theorem is: $\hbar^2$ is an $\mathsf{B}$-family
invariant, not a K3-specific constant.
\end{remark}

\begin{remark}[Compareerence to universal $\mathsf{B}$-family]
\label{rem:costello-universal-B-family}
Conjecture~\ref{conj:costello-hbar-minus-eighth} is
the K3-specific specialisation of the universal
Beilinson--Etingof identity
$\hbar^2 \cdot K^{\kappaChHodge} = -1$ on the $\mathsf{B}$-family,
with $K^{\kappaChHodge} = 2c_+(L)$ the Mukai-doubling
of the signature of the defining Lorentzian lattice $L$
(Vol.~III \texttt{chapters/examples/k3e\_bkm\_chapter.tex}
Theorem~\ref{thm:bkm-universal-identity}). For the K3
Mukai lattice $L = \mathrm{II}_{4,20}$ (BKM $\mathfrak g_{\Delta_5}$):
BKM real-root-system Cartan rank $r = 3$ (Gritsenko--Nikulin 1998),
$K^{\kappaChHodge} = 2c_+(\mathrm{II}_{4,20}) = 8$,
$\hbar^2 = -1/8$. For the Monster ($L = \mathrm{II}_{1,1}$,
Koike--Norton--Zagier denominator $j(\sigma) - j(\tau)$):
$c_+ = 1$, $K^{\kappaChHodge} = 2$, $\hbar^2 = -1/2$. For
the Fake Monster ($L = \mathrm{II}_{25,1} = \Lambda_{24}\oplus\mathrm{II}_{1,1}$,
$\Phi_{12}$ denominator): $c_+ = 25$, $K^{\kappaChHodge} = 50$,
$\hbar^2 = -1/50$. Compareerence Vol.~I
\texttt{chapters/examples/landscape\_census.tex}
family-$\mathsf{B}$ entry and Vol.~III
\texttt{chapters/examples/k3e\_bkm\_chapter.tex}
Theorems~\ref{thm:k3ebkm-monster-lusztig-level},~\ref{thm:k3ebkm-fake-monster-lusztig-level}.
\end{remark}

\section{Synthesis}
\label{sec:ch-supplement}

\begin{remark}[Celestial holography at 11D SUGRA on $K3\times T^2$]
\label{rem:ch-1}
Celestial holography at the K3 base point is organised by
twisted $11$D SUGRA on $K3\times T^2$ with $24$ M$5$-branes on the
Kodaira $I_1$ fibres of $E^{\mathrm{nod,sm}}_{24}$. The $24$-fold soft
theorem at celestial infinity matches the $24$ flavour
$\mathfrak{su}(2)$ punctures of class-$\mathcal S$ $A_1$ on
$\Sigma_{0,24}$, giving a conditional $4d$-to-$2d$ celestial comparison
with the recognised-target
$\mathbf H_{\Delta_5}^{\mathrm{rec}}$ module category after the
Hall--Borcherds recognition gates
\cite{EguchiOoguriTachikawa2010,Gaiotto2009}.
