...
For $\mathcal N=4$ SYM with gauge group $SU(N)$, the celestial
chiral algebra on $\mathbb{CP}^1$ is an extension of
$\widehat{\mathfrak{gl}}_N$ by soft graviton modes.
The holographic modular Koszul datum:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $\cA$: the celestial OPE algebra of conformally soft
  gluon and graviton modes $\mathcal O^a_{\Delta,J}$.
\item $\cA^!$: the celestial Yangian $Y(\mathfrak{gl}_N)$.
\item $r(z)\sim\mathrm{Split}_{+}(z)/z$: the collinear
  splitting function.
\item $\kappa\propto\beta_1(N)$: the one-loop beta function.
  At $\mathcal N=4$: $\beta_1=0$, $\kappa=0$, the shadow
  connection is flat.
\item $\nabla^{\mathrm{hol}}$: flat for $\mathcal N=4$;
  celestial amplitudes satisfy KZ-type differential equations.
\end{enumerate}
The collinear limit $z_1\to z_2$ is the collision residue
