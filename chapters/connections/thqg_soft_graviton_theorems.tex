...
result: the leading soft factor is a consequence of the bar complex
...
\index{BMS group!supertranslation reading|textbf}
...
\item The obstruction term $o_3(\cA)$ is a closed cochain in the
  ...
  subleading celestial soft factor, hence in the
  celestial/gravitational reading its anomalous contribution.
\end{itemize}
...
$L_\infty$-algebra $\gAmod$.  Thus the celestial soft hierarchy at
...
celestial/gravitational reading is free.  For class-$\mathbf{M}$
algebras ($r_{\max} = \infty$), the $L_\infty$-algebra is maximally
non-formal and the celestial/gravitational reading has infinitely many
independent interactions.  The operadic complexity
...
independent soft statements in the hierarchy.  The
...
\emph{celestial OPE algebra}, whose Ward identities are the celestial
soft factors.  In the celestial/gravitational reading, these Ward
identities are read as the corresponding soft hierarchy.
This subsection
develops the connection between the shadow tower and celestial
holography.
...
\item $\mathfrak{S}(\cA)$ is generated by the arity-$2$ and
  ...
  reading correspond to the leading and subleading soft sectors,
...
\item For class-$\mathbf{G}$ algebras, only the arity-$2$ sector
  survives, so $\mathfrak{S}(\cA)$ is the binary celestial soft
  algebra.
\item For class-$\mathbf{L}$ algebras, only arities~$2$ and~$3$
  survive, so $\mathfrak{S}(\cA)$ is the $(2,3)$-generated
...
$(2,3)$-generated celestial soft algebra is all of
...
normal-ordered products $:T^{r-1}:$ and their descendants, giving
the standard $\mathcal{W}_{1+\infty}$ package.
...
factors are read as the Ward identities of the
$\mathcal{W}_{1+\infty}$ currents: the arity-$r$ celestial soft
factor corresponds to the Ward identity of the spin-$(r-1)$
$\mathcal{W}$-current.
\end{remark}
...
$\mathbf{G}$ &
  ...
  Supertranslation sector of the BMS package / free (linearized) gravity \\
$\mathbf{L}$ &
  $3$ &
  $(2,3)$-generated celestial soft algebra &
  Extended BMS package / semistrict higher-spin gravity \\
$\mathbf{C}$ &
  $4$ &
  $(2,3)$-generated algebra extended by $\mathfrak{q}_4$ &
  Quartic extension of the BMS package / quartic gravity \\
$\mathbf{M}$ &
  $\infty$ &
  Full infinite celestial soft algebra &
  Expected large-$c$ model via $\mathcal{W}_{1+\infty}$ / full gravitational $L_\infty$-package \\
\bottomrule
\end{tabular}
\end{center}
...
reading this is modeled by $\mathcal{W}_{1+\infty}$ as in
...
\item \textbf{(G5)} Gravitational Yangian: The arity-$2$ shadow
  at genus~$0$ is the $r$-matrix, so the leading celestial soft
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item \emph{Loop-level celestial soft factors.}  The genus-$g$ soft
