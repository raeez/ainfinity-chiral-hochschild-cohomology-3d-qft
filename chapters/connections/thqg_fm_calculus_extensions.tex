...
is oriented by the inward-radial convention
\[
\operatorname{or}(\FM_n(\C))
=(-d\varepsilon_S)\wedge\operatorname{or}(D_S).
\]
The outward normal is \(-\partial_{\varepsilon_S}\), and the
regularized residue is
\((\varepsilon_S\iota_{\partial_{\varepsilon_S}}\omega)|_{\varepsilon_S=0}\).

The incidence sign \(\epsilon_{D_S}\in\{\pm1\}\) for a subset
...
\epsilon_{D_S}\in\{\pm1\},
\end{equation}
where the sign is the Stokes-residue incidence number obtained from
the coordinate permutation, the outward-normal factor, and comparison
with the ordered product orientation on \(D_S\). For a consecutive cluster
$S = \{s{+}1, \ldots, s{+}\ell\}$ in $\FM_n(\C)$, this
is computed by the same rule as
Proposition~\ref{prop:general-orient-sign}. The explicit values for
\(n=4\) are:
\(\epsilon_{D_{\{1,2\}}}=+1\),
\(\epsilon_{D_{\{2,3\}}}=+1\),
\(\epsilon_{D_{\{3,4\}}}=-1\),
\(\epsilon_{D_{\{1,2,3\}}}=+1\),
\(\epsilon_{D_{\{2,3,4\}}}=-1\), and
\(\epsilon_{D_{\{1,2,3,4\}}}=-1\)
...
We display the incidence sign \(\epsilon_{D_S}\)
(Definition~\ref{def:fm-boundary-orientation}) and the
effective Stasheff sign \(\tau(S)\) (incorporating the
...
\(\tau(S)\) are determined by the FM-to-Stasheff
identification
...
\noindent
The effective signs \(\tau(S)\) for the six consecutive
...
The computation gives the FM integrals $m_k$ for each of the five
...
$\kappaChHodge(\hat{\mathfrak{g}}_k) = \dim\mathfrak{g}\cdot
...
The computation gives the full associator by evaluating both iterated
compositions. Use the spectral-parameter convention:
...
\item $-(c/6)\,\lambda_1\lambda_2(\lambda_1 + \lambda_2)$:
 the central cubic comparison channel, arising from the
 triple collision of $T$-insertions with three algebraic propagators
 meeting at the $\FM_3(\C)$ interior. After a metric comparison datum
 has been fixed, this channel can be compared with a cubic
 gravitational vertex. Each propagator
...
$\kappaChHodge(\beta\gamma) = c_{\beta\gamma}/2$ introduces
...
$\FM_k(\C)$, giving $m_k \ne 0$ for all $k \ge 3$.
The support depth is \(r^\Theta_{\max}=\infty\), and the
tower is classified as mixed archetype (class M in
the support-depth classification of Vol~I,
...
% ===================================== The half-plane with points% Boundary line (R-direction)
...
+ d_{\mathrm{top}}^2$. Verify each summand
...
$\kappaChHodge(\A) \cdot \omega_g$ (the curvature class).
...
+ \kappaChHodge(\A) \cdot \omega_g$, where the curvature
term $\kappaChHodge(\A) \cdot \omega_g$ acts as a
$0$-ary operation (the curved $\Ainf$ structure
element $m_0$). The identity $d_g^2 = 0$ still holds,
but now as a \emph{curved} identity:
\[
d_{\mathrm{hol}}^2 = -\kappaChHodge(\A) \cdot \omega_g,
\qquad
d_{\mathrm{top}}^2 = 0,
\qquad
\text{cross terms cancel as before.}
\]
The curvature $\kappaChHodge(\A)$ is the modular
...
\section{Synthesis}
\label{sec:fmcalc-ext-supplement}

\begin{remark}[Francis--Morrison calculus at the K3 base]
\label{rem:fmcalc-ext-1}
