...
% Higher-arity FM integrals, explicit A-infinity for standard families,
...
\section{Higher-arity FM integrals and the standard landscape}
\label{sec:fm-calculus-extensions}

\noindent
Section~\ref{sec:FM_calculus} proved the $\Ainf$ Stasheff identities
by Stokes' theorem on $\FM_k(\C)$, with detailed sign computations
through $n = 4$.  This section extends the calculus in three
directions: (i)~the explicit boundary stratification and Stasheff
verification at arities~$4$ and~$5$, exhibiting the associahedron
combinatorics in full; (ii)~the complete $\Ainf$ operations for
...
on the product $\FM_n(\C) \times \Conf_n(\R)$ that produces the
...
\subsection{Higher-arity FM integrals in full}
\label{subsec:higher-arity-fm}

\subsubsection{The boundary stratification of $\FM_4(\C)$}
\label{subsubsec:fm4-stratification}
...
\label{prop:fm4-boundary-strata}
...
The factorization $D_S \cong \FM_{4-|S|+1}(\C) \times \FM_{|S|}(\C)$
...
\label{lem:nonconsecutive-vanishing-ext}%
...
\label{def:fm-boundary-orientation}%
...
coordinates:
\begin{equation}\label{eq:fm-boundary-sign}
\epsilon(S)
\;=\;
(-1)^{\sigma_S},
\end{equation}
...
\label{subsubsec:m4-integral}

\begin{construction}[The $m_4$ weight form]
\label{const:m4-weight-form}
...
\label{subsubsec:fm5-stratification}

\begin{proposition}[Boundary strata of $\FM_5(\C)$;
\ClaimStatusProvedHere]
\label{prop:fm5-boundary-strata}
...
\label{subsec:standard-family-ainf}
...
(Gaussian archetype): the shadow Postnikov tower
terminates at the scalar level $\kappa = 1/2$.
\end{proof}

\begin{remark}[FM interpretation of vanishing]
\label{rem:heisenberg-fm-vanishing}
...
This is shadow depth $r_{\max} = 3$: the shadow tower
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item The binary operation is
\begin{equation}\label{eq:virasoro-m2-ext}
m_2(T, T;\, \lambda)
= \frac{c}{2}\,\lambda^3
+ 2T\,\lambda + \partial T.
\end{equation}
...
\item The ternary operation is
\begin{equation}\label{eq:virasoro-m3-ext}
m_3(T,T,T;\, \lambda_1, \lambda_2)
= -c\,\lambda_1\lambda_2(\lambda_1 + \lambda_2)
...
\begin{equation}\label{eq:virasoro-quartic-ext}
...
\item Quartic pole $c/2 \cdot (z-w)^{-4}$:
  $\frac{c}{2} \cdot \frac{\lambda^3}{3!} \cdot 3!
  = \frac{c}{2}\lambda^3$.
  (The factor $3! = 6$ arises from the normalization
  $\lambda^n/n!$ in the $\lambda$-bracket convention,
  combined with the residue extraction at order $n+1$;
  the net effect is $c/(2 \cdot 6) \cdot 6 = c/2$
  times $\lambda^3$ as in the
  $\lambda$-bracket~\eqref{eq:virasoro-lambda-bracket-ext}
  with the identification $c/12 \cdot \lambda^3$ being
  the coefficient of $\lambda^3$ in
  $\{T{}_\lambda T\}$, which translates to $c/2 \cdot
  \lambda^3$ in the $m_2$ normalization.)
...
= \frac{c}{2}\lambda_1^3 + 2T\lambda_1 + \partial T$:
\begin{align*}
&m_2\!\left(\frac{c}{2}\lambda_1^3 + 2T\lambda_1
+ \partial T,\; T;\; \lambda_2\right)
\\
&= \underbrace{\frac{c}{2}\lambda_1^3
...
\\
&= 0
+ 2\lambda_1\!\left[\frac{c}{2}\lambda_2^3 + 2T\lambda_2
+ \partial T\right]
+ \left[-\lambda_2\!\left(\frac{c}{2}\lambda_2^3 + 2T\lambda_2
+ \partial T\right)\right]
\\
&= c\lambda_1\lambda_2^3 + 4T\lambda_1\lambda_2
+ 2(\partial T)\lambda_1
- \frac{c}{2}\lambda_2^4 - 2T\lambda_2^2
- (\partial T)\lambda_2.
\end{align*}
...
= \frac{c}{2}\lambda_2^3 + 2T\lambda_2 + \partial T$:
\begin{align*}
&m_2\!\left(T,\; \frac{c}{2}\lambda_2^3 + 2T\lambda_2
+ \partial T;\; \lambda_1\right)
\\
&= \underbrace{\frac{c}{2}\lambda_2^3
...
\\
&= 0
+ 2\lambda_2\!\left[\frac{c}{2}\lambda_1^3 + 2T\lambda_1
+ \partial T\right]
+ (\lambda_1 + \partial)\!\left[\frac{c}{2}\lambda_1^3
...
(\lambda_1 + \partial)\!\left[\frac{c}{2}\lambda_1^3
+ 2T\lambda_1 + \partial T\right]
&= \frac{c}{2}\lambda_1^4 + 2T\lambda_1^2
+ (\partial T)\lambda_1
+ \underbrace{\partial\!\left(\frac{c}{2}\lambda_1^3\right)}_{= 0}
...
\[
\text{Right}
= c\lambda_2\lambda_1^3 + 4T\lambda_1\lambda_2
...
\begin{align*}
\text{$c$-terms:} &\quad
  c\lambda_1\lambda_2^3 - \tfrac{c}{2}\lambda_2^4
  - c\lambda_1^3\lambda_2 - \tfrac{c}{2}\lambda_1^4, \\
\text{$T$-terms:} &\quad
...
\end{align*}
Therefore:
\begin{equation}\label{eq:virasoro-associator-ext}
\boxed{
A_3(T,T,T)
= c\lambda_1\lambda_2^3
- \frac{c}{2}(\lambda_1^4 + \lambda_2^4)
- c\lambda_1^3\lambda_2
...
yields:
\begin{equation}\label{eq:virasoro-m3-derivation}
m_3(T,T,T;\, \lambda_1, \lambda_2)
= -c\,\lambda_1\lambda_2(\lambda_1 + \lambda_2)
- 4T(\lambda_1 + \lambda_2)
- 4\,\partial T.
...
\begin{itemize}
\item $-c\,\lambda_1\lambda_2(\lambda_1 + \lambda_2)$:
...
new terms at each arity that cannot be absorbed by the
...
The general pattern: at arity $k$, $m_k$ is given by
...
\label{subsubsec:betagamma-ainf}

\begin{theorem}[$\beta\gamma$ $\Ainf$ structure;
\ClaimStatusProvedHere]
\label{thm:betagamma-ainf}
...
\begin{equation}\label{eq:betagamma-lambda-bracket}
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item The binary operation is
\begin{equation}\label{eq:betagamma-m2}
...
\begin{equation}\label{eq:betagamma-quartic}
\mu_{\beta\gamma}
= \langle \eta,\, m_3(\eta,\eta,\eta) \rangle
= 0
\end{equation}
on the weight-changing line (by rank-one abelian
rigidity, Corollary~\ref*{cor:nms-betagamma-mu-vanishing}
...
shadow Postnikov tower.  The quartic invariant
...
\medskip
\textbf{Step 4: Termination at arity $5$.}
The quintic obstruction vanishes: all Stasheff compositions
at arity $5$ involve $m_3$ or $m_4$ in non-curved positions,
...
term
\begin{equation}\label{eq:w3-m2-WW}
m_2(W,W;\, \lambda)
= \frac{c}{360}\lambda^5
+ \beta\!\left[\frac{c}{2}\lambda^3 + 2T\lambda
...
\item Sextic pole $c/3 \cdot (z-w)^{-6}$:
  contributes $\frac{c}{3} \cdot \frac{\lambda^5}{5!}
  \cdot 5! = \frac{c}{3} \cdot \frac{\lambda^5}{120}
  \cdot 120$.  In the $\lambda$-bracket normalization,
  the coefficient of $\lambda^5$ in $\{W{}_\lambda W\}$
  is $c/(3 \cdot 5!) = c/360$, giving $m_2$ term
  $\frac{c}{360}\lambda^5$.
\item Quartic pole: $2T \cdot \lambda^3/3! \cdot 3!
  = 2T\lambda^3$ after Borel, but this is the
  Zamolodchikov $\beta$-dependent piece.
...
arity that cannot be absorbed by the weight-$3$ null
...
\S\ref*{sec:concordance-koszulness-programme}).
\end{proof}

\begin{remark}[Shadow depth classification revisited]
\label{rem:shadow-depth-table-ext}
...
\label{subsubsec:product-structure}

\begin{definition}[The product configuration space]
\label{def:product-config}
For $n \ge 1$, define
\begin{equation}\label{eq:product-config}
\mathcal{C}_n
:= \FM_n(\C) \times \Conf_n^<(\R),
\end{equation}
...
composition}
\label{subsubsec:hol-collision}

\begin{proposition}[Holomorphic boundary contributions;
\ClaimStatusProvedHere]
\label{prop:hol-boundary-contribution}
...
\begin{equation}\label{eq:hol-boundary-residue}
...
\begin{equation}\label{eq:coproduct-from-top}
...
This is precisely the bar coproduct: the topological
...
\label{subsubsec:swiss-cheese-chain}

\begin{theorem}[Swiss-cheese structure on $\mathcal{C}_n$;
\ClaimStatusProvedHere]
\label{thm:swiss-cheese-product}
The collection
...
\begin{equation}\label{eq:sc-composition}
...
\begin{corollary}[Bar complex = Swiss-cheese algebra;
\ClaimStatusProvedHere]
\label{cor:bar-swiss-cheese}
...
boundary), is a $\SCchtop$-algebra.  The bar
...
\kappa(\A) \cdot \omega_g$, making $B(\A)$ a
\emph{curved} Swiss-cheese algebra.
\end{corollary}
...
\begin{remark}[Genus $g \ge 1$: curved Swiss-cheese]
\label{rem:curved-swiss-cheese}
