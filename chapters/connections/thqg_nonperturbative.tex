...
$K3\times K3$~\cite[\S 2]{Vafa1996F} to M-theory on
...
\begin{theorem}[Borcherds product of $\Delta_5$ is the
...
triangle, has scalar automorphic denominator given by the
Gritsenko--Nikulin product for the primitive denominator $\Delta_5$.
Write
\[
 \phi_{0,1}^{K3}(\tau,z)=\sum_{n\geq 0,\ r\in\mathbb Z} f(n,r)q^n y^r.
\]
...
weight-zero index-one weak Jacobi form $\phi_{0,1}^{K3}$; in the
...
and collecting the $p^n$-Fourier coefficient yields (after the two
separating \(\eta^{12}\) normalisations absorb the one-loop paramodular anomaly,
...
(i)~the Eichler--Zagier tabulation of $\phi_{0,1}^{K3}$-coefficients
...
Let \(\mathscr E_\hbar\) be the \(p\)-adic loop-order homomorphism
\[
\mathscr E_\hbar\!\left(\sum_{n\geq 0}a_n(q,y)p^n\right)
=\sum_{n\geq 0}a_n(q,y)\hbar^n .
\]
Let
$\mathcal{Z}^{\mathrm{pert}}_{\mathrm{HT}}(\hbar, Z)
:= \sum_{n\geq 0}\hbar^n c_n^{\mathrm{triangle}}(Z)$
...
complex-moduli base \(Z=(\tau,z,\sigma)\in\overline{\mathcal{A}_2}\). Assume:
...
transseries;
\item the homomorphism \(\mathscr E_\hbar\) identifies Fourier--Jacobi
order with loop order in the chosen $\hbar$-expansion.
...
\begin{equation}\label{eq:thqg-np-completion}
 \mathcal{Z}^{\mathrm{full}}_{\mathrm{HT}}(\hbar, Z)
 \;=\; \mathcal{Z}^{\mathrm{pert}}_{\mathrm{HT}}(\hbar, Z)
 \;+\;\!\!\sum_{[C]\in H_2^+(K3)_{\rig}}\!\!
 e^{-2\pi\vol(C)/|\hbar|}\cdot c^{\mathrm{D3}}_{[C]}
 \;+\; \sum_{q = 1}^{7}\Gamma^{\mathrm{Schwinger}}_{H_1, q}(\hbar),
\end{equation}
and its automorphic part is the separating-boundary normalised
Igusa square \(\Phi_{10}^{\mathrm{un}}(Z)=\Delta_5(Z)^2\):
\begin{equation}\label{eq:thqg-np-phi10-identity}
 \mathcal{Z}^{\mathrm{full}}_{\mathrm{HT}}(\hbar, Z)
 \;=\;
 \mathscr E_\hbar\!\left[
 \log\frac{\Phi_{10}^{\mathrm{un}}(Z)}
 {\eta(\tau)^{24}\eta(\sigma)^{24}}
 \right],
\end{equation}
where the \(p\)-adic Fourier--Jacobi expansion is taken at the chosen
separating cusp. At Lusztig specialisation $\hbar_{\mathrm{qg}}^2 =
...
\(\mathscr E_\hbar\log\bigl(\Phi_{10}^{\mathrm{un}}/
(\eta(\tau)^{24}\eta(\sigma)^{24})\bigr)\): this is the content of
...
\begin{remark}[Five independent derivations]
...
threshold correction on $K3\times T^2$ as
\(\log(\Phi_{10}^{\mathrm{un}}/(\eta(\tau)^{24}\eta(\sigma)^{24}))\)
directly, with D$3$-instanton tail given by Fourier coefficients of
$\phi_{0,1}^{K3}$ term by term.
\item \emph{$F$-theory path.} Vafa~$1996$
\cite[\S 2]{Vafa1996F} identifies Euclidean D$3$-branes wrapping
...
\end{enumerate}
All five derivations agree on the Fourier coefficients of $\phi_{0,1}^{K3}$; the
leading values
\[
...
Costello~$2011$ Chapter~$4$~\cite[Ch.~4]{costello-renormalization} supplies the
...
\subsection{Cross-channel resurgent structure on the multi-weight tower}
\label{subsec:cross-channel-resurgence}
\index{resurgent structure!cross-channel multi-weight}

The scalar tower $F_g^{\mathrm{scal}}(\cA) = \kappaChHodge(\cA)\,\lambda_g^{\mathrm{FP}}$
is Gevrey-$0$. The cross-channel correction
$\delta F_g^{\mathrm{cross}}$ on multi-weight algebras (the $\cW_3$
witness above) grows factorially in $g$, and Proposition~\ref{prop:cross-channel-truncated-no-closed-form}
shows no separable closed-form generating function exists. The
resurgent structure of $\delta F_g^{\mathrm{cross}}$ is the chain-level
trans-series datum that completes the
classical-to-quantum bridge of CP3.

\begin{theorem}[Resurgent structure of $\delta F_g^{\mathrm{cross}}$;
licensing $\alpha + \beta + \gamma + \delta + \varepsilon$;
\ClaimStatusConditional{} on F3 closure of $A_{\mathrm{cross}}(c)$
and $\hypStokes$]
\label{thm:cross-channel-resurgent-structure}
\index{cross-channel resurgent structure}
\index{Borel transform!cross-channel logarithmic branch}
\index{alien derivative!cross-channel bridge equation}
Let $A_{\mathrm{cross}}(c)$ denote the cross-channel instanton action
on the multi-weight $\cW_3$ tower, with the bound
$A_{\mathrm{cross}} / A_{\mathrm{scal}} \in [1.7741,\, 3.1]$ from the
$g = 2, 3, 4$ data \textup{(}lower bound $\sqrt{56/(R_4/R_3)} = 1.7741$,
$R_3 = 42/31$, $R_4 = 9184/381$, $A_{\mathrm{scal}} = (2\pi)^2$\textup{)}.
The Gevrey-$1$ Borel transform
\[
 B^{\mathrm{cross}}_c[\delta F^{\mathrm{cross}}](\xi)
 \;=\;
 \sum_{g \ge 2} \delta F_g^{\mathrm{cross}}(c)\, \xi^{g - 1}/\Gamma(g)
\]
has logarithmic branch points at
\[
 \xi_n^{\pm} \;=\; \pm n A_{\mathrm{cross}}(c),
\qquad n \in \Z_{\ge 1},
\]
the $\pm$-doubling inheriting from the Koszul $c \leftrightarrow 26 - c$
self-duality of the scalar bar lane via the Yuan--Latyntsev
$R$-matrix. The alien derivative at the one-instanton sector
satisfies Ecalle's bridge equation
\[
 \Delta_{A_{\mathrm{cross}}}\,\mathcal Z(\hbar)
 \;=\;
 S_1^{\mathrm{cross}}(c)\,
 e^{-A_{\mathrm{cross}}/\hbar}\,
 \mathcal Z^{(1)}(\hbar),
\qquad
 S_1^{\mathrm{cross}}(c)
 \;=\;
 \frac{1}{2 \pi i}\,
 \det\!\Bigl( R_+(z) R_-(z)^{-1} \Bigr)\Big|_{z = z_{\mathrm{cross}}(c)},
\]
with $R(z)$ the Givental $R$-matrix of the $A_2$ Frobenius CohFT
and $z_{\mathrm{cross}}(c)$ the cross-channel Stokes ray. The
trans-series completion of the partition function reads
\[
 Z(\hbar)
 \;=\;
 Z^{\mathrm{pert}}(\hbar)
 \;+\;
 \sum_{n \ge 1}
  c_n^{\mathrm{cross}}(\hbar)\,
  e^{-n A_{\mathrm{cross}}(c)/\hbar}\,
  Z^{(n)}(\hbar),
\]
with $Z^{\mathrm{pert}}(\hbar) = \kappaChHodge \cdot
(\sqrt\hbar/2)/\sin(\sqrt\hbar/2)$ and $Z^{(n)}$ a polynomial of degree
$\le n^2$ in $\hbar$ on the semisimple $A_2$ Frobenius locus
\textup{(}Aniceto--Basar--Schiappa~2019\textup{)}.
\end{theorem}

\begin{proof}
The logarithmic branch type is forced by the Gevrey-$1$ ansatz: a
square-root would give half-instantons, inconsistent with the integer
genus expansion. The $\pm$-doubling is the operadic shadow of the
Koszul self-duality $\mathrm{Vir}_c \leftrightarrow \mathrm{Vir}_{26 - c}$
acting on the bar coalgebra and transported through the
Yuan--Latyntsev $R$-matrix to the cross-channel sector. The bridge
equation is Ecalle's universal identity \cite{Ecalle1981} once the
sectorial analytic continuation is supplied by $\hypStokes$. The
Stokes constant formula is the discontinuity of the Givental
$R$-matrix at the Stokes ray $z_{\mathrm{cross}}(c)$, which lies on
the Dubrovin connection of the $\cW_3$ Frobenius manifold. The
trans-series polynomial bound on $Z^{(n)}$ follows from
Givental--Teleman semisimplicity on the $A_2$ locus. The $A_{\mathrm{cross}}$
bound is recovered analytically from the Gevrey-$1$ second-difference
test on the ratios $R_3, R_4$ given in the statement; once F3
delivers the closed form $A_{\mathrm{cross}}(c)$ on the $A_2$
Frobenius manifold, the trans-series is uniquely determined.
\end{proof}
