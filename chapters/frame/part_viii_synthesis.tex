...
%%
%% Synthesis-only file. Compareerences existing theorems by \label;
...
%% =====================================
\section{Four threads converging on the Koszulness Moduli scheme
...
The proof is conditional on the data stated there: (i) a
Drinfeld--Sokolov special deformation retract
\(Q_{\mathrm{DS}}h+hQ_{\mathrm{DS}}=\mathrm{id}-i\circ p\) in the
completed ambient;
(ii) a chiral HKR/brace model functorial for the DS BRST complex; and
(iii) convergence of the HPL planar-tree sums in
\(\hypAmbientWtCpl\). Arakawa's DS and associated-variety results
enter on their actual lisse/admissible or stated finite surfaces; they
are not the false assertion that every generic non-critical
\(W\)-algebra is \(C_{2}\)-cofinite.
...
weight-completed/pro ambient: after the DS SDR, HKR/brace,
completion, HT comparison, and mixed-lift hypotheses are supplied, the
bulk is the completed derived chiral centre. The strict original-complex
$E_3$ chain lift remains conditional as recorded in the chiral Higher
Deligne and universal-holography chapters.
...
through the completed HPL hypothesis, not through generic
\(C_{2}\)-cofiniteness. Weight-completeness permits the HPL transfer
to be formulated weightwise; convergence in the inverse limit is a
separate hypothesis in Theorem~\ref{thm:chd-ds-hochschild}. The
chain-level quasi-isomorphism survives completion only on that
hypothesis package, which is why
Corollary~\ref{cor:universal-holography-class-M} is conditional in the
weight-completed/pro ambient.
...
The associator-independent chain-level formulation is not a theorem:
it is killed by the non-trivial
$\mathrm{GRT}_{1}(\Q)$-torsor of Drinfeld associators
(cf.\ Section~\ref{sec:chd-not-proved}).  The
associator-free statement lives on cohomology and on the bar-side
invariants \(\kappa\), the shadow tower, and Koszul depth; the
chain-level mixed object is a \(\Phi\)-dependent representative. The
chiral structure uses this torsor to distinguish faces
(Section~\ref{sec:part-viii-koszulness-moduli}), so the
...
The structural form of the four theorem threads is the moduli scheme
...
chapter: branchwise reconstruction of $E_{1}$-chiral algebras on the
...
$\kappaChHodge$ separates the Heisenberg levels
(Proposition~\ref{prop:six-slot-distinguishes-ch}). The completeness
criterion is Theorem~\ref{thm:fingerprint-complete-ch}: equality of
fingerprints becomes reconstructive only after the branchwise datum
\(\mathfrak R_c\) is installed.
...
mechanism making this a forced one-loop identification rather than a
...
%% =====================================
\section{The closed atlas: seven theorems, the six-slot fingerprint,
and the $\mathrm{GRT}_{1}(\Q)$-parametrised face family}
...
Six-slot branchwise fingerprint reconstruction & Theorem &
...
read with the hypotheses of its home theorem. Three obligations lie
outside the atlas: the degenerate admissible logarithmic lane; the
\(\Phi\)-dependent chain-level Deligne--Tamarkin torsor, whose
associator-independent chain-level collapse is false; and finite
improvements to the weight-completed bridge. These are formulated in
...
The structural form is the closed atlas: seven theorems, an infinite
