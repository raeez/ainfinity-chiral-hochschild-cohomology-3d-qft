...
$\cL \times_{\cM}^h \cL$ within the chirally Koszul world
(all four classes satisfy PBW concentration,
...
\item \emph{Virasoro} (class~$\mathbf{M}$, depth~$\infty$):
  ...
  The self-intersection is not clean, but the Virasoro
  is still chirally Koszul; shadow depth $\infty$ measures
  the complexity of the self-intersection geometry, not
  a failure of chiral Koszulness.
\end{itemize}
\end{remark}
...
mirror symmetry is \emph{MC gauge equivalence}
