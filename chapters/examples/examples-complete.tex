...
\begin{construction}[Shadow archetype classification]
% label removed: constr:vol2-shadow-archetype
Each chiral algebra $\cA$ in the standard landscape carries a shadow archetype classifying its nonlinear complexity:
\begin{center}
\begin{tabular}{cccl}
\toprule
Class & Depth $r_{\max}$ & Archetype & Examples \\
\midrule
$\mathsf{G}$ & $2$ & Gaussian & Heisenberg, free bosons, lattice VOAs \\
$\mathsf{L}$ & $3$ & Lie/tree & $\hat{\mathfrak{g}}_k$, affine \\
$\mathsf{C}$ & $4$ & Contact & $\beta\gamma$, symplectic fermions \\
$\mathsf{M}$ & $\infty$ & Mixed & $\mathrm{Vir}_c$, $\mathcal{W}_N$, $\mathcal{W}_{1+\infty}$ \\
\bottomrule
\end{tabular}
\end{center}
Shadow depth classifies the \emph{complexity} of the Lagrangian self-intersection within the standard families. All four classes are chirally Koszul (Vol~I, PBW universality); class~$\mathsf{G}$ is formal, classes~$\mathsf{L}$ and~$\mathsf{C}$ have finite Lie and contact transfer, and class~$\mathsf{M}$ (Virasoro, $\mathcal{W}_N$) has infinite shadow depth and non-formal Swiss-cheese structure, with well-defined Koszul duals carrying higher $A_\infty$ operations (Vol~I, Theorem~\ref*{V1-thm:koszul-equivalences-meta}).
\end{construction}
...
\subsubsection{Free Sesquilinearity}

The free bracket satisfies the sesquilinearity axioms
...
be the contracting homotopy on \((\phi^2)\). The homological
perturbation formula writes \(m_n^J\) as a sum over rooted trees whose
vertices are the Taylor tensors \(W^{(r)}\), whose internal edges are
\(h\), and whose root is followed by \(\pi\). Since
\[
...
(see Section \ref{sec:line_operators}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Example IV: Nonabelian Chern--Simons ($SU(2)$ at level $k$)}
% label removed: ex:nonabelian_CS_complete
...
$m_3$, and the open-colour Koszul dual is the Yangian
$Y(\mathfrak{sl}_2)$.
...
\begin{equation}% label removed: eq:su2-cs-action
...
The BV-BRST differential acts by
\begin{equation}% label removed: eq:su2-brst
...
\begin{equation}% label removed: eq:su2-propagator
...
% label removed: prop:su2-lambda-bracket
Assume \textrm{(H1)--(H4)}. Setting
$J^a(z) := A_z^a(t,z)|_{t=0}$ (boundary current), the
$\lambda$-bracket is:
\begin{equation}% label removed: eq:su2-lambda
\{J^a{}_\lambda J^b\}
...
\begin{proposition}[Nonabelian $m_3$]
% label removed: prop:su2-m3
Assume \textrm{(H1)--(H4)}. The cubic operation $m_3$ is given
by the triangle Feynman diagram with three external legs
connected by propagators through two cubic vertices:
\begin{equation}% label removed: eq:su2-m3
...
$f^{a}{}_{de}\, f^{ebc}$. The FM integral converges by
hypothesis~(H3). The Stokes argument: the integrand is a
...
% label removed: prop:su2-truncation
Assume \textrm{(H1)--(H4)}. For $SU(2)$ Chern--Simons:
\begin{enumerate}[label=\textup{(\roman*)}]
\item $m_4 \neq 0$ at the chain level: diagrams with three cubic
 vertices contribute. This chain-level contribution is $Q$-exact.
...
% label removed: thm:su2-pva
Assume \textrm{(H1)--(H4)}. The cohomology
...
By Theorem~\ref{thm:lines_as_modules}, the identification
$\mathcal{C}_{\mathrm{line}} \simeq \cA^!_{\mathrm{line}}\text{-}\mathbf{mod}$
shows that the Koszul dual
$\cA^!_{\mathrm{line}}$ of the boundary algebra controls line operators.

\begin{theorem}[Open-colour Koszul dual is $Y(\mathfrak{sl}_2)$]
% label removed: thm:su2-yangian
Assume \textrm{(H1)--(H4)}. The open-colour Koszul dual of
...
\item spectral $R$-matrix from the Laplace transform
 ...
 \begin{equation}% label removed: eq:su2-rmatrix
 r(z) = \frac{k\,\Omega}{z},
 \qquad
...
$k\,\delta^{ab}/z^2$ gives, after $d\log$ absorption, the collision $r$-matrix $r(z) = k\,\Omega_{\mathrm{tr}}/z$, the standard rational
...
% label removed: prop:su2-curvature
Assume \textrm{(H1)--(H4)}. The genus-$1$ curvature
(Volume~I, Theorem~D) evaluates to
\begin{equation}% label removed: eq:su2-curvature
...
\begin{proof}
The general formula for the modular characteristic of affine
$\widehat{\fg}_k$ is $\kappa = \dim(\fg)\cdot(k+h^\vee)/(2h^\vee)$
(Vol~I, Theorem~D). For $\fg = \mathfrak{sl}_2$
with $\dim\mathfrak{sl}_2 = 3$ and $h^\vee = 2$, this gives
$\kappa = 3(k+2)/4$.
Under $k \mapsto k' = -k - 2h^\vee = -k - 4$:
$\kappaChHodge(k') = 3(-k-4+2)/4 = 3(-k-2)/4 = -3(k+2)/4$,
so $\kappaChHodge(k) + \kappaChHodge(k') = 3(k+2)/4 - 3(k+2)/4 = 0$,
confirming complementarity $\kappa + \kappa' = 0$
(the constant is independent of~$k$).
\end{proof}

\subsubsection{Verification of (H1)--(H4)}
% label removed: subsubsec:su2-verification

\begin{proposition}[Standing hypotheses for $SU(2)$ CS]
% label removed: prop:su2-hypotheses
The hypotheses \textrm{(H1)--(H4)} are verified for $SU(2)$
Chern--Simons theory at integer level~$k \geq 1$:
...
\item Factorization compatibility: the BV observables factor
 ...
 $C_\ast(W(\SCchtop))$ follows from the product-formal local-shadow
 recognition theorem (Theorem~\ref{thm:recognition}).
\end{enumerate}
\end{proposition}
...
(Proposition~\ref*{V1-prop:chriss-ginzburg-structure} of Vol~I), Seiberg
duality is MC gauge equivalence
(strict model; gauge equivalence in $\Convinf$, Convention~\ref*{V1-rem:two-level-convention} of Vol~I):
$\Theta_{\mathrm{SU}(N)} \sim_{\mathrm{MC}}
\Theta_{\mathrm{SU}(N_f-N)}$ in the ambient algebra
$\mathfrak{g}^{\mathrm{amb}}$. The shadow algebras
(Definition~\ref*{V1-def:shadow-algebra} of Vol~I) of both sides are
