...
recursions, one for each ordered input type.  The key insight is that
...
at arity~$k$, provided the inputs are $Q$-closed
...
pole order (AP19).  All inner products are BPZ, not Fock (AP26).
We write $\ell = \lambda_1$, $\mu = \lambda_2$, $\nu = \lambda_3$
as abbreviations for the spectral parameters.

\subsubsection{The Stasheff Relation at Arity $k$}
...
For arity~$k$ with inputs from $\{T, W\}$, there are $2^k$ ordered
input types.  We write $m_k(X_1, \ldots, X_k)$ for each.  At each
arity, the Stasheff relation~\eqref{eq:w3-stasheff-k} applied to a
...
\subsubsection{Arity 3: Review of Known $m_3$}
...
at arity~3, which for $Q$-closed inputs reads
\[
...
This pattern persists at higher arity.
\end{remark}

\subsubsection{Arity 4: The $m_4$ Operations from Stasheff}

At arity~4, the Stasheff relation~\eqref{eq:w3-stasheff-k} for
...
More precisely, $m_4(T,T,T,T)$ equals the negative of the arity-4
...
\subsubsection{Arity 5: The $m_5$ Operations}

At arity~5, the Stasheff relation involves $m_2 \circ m_4$,
...
\begin{proof}
The Stasheff identity~\eqref{eq:w3-stasheff-k} at each arity~$k$ is
precisely the component of the Maurer--Cartan
equation~\eqref{eq:w3-MC-system} at arity~$k$.  The Gerstenhaber
...
variable $t$ (or $w$) extracts the arity-$(k+1)$ component with one
...
in $T$).  At arity~$k$, the Stasheff compositions produce
...
force genuine coupling to composite fields at every arity.
\end{remark}
...
\medskip
\noindent
\textbf{Degree 1} ($\HH^1$): The space of outer derivations.
\begin{equation}
\HH^1_{\mathrm{ch}}(W_3) \cong \C\langle \partial_c T,\, \partial_c W \rangle \cong \C^2.
\end{equation}
\begin{itemize}
\item The class $\partial_c T$ represents the infinitesimal deformation
  of $T$ by varying the central charge: $T \mapsto T + \epsilon\, \partial_c T$,
  with $\partial_c m_2(T,T) = \tfrac{1}{12}\lambda^3 \ne 0$.
\item The class $\partial_c W$ represents the induced deformation of $W$:
  $W \mapsto W + \epsilon\, \partial_c W$, with $\partial_c m_2(W,W)
  = \tfrac{1}{360}\lambda^5 + \tfrac{32}{(5c+22)^2}(\ldots)\ne 0$.
\end{itemize}
In bi-graded notation: $\HH^{1,(1,0)} \cong \C$ (the $\partial_c T$ class)
and $\HH^{1,(0,1)} \cong \C$ (the $\partial_c W$ class).

\medskip
\noindent
\textbf{Degree 2} ($\HH^2$): The obstruction space.
\begin{equation}
\HH^2_{\mathrm{ch}}(W_3)
= \HH^{2,(2,0)} \oplus \HH^{2,(1,1)} \oplus \HH^{2,(0,2)}.
\end{equation}
\begin{itemize}
\item $\HH^{2,(2,0)}_{\mathrm{ch}}(W_3) = 0$: no obstructions to
  deforming the $T$--$T$ bracket alone. This follows because the
  Virasoro algebra is rigid as a Lie conformal algebra
  (Fialowski--Schlichenmaier~\cite{FiaSch12}): every infinitesimal
  deformation extends.
\item $\HH^{2,(1,1)}_{\mathrm{ch}}(W_3) \cong \C$: there is a
  one-dimensional obstruction space controlling the $T$--$W$ mixing.
  The nontrivial class represents the constraint that changing $c$ in
...
\item $\HH^{2,(0,2)}_{\mathrm{ch}}(W_3) \cong \C$: a one-dimensional
  obstruction from the $W$--$W$ sector.  The nontrivial class is the
  Zamolodchikov $c_{334}^2$ structure constant, which is determined by
  $c$ and the normalisation convention for $W$.
  This class represents the rigidity of the $W_3$ algebra: at generic~$c$,
  there is no continuous family of deformations beyond varying~$c$.
\end{itemize}
...
\hline
\text{dimension} & 1 & 2 & 2 & 0\ (\text{generic } c) \\
\text{generators} & \C & \partial_c T,\, \partial_c W
  & \HH^{2,(1,1)},\, \HH^{2,(0,2)} & - \\
\end{array}
\end{equation}
\end{theorem}
...
$\delta_a(b) = \{a_\lambda b\}|_{\lambda=0}$ for $a \in W_3$.  The
outer derivation space $\HH^1 = \mathrm{Der}(W_3)/\mathrm{Inn}(W_3)$
consists of derivations not of this form.

The derivation $\delta_c: T \mapsto \partial_c T = \tfrac{\partial}{\partial c}
\{T_\lambda T\}\big|_{\lambda=0}$ (and similarly for $W$) is outer
because the $c$-dependence of the brackets is not captured by any inner
derivation (the central charge is not an element of $W_3$).

To show $\dim \HH^1 = 2$: any derivation $\delta$ of $W_3$ is
determined by $\delta(T)$ and $\delta(W)$.  The compatibility
$\delta\{T_\lambda T\} = \{(\delta T)_\lambda T\} + \{T_\lambda (\delta T)\}$
constrains $\delta(T)$ to lie in a finite-dimensional space.  A direct
computation using the $\lambda$-bracket structure shows $\delta(T)
= \alpha\, \partial_c T + (\text{inner})$ and $\delta(W) = \beta\,
\partial_c W + (\text{inner})$, with $\alpha, \beta$ related by the
compatibility of the $\{T_\lambda W\}$ bracket.  At generic~$c$ (away
from $c = 0$ and $c = -22/5$), $\alpha$ and $\beta$ are independent,
giving $\dim \HH^1 = 2$.

\emph{$\HH^2$:} The bi-graded computation proceeds sector by sector.

$(2,0)$-sector: a $2$-cocycle $\varphi: T \otimes T \to W_3$ satisfying
$d_{\mathrm{Hoch}} \varphi = 0$ with $\varphi$ non-exact.
The rigidity of the Virasoro Lie conformal algebra (i.e.,
$\HH^2(\mathrm{Vir}) = 0$) implies $\HH^{2,(2,0)} = 0$.
...
For Virasoro: $\HH^0 \cong \C$, $\HH^1 \cong \C$ (one parameter $c$),
$\HH^2 = 0$ (rigid).  For $W_3$: $\HH^0 \cong \C$,
$\HH^1 \cong \C^2$ (two parameters, but constrained to vary together by
the Jacobi identity, reducing the moduli to one-dimensional as expected),
$\HH^2 \cong \C^2$ (two obstruction classes, both trivially satisfied
for the Zamolodchikov family).  The ``extra'' $\HH^1$ and $\HH^2$
dimensions for $W_3$ reflect the richer algebraic structure of two
generators with cross-brackets.
\end{remark}
...
$\dim \HH^1_{\mathrm{eff}} = 1$ (the effective moduli space is
one-dimensional), even though the formal bi-graded count gives
$\dim \HH^1 = 2$.  The Zamolodchikov constraint is the unique
relation in $\HH^{2,(0,2)}$ that forces $\beta = \beta(\alpha)$ in
the derivation $\delta = \alpha\, \partial_c T + \beta\, \partial_c W$.
\end{remark}
...
The weight-$\le 3$ system additionally includes
