\section{W-Algebra Computations Under KZ Analytic SDR}

The following computations extend the stable core of
\S\ref{sec:W-algebras} to the $W_3$ results under KZ analytic SDR,
quantization data, explicit mode formulas, and computational algorithms.
The ambient is the finite-pole principal Drinfeld--Sokolov $W_3$
model, together with the analytic SDR hypotheses explicitly stated
below. No statement in this section applies to logarithmic triplet
$\cW(p)$ algebras unless a separate regular-sector shadow envelope and
the logarithmic $TW/WW$ factorisation map have been supplied. For
physical realisations, the conditions of Theorem~\ref{thm:physics-bridge}
apply.

%% =====================================
%% W_3 results under KZ analytic SDR
%% =====================================

\subsection{$W_3$ $A_\infty$ operations under KZ analytic SDR}

\subsubsection{$\lambda$-Brackets $m_2$}

\begin{proposition}[$W_3$ Binary Operations]
% label removed: prop:w3-m2
Assume the KZ $W_3$ realization satisfies the analytic SDR package of
Assumption~\textup{\ref{assum:kz-analytic-sdr-package}} and the
Virasoro wheel-residue/split-projection hypotheses.
...
Explicit formulas beyond the displayed arities require a finite-pole
recursion for the corresponding graph residues.

\subsubsection{Spectral $r$-Matrix for $W_3$}

The $W_3$ spectral $r$-matrix is a $2 \times 2$ matrix in the $\{T, W\}$ basis:
\begin{equation}
K^{W_3}(u, v) = \begin{pmatrix}
K^{TT}(u,v) & K^{TW}(u,v) \\
K^{WT}(u,v) & K^{WW}(u,v)
\end{pmatrix}.
\end{equation}
...
\begin{align}
K^{TT}(u,v) &= \frac{c/12}{u^4 v} + \frac{2T \otimes \mathbf{1}}{u^2 v} + \frac{(\partial T) \otimes \mathbf{1}}{u v},\\
K^{TW}(u,v) &= \frac{3W \otimes \mathbf{1}}{u^2 v} + \frac{(\partial W) \otimes \mathbf{1}}{u v},\\
K^{WW}(u,v) &= \frac{c/360}{u^6 v} + \frac{T \otimes \mathbf{1}}{3u^4 v} + \frac{(\partial T) \otimes \mathbf{1}}{2u^3 v} \\
&\quad + \frac{(32/(5c+22))\,\Lambda \otimes \mathbf{1}}{u^2 v} + \frac{(16/(5c+22))\,(\partial\Lambda) \otimes \mathbf{1}}{u v}.
\end{align}
The entry $K^{WT}$ is determined from $K^{TW}$ by PVA skew-symmetry.

\begin{theorem}[$W_3$ PVA Jacobi kernel; \ClaimStatusProvedElsewhere]
% label removed: thm:w3-CYBE
Let\/ $\cA=\mathcal W_{3,c}$ be the principal DS $W_3$ algebra with
the standard finite-pole $\lambda$-brackets.
The coefficient kernel encodes the vanishing PVA Jacobiator:
\begin{equation}
\{X_\lambda \{Y_\mu Z\}\}
- \{Y_\mu \{X_\lambda Z\}\}
- \{\{X_\lambda Y\}_{\lambda+\mu} Z\}
\;=\;0
\end{equation}
for $X,Y,Z\in\{T,W\}$. No non-dynamical CYBE for this
field-dependent kernel is asserted here.
\end{theorem}

\begin{proof}
The statement is the Jacobi identity for the Zamolodchikov
$W_3$ Poisson vertex algebra in the standard finite-pole
normalisation~\cite{BouFeiMal90}.
...
\begin{proposition}[DS-compatible $W_3$ Hochschild deformation classes]
% label removed: prop:w3-hochschild
Let\/ $\cA=\mathcal W_{3,c}$, and compute chiral Hochschild cochains
in the bi-graded finite-pole model generated by $T$ and $W$ at generic
$c$ and away from $5c+22=0$. Differentiating the Zamolodchikov
one-parameter family gives a two-dimensional subspace of Hochschild
$2$-cocycles generated by the central-charge variation and the
$W$-normalisation variation. The assertion that these classes exhaust
the full chiral Hochschild cohomology is equivalent to the vanishing of
the DS-comparison obstruction
\[
 \mathfrak{o}_{\mathrm{DS\text{-}HH}}(W_3)
 \in
 H^{\ge 1}\!\left(
 \operatorname{Cone}\!\left[
 \CH^\bullet_{\mathrm{ch}}(W_3)
 \longrightarrow
 H^\bullet_{\mathrm{DS}}
 \CH^\bullet_{\mathrm{ch}}(V_k(\mathfrak{sl}_3))
 \right]\right).
\]
Only the displayed finite-pole cocycle subspace is used here.
\end{proposition}

This finite-pole subcomplex controls:
\begin{itemize}
...
Let\/ $\cA$ be a principal $W$-algebra whose generators have spins
$(s_1,\ldots,s_n)$.
...
\subsection{Finite Symbolic Engines}

\subsubsection{Symbolic evaluation of graph weights}

For $k\geq5$, the operation $m_k$ is a finite sum over connected
graphs with $k$ external legs and bounded loop number. Each summand is
defined by four algebraic operations:
\begin{enumerate}
\item \textbf{Graph enumeration:} connected graphs with fixed external
arity and the permitted loop number.
\item \textbf{Wick contraction:} pairings of fields with the HT
propagator.
\item \textbf{Configuration integral:} the corresponding
Fulton--MacPherson integral or Feynman-parameter integral.
\item \textbf{Laurent extraction:} the pole part in the spectral
parameters $\lambda_i$.
\end{enumerate}

\subsubsection{Obstruction Questions}

\begin{enumerate}
\item \textbf{Non-principal formality:} Principal $\mathcal{W}_N$
families are class~$\mathbf{M}$ on the verified finite-rank DS
survival loci. For non-principal $\mathcal{W}^k(\mathfrak{g}, f)$,
finite shadow depth is controlled by the corresponding DS survival
map and is not inferred from the principal case.

\item \textbf{Quantum group comparison:} The quantized $r$-matrix of
$W_3$ can be compared with $Y(\mathfrak{sl}_3)$ only after the DS
screening quotient and the evaluation-core comparison surface have
both been fixed.

\item \textbf{Higher $W_n$:} For fixed $n$, can the recursive
generating function be extended after supplying the finite DS envelope?
No all-rank uniform envelope is asserted.

\item \textbf{Non-perturbative sectors:} The local BV graph calculus
does not include instanton sectors; such corrections require a global
HT moduli problem with a compactness and orientation package.

\item \textbf{Boundary conditions:} A Neumann--Dirichlet comparison
changes the boundary factorization algebra and hence the transferred
$A_\infty$ operations; the comparison is a statement about the
corresponding boundary BRST complexes.
\end{enumerate}
...
\subsection{Finite-Pole Output}

The finite-pole Virasoro and $W_3$ calculations used in this section
consist of:
\begin{itemize}
\item Explicit propagators and $\lambda$-brackets $m_2$;
\item Displayed formulas and recursion schemes for the verified
low-arity operations;
\item Generating functions encoding finite-weight recursive structures;
\item Spectral $r$-matrices and their role in quantization;
\item The DS-compatible Hochschild cocycle subspace governing the
finite-pole deformation problem.
\end{itemize}

Full unprojected all-arity survival, logarithmic $\cW(p)$ tempering,
and all-rank $\cW_N$ uniformity require the separate obstruction
classes stated above and in the class~$\mathsf{M}$ tempering chapters.
...
Assume the KZ Virasoro realization satisfies the analytic SDR package
of Assumption~\textup{\ref{assum:kz-analytic-sdr-package}} and the
wheel-residue survival hypothesis of
...
$\{W_\lambda W\}$ serve as the initial data \(m_2\). The Stasheff
relation at degree~$k$ determines the boundary source for \(d m_k\);
the operation \(m_k\) itself is selected only after fixing the chain
contraction \(h\). On a minimal cohomology model the arity-four identity
contains no \(m_4\); the first minimal relation involving \(m_4\) is the
arity-five identity.
...
For $Q$-closed inputs the $m_1$-terms vanish on cohomology, so the
displayed sum runs over $p, q \ge 2$ only. On the chain model the same
sum is the source cancelled by the \(m_1m_k\) and \(m_km_1\) terms.
...
specific input sequence $(X_1, \ldots, X_k)$ determines the source for
\(d m_k(X_1,\ldots,X_k)\) in terms of compositions
$m_p(\ldots, m_q(\ldots), \ldots)$ with $p, q < k$.
...
\end{equation}
Then \(d m_k=\Assoc_k\). After a contraction \(h\) is fixed, the
chosen primitive is \(m_k=-h\,\Assoc_k\) up to the displayed sign
convention.
...
\begin{proposition}[All $m_3$ for $W_3$]
...
\subsubsection{Degree 4: The arity-four source for $m_4$}
...
= \mathrm{Obs}_4(X_1,X_2,X_3,X_4).
\end{split}
\end{equation}
(Signs: with $|X_i|' = -1$, we have $\varepsilon(i,q) = (q-1)(i-1)$;
for $q=2$: $(i-1)$; for $q=3$: $2(i-1)$. The terms with even sign
exponent appear with $+$, odd with $-$. The full chain relation is
\(d m_4+\mathrm{Obs}_4=0\); after passing to cohomology it says
\(\mathrm{Obs}_4=0\), not a value for \(m_4\).)

\begin{theorem}[Explicit $m_4$ for the $T$-sector of $W_3$]
...
More precisely, in the fixed contraction gauge used in this split,
$m_4(T,T,T,T)$ is the chosen primitive \(-h\,\Assoc_4(T,T,T,T)\) of
the degree-four source, which is a sum of five compositions involving
$m_2$ and $m_3$ as in~\eqref{eq:w3-stasheff-4}. Each source term is
polynomial in $(\ell,\mu,\nu)$ (no negative powers), as befits a
boundary contribution on $\FM_4(\C)$.
...
\begin{theorem}[Explicit $m_4(T,T,T,W)$]
...
\begin{proposition}[Structure of all $m_4$ types for $W_3$]
...
\begin{theorem}[The $T$-sector $m_5$]
...
\begin{theorem}[Generating Function System for $W_3$]
...
\begin{theorem}[Non-closure of the $W_3$ PDE system]
...
\begin{theorem}[DS-compatible first-order $\HH$ classes for $W_3$]
\label{thm:w3-bigraded-HH}
Work in the finite-pole principal DS model of $W_3$ at generic
$c$ with $5c+22\ne0$, and impose the primary normalisation
$\{T_\lambda W\}=\partial W+3\lambda W$. The derivatives of the
Zamolodchikov $\lambda$-brackets with respect to the central charge and
to the $W$-normalisation define two Hochschild $2$-cocycles
\[
 \Theta_c^T=\partial_c\{T_\lambda T\},\qquad
 \Theta_c^W=\partial_c\{W_\lambda W\}
 \quad\text{together with the induced normalisation variation of }W.
\]
They span the DS-compatible finite-pole deformation slice
\begin{equation}
\HH^2_{\mathrm{DS\text{-}fp}}(W_3)
\;=\;
\C\langle \Theta_c^T,\Theta_c^W\rangle .
\end{equation}
The full chiral Hochschild statement is obtained from this theorem
only under the DS-Hochschild comparison hypothesis
\[
 \mathfrak{o}_{\mathrm{DS\text{-}HH}}(W_3)=0
\]
from Proposition~\ref{prop:w3-hochschild}; without that hypothesis the
theorem identifies a verified finite-pole deformation slice, not the
entire Hochschild cohomology of the vertex algebra.
\end{theorem}

\begin{proof}
The Jacobi identity for the $W_3$ $\lambda$-brackets is a polynomial
identity in the parameters $c$ and the chosen normalisation of~$W$.
Differentiating that identity with respect to either parameter gives
the Hochschild cocycle equation
$d_{\mathrm{Hoch}}\Theta=0$. The central-charge derivative is not a
coboundary inside the finite-pole DS slice because $c$ is an external
parameter, not a field of the vertex algebra. The $W$-normalisation
derivative is likewise non-zero in the slice before quotienting by the
choice of primary generator; the Zamolodchikov relation
$32/(5c+22)$ in the $W$--$W$ bracket ties it to the same
one-parameter family.

This proves the finite-pole statement. To identify the slice with full
chiral Hochschild cohomology one must compare the DS BRST reduction of
the affine Hochschild complex with the intrinsic Hochschild complex of
$W_3$. The cone of that comparison is exactly
$\mathfrak{o}_{\mathrm{DS\text{-}HH}}(W_3)$, so no full-Hochschild
exhaustion follows unless that obstruction vanishes.
\end{proof}

\begin{remark}[Comparison with Virasoro Hochschild]
\label{rem:w3-vs-vir-HH}
For Virasoro the central-charge derivative gives the familiar
Gel'fand--Fuchs deformation class. For $W_3$ the finite-pole DS slice
has two visible first-order directions, central charge and
$W$-normalisation, but the Zamolodchikov Jacobi identity ties their
effective deformation to the one-parameter family. Full Hochschild
dimensions are not asserted without the DS-Hochschild comparison.
\end{remark}
...
not an independent parameter: it is determined by $c$ via the Jacobi
identity for $(W,W,W)$. On the finite-pole DS slice this gives the
relation forcing $\beta=\beta(\alpha)$ in
$\varphi=\alpha\,\Theta_c^T+\beta\,\Theta_c^W$. This is a statement
about the verified slice, not a computation of the full chiral
Hochschild group.
\end{remark}
...
\begin{proposition}[Weight-1 Closure]
...
computation from the lower-weight ones.
\end{proposition}
...
\section{K3 base-point $W$-algebra deformations}
\label{sec:wac-supplement}

\begin{remark}[KZ/DS $W$-algebra deformations at the K3 base point]
\label{rem:wac-1}
At the K3 base point the KZ/DS $W$-algebra deformations are the
$W$-algebra variants parameterised by auxiliary data (level, central
charge, twist). Conditional on the Vol.~III/source recognition of
$\mathbf H_{\Delta_5}$ as the $\Phi_2$-image of K3, and on a finite
lift of the $\widetilde M_{24}$ Schur cocycle to that source, they use
$\mathbf H_{\Delta_5}$ as anchor and fibre over the moduli of
class-$\mathcal S$ data on $\Sigma_{0,24}$ (puncture positions,
flavour holonomies). Under the same recognition the order-$6$ Schur
cocycle restricts the KZ/DS fibres to a finite cover of the
$\mathrm{Sp}_4(\mathbb Z)$-modular locus on $\bar{\mathcal A}_2$
\cite{ChengDuncanHarvey2014,Gaiotto2009}. Without this Vol.~III/source
recognition, the paragraph supplies only the finite-pole KZ/DS
deformation slice with $\mathbf H_{\Delta_5}$ as candidate scalar
target. Compare Vol.~III Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\begin{remark}[$W$-algebra closure at the Lusztig point]
\label{rem:wac-2}
The K3 $W$-algebra deformation closes at the Lusztig specialisation
$\hbar^2 = -1/8$, with $K^{\kappaChHodge} = 8$ root-of-unity
order. Beyond this point, the deformation family becomes non-associative
(pentagon coboundary $\phi^{(3)}$ non-trivial at $\hbar^3$), forcing the
KZ/DS fibre to stratify by $\mathbb Z/8$-cohomology class
\cite{Lusztig1990,EtingofKazhdan2007PartV}. Compare
Chapter~\ref{V3-ch:k3-chiral-bialgebra-platonic}.
\end{remark}

\section{K3-specific vs CY-2-general scope}
\label{sec:wac-supplement-ii}

\begin{remark}[The K3-specific scope of $\mathbf{H}_{\Delta_5}$]
\label{rem:wac-W15-1-scope}
When the Vol.~III/source recognition is invoked, the K3 chiral
bialgebra~$\mathbf{H}_{\Delta_5}$ is an output of the CY-to-chiral
functor~$\Phi$ evaluated specifically at the K3 surface. This chapter
does not construct that output. The KZ/DS $W$-algebra deformations above
parameterise
deformations of $W_3$ at central charge~$c$ and of the
$W_{1+\infty}$/Miki tower at levels $(q_1,q_2,q_3 = (q_1 q_2)^{-1})$
on the Frenkel--Kac vertex closure side. The K3-specific scope is
that the scalar $\mathbf{H}_{\Delta_5}$ target lane is selected by
\emph{three topological inputs},
\begin{enumerate}[label=\textup{(T\arabic*)},leftmargin=2em,
 topsep=0.3ex,itemsep=0.3ex]
\item $\chi_{\mathrm{top}}(X) = 24$ (matching the 24-node Ran-space
 base, the 24 $I_1$~Kodaira fibres of elliptic~K3, and the 24
 maximal-regular~$\mathfrak{su}(2)$ punctures on~$\Sigma_{0,24}$);
\item $c_1(X) = 0$ (Calabi--Yau condition; Serre functor~$S_X = [d_X]$
 without twist);
\item $h^{2,0}(X) = 1$ (shift~$[2]$ bar-cobar, selecting the candidate
 super-quasi-Hopf target lane for~$\mathbf{H}_{\Delta_5}$).
\end{enumerate}
Of the four CY-2 surfaces (Kodaira--Enriques--Kodaira classification:
K3, abelian $T^4$, Enriques, bielliptic), only K3 satisfies
(T1)--(T3). In particular:
\begin{itemize}
\item $T^4$: $\chi_{\mathrm{top}} = 0$, fails (T1). The~$\Phi$-image
 is a Heisenberg vertex algebra of signature~$(4,4)$; the automorphic
 data is the~$\mathrm{O}(4,4)$ Narain partition function; no~$\Delta_5$.
\item Enriques surface: $\chi_{\mathrm{top}} = 12$, $h^{2,0} = 0$,
 fails (T1) and (T3). The scalar analogue is the level-$2$ Enriques
 paramodular row. A genuine \(\mathbb Z/2\)-gauging of
 \(\mathbf{H}_{\Delta_5}\) is a source/orbifold-comparison object, not a
 consequence of the scalar elliptic-genus row alone; the Igusa-form
 replacement is a Siegel paramodular form at level $N = 2$
 on $K(2)\subset\mathrm{Sp}_4(\Q)$
 \cite{OguisoSakai2001,DolgachevKondo2002}.
\item Kummer surface $\mathrm{Kum}(T^4)$: topologically K3 with 16
 special $(-2)$-curves. The scalar Igusa datum restricts
 to~$\Delta_5|_{\mathcal{A}_2^{\mathrm{Kum}}}$. Identifying the
 $\Phi$-image with $\mathbf{H}_{\Delta_5}$ restricted to the Kummer
 sublattice~$\Lambda_{\mathrm{Kum}}\subset \mathrm{II}_{3,19}$
 requires the Vol.~III/source recognition of $\mathbf{H}_{\Delta_5}$
 and a chain-compatible Kummer sublattice comparison.
\item Bielliptic surface ($T^4/G$ with $G$ finite freely acting):
 $\chi_{\mathrm{top}} = 0$, fails (T1) and (T3) (since $h^{2,0} = 0$).
 The~$\Phi$-image is an orbifold of the~$T^4$ Heisenberg vertex
 algebra by~$G$; no~$\Delta_5$.
\end{itemize}
The KZ/DS $W$-algebras of this chapter are the deformation
catalogues \emph{at the generic stalk} of the family; the
K3-specific crown~$\mathbf{H}_{\Delta_5}$ is the candidate scalar fibre
at the K3~base point, and becomes a chiral-bialgebra fibre only under
the Vol.~III/source recognition. The sisters
(Enriques/Kummer/bielliptic) are different scalar lanes at different
base points, not specialisations of a universal family. Compare Vol.~III preface
Remark~\ref{rem:v3-preface-W15-3-specific-vs-general} and
Vol.~III introduction
Remark~\ref{rem:v3-intro-W15-3-scope}.
\end{remark}

\begin{remark}[Half-K3 $= \mathrm{dP}_9$ is not CY-2; 12 fibres, not 24]
\label{rem:wac-W15-2-halfK3}
The half-K3 (rational elliptic surface
$\mathrm{dP}_9 = \mathrm{Bl}_9\mathbb{P}^2$) is not CY-2
($c_1\ne 0$; fails~(T2)). Its elliptic fibration has 12~$I_1$~fibres,
exactly half the K3~count; F-theory compactification on
$\mathrm{dP}_9$ is an~8d heterotic theory whose~1-loop threshold is
1-loop-trivial at the level of a weight-$5$~Siegel analogue.
The only~KZ/DS $W$-algebra at half-K3 is the
unbroken~$E_8$ current algebra at level~1 from the 12-brane
coincidence point; there is no $\mathbf{H}_{\Delta_5}$-analogue.
This is consistent with the 24-point rigidity of the bi-based Ran
datum: 12 base points is not enough to host a $\Delta_5$-denominator
Borcherds product, because the singular theta lift
on~$\mathrm{II}_{1,9}$ does not produce a weight-$5$ Siegel~cusp form
\cite{MorrisonVafa1996II,Borcherds1998}. The ``half-K3
$W$-algebra'' is therefore 1-loop-trivial, not a Lusztig-closed
specialisation.
\end{remark}

\begin{remark}[Heterotic decoupling limit as a comparison object]
\label{rem:wac-W15-3-decoupling}
From the Vol II perspective, the decoupling limit is a target criterion:
after the Vol.~III/source recognition package has constructed
\(\mathbf{H}_{\Delta_5}\), it must admit a presentation as a chiral algebra
living on a curve~$C$ with topological~factorisation coproduct on
an~$\R$-direction, with no residual gravitational bulk coupling. On the
heterotic frame, the target degeneration is realised by taking
$g_s \to 0$ (decouples string-loop gravity),
$R_{T^2}\to R_{\mathrm{sd}} = \sqrt{\alpha'/2}$ (isolates the
self-dual-radius chiral~$T^2$~sector into the~$\mathrm{II}_{3,19}$
Narain lattice VOA), and $R_{T^4}\to\infty$ (decouples the
transverse~$T^4$ Kaluza--Klein tower)~\cite{HarveyMoore1996,
NahmWendland2001,TaorminaWendland2011}. Conditional on that recognition,
the decoupled limit is the stand-alone chiral bialgebra whose
$\mathsf{SC}^{\mathrm{ch,top}}$-pair is
$\bigl(\mathbf{H}_{\Delta_5},\,\mathbf{H}_{\Delta_5}^{R}\bigr)$
with $\mathbf{H}_{\Delta_5}^{R}$ the Miki-tower topological
closure. This is the condition that would make \(\mathbf H_{\Delta_5}\) a
fourth Drinfeld taxon (Etingof--Kazhdan super Part~V super-quasi-Hopf; see
Vol.~III preface Remark~\ref{rem:v3-preface-4-etingof-super-EK}) rather
than an anomaly contribution inside a 10d~heterotic or 11d~M-theory action.
The criterion is not a Vol II construction of \(\mathbf H_{\Delta_5}\); it
states the decoupling property that the recognised Vol.~III object must
satisfy. Compare
Vol.~III preface~Remark~\ref{rem:v3-preface-W15-1-decoupling}
and Vol.~III introduction~Remark~\ref{rem:v3-intro-W15-2-decoupling}.
\end{remark}

\section{the CY-2 Siegel-weight column}
\label{sec:wac-supplement-iii}

The K3 column separates five adjacent surfaces: Enriques, $T^4$,
Kummer, bielliptic, and the non-CY half-K3. For each surface the
failed topological input is visible in the resulting
Siegel-form weight or affine-Kac--Moody level of the scalar target
attached to the candidate $\Phi$-image, and verifies the CY-$2$
dichotomy across all five surfaces.

\begin{theorem}[Enriques Siegel weight $= 5/2$; conditional
$\mathbb{Z}/2$-orbifold recognition]
\label{thm:wac-W17-enriques-weight}
Let $\mathrm{En} = K3/\iota$ be an Enriques surface. The scalar
Enriques elliptic genus satisfies
\[
 \phi^{\mathrm{Enr}}_{0,1}(\tau, z)
 \;=\; (1/2)\phi_{0,1}^{K3}(\tau, z),
\]
and its Gritsenko--Nikulin additive/Borcherds lift on the Enriques
lattice $\mathrm{II}_{2,10}(2)$ is a Siegel paramodular form
$\Delta_5^{\mathrm{Enr}}$ of weight~$5/2$ on
$K(2)\subset\mathrm{Sp}_4(\mathbb{Q})$
(Gritsenko--Nikulin 1998 \textup{J.~reine angew.~Math.}~507,
Theorem~5.2; Hulek--Sankaran 2011
\textup{Algebra and Number Theory}~5, Theorem~3.4;
Borisov--Libgober 1998 \textup{Commun.~Math.~Phys.}~191,
Theorem~4.2). The scalar weight degradation $5\mapsto 5/2$
is dictated by
$\chi_{\mathrm{top}}(\mathrm{En})/\chi_{\mathrm{top}}(K3) = 1/2$.

Assume in addition actual source/orbifold recognition: the finite
Hall--Borcherds source for $\mathbf{H}_{\Delta_5}$ carries the
Enriques involution, $\Phi_2$ is equivariant for that involution, and
orbifold descent exists in the chiral and topological slots. Then the
recognised Enriques output may be denoted
\[
\mathbf{H}_{\Delta_5}^{\mathrm{Enr}}
= \mathbf{H}_{\Delta_5}\,/\!\!/\,\langle\iota\rangle,
\]
and its scalar genus-$2$ Siegel character is
\[
 \chi^{\mathrm{Enr}}_2(Z)
 \;=\;
 1/\Delta_5^{\mathrm{Enr}}(Z).
\]
Without this source/orbifold recognition, the scalar elliptic-genus
halving proves only the weight-$5/2$ automorphic input, not the
existence of a BKM or chiral-bialgebra orbifold.
\end{theorem}

\begin{proof}
The $\iota$-equivariant Borcherds lift on the integral lattice
$\mathrm{II}_{2,10}(2) = E_8(-2)\oplus\mathrm{II}_{2,2}(2)$
(Gritsenko--Nikulin 1998, \S 3) produces an automorphic product of
weight $w = (1/2)\cdot r = (1/2)\cdot 5 = 5/2$, with $r$ the
zero-coefficient of the vector-valued input form. The
half-integer weight is forced by the metaplectic $2$-cover of
$\mathrm{Sp}_4$ (Oguiso--Sakai 2001
\textup{Nagoya~Math.~J.}~163, Proposition~4.1): the $2$-scaling of
the lattice requires a metaplectic input. This proves the scalar
automorphic statement. The interpretation of
$\Phi_{10}^{\mathrm{Enr}} = (\Delta_5^{\mathrm{Enr}})^2$ as a BKM
denominator and as the B-side Kirillov--Ostrik orbifold gauging is
the additional source-recognition hypothesis
by the Vol.~III derived-categorical Enriques analogue
(Theorem~\ref{thm:dcy-W17-enriques-analogue}).
\end{proof}

\begin{remark}[Kummer Siegel weight $= 5$ with Kummer sublattice
restriction]
\label{rem:wac-W17-kummer-weight}
The Kummer K3 $\mathrm{Kum}(A)$ is topologically K3, so
$\chi_{\mathrm{top}} = 24$, $c_1 = 0$, $h^{2,0} = 1$: all three
K3 scalar-lane hypotheses hold. The Igusa automorphic datum restricts
to the Kummer locus, with scalar Siegel character
\[
 \chi^{\mathrm{Kum}}_2(Z)
 \;=\;
 1/\Delta_5(Z)\big|_{\mathcal{A}_2^{\mathrm{Kum}}},
\]
where the Kummer locus $\mathcal{A}_2^{\mathrm{Kum}}\subset\mathcal{A}_2$
is the codimension-$1$ sub-locus of principally-polarised abelian
surfaces admitting a Kummer presentation (Mukai 1984
\textup{Nagoya~Math.~J.}~81, Theorem~1). The weight of
$\Delta_5\big|_{\mathcal{A}_2^{\mathrm{Kum}}}$ is~$5$ (same as the
K3 crown: no weight degradation, only sublattice restriction).
The Gritsenko--Nikulin lift survives the restriction because the
input elliptic genus $\phi_{0,1}^{K3}$ is a form on the full
moduli of K3 and the restriction to Kummer is an embedding,
not a quotient. The stronger identification
\[
 \Phi_2(\mathrm{Kum}(A))
 \simeq
 \mathbf{H}_{\Delta_5}\big|_{\Lambda_{\mathrm{Kum}}}
\]
requires the Vol.~III/source recognition of $\mathbf{H}_{\Delta_5}$ and
a chain-compatible comparison between the Kummer sublattice and the
recognised Hall--Borcherds source. The scalar restriction of
$\Delta_5$ alone does not construct that chiral bialgebra.
\end{remark}

\begin{remark}[$T^4$ and bielliptic: Siegel weight $= 0$,
Narain partition function instead]
\label{rem:wac-W17-T4-weight}
For $T^4$ and bielliptic surfaces, $\chi_{\mathrm{top}} = 0$
forces the Borcherds multiplicative lift of the elliptic genus
$\phi_{0,1} = 0$ to be trivial: the automorphic product
$\mathrm{Borch}(0) = 1$ (constant function), of weight~$0$. The
replacement on the $\mathsf{SC}^{\mathrm{ch,top}}$ side is the
Narain partition function $Z_{\mathrm{II}_{4,4}}(\tau, \bar\tau)$
for~$T^4$, and its~$G$-orbifold $Z_{\mathrm{II}_{4,4}}^G$ for
bielliptic $T^4/G$. Neither is a Siegel cusp form; both are
non-holomorphic modular functions on
$\mathrm{O}(4,4;\mathbb{Z})\backslash\mathbb{H}$. The chiral
avatar is the vertex algebra
$V_{\mathrm{II}_{4,4}}$ (Nahm--Wendland 2001
\textup{Comm.~Math.~Phys.}~216, Theorem~4.1); in taxonomic terms
this is Drinfeld taxon~G (abelian Heisenberg), not taxon~$4$.
This is the $N = 1$ Conway-type BKM degeneration: the imaginary
root system of the fourth taxon collapses, leaving only the
real-root Narain Cartan.
\end{remark}

\begin{remark}[Half-K3 $=\mathrm{dP}_9$: affine level $k = 1$, weight
$= 0$]
\label{rem:wac-W17-halfK3-level}
For $\mathrm{dP}_9 = \mathrm{Bl}_9\mathbb{P}^2$, the singular
theta lift on $\mathrm{II}_{1,9}$ is a weight-$0$ automorphic
function on $\mathrm{O}(1,9;\mathbb{Z})\backslash\mathbb{H}^9$,
not a weight-$5$ Siegel cusp form (Borcherds 1998
\textup{Invent.~Math.}~132, Theorem~10.1: the weight of the
output equals half the minimum norm of negative-norm vectors in
the theta-lift input, which requires a hyperbolic plane summand
not present in $\mathrm{II}_{1,9}$). The $\Phi_2$-image is the
affine~$\widehat{E_8}$ Kac--Moody algebra at level~$k = 1$
(Seiberg 1988 \textup{Phys.~Lett.~B}~206, \S3;
Morrison--Vafa 1996 \textup{Nucl.~Phys.~B}~476, II.\S4). The
level is fixed by the $12$ $I_1$-fibre count via the
anomaly-cancellation condition
$k \cdot h^\vee(E_8) = 30 \cdot 1 = 30 \stackrel{?}{=}
c_{\mathrm{1-loop}}(\mathrm{dP}_9)$ with $h^\vee(E_8) = 30$;
after subtracting the gravitational contribution
$c_{\mathrm{grav}} = -2c_1(\mathrm{dP}_9)^2 = -2\cdot 9 = -18$,
the chiral-side level matches $k = 1$. The chiral algebra lives
on the unbroken~$E_8$ gauge locus of the F-theory moduli; in
taxonomic terms this is Drinfeld taxon~L (affine Kac--Moody at
specific level), \emph{not} the fourth taxon.
\end{remark}

\begin{remark}[CY-2 Siegel-weight column]
\label{rem:wac-W17-weight-column}
Reading the CY-$2$ dichotomy along the Siegel-weight axis gives
the following column (paralleling the four-taxon B-side table
of Vol.~III Remark~\ref{rem:dcy-W17-landscape-partition}):
\begin{center}
\begin{tabular}{lccl}
\toprule
Surface $X$ & $\chi_{\mathrm{top}}$ & Siegel weight & Replacement datum \\
\midrule
K3 & $24$ & $5$ & $\Delta_5$ on~$\mathrm{Sp}_4$ \\
Kummer K3 & $24$ & $5$ & $\Delta_5\big|_{\mathcal{A}_2^{\mathrm{Kum}}}$ \\
Enriques & $12$ & $5/2$ & $\Delta_5^{\mathrm{Enr}}$ on~$K(2)$, metaplectic \\
$T^4$ & $0$ & $0$ & $Z_{\mathrm{II}_{4,4}}$ (Narain) \\
bielliptic & $0$ & $0$ & $Z_{\mathrm{II}_{4,4}}^G$ \\
$\mathrm{dP}_9$ & $12$ & $0$ & $\widehat{E_8}_{\,k=1}$ (affine) \\
\bottomrule
\end{tabular}
\end{center}
The weight column is linear in~$\chi_{\mathrm{top}}$ within the
fourth-taxon stratum (K3, Kummer, Enriques):
$w_{\mathrm{Borch}} = 5\cdot(\chi_{\mathrm{top}}/24)$. The weight
collapses to~$0$ on the taxon-G stratum ($T^4$, bielliptic) and
the taxon-L stratum ($\mathrm{dP}_9$) for different
reasons: vanishing elliptic genus on taxon-G, lattice-signature
failure on taxon-L. This is the first-principles arithmetic
content of the assertion that three topological invariants
$(\chi, c_1, h^{2,0}) = (24, 0, 1)$ select the scalar K3 lane: the
Borcherds weight~$5$ arises exactly when
$(\chi/24, c_1, h^{2,0}) = (1, 0, 1)$, and any degradation of
one invariant collapses the scalar weight to a lower value
(half-integer) or to~$0$. Construction of the corresponding
chiral-bialgebra object is the separate Vol.~III/source recognition
problem. No KZ/DS $W$-algebra of this
chapter supports any of the non-trivial automorphic products
beyond the scalar K3 and Enriques target rows; the deformation
catalogues select the K3-column scalar lane at
$(\chi, c_1, h^{2,0}) = (24, 0, 1)$ and nothing else.
\end{remark}

\begin{remark}[Specific-vs-general discipline on the CY-2
landscape]
\label{rem:wac-W17-specific-vs-general}
The specific/general distinction holds columnwise:
the K3 row is the specific crown of the CY-$2$ column; the
Enriques row is the scalar $\mathbb{Z}/2$-gauged row at
weight~$5/2$; its chiral-bialgebra orbifold interpretation requires
the source recognition of Theorem~\ref{thm:wac-W17-enriques-weight};
the $T^4$/bielliptic rows are the general Narain
degeneration (no Siegel lift); the $\mathrm{dP}_9$ row is the
general affine limit (no Siegel lift, different taxon). The
partition is not a specialisation relation: no row is a limit
or deformation of another within the CY-$2$ landscape under
$\Phi_2$. The KZ/DS $W$-algebras of this chapter
parameterise deformations \emph{within} a fixed taxon (K3
row, via the $W_{1+\infty}$/Miki tower; or $T^4$ row, via the
$\mathrm{O}(4,4)$ Narain moduli); transitions across rows
correspond to changing CY-$2$ surface, not to changing
deformation parameter. This is the \emph{stratified} structure
of $\mathrm{ChirAlg}$ under~$\Phi_2$: strata are labelled by
$(\chi_{\mathrm{top}}, c_1, h^{2,0})$, and each stratum carries
its own taxon and its own deformation catalogue.
\end{remark}

\section{Conditional Enriques BKM and the four-row $\Psi$-landscape}
\label{sec:wac-supplement-iv}

The Enriques scalar Siegel weight is fixed at $5/2$ as the
$\mathbb{Z}/2$-gauged companion of K3's weight-$5$ crown
(Theorem~\ref{thm:wac-W17-enriques-weight}). The
$\mathsf{SC}^{\mathrm{ch,top}}$ consequences below are conditional on
the source/orbifold recognition stated there: after that recognition,
the Enriques BKM superalgebra
$\mathfrak{g}_{\Delta_5}^{\mathrm{Enr}}$ sits as a distinguished
deformation face of the fourth-taxon column, with explicit Cartan,
orbifold root-sector grading, and conjectural
$2.M_{12}$-moonshine module structure. The Vol.~III construction in
\S\ref{sec:bkm-enriques-W18} supplies the BKM side; the present section records the
$W$-algebra deformation-landscape projection.

\begin{theorem}[Enriques elliptic-genus halving and conditional BKM
multiplicities]
\label{thm:wac-W18-enriques-halving}
\index{Enriques!elliptic-genus halving}
\index{BKM!multiplicity halving}
Under the Borisov--Libgober 2000 \emph{Duke Math.~J.}~104, Thm~4.1
construction, the Enriques elliptic genus is the
$\iota$-symmetrisation
\[
 \phi^{\mathrm{En}}_{0,1}(\tau, z)
 \;=\;
 \tfrac{1}{2}\bigl[\phi^{K3}_{0,1}(\tau,z)
 + \phi^{K3}_{0,1}(\tau + 1/2, z)\bigr],
\]
a weight-$0$ index-$1$ weak Jacobi form on~$\Gamma_0(2)$ whose
Fourier coefficients $f^{\mathrm{En}}(N,\ell)$ are exactly one
half of the K3 Fourier coefficients $f^{K3}(N,\ell)$ on the
$\iota$-invariant scalar spectrum.

Assume in addition the source/orbifold recognition of
Theorem~\ref{thm:wac-W17-enriques-weight}: an
$\iota$-equivariant finite Hall--Borcherds source exists, its
Borcherds denominator descends through the Enriques orbifold, and the
root sectors of the orbifold BKM are identified with the corresponding
Fourier sectors. Then the positive-imaginary-root
supermultiplicities in the recognised Enriques BKM
$\mathfrak{g}_{\Delta_5}^{\mathrm{Enr}}$ are read from the orbifold
denominator coefficients
\[
 \mathrm{smult}(\alpha)
 \;=\;
 f^{\mathrm{En}}(N,\ell)
 \qquad\text{at } \alpha^2 = 2N - \ell^2 \leq 0.
\]
On the invariant untwisted sector this gives
$f^{\mathrm{En}}(N,\ell)=\tfrac12 f^{K3}(N,\ell)$. It does not assert
ordinary multiplicity halving for every root of a BKM object: twisted
sectors, anti-invariant sectors, and parity signs are part of the
orbifold denominator data. Scalar elliptic-genus halving alone proves
the weight-$5/2$ Borcherds input; it does not construct
...
\begin{remark}[First ten Fourier coefficients]
\label{rem:wac-W18-ten-fourier-coeffs}
\index{Enriques!Fourier coefficients}
Denote by $c^{\mathrm{En}}(N,\ell)$ the $(q^N y^\ell)$-coefficient
of $\phi^{\mathrm{En}}_{0,1}$. The first ten coefficients
(organised by $4N - \ell^2$ discriminant, which indexes the
imaginary-root norm under $\alpha^2 = \ell^2 - 4N$) are:
\begin{center}
\begin{tabular}{cccc}
\toprule
$(N,\ell)$ & $4N - \ell^2$ & $c^{K3}(N,\ell)$ &
 $c^{\mathrm{En}}(N,\ell)$ \\
\midrule
$(0,0)$ & $0$ & $20$ & $10$ \\
$(0,\pm 1)$ & $-1$ & $2$ & $1$ \\
...
$(1,\pm 1)$ & $3$ & $96$ & $48$ \\
$(1,\pm 2)$ & $0$ & $20$ & $10$ \\
$(2,0)$ & $8$ & $[K3\text{-Table 1}]$ &
 $(1/2)[K3\text{-Table 1}]$ \\
$(2,\pm 1)$ & $7$ & ditto & ditto \\
$(2,\pm 2)$ & $4$ & ditto & ditto \\
$(2,\pm 3)$ & $-1$ & ditto & ditto \\
$(3,0)$ & $12$ & ditto & ditto \\
\bottomrule
\end{tabular}
\end{center}
In a recognised Enriques BKM source, negative $c^{\mathrm{En}}$ at
$(1,0)$ contributes odd (fermionic) simple-root supermultiplicity.
As scalar elliptic-genus data alone it records a signed denominator
coefficient, not a construction of the BKM object. Sign-sum verification:
$\sum_{\ell\in\mathbb{Z}}c^{\mathrm{En}}(1,\ell) =
(10 + 48 - 116 + 48 + 10)/1 = 0$, matching
the index-$1$ Jacobi identity vanishing.
Primary: Borisov--Libgober 2000 Thm~4.1;
Eguchi--Ooguri--Tachikawa 2010 Table~1 (K3 side coefficients
$20, 2, -232, 96, 20$);
Klemm--Kreuzer--Scheidegger--Theisen 2005
\emph{Adv.~Theor.~Math.~Phys.}~9, Thm~3.2 (Enriques side
Fourier dictionary).
\end{remark}

\begin{theorem}[Conditional $\mathsf{SC}^{\mathrm{ch,top}}$ face of
$\mathbf{H}_{\Delta_5}^{\mathrm{Enr}}$]
\label{thm:wac-W18-enriques-sc-face}
\index{Enriques!$\mathsf{SC}^{\mathrm{ch,top}}$ face}
\index{$\mathbb{Z}/2$-gauging!$\mathsf{SC}^{\mathrm{ch,top}}$}
Assume the source/orbifold recognition of
Theorem~\ref{thm:wac-W17-enriques-weight} and assume that the
Enriques involution lifts as an automorphism of both colour slots of
the Kirillov--Ostrik $\mathsf{SC}^{\mathrm{ch,top}}$ datum. Then the
$\mathbb{Z}/2$-gauging
$\mathbf{H}_{\Delta_5}\to\mathbf{H}_{\Delta_5}^{\mathrm{Enr}}$
commutes with the Swiss-cheese $\mathsf{SC}^{\mathrm{ch,top}}$
bulk-boundary structure: if
$(A^{K3}_{\mathrm{chir}}, A^{K3}_{\mathrm{top}})$
is the Kirillov--Ostrik $\mathsf{SC}^{\mathrm{ch,top}}$ datum
of~$\mathbf{H}_{\Delta_5}$, then
\[
 \bigl(A^{\mathrm{En}}_{\mathrm{chir}}, A^{\mathrm{En}}_{\mathrm{top}}\bigr)
 \;=\;
 \bigl(A^{K3}_{\mathrm{chir}}/\!/\langle\iota\rangle,\,
 A^{K3}_{\mathrm{top}}/\!/\langle\iota\rangle\bigr),
\]
with the orbifold taken in the chiral and topological slots
separately and compatibly. The holomorphic bar differential
$d^{K3}_{\mathrm{hol}}$ restricts to the $\iota$-invariant
subspace; the topological coproduct
$\Delta^{K3}_{\mathrm{top}}$ likewise descends to
$\iota$-coinvariants. The resulting
$(A^{\mathrm{En}}_{\mathrm{chir}}, A^{\mathrm{En}}_{\mathrm{top}})$
is the $\mathsf{SC}^{\mathrm{ch,top}}$-face of
$\mathbf{H}_{\Delta_5}^{\mathrm{Enr}}$ realised on
$\mathrm{En}\times\mathbb{R}$ in place of
$\mathrm{K3}\times\mathbb{R}$. Primary:
Kirillov--Ostrik 2002 \emph{Adv.~Math.}~171, Thm~5.1 (orbifold
$\mathbb{Z}/2$-gauging of chiral-categorical data);
Gaiotto--Witten 2019 \emph{Adv.~Theor.~Math.~Phys.}~23,
Thm~5.4 ($\mathsf{SC}^{\mathrm{ch,top}}$ structure on
$A_\infty$-chiral algebras).
\end{theorem}

\begin{remark}[Topologisation on the Enriques row]
\label{rem:wac-W18-enriques-topologisation}
\index{Enriques!topologisation}
\index{$E_3^{\mathrm{top}}$!Enriques}
The K3 chiral bialgebra $\mathbf{H}_{\Delta_5}$ topologises to
$E_3^{\mathrm{top}}$ via the Mukai-Heisenberg conformal vector
(Vol.~III Theorem~\ref{V3-thm:k3-abelian-at-lie}). Conditional on the Enriques
source/orbifold recognition above, the $\mathbb{Z}/2$-gauged sibling
$\mathbf{H}_{\Delta_5}^{\mathrm{Enr}}$ would inherit an
$\iota$-invariant conformal vector
$T^{\mathrm{En}} = T^{K3}|_{\iota\text{-even}}$, which should
produce an $E_3^{\mathrm{top}}$-topologisation on
the $\iota$-invariant (non-critical) parameter stratum. The
conjecture is the $\mathbb{Z}/2$-gauged version of the topologisation
theorem. Explicit: the Sugawara ladder
$T^{\mathrm{En},(n)} = [Q^{\mathrm{En}}_{\mathrm{tot}}, G^{\mathrm{En},(n)}]$
for $n = 1$ produces one inner stress tensor, giving
$k = 1$ hence $E_{k+2}^{\mathrm{top}} = E_3^{\mathrm{top}}$,
matching the K3 endpoint. Full proof requires checking the
orbifold chain-homotopy descent on the
Costello--Gwilliam $\mathsf{SC}^{\mathrm{ch,top}}$-cobar model
(Costello--Gwilliam 2017 \emph{Factorization Algebras}, Ch.~10);
this is open for the Enriques row.
\end{remark}

\begin{remark}[Four-row $\Psi$-landscape, Vol.~II reading]
\label{rem:wac-W18-four-row-landscape}
\index{$\Psi$-landscape!Vol~II reading}
\index{four-row landscape}
Reading the $\Psi$-landscape of Vol.~III
along the Vol.~II Siegel-weight and $\mathsf{SC}^{\mathrm{ch,top}}$-structure axes
gives:
\begin{center}
\begin{tabular}{lccl}
\toprule
Input $X$ & Siegel weight & $\mathsf{SC}^{\mathrm{ch,top}}$-structure &
 Topologisation endpoint \\
\midrule
K3 & $5$ & $(A^{K3}_{\mathrm{chir}}, A^{K3}_{\mathrm{top}})$ on
 $\mathrm{K3}\times\mathbb{R}$ & $E_3^{\mathrm{top}}$
 (proved) \\
Enriques & $5/2$ &
 source/orbifold target
 $(A^{K3}_{\mathrm{chir}}, A^{K3}_{\mathrm{top}})/\!/\langle\iota\rangle$
 on $\mathrm{En}\times\mathbb{R}$ & $E_3^{\mathrm{top}}$
 (under Enriques descent conjecture) \\
Monster & $0$ & central $V^{\natural}$ + chiral Griess
 on $\mathbb{R}^{1,1}\times\mathbb{R}$ & $E_\infty^{\mathrm{top}}$
 (critical/chiral) \\
Fake-Monster & $12$ & Leech-lattice VOA + bosonic string
 $V^{\mathrm{II}_{25,1}}$ & $E_3^{\mathrm{top}}$ (Borcherds 1990) \\
\bottomrule
\end{tabular}
\end{center}
Three of the four rows topologise to $E_3^{\mathrm{top}}$
(K3, Enriques, Fake-Monster); the Monster row degenerates to
$E_\infty^{\mathrm{top}}$ at the critical central charge
$c = 24$. The two CY-$2$-input rows (K3, Enriques) are related by a
$\mathbb{Z}/2$-gauging only after the source/orbifold-comparison object
is supplied; the scalar Enriques elliptic-genus row alone supplies the
weight-$5/2$ automorphic input. The two pure-automorphic rows (Monster,
Fake-Monster) are the FLM and Borcherds endpoints of the four-taxon
census. The universal three-faces identity
$\hbar^2\cdot K^{\kappaChHodge} = -1$ gives
$(\hbar^2, K^{\kappaChHodge})
\in \{(-1/2, 2),\, (-1/4, 4),\, (-1/8, 8),\, (-1/50, 50)\}$
along the four rows.
\end{remark}

\begin{remark}[Enriques-specific vs CY-$3$ Enriques conjecture]
\label{rem:wac-W18-enriques-cy2-vs-cy3}
\index{Enriques!CY-$2$ vs CY-$3$}
\index{dimension awareness!$\Psi_2$ vs $\Psi_3$}
The Enriques row of this section refers to the CY-$2$ datum
$\mathrm{En}$ under~$\Psi_2$, producing weight-$5/2$
$\Delta_5^{\mathrm{Enr}}$. The distinct weight-$4$ form
$\Delta_3(\mathrm{Enr}\times E)$ of
Vol.~III \texttt{k3\_chiral\_algebra.tex} arises from the CY-$3$
orbifold $(K3\times E)/\mathbb{Z}/2$ under~$\Psi_3$; the two
outputs are functorially distinct and live on different
moduli spaces. No KZ/DS $W$-algebra of this chapter
supports either; the deformation tower has its domain of
validity inside the K3 row, and the Enriques/CY-$3$
orbifold extensions are separate
$W_{1+\infty}$-Miki-tower lifts. Compare
Vol.~III \texttt{derived\_categories\_cy.tex},
Remark~\ref{rem:dcy-W18-enriques-cy2-vs-cy3}.
\end{remark}
