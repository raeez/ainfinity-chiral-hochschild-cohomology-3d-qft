\section{Complete Computations: Nonabelian Affine Extension}
\label{sec:examples_complete_conditional}

The material below extends the complete computations of
Section~\ref{sec:examples_complete} to the nonabelian setting.
The physics-bridge verification
(Proposition~\ref{prop:su2-hypotheses}) records the precise hypotheses under
which $SU(2)$ Chern--Simons lies in the scope of
Theorem~\ref{thm:physics-bridge}. The explicit
$\Ainf$ computation through degree~$4$ proceeds at the
chain level before the comparison model enters; cohomological tower
termination uses the one-loop Koszul affine comparison model with
compatible nullhomotopies. The Jacobi mechanism for $m_4^H = 0$ and the
cohomological tower bound $r_{\max} = 3$ appear in
...
Fix 3d $SU(2)$ Chern--Simons theory on $\R_t \times \C_z$
...
$[h,f] = -2f$). Compared with the abelian case,
$Q$ carries a \emph{quadratic} term from the gauge
self-interaction.
...
\begin{proposition}[Nonabelian $m_3$: explicit formula]
\label{prop:su2-m3}
...
On the chosen one-loop Koszul affine comparison model, the supplied
transfer homotopy makes the nonzero triples $Q$-boundaries. This is a
model-specific witness, not a consequence of ordinary
...
sesquilinearity, and the vacuum rule that
$m_2(\mathbf{1}, a)=a$ has no singular part.
...
corresponds to the left-endpoint gauge. Its boundary decomposes into
two codimension-$1$ faces ($z_1 \to z_2$ and $z_2 \to z_3$), whose
contributions are the compositions $m_2 \circ m_2$. The Lie conformal
Jacobi identity identifies this boundary with the associator defect.
Passing from this boundary identity to $[m_3]=0$ requires the
compatible transfer homotopy specified by the affine comparison model.
\end{proof}

\subsubsection{Higher operations $m_{k \geq 4}$}

\begin{proposition}[Termination of the $\Ainf$ tower at degree~$3$]
...
\item $m_4 \neq 0$ at chain level: the square Feynman diagram
 ...
 $\FM_4(\C)$. Its transferred cohomology class is annihilated by the
 compatible affine nullhomotopy.
...
\item On the chosen one-loop Koszul affine comparison model, the
 Virasoro wheel is absent while the Lie--Jacobi transfer survives.
 The PVA on $H^\bullet$ is $\widehat{\mathfrak{sl}}_2$ at level~$k$
 with the standard $\lambda$-bracket. Shadow depth $r_{\max} = 3$
 (class~$\mathbf{L}$).
\end{enumerate}
\end{proposition}
...
Part~(iv): by induction on degree on the specified comparison model.
Base case: $m_4^H = 0$ from
part~(i). Inductive step: for $k \geq 5$, the Stasheff relation
involves only $m_2$ and $m_3$ after the higher transferred operations
are annihilated by the nullhomotopy hypothesis. The compositions
$m_3 \circ m_3$ at degree $5$
...
\begin{theorem}[PVA for nonabelian CS]
\label{thm:su2-pva}
...
the open-colour Koszul dual $\cA^!_{\mathrm{line}}$ controls line
operators.
...
\item spectral $R$-matrix from the Laplace transform
 ...
 \end{equation}
 where $\Omega$ is the Casimir element and $k$ is the affine level; at $k = 0$ the $r$-matrix vanishes identically;
...
\]
The field-dependent term $f^{ab}{}_c\, J^c/z$ is operator-valued and
does not contribute to the classical $r$-matrix tensor; the central
term $k\,\delta^{ab}/z^2$, after $d\log$ absorption (reducing the
pole order by one), yields the collision $r$-matrix
$r(z) = k\,\Omega_{\mathrm{tr}}/z$, the standard rational
...
\kappaChHodge(\widehat{\mathfrak{sl}}_2, k) = \frac{3(k + 2)}{4},
\end{equation}
where $3 = \dim(\mathfrak{sl}_2)$ and $h^\vee = 2$ is the
dual Coxeter number. By complementarity (Volume~I, Theorem~C):
\begin{equation}
\kappaChHodge(\widehat{\mathfrak{sl}}_{2,k})
+ \kappaChHodge(\widehat{\mathfrak{sl}}_{2,-k-4}) = 0.
\end{equation}
...
and $h^\vee = 2$). The curvature $\kappaChHodge(V_k(\mathfrak{sl}_2)) = \dim\mathfrak{g}\cdot(k+h^\vee)/(2h^\vee) = 3(k+2)/4$, matching Volume~I's computation.
Under $k \mapsto k' = -k - 2h^\vee = -k - 4$:
$\kappaChHodge(V_{k'}(\mathfrak{sl}_2)) = 3(k'+2)/4 = 3(-k-4+2)/4 = 3(-k-2)/4 = -3(k+2)/4 = -\kappaChHodge(V_k(\mathfrak{sl}_2))$,
confirming complementarity $\kappaChHodge(V_k(\mathfrak{sl}_2)) + \kappaChHodge(V_{k'}(\mathfrak{sl}_2)) = 0$.
\end{proof}
...
scope of Theorem~\ref{thm:physics-bridge}.
\begin{enumerate}[label=\textup{(\roman*)}]
\item BV data and one-loop finiteness: the action
 \eqref{eq:su2-cs-action} is a classical BV theory with well-defined
 HT twist. Gauge fixing yields a propagator with standard properties.
 One-loop finiteness holds because 3d CS is topological: the only
 UV divergence is the one-loop shift $k \to k + h^\vee$
...
\item The interaction vertex $f^a{}_{bc}$
 is polynomial in the fields; Feynman integrands are
 logarithmic forms on $\FM_k(\C)$. AOS relations hold by
 the theory of configuration space integrals for
 polynomial interactions. Stokes exactness holds on
 $\FM_k(\C)$ by the compactification's smooth-with-corners
 structure.
\end{enumerate}
Theorem~\ref{thm:physics-bridge} applies; the resulting
observables form a logarithmic $\SCchtop$-algebra, and the induced
factorization compatibility is part of the resulting
$C_\ast(W(\SCchtop))$-algebra structure.
\end{proposition}

\subsection{Summary Table for the Nonabelian Affine Extension}
...
\hline
$SU(2)$ CS & $\frac{f^{ab}{}_c J^c}{z_{12}} + \frac{k\delta^{ab}}{z_{12}^2}$ & $\neq 0$ (explicit) & $m_4^H = 0$ on the specified comparison model; $m_{\geq 5} = 0$ by degree counting & $\widehat{\mathfrak{sl}}_2$ at level $k$ \\
\hline
\end{tabular}
\end{center}
