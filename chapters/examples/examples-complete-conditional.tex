...
All results are conditional on the standing hypotheses \textrm{(H1)--(H4)}.
...
\]
At the classical level (setting $J^c = 0$ in the center), the
$r$-matrix reduces to $r(z) = \Omega / z$, the standard rational
...
\kappa(\widehat{\mathfrak{sl}}_2, k) = \frac{3k}{k + 2},
\end{equation}
...
and $h^\vee = 2$).  The curvature $\kappa = c/24 \cdot [\text{normalization}]$, matching Volume~I's computation.
Under $k \mapsto k' = -k - 2h^\vee = -k - 4$:
$c(k') = 3(-k-4)/(-k-4+2) = 3(-k-4)/(-k-2) = 3(k+4)/(k+2)$,
so $c(k) + c(k') = 3k/(k+2) + 3(k+4)/(k+2) = 3(2k+4)/(k+2) = 6$,
confirming complementarity $\kappa + \kappa' = 6/24 = 1/4$
