...
\label{rem:ghost-identity-alphaN-active}%
\index{ghost central charge!WN@$\mathcal{W}_N$ identity}%
The paired central charge
$c_{\mathrm{pair}}^{(N)} = \alpha_N$ has a direct
...
\label{thm:self-dual-halving}%
\index{self-dual point!genus tower halving}%
At the Koszul self-dual point $c^* = \alpha_N/2$,
the effective curvature $\kappa_{\mathrm{eff}}$ depends
linearly on~$c$, so the ratio
$\kappa_{\mathrm{eff}}(c^*)/\kappa_{\mathrm{eff}}(0)
= (c^* - \alpha_N)/(0 - \alpha_N)
= 1/2$.
Therefore $F_g(c^*) = \tfrac{1}{2}\,F_g(0)$
for all~$g \ge 1$ and all~$N \ge 2$: Koszul
self-duality universally halves the genus tower.
\end{remark}
