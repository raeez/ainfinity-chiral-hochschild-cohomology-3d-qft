...
= \Bigl(\sum_{i=1}^n s_i\Bigr) - (n+1),
\end{equation}
provided $s_i \ge 3$ for at least $2$ of the inputs
\textup{(}so the binding collision channel has a
non-primary OPE\textup{)}.  For sectors with $\le 1$
higher-spin input, the gap position is governed by the
Virasoro sub-sector.
