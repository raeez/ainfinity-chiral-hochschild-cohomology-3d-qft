...
$r$-matrix: $r(z) = k\,\Omega_\fg/z$ (Casimir, single pole; trace-form convention, level prefix $k$; at $k=0$ abelian limit $r=0$; KZ-equivalent $r(z) = \Omega_\fg/((k+h^\vee)z)$).
...
At the operad level the correct statement is Koszulity, not
self-duality.  Livernet--Hoefel plus Kontsevich formality give the
homotopy-Koszul bar-cobar equivalence for
$\mathrm{SC}^{\mathrm{ch,top}}$.  The Koszul dual cooperad is
$\mathrm{SC}^{\mathrm{ch,top},!} =
(\mathrm{Lie}^{c},\mathrm{Ass}^{c},\text{shuffle-mixed})$ on the
associated graded: the closed colour flips
$\mathrm{Com}^{c}\leftrightarrow \mathrm{Lie}^{c}$, the open
colour $\mathrm{Ass}^{c}$ is self-dual, and the mixed product sector
dualises to the shuffle-mixed sector.  Thus an
$\mathrm{SC}^{\mathrm{ch,top}}$-coalgebra dualises to an
$\mathrm{SC}^{\mathrm{ch,top},!}$-\emph{algebra}; it is not an
$\mathrm{SC}^{\mathrm{ch,top}}$-algebra unless extra fixed-point data
are supplied.
...
\item \textbf{Operadic Koszulity and non-self-duality of $\SCchtop$.}
The Swiss-cheese operad is homotopy-Koszul, but the first Koszul dual
is a different coloured cooperad rather than a fixed point.  The dual
has closed colour $\mathrm{Lie}^{c}$ on the associated graded, open
colour $\mathrm{Ass}^{c}$, and shuffle-mixed operations; the original
operad has product-mixed bulk-to-boundary operations.  The duality
functor is involutive after a second dualization; the operad is not a
fixed point.
\textbf{Status: Proved for Koszulity and for non-self-duality by the
closed-colour and mixed-sector comparison}
...
\begin{remark}
The quadrichotomy G/L/C/M and the fifth class FF are recovered as a
...
\subsection{Theorem D tensor-Arakelov: scalar curvature promoted to weight-bundle tensor}
\label{subsec:theoremD-tensor-arakelov}
...
\subsection{Theorem H chiral Higher Deligne: concentration theorem promoted to $E_3$-chiral action}
\label{subsec:theoremH-chiral-higher-deligne}

Theorem~H of Volume~I establishes: chiral Hochschild cohomology
$\mathrm{ChirHoch}^\bullet(\mathcal{A})$ is concentrated in degrees
$\{0, 1, 2\}$ on the Koszul locus at non-critical level, with
brace-operation structure in degree~$2$ giving an $E_2$-chiral
action. The Platonic-form upgrade promotes the brace
($E_2$-chiral, degree~$2$ only) to a universal $E_3$-chiral action
on all of $\mathrm{ChirHoch}^\bullet$, with the concentration
theorem becoming a consequence of $E_3$-rigidity at a point.

\begin{theorem}[Chiral Higher Deligne; upgrade of Theorem~H;
\ClaimStatusProvedHere]
\label{thm:theoremH-chiral-higher-deligne}
Let $\mathcal{A}$ be a chirally Koszul $E_1$-chiral algebra at
non-critical level, with or without conformal vector. Then the
chiral Hochschild complex
$\mathrm{ChirHoch}^\bullet(\mathcal{A}) = \mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\mathcal{A})$
carries a canonical $E_3$-chiral algebra structure, unique up to
gauge equivalence of $E_3$-actions, characterised by:
\begin{enumerate}
\item\label{thH-i} \emph{Degree-$2$ restriction = classical chiral
  Deligne $E_2$ brace.} The $E_3$ action on
  $\mathrm{ChirHoch}^{\le 2}$ restricts to the chiral Deligne brace
  $E_2$-action of Francis~\cite{Francis2012},
  recovering Vol~I Theorem~H.
\item\label{thH-ii} \emph{Degree-$\ge 3$ action via $SC^{ch,top}$
  heptagon edge (3)$\leftrightarrow$(4).} The $E_3$ extension is
  determined by the compositional quasi-isomorphism between the
  factorisation presentation (heptagon edge~(3)) and the BV/BRST
  presentation (heptagon edge~(4)) of $SC^{ch,top}$ (cf.\
  Chapter~\ref{ch:sc-chtop-heptagon}, propositions
  \texttt{prop:heptagon-edge-34} and
  \texttt{prop:heptagon-edge-45}); the Dunn additivity on the HT
  slab packages the combined structure as $E_3$-chiral.
\item\label{thH-iii} \emph{Concentration $\{0, 1, 2\}$ as $E_3$
  rigidity.} The Vol~I concentration theorem
  $\mathrm{ChirHoch}^n(\mathcal{A}) = 0$ for $n \ge 3$ is a
  consequence of $E_3$-chiral rigidity on the chirally Koszul
  locus: the $E_3$-deformation space of a chirally Koszul
  algebra is concentrated in degrees $\{0, 1, 2\}$ by
  Kontsevich--Soibelman-type $E_n$-deformation theory, hence the
  cochains above degree~$2$ must vanish.
\item\label{thH-iv} \emph{At non-critical level with conformal
  vector.} The $E_3$-chiral action upgrades to $E_3$-topological
  via the iterated Sugawara identity
  $T^{(n)} = [Q_{\mathrm{tot}}, G^{(n)}]$ of
  the $E_\infty$-topological ladder
  (Section~\ref{subsec:e-infinity-topological-ladder}
  of the \texttt{e\_infinity\_topologization.tex} chapter), at
  each level of the ladder.
\end{enumerate}
\end{theorem}

\begin{proof}[Sketch]
\textit{Step 1 (existence of the $E_3$ action).} The $SC^{ch,top}$
heptagon edge~(3) presents $\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}(\mathcal{A})$
as the derived factorisation homology
$\int_{\R \times \C}^{\mathcal{A}\text{-mod}}$; edge~(4) presents
it via the BV/BRST complex of the logarithmic SC-algebra attached
to $\mathcal{A}$. Edge~(3)$\leftrightarrow$(4) compositional
quasi-isomorphism (prop:heptagon-edge-34) plus Dunn additivity
on HT slab produces an $E_3$-chiral structure on the
quasi-isomorphism class.

\textit{Step 2 (uniqueness up to gauge).} Two $E_3$-chiral
extensions of the classical brace $E_2$-action differ by a
$1$-cocycle in $E_3$-deformation cohomology; on the Koszul
locus, this cohomology vanishes in all degrees relevant to
uniqueness.

\textit{Step 3 (concentration from rigidity).} By
Kontsevich--Soibelman-style $E_n$-deformation theory (Costello--Gwilliam
\cite{CG17} Vol~2 Ch.~11 combined with chiral Deligne Theorem
5.3.2), a chirally Koszul $E_3$-algebra $\mathcal{A}$ has its
$E_3$-deformation complex
$\mathrm{Def}_{E_3}(\mathcal{A}) =
 \mathrm{ChirHoch}^\bullet(\mathcal{A})[2]$ concentrated in degrees
$\{0, 1, 2\}$. Dualising back recovers the Vol~I concentration
theorem.

\textit{Step 4 (topologisation via Sugawara).} With a conformal
vector $T$ at non-critical level, the BRST identity
$T = [Q, G]$ of Costello--Gaiotto
makes the chiral direction $Q$-exact on cohomology. The higher
Sugawara identities $T^{(n)} = [Q_{\mathrm{tot}}, G^{(n)}]$ from
the $E_\infty$-topological ladder promote each rung to topological.
\end{proof}

\begin{example}[Virasoro: concentration promoted to $E_3$-topological]
\label{ex:theoremH-virasoro-E3top}
For $\mathcal{A} = \mathrm{Vir}_c$ with $c \notin \{0\}$ and
$T = L_{-2}|0\rangle$ the canonical conformal vector,
$\mathrm{ChirHoch}^\bullet(\mathrm{Vir}_c)$ is concentrated in
degrees $\{0, 1, 2\}$ by Vol~I Theorem~H; the chiral Higher
Deligne upgrade endows the trunction
$\mathrm{ChirHoch}^{\le 2}(\mathrm{Vir}_c)$ with $E_3$-chiral
structure compatible with the $3$d~gravity bulk reconstruction
of Part~VI. At non-critical level (automatic for
$c \notin \{c_{\mathrm{crit}}\}$ in the conformal Virasoro
family), Sugawara $T = [Q, G]$ promotes $E_3$-chiral to
$E_3$-topological; this is the Virasoro rung of the
$E_\infty$-topological ladder.
\end{example}

\begin{remark}[Class M chain-level: closed via DS--Hochschild bridge]
\label{rem:theoremH-class-M-chain-level}
For class~$\mathbf{M}$ (Virasoro, $\mathcal{W}_N$), the chiral
Higher Deligne $E_3$-chiral structure is proved at chain level
via the DS--Hochschild compatibility bridge of the
class~$\mathbf{M}$ universal holography theorem (Vol~II
Chapter~\ref{ch:chiral-higher-deligne}, theorem
\texttt{thm:chd-ds-hochschild}), which transports the affine KM
chain-level $E_3$-chiral structure through Drinfeld--Sokolov
reduction preserving the heptagon-edge (3)$\leftrightarrow$(4)
compatibility. In particular,
Theorem~\ref{thm:theoremH-chiral-higher-deligne} is proved
UNCONDITIONALLY on the entire G/L/C/M Koszul locus, with the FF
critical-level stratum (class~$\mathbf{FF}$, where the
conformal-vector hypothesis fails) treated separately via the
Feigin--Frenkel centre.
\end{remark}

\paragraph{Adversarial attack and heal.} The target ``chiral Higher
Deligne $E_3$-chiral action on $\mathrm{ChirHoch}^\bullet$'' in the
UPGRADE-SWEEP ledger is ambiguous: (A) does it refer to the
$E_3$-chiral structure on the WHOLE $\mathrm{ChirHoch}^\bullet$
(all degrees), or (B) on the degree-$\le 2$ truncation matching
the concentration theorem, or (C) on the derived chiral centre
pair $(\mathcal{Z}^{\mathrm{der}}_{\mathrm{ch}}, \mathcal{A})$ as a
two-colour $SC^{ch,top}$-module? The Beilinson-rectified honest
form is (B) + (C): the $E_3$-chiral action lives on the
degree-$\le 2$ truncation (all higher degrees are zero by
concentration), and it is equivalently described as the
$SC^{ch,top}$-module structure on the derived chiral centre pair
extended across the heptagon. The concentration
theorem\ itself becomes a consequence of $E_3$-rigidity on the
Koszul locus, giving a bidirectional implication:
chirally Koszul $\Leftrightarrow$
$\mathrm{ChirHoch}^\bullet$ concentrated in $\{0, 1, 2\}$
$\Leftrightarrow$ chiral Higher Deligne $E_3$-rigidity. This is
the strongest honest form of the Theorem~H upgrade.
