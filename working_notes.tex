...
\providecommand{\SCmod}{\mathsf{SC}^{\mathrm{mod}}}
\providecommand{\cE}{\mathcal{E}}
\providecommand{\FT}{\mathsf{FT}}
\providecommand{\Ob}{\operatorname{Ob}}
\providecommand{\Xloc}{\mathcal{X}_{\mathrm{loc}}}
\providecommand{\Bmod}{B^{\mathrm{mod}}}
\providecommand{\barBch}{\barB^{\mathrm{ch}}}
...
\providecommand{\ClaimStatusProvedHere}{\textup{proved here}}
\providecommand{\ClaimStatusProvedElsewhere}{\textup{proved elsewhere}}
\providecommand{\ClaimStatusComputed}{\textup{computed}}
\providecommand{\ClaimStatusConditional}{\textup{conditional}}
\providecommand{\ClaimStatusConjectured}{\textup{conjectural}}
...
they present the two visible faces of the bulk--boundary datum
controlled by the Swiss-cheese operad: the bar complex is the
Koszul-resolution surface, while the full two-colour algebra is
the pair $(Z^{\mathrm{der}}_{\mathrm{ch}}(A),A)$, or equivalently
$(C^\bullet_{\mathrm{ch}}(A,A),A)$ in the Hochschild model.  This is
the operadic fingerprint of a $3$d holomorphic-topological field theory
...
splittings).  The bar differential records the holomorphic
collision input, and the bar coproduct records the ordered
topological input.  The full $\SCchtop$-datum is not the bar
complex alone but the two-colour bulk--boundary pair to which the
bar construction is a Koszul resolution.
...
% =================================================================
\section{The local recognition theorem}
\label{sec:recognition}
% =================================================================

A factorisation algebra on a product rectangle in $\C \times \R$,
holomorphic along the curve direction and locally constant along
the line direction, is presented by an algebra over a cofibrant
coloured model $W(\SCchtop)$.  The statement below is local and
product-Weiss: it is not a global classification of arbitrary
factorisation algebras on all $3$-manifolds.
...
\begin{theorem}[Local recognition; product-Weiss form]
\label{thm:recognition-wn}
There is an equivalence of $\infty$-categories between:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
\item algebras over a fixed cofibrant logarithmic model
  $W(\SCchtop)$, and
\item factorisation algebras on product rectangles in
  $\C \times \R$ that are holomorphic in $\C$, locally constant
  in~$\R$, and satisfy product-Weiss descent.
\end{enumerate}
\end{theorem}
...
The recognition theorem converts product-local geometric data
(a factorisation algebra on product rectangles in $\C\times\R$)
into algebraic data (an operad algebra).  Without the
product-Weiss hypotheses, the Swiss-cheese structure is only a
presentation; with them, the two-coloured operad is the local
geometry.
...
on $\Eone(m)$).  Its output colours are only closed and open:
...
colour are compatible boundary faces of the same two-colour
coherence, not literally the same identity.

The Swiss-cheese operad is Koszul in the fixed coloured model
used here.

\begin{theorem}[Homotopy-Koszulity; model-dependent form]
\label{thm:homotopy-koszul-wn}
After choosing a cofibrant coloured model and an associator
identifying the holomorphic configuration-space model with the
topological Swiss-cheese model, $\SCchtop$ is homotopy-Koszul.
\end{theorem}
...
(2)~formality, with the chosen associator, gives a quasi-isomorphism
$\Phi\colon \SCchtop \to \mathrm{SC}$;
(3)~bar-cobar preserves quasi-isomorphisms; two-out-of-three.

Consequences are therefore inherited in this model.  Bar-cobar on
cofibrant/fibrant $\SCchtop$-algebras gives the expected derived
adjunction, and a Quillen equivalence only under the standard
admissibility and conilpotence hypotheses.  Lines are
$\cA^!$-modules when the open-colour Koszul dual is constructed;
the dg-shifted Yangian requires that ordered-bar construction, not
merely the slogan ``Swiss-cheese Koszulity''.
...
graviton.  Under the DS-transfer ghost-number tree condition of
Theorem~\ref{thm:gravitational-primitivity-wn}, the transferred
coproduct is primitive:
...
The mechanism is a ghost-number obstruction in the
Drinfeld--Sokolov transfer; the precise criterion is given in
...
The algebraic content is the absence of a coproduct cross-term in
the transferred chiral bialgebra.  A physical no-production
statement requires Lorentzian reconstruction and unitarity in
addition to this algebra.
...
Coproduct $\Delta_z$ & nontrivial (Casimir cross-term)
  & primitive under DS tree condition \\[3pt]
...
no cross-term once the DS tree condition is imposed.  This is the
central algebraic fact available in the present complex; its
physical interpretation is separate.
...
$r$-matrix: $r_{\mathrm{OPE}}(z) = k\,\Omega_\fg/z$
(Casimir, single pole; trace-form convention, level prefix $k$).
The KZ kernel is $\Omega_\fg/((k+h^\vee)z)$, while the
line-side Yangian normalisation is $r_{\mathrm{Y}}(z)=\hbar\Omega_\fg/z$
with $\hbar=(k+h^\vee)^{-1}$.
...
on cohomology.  The bar complex is quadratic.  The chiral
Verdier/Koszul dual is $\widehat\fg_{-k-2h^\vee}$, another affine
algebra at the reflected level; the open line-side dual is the
Yangian $Y_\hbar(\fg)$.  Lines are modelled by the open dual, and
highest-weight modules for the reflected affine algebra enter on
the closed/chiral dual lane.
...
\noindent
\textbf{$r$-matrix.}
$r(z) = k\,\Omega_{\fsl_2}/z = k\,(J^a \otimes J_a)/z$, a single-pole
solution of the classical Yang--Baxter equation (trace-form convention, level prefix $k$; at $k=0$ the abelian limit $r = 0$; KZ-equivalent $r(z) = \Omega_{\fsl_2}/((k+2)z)$, where $h^\vee(\fsl_2)=2$).
...
in the flat relative-Feynman-transform bicomplex.  This is a
local algebraic model; it is not the global curved factorisation
object.

At genus $g \geq 1$, the curvature
$\dfib^{\,2} = \kappa \cdot \omega_g$ belongs to the global
Ran/D-module or coderived model.  It forces a passage from the
flat local shadow to curved global factorisation data.  The
modular MC element
...
The annulus trace (the $S^1$ factorisation homology of a strongly
admissible $E_1$-chiral open sector $\cC_{\mathrm{op}}$) produces
the genus-$1$ modular datum.  When $\kappa\ne0$ this is a curved
circle-bar object and carries modulus dependence.
For $g\ge2$, the same trace construction supplies the expected
sewing inputs only after a modular open/closed cooperad on
$\cC_{\mathrm{op}}$ has been constructed.  Thus the genus-$g$
datum is conditional on CT-$2$: bordered FM clutching,
codimension-$2$ coassociativity, the $(\infty,2)$-categorical
lift, and coderived compatibility.  The proved data are the
annulus trace and the all-genera scalar curvature
$\dfib^{\,2}=\kappa\cdot\omega_g$; the full composition law of
traces remains the modular-cooperad frontier.
...
\hline
Annulus trace $\simeq \HH_*(\cC_{\mathrm{op}})$ under strong
  admissibility & proved \\[3pt]
Full modular cooperad & equivalent to the clutching theorem below \\[3pt]
Flat relative FT bicomplex $D^2 = 0$ & proved \\[3pt]
Curved global model $d^2 = \kappa \cdot \omega_g$ & proved in the
  global factorisation setting
\end{tabular}
\end{center}

\begin{theorem}[Global Ran model structures and descent criterion]
\label{thm:global-ran-model-descent-criterion}
Let $C$ be a smooth curve and let
$\mathsf{FactCoAlg}^{\mathrm{cpt}}_{\mathrm{curv}}(\Ran(C))$ denote
curved conilpotent factorisation coalgebras with compact
support along diagonals.  The chiral Booth--Lazarev adjunction
globalises over $\Ran(C)$ if and only if the following three
conditions hold:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
\item cofibrations are generated by compact diagonal cells and
Smith recognition applies on every finite Ran stratum;
\item weak equivalences are local for the Ran descent topology
and are detected by the chiral Maurer--Cartan functor;
\item the curvature class
$m_0^{(g,n)}=\kappa_{\mathrm{ch}}\lambda_g^1$ is represented in
a CDG coefficient object whose pullback to every separating and
non-separating boundary stratum is Mumford's clutching pullback.
\end{enumerate}
Under these hypotheses, the bar--cobar adjunction is a Quillen
equivalence after descent from finite Ran strata.  If any one of
the three conditions fails, the global statement is not a
theorem: the obstruction lies respectively in model existence,
descent, or curvature compatibility.
\end{theorem}

\begin{proof}
On a finite Ran stratum the local chiral coalgebra category is a
curved version of the Booth--Lazarev MC model category once the
compact diagonal cells generate cofibrations and weak
equivalences are MC-detected.  Condition~(ii) is exactly the
sheaf condition needed to glue these local Quillen equivalences
over the Ran topology.  Condition~(iii) says that the square of
the curved differential is the same tautological class after
clutching as before clutching; Mumford's formula for
$\lambda_g^1$ supplies the compatibility.  The three conditions
therefore give descent of the local Quillen equivalence.  The
converse is formal: a descended Quillen equivalence supplies a
cofibrantly generated model structure, local weak equivalences,
and a globally well-defined curved differential, hence the three
listed properties.
\end{proof}

\begin{theorem}[Dense IndHilb sewing comparison]
\label{thm:dense-indhilb-sewing-comparison}
Let $\cA$ be a positive-energy chiral algebra with finite
conformal-weight spaces and let $\widehat{\cA}_{\mathrm{Hilb}}$
be a nuclear Hilbert completion for which
$\sum_n \dim_\C \cA[n]|q|^n<\infty$ for every $|q|<1$.  The
algebraic coderived sewing complex embeds as a dense
$\mathbb Q$-structure in the IndHilb sewing completion if and
only if:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
\item the algebraic weight filtration is separated and exhaustive
after Hilbert completion;
\item the sewing operators $q^{L_0}$ are trace-class and preserve
the algebraic lattice of matrix coefficients;
\item the coderived null system maps to the Hilbert closure of
zero.
\end{enumerate}
The trace-class estimate proves convergence of the analytic
sewing maps; density is exactly the additional separated-lattice
and null-system comparison.
\end{theorem}

\begin{proof}
Trace-classness follows from the displayed summability condition.
If (i)--(iii) hold, the algebraic finite-weight subspace is dense
in the Hilbert completion, sewing preserves its matrix-coefficient
lattice, and quotienting by coderived nulls agrees with Hilbert
closure.  Hence the coderived sewing object is a dense
$\mathbb Q$-form of the IndHilb object.  Conversely, any dense
IndHilb comparison restricts to a separated exhaustive
finite-weight filtration, the analytic sewing maps must be
trace-class on the completion, and the kernel of the comparison
is precisely the algebraic null system whose image has Hilbert
closure zero.
\end{proof}

\begin{theorem}[All-genus trace law from modular clutching]
\label{thm:all-genus-trace-law-modular-clutching}
For a strongly admissible open sector $\cC_{\mathrm{op}}$, the
annulus trace extends to a full all-genus trace law precisely
when $\cC_{\mathrm{op}}$ carries the modular open/closed
cooperad structure whose edge cocompositions are the bordered FM
sewing functors.  In that case the genus-$g$ trace is the sum
over stable graphs
\[
  \Tr_g(\cC_{\mathrm{op}})
  =
  ...
  \operatorname{Tr}\!\left(
    \bigotimes_{e\in E(\Gamma)} S_e
    \circ
    \bigotimes_{v\in V(\Gamma)} \Theta_v
  \right),
\]
and the clutching identities are exactly the coassociativity
relations of the modular cooperad.  Thus there is no separate
all-genus trace frontier after the modular clutching theorem:
the two statements are equivalent.
\end{theorem}

\begin{proof}
A modular cooperad supplies a sewing operator $S_e$ for every
edge and a vertex operation $\Theta_v$ for every component of a
stable graph.  Taking the categorical trace over internal edge
states gives the displayed graph sum.  Coassociativity of the
cooperad identifies the two possible orders of contracting any
pair of edges, which is exactly the all-genus trace recursion.
Conversely, a trace law natural for every stable graph determines
the edge cocompositions by cutting one edge at a time; equality
of traces under iterated cuts is precisely modular
coassociativity.
\end{proof}
...
\begin{theorem}[Conditional DS-transfer primitivity]
\label{thm:gravitational-primitivity-wn}
For every principal Drinfeld--Sokolov reduction of every simple
Lie algebra, assume the DS transfer data satisfy the
ghost-number tree condition: every nontrivial HPL tree
contributing to the transferred coproduct has nonzero total ghost
number after projection to BRST cohomology.  Then the transferred
dg-shifted Yangian coproduct on the $\cW$-algebra is strictly
primitive:
\[
\Delta_z^W(x)
\;=\;
...
homotopy $h$, and outputs a tensor product.  By the stated
ghost-number tree condition, every tree with at least one
internal vertex lands outside total ghost number $0$ after
projection to BRST cohomology.  The projection $p\otimes p$
therefore annihilates every nontrivial tree.  The only surviving
contribution is the trivial tree, which gives the primitive term
$\tau_z(x) \otimes 1 + 1 \otimes x$.
\end{proof}

\sep

This is an algebraic no-cross-term statement.  Interpreting it as
absence of black-hole or particle production requires an
additional Lorentzian holographic reconstruction theorem.  The
ghost field $c \in \fn_+^*$ has ghost number $+1$; the antighost
$b \in \fn_+$ has ghost number $-1$; and the projection $p$
annihilates everything outside ghost number $0$.
...
\begin{theorem}[Lorentzian reconstruction criterion for no-production]
\label{thm:lorentzian-no-production-criterion}
The algebraic identity
\[
\Delta_z^W(x)=\tau_z(x)\otimes 1+1\otimes x
\]
implies a physical no-particle-production statement exactly after
Lorentzian reconstruction supplies:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
\item a positive-energy Hilbert space whose Euclidean chiral
correlators are boundary values of Wightman distributions;
\item unitarity and spectral positivity for the reconstructed
time-translation generator;
\item LSZ asymptotic fields whose coproduct is the analytic
continuation of the chiral coproduct.
\end{enumerate}
Under these hypotheses the primitive coproduct forbids a
one-particle graviton state from having a two-particle production
component.  Without them, the theorem remains an algebraic
no-cross-term statement and has no independent Lorentzian
scattering content.
\end{theorem}

\begin{proof}
The primitive coproduct is an equality in the Euclidean chiral
bialgebra.  Lorentzian reconstruction converts the same
correlators into Wightman distributions on a positive-energy
Hilbert space, and LSZ identifies production amplitudes with the
two-particle component of the analytically continued coproduct.
If the coproduct has no cross-term, that component is zero.  The
three listed hypotheses are also necessary for the phrase
``particle production'' to have its scattering-theoretic meaning:
without a Lorentzian Hilbert space, positivity, and asymptotic
fields, there is no LSZ amplitude to identify.
\end{proof}

\begin{theorem}[Admissible global analytic model]
\label{thm:admissible-global-analytic-model}
Let $\mathsf{Adm}^{\mathrm{oc}}(C)$ be the full subcategory of
open/closed chiral sectors on a smooth curve $C$ whose objects
$\cC_{\mathrm{op}}$ carry the following four structures:
\begin{enumerate}[label=\textup{(\roman*)},nosep]
\item compact diagonal-cell model structures on every finite Ran
stratum, with MC-detected weak equivalences and Ran descent;
\item a modular open/closed cooperad over bordered FM clutching,
with curvature represented by the CDG object carrying
$\kappa_{\mathrm{ch}}\lambda_g^1$;
\item a positive-energy nuclear Hilbert completion with trace-class
sewing operators $q^{L_0}$ preserving the algebraic matrix-coefficient
lattice;
\item Osterwalder--Schrader reflection positivity and an LSZ
asymptotic-field functor for the Lorentzian continuation.
\end{enumerate}
On $\mathsf{Adm}^{\mathrm{oc}}(C)$ the four remaining Vol~II
frontiers are theorems:
\[
\begin{gathered}
\Omega^{\mathrm{ch}}\dashv B^{\mathrm{ch}}
\quad\text{is a descended Ran Quillen equivalence},\\
\ChirHoch_{\mathrm{alg}}(\cC_{\mathrm{op}})
\subset
\ChirHoch_{\mathrm{IndHilb}}(\widehat{\cC}_{\mathrm{op}})
\quad\text{is a dense }\mathbb Q\text{-form},\\
\Tr_g(\cC_{\mathrm{op}})
=
\sum_{\Gamma\in G_g}
|\operatorname{Aut}\Gamma|^{-1}
\Tr\!\left(
  \bigotimes_{e}S_e\circ\bigotimes_v\Theta_v
\right),\\
\Delta_z^W(x)=\tau_z(x)\otimes1+1\otimes x
\quad\Longrightarrow\quad
\text{zero LSZ production amplitude.}
\end{gathered}
\]
No larger theorem is available from chiral algebra alone: omitting
\textup{(i)}, \textup{(ii)}, \textup{(iii)}, or \textup{(iv)}
removes respectively the global Ran model, the all-genus trace
law, the analytic sewing comparison, or the physical Lorentzian
interpretation.
\end{theorem}

\begin{proof}
Condition~(i) is exactly
Theorem~\ref{thm:global-ran-model-descent-criterion}; it gives the
descended Quillen equivalence.  Conditions~(ii) and the modular
cooperad part of the same datum are exactly
Theorem~\ref{thm:all-genus-trace-law-modular-clutching}; they give
the stable-graph trace formula and its coassociativity.  Condition~(iii)
is exactly Theorem~\ref{thm:dense-indhilb-sewing-comparison}; it
identifies the algebraic coderived sewing complex as the dense
rational form inside the analytic Hilbert completion.  Condition~(iv),
together with Theorem~\ref{thm:lorentzian-no-production-criterion},
turns the primitive coproduct into a statement about LSZ amplitudes.
The necessity of the four clauses follows by reading the four
theorems backwards.
\end{proof}
...
On the chirally Koszul locus, under the compact-generation and
derived-centre hypotheses in the boundary-linear exact sector, the
corrected bulk-boundary-line triangle is:
...
The bar complex is the ordered/chiral coalgebraic resolution whose
operations compute the derived-centre Swiss-cheese pair, as the
Steinberg variety presents a convolution algebra through a
self-intersection.

Both are algebro-geometric presentations of algebraic structures
through configuration-space or intersection resolutions.  The
...
collision residues and coproduct from deconcatenation is not by
itself the Swiss-cheese algebra; it is the resolution feeding the
bulk--boundary pair.  A factorisation-homological comparison with
Steinberg convolution remains a structural programme.
...
\hline
$\SCchtop$ homotopy-Koszulity & fixed coloured model + associator &
  \textsc{conditional} \\[3pt]
Recognition theorem & product-Weiss essential image &
  \textsc{conditional} \\[3pt]
...
The parallel announced in \S\ref{sec:koszul-triangle} is a
structural analogy with theorem-level pieces on each side; it is
not yet an identification theorem between the two geometries.
...
The analogy would become an identification only after constructing
a factorisation-homological comparison functor from the chiral
configuration-space category to the Steinberg convolution category.
In the present notes this functor is not constructed.  The safe
claim is the parallel of roles: Fulton--MacPherson compactification
plays for chiral collision geometry a role analogous to the
Springer resolution in Hecke theory, and the derived centre
plays the role of a convolution endomorphism algebra.
...
\cC_{\mathrm{op}}(I_2).
\]
For a strongly admissible $E_1$-chiral open sector, the two-arc
cover produces the two-sided bar complex, hence the cyclic bar
complex.  Under this admissibility hypothesis Hochschild homology
$\HH_*(\cC_{\mathrm{op}})$ is the annular trace object.  When
$\kappa\ne0$ this trace is a curved annular object, not an ordinary
flat invariant.
...
Clutching: for $g\ge2$, the closed genus-$g$ surface $\Sigma_g$
has a pants decomposition with $2g-2$ pairs of pants and
$3g-3$ sewing circles.  The categorical-trace construction often
uses a different handle presentation: $g$ self-clutchings from
$2g$ punctures.  These are distinct combinatorial models and
should not be identified.  In the pants model each sewing circle
contributes an $\HH_*$ factor, so the modular partition function is
...
$\cA = V_k(\fg)$, and the chiral Verdier dual is
$\cA^!_{\mathrm{ch}} = V_{-k-2h^\vee}(\fg)$.  The open line-side
dual is $\cA^!_{\mathrm{line}}=Y_\hbar(\fg)$.  The
...
universal coefficient $\lambda_g^{\mathrm{FP}}$ is positive, while
$F_g$ has the uniform sign of $\kappa_{\mathrm{eff}}$.  Thus
subcritical $c<26$ gives negative genus coefficients in this
normalization.

\sep

The collision residue
$r^{\mathrm{Vir}}(z) = (c/2)/z^3 + 2T/z$ supplies the Virasoro data
that enter the Cardy/BTZ dictionary under the Brown--Henneaux
hypotheses.  The leading coefficient $c/2 = \kappa$ is the curvature
scalar that enters the Cardy exponent; it is not itself the
Bekenstein--Hawking entropy.  The subleading $2T/z$ is
state-dependent braiding: it distinguishes states within the same
Virasoro theory.

The primitive coproduct is the algebraic analogue of a no-splitting
statement.  It does not by itself prove a statement about black-hole
dynamics; it says that, in this chiral algebra, the only local channel
is braiding.  The production vertex $\Omega(w{+}z,w)$ of gauge theory
(the Casimir cross-term) is absent.
...
$\cA^!_{\mathrm{ch}}$ & $V_{-k-2h^\vee}(\fg)$ & $\mathrm{Vir}_{26-c}$
...
The contact square-class
\[
\Delta_{\mathrm{ct}} := 8\kappa S_4 = \frac{40}{5c+22}
\]
controls the convergence radius of the shadow obstruction tower.
It is not the polynomial discriminant of $Q_{\mathrm{Vir}}$.
When $\Delta_{\mathrm{ct}} \neq 0$ (all $c \neq 0$): the tower is
infinite, class~M.  The bar complex
...
The actual discriminant and the contact square-class control three
quantities at once:
\begin{enumerate}[label=(\roman*),nosep]
\item The depth of the gravitational perturbation series
  (infinite for all $c \neq 0$, because
  $\Delta_{\mathrm{ct}} = 40/(5c{+}22) \neq 0$).
...
\item The complementarity with the dual theory at $c' = 26 - c$:
  $\Delta_{\mathrm{ct}}(c) + \Delta_{\mathrm{ct}}(26{-}c)
  = 6960/[(5c{+}22)(152{-}5c)]$,
...
The chiral Cardy contribution
$2\pi\sqrt{c\Delta/6}$, with $\Delta=L_0-c/24$,
...
is an analytic property of the shadow series at central
charge~$c$.  It should not be identified with splitting of a
prime in the associated quadratic field without an additional
...
The critical cubic is the analytic boundary of the gravitational
shadow curve: it separates the convergent regime from the
divergent regime.  Any relation with Minkowski bounds in a number
field is conjectural and is not used here.
...
ramifies.  The scalar effective genus lane becomes topological.
The Clifford algebra degenerates from a matrix algebra
($2^g$-dimensional) to an exterior algebra
($2^{2g}$-dimensional).  This does not by itself imply that the
full BTZ partition function collapses; the non-scalar contact and
shadow data must be treated separately.
...
\noindent\textbf{The analytic radius parameter.}\enspace
The growth rate
$\rho = \sqrt{36 + 80/(5c{+}22)}\,/\,c$ is the analytic radius
parameter for the shadow obstruction tower.  It should not be
identified with a fundamental unit without a separate
number-theoretic theorem.  It controls convergence of the shadow
series:
\begin{itemize}[nosep]
...
instead of $\Q$.  The Cardy/Brown--Henneaux entropy reads the
leading asymptotic coefficient after the physical dictionary is
imposed.  The convergence of the perturbation series is the
analytic boundary of this quadratic extension; arithmetic
splitting requires an additional construction.  The Koszul sign is the Galois involution.
The critical cubic is the analytic boundary of the shadow series.
The two ramified parameters are the Kac determinant zeros.  The
complementarity identity is a duality constraint on discriminants.
Arithmetic interpretations beyond this analytic statement are
conjectural.
...
the contact square-class
$\Delta_{\mathrm{ct}}(c_{p,q}) = 40/(5c_{p,q} + 22)$ is a
specific rational number, and
$K_{\mathrm{ct}}(c_{p,q}) := \Q(\sqrt{\Delta_{\mathrm{ct}}(c_{p,q})})$
is a specific real quadratic contact field.  The actual branch
field of $\sqrt{Q_{\mathrm{Vir}}}$ has square-class
$-2\Delta_{\mathrm{ct}}$.

\sep

\noindent\textbf{The physical picture.}\enspace
The curvature scalar $\kappa = c/2$ at a rational
central charge $c_{p,q}$ is a rational number; the
Bekenstein--Hawking entropy is obtained only after the
Brown--Henneaux/Cardy asymptotic step.  The
higher-genus corrections $F_g = \kappa \cdot \lambda_g^{\mathrm{FP}}$
are rational multiples of~$\kappa$, hence rational.  The
perturbative gravitational partition function
...
is algebraic and genuinely quadratic away from the rational-square
locus for $\Delta_{\mathrm{ct}}(c)$.  For the listed minimal-model examples below
the squarefree part is nontrivial.  The arithmetic content of the
shadow obstruction tower is visible in this quadratic branch
structure, not in any finite truncation of the perturbative
expansion.
...
$\Delta_{\mathrm{ct}}(c_0) = 40b/a$, and the specialised contact
field is $K_{\mathrm{ct}}(c_0) =
\Q(\sqrt{\mathrm{sqfree}(40ab)})$.  The branch field is
$K_{\mathrm{br}}(c_0)=\Q(\sqrt{-2\,\Delta_{\mathrm{ct}}(c_0)})$.

\medskip

\noindent\textit{$c = 1/2$ (Ising model).}\enspace
$5c + 22 = 49/2$.  $\Delta_{\mathrm{ct}} = 80/49$.
$\sqrt{80/49} = 4\sqrt{5}/7$.
The Ising contact field is $\Q(\sqrt{5})$; the branch field is
$\Q(\sqrt{-10})$.
...
$5c + 22 = 51/2$.  $\Delta_{\mathrm{ct}} = 80/51$.
$K_{\mathrm{ct}}(7/10) = \Q(\sqrt{255})$ where $255 = 3 \cdot 5 \cdot 17$.
The branch field is $\Q(\sqrt{-510})$.
...
$K_{\mathrm{ct}}(4/5) = \Q(\sqrt{65})$.  Discriminant~$65$.  $h = 2$.
The branch field is $\Q(\sqrt{-130})$.
...
$K_{\mathrm{ct}}(13) = \Q(\sqrt{870})$ where $870 = 2 \cdot 3 \cdot 5 \cdot 29$.
The branch field is $\Q(\sqrt{-435})$.
...
$K_{\mathrm{ct}}(26) = \Q(\sqrt{95})$.  Discriminant $380$.  $h = 2$.
The branch field is $\Q(\sqrt{-190})$.
...
At $c = 1$ and $c = 25$: the ratios are reciprocal squares;
for $c = 25$ one has $147/27 = (7/3)^2$.
Both give $\Q(\sqrt{30})$.
...
The three-tier picture maps precisely:
\begin{itemize}[nosep]
...
of algebras over $\mathsf{SC}^{\mathrm{ch,top,an}}_q$ for every
  $q \in (0,1)$ in the checked finite-type cases; outside that
  range this is a conditional analytic-completion statement.
...
semisimple-purity locus (classes~G, L, C), the heart is expected
to carry an IC pairing~$Q_{\mathrm{IC}}$ on the bar harmonic
space, induced by the Hodge--Riemann bilinear relations on IC
complexes of the FM compactification (de~Cataldo--Migliorini).
Definiteness depends on the real form, polarisation, and grading
conventions and is not asserted here as an unconditional theorem.

This upgrades IC purity from a \emph{vanishing} statement
(weight spectral sequence degenerates $\Rightarrow$
Koszulness) to a \emph{polarised-pairing} statement.  A genuine
positivity theorem would require the missing real/polarised
structure; the analogy with Elias--Williamson positivity is
therefore conjectural at this stage.

For class~M (Virasoro), $Q_{\mathrm{IC}}$ is expected to be
\emph{non-definite}---the non-split extensions in
...
The bar complex of a chiral algebra isolates algebraic data that
ordinary perturbative expansions do not package canonically.

It supplies the scalar genus-$g$ free energy in
closed form:
$F_g = (c/2) \cdot \lambda_g^{\mathrm{FP}}$, the
$\hat{A}$-genus coefficients times the modular characteristic.

It supplies the shadow tower of higher-arity corrections,
...
It gives the DS-transfer no-cross-term criterion
(\S\ref{sec:primitive-coproduct}): under the ghost-number tree
condition, the algebraic shadow has no splitting coproduct.

It records a precise arithmetic shadow.  At rational central charge $c_0$,
the shadow metric $Q_{\mathrm{Vir}}$ specialises to a specific
quadratic polynomial over~$\Q$, and
$K_{\mathrm{ct}}(c_0) = \Q(\sqrt{d(c_0)})$ is a specific real
quadratic contact field (the contact square-class
$\Delta_{\mathrm{ct}} = 40/(5c{+}22) > 0$ for $c > 0$).  The
chiral Cardy contribution $2\pi\sqrt{c\Delta/6}$
...
contact square-class $\Delta_{\mathrm{ct}}>0$ produces a real
quadratic contact extension.  The class number of $K_{\mathrm{ct}}(c_0)$ measures the arithmetic complexity
...
\cdot 29$ appears in the numerator of
$\Delta_{\mathrm{ct}}(c) + \Delta_{\mathrm{ct}}(26{-}c)$.
...
\item The \emph{virtual bar $K$-class}
  ...
  along the universal curve.  This is well-defined in the
  perfect finite-type standard-landscape cases checked here: the
  perfectness criterion (Koszul acyclicity + finite-dimensional
  fiber cohomology) is assumed outside that range.  Critical-level
  and non-perfect fibres require separate coderived treatment.

\item The \emph{center sheaf}
  $\cZ_{\mathrm{Vir}_c} := R^*\pi_{g*}(\barB^{(g)},\, d_{\mathrm{fib}})$,
  a constructible complex on $\overline{\cM}_g$ whose restriction to
  each smooth finite-type stratum is a local system with fiber
...
different conditions at different central charges.
...
the $T$-line.  The coefficient of $x_W^4$ carries the nonlinear
$W_3$ composite
$\Lambda = \normord{TT} - \frac{3}{10}\partial^2 T$ through the
$\{W_\lambda W\}$ bracket, but its numerical value must be read
only after fixing the shadow-normalisation convention.
...
\begin{proposition}[Conditional $W$-essential depth of $\cW_3$]
\label{prop:w3-essential-depth}
Assume the stated $W_3$ OPE normalization and the shadow
convention fixed above.  The spin-depth is $3$, and the last
new arity is $4$.  Specifically:
\begin{enumerate}[label=\textup{(\roman*)}]
...
while the spin-depth is $d_W(\cW_3)=3=s_{\max}$.  These are two
different indices: the last-new arity is not the same number as
the highest spin.
\end{proposition}
...
The OPE provides the independent initial datum
$10240/[c(5c{+}22)^3]$, whose normalized value has a third power
of $(5c{+}22)$ in the denominator---impossible from the recursion
applied to $\mathrm{Sh}_2$ and $\mathrm{Sh}_3$ alone (which
produce denominators with at most $(5c{+}22)^1$).
...
The proposition confirms the scoped picture: the spin-depth is
$d_W(\cW_3) = 3 = N$ for $N = 3$, while the three independent
new-data levels occur at arities $2$, $3$, and $4$:
\begin{center}
\small
...
\item The highest such arity is $r = N$, arising from the
  self-bracket $\{W^{(N)}_\lambda W^{(N)}\}$, whose leading
  $\lambda$-degree is $2N-1$, equivalently whose OPE leading pole
  has order $2N$, and which introduces the weight-$(2N-2)$
...
\item \emph{Koszul dual}: $\cH_k^! = \mathrm{Sym}^{\mathrm{ch}}(V^*)$.
  ...
  $\cH_{-k}$ as a chiral algebra.  The dual has curvature
  $\kappa(\cH_k^!) = -k$.
...
\end{array}
\]
The $v_5 = 1$ column for $r \ge 4$ is a finite verified pattern
(checked through $r = 25$ at seven central charges; see
Proposition~\ref{thm:universal-5-unit} below).
...
\begin{proposition}[Finite denominator-driven locality verification]
\label{prop:D-prime-locality}
Let $D(c) := 5c + 22$ and let $p$ be a prime with
$v_p(D) > 0$ and $v_p(c) = 0$.  In the verified ranges below,
\[
v_p(r\,\mathrm{Sh}_r)
...
$\{5c + 22 = 0\}$ only up to the first numerator-prime
exception.
...
divisor $\{5c + 22 = 0\}$ in the $c$-moduli in the verified
range.  Beyond that range numerator primes can and do interfere.

\subsection*{The seventh result: universal $5$-adic unit
in the numerator}

\begin{proposition}[Finite $5$-adic unit verification]
\label{thm:universal-5-unit}
For the central charges and arities listed below,
\[
v_5\bigl(N_r(c)\bigr) = 1.
\]
Thus the numerator polynomial $N_r(c)$ carries exactly one
factor of~$5$ in the tested region.  Verified at
$c = 1/2, -3/5, 1/3, 7, -1, 25/3, 13$ through $r = 15$,
and at $c = 7$ through $r = 25$.  The universal statement is a
conjectural pattern until a proof controls cancellations in the
full numerator polynomial.  The primitive numerator has baseline
$5$-content one in these computations, but special rational
specialisations may acquire extra factors; for example
$N_6(c)=160(45c+193)$ gains additional $5$-adic valuation at
$c=-188/45$.
\end{proposition}
...
\smallskip
In the tested range no cancellation removes this first-order
factor.  A full theorem would require a no-cancellation argument
for all higher terms and all rational central charges.
\end{proof}
...
$80 = 8 \cdot 10$.  In the tested primitive numerators, every
shadow coefficient at $r \ge 4$ inherits one baseline factor
of~$5$ from this quartic contact term.  Extra factors can appear
at special rational central charges, so the all-$c$ statement
remains conjectural.
\end{remark}
...
\item \emph{Primitive $5$-tier} ($p = 5$): the computations show
  $v_5(N_r) = 1$ for the listed central charges and arities,
  originating from the quartic contact coefficient
  $80/(5c{+}22)$.  The unrestricted all-$c$ statement is
  conjectural.
...
\item A weaker form of $p$-adic locality is verified only in the
  listed classes at $p = 2, 3, 5$.  It is false in general, as
  the $p=7$ counterexample above shows.
...
\item \textbf{(New)} The numerator polynomial $N_r(c)$ carries
  a baseline factor of~$5$ in the tested primitive cases.  This
  finite verification points to, but does not prove, a universal
  arithmetic invariant originating from the Virasoro quartic
  contact coefficient $80/(5c{+}22)$
  (Proposition~\ref{thm:universal-5-unit}).
...
The Virasoro case $K_2 = 13$ is the seed; the general formula
...
(a rational partial harmonic sum for each finite~$N$, with
logarithmic asymptotic character as $N\to\infty$)
...
% =================================================================
\section{$SU(2)$ Chern--Simons: low-arity $\Ainf$ test}
\label{sec:su2-ainf}
% =================================================================

We record the low-arity $\Ainf$ tests for the bar complex
$\barB(\widehat{\fsl}_2{}_k)$, for $SU(2)$
...
\subsection{The nested-composition obstruction}

\begin{proposition}[Nested-composition obstruction for $\widehat{\fsl}_2$]
...
The displayed $A_3$ is a two-composition obstruction, not the full
Lie conformal Jacobiator; the cyclic Jacobi sum cancels as it must.
...
\item \textbf{$m_4$} (square diagram): three cubic vertices,
  color $f^{ab}{}_g\, f^{gc}{}_h\, f^{hd}{}_e$.  Its possible
  chain-level contribution requires an explicit $\FM_4(\C)$
  integral and is not asserted here.  Its cohomology class
  vanishes by the Lie conformal Jacobi identity and HPL transfer.

\item \textbf{Higher arities}: no chain-level truncation is
  asserted in these notes.  On cohomology, $m_k^H=0$ for
  $k\ge 3$.
\end{enumerate}
\end{proposition}
...
\begin{remark}[Three duality operations, one affine output]
\begin{center}
\begin{tabular}{lll}
\hline
...
\begin{proposition}[Low-arity affine $\Ainf$ summary]
...
$m_3$ (chain level) & nonzero in the nested-composition model; $Q$-exact \\
$m_4$ and higher (chain level) & not asserted without explicit FM integrals \\
PVA associator & $2\delta^{ac}T^b - \delta^{ab}T^c
  - \delta^{bc}T^a + \hat{k}\epsilon^{abc}\lambda_2$ \\
Shadow depth & $3$ (class $\mathbf{L}$) \\
Koszul dual & $k' = -k - 4$ \\
Curvature & $3(k+2)/4$ \\
$\varkappa := \kappa + \kappa'$ (scalar) & $0$ \quad (Trinity $K = c+c^! = 2\dim(\fsl_2) = 6$) \\
$r$-matrix & $r_{\mathrm{OPE}}=k\,\Omega_{\mathrm{tr}}/z$; $r_{\mathrm{KZ}}=\Omega/((k+2)z)$; $r_{\mathrm{Y}}=\hbar\Omega/z$ \\
...
\end{tabular}
\end{center}
The key answer is cohomological: the $\Ainf$ tower terminates on
cohomology at arity~$2$ (strict formality).  Chain-level
operations are bookkeeping for the chosen resolution and require
explicit configuration-space integrals before nonvanishing or
truncation claims are made.  The mechanism is the Lie conformal
Jacobi identity: it makes the higher transferred operations
$Q$-exact.
\end{remark}
...
\item \emph{Dual-level verification.}  The Koszul dual central
...
(simple pole of order~$1$).  Since the bar kernel absorbs
...
($\kappa + \kappa^! = 13$) and for general W-algebras
...
% =================================================================
\section{Cardy asymptotics and BTZ entropy: conditional bar dictionary}
\label{sec:btz-entropy-cardy}
% =================================================================

The modular MC element $\Theta_{\mathrm{grav}}$ of the Virasoro
algebra records the genus-one and shadow data entering the
standard Cardy/BTZ dictionary.  The extraction of BTZ entropy
requires the usual modular-invariance, vacuum-dominance, and
Brown--Henneaux hypotheses.

The result is therefore a conditional algebraic dictionary from
the single algebraic datum: the Virasoro $\lambda$-bracket
...
\subsection{Cardy asymptotics under modular hypotheses}
...
to shifted cylinder energies
\[
\Delta_\pm := L_0^\pm-\frac{c}{24}
= \frac{M\ell \pm J}{2},
\]
equivalently to highest weights
...
$Z \sim \exp(\pi c\,\mathrm{Im}(-1/\tau)/12)$
(equivalently $\exp(\pi^2 c/(6\beta))$ in the real-temperature
normalization).
The saddle-point of
$f(\tau) = \pi c/(12\tau) + 2\pi\tau\Delta_+$
is at $\tau_* = i\sqrt{c/(24\Delta_+)}$, giving
$f(\tau_*) \approx 2\pi\sqrt{c\Delta_+/6}$, and similarly in
the anti-holomorphic sector.

\begin{proposition}[Cardy asymptotics under modular hypotheses]
...
saddle-point at $\tau_* = i\sqrt{c/(24\Delta_+)}$.  The
algebraic inputs are controlled by $\kappa=c/2$; the analytic
modular and saddle hypotheses are additional assumptions.
\end{proof}

\subsection{BTZ black hole entropy}
\label{subsec:btz-entropy-identification}

\begin{proposition}[BTZ entropy under Brown--Henneaux/Cardy hypotheses]
...
Equivalently, $S_{\mathrm{BTZ}} = 2\pi\sqrt{\kappa\Delta_+/3}
+ 2\pi\sqrt{\kappa\Delta_-/3}$ with $\kappa = c/2$.
\end{proposition}

\begin{proof}
Substituting $c = 3\ell/(2G)$,
$\Delta_+ = (M\ell+J)/2$,
$\Delta_- = (M\ell-J)/2$ and using
$M=(r_+^2+r_-^2)/(8G\ell^2)$ and
$J=r_+r_-/(4G\ell)$, equivalently
$(r_+\pm r_-)^2=8G\ell(M\ell\pm J)$, gives
$S_{\mathrm{BTZ}} = 2\pi r_+/(4G)$.
\end{proof}
...
\begin{observation}[Paired Cardy formula on the real interval]
\label{thm:btz-paired-cardy-wn}
For $0 \le c \le 26$ and fixed shifted energy $\Delta\ge 0$,
...
The bar complex supplies the Virasoro curvature and shadow data
entering the Cardy expression.  Identifying that expression with
BTZ entropy is the Brown--Henneaux/Cardy step, not a theorem of
the bar complex alone.
...
\subsection{Route 3: explicit DS computation}
...
No local compute oracle is invoked in this note; the displayed
constants are carried by the Ward identity and the cited DS/OPE
formulae.
...
\item The principal Drinfeld--Sokolov computation confirms it independently
  through the displayed OPE constants.
\end{enumerate}
\end{theorem}
...
H^2(\overline{\mathcal{M}}_{1,1})$ is the Hodge class.  The
exponentiated partition function is the corresponding section of
the Hodge determinant line; the curvature class itself is not an
$H^0$ class.
...
and $\omega_g=c_1(\lambda_{\mathrm{Hodge}})
\in H^2(\overline{\cM}_g)$ is the Hodge curvature class.  In
particular:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\end{enumerate}
Reformulation~(b) has a precise source and obstruction boundary:
\emph{critical-level Kac--Moody algebras}.  The scalar tower
vanishes there, while the Feigin--Frenkel centre supplies the
oper sector; the full sewing-compatible genus-$g$ bar-cohomology
identification is the critical-level modular-tower problem.
\end{remark}
...
Free fermions & $n = 0$ (trivial) & $0$ & $2$
  & trivial \\
$\beta\gamma_\mu$ & $6\mu^2-6\mu+1=0$; standard $\mu=1$ has
  $\kappa=1$ & case-dependent & $4$
  & spin-tuned only \\
...
\item Spin-parametrised $\beta\gamma_\mu$ systems at
  $\mu=(3\pm\sqrt3)/6$, if that family is included; the fixed
  standard $\mu=1$ system is not uncurved.
\end{enumerate}
\end{proposition}
...
\kappa(\beta\gamma_\mu) &= 6\mu^2 - 6\mu + 1.
\end{align*}
...
For $\beta\gamma_\mu$: the spin-parametrised family has zero
curvature at $\mu=(3\pm\sqrt3)/6$.  The commonly used standard
first-order system with $\mu=1$ has $\kappa=1$ and is not
uncurved.
...
\begin{theorem}[Critical level: scalar vanishing and oper-sector boundary]
...
\item The Feigin--Frenkel centre supplies a canonical oper
  sector: locally
  $Z(V_{-\hvee}(\fg))\cong
  \cO(\mathrm{Op}_{\fg^\vee}(\bD))$.  On a smooth curve
  $\Sigma_g$, critical-level conformal blocks are organised by
  $\fg^\vee$-opers on $\Sigma_g$.
\item The stronger identification
  \[
  ...
  \]
  together with compatibility under sewing and clutching is not
  proved here.  It is exactly the critical-level modular-tower
  problem.
\item The scalar shadow free energies vanish:
  $F_g(V_{-\hvee}(\fg)) = 0$ for all $g \geq 1$.
  Any nontrivial critical-level genus data must therefore reside
  in the non-scalar oper sector, not in scalar genus amplitudes.
\end{enumerate}
\end{theorem}
...
Part~(iii): the Feigin--Frenkel theorem identifies the
centre $Z(V_{-\hvee}(\fg))$ with
$\cO(\mathrm{Op}_{\fg^\vee}(\bD))$, functions on
$\fg^\vee$-opers on the formal disc.  Globalising to a curve
organises critical-level conformal blocks by opers.  For
$\fg=\fsl_2$, these are projective connections; for $g\ge2$
the underlying affine space has dimension $3(g-1)$.  This is
the theorem-level oper sector.  Transporting it through the
genus-$g$ bar complex and proving compatibility with sewing
and clutching is the modular-tower problem stated in part~(iv).

Part~(v): the scalar shadow free energy is
$F_g = \kappa \cdot \lambda_g^{\mathrm{FP}} = 0$
since $\kappa = 0$.
\end{proof}

\begin{corollary}[Critical-level all-genus trace boundary]
\label{cor:critical-level-all-genus-trace-boundary}
At critical level the scalar all-genus trace law is completely
closed: $F_g(V_{-\hvee}(\fg))=0$ for every $g\ge1$.  The only
remaining genus-$g$ data are non-scalar opers, and their sewing
law is exactly the modular oper-sector comparison in
Theorem~\ref{thm:critical-level-nontrivial}\textup{(iv)}.  Thus
the scalar trace problem and the oper sewing problem are
disjoint; solving one cannot be used as evidence for the other.
\end{corollary}

\begin{remark}[The critical level as the Langlands point]
\label{rem:critical-langlands}
The non-scalar oper sector of $V_{-\hvee}(\fg)$ is precisely
the critical-level input of the geometric Langlands program:
$\fg^\vee$-opers are the spectral data of the Langlands
correspondence.  The critical-level vertex algebra sees the
Langlands dual through its centre and conformal blocks, even
though its curvature and scalar genus amplitudes vanish
identically.

This is the correct avatar of the ``Type~H'' phenomenon:
not a secondary curvature $\kappa_2 \neq 0$ (which does not
exist), but an uncurved critical-level complex carrying a
canonical oper sector.  The critical level is the unique point
in the affine lineage where the curvature vanishes and the
Feigin--Frenkel center provides the replacement datum.
\end{remark}
...
\begin{theorem}[Curvature-zero classification and nontrivial critical-level cohomology]
...
(the $d\log$ kernel absorbs one pole), the collision
...
This section records the expected Koszul-dual central-charge
involution for $\cW_3$, using Verdier duality on the bar coalgebra
$\barB(\cW_3)$ and the BPZ inner product on weight spaces.  The
statement depends on the DS-bar compatibility and chiral Koszulness
input identified below:

\begin{proposition}[Conditional Koszul dual of $\cW_3$]
\label{thm:koszul-dual-W3}
Assuming DS-bar compatibility and chiral Koszulness for the
$\cW_3$ bar coalgebra, the Koszul dual of the $\cW_3$ algebra at
central charge~$c$ is
...
is $c = 50$.  In the standard $W_3$ normalisation, the
$\partial\Lambda$ coefficient is $16/(22+5c)$, equivalently the
$\Lambda$ coefficient is $32/(22+5c)$.  Under the involution these
become $16/(522-5c)$ and $32/(522-5c)$.
\end{proposition}

The rest of this section records the calculation supporting this
...
\begin{warning}[BPZ pairing is not Fock pairing]
...
\end{equation}
where the factor $(5c + 22)$ reflects the pole in the $W_3$
nonlinear coefficients
$\beta_{\partial\Lambda}=16/(22+5c)$ and
$\beta_{\Lambda}=32/(22+5c)$ at $c=-22/5$.  The Verdier duality exchanges zeros and poles of
...
\medskip
\noindent\emph{Step 4: Transformation of the $W_3$ nonlinear coefficient.}
The $\partial\Lambda$ coefficient is $16/(22 + 5c)$, equivalently
the $\Lambda$ coefficient is $32/(22+5c)$, in the $WW$ OPE.  This
coefficient is not an independent parameter: it is determined by
$c$ through the Jacobi identity of the $\cW_3$ algebra
(Zamolodchikov's uniqueness theorem).  Under $c \mapsto 100 - c$:
\begin{equation}\label{eq:beta-transform}
\beta'_{\partial\Lambda}
\;=\; \frac{16}{22 + 5(100 - c)}
\;=\; \frac{16}{522 - 5c}.
\end{equation}
Equivalently, $\beta'_\Lambda = 32/(522-5c)$ for the
undifferentiated $\Lambda$ coefficient.
At the self-dual point $c = 50$:
\begin{equation}
\beta_{\partial\Lambda}\big|_{c=50}
= \frac{16}{22 + 250} = \frac{16}{272} = \frac{1}{17},
\qquad
\beta_{\Lambda}\big|_{c=50}=\frac{2}{17}.
\end{equation}
\end{proof}

\subsection{The modular characteristic and complementarity sum}

\begin{proposition}[Modular characteristic of $\cW_3$ from genus universality]
\label{prop:kappa-W3}
...
\begin{proposition}[Conditional complementarity sum for $\cW_3$]
...
\[
\barB^{\mathrm{ch}}(\mathrm{DS}_\chi(\cA))
\;\simeq\;
\mathrm{DS}_\chi(\barB^{\mathrm{ch}}(\cA))
\qquad
\text{as ordered/chiral bar complexes.}
\]
The $\SCchtop$ structure is then placed on the derived-centre
pair $(Z^{\mathrm{der}}_{\mathrm{ch}}(\mathrm{DS}_\chi(\cA)),
\mathrm{DS}_\chi(\cA))$, not on a bare one-colour bar complex.
...
$m_3^{\mathrm{Vir}} \ne 0$ from affine data whose transferred
cohomology operations satisfy $m_n^H=0$ for $n\ge3$.  The
chain-level affine Swiss-cheese $m_3$ is resolution-dependent and
$Q$-exact; the BRST homotopy transfer manufactures the Virasoro
ternary operation from the affine bracket and Sugawara composite.
...
(from the $k\lambda$ central terms).  The transferred cohomology
operations vanish, $m_n^H=0$ for $n\ge3$, by the Lie conformal
Jacobi identity.  Chain-level higher operations in a chosen
Swiss-cheese resolution may be nonzero but are $Q$-exact
(class~$\mathbf{L}$, shadow depth $r_{\max}=3$).
...
\;=\;
s^{-1}\bigl(\partial T + 2T\lambda
+ \frac{c}{12}\lambda^3\bigr).
\end{equation}
...
The affine bar complex at arity~$3$ has no nontrivial transferred
cohomology operation; any chain-level affine ternary term is
$Q$-exact in the chosen resolution.
...
(i)~the affine algebra has $m_3^H=0$ after cohomology transfer,
while any chain-level affine $m_3$ in the chosen resolution is
$Q$-exact;
...
\begin{proposition}[Conditional DS-bar intertwining beyond arity three]%
\label{prop:wn-ds-bar-all-arities}
The DS-bar comparison is verified here at arities $2$ and $3$.
For every arity $n\ge4$ it extends provided the generic vanishing
$\HH^{2,n}(\mathrm{Vir}_c,\mathrm{Vir}_c)=0$ and the HPL
compatibility for the chosen bar model hold.
\end{proposition}

\begin{proof}[Conditional induction]
The local calculation supplies the base arities $2$ and $3$.
The proposed inductive step uses
$\HH^{2,n}(\mathrm{Vir}_c, \mathrm{Vir}_c) = 0$ for
generic~$c$ to propagate uniqueness from arities $\le n-1$
to arity~$n$.  The Hochschild vanishing and HPL compatibility are
external obligations, so the all-arities statement is conditional.
\end{proof}

\subsection{Summary and significance}

The chain-level verification establishes locally: (1)~at arity~$2$,
...
(3)~beyond arity~$3$, Hochschild vanishing would propagate the
uniqueness under the stated generic hypotheses.
...
\smallskip\noindent
\textbf{$M(2,5)$:}
Verlinde ring $\Z[1,\varphi]/(\varphi^2 = 1+\varphi)$.
For $N_{\mathrm{tot}}=N_1+N_\varphi$ the matrix in the basis
$(1,\varphi)$ is
$\begin{psmallmatrix}1&1\\[1pt]1&2\end{psmallmatrix}$, with
eigenvalues $(3\pm\sqrt5)/2$.
\[
\mathrm{Sh}_r^{\mathrm{Ver}}
= (1+\varphi)^r \quad\text{reduced by }\varphi^2=1+\varphi.
\]
Growth rate: $\lambda_{\max} = (3+\sqrt5)/2$.
...
The growth is controlled by the Perron--Frobenius eigenvalue of
the total fusion matrix $N_{\mathrm{tot}}=\sum_i N_i$, not by
the global dimension $\sum_i d_i^2$.  In a unitary normalization
this is the sum of the positive Frobenius--Perron dimensions.
For non-unitary models one should compute $N_{\mathrm{tot}}$
directly.
For $M(2,5)$ the total fusion matrix has spectral radius $2$.
...
\subsection{Formal collision-kernel identity}

\begin{proposition}[Collision-kernel identity on completed modules]
\label{prop:gy-rtt}
On a chosen completed Virasoro module on which the sums
$\Omega_r$ are defined, the transfer kernel
$T_V(z) = \sum_{r \ge 0} z^{-r-1}\,\pi_V(\Omega_r)$
satisfies the formal collision identity
\[
r_{12}(z_1 - z_2)\,T_1(z_1)\,T_2(z_2)
= T_2(z_2)\,T_1(z_1)\,r_{12}(z_1 - z_2),
\]
only after the simple-pole/fundamental reduction.  In the full
gravitational setting the higher-pole Virasoro collision kernel
gives a four-term completed-module identity, not a standard FRT
or RTT presentation.
\end{proposition}
...
\begin{proposition}[Formal structure of $Y^{\mathrm{dg}}(\mathrm{Vir}_c)$]
\label{thm:gy-structure}
The gravitational Yangian has the following dg-shifted bialgebra
structure:
\begin{enumerate}[label=\textup{(\roman*)}]
...
\item \emph{As a dg-shifted bialgebra,}
  the additional structure is the infinite $A_\infty$ product tower
  $\{m_k^{\mathrm{Vir}}\}_{k \ge 3}$, the primitive coproduct,
  and the higher-pole $r$-matrix.  No antipode/Hopf structure is
  asserted.

\item \emph{The $A_\infty$ corrections}
  (quartic contact $10/[c(5c+22)]$, quintic, etc.) are contact and
  shadow corrections.  Identifying them with physical scattering
  amplitudes requires additional analytic input.
...
& Formal collision-kernel identities; no standard RTT presentation
\\
Level-$1$--$1$ bracket
...
\\
$r$-matrix
& Simple pole: $r(z) = k\,\Omega_{\mathrm{tr}}/z$ \textup{(trace convention)}
...
\section{Kontsevich integral from the bar complex: trefoil checks}
...
$R(z) = \exp(\hbar\, r(z)) = \exp(\hbar k\,\Omega_{\mathrm{tr}}/z)$,
...
\begin{equation}\label{eq:trefoil-braid}
  Z(3_1)
  \;=\;
  \operatorname{tr}_{M}\!\bigl(R(z_{12})^3\bigr),
\end{equation}
...
The quantum $R$-matrix $R = \exp(\hbar k\,\Omega_{\mathrm{tr}}/z)$ on
...
For the trefoil at generic $q$ (which we expand perturbatively), the
unnormalised leading trace is:
\begin{equation}\label{eq:markov-trace}
  Z_{V}^{\mathrm{unnorm}}(3_1)
  \;=\;
...
(at leading order in $q$).  The normalized Kontsevich/Jones
quantity divides by the quantum dimension of the unknot and applies
the framing correction; the unnormalised trace is not itself a
partition function with constant term~$1$.
...
\begin{align}
  R^3
  &= \exp\!\bigl(3\hbar k\,\Omega_{\mathrm{tr}}/z\bigr)
  \notag\\
  &= \id
  + 3\hbar k\,\Omega_{\mathrm{tr}}/z
...
\begin{computation}[The connected degree-$2$ normalization for
the trefoil; conditional on standard Kontsevich normalization]
...
\begin{proof}[Compatibility check from the bar complex]
We check that the bar-complex expression has the correct connected
degree-$2$ shape for $\barB(\widehat{\fsl}_2{}_k)$.  The numerical
equality $z_2(3_1)=1/24$ is the Bar--Natan/Kontsevich normalization
input, not an independent evaluation of the compactified
configuration-space integral here.

\medskip
\noindent\textbf{Step A: Bar-complex R-matrix for the trefoil.}
The trefoil is the closure of $\sigma_1^3$.  The bar complex
produces the $R$-matrix $R(z) = \exp(\hbar k\,\Omega_{\mathrm{tr}}/z)$
...
\[
  ...
  \operatorname{tr}_{M}\bigl(R(z_{12})^3\bigr),
\]
the Markov trace of the braid product, not a tensor-power trace.
...
With the standard Kontsevich normalization fixed, the connected
degree-$2$ contribution after the logarithm is
\begin{equation}\label{eq:connected-2}
  ...
  \;=\;
  \frac{1}{24}\;D_2^{\mathrm{conn}},
\end{equation}
where $D_2^{\mathrm{conn}}$ denotes the connected degree-$2$
chord-diagram class in the chosen normalization.
...
\[
  ...
  -k\,\log\,\eta(\tau).
\]
The leading $q$-exponent coefficient of $F_1$ is $-k/24$.  For
the trefoil, the three crossings each contribute a factor of $k$
from the Casimir.  The remaining scalar is the normalized
Kontsevich/Bar--Natan degree-$2$ integral:
\[
  z_2(3_1) =
  (\text{bar Casimir factor}) \cdot
  (\text{normalized degree-$2$ Kontsevich integral})
  = \frac{1}{24}\;D_2^{\mathrm{conn}}.
\]
...
\medskip
\noindent\textbf{Step D: Weight-system status.}
The $\fsl_2$ weight-system evaluation depends on the chosen
normalization of the connected degree-$2$ chord class
$D_2^{\mathrm{conn}}$.  It is not used here as an independent
numerical verification of the universal coefficient.
\end{proof}
...
$F_1=-k\,\log\eta(\tau)$ through its leading $q$-exponent
coefficient $-k/24$, and in the Bernoulli number $B_2/4$ is a
manifestation of a single phenomenon:
...
(Theorem~\ref{thm:one-loop-koszul}) gives vanishing of the
transferred cohomology operations $m_k^H$ for $k \ge 3$.
Therefore, the degree-$3$ bar differential
...
\begin{remark}[Why cohomological $m_3$-vanishing matters]
...
\end{align*}
The Arnold--Orlik--Solomon relation gives the $4T$ relation in the
chord-diagram algebra.  Via the STU bridge it induces the IHX
relation in the Jacobi/trivalent diagram lane.  The relative signs
come from orientation reversal on the boundary strata of
$\FM_3(\C)$.
...
The numerical coefficient requires a specified top cycle,
orientation, and normalization convention on the compactified
configuration space; no definite $\FM_3$ integral is asserted here.
...
At degree~$3$ the Arnold relation gives the IHX structure in the
bar complex.  Matching the full coloured-Jones coefficient requires
a separate normalization of the degree-$3$ weight-system class and
is treated here as structural evidence, not as an unconditional
...
$2$ & $\frac{1}{24}\,D_2^{\mathrm{conn}}$ & $\frac{1}{24}\,D_2^{\mathrm{conn}}$ & conditional &
  Normalization~\ref{comp:degree-2-trefoil} \\
$3$ & IHX class & $Z_3(3_1)$ & structural &
  Arnold/IHX, $\S$\ref{subsec:degree-3-trefoil}
\end{tabular}
\end{center}
...
\begin{enumerate}
\item The degree-$2$ coefficient $z_2(3_1)=1/24$ is compatible
  with the bar complex of $\widehat{\fsl}_2{}_k$ after the
  standard Kontsevich/Bar--Natan normalization is fixed.  The
  $1/24$ is not independently derived here from a top-cycle
  integral.

\item The same $1/24$ is the leading $q$-exponent coefficient in
  the genus-$1$ free energy $F_1=-k\,\log\eta(\tau)$; both are
  manifestations of the curved bar structure
  $d_{\barB}^2 = \kappa\,\omega_1$.
  This supplies a normalization check, not an independent theorem.

\item At degree~$3$, the IHX relation is derived from the
  Arnold relation on $\FM_3(\C)$ (not imposed by hand), and
  the vanishing of the transferred cohomology operation $m_3^H$
  for the affine lineage ensures compatibility with the
  chord-diagram formalism.

\item The degree-$2$ coefficient is conditional on the stated
  normalization.  Degree~$3$ supplies Arnold/IHX structural
  evidence; the numerical weight-system normalization is not used
  as an independent theorem here.  The full identification
  $Z_{A_\infty}(K)=Z(K)$ at all orders remains conjectural
  (Theorem~\ref{thm:bar-kontsevich}).
\end{enumerate}
...
\item in the computed arities, the top-derivative coefficient is
  fixed by the same Stasheff recursion, but no closed formula
  $f_{k-2}^{(k)}\equiv 1$ is asserted;
...
\item The field-valued part ($T$, $\partial T$, $\ldots$)
  is \textbf{independent of~$c$}.  The coefficient of
  $\partial^j T$ has degree $k-1-j$ when that derivative channel
  is present.  No universal top-derivative coefficient is asserted:
  the stable arity-$4$ formula has no $\partial^3T$ term, while
  the arity-$5$ symmetric formula has leading coefficient~$-1$.
...
The bar kernel absorbs one pole.  The affine OPE, KZ, and Yangian
normalisations are distinct:
\[
r_{\mathrm{OPE}}(z)=\frac{k\,\Omega}{z},
\qquad
r_{\mathrm{KZ}}(z)=\frac{\Omega}{(k+2)z},
\qquad
r_{\mathrm{Y}}(z)=\frac{\hbar\,\Omega}{z},
\qquad
\Omega = e \otimes f + f \otimes e + \tfrac12\,h \otimes h
\quad(\text{Casimir of } \mathfrak{sl}_2).
\]
The open-colour Yangian uses $\hbar=(k+2)^{-1}$; it is not the
same normalisation as the OPE residue.
...
The ordered and symmetric chiral bar complexes expose two
projections of the bulk--boundary datum governed by
$\mathrm{SC}^{\mathrm{ch,top}}$.  They should not be conflated
with a literal self-dual $\mathrm{SC}^{\mathrm{ch,top}}$-coalgebra.
Each projection has its own duality operation and output category.
...
B^\Sigma_n(\cA)
\;\simeq\;
\bigl(\pi_{n,*}\bar{B}^{\mathrm{ord}}_n(\cA)\bigr)
^{\Sigma_n,\mathcal{L}_R},
\]
where $\pi_n:\Conf_n^{\mathrm{ord}}(X)\to\Conf_n(X)$ is the
covering map and $\mathcal{L}_R$ is the local system determined
by the braiding.  This reduces to ordinary invariants or
coinvariants only in the pole-free case.
...
(Francis--Gaitsgory), while
$\mathrm{Ass}^{\mathrm{ch}}{}^! = \mathrm{Ass}^{\mathrm{ch}}$
in the associative lane.  The two-coloured quadratic dual of the
Swiss-cheese structure is not literally the same operad: the
closed colour dualises to the Lie/chiral side, the open colour to
the associative side, and the mixed operations dualise to the
corresponding shuffle/brace module.  Thus an ordered bar
construction produces a dual two-colour algebra only after choosing
the relevant coloured Koszul model; $\cA^!_{\mathrm{ch}}$ and
$\cA^!_{\mathrm{line}}$ are projections of that constructed dual
datum, not consequences of unqualified self-duality.
...
$\cH_k^! \neq \cH_{-k}$ as objects in the present classification).
$\cA^!_{\mathrm{line}} = Y(u(1)) = k[h_0, h_1, \ldots]$
...
\subsection{Conjectural chromatic directions}

The following directions are retained as conjectural tests.  Each item
records the mathematical obstruction that must be solved before the idea
can enter the stable core.
...
The unstable source material (1710 lines) is preserved in
...
$S_{\mathrm{Cardy}} = 2\pi\sqrt{c\Delta/6}$ for shifted energy
$\Delta=L_0-c/24$.  The quartic pole
...
Three signatures, with different operadic status:
\begin{itemize}
\item \textbf{Lorentzian $(2,1)$}: the light-cone constraint
  $|{\Delta z}| = |{\Delta t}|$ is treated as a gauge/local-shadow
  heuristic in the HT twist.  No equality of Lorentzian causal
  operads with $\SCchtop$ is asserted.
\item \textbf{Euclidean $(3,0)$}: the $E_3$ operad
  $C_*(\FM_k(\R^3))$, one-coloured, fully symmetric.  There is no
  Dunn-additivity promotion of the raw bicoloured
  $\SCchtop$-algebra to $E_3$.  An $E_3^{\mathrm{top}}$ structure
  appears only after the topologisation/cohomology hypotheses
  supplied by inner stress tensors.
...
\item \textbf{Monstrous moonshine at rank 24.}
  For a rank-24 lattice VOA: $\kappa = 24$ and
  $Z_1(V_\Lambda;\tau)=\Theta_\Lambda(\tau)/\eta(\tau)^{24}$.
  The special expression $\eta(\tau)^{-24}=1/\Delta(\tau)$ is the
  rank-24 Heisenberg oscillator factor, not a general lattice-VOA
  character.  For the Leech lattice:
  $Z_1(V_{\mathrm{Leech}};\tau)=\Theta_{\mathrm{Leech}}(\tau)/
  \eta(\tau)^{24}=j(\tau)-720$ (exact).  The discriminant $\Delta
...
The spectral $R$-matrix $R(z) = \exp(\hbar k\,\Omega_{\mathrm{tr}}/z)$ of
$\widehat{\fsl}_2{}_k$
...
$\operatorname{tr}_{M}(R(z_{12})^3)$ with
$R^3 = \exp(3\hbar k\,\Omega_{\mathrm{tr}}/z)$
gives
\[
  Z_V(3_1)
  \;=\;
...
$2$ & $z_2 = \tfrac{1}{24}\,D_2^{\mathrm{conn}}$ after normalization &
  $B_2/4 = 1/24$; normalization-dependent \\
$3$ & IHX class &
  Arnold relation; numerical weight-system normalization not fixed here
\end{tabular}
\end{center}

\noindent
The degree-$1$ term is pure framing ($z_1 = w/2$, removed by
framing correction).  In the standard Kontsevich normalization,
the first nontrivial Vassiliev invariant is $z_2(3_1) = 1/24$.

\subsection*{The coefficient $1/24$}

The coincidence of $1/24$ in three contexts --- the degree-$2$
trefoil invariant, the leading $q$-exponent coefficient in
$F_1=-\kappa\,\log\eta(\tau)$, and the constant term
...
The $\fsl_2$ weight-system evaluation depends on the chosen
normalization of the connected degree-$2$ chord class and is not
used here as an independent theorem.

\subsection*{Degree~$3$: IHX from Arnold}

For the affine lineage, one-loop exactness
(Theorem~\ref{thm:one-loop-koszul}) forces the transferred
cohomology operation $m_3^H$ to vanish.
...
The degree-$2$ coefficient is verified in the stated normalization.
At degree~$3$, Arnold cancellation gives structural IHX evidence;
the numerical weight-system normalization is not fixed in this
summary.  The full identification $Z_{A_\infty}(K)=Z(K)$ at all
orders remains conjectural (Theorem~\ref{thm:bar-kontsevich}).
At degree~$4$, the chord
...
% =================================================================
\section{Genus-$1$ curved bar complex: stable summary}
\label{sec:genus1-bar-stable}
% =================================================================
%
% Stable genus-1 computation: floor-function precedence,
% normalisation, and modular-characteristic language fixed.
...
\section{Celestial amplitudes from the bar complex: stable summary}
\label{sec:celestial-bar-stable}

Audit of the celestial amplitude computation against the core
chapters: Theorem~\ref{thm:ch-core-celestial-gluon-ope}
...
\item \textbf{Gluon.}  The $\lambda$-bracket $\{J^a{}_\lambda J^b\} =
  ...
  absorption ($z^{-n}\mapsto z^{-(n-1)}$), the structure-constant
  term drops to $z^0$ and the level term survives at $z^{-1}$:
  $r^{\mathrm{KM}}(z) = k\Omega_{\mathrm{tr}}/z$.  Colour-ordered projection of the
...
\item \textbf{MHV at $n=4$.}  Conditional on the stated
  normalisation of the FM orientation, the bar-complex integral
  ...
  $\langle 23\rangle^4$.  Under these identifications this
  recovers the Parke--Taylor form; the statement is a
  normalization check, not an independent proof of the physical
  amplitude.
\end{enumerate}
...
poles are present, and the $R$-matrix descent is genuinely
twisted, not naive.

\subsection*{Residual questions}
...
% =================================================================
\section{Virasoro spectral $R$-matrix: stable extract}
\label{sec:virasoro-spectral-R-stable}
% =================================================================
%
% Stable extract: Laplace transform, CYBE, collision-residue
% convention, Ponsot--Teschner comparison, and quantum corrections.
...
\begin{equation}\label{eq:vir-rL-stable}
r^L(z)
\;=\;
\frac{\partial T}{z}
+ \frac{2T}{z^2}
...
The \emph{collision residue} (bar-theoretic $r$-matrix: $d\log$
absorbs one power) is
\begin{equation}\label{eq:vir-rcoll-stable}
r^{\mathrm{coll}}(z)
\;=\;
...
\begin{equation}\label{eq:R-level0-stable}
...
\subsection*{Normalization notes}
...
% =================================================================
\section{Virasoro $m_4$ and $m_5$: stable formulas}
\label{sec:virasoro-m4-m5-stable}
% =================================================================
...
stable formulas with corrected structural claims, the
...
relation is read in the chiral $\lambda$-formalism after applying
sesquilinearity to the translation terms.  In this convention it
has the direct form $m_4+\mathrm{Obs}_4=0$, where
$\mathrm{Obs}_4$ involves only $m_2$ and $m_3$.
Thus $m_4 = -\mathrm{Obs}_4$ is \emph{forced}, not a free cocycle.
If one writes instead $\partial(m_4)+\mathrm{Obs}_4=0$, a genuine
contracting homotopy must be inserted before solving for $m_4$.

\begin{equation}\label{eq:m4-stable}
\begin{aligned}
...
\subsection{The projected normally ordered kernel for
$\{\normord{TT}{}_\lambda T\}$}

By the Dong lemma, the $T$-linear/projected component used by the
homotopy transfer is:
\begin{equation}\label{eq:TT-T-stable}
\{\normord{TT}{}_\lambda T\}
\;=\;
...
\;-\; \tfrac{c}{40}\,\lambda^5.
\end{equation}
This is not the full normally ordered OPE; omitted channels are
projected away in the HPL computation of $m_4$.  The normally
ordered product provides the chain homotopy between the full
(associative) OPE and the (non-associative) $\lambda$-bracket.

\subsection{$m_5$: explicit polynomials $P_1$ and $P_0$}

\begin{equation}\label{eq:m5-stable}
\begin{aligned}
m_5(T^5;\,
...
with minor errors.  The stable versions:
...
\begin{proposition}[Leading celestial Ward shadow]
\label{thm:weinberg-from-bar}
The arity-$2$ shadow $S_2$ gives the two-dimensional celestial
Ward shadow of Weinberg's leading soft factor:
$S^{(0)}_i = \partial_{z_i}/(z_0 - z_i)$.
In bar-complex language, $d_{\barB}$ on $[s^{-1}T \mid s^{-1}\cO_i]$
extracts $T_{(0)}\cO_i = \partial\cO_i$.
\end{proposition}

\begin{proposition}[Subleading celestial Ward shadow]
...
\begin{proposition}[Conditional sub-subleading shadow from $m_3$]
\label{thm:sub-subleading-m3}
The sub-subleading celestial shadow requires the ternary $\Ainf$
...
\begin{proposition}[Conditional soft-shadow hierarchy]
\label{thm:soft-ainf-identification}
Conditional on the celestial Mellin bridge and the usual IR
normalisations, the tower of soft graviton Ward identities is
shadowed by the
...
$S^{(3)}$ & $S_5$ & $m_5$ &
  Non-trivial & Conditional prediction \\[2pt]
$S^{(p)}$ & $S_{p+2}$ & $m_{p+2}$ &
  Non-trivial ($p$ even) & Conditional higher shadow
\end{tabular}
\end{center}
...
giving infinitely many algebraic HT-celestial shadow classes.
Their interpretation as physical four-dimensional soft theorems
is conditional and is not asserted here.

At large $c$ (semiclassical): $S^{(0)}, S^{(1)}$ are exact
($O(G^0)$) in the shadow model; all higher shadow classes are
suppressed by powers of $G = 3/(2c)$.
...
three soft-shadow classes.  $\beta\gamma$ (class C,
$r_{\max} = 4$): four.  Virasoro and $\cW_N$ (class M,
$r_{\max} = \infty$): infinitely many.  The shadow depth
classification G/L/C/M mirrors the depth of the algebraic
soft-shadow tower.  Its interpretation as a tower of physical
four-dimensional soft theorems is conditional on the celestial
bridge and is not asserted here.
\end{question}
